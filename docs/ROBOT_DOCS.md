...
| `ERR-CALIBRATION-ADOPTION-001` | a retained adoption receipt, chain or recording could not be read or validated, or the adoption could not be staged or committed (custody, ledger refusal, cancellation) | inspect the cause; retry only after repair; damaged adoption custody is never treated as not adopted |
| `ERR-CALIBRATION-ADOPTION-APPROVAL-STALE-001` | a 'calibration adopt --approve' digest matches neither this adoption over the bound cameras' current retained adoptions nor the approval that retained them (stale or tampered); nothing was written | rerun without the approval and review the current approval digest |
| `ERR-CALIBRATION-ADOPTION-INPUT-001` | a 'calibration adopt' request is outside its contract (no or more than 16 bindings, a camera or sensor bound twice, a camera the calibration does not name, a zero handle or generation); nothing was written | correct the bindings; do not retry unchanged |
| `ERR-CALIBRATION-ADOPTION-REGRESSION-001` | the adoption would lower the camera's intrinsics or extrinsics generation below its current adoption, or re-adopt a calibration the camera already superseded; adoption is monotone and history is never erased; nothing was written | recalibrate the camera for a current generation and adopt that calibration |
| `ERR-CALIBRATION-ADOPTION-SENSOR-CONFLICT-001` | the camera's adoptions already bind another sensor, or the sensor is already bound to another camera handle; a camera's sensor binding is fixed; nothing was written | bind the camera to its adopted sensor; a replaced physical camera takes a new handle |
| `ERR-CALIBRATION-ADOPTION-SENSOR-MISMATCH-001` | an adopted camera's calibration was applied to a recording of another sensor than the one its adoption binds; refused before any frame is decoded, nothing was appended | pair each --camera with a recording of its adopted sensor |
| `ERR-CALIBRATION-ADOPTION-SENSOR-UNRETAINED-001` | 'calibration adopt --bind NAME:SENSOR' names a sensor that no readable retained sensor capsule names, so the camera cannot be bound to it; nothing was written | import evidence of that sensor first, or correct the sensor id |
| `ERR-CALIBRATION-ADOPTION-STALE-001` | 'corroborate --calibration' names a calibration a camera's retained adoption has superseded (that calibration, or one at a generation not above the adopted one, while a later adoption is current); refused before any frame is decoded, nothing was analysed or appended; currency is the retained owner adoption, not a physical observation | corroborate with the camera's currently adopted calibration ('calibration show') |
| `ERR-CALIBRATION-ADOPTION-UNADOPTED-001` | 'corroborate --calibration' names a calibration never adopted for a camera whose deployment retains adoptions of other calibrations; refused before any frame is decoded, nothing was appended | adopt the calibration first ('calibration adopt') or use the adopted one |
...
| `ERR-SITE-CALIBRATION-FORMAT-001` | a site calibration file is not a canonical FSSCAL01 or FSSCAL02 record (wrong magic, length, truncated, non-finite, unregistered value, trailing bytes, noncanonical encoding, or a FSSCAL02 record without a frame camera); it was not used | regenerate it with fss-event calibrate; never hand-edit a calibration |
| `ERR-SITE-CALIBRATION-FRAME-DECODE-001` | a camera's JPEG frame was refused before any localization: the first-party baseline decoder rejected it (malformed, truncated, unsupported coding or component interpretation, limit or budget) or its decoded size differs from the metadata's declared image size; nothing was calibrated or written | supply the complete baseline JPEG the metadata describes (or correct the metadata's image and interpretation lines); do not retry unchanged |
...
| `ERR-SITE-CALIBRATION-GENERATION-STALE-001` | an owner-asserted current camera generation ('corroborate --camera-generation NAME:INTRINSICS:EXTRINSICS') differs from the intrinsics/extrinsics generation the calibrated pose depends on: the calibration is stale for that camera (moved, cropped, zoomed or relensed); refused before any source is read, nothing was analysed or appended; the currency is the owner's assertion, never observed | recalibrate the camera for its current generation, or correct the assertion if the camera did not change |
| `ERR-SITE-CALIBRATION-GENERATION-UNBOUND-001` | '--camera-generation' names a corroborated camera whose pose does not come from the supplied calibration (a --pose camera or one the calibration does not name), so the assertion binds nothing; nothing was analysed | assert generations only for cameras the calibration poses |
| `ERR-SITE-CALIBRATION-INPUT-INVALID-001` | a site calibration request, camera observation file (fss.site_camera_observations.v1) or frame metadata file (fss.site_camera_frame.v1) is malformed or out of bounds: camera count outside 2..16, duplicate names or handles, a missing or repeated header line, a line the file kind does not admit (pixels, descriptor, feature or tie lines in frame metadata), a zero handle or generation, a non-finite number, a pixel outside the image, a feature or tie limit, or mixed descriptor generations; nothing was calibrated or written | correct the observation or metadata files or options; do not retry unchanged |
