...
- Target: `KnowledgeCell`; field: `evidence`
  ...
  - Reconciled value: Laundering is refused wherever the producing cell is known: KnowledgeCell::verify_no_evidence_laundering(prior), the public API point, refuses exactly the registered pairs, and the production classification path classify_reference_meaningful_delta (shared by classify_reference_meaningful_delta_in_lineage) calls it for every result cell whose basis frame carries the claim's previous version (fss-2nwxm). A lone relabelled cell whose producing cell is absent is still accepted; SituationFrame::validate and MeaningfulDelta::validate judge the SET of cells (independent of order and claim-id naming) and accept shared evidence between classes; no cross-claim verdict is drawn across basis and result frames.
  - Reason: An evidence reference is an untyped ContentDigest that names no producing cell, so shared evidence cannot be attributed to its author: guessing by list position or claim id refused honest derivations (P4r, P5r) and made verdicts order-dependent (Q1, Q2), and the basis-versus-result check refused honest unchanged carry-overs (identical frames [Observed(D), Derived(D)] returned evidence_laundering_detected) and was removed. Where the result carries the claim's previous version the producing cell IS known, so the registered pairs are enforced there on the production path, and a repository guard (ERR-PROV-LAUUNDERING-UNWIRED-001) fails if the refusal loses its non-test caller (fss-2nwxm). Full enforcement needs typed evidence references carrying origin provenance, tracked under fss-gefi6.
  - Status: `open`
