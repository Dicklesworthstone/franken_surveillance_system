...
| `DEP-AUD-027` | error | a qualification script (scripts/qualify.sh or scripts/release_qualify.sh) does not seal Cargo and rustup offline (missing top-level CARGO_NET_OFFLINE=true or RUSTUP_AUTO_INSTALL=0 export, an override, or a cargo invocation without --offline) | export CARGO_NET_OFFLINE=true and RUSTUP_AUTO_INSTALL=0 at top level and pass --offline to every cargo invocation in scripts/qualify.sh and scripts/release_qualify.sh; this is Cargo/rustup sealing, not OS network isolation | `GATE-000`, `QL-POLICY-001` | seal scripts/qualify.sh and scripts/release_qualify.sh offline before re-running qualification |
