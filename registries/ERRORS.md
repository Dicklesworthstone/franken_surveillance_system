...
| `ERR-CLAIM-PROOF-UNPROVEN-PLACEHOLDER-001` | proof claim formal artifact contains an unproven placeholder outside comments (Lean sorry/sorryAx/admit, TLAPS OMITTED) | complete the proof; a placeholder is never a checked proof |
| `ERR-CLAIM-GENERATION-UNBOUND-001` | promoted proof claim is cited by no claim row declaring its current generation (Generation column), or its citing rows conflict | declare the claim row's current generation and bind the bundle to exactly it |
