...
pub struct LocalInspection {
    ...
    /// Whether the snapshot is possibly stale because a concurrent writer holds a lock.
    pub possibly_stale: bool,
    /// Whether modifications are possibly in flight due to an active writer or pending temporary files.
...
impl LocalRootPublisher {
    ...
    fn open_through(
        ...
        // Re-verify and hold all candidate manifests (including broken roots, e.g. indeterminate slots)
        for manifest_root in &classification.manifests_to_hold {
            let _ = publisher.spool.verify(*manifest_root);
        }

        // Re-verify all referenced objects for visible roots.
        // A verification failure demotes the root to BrokenRootReason::ReferenceBlocked { reason: BlockReason::Corrupt }
        // instead of failing the whole open through `?`.
        let all_referenced: BTreeSet<ContentDigest> = classification
            .root_closures
            .values()
            .flat_map(|closure| closure.iter().copied())
            .collect();
        let mut failed_objects: BTreeMap<ContentDigest, BlockReason> = BTreeMap::new();
        for object in all_referenced {
            match publisher.spool.verify(object) {
                Ok(SpoolObjectState::Verified) => {}
                Ok(SpoolObjectState::Staged) => {
                    failed_objects.insert(object, BlockReason::NotVerified);
                }
                Ok(SpoolObjectState::Corrupt(_)) | Err(SpoolError::Corrupt { .. }) => {
                    failed_objects.insert(object, BlockReason::Corrupt);
                }
                Err(SpoolError::Missing(_)) => {
                    failed_objects.insert(object, BlockReason::Missing);
                }
                Err(error) => return Err(publisher.spool_error(error)),
            }
        }

        if !failed_objects.is_empty() {
            let mut newly_broken = Vec::new();
            for (slot, entry) in &publisher.visible {
                if let Some(closure) = classification.root_closures.get(slot) {
                    for (obj, reason) in &failed_objects {
                        if closure.contains(obj) {
                            let relative = Path::new(LOCAL_ROOTS_DIR)
                                .join(format!("{slot}{ROOT_RECORD_SUFFIX}"));
                            let role = if *obj == entry.visible.root {
                                ReferenceRole::ManifestBody
                            } else if entry.children.contains(obj) {
                                ReferenceRole::Child
                            } else {
                                ReferenceRole::Descendant
                            };
                            newly_broken.push((
                                slot.clone(),
                                BrokenRoot {
            ...
            for (slot, broken_root) in newly_broken {
                publisher.visible.remove(&slot);
                publisher.broken_slots.insert(slot);
                publisher.recovery.broken_roots.push(broken_root);
            }
            publisher
                .recovery
                .broken_roots
            ...
            for entry in publisher.visible.values() {
                referenced.extend(publisher.closure(entry.visible.root, &entry.children));
            }
            let spool_objects = publisher
                .recovery
                .spool
                .admitted
                .iter()
                .copied()
                .chain(publisher.recovery.spool.corrupt.iter().map(|c| c.digest))
                .collect::<BTreeSet<_>>();
            publisher.recovery.unreferenced_objects =
    ...
    /// Mutable access to the owned staging spool.
...
pub(crate) struct LocalClassification {
    ...
    pub(crate) manifests_to_hold: BTreeSet<ContentDigest>,
}
...
pub(crate) fn classify_local(
        ...
        Err(err) if err.kind() == io::ErrorKind::NotFound => {
            if !mutating {
                return Ok(LocalClassification {
                    ...
                    manifests_to_hold: BTreeSet::new(),
                });
            }
    ...
    let mut manifests_to_hold: BTreeSet<ContentDigest> = BTreeSet::new();
    ...
    if roots_exist {
        ...
        for (name, file_type) in entries {
            ...
            manifests_to_hold.insert(root_digest);
            let child_count = manifest.children().len();
...
pub fn inspect_with_options(
    ...
    let lock_paths = vec![
        ...
        root.join("ledger").join("journal.fssj"),
    ];
    let writer_state = detect_writers(io.as_ref(), &lock_paths, lock_table, options);
    let possibly_stale = writer_state.is_held();
    let possibly_in_flight = writer_state.is_held()
        || !classification.redundant_temps.is_empty()
        || !spool_inspection.report.orphaned_staging.is_empty();
...
pub fn read_verified_with_io(
    ...
    let spool_root = root.join(LOCAL_SPOOL_DIR);
    match io.symlink_metadata(&spool_root) {
        ...
        Err(err) => {
            return Err(io_error(LocalIoOperation::Inspect, &spool_root, &err));
        }
    }
