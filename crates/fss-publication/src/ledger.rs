...
pub struct RootLedgerReconciliation {
    ...
    /// First byte offset of an incomplete ledger tail, if present.
...
/// Classifies the relationship between durable local roots and canonical ledger claims
/// without acquiring locks or modifying state.
#[must_use]
pub fn inspect_linkage(
    ...
    for visible in local.visible_roots() {
        let slot = &visible.slot;
        let Ok(object_id) = root_reachability_object_id(slot) else {
            report.unledgerable.push(slot.clone());
            continue;
        ...
        let closure_len = local.root_closure(slot).map(|c| c.len());
    ...
    report.broken = local.broken_slots().cloned().collect();
