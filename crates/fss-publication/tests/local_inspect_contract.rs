...
use fss_publication::decode_st_dev;
use fss_publication::{
...
fn test_local_writer_detection_lock_symlink_or_dir() -> TestResult {
    ...
    assert!(matches!(state_dir, WriterState::InvalidLayout { .. }));
    ...
    match open_res {
        ...
        other => {
            return Err(
                format!("expected Spool(InvalidLayout) for directory LOCK, got {other:?}").into(),
            );
        }
    }
    ...
    assert!(matches!(state_sym, WriterState::InvalidLayout { .. }));
...
impl SpoolIo for ReplacingSpoolIo {
    ...
    fn try_lock(&self, file: &File) -> Result<(), std::fs::TryLockError> {
        HostSpoolIo.try_lock(file)
    }
    fn try_lock_shared(&self, file: &File) -> Result<(), std::fs::TryLockError> {
...
fn test_linkage_read_only_recording_io_proof() -> TestResult {
    ...
    assert!(!recording_journal_io.calls().is_empty());
