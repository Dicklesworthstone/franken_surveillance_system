...
use fss_object::{
    HostSpoolIo, ObjectManifest, RecordingSpoolIo, SPOOL_HOLDS_DIR, SPOOL_STAGING_DIR, SpoolError,
    SpoolIo, SpoolIoCall, SpoolLimits, SpoolObjectState, StagingSpool,
};
use fss_publication::{
    LOCAL_ROOTS_DIR, LOCAL_SPOOL_DIR, LocalPublicationLimits, LocalPublicationState,
    LocalRootPublisher, LockTableSource, MAX_LOCK_TABLE_BYTES, ROOT_INDETERMINATE_SUFFIX,
    ROOT_RECORD_SUFFIX, ROOT_TEMP_SUFFIX, SlotName, StringLockTableSource, UnknownLockReason,
    WriterDetectionOptions, WriterState,
};
...
fn r66_c_fixture_non_mutation_differential_and_a_promotion() -> TestResult {
    ...
    // Documented extra: inspection observed no fsync, so its admitted roots stay Visible where
    // the open reports them Durable after its directory fsync.
    let open_durable = pb.recovery_report().clone();
    assert!(
        open_durable
            .roots
            .iter()
            .all(|root| root.state == LocalPublicationState::Durable)
    );
    assert!(li.durability_not_resynced);
    let mut open_report = open_durable;
    for root in &mut open_report.roots {
        root.state = LocalPublicationState::Visible;
    }
    assert_eq!(
        li.redundant_temps,
        vec![
            Path::new(LOCAL_ROOTS_DIR).join(format!("good{ROOT_RECORD_SUFFIX}{ROOT_TEMP_SUFFIX}"))
        ]
    );
    assert!(li.holds_migration_pending);
...
fn r66_b_writer_detection_with_lock_held() -> TestResult {
    ...
    // The opt-in probe takes exactly one shared lock and makes no other mutating call.
    assert_eq!(calls, vec![SpoolIoCall::TryLockShared]);
    assert!(s.is_held());
    ...
    // The caller's own flock would read as a writer, so self_holds_lock is honoured on the
    // lock-table path as on the probe path.
    let (s, calls) = probe(&root, None, false, true)?;
...
fn r66_broken_root_manifest_is_held_after_open() -> TestResult {
    ...
    let p2 = LocalRootPublisher::open(&root, limits())?;
    assert!(
        p2.recovery_report()
            .broken_roots
            .iter()
    ...
    // Reopened alone, the spool reports every object Staged; the durable hold, not the object
    // state, is what refuses the discard.
    let mut spool = StagingSpool::open(root.join(LOCAL_SPOOL_DIR), limits().spool)?;
    assert_eq!(spool.state(manifest_root), Some(SpoolObjectState::Staged));
    match spool.discard_staged(manifest_root) {
        Err(SpoolError::VerificationHeld { digest }) => assert_eq!(digest, manifest_root),
