#![forbid(unsafe_code)]
//! FSS-059 malformed-media gauntlet for the H.264 pixel decoder.
//!
//! `hostile_input.rs` already covers random bit flips (900 rounds) and byte-step
//! truncation prefixes; this gauntlet adds the remaining FSS-059 classes over the
...
//! (STAP-A / AP sizes) instead of here.
//!
...
use fss_codec_h264::{DecodeError, Decoder, DecoderLimits, annex_b_nal_units};
use media_mutation::{
...
const FIXTURES: [(&str, &[u8]); 8] = [
    ...
    ("baseline_i64", include_bytes!("fixtures/baseline_i64.h264")),
    (
        "ip_100x60_crop",
...
/// Narrowed owner limits: every fixture fits (largest is QCIF, 99 macroblocks).
...
fn seed(name: &str, bytes: &[u8]) -> Seed {
    ...
    // A few forged NAL headers: IDR, SPS, PPS, data partition, and a forbidden bit.
    for header in [0x65_u8, 0x67, 0x68, 0x22, 0x80, 0x0E, 0x14] {
        seed.markers.push(vec![0, 0, 1, header]);
    }
    seed
}
...
fn decode(bytes: &[u8]) -> Result<Outcome, String> {
    ...
    let mut take = |picture: fss_codec_h264::Picture| -> Result<(), String> {
...
#[test]
fn h264_decoder_survives_mutation_gauntlet() {
    const RNG_SEED: u64 = 0x0F55_0059_0000_0264;
    ...
    finish("h264", &tally, failures, MUTANTS);
}
...
#[test]
fn h264_structural_truncation_and_nal_loss_publish_only_clean_pictures() {
    let mut failures = Vec::new();
    let mut cuts = 0;
    let mut deletions = 0;
    for (name, bytes) in FIXTURES {
        ...
        for cut in seed.structural_cuts() {
            ...
            check("h264-cut", name, 0, cut, &mutation, &mut failures, run);
        }
        for (index, unit) in seed.units.iter().enumerate() {
            ...
            check("h264-delete", name, 0, index, &mutation, &mut failures, run);
        }
    }
    println!(
        "FSS-059 gauntlet h264-structural: {cuts} NAL-boundary cuts, {deletions} single-NAL deletions"
    );
