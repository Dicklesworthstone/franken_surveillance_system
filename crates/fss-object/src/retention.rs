...
impl Default for RetentionLimits {
    fn default() -> Self {
        Self {
            max_entries: MAX_RETENTION_ENTRIES,
            max_edges: MAX_RETENTION_EDGES,
            max_holds: MAX_RETENTION_HOLDS,
            max_receipts: MAX_RETENTION_RECEIPTS,
        }
    }
}
...
impl RetentionPlan {
    ...
    #[must_use]
    pub fn selected(&self) -> &[ContentDigest] {
        &self.selected
    }
    ...
    fn computed_digest(&self) -> Result<ContentDigest, RetentionError> {
        ...
        e.digest(self.state);
        e.digest(self.policy);
        e.i128(self.prepared_at.0);
        e.u64(self.budget.max_objects as u64);
        e.u64(self.budget.max_deleted_bytes);
        e.u64(self.budget.max_staging_bytes);
        e.u64(self.selected_bytes);
        e.u64(self.selected.len() as u64);
        for digest in &self.selected {
            e.digest(*digest);
        }
        for set in [
            &self.blocked_due,
...
impl RetentionStore {
    ...
    pub fn new(policy: ContentDigest, objects: ObjectLimits, limits: RetentionLimits) -> Self {
        ...
        Self {
            custody: InMemoryObjectStore::new(objects),
            policy,
            limits,
    ...
    #[must_use]
    pub const fn custody(&self) -> &InMemoryObjectStore {
        &self.custody
    }
    ...
    pub fn put_derivative(
        &mut self,
        bytes: &[u8],
        rule: RetentionRule,
        sources: BTreeSet<ContentDigest>,
    ) -> Result<ContentDigest, RetentionError> {
        if sources.is_empty() {
    ...
    fn put(
        ...
        if let Some(existing) = self.entries.get(&digest) {
            if existing != &entry {
                return Err(RetentionError::ConflictingRegistration);
            }
        ...
        let edges = self
            .entries
            .values()
            .try_fold(entry.dependencies.len(), |n, entry| {
                n.checked_add(entry.dependencies.len())
            })
        ...
        match manifest {
            Some(manifest) => {
                self.custody.publish_manifest(manifest)?;
            }
            None => {
                self.custody.put_verified(bytes)?;
            }
        }
    ...
    pub fn add_hold(
        ...
        self.live_entry(subject)?;
        self.live_entry(witness)?;
        self.custody.read_verified(subject)?;
        self.custody.read_verified(witness)?;
        let hold = Hold {
            subject,
            witness,
            released_by: None,
        ...
        let next = self
            ...
            .checked_add(1)
            .ok_or(RetentionError::CapacityExceeded)?;
        self.holds.insert(id.to_owned(), hold);
        self.revision = next;
        Ok(())
    }
    ...
    pub fn release_hold(
        ...
        self.holds
            .get_mut(id)
            .ok_or(RetentionError::HoldConflict)?
            .released_by = Some(witness);
        self.revision = next;
        Ok(())
    }
    ...
    fn encode_state(&self) -> Result<Vec<u8>, RetentionError> {
        let mut e = CanonicalEncoder::new();
        e.text("fss.reference_retention_state.v1");
        e.digest(self.policy);
        e.u64(self.revision);
        for limit in [
            self.limits.max_entries,
            self.limits.max_edges,
            self.limits.max_holds,
            self.limits.max_receipts,
        ] {
            e.u64(limit as u64);
        }
        e.u64(self.custody.limits().max_objects as u64);
        e.u64(self.custody.limits().max_total_bytes);
        e.bool(self.last_commit_at.is_some());
        if let Some(time) = self.last_commit_at {
            e.i128(time.0);
        }
        e.u64(self.entries.len() as u64);
        for (digest, entry) in &self.entries {
            e.digest(*digest);
            e.i128(entry.rule.retain_until.0);
            e.text(&entry.rule.reason);
            e.u64(entry.bytes);
            e.bool(entry.manifest);
            encode_set(&entry.dependencies, &mut e);
            e.bool(entry.deleted_by.is_some());
            if let Some(plan) = entry.deleted_by {
                e.digest(plan);
            }
        }
        ...
        for (id, hold) in &self.holds {
            e.text(id);
            e.digest(hold.subject);
            e.digest(hold.witness);
            e.bool(hold.released_by.is_some());
            if let Some(witness) = hold.released_by {
                e.digest(witness);
            }
        }
        Ok(e.finish_checked()?)
    ...
    pub fn prepare_expiry(
        ...
        let live: BTreeSet<_> = self
            .entries
            .iter()
            .filter_map(|(digest, entry)| entry.deleted_by.is_none().then_some(*digest))
            .collect();
        let not_due: BTreeSet<_> = live
            .iter()
            .filter(|digest| {
                self.entries
                    .get(*digest)
                    .is_some_and(|entry| now < entry.rule.retain_until)
            })
            .copied()
            .collect();
        let held_roots: BTreeSet<_> = self
            .holds
            .values()
            .filter(|hold| hold.released_by.is_none())
            .flat_map(|hold| [hold.subject, hold.witness])
            .collect();
        let mut protected = not_due.clone();
        protected.extend(&held_roots);
        let mut pending: Vec<_> = protected.iter().copied().collect();
        while let Some(digest) = pending.pop() {
            for dependency in &self.live_entry(digest)?.dependencies {
                if protected.insert(*dependency) {
        ...
        let mut dependents: BTreeMap<_, usize> =
            eligible.iter().map(|digest| (*digest, 0)).collect();
        ...
        let mut ready: BTreeSet<_> = dependents
            .iter()
            .filter_map(|(digest, count)| (*count == 0).then_some(*digest))
            .collect();
        let mut selected = Vec::new();
        let mut selected_bytes = 0_u64;
        while let Some(digest) = ready.pop_first() {
            let entry = self.live_entry(digest)?;
            let Some(total) = selected_bytes.checked_add(entry.bytes) else {
                continue;
            };
            if selected.len() >= budget.max_objects || total > budget.max_deleted_bytes {
                continue;
            }
        ...
        let mut plan = RetentionPlan {
            ...
            held_roots,
            not_due,
            digest: ContentDigest::sha256(b"unpublished-retention-plan"),
        };
    ...
    fn verify_live_graph(&self) -> Result<(), RetentionError> {
        if self.entries.len() != self.custody.object_count() {
            return Err(RetentionError::InvalidRecord);
        }
        for (digest, entry) in &self.entries {
