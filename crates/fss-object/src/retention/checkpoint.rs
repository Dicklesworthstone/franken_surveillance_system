...
impl RetentionStore {
    ...
    pub fn checkpoint(&self, max_bytes: usize) -> Result<RetentionCheckpoint, RetentionError> {
        ...
        e.text(FORMAT);
        e.bytes(&self.encode_state()?);
        e.u64(self.receipts.len() as u64);
        for receipt in self.receipts.values() {
            receipt.encode_checkpoint(&mut e);
        }
    ...
    pub fn restore_checkpoint(
        ...
        ceilings: RetentionLimits,
        budget: RetentionRecoveryBudget,
    ) -> Result<Self, RetentionError> {
        if bytes.len()
            > budget
                .max_checkpoint_bytes
                .min(MAX_RETENTION_CHECKPOINT_BYTES)
        {
            return Err(RetentionError::CapacityExceeded);
        }
        ...
        if state.text()? != "fss.reference_retention_state.v1" {
            return Err(RetentionError::InvalidRecord);
        }
        ...
        let limits = RetentionLimits {
            ...
            max_receipts: count(
                &mut state,
                ceilings.max_receipts.min(MAX_RETENTION_RECEIPTS),
            )?,
        };
        let object_limits = ObjectLimits::new(count(&mut state, usize::MAX)?, state.u64()?);
        if object_limits != custody.limits() {
            return Err(RetentionError::InvalidRecord);
        }
        let last_commit_at = if state.bool()? {
            Some(TimestampNs(state.i128()?))
        } else {
            None
        };
        ...
        for _ in 0..count(&mut state, limits.max_entries)? {
            ...
            let rule = RetentionRule {
                retain_until: TimestampNs(state.i128()?),
                reason: text(&mut state)?,
            };
            let payload_bytes = state.u64()?;
            let manifest = state.bool()?;
            if payload_bytes > crate::MAX_OBJECT_BYTES as u64 {
                return Err(RetentionError::InvalidRecord);
            }
        ...
        for _ in 0..count(&mut state, limits.max_holds)? {
            ...
            holds.insert(
                ...
                Hold {
                    ...
                    witness: state.digest()?,
                    released_by: if state.bool()? {
                        Some(state.digest()?)
                    } else {
                        None
                    },
        ...
        for _ in 0..count(&mut d, limits.max_receipts)? {
            let receipt =
                DeletionReceipt::decode_checkpoint(&mut d, entries.len() - total_deleted)?;
    ...
    fn validate_recovered_metadata(&self) -> Result<(), RetentionError> {
        ...
        let mutations = self.entries.len()
            + self.holds.len()
            + self.receipts.len()
            + self
                .holds
                .values()
                .filter(|hold| hold.released_by.is_some())
                .count();
        if self.revision != mutations as u64 {
            return Err(RetentionError::InvalidRecord);
        }
        ...
        for (digest, entry) in &self.entries {
            if !valid_text(&entry.rule.reason) {
                return Err(RetentionError::InvalidRecord);
            }
        ...
        if live_bytes != self.custody.total_bytes() {
            return Err(RetentionError::InvalidRecord);
        }
        ...
        for (id, hold) in &self.holds {
            if !valid_text(id)
                || !self.entries.contains_key(&hold.subject)
                || !self.entries.contains_key(&hold.witness)
            {
                return Err(RetentionError::InvalidRecord);
            }
            if let Some(released) = hold.released_by {
                if !self.entries.contains_key(&released) {
                    return Err(RetentionError::InvalidRecord);
                }
            } else {
                self.live_entry(hold.subject)?;
                self.live_entry(hold.witness)?;
            }
        }
        ...
        for (plan, receipt) in &self.receipts {
            ...
            if released_bytes != receipt.released_bytes() {
                return Err(RetentionError::InvalidRecord);
            }
            latest_time = Some(
                latest_time.map_or(receipt.committed_at(), |time: TimestampNs| {
                    time.max(receipt.committed_at())
                }),
            );
        }
        if self
            .entries
            .iter()
            .any(|(digest, entry)| entry.deleted_by.is_some() != deleted.contains(digest))
...
fn require_next(prior: Option<ContentDigest>, next: ContentDigest) -> Result<(), RetentionError> {
    if prior.is_some_and(|value| value >= next) {
        return Err(RetentionError::InvalidRecord);
    }
    Ok(())
}
