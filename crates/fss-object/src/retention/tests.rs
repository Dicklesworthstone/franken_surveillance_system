...
fn rule(until: i128) -> RetentionRule {
    RetentionRule {
        ...
        reason: "rule:rolling-media".to_owned(),
    }
}
...
fn store() -> RetentionStore {
    RetentionStore::new(
        ContentDigest::sha256(b"policy:v1"),
...
fn grant(
    ...
    RetentionAuthorization {
        policy_digest: store.policy,
        plan_digest: plan.digest(),
        witness,
        permitted_objects: plan.selected().iter().copied().collect(),
        issued_at: TimestampNs(0),
        expires_at: TimestampNs(10_000),
    }
}
...
fn manifest_metadata_and_transitive_derivatives_are_real_dependencies() -> TestResult {
    ...
    let root = s.publish_manifest(
        ObjectManifest::new("event", [embedding], Some(metadata))?,
        rule(100),
    )?;
    let plan = s.prepare_expiry(TimestampNs(20), budget(128))?;
    assert!(plan.selected().is_empty());
    assert_eq!(
        plan.blocked_due(),
        &BTreeSet::from([source, crop, embedding, metadata])
    );
...
fn active_hold_pins_entire_graph_and_witness_until_explicit_release() -> TestResult {
    ...
    assert!(matches!(
        s.add_hold(s.state_digest()?, "hold:case", held, proof),
...
fn bounded_batches_delete_dependents_before_sources_without_dangling_roots() -> TestResult {
    ...
    assert!(
        s.prepare_expiry(TimestampNs(10), budget(1))?
            .selected()
            .is_empty()
    );
    assert_eq!(
        s.custody().total_bytes(),
        s.custody().read_verified(w)?.len() as u64
    );
    Ok(())
}
...
fn exact_deadline_and_zero_budget_have_explicit_results() -> TestResult {
    ...
    let source = s.put_source(b"source", rule(10))?;
    assert!(
        s.prepare_expiry(TimestampNs(9), budget(1))?
            .selected()
            .is_empty()
    );
    assert_eq!(
        s.prepare_expiry(TimestampNs(10), budget(1))?.selected(),
        &[source]
    );
...
fn registration_is_idempotent_but_never_rewrites_retention_or_dependencies() -> TestResult {
    ...
    assert!(matches!(
        ...
        Err(RetentionError::ConflictingRegistration)
    ));
    assert!(
        s.put_derivative(
            b"derived",
            rule(1),
            BTreeSet::from([ContentDigest::sha256(b"absent")])
        )
        .is_err()
    );
    assert_eq!(s.state_digest()?, before);
...
fn opaque_manifest_bytes_cannot_be_promoted_without_dependency_registration() -> TestResult {
    ...
    assert!(matches!(
        s.publish_manifest(manifest, rule(1)),
        Err(RetentionError::ConflictingRegistration)
    ));
    Ok(())
}
...
fn new_hold_or_new_parent_invalidates_a_prepared_plan() -> TestResult {
    for hold in [false, true] {
        ...
        let authority = grant(&s, &plan, w);
        if hold {
            s.add_hold(s.state_digest()?, "hold:new", source, w)?;
        } else {
            s.put_derivative(b"new dependent", rule(100), BTreeSet::from([source]))?;
        }
        let before = s.state_digest()?;
        let bytes = s.custody().total_bytes();
        assert!(matches!(
            s.execute_expiry(&plan, &authority, TimestampNs(10)),
            Err(RetentionError::StalePlan)
        ));
        assert_eq!(s.state_digest()?, before);
        assert_eq!(s.custody().total_bytes(), bytes);
...
fn projected_authority_policy_scope_and_lease_are_enforced_before_mutation() -> TestResult {
    for fault in 0..5 {
        ...
        assert!(matches!(
            s.execute_expiry(&plan, &authority, TimestampNs(10)),
            Err(RetentionError::Unauthorized)
        ));
        assert_eq!(s.state_digest()?, before);
        s.custody().read_verified(source)?;
    }
...
fn custody_corruption_and_staging_budget_return_no_partial_deletion() -> TestResult {
    for corrupt in [false, true] {
        let mut s = store();
        let w = witness(&mut s)?;
        let a = s.put_source(b"a", rule(1))?;
        let b = s.put_source(b"b", rule(1))?;
        ...
        assert_eq!(s.state_digest()?, before);
        assert_eq!(s.custody().total_bytes(), bytes);
        assert!(!s.custody().is_tombstoned(a));
        assert!(!s.custody().is_tombstoned(b));
...
fn exact_retry_preserves_receipt_and_cannot_resurrect_payload() -> TestResult {
    ...
    assert!(
        s.put_derivative(b"resurrection", rule(20), BTreeSet::from([source]))
            .is_err()
    );
...
fn clock_regression_and_resealed_selection_are_refused() -> TestResult {
    ...
    let authority = grant(&s, &plan, w);
    assert!(matches!(
        s.execute_expiry(&plan, &authority, TimestampNs(9)),
        Err(RetentionError::ClockRegression)
    ));
    let mut forged = plan.clone();
    forged.selected.clear();
    forged.digest = forged.computed_digest()?;
    let forged_authority = grant(&s, &forged, w);
    assert!(matches!(
        s.execute_expiry(&forged, &forged_authority, TimestampNs(10)),
        Err(RetentionError::StalePlan)
    ));
    s.execute_expiry(&plan, &authority, TimestampNs(10))?;
    assert!(matches!(
...
fn entry_edge_hold_and_receipt_ceilings_do_not_evict_protection() -> TestResult {
    ...
    let source = s.put_source(b"source", rule(1))?;
    assert!(matches!(
        s.put_source(b"overflow", rule(1)),
        Err(RetentionError::CapacityExceeded)
    ));
    assert!(matches!(
        s.add_hold(s.state_digest()?, "hold:x", source, w),
        Err(RetentionError::CapacityExceeded)
    ));
    let plan = s.prepare_expiry(TimestampNs(10), budget(128))?;
    let authority = grant(&s, &plan, w);
    ...
    assert!(matches!(
        t.put_derivative(b"derived", rule(1), BTreeSet::from([source])),
        Err(RetentionError::CapacityExceeded)
    ));
...
fn unregistered_custody_mutation_is_not_silently_adopted() -> TestResult {
    ...
    assert!(matches!(
        s.prepare_expiry(TimestampNs(10), budget(128)),
        Err(RetentionError::InvalidRecord)
    ));
    Ok(())
}
...
fn independent_insertion_order_produces_identical_state_and_plan() -> TestResult {
    ...
    for bytes in [b"a".as_slice(), b"b", b"c"] {
        a.put_source(bytes, rule(1))?;
    }
    for bytes in [b"c".as_slice(), b"a", b"b"] {
        b.put_source(bytes, rule(1))?;
    }
    assert_eq!(a.state_digest()?, b.state_digest()?);
    assert_eq!(
        a.prepare_expiry(TimestampNs(10), budget(2))?,
        b.prepare_expiry(TimestampNs(10), budget(2))?
    );
    Ok(())
}
    ...
    for edges in 0_u32..64 {
        for deadlines in 0_u32..16 {
            ...
            for node in 0..4 {
                ...
                nodes.push(if dependencies.is_empty() {
                    s.put_source(&bytes, rule(until))?
                } else {
                    s.put_derivative(&bytes, rule(until), dependencies)?
                });
            }
            for count in 0..5 {
                ...
                for (digest, entry) in &s.entries {
                    if !selected.contains(digest) {
                        assert!(
                            entry.dependencies.is_disjoint(&selected),
                            "edges={edges} deadlines={deadlines} count={count}"
                        );
                    }
                }
            }
