...
const PRIVATE_SOURCE: &[u8] =
    b"original private camera bytes are never retention-checkpoint payload";
fn rule(until: i128) -> RetentionRule {
    RetentionRule {
        retain_until: TimestampNs(until),
        reason: "rule:reference".to_owned(),
    }
}
...
fn fixture() -> Result<(RetentionStore, ContentDigest, ContentDigest, ContentDigest), Box<dyn Error>>
{
    let mut s = RetentionStore::new(
        ContentDigest::sha256(b"retention-policy"),
...
fn restore(s: &RetentionStore, cp: &RetentionCheckpoint) -> Result<RetentionStore, RetentionError> {
    RetentionStore::restore_checkpoint(
        cp.as_bytes(),
        cp.digest(),
        s.policy,
        s.custody(),
...
fn checkpoint_preserves_holds_released_ids_and_canonical_plan() -> TestResult {
    ...
    assert!(
        !cp.as_bytes()
            ...
            .any(|bytes| bytes == PRIVATE_SOURCE)
    );
    let mut restored = restore(&s, &cp)?;
    assert_eq!(restored.state_digest()?, s.state_digest()?);
    assert_eq!(restored.checkpoint(1_000_000)?, cp);
    assert_eq!(
        restored.prepare_expiry(TimestampNs(10), sweep())?,
        s.prepare_expiry(TimestampNs(10), sweep())?
    );
    assert!(
        restored
            .prepare_expiry(TimestampNs(10), sweep())?
            .selected()
            .is_empty()
    );
    assert!(matches!(
        restored.add_hold(restored.state_digest()?, "hold:released", source, w),
...
fn checkpoint_preserves_deletion_receipts_and_exact_retry_without_payloads() -> TestResult {
    ...
    assert_eq!(
        &receipt,
        s.receipts.get(&plan.digest()).ok_or("missing receipt")?
    );
    assert_eq!(restored.checkpoint(1_000_000)?, cp);
...
fn every_truncation_mutation_and_trailing_byte_fails_the_pinned_root() -> TestResult {
    ...
    let mut extra = cp.as_bytes().to_vec();
    extra.push(0);
    assert!(
        RetentionStore::restore_checkpoint(
            &extra,
            ContentDigest::sha256(&extra),
            s.policy,
            s.custody(),
            RetentionLimits::default(),
            recovery()
...
fn accepted_policy_metadata_and_clone_ceilings_are_mandatory() -> TestResult {
    ...
    for budget in [
        RetentionRecoveryBudget {
            max_checkpoint_bytes: cp.as_bytes().len() - 1,
            ..recovery()
        },
        RetentionRecoveryBudget {
            max_custody_bytes: 0,
...
fn missing_foreign_corrupt_and_resurrected_objects_are_rejected() -> TestResult {
    ...
    let mut foreign = s.custody().clone();
    foreign.put_verified(b"foreign")?;
    let mut corrupt = s.custody().clone();
    corrupt.corrupt_for_test(source)?;
...
fn changed_tombstone_authority_is_not_equivalent_custody() -> TestResult {
    ...
    for digest in [derivative, source] {
        alternate.tombstone(
            digest,
            TombstoneRecord::new(
                ObjectId::parse("object:other-authority")?,
                generation.next()?,
                generation,
