...
pub(crate) struct SpoolClassification {
    ...
    pub(crate) over_capacity: Option<SpoolError>,
}
...
impl StagingSpool {
    ...
    pub fn open_with_io(
        ...
        if let Some(err) = classification.over_capacity {
            return Err(err);
        }
        if legacy {
            migrate_holds(
...
pub(crate) fn classify_spool(
    ...
    let over_capacity =
        if index.len() > limits.max_objects || occupied_bytes > limits.max_total_bytes {
            Some(SpoolError::RecoveredOverCapacity {
                objects: index.len(),
...
pub fn inspect_with_io(
    ...
    Ok(SpoolInspection {
        ...
        over_capacity: classification.over_capacity.is_some(),
    })
}
