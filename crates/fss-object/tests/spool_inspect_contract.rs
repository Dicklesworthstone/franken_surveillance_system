...
//! - Lock-free verified payload reading via [`read_verified_payload`].
...
use fss_object::{
    ...
    SPOOL_OBJECTS_DIR, SPOOL_STAGING_DIR, SpoolError, SpoolLimits, StagingSpool, inspect,
...
fn test_spool_differential_corrupt_object() -> TestResult {
    ...
    let last = bytes.last_mut().ok_or(SpoolError::Corrupt {
        digest,
...
fn test_spool_read_only_recording_io_proof() -> TestResult {
    ...
    let inspection = inspect_with_io(&root, &l, &recording_io)?;
    assert_eq!(inspection.report.admitted.len(), 1);
...
fn test_spool_read_verified_payload_lock_free() -> TestResult {
    ...
    let read_bytes = read_verified_payload(&root, digest, MAX_OBJECT_BYTES, &HostSpoolIo)?;
    assert_eq!(read_bytes, payload);

    let mut raw = digest.bytes();
    raw[0] ^= 0x01;
    let bad_digest = ContentDigest::new(DigestAlgorithm::Sha256, raw);
    let err = read_verified_payload(&root, bad_digest, MAX_OBJECT_BYTES, &HostSpoolIo);
    assert!(err.is_err());
