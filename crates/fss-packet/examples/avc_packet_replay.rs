...
use fss_packet::avc::{
    AvcAssemblyStep, AvcPictureGroup, AvcReceiveLimits, AvcReceivePoll,
...
fn main() -> Result<(), Error> {
    replay("baseline", include_bytes!("../tests/fixtures/avc/baseline.264"),
        &[90_000, 93_600, 97_200, 100_800])?;
    replay("high_cropped", include_bytes!("../tests/fixtures/avc/high_cropped.264"),
...
fn replay(name: &str, bytes: &[u8], times: &[u32]) -> Result<(), Error> {
    ...
    let key = StreamKey { ingress: 17, generation: 1, ssrc: 7 };
    ...
    println!("{{\"kind\":\"terminal\",\"fixture\":\"{name}\",\"pictures\":{pictures},\"retained_bytes\":0,\"decoded_by_this_receiver\":false,\"qualification\":\"reference_rehearsal_only\"}}");
    Ok(())
}
...
fn drain(r: &mut AvcReceiver, name: &str, now: u64, finishing: bool) -> Result<usize, Error> {
    ...
    for _ in 0..1_024 {
        match r.poll(now)? {
            AvcReceivePoll::Source { source, queued_nals, picture, fragment, gap_before, .. } => {
                if picture.is_some() || fragment.is_some() || gap_before { return Err("clean source discontinuity".into()); }
                let digest = ContentDigest::try_sha256(source.bytes())?.to_text();
                println!("{{\"kind\":\"source\",\"fixture\":\"{name}\",\"sequence\":{},\"digest\":\"{digest}\",\"queued_nals\":{queued_nals}}}", source.sequence());
            }
            AvcReceivePoll::Assembly(AvcAssemblyStep::Accepted(output)) => {
                if output.retired.is_some() { return Err("clean assembly retired".into()); }
                if let Some(picture) = output.picture { report(name, &picture)?; count += 1; }
            }
            AvcReceivePoll::Picture(picture) => { report(name, &picture)?; count += 1; }
            AvcReceivePoll::Pending { .. } if !finishing => return Ok(count),
            AvcReceivePoll::Ended { fragment: None, interrupted_picture: None, tail } if finishing => {
                if let Some(tail) = tail {
                    if tail.retired.is_some() { return Err("clean tail retired".into()); }
                    if let Some(picture) = tail.picture { report(name, &picture)?; count += 1; }
                }
                return Ok(count);
            }
...
fn report(name: &str, picture: &AvcPictureGroup) -> Result<(), Error> {
    ...
    println!("{{\"kind\":\"picture_group\",\"fixture\":\"{name}\",\"timestamp\":{},\"frame_num\":{},\"nals\":{},\"bytes\":{},\"width\":{width},\"height\":{height},\"boundary\":\"{:?}\",\"saw_first_macroblock\":{},\"discontinuity_before\":{},\"complete_picture_certified\":false}}",
        picture.timestamp(), picture.identity().frame_num(), picture.nals().len(), picture.byte_len(),
        picture.boundary(), picture.saw_first_macroblock(), picture.discontinuity_before());
    for nal in picture.nals() {
        let digest = ContentDigest::try_sha256(nal.bytes())?.to_text();
        println!("{{\"kind\":\"group_nal\",\"fixture\":\"{name}\",\"digest\":\"{digest}\",\"source_spans\":{}}}", nal.sources().len());
    }
    Ok(())
}
