...
use fss_packet::{
    H264Depacketizer, H264Error, H264Limits, H264Mode, PacketLimits, RtpPacket, SequenceTracker,
    StreamKey,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    .enumerate()
    {
        ...
        println!(
            "{{\"kind\":\"packet\",\"fixture\":\"rtp-h264-loss-wrap-v1\",\"ingress\":1,\"generation\":1,\"ssrc\":7,\"ordinal\":{ordinal},\"sequence\":{seq},\"digest\":\"{digest}\",\"class\":\"{:?}\",\"missing\":{}}}",
            observed.class, observed.stats.missing
        );
        ...
        let extended = observed
            .extended_sequence
            .ok_or("unique packet lacks extended sequence")?;
        match media.push(key, extended, packet, ordinal as u64 * 1_000_000) {
            ...
            Err(failure) => {
                ...
                println!(
                    "{{\"kind\":\"discard\",\"ingress\":1,\"generation\":1,\"reason\":\"Gap\",\"first_sequence\":{},\"last_sequence\":{},\"bytes\":{}}}",
                    discard.first_sequence, discard.last_sequence, discard.byte_len
                );
            }
        }
    }
    ...
    println!(
        "{{\"kind\":\"terminal\",\"ingress\":1,\"generation\":1,\"complete_nals\":{complete},\"safe_failures\":{failures},\"missing\":1,\"cancelled_bytes\":{},\"pending_bytes\":0,\"qualification\":\"reference_rehearsal_only\"}}",
        discard.byte_len
    );
    Ok(())
}
