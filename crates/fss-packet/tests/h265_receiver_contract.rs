...
use fss_packet::{
    ContinuityError, H265Error, H265Limits, H265Output, H265ReceiveError,
    H265ReceivePoll, H265Receiver, H265Status, OrderedRtpPacket, QueueDiscardReason,
    ReorderDisposition, ReorderError, ReorderGapReason, ReorderLimits, StreamKey,
};
...
fn packet_step(receiver: &mut H265Receiver, now: u64) -> Result<PacketStep, Box<dyn std::error::Error>> {
    match receiver.poll(now)? {
        H265ReceivePoll::Packet { source, reconstruction } => Ok((source, reconstruction)),
    ...
    for seq in 0..4 {
        ...
        loop {
            match receiver.poll(u64::from(seq))? {
                H265ReceivePoll::Packet { reconstruction, .. } => { reconstruction?; }
...
fn out_of_order_fragments_reconstruct_with_exact_original_datagrams() -> TestResult {
    ...
    for (index, wire) in [&start, &middle, &end].into_iter().enumerate() {
        ...
        assert_eq!(&wire[span.wire_range.clone()], &output.nals[0].bytes()[span.nal_range.clone()]);
    }
...
fn malformed_codec_output_still_returns_its_original_source() -> TestResult {
    ...
    assert!(matches!(error, H265ReceiveError::Codec(failure) if failure.reason == H265Error::Malformed));
    assert_eq!(receiver.queued_packets(), 0);
    receiver.ingest(KEY, &packet(5, true, &[0x26, 1, 3]), 5)?;
    assert_eq!(packet_step(&mut receiver, 5)?.1?.nals[0].bytes(), [0x26, 1, 3]);
    Ok(())
}
...
fn delivery_deadline_retires_chain_before_returning_orphaned_end_packet() -> TestResult {
    ...
    let codec = H265Limits { max_pending_age_ns: 100, ..H265Limits::default() };
    ...
    assert!(matches!(receiver.poll(14)?, H265ReceivePoll::Pending { wake_at_ns: Some(15) }));
    ...
    assert_eq!(discarded.ok_or("pending FU was not retired")?.reason, H265Error::Gap);
    let (source, reconstruction) = packet_step(&mut receiver, 15)?;
    assert_eq!(source.bytes(), end);
    assert!(matches!(reconstruction, Err(H265ReceiveError::Codec(failure))
...
fn fragment_timer_runs_without_packets_and_does_not_lose_queued_source() -> TestResult {
    ...
    assert!(matches!(receiver.poll(13)?, H265ReceivePoll::Pending { wake_at_ns: Some(14) }));
    let clean = packet(5, true, &[0x26, 1, 9]);
    receiver.ingest(KEY, &clean, 14)?;
    assert!(matches!(receiver.poll(14)?, H265ReceivePoll::FragmentDiscarded(discard)
...
fn queue_backpressure_does_not_consume_the_refused_sequence() -> TestResult {
    let reorder = ReorderLimits { max_packets: 1, ..ReorderLimits::default() };
    let mut receiver = open(reorder, H265Limits::default())?;
    receiver.ingest(KEY, &packet(4, false, &[0x62, 1, 0x93, 1]), 4)?;
    let end = packet(5, true, &[0x62, 1, 0x53, 2]);
    let before = receiver.stats();
    assert_eq!(receiver.ingest(KEY, &end, 5).err().ok_or("capacity was not enforced")?,
    ...
    assert_eq!(admission.transport.disposition, ReorderDisposition::Buffered);
    assert_eq!(packet_step(&mut receiver, 5)?.1?.nals[0].bytes(), [0x26, 1, 1, 2]);
    Ok(())
}
...
fn finish_drains_accepted_fragments_before_finalizing_the_codec() -> TestResult {
    ...
    receiver.finish();
    assert_eq!(packet_step(&mut receiver, 5)?.1?.status, H265Status::FragmentPending);
    assert_eq!(packet_step(&mut receiver, 5)?.1?.nals[0].bytes(), [0x26, 1, 1, 2]);
    assert!(matches!(receiver.poll(5)?, H265ReceivePoll::Ended { discarded: None }));
    assert!(matches!(receiver.poll(6)?, H265ReceivePoll::Ended { discarded: None }));
    assert_eq!(receiver.next_wake_ns(), None);
    assert!(receiver.ingest(KEY, &packet(6, true, &[0x26, 1, 3]), 6).is_err());
    Ok(())
}
...
fn finish_emits_intervening_gap_and_incomplete_eof_retirement_once() -> TestResult {
    ...
    receiver.finish();
    assert!(matches!(receiver.poll(4)?, H265ReceivePoll::Gap { gap, discarded: None }
        if gap.first_sequence == 4 && gap.last_sequence == 4 && gap.reason == ReorderGapReason::EndOfInput));
    packet_step(&mut receiver, 4)?.1?;
    assert!(matches!(receiver.poll(4)?, H265ReceivePoll::Ended { discarded: Some(discard) }
        if discard.reason == H265Error::EndOfInput && discard.first_sequence == 5));
    assert!(matches!(receiver.poll(4)?, H265ReceivePoll::Ended { discarded: None }));
    Ok(())
}
...
fn cancellation_accounts_for_queue_and_codec_without_publishing_either() -> TestResult {
    ...
    assert_eq!(cancelled.fragment.ok_or("missing FU cancellation")?.reason, H265Error::Cancelled);
    assert_eq!(receiver.queued_bytes(), 0);
    assert_eq!(receiver.pending_nal_bytes(), 0);
    assert!(matches!(receiver.poll(5)?, H265ReceivePoll::Ended { discarded: None }));
...
fn confirmed_source_restart_fences_both_layers_and_requires_new_generation() -> TestResult {
    ...
    assert_eq!(suspect.transport.disposition, ReorderDisposition::NotAdmitted);
    ...
    let queue = restart.transport.discarded.ok_or("missing queue retirement")?;
    assert_eq!(queue.reason, QueueDiscardReason::RestartRequired);
    assert_eq!(queue.packets, 1);
    assert_eq!(restart.discarded.ok_or("missing codec retirement")?.reason, H265Error::Gap);
    assert!(matches!(receiver.poll(7)?, H265ReceivePoll::Ended { discarded: None }));
    assert!(receiver.restart(KEY, 96, 0, ReorderLimits::default(), H265Limits::default()).is_err());
    let new_key = StreamKey { generation: 2, ..KEY };
    let mut next = receiver.restart(new_key, 96, 0, ReorderLimits::default(), H265Limits::default())?;
    prime(&mut next, new_key)?;
    next.ingest(new_key, &packet(4, true, &[0x62, 1, 0x53, 2]), 4)?;
    assert!(matches!(packet_step(&mut next, 4)?.1, Err(H265ReceiveError::Codec(failure))
        if failure.reason == H265Error::MissingStart));
    assert!(next.ingest(KEY, &packet(5, true, &[0x26, 1, 3]), 5).is_err());
    Ok(())
}
...
fn speculative_restart_does_not_discard_old_owner_work() -> TestResult {
    ...
    receiver.ingest(KEY, &packet(4, false, &[0x62, 1, 0x93, 1]), 4)?;
    packet_step(&mut receiver, 4)?.1?;
    let next = receiver.restart(StreamKey { generation: 2, ..KEY }, 96, 0,
        ReorderLimits::default(), H265Limits::default())?;
...
fn reversed_time_and_conflicting_duplicate_refusals_preserve_pending_work() -> TestResult {
    ...
    let before = receiver.stats();
    assert_eq!(receiver.poll(4).err().ok_or("reversed clock accepted")?,
        H265ReceiveError::Transport(ReorderError::Continuity(ContinuityError::ClockReversed)));
    assert_eq!(receiver.ingest(KEY, &packet(5, true, &[0x62, 1, 0x53, 9]), 6).err()
        .ok_or("conflicting bytes accepted")?, H265ReceiveError::Transport(ReorderError::ConflictingDuplicate));
    assert_eq!(receiver.stats(), before);
...
fn unsupported_don_and_resource_configuration_fail_before_admission() -> TestResult {
    let error = H265Receiver::new(KEY, 96, 1, ReorderLimits::default(), H265Limits::default())
        .err().ok_or("unsupported DON silently admitted")?;
    assert!(matches!(error, H265ReceiveError::Codec(failure) if failure.reason == H265Error::Unsupported));
    let error = H265Receiver::new(KEY, 96, 0,
        ReorderLimits { max_packets: 0, ..ReorderLimits::default() }, H265Limits::default())
        .err().ok_or("unbounded queue configuration admitted")?;
    assert_eq!(error, H265ReceiveError::Transport(ReorderError::Configuration));
    Ok(())
}
