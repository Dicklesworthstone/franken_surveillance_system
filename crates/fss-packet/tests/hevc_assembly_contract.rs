...
use fss_packet::hevc::{
    ...
    HevcAssemblyOutput as Output, HevcAssemblyRefusal, HevcAssemblyStep as Step, HevcBoundary as B,
    HevcPrefixError as S, HevcRetirementReason as R, parse_slice_prefix,
};
...
fn slice(kind: u8, first: bool, pps: u8, tid: u8, no_output: bool) -> Vec<u8> {
    ...
    if (16..=21).contains(&kind) {
        bits.push(u8::from(no_output));
    }
...
fn nal(
    ...
    result
        .nals
        .pop()
        .ok_or_else(|| "fixture did not produce a NAL".into())
}
            ...
            for tid in 1..=7 {
                for first in [false, true] {
                    for no_output in [false, true] {
                        ...
                        assert_eq!(
                            parsed.no_output_of_prior_pics,
                            if kind >= 16 { Some(no_output) } else { None }
                        );
                    }
                }
            }
...
fn prefix_parser_refuses_truncation_corruption_reserved_values_and_other_layers() -> TestResult {
    ...
    assert_eq!(
        parse_slice_prefix(&[3, 1, 0xc0], 1024),
        Err(S::UnsupportedLayer)
    );
    assert_eq!(
        parse_slice_prefix(&[2, 9, 0xc0], 1024),
        Err(S::UnsupportedLayer)
    );
    for kind in (10..=15).chain(22..=63) {
        assert_eq!(
            parse_slice_prefix(&[kind << 1, 1, 0xc0], 1024),
            Err(S::UnsupportedNal)
        );
    }
    for pps in [64, 127, 255] {
        assert_eq!(
            parse_slice_prefix(&slice(1, true, pps, 1, false), 1024),
            Err(S::Malformed)
        );
    }
...
fn rtp_marker_does_not_split_multislice_picture_and_next_first_slice_does() -> TestResult {
    ...
    assert!(
        accepted(a.push(nal(KEY, 1, 100, &first, true)?, 0))?
            .picture
            .is_none()
    );
    assert!(
        accepted(a.push(nal(KEY, 2, 100, &next, true)?, 1))?
            .picture
            .is_none()
    );
    let picture = push(&mut a, 3, 200, &first, 2)?
        .picture
        .ok_or("missing first-slice boundary")?;
...
fn leading_and_suffix_metadata_are_preserved_in_the_correct_group() -> TestResult {
    ...
    let picture = output
        .picture
        .ok_or("prefix failed to delimit previous picture")?;
...
fn aud_and_end_markers_have_validated_trailing_bits_and_explicit_ownership() -> TestResult {
    for (kind, boundary) in [(36, B::EndOfSequence), (37, B::EndOfBitstream)] {
        ...
        assert_eq!(
            standalone
                .standalone
                .ok_or("unattached end marker vanished")?
                .bytes(),
            [kind << 1, 1, 0x80]
        );
        if kind == 37 {
            assert_eq!(
                refused(a.push(nal(KEY, 2, 100, &slice(1, true, 0, 1, false), false)?, 1))?.reason,
                E::Closed
            );
        }
        let mut a = HevcAssembler::new(KEY, Limits::default())?;
        push(&mut a, 2, 100, &slice(1, true, 0, 1, false), 1)?;
        let old = push(&mut a, 3, 200, &[70, 1, 0x50], 2)?
            .picture
            .ok_or("AUD did not delimit")?;
        assert_eq!(old.boundary(), B::AccessUnitDelimiter);
        push(&mut a, 4, 200, &slice(1, true, 0, 1, false), 3)?;
        let ended = push(&mut a, 5, 200, &[kind << 1, 1, 0x80], 4)?
            .picture
            .ok_or("end marker lost picture")?;
    ...
    for invalid in [
        ...
        let mut a = HevcAssembler::new(KEY, Limits::default())?;
        assert_eq!(
            refused(a.push(nal(KEY, 1, 0, &invalid, false)?, 0))?.reason,
            E::Syntax(S::Malformed)
        );
    }
    Ok(())
}
...
fn missing_first_slice_and_suffix_without_picture_never_become_output() -> TestResult {
    for invalid in [
        ...
        assert_eq!(
            failure.retired.ok_or("metadata retirement missing")?.nals,
            1
        );
        assert_eq!(a.pending_bytes(), 0);
    }
...
fn conflicting_slice_identity_timestamp_and_post_suffix_vcl_fail_closed() -> TestResult {
    for (bytes, timestamp, expected) in [
        ...
        assert_eq!(
            refused
                .retired
                .ok_or("inconsistent group was not retired")?
                .reason,
            R::InvalidInput
        );
        assert_eq!(a.pending_bytes(), 0);
    ...
    assert_eq!(
        refused(a.push(nal(KEY, 3, 100, &slice(1, false, 0, 1, false), false)?, 2))?.reason,
        E::Ordering
    );
    Ok(())
}
...
fn expiry_is_fixed_at_first_prefix_and_cannot_be_erased_by_a_late_boundary() -> TestResult {
    ...
    let picture = push(&mut a, 4, 300, &slice(1, true, 0, 1, false), 11)?
        .picture
        .ok_or("new picture missing")?;
    assert!(picture.discontinuity_before());
    let following = push(&mut a, 5, 400, &slice(1, true, 0, 1, false), 12)?
        .picture
        .ok_or("following picture missing")?;
    assert!(!following.discontinuity_before());
    assert_eq!(
        a.expire(22)?.ok_or("silent timeout missing")?.reason,
        R::Deadline
    );
...
fn automatic_sequence_gap_and_owner_declared_loss_invalidate_pending_pictures() -> TestResult {
    for explicit in [false, true] {
        ...
        if explicit {
            assert_eq!(
                a.discontinuity(KEY, 1)?
                    .ok_or("missing discontinuity")?
                    .reason,
                R::InputDiscontinuity
            );
        }
        ...
        if !explicit {
            assert_eq!(
                out.retired.ok_or("implicit gap ignored")?.reason,
                R::InputDiscontinuity
            );
        }
...
fn duplicate_and_reversed_source_spans_refuse_but_adjacent_ap_members_bind() -> TestResult {
    ...
    let mut ap = vec![96, 1];
    for bytes in [&first, &next] {
        ap.extend_from_slice(&(bytes.len() as u16).to_be_bytes());
        ap.extend_from_slice(bytes);
    }
...
fn wrong_owner_and_reversed_clock_leave_pending_bytes_and_source_watermark_unchanged() -> TestResult
{
    ...
    assert_eq!(
        refused(a.push(nal(KEY, 2, 200, &first, false)?, 9))?.reason,
        E::ClockReversed
    );
    assert_eq!(a.pending_bytes(), first.len());
    assert_eq!(
        push(&mut a, 2, 200, &first, 11)?
            .picture
            .ok_or("valid retry failed")?
            .timestamp(),
        100
    );
    Ok(())
...
fn byte_nal_span_and_deadline_limits_retire_before_silently_partial_output() -> TestResult {
    ...
    for limits in [
        ...
        assert_eq!(
            refusal.retired.ok_or("limit retirement missing")?.reason,
            R::Limit
        );
        assert_eq!(a.pending_bytes(), 0);
    }
    let mut a = HevcAssembler::new(KEY, Limits::default())?;
    assert_eq!(
        refused(a.push(nal(KEY, 1, 100, &first, false)?, u64::MAX))?.reason,
        E::Limit
    );
...
fn metadata_only_eof_repeated_aud_and_cancellation_have_exact_retirement() -> TestResult {
    ...
    assert_eq!(
        replaced.retired.ok_or("old empty AU vanished")?.reason,
        R::NoPicture
    );
    let out = a.finish(2)?;
    assert!(out.picture.is_none());
    ...
    assert_eq!(
        refused(a.push(nal(KEY, 3, 300, &slice(1, true, 0, 1, false), false)?, 3))?.reason,
        E::Closed
    );
    let mut a = HevcAssembler::new(KEY, Limits::default())?;
    push(&mut a, 1, 100, &slice(1, true, 0, 1, false), 0)?;
    assert_eq!(
        a.cancel().ok_or("cancel receipt missing")?.reason,
        R::Cancelled
    );
...
fn finished_timestamp_cannot_be_reopened_by_a_repeated_first_slice() -> TestResult {
    ...
    assert_eq!(
        refused(a.push(nal(KEY, 3, 100, &first, false)?, 2))?.reason,
        E::TimestampMismatch
    );
...
fn limits_and_payload_free_debug_are_public_contracts() -> TestResult {
    for limits in [
        ...
        Limits {
            max_bytes: 64 * 1_024 * 1_024 + 1,
            ..Limits::default()
        },
        Limits {
            max_source_spans: 262_145,
            ..Limits::default()
        },
        Limits {
            max_age_ns: 60_000_000_001,
            ..Limits::default()
        },
    ] {
        assert_eq!(
            HevcAssembler::new(KEY, limits).err(),
