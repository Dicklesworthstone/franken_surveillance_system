...
use fss_packet::{H264Depacketizer, H264Limits, H264Mode, NalUnit, PacketLimits, RtpPacket, StreamKey};
use fss_packet::avc::{AvcAssemblyOutput, AvcAssemblyStep, AvcPps, AvcSps, AvcSyntaxLimits, parse_pps, parse_sps};

pub type Error = Box<dyn std::error::Error>;
pub const KEY: StreamKey = StreamKey { ingress: 17, generation: 1, ssrc: 7 };
    ...
    while at + 3 <= bytes.len() {
        ...
        if prefix != 0 {
            if let Some(begin) = start {
                ...
                if begin < end { result.push(&bytes[begin..end]); }
            }
            start = Some(at + prefix); at += prefix;
        } else { at += 1; }
    }
    if let Some(begin) = start { if begin < bytes.len() { result.push(&bytes[begin..]); } }
    result
}
...
pub fn wire(key: StreamKey, sequence: u64, timestamp: u32, marker: bool, payload: &[u8]) -> Vec<u8> {
...
pub fn nal(key: StreamKey, sequence: u64, timestamp: u32, marker: bool, payload: &[u8]) -> Result<NalUnit, Error> {
    let bytes = wire(key, sequence, timestamp, marker, payload);
    let packet = RtpPacket::parse(&bytes, PacketLimits::default())?;
    let mut receiver = H264Depacketizer::new(key, 96, H264Mode::NonInterleaved, H264Limits::default())?;
    receiver.push(key, sequence, packet, 0)?.nals.pop().ok_or_else(|| "no reconstructed NAL".into())
}
...
pub fn slice(first_mb: u32, frame: u32) -> Vec<u8> {
    ...
    for value in [first_mb, 0, 0] {
        ...
        for shift in (0..width).rev() { bits.push((code >> shift) & 1 != 0); }
    }
    for shift in (0..4).rev() { bits.push((frame >> shift) & 1 != 0); }
