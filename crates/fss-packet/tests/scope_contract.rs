...
use fss_packet::{
    ContinuityError, H264Depacketizer, H264Error, H264Limits, H264Mode, NtpTimestamp, PacketLimits,
    RtpPacket, SenderReport, SenderReportClock, SequenceTracker, StreamKey,
};
...
fn identical_wire_ssrc_and_epoch_cannot_cross_logical_ingress() -> TestResult {
    ...
    assert_eq!(
        sequence.observe(other, packet),
        Err(ContinuityError::StreamMismatch)
    );
    assert_eq!(sequence, before);
    let newer_other = StreamKey {
        generation: 2,
        ..other
    };
    assert_eq!(
        sequence.restart(newer_other, 96),
        Err(ContinuityError::StreamMismatch)
    );
    let mut media =
    ...
    let failure = media
        .push(other, 2, packet, 1)
        .err()
        .ok_or("foreign ingress accepted")?;
...
fn unavailable_sender_clock_and_cross_ingress_estimates_are_not_capture_time() -> TestResult {
    ...
    assert_eq!(
        SenderReportClock::new(KEY, 90_000, report, 0, 0, 90_000),
        Err(ContinuityError::NoSenderReport)
    );
    report.ntp.seconds = 1;
    let clock = SenderReportClock::new(KEY, 90_000, report, 0, 0, 90_000)?;
    let other = StreamKey { ingress: 2, ..KEY };
    assert_eq!(
        clock.estimate(other, 0),
        Err(ContinuityError::StreamMismatch)
    );
    Ok(())
}
...
fn contradictory_or_unbounded_limits_refuse_before_allocating() -> TestResult {
    ...
    assert_eq!(
        SequenceTracker::new(zero, 96),
        Err(ContinuityError::Configuration)
    );
    assert!(
        H264Depacketizer::new(zero, 96, H264Mode::NonInterleaved, H264Limits::default()).is_err()
    );
    for limits in [
        H264Limits {
            max_nal_bytes: 0,
...
fn replacement_start_retires_old_chain_without_mixing_payloads() -> TestResult {
    ...
    for (sequence, marker, payload) in [
        ...
        if sequence == 2 {
            let discard = output
                .discarded
                .ok_or("old chain lacked retirement receipt")?;
