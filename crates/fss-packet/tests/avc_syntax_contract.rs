...
use fss_packet::avc::{
    AvcError, AvcSliceType, AvcSyntaxLimits, PocMode, parse_pps, parse_slice_identity, parse_sps,
};
...
fn impossible_crops_and_uncropped_resource_excess_are_refused() {
    let large_crop = SpsFixture {
        crop: Some([80, 0, 0, 0]),
        ..SpsFixture::default()
    };
    assert_eq!(
        parse_sps(&large_crop.nal(), AvcSyntaxLimits::default()),
        Err(AvcError::Malformed)
    );
    let cropped = SpsFixture {
        crop: Some([0, 70, 0, 50]),
        ..SpsFixture::default()
    };
    assert_eq!(
...
fn timing_and_maximal_hrd_are_bounded_and_not_capture_time() -> TestResult {
    ...
    assert_eq!(
        (timing.num_units_in_tick, timing.time_scale),
        (1_001, 60_000)
    );
...
fn zero_timing_and_invalid_emulation_prevention_fail() {
    let zero = SpsFixture {
        timing: Some((0, 60_000)),
        ..SpsFixture::default()
    }
    .nal();
    assert_eq!(
        parse_sps(&zero, AvcSyntaxLimits::default()),
        Err(AvcError::Malformed)
    );
    let escaped = SpsFixture {
        timing: Some((1, 60_000)),
        ..SpsFixture::default()
    }
    .nal();
...
fn every_parameter_set_byte_truncation_fails() -> TestResult {
    ...
    for end in 0..bytes.len() {
        assert!(
            parse_sps(&bytes[..end], limits).is_err(),
            "SPS prefix {end}"
        );
    }
    let sps = parse_sps(&bytes, limits)?;
    let pps = PpsFixture::default().nal();
    for end in 0..pps.len() {
        assert!(
            parse_pps(&pps[..end], &sps, limits).is_err(),
            "PPS prefix {end}"
        );
    }
    Ok(())
...
fn unsupported_profile_sample_formats_and_slice_groups_fail_explicitly() -> TestResult {
    ...
    assert_eq!(
        ...
        Err(AvcError::UnsupportedProfile)
    );
    for (chroma, depth) in [(0, 0), (2, 0), (3, 0), (1, 2)] {
        assert_eq!(
            ...
            Err(AvcError::UnsupportedSampleFormat)
        );
    }
    ...
    assert_eq!(
        ...
        Err(AvcError::UnsupportedSliceGroups)
    );
    Ok(())
}
...
fn same_numeric_sps_id_cannot_rebind_existing_pps() -> TestResult {
    let limits = AvcSyntaxLimits::default();
    let sf = SpsFixture::default();
    let pf = PpsFixture::default();
    ...
    assert_eq!(
        parse_slice_identity(&slice, &b, &pps, limits),
        Err(AvcError::ParameterSetMismatch)
    );
...
fn mismatched_pps_sps_and_slice_ids_fail_closed() -> TestResult {
    ...
    assert_eq!(
        parse_slice_identity(
            &SliceFixture {
                pps: 1,
                ..SliceFixture::default()
            }
            .nal(&sf, &pf),
            &sps,
            &pps,
...
fn reference_zero_distinction_not_nri_rank_changes_picture_identity() -> TestResult {
    ...
    let sf = SpsFixture::default();
    let pf = PpsFixture::default();
    let s = parse_sps(&sf.nal(), l)?;
    let p = parse_pps(&pf.nal(), &s, l)?;
    let parse = |header| {
        parse_slice_identity(
            &SliceFixture {
                header,
...
fn poc_lsb_and_each_delta_field_detect_picture_boundaries() -> TestResult {
    for poc in 0..=2 {
        ...
        let lsb = parse(SliceFixture {
            poc_lsb: 3,
            ..SliceFixture::default()
        })?;
        let d0 = parse(SliceFixture {
            delta: [1, 0],
            ..SliceFixture::default()
        })?;
        let d1 = parse(SliceFixture {
            delta: [0, -1],
            ..SliceFixture::default()
        })?;
...
fn field_and_bottom_field_flags_have_distinct_picture_identities() -> TestResult {
    ...
    let frame = parse(false, false)?;
    let top = parse(true, false)?;
    let bottom = parse(true, true)?;
...
fn redundant_picture_count_is_preserved_without_becoming_primary() -> TestResult {
    ...
    let s = parse_sps(&sf.nal(), l)?;
    let p = parse_pps(&pf.nal(), &s, l)?;
    let parsed = parse_slice_identity(
        &SliceFixture {
            redundant: 7,
            ..SliceFixture::default()
        }
...
fn tighter_use_time_limits_cannot_be_bypassed_by_preparsed_parameters() -> TestResult {
    ...
    for reduced in [
        ...
        AvcSyntaxLimits {
            max_parameter_set_bytes: 2,
            ..l
        },
        AvcSyntaxLimits {
            max_slice_identity_bits: 1,
            ..l
        },
    ] {
        assert_eq!(
            parse_slice_identity(&slice, &s, &p, reduced),
            Err(AvcError::Limit)
        );
    }
    Ok(())
...
fn invalid_idr_partitioned_and_unsupported_slice_types_fail() -> TestResult {
    ...
    for header in [2, 3, 4, 19, 20, 21] {
        assert_eq!(
            parse_slice_identity(&[header, 0xff], &s, &p, l),
            Err(AvcError::UnsupportedPicture)
        );
    }
    for kind in [3, 4, 8, 9] {
        assert_eq!(
    ...
    assert_eq!(
        parse_slice_identity(&invalid_idr.nal(&sf, &pf), &s, &p, l),
        Err(AvcError::Malformed)
    );
    Ok(())
}
...
fn syntax_limits_are_checked_before_any_input_traversal() {
    ...
    for invalid in [
        ...
        AvcSyntaxLimits {
            max_slice_identity_bits: 65_537,
            ..l
        },
    ] {
...
fn metadata_debug_does_not_expose_parameter_set_bytes() -> TestResult {
    let l = AvcSyntaxLimits::default();
    let s = parse_sps(&SpsFixture::default().nal(), l)?;
...
fn real_baseline_fixture_matches_retained_laboratory_expectations() -> TestResult {
    ...
    let s = parse_sps(nals[0], l)?;
    let p = parse_pps(nals[1], &s, l)?;
    assert_eq!(s.display_dimensions(), (160, 128));
    assert_eq!(s.poc_mode(), PocMode::Implicit);
    let pictures: Vec<_> = nals
        .into_iter()
        .filter(|n| matches!(n[0] & 31, 1 | 5))
        .map(|n| parse_slice_identity(n, &s, &p, l))
        .collect::<Result<_, _>>()?;
    assert_eq!(
        pictures.iter().map(|p| p.frame_num()).collect::<Vec<_>>(),
        [0, 1, 2, 0]
    );
    assert_eq!(
        pictures.iter().map(|p| p.idr_pic_id()).collect::<Vec<_>>(),
...
fn real_high_fixture_preserves_cropping_and_b_picture_reorder() -> TestResult {
    ...
    assert_eq!(
        pictures
            .iter()
            .map(|p| p.pic_order_cnt_lsb())
            .collect::<Vec<_>>(),
        [Some(0), Some(6), Some(2), Some(4), Some(10), Some(8)]
    );
    assert_eq!(
        pictures
            .iter()
            .filter(|p| p.slice_type() == AvcSliceType::B)
            .count(),
        3
    );
    assert!(pictures.windows(2).all(|p| p[1].starts_new_picture(p[0])));
