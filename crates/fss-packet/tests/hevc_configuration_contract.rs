...
use fss_packet::hevc::{
    HevcConfiguration as Config, HevcConfigurationError as E, HevcConfigurationLimits as Limits,
};
type TestResult = Result<(), Box<dyn std::error::Error>>;
const VPS8: &str = "40010c01ffff01600000030090000003000003001eba0240";
const SPS8: &str =
    "42010101600000030090000003000003001ea020831d565ba4a4c2f016808000000300800000030284";
const VPS10: &str = "40010c01ffff02200000030090000003000003001eba0240";
const SPS10: &str =
    "42010102200000030090000003000003001ea020831365ba4a4c2f0168080000030008000003002840";
...
impl Bits {
    fn put(&mut self, n: u32, bits: usize) {
        for bit in (0..bits).rev() {
            self.0.push(n & (1 << bit) != 0);
        }
    }
...
fn tuple(
    ...
    vps.put(u32::from(v), 4);
    vps.put(3, 2);
    vps.put(0, 6);
    vps.put(u32::from(layers - 1), 3);
    vps.put(1, 1);
    vps.put(65535, 16);
    profile(&mut vps, layers, sub, depth != 0);
    let mut sps = Bits::default();
    sps.put(u32::from(v), 4);
    sps.put(u32::from(layers - 1), 3);
    sps.put(1, 1);
    ...
    sps.ue(depth);
    sps.ue(depth);
    let mut pps = Bits::default();
    pps.ue(u32::from(p));
    pps.ue(u32::from(s));
    [vps.nal(32), sps.nal(33), pps.nal(34)]
}
fn parse(n: &[Vec<u8>; 3]) -> Result<Config, E> {
    Config::parse(&n[0], &n[1], &n[2], Limits::default())
}

#[test]
fn retained_x265_main_and_main10_metadata_matches_pinned_ffprobe_fixture() -> TestResult {
    for (vps, sps, depth, coded, display) in [
        (VPS8, SPS8, 8, (64, 48), (62, 46)),
        (VPS10, SPS10, 10, (64, 48), (64, 48)),
    ] {
        let n = [hex(vps), hex(sps), hex(PPS)];
        let c = parse(&n)?;
        assert_eq!(c.coded_dimensions(), coded);
        assert_eq!(c.display_dimensions(), display);
        assert_eq!(c.bit_depth(), depth);
        assert_eq!(c.temporal_layers(), 1);
        assert!(c.temporal_nested());
    ...
    for layers in 1..=7 {
        for sub in [false, true] {
            for depth in [0, 2] {
                ...
                let c = parse(&n)?;
                assert_eq!(c.temporal_layers(), layers);
                assert_eq!(c.display_dimensions(), (634, 466));
                assert_eq!(c.bit_depth(), 8 + depth as u8);
                assert_eq!(c.pps_id(), 63);
            }
        }
    }
    Ok(())
...
fn mismatched_ids_temporal_declarations_and_general_profiles_are_not_merged() -> TestResult {
    ...
    for replacement in [
        ...
        assert_eq!(
            Config::parse(&replacement[0], &n[1], &n[2], Limits::default()).err(),
            Some(E::Mismatch)
        );
    }
    let wrong_pps = tuple((3, 6, 5), (640, 480), [0; 4], 1, 0, false);
    assert_eq!(
        Config::parse(&n[0], &n[1], &wrong_pps[2], Limits::default()).err(),
        Some(E::Mismatch)
    );
    Ok(())
}
...
fn dimension_pixel_and_byte_budgets_apply_before_unbounded_work() -> TestResult {
    ...
    for limits in [
        ...
        Limits {
            max_luma_samples: 640 * 480 - 1,
            ..Limits::default()
        },
        Limits {
            max_parameter_bytes: n[0].len() - 1,
            ..Limits::default()
        },
    ] {
        assert_eq!(
            Config::parse(&n[0], &n[1], &n[2], limits).err(),
            Some(E::Limit)
        );
    }
    for limits in [
...
fn invalid_crop_zero_dimensions_and_unsupported_depth_do_not_underflow() -> TestResult {
    for (size, crop) in [
        ...
        assert_eq!(
            parse(&tuple((0, 0, 0), size, crop, 1, 0, false)).err(),
            Some(E::Malformed)
        );
    }
    for depth in [1, 3, 4, 8] {
        assert_eq!(
            parse(&tuple((0, 0, 0), (640, 480), [0; 4], 1, depth, false)).err(),
            Some(E::Unsupported)
        );
    }
    Ok(())
...
fn whole_ebsp_framing_is_checked_even_after_the_interpreted_prefix() -> TestResult {
    ...
    for suffix in [
        ...
        let mut pps = n[2].clone();
        pps.extend_from_slice(suffix);
        assert_eq!(
            Config::parse(&n[0], &n[1], &pps, Limits::default()).err(),
            Some(E::Malformed)
        );
    }
    let mut pps = n[2].clone();
    pps.extend_from_slice(&[0, 0, 3, 0, 0x80]);
...
fn nal_kind_layer_temporal_and_forbidden_bit_checks_are_explicit() -> TestResult {
    ...
    for (index, mask, expected) in [
        ...
        let mut vps = n[0].clone();
        vps[index] ^= mask;
        assert_eq!(
            Config::parse(&vps, &n[1], &n[2], Limits::default()).err(),
            Some(expected)
        );
    }
    for end in 0..10 {
...
fn geometry_grid_keeps_conformance_units_and_original_bytes() -> TestResult {
    for width in [16, 64, 640, 8192] {
        for height in [16, 48, 480, 8192] {
            for crop in 0..4 {
                let n = tuple((15, 15, 63), (width, height), [crop; 4], 1, 0, false);
                let c = parse(&n)?;
                assert_eq!(
                    c.display_dimensions(),
                    (width - 4 * crop, height - 4 * crop)
                );
                assert_eq!(c.sps(), n[1]);
            }
        }
