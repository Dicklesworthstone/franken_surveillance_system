...
use fss_packet::{
    ...
    H264ReceiveError, H264ReceivePoll, H264Receiver, H264Status, OrderedRtpPacket, PacketLimits,
    QueueDiscardReason, ReorderDisposition, ReorderError, ReorderGapReason, ReorderLimits,
    RtpPacket, StreamKey,
};
...
fn codec_deadline_wakes_without_network_and_duplicates_cannot_extend_it() -> TestResult {
    ...
    assert_eq!(
        receiver.poll(101)?,
        H264ReceivePoll::Pending { wake_at_ns: None }
    );
    Ok(())
}
...
fn codec_timeout_precedes_later_transport_deadline_without_consuming_queued_source() -> TestResult {
    ...
    assert!(matches!(
        receiver.poll(6)?,
        H264ReceivePoll::FragmentDiscarded(_)
    ));
...
fn eof_drains_complete_queued_fu_before_finalizing_codec() -> TestResult {
    ...
    receiver.finish();
    assert_eq!(
        delivered(&mut receiver, 2, 11)?.1?.status,
...
fn eof_reports_unfinished_fragment_once_without_fabricating_complete_output() -> TestResult {
    ...
    assert_eq!(
        discarded.ok_or("expected retirement")?.reason,
...
fn cancellation_accounts_for_both_queues_and_is_idempotent() -> TestResult {
    ...
    assert_eq!(
        receipt.fragment.ok_or("expected fragment receipt")?.reason,
        H264Error::Cancelled
    );
    assert_eq!(receiver.queued_bytes(), 0);
    assert_eq!(receiver.pending_nal_bytes(), 0);
...
fn restart_retires_both_layers_and_cannot_reuse_old_generation() -> TestResult {
    ...
    assert_eq!(
        restart
            .transport
            .discarded
            .ok_or("expected queue receipt")?
            .packets,
        1
    );
    assert_eq!(
        restart.discarded.ok_or("expected codec receipt")?.reason,
        H264Error::Gap
    );
    assert_eq!(receiver.next_wake_ns(), None);
    assert_eq!(
    ...
    assert_eq!(
        receiver
            .restart(KEY, 96, H264Mode::NonInterleaved, limits(), codec_limits())
            .err(),
    ...
    let mut fresh = receiver.restart(
        newer,
        96,
...
fn shared_clock_guards_ingest_and_poll_without_consuming_queued_data() -> TestResult {
    ...
    let clock_error =
        H264ReceiveError::Transport(ReorderError::Continuity(ContinuityError::ClockReversed));
    assert_eq!(receiver.poll(4).err(), Some(clock_error.clone()));
    assert_eq!(
        receiver.ingest(KEY, &wire(12, true, &[0x65]), 4).err(),
        Some(clock_error)
    );
...
fn no_traffic_and_eof_during_probation_do_not_create_a_nal_or_gap() -> TestResult {
    let mut receiver =
        H264Receiver::new(KEY, 96, H264Mode::NonInterleaved, limits(), codec_limits())?;
    assert_eq!(receiver.next_wake_ns(), None);
    assert_eq!(
        receiver.poll(0)?,
        H264ReceivePoll::Pending { wake_at_ns: None }
...
fn standalone_fragment_timer_and_gap_hooks_do_not_change_packet_admission() -> TestResult {
    let mut depacketizer =
        H264Depacketizer::new(KEY, 96, H264Mode::NonInterleaved, codec_limits())?;
    let start = wire(11, false, &[0x7c, 0x85, 1]);
    depacketizer.push(
        KEY,
        11,
        RtpPacket::parse(&start, PacketLimits::default())?,
        1,
    )?;
    ...
    let output = depacketizer.push(
        KEY,
        13,
        RtpPacket::parse(&next, PacketLimits::default())?,
        2,
    )?;
    assert!(output.gap_before);
...
fn unrepresentable_fu_deadline_is_refused_but_boundary_deadline_expires() -> TestResult {
    ...
    assert_eq!(
        boundary.expire(u64::MAX)?.ok_or("expected expiry")?.reason,
        H264Error::Deadline
    );
    Ok(())
}
