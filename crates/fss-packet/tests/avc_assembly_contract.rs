...
fn aud_less_multi_slice_group_is_not_split_by_first_macroblock_zero() -> TestResult {
    ...
    assert!(
        accept(a.push(nal(KEY, 1, 90, false, &slice(20, 1))?, 0))?
            .picture
            .is_none()
    );
    assert!(
        accept(a.push(nal(KEY, 2, 90, false, &slice(0, 1))?, 1))?
            .picture
            .is_none()
    );
...
fn one_push_can_close_previous_picture_and_queue_marked_next_picture() -> TestResult {
    ...
    let last = accept(a.push(refused.nal, 2))?
        .picture
        .ok_or("retry picture")?;
...
fn false_marker_cannot_create_duplicate_picture_groups() -> TestResult {
    ...
    match a.push(nal(KEY, 2, 90, true, &slice(20, 1))?, 1) {
        AvcAssemblyStep::Refused(r) => {
            assert_eq!(r.reason, AvcAssemblyError::PictureAlreadyEmitted)
        }
...
fn wrong_epoch_and_reversed_time_cannot_retire_another_pending_picture() -> TestResult {
    ...
    for (key, now, expected) in [
        (other, 101, AvcAssemblyError::StreamMismatch),
        (KEY, 99, AvcAssemblyError::ClockReversed),
    ] {
    ...
    assert_eq!(
        a.discontinuity(other, 101),
        Err(AvcAssemblyError::StreamMismatch)
    );
    assert_eq!(a.pending_bytes(), retained);
    Ok(())
}
...
fn overlapping_source_spans_are_refused_without_extending_deadline() -> TestResult {
    let mut a = assembler(AvcAssemblyLimits {
        max_age_ns: 10,
        ..AvcAssemblyLimits::default()
    })?;
    accept(a.push(nal(KEY, 1, 90, false, &slice(0, 1))?, 0))?;
    ...
    assert!(matches!(
        a.poll(10)?,
        AvcAssemblyPoll::Pending { wake_at_ns: None }
    ));
    Ok(())
}
...
fn distinct_stap_nal_spans_in_one_datagram_are_admitted_in_order() -> TestResult {
    ...
    let reconstructed = d.push(
        KEY,
        1,
        RtpPacket::parse(&bytes, PacketLimits::default())?,
        0,
    )?;
    let mut a = AvcAssembler::new(
        KEY,
        sps,
        pps,
        AvcSyntaxLimits::default(),
...
fn exact_parameter_change_fences_epoch_and_requires_explicit_restart() -> TestResult {
    ...
    match a.push(nal(KEY, 2, 100, false, &changed)?, 1) {
        AvcAssemblyStep::Refused(r) => {
            ...
            assert_eq!(
                r.retired.ok_or("missing old-generation retirement")?.reason,
    ...
    assert!(matches!(
        a.push(nal(KEY, 3, 100, true, &slice(0, 2))?, 2),
        AvcAssemblyStep::Refused(_)
    ));
    assert!(matches!(
        a.restart(KEY, new_sps.clone(), new_pps.clone()),
        Err(AvcAssemblyError::GenerationRequired)
    ));
    let next_key = StreamKey {
        generation: 2,
        ..KEY
    };
    let (mut next, old) = a.restart(next_key, new_sps, new_pps)?;
    assert!(old.is_none());
    let p = accept(next.push(nal(next_key, 1, 100, true, &slice(0, 2))?, 0))?
        .picture
        .ok_or("new epoch")?;
...
fn repeated_identical_parameter_bytes_do_not_change_configuration() -> TestResult {
    ...
    let p = accept(a.push(nal(KEY, 3, 90, true, &slice(0, 1))?, 2))?
        .picture
        .ok_or("picture")?;
...
fn gaps_retire_partial_group_and_preserve_discontinuity_on_recovery() -> TestResult {
    ...
    let p = accept(a.push(nal(KEY, 4, 90, true, &slice(20, 1))?, 2))?
        .picture
        .ok_or("recovered partial group")?;
...
fn metadata_prefix_after_vcl_ends_previous_group_without_fabricating_a_frame() -> TestResult {
    ...
    assert_eq!(
        output.picture.ok_or("previous group")?.boundary(),
        AvcBoundary::NextAccessUnitPrefix
    );
    let end = a.finish(2)?;
    assert!(end.picture.is_none());
    assert_eq!(
        end.retired.ok_or("metadata-only retirement")?.reason,
        AvcRetirementReason::NoPrimaryPicture
    );
    Ok(())
}
...
fn malformed_codec_input_returns_its_original_nal_and_retires_pending_group() -> TestResult {
    ...
    match a.push(nal(KEY, 2, 90, true, &[0x41, 0])?, 1) {
        AvcAssemblyStep::Refused(r) => {
            ...
            assert_eq!(
                r.retired.ok_or("old grouping retirement")?.reason,
...
fn nal_and_byte_capacity_retire_derivative_without_losing_rejected_source() -> TestResult {
    for limits in [
        ...
        assert_eq!(a.pending_bytes(), 0);
        let recovered = accept(a.push(nal(KEY, 3, 100, true, &slice(0, 2))?, 2))?
            .picture
            .ok_or("recovered group")?;
...
fn cancellation_and_restart_have_exactly_once_retirement() -> TestResult {
    ...
    accept(a.push(nal(KEY, 1, 90, false, &slice(0, 1))?, 0))?;
    assert!(matches!(
        a.restart(KEY, sps.clone(), pps.clone()),
        Err(AvcAssemblyError::GenerationRequired)
    ));
    assert_eq!(a.pending_nals(), 1);
    let (mut next, old) = a.restart(
        StreamKey {
            generation: 2,
            ..KEY
        },
    ...
    assert_eq!(
        old.ok_or("restart retirement")?.reason,
        AvcRetirementReason::Restarted
    );
    ...
    assert_eq!(
        next.cancel().ok_or("cancel retirement")?.reason,
        AvcRetirementReason::Cancelled
    );
...
fn explicit_stream_end_is_not_relabelled_as_an_unverified_tail() -> TestResult {
    let mut a = assembler(AvcAssemblyLimits::default())?;
    accept(a.push(nal(KEY, 1, 90, false, &slice(0, 1))?, 0))?;
    let p = accept(a.push(nal(KEY, 2, 90, false, &[11, 0x80])?, 1))?
        .picture
        .ok_or("stream-end group")?;
...
fn receiver_fixture(bytes: &[u8], times: &[u32], dimensions: (u32, u32)) -> TestResult {
    ...
    let sps = parse_sps(nals[0], l)?;
    let pps = parse_pps(nals[1], &sps, l)?;
            ...
            match receiver.poll(now)? {
                H264ReceivePoll::Packet {
                    ...
                    for nal in output.nals {
                        ...
                        while let AvcAssemblyPoll::Picture(picture) = assembler.poll(now)? {
                            groups.push(picture);
                        }
                    }
                }
    ...
    assert!(matches!(
        receiver.poll(100)?,
        H264ReceivePoll::Ended { discarded: None }
    ));
    if let Some(picture) = assembler.finish(100)?.picture {
        groups.push(picture);
    }
    assert_eq!(groups.len(), times.len());
    assert_eq!(
        groups.iter().map(|g| g.timestamp()).collect::<Vec<_>>(),
        times
    );
    assert!(
        groups
            .iter()
            .all(|g| g.sps().display_dimensions() == dimensions)
    );
    assert!(
        groups
            .iter()
            .all(|g| g.saw_first_macroblock() && !g.discontinuity_before())
    );
    assert_eq!(
        groups.last().ok_or("no pictures")?.boundary(),
