...
use fss_packet::avc::{
    AvcAssemblyError, AvcAssemblyStep, AvcBoundary, AvcPictureGroup,
    AvcReceiveError, AvcReceiveLimits, AvcReceivePoll, AvcReceiver,
    AvcRetirementReason, parse_pps, parse_sps,
};
...
fn receiver(limits: AvcReceiveLimits) -> Result<AvcReceiver, Error> {
    ...
    assert_eq!(admission.transport.transport.disposition, ReorderDisposition::NotAdmitted);
    Ok(receiver)
}
...
fn drain_clean(r: &mut AvcReceiver, now: u64) -> Result<Vec<AvcPictureGroup>, Error> {
    ...
    for _ in 0..1_024 {
        match r.poll(now)? {
            AvcReceivePoll::Source { picture, fragment, gap_before, .. } => {
            ...
            AvcReceivePoll::Assembly(AvcAssemblyStep::Accepted(output)) => {
                ...
                if let Some(picture) = output.picture { pictures.push(picture); }
            }
...
fn real_source_is_exposed_before_owned_picture_admission() -> TestResult {
    ...
    match r.poll(1)? {
        AvcReceivePoll::Source { source: saved, queued_nals, .. } => {
...
fn gap_automatically_retires_picture_and_fragment_before_later_nals() -> TestResult {
    let mut r = receiver(limits())?; pending_picture(&mut r)?;
    ...
    assert!(matches!(r.poll(3)?, AvcReceivePoll::Pending { wake_at_ns: Some(13) }));
    match r.poll(13)? {
        AvcReceivePoll::Gap { fragment, picture, .. } => {
            assert_eq!(fragment.ok_or("incomplete FU receipt")?.reason, H264Error::Gap);
            assert_eq!(picture.ok_or("picture receipt")?.reason, AvcRetirementReason::InputDiscontinuity);
        }
        _ => return Err("loss was not automatically propagated".into()),
    }
    assert!(matches!(r.poll(13)?, AvcReceivePoll::CodecRefused { picture: None, .. }));
...
fn malformed_stap_keeps_original_and_invalidates_pending_picture() -> TestResult {
    ...
    match r.poll(2)? {
        AvcReceivePoll::CodecRefused { source: saved, picture, .. } => {
            assert_eq!(saved.bytes(), source);
...
fn fragment_timeout_automatically_invalidates_picture_without_new_packets() -> TestResult {
    ...
    assert_eq!(r.retained_nal_bytes(), 0);
    assert!(matches!(r.poll(32)?, AvcReceivePoll::Pending { wake_at_ns: None }));
    Ok(())
}

#[test]
fn picture_deadline_precedes_later_complete_nal_admission() -> TestResult {
    let mut policy = limits(); policy.assembly.max_age_ns = 10;
    let mut r = receiver(policy)?; pending_picture(&mut r)?;
    r.ingest(KEY, &wire(KEY, 2, 100, false, &slice(0, 2)), 2)?;
    assert!(matches!(r.poll(2)?, AvcReceivePoll::Source { queued_nals: 1, .. }));
    match r.poll(11)? {
        AvcReceivePoll::PictureRetired(retired) => assert_eq!(retired.reason, AvcRetirementReason::Deadline),
    ...
    r.finish();
    match r.poll(11)? {
        AvcReceivePoll::Ended { tail: Some(tail), .. } => {
...
fn eof_with_incomplete_fu_retires_picture_instead_of_publishing_tail() -> TestResult {
    let mut r = receiver(limits())?; pending_picture(&mut r)?;
    r.ingest(KEY, &wire(KEY, 2, 90, false, &fragment_start()), 2)?;
    drain_clean(&mut r, 2)?; r.finish();
    assert_eq!(r.next_wake_ns(), Some(2));
    match r.poll(3)? {
        AvcReceivePoll::Ended { fragment, interrupted_picture, tail: Some(tail) } => {
            assert_eq!(fragment.ok_or("EOF fragment")?.reason, H264Error::EndOfInput);
    ...
    assert!(matches!(r.poll(3)?, AvcReceivePoll::Ended { fragment: None, interrupted_picture: None, tail: None }));
...
fn cancellation_accounts_for_packet_queue_complete_nal_queue_and_picture() -> TestResult {
    ...
    assert_eq!(receipt.picture.ok_or("pending picture cancellation")?.nals, 1);
    ...
    assert!(r.ingest(KEY, &wire(KEY, 4, 110, true, &slice(0, 3)), 4).is_err());
    Ok(())
}
...
fn confirmed_source_restart_retires_every_old_epoch_layer() -> TestResult {
    ...
    assert_eq!(confirmed.transport.transport.disposition, ReorderDisposition::RestartRequired);
    assert_eq!(confirmed.queued_nals.ok_or("restart NAL queue")?.nals, 1);
    assert_eq!(confirmed.picture.ok_or("restart picture")?.nals, 1);
    assert_eq!(r.retained_nal_bytes(), 0);
    assert!(matches!(r.poll(5)?, AvcReceivePoll::Ended { tail: None, .. }));
    Ok(())
}
...
fn refused_restart_or_clock_change_preserves_queued_and_pending_work() -> TestResult {
    let mut r = receiver(limits())?; pending_picture(&mut r)?;
    let retained = r.retained_nal_bytes(); let stats = r.stats();
    assert!(matches!(r.restart(KEY, 96, H264Mode::NonInterleaved, limits(), parameters()?),
        Err(AvcReceiveError::Assembly(AvcAssemblyError::GenerationRequired))));
    assert!(matches!(r.poll(0), Err(AvcReceiveError::Assembly(AvcAssemblyError::ClockReversed))));
    assert!(r.ingest(StreamKey { ingress: 99, ..KEY }, &wire(KEY, 2, 90, false, &slice(20, 1)), 2).is_err());
    assert_eq!(r.retained_nal_bytes(), retained); assert_eq!(r.stats(), stats);
    let key = StreamKey { generation: 2, ..KEY };
    let (mut next, receipt) = r.restart(key, 96, H264Mode::NonInterleaved, limits(), parameters()?)?;
...
fn exact_configuration_change_stops_transport_admission_without_laundering_picture() -> TestResult {
    let mut r = receiver(limits())?; pending_picture(&mut r)?;
    let (sps, _) = parameters()?; let mut changed = sps.nal_bytes().to_vec();
    ...
    assert!(r.ingest(KEY, &wire(KEY, 3, 100, true, &slice(0, 2)), 3).is_err());
...
fn transport_backpressure_does_not_consume_sequence_or_picture() -> TestResult {
    let mut policy = limits(); policy.reorder.max_packets = 1;
    let mut r = receiver(policy)?;
    let one = wire(KEY, 1, 90, false, &slice(0, 1));
    let two = wire(KEY, 2, 90, true, &slice(20, 1));
    r.ingest(KEY, &one, 1)?; let stats = r.stats();
    assert!(r.ingest(KEY, &two, 2).is_err());
    assert_eq!(r.stats(), stats);
    drain_clean(&mut r, 2)?;
    let retry = r.ingest(KEY, &two, 2)?;
    assert_eq!(retry.transport.transport.disposition, ReorderDisposition::Buffered);
    let pictures = drain_clean(&mut r, 2)?;
    assert_eq!(pictures.len(), 1); assert_eq!(pictures[0].nals().len(), 2);
    assert!(!pictures[0].discontinuity_before());
    Ok(())
}
...
fn syntax_refusal_returns_owned_nal_before_clean_recovery() -> TestResult {
    ...
    match r.poll(2)? {
        AvcReceivePoll::Assembly(AvcAssemblyStep::Refused(refusal)) => {
            ...
            assert_eq!(refusal.retired.ok_or("malformed prefix retirement")?.nals, 1);
        }
...
fn debug_never_prints_queued_media() -> TestResult {
    ...
    let mut sei = vec![6]; sei.extend_from_slice(secret);
...
fn fixture(bytes: &[u8], times: &[u32], dimensions: (u32, u32)) -> TestResult {
    let nals = split(bytes); let policy = AvcReceiveLimits::default();
    let sps = parse_sps(nals[0], policy.syntax)?; let pps = parse_pps(nals[1], &sps, policy.syntax)?;
    let mut r = AvcReceiver::new(KEY, 96, H264Mode::NonInterleaved, policy, (sps, pps))?;
    r.ingest(KEY, &wire(KEY, 0, times[0], false, nals[0]), 0)?;
    let mut pictures = Vec::new(); let mut frame = 0; let mut sources = 0;
    for (index, nal) in nals.iter().enumerate() {
        ...
        match r.poll(now)? {
            AvcReceivePoll::Source { source: saved, picture, fragment, .. } => {
                assert_eq!(saved.bytes(), source);
                assert!(picture.is_none() && fragment.is_none()); sources += 1;
            }
    ...
    r.finish();
    match r.poll(100)? {
        AvcReceivePoll::Ended { fragment: None, interrupted_picture: None, tail: Some(tail) } => {
            assert!(tail.retired.is_none());
            if let Some(picture) = tail.picture { pictures.push(picture); }
        }
        _ => return Err("clean EOF did not produce classified tail".into()),
    }
    assert_eq!(sources, nals.len()); assert_eq!(pictures.len(), times.len());
    assert_eq!(pictures.iter().map(|p| p.timestamp()).collect::<Vec<_>>(), times);
    assert!(pictures.iter().all(|p| p.sps().display_dimensions() == dimensions));
    assert!(pictures.iter().all(|p| p.saw_first_macroblock() && !p.discontinuity_before()));
    assert_eq!(pictures.last().ok_or("no final picture")?.boundary(), AvcBoundary::EndOfInputUnverified);
    assert_eq!(r.retained_nal_bytes(), 0);
    Ok(())
}
...
fn real_baseline_fixture_reaches_picture_groups_through_one_receiver() -> TestResult {
    fixture(include_bytes!("fixtures/avc/baseline.264"), &[90_000, 93_600, 97_200, 100_800], (160, 128))
}

#[test]
fn real_high_b_picture_fixture_preserves_nonmonotonic_media_timestamps() -> TestResult {
    fixture(include_bytes!("fixtures/avc/high_cropped.264"), &[90_000, 100_800, 93_600, 97_200, 108_000, 104_400], (64, 36))
}
