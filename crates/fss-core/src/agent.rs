...
///
/// All fields are private; callers construct cells via [`KnowledgeCell::new`] and inspect them
/// via accessor methods. Direct field reassignment after construction is prevented:
///
/// ```compile_fail
/// use fss_core::{KnowledgeCell, KnowledgeCellParams, KnowledgeState, ProvenanceClass};
///
/// let mut cell = KnowledgeCell::new(KnowledgeCellParams {
///     claim_id: "claim:test:private".to_string(),
///     statement: "Testing private fields".to_string(),
...
///     state_basis: None,
/// }).unwrap();
/// cell.provenance = ProvenanceClass::Observed;
/// ```
...
pub struct KnowledgeCell {
    ...
    /// Validity end, when bounded.
    valid_until: Option<TimestampNs>,
    /// Typed basis required by states whose registry meaning names one; `None` otherwise.
    state_basis: Option<KnowledgeStateBasis>,
}
...
impl KnowledgeCell {
    ...
    /// Constructs an unvalidated [`KnowledgeCell`] for internal tests and defense-in-depth boundaries.
    pub(crate) fn new_unvalidated(params: KnowledgeCellParams) -> Self {
        Self {
            claim_id: params.claim_id,
            statement: params.statement,
            knowledge_state: params.knowledge_state,
    ...
    /// Converts this cell back into its constituent parameters.
    #[must_use]
    pub fn to_params(&self) -> KnowledgeCellParams {
        KnowledgeCellParams {
...
mod tests {
    ...
    fn known_cell() -> Result<KnowledgeCell, ContractError> {
    ...
    fn basisless_stale_cell() -> KnowledgeCell {
        KnowledgeCell::new_unvalidated(KnowledgeCellParams {
    ...
    fn test_capsule_with_cell(cell: KnowledgeCell) -> Result<SituationCapsule, ContractError> {
        ...
        Ok(SituationCapsule {
            ...
            revision: 1,
            contract_basis: ContractBasis::genesis("genesis"),
            ...
            mission_state: None,
            context_compression_receipt: None,
        })
    }

    #[test]
    fn invalid_unvalidated_cells_refused_at_cell_and_capsule_level() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(pred_known.validate(), Err(ContractError::PredictedKnownForbidden));
        assert!(!pred_known.is_irreversible_effect_premise(now));
        let cap_pred_known = test_capsule_with_cell(pred_known)?;
        assert_eq!(cap_pred_known.validate(), Err(ContractError::PredictedKnownForbidden));
        assert_eq!(cap_pred_known.decision_fingerprint(), Err(ContractError::PredictedKnownForbidden));

        // 2. Basisless indeterminate
        let basisless_indeterminate = KnowledgeCell::new_unvalidated_for_test(KnowledgeCellParams {
        ...
        assert_eq!(basisless_indeterminate.validate(), Err(ContractError::ReconciliationBasisRequired));
        ...
        assert_eq!(cap_indet.validate(), Err(ContractError::ReconciliationBasisRequired));
        assert_eq!(cap_indet.decision_fingerprint(), Err(ContractError::ReconciliationBasisRequired));
        ...
        assert_eq!(basisless_stale.validate(), Err(ContractError::StaleBasisRequired));
        ...
        assert_eq!(cap_stale.decision_fingerprint(), Err(ContractError::StaleBasisRequired));
        ...
        assert_eq!(basisless_redacted.validate(), Err(ContractError::RedactionMarkerRequired));
        ...
        assert_eq!(cap_redacted.validate(), Err(ContractError::RedactionMarkerRequired));
        assert_eq!(cap_redacted.decision_fingerprint(), Err(ContractError::RedactionMarkerRequired));

        // 5. Misattached redaction on Known
        let secret_misattached = "SECRET-misattached-redaction";
        let marker = RedactionMarker {
            replacement_statement: REDACTED_STATEMENT_MARKER.to_string(),
            policy_reference: "fss-privacy-policy-v1".to_string(),
            redacted_at: TimestampNs(1_000_000_000),
            reason: "operator_confidentiality".to_string(),
        };
        ...
        assert_eq!(misattached_known.validate(), Err(ContractError::KnowledgeStateBasisMismatch));
        assert!(misattached_known.withholds_statement());
        assert_eq!(misattached_known.disclosable_statement(), REDACTED_STATEMENT_MARKER);
        assert!(!format!("{misattached_known:?}").contains(secret_misattached));
        assert!(!format!("{misattached_known:#?}").contains(secret_misattached));
        let cap_misattached_known = test_capsule_with_cell(misattached_known)?;
        assert_eq!(cap_misattached_known.validate(), Err(ContractError::KnowledgeStateBasisMismatch));
        assert_eq!(cap_misattached_known.decision_fingerprint(), Err(ContractError::KnowledgeStateBasisMismatch));
        ...
        assert_eq!(misattached_stale.validate(), Err(ContractError::StaleBasisRequired));
        assert!(misattached_stale.withholds_statement());
        assert_eq!(misattached_stale.disclosable_statement(), REDACTED_STATEMENT_MARKER);
        ...
        assert_eq!(cap_misattached_stale.validate(), Err(ContractError::StaleBasisRequired));
        assert_eq!(cap_misattached_stale.decision_fingerprint(), Err(ContractError::StaleBasisRequired));
        ...
        assert_eq!(misattached_known.cell_digest(), other_misattached.cell_digest());
