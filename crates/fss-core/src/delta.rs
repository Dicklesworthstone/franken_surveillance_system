...
impl MeaningfulDelta {
    ...
    pub fn coalesce(
        ...
        // the earlier cell is the claim's previous version: coalescing never folds a relabel that
        // launders its evidence into the final cell state (PROV-001, PROV-002; fss-2nwxm).
    ...
    pub fn validate_removals_against(
        ...
        for claim in &self.removed_claim_ids {
            if !basis.iter().any(|cell| cell.claim_id() == claim.as_str()) {
                return Err(ContractError::NotFound);
            }
            if result.iter().any(|cell| cell.claim_id() == claim.as_str()) {
...
fn validate_removed_claims(
    ...
    if !strictly_sorted
        || removed.iter().any(|claim| {
            claim.is_empty() || changed.iter().any(|cell| cell.claim_id() == claim.as_str())
        })
    {
