...
/// opened, lock-held durable authority handle rather than an arbitrary in-memory construction (INV-063).
///
/// # Threat Model
...
/// use fss_core::ReferenceLedger;
///
/// let rewound = ReferenceLedger::replay("site:us-east:primary", vec![]).unwrap();
...
/// ```
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct AuthoritativeLedger {
    anchor: LedgerAnchor,
}

impl AuthoritativeLedger {
    ...
    /// it trusts the caller to hold the writer lock and the on-disk head; and calling it from anywhere
    /// else is a bug.
    ///
    ...
    /// to forge authority.
    #[cfg(test)]
    pub(crate) fn for_test(ledger: &crate::ReferenceLedger) -> Result<Self, ContractError> {
        Self::from_authority(ledger.current().anchor.clone())
    }
}

impl sealed::Sealed for AuthoritativeLedger {}

impl CurrentAnchorSource for AuthoritativeLedger {
    fn current_anchor(&self) -> &LedgerAnchor {
        &self.anchor
    }
}
...
impl AuthorityAnchor {
    ...
    pub fn from_committed_head(ledger: &AuthoritativeLedger) -> Result<Self, ContractError> {
