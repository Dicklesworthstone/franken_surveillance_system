...
use super::{
    ...
    encode_optional_text, encode_text_set, valid_text, Completeness, HandleAvailability,
    HydrationArtifact, HydrationError, HydrationLevel, SemanticHandle, MAX_ARTIFACT_BYTES,
    MAX_REQUEST_SET_ITEMS,
};
use crate::canonical::{
...
impl RedactedRegion {
    ...
    pub fn new(x: u32, y: u32, width: u32, height: u32, method: impl Into<String>) -> Result<Self, ContractError> {
...
impl KeyframeArtifact {
    ...
    pub fn validate(&self) -> Result<(), ContractError> {
        if !valid_text(&self.stream_id) || self.width == 0 || self.height == 0 || !valid_text(&self.format) {
...
impl CanonicalDecode for KeyframeArtifact {
    fn decode_canonical(decoder: &mut CanonicalDecoder<'_>) -> Result<Self, ContractError> {
        ...
        let region_count = usize::try_from(decoder.u64()?).map_err(|_| ContractError::InvalidDigest)?;
...
impl CanonicalDecode for DecisionArtifactKind {
    fn decode_canonical(decoder: &mut CanonicalDecoder<'_>) -> Result<Self, ContractError> {
        ...
        match tag {
            ...
            3 => Ok(Self::Trajectory(TrajectoryArtifact::decode_canonical(decoder)?)),
            4 => Ok(Self::GraphNeighborhood(GraphNeighborhoodArtifact::decode_canonical(decoder)?)),
            5 => Ok(Self::AudioFeatures(AudioFeaturesArtifact::decode_canonical(decoder)?)),
...
impl H2DecisionArtifact {
    ...
    fn encode_body(&self, encoder: &mut CanonicalEncoder) {
        ...
        encoder.u8(match self.completeness {
            ...
            Completeness::CertifiedOmission => 3,
            Completeness::Degraded => 4,
            Completeness::Unknown => 5,
            Completeness::NotObservable => 6,
            Completeness::Unauthorized => 7,
            Completeness::Stale => 8,
        });
...
impl CanonicalDecode for H2DecisionArtifact {
    fn decode_canonical(decoder: &mut CanonicalDecoder<'_>) -> Result<Self, ContractError> {
        ...
        let root_count = usize::try_from(decoder.u64()?).map_err(|_| ContractError::InvalidDigest)?;
        ...
        let completeness = match decoder.u8()? {
            ...
            2 => Completeness::Partial,
            3 => Completeness::CertifiedOmission,
            4 => Completeness::Degraded,
            5 => Completeness::Unknown,
            6 => Completeness::NotObservable,
            7 => Completeness::Unauthorized,
            8 => Completeness::Stale,
