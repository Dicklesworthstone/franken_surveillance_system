...
use super::{
    decode_text_set, encode_text_set, HydrationError, HydrationLevel, SemanticHandle,
};
...
use crate::contract::{Completeness, ContractError, KnowledgeState, ProvenanceClass};
use crate::sensor_capsule::OmissionReason;
use crate::{
    BudgetVector, ContentDigest, ContractBasis, LedgerAnchor, TimestampNs, WorldFact,
};
...
impl SynopsisQuality {
    ...
    pub fn validate(&self) -> Result<(), HydrationError> {
        if let Some(digest) = self.calibration_digest {
            if digest.bytes().iter().all(|&b| b == 0) {
...
impl H1SemanticSynopsis {
    ...
    pub fn from_semantic_handle(
        ...
        let classification = content
            .classification
            .unwrap_or(SynopsisClassification::SemanticSynopsis);
    ...
    pub fn to_knowledge_cells(&self) -> Vec<KnowledgeCell> {
        self.facts
            ...
            .map(|fact| KnowledgeCell {
                ...
                evidence: vec![fact.evidence_digest, self.anchor.state_root],
...
impl CanonicalDecode for H1SemanticSynopsis {
    fn decode_canonical(decoder: &mut CanonicalDecoder<'_>) -> Result<Self, ContractError> {
        ...
        let estimated_cost = BudgetVector::decode_canonical(decoder)?;
        ...
        for _ in 0..facts_count {
            ...
            if let Some(ref prev) = prev_fact_id {
                if fact.fact_id <= *prev {
        ...
        for _ in 0..contra_count {
            ...
            if let Some(ref prev) = prev_contra_id {
                if contra.contradiction_id() <= prev.as_str() {
                    return Err(ContractError::NonCanonicalOrdering);
                }
            }
