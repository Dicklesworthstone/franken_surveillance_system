...
use crate::{
    AttrValue, GraphValidator, ModelIrError, ModelIrGraph, ProducerId, TensorPort,
    infer_operator_outputs,
};
use fss_core::{CanonicalEncoder, ContentDigest};
use fss_tensor::{MAX_STORAGE_BYTES, TensorError};
...
impl Default for MemoryPlanLimits {
    fn default() -> Self {
        Self {
            max_nodes: MAX_MEMORY_PLAN_NODES,
            max_values: MAX_MEMORY_PLAN_VALUES,
...
impl TensorLifetime {
    ...
    pub fn release_after(&self) -> Option<usize> {
        self.release_after
    }
}
...
impl MemoryStep {
    ...
    pub fn release_after(&self) -> &[String] {
        &self.release_after
    }
...
impl MemoryPlan {
    ...
    pub fn compile_cancellable(
        ...
        for port in graph.inputs() {
            producers.insert(port.name(), ProducerId::GraphInput);
        }
        for node in graph.nodes() {
            for name in node.outputs() {
                producers.insert(name.as_str(), ProducerId::Node(node.id()));
            }
        }
        ...
        for (position, node) in order.iter().enumerate() {
            checkpoint(&mut cancelled)?;
            for name in node.outputs() {
                last_use.entry(name.as_str()).or_insert(position);
            }
            for name in node.inputs() {
                last_use.insert(name.as_str(), position);
            }
        }
        ...
        for (position, node) in order.iter().enumerate() {
            ...
            let input_ports = node
                ...
                .map(|name| {
                    values
                        .get(name)
                        .map(|v| &v.port)
                        .ok_or(MemoryPlanError::Inconsistent)
                })
                .collect::<Result<Vec<_>, _>>()?;
            let outputs = infer_operator_outputs(
                node.id(),
                node.op(),
                &input_ports,
                node.outputs(),
            ...
            for port in outputs {
                ...
                let release_after = if pinned.contains(port.name()) {
                    None
                } else {
                    Some(
                        *last_use
        ...
        steps
            ...
            .map_err(|_| MemoryPlanError::Limit("allocation"))?;
        ...
        for (position, node) in order.iter().enumerate() {
            ...
            for name in node.outputs() {
                resident = add(
                    resident,
                    values.get(name).ok_or(MemoryPlanError::Inconsistent)?.bytes,
                )?;
            }
            ...
            for name in &releases[position] {
                resident = resident
                    .checked_sub(values.get(name).ok_or(MemoryPlanError::Inconsistent)?.bytes)
                    .ok_or(MemoryPlanError::Inconsistent)?;
            }
            steps.push(MemoryStep {
                node_id: node.id().to_owned(),
                produced: node.outputs().to_vec(),
                release_after: std::mem::take(&mut releases[position]),
                live_bytes,
        ...
        e.text(MEMORY_PLAN_DOMAIN);
        e.digest(graph_digest);
        for count in [
            input_bytes,
            cumulative_bytes,
            peak_live_bytes,
        ...
        for step in &steps {
            e.text(&step.node_id);
            e.u64(step.live_bytes as u64);
            e.u64(step.retained_bytes as u64);
    ...
    pub fn values(&self) -> &BTreeMap<String, TensorLifetime> {
        &self.values
    }
    ...
    pub fn peak_with_scratch(&self, scratch: &[usize]) -> Result<usize, MemoryPlanError> {
        if scratch.len() != self.steps.len() {
            return Err(MemoryPlanError::ScratchLength);
        }
        self.steps
            .iter()
            .zip(scratch)
            .try_fold(self.input_bytes, |peak, (step, bytes)| {
...
fn payload_bytes(port: &TensorPort) -> Result<usize, MemoryPlanError> {
    ...
    if bytes > MAX_STORAGE_BYTES {
        return Err(MemoryPlanError::Limit("tensor payload"));
    }
    Ok(bytes)
}
fn charge(
    ...
    *total = add(*total, amount)?;
    if *total > ceiling {
        Err(MemoryPlanError::Limit(kind))
    } else {
        Ok(())
    }
}
fn admit(
    ...
    if limits.max_nodes > MAX_MEMORY_PLAN_NODES
        || limits.max_values > MAX_MEMORY_PLAN_VALUES
    ...
    if graph.nodes().len() > limits.max_nodes {
        return Err(MemoryPlanError::Limit("nodes"));
    }
    let mut values = graph.inputs().len();
    if values > limits.max_values {
        return Err(MemoryPlanError::Limit("values"));
    }
    ...
    for node in graph.nodes() {
        ...
        for (name, value) in node.attributes() {
            ...
            let bytes = match value {
                ...
                AttrValue::IntList(value) => value
                    .len()
                    .checked_mul(8)
                    .ok_or(MemoryPlanError::Overflow)?,
                AttrValue::FloatList(value) => value
                    .len()
                    .checked_mul(8)
                    .ok_or(MemoryPlanError::Overflow)?,
