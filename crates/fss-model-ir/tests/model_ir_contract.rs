...
fn test_schema_domain_and_pinned_counts() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        domain_count, 45,
        "registries/DIGEST_DOMAINS.md count must remain pinned at 45"
    );
...
fn test_finding_10_model_ir_version_unsupported_mapping() -> Result<(), Box<dyn Error>> {
    ...
    // Direct construction of Unsupported(1) is rejected by validator
    let graph_alias = ModelIrGraph::new(
        "unsupported_1_alias_graph",
    ...
    match graph_alias.validate() {
        Err(ModelIrError::InvalidAttribute {
            node_id,
            attr_name,
            reason,
        }) => {
            assert_eq!(node_id, "graph");
...
fn test_reshape_allowzero_onnx_and_freetext_defaults() -> Result<(), Box<dyn Error>> {
    ...
    // Free-text defaults pinned in operator specs
...
fn test_mutant_m5b_table_tampering_refused() -> Result<(), Box<dyn Error>> {
    ...
    // Tampering flag causes validate() to reject with InvalidAttribute
    fss_model_ir::set_operator_table_tampered_for_test(true);
    match graph.validate() {
        Err(ModelIrError::InvalidAttribute {
            node_id,
            attr_name,
            ...
            assert!(reason.contains("table tampered"));
        }
        other => {
            fss_model_ir::set_operator_table_tampered_for_test(false);
            return Err(
                format!("expected InvalidAttribute when table tampered, got {other:?}").into(),
            );
        }
    }

    // Reset tampering flag
    fss_model_ir::set_operator_table_tampered_for_test(false);
    graph.validate()?;
    Ok(())
}
