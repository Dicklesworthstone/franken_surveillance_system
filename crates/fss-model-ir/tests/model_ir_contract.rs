...
fn test_schema_domain_and_pinned_counts() -> Result<(), Box<dyn Error>> {
    ...
    // 62 = 59 + SCHEMA-DOMAIN-FILE-IMPORT-MANIFEST-001, SCHEMA-DOMAIN-FILE-INGEST-LIMITS-001 and
    // SCHEMA-DOMAIN-FILE-IMPORT-IDENTITY-001 (fss-2h5zq.23).
    assert_eq!(
        domain_count, 62,
        "registries/DIGEST_DOMAINS.md count must remain pinned at 62"
    );
