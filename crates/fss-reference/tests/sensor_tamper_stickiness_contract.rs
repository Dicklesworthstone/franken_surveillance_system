...
-> Result<(), Box<dyn Error>> {
    ...
    // Defense in depth: forged receipt defense:
...
/// PR2: an alert is refused on a revision that drops an unretired tamper, with the HONEST
...
// ==== Round 4d (fss-2uftm): situation compilation refuses a substituted earlier revision ====
//
// As in the alert tests: each case plants a current revision whose receipt carries a lineage of the
// right length that is not the ledger's own, with a witness honest to that supplied lineage, so the
// witness cross-check passes and only the named admission check refuses.

/// Publishes `rev` under `object_event` straight through the authority publisher, with the
/// (prior, new) `generations`, the witness of `status_lineage` followed by `rev`, and a receipt
/// carrying `receipt_prior`.
fn r4d_plant(
    h: &mut TestHarness,
    rev: &EventHypothesis,
    object_event: &str,
    generations: (Option<u64>, u64),
    status_lineage: &[EventHypothesis],
    receipt_prior: &[EventHypothesis],
) -> Result<ReferenceEventReceipt, Box<dyn Error>> {
    for m in &rev.model_receipts {
        h.objects.require_verified(*m)?;
    }
    let event_object_digest = h.objects.put_verified(&rev.canonical_bytes())?;
    let event_revision_digest = h.objects.put_verified(&revision_encoding(rev))?;
    let manifest = ObjectManifest::new(
        "event-revision",
    ...
    let delta = EvidenceDelta {
        ...
        family: "event_revision".to_owned(),
        object_id: ObjectId::parse(format!("object:event:{object_event}"))?,
        prior_generation,
        new_generation,
    ...
    let tamper_delta = EvidenceDelta {
        ...
        family: "sensor_tamper_status".to_owned(),
        object_id: ObjectId::parse(format!("object:event:{object_event}:tamper"))?,
        prior_generation,
        new_generation,
        validity: rev.interval,
        plane: Plane::Authority,
    ...
    let authority_anchor = {
        let mut publisher = AuthorityPublisher::new(&h.objects, &mut h.authority);
        let batch = publisher.prepare_batch(
            BatchId::parse(format!("batch:event:{object_event}:{new_generation}"))?,
            vec![delta, tamper_delta],
            [event_root],
        )?;
        publisher.append(batch)?
    ...
    Ok(ReferenceEventReceipt {
        ...
        authority_anchor,
        lineage_tamper_status: status,
        prior_revision_encodings: receipt_prior.iter().map(revision_encoding).collect(),
    })
}

/// A Corroborated revision of `event` from two fresh person observations.
fn r4d_candidate(
    h: &mut TestHarness,
    name: &str,
    event: &str,
...
) -> Result<Result<ReferenceSituation, ReferenceError>, Box<dyn Error>> {
    let decision = ReferencePolicyDecision {
        event: current.clone(),
        action,
    };
    Ok(compile_reference_situation(
        test_request(&decision, receipt)?,
        &h.authority,
    ))
}

fn r4d_assert_stale(outcome: &Result<ReferenceSituation, ReferenceError>, case: &str) {
    assert!(
        matches!(outcome, Err(ReferenceError::StaleEventAuthority)),
        "{case}: situation compiled from a substituted lineage: {:?}",
        outcome.as_ref().map(r4b_physical)
    );
}

/// MHD: a fabricated clean revision 2 hides the published tamper revision 2; without the digest
/// check the situation would compile over the fabricated lineage with the tamper gone.
#[test]
fn round4d_situation_refuses_substituted_digest() -> Result<(), Box<dyn Error>> {
    let n = "r4d-sit-digest";
    let eid = "event:r4d:sit-digest";
    let mut h = TestHarness::new(n)?;
    let d1 = r4d_candidate(&mut h, n, eid, 50)?;
    let _r1 = publish_reference_event(&d1, &mut h.objects, &mut h.authority)?;
    let t = h.observation(
        n,
        "lane2",
    ...
    let _r2 = publish_reference_event(&d2, &mut h.objects, &mut h.authority)?;
    let fake = r4d_candidate(&mut h, &format!("{n}-fake"), eid, 500)?;
    let fake_2 = r4d_renumbered(&fake.event, 2, d1.event.revision_digest())?;
    let cand = r4d_candidate(&mut h, &format!("{n}-cur"), eid, 700)?;
    let current = r4d_renumbered(&cand.event, 3, fake_2.revision_digest())?;
    let supplied = [d1.event.clone(), fake_2];
    let receipt = r4d_plant(&mut h, &current, eid, (Some(2), 3), &supplied, &supplied)?;
    let outcome = r4d_compile(&h, &current, cand.action, &receipt)?;
    h.cleanup();
    r4d_assert_stale(&outcome, "digest");
    Ok(())
}

/// MHG: the current revision is numbered 3 but published as generation 2.
#[test]
fn round4d_situation_refuses_substituted_generation() -> Result<(), Box<dyn Error>> {
    let n = "r4d-sit-generation";
    let eid = "event:r4d:sit-generation";
    let mut h = TestHarness::new(n)?;
    let d1 = r4d_candidate(&mut h, n, eid, 50)?;
    let _r1 = publish_reference_event(&d1, &mut h.objects, &mut h.authority)?;
    let cand = r4d_candidate(&mut h, &format!("{n}-cur"), eid, 700)?;
    let current = r4d_renumbered(&cand.event, 3, d1.event.revision_digest())?;
    let supplied = [d1.event.clone()];
    let receipt = r4d_plant(&mut h, &current, eid, (Some(1), 2), &supplied, &supplied)?;
    let outcome = r4d_compile(&h, &current, cand.action, &receipt)?;
    h.cleanup();
    r4d_assert_stale(&outcome, "generation");
    Ok(())
}

/// MHE: a revision of another event is planted as the event object's genesis.
#[test]
fn round4d_situation_refuses_substituted_event_id() -> Result<(), Box<dyn Error>> {
    let n = "r4d-sit-event";
    let eid = "event:r4d:sit-event";
    let mut h = TestHarness::new(n)?;
    let foreign = r4d_candidate(&mut h, &format!("{n}-foreign"), "event:r4d:foreign", 50)?;
    let _ = r4d_plant(&mut h, &foreign.event, eid, (None, 1), &[], &[])?;
    let cand = r4d_candidate(&mut h, &format!("{n}-cur"), eid, 700)?;
    let current = r4d_renumbered(&cand.event, 2, foreign.event.revision_digest())?;
    let supplied = [foreign.event.clone()];
    let receipt = r4d_plant(&mut h, &current, eid, (Some(1), 2), &supplied, &supplied)?;
    let outcome = r4d_compile(&h, &current, cand.action, &receipt)?;
    h.cleanup();
    r4d_assert_stale(&outcome, "event id");
    Ok(())
}

/// MHS: the current revision supersedes a genesis the ledger never published.
#[test]
fn round4d_situation_refuses_broken_supersession_chain() -> Result<(), Box<dyn Error>> {
    let n = "r4d-sit-supersedes";
    let eid = "event:r4d:sit-supersedes";
    let mut h = TestHarness::new(n)?;
    let d1 = r4d_candidate(&mut h, n, eid, 50)?;
    let _r1 = publish_reference_event(&d1, &mut h.objects, &mut h.authority)?;
    let stray = r4d_candidate(&mut h, &format!("{n}-stray"), eid, 500)?;
    let cand = r4d_candidate(&mut h, &format!("{n}-cur"), eid, 700)?;
    let current = r4d_renumbered(&cand.event, 2, stray.event.revision_digest())?;
    let supplied = [d1.event.clone()];
    let receipt = r4d_plant(&mut h, &current, eid, (Some(1), 2), &supplied, &supplied)?;
    let outcome = r4d_compile(&h, &current, cand.action, &receipt)?;
    h.cleanup();
    r4d_assert_stale(&outcome, "supersedes");
    Ok(())
}

/// The ledger's own earlier revisions supplied in the wrong order.
#[test]
fn round4d_situation_refuses_reordered_lineage() -> Result<(), Box<dyn Error>> {
    let n = "r4d-sit-reordered";
    let eid = "event:r4d:sit-reordered";
    let mut h = TestHarness::new(n)?;
    let d1 = r4d_candidate(&mut h, n, eid, 50)?;
    let _r1 = publish_reference_event(&d1, &mut h.objects, &mut h.authority)?;
    let middle = r4d_candidate(&mut h, &format!("{n}-middle"), eid, 500)?;
    let middle_2 = r4d_renumbered(&middle.event, 2, d1.event.revision_digest())?;
    let genesis = [d1.event.clone()];
    let _ = r4d_plant(&mut h, &middle_2, eid, (Some(1), 2), &genesis, &genesis)?;
    let cand = r4d_candidate(&mut h, &format!("{n}-cur"), eid, 700)?;
    let current = r4d_renumbered(&cand.event, 3, middle_2.revision_digest())?;
    let receipt = r4d_plant(
        &mut h,
        &current,
        eid,
        (Some(2), 3),
        &[d1.event.clone(), middle_2.clone()],
        &[middle_2, d1.event.clone()],
    )?;
    let outcome = r4d_compile(&h, &current, cand.action, &receipt)?;
    h.cleanup();
    r4d_assert_stale(&outcome, "reordered");
    Ok(())
}
