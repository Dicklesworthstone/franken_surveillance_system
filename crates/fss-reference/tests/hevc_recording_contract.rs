...
fn index(
    ...
    e.digest(ContentDigest::sha256(init));
    e.digest(ContentDigest::sha256(media));
    for (i, r) in plan.parameter_ranges().iter().enumerate() {
        if mutation == 6 && i == 0 {
            range(&mut e, &(r.start + 1..r.end));
        } else {
            range(&mut e, r);
        }
    }
    ...
    for (i, sample) in plan.samples().iter().enumerate() {
        ...
        e.u32(sample.rtp_timestamp + u32::from(mutation == 2 && i == 0));
        e.bool(if mutation == 3 && i == 0 {
            !sample.idr
        } else {
            sample.idr
        });
        e.u64(if mutation == 1 && i == 0 {
            4
        } else {
    ...
    for (i, m) in plan.mappings().iter().enumerate() {
        ...
        for span in &m.sources {
            e.u64(span.sequence);
            if mutation == 4 && i == 0 {
                range(&mut e, &(span.wire_range.start + 1..span.wire_range.end));
            } else {
...
fn manifest(objects: RecordingObjects<'_>) -> Result<ObjectManifest, Error> {
    Ok(ObjectManifest::new(
        ...
        [
            ...
            ContentDigest::sha256(objects.initialization),
...
fn real_recording_retains_all_originals_and_replays_identical_media_and_maps() -> TestResult {
    ...
    assert_eq!(
        ...
        index(
            &plan,
            plan.objects().source,
            plan.objects().initialization,
            plan.objects().media,
            0
        )?
    );
    let verified = verify_hevc_recording(plan.manifest(), plan.objects(), &scope()?)?;
    assert_eq!(&verified.recording, plan.summary());
    assert_eq!(verified.source_only_nals, 0);
    ...
    for mapping in plan.mappings() {
        for span in &mapping.sources {
            let packet = originals
                .iter()
                .find(|p| p.sequence == span.sequence)
                .ok_or("missing original")?;
            assert_eq!(
                &packet.bytes[span.wire_range.clone()],
                &plan.objects().media[mapping.range.start + span.nal_range.start
...
fn boundary_witness_remains_in_source_instead_of_becoming_a_media_sample() -> TestResult {
    ...
    assert_eq!(plan.summary().packets, 8);
    assert_eq!(plan.summary().samples, 2);
    assert_eq!(plan.source_only_nals(), 3);
    assert_eq!(plan.summary().nals, 5);
    assert_eq!(
        plan.samples()[1].boundary,
        HevcBoundary::NextAccessUnitPrefix
    );
...
fn aggregation_and_fragmentation_retain_exact_source_and_header_synthesis() -> TestResult {
    ...
    let kind = (n[3][0] >> 1) & 63;
    let mut start = vec![0x62, 1, 0x80 | kind];
    start.extend_from_slice(&n[3][2..split]);
    let mut end = vec![0x62, 1, 0x40 | kind];
    end.extend_from_slice(&n[3][split..]);
    let source = vec![
        packet(65_534, 0, false, &aggregate(&n[..3])),
        packet(65_535, 0, false, &start),
        packet(65_536, 0, true, &end),
        packet(65_537, 0, true, &[0x48, 1, 0x80]),
    ];
    let plan = prepare(&source, &timings(1))?;
    assert_eq!(plan.summary().packets, 4);
    assert_eq!(plan.summary().nals, 5);
...
fn gaps_duplicates_reordering_wrong_payload_and_wrong_ssrc_are_not_sealable() -> TestResult {
    for case in 0..6 {
        ...
        match case {
            ...
            1 => {
                source.insert(4, source[3].clone());
            }
            2 => source.swap(3, 4),
...
fn saved_arrival_order_is_not_a_decode_clock_and_changes_only_the_bound_source_root() -> TestResult
{
    ...
    assert_eq!(a.manifest(), again.manifest());
    assert_eq!(a.objects().index, again.objects().index);
    Ok(())
}
...
fn every_scope_coordinate_is_externally_pinned_and_avc_reader_stays_separate() -> TestResult {
    ...
    for case in 0..5 {
        ...
        assert_eq!(
            verify_hevc_recording(plan.manifest(), plan.objects(), &expected).err(),
            Some(E::Scope)
        );
    }
...
#[test]
fn rehashing_forged_boundaries_timestamps_idr_flags_maps_or_lookahead_does_not_validate_them()
-> TestResult {
    let plan = fixture()?;
    let original = plan.objects();
    for mutation in 1..=7 {
        let encoded = index(
            &plan,
            original.source,
        ...
        assert!(
            verify_hevc_recording(&root, objects, &scope()?).is_err(),
            "rehash mutation {mutation}"
        );
    }
    Ok(())
}
...
fn malformed_canonical_suffix_and_wrong_manifest_role_are_never_accepted() -> TestResult {
    let plan = fixture()?;
    let original = plan.objects();
    let mut appended = original.index.to_vec();
    appended.push(0);
    let objects = RecordingObjects {
        index: &appended,
        ..original
    ...
    assert!(verify_hevc_recording(&manifest(objects)?, objects, &scope()?).is_err());
    let wrong = ObjectManifest::new(
        "avc_recording_window_v1",
        [
            ContentDigest::sha256(original.source),
            ContentDigest::sha256(original.initialization),
            ContentDigest::sha256(original.media),
        ],
        plan.manifest().metadata_digest(),
    )?;
    assert!(verify_hevc_recording(&wrong, original, &scope()?).is_err());
    let mut truncated = original.source.to_vec();
    truncated.pop();
    let encoded = index(
        &plan,
        &truncated,
...
fn fixed_work_and_combined_object_limits_fail_before_unbounded_replay() -> TestResult {
    ...
    assert_eq!(
        prepare_hevc_recording(scope()?, &config, 90_000, &timings(257), &borrowed(&source)).err(),
        Some(E::Limit)
    );
    assert!(prepare_hevc_recording(scope()?, &config, 0, &timings(4), &borrowed(&source)).is_err());
    let too_many: Vec<_> = (0..4097)
        .map(|i| packet(i, 0, true, &[0x26, 1, 0xa0]))
        .collect();
    assert_eq!(
        prepare_hevc_recording(scope()?, &config, 90_000, &timings(1), &borrowed(&too_many)).err(),
        Some(E::Limit)
    );
    let plan = fixture()?;
    let huge = vec![0; fss_reference::rtsp::recording::MAX_RECORDING_BYTES + 1];
    assert_eq!(
        verify_hevc_recording(
            plan.manifest(),
            RecordingObjects {
                source: &huge,
                ..plan.objects()
