...
use fss_object::{
    HostSpoolIo, ObjectManifest, RecordingSpoolIo, SPOOL_LOCK_FILE, SpoolIo, SpoolLimits,
};
use fss_publication::{
...
/// Delegates every call to [`HostSpoolIo`], except that `symlink_metadata` reports `NotFound` for
/// exactly the hidden paths. It counts those lookups so a test can prove they reached this seam.
#[derive(Debug)]
struct HiddenLockFilesIo {
    hidden: Vec<PathBuf>,
    hidden_lookups: Mutex<usize>,
}

impl HiddenLockFilesIo {
    fn new(hidden: Vec<PathBuf>) -> Self {
        Self {
            hidden,
            hidden_lookups: Mutex::new(0),
        }
    }

    fn hidden_lookups(&self) -> usize {
        self.hidden_lookups.lock().map(|n| *n).unwrap_or(usize::MAX)
    }
}

impl SpoolIo for HiddenLockFilesIo {
    ...
    fn symlink_metadata(&self, path: &Path) -> io::Result<Metadata> {
        if self.hidden.iter().any(|hidden| hidden == path) {
            if let Ok(mut lookups) = self.hidden_lookups.lock() {
                *lookups += 1;
            }
...
/// Full bytes and nanosecond mtime of every regular file under `root`, with no size cap.
fn file_bytes_and_mtimes(root: &Path) -> Res<BTreeMap<PathBuf, (String, i64, i64)>> {
    let mut out = BTreeMap::new();
    let mut pending = vec![root.to_path_buf()];
    while let Some(path) = pending.pop() {
        let meta = fs::symlink_metadata(&path)?;
        if meta.file_type().is_dir() {
            ...
            let digest = ContentDigest::sha256(&fs::read(&path)?).to_text();
            out.insert(
                path.strip_prefix(root)?.to_path_buf(),
                (digest, meta.mtime(), meta.mtime_nsec()),
            );
        }
    }
    Ok(out)
...
/// The foreign-bytes plan digest is the doctor's one filesystem access outside `DoctorIo`
/// (`fs::canonicalize` and `fs::metadata` inside fss-ledger's plan constructor). With foreign
/// bytes in both journals, both plans are built, every file keeps its bytes and mtime, the seam
/// records no mutating call, and the report names both plan digests.
#[test]
fn foreign_bytes_plan_digest_is_the_only_off_seam_read_and_changes_nothing() -> TestResult {
    let dep = init("foreign_off_seam")?;
    let ledger_path = dep.join(RELATIVE_PATH_LEDGER);
    let effects_path = dep.join(RELATIVE_PATH_EFFECTS);
    append_bytes(&ledger_path, b"FOREIGN_GARBAGE_BYTES_WITHOUT_MAGIC")?;
    append_bytes(&effects_path, b"FOREIGN_GARBAGE_BYTES_WITHOUT_MAGIC")?;
    let files_before = file_bytes_and_mtimes(&dep)?;
    assert!(files_before.contains_key(Path::new(RELATIVE_PATH_LEDGER)));
    assert!(files_before.contains_key(Path::new(RELATIVE_PATH_EFFECTS)));
    let tree_before = tree_digest(&dep)?;
    ...
    assert_eq!(file_bytes_and_mtimes(&dep)?, files_before);
    assert_eq!(tree_digest(&dep)?, tree_before);
    assert_eq!(recording.mutating_calls(), Vec::new());
...
/// Both writer observations must use `DoctorIo::fs`. The seam hides exactly the deployment's lock
/// files, so a doctor that stats them through the seam reports `no_lock_file` before and after the
/// journals. A doctor that bypassed the seam would find the real lock files and report another
/// state.
#[test]
fn writer_detection_observes_lock_files_through_the_doctor_io_seam() -> TestResult {
    let dep = init("writer_seam")?;
    let hidden = HiddenLockFilesIo::new(lock_files(&dep));
    let io = DoctorIo {
        fs: &hidden,
        files: &HostJournalReadIo,
        lock_table: &HostLockTableSource,
    ...
    expected.set(
        ...
        check(
            ...
            &[
                ("possibly_stale", "false".to_owned()),
                ("probe_method", s("proc_locks")),
                ("writer_state", s("no_lock_file")),
                ("writer_state_after", s("no_lock_file")),
            ],
            &[finding("no_lock_file", "info", None, None)],
            None,
        ),
    )?;
    assert_eq!(report.to_json(), expected.json());
    assert_eq!(report.exit_code(), 0);
    // Four lock paths before the journals and three (no effects journal) after them.
    assert_eq!(hidden.hidden_lookups(), 7);

    // Control: through the host seam the same root has real lock files and a clean writer.
    assert_eq!(
        check_json(&inspect_deployment(&dep), "deployment.writer")?,
        writer_clean()
    );
    Ok(())
}
