...
fn pair() -> Result<RecordingCatalog, Error> {
    ...
    Ok(prepare_catalog(
        ...
        &[
            CatalogWindow {
                slot: &SlotName::parse("first")?,
                recording: &a,
            },
            CatalogWindow {
                slot: &SlotName::parse("second")?,
                recording: &b,
            },
        ],
    )?)
...
fn canonical_roundtrip_pins_every_leaf_and_exact_time_basis() -> TestResult {
    ...
    assert_eq!(decoded.index_bytes(), c.index_bytes());
    ...
    for (_, d, _) in a.children() {
        assert!(c.manifest().children().contains(&d));
    }
    Ok(())
}
#[test]
fn range_selects_full_windows_and_reports_internal_and_external_unknowns() -> TestResult {
    let c = pair()?;
    let s = c.select(0..18000, CatalogQueryLimits::default())?;
...
fn half_open_boundaries_do_not_double_select_neighbors() -> TestResult {
    let c = pair()?;
    assert!(
        c.select(7200..10800, CatalogQueryLimits::default())?
            .windows()
            .is_empty()
    );
...
fn budget_failure_never_becomes_a_truncated_success() -> TestResult {
    ...
    let total = c
        .select(0..18000, CatalogQueryLimits::default())?
        .output_bytes();
    assert_eq!(
        c.select(
            0..18000,
...
fn empty_index_answer_is_unknown_not_absence() -> TestResult {
    let c = pair()?;
    let s = c.select(20_000..u64::MAX, CatalogQueryLimits::default())?;
    assert!(s.windows().is_empty());
    assert_eq!(s.output_bytes(), 0);
    assert_eq!(s.unindexed(), std::slice::from_ref(&(20_000..u64::MAX)));
    assert_eq!(
        c.select(5..5, CatalogQueryLimits::default()),
        Err(CatalogError::Interval)
    );
    assert_eq!(
        c.select(
...
fn overlapping_unordered_and_duplicate_slots_are_refused() -> TestResult {
    let a = window(1, 3600)?;
    let b = window(3, 5000)?;
    let x = SlotName::parse("x")?;
    let y = SlotName::parse("y")?;
    assert!(matches!(
        prepare_catalog(
            catalog_scope()?,
            &[
...
fn exact_scope_timebase_and_generation_are_not_inferred_from_the_page() -> TestResult {
    ...
    for field in 0..5 {
        ...
        assert!(matches!(
            verify_catalog(c.manifest(), c.index_bytes(), &s),
            Err(CatalogError::Scope)
        ));
    }
    Ok(())
}
...
fn rehashed_root_cannot_hide_an_omitted_original_object() -> TestResult {
    let c = pair()?;
    let meta = c.manifest().metadata_digest().ok_or("metadata")?;
    let mut children: Vec<_> = c
        .manifest()
        .children()
        .iter()
        .copied()
    ...
    assert!(matches!(
        verify_catalog(&altered, c.index_bytes(), &catalog_scope()?),
...
fn page_count_limit_is_refused_before_copying_unbounded_inputs() -> TestResult {
    let a = window(1, 3600)?;
    let slot = SlotName::parse("a")?;
    assert!(matches!(
        prepare_catalog(catalog_scope()?, &[]),
        Err(CatalogError::Limit)
    ));
    let inputs: Vec<_> = (0..MAX_CATALOG_WINDOWS + 1)
        .map(|_| CatalogWindow {
            slot: &slot,
            recording: &a,
        })
        .collect();
    assert!(matches!(
        prepare_catalog(catalog_scope()?, &inputs),
        Err(CatalogError::Limit)
    ));
    Ok(())
}
...
fn query_partition_covers_each_tick_exactly_once() -> TestResult {
    ...
    for start in (0..16000).step_by(700) {
        for width in [1, 3599, 3600, 7201] {
            ...
            for adjacent in parts.windows(2) {
