...
use fss_packet::{
    H265Error, H265Limits, H265ReceiveError, H265ReceivePoll, ReorderDisposition, ReorderLimits,
    StreamKey,
};
...
use fss_reference::rtsp::framed::{
    MAX_WIRE_CHUNK, RtspWireFrame, WIRE_LIFETIME_NS, WireIntakeError,
};
...
fn nals(events: &[P]) -> Vec<Vec<u8>> {
    let mut output = Vec::new();
    for event in events {
        if let P::Media(H265ReceivePoll::Packet {
            reconstruction: Ok(result),
            ..
        }) = event
        {
...
fn playing(
    ...
    feed(
        &mut client,
        &response(3, 200, &[("Session", "private-session")], &[]),
        5,
    )?;
...
fn authenticated(codec: H265Limits) -> Result<RtspHevcClient, Box<dyn std::error::Error>> {
    ...
    let mut client = RtspHevcClient::with_digest(
        config(),
        KEY,
    ...
    assert!(matches!(
        output.last(),
        Some(P::AuthenticationRequired { cseq: 1, .. })
    ));
    ...
    feed(
        &mut client,
        &response(2, 200, &[("Content-Type", "application/sdp")], &sdp(false)),
        3,
    )?;
    ...
    feed(
        &mut client,
        &response(4, 200, &[("Session", "private-session")], &[]),
        7,
    )?;
    assert_eq!(client.state(), ClientState::Playing);
    prime(&mut client)?;
...
fn every_tcp_description_split_preserves_exact_control_source_and_selected_codec() -> TestResult {
    ...
    for split in 1..wire.len() {
        let mut client = RtspHevcClient::new(
            config(),
            KEY,
            ReorderLimits::default(),
            H265Limits::default(),
        ...
        let P::Control { progress, source } = &after[0] else {
            return Err("missing control source".into());
        };
        ...
        assert_eq!(
            client
                .media()
                .ok_or("missing HEVC negotiation")?
                .payload_type(),
            98
        );
    }
...
fn play_ack_and_first_rtp_coalesced_in_one_tcp_chunk_are_processed_in_order() -> TestResult {
    ...
    assert!(
        matches!(&events[0], P::Control { progress: ClientProgress::Accepted(ClientState::Playing), source }
...
fn fragmented_hevc_reorders_without_losing_wire_envelopes_or_nal_provenance() -> TestResult {
    ...
    let sources: Vec<_> = events
        ...
        .filter_map(|event| match event {
            P::Rtp { source, .. } => Some(source.expose_wire().to_vec()),
            _ => None,
        })
        .collect();
    ...
    let nal = events
        ...
        .find_map(|event| match event {
            P::Media(H265ReceivePoll::Packet {
                reconstruction: Ok(result),
                ..
            }) => result.nals.first(),
            _ => None,
    ...
    for (index, wire) in [&start, &middle, &end].into_iter().enumerate() {
        ...
        assert_eq!(
            &wire[4 + span.wire_range.start..4 + span.wire_range.end],
...
fn media_before_play_or_on_wrong_channel_is_returned_intact_and_fenced() -> TestResult {
    for early in [true, false] {
        ...
        assert_eq!(
            source.ok_or("refused media disappeared")?.expose_wire(),
            wire
        );
...
fn setup_ssrc_mismatch_is_rejected_before_media_configuration() -> TestResult {
    ...
    feed(
        &mut client,
        &response(1, 200, &[("Content-Type", "application/sdp")], &sdp(false)),
        1,
    )?;
    client.request(C::Setup, 2)?;
    let wire = response(
        ...
        &[
            ...
            (
                "Transport",
                "RTP/AVP/TCP;unicast;interleaved=0-1;ssrc=00000009",
            ),
        ],
        &[],
    );
...
fn codec_refusal_keeps_both_original_views_and_does_not_invent_stream_failure() -> TestResult {
    ...
    assert!(
        events
            .iter()
            .any(|p| matches!(p, P::Rtp { source, .. } if source.expose_wire() == wire))
    );
    ...
    assert_eq!(client.state(), ClientState::Playing);
    assert_eq!(
        nals(&feed(&mut client, &packet(5, true, &[0x26, 1, 8]), 21)?),
        vec![vec![0x26, 1, 8]]
    );
    Ok(())
}
...
fn compound_and_reduced_rtcp_are_negotiated_not_inferred_from_invalid_input() -> TestResult {
    ...
    for reduced in [false, true] {
        ...
        let P::Rtcp { source, validation } = &events[0] else {
            return Err("missing RTCP result".into());
        };
        assert_eq!(source.expose_wire(), wire);
    ...
    assert!(matches!(
        &feed(&mut client, &interleaved(1, &compound), 6)?[0],
        P::Rtcp {
            validation: Ok(2),
            ..
        }
    ...
    assert!(matches!(
        &feed(&mut client, &interleaved(1, &damaged), 7)?[0],
        P::Rtcp {
            validation: Err(_),
            ..
        }
...
fn delivery_gap_and_late_recovery_cannot_resurrect_a_hevc_fragment_chain() -> TestResult {
    ...
    assert!(
        matches!(&events[0], P::Media(H265ReceivePoll::Gap { gap, discarded: Some(discard) })
    ...
    assert!(
        matches!(&late[0], P::Rtp { admission, .. } if admission.transport.disposition == ReorderDisposition::TooLate)
    );
...
fn full_queue_retains_one_exact_retry_then_admits_it_once_after_timer_progress() -> TestResult {
    ...
    let rejected = client
        .ingest(&packet(7, true, &[0x26, 1, 7]), 22)
        .err()
        .ok_or("backpressure ignored")?;
    ...
    assert_eq!(
        sources[0].1.transport.disposition,
        ReorderDisposition::Buffered
    );
    assert!(events.iter().any(
        |p| matches!(p, P::Media(H265ReceivePoll::Packet { source, .. })
...
fn held_retry_cannot_outlive_its_original_wire_residence_deadline() -> TestResult {
    ...
    assert_eq!(
        retired.retry.ok_or("held frame disappeared")?.expose_wire(),
        waiting
    );
    assert_eq!(
        retired.video.ok_or("RTP retirement missing")?.queue.packets,
        1
    );
...
fn impossible_single_datagram_budget_refuses_instead_of_waiting_forever() -> TestResult {
    ...
    assert_eq!(
        source.ok_or("oversized datagram lost")?.expose_wire(),
        oversized
    );
    assert_eq!(client.next_wake_ns(), None);
    Ok(())
...
fn blocked_partial_wire_waits_for_hard_expiry_not_a_busy_keepalive_loop() -> TestResult {
    ...
    assert!(matches!(
        client.poll(1_000_000_004)?,
        P::Pending {
            wake_at_ns: Some(2_000_000_004)
        }
    ));
...
fn fragment_expiry_runs_without_network_and_before_consuming_new_wire() -> TestResult {
    ...
    assert!(
        matches!(client.poll(30)?, P::Media(H265ReceivePoll::FragmentDiscarded(discard))
...
fn late_final_byte_cannot_erase_partial_frame_expiry() -> TestResult {
    ...
    let failure = client
        .ingest(&wire[wire.len() - 1..], WIRE_LIFETIME_NS + 20)
        .err()
        .ok_or("expired partial frame accepted")?;
    assert_eq!(failure.reason, E::Wire(WireIntakeError::Deadline));
    assert_eq!(
        failure
            .retirement
            .ok_or("missing retirement")?
            .wire
            .expose(),
        &wire[..wire.len() - 1]
    );
    ...
    for truncated in [false, true] {
        ...
        if truncated {
            ...
            assert!(matches!(
                events.last(),
                Some(P::Ended {
                    media: Some(H265ReceivePoll::Ended { discarded: None }),
                    retirement: Some(_)
                })
            ));
        }
...
fn eof_incomplete_fragment_is_retired_exactly_once_without_a_complete_nal() -> TestResult {
    ...
    drain(&mut client, 20)?;
    let P::Ended {
        media: Some(H265ReceivePoll::Ended {
            discarded: Some(discard),
        }),
        retirement: Some(retired),
    } = client.poll(20)?
    else {
        return Err("missing incomplete EOF retirement".into());
    };
    ...
    assert!(
        retired
            .video
            .ok_or("missing video accounting")?
            .fragment
            .is_none()
    );
    assert!(retired.session.remote_session_may_exist);
...
fn teardown_ack_stops_admission_and_returns_lookahead_in_terminal_custody() -> TestResult {
    ...
    assert!(matches!(
        client.poll(22)?,
        P::Control {
            progress: ClientProgress::Accepted(ClientState::Closed),
            ..
        }
    ));
    let P::Ended {
        ...
        return Err("teardown did not finalize accepted codec state".into());
    };
    assert_eq!(discard.reason, H265Error::EndOfInput);
...
#[test]
fn cancellation_accounts_for_queue_fragment_and_partial_tcp_without_duplicate_receipts()
-> TestResult {
    let mut client = primed(ReorderLimits::default(), H265Limits::default())?;
    feed(&mut client, &packet(4, false, &[0x62, 1, 0x93, 1]), 20)?;
    feed(&mut client, &packet(6, true, &[0x62, 1, 0x53, 3]), 21)?;
    ...
    assert_eq!(
        video.fragment.ok_or("missing fragment retirement")?.reason,
        H265Error::Cancelled
    );
    assert_eq!(retired.wire.expose(), b"$\0");
    assert!(retired.session.remote_session_may_exist);
...
fn confirmed_source_restart_closes_connection_and_preserves_both_retirement_layers() -> TestResult {
    ...
    let P::Rtp {
        ...
        return Err("confirmed restart was not terminal".into());
    };
    assert_eq!(
...
fn digest_authentication_reuses_shared_session_and_returns_consumed_raw_challenge() -> TestResult {
    ...
    let plain = client
        .request(C::KeepAlive, 21)
        .err()
        .ok_or("Digest bypass was permitted")?;
    assert!(matches!(
        plain.reason,
        E::Session(ClientError::Authentication(_))
    ));
...
fn unsupported_or_unsolicited_challenges_are_not_presented_as_credential_requests() -> TestResult {
    for wire in [
        ...
        response(
            1,
            401,
            &[("WWW-Authenticate", "Basic realm=\"owner-realm\"")],
            &[],
        ),
        response(
            1,
            407,
            &[("Proxy-Authenticate", "Digest realm=\"owner-realm\"")],
            &[],
        ),
    ] {
        let mut client = RtspHevcClient::with_digest(
        ...
        assert!(matches!(
            error,
            E::Session(ClientError::CseqMismatch) | E::Authentication(_)
        ));
        assert!(source.is_none());
        assert_eq!(
            retired
                .challenge
                .ok_or("challenge evidence disappeared")?
                .expose_wire(),
            wire
        );
        assert_eq!(client.state(), ClientState::Closed);
...
fn rejected_digest_realm_preserves_the_challenge_and_unprocessed_lookahead() -> TestResult {
    ...
    let failure = client
        .respond_digest(&credentials, [2; 16], 2)
        .err()
        .ok_or("realm mismatch accepted")?;
    assert!(matches!(failure.reason, E::Authentication(_)));
    let retired = failure.retirement.ok_or("missing retirement")?;
    assert_eq!(
        retired.challenge.ok_or("missing challenge")?.expose_wire(),
        wire
    );
    assert_eq!(retired.wire.expose(), suffix);
    Ok(())
...
fn original_request_deadline_and_wire_age_survive_held_digest_challenges() -> TestResult {
    ...
    for request_expires_first in [true, false] {
        ...
        let deadline = if request_expires_first {
            100
        } else {
            WIRE_LIFETIME_NS + 20
        };
        ...
        assert!(matches!(
            client.poll(deadline - 1)?,
            P::AuthenticationRequired { cseq: 1, .. }
        ));
        let failure = client
            .respond_digest(&credentials, [2; 16], deadline)
            .err()
            .ok_or("expired challenge responded")?;
        let expected = if request_expires_first {
            E::Session(ClientError::ResponseTimeout)
        } else {
            E::Wire(WireIntakeError::Deadline)
        };
...
fn codec_timers_keep_running_while_digest_keepalive_waits_for_credentials() -> TestResult {
    ...
    assert!(
        retired
            .video
            .ok_or("missing video retirement")?
            .fragment
            .is_none()
    );
    Ok(())
...
fn no_codec_fallback_limits_and_reversed_time_are_enforced_at_the_public_boundary() -> TestResult {
    ...
    client.request(C::Describe, 0)?;
    let failure = client
        .ingest(&vec![0; MAX_WIRE_CHUNK + 1], 1)
        .err()
        .ok_or("chunk limit ignored")?;
    ...
    assert_eq!(
        client.poll(19).err(),
        Some(E::Session(ClientError::ClockReversed))
    );
    assert_eq!(client.retained_nal_bytes(), 3);
    assert_eq!(
        nals(&feed(
            &mut client,
            &packet(5, true, &[0x62, 1, 0x53, 2]),
            21
        )?),
        vec![vec![0x26, 1, 1, 2]]
    );
