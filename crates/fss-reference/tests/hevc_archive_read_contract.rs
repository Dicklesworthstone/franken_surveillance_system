...
fn namespace_and_empty_inventory_are_codec_separated_and_avc_bytes_stay_frozen() -> TestResult {
    ...
    for (domain, prefix, expected_digest, expected_slot) in [
        ("fss.local_recording_archive_namespace.v1", "fssa1", avc.digest(), avc.window_slot(0)?),
        ("fss.local_hevc_recording_archive_namespace.v1", "fssh1", hevc.digest(), hevc.window_slot(0)?),
    ] {
        let mut e = CanonicalEncoder::new();
        e.text(domain); e.text(scope.recording.sensor.as_str()); e.text(scope.recording.stream.as_str());
        ...
        assert_eq!(expected_slot.as_str(), format!("{prefix}-{}-w-0000000000000000", text.strip_prefix("sha256:").ok_or("hash")?));
    }
    ...
    e.digest(a.namespace().digest()); e.u64(0); e.u64(0); e.u64(0);
    assert_eq!(a.digest()?, ContentDigest::try_sha256(&e.finish_checked()?)?);
    assert!(matches!(HevcArchiveSnapshot::load(&p, namespace()?, archive_limits(), &Stop), Err(ArchiveError::Cancelled)));
...
fn reopen_reads_three_pages_as_typed_whole_hevc_windows_and_explicit_decode_gaps() -> TestResult {
    ...
    let digest = before.digest()?; drop(before); drop(p);
    ...
    let mut read = HevcArchiveRead::new(&p, &snapshot, 100..300_000, ArchiveQueryLimits::default(), 100)?;
    ...
    for (i, expected) in windows.iter().enumerate() {
        let HevcArchiveReadProgress::Window { ordinal, requested_interval, recording } = read.step(i as u64, &NeverCancel)?
            else { return Err("missing typed HEVC window".into()); };
        assert_eq!(ordinal, i);
        assert_eq!(requested_interval, (if i == 0 { 100 } else { i as u64 * 90_000 })..i as u64 * 90_000 + 72_000);
    ...
    let HevcArchiveReadProgress::Complete(receipt) = read.step(3, &NeverCancel)? else { return Err("no completion".into()); };
    assert_eq!(receipt.windows, 3); assert_eq!(receipt.unindexed, gaps);
    assert_eq!(receipt.scope, fixture::scope()?); assert_eq!(receipt.snapshot_digest, digest);
    assert_eq!(receipt.output_bytes, windows.iter().map(|w| w.byte_len() as u64).sum::<u64>());
    assert!(matches!(read.step(200, &Stop)?, HevcArchiveReadProgress::Exhausted));
    Ok(())
}
...
fn durable_tail_is_not_indexed_and_a_new_page_does_not_rewrite_an_old_snapshot() -> TestResult {
    ...
    assert_eq!(old.windows().len(), 3); assert_eq!(old.indexed_windows(), 2);
    assert_eq!(old.unindexed_windows()[0].root(), windows[2].manifest().root());
    assert!(old.select(180_000..252_000, ArchiveQueryLimits::default())?.ordinals().is_empty());
    ...
    assert_eq!(new.select(180_000..252_000, ArchiveQueryLimits::default())?.ordinals(), &[2]);
    let mut read = HevcArchiveRead::new(&p, &old, 180_000..252_000, ArchiveQueryLimits::default(), 100)?;
    let HevcArchiveReadProgress::Complete(receipt) = read.step(0, &NeverCancel)? else { return Err("no empty receipt".into()); };
    assert_eq!(receipt.windows, 0); assert_eq!(receipt.unindexed, vec![180_000..252_000]);
    Ok(())
}
...
fn selection_prices_whole_windows_and_refuses_all_on_any_limit_overflow() -> TestResult {
    ...
    assert!(matches!(s.select(1..2, ArchiveQueryLimits { max_windows: 1,
        max_output_bytes: windows[0].byte_len() as u64 - 1 }), Err(ArchiveError::Limit)));
    assert_eq!(s.select(1..2, ArchiveQueryLimits { max_windows: 1,
        max_output_bytes: windows[0].byte_len() as u64 })?.output_bytes(), windows[0].byte_len() as u64);
    assert!(matches!(s.select(0..300_000, ArchiveQueryLimits { max_windows: 2,
...
fn recovery_refuses_holes_overlapping_pages_wrong_bindings_and_same_prefix_garbage() -> TestResult {
    for case in 0..4 {
        ...
        let ns = namespace()?;
        let a = fixture::window(0)?; let b = fixture::window(90_000)?;
        if case == 0 {
            ...
            if case == 1 {
                let c = page(&ns, 1, &[b])?; publish_page(&mut p, &ns, 1, &c)?;
            } else {
                let c = page(&ns, 0, &[a, b])?;
...
fn codec_namespace_relabeling_does_not_select_a_weaker_window_verifier() -> TestResult {
    ...
    assert_eq!(HevcArchiveSnapshot::load(&p, ns, archive_limits(), &NeverCancel)?.windows().len(), 3);
    Ok(())
}
...
fn corrupted_source_or_false_catalog_descriptor_prevents_recovery() -> TestResult {
    ...
    let index = fixture::index_with_bytes("fss.hevc_recording_catalog.v1", ns.scope(),
        &[(&slot, windows[0].publication_plan())], None, Some(1))?;
    let manifest = fixture::manifest("hevc_recording_catalog_v1", &index, &[windows[0].publication_plan()], None)?;
    ...
    assert!(matches!(HevcArchiveSnapshot::load(&p, ns, archive_limits(), &NeverCancel), Err(ArchiveError::Metadata)));
    Ok(())
}
...
fn later_corruption_preserves_earlier_owned_output_but_blocks_aggregate_success() -> TestResult {
    ...
    let snapshot = HevcArchiveSnapshot::load(&p, ns, archive_limits(), &NeverCancel)?;
    let mut read = HevcArchiveRead::new(&p, &snapshot, 0..300_000, ArchiveQueryLimits::default(), 100)?;
    let HevcArchiveReadProgress::Window { recording, .. } = read.step(0, &NeverCancel)? else { return Err("first window".into()); };
...
fn cancellation_at_every_read_probe_prevents_window_disclosure_and_stops_attempt() -> TestResult {
    ...
    let mut baseline = HevcArchiveRead::new(&p, &snapshot, 1..2, ArchiveQueryLimits::default(), 100)?;
    assert!(matches!(baseline.step(0, &count)?, HevcArchiveReadProgress::Window { .. }));
    assert!(count.calls.get() > 1);
    for stop_at in 1..=count.calls.get() {
        let cancel = Probe { calls: std::cell::Cell::new(0), stop_at };
        let mut read = HevcArchiveRead::new(&p, &snapshot, 1..2, ArchiveQueryLimits::default(), 100)?;
        assert!(read.step(0, &cancel).is_err(), "probe {stop_at}");
        assert_eq!(read.returned_windows(), 0);
        assert!(matches!(read.step(1, &NeverCancel), Err(ArchiveError::Blocked)));
    }
    Ok(())
}
...
fn deadline_and_partial_cancellation_never_complete_but_clock_reversal_is_retryable() -> TestResult {
    ...
    let snapshot = HevcArchiveSnapshot::load(&p, ns, archive_limits(), &NeverCancel)?;
    let mut read = HevcArchiveRead::new(&p, &snapshot, 0..300_000, ArchiveQueryLimits::default(), 10)?;
    assert!(matches!(read.step(5, &NeverCancel)?, HevcArchiveReadProgress::Window { ordinal: 0, .. }));
    assert!(matches!(read.step(4, &NeverCancel), Err(ArchiveError::ClockReversed)));
    assert!(matches!(read.step(6, &NeverCancel)?, HevcArchiveReadProgress::Window { ordinal: 1, .. }));
    assert!(matches!(read.step(10, &NeverCancel), Err(ArchiveError::Deadline)));
    assert_eq!(read.returned_windows(), 2);
    assert!(matches!(read.step(11, &NeverCancel), Err(ArchiveError::Blocked)));
    let mut cancelled = HevcArchiveRead::new(&p, &snapshot, 0..300_000, ArchiveQueryLimits::default(), 100)?;
    cancelled.step(0, &NeverCancel)?;
    assert!(matches!(cancelled.step(1, &Stop), Err(ArchiveError::Cancelled)));
...
fn empty_answers_and_completion_recheck_all_pages_in_the_selected_inventory() -> TestResult {
    ...
    let snapshot = HevcArchiveSnapshot::load(&p, ns, archive_limits(), &NeverCancel)?;
    let mut read = HevcArchiveRead::new(&p, &snapshot, 500_000..600_000, ArchiveQueryLimits::default(), 100)?;
    corrupt(&root, snapshot.pages()[2].catalog().manifest().metadata_digest().ok_or("metadata")?)?;
...
fn recovery_limits_and_foreign_owner_cannot_produce_false_success() -> TestResult {
    ...
    for limits in [
        ArchiveLimits { max_windows: 2, ..archive_limits() },
        ArchiveLimits { max_pages: 2, ..archive_limits() },
        ArchiveLimits { max_scan_roots: 1, ..archive_limits() },
    ] { assert!(matches!(HevcArchiveSnapshot::load(&p, ns.clone(), limits, &NeverCancel), Err(ArchiveError::Limit))); }
    let snapshot = HevcArchiveSnapshot::load(&p, ns, archive_limits(), &NeverCancel)?;
    let foreign = LocalRootPublisher::open(fresh("foreign_owner")?, limits())?;
    let mut read = HevcArchiveRead::new(&foreign, &snapshot, 1..2, ArchiveQueryLimits::default(), 100)?;
    assert!(read.step(0, &NeverCancel).is_err());
    assert_eq!(read.returned_windows(), 0);
    // A request for an unindexed interval still cannot skip missing page roots.
    let mut empty = HevcArchiveRead::new(&foreign, &snapshot, 500_000..600_000, ArchiveQueryLimits::default(), 100)?;
