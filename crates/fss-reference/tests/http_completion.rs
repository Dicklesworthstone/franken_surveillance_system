...
impl Directory {
    fn new() -> Test<Self> {
        for n in 0..128 {
            let path =
                std::env::temp_dir().join(format!("fss-http-end-{}-{n}", std::process::id()));
...
impl HttpCameraAuthority for Authority {
    fn checkpoint(
        ...
        if now >= deadline || self.clock.elapsed() >= Duration::from_secs(30) {
...
fn capture(
    ...
    wire: &[u8],
) -> Test<(HttpCamera, HttpWireArchive, Vec<[u8; 32]>)> {
    ...
    assert_eq!(
        PreparedHttpCompletion::from_camera(&camera, &archive, &mut work()).err(),
        Some(HttpCompletionError::NotReady)
    );
    ...
    for _ in 0..100_000 {
        ...
        match camera.step(2, &auth, &mut b) {
            ...
            Ok(HttpCameraStep::FrameReady) => {
                assert_eq!(
                    PreparedHttpCompletion::from_camera(&camera, &archive, &mut w).err(),
                    Some(HttpCompletionError::NotReady)
                );
                ...
                keys.push(http_rgb_exposure(&f, &mut w)?);
            }
            Ok(HttpCameraStep::Complete)
            | Err(HttpCameraError::Http(_) | HttpCameraError::Multipart(_)) => {
...
fn access<'a, 'cx>(
    ...
    w: &'a mut WorkBudget<'cx>,
    b: &'a mut DecodeBudget<'cx>,
    ...
    for _ in 0..100_000 {
        match r.step(access(p, &NeverCancel, &mut w, &mut b))? {
            HttpReplayStep::FrameReady => {
                let receipt = r
                    .pending_frame()
                    .ok_or("replay frame lost")?
                    .part()
                    .receipt();
                let f = r.take_frame(
                    receipt.ordinal,
                    receipt.encoded_sha256,
                    access(p, &NeverCancel, &mut w, &mut b),
                )?;
                assert_eq!(f.part().bytes(), JPEG);
                keys.push(http_rgb_exposure(&f, &mut w)?);
            }
...
fn explicit_length_and_chunk_end_records_match_replayed_native_accounting() -> Test {
    for mode in [0, 1] {
        ...
        plan.publish(&archive, &mut p, &NeverCancel, &mut work())?;
        let proof =
            VerifiedHttpCompletion::load(&p, &archive, plan.pin(), &NeverCancel, &mut work())?;
        assert_eq!(
            proof.http_end().termination,
        ...
        assert_eq!(
            ...
            Err(HttpCompletionError::NotReady)
        );
        let mut keys = Vec::new();
        assert_eq!(drain(&mut r, &p, &mut keys)?, HttpReplayStep::Complete);
        assert_eq!(keys, live);
...
fn terminal_retry_and_lost_return_reuse_exact_root_and_source_closure() -> Test {
    ...
    drop(p);
    let mut p = d.open()?;
    let a = load(&p, pin.wire)?;
    assert_eq!(
        VerifiedHttpCompletion::load(&p, &a, pin, &NeverCancel, &mut work())?.pin(),
        pin
    );
    assert_eq!(
        plan.publish(&a, &mut p, &NeverCancel, &mut work())?.outcome,
        PublishOutcome::AlreadyPublished
    );
    Ok(())
}
...
fn every_publication_cut_has_honest_cold_recovery() -> Test {
    for cut in [
        ...
        assert!(plan.publish(&a, &mut p, &NeverCancel, &mut work()).is_err());
...
fn cancelled_or_underfunded_terminal_publication_does_not_advance_roots() -> Test {
    ...
    assert_eq!(
        plan.publish(&a, &mut p, &Stop, &mut work()).err(),
        Some(HttpCompletionError::Cancelled)
    );
    assert!(
        plan.publish(&a, &mut p, &NeverCancel, &mut WorkBudget::new(0))
            .is_err()
    );
...
fn stale_completion_or_different_source_prefix_is_not_admitted() -> Test {
    ...
    let wrong = HttpCompletionPin {
        wire: HttpWirePin {
            bytes: pin.wire.bytes - 1,
            ..pin.wire
        },
        ..pin
    };
    assert_eq!(
        VerifiedHttpCompletion::load(&p, &a, wrong, &NeverCancel, &mut work()).err(),
        Some(HttpCompletionError::Mismatch)
    );
    let wrong = HttpCompletionPin {
        root: fss_core::ContentDigest::sha256(b"not that completion"),
        ..pin
    };
    assert_eq!(
        VerifiedHttpCompletion::load(&p, &a, wrong, &NeverCancel, &mut work()).err(),
        Some(HttpCompletionError::NotDurable)
    );
    Ok(())
}
#[test]
fn previously_verified_end_cannot_override_corrupted_originals_or_current_revocation() -> Test {
    ...
    assert_eq!(
        drain(&mut r, &p, &mut Vec::new())?,
        HttpReplayStep::PrefixExhausted
    );
    let before = r.position();
    assert_eq!(
        r.finish_completed(&proof, access(&p, &Stop, &mut work(), &mut framing())),
    ...
    assert!(matches!(
        ...
        Err(HttpCompletionError::Archive(_))
    ));
    ...
    assert!(matches!(
        r.finish_completed(
            ...
            access(
                ...
                &NeverCancel,
                &mut work(),
                &mut DecodeBudget::cancellable(WORK, &cancelled)
            )
        ),
...
fn operator_checker_consumes_actual_terminal_witness_and_decodes_every_cold_frame() -> Test {
    ...
    use fss_reference::ingest::http_replay::check::{
        HttpCheckDecode, HttpCheckLimits, HttpCheckRequest, HttpCheckSource, HttpCheckStatus,
    ...
    for mode in [0, 1, 2] {
        ...
        let p = d.open()?;
        let s = scope();
        let digest = |b| ContentDigest::new(DigestAlgorithm::Sha256, b);
        let request = HttpCheckRequest {
            source: HttpCheckSource {
                source: digest(s.stream.source),
                generation: s.stream.generation,
                receive_clock: digest(s.receive_clock),
                retention_evidence: digest(s.retention_evidence),
            },
            head: pin.wire.head,
            reads: pin.wire.reads,
            bytes: pin.wire.bytes,
        ...
        assert!(
            report
                .frames
                .iter()
                .all(|f| f.dimensions == Some([17, 13]) && f.luma.is_some())
        );
