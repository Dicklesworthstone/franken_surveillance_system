...
fn drive_ready(w: &mut RecordingArchiveWriter<'_>, now: &mut u64) -> TestResult {
    for _ in 0..80 {
        let out = w.step(*now, &NeverCancel)?;
        *now += 1;
        if matches!(out, ArchiveWriteProgress::Ready { .. }) {
            return Ok(());
        }
    }
...
fn drive_finished(w: &mut RecordingArchiveWriter<'_>, now: &mut u64) -> TestResult {
    for _ in 0..80 {
        let out = w.step(*now, &NeverCancel)?;
        *now += 1;
        if matches!(out, ArchiveWriteProgress::Finished { .. }) {
            return Ok(());
        }
    }
...
fn must_refuse(
    value: Result<ArchiveAdmission, ArchiveWriteRefusal>,
) -> Result<ArchiveWriteRefusal, Error> {
    match value {
        Err(refusal) => Ok(refusal),
        Ok(_) => Err("unexpected archive admission".into()),
    }
}
struct Stop;
...
fn automatic_full_pages_and_final_partial_page_survive_restart_and_query() -> TestResult {
    let (path, mut p) = owner("writer_pages")?;
    let ns = namespace()?;
    let mut now = 0;
    ...
    assert_eq!(w.snapshot().indexed_windows(), 2);
    w.finish();
    drive_finished(&mut w, &mut now)?;
    assert!(matches!(
        w.step(now, &NeverCancel)?,
        ArchiveWriteProgress::Exhausted
    ));
    let retired = w.retire();
    assert!(retired.pending.is_none());
    assert_eq!(retired.snapshot.pages().len(), 2);
    assert_eq!(retired.snapshot.indexed_windows(), 3);
    ...
    for i in 0..3 {
        assert!(
            matches!(read.step(i, &NeverCancel)?, ArchiveReadProgress::Window { ordinal, .. } if ordinal == i as usize)
        );
    }
    assert!(
        matches!(read.step(3, &NeverCancel)?, ArchiveReadProgress::Complete(r) if r.windows == 3 && r.unindexed.is_empty())
    );
    Ok(())
}
#[test]
fn backpressure_returns_exact_unconsumed_recording_without_advancing_ordinal() -> TestResult {
    let (_, mut p) = owner("writer_pressure")?;
    let mut now = 0;
    let mut w = RecordingArchiveWriter::open(
        &mut p,
        namespace()?,
        ArchiveLimits::default(),
        0,
    ...
    let a = window(1, 0)?;
    let b = window(3, 3600)?;
    let b_root = b.manifest().root();
    let bytes = a.byte_len();
    w.offer(a, bytes, now)?;
    let bytes = b.byte_len();
    let refusal = must_refuse(w.offer(b, bytes, now))?;
    assert!(matches!(*refusal.reason, ArchiveError::Backpressure));
    assert_eq!(refusal.recording.manifest().root(), b_root);
    drive_ready(&mut w, &mut now)?;
...
fn cancellation_during_child_staging_retains_exact_plan_for_explicit_retry() -> TestResult {
    ...
    let a = window(1, 0)?;
    let root = a.manifest().root();
    let bytes = a.byte_len();
    w.offer(a, bytes, 0)?;
    assert!(w.step(0, &AfterCalls(Cell::new(0))).is_err());
    assert_eq!(
        w.pending()
            .ok_or("pending originals lost")?
            .manifest()
            .root(),
        root
    );
    ...
    w.retry(1, 1000)?;
    assert!(
        matches!(w.step(1, &NeverCancel)?, ArchiveWriteProgress::WindowDurable { ordinal: 0, receipt } if receipt.root == root)
    );
...
fn durable_tail_is_indexed_on_restart_before_new_capture_is_admitted() -> TestResult {
    let (path, mut p) = owner("writer_tail_restart")?;
    let ns = namespace()?;
    {
        let mut w = RecordingArchiveWriter::open(
            &mut p,
        ...
        let retained = w.retire();
        assert_eq!(retained.snapshot.unindexed_windows().len(), 1);
    }
    drop(p);
    let mut p = LocalRootPublisher::open(path, owner_limits())?;
    let mut w =
        RecordingArchiveWriter::open(&mut p, ns, ArchiveLimits::default(), 0, 1000, &NeverCancel)?;
    let b = window(3, 3600)?;
    let bytes = b.byte_len();
...
fn ambiguous_window_rename_keeps_originals_and_reopen_does_not_duplicate_them() -> TestResult {
    let (path, mut p) = owner("writer_window_crash")?;
    let ns = namespace()?;
    ...
    assert!(w.retry(1, 1000).is_err());
    let retained = w.retire();
    assert_eq!(
        retained
            .pending
            .as_ref()
            .ok_or("lost originals")?
            .manifest()
            .root(),
        root
    );
    ...
    drive_ready(&mut w, &mut now)?;
    assert_eq!(w.snapshot().indexed_windows(), 1);
    assert_eq!(w.snapshot().windows()[0].root(), root);
    let refusal = must_refuse(w.offer(retained.pending.ok_or("pending")?, bytes, now))?;
    assert!(matches!(*refusal.reason, ArchiveError::Duplicate));
...
fn every_catalog_crash_cutpoint_preserves_durable_windows_and_recovery_distinctions() -> TestResult
{
    ...
    .enumerate()
    {
        let (path, mut p) = owner(&format!("writer_page_crash_{index}"))?;
        let ns = namespace()?;
        let a = window(1, 0)?;
        ...
        for now in 0..12 {
            if w.step(now, &NeverCancel).is_err() {
                failed = true;
                break;
            }
        }
        assert!(failed);
        let retained = w.retire();
        assert_eq!(retained.snapshot.windows().len(), 1);
        assert_eq!(retained.snapshot.indexed_windows(), 0);
        let expected_page = retained
            .prepared_page
            .ok_or("lost immutable page")?
            .manifest()
            .root();
        drop(p);
        let mut p = LocalRootPublisher::open(path, owner_limits())?;
        if cut == PublishCutPoint::AfterRootTempWrite {
            assert!(matches!(
                ArchiveSnapshot::load(&p, ns, ArchiveLimits::default(), &NeverCancel),
                Err(ArchiveError::RecoveryRequired)
            ));
            continue;
        }
        let mut recovered = RecordingArchiveWriter::open(
            &mut p,
            ns,
            ArchiveLimits::default(),
            0,
        ...
        let mut now = 0;
        drive_ready(&mut recovered, &mut now)?;
        assert_eq!(recovered.snapshot().indexed_windows(), 1);
        assert_eq!(
            recovered.snapshot().pages()[0].catalog().manifest().root(),
            expected_page
        );
...
fn deadline_and_clock_reversal_do_not_consume_pending_media() -> TestResult {
    ...
    w.offer(a, bytes, 5)?;
    assert!(matches!(
        w.step(4, &NeverCancel),
        Err(ArchiveError::ClockReversed)
    ));
    assert!(matches!(
        w.step(10, &NeverCancel),
        Err(ArchiveError::Deadline)
    ));
    assert_eq!(w.pending().ok_or("pending")?.manifest().root(), root);
    w.retry(11, 100)?;
    assert!(
        matches!(w.step(11, &NeverCancel)?, ArchiveWriteProgress::WindowDurable { receipt, .. } if receipt.root == root)
    );
    assert!(w.retire().pending.is_none());
    Ok(())
}
...
fn capacity_and_reservation_refusals_never_evict_old_evidence() -> TestResult {
    ...
    assert!(matches!(*refused.reason, ArchiveError::Limit));
    assert!(w.snapshot().windows().is_empty());
    w.offer(*refused.recording, bytes, 0)?;
    let mut now = 0;
    drive_ready(&mut w, &mut now)?;
    let b = window(3, 3600)?;
    let bytes = b.byte_len();
    assert!(matches!(
        *must_refuse(w.offer(b, bytes, now))?.reason,
        ArchiveError::Limit
    ));
    w.finish();
    drive_finished(&mut w, &mut now)?;
...
fn corrupt_namespace_root_is_not_reinterpreted_as_a_shorter_archive() -> TestResult {
    let (path, mut p) = owner("writer_corrupt_root")?;
    let ns = namespace()?;
    let a = window(1, 0)?;
    let slot = ns.window_slot(0)?;
    publish_window(&mut p, &slot, &a)?;
    drop(p);
    std::fs::write(
        path.join("roots").join(format!("{slot}.root")),
        b"corrupt fixture root record",
    )?;
    let p = LocalRootPublisher::open(path, owner_limits())?;
    assert!(matches!(
        ArchiveSnapshot::load(&p, ns, ArchiveLimits::default(), &NeverCancel),
