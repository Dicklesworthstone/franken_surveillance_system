...
use fss_reference::rtsp::hevc_client::{
    HevcClientPoll,
    pictures::{HevcPictureClientPoll as Event, RtspHevcPictureClient},
};
use fss_reference::rtsp::hevc_recording_capture::{
    CaptureError, HevcCapturePoll as P, HevcRecordingCapture, MAX_PENDING_EVENT_AGE_NS,
    TimedHevcCapture,
};
...
fn config() -> ClientConfig {
    ClientConfig {
        presentation_uri: "rtsp://fixture.local/live/".into(),
        control_root_uri: "rtsp://fixture.local/live".into(),
        media_index: 0,
        channels: (0, 1),
...
fn waiting(event: &Event, now: u64) -> bool {
    matches!(event, Event::Pending { wake_at_ns } if wake_at_ns.is_none_or(|at| at > now))
        || matches!(event, Event::Client { event, .. }
...
fn client(digest: bool) -> Result<RtspHevcPictureClient, Error> {
    let mut client = if digest {
        RtspHevcPictureClient::with_digest(
            config(),
            KEY,
    ...
    control(
        &mut client,
        &response(cseq, &[("Content-Type", "application/sdp")], sdp),
        3,
    )?;
    let setup = if digest {
        client.request_digest(C::Setup, &credentials, [3; 16], 4)?
    } else {
        client.request(C::Setup, 4)?
    };
    control(
        &mut client,
        &response(
    ...
    let play = if digest {
        ...
        client.request(C::Play, 6)?
    };
    control(
        &mut client,
        &response(play.cseq(), &[("Session", "test-session")], &[]),
        7,
    )?;
    Ok(client)
}
...
impl Harness {
    fn new(digest: bool, limits: CollectorLimits) -> Result<Self, Error> {
        let collector =
            HevcRecordingCollector::new(scope()?, KEY, PT, configuration()?, 90_000, limits)?;
        let mut h = Self {
            client: client(digest)?,
            capture: HevcRecordingCapture::new(collector),
            output: Vec::new(),
            timed: Vec::new(),
        };
        h.feed(
            &framed(0, &packet(0, 0, false, &nals()?[0]).bytes),
            10,
            true,
        )?; // probation, not recording source
        Ok(h)
    }
    ...
    fn pump(&mut self, now: u64, automatic_timing: bool) -> Result<(), Error> {
        for _ in 0..1000 {
            let event = self.client.poll(now)?;
            let stop = waiting(&event, now);
            self.capture.offer(event, now)?;
            if self.drain(now, automatic_timing)? || stop {
                return Ok(());
            }
        }
    ...
    fn drain(&mut self, now: u64, automatic_timing: bool) -> Result<bool, Error> {
        for _ in 0..1000 {
            ...
            let blocked = matches!(
                &output,
                P::TimingRequired(_) | P::Backpressure { .. } | P::Stopped { .. } | P::Ended { .. }
            );
...
#[test]
fn plain_and_digest_tcp_streams_collect_identical_media_without_changing_authentication()
-> TestResult {
    let mut roots = Vec::new();
    for digest in [false, true] {
        let mut h = Harness::new(digest, CollectorLimits::default())?;
        for (i, p) in input()?.iter().enumerate() {
            h.feed(&framed(0, &p.bytes), 20 + i as u64, true)?;
        }
        h.client.finish();
        h.pump(40, true)?;
        let windows = h.windows();
        assert_eq!(windows.len(), 2);
        assert_eq!(h.timed.len(), 4);
        for w in &windows {
        ...
        roots.push(
            windows
                .iter()
                .map(|w| w.manifest().root())
                .collect::<Vec<_>>(),
        );
        assert!(matches!(
            h.output.last(),
            Some(P::Ended { retained: Some(_) })
        ));
...
fn timing_is_explicit_retryable_and_does_not_refresh_the_held_picture_deadline() -> TestResult {
    ...
    for (i, p) in input()?[..5].iter().enumerate() {
        h.feed(&framed(0, &p.bytes), 20 + i as u64, false)?;
    }
    let deadline = h.capture.next_wake_ns();
    let mut wrong = timings(1)[0];
    wrong.duration = 0;
    assert!(matches!(
        h.capture.supply_timing(wrong, 25),
        Err(CaptureError::Collection(E::Timeline))
    ));
    assert_eq!(h.capture.next_wake_ns(), deadline);
    assert!(
        matches!(h.capture.poll(25)?, P::TimingRequired(request) if request.idr && request.rtp_timestamp == 0)
    );
...
fn expired_timing_returns_the_picture_and_all_originals_without_an_eof_seal() -> TestResult {
    ...
    assert!(matches!(
        h.capture.supply_timing(timings(1)[0], at),
        Err(CaptureError::Collection(E::Deadline))
    ));
    let P::Stopped {
        reason, retained, ..
    } = h.capture.poll(at)?
    else {
        return Err("timing did not expire".into());
    };
...
fn packet_gap_is_fenced_before_seal_and_preserves_the_invalidating_event() -> TestResult {
    ...
    let P::Stopped {
        ...
        return Err("gap not fenced".into());
    };
...
fn incomplete_fu_eof_cannot_flush_the_preceding_pending_recording() -> TestResult {
    ...
    let Some(P::Stopped { retained, .. }) = h.output.last() else {
        return Err("incomplete EOF not fenced".into());
    };
...
fn source_capacity_holds_exact_event_without_advancing_or_busy_polling() -> TestResult {
    ...
    assert_eq!(
        h.capture.next_wake_ns(),
        Some(21 + MAX_PENDING_EVENT_AGE_NS)
    );
    ...
    assert!(
        matches!(retired.event.as_deref(), Some(Event::Source { source, .. }) if source.bytes() == input[1].bytes)
    );
...
fn eob_picture_keeps_remote_uncertainty_and_seals_only_after_explicit_timing() -> TestResult {
    ...
    assert!(
        matches!(h.timed.last().ok_or("EOB picture")?.event.as_ref(),
