...
use fss_reference::rtsp::datagram_archive::{DatagramArchive, DatagramArchiveLimits, DatagramScope};
...
use fss_reference::rtsp::recording_recipe::{RecordingRecipe, RecordingRecipeLimits, RecordingTimingDecision};
use fss_reference::rtsp::recording_recipe::storage::{PreparedRecordingRecipe, RecordingRecipePin, RecipeStorageLimits};
use fss_reference::rtsp::recording_recipe::storage::operation::*;

pub type Test<T = ()> = Result<T, Box<dyn std::error::Error>>;
static NEXT: AtomicU64 = AtomicU64::new(0);
pub struct Fixture { pub path: PathBuf, pub pin: RecordingRecipePin, pub expected: Vec<ContentDigest> }
impl Drop for Fixture { fn drop(&mut self) { let _ = std::fs::remove_dir_all(&self.path); } }
pub struct Clock(pub u64);
impl ReconstructionClock for Clock { fn now_ns(&self) -> Option<u64> { Some(self.0) } }
pub fn work() -> WorkBudget<'static> { WorkBudget::new(1_000_000_000_000) }
pub fn bounds() -> AvcReplayBounds { AvcReplayBounds { max_source_bytes: 1_048_576, max_steps: 4096, deadline_ns: u64::MAX } }
pub fn storage() -> LocalPublicationLimits {
    LocalPublicationLimits::new(256, 1024, 256, 1024, SpoolLimits::new(2048, 32 * 1024 * 1024, 1_048_576, 2048))
}
pub fn source_scope() -> Test<DatagramScope> {
    Ok(DatagramScope { binding: TcpBinding::new(source::KEY, "127.0.0.1:554".parse()?, "camera.local",
        TcpSecurityPolicy::OwnerApprovedPlaintext)?, channels: (0, 1),
...
pub fn limits() -> RecipeLoadLimits {
    RecipeLoadLimits { source: DatagramArchiveLimits { max_datagrams: 128, max_payload_bytes: 1_048_576,
        max_scan_roots: 1024, max_spool_object_bytes: 1_048_576 },
        recipe: RecordingRecipeLimits::default(), storage: RecipeStorageLimits::default() }
}
pub fn select(f: &Fixture) -> Test<RecipeSelection> {
    Ok(RecipeSelection { recipe: f.pin.recipe, root: f.pin.root, scope: source_scope()? })
}
pub fn load(f: &Fixture, p: &LocalRootPublisher) -> Test<LoadedRecordingRecipe> {
    Ok(LoadedRecordingRecipe::load(p, select(f)?, limits(), &NeverCancel, &mut work())?)
}
pub fn open(path: &Path) -> Test<LocalRootPublisher> { Ok(LocalRootPublisher::open(path, storage())?) }
pub fn seed(name: &str, pictures: usize, bad_last: bool, fragment_only: bool) -> Test<Fixture> {
    let path = std::env::temp_dir().join(format!("fss-reconstruction-{name}-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
    let mut p = open(&path)?;
    ...
    if fragment_only {
        ...
        for i in 0..pictures {
            ...
            packet[1] |= 128; observations.push((11 + i as u64, packet));
        }
    }
    ...
    let avc = AvcReplaySpec { payload_type: 96, mode: H264Mode::NonInterleaved, sps, pps,
        limits: AvcReceiveLimits::default(), reduced_rtcp: false,
        configuration_evidence: ContentDigest::sha256(b"operator accepted native fixture parameters") };
    let recording = RecordingReplaySpec { scope: RecordingScope {
        sensor: SensorId::parse("reconstruction-sensor")?, stream: StreamId::parse("reconstruction-video")?,
        generation: source::KEY.generation, anchor: ContentDigest::sha256(b"operation history anchor"),
    ...
    let mut replay = DatagramRecordingReplay::new(&archive, avc, recording.clone(), bounds(), 1000)?;
    ...
    assert!(finished);
    if bad_last { decisions.last_mut().ok_or("no timing to mutate")?.picture.rtp_timestamp += 1; }
    let recipe = RecordingRecipe::new(&archive, avc, recording, decisions, limits().recipe)?;
    let prepared = PreparedRecordingRecipe::prepare(&recipe, &archive, &p, limits().storage, &NeverCancel, &mut work())?;
