...
use fss_reference::ingest::http_replay::{
    HttpReplayAccess, HttpReplayLimits, HttpReplayStep, HttpWireReplay,
};
...
fn every_live_result_is_ledgered_before_transfer_and_replays_after_all_owners_close() -> Test {
    ...
    let (capture, camera, mut server) = session(
        model.model(),
        &head,
        &mut owner,
        response(&[jpeg(240), jpeg(240)], true),
        257,
    )?;
    ...
    let mut r =
        HttpRgbEvidenceRecording::attach(recording, limits()).map_err(|_| "evidence attachment")?;
    ...
    for n in 1..=2 {
        ...
        assert_eq!(
            pin.archive.capture,
            r.accepted_admission().ok_or("admission")?.source().capture
        );
        ...
        assert_eq!(
            b.copy.used(),
            copy_used,
            "exact prepare retry must not re-encode"
        );
        assert!(matches!(
            r.take_result(
                pin,
        ...
        assert!(matches!(
            ...
            Err(HttpRgbEvidenceError::Mismatch)
        ));
        let committed = r.commit_evidence(
            pin,
            &publisher,
            &mut deployment,
            auth.access(&camera),
    ...
    let HttpRgbRecordingStep::CompletionPrepared(completion) =
        ...
        return Err("native terminal missing".into());
    };
    r.commit_completion(completion, &mut publisher, camera.access(&NeverCancel))?;
    let source_scope = r.recording().scope();
    assert_eq!(
        pins[0].encoded, pins[1].encoded,
        "identical JPEG bytes must not alias exposures"
    );
    ...
    let original = HttpWireArchive::load(
        &publisher,
        source_scope,
        completion.wire,
        limits().source,
    ...
    let mut cursor = HttpWireReplay::new(
        &original,
        completion.wire,
        HttpReplayLimits {
    ...
    for (index, pin) in pins.iter().copied().enumerate() {
        ...
        assert!(matches!(
            ...
            Err(HttpRgbEvidenceReplayError::Mismatch)
        ));
        let mut wrong = pin;
        ...
        assert!(matches!(
            ...
            Err(HttpRgbEvidenceReplayError::Denied)
        ));
        auth.reads.set(true);
        assert_eq!(
            b.decoder.used(),
            decoded_before,
            "restore/source refusals cannot execute a decoder"
        );
        ...
        assert_eq!(
            b.decoder.used(),
            decoded_before,
            "restored bytes alone are not executed inference"
        );
        auth.reads.set(false);
        assert!(matches!(
            restored.replay(
        ...
        if index == 0 {
            ...
            let wrong_restored = restore_http_rgb_evidence(
                wrong_result,
                source,
                &mut deployment,
                &auth,
        ...
        if cold_owner.is_none() {
            ...
            assert!(matches!(
                replayed.verify_temporal(cold_owner.as_ref().ok_or("cold tracker")?),
                Err(HttpRgbEvidenceReplayError::TemporalPending)
            ));
        } else {
        ...
        assert_eq!(replayed.verify_temporal(cold)?.pin(), pin);
        assert_eq!(
            cold.tracker().exposure_count(),
            count,
            "verification never re-assimilates an exposure"
        );
        let frame = cursor.take_frame(
            pin.ordinal,
            pin.encoded,
            HttpReplayAccess {
                publisher: &publisher,
        ...
        assert_eq!(
            ContentDigest::sha256(frame.part().bytes()).bytes(),
            pin.encoded
        );
    }
    let mut ended = false;
    ...
    assert_eq!(
        cold_owner.ok_or("cold tracker")?.tracker().exposure_count(),
        2
    );
    assert_eq!(
        deployment.current_anchor(),
        &anchor,
        "cold reconstruction is read-only"
    );
    Ok(())
}
...
fn write_and_read_revocation_keep_exact_prepared_and_native_ownership() -> Test {
    ...
    let (e, replay) = evidence(&r, &model, &head, privacy.mask(), &mut b, &cx)?;
    let mut deployment =
        ReferenceDeployment::open(&directory.0.join("derived"), "site:http-rgb-evidence", &cx)?;
    let auth = ArchiveAuthority::new();
    ...
    assert!(matches!(
        ...
        Err(HttpRgbEvidenceError::Archive(RgbArchiveError::Denied))
    ));
    assert!(r.prepared().is_none());
...
fn lost_ledger_after_publication_blocks_delivery_until_exact_key_recovery() -> Test {
    ...
    assert!(matches!(
        ...
        Err(HttpRgbEvidenceError::Archive(RgbArchiveError::NotCommitted))
    ));
    assert_eq!(r.recording().work().transferred, 0);
    assert_eq!(r.prepared(), Some(pin));
    assert_eq!(r.recording().capture().completion(), Some(result));
