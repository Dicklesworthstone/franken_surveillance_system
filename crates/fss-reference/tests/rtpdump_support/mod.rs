...
use fss_reference::{ADP_REPLAY_ROW_ID, ReplayCx};
...
pub fn cx() -> Result<ReplayCx, Error> {
    let auth = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:rtpdump-test".into(),
        operation_id: OperationId::parse("operation:rtpdump-test")?,
        principal: "operator:rtpdump-test".into(),
        ...
        anchor_universe: ContentDigest::sha256(b"rtpdump-tests"),
        generation: 1,
    })?;
...
pub fn rtp(seq: u16, marker: bool, payload: &[u8]) -> Vec<u8> {
    ...
    b.extend_from_slice(&90000_u32.to_be_bytes());
...
pub fn dump(packets: &[(u16, bool, &[u8], u32)]) -> Vec<u8> {
    ...
    for (seq, marker, payload, offset) in packets {
        let wire = rtp(*seq, *marker, payload);
        record(&mut b, &wire, wire.len() as u16, *offset);
    }
    b
}
...
pub fn nals() -> Vec<&'static [u8]> {
    ...
    let mut at = 0;
    while at + 3 <= b.len() {
        let prefix = if b.get(at..at + 4) == Some(&[0, 0, 0, 1]) {
            4
        } else if b[at..at + 3] == [0, 0, 1] {
            3
        } else {
            0
        };
        if prefix > 0 {
            starts.push((at, at + prefix));
            at += prefix;
        } else {
            at += 1;
        }
...
pub fn real_dump(fragmented: bool) -> Vec<u8> {
    ...
    let priming = rtp(seq, false, &[0x09, 0xf0]);
    record(&mut b, &priming, priming.len() as u16, 0);
    ...
    for (i, n) in nals().iter().enumerate() {
        if fragmented && matches!(n[0] & 31, 1 | 5) {
            ...
            for (j, bytes) in [&n[1..mid], &n[mid..]].into_iter().enumerate() {
                let mut payload = vec![
                    (n[0] & 0x60) | 28,
                    (n[0] & 31) | if j == 0 { 128 } else { 64 },
                ];
                payload.extend_from_slice(bytes);
                let wire = rtp(seq, j == 1, &payload);
                record(&mut b, &wire, wire.len() as u16, i as u32);
                seq = seq.wrapping_add(1);
            }
        } else {
            let wire = rtp(seq, matches!(n[0] & 31, 1 | 5), n);
            record(&mut b, &wire, wire.len() as u16, i as u32);
            seq = seq.wrapping_add(1);
        }
    }
