...
use fss_reference::rtsp::recording_archive::{ArchiveLimits, ArchiveNamespace, ArchiveRetirement,
...
fn fresh(name: &str) -> Result<PathBuf, Error> {
    let path = Path::new(env!("CARGO_TARGET_TMPDIR")).join("archive_work_checkpoint").join(name);
...
fn namespace() -> Result<ArchiveNamespace, Error> {
    Ok(ArchiveNamespace::new(CatalogScope { recording: scope()?,
        decode_clock: ContentDigest::sha256(b"explicit recording decode clock"), time_scale: 90_000 })?)
}
...
fn pending(p: &mut LocalRootPublisher) -> Result<ArchiveRetirement, Error> {
    ...
    let recording = window(0)?; let bytes = recording.byte_len();
    w.offer(recording, bytes, 0)?;
    Ok(w.retire())
}
fn save(p: &mut LocalRootPublisher, work: &ArchiveRetirement) -> Result<(SlotName, ContentDigest), Error> {
...
fn drain(p: &mut LocalRootPublisher, work: ArchiveRetirement) -> Result<usize, Error> {
    ...
    let mut resume = RecordingArchiveResume::open(p, work, ArchiveResumeConfig {
        expected_retirement_digest: digest, max_window_bytes: 1024 * 1024,
        max_steps: 100, deadline_ns: 1000,
    }, 2, &NeverCancel)?;
    for _ in 0..100 {
        if let ArchiveResumeProgress::Archive(ArchiveWriteProgress::Finished { windows, .. }) = resume.step(2, &NeverCancel)? {
            return Ok(windows);
        }
    }
...
fn publish_window(p: &mut LocalRootPublisher, w: &PreparedRecording, ordinal: usize) -> Test {
    let mut job = RecordingPublication::new(w, p, namespace()?.window_slot(ordinal)?, w.byte_len(), 1000)?;
    for _ in 0..5 { if matches!(job.step(2, &NeverCancel)?, RecordingProgress::Published(_)) { return Ok(()); } }
...
fn cold_restart_recovers_original_bytes_without_original_rust_objects() -> Test {
    ...
    let (pin, expected, source, media, index, old_digest) = {
        ...
        let digests = (ContentDigest::sha256(objects.source), ContentDigest::sha256(objects.media), ContentDigest::sha256(objects.index));
        let digest = archive_retirement_digest(&work)?;
    ...
    let mut p = LocalRootPublisher::open(&path, storage_limits())?;
    let work = load_archive_work(&p, &pin.0, pin.1, ArchiveWorkLimits::default(), &NeverCancel)?;
...
fn old_prepared_page_and_next_unoffered_window_both_survive_cold_restart() -> Test {
    ...
    let (pin, first, second, page_root) = {
        ...
        let first = window(0)?; let root = first.manifest().root(); let bytes = first.byte_len();
        w.offer(first, bytes, 0)?;
        assert!(matches!(w.step(1, &NeverCancel)?, ArchiveWriteProgress::WindowDurable { .. }));
        w.flush();
        for _ in 0..3 { w.step(1, &NeverCancel)?; }
        let mut work = w.retire();
        let page_root = work.prepared_page.as_ref().ok_or("page missing")?.manifest().root();
        let next = window(3600)?; let second = next.manifest().root(); work.pending = Some(next);
        (save(&mut p, &work)?, root, second, page_root)
    };
    let mut p = LocalRootPublisher::open(&path, storage_limits())?;
    let work = load_archive_work(&p, &pin.0, pin.1, ArchiveWorkLimits::default(), &NeverCancel)?;
    assert_eq!(work.prepared_page.as_ref().ok_or("lost page")?.manifest().root(), page_root);
    assert_eq!(drain(&mut p, work)?, 2);
    let snapshot = ArchiveSnapshot::load(&p, namespace()?, limits(), &NeverCancel)?;
    assert_eq!(snapshot.windows().iter().map(|w| w.root()).collect::<Vec<_>>(), vec![first, second]);
...
fn repeated_save_does_not_allocate_an_archive_ordinal_or_duplicate_custody_roots() -> Test {
    ...
    assert_eq!(plan.publish(&mut p, 2, 1000, &NeverCancel)?.outcome, PublishOutcome::AlreadyPublished);
...
fn window_committed_after_checkpoint_is_reconciled_without_a_second_ordinal() -> Test {
    ...
    let pin = {
        let mut p = LocalRootPublisher::open(&path, storage_limits())?;
        let work = pending(&mut p)?; let pin = save(&mut p, &work)?;
    ...
    let mut resume = RecordingArchiveResume::open(&mut p, work, ArchiveResumeConfig {
        expected_retirement_digest: digest, max_window_bytes: 1024 * 1024, max_steps: 100, deadline_ns: 1000,
    }, 2, &NeverCancel)?;
    assert!(matches!(resume.reconciliation().window, RetiredPublicationState::AlreadyDurable(_)));
    for _ in 0..100 {
...
fn each_root_cut_distinguishes_unpublished_work_from_a_lost_final_ack() -> Test {
    for (i, cut) in [PublishCutPoint::AfterChildrenVerified, PublishCutPoint::AfterManifestBody,
        ...
        let pin = {
            let mut p = LocalRootPublisher::open(&path, storage_limits())?;
            let work = RecordingArchiveWriter::open(&mut p, namespace()?, limits(), 0, 1000, &NeverCancel)?.retire();
            let plan = PreparedArchiveWork::prepare(&work, &p, ArchiveWorkLimits::default(), &NeverCancel)?;
            let pin = (plan.slot().clone(), plan.root());
        ...
        let result = load_archive_work(&p, &pin.0, pin.1, ArchiveWorkLimits::default(), &NeverCancel);
        if cut == PublishCutPoint::AfterRootRename { assert!(result.is_ok()); }
...
fn cancellation_and_resource_refusals_preserve_work_without_publishing() -> Test {
    ...
    let work = pending(&mut p)?; let root = work.pending.as_ref().ok_or("missing window")?.manifest().root();
    for bounds in [ArchiveWorkLimits { max_pending_bytes: 0, ..ArchiveWorkLimits::default() },
        ArchiveWorkLimits { max_graph_objects: 1, ..ArchiveWorkLimits::default() },
    ...
    assert_eq!(work.pending.as_ref().ok_or("lost window")?.manifest().root(), root);
    Ok(())
}
...
fn current_unaccounted_extra_recordings_are_not_silently_adopted() -> Test {
    ...
    assert!(load_archive_work(&p, &pin.0, pin.1, ArchiveWorkLimits::default(), &NeverCancel).is_err());
    Ok(())
}
...
fn wrong_pin_tighter_external_limits_and_revoked_read_are_refused() -> Test {
    ...
    let work = pending(&mut p)?; let pin = save(&mut p, &work)?;
    assert!(load_archive_work(&p, &pin.0, ContentDigest::sha256(b"wrong pin"), ArchiveWorkLimits::default(), &NeverCancel).is_err());
    let mut bounds = ArchiveWorkLimits::default(); bounds.archive.max_windows = 1;
...
fn corrupt_checkpointed_source_is_not_resurrected_or_replaced() -> Test {
    ...
    let object = path.join("spool").join("objects").join(text.strip_prefix("sha256:").ok_or("algorithm")?);
