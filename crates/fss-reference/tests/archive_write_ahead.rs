...
use fss_reference::rtsp::recording_archive::{
    ArchiveError, ArchiveLimits, ArchiveNamespace, ArchiveWriteProgress, RecordingArchiveWriter,
};
...
fn namespace() -> Test<ArchiveNamespace> {
    Ok(ArchiveNamespace::new(CatalogScope {
        recording: scope()?,
        decode_clock: ContentDigest::sha256(b"write-ahead test clock"),
        time_scale: 90_000,
    })?)
}
...
fn open(p: &mut LocalRootPublisher) -> Test<CheckpointedArchiveWriter<'_>> {
    Ok(CheckpointedArchiveWriter::open(
        p,
        namespace()?,
        limits(),
    ...
    for _ in 0..100 {
        match w.step(2, &NeverCancel)? {
            ...
            CheckpointedArchiveProgress::WorkDurable {
                ...
                assert_eq!(checkpoint.root(), receipt.root);
                checkpoints.push(checkpoint);
            }
            CheckpointedArchiveProgress::Archive(
                ArchiveWriteProgress::Ready { .. } | ArchiveWriteProgress::Finished { .. },
            ) => return Ok(checkpoints),
            CheckpointedArchiveProgress::Archive(ArchiveWriteProgress::Exhausted) => {
                return Err("unexpected exhaustion".into());
            }
            _ => {}
        }
    }
...
fn prepare_page(w: &mut CheckpointedArchiveWriter<'_>) -> Test {
    assert!(matches!(
        w.step(2, &NeverCancel)?,
        CheckpointedArchiveProgress::Archive(ArchiveWriteProgress::PageStarted { .. })
    ));
    assert!(matches!(
        w.step(2, &NeverCancel)?,
        CheckpointedArchiveProgress::Archive(ArchiveWriteProgress::PageWindowVerified { .. })
    ));
    assert!(matches!(
        w.step(2, &NeverCancel)?,
        CheckpointedArchiveProgress::Archive(ArchiveWriteProgress::CatalogPrepared { .. })
    ));
    Ok(())
}
struct Cancel;
...
fn unacknowledged_pin_cannot_publish_or_release_original_source() -> Test {
    ...
    let mut w = open(&mut p)?;
    let root = offer(&mut w, 0)?;
    let pin = pin(&mut w)?;
    for _ in 0..5 {
        assert!(
            matches!(w.step(2, &NeverCancel)?, CheckpointedArchiveProgress::PinRequired(p) if p == pin)
        );
        assert!(w.snapshot().windows().is_empty());
        assert_eq!(w.pending().ok_or("lost source")?.manifest().root(), root);
    }
    let work = w.retire();
    assert_eq!(work.pending_checkpoint, Some(pin));
...
fn automatic_checkpoint_is_byte_identical_to_the_existing_retired_work_format() -> Test {
    ...
    let pin = pin(&mut w)?;
    let work = w.retire();
    let prepared = PreparedArchiveWork::prepare(
        &work.archive,
        &p,
        ArchiveWorkLimits::default(),
        &NeverCancel,
    ...
    assert_eq!(
        pin.retirement_digest(),
        archive_retirement_digest(&work.archive)?
    );
    assert_eq!(pin.root(), prepared.root());
    assert_eq!(pin.slot(), prepared.slot());
...
fn cold_restart_after_checkpoint_loses_no_pending_media_before_normal_publication() -> Test {
    ...
    let mut resume = RecordingArchiveResume::open(
        ...
        work,
        ArchiveResumeConfig {
            expected_retirement_digest: pin.retirement_digest(),
...
fn lost_normal_window_ack_reconciles_without_a_second_ordinal() -> Test {
    ...
    let (pin, root) = {
        ...
        assert!(matches!(
            w.step(2, &NeverCancel)?,
            CheckpointedArchiveProgress::Archive(ArchiveWriteProgress::WindowDurable { .. })
        ));
        (pin, root)
    };
    let mut p = LocalRootPublisher::open(&path, storage_limits())?;
    ...
    let resume = RecordingArchiveResume::open(
        &mut p,
        work,
        ArchiveResumeConfig {
            expected_retirement_digest: pin.retirement_digest(),
...
fn automatic_catalog_barrier_precedes_index_io_and_survives_lost_page_ack() -> Test {
    ...
    let page_pin = {
        ...
        assert!(matches!(
            w.step(2, &NeverCancel)?,
            CheckpointedArchiveProgress::Archive(ArchiveWriteProgress::CatalogIndexStaged { .. })
        ));
        assert!(matches!(
            w.step(2, &NeverCancel)?,
            CheckpointedArchiveProgress::Archive(ArchiveWriteProgress::CatalogPublished { .. })
        ));
        pin
    };
    let mut p = LocalRootPublisher::open(&path, storage_limits())?;
    ...
    assert_eq!(
        archive_retirement_digest(&work)?,
        page_pin.retirement_digest()
    );
    let resume = RecordingArchiveResume::open(
        &mut p,
        work,
        ArchiveResumeConfig {
            expected_retirement_digest: page_pin.retirement_digest(),
            max_window_bytes: 0,
            max_steps: 100,
            deadline_ns: 1000,
        },
...
fn recovered_unindexed_tail_is_protected_before_new_page_publication() -> Test {
    ...
    let mut p = LocalRootPublisher::open(&path, storage_limits())?;
    let mut w = open(&mut p)?;
    assert_eq!(w.snapshot().unindexed_windows().len(), 1);
    let pins = drain(&mut w)?;
...
fn fixed_page_threshold_protects_every_window_and_every_page_once() -> Test {
    ...
    for i in 0..3 {
        offer(&mut w, i * 3600)?;
        pins.extend(drain(&mut w)?);
    }
    w.finish(2)?;
    pins.extend(drain(&mut w)?);
    assert_eq!(pins.len(), 5); // three original windows plus full/final-partial catalog pages
    assert_eq!(w.snapshot().windows().len(), 3);
    assert_eq!(w.snapshot().indexed_windows(), 3);
...
fn byte_graph_and_work_budgets_refuse_without_weakening_the_barrier() -> Test {
    ...
    let mut w = CheckpointedArchiveWriter::open(
        &mut p,
        namespace()?,
        limits(),
        ArchiveWorkLimits {
    ...
    let bytes = window.byte_len();
    let refusal = w
        .offer(window, bytes, 1)
        .err()
        .ok_or("byte budget ignored")?;
    assert_eq!(refusal.recording.manifest().root(), root);
    ...
    for (name, budget) in [
        ...
        assert!(w.pending_checkpoint().is_none());
    }
    Ok(())
}
...
fn pin_waits_and_acknowledgements_cannot_renew_clock_or_step_budget() -> Test {
    ...
    offer(&mut w, 0)?;
    let checkpoint = pin(&mut w)?;
    let remaining = w.remaining_steps();
    assert!(matches!(
        w.step(1, &NeverCancel),
...
fn stale_pin_acknowledgement_does_not_unlock_another_checkpoint() -> Test {
    ...
    w.finish(2)?;
    prepare_page(&mut w)?;
    let current = pin(&mut w)?;
    assert_ne!(current.root(), old.root());
    assert!(matches!(
        w.acknowledge_checkpoint(&old, 2, &NeverCancel),
        Err(ArchiveError::Metadata)
    ));
    assert!(
        matches!(w.step(2, &NeverCancel)?, CheckpointedArchiveProgress::PinRequired(p) if p == current)
    );
    assert_eq!(w.snapshot().indexed_windows(), 0);
    Ok(())
}
...
fn work_root_lost_ack_uses_the_pin_announced_before_any_checkpoint_write() -> Test {
    ...
    assert_eq!(archive_retirement_digest(&work)?, pin.retirement_digest());
    assert!(work.snapshot.windows().is_empty());
