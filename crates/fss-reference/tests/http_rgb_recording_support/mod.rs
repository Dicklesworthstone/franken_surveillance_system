...
use fss_publication::{
    LocalPublicationLimits, LocalRootPublisher, NeverCancel, PublishCancellation,
};
    ...
    pub fn new(label: &str) -> Test<Self> {
        for attempt in 0..128 {
            let path = std::env::temp_dir().join(format!(
                "fss-rgb-recording-{label}-{}-{attempt}",
                std::process::id()
            ));
...
impl Authority {
    pub fn deny(&self, operation: HttpCameraOperation) {
        self.denied.set(Some(operation));
    }
...
impl HttpCameraAuthority for Authority {
    fn checkpoint(
        ...
        if self.denied.get() == Some(operation) {
...
pub fn session<'m, 't>(
    ...
    if wire.len() > 65536 {
        return Err("wire fixture bound".into());
    }
...
pub fn attach<'m, 't>(
    capture: HttpRgbCapture<'m, 't>,
    publisher: &LocalRootPublisher,
    auth: &Authority,
    limits: HttpCheckLimits,
) -> Test<HttpRgbRecording<'m, 't>> {
    ...
    for _ in 0..50000 {
        ...
        match recording.poll(auth.access(&NeverCancel))? {
            ...
            HttpRgbRecordingStep::WirePrepared(plan) if commit_reads => {
                recording
                    .commit_wire(plan, publisher, auth.access(&NeverCancel))?
                    .acknowledgement()?;
            }
