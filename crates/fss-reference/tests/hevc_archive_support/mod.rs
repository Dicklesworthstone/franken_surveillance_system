...
use fss_publication::{LocalPublicationLimits, LocalPublicationState, LocalRootPublisher,
    NeverCancel, PublishCancellation, PublishCutPoint, SlotName};
...
pub fn limits() -> LocalPublicationLimits {
    LocalPublicationLimits::new(64, 512, 16, 512,
        SpoolLimits::new(512, 64 * 1024 * 1024, 1024 * 1024, 1024))
}
pub fn archive_limits() -> ArchiveLimits {
    ArchiveLimits { max_windows: 32, max_pages: 32, max_scan_roots: 256, windows_per_page: 2 }
}
pub fn fresh(name: &str) -> Result<PathBuf, Error> {
    let root = Path::new(env!("CARGO_TARGET_TMPDIR")).join("hevc_archive_contract").join(name);
...
pub fn publish_window(p: &mut LocalRootPublisher, slot: &SlotName, w: &PreparedHevcRecording) -> TestResult {
    let mut job = RecordingPublication::new(w.publication_plan(), p, slot.clone(), w.byte_len(), 1000)?;
...
pub fn page(ns: &HevcArchiveNamespace, first: usize, windows: &[PreparedHevcRecording])
    -> Result<HevcRecordingCatalog, Error>
{
    let mut builder = HevcCatalogBuilder::new(ns.scope().clone())?;
    for (offset, w) in windows.iter().enumerate() { builder.push(&ns.window_slot(first + offset)?, w)?; }
    Ok(builder.prepare()?)
}
pub fn publish_page(p: &mut LocalRootPublisher, ns: &HevcArchiveNamespace,
    first: usize, catalog: &HevcRecordingCatalog) -> TestResult
{
    let mut job = HevcCatalogPublication::new(catalog, p, ns.page_slot(first)?, catalog.byte_len(), 1000)?;
    for at in 0..catalog.entries().len() as u64 {
        assert!(matches!(job.step(at, &NeverCancel)?, CatalogProgress::WindowVerified { .. }));
    }
    let at = catalog.entries().len() as u64;
    assert!(matches!(job.step(at, &NeverCancel)?, CatalogProgress::IndexStaged { .. }));
    assert!(matches!(job.step(at + 1, &NeverCancel)?, CatalogProgress::Published(_)));
    Ok(())
}
pub fn seed(name: &str, indexed: usize)
    -> Result<(PathBuf, LocalRootPublisher, HevcArchiveNamespace, Vec<PreparedHevcRecording>), Error>
{
    let root = fresh(name)?;
    let mut p = LocalRootPublisher::open(&root, limits())?;
    let ns = namespace()?;
    let windows = [0, 90_000, 180_000].into_iter().map(fixture::window).collect::<Result<Vec<_>, _>>()?;
    for (ordinal, w) in windows.iter().enumerate() { publish_window(&mut p, &ns.window_slot(ordinal)?, w)?; }
