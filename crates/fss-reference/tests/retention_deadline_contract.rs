...
use fss_core::{BudgetVector, CaptureInterval, ContentDigest, OperationId, SensorId, StreamId, TimestampNs};
use fss_publication::SlotName;
use fss_reference::deletion::holds::{
    CAP_HOLD_COMMIT, CAP_HOLD_PREPARE, MAX_ACTIVE_HOLDS, HoldError, HoldOutcome, HoldRecord,
    HoldRequest, HoldState, RetentionReadiness, STAGE_HOLD_COMMITTED, STAGE_HOLD_STAGED,
    commit_hold, list_holds, preview_hold,
};
use fss_reference::deletion::{CommitOutcome, DeletionError, commit_deletion, plan_deletion};
use fss_reference::ingest::{FileFormatHint, FileIngestAdapter, FileIngestRequest, RetainedFileImport, RetainedReadLimits};
...
impl Directory {
    fn new(name: &str) -> TestResult<Self> {
        for attempt in 0..100 {
            let path = std::env::temp_dir().join(format!("fss-retention-{name}-{}-{attempt}", std::process::id()));
...
impl Fixture {
    fn new(name: &str) -> TestResult<Self> {
        ...
        let authority = ContextAuthority::new_root(RootAuthoritySpec {
            trace_id: "trace:retention".into(), operation_id: OperationId::parse("operation:retention")?,
            principal: PRINCIPAL.into(),
            capabilities: ["ADP-REPLAY-001", CAP_HOLD_PREPARE, CAP_HOLD_COMMIT].into_iter().map(str::to_owned).collect(),
    ...
    fn ingest(&mut self, name: &str, gray: u8) -> TestResult<ContentDigest> {
        ...
        fs::write(&path, encode_jpeg(16, 16, &[gray; 256], &JpegConfig {
            quality: 90, subsampling: Subsampling::Grayscale, restart_interval: 0, custom_markers: Vec::new(),
        })?)?;
        let request = FileIngestRequest::new(path, SensorId::parse(format!("sensor:{name}"))?, StreamId::parse(format!("stream:{name}"))?)
    ...
    fn apply(&mut self, request: &HoldRequest) -> TestResult<HoldRecord> {
        ...
        Ok(commit_hold(&mut self.deployment, request, preview.record.approval(), &self.authority, &self.cx)?.record)
    }
...
fn request(id: &str, import_identity: ContentDigest, state: HoldState) -> HoldRequest {
    HoldRequest { hold_id: id.into(), import_identity, state, reason: "Owner retention decision".into() }
}
fn until(deadline: i128) -> HoldState { HoldState::Until { not_before: TimestampNs(deadline) } }
...
fn expired(deadline: i128, first: i128, last: i128) -> TestResult<HoldState> {
    Ok(HoldState::Expired { not_before: TimestampNs(deadline), attested_now: time(first, last)? })
}
fn blocked_by(f: &Fixture, import: ContentDigest, id: &str) -> TestResult<bool> {
    Ok(plan_deletion(&f.deployment, import, &f.cx)?.blockers.iter().any(|b| b.kind == "evidence_hold" && b.subject == id))
}
...
fn minimum_retention_survives_reopen_and_requires_exact_expiry_before_deletion() -> TestResult {
    ...
    let mut f = Fixture { deployment, cx, authority, directory };
    assert_eq!(list_holds(&f.deployment, &f.authority, &f.cx)?, vec![placed.clone()]);
    assert_eq!(placed.request().state.readiness(time(10, 12)?)?, RetentionReadiness::EligibleForExpiry);
    ...
    assert!(matches!(preview_hold(&f.deployment, &request("minimum", import, HoldState::Released), &f.authority, &f.cx), Err(HoldError::Conflict)));
    assert!(matches!(preview_hold(&f.deployment, &request("minimum", import, expired(10, 9, 12)?), &f.authority, &f.cx), Err(HoldError::RetentionNotElapsed)));
    assert!(matches!(preview_hold(&f.deployment, &request("minimum", import, expired(9, 10, 12)?), &f.authority, &f.cx), Err(HoldError::Conflict)));
    ...
    let refused = commit_deletion(&mut f.deployment, blocked.digest()?, blocked.approval_digest(PRINCIPAL)?, PRINCIPAL, &f.cx);
    ...
    assert!(matches!(commit_hold(&mut f.deployment, &expiry, placed.approval(), &f.authority, &f.cx), Err(HoldError::StaleApproval)));
    assert_eq!(*f.deployment.current_anchor(), head);
    ...
    let head = f.deployment.current_anchor().clone();
    assert_eq!(commit_hold(&mut f.deployment, &expiry, done.approval(), &f.authority, &f.cx)?.outcome, HoldOutcome::AlreadyCurrent);
    assert_eq!(*f.deployment.current_anchor(), head);
    assert!(matches!(commit_deletion(&mut f.deployment, blocked.digest()?, blocked.approval_digest(PRINCIPAL)?, PRINCIPAL, &f.cx), Err(DeletionError::StalePlan(_))));
    ...
    assert_eq!(commit_deletion(&mut f.deployment, plan.digest()?, plan.approval_digest(PRINCIPAL)?, PRINCIPAL, &f.cx)?.outcome, CommitOutcome::Completed);
    assert_eq!(list_holds(&f.deployment, &f.authority, &f.cx)?, vec![done]);
    assert!(matches!(preview_hold(&f.deployment, &retain, &f.authority, &f.cx), Err(HoldError::ReleasedIdentifier)));
    Ok(())
}
...
fn later_shared_derivatives_remain_protected_by_each_independent_hold() -> TestResult {
    ...
    let staged = f.deployment.stage_and_publish(&slot, &[shared.as_slice()], &f.cx)?;
    f.deployment.publish_and_commit(&slot, &staged.manifest, time(0, 0)?, &f.cx)?;
    for id in ["short", "long", "investigation"] { assert!(blocked_by(&f, south, id)?); }
    ...
    assert!(plan_deletion(&f.deployment, south, &f.cx)?.blockers.is_empty());
    Ok(())
}
...
fn altered_time_bounds_and_intervening_authority_invalidate_expiry_approval() -> TestResult {
    ...
    for bounds in [(11, 12), (10, 13)] {
        ...
        assert!(matches!(commit_hold(&mut f.deployment, &changed, preview.record.approval(), &f.authority, &f.cx), Err(HoldError::StaleApproval)));
    }
    assert_eq!(*f.deployment.current_anchor(), head);
    assert_eq!(f.deployment.publisher().spool().digests().count(), objects);
    ...
    let head = f.deployment.current_anchor().clone();
    let objects = f.deployment.publisher().spool().digests().count();
    assert!(matches!(commit_hold(&mut f.deployment, &expiry, preview.record.approval(), &f.authority, &f.cx), Err(HoldError::StaleApproval)));
    assert_eq!(*f.deployment.current_anchor(), head);
    assert_eq!(f.deployment.publisher().spool().digests().count(), objects);
...
fn cancellation_before_expiry_append_keeps_protection_and_post_commit_is_success() -> TestResult {
    for stage in [STAGE_HOLD_STAGED, STAGE_HOLD_COMMITTED] {
        ...
        let result = commit_hold(&mut f.deployment, &expiry, preview.record.approval(), &f.authority, &f.cx);
        ...
        if stage == STAGE_HOLD_STAGED {
            ...
            assert_eq!(*f.deployment.current_anchor(), head);
            assert!(blocked_by(&f, import, "minimum")?);
            assert_eq!(commit_hold(&mut f.deployment, &expiry, preview.record.approval(), &f.authority, &f.cx)?.outcome, HoldOutcome::Committed);
        } else {
            assert_eq!(result?.outcome, HoldOutcome::Committed);
            assert!(!blocked_by(&f, import, "minimum")?);
            assert_eq!(commit_hold(&mut f.deployment, &expiry, preview.record.approval(), &f.authority, &f.cx)?.outcome, HoldOutcome::AlreadyCurrent);
        }
    }
    Ok(())
}
...
fn deadline_holds_share_the_active_bound_and_only_committed_expiry_frees_capacity() -> TestResult {
    ...
    assert!(matches!(preview_hold(&f.deployment, &extra, &f.authority, &f.cx), Err(HoldError::Limit)));
    ...
    assert_eq!(records.iter().filter(|r| r.request().state.is_active()).count(), MAX_ACTIVE_HOLDS);
    Ok(())
}
