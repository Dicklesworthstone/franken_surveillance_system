...
use fss_reference::rtsp::datagram_archive::{
    DatagramArchive, DatagramArchiveLimits, DatagramPin, DatagramScope,
};
...
pub fn spec() -> Test<AvcReplaySpec<'static>> {
    ...
    Ok(AvcReplaySpec {
        ...
        configuration_evidence: ContentDigest::sha256(
            b"owner pinned fixture SDP and replay policy",
        ),
    })
}
...
pub fn save(path: &Path, observations: &[(u8, u64, Vec<u8>)]) -> Test<DatagramPin> {
    ...
    for (channel, now, payload) in observations {
        ...
        source::send(
            &mut parser,
            &source::interleaved(*channel, payload),
            4096,
            *now,
            &mut events,
        )?;
