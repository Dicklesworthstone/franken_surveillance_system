...
use fss_reference::rtsp::datagram_reconstruction::recording::{DatagramRecordingReplay, RecordingReplaySpec, RecordingReplayStep};
...
fn recording() -> Test<RecordingReplaySpec> {
    Ok(RecordingReplaySpec { scope: RecordingScope {
        sensor: SensorId::parse("recipe-sensor")?, stream: StreamId::parse("recipe-video")?,
        generation: source::KEY.generation, anchor: ContentDigest::sha256(b"recipe history anchor"),
        receive_clock: scope()?.receive_clock }, time_scale: 90_000, limits: CollectorLimits::default(),
        timing_evidence: ContentDigest::sha256(b"independently accepted explicit recipe timing") })
}
fn recipe_limits() -> RecordingRecipeLimits { RecordingRecipeLimits::default() }
fn record_decisions(p: &LocalRootPublisher, a: &DatagramArchive)
...
fn fixture(name: &str) -> Test<(LocalRootPublisher, DatagramArchive, RecordingRecipe, Vec<PreparedRecording>)> {
    let path = fresh(name)?; let pin = save(&path, &observations(true)?)?;
    let (p, a) = reopen(&path, pin)?;
    let (recipe, expected) = record_decisions(&p, &a)?;
    Ok((p, a, recipe, expected))
}
fn run(recipe: &RecordingRecipe, a: &DatagramArchive, p: &LocalRootPublisher, now: u64)
    ...
    for _ in 0..512 {
        match replay.step(p, now, &NeverCancel, &mut work())? {
            ...
            RecipeReplayStep::Replay(RecordingReplayStep::FinishedPrefix { retained }) => {
                ...
                assert!(matches!(replay.step(p, now, &NeverCancel, &mut work())?, RecipeReplayStep::Ended));
                return Ok(windows);
            }
            RecipeReplayStep::Replay(RecordingReplayStep::Stopped { .. }) => return Err("native interpretation stopped".into()),
...
fn portable_recipe_replays_byte_identical_source_media_index_and_signed_timing() -> Test {
    ...
    let bytes = recipe.canonical_bytes().to_vec(); let identity = recipe.identity(); drop(recipe);
    let decoded = RecordingRecipe::from_canonical_bytes(&bytes, &a, recipe_limits())?;
    assert_eq!(decoded.identity(), identity); assert_eq!(decoded.canonical_bytes(), bytes);
...
fn every_truncated_encoding_and_trailing_or_unknown_policy_is_refused() -> Test {
    ...
    let mut changed = bytes.to_vec(); changed[8] ^= 1;
...
fn source_prefix_and_parameter_generation_cannot_be_rebound() -> Test {
    ...
    assert!(RecordingRecipe::from_canonical_bytes(recipe.canonical_bytes(), &empty, recipe_limits()).is_err());
    let mut changed = recording()?; changed.scope.generation += 1;
    assert!(RecordingRecipe::new(&a, spec()?, changed, recipe.timings().to_vec(), recipe_limits()).is_err());
    let mut changed = spec()?; changed.pps = changed.sps;
    assert!(RecordingRecipe::new(&a, changed, recording()?, recipe.timings().to_vec(), recipe_limits()).is_err());
    Ok(())
}
#[test]
fn external_byte_timing_receiver_and_collector_ceilings_do_not_rewrite_stored_limits() -> Test {
    ...
    let mut receiver = recipe_limits(); receiver.receiver.reorder.max_bytes -= 1;
    let mut collector = recipe_limits(); collector.collector.max_samples -= 1;
    for bound in [RecordingRecipeLimits { max_bytes: recipe.canonical_bytes().len() - 1, ..recipe_limits() },
        RecordingRecipeLimits { max_timings: 0, ..recipe_limits() }, receiver, collector] {
        assert!(matches!(RecordingRecipe::from_canonical_bytes(recipe.canonical_bytes(), &a, bound),
...
fn malformed_overflowing_or_wrong_epoch_timing_is_refused_before_replay() -> Test {
    ...
    for mutation in 0..6 {
        ...
        assert!(matches!(RecordingRecipe::new(&a, spec()?, recording()?, decisions, recipe_limits()),
...
fn missing_and_unused_decisions_are_not_defaulted_or_ignored() -> Test {
    ...
    for unused in [false, true] {
        ...
        if unused {
            ...
            let mut extra = decisions[0]; extra.timing.decode_time += 3600; decisions.push(extra);
        }
        let changed = RecordingRecipe::new(&a, spec()?, recording()?, decisions, recipe_limits())?;
        let mut replay = PlannedRecordingReplay::new(&changed, &a, recipe_limits(), bounds(), 1000)?;
        let mut refused = false;
        for _ in 0..128 {
            match replay.step(&p, 1000, &NeverCancel, &mut work()) {
                Err(failure) => {
                    assert!(matches!((&failure.reason, unused), (RecordingRecipeError::MissingTiming, false)
...
fn matched_timing_waits_one_step_without_read_ahead_and_clock_refusal_is_retryable() -> Test {
    ...
    for _ in 0..128 {
        if let RecipeReplayStep::Replay(RecordingReplayStep::Capture(CapturePoll::TimingRequired(_))) =
            ...
            assert!(matches!(replay.step(&p, 1000, &NeverCancel, &mut work())?, RecipeReplayStep::TimingApplied { index: 0, .. }));
            assert_eq!(replay.observations_read(), observed); return Ok(());
        }
    }
...
fn incomplete_fragment_prefix_is_not_flushed_into_a_picture_or_recording() -> Test {
    let path = fresh("recipe_fragment")?; let mut input = observations(true)?; let _ = input.pop();
    let pin = save(&path, &input)?; let (p, a) = reopen(&path, pin)?;
    let recipe = RecordingRecipe::new(&a, spec()?, recording()?, vec![], recipe_limits())?;
    assert!(run(&recipe, &a, &p, 1000)?.is_empty()); Ok(())
}
#[test]
fn source_backpressure_does_not_authorize_an_unrecorded_manual_seal() -> Test {
    let (p, a, recipe, _) = fixture("recipe_pressure")?;
    let mut low = recording()?; low.limits.max_source_bytes = 12;
    let limited = RecordingRecipe::new(&a, spec()?, low, recipe.timings().to_vec(), recipe_limits())?;
    let mut replay = PlannedRecordingReplay::new(&limited, &a, recipe_limits(), bounds(), 1000)?;
    for _ in 0..128 {
        if let Err(failure) = replay.step(&p, 1000, &NeverCancel, &mut work()) {
            assert!(matches!(failure.reason, RecordingRecipeError::CollectionPressure));
            assert!(failure.retirement.is_some()); return Ok(());
        }
    }
...
fn current_cancellation_and_work_exhaustion_fence_instead_of_skipping_a_decision() -> Test {
    ...
    for cancelled in [true, false] {
        let mut replay = PlannedRecordingReplay::new(&recipe, &a, recipe_limits(), bounds(), 1000)?;
        let result = if cancelled { replay.step(&p, 1000, &Stop, &mut work()) }
            else { replay.step(&p, 1000, &NeverCancel, &mut fss_geometry::WorkBudget::new(0)) };
        assert!(result.err().ok_or("refused work succeeded")?.retirement.is_some());
        assert_eq!(replay.timings_applied(), 0);
        assert!(matches!(replay.step(&p, 1000, &NeverCancel, &mut work())?, RecipeReplayStep::Ended));
    }
    Ok(())
