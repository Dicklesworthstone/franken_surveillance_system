...
use fss_reference::rtsp::recording_collector::{
    CollectedPicture, CollectorAdmission, CollectorLimits, RecordingCollector, RecordingTiming,
};

pub type Error = Box<dyn std::error::Error>;
pub type TestResult = Result<(), Error>;

pub fn key() -> StreamKey {
    StreamKey {
        ingress: 31,
        generation: 1,
        ssrc: 7,
...
pub fn scope() -> Result<RecordingScope, Error> {
    Ok(RecordingScope {
        sensor: SensorId::parse("collector-fixture")?,
        stream: StreamId::parse("video")?,
        generation: 1,
...
pub fn accepted(out: CollectorAdmission) -> Result<bool, Error> {
    match out {
        CollectorAdmission::Accepted {
            window_ready,
            unselected,
        } if unselected.is_empty() => Ok(window_ready),
...
pub fn sample(
    ...
    if pictures.len() != 1 {
        return Err("sample did not produce exactly one picture".into());
    }
    Ok(Sample {
