...
impl Temp {
    fn new(name: &str) -> Test<Self> {
        let path =
            std::env::temp_dir().join(format!("fss-rgb-archive-{}-{name}", std::process::id()));
...
impl Access {
    fn new() -> Self {
        Self {
            reads: Cell::new(true),
            writes: Cell::new(true),
            calls: Cell::new(0),
            deny_at: Cell::new(usize::MAX),
        }
    }
}
fn retention() -> ContentDigest {
    ContentDigest::sha256(b"explicit original-byte retention test grant")
}
...
fn plan(
    ...
    cx: &ReplayCx,
) -> Test<PreparedRgbArchive> {
    let e = capture(exposure, availability, cx)?;
    let r = replay(&e, cx)?;
    Ok(PreparedRgbArchive::new(
        &e,
        &r,
        retention(),
        RgbArchiveLimits::default(),
        &mut RgbEvidenceBudget::new(WORK),
        &mut WorkBudget::new(STORAGE_WORK),
        cx,
    )?)
}
#[test]
fn cold_originals_recompute_the_same_native_inference_without_source_files() -> Test {
    let temp = Temp::new("cold")?;
    let root = temp.0.join("deployment");
    let cx = context(&temp.0)?;
    let access = Access::new();
    let prepared = plan(1, TrackingAvailability::Available, &cx)?;
    ...
    drop(prepared);
    drop(d); // No source envelope, imported model, tensors or deployment left.
    let mut d = ReferenceDeployment::reopen(&root, "site:rgb-custody", &cx)?;
    let e = restore_rgb_evidence(
        &mut d,
        pin,
        RgbArchiveLimits::default(),
        &access,
    ...
    assert_eq!(e.graph(), rgb_evidence_support::graph()?);
    assert_eq!(e.weights(), rgb_evidence_support::weights());
    ...
    assert_eq!(
        actual.run().inference().output_digest(),
        expected.run().inference().output_digest()
    );
...
fn byte_retention_denial_precedes_all_object_and_ledger_writes() -> Test {
    ...
    assert!(matches!(
        ...
        Err(RgbArchiveError::Denied)
    ));
    assert!(d.ledger().batches().is_empty());
    assert!(d.publisher().root(&p.pin().slot()?).is_none());
    a.writes.set(true);
    assert_eq!(
        p.publish(&mut d, &a, &mut WorkBudget::new(STORAGE_WORK), &cx)?
            .outcome,
...
fn unledgered_durable_root_is_not_reinterpreted_as_a_completed_publication() -> Test {
    let t = Temp::new("root-before-ledger")?;
    let root = t.0.join("deployment");
    let cx = context(&t.0)?;
    let a = Access::new();
    let p = plan(1, TrackingAvailability::Available, &cx)?;
    let mut d = ReferenceDeployment::open(&root, "site:rgb-custody", &cx)?;
    ...
    assert!(matches!(
        ...
        Err(RgbArchiveError::NotCommitted)
    ));
    assert!(d.ledger().batches().is_empty());
    p.publish(&mut d, &a, &mut WorkBudget::new(STORAGE_WORK), &cx)?;
    assert_eq!(d.ledger().batches().len(), 1);
...
fn changed_capture_or_retention_pin_is_never_silently_rebound() -> Test {
    ...
    for field in 0..4 {
        ...
        match field {
            0 => pin.capture[1] += 1,
            1 => pin.evidence = ContentDigest::sha256(b"other"),
            2 => pin.retention = ContentDigest::sha256(b"other"),
            _ => pin.root = ContentDigest::sha256(b"other"),
        }
        assert!(
            restore_rgb_evidence(
                &mut d,
...
fn independent_bound_and_budget_failures_preserve_retryability() -> Test {
    ...
    assert!(d.ledger().batches().is_empty());
    p.publish(&mut d, &a, &mut WorkBudget::new(STORAGE_WORK), &cx)?;
    let limits = RgbArchiveLimits {
        maximum_spool_object_bytes: 1024,
        ..RgbArchiveLimits::default()
    };
    assert!(matches!(
        restore_rgb_evidence(
...
fn interrupted_and_unobservable_exposures_replay_without_availability_upgrade() -> Test {
    ...
    .enumerate()
    {
        let p = plan(i as u8 + 1, status, &cx)?;
        p.publish(&mut d, &a, &mut WorkBudget::new(STORAGE_WORK), &cx)?;
        let e = restore_rgb_evidence(
            &mut d,
            p.pin(),
...
fn repeated_models_are_shared_objects_not_per_frame_envelopes() -> Test {
    ...
    for exposure in [1, 2] {
        ...
        let manifest =
            fss_object::ObjectManifest::from_canonical_bytes(&d.publisher().spool().read(r.root)?)?;
        assert!(manifest.children().contains(&graph));
        assert!(manifest.children().contains(&weights));
        roots.push(r.root);
    }
    assert_ne!(roots[0], roots[1]);
    assert_eq!(
        d.publisher().spool().read(weights)?,
        rgb_evidence_support::weights()
    );
    Ok(())
}
...
fn late_original_corruption_refuses_restore_without_rewriting_canonical_history() -> Test {
    ...
    p.publish(&mut d, &a, &mut WorkBudget::new(STORAGE_WORK), &cx)?;
    let anchor = d.current_anchor().clone();
    assert!(damage_exact_payload(
        &d.root().join("objects/spool"),
        &rgb_evidence_support::jpeg(1)
    )?);
    assert!(
        restore_rgb_evidence(
            &mut d,
