...
use fss_publication::{
    LocalRootPublisher, NeverCancel, PublishCancellation, PublishCutPoint, SlotName,
};
use fss_reference::rtsp::datagram_archive::DatagramArchive;
use fss_reference::rtsp::datagram_reconstruction::recording::{
    DatagramRecordingReplay, RecordingReplayError, RecordingReplaySpec, RecordingReplayStep,
    RecordingReplayStop,
};
...
use fss_reference::rtsp::recording::{
    PreparedRecording, RecordingPacket, RecordingScope, prepare_recording,
};
...
fn recording_spec() -> Test<RecordingReplaySpec> {
    Ok(RecordingReplaySpec {
        scope: RecordingScope {
            sensor: SensorId::parse("sensor:reconstruction")?,
            stream: StreamId::parse("video:reconstruction")?,
            generation: source::KEY.generation,
            anchor: ContentDigest::sha256(b"accepted authority/history anchor"),
...
fn await_timing(
    r: &mut DatagramRecordingReplay<'_>,
    p: &LocalRootPublisher,
    now: u64,
) -> Test<PictureTimingRequest> {
    for _ in 0..256 {
        ...
        match r.step(p, now, &NeverCancel, &mut work())? {
            RecordingReplayStep::Capture(event) => match *event {
                CapturePoll::TimingRequired(_) => {
                    assert!(matches!(
                        r.supply_timing(timing(), now, &NeverCancel, &mut work())?,
                ...
                other => {
                    return Err(format!(
                        "unexpected reconstruction output: {:?}",
                        RecordingReplayStep::Capture(Box::new(other))
                    )
                    .into());
                }
            ...
            RecordingReplayStep::PrefixReady { .. } => {
                r.finish_prefix(now, &NeverCancel, &mut work())?
            }
            RecordingReplayStep::FinishedPrefix { retained } => {
                assert!(matches!(
                    retained.prefix.as_deref(),
                    Some(AvcReplayStep::PrefixExhausted { .. })
                ));
                assert!(matches!(
                    r.step(p, now, &NeverCancel, &mut work())?,
...
fn direct_expected(input: &[(u8, u64, Vec<u8>)]) -> Test<PreparedRecording> {
    ...
    let packets: Vec<_> = input
        ...
        .enumerate()
        .skip(1)
        .map(|(i, (_, now, bytes))| RecordingPacket {
            sequence: i as u64 + 1,
            received_ns: *now,
            bytes,
        })
...
fn cold_datagrams_become_identical_canonical_media_and_publish_with_the_same_owner() -> Test {
    ...
    let root = {
        ...
        let mut job = RecordingPublication::new(
            &window,
            &mut p,
            SlotName::parse("reconstructed-window")?,
            window.byte_len(),
            u64::MAX,
        )?;
        ...
        for _ in 0..5 {
            if let RecordingProgress::Published(receipt) =
                job.step(1_000_000_000_001, &NeverCancel)?
            {
                assert_eq!(receipt.root, root);
                assert_eq!(
                    receipt.claims.local,
                    fss_publication::LocalPublicationState::Durable
                );
                published = true;
                break;
            }
...
fn missing_timing_blocks_the_next_original_read_and_preserves_the_same_request() -> Test {
    ...
    for _ in 0..8 {
        match r.step(&p, 1000, &NeverCancel, &mut work())? {
            RecordingReplayStep::Capture(event)
                if matches!(*event, CapturePoll::TimingRequired(_)) =>
            {
                let CapturePoll::TimingRequired(same) = *event else {
                    return Err("timing request lost".into());
                };
...
fn invalid_timing_is_correctable_without_losing_or_replacing_the_picture() -> Test {
    let path = fresh("recording_timing")?;
    let pin = save(&path, &observations(false)?)?;
    let (p, a) = reopen(&path, pin)?;
    let mut r = DatagramRecordingReplay::new(&a, spec()?, recording_spec()?, bounds(), 1000)?;
    let request = await_timing(&mut r, &p, 1000)?;
    ...
    match r.step(&p, 1000, &NeverCancel, &mut work())? {
        RecordingReplayStep::Capture(event) if matches!(*event, CapturePoll::Window(_)) => {
            let CapturePoll::Window(window) = *event else {
                return Err("corrected window unavailable".into());
            };
            assert_eq!(
                window.manifest().root(),
                completed(&p, &a, 1000)?.manifest().root()
            );
        }
...
fn incomplete_fragment_finishes_only_the_selected_prefix_not_a_recording() -> Test {
    let path = fresh("recording_partial")?;
    ...
    for _ in 0..128 {
        match r.step(&p, 1000, &NeverCancel, &mut work())? {
            RecordingReplayStep::PrefixReady { .. } => {
                r.finish_prefix(1000, &NeverCancel, &mut work())?
            }
            RecordingReplayStep::FinishedPrefix { retained } => {
                let c = retained.capture.ok_or("capture retirement lost")?;
                assert!(c.collection.ready.is_none());
                assert!(c.collection.pending.pictures.is_empty());
                assert_eq!(c.collection.pending.sources.len(), 1);
                assert!(matches!(
                    retained.prefix.as_deref(),
                    Some(AvcReplayStep::PrefixExhausted { replay_ns: 11, .. })
                ));
                return Ok(());
            }
            RecordingReplayStep::Source(_) | RecordingReplayStep::MediaQueued => {}
            ...
            other => {
                return Err(format!("partial fragment became recording output: {other:?}").into());
            }
        }
    }
...
fn unmarked_final_picture_is_retired_not_finished_into_an_invented_window() -> Test {
    let path = fresh("recording_unmarked")?;
    let mut input = observations(false)?;
    input[1].2[1] &= 127;
    let pin = save(&path, &input)?;
    let (p, a) = reopen(&path, pin)?;
        ...
        match r.step(&p, 1000, &NeverCancel, &mut work())? {
            ...
            RecordingReplayStep::FinishedPrefix { retained } => {
                match *retained.prefix.ok_or("prefix lost")? {
                    AvcReplayStep::PrefixExhausted { retired, .. } => {
                        assert!(retired.picture.is_some())
                    }
            ...
            RecordingReplayStep::Capture(event) if matches!(*event, CapturePoll::Receiver(_)) => {}
            other => {
                return Err(format!("unmarked tail became completed picture: {other:?}").into());
            }
        }
    }
...
fn an_observed_gap_stops_collection_and_returns_prior_unsealed_work() -> Test {
    let path = fresh("recording_gap")?;
    let mut input = observations(false)?;
    let s = spec()?;
    input.extend([
        (0, 12, raw(4, 18000, false, s.sps)),
        (0, 200_000_020, raw(5, 18000, false, s.sps)),
    ]);
    let pin = save(&path, &input)?;
    let (p, a) = reopen(&path, pin)?;
    ...
    for _ in 0..256 {
        match r.step(&p, 1000, &NeverCancel, &mut work())? {
            ...
            RecordingReplayStep::Stopped {
                trigger: RecordingReplayStop::Collection(event),
                retained,
            } => {
...
fn collection_pressure_retains_the_unconsumed_source_and_blocks_read_ahead() -> Test {
    let path = fresh("recording_pressure")?;
    let pin = save(&path, &observations(false)?)?;
    let (p, a) = reopen(&path, pin)?;
    let mut rs = recording_spec()?;
    rs.limits.max_source_bytes = 12;
    let mut r = DatagramRecordingReplay::new(&a, spec()?, rs, bounds(), 1000)?;
    for _ in 0..128 {
        match r.step(&p, 1000, &NeverCancel, &mut work())? {
            RecordingReplayStep::Capture(event)
                if matches!(*event, CapturePoll::Backpressure(_)) =>
            {
...
fn cancellation_after_window_extraction_keeps_the_entire_prepared_recording() -> Test {
    let path = fresh("recording_withheld")?;
    let pin = save(&path, &observations(false)?)?;
    let (p, a) = reopen(&path, pin)?;
    let mut r = DatagramRecordingReplay::new(&a, spec()?, recording_spec()?, bounds(), 1000)?;
    ...
    assert!(r.seal(1000, &NeverCancel, &mut work())?);
    let failure = r
        .step(&p, 1000, &StopAt(std::cell::Cell::new(2)), &mut work())
        .err()
        .ok_or("post-window cancellation ignored")?;
    let retired = failure.retirement.ok_or("retirement missing")?;
    match *retired.withheld.ok_or("prepared recording lost")? {
        RecordingReplayStep::Capture(event) if matches!(*event, CapturePoll::Window(_)) => {
            let CapturePoll::Window(window) = *event else {
                return Err("wrong withheld result".into());
            };
            assert_eq!(
                window.manifest().root(),
                completed(&p, &a, 1000)?.manifest().root()
...
fn cancellation_after_timing_admission_keeps_its_outcome_and_completed_picture() -> Test {
    let path = fresh("recording_timing_cancel")?;
    let pin = save(&path, &observations(false)?)?;
    let (p, a) = reopen(&path, pin)?;
    let mut r = DatagramRecordingReplay::new(&a, spec()?, recording_spec()?, bounds(), 1000)?;
    await_timing(&mut r, &p, 1000)?;
    let failure = r
        .supply_timing(
            timing(),
            1000,
            &StopAt(std::cell::Cell::new(2)),
            &mut work(),
        )
        .err()
        .ok_or("timing cancellation ignored")?;
...
fn prefix_finish_cannot_bypass_unread_input_or_renew_expired_timing() -> Test {
    let path = fresh("recording_lease")?;
    let pin = save(&path, &observations(false)?)?;
    let (p, a) = reopen(&path, pin)?;
    let mut b = bounds();
    b.deadline_ns = 2000;
    ...
    let failure = r
        .finish_prefix(1000, &NeverCancel, &mut work())
        .err()
        .ok_or("early finish accepted")?;
    assert!(matches!(
        failure.reason,
        RecordingReplayError::PrefixNotReady
    ));
    assert!(failure.retirement.is_none());
    await_timing(&mut r, &p, 1000)?;
    let failure = r
        .supply_timing(timing(), 2000, &NeverCancel, &mut work())
        .err()
        .ok_or("lease renewed")?;
    assert!(
        failure
...
fn recording_scope_and_timing_policy_are_bound_before_reconstruction() -> Test {
    let path = fresh("recording_scope")?;
    let pin = save(&path, &observations(false)?)?;
    let (_, a) = reopen(&path, pin)?;
    let original = DatagramRecordingReplay::new(&a, spec()?, recording_spec()?, bounds(), 1000)?
        .interpretation();
    let mut rs = recording_spec()?;
    rs.timing_evidence = ContentDigest::sha256(b"different media-clock evidence");
    assert_ne!(
        DatagramRecordingReplay::new(&a, spec()?, rs, bounds(), 1000)?.interpretation(),
        original
    );
    let mut rs = recording_spec()?;
    rs.limits.max_samples -= 1;
    assert_ne!(
        DatagramRecordingReplay::new(&a, spec()?, rs, bounds(), 1000)?.interpretation(),
        original
    );
    let mut rs = recording_spec()?;
    rs.scope.receive_clock = ContentDigest::sha256(b"wrong receive clock");
    assert!(DatagramRecordingReplay::new(&a, spec()?, rs, bounds(), 1000).is_err());
    let mut rs = recording_spec()?;
    rs.scope.generation += 1;
    assert!(DatagramRecordingReplay::new(&a, spec()?, rs, bounds(), 1000).is_err());
    Ok(())
}
#[test]
fn clock_regression_preserves_pending_timing_for_a_corrected_call() -> Test {
    let path = fresh("recording_clock_refusal")?;
    let pin = save(&path, &observations(false)?)?;
    let (p, a) = reopen(&path, pin)?;
    let mut r = DatagramRecordingReplay::new(&a, spec()?, recording_spec()?, bounds(), 1000)?;
    let request = await_timing(&mut r, &p, 1000)?;
    let failure = r
        .supply_timing(timing(), 999, &NeverCancel, &mut work())
        .err()
        .ok_or("backwards clock accepted")?;
    assert!(matches!(
        failure.reason,
        RecordingReplayError::Replay(AvcReplayError::ClockReversed)
    ));
    assert!(failure.retirement.is_none());
    assert_eq!(await_timing(&mut r, &p, 1000)?, request);
    Ok(())
