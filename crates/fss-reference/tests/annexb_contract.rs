...
use fss_reference::{
    ADP_REPLAY_ROW_ID, AnnexBError, AnnexBLimits, AnnexBScan, CEILING_MAX_NAL_BYTES,
    DEFAULT_MAX_INPUT_BYTES, ReplayCx, ReplayIoAuthority, SourceSpan, split_annexb,
};
...
fn test_cx(label: &str) -> Result<ReplayCx, Box<dyn Error>> {
    ...
    let scratch_root = std::env::var_os("CARGO_TARGET_TMPDIR")
        ...
        .join(format!("test-replay-cx-annexb-contract-{label}"));
...
#[test]
fn test_01_empty_input() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("test_01")?;
...
#[test]
fn test_02_oversize_input_rejected_before_allocation() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("test_02")?;
...
#[test]
fn test_03_no_start_code_found() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("test_03")?;
    let limits = AnnexBLimits::default();
...
#[test]
fn test_04_zero_length_nal_rejected() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("test_04")?;
    let limits = AnnexBLimits::default();
...
#[test]
fn test_05_truncated_nal_at_eof() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("test_05")?;
    let limits = AnnexBLimits::default();
...
#[test]
fn test_06_forbidden_bit_set_rejected() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("test_06")?;
    let limits = AnnexBLimits::default();
...
#[test]
fn test_07_nal_too_large_rejected() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("test_07")?;
...
#[test]
fn test_08_too_many_nals_rejected() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("test_08")?;
...
#[test]
fn test_09_too_many_access_units_rejected() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("test_09")?;
...
#[test]
fn test_11_malformed_emulation_prevention() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("test_11")?;
    let limits = AnnexBLimits::default();
...
#[test]
fn test_13_cooperative_cancellation_and_drain() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("test_13")?;
    let limits = AnnexBLimits::default();
...
#[test]
fn test_20_exact_equality_planted_bypasses() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("test_20")?;
    let limits = AnnexBLimits::default();
...
#[test]
fn probe_q12_cancel_before_scan() -> Result<(), Box<dyn Error>> {
    let cx = test_cx("probe_q12")?;
    cx.request_cancellation();
    assert_eq!(
