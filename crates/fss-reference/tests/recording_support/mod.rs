...
pub fn scope() -> Result<RecordingScope, Error> {
    Ok(RecordingScope {
        sensor: SensorId::parse("sensor-fixture")?,
        stream: StreamId::parse("stream-fixture")?,
        generation: 1,
        anchor: ContentDigest::sha256(b"owner-authority-anchor"),
        receive_clock: ContentDigest::sha256(b"host-clock-epoch"),
    })
}
pub fn fixture(ingress: u128, fragmented: bool) -> Result<Fixture, Error> {
    let nals = split(include_bytes!(
        "../../../fss-packet/tests/fixtures/avc/baseline.264"
    ));
    ...
    if result.pictures.len() != 1 {
        return Err("fixture did not produce one marker-bounded IDR group".into());
    }
    Ok(result)
}
fn drain(
        ...
        match receiver.poll(now)? {
            ...
            AvcReceivePoll::Assembly(AvcAssemblyStep::Accepted(output)) => {
                if output.retired.is_some() {
                    return Err("clean fixture retired".into());
                }
