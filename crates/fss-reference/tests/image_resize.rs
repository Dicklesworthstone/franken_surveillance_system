...
fn options(filter: ResizeFilter, aspect: ResizeAspect) -> ResizeOptions {
    ResizeOptions { filter, aspect, budget: ExecBudget::unlimited() }
}

fn image(bytes: &[u8], h: usize, w: usize, c: usize) -> ImageBytes<'_> {
    ImageBytes { bytes, height: h, width: w, channels: c, generation: Generation::GENESIS }
}
...
fn nearest_expands_rgb_into_planar_model_input() -> TestResult {
    ...
    let result = program.execute_resized_bytes(
        ...
        options(ResizeFilter::Nearest, ResizeAspect::Stretch), &ScalarExecCx::new(),
    )?;
    assert_eq!(result.tensor.shape().dims(), &[1, 3, 2, 4]);
    assert_eq!(result.tensor.to_vec::<f32>()?, vec![
...
fn bilinear_uses_half_pixel_centers_and_clamped_edges() -> TestResult {
    ...
    let result = program.execute_resized_bytes(
        ...
        options(ResizeFilter::Bilinear, ResizeAspect::Stretch), &ScalarExecCx::new(),
    )?;
    assert_eq!(result.tensor.to_vec::<f32>()?, vec![0., 50., 100., 50., 100., 150., 100., 150., 200.]);
    let downsample = PreprocessProgram::new(1, 1, ChannelTransform::LumaOnly, false)
        .execute_resized_bytes(image(&[0, 100, 100, 200], 2, 2, 1),
            options(ResizeFilter::Bilinear, ResizeAspect::Stretch), &ScalarExecCx::new())?;
...
fn identity_resize_preserves_legacy_f32_bits() -> TestResult {
    ...
    for transform in [ChannelTransform::Rgb, ChannelTransform::LumaOnly] {
        for scale in [false, true] {
            ...
            let expected: Vec<u32> = legacy.to_vec::<f32>()?.iter().map(|v| v.to_bits()).collect();
            for filter in [ResizeFilter::Nearest, ResizeFilter::Bilinear] {
                let result = program.execute_resized_bytes(image(&rgb, 3, 4, 3),
                    options(filter, ResizeAspect::Stretch), &ScalarExecCx::new())?;
                let actual: Vec<u32> = result.tensor.to_vec::<f32>()?.iter().map(|v| v.to_bits()).collect();
                assert_eq!(actual, expected);
            }
            // The existing v1 encoding and dimension-mismatch behavior stay unchanged.
            assert_eq!(program.canonical_bytes().len(), 18);
            assert!(program.execute_bytes(&rgb, 2, 6, 3, Generation::GENESIS).is_err());
        }
    }
    Ok(())
}
...
fn letterbox_preserves_aspect_and_inverse_detection_geometry() -> TestResult {
    ...
    let result = program.execute_resized_bytes(image(&[200; 8], 2, 4, 1),
        options(ResizeFilter::Nearest, ResizeAspect::Letterbox(7)), &ScalarExecCx::new())?;
    ...
    let tall = program.execute_resized_bytes(image(&[200; 8], 4, 2, 1),
        options(ResizeFilter::Nearest, ResizeAspect::Letterbox(7)), &ScalarExecCx::new())?;
    assert_eq!((tall.geometry.image_height, tall.geometry.image_width, tall.geometry.top, tall.geometry.left), (5, 2, 0, 1));
    Ok(())
}
...
fn replay_digests_bind_pixels_dimensions_generation_and_program() -> TestResult {
    ...
    let different = program.execute_resized_bytes(image(&[1, 2, 3, 5], 2, 2, 1), opts, &ScalarExecCx::new())?;
    assert_ne!(a.input_digest, different.input_digest);
    assert_ne!(a.output_digest, different.output_digest);
    let next = program.execute_resized_bytes(ImageBytes { generation: Generation::GENESIS.next()?, ..original }, opts, &ScalarExecCx::new())?;
    assert_ne!(a.input_digest, next.input_digest);
    assert_ne!(a.output_digest, next.output_digest);
    let reshaped = program.execute_resized_bytes(image(&[1, 2, 3, 4], 1, 4, 1), opts, &ScalarExecCx::new())?;
    assert_ne!(a.input_digest, reshaped.input_digest);
    assert_ne!(a.program_digest, program.resize_digest(ResizeFilter::Bilinear, ResizeAspect::Stretch));
    assert_ne!(a.program_digest, program.resize_digest(ResizeFilter::Nearest, ResizeAspect::Letterbox(0)));
    assert_ne!(program.resize_digest(ResizeFilter::Nearest, ResizeAspect::Letterbox(0)),
...
fn budget_admission_is_inclusive_and_accounts_for_tensor_copy() -> TestResult {
    ...
    assert!(matches!(program.execute_resized(&tensor, opts, &ScalarExecCx::new()), Err(ExecError::BudgetExceeded { .. })));
    ...
    assert!(matches!(program.execute_resized_bytes(input, opts, &ScalarExecCx::new()), Err(ExecError::BudgetExceeded { .. })));
...
fn malformed_and_hostile_inputs_fail_before_pixel_allocation() -> TestResult {
    let program = PreprocessProgram::new(2, 2, ChannelTransform::LumaOnly, false);
    let opts = options(ResizeFilter::Nearest, ResizeAspect::Stretch);
    for bad in [image(&[], 0, 1, 1), image(&[1], 2, 2, 1), image(&[1, 2], 1, 1, 2), image(&[], usize::MAX, 2, 1)] {
        assert!(program.execute_resized_bytes(bad, opts, &ScalarExecCx::new()).is_err());
    }
    let huge = PreprocessProgram::new(usize::MAX, usize::MAX, ChannelTransform::Rgb, false);
    assert!(huge.execute_resized_bytes(image(&[1, 2, 3], 1, 1, 3), opts, &ScalarExecCx::new()).is_err());
    let f32_input = Tensor::from_values(Shape::new(vec![1, 1, 1])?, &[1_f32], Generation::GENESIS)?;
    assert!(matches!(program.execute_resized(&f32_input, opts, &ScalarExecCx::new()), Err(ExecError::UnsupportedDType { .. })));
    let wrong_rank = Tensor::from_values(Shape::new(vec![1])?, &[1_u8], Generation::GENESIS)?;
    assert!(program.execute_resized(&wrong_rank, opts, &ScalarExecCx::new()).is_err());
    Ok(())
}
...
fn cancellation_returns_no_tensor_and_finalizes_drain() {
    ...
    let result = PreprocessProgram::new(2, 2, ChannelTransform::LumaOnly, false)
        .execute_resized_bytes(image(&[1; 4], 2, 2, 1),
            options(ResizeFilter::Nearest, ResizeAspect::Stretch), &cx);
    assert!(matches!(result, Err(ExecError::CancellationRequested { .. })));
