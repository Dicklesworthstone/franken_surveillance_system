...
use fss_reference::agent_session::workspace::{
...
fn initial_workspace(f: &Fixture) -> TestResult<WorkspaceWrite> {
    ...
    let capsule = SessionCapsule::new(SessionCapsuleParams {
        session_id: f.session.session_id.clone(),
        revision: 0,
        principal: f.session.principal_id.as_str().to_owned(),
        capability_projection: f.session.capabilities.iter().cloned().collect(),
        objective_digest: ContentDigest::sha256(b"investigate retained reference source")
            .to_string(),
        base_anchor: f.session.current_anchor.clone(),
        current_anchor: f.session.current_anchor.clone(),
        situation_capsule_digest: fingerprint.to_string(),
        decision_digest: fingerprint.to_string(),
...
fn advance_workspace(previous: &WorkspaceRevision) -> WorkspaceWrite {
    ...
    capsule.revision += 1;
    WorkspaceWrite {
        expected_head: Some(previous.digest()),
        capsule,
        mode: WorkspaceWriteMode::Advance,
    }
}
...
fn rebase_request(
    ...
    let mut write = advance_workspace(previous);
    write.mode = WorkspaceWriteMode::Rebase;
    ...
    write.capsule.situation_capsule_digest =
        ContentDigest::sha256(b"fixture:rebased-situation").to_string();
    write.capsule.decision_digest = ContentDigest::sha256(b"fixture:rebased-decision").to_string();
    write
        .capsule
        .epistemic_debt
        .extend(write.capsule.assumptions.clone());
    write.capsule.assumptions.clear();
    write.capsule.next_actions.clear();
    let refresh = SessionRefresh {
        expected_session_digest: session.session_digest(),
        current_anchor: write.capsule.current_anchor.clone(),
...
fn cursor_consumed(f: &Fixture, owner: &DurableDisclosureStore) -> TestResult<bool> {
    ...
    Ok(owner
        ...
        .ok_or("lost source continuation")?
        .consumed)
}
...
fn workspace_and_real_source_admissions_recover_under_one_root() -> TestResult {
    ...
    let first = owner.publish_workspace(
        &f.session.principal_id,
        initial_workspace(&f)?,
        TimestampNs(1_000),
    )?;
    f.advance(&mut owner)?;
    let second = owner.publish_workspace(
        &f.session.principal_id,
        advance_workspace(&first.result),
        TimestampNs(1_001),
    )?;
    ...
    let resumed = owner.resume_workspace(
        &f.session.principal_id,
    ...
    let source = owner.hydrate_context_slot_from_source(
        &f.session.principal_id,
        &f.publication,
        &f.read,
        &f.custody,
    ...
    assert_eq!(
        ...
        third.workspace_checkpoint_digest
    );
    assert!(
        resumed
            .result
            ...
            .capsule()
            .bookmarked_evidence
            .contains(&f.descriptor.subject_digest)
    );
    assert!(
        resumed
            .result
...
fn rejected_workspace_rebase_keeps_the_prior_context_deliverable() -> TestResult {
    ...
    assert!(matches!(
        owner.rebase_workspace(&f.session.principal_id, refresh, write, TimestampNs(1_002)),
...
fn committed_rebase_does_not_retarget_old_context_or_consume_its_cursor() -> TestResult {
    ...
    let done = owner.rebase_workspace(
        &f.session.principal_id,
        refresh.clone(),
        write.clone(),
        TimestampNs(1_002),
    ...
    assert_eq!(
        session.symbol_table_generation,
        f.session.symbol_table_generation + 1
    );
    let resumed = owner.resume_workspace(
        &f.session.principal_id,
        &f.session.session_id,
    ...
    let old = owner.resume_workspace(
        &f.session.principal_id,
        &f.session.session_id,
        first.result.digest(),
        TimestampNs(1_002),
...
fn workspace_persistence_failure_fences_source_disclosure_without_losing_recovery() -> TestResult {
    ...
    drop(owner);
    let limited = DurableSessionLimits {
        max_records: records,
        ..f.limits
    };
    let mut owner =
        DurableDisclosureStore::open_existing(f.path(), root, limited, f.blueprint.clone())?;
    assert!(matches!(
