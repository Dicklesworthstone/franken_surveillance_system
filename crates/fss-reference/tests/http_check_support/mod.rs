...
use fss_reference::ingest::http_replay::check::{
    HttpCheckDecode, HttpCheckRequest, HttpCheckSource,
};
use std::path::{Path, PathBuf};
pub type Test<T = ()> = Result<T, Box<dyn std::error::Error>>;
pub const JPEG: &[u8] = include_bytes!(concat!(
...
impl Directory {
    pub fn new() -> Test<Self> {
        for n in 0..128 {
            let path =
                std::env::temp_dir().join(format!("fss-http-check-{}-{n}", std::process::id()));
...
pub struct Fixture {
    pub request: HttpCheckRequest,
    pub wire_digest: ContentDigest,
}
pub fn fixture(root: &Path, close: bool, images: &[&[u8]]) -> Test<Fixture> {
    let mut p = open(root)?;
    let source = HttpCheckSource {
        source: ContentDigest::sha256(b"synthetic original stream"),
        generation: 1,
        receive_clock: ContentDigest::sha256(b"fixture receive clock"),
        retention_evidence: ContentDigest::sha256(b"fixture retention"),
    };
    ...
    let slot = SlotName::parse(&format!(
        "fsshw1-{}-00000001",
        namespace.strip_prefix("sha256:").ok_or("algorithm")?
    ))?;
