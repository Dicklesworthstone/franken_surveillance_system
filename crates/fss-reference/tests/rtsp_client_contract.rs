...
use fss_reference::rtsp::client::{
    ClientChannel, ClientCommand as C, ClientConfig, ClientError as E, ClientProgress as P,
...
fn ready() -> Result<RtspClientSession, Box<dyn std::error::Error>> {
    ...
    c.accept(
        &response(
            ...
            200,
            &[
                ("Session", "opaque-token;timeout=60"),
                (
                    "Transport",
...
fn complete_negotiation_has_exact_requests_and_no_early_media() -> TestResult {
    ...
    assert_eq!(
        options.bytes(),
        b"OPTIONS rtsp://camera.local/live/ RTSP/1.0\r\nCSeq: 1\r\n\r\n"
    );
    ...
    assert!(
        std::str::from_utf8(setup.bytes())?
            .starts_with("SETUP rtsp://camera.local/live/trackID=0 ")
    );
    c.accept(
        &response(
            3,
...
fn matching_response_replay_and_changed_session_are_refused() -> TestResult {
    ...
    c.request(C::Play, 4)?;
    assert_eq!(
        c.accept(&response(2, 200, &[("Session", "opaque-token")], &[]), 5),
        Err(E::CseqMismatch)
    );
    assert_eq!(
        c.accept(&response(3, 200, &[("Session", "other-token")], &[]), 5),
        Err(E::Session)
    );
...
fn timeout_is_exact_and_interim_responses_do_not_extend_it() -> TestResult {
    ...
    c.request(C::Describe, 0)?;
    assert_eq!(
        c.accept(&response(1, 100, &[], &[]), 9_999_999_999)?,
        P::Interim
    );
...
fn session_lifetime_uses_issue_time_not_delayed_ack_time() -> TestResult {
    ...
    c.accept(
        &response(
            ...
            200,
            &[
                ("Session", "opaque-token;timeout=2"),
                ("Transport", "RTP/AVP/TCP;unicast;interleaved=0-1"),
            ],
            &[],
        ),
...
fn credentials_and_redirects_never_create_authority_or_echo_input() -> TestResult {
    let mut cfg = config();
    cfg.presentation_uri = "rtsp://name:secret@camera.local/live/".into();
    ...
    for status in [301, 302, 401, 407, 454, 461, 500] {
        ...
        let mut r = response(
            2,
            status,
            &[("Location", "rtsp://hostile.local/private")],
            &[],
        );
        ...
        assert_eq!(
            c.accept(
                &response(
                    2,
                    200,
                    &[("Session", "opaque-token"), ("Transport", transport)],
                    &[]
                ),
                3
            ),
...
fn session_token_and_timeout_grammar_are_strict() -> TestResult {
    for value in [
        ...
        "token;timeout=3601",
        "token;timeout=+1",
        "token;timeout=60;timeout=60",
        "bad token;timeout=60",
        "token;other=secret",
    ] {
        let mut c = described()?;
        c.request(C::Setup, 2)?;
...
fn media_selection_and_duplicate_sdp_attributes_are_refused() -> TestResult {
    ...
    assert!(
        c.accept(
            &response(
                ...
                200,
                &[("Content-Type", "application/sdp")],
                original.replace("RTP/AVP 96", "RTP/AVP 96 97").as_bytes()
            ),
            1
        )
        .is_ok()
    ...
    for body in [
        original.replace("m=video", "m=audio"),
...
fn control_urls_cannot_escape_the_explicit_subtree() -> TestResult {
    for control in [
        ...
        "../admin",
        "//other/live",
        "rtsp://camera.local/live-secret/track",
        "%2e%2e/admin",
        "track#secret",
        "track%0d%0aHeader",
    ] {
...
fn content_base_is_validated_and_relative_control_resolution_is_exact() -> TestResult {
    ...
    c.accept(
        &response(
            ...
            &[
                ("Content-Type", "application/sdp"),
                ("Content-Base", "rtsp://camera.local/live/nested/"),
            ],
            sdp().as_bytes(),
        ),
        1,
    ...
    assert!(
        std::str::from_utf8(request.bytes())?
            .starts_with("SETUP rtsp://camera.local/live/nested/trackID=0 ")
    );
    let mut c = RtspClientSession::new(config())?;
    c.request(C::Describe, 0)?;
    assert_eq!(
...
fn debug_redacts_urls_tokens_and_parameter_sets() -> TestResult {
    ...
    for debug in [
        ...
        for secret in ["camera.local", "opaque-token", "trackID", "Z0LA"] {
...
fn every_description_wire_split_reaches_the_same_negotiation() -> TestResult {
    ...
    for split in 0..=wire.len() {
        ...
        let mut c = RtspClientSession::new(config())?;
        c.request(C::Describe, 0)?;
        let RtspEvent::Response(r) = &events[0] else {
            return Err("expected parsed response".into());
        };
        c.accept(r, 1)?;
...
fn fmtp_duplicate_spelling_cannot_change_packetization_silently() -> TestResult {
    for duplicate in ["packetization-mode =0", "PACKETIZATION-MODE=0"] {
        let body = sdp().replace(
            "packetization-mode=1",
            &format!("packetization-mode=1;{duplicate}"),
        );
        let mut c = RtspClientSession::new(config())?;
        c.request(C::Describe, 0)?;
        assert_eq!(
...
fn signaled_profile_must_match_exact_sps_profile_constraints_and_level() -> TestResult {
    for profile in ["42c00b", "42C00B", "64000b", "42c00c", "42c00", "42c0GG"] {
        let body = sdp().replace(
            "packetization-mode=1",
            &format!("packetization-mode=1;profile-level-id={profile}"),
        );
        let mut c = RtspClientSession::new(config())?;
        c.request(C::Describe, 0)?;
        let result = c.accept(
