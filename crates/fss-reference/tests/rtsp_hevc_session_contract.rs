...
use fss_reference::rtsp::client::{
    ClientCodec, ClientCommand as C, ClientConfig, ClientError as E, ClientProgress as P,
    ClientState as S, RtspClientSession,
};
...
fn describe(body: &str) -> Result<RtspClientSession, E> {
    ...
    client.accept(
        &response(1, &[("Content-Type", "application/sdp")], body.as_bytes()),
        1,
    )?;
    Ok(client)
}
...
fn hevc_description_retains_exact_parameter_sets_and_cannot_be_consumed_as_avc() -> TestResult {
    ...
    assert!(
        setup
            .bytes()
            .starts_with(b"SETUP rtsp://camera.local/live/trackID=0 ")
    );
...
fn missing_parameter_sets_remain_missing_and_partial_lists_are_not_completed() -> TestResult {
    ...
    assert_eq!(
        client.hevc_media().ok_or("missing media")?.vps(),
        &[vec![0x40, 1, 1, 2], vec![0x40, 1, 2, 3]]
    );
    Ok(())
}
...
fn hevc_uses_the_same_setup_play_keepalive_teardown_and_remote_uncertainty_rules() -> TestResult {
    ...
    client.accept(
        &response(
            2,
            &[
                ("Session", "private-token;timeout=60"),
                (
                    "Transport",
    ...
    assert_eq!(
        client.accept(&response(3, &[("Session", "private-token")], &[]), 5)?,
        P::Accepted(S::Playing)
    );
    ...
    assert_eq!(
        client.admit_channel(0, 30_000_000_006),
        Err(E::MediaNotAdmitted)
    );
    client.accept(
        &response(5, &[("Session", "private-token")], &[]),
        30_000_000_007,
    )?;
...
fn unsupported_don_multi_stream_and_unknown_requirements_never_become_defaults() -> TestResult {
    for attr in [
        ...
        "sprop-depack-buf-nalus=1",
        "sprop-depack-buf-bytes=1",
        "packetization-mode=1",
        "sprop-parameter-sets=Z0LACw==,aAE=",
        "unknown-required=1",
        "sprop-max-don-diff=-1",
        "sprop-max-don-diff=+0",
        "sprop-max-don-diff=0x0",
    ] {
        assert_eq!(
            describe(&sdp(&format!("a=fmtp:98 {attr}\r\n"))).err(),
            Some(E::Description),
            "{attr}"
        );
    }
    describe(&sdp(
        "a=fmtp:98 tx-mode=SRST;sprop-max-don-diff=0;sprop-depack-buf-nalus=0;sprop-depack-buf-bytes=0\r\n",
    ))?;
    Ok(())
}
...
fn malformed_parameter_sets_are_not_retained_or_echoed() -> TestResult {
    for attr in [
        ...
        "sprop-vps=wAEBAg==",
        "sprop-vps=QAEBAh==",
        "sprop-vps=QAEBAg==SECRET",
        "sprop-vps=QAABAg==",
        "sprop-vps=QAEBAg==, QAEBAg==",
    ] {
        let error = describe(&sdp(&format!("a=fmtp:98 {attr}\r\n")))
            .err()
            .ok_or("bad sets accepted")?;
        assert_eq!(error, E::Description);
        assert!(!format!("{error:?}").contains("SECRET"));
    }
    let many = std::iter::repeat_n("QAEBAg==", 17)
        .collect::<Vec<_>>()
        .join(",");
    assert_eq!(
        describe(&sdp(&format!("a=fmtp:98 sprop-vps={many}\r\n"))).err(),
        Some(E::Description)
    );
    Ok(())
}
...
fn explicit_media_index_prevents_automatic_selection_of_another_track_or_audio() -> TestResult {
    ...
    assert_eq!(
        client
            .hevc_media()
            .ok_or("explicit video not selected")?
            .payload_type(),
        98
    );
    Ok(())
...
fn multi_stream_source_specific_and_multiplexed_rtcp_signaling_is_refused() -> TestResult {
    ...
    let session_group =
        sdp(sets()).replace("a=control:*", "a=group:DDP video other\r\na=control:*");
...
fn profile_signaling_and_reduced_rtcp_are_retained_without_a_decode_claim() -> TestResult {
    let client = describe(&sdp(
        "a=fmtp:98 profile-space=3;profile-id=1;tier-flag=1;level-id=120;interop-constraints=A0b1C2d3E4f5\r\na=rtcp-rsize\r\n",
    ))?;
    let media = client.hevc_media().ok_or("missing media")?;
    ...
    assert_eq!(
        profile.interop_constraints,
        Some([0xa0, 0xb1, 0xc2, 0xd3, 0xe4, 0xf5])
    );
    ...
    for attr in [
        ...
        "tier-flag=2",
        "level-id=256",
        "interop-constraints=1234",
        "interop-constraints=Z00000000000",
        "level-id=+120",
    ] {
        assert_eq!(
            describe(&sdp(&format!("a=fmtp:98 {attr}\r\n"))).err(),
...
fn hostile_control_paths_and_content_bases_never_expand_owner_scope() -> TestResult {
    for control in [
        "rtsp://other.local/live/track",
        "../private",
        "rtsp://name:secret@camera.local/live/track",
        "/private/track",
    ] {
    ...
    assert_eq!(
        client.accept(
            &response(
                ...
                &[
                    ("Content-Type", "application/sdp"),
                    ("Content-Base", "rtsp://other.local/live/")
                ],
                sdp(sets()).as_bytes()
            ),
            1
