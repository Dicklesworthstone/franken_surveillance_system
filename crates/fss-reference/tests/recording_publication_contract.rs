...
use fss_reference::rtsp::recording::local::{
    RecordingIoError, RecordingProgress, RecordingPublication, load_recording,
};
...
fn fresh(name: &str) -> Result<PathBuf, Error> {
    let path = Path::new(env!("CARGO_TARGET_TMPDIR"))
        .join("recording_publication_contract")
        .join(name);
...
fn slot() -> Result<SlotName, Error> {
    Ok(SlotName::parse("recording-001")?)
}
...
fn source_is_staged_first_and_no_root_is_visible_after_children() -> TestResult {
    ...
    assert!(matches!(
        ...
        Err(RecordingIoError::NotDurable)
    ));
    assert_eq!(publish(&plan, &mut p)?, PublishOutcome::Published);
    Ok(())
}
#[test]
fn reopen_recovers_source_media_and_canonical_scope_without_remux() -> TestResult {
    ...
    assert_eq!(recovered.summary(), plan.summary());
    Ok(())
}
#[test]
...
fn cancellation_preserves_staged_source_without_a_published_root() -> TestResult {
    ...
    let mut p = LocalRootPublisher::open(fresh("cancel")?, limits())?;
    {
        ...
        assert!(matches!(
            ...
            Err(RecordingIoError::Cancelled)
        ));
        assert!(matches!(
            job.step(2, &NeverCancel),
            Err(RecordingIoError::Stopped)
        ));
    }
    assert!(p.root(&slot()?).is_none());
    assert_eq!(p.spool().read(plan.children()[0].1)?, plan.objects().source);
    Ok(())
}
#[test]
...
fn budget_and_expired_deadline_fail_before_publication() -> TestResult {
    ...
    assert!(RecordingPublication::new(&plan, &mut p, slot()?, plan.byte_len() - 1, 100).is_err());
    {
        let mut job = RecordingPublication::new(&plan, &mut p, slot()?, plan.byte_len(), 100)?;
        assert!(matches!(
            job.step(100, &NeverCancel),
            Err(RecordingIoError::Deadline)
        ));
    }
    assert!(p.root(&slot()?).is_none());
    Ok(())
...
fn clock_reversal_does_not_consume_a_child() -> TestResult {
    ...
    assert!(matches!(
        job.step(9, &NeverCancel),
        Err(RecordingIoError::ClockReversed)
    ));
    assert!(matches!(
        job.step(10, &NeverCancel)?,
        RecordingProgress::ChildStaged {
...
fn conflicting_recording_never_overwrites_an_existing_slot() -> TestResult {
    ...
    let plan = f.prepare()?;
    let mut p = LocalRootPublisher::open(fresh("conflict")?, limits())?;
    publish(&plan, &mut p)?;
    f.packets[0].1 += 1;
    let other = f.prepare()?;
    assert!(matches!(
        RecordingPublication::new(&other, &mut p, slot()?, other.byte_len(), 100),
        Err(RecordingIoError::RootConflict)
    ));
    assert_eq!(
        p.root(&slot()?).ok_or("lost root")?.root,
        plan.manifest().root()
    );
    Ok(())
}
...
fn exact_root_is_required_and_corrupted_source_is_never_served() -> TestResult {
    ...
    let text = plan.children()[0].1.to_text();
