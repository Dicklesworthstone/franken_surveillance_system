...
pub fn drain(c: &mut DigestAvcClient, now: u64, out: &mut Vec<DigestAvcPoll>) -> TestResult {
    for _ in 0..2048 {
        ...
        match &event {
            DigestAvcPoll::Client { event: inner, .. } if matches!(&**inner, AvcClientPoll::Pending { wake_at_ns } if wake_at_ns.is_none_or(|at| at > now)) =>
            {
                return Ok(());
            }
...
pub fn send(
    c: &mut DigestAvcClient,
    bytes: &[u8],
    chunk: usize,
    now: u64,
    out: &mut Vec<DigestAvcPoll>,
) -> TestResult {
...
pub fn playing() -> Result<DigestAvcClient, Error> {
    ...
    let mut out = Vec::new();
    assert_eq!(
        c.request(ClientCommand::Describe, &creds, [0; 16], 0)?
            .cseq(),
        1
    );
    send(
    ...
    assert_eq!(c.respond(&creds, [17; 16], 2)?.cseq(), 2);
    ...
    assert_eq!(
        c.request(ClientCommand::Setup, &creds, [18; 16], 4)?.cseq(),
        3
    );
    send(
        &mut c,
    ...
    assert_eq!(
        c.request(ClientCommand::Play, &creds, [19; 16], 6)?.cseq(),
        4
    );
    send(
        &mut c,
        &response(4, "Session: fixture\r\n", ""),
        7,
        7,
        &mut out,
    )?;
