...
use fss_reference::rtsp::client::{ClientCommand as C, ClientConfig, ClientError, ClientProgress, ClientState};
...
fn ready(limits: AvcReceiveLimits, reduced: bool) -> Result<RtspAvcClient, Box<dyn std::error::Error>> {
    let mut c = RtspAvcClient::new(config(), KEY, limits)?;
    c.request(C::Describe, 0)?;
    c.ingest(&response(1, "Content-Type: application/sdp\r\n", &description(reduced)), 1)?;
    assert!(matches!(c.poll(1)?, P::Control(ClientProgress::Accepted(ClientState::Described))));
    c.request(C::Setup, 2)?;
    c.ingest(&response(2, "Session: fixture;timeout=60\r\nTransport: RTP/AVP/TCP;unicast;interleaved=0-1;ssrc=7\r\n", ""), 3)?;
    assert!(matches!(c.poll(3)?, P::Control(ClientProgress::Accepted(ClientState::Ready))));
    Ok(c)
}
fn playing(limits: AvcReceiveLimits, reduced: bool) -> Result<RtspAvcClient, Box<dyn std::error::Error>> {
    let mut c = ready(limits, reduced)?;
    c.request(C::Play, 4)?;
    c.ingest(&response(3, "Session: fixture\r\n", ""), 5)?;
    assert!(matches!(c.poll(5)?, P::Control(ClientProgress::Accepted(ClientState::Playing))));
    Ok(c)
}
fn packet(seq: u16, timestamp: u32, nal: &[u8]) -> Vec<u8> {
    let mut out = vec![0x80, 96]; out.extend_from_slice(&seq.to_be_bytes());
    out.extend_from_slice(&timestamp.to_be_bytes()); out.extend_from_slice(&KEY.ssrc.to_be_bytes());
    out.extend_from_slice(nal); out
}
fn interleaved(channel: u8, payload: &[u8]) -> Vec<u8> {
    let mut out = vec![b'$', channel]; out.extend_from_slice(&(payload.len() as u16).to_be_bytes());
...
fn drain(c: &mut RtspAvcClient, now: u64, output: &mut Vec<P>) -> TestResult {
    for _ in 0..2_048 {
        ...
        match &event {
            ...
            P::Backpressure { .. } | P::Control(ClientProgress::KeepAliveDue) => { output.push(event); return Ok(()); }
            P::Ended { .. } => { output.push(event); return Ok(()); }
...
fn send(c: &mut RtspAvcClient, wire: &[u8], now: u64, out: &mut Vec<P>) -> TestResult {
    c.ingest(wire, now)?; drain(c, now, out)
}
...
fn tcp_chunking_preserves_real_source_bytes_and_all_four_picture_groups() -> TestResult {
    ...
    for chunk_size in [1, 2, 3, 7, 127, 4_096] {
        ...
        let mut received = Vec::new(); let mut wire = Vec::new(); let mut original = Vec::new();
        ...
        for p in &original { wire.extend_from_slice(&interleaved(0, p)); }
        for chunk in wire.chunks(chunk_size) { send(&mut c, chunk, 6, &mut received)?; }
        c.finish(); drain(&mut c, 7, &mut received)?;
        let sources: Vec<&[u8]> = received.iter().filter_map(|e| match e {
            P::Rtp { source, retirement: None, .. } => Some(source.payload()), _ => None,
        }).collect();
        assert_eq!(sources, original.iter().map(Vec::as_slice).collect::<Vec<_>>());
        let mut pictures = 0;
        for event in &received {
            match event {
                P::Media(AvcReceivePoll::Assembly(AvcAssemblyStep::Accepted(out))) => pictures += usize::from(out.picture.is_some()),
                P::Media(AvcReceivePoll::Picture(_)) => pictures += 1,
                P::Ended { media: Some(AvcReceivePoll::Ended { tail: Some(tail), .. }), retirement: Some(retired) } => {
                ...
                P::Media(AvcReceivePoll::Assembly(AvcAssemblyStep::Refused(_))) => return Err("clean NAL refused".into()),
                _ => {},
            }
        }
...
fn media_before_play_and_unknown_channels_return_original_bytes() -> TestResult {
    for early in [true, false] {
        let mut c = if early { ready(AvcReceiveLimits::default(), false)? } else { playing(AvcReceiveLimits::default(), false)? };
        let raw = packet(1, 90_000, &[0x67, 1]);
        c.ingest(&interleaved(if early { 0 } else { 17 }, &raw), 6)?;
        let P::Fault { reason: E::Session(ClientError::MediaNotAdmitted), source: Some(source), retirement } = c.poll(6)?
            else { return Err("unnegotiated media was not fenced".into()); };
...
fn server_ssrc_must_match_owner_binding_before_play() -> TestResult {
    ...
    c.poll(1)?;
    c.request(C::Setup, 2)?;
    c.ingest(&response(2, "Session: fixture\r\nTransport: RTP/AVP/TCP;unicast;interleaved=0-1;ssrc=8\r\n", ""), 3)?;
    assert!(matches!(c.poll(3)?, P::Fault { reason: E::StreamBinding, .. }));
...
fn sdp_parameter_bytes_reach_real_syntax_validation() -> TestResult {
    ...
    assert!(matches!(c.poll(3)?, P::Fault { reason: E::Syntax(_), .. }));
    Ok(())
}
...
fn rtcp_reduced_size_is_negotiated_and_bad_suffix_preserves_source() -> TestResult {
    for reduced in [false, true] {
        ...
        let P::Rtcp { source, validation } = c.poll(6)? else { return Err("RTCP event missing".into()); };
        assert_eq!(source.payload(), rr); assert_eq!(validation.is_ok(), reduced);
        let mut compound = rr.to_vec(); compound.extend_from_slice(&[0x81, 202, 0, 3, 0, 0, 0, 7, 1, 3, b'f', b's', b's', 0, 0, 0]);
        c.ingest(&interleaved(1, &compound), 7)?;
        assert!(matches!(c.poll(7)?, P::Rtcp { validation: Ok(2), .. }));
        compound.push(0);
        c.ingest(&interleaved(1, &compound), 8)?;
        let P::Rtcp { source, validation: Err(_) } = c.poll(8)? else { return Err("malformed RTCP accepted".into()); };
...
fn eof_inside_framing_retires_partial_input_without_fabricating_a_tail() -> TestResult {
    let mut c = playing(AvcReceiveLimits::default(), false)?;
    c.ingest(&[b'$', 0, 0, 10, 0x80], 6)?; c.finish();
    let P::Fault { reason: E::Truncated, retirement, .. } = c.poll(6)? else { return Err("truncated EOF accepted".into()); };
...
fn slow_partial_frame_and_session_deadlines_fire_without_more_input() -> TestResult {
    ...
    let mut c = playing(AvcReceiveLimits::default(), false)?;
    assert!(matches!(c.poll(60_000_000_004)?, P::Fault { reason: E::Session(ClientError::SessionExpired), .. }));
    Ok(())
}
...
fn input_budget_and_event_backpressure_refuse_before_consumption() -> TestResult {
    let mut c = playing(AvcReceiveLimits::default(), false)?;
    assert_eq!(c.ingest(&vec![0; 4_097], 6).err().ok_or("oversize accepted")?.reason, E::InputLimit);
    let frame = interleaved(1, &[0x80, 201, 0, 1, 0, 0, 0, 7]);
    c.ingest(&frame, 6)?;
    assert_eq!(c.ingest(&frame, 6).err().ok_or("queued event overwritten")?.reason, E::Backpressure);
...
fn queue_pressure_retains_one_retry_and_late_delivery_is_explicit() -> TestResult {
    let mut limits = AvcReceiveLimits::default(); limits.reorder.max_packets = 1; limits.reorder.max_delay_ns = 10;
    let mut c = playing(limits, false)?; let mut output = Vec::new();
    for seq in [0, 1, 3] { send(&mut c, &interleaved(0, &packet(seq, 90_000, nals(BASELINE)[0])), 6, &mut output)?; }
    ...
    assert_eq!(c.ingest(&missing, 7).err().ok_or("retry overwritten")?.reason, E::Backpressure);
    ...
    assert!(output.iter().any(|e| matches!(e, P::Media(AvcReceivePoll::Gap { .. }))));
    Ok(())
}

#[test]
fn confirmed_sequence_restart_closes_the_old_connection() -> TestResult {
    let mut c = playing(AvcReceiveLimits::default(), false)?; let mut output = Vec::new();
    for seq in [0, 1, 30_000, 30_001] { send(&mut c, &interleaved(0, &packet(seq, 90_000, nals(BASELINE)[0])), 6, &mut output)?; }
    assert!(output.iter().any(|e| matches!(e, P::Rtp { retirement: Some(_), admission, .. }
...
fn valid_prefix_cannot_hide_a_deferred_wire_error() -> TestResult {
    ...
    let mut wire = response(1, "", ""); wire.extend_from_slice(b"RTSP/1.0 200 OK\r\n\r\n");
    c.ingest(&wire, 1)?;
    assert!(matches!(c.poll(1)?, P::Control(_)));
    assert!(matches!(c.poll(1)?, P::Fault { reason: E::Wire(_), .. }));
    Ok(())
}
