...
fn store_limits() -> RecipeStorageLimits {
    RecipeStorageLimits { max_source_roots: 128, max_spool_object_bytes: 65536 }
}
fn publish(p: &mut LocalRootPublisher, a: &DatagramArchive, recipe: &RecordingRecipe)
    -> Test<RecordingRecipePin> {
    let job = PreparedRecordingRecipe::prepare(recipe, a, p, store_limits(), &NeverCancel, &mut work())?;
    let pin = job.pin().clone();
    let result = job.publish(p, &NeverCancel, &mut work())?;
    assert_eq!(result.pin, pin); assert_eq!(result.local.claims.local, LocalPublicationState::Durable);
    Ok(pin)
}
fn load(p: &LocalRootPublisher, a: &DatagramArchive, pin: &RecordingRecipePin) -> Test<RecordingRecipe> {
    Ok(load_recording_recipe(p, pin, a, recipe_limits(), store_limits(), &NeverCancel, &mut work())?)
}
#[test]
fn source_closed_recipe_survives_process_state_loss_and_rebuilds_publishable_identical_media() -> Test {
    let path = fresh("recipe_storage_cold")?; let source_pin = save(&path, &observations(true)?)?;
    let (pin, expected) = {
    ...
    let result_root = {
        ...
        assert_eq!(windows.len(), 1); let window = windows.remove(0);
        ...
        let mut writer = RecordingPublication::new(&window, &mut p, SlotName::parse("recipe-restored-window")?,
            window.byte_len(), u64::MAX)?;
        let mut done = false;
        for _ in 0..5 {
            if let RecordingProgress::Published(receipt) = writer.step(4_000_000_000_001, &NeverCancel)? {
                assert_eq!(receipt.root, root); done = true; break;
            }
        }
        assert!(done); root
    };
    let p = LocalRootPublisher::open(&path, storage())?;
    let loaded = load_recording(&p, &SlotName::parse("recipe-restored-window")?, result_root,
        &recording()?.scope, &NeverCancel)?;
    assert_eq!(loaded.objects().media, expected.objects().media);
    assert_eq!(loaded.objects().index, expected.objects().index); Ok(())
}
#[test]
fn exact_recipe_retry_reverifies_one_root_without_duplicating_source_observations() -> Test {
    ...
    assert_eq!(first.pin, second.pin); assert_eq!(p.visible_roots().count(), before + 1);
    assert_eq!(a.pin().datagrams, 3); Ok(())
}
#[test]
fn every_root_publication_cut_preserves_candidate_and_never_reports_false_durability() -> Test {
    for (index, cut) in [PublishCutPoint::AfterChildrenVerified, PublishCutPoint::AfterManifestBody,
        ...
        let pin = {
            ...
            let job = PreparedRecordingRecipe::prepare(&recipe, &a, &p, store_limits(), &NeverCancel, &mut work())?;
            let pin = job.pin().clone(); p.inject_crash_at(cut);
            assert!(job.publish(&mut p, &NeverCancel, &mut work()).is_err());
            assert_eq!(job.pin(), &pin); assert!(p.is_poisoned()); pin
        };
        let (p, a) = reopen(&path, source_pin)?;
        let result = load(&p, &a, &pin);
        if cut == PublishCutPoint::AfterRootRename { assert_eq!(result?.identity(), pin.recipe); }
...
fn corrupt_original_after_preparation_blocks_publication_and_after_commit_blocks_loading() -> Test {
    for committed in [false, true] {
        let (mut p, a, recipe, _) = fixture(if committed { "recipe_storage_corrupt_committed" } else { "recipe_storage_corrupt_prepared" })?;
        let job = PreparedRecordingRecipe::prepare(&recipe, &a, &p, store_limits(), &NeverCancel, &mut work())?;
        let pin = job.pin().clone();
        if committed { let _ = job.publish(&mut p, &NeverCancel, &mut work())?; }
        let hex: String = a.records()[0].payload_digest.bytes().iter().map(|b| format!("{b:02x}")).collect();
        std::fs::write(p.root_dir().join("spool/objects").join(hex), b"corrupt original envelope")?;
        if committed { assert!(load(&p, &a, &pin).is_err()); }
        else { assert!(job.publish(&mut p, &NeverCancel, &mut work()).is_err()); assert!(p.root(&pin.slot).is_none()); }
    }
    Ok(())
}
#[test]
fn incomplete_or_extra_source_graph_cannot_impersonate_recipe_publication() -> Test {
    for extra in [false, true] {
        let (mut p, a, recipe, _) = fixture(if extra { "recipe_storage_extra" } else { "recipe_storage_missing" })?;
        let original = PreparedRecordingRecipe::prepare(&recipe, &a, &p, store_limits(), &NeverCancel, &mut work())?;
        let mut pin = original.pin().clone(); drop(original);
        let metadata = p.stage_object(recipe.canonical_bytes())?;
        let mut children: Vec<_> = a.records().iter().map(|r| r.pin.head).collect();
        if extra { children.push(p.stage_object(b"unaccounted source")?); } else { let _ = children.pop(); }
...
fn resource_and_cancellation_refusals_have_no_new_root_side_effects() -> Test {
    ...
    let before = p.visible_roots().count();
    for ceiling in [RecipeStorageLimits { max_source_roots: 2, ..store_limits() },
        RecipeStorageLimits { max_spool_object_bytes: 1024, ..store_limits() }] {
        assert!(PreparedRecordingRecipe::prepare(&recipe, &a, &p, ceiling, &NeverCancel, &mut work()).is_err());
    }
    let job = PreparedRecordingRecipe::prepare(&recipe, &a, &p, store_limits(), &NeverCancel, &mut work())?;
    assert!(job.publish(&mut p, &Stop, &mut work()).is_err());
    assert!(job.publish(&mut p, &NeverCancel, &mut fss_geometry::WorkBudget::new(0)).is_err());
...
fn independently_selected_pin_and_source_cannot_be_replaced_on_load() -> Test {
    ...
    let pin = publish(&mut p, &a, &recipe)?;
    let mut wrong = pin.clone(); wrong.recipe = ContentDigest::sha256(b"other recipe");
    assert!(load(&p, &a, &wrong).is_err());
    let mut wrong = pin.clone(); wrong.source.payload_bytes += 1;
    assert!(load(&p, &a, &wrong).is_err());
    let mut wrong = pin; wrong.root = ContentDigest::sha256(b"other graph");
    assert!(load(&p, &a, &wrong).is_err()); Ok(())
}
#[test]
fn retained_recipe_is_not_a_claim_that_its_incorrect_timing_program_executed() -> Test {
    ...
    let mut decisions = recipe.timings().to_vec(); decisions[0].picture.rtp_timestamp += 1;
    ...
    assert_eq!(p.visible_roots().count(), a.records().len() + 1); Ok(())
}
#[test]
...
fn cancellation_after_timing_admission_preserves_the_withheld_native_result() -> Test {
    ...
    for at in 1..=8 {
        ...
        if let Err(failure) = replay.step(&p, 1000, &Cut { calls: Cell::new(0), at }, &mut work()) {
            ...
            let retired = failure.retirement.ok_or("fatal cancellation lacks ownership")?;
            saw_withheld |= retired.recording.as_ref().is_some_and(|r| r.withheld_timing.is_some());
        }
    }
    assert!(saw_withheld, "no exercised cancellation retained the admitted timing outcome"); Ok(())
}
