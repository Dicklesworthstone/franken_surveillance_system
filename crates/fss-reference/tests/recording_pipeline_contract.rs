    ...
    fn new(label: &str) -> TestResult<Self> {
        for n in 0..100 {
            let path = std::env::temp_dir()
                .join(format!("fss-pipeline-{label}-{}-{n}", std::process::id()));
...
fn context(root: &Path) -> TestResult<ReplayCx> {
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:pipeline".into(),
        operation_id: OperationId::parse("operation:pipeline")?,
        principal: "principal:pipeline".into(),
        ...
        anchor_universe: ContentDigest::sha256(b"site:pipeline"),
        generation: 1,
    })?;
    authority.validate()?;
...
fn fixture(label: &str) -> TestResult<Fixture> {
    ...
    let imported = FileIngestAdapter::ingest(
        FileIngestRequest::new(
            &source,
            SensorId::parse("sensor:pipeline")?,
            StreamId::parse("stream:pipeline")?,
        )
...
fn budget() -> RecordingBudget<'static> {
    RecordingBudget::new(
        DecodeBudget::new(1_000_000_000),
        1_000_000_000,
...
fn model_receipts(deployment: &ReferenceDeployment) -> usize {
    deployment
        ...
        .filter(|delta| delta.family == "model_invocation_receipt")
        .count()
}
...
fn complete_recording_replays_after_restart_with_zero_numeric_allowance() -> TestResult {
    ...
    drop(f.deployment);
    let mut reopened =
        ReferenceDeployment::open(&f.dir.0.join("deployment"), "site:pipeline", &f.cx)?;
    let restored_model = RecordedModel::decode(
        &reopened.publisher().spool().read(f.model.digest())?,
        f.model.digest(),
    )?;
    let mut reused = RecordingBudget::new(
        DecodeBudget::new(0),
        0,
        AnalysisBudget::new(1_000_000, 1_000_000),
    );
    let again = run_recording(
        &mut reopened,
        &f.request,
        &restored_model,
        &mut reused,
    ...
    assert_eq!(
        again.progress.new_decodes + again.progress.new_inferences,
        0
    );
    ...
    AnalysisReport::verify(
        &reopened,
        again.report.encoded(),
        again.report.digest(),
        &f.request.limits,
...
fn cumulative_model_budget_stops_at_exact_prefix_and_retry_reuses_it() -> TestResult {
    ...
    let mut limited = RecordingBudget::new(
        DecodeBudget::new(1_000_000_000),
        cost,
        AnalysisBudget::new(1_000_000, 1_000_000),
    );
    let Err(error) = run_recording(
        &mut f.deployment,
        &f.request,
        &f.model,
        &mut limited,
    ...
    let mut remaining = RecordingBudget::new(
        DecodeBudget::new(1_000_000_000),
        2 * cost,
        AnalysisBudget::new(1_000_000, 1_000_000),
    );
    let done = run_recording(
        &mut f.deployment,
        &f.request,
        &f.model,
        &mut remaining,
...
fn detector_budget_failure_keeps_all_runs_but_never_returns_a_partial_report() -> TestResult {
    ...
    let mut work = RecordingBudget::new(
        DecodeBudget::new(1_000_000_000),
        1_000_000_000,
        AnalysisBudget::new(0, 0),
    );
    let Err(error) = run_recording(
        &mut f.deployment,
        &f.request,
    ...
    assert_eq!(model_receipts(&f.deployment), 3);
    let before = f.deployment.current_anchor().clone();
    let mut retry = RecordingBudget::new(
        DecodeBudget::new(0),
        0,
        AnalysisBudget::new(1_000_000, 1_000_000),
    );
    let complete = run_recording(
        &mut f.deployment,
        &f.request,
        &f.model,
        &mut retry,
...
fn interrupted_decode_root_resumes_then_completes_the_same_pipeline() -> TestResult {
    ...
    let fresh = context(&f.dir.0.join("deployment"))?;
    let mut reopened =
        ReferenceDeployment::open(&f.dir.0.join("deployment"), "site:pipeline", &fresh)?;
    let completed = run_recording(
        &mut reopened,
        &f.request,
        &f.model,
        &mut budget(),
...
fn missing_completed_pixels_are_not_silently_regenerated_by_retry() -> TestResult {
    ...
    let path = f
        ...
        .publisher()
        .spool()
        .object_path(ContentDigest::sha256(frame.pixels()));
    fs::remove_file(&path)?;
    let before = f.deployment.current_anchor().clone();
    let mut work = budget();
    let Err(error) = run_recording(
...
fn corrupted_completed_pixels_refuse_without_spending_decode_allowance() -> TestResult {
    ...
    let path = f
        ...
        .spool()
        .object_path(ContentDigest::sha256(frame.pixels()));
    let mut corrupt = frame.pixels().to_vec();
    corrupt[0] ^= 1;
    fs::write(&path, &corrupt)?;
    let before = f.deployment.current_anchor().clone();
    let mut work = DecodeBudget::new(1_000_000_000);
    assert!(
        RecordedFrame::decode_and_publish(&mut f.deployment, &selected, &mut work, &f.cx).is_err()
    );
...
fn cached_runs_still_enforce_tensor_and_decoded_frame_bounds() -> TestResult {
    ...
    f.request.maximum_tensor_bytes =
        f.model
            .graph()
            .inputs()
            .iter()
            .try_fold(0_usize, |sum, port| {
                Ok::<usize, fss_tensor::TensorError>(sum + port.shape().size_bytes(port.dtype())?)
            })?;
    let Err(refused) = run_recording(
        &mut f.deployment,
        &f.request,
        &f.model,
        &mut budget(),
