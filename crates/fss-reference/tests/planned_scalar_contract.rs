...
use fss_model_ir::{
    AttrValue, AttributeMap, GraphNode, MemoryPlanLimits, ModelIrGraph, ModelIrVersion, OpCode,
    TensorPort,
};
...
fn exact(plan: &CompiledScalarPlan) -> PeakExecBudget {
    let r = plan.minimum_requirements();
    PeakExecBudget::new(r.macs, r.peak_bytes, r.scratch_bytes)
}
fn generous() -> PeakExecBudget {
    PeakExecBudget::new(100_000_000, 100_000_000, 100_000_000)
}
...
fn parity<S: AsRef<str>>(graph: &ModelIrGraph, inputs: &[(S, Tensor)]) -> Test {
    ...
    assert_eq!(
        ...
        planned.execution().allocated_bytes()
    );
    assert_eq!(
        legacy.nodes_executed(),
        planned.execution().nodes_executed()
    );
...
fn deep_graph_executes_below_the_legacy_cumulative_memory_limit() -> Test {
    ...
    assert!(matches!(
        ScalarExecutor::run(&g, &input, ExecBudget::new(400, 80), &ScalarExecCx::new()),
...
fn each_resource_boundary_refuses_one_below_and_accepts_exact() -> Test {
    ...
    for below in [
        PeakExecBudget {
            max_macs: b.max_macs - 1,
            ..b
        },
        PeakExecBudget {
            max_peak_bytes: b.max_peak_bytes - 1,
            ..b
        },
        PeakExecBudget {
            max_scratch_bytes: b.max_scratch_bytes - 1,
            ..b
        },
    ] {
        assert!(matches!(
            p.run(&input, below, &ScalarExecCx::new()),
...
fn actual_strides_and_offsets_are_not_replaced_by_contiguous_assumptions() -> Test {
    ...
    let input = tensor(&[2, 4], &[-3., 1., 2., 0., 4., -5., 6., 0.])?
        .slice(1, 0, 3, 1)?
        .transpose(0, 1)?;
...
fn missing_duplicate_extra_and_wrong_generation_bindings_fail_closed() -> Test {
    ...
    for bindings in [
        vec![],
        vec![("wrong", x.clone())],
        vec![("x", x.clone()), ("x", x.clone())],
        vec![("x", x.clone()), ("extra", x.clone())],
    ] {
        assert!(matches!(
            p.run(&bindings, generous(), &ScalarExecCx::new()),
            Err(PlannedExecError::InputSet)
        ));
    }
    ...
    for wrong in [stale, wrong_shape, wrong_dtype] {
        assert!(matches!(
            p.run(&[("x", wrong)], generous(), &ScalarExecCx::new()),
            Err(PlannedExecError::InputContract)
        ));
    }
...
fn cancellation_drains_and_does_not_poison_reusable_plan() -> Test {
    ...
    assert!(matches!(
        CompiledScalarPlan::compile(&g, MemoryPlanLimits::default(), &cx),
        Err(PlannedExecError::Execution(
            ExecError::CancellationRequested { .. }
        ))
    ));
...
fn unused_nodes_execute_and_pass_through_outputs_remain_exact() -> Test {
    ...
    let inputs = [
        (
            "x",
            tensor(&[4], &[-2., -0., f32::from_bits(0x7fc01234), 8.])?,
        ),
        ("spare", tensor(&[2], &[1., 2.])?),
    ];
    let p = plan(&g)?;
    let result = p.run(&inputs, exact(&p), &ScalarExecCx::new())?;
    assert_eq!(result.execution().nodes_executed(), 1);
    assert_eq!(result.receipt().released_values(), 1);
    assert_eq!(
        bits(
            result
                .execution()
...
fn zero_sized_softmax_still_reserves_its_axis_scratch() -> Test {
    ...
    assert!(matches!(
        p.run(&input, PeakExecBudget::new(0, 0, 0), &ScalarExecCx::new()),
        Err(PlannedExecError::BudgetExceeded { .. })
    ));
    let result = p.run(&input, exact(&p), &ScalarExecCx::new())?;
    assert_eq!(
...
fn late_invalid_embedding_index_cannot_publish_an_earlier_graph_output() -> Test {
    ...
    let g = ModelIrGraph::new_validated(
        ...
        GEN,
        vec![port("x", &[2, 2])?, ids],
        vec![port("a", &[2, 2])?, port("z", &[1, 2])?],
        vec![
            node("a", OpCode::Relu, &["x"], "a", AttributeMap::new())?,
            node(
                "z",
                OpCode::Embedding,
                &["ids", "a"],
    ...
    assert!(matches!(
        p.run(&inputs, exact(&p), &ScalarExecCx::new()),
        Err(PlannedExecError::Execution(ExecError::ShapeMismatch { .. }))
    ));
    let valid = [
        ("x", inputs[0].1.clone()),
        ("ids", Tensor::from_values(Shape::new([1])?, &[1_i64], GEN)?),
    ];
    parity(&g, &valid)
}
...
fn compiled_identity_binds_graph_and_is_independent_of_construction_order() -> Test {
    ...
    nodes.reverse();
    let reordered = ModelIrGraph::new_validated(
        g.id(),
        g.version(),
        g.generation(),
        g.inputs().to_vec(),
    ...
    let changed = ModelIrGraph::new_validated(
        "graph:different",
        g.version(),
        g.generation(),
        g.inputs().to_vec(),
...
fn every_f32_operator_family_matches_backend_work_and_payload_bits() -> Test {
    let cases = vec![
        ...
        (
            ...
            vec![vec![2, 2]],
            vec![4],
            attrs(&[("shape", AttrValue::IntList(vec![4]))]),
        ),
        (
            OpCode::Transpose,
            vec![vec![2, 2]],
...
fn integer_embedding_transfer_and_zero_width_validation_match_backend() -> Test {
    for width in [0, 2] {
        ...
        let g = ModelIrGraph::new_validated(
            ...
            ModelIrVersion::V1,
            GEN,
            vec![indices, port("table", &[3, width])?],
            vec![port("y", &[3, width])?],
            vec![node(
                "lookup",
                OpCode::Embedding,
        ...
        let inputs = [
            (
                "ids",
                Tensor::from_values(Shape::new([3])?, &[2_i64, 0, 2], GEN)?,
            ),
            ("table", tensor(&[3, width], &values)?),
        ];
        let p = plan(&g)?;
        assert_eq!(p.minimum_requirements().scratch_bytes, 0);
        parity(&g, &inputs)?;
    }
    Ok(())
}
...
fn duplicate_declared_outputs_are_refused_by_the_original_ir_contract() -> Test {
    ...
    outputs.extend(original.outputs().iter().cloned());
    ...
    assert!(matches!(
        CompiledScalarPlan::compile(&g, MemoryPlanLimits::default(), &ScalarExecCx::new()),
        Err(PlannedExecError::Planning(
            fss_model_ir::MemoryPlanError::Ir(
...
fn reshape_work_can_be_zero_without_bypassing_payload_admission() -> Test {
    ...
    let p = plan(&g)?;
    let input = [("x", tensor(&[2, 2], &[1., -0., 3., 4.])?)];
...
fn unused_integer_input_is_not_hidden_by_node_partitioning() -> Test {
    let g = ModelIrGraph::new_validated(
        "graph:unused-integer",
        ModelIrVersion::V1,
        GEN,
        vec![
            port("x", &[4])?,
            TensorPort::new("ids", DType::I64, Shape::new([1])?, GEN)?,
        ],
        vec![port("y", &[4])?],
        vec![node(
            "relu",
...
fn generated_branched_graphs_reproduce_full_execution_after_retirement() -> Test {
    ...
    for case in 0..512 {
        ...
        let g = ModelIrGraph::new_validated(
            format!("graph:generated:{case}"),
            ModelIrVersion::V1,
            GEN,
            vec![port("x", &[4])?],
            vec![port(&names[12], &[4])?, port(&names[retained], &[4])?],
            nodes,
        )?;
