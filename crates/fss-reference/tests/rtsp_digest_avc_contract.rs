...
use fss_reference::rtsp::{
    authentication::{AuthenticationError, DigestPolicy},
    avc_client::{AvcClientError, AvcClientPoll, authenticated::*},
    client::{
        ClientCommand as C, ClientError, ClientProgress, ClientState,
        authenticated::DigestClientError,
    },
...
fn fragmented_challenge_signs_exact_original_target_and_new_cseq() -> TestResult {
    ...
    send(
        &mut c,
        &challenge(1, "server-nonce-1", false),
        1,
        1,
        &mut out,
    )?;
    assert!(matches!(
        out.last(),
        Some(DigestAvcPoll::AuthenticationRequired { cseq: 1, .. })
    ));
    ...
    assert!(
        wire.contains(
            "response=\"f5eba878612218aea53e17a3eb4129c90ffdfdfc6de8d96b5511029dc894177b\""
        )
    );
...
fn authenticated_negotiation_preserves_real_rtp_and_all_four_avc_groups() -> TestResult {
    for chunk in [1, 7, 4096] {
        let mut c = playing()?;
        let nals = nals();
        let mut original = vec![packet(0, 90_000, nals[0])];
        let mut frame = 0;
        ...
        let mut out = Vec::new();
        send(&mut c, &wire, chunk, 8, &mut out)?;
        c.finish();
        drain(&mut c, 9, &mut out)?;
        ...
        for e in &out {
            match e {
                DigestAvcPoll::Client { event: inner, .. }
                    if matches!(
                        &**inner,
                        AvcClientPoll::Media(AvcReceivePoll::Assembly(AvcAssemblyStep::Accepted(
                            _
                        )))
                    ) =>
                {
                    if let AvcClientPoll::Media(AvcReceivePoll::Assembly(
                        AvcAssemblyStep::Accepted(o),
                    )) = &**inner
                    {
                ...
                DigestAvcPoll::Client {
                    event: inner,
                    wire_retirement: Some(w),
                } if matches!(
                    &**inner,
                    AvcClientPoll::Ended {
                        media: Some(AvcReceivePoll::Ended { tail: Some(_), .. }),
                        ..
                    }
                ) =>
                {
                    if let AvcClientPoll::Ended {
                        media: Some(AvcReceivePoll::Ended { tail: Some(o), .. }),
                        ..
                    } = &**inner
                    {
                        assert_eq!(
                            o.picture.as_ref().ok_or("EOF picture")?.boundary(),
                            AvcBoundary::EndOfInputUnverified
                        );
                        assert!(w.pending.is_empty());
                        assert!(w.challenge.is_none());
                        pictures += 1;
                    }
                }
                DigestAvcPoll::Fault { .. } => return Err("clean fixture refused".into()),
                DigestAvcPoll::Client { event: inner, .. }
                    if matches!(
                        &**inner,
                        AvcClientPoll::Fault { .. }
...
fn keepalive_challenge_does_not_suspend_media_fragment_deadlines() -> TestResult {
    ...
    send(
        &mut c,
        &interleaved(0, &packet(0, 90_000, nals()[0])),
        4096,
        8,
        &mut out,
    )?;
    send(
        &mut c,
        &interleaved(0, &packet(1, 90_000, &[0x5c, 0x81, 0x80])),
        4096,
        9,
        &mut out,
    )?;
    ...
    send(
        &mut c,
        &challenge(5, "server-nonce-2", true),
        7,
        11,
        &mut out,
    )?;
    ...
    assert!(
        out.iter()
            .any(|e| matches!(e, DigestAvcPoll::Client { event, .. }
        if matches!(&**event, AvcClientPoll::Media(AvcReceivePoll::FragmentRetired { .. }))))
    );
    assert!(matches!(
        out.last(),
        Some(DigestAvcPoll::AuthenticationRequired { cseq: 5, .. })
    ));
...
fn challenge_retry_cannot_extend_the_original_response_deadline() -> TestResult {
    ...
    let mut c = DigestAvcClient::new(
        cfg,
        KEY,
        AvcReceiveLimits::default(),
        "fixture-camera",
    ...
    assert!(
        matches!(c.poll(100)?, DigestAvcPoll::Client { event, wire_retirement: Some(_), .. }
...
fn wrong_cseq_basic_and_proxy_challenges_never_request_credentials() -> TestResult {
    for kind in 0..3 {
        ...
        c.ingest(&bytes, 1)?;
        match c.poll(1)? {
            DigestAvcPoll::Fault { retirement, .. } => assert_eq!(
                retirement
                    .wire
                    .challenge
                    .ok_or("refused response")?
                    .expose_wire(),
                bytes
            ),
...
fn wrong_realm_and_legacy_downgrade_close_without_losing_the_challenge() -> TestResult {
    for wrong in ["realm=\"foreign\"", "algorithm=MD5"] {
        ...
        if wrong.starts_with("realm") {
            bytes = bytes.replace("realm=\"fixture-camera\"", wrong);
        } else {
            bytes = bytes.replace("algorithm=SHA-256", wrong);
        }
        send(&mut c, bytes.as_bytes(), 7, 1, &mut out)?;
        let failure = c
            .respond(&creds, [17; 16], 2)
            .err()
            .ok_or("must refuse challenge")?;
...
fn wrong_password_loop_stops_on_repeated_unstale_nonce() -> TestResult {
    ...
    c.respond(&creds, [17; 16], 2)?;
    drain(&mut c, 2, &mut out)?;
    send(
        &mut c,
        &challenge(2, "server-nonce-1", false),
        4096,
        3,
        &mut out,
    )?;
    let error = c
        .respond(&creds, [18; 16], 4)
        .err()
        .ok_or("replayed nonce accepted")?;
    assert_eq!(
        error.reason,
        DigestAvcError::Authentication(DigestClientError::Authentication(
            AuthenticationError::Replay
        ))
    );
    assert!(error.retirement.is_some());
    assert!(c.respond(&creds, [19; 16], 5).is_err());
    Ok(())
}
#[test]
fn coalesced_media_waits_for_challenge_without_rewrite_or_double_admission() -> TestResult {
    ...
    let bytes = [
        challenge(5, "server-nonce-2", true),
        interleaved(0, &packet),
    ]
    .concat();
    send(&mut c, &bytes, 4096, 9, &mut out)?;
    let before = c.buffered_wire_bytes();
    assert!(matches!(
        c.ingest(b"unconsumed", 10),
        Err(DigestAvcFailure {
            reason: DigestAvcError::Backpressure,
            retirement: None
        })
    ));
    assert_eq!(c.buffered_wire_bytes(), before);
    c.respond(&creds, [21; 16], 10)?;
    drain(&mut c, 10, &mut out)?;
    let sources: Vec<_> = out
        .iter()
        .filter_map(|e| match e {
            DigestAvcPoll::Client { event, .. }
                if matches!(&**event, AvcClientPoll::Rtp { .. }) =>
            {
                match &**event {
                    AvcClientPoll::Rtp { source, .. } => Some(source),
                    _ => None,
                }
            }
            _ => None,
    ...
    assert_eq!(sources[0].payload(), packet);
    assert_eq!(sources[0].received_ns(), 9);
    Ok(())
}
#[test]
fn stale_pre_retry_success_cannot_complete_the_authenticated_request() -> TestResult {
    ...
    assert!(
        matches!(out.last(), Some(DigestAvcPoll::Client { event, wire_retirement: Some(_), .. })
        if matches!(&**event, AvcClientPoll::Fault {
...
fn waiting_credentials_retain_the_original_frame_deadline_and_cancel_ownership() -> TestResult {
    ...
    match c.poll(1 + WIRE_LIFETIME_NS)? {
        DigestAvcPoll::Fault { retirement, .. } => assert_eq!(
            retirement.wire.challenge.ok_or("held frame")?.expose_wire(),
            bytes
        ),
        other => return Err(format!("deadline missing: {other:?}").into()),
    }
    let again = c.cancel();
    assert!(again.wire.pending.is_empty());
    assert!(again.wire.challenge.is_none());
    Ok(())
}
#[test]
fn eof_during_challenge_cannot_release_a_new_signed_request() -> TestResult {
    ...
    c.finish();
    assert!(c.respond(&creds, [17; 16], 2).is_err());
    assert!(matches!(
        c.poll(2)?,
        DigestAvcPoll::Fault {
            reason: DigestAvcError::AuthenticationAtEof,
            ..
        }
    ));
    Ok(())
...
fn authenticated_teardown_preserves_remote_and_local_retirement_receipts() -> TestResult {
    ...
    send(
        &mut c,
        &response(request.cseq(), "Session: fixture\r\n", ""),
        4096,
        9,
        &mut out,
    )?;
