...
fn namespace_pins_every_scope_field_and_has_canonical_bounded_slots() -> TestResult {
    ...
    assert!(matches!(
        n.window_slot(MAX_ARCHIVE_WINDOWS),
...
fn empty_archive_is_entirely_unindexed_not_evidence_of_absence() -> TestResult {
    ...
    match read.step(0, &NeverCancel)? {
        ArchiveReadProgress::Complete(r) => {
            ...
            assert_eq!(r.unindexed, vec![0..u64::MAX]);
        }
...
fn restart_discovers_and_verifies_range_across_multiple_catalog_pages() -> TestResult {
    let (path, mut p) = owner("pages")?;
    let ns = namespace()?;
    let a = window(1, 3600)?;
    let b = window(3, 10800)?;
    let x = ns.window_slot(0)?;
    let y = ns.window_slot(1)?;
    publish_window(&mut p, &x, &a)?;
    publish_window(&mut p, &y, &b)?;
    publish_page(&mut p, &ns, 0, &[(&x, &a)])?;
    publish_page(&mut p, &ns, 1, &[(&y, &b)])?;
    drop(p);
    ...
    for i in 0..2 {
        match read.step(i, &NeverCancel)? {
            ArchiveReadProgress::Window {
                ...
                assert_eq!(ordinal, i as usize);
                assert_eq!(recording.manifest().root(), s.windows()[ordinal].root());
            }
            other => return Err(format!("unexpected {other:?}").into()),
        }
    }
    match read.step(2, &NeverCancel)? {
        ArchiveReadProgress::Complete(r) => {
            ...
            assert_eq!(r.output_bytes, (a.byte_len() + b.byte_len()) as u64);
...
fn discovery_and_output_budgets_refuse_before_returning_partial_success() -> TestResult {
    ...
    assert!(matches!(
        s.select(
            0..10000,
            ArchiveQueryLimits {
                max_output_bytes: a.byte_len() as u64 - 1,
    ...
    assert!(
        s.select(7200..8000, ArchiveQueryLimits::default())?
            .ordinals()
            .is_empty()
    );
    Ok(())
