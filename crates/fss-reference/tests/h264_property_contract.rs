...
//!
//! Verifies:
//! - Byte-exact NAL equivalence across 720 (seed, gop, frame_count, mtu, two_slice_au) configs
//!   between Annex-B output (split by an independent start-code scanner) and RTP depacketized NALs.
//! - Every NAL's final byte is strictly nonzero (guarantees nonzero final byte).
...
fn test_h264_annexb_rtp_nal_equivalence_property() -> Result<(), Box<dyn Error>> {
    // Systematic matrix yielding exactly 720 distinct parameter configurations (10 seeds * 4 gops * 3 frame_counts * 3 mtus * 2 two_slice).
    ...
    assert_eq!(
        total_configs, 720,
        "must test exactly 720 configs, observed {total_configs}"
    );
    assert!(
...
fn test_ssrc_reset_mtu150_single_slice_fua_succeeds() -> Result<(), Box<dyn Error>> {
    ...
    let records = independent_parse_rtpdump_records(&ssrc_reset.bytes)?;
    assert_eq!(
        records.len(),
        ssrc_reset.packets.len(),
        "wire packet record count must match descriptor count"
    );

    let mut reassembled_nal = Vec::new();
    let num_fragments = slice_fragments.len();

    for (frag_idx, frag) in slice_fragments.iter().enumerate() {
        let expected_seq = 1002u16.wrapping_add(frag_idx as u16);
        assert_eq!(
            frag.sequence, expected_seq,
            "descriptor sequence number for fragment {frag_idx} must be contiguous starting from 1002"
        );
        ...
        let rec = &records[6 + frag_idx];
        assert!(
            rec.len() >= 14,
            "fragment {frag_idx} wire packet too short for RTP header and FU-A header"
        );
        let wire_seq = u16::from_be_bytes([rec[2], rec[3]]);
        assert_eq!(
            wire_seq, expected_seq,
            "wire sequence number for fragment {frag_idx} must be contiguous starting from 1002"
        );

        let fu_indicator = rec[12];
        let fu_header = rec[13];
        assert_eq!(
            fu_indicator & 0x1f,
            28,
            "fragment {frag_idx} wire payload must be FU-A (type 28)"
        );
        assert_eq!(
            fu_header & 0x1f,
            5,
            "fragment {frag_idx} FU-A inner NAL unit type must be 5 (IDR slice)"
        );
        ...
        if frag_idx == 0 {
            assert!(s_bit, "first fragment must have S bit set (S=1)");
            assert!(!e_bit, "first fragment must NOT have E bit set (E=0)");
            assert!(
                !frag.marker,
                "first FU-A fragment must NOT have RTP marker bit set"
            );
            let reconstructed_nal_header = (fu_indicator & 0xe0) | (fu_header & 0x1f);
            reassembled_nal.push(reconstructed_nal_header);
        } else if frag_idx + 1 == num_fragments {
            assert!(!s_bit, "last fragment must NOT have S bit set (S=0)");
            assert!(e_bit, "last fragment must have E bit set (E=1)");
            assert!(
                frag.marker,
            ...
            assert!(
                !s_bit,
                "intermediate fragment {frag_idx} must NOT have S bit set (S=0)"
            );
            assert!(
                !e_bit,
                "intermediate fragment {frag_idx} must NOT have E bit set (E=0)"
            );
            assert!(
                !frag.marker,
                "intermediate FU-A fragment {frag_idx} must NOT have RTP marker bit set"
            );
        }

        reassembled_nal.extend_from_slice(&rec[14..]);
    }

    // Reassembled NAL must equal the AU0 IDR slice from independent Annex-B scan
    let annexb_nals = independent_split_annexb(&annexb.bytes);
    let au0_idr_slice = annexb_nals
        .iter()
        .find(|n| (n[0] & 0x1f) == 5)
        .ok_or("missing AU0 IDR slice in Annex-B")?;
    assert_eq!(
        &reassembled_nal, au0_idr_slice,
        "reassembled FU-A IDR slice must equal AU0 IDR slice from independent Annex-B scan"
    );
