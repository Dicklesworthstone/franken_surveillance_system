...
use fss_reference::rtsp::recording::{
    PreparedRecording, RECORDING_KIND, RecordingObjects, verify_recording,
};
...
fn golden(fragmented: bool, expected: [&str; 5]) -> TestResult {
    ...
    assert_eq!(
        plan.manifest().root().to_text(),
        format!("sha256:{}", expected[4])
    );
    Ok(())
}
#[test]
...
fn replace_digest(index: &mut [u8], before: &[u8], after: &[u8]) -> TestResult {
    let old_digest = ContentDigest::sha256(before);
    let new_digest = ContentDigest::sha256(after);
    let old = old_digest.bytes();
    let new = new_digest.bytes();
    let positions: Vec<_> = index
        .windows(old.len())
        .enumerate()
        .filter_map(|(i, b)| (b == old.as_ref()).then_some(i))
        .collect();
    if positions.len() != 1 {
        return Err("fixture index must contain exactly one child reference".into());
    }
    let at = positions[0];
    index[at..at + new.len()].copy_from_slice(new.as_ref());
    Ok(())
}
#[test]
fn changed_original_packet_is_rejected_even_after_all_hashes_are_recomputed() -> TestResult {
    ...
    *source.last_mut().ok_or("empty source")? ^= 1;
    ...
    assert!(verify_recording(&rehash(objects)?, objects, &scope()?).is_err());
    Ok(())
}
#[test]
...
fn unused_object_cannot_be_smuggled_into_the_recording_root() -> TestResult {
    ...
    let children = [
        ContentDigest::sha256(plan.objects().source),
        ContentDigest::sha256(plan.objects().initialization),
        ContentDigest::sha256(plan.objects().media),
        ContentDigest::sha256(b"unrequested-object"),
    ];
    let manifest = ObjectManifest::new(
        RECORDING_KIND,
        children,
        Some(ContentDigest::sha256(plan.objects().index)),
    )?;
    assert!(verify_recording(&manifest, plan.objects(), &scope()?).is_err());
    Ok(())
}
