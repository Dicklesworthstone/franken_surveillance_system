...
use fss_core::{
    ...
    ContentDigest, ContextExpansionBindingSet, ContractBasis, ContractBasisRegistryBytes, EventId,
    Generation, HandleAvailability, HydrationArtifact, HydrationLevel, HydrationPurpose,
    LaboratoryAccess, MissionId, ObjectId, PrincipalId, ProbabilityInterval, ResourcePressure,
    SemanticHandle, SemanticHandleSpec, SensorId, SessionId, TimestampNs, TombstoneReason,
    TombstoneRecord,
};
...
impl Fixture {
    fn new() -> TestResult<(Self, DurableDisclosureStore)> {
        ...
        let mut ledger = DurableReferenceLedger::open(
            directory.0.join("authority.journal"),
            "site:durable-context",
        ...
        let camera = VirtualCameraSpec {
            ...
            sensor_id: SensorId::parse("sensor:durable-context")?,
            seed: 71,
            packet_count: 3,
            packet_bytes: 32,
            start_ns: 100,
        ...
        let source = capture
            .source_packets
            .first()
            .ok_or("missing source packet")?
            .bytes
            .clone();
        let subject = objects.put_verified(&source)?;
        let source_root = objects
            .publish_manifest(ObjectManifest::new("context-source", [subject], None)?)?
            .root;
        ...
        let decision =
            evaluate_unknown_presence(EventId::parse("event:durable-context")?, vec![observation])?;
        ...
        let situation = compile_reference_situation(
            ReferenceSituationRequest {
                ...
                objective_id: "objective:durable-context".to_owned(),
                revision: 1,
        ...
        let descriptor = SemanticHandle::publish(SemanticHandleSpec {
            ...
            subject_id: "source:durable-context".to_owned(),
            subject_digest: subject,
            semantic_type: "source_object".to_owned(),
            source_id: camera.sensor_id.as_str().to_owned(),
        ...
        let slots = ContextExpansionBindingSet::required_slots(
        ...
        let specs = slots
            .into_iter()
            .map(|slot_id| ReferenceExpansionBindingSpec {
                slot_id,
                descriptor: descriptor.clone(),
                hydration_level: HydrationLevel::H2,
        ...
        blueprint.register_artifact(
            ...
            HydrationArtifact::publish(
                HydrationLevel::H2,
                "text/plain",
                b"bounded source preview".to_vec(),
                [subject],
                Completeness::Complete,
                None,
            )?,
        )?;
        blueprint.bind_source_object(
            &descriptor.handle_id,
            descriptor.descriptor_digest,
            source_root,
            &custody,
        ...
        let session = owner.open_session(
            AgentSessionParams {
    ...
    fn advance(&mut self, owner: &mut DurableDisclosureStore) -> TestResult {
        let delivery = owner.hydrate_context_slot(
        ...
        delivery
            .response
            .verify_for(&self.publication, &self.session)?;
        self.read.continuation = Some(
            delivery
                .response
                .response
                .receipt
...
#[test]
fn compiled_context_recovers_source_continuation_and_admission_without_session_aliases()
-> TestResult {
    ...
    drop(owner);
    let mut owner =
        DurableDisclosureStore::open_existing(f.path(), root, f.limits, f.blueprint.clone())?;
    let result = owner.hydrate_context_slot_from_source(
    ...
    result
        .response
        .verify_source_for(&f.publication, &f.session, binding)?;
    ...
    assert!(
        !journal
            .windows(f.source.len())
            .any(|bytes| bytes == f.source)
    );
    Ok(())
}
...
fn recovered_context_cursor_does_not_restore_a_revoked_source_grant() -> TestResult {
    let (mut f, mut owner) = Fixture::new()?;
    f.advance(&mut owner)?;
    f.session = owner.refresh(
        &f.session.principal_id,
        &f.session.session_id,
        SessionRefresh {
            expected_session_digest: f.session.session_digest(),
            current_anchor: f.session.current_anchor.clone(),
            capabilities: BTreeSet::from(["capability:preview".to_owned()]),
            privacy_scope: f.session.privacy_scope.clone(),
        },
        TimestampNs(1_002),
    )?;
