...
use fss_reference::rtsp::recording::{MAX_RECORDING_BYTES, PreparedRecording};
...
fn path(name: &str) -> Result<std::path::PathBuf, Error> {
    let p = std::path::Path::new(env!("CARGO_TARGET_TMPDIR"))
        .join("recording_catalog_contract")
        .join(name);
...
fn catalog_scope() -> Result<CatalogScope, Error> {
    Ok(CatalogScope {
        recording: scope()?,
        decode_clock: ContentDigest::try_sha256(b"explicit-dts-epoch")?,
        time_scale: 90_000,
    })
}
fn window(seq: u64, dts: u64) -> Result<PreparedRecording, Error> {
    ...
    a.source(&mut c, 1)?;
    accepted(c.push_picture(a.timed(dts), 1))?;
    c.seal(2)?;
    c.take_ready().ok_or_else(|| "missing window".into())
}
fn publish_window(
...
fn setup(name: &str) -> Result<(std::path::PathBuf, LocalRootPublisher, RecordingCatalog), Error> {
    ...
    let x = SlotName::parse("first")?;
    let y = SlotName::parse("second")?;
    publish_window(&mut p, &x, &a)?;
    publish_window(&mut p, &y, &b)?;
    let c = prepare_catalog(
        catalog_scope()?,
        &[
            CatalogWindow {
                slot: &x,
...
fn publish(p: &mut LocalRootPublisher, c: &RecordingCatalog) -> TestResult {
    ...
    for i in 0..2 {
        assert!(
            matches!(job.step(i, &NeverCancel)?, CatalogProgress::WindowVerified { ordinal, .. } if ordinal == i as usize)
        );
    }
    assert!(matches!(
...
fn publish_reopen_and_retrieve_two_whole_windows_with_explicit_gaps() -> TestResult {
    ...
    let mut read = RecordingRangeRead::new(
        &p,
        &loaded,
        &slot,
        5000..12000,
    ...
    for (now, ordinal, overlap) in [(0, 0, 5000..7200), (1, 1, 10800..12000)] {
        match read.step(now, &NeverCancel)? {
            RangeProgress::Window {
                ...
                assert_eq!(
                    recording.summary().decode_interval,
                    c.entries()[ordinal].decode_interval()
                );
            }
    ...
    match read.step(2, &NeverCancel)? {
        RangeProgress::Complete(r) => {
            ...
            assert_eq!(r.unindexed, vec![7200..10800]);
            assert_eq!(r.output_bytes, read.selection().output_bytes());
            assert_eq!(
                read.selection().unindexed(),
                std::slice::from_ref(&(7200..10800))
            );
        }
        other => return Err(format!("unexpected {other:?}").into()),
    }
    assert!(matches!(
        ...
        RangeProgress::Exhausted
    ));
    Ok(())
}
#[test]
fn unarchived_window_cannot_publish_a_discovery_root() -> TestResult {
    let root = path("unpublished")?;
    let mut p = LocalRootPublisher::open(&root, limits())?;
    let w = window(1, 0)?;
    let c = prepare_catalog(
        catalog_scope()?,
        &[CatalogWindow {
            slot: &SlotName::parse("missing")?,
            recording: &w,
        }],
    )?;
    let slot = SlotName::parse("catalog")?;
    {
        ...
        assert!(matches!(
            ...
            Err(CatalogIoError::Storage(RecordingIoError::NotDurable))
        ));
        assert!(matches!(
            job.step(1, &NeverCancel),
            Err(CatalogIoError::Storage(RecordingIoError::Stopped))
        ));
    }
...
fn cancellation_after_index_stage_leaves_no_catalog_and_retry_is_exact() -> TestResult {
    ...
    let slot = SlotName::parse("catalog")?;
    {
        let mut job = CatalogPublication::new(&c, &mut p, slot.clone(), c.byte_len(), 100)?;
        for i in 0..3 {
            let _ = job.step(i, &NeverCancel)?;
        }
        assert!(matches!(
            job.step(3, &Stop),
            Err(CatalogIoError::Storage(RecordingIoError::Cancelled))
        ));
    }
    assert!(p.root(&slot).is_none());
    publish(&mut p, &c)?;
...
fn expired_request_never_yields_more_media_and_reversed_time_can_retry() -> TestResult {
    ...
    assert!(matches!(
        ...
        Err(CatalogIoError::Storage(RecordingIoError::ClockReversed))
    ));
    assert!(matches!(
    ...
    assert!(matches!(
        ...
        Err(CatalogIoError::Storage(RecordingIoError::Deadline))
    ));
    assert!(matches!(
...
fn empty_query_result_completes_with_unknown_interval_not_absence() -> TestResult {
    let (_, mut p, c) = setup("no_matches")?;
    publish(&mut p, &c)?;
    let mut read = RecordingRangeRead::new(
        &p,
        &c,
    ...
    match read.step(0, &NeverCancel)? {
        RangeProgress::Complete(r) => {
            ...
            assert_eq!(r.unindexed, vec![8000..9000]);
        }
        other => return Err(format!("unexpected {other:?}").into()),
    }
    Ok(())
...
fn root_scope_budget_and_publisher_bounds_fail_before_output() -> TestResult {
    ...
    assert!(matches!(
        load_catalog(
            &p,
            &slot,
            ContentDigest::try_sha256(b"wrong")?,
            &catalog_scope()?,
            &NeverCancel
        ),
        Err(CatalogIoError::Storage(RecordingIoError::RootConflict))
    ));
    let mut wrong = catalog_scope()?;
    wrong.decode_clock = ContentDigest::try_sha256(b"other-clock")?;
    assert!(matches!(
        load_catalog(&p, &slot, c.manifest().root(), &wrong, &NeverCancel),
        Err(CatalogIoError::Catalog(CatalogError::Scope))
    ));
    assert!(matches!(
        RecordingRangeRead::new(
            &p,
            &c,
            &slot,
            0..18000,
    ...
    assert!(matches!(
        CatalogPublication::new(&c, &mut p, SlotName::parse("next")?, c.byte_len() - 1, 10),
        Err(CatalogIoError::Storage(RecordingIoError::Budget))
    ));
    let root = path("overpermissive_spool")?;
    let mut l = limits();
    l.spool.max_object_bytes = MAX_RECORDING_BYTES + 1;
    l.spool.max_total_bytes = 64 * 1024 * 1024;
    let mut other = LocalRootPublisher::open(root, l)?;
    assert!(matches!(
        CatalogPublication::new(&c, &mut other, slot, c.byte_len(), 10),
        Err(CatalogIoError::Storage(RecordingIoError::Budget))
    ));
    Ok(())
}
...
fn lie_about_window_size(c: &RecordingCatalog) -> Result<RecordingCatalog, Error> {
    ...
    for _ in 0..5 {
        d.u64()?;
    } // start, end, packet/sample/NAL counts
    let at = d.offset();
...
fn self_consistent_catalog_lie_is_not_published_or_delivered_as_media() -> TestResult {
    ...
    let forged = lie_about_window_size(&c)?;
    let slot = SlotName::parse("catalog")?;
    {
        let mut job =
            CatalogPublication::new(&forged, &mut p, slot.clone(), forged.byte_len(), 100)?;
        assert!(matches!(
            job.step(0, &NeverCancel),
            Err(CatalogIoError::Catalog(CatalogError::WindowMismatch))
        ));
    }
    assert!(p.root(&slot).is_none());
...
fn every_root_publication_crash_boundary_recovers_without_false_visibility() -> TestResult {
    ...
    .enumerate()
    {
        ...
        {
            let mut job = CatalogPublication::new(
                &c,
                &mut p,
                SlotName::parse("catalog")?,
                c.byte_len(),
            ...
            for now in 0..3 {
                let _ = job.step(now, &NeverCancel)?;
            }
        ...
        let retry_slot = if cut == PublishCutPoint::AfterRootRename {
            slot
        } else {
            SlotName::parse("recovered")?
        };
        let mut retry = CatalogPublication::new(&c, &mut reopened, retry_slot, c.byte_len(), 100)?;
        for now in 0..3 {
            let _ = retry.step(now, &NeverCancel)?;
        }
        match retry.step(3, &NeverCancel)? {
            CatalogProgress::Published(r) => assert_eq!(
                r.outcome,
                if cut == PublishCutPoint::AfterRootRename {
