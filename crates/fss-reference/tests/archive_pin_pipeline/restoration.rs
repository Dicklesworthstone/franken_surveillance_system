...
fn candidate(
    ...
    let CheckpointedArchiveProgress::PinRequired(pin) = w.step(0, &NeverCancel)? else {
        return Err("pin missing".into());
    };
...
fn reopen(path: &Path) -> Test<(LocalRootPublisher, ArchivePinJournal)> {
    Ok((
        LocalRootPublisher::open(path.join("media"), storage())?,
...
fn candidate_work_restores_after_every_original_object_and_receipt_is_lost() -> Test {
    ...
    assert_eq!(
        (result.durable_windows, result.indexed_windows, result.pages),
        (1, 0, 0)
    );
...
fn lost_restoration_response_is_an_exact_retry_without_new_journal_or_archive_roots() -> Test {
    ...
    let anchor = {
        ...
        candidate(&mut p, &mut pins, true)?;
        let _lost_reply =
            pins.restore_work(&mut p, ArchiveWorkLimits::default(), 0, 1000, &NeverCancel)?;
        pins.anchor()
    };
    let (mut p, mut pins) = reopen(&path)?;
    ...
    let result = pins.restore_work(&mut p, ArchiveWorkLimits::default(), 0, 1000, &NeverCancel)?;
    assert_eq!(
        result.window.as_ref().ok_or("window receipt")?.outcome,
...
fn uncertain_normal_window_commit_keeps_same_journal_selection_for_reopen() -> Test {
    ...
    assert_eq!(result.durable_windows, 1);
    assert_eq!(pins.anchor().sequence, 3);
    Ok(())
}
#[test]
fn uncommitted_candidate_is_not_discarded_or_reported_as_success() -> Test {
    ...
    candidate(&mut p, &mut pins, false)?;
...
fn cancellation_and_expired_admission_do_not_confirm_or_restore() -> Test {
    ...
    let count = p.visible_roots().count();
    assert!(matches!(
        pins.restore_work(&mut p, ArchiveWorkLimits::default(), 0, 1000, &Cancel),
...
fn original_catalog_restores_without_constructing_a_different_page() -> Test {
    ...
    let expected = {
        ...
        let mut page = None;
        for _ in 0..16 {
            if let JournaledArchiveProgress::WorkConfirmed { checkpoint, .. } =
                w.step(0, &NeverCancel)?
            {
                page = Some(checkpoint.root());
                break;
            }
        }
        let retired = w.retire();
        assert!(retired.writer.archive.prepared_page.is_some());
        (
            page.ok_or("checkpoint not reached")?,
            retired
                .writer
                .archive
                .prepared_page
    ...
    assert_eq!(
        result.catalog.as_ref().ok_or("catalog receipt")?.root,
        expected.1
    );
...
fn source_corruption_is_not_a_license_to_confirm_a_candidate() -> Test {
    ...
    let file = path
        ...
        .join(source.strip_prefix("sha256:").ok_or("digest")?);
