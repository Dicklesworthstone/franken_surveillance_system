...
use fss_reference::rtsp::tcp::{TcpAuthority, TcpBinding, TcpDenial, TcpLimits, TcpOperation,
    TcpSecurityPolicy, TcpWriteStep};
use std::cell::Cell;
use std::io::{Read, Write};
use std::net::{Ipv4Addr, TcpListener, TcpStream};
use std::time::Duration;
const LEASE: u64 = 100_000_000_000;
struct Authority { binding: TcpBinding, checks: Cell<u64>, revoked: Cell<bool> }
...
fn config(peer: std::net::SocketAddr) -> Test<(LiveAvcConfig,LiveRecordingConfig,LiveArchiveConfig,Authority)> {
    let key = StreamKey { ingress: 31, generation: 1, ssrc: 7 };
    let binding = TcpBinding::new(key, peer, "camera.local", TcpSecurityPolicy::OwnerApprovedPlaintext)?;
    let authority = Authority { binding: binding.clone(), checks: Cell::new(0), revoked: Cell::new(false) };
    let live = LiveAvcConfig { protocol: ClientConfig { presentation_uri: "rtsp://camera.local/live/".into(),
        control_root_uri: "rtsp://camera.local/live".into(), media_index: 0, channels: (0,1),
        response_timeout_ns: 10_000_000_000, default_session_timeout_seconds: 60 }, binding,
        transport: TcpLimits::default(), media: AvcReceiveLimits::default(), realm: "fixture-camera".into(),
        digest_policy: DigestPolicy::default(), deadline_ns: LEASE, max_steps: 100_000 };
    let recording = LiveRecordingConfig { scope: scope()?, payload_type: 96, time_scale: 90_000, limits: CollectorLimits::default() };
    let archive = LiveArchiveConfig { namespace: namespace()?, limits: ArchiveLimits { windows_per_page: 1, ..limits() },
        max_window_bytes: 1024 * 1024, max_steps: 100_000, publication_deadline_ns: 2 * LEASE,
...
struct Fixture<'a,'p> { driver: JournaledLiveAvcArchive<'a,'p>, peer: TcpStream, authority: Authority }
impl<'a,'p> Fixture<'a,'p> {
    fn playing(p: &'a mut LocalRootPublisher, pins: &'p mut ArchivePinJournal) -> Test<Self> {
        ...
        let driver = JournaledLiveAvcArchive::connect(live, recording, archive, ArchiveWorkLimits::default(),
            p, pins, 0, &authority, &NeverCancel)?;
        ...
        let mut f = Self { driver, peer, authority };
        assert!(matches!(f.driver.poll(SocketReadiness::default(), 0, &f.authority, &NeverCancel)?,
            JournaledLiveArchiveStep::Live(LiveArchiveStep::Archive(ArchiveWriteProgress::Ready { .. }))));
    ...
    fn command(&mut self, command: ClientCommand, now: u64) -> Test<u32> {
        let request = self.driver.request(command, &credentials()?, [40;16], now, &self.authority, &NeverCancel)?;
        for _ in 0..4096 {
            match self.driver.poll(SocketReadiness { readable: false, writable: true }, now, &self.authority, &NeverCancel)? {
                JournaledLiveArchiveStep::Live(LiveArchiveStep::Recording(step)) => match *step {
                    LiveRecordingStep::Network(LiveAvcStep::Write(TcpWriteStep::Sent { cseq, bytes })) => {
                        assert_eq!(cseq, request.cseq); self.peer.read_exact(&mut vec![0;bytes])?; return Ok(cseq);
                    }
    ...
    fn receive(&mut self, bytes: &[u8], now: u64) -> Test<bool> {
        self.peer.write_all(bytes)?; let mut count = 0;
        for _ in 0..4096 {
            match self.driver.poll(SocketReadiness { readable: true, writable: false }, now, &self.authority, &NeverCancel)? {
                JournaledLiveArchiveStep::Live(LiveArchiveStep::Recording(step)) => match *step {
                    LiveRecordingStep::Network(LiveAvcStep::Wire(chunk)) => { count += chunk.expose().len(); }
    ...
    fn datagram(&mut self, bytes: &[u8], now: u64) -> Test<bool> {
        let mut wire = vec![b'$',0]; wire.extend_from_slice(&u16::try_from(bytes.len())?.to_be_bytes());
        wire.extend_from_slice(bytes); self.receive(&wire, now)
    }
    fn window(&mut self) -> Test {
        let f = fixture(31, true)?; let first = f.packets.first().ok_or("no originals")?;
        self.datagram(&recording_support::packet(0, false, &first.2[12..]), 6)?;
        let mut timing = false;
        for (i, (_, _, bytes)) in f.packets.iter().enumerate() { timing |= self.datagram(bytes, 7+i as u64)?; }
        assert!(timing);
        let _ = self.driver.supply_timing(RecordingTiming { decode_time: 700, duration: 3600, composition_offset: 0 },
            100, &self.authority, &NeverCancel)?;
        assert!(self.driver.seal(101, &self.authority, &NeverCancel)?);
        assert!(matches!(self.driver.poll(SocketReadiness::default(), 101, &self.authority, &NeverCancel)?,
...
fn live_camera_source_crosses_both_journal_barriers_without_socket_read_ahead() -> Test {
    ...
    let mut f = Fixture::playing(&mut p, &mut pins)?; f.window()?;
    let reads = f.driver.totals().ok_or("totals")?.read_calls;
    f.peer.write_all(b"must not be consumed during archive and pin writes")?;
    ...
    assert!(ready); assert_eq!((candidates,confirmed),(2,2));
    assert_eq!(f.driver.totals().ok_or("totals")?.read_calls, reads);
    assert_eq!(f.driver.journal_anchor().sequence, 5);
    let retired = f.driver.cancel().ok_or("retirement")?;
    assert!(retired.live.live.recording.is_some()); assert!(retired.unrecorded_confirmation.is_none());
    drop(f);
...
fn independent_pin_failure_closes_live_socket_and_keeps_unpublished_original() -> Test {
    ...
    let mut p = LocalRootPublisher::open(path.join("media"), storage())?;
    let mut pins = ArchivePinJournal::create(path.join("pins"), pin_scope()?, ArchivePinLimits { max_records: 2, ..ArchivePinLimits::default() }, &NeverCancel)?;
    let mut f = Fixture::playing(&mut p, &mut pins)?; f.window()?;
    let _ = f.driver.poll(SocketReadiness::default(), 101, &f.authority, &NeverCancel)?;
    let error = f.driver.poll(SocketReadiness::default(), 101, &f.authority, &NeverCancel).err().ok_or("confirmation bypassed")?;
    assert!(matches!(error.reason, JournaledLiveArchiveError::Pins(ArchivePinError::Limit)));
    let r = error.retirement.ok_or("source retirement lost")?;
    assert!(r.unrecorded_confirmation.is_some()); assert!(r.pins.candidate().is_some());
    let archive = r.live.live.archive.ok_or("archive retirement lost")?;
    assert!(archive.pending.is_some()); assert!(archive.snapshot.windows().is_empty());
    assert_eq!(f.driver.state(),ClientState::Closed);
    assert!(matches!(f.driver.poll(SocketReadiness::default(), 102, &f.authority, &NeverCancel)?,
        JournaledLiveArchiveStep::Live(LiveArchiveStep::Ended)));
    drop(f); assert!(p.root(&namespace()?.window_slot(0)?).is_none());
    Ok(())
}
#[test]
fn wrong_pin_scope_prevents_any_live_connection_attempt() -> Test {
    ...
    let mut p = LocalRootPublisher::open(path.join("media"), storage())?;
    let wrong = ArchivePinScope { archive_namespace: ContentDigest::sha256(b"other camera"), ..pin_scope()? };
    let mut pins = ArchivePinJournal::create(path.join("pins"), wrong, ArchivePinLimits::default(), &NeverCancel)?;
    let listener = TcpListener::bind((Ipv4Addr::LOCALHOST,0))?;
    let (live,recording,archive,authority) = config(listener.local_addr()?)?;
    let error = JournaledLiveAvcArchive::connect(live, recording, archive, ArchiveWorkLimits::default(),
        &mut p, &mut pins, 0, &authority, &NeverCancel).err().ok_or("scope was accepted")?;
...
fn revoked_camera_authority_cannot_be_bypassed_by_persisting_its_candidate() -> Test {
    ...
    let error = f.driver.poll(SocketReadiness { readable: true, writable: true }, 102, &f.authority, &NeverCancel).err().ok_or("revoked camera continued")?;
    assert!(error.retirement.is_some()); assert_eq!(f.driver.journal_anchor(), anchor);
    assert_eq!(f.driver.state(),ClientState::Closed);
    drop(f); assert!(p.visible_roots().next().is_none());
    Ok(())
}
