...
use fss_reference::ingest::http_archive::{HttpArchiveLimits, HttpWireArchive};
...
impl Directory {
    fn new() -> Test<Self> {
        for n in 0..128 {
            let p =
                std::env::temp_dir().join(format!("fss-record-http-{}-{n}", std::process::id()));
...
fn limits() -> HttpRecordingLimits {
    ...
    l.media.maximum_reads = 64;
    l.media.maximum_source_bytes = 65536;
    l.media.maximum_spool_object_bytes = 65536;
    l.media.maximum_scan_roots = 1024;
    l.media.maximum_frames = 2;
    l.media.read_bytes = 257;
    l.media.source_work = WORK;
    l.decode = HttpCheckDecode::Grayscale;
    l
}
fn source() -> HttpCheckSource {
    HttpCheckSource {
        source: ContentDigest::sha256(b"owned camera recording"),
        generation: 1,
        receive_clock: ContentDigest::sha256(b"receive clock"),
        retention_evidence: ContentDigest::sha256(b"retain original headers and images"),
    }
}
struct Authority {
    route: HttpCameraRoute,
    start: Instant,
    deny: Cell<Option<(HttpCameraOperation, usize)>>,
    seen: Cell<usize>,
}
impl Authority {
    fn deny(&self, op: HttpCameraOperation, n: usize) {
        self.seen.set(0);
        self.deny.set(Some((op, n)));
    }
...
impl HttpCameraAuthority for Authority {
    fn checkpoint(
        ...
        if now >= deadline
            || self.start.elapsed().as_nanos() + u128::from(NOW) >= u128::from(deadline)
        {
            return Err(HttpCameraDenial::Deadline);
        }
        if let Some((target, nth)) = self.deny.get()
            && target == op
        {
...
impl Server {
    fn poll(&mut self) -> Test {
        ...
        if self.request.ends_with(b"\r\n\r\n") {
            ...
            self.socket.write_all(&self.wire)?;
...
) -> Test<(HttpRecording, Authority, Server)> {
...
fn next(r: &mut HttpRecording, a: &Authority, s: &mut Server) -> Test<HttpRecordingStep> {
    s.poll()?;
    Ok(r.poll(a.access(&NeverCancel))?)
}
...
fn to_frame(
    ...
    a: &Authority,
    s: &mut Server,
) -> Test<HttpRecordingFrameKey> {
    for _ in 0..50000 {
        match next(r, a, s)? {
            HttpRecordingStep::WirePrepared(plan) => {
                r.commit_wire(plan, p, a.access(&NeverCancel))?
                    .acknowledgement?;
            }
...
fn capture_publishes_before_parse_and_cold_recovery_verifies_all_three_framings() -> Test {
    for mode in 0..3 {
        ...
        for _ in 0..50000 {
            match next(&mut r, &a, &mut s)? {
                HttpRecordingStep::WirePrepared(plan) => {
                    ...
                    let roots = p.visible_roots().count();
                    for _ in 0..3 {
                        assert_eq!(
                            r.poll(a.access(&NeverCancel))?,
                            HttpRecordingStep::WirePrepared(plan)
                        );
                    }
                    assert_eq!(r.camera().totals(), before);
                    assert_eq!(p.visible_roots().count(), roots);
                    assert!(
                        !r.camera()
                            .pending_wire()
                            .ok_or("wire missing")?
                    ...
                    assert_eq!(
                        receipt.publication.local.claims.local,
                ...
                HttpRecordingStep::CompletionPrepared(pin) => {
                    ...
                    assert_eq!(
                        r.poll(a.access(&NeverCancel))?,
                        HttpRecordingStep::Complete(pin)
                    );
                    done = Some(pin);
                    break;
                }
                HttpRecordingStep::Pending | HttpRecordingStep::Advanced => {
                    std::thread::yield_now()
                }
                _ => return Err("unexpected complete".into()),
            }
        }
        let completion = done.ok_or("recording not complete")?;
        let scope = r.scope();
        assert_eq!(r.transferred_frames(), 2);
        let retired = r.retire();
        assert_eq!(retired.complete, Some(completion));
        drop(retired);
        drop(p);
        drop(s);
        let p = d.open()?;
        ...
        assert_eq!(verified.frames(), 2);
        assert_eq!(verified.peer_eof(), mode == 2);
    }
    Ok(())
}
#[test]
fn existing_generation_is_refused_before_a_second_connection() -> Test {
    ...
    let plan = to_wire(&mut r, &a, &mut s)?;
    r.commit_wire(plan, &mut p, a.access(&NeverCancel))?
        .acknowledgement?;
    let request = HttpRecordingRequest {
        route: a.route.clone(),
        scope: r.scope(),
        limits: limits(),
        deadline_ns: DEADLINE,
    };
    let error = match HttpRecording::connect(request, &p, a.access(&NeverCancel)) {
        Ok(_) => return Err("generation reused".into()),
        Err(e) => e,
    };
    assert!(!error.attempted);
    assert!(matches!(error.reason, HttpRecordingError::Archive(_)));
    Ok(())
}
#[test]
fn every_wire_publication_crash_cut_preserves_raw_input_and_exact_expected_pin() -> Test {
    for cut in [
        ...
        assert_eq!(r.pin(), before);
        assert_eq!(r.pending_wire_plan(), Some(plan));
        assert_eq!(
            r.camera().pending_wire().ok_or("raw dropped")?.bytes(),
            bytes
        );
        assert!(
            !r.camera()
        ...
        if cut == PublishCutPoint::AfterRootRename {
            let receipt = r.commit_wire(plan, &mut p, a.access(&NeverCancel))?;
            assert_eq!(
                receipt.publication.local.outcome,
                PublishOutcome::AlreadyPublished
            );
            assert_eq!(r.pin(), plan.expected_pin());
            assert_eq!(receipt.acknowledgement, Ok(()));
        }
...
fn late_camera_revocation_preserves_successful_wire_publication() -> Test {
    ...
    let (mut r, a, mut s) = connect(&p, wire(0, 2, JPEG), limits())?;
    let plan = to_wire(&mut r, &a, &mut s)?;
    a.deny(HttpCameraOperation::AcknowledgeWire, 1);
    ...
    let retired = r.retire();
    assert!(retired.source.wire.is_some());
    assert_eq!(retired.wire_plan, Some(plan));
    Ok(())
}
struct Stop;
...
fn storage_denial_does_not_acknowledge_or_lose_the_pending_read() -> Test {
    ...
    let before = r.pin();
    assert!(matches!(
        r.commit_wire(plan, &mut p, a.access(&Stop)),
...
fn final_frame_release_denial_retains_decoded_pixels_and_original_frame() -> Test {
    ...
    let retired = r.retire();
    assert_eq!(
        retired.source.frame.ok_or("frame lost")?.part().bytes(),
        JPEG
    );
    assert_eq!(
        retired
            .frame_check
            .ok_or("accepted decode lost")?
            .decoded
            .ok_or("pixels lost")?
            .dimensions(),
        [17, 13]
    );
    assert_eq!(retired.transferred_frames, 0);
    Ok(())
}
#[test]
fn decode_budget_is_not_refilled_and_failed_decode_never_implies_an_empty_scene() -> Test {
    ...
    assert!(r.pin().bytes > 0);
    assert!(r.camera().pending_frame().is_some());
    assert!(r.completion().is_none());
    assert!(r.retire().frame_error.is_some());
    Ok(())
}
#[test]
fn frame_limit_leaves_a_partial_recording_without_issuing_a_terminal_root() -> Test {
    ...
    assert!(stopped);
    assert_eq!(r.transferred_frames(), 1);
    assert!(r.completion().is_none());
    assert!(r.retire().source.frame.is_some());
    Ok(())
}
#[test]
fn stale_wire_and_frame_keys_cannot_release_later_input() -> Test {
    ...
    let mut p = d.open()?;
    let (mut r, a, mut s) = connect(&p, wire(0, 2, JPEG), limits())?;
    let first = to_wire(&mut r, &a, &mut s)?;
    r.commit_wire(first, &mut p, a.access(&NeverCancel))?
        .acknowledgement?;
    assert!(matches!(
        r.commit_wire(first, &mut p, a.access(&NeverCancel)),
...
fn preflight_capacity_and_authority_refusals_make_no_connection() -> Test {
    ...
    for bad in [true, false] {
        let mut l = limits();
        if bad {
            l.media.maximum_spool_object_bytes = 1024;
        }
        let request = HttpRecordingRequest::new(
            source(),
            listener.local_addr()?,
        ...
        if !bad {
            a.deny(HttpCameraOperation::Connect, 1);
        }
        let result = HttpRecording::connect(request, &p, a.access(&NeverCancel));
        let error = match result {
            Ok(_) => return Err("preflight accepted".into()),
            Err(e) => e,
        };
        assert!(!error.attempted);
