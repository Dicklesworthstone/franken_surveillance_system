...
use fss_reference::rtsp::hevc_recording_collector::{
    HevcCollectionAdmission, HevcRecordingCollector,
};
...
use fss_reference::rtsp::recording_collector::{
    CollectionStop, CollectorError as E, CollectorLimits,
};
...
fn observations(packets: &[Packet]) -> Result<Vec<(u64, HevcPictureGroup)>, Error> {
    ...
    for p in packets {
        let output = dep.push(
            KEY,
            p.sequence,
            RtpPacket::parse(&p.bytes, PacketLimits::default())?,
            0,
        )?;
        ...
        for nal in output.nals {
            match a.push(nal, 0) {
                HevcAssemblyStep::Accepted(output) => {
                    ...
                    assert!(output.standalone.is_none());
                    if let Some(picture) = output.picture {
                        pictures.push((p.sequence, picture));
                    }
                }
...
fn push(c: &mut HevcRecordingCollector, p: &Packet, now: u64) -> Result<(), Error> {
    c.push_source(KEY, borrowed(std::slice::from_ref(p))[0], now)?;
    Ok(())
}
fn drive(
    c: &mut HevcRecordingCollector,
    packets: &[Packet],
    take: bool,
    ...
    for (i, p) in packets.iter().enumerate() {
        push(c, p, i as u64)?;
        for (index, (_, picture)) in groups
            .iter()
            .enumerate()
            .filter(|(_, (seq, _))| *seq == p.sequence)
        {
            c.push_picture(picture, timing[index], i as u64)?;
            if take && let Some(window) = c.take_ready() {
                windows.push(window);
            }
        }
    }
...
#[test]
fn continuous_fixture_yields_two_replay_verified_idr_windows_with_exact_shared_witness()
-> TestResult {
    let packets = packets()?;
    ...
    let left = windows[0].packets()?;
    let right = windows[1].packets()?;
    assert_eq!(left.last().ok_or("left witness")?.bytes, packets[5].bytes);
    assert_eq!(
        right.first().ok_or("right witness")?.bytes,
        packets[5].bytes
    );
    for window in &windows {
        verify_hevc_recording(window.manifest(), window.objects(), &scope()?)?;
    }
    let trailing = c.cancel();
    assert_eq!(trailing.reason, CollectionStop::EndOfInput);
    assert!(trailing.pictures.is_empty());
    assert!(trailing.ready.is_none());
    assert_eq!(
        trailing.sources.last().ok_or("trailing EOS")?.bytes(),
        packets[10].bytes
    );
    Ok(())
}
...
fn aggregation_starting_a_new_idr_is_cut_without_splitting_or_losing_parameters() -> TestResult {
    ...
    input.extend_from_slice(&original[9..]);
    resequence(&mut input);
    let mut c = collector(CollectorLimits::default())?;
    let mut windows = drive(&mut c, &input, true)?;
    c.finish(20)?;
    windows.push(c.take_ready().ok_or("final root")?);
    assert_eq!(windows.len(), 2);
    assert_eq!(windows[0].source_only_nals(), 4);
    assert_eq!(
        windows[0].packets()?.last().ok_or("AP witness")?.bytes,
        windows[1].packets()?.first().ok_or("AP start")?.bytes
    );
    for w in windows {
        verify_hevc_recording(w.manifest(), w.objects(), &scope()?)?;
    }
    Ok(())
}
...
fn same_packet_closes_previous_picture_and_next_idr_so_the_cut_is_deferred() -> TestResult {
    ...
    let mut group = n[5..9].to_vec();
    group.push(vec![0x48, 1, 0x80]);
    input.push(packet(3, 36_000, true, &aggregate(&group)));
    let mut c = collector(CollectorLimits::default())?;
    let mut windows = drive(&mut c, &input, true)?;
    assert!(windows.is_empty());
    c.finish(20)?;
    windows.push(c.take_ready().ok_or("deferred root")?);
...
fn fragmented_boundary_parameter_keeps_every_fu_in_both_source_roots() -> TestResult {
    ...
    for (flags, body) in [(0x80, &vps[2..split]), (0x40, &vps[split..])] {
        let mut fu = vec![0x62, 1, flags | 32];
        fu.extend_from_slice(body);
        input.push(packet(0, 36_000, flags == 0x40, &fu));
    }
    input.extend_from_slice(&original[6..]);
    resequence(&mut input);
    let mut c = collector(CollectorLimits::default())?;
    let mut windows = drive(&mut c, &input, true)?;
    c.finish(20)?;
    windows.push(c.take_ready().ok_or("final")?);
    let left = windows[0].packets()?;
    let right = windows[1].packets()?;
...
fn ready_output_backpressures_sources_and_retries_without_advancing_sequence() -> TestResult {
    ...
    assert!(c.has_ready());
    let before = c.retained_packets();
    assert_eq!(
        c.push_source(KEY, borrowed(&packets[10..])[0], 10),
        Err(E::Backpressure)
    );
...
fn corrected_timing_retries_same_borrowed_picture_and_preserves_source() -> TestResult {
    ...
    let mut c = collector(CollectorLimits::default())?;
    for (i, p) in input[..5].iter().enumerate() {
        push(&mut c, p, i as u64)?;
    }
    let mut t = timings(1)[0];
    t.duration = 0;
    assert_eq!(c.push_picture(&groups[0].1, t, 4).err(), Some(E::Timeline));
    assert_eq!(c.retained_samples(), 0);
    assert_eq!(c.retained_packets(), 5);
    assert!(matches!(
        c.push_picture(&groups[0].1, timings(1)[0], 4)?,
        HevcCollectionAdmission::Accepted { .. }
    ));
    assert_eq!(c.retained_samples(), 1);
    assert_eq!(
        c.push_picture(&groups[0].1, timings(2)[1], 4).err(),
        Some(E::SourceOrder)
    );
    Ok(())
}
...
fn wrong_owner_missing_source_and_reversed_clock_leave_collection_unchanged() -> TestResult {
    ...
    let mut c = collector(CollectorLimits::default())?;
    assert_eq!(
        c.push_picture(&groups[0].1, timings(1)[0], 0).err(),
        Some(E::MissingSource)
    );
    ...
    assert_eq!(
        c.push_source(wrong, borrowed(&input[1..2])[0], 11),
        Err(E::StreamMismatch)
    );
    assert_eq!(
        c.push_source(KEY, borrowed(&input[1..2])[0], 9),
        Err(E::ClockReversed)
    );
    assert_eq!(
        c.push_source(KEY, borrowed(&input[2..3])[0], 11),
        Err(E::SourceOrder)
    );
...
fn rotation_preserves_original_lookahead_age_and_expiry_returns_all_ownership() -> TestResult {
    ...
    assert_eq!(
        stopped.ready.ok_or("prepared root lost")?.summary().samples,
        2
    );
    assert_eq!(
        stopped.sources.first().ok_or("lookahead lost")?.sequence(),
        input[5].sequence
    );
    assert_eq!(stopped.pictures.len(), 1);
    assert_eq!(c.retained_source_bytes(), 0);
...
fn startup_predicted_picture_is_returned_to_owner_selection_not_fabricated_as_idr() -> TestResult {
    ...
    assert!(windows.is_empty());
    c.finish(10)?;
    windows.push(c.take_ready().ok_or("IDR root")?);
...
fn explicit_seal_preserves_real_witness_and_unclosed_eof_tail_is_not_recorded() -> TestResult {
    ...
    c.finish(20)?;
    windows.push(c.take_ready().ok_or("closed prefix")?);
    assert_eq!(
        windows.iter().map(|w| w.summary().samples).sum::<usize>(),
        3
    );
...
fn source_and_sample_capacity_refusals_preserve_exact_retryable_inputs() -> TestResult {
    ...
    assert_eq!(
        c.push_source(KEY, borrowed(&input[1..2])[0], 1),
        Err(E::Capacity)
    );
    ...
    for (i, p) in input[..6].iter().enumerate() {
        push(&mut c, p, i as u64)?;
    }
    c.push_picture(&groups[0].1, timings(2)[0], 5)?;
    assert_eq!(
        c.push_picture(&groups[1].1, timings(2)[1], 5).err(),
        Some(E::Capacity)
    );
    assert_eq!(c.retained_samples(), 1);
    assert_eq!(c.retained_packets(), 6);
    Ok(())
}
...
fn deterministic_roots_keep_receive_times_and_reject_rebound_source_bytes() -> TestResult {
    ...
    assert_eq!(
        c.push_picture(&groups[0].1, timings(1)[0], 4).err(),
        Some(E::Source)
    );
    assert_eq!(c.retained_samples(), 0);
    Ok(())
