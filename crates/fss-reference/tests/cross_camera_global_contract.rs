...
use fss_reference::ingest::cross_camera::{
    ...
    CameraObservation, CrossCameraConfig, CrossCameraError, CrossCameraReport, MAX_CAMERA_ID_BYTES,
    MAX_CROSS_CAMERA_OBSERVATIONS, associate, associate_detailed,
};
...
fn config() -> CrossCameraConfig {
    CrossCameraConfig {
        max_time_delta_ns: 100,
        max_position_distance: 1.5,
        min_confidence: 0.1,
    }
}
...
fn crossing_targets_use_the_better_global_pairing_not_the_best_first_edge() -> Test {
    ...
    assert_eq!(
        result.left_dispositions(),
        &[
            AssociationDisposition::Matched(1),
            AssociationDisposition::Matched(0)
        ]
    );
...
fn tied_assignments_are_retained_and_the_pair_only_api_refuses_to_flatten_them() -> Test {
    ...
    assert_eq!(
        result.left_dispositions(),
        &[AssociationDisposition::Unresolved; 2]
    );
    assert_eq!(
        result.right_dispositions(),
        &[AssociationDisposition::Unresolved; 2]
    );
    for alternative in result.alternatives() {
        assert_eq!(alternative.cost, result.assignment_cost());
        assert_ne!(
            alternative.columns[alternative.excluded.0],
            Some(alternative.excluded.1)
        );
        assert_ne!(alternative.columns[0], alternative.columns[1]);
    }
    assert!(matches!(
        associate(&config(), &left, &right),
        Err(CrossCameraError::AmbiguousAssignment)
    ));
    Ok(())
}
...
fn stable_pair_survives_alongside_a_different_ambiguous_component() -> Test {
    ...
    assert_eq!(
        ...
        &[
            ...
            AssociationDisposition::Unresolved,
            AssociationDisposition::Matched(2),
        ]
    );
    assert_eq!(
        result.right_dispositions()[2],
        AssociationDisposition::Matched(2)
    );
    let stable = result.stable_pairs(&mut WorkBudget::new(100_000))?;
    assert_eq!(stable.len(), 1);
    assert_eq!(
        (stable[0].first.track_id, stable[0].second.track_id),
        (3, 30)
    );
    for alternative in result.alternatives() {
...
fn zero_gain_edge_is_ambiguous_with_leaving_the_observation_unmatched() -> Test {
    ...
    let result = associate_detailed(
        &config,
        0,
        &[obs("left", 1, 0.0)],
        &[obs("right", 2, config.max_position_distance)],
...
fn geometric_norm_preserves_large_and_tiny_finite_coordinates() -> Test {
    for scale in [1.0e200, 1.0e-200] {
        let config = CrossCameraConfig {
            max_position_distance: 2.0 * scale,
            ..config()
        };
        let left = [obs("left", 1, 0.0)];
...
fn malformed_observations_are_rejected_even_when_the_other_camera_is_empty() {
    for nonfinite in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
        assert!(matches!(
            report(&[obs("left", 1, nonfinite)], &[]),
            Err(CrossCameraError::InvalidObservation(_))
        ));
    }
    for camera in ["", "hidden\nrecord"] {
        assert!(matches!(
            report(&[obs(camera, 1, 0.0)], &[]),
            Err(CrossCameraError::InvalidObservation(_))
        ));
    }
    assert!(matches!(
        report(&[obs("left", 0, 0.0)], &[]),
        Err(CrossCameraError::InvalidObservation(_))
    ));
    assert!(matches!(
        report(&[obs(&"x".repeat(MAX_CAMERA_ID_BYTES + 1), 1, 0.0)], &[]),
        Err(CrossCameraError::Limit)
    ));
    assert!(matches!(
        report(&[obs("left", 1, 0.0), obs("left", 1, 1.0)], &[]),
        Err(CrossCameraError::DuplicateObservation)
    ));
    assert!(matches!(
        report(&[obs("left", 1, 0.0), obs("other", 2, 0.0)], &[]),
        Err(CrossCameraError::InvalidObservation(_))
    ));
}
...
fn empty_batches_and_unmatched_tracks_are_retained_without_forced_pairs() -> Test {
    ...
    assert_eq!(
        no_right.left_dispositions(),
        &[AssociationDisposition::NoCandidate; 2]
    );
...
fn full_cartesian_report_keeps_named_time_position_confidence_and_same_camera_exclusions() -> Test {
    ...
    assert_eq!(
        ...
        vec![
            AssociationScore::Excluded(AssociationExclusion::Time),
            AssociationScore::Excluded(AssociationExclusion::Position),
            AssociationScore::Excluded(AssociationExclusion::Confidence)
        ]
    );
    let same = report(&left, &left)?;
    assert_eq!(
        same.candidates()[0].score,
        AssociationScore::Excluded(AssociationExclusion::SameCamera)
    );
    Ok(())
}
...
fn complete_input_limits_cancel_and_late_budget_exhaustion_are_typed() -> Test {
    let oversized: Vec<_> = (0..=MAX_CROSS_CAMERA_OBSERVATIONS)
        .map(|i| obs("left", i as u64 + 1, 0.0))
        .collect();
    assert!(matches!(
        report(&oversized, &[]),
        Err(CrossCameraError::Limit)
    ...
    assert!(matches!(
        ...
        Err(CrossCameraError::Assignment(ImageTrackingError::Geometry(
    ...
    assert!(matches!(
        associate_detailed(
            ...
            &left,
            &right,
            &mut WorkBudget::new(budget.used() - 1)
        ),
        Err(CrossCameraError::Assignment(ImageTrackingError::Geometry(
...
fn enumerate(costs: &[Option<u64>], columns: usize, excluded: Option<(usize, usize)>) -> u64 {
    ...
    for a in 0..columns {
        for b in 0..columns {
            if a == b || excluded == Some((0, a)) || excluded == Some((1, b)) {
                continue;
            }
...
fn global_objective_and_every_ambiguity_decision_match_exhaustive_assignments() -> Test {
    ...
    for seed in 0..200_u32 {
        let left = [
            obs("left", 1, f64::from(seed % 7)),
            obs("left", 2, f64::from((seed / 7) % 7)),
        ];
        let right = [
            obs("right", 10, f64::from((seed * 3) % 11)),
            obs("right", 20, f64::from((seed * 5 + 1) % 11)),
            obs("right", 30, f64::from((seed * 7 + 2) % 11)),
        ];
        ...
        for candidate in result.candidates() {
            if let AssociationScore::Admissible { units, .. } = candidate.score {
                costs[candidate.left * 5 + candidate.right] =
                    Some(u64::from(ASSOCIATION_SCORE_SCALE - units));
            }
        }
            ...
            .filter(|candidate| candidate.selected)
        {
            ...
            assert_eq!(
                candidate.ambiguous,
                alternate <= best + result.effective_margin()
            );
        }
    }
    Ok(())
