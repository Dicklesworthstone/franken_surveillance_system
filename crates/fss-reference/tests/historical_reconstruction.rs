...
use fss_reference::rtsp::datagram_reconstruction::recording::{
    DatagramRecordingReplay, RecordingReplayStep,
};
...
fn continued_capture_does_not_change_old_recordings_result_identity_or_selected_counts() -> Test {
    ...
    let before = load(&f, &p)?;
    let expected = PreparedReconstruction::prepare(
        &before,
        &p,
        bounds(),
    ...
    let actual = PreparedReconstruction::prepare(
        &after,
        &p,
        bounds(),
        ReconstructionLimits::default(),
    ...
    for (a, b) in actual.windows().iter().zip(expected.windows()) {
        ...
        assert_eq!(a.objects().source, b.objects().source);
        assert_eq!(a.objects().media, b.objects().media);
        assert_eq!(a.objects().index, b.objects().index);
    }
...
fn lost_publication_reply_then_new_capture_then_cold_retry_reuses_original_roots() -> Test {
    ...
    let first = {
        ...
        let pin = plan
            .publish(&mut p, u64::MAX, &Clock(1000), &NeverCancel, &mut work())?
            .pin;
        append_tail(&mut p)?;
        pin
    };
    let mut p = open(&f.path)?;
    let head = current(&p)?.pin();
    let loaded = load(&f, &p)?;
    let plan = PreparedReconstruction::prepare(
        &loaded,
        &p,
    ...
    assert!(
        retry
            .windows
            .iter()
            .all(|r| r.outcome == PublishOutcome::AlreadyPublished)
    );
...
fn growth_between_loading_preparation_and_publication_never_retargets_the_input() -> Test {
    ...
    assert_eq!(result.windows.len(), f.expected.len());
    assert_eq!(current(&p)?.pin(), head);
    Ok(())
}
...
fn later_fragment_completion_does_not_repair_an_older_incomplete_recipe() -> Test {
    ...
    let mut packet = source::packet(3, 9000, &payload);
    packet[1] |= 128;
    ...
    assert_eq!(actual.pin(), expected.pin());
    assert_eq!(actual.summary(), expected.summary());
    assert!(actual.windows().is_empty());
    assert!(actual.summary().fragment_bytes > 0);
...
fn old_and_new_recipes_over_one_growing_camera_chain_remain_independently_replayable() -> Test {
    ...
    let nals = source::nals();
    let idr = *nals.iter().find(|n| n[0] & 31 == 5).ok_or("missing IDR")?;
    let mut packet = source::packet(3, 12600, idr);
    packet[1] |= 128;
    append_source(&mut p, 0, &packet)?;
    let all = current(&p)?;
    let avc = old.recipe().avc_spec();
    let recording = old.recipe().recording_spec().clone();
    ...
    for _ in 0..256 {
        match replay.step(&p, 1000, &NeverCancel, &mut work())? {
            ...
            {
                let CapturePoll::TimingRequired(picture) = *event else {
            ...
            RecordingReplayStep::Stopped { .. } | RecordingReplayStep::Ended => {
                return Err("extended replay stopped".into());
            }
            _ => {}
        }
    }
    ...
    let new_pin = PreparedRecordingRecipe::prepare(
        &recipe,
        &all,
        &p,
        limits().storage,
    ...
    .publish(&mut p, &NeverCancel, &mut work())?
    .pin;
    let newer = LoadedRecordingRecipe::load(
        &p,
        RecipeSelection {
            recipe: new_pin.recipe,
            root: new_pin.root,
            scope: source_scope()?,
        },
        limits(),
    ...
    let historical = load(&f, &p)?;
    let old_plan = PreparedReconstruction::prepare(
        &historical,
        &p,
        bounds(),
        ReconstructionLimits::default(),
    ...
    let new_plan = PreparedReconstruction::prepare(
        &newer,
        &p,
        bounds(),
        ReconstructionLimits::default(),
    ...
    assert_eq!(old_plan.summary().timings_applied, 1);
    assert_eq!(new_plan.summary().timings_applied, 2);
...
fn corrupt_descendant_after_preparation_blocks_every_output_write() -> Test {
    ...
    std::fs::write(
        f.path
            .join("spool/objects")
            .join(digest.strip_prefix("sha256:").ok_or("digest")?),
        b"bad envelope",
    )?;
    let failure = plan
        .publish(&mut p, u64::MAX, &Clock(2000), &NeverCancel, &mut work())
        .err()
        .ok_or("ignored corrupt descendant")?;
...
fn full_namespace_bounds_and_observed_head_rollback_are_not_bypassed_by_old_recipe() -> Test {
    ...
    ceiling.source.max_datagrams = f.pin.source.datagrams as usize;
    assert!(
        LoadedRecordingRecipe::load(&p, select(&f)?, ceiling, &NeverCancel, &mut work()).is_err()
    );
    let loaded = load(&f, &p)?;
    let plan = PreparedReconstruction::prepare(
        &loaded,
    ...
    let slot = p
        .visible_roots()
        .find(|r| r.root == head.head)
        .ok_or("tail root missing")?
        .slot
        .clone();
    ...
    let failure = plan
        .publish(&mut p, u64::MAX, &Clock(1000), &NeverCancel, &mut work())
        .err()
        .ok_or("forgot later observed head")?;
    assert!(failure.windows.is_empty());
    assert!(p.root(&plan.pin().slot).is_none());
    Ok(())
}
...
fn late_timing_mismatch_still_returns_unpublished_windows_when_history_grows() -> Test {
    ...
    let loaded = load(&f, &p)?;
    let before = p.visible_roots().count();
    let failure = PreparedReconstruction::prepare(
        &loaded,
        &p,
        bounds(),
        ReconstructionLimits::default(),
