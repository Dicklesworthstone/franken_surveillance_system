...
fn every_split_preserves_the_exact_challenge_and_opaque_body() -> TestResult {
    ...
    for cut in 0..=bytes.len() {
        ...
        assert!(
            matches!(frame.event(), RtspEvent::AuthRequired { response, .. } if response.status_code == 401)
        );
...
fn mixed_frames_are_delivered_in_wire_order_without_a_batch_queue() -> TestResult {
    ...
    let mut input = RtspWireIntake::new();
    input.ingest(&bytes, 5)?;
    for expected in [first.as_slice(), packet, second] {
        let frame = input.poll(10)?.ok_or("frame")?;
        assert_eq!(frame.expose_wire(), expected);
        assert_eq!(frame.received_ns(), 5);
    }
...
fn incomplete_binary_payload_cannot_be_reinterpreted_as_a_challenge() -> TestResult {
    ...
    wire.extend_from_slice(&(challenge.len() as u16).to_be_bytes());
    wire.extend_from_slice(&challenge);
    let mut input = RtspWireIntake::new();
    for chunk in wire.chunks(3) {
        input.ingest(chunk, 1)?;
        if let Some(frame) = input.poll(1)? {
            assert!(
                matches!(frame.event(), RtspEvent::Interleaved { span, .. } if *span == challenge)
            );
            assert_eq!(frame.expose_wire(), wire);
        }
    }
    input.finish();
    assert!(input.poll(1)?.is_none());
    assert!(input.is_ended());
    Ok(())
}
#[test]
fn intake_backpressure_does_not_consume_input_or_advance_time() -> TestResult {
    let bytes = response(1, b"");
    let mut input = RtspWireIntake::new();
    input.ingest(&bytes, 1)?;
    assert_eq!(
        input.ingest(b"not admitted", 100),
        Err(WireIntakeError::Backpressure)
    );
...
fn deadline_is_checked_before_a_late_final_byte_can_clear_it() -> TestResult {
    let bytes = response(1, b"");
    let mut input = RtspWireIntake::new();
    input.ingest(&bytes[..bytes.len() - 1], 0)?;
    assert!(input.poll(0)?.is_none());
    assert_eq!(
        input.ingest(&bytes[bytes.len() - 1..], WIRE_LIFETIME_NS),
        Err(WireIntakeError::Deadline)
    );
    assert!(matches!(
        input.poll(WIRE_LIFETIME_NS),
        Err(WireIntakeError::Deadline)
    ));
...
fn lookahead_uses_original_admission_time_not_processing_time() -> TestResult {
    ...
    assert!(input.poll(100)?.is_none());
    assert_eq!(input.deadline_ns(), Some(5 + WIRE_LIFETIME_NS));
    assert!(matches!(
        input.poll(5 + WIRE_LIFETIME_NS),
        Err(WireIntakeError::Deadline)
    ));
...
fn valid_prefix_survives_but_malformed_suffix_never_resynchronizes() -> TestResult {
    let valid = response(1, b"");
    let bad = b"RTSP/2.0 200 NO\r\nCSeq: 2\r\n\r\n";
    let mut input = RtspWireIntake::new();
    input.ingest(&[valid.as_slice(), bad, valid.as_slice()].concat(), 1)?;
    ...
    assert_eq!(
        input.cancel().expose(),
        [bad.as_slice(), valid.as_slice()].concat()
    );
    Ok(())
}
#[test]
fn conflicting_duplicate_signed_and_folded_lengths_are_refused() -> TestResult {
    for header in [
        ...
        input.ingest(&bytes, 1)?;
        assert!(input.poll(1).is_err());
        assert_eq!(input.cancel().expose(), bytes);
    }
    Ok(())
}
#[test]
