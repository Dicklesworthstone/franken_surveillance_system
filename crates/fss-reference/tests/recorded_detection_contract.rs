...
use fss_reference::ingest::detections::{BoxEncoding, CoordinateSpace, DetectionBudget, DetectionContract,
    DetectionError, DetectionFrame, DetectionSpec};
...
impl Directory {
    fn new() -> TestResult<Self> {
        for n in 0..100 {
            let path = std::env::temp_dir().join(format!("fss-detections-{}-{n}", std::process::id()));
...
fn detector_model(width: u32, height: u32) -> TestResult<RecordedModel> {
    ...
    let graph = ModelIrGraph::new_validated("model:detector-row-fixture", ModelIrVersion::V1, generation,
        vec![port("image",vec![1,1,height as usize,width as usize])?,
            port("weights",vec![pixels,18])?, port("bias",vec![1,18])?],
        ...
        vec![node("flatten",OpCode::Reshape,&["image"],"flat",
            ...
            node("add",OpCode::Add,&["zero","bias"],"rows",AttributeMap::new())?,
            node("reshape",OpCode::Reshape,&["rows"],"detections",
    ...
    Ok(RecordedModel::publish(&graph,"image",true,BTreeMap::from([
        ...
        ("bias".into(),vec![0.125,0.125,0.5,0.5,0.9,0., 0.125,0.125,0.5,0.5,0.8,0.,
            0.125,0.125,0.5,0.5,0.7,1.]),
    ]))?)
}
fn spec(model: &RecordedModel) -> DetectionSpec {
    DetectionSpec { model_digest:model.digest(), output_port:"detections".into(),
        ...
        maximum_rows:4096, maximum_detections:256 }
}
#[test]
fn retained_model_detections_reopen_without_source_or_model_files() -> TestResult {
    let dir = Directory::new()?; let root = dir.0.join("deployment"); let source = dir.0.join("source.mjpeg");
    fs::write(&source,[JPEG,JPEG].concat())?;
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id:"trace:detection-test".into(), operation_id:OperationId::parse("operation:detection-test")?,
        principal:"principal:detection-test".into(), capabilities:vec!["ADP-REPLAY-001".into()],
        ...
        anchor_universe:ContentDigest::sha256(b"site:detection-test"),generation:1,
    })?;
    authority.validate()?;
    ...
    let imported = FileIngestAdapter::ingest(FileIngestRequest::new(&source,
        SensorId::parse("sensor:detection-test")?,StreamId::parse("stream:detection-test")?)
    ...
    let [w,h] = frame.receipt().dimensions(); let model = detector_model(w,h)?;
    ...
    let first = DetectionFrame::read(&deployment,run.identity(),&request,&contract,&mut budget,&cx)?;
    assert_eq!(first.counts(),[3,0,1]); assert_eq!(first.detections().len(),2);
    assert_eq!(first.run_identity(),run.identity()); assert_eq!(first.frame_root(),frame.publication_root());
    assert_eq!(first.capsule(),frame.receipt().capsule());
    assert_eq!(*deployment.current_anchor(),before);
    assert!(matches!(DetectionFrame::read(&deployment,run.identity(),&request,&contract,
        &mut DetectionBudget::new(0),&cx),Err(DetectionError::BudgetExceeded)));
    let mut wrong = spec(&model); wrong.model_digest = ContentDigest::sha256(b"different weights");
    assert!(matches!(DetectionFrame::read(&deployment,run.identity(),&request,&DetectionContract::new(wrong)?,
        &mut budget,&cx),Err(DetectionError::InvalidContract)));
    request.segment_index = 1;
    RecordedFrame::decode_and_publish(&mut deployment,&request,&mut DecodeBudget::new(100_000_000),&cx)?;
    assert!(DetectionFrame::read(&deployment,run.identity(),&request,&contract,&mut budget,&cx).is_err());
    request.segment_index = 0;
    let after = deployment.current_anchor().clone();
    fs::remove_file(&source)?; drop(deployment);
    let reopened = ReferenceDeployment::open(&root,"site:detection-test",&cx)?;
    let restored = DetectionFrame::read(&reopened,run.identity(),&request,&contract,&mut budget,&cx)?;
    assert_eq!(first.encoded()?,restored.encoded()?); assert_eq!(first.digest()?,restored.digest()?);
    assert_eq!(*reopened.current_anchor(),after);
    cx.request_cancellation();
    assert!(matches!(DetectionFrame::read(&reopened,run.identity(),&request,&contract,&mut budget,&cx),Err(DetectionError::Cancelled)));
    Ok(())
}
