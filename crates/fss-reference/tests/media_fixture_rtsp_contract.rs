...
/// Pinned SHA-256 digest of clean RTSP transcript fixture.
const PINNED_CLEAN_SHA256: &str =
    "f0640fca1a33fe43e8335cb01013e0f7d98589b86c45a8480ef75ced345d24aa";
/// Pinned SHA-256 digest of auth_required RTSP transcript fixture.
const PINNED_AUTH_REQUIRED_SHA256: &str =
    "c2d19f964ccfaa656b89e7c48cd9d7272558a420b7a687ab40a50aa88ed63a0c";
/// Pinned SHA-256 digest of interleave_split RTSP transcript fixture.
const PINNED_INTERLEAVE_SPLIT_SHA256: &str =
    "a0eccf64599579ee98d0b335b2b16d957a70605166dae3a559dbf665d49f8ae1";
/// Pinned SHA-256 digest of bad_content_length RTSP transcript fixture.
const PINNED_BAD_CONTENT_LENGTH_SHA256: &str =
    "4ba38d4cf689abe2d505cbed0321d31397eb5b1347ed1106c9cb9a4ede5b1342";
/// Pinned SHA-256 digest of session_timeout RTSP transcript fixture.
const PINNED_SESSION_TIMEOUT_SHA256: &str =
    "54557ffaaa43a00c205ab7a807186502784a19ddb13300e3b32b84fb29f5cacd";
/// Pinned SHA-256 digest of rtcp_rsize RTSP transcript fixture.
const PINNED_RTCP_RSIZE_SHA256: &str =
    "e503ecc51b1d34afbdb53e50f14ee31a494b92d1c142c74c96d23e83a208f88e";
/// Pinned SHA-256 digest of sr_absent RTSP transcript fixture.
const PINNED_SR_ABSENT_SHA256: &str =
    "e154d42cdf5d5a76d3304a6d13a13d98e56b922bf55ba783bb0e581d42d10e32";
/// Pinned SHA-256 digest of get_parameter_keepalive RTSP transcript fixture.
const PINNED_GET_PARAMETER_SHA256: &str =
    "60acc52f1e58877ea5aceb2b40eb0a3ecf313b9a7a13ac0f91383269b7b46675";

/// Expected per-record byte lengths for clean transcript.
const CLEAN_RECORD_LENGTHS: &[u32] = &[
    86, 78, 85, 368, 115, 106, 90, 126, 72, 18, 18, 33, 66, 1204, 248, 337, 18, 401, 72, 18, 545,
    18, 401, 18, 401, 72, 79, 47,
];

/// Expected per-record byte lengths for auth_required transcript.
const AUTH_REQUIRED_RECORD_LENGTHS: &[u32] = &[86, 132];

/// Expected per-record byte lengths for interleave_split transcript.
const INTERLEAVE_SPLIT_RECORD_LENGTHS: &[u32] = &[
    86, 78, 85, 368, 115, 106, 90, 126, 72, 18, 18, 33, 66, 600, 604, 248, 337, 18, 401, 72, 18,
    545, 18, 401, 18, 401, 72, 79, 47,
];

/// Expected per-record byte lengths for bad_content_length transcript.
const BAD_CONTENT_LENGTH_RECORD_LENGTHS: &[u32] = &[86, 78, 85, 377];

/// Expected per-record byte lengths for session_timeout transcript.
const SESSION_TIMEOUT_RECORD_LENGTHS: &[u32] = &[
    86, 78, 85, 368, 115, 106, 90, 126, 72, 18, 18, 33, 66, 1204, 248, 337, 18, 401, 72, 18, 545,
    18, 401, 18, 401, 72, 79, 47,
];

/// Expected per-record byte lengths for rtcp_rsize transcript.
const RTCP_RSIZE_RECORD_LENGTHS: &[u32] = &[
    86, 78, 85, 382, 115, 106, 90, 126, 32, 18, 18, 33, 66, 1204, 248, 337, 18, 401, 32, 18, 545,
    18, 401, 18, 401, 32, 79, 47,
];

/// Expected per-record byte lengths for sr_absent transcript.
const SR_ABSENT_RECORD_LENGTHS: &[u32] = &[
    86, 78, 85, 368, 115, 106, 90, 126, 18, 18, 33, 66, 1204, 248, 337, 18, 401, 18, 545, 18, 401,
    18, 401, 79, 47,
];

/// Expected per-record byte lengths for get_parameter_keepalive transcript.
const GET_PARAMETER_RECORD_LENGTHS: &[u32] = &[
    86, 78, 85, 368, 115, 106, 90, 126, 72, 18, 18, 33, 66, 1204, 248, 337, 18, 401, 72, 18, 545,
...
struct IndependentRecord {
    ...
    direction: u8,
    /// Offset timestamp in milliseconds from session start.
    offset_ms: u32,
...
fn independent_parse_container(bytes: &[u8]) -> Result<Vec<IndependentRecord>, String> {
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct IndependentInterleavedFrame {
    /// Interleaved channel identifier (e.g. 0 for RTP, 1 for RTCP).
    channel: u8,
    /// Extracted frame payload bytes.
    payload: Vec<u8>,
}

/// Independently parses interleaved binary stream ($) across S2C records.
fn independent_parse_interleaved_stream(
...
fn independent_walk_rtcp(frame: &[u8], is_rtcp_rsize: bool) -> Result<(), String> {
    if frame.len() < 4 {
...
fn independent_verify_sdp(sdp: &str, is_rtcp_rsize: bool) -> Result<(), String> {
    let required_lines = [
        "v=0",
...
fn independent_verify_no_credentials(data: &[u8]) -> Result<(), String> {
