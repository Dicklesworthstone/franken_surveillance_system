...
use fss_reference::{
    ReferenceDeployment,
    ingest::{FileFormatHint, FileIngestAdapter, FileIngestRequest},
};
use rtpdump_support::*;

fn scope() -> Result<RtpImportScope, Error> {
    Ok(RtpImportScope {
        sensor: SensorId::parse("sensor:rtp-fixture")?,
        stream: StreamId::parse("stream:recorded-rtp")?,
        receive_time: TimestampNs(1_000_000_000),
    })
}
...
fn real_rtp_import_publishes_source_capsules_and_reopens_exactly() -> TestResult {
    let root = new_directory("roundtrip")?;
    let cx = cx()?;
    ...
    assert_eq!(verified.nals().len(), nals().len());
    drop(dep);
    ...
    assert_eq!(verified.source(), b);
    for (actual, expected) in verified.nals().iter().zip(nals()) {
        assert_eq!(actual.nal.bytes(), expected);
    }
    Ok(())
}
#[test]
fn generic_file_adapter_has_an_explicit_owner_bound_rtp_entrypoint() -> TestResult {
    let root = new_directory("file")?;
    let source = root.join("input.rtp");
    let b = real_dump(false);
...
fn malformed_suffix_is_retained_and_never_relabelled_clean() -> TestResult {
    let mut b = real_dump(false);
    b.extend_from_slice(&[0, 20, 0, 12]);
    let cx = cx()?;
    let root = new_directory("tail")?;
    let mut dep = ReferenceDeployment::open(&root, "site:rtpdump-tail", &cx)?;
    ...
    let receipt = publish_rtp_import(plan, &cx, &mut dep)?;
    let loaded = load_rtp_import(&receipt, &cx, &dep)?;
    assert_eq!(loaded.source(), b);
    assert!(matches!(
        loaded.report().end(),
        ImportEnd::FramingRefused(_)
    ));
    Ok(())
}
#[test]
fn wrong_binding_originals_are_preserved_but_never_decode_into_capsules() -> TestResult {
    ...
    assert!(
        plan.report()
            .records()
            .iter()
            .all(|r| matches!(r.disposition, RecordDisposition::StreamRefused(_)))
    );
    assert_eq!(plan.report().input_bytes(), b.len());
    Ok(())
}
#[test]
fn budgets_refuse_without_an_incomplete_successful_plan() -> TestResult {
    ...
    for l in [
        ...
        RtpImportLimits {
            max_payload_bytes: b.len() - 1,
            ..RtpImportLimits::default()
        },
    ] {
        assert!(matches!(
            prepare_rtp_import(&b, scope()?, config(), l, &cx),
            Err(RtpImportError::Limit)
        ));
    }
    let mut cfg = config();
    cfg.dump.max_records = 1;
    assert!(matches!(
        prepare_rtp_import(&b, scope()?, cfg, RtpImportLimits::default(), &cx),
        Err(RtpImportError::Limit)
    ));
    Ok(())
}
...
fn process_local_ingress_handle_does_not_change_durable_identity() -> TestResult {
    let b = real_dump(true);
    let cx = cx()?;
    let a = prepare_rtp_import(&b, scope()?, config(), RtpImportLimits::default(), &cx)?;
    let mut other = config();
    other.key.ingress = 999;
...
fn receive_time_and_stream_epoch_are_bound_into_the_import_identity() -> TestResult {
    ...
    let a = prepare_rtp_import(&b, scope()?, config(), RtpImportLimits::default(), &cx)?;
    let mut s = scope()?;
    s.receive_time = TimestampNs(2_000_000_000);
    let changed = prepare_rtp_import(&b, s, config(), RtpImportLimits::default(), &cx)?;
    assert_ne!(a.manifest(), changed.manifest());
    let mut cfg = config();
    cfg.key.generation = 2;
    let changed = prepare_rtp_import(&b, scope()?, cfg, RtpImportLimits::default(), &cx)?;
    assert_ne!(a.manifest(), changed.manifest());
    Ok(())
}
#[test]
fn identical_retry_does_not_create_a_different_root_or_capsule_batch() -> TestResult {
    let root = new_directory("retry")?;
    let cx = cx()?;
    let b = real_dump(false);
    let mut dep = ReferenceDeployment::open(&root, "site:rtpdump-retry", &cx)?;
    let a = publish_rtp_import(
        prepare_rtp_import(&b, scope()?, config(), RtpImportLimits::default(), &cx)?,
        &cx,
        &mut dep,
    )?;
    let count = dep.ledger().batches().len();
    let b = publish_rtp_import(
        prepare_rtp_import(&b, scope()?, config(), RtpImportLimits::default(), &cx)?,
        &cx,
        &mut dep,
    )?;
    assert_eq!(a.root(), b.root());
    assert_eq!(dep.ledger().batches().len(), count);
    Ok(())
}
#[test]
fn cancellation_before_publication_keeps_spool_and_ledger_unchanged() -> TestResult {
    ...
    assert!(matches!(
        publish_rtp_import(plan, &cx, &mut dep),
        Err(RtpImportError::Cancelled)
    ));
...
fn file_snapshot_read_is_bounded_and_directory_inputs_are_refused() -> TestResult {
    let root = new_directory("bounded_read")?;
    let path = root.join("input.rtp");
    let b = real_dump(false);
    std::fs::write(&path, &b)?;
    let cx = cx()?;
    assert!(matches!(
        read_rtp_snapshot(std::fs::File::open(&path)?, b.len() - 1, &cx),
        Err(RtpImportError::Limit)
    ));
    assert_eq!(
        read_rtp_snapshot(std::fs::File::open(&path)?, b.len(), &cx)?,
        b
    );
    assert!(read_rtp_snapshot(std::fs::File::open(&root)?, 100, &cx).is_err());
    Ok(())
}
#[test]
fn unavailable_fragment_is_reported_without_a_false_picture() -> TestResult {
    ...
    assert!(p.report().nals().is_empty());
    assert!(p.report().final_discard().is_some());
    Ok(())
}
#[test]
fn capsule_ledger_partitions_are_fixed_and_idempotent_for_larger_captures() -> TestResult {
    let root = new_directory("partitions")?;
    let cx = cx()?;
    let mut b = header();
    for sequence in 0..=130_u16 {
        let wire = rtp(sequence, true, &[0x67, 0x42, 0, 0x1e]);
        record(&mut b, &wire, wire.len() as u16, sequence as u32);
    }
    let mut cfg = config();
    cfg.dump.max_records = 256;
    let mut dep = ReferenceDeployment::open(&root, "site:rtpdump-partitions", &cx)?;
    let receipt = publish_rtp_import(
        prepare_rtp_import(&b, scope()?, cfg, RtpImportLimits::default(), &cx)?,
        &cx,
        &mut dep,
    )?;
    ...
    let count = dep.ledger().batches().len();
    let again = publish_rtp_import(
        prepare_rtp_import(&b, scope()?, cfg, RtpImportLimits::default(), &cx)?,
        &cx,
        &mut dep,
    )?;
    assert_eq!(receipt.root(), again.root());
    assert_eq!(count, dep.ledger().batches().len());
    assert_eq!(load_rtp_import(&again, &cx, &dep)?.nals().len(), 130);
    Ok(())
}
#[test]
fn readback_detects_source_corruption_despite_a_retained_publication_receipt() -> TestResult {
    use std::io::Write;
    let root = new_directory("tamper")?;
    let cx = cx()?;
    let b = real_dump(false);
    ...
    let path = dep
        .publisher()
        .spool()
        .object_path(ContentDigest::sha256(&b));
    let mut file = std::fs::OpenOptions::new().append(true).open(path)?;
    file.write_all(&[1])?;
    file.sync_all()?;
    drop(file);
    assert!(load_rtp_import(&receipt, &cx, &dep).is_err());
    Ok(())
}
#[test]
fn invalid_binding_and_zero_chunk_policy_refuse_before_opening_source() -> TestResult {
    let root = new_directory("preflight")?;
    let cx = cx()?;
    let mut dep = ReferenceDeployment::open(&root, "site:rtpdump-preflight", &cx)?;
    let owner = scope()?;
    let request = || {
        FileIngestRequest::new(
            root.join("not-present.rtp"),
            owner.sensor.clone(),
            owner.stream.clone(),
        )
    };
    let mut cfg = config();
    cfg.payload_type = 255;
    assert!(matches!(
        FileIngestAdapter::ingest_rtp(request(), cfg, RtpImportLimits::default(), &cx, &mut dep),
        Err(RtpImportError::Replay(_))
    ));
    let mut req = request();
    req.limits.chunk_bytes = 0;
    assert!(matches!(
        FileIngestAdapter::ingest_rtp(req, config(), RtpImportLimits::default(), &cx, &mut dep),
        Err(RtpImportError::Binding)
    ));
    Ok(())
}
