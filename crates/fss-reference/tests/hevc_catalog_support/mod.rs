...
pub fn scope() -> Result<CatalogScope, Error> {
    Ok(CatalogScope { recording: source::scope()?,
        decode_clock: ContentDigest::sha256(b"explicit-hevc-decode-epoch"), time_scale: 90_000 })
}
pub fn window(base: u64) -> Result<PreparedHevcRecording, Error> {
    ...
    Ok(prepare_hevc_recording(scope()?.recording, &source::configuration()?, 90_000,
        &timings, &source::borrowed(&source::packets()?))?)
}
pub fn slot(n: usize) -> Result<SlotName, Error> { Ok(SlotName::parse(&format!("hevc-window-{n:03}"))?) }
    ...
    reported_roots: Option<&[ContentDigest]>) -> Result<Vec<u8>, Error>
{
    ...
    e.u64(basis.recording.generation); e.digest(basis.recording.anchor);
    e.digest(basis.recording.receive_clock); e.digest(basis.decode_clock); e.u32(basis.time_scale);
...
pub fn manifest(kind: &str, index: &[u8], windows: &[&PreparedRecording],
    roots: Option<&[ContentDigest]>) -> Result<ObjectManifest, Error>
{
    ...
    Ok(ObjectManifest::new(kind, children, Some(ContentDigest::sha256(index)))?)
}
