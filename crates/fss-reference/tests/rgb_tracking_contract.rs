...
use fss_twin::image_zones::{
    ImageZoneBasis, ImageZoneEventKind, ImageZonePolicy, ImageZoneRelation, ImageZoneSpec,
};
...
fn actual_pixels_move_one_selected_class_through_entry_dwell_and_exit() -> Test {
    ...
    let outside = detection(&model, &head, 16, 1)?;
    let inside = detection(&model, &head, 240, 2)?;
    observe(&mut tracker, &outside, TrackingAvailability::Available)?;
    let id = tracker.tracker().tracks()[0].id();
    assert_eq!(
        tracker.zone_report().ok_or("zones")?.cells()[0].relation,
    ...
    assert_eq!(tracker.tracker().exposure_count(), 4);
    Ok(())
}
#[test]
fn multilabel_rows_remain_one_object_and_other_classes_are_explicit() -> Test {
    ...
    let mut tracker = tracker(&head, tracking_policy())?;
    observe(&mut tracker, &run, TrackingAvailability::Available)?;
    ...
    assert_eq!(
        p.decisions()
            .iter()
            .filter(|d| d.proposal.is_none())
            .count(),
        1
    );
    assert_eq!(p.detection_digest(), run.report().digest());
    assert_eq!(p.frame().evidence, p.digest().bytes());
    assert_eq!(
        p.frame().source.image.pixels,
        run.inference().decode_receipt().rgb_sha256
    );
    assert_eq!(tracker.contract().label(), "numeric-a");
...
fn subpixel_cover_is_outward_and_preserves_the_exact_original_box() -> Test {
    let model = model(1)?;
    let head = head(&model)?;
    let run = detection(&model, &head, 100, 1)?;
    let mut tracker = tracker(&head, tracking_policy())?;
    observe(&mut tracker, &run, TrackingAvailability::Available)?;
    let p = tracker.prepared().ok_or("prepared")?;
    for d in p.decisions().iter().filter(|d| d.proposal.is_some()) {
        let q = d.detection.bounds();
        let b = d.proposal.ok_or("proposal")?;
...
fn unavailable_input_interrupts_observation_without_inventing_exit() -> Test {
    ...
    for unavailable in [
        ...
        assert!(
            events
                .iter()
                .any(|e| e.kind == ImageZoneEventKind::ObservationInterrupted)
        );
        assert!(
            !events
                .iter()
...
fn report_inference_admission_and_class_contract_cannot_be_cross_wired() -> Test {
    let model = model(1)?;
    let head = head(&model)?;
    let a = detection(&model, &head, 16, 1)?;
    let b = detection(&model, &head, 240, 2)?;
    let mut tracker = tracker(&head, tracking_policy())?;
    let before = tracker.tracker().digest();
    let admit = admission(a.report().source(), TrackingAvailability::Available)?;
    assert!(matches!(
        tracker.observe(b.inference(), a.report(), admit, &mut WorkBudget::new(WORK)),
        Err(RgbTrackingError::BindingMismatch)
    ));
    assert!(matches!(
        tracker.observe(b.inference(), b.report(), admit, &mut WorkBudget::new(WORK)),
        Err(RgbTrackingError::BindingMismatch)
    ));
...
fn repeated_exposure_and_changed_mask_are_refused_before_temporal_mutation() -> Test {
    ...
    observe(&mut tracker, &run, TrackingAvailability::Available)?;
    let before = tracker.tracker().digest();
    let zone = tracker.zone_report().ok_or("zones")?.digest();
    assert!(matches!(
        tracker.observe(
            run.inference(),
            run.report(),
        ...
        Err(RgbTrackingError::Tracking(
            ImageTrackingError::ReusedExposure
        ))
    ));
    let mut s = run.report().source();
    s.permission_mask = [77; 32];
    assert!(matches!(
        tracker.observe(
            run.inference(),
            run.report(),
            admission(s, TrackingAvailability::Available)?,
            &mut WorkBudget::new(WORK)
        ),
        Err(RgbTrackingError::BindingMismatch)
    ));
    assert_eq!(tracker.tracker().digest(), before);
    assert_eq!(tracker.zone_report().ok_or("zones")?.digest(), zone);
    Ok(())
}
#[test]
fn complete_selected_class_overflow_never_silently_truncates() -> Test {
    ...
    assert!(matches!(
        ...
        Err(RgbTrackingError::Limit)
    ));
    assert_eq!(tracker.tracker().digest(), before);
    assert_eq!(tracker.tracker().exposure_count(), 0);
    Ok(())
}
#[test]
fn every_budget_cut_is_atomic_or_retains_the_exact_pending_zone_input() -> Test {
    ...
    let mut reference = tracker(&head, tracking_policy())?;
    observe(&mut reference, &a, TrackingAvailability::Available)?;
    ...
    for limit in 0..work.used() {
        let mut current = tracker(&head, tracking_policy())?;
        observe(&mut current, &a, TrackingAvailability::Available)?;
        let before = current.tracker().digest();
        match current.observe(
            b.inference(),
            b.report(),
            admit,
            ...
            Err(_) => {
                assert_eq!(current.tracker().digest(), before);
                assert_eq!(current.tracker().exposure_count(), 1);
                assert_eq!(
                    current.observe(
                        b.inference(),
                        b.report(),
            ...
            Ok(RgbZoneProgress::Pending { .. }) => {
                saw_pending = true;
                assert_eq!(current.tracker().exposure_count(), 2);
                assert!(current.zone_report().is_none());
                assert_eq!(
                    current
                        .prepared()
                        .ok_or("lost preparation")?
                        .detection_digest(),
                    b.report().digest()
                );
                assert!(matches!(
                    current.observe(b.inference(), b.report(), admit, &mut WorkBudget::new(WORK)),
                    Err(RgbTrackingError::PendingZones)
                ));
            ...
            Ok(RgbZoneProgress::Complete { .. }) => {
                return Err("insufficient budget completed".into());
            }
        }
    }
    assert!(saw_pending);
...
fn cancellation_and_wrong_zone_basis_do_not_consume_input() -> Test {
    ...
    assert!(matches!(
        tracker.observe(
            ...
            run.report(),
            admission(run.report().source(), TrackingAvailability::Available)?,
            &mut WorkBudget::cancellable(WORK, &flag)
        ),
        Err(RgbTrackingError::Work(GeometryError::Cancelled))
    ));
    assert_eq!(tracker.tracker().digest(), before);
    let mut wrong = RgbZoneTracker::new(
        [9; 32],
        RgbTrackingContract::new(&head, 0, ContentDigest::sha256(b"selected"))?,
        tracking_policy(),
        ImageZoneBasis {
            camera: 99,
            clock: 2,
        ...
        &[ImageZoneSpec {
            id: 1,
            vertices: vec![[1, 1], [30, 1], [30, 15], [1, 15]],
            margin: 0,
            dwell_ns: None,
        }],
        &mut WorkBudget::new(WORK),
    ...
    assert_eq!(wrong.tracker().exposure_count(), 0);
    Ok(())
}
