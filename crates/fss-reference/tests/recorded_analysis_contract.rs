...
impl Directory {
    fn new() -> TestResult<Self> {
        for n in 0..100 {
            let path =
                std::env::temp_dir().join(format!("fss-analysis-{}-{n}", std::process::id()));
...
fn retained_sequence_rebuilds_after_restart_and_rejects_forged_reports() -> TestResult {
    ...
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:analysis-test".into(),
        operation_id: OperationId::parse("operation:analysis-test")?,
        principal: "principal:analysis-test".into(),
        ...
        anchor_universe: ContentDigest::sha256(b"site:analysis-test"),
        generation: 1,
    })?;
    ...
    let imported = FileIngestAdapter::ingest(
        FileIngestRequest::new(
            &source,
            SensorId::parse("sensor:analysis-test")?,
            StreamId::parse("stream:analysis-test")?,
        )
    ...
    let first = RecordedFrame::decode_and_publish(
        &mut deployment,
        &request,
    ...
    for observation in observations {
        ...
        assert_eq!(
            observation.detection().capsule(),
            &observation.tracking().capsule
        );
    }
    assert!(observations[2].tracking().tracks[0].confirmed);
    assert_eq!(observations[2].tracking().tracks[0].observations, 3);
    assert_eq!(
        observations[2].tracking().predecessor,
        Some(observations[1].tracking().digest()?)
    );
    assert_eq!(budget.detection.used(), 3);
    assert!(budget.association.used() > 0);
    assert_eq!(*deployment.current_anchor(), before);

    let gap = AnalysisPlan::new(
        imported.import_identity,
        ComponentInterpretation::Grayscale,
        spec(&model),
        tracking(),
        vec![selections[0], selections[2]],
    )?;
    let gap_report = AnalysisReport::read(
        &deployment,
        &gap,
        &limits,
        &mut AnalysisBudget::new(100, 10_000),
        &cx,
    )?;
    assert!(
        gap_report.observations()[1]
            .tracking()
            .resets
            .contains(&TrackReset::SequenceGap)
    );
    assert_eq!(gap_report.observations()[1].tracking().retired[0].id, id);
    assert_ne!(gap_report.observations()[1].tracking().tracks[0].id, id);

    let mut duplicates = selections.clone();
    duplicates[1].segment_index = 0;
    assert!(
        AnalysisPlan::new(
            imported.import_identity,
            ComponentInterpretation::Grayscale,
    ...
    wrong[1].run_identity = wrong[0].run_identity;
    let rebound = AnalysisPlan::new(
        imported.import_identity,
        ComponentInterpretation::Grayscale,
        spec(&model),
        tracking(),
    ...
    assert!(matches!(
        AnalysisReport::read(
            ...
            &plan,
            &AnalysisLimits {
                maximum_report_bytes: plan.encoded().len(),
                ..limits.clone()
            },
            &mut AnalysisBudget::new(100, 10_000),
            &cx
    ...
    let restored = AnalysisReport::verify(
        &reopened,
        report.encoded(),
        report.digest(),
        &limits,
...
fn inference_retry_revalidates_completion_and_resumes_visible_root_after_restart() -> TestResult {
    ...
    let context = || -> TestResult<ReplayCx> {
        let authority = ContextAuthority::new_root(RootAuthoritySpec {
            ...
            privacy_scope: "privacy:test".into(),
            retention_scope: "retention:test".into(),
            anchor_universe: ContentDigest::sha256(b"site:analysis-retry"),
            generation: 1,
        })?;
    ...
    let imported = FileIngestAdapter::ingest(
        FileIngestRequest::new(
            &source,
            SensorId::parse("sensor:retry")?,
            StreamId::parse("stream:retry")?,
        )
        .with_receive_time(TimestampNs(1_000_000_000)),
        &cx,
        &mut deployment,
    ...
    assert_eq!(
        first.identity(),
        RecordedInference::identity_for(&frame, &model)
    );
    ...
    let repeated = RecordedInference::run_and_publish(
        &mut deployment,
        &request,
        &model,
    ...
    let fresh = context()?;
    assert!(matches!(
        RecordedInference::open(&deployment, identity, &request, &fresh),
