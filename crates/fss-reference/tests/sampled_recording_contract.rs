...
use fss_core::{
    BudgetVector, CanonicalDecoder, ContentDigest, OperationId, SensorId, StreamId, TimestampNs,
};
use fss_reference::ingest::activity::{
    ActivityError, ActivityPolicy, ActivitySampler, SamplingReason,
};
...
use fss_reference::ingest::recording_pipeline::{
    RecordingBudget, RecordingRequest, RecordingStage, run_recording,
};
    ...
    fn new(label: &str) -> TestResult<Self> {
        for n in 0..100 {
            let path = std::env::temp_dir()
                .join(format!("fss-sampling-{label}-{}-{n}", std::process::id()));
...
fn context(root: &Path) -> TestResult<ReplayCx> {
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:sampling".into(),
        operation_id: OperationId::parse("operation:sampling")?,
        principal: "principal:sampling".into(),
        ...
        anchor_universe: ContentDigest::sha256(b"site:sampling"),
        generation: 1,
    })?;
    authority.validate()?;
...
fn fixture(label: &str, count: usize) -> TestResult<Fixture> {
    ...
    let imported = FileIngestAdapter::ingest(
        FileIngestRequest::new(
            &source,
            SensorId::parse("sensor:sampling")?,
            StreamId::parse("stream:sampling")?,
        )
    ...
    let request = RecordingRequest {
        ...
        first_segment: 0,
        segment_count: count,
        maximum_tensor_bytes: 64 * 1024 * 1024,
...
fn policy(stride: u32) -> ActivityPolicy {
    ActivityPolicy {
        ...
        post_activity_frames: 0,
    }
}
fn budget(model_units: u64, decode_units: u64) -> RecordingBudget<'static> {
    RecordingBudget::new(
        DecodeBudget::new(decode_units),
        model_units,
        AnalysisBudget::new(1_000_000, 1_000_000),
    )
}
fn normal_budget() -> RecordingBudget<'static> {
    budget(1_000_000_000, 1_000_000_000)
}
...
fn quiet_recording_executes_only_sentinels_and_keeps_all_source_decisions() -> TestResult {
    ...
    assert_eq!(out.report.decisions().len(), 9);
    assert_eq!(out.report.skipped_frames(), 6);
    assert_eq!(out.progress.new_decodes, 9);
    assert_eq!(out.progress.new_inferences, 3);
    ...
    for d in out.report.decisions() {
        RecordedFrame::open(
            &f.deployment,
            &source(&f.request, d.segment_index() as usize),
            &f.cx,
        )?;
    }
...
fn required_quiet_frames_override_sampling_and_policy_is_canonical() -> TestResult {
    ...
    let reverse = SamplingPlan::new(
        &f.request,
        policy(3),
        10_000,
        required.into_iter().rev().collect(),
    )?;
    ...
    assert!(
        out.report.decisions()[2]
            .reasons()
            .contains(&SamplingReason::Required)
    );
    assert!(
        out.report.decisions()[5]
            .reasons()
            .contains(&SamplingReason::Required)
    );
    Ok(())
...
fn no_comparison_budget_forces_inference_instead_of_inventing_quiet() -> TestResult {
    ...
    assert_eq!(out.progress.new_inferences, 7);
    assert_eq!(out.report.skipped_frames(), 0);
    assert_eq!(out.report.comparisons_used(), 0);
    assert!(
        out.report
            .decisions()
            .iter()
            .any(|d| d.reasons().contains(&SamplingReason::ComparisonBudgetFloor))
    );
    assert!(
        out.report
            .decisions()
            .iter()
            .filter(|d| d.reasons().contains(&SamplingReason::ComparisonBudgetFloor))
...
fn restart_reuses_selected_numeric_work_and_verifies_skipped_evidence_read_only() -> TestResult {
    ...
    drop(f.deployment);
    let mut deployment =
        ReferenceDeployment::open(&f.dir.0.join("deployment"), "site:sampling", &f.cx)?;
    let plan = SamplingPlan::decode(plan.encoded(), plan.digest())?;
    let mut work = budget(0, 0);
    let again = run_sampled_recording(
        &mut deployment,
        &f.request,
        &f.model,
        &plan,
    ...
    assert_eq!(again.progress.reused_decodes, 7);
    assert_eq!(again.progress.reused_inferences, 3);
    assert_eq!(work.model_charged(), 0);
    assert_eq!(work.decode.used(), 0);
    let verified = SampledReport::verify(
        &deployment,
        out.report.encoded(),
        out.report.digest(),
        &f.request.limits,
        10_000,
        &mut AnalysisBudget::new(1_000_000, 1_000_000),
        &f.cx,
    )?;
...
fn model_budget_cannot_drop_a_due_sentinel_and_exact_retry_reuses_prefix() -> TestResult {
    let mut oracle = fixture("one-cost", 1)?;
    let mut one = normal_budget();
    run_recording(
        &mut oracle.deployment,
        &oracle.request,
        &oracle.model,
        &mut one,
        &ScalarExecCx::new(),
    ...
    let failed = run_sampled_recording(
        &mut f.deployment,
        &f.request,
        &f.model,
        &plan,
    ...
    .err()
    .ok_or("due sentinel was skipped")?;
    assert_eq!(failed.progress.stage, RecordingStage::Inference);
    assert_eq!(failed.progress.next_segment, Some(3));
    assert_eq!(failed.progress.completed.len(), 1);
    assert_eq!(failed.decisions.len(), 4);
    assert!(failed.decisions[3].selected());
    assert_eq!(limited.model_charged(), cost);
    let mut remaining = budget(cost * 2, 1_000_000_000);
    let done = run_sampled_recording(
        &mut f.deployment,
        &f.request,
        &f.model,
        &plan,
...
fn cancelled_sampling_does_not_execute_a_model_and_retains_decoded_progress() -> TestResult {
    ...
    .err()
    .ok_or("sampling cancellation ignored")?;
    assert_eq!(failed.progress.stage, RecordingStage::Sampling);
    assert_eq!(failed.progress.new_decodes, 1);
    assert_eq!(failed.progress.new_inferences, 0);
    assert!(failed.decisions.is_empty());
    assert_eq!(failed.comparisons_used, 0);
    ...
    assert_eq!(out.progress.reused_decodes, 1);
    assert_eq!(out.progress.new_inferences, 1);
    Ok(())
}
...
fn sampler_retries_are_idempotent_and_changed_requirements_do_not_rebind_history() -> TestResult {
    ...
    assert_eq!(
        sampler.push(&frame, Some(ContentDigest::sha256(b"new basis")), &f.cx),
        Err(ActivityError::ReplayConflict)
    );
    let later = RecordedFrame::decode_and_publish(
        &mut f.deployment,
        &source(&f.request, 2),
        &mut DecodeBudget::new(1_000_000),
        &f.cx,
    ...
    assert_eq!(
        sampler.push(&frame, None, &f.cx),
        Err(ActivityError::OutOfOrder)
    );
    Ok(())
}
