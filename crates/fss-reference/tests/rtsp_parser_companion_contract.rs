...
//!   and user:pass@ URIs never appear in Debug output of any event or Debug/Display of any error,
//!   verified across all 8 transcripts and all parser error kinds
//! - Bounded inputs: exact boundary behavior at limit N (Ok) and N+1 (HeaderLimit / BodyLimit)
//! - Framing and protocol invariants: CSeq required, Content-Length exact framing, userinfo forbidden,
...
fn test_transcript_clean_literal_expected_sequences() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        r#"CAPLOG {{"step":"test_transcript_clean_literal_expected_sequences","verdict":"pass","duration_ms":{}}}"#,
...
fn test_transcript_auth_required_literal_expected_sequences() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        r#"CAPLOG {{"step":"test_transcript_auth_required_literal_expected_sequences","verdict":"pass","duration_ms":{}}}"#,
        start.elapsed().as_millis()
    );
    Ok(())
}
...
-> Result<(), Box<dyn Error>> {
    ...
    println!(
        r#"CAPLOG {{"step":"test_transcript_interleave_split_reassembly_and_literal_expected_sequences","verdict":"pass","duration_ms":{}}}"#,
        start.elapsed().as_millis()
    );
    Ok(())
}
...
fn test_transcript_bad_content_length_literal_expected_sequences() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        r#"CAPLOG {{"step":"test_transcript_bad_content_length_literal_expected_sequences","verdict":"pass","duration_ms":{}}}"#,
        start.elapsed().as_millis()
    );
    Ok(())
}
...
fn test_transcript_session_timeout_header_literal_expected_sequences() -> Result<(), Box<dyn Error>>
{
    ...
    println!(
        r#"CAPLOG {{"step":"test_transcript_session_timeout_header_literal_expected_sequences","verdict":"pass","duration_ms":{}}}"#,
        start.elapsed().as_millis()
    );
    Ok(())
}
...
fn test_transcript_rtcp_rsize_literal_expected_sequences() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        r#"CAPLOG {{"step":"test_transcript_rtcp_rsize_literal_expected_sequences","verdict":"pass","duration_ms":{}}}"#,
        start.elapsed().as_millis()
    );
    Ok(())
}
...
fn test_transcript_sr_absent_literal_expected_sequences() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        r#"CAPLOG {{"step":"test_transcript_sr_absent_literal_expected_sequences","verdict":"pass","duration_ms":{}}}"#,
        start.elapsed().as_millis()
    );
    Ok(())
}
...
fn test_transcript_get_parameter_keepalive_literal_expected_sequences() -> Result<(), Box<dyn Error>>
{
    ...
    println!(
        r#"CAPLOG {{"step":"test_transcript_get_parameter_keepalive_literal_expected_sequences","verdict":"pass","duration_ms":{}}}"#,
        start.elapsed().as_millis()
    );
    Ok(())
}
...
fn test_incremental_feeding_all_byte_splits_and_single_byte_chunks() -> Result<(), Box<dyn Error>> {
    ...
    if !surfaced_error && matches!(byte_parser.feed(b""), Err(RtspError::BadContentLength(_))) {
        surfaced_error = true;
    }
    assert!(
        surfaced_error,
    ...
    println!(
        r#"CAPLOG {{"step":"test_incremental_feeding_all_byte_splits_and_single_byte_chunks","verdict":"pass","duration_ms":{}}}"#,
        start.elapsed().as_millis()
    );
    Ok(())
}
...
fn test_credential_byte_scan_debug_and_error_output() -> Result<(), Box<dyn Error>> {
    ...
    const TRANSCRIPT_NONCE: &str = "dcd98b7102dd2f0e8b11d0f600bfb0c093";

    let forbidden_secrets = [
        SECRET_AUTH_BEARER,
        SECRET_PROXY_AUTH,
        SECRET_WWW_AUTH_NONCE,
        SECRET_USERINFO_PASS,
        TRANSCRIPT_NONCE,
    ];

    let assert_no_secrets = |text: &str, context: &str| {
        for secret in &forbidden_secrets {
            assert!(
                !text.contains(secret),
                "Secret {secret:?} leaked in {context}: {text}"
            );
        }
    };

    let check_error_debug_and_display = |err: &RtspError, context: &str| {
        let dbg = format!("{err:?}");
        let disp = format!("{err}");
        assert_no_secrets(&dbg, &format!("{context} (Debug)"));
        assert_no_secrets(&disp, &format!("{context} (Display)"));
    };
    ...
    assert_no_secrets(&debug_output, "Authorization header event Debug");
    ...
    assert_eq!(events.len(), 1);
    let debug_output = format!("{:?}", events[0]);
    assert_no_secrets(&debug_output, "Proxy-Authorization header event Debug");
    if let RtspEvent::Request(req) = &events[0] {
        assert_eq!(
    ...
    assert_eq!(events.len(), 1);
    let debug_output = format!("{:?}", events[0]);
    assert_no_secrets(&debug_output, "WWW-Authenticate 401 response event Debug");
    ...
    assert_no_secrets(
        &debug_output,
        "WWW-Authenticate 200 OK response event Debug",
    );
    ...
    // 5. Userinfo in Request URI: rejected with UserinfoNotPermitted, error Debug and Display do not echo credentials
    ...
    assert!(matches!(
        res,
        Err(RtspError::UserinfoNotPermitted(UserinfoSite::RequestUri))
    ));
    let err = res.err().ok_or("expected error")?;
    check_error_debug_and_display(&err, "userinfo in request URI error");

    // 6. Userinfo across all header URI sites: rejected with UserinfoNotPermitted, error Debug and Display do not echo credentials
    for (wire_hdr, expected_site) in [
        (
            format!(
                "RTSP/1.0 200 OK\r\n\
                 CSeq: 1\r\n\
            ...
            UserinfoSite::ContentBase,
        ),
        (
            format!(
                "RTSP/1.0 200 OK\r\n\
                 CSeq: 1\r\n\
                 Content-Location: rtsp://user:{SECRET_USERINFO_PASS}@fixture.invalid/live/\r\n\r\n"
            ),
        ...
        (
            format!(
                "RTSP/1.0 302 Found\r\n\
                 CSeq: 1\r\n\
                 Location: rtsp://user:{SECRET_USERINFO_PASS}@fixture.invalid/live/\r\n\r\n"
            ),
            UserinfoSite::Location,
        ),
        (
            format!(
                "RTSP/1.0 200 OK\r\n\
                 CSeq: 1\r\n\
                 Range: npt=0-\r\n\
                 RTP-Info: url=rtsp://user:{SECRET_USERINFO_PASS}@fixture.invalid/live/track1;seq=1\r\n\r\n"
            ),
            UserinfoSite::RtpInfo { entry: 0 },
        ),
    ] {
        let mut parser = RtspParser::new();
        let res = parser.feed(wire_hdr.as_bytes());
        assert_eq!(
            res,
            Err(RtspError::UserinfoNotPermitted(expected_site)),
            "header URI userinfo rejection site mismatch"
        );
        let err = res.err().ok_or("expected error")?;
        check_error_debug_and_display(&err, &format!("userinfo header error ({expected_site:?})"));
    }

    // 7. Exhaustive scan over EVERY event from all 8 committed transcripts (C2S and S2C)
    let all_transcripts = [
        "clean.transcript",
        "auth_required.transcript",
        "interleave_split.transcript",
    ...
    let mut total_scanned_events = 0;
    for name in all_transcripts {
        ...
        for (stream_name, stream) in [("C2S", &c2s), ("S2C", &s2c)] {
            ...
            let mut parser = RtspParser::new();
            match parser.feed(stream) {
                Ok(events) => {
                    total_scanned_events += events.len();
                    for (idx, ev) in events.iter().enumerate() {
                        let ev_debug = format!("{ev:?}");
                        assert_no_secrets(&ev_debug, &format!("{name} {stream_name} event[{idx}]"));
                    }
                    if let Err(err) = parser.feed(b"") {
                        check_error_debug_and_display(
                            &err,
                            &format!("{name} {stream_name} trailing error"),
                        );
                    }
                }
                Err(err) => {
                    check_error_debug_and_display(
                        &err,
                        &format!("{name} {stream_name} feed error"),
                    );
                }
            }
        }
    ...
    assert!(
        total_scanned_events >= 80,
        "expected at least 80 events across all 8 transcripts, scanned {total_scanned_events}"
    );

    // 8. Exhaustive scan over Debug AND Display of every parser error kind
    let mut error_kinds: Vec<RtspError> = Vec::new();

    if let Err(e @ RtspError::MissingCSeq) =
        RtspParser::new().feed(b"OPTIONS rtsp://fixture.invalid/stream RTSP/1.0\r\n\r\n")
    {
        error_kinds.push(e);
    }
    if let Err(e @ RtspError::BadCSeq(_)) = RtspParser::new()
        .feed(b"OPTIONS rtsp://fixture.invalid/stream RTSP/1.0\r\nCSeq: invalid_numeric\r\n\r\n")
    {
        error_kinds.push(e);
    }
    if let Err(e @ RtspError::DuplicateCSeq(_)) = RtspParser::new()
        .feed(b"OPTIONS rtsp://fixture.invalid/stream RTSP/1.0\r\nCSeq: 1\r\nCSeq: 2\r\n\r\n")
    {
        error_kinds.push(e);
    }
    if let Err(e @ RtspError::MalformedStartLine(_)) =
        RtspParser::new().feed(b"INVALID_PROTOCOL_LINE\r\n\r\n")
    {
        error_kinds.push(e);
    }
    if let Err(e @ RtspError::LineFoldingNotPermitted) = RtspParser::new()
        .feed(b"OPTIONS rtsp://fixture.invalid/stream RTSP/1.0\r\nCSeq: 1\r\n Folded: line\r\n\r\n")
    {
        error_kinds.push(e);
    }
    if let Err(e @ RtspError::Unsupported { .. }) =
        RtspParser::new().feed(b"RECORD rtsp://fixture.invalid/stream RTSP/1.0\r\nCSeq: 1\r\n\r\n")
    {
        error_kinds.push(e);
    }
    if let Err(e @ RtspError::UnsupportedVersion(_)) =
        RtspParser::new().feed(b"OPTIONS rtsp://fixture.invalid/stream RTSP/2.0\r\nCSeq: 1\r\n\r\n")
    {
        error_kinds.push(e);
    }
    if let Err(e @ RtspError::NulByte(_)) = RtspParser::new()
        .feed(b"OPTIONS rtsp://fixture.invalid/\0stream RTSP/1.0\r\nCSeq: 1\r\n\r\n")
    {
        error_kinds.push(e);
    }
    if let Err(e @ RtspError::Utf8(_)) = RtspParser::new()
        .feed(b"OPTIONS rtsp://fixture.invalid/stream RTSP/1.0\r\nCSeq: 1\r\nBad: \xFF\r\n\r\n")
    {
        error_kinds.push(e);
    }
    if let Err(e @ RtspError::BadContentLength(_)) = RtspParser::new().feed(
        b"OPTIONS rtsp://fixture.invalid/stream RTSP/1.0\r\nCSeq: 1\r\nContent-Length: -1\r\n\r\n",
    ) {
        error_kinds.push(e);
    }
    if let Err(e @ RtspError::DuplicateContentLength(_)) = RtspParser::new().feed(
        b"OPTIONS rtsp://fixture.invalid/stream RTSP/1.0\r\nCSeq: 1\r\nContent-Length: 1\r\nContent-Length: 2\r\n\r\n",
    ) {
        error_kinds.push(e);
    }
    if let Err(e @ RtspError::BadTransport(_)) = RtspParser::new().feed(
        b"SETUP rtsp://fixture.invalid/stream RTSP/1.0\r\nCSeq: 1\r\nTransport: bad_transport\r\n\r\n",
    ) {
        error_kinds.push(e);
    }
    let mut hl_parser = RtspParser::with_limits(RtspLimits {
        max_line_bytes: 10,
        ..RtspLimits::default()
    });
    if let Err(e @ RtspError::HeaderLimit(_)) =
        hl_parser.feed(b"OPTIONS rtsp://fixture.invalid/stream RTSP/1.0\r\nCSeq: 1\r\n\r\n")
    {
        error_kinds.push(e);
    }
    let mut bl_parser = RtspParser::with_limits(RtspLimits {
        max_body_bytes: 5,
        ..RtspLimits::default()
    });
    if let Err(e @ RtspError::BodyLimit(_)) =
        bl_parser.feed(b"RTSP/1.0 200 OK\r\nCSeq: 1\r\nContent-Length: 10\r\n\r\n1234567890")
    {
        error_kinds.push(e);
    }
    let mut il_parser = RtspParser::with_limits(RtspLimits {
        max_interleaved_bytes: 2,
        ..RtspLimits::default()
    });
    if let Err(e @ RtspError::BadInterleavedLength(_)) = il_parser.feed(b"$\x00\x00\x05hello") {
        error_kinds.push(e);
    }
    let mut poison_parser = RtspParser::with_limits(RtspLimits {
        max_body_bytes: 5,
        ..RtspLimits::default()
    });
    let _ =
        poison_parser.feed(b"RTSP/1.0 200 OK\r\nCSeq: 1\r\nContent-Length: 10\r\n\r\n1234567890");
    if let Err(e @ RtspError::Poisoned(_)) =
        poison_parser.feed(b"OPTIONS rtsp://fixture.invalid/stream RTSP/1.0\r\nCSeq: 2\r\n\r\n")
    {
        error_kinds.push(e);
    }
    ...
    assert_eq!(
        error_kinds.len(),
        16,
        "all 16 non-userinfo error kinds must be constructed"
    );
    for err in &error_kinds {
        check_error_debug_and_display(err, "exhaustive error variant scan");
    }

    println!(
        r#"CAPLOG {{"step":"test_credential_byte_scan_debug_and_error_output","verdict":"pass","duration_ms":{}}}"#,
        start.elapsed().as_millis()
    );
    Ok(())
}
...
fn test_bounded_inputs_limits_n_and_n_plus_one() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        r#"CAPLOG {{"step":"test_bounded_inputs_limits_n_and_n_plus_one","verdict":"pass","duration_ms":{}}}"#,
        start.elapsed().as_millis()
    );
    Ok(())
}
...
#[test]
fn test_framing_invariants_and_protocol_validation() -> Result<(), Box<dyn Error>> {
    let start = Instant::now();
    // Invariant 1: Mandatory CSeq check on requests and responses
    ...
    // Invariant 2: Content-Length exact framing
    ...
    // Invariant 3: Userinfo check in Request-URI and headers
    ...
    // Invariant 4: Pending-error-before-buffering preservation
    ...
    // Invariant 5: Obsolete line folding forbidden
    ...
    println!(
        r#"CAPLOG {{"step":"test_framing_invariants_and_protocol_validation","verdict":"pass","duration_ms":{}}}"#,
        start.elapsed().as_millis()
    );
    Ok(())
}
