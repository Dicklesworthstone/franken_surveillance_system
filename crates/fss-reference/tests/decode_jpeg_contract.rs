...
//! - Independent fidelity: every grey fixture's max error vs source is bounded by the committed
//!   fixture manifest's independent `encoder_max_error` (quality 100 included), and 4:2:0 chroma
//!   row replication is checked against the source pixels of a stripe pattern.
//! - Adobe APP14 transform 0 on 3 components refused; the 0xFF marker prefix is required while
//!   0xFF fill bytes stay legal; a tiny header claiming a huge image is refused before the planes
//!   are allocated.
...
use fss_reference::media_fixture::jpeg::{
    GeneratedJpegFixture, JpegConfig, Subsampling, brown_luma_96x96, build_jpeg_manifest_json,
    compute_psnr, encode_jpeg, generate_all_jpeg_fixtures, generate_colorbars_rgb,
    generate_gradient_rgb, source_pixels,
};
use fss_reference::{
    ADP_REPLAY_ROW_ID, JPEG_DECODER_GENERATION, JpegDecodeError, JpegDecodeLimits, JpegSubsampling,
    ReplayCx, decode_baseline_jpeg,
};
...
fn test_cx() -> Result<ReplayCx, Box<dyn Error>> {
    ...
    let spec = RootAuthoritySpec {
        trace_id: format!("trace:jpeg-contract-{id}"),
        operation_id: OperationId::parse(format!("operation:jpeg-contract-{id}"))?,
        principal: format!("operator:jpeg-contract-{id}"),
    ...
    let scratch_root =
        PathBuf::from(env!("CARGO_TARGET_TMPDIR")).join(format!("test-jpeg-cx-{id}"));
...
/// Regenerates the .5 fixtures and proves the committed `fixture_manifest.json` carries exactly
/// their metadata, so `encoder_max_error` below is the committed manifest value. That value is
/// the encoder-side float-IDCT reconstruction error (agreed by the .5 test-side independent
/// decoder), never output of the decoder under test.
fn manifest_checked_fixtures() -> Result<Vec<GeneratedJpegFixture>, Box<dyn Error>> {
    let jpegs = generate_all_jpeg_fixtures()?;
    let manifest_path = repo_root()?.join("tests/fixtures/media/jpeg/fixture_manifest.json");
    let on_disk = fs::read_to_string(&manifest_path)?;
    let regenerated = build_jpeg_manifest_json(&jpegs);
    require!(
        on_disk.trim_end() == regenerated.trim_end(),
        "{} differs from the regenerated manifest",
        manifest_path.display()
    );
    Ok(jpegs)
}

fn max_abs_error(a: &[u8], b: &[u8]) -> Result<u32, Box<dyn Error>> {
    require!(
        a.len() == b.len(),
        "length mismatch: {} vs {}",
        a.len(),
    ...
    Ok(a.iter()
        .zip(b)
        .fold(0u32, |m, (x, y)| m.max(u32::from(x.abs_diff(*y)))))
}

fn adobe_app14(transform: u8) -> [u8; 16] {
    [
...
fn marker_pos(bytes: &[u8], marker: u8) -> Result<usize, Box<dyn Error>> {
    Ok((0..bytes.len().saturating_sub(1))
        .find(|&i| bytes[i] == 0xFF && bytes[i + 1] == marker)
        .ok_or_else(|| format!("marker 0xFF{marker:02X} not found"))?)
}
...
fn test_limit_exceeded_before_allocation() -> Result<(), Box<dyn Error>> {
    ...
    // Header-only stream with SOF0 but no SOS / entropy scan data:
    // Proves that LimitExceeded is raised at SOF0 before any allocation,
    // rather than Truncated at SOS parsing or scan time.
    let header_only_sof = [
        0xFF, 0xD8, // SOI
        0xFF, 0xC0, // SOF0
        0x00, 0x0B, // length 11 (8 + 3*1 for 1 component)
        0x08, // precision 8
        0x00, 0x60, // height = 96
        0x00, 0x60, // width = 96
        0x01, // 1 component
    ...
    // 1. max_width limit (must fail before allocation at SOF0, exact message)
    ...
    let res_hdr_w = decode_baseline_jpeg(&header_only_sof, limits_width, &cx);
    require!(
        matches!(res_hdr_w, Err(JpegDecodeError::LimitExceeded { ref limit }) if limit == "width 96 exceeds max_width 50"),
        "header-only stream must yield LimitExceeded at SOF0 for width, not Truncated, got: {res_hdr_w:?}"
    );

    // 2. max_height limit (must fail before allocation at SOF0, exact message)
    ...
    let res_hdr_h = decode_baseline_jpeg(&header_only_sof, limits_height, &cx);
    require!(
        matches!(res_hdr_h, Err(JpegDecodeError::LimitExceeded { ref limit }) if limit == "height 96 exceeds max_height 50"),
        "header-only stream must yield LimitExceeded at SOF0 for height, not Truncated, got: {res_hdr_h:?}"
    );

    // 3. max_pixels limit (must fail before allocation at SOF0, exact message)
    ...
    let res_hdr_p = decode_baseline_jpeg(&header_only_sof, limits_pixels, &cx);
    require!(
        matches!(res_hdr_p, Err(JpegDecodeError::LimitExceeded { ref limit }) if limit == "total pixels 9216 exceeds max_pixels 5000"),
        "header-only stream must yield LimitExceeded at SOF0 for pixels, not Truncated, got: {res_hdr_p:?}"
    );

    // 4. max_tensor_bytes limit (must fail before allocation at SOF0, exact message)
    ...
    let res_hdr_t = decode_baseline_jpeg(&header_only_sof, limits_tensor, &cx);
    require!(
        matches!(res_hdr_t, Err(JpegDecodeError::LimitExceeded { ref limit }) if limit == "tensor bytes 9216 exceeds max_tensor_bytes 5000"),
        "header-only stream must yield LimitExceeded at SOF0 for tensor_bytes, not Truncated, got: {res_hdr_t:?}"
    );
...
fn test_brown_luma_q100_bounded_error() -> Result<(), Box<dyn Error>> {
    ...
    let jpegs = manifest_checked_fixtures()?;
    let q100 = jpegs
        .iter()
        .find(|j| j.name == "brown_luma_q100.jpg")
        .ok_or("brown_luma_q100.jpg missing from the fixture set")?;
    require!(
        q100.quality == 100,
        "brown_luma_q100 quality {}",
        q100.quality
    );
    let bound = q100
        .encoder_max_error
        .ok_or("brown_luma_q100 manifest row carries no encoder_max_error")?;
    let min_psnr = q100
        .encoder_psnr
        .ok_or("brown_luma_q100 manifest row carries no encoder_psnr")?
        - Q100_PSNR_MARGIN_DB;

    let src = source_pixels(&q100.name)?;
    require!(
        src.as_slice() == brown_luma_96x96(),
        "brown_luma_q100 source must be brown_luma_96x96"
    );

    let decoded = decode_baseline_jpeg(&q100.file_bytes, limits, &cx)?;
    let luma = decoded.luma();

    // At Q100 the error is dominated by IDCT arithmetic; the bound is the independent
    // float-IDCT reconstruction error recorded in the committed manifest (1 at .5).
    let max_err = max_abs_error(&src, luma)?;
    require!(
        max_err <= bound,
        "Q100 max error vs source {max_err} exceeds the manifest's independent encoder_max_error {bound}"
    );

    let psnr = compute_psnr(&src, luma)?;
    require_psnr_ge(
        psnr,
        min_psnr,
        "Q100 brown_luma_96x96 vs manifest encoder_psnr",
    )?;

    Ok(())
}

/// Allowed PSNR shortfall at Q100 against the manifest's independent float reconstruction.
const Q100_PSNR_MARGIN_DB: f64 = 1.0;

#[test]
fn test_every_gray_fixture_within_independent_manifest_max_error() -> Result<(), Box<dyn Error>> {
    let cx = test_cx()?;
    let limits = JpegDecodeLimits::default();

    let jpegs = manifest_checked_fixtures()?;
    let mut checked = 0usize;
    for j in jpegs.iter().filter(|j| j.channels == 1) {
        let bound = j
            .encoder_max_error
            .ok_or_else(|| format!("{} manifest row carries no encoder_max_error", j.name))?;
        let src = source_pixels(&j.name)?;
        let decoded = decode_baseline_jpeg(&j.file_bytes, limits, &cx)?;
        let max_err = max_abs_error(&src, decoded.luma())?;
        require!(
            max_err <= bound,
            "{}: max error vs source {max_err} exceeds the manifest's independent encoder_max_error {bound}",
            j.name
        );
        checked += 1;
    }
    require!(checked == 5, "expected 5 grey fixtures, checked {checked}");

    Ok(())
}

#[test]
fn test_yuv420_chroma_row_replication_matches_source_stripes() -> Result<(), Box<dyn Error>> {
    let cx = test_cx()?;
    let limits = JpegDecodeLimits::default();

    // Two-row horizontal bands alternating between two colours. With 4:2:0 each chroma sample
    // row covers exactly one band, so correct replication (chroma row y/2) reproduces every
    // band, while pairing the wrong luma rows gives the other band's chroma to odd rows.
    let (width, height) = (32u32, 32u32);
    let band_a = [200u8, 60, 60];
    let band_b = [60u8, 60, 200];
    let mut rgb_src = Vec::with_capacity((width * height * 3) as usize);
    for y in 0..height {
        let colour = if (y / 2) % 2 == 0 { band_a } else { band_b };
        for _ in 0..width {
            rgb_src.extend_from_slice(&colour);
        }
    }
    let config = JpegConfig {
        quality: 100,
    ...
    let max_err = max_abs_error(&rgb_src, &decoded_rgb)?;
    let psnr = compute_psnr(&rgb_src, &decoded_rgb)?;
    require!(
        max_err <= STRIPE_MAX_ERROR,
        "4:2:0 stripe max error vs source {max_err} > {STRIPE_MAX_ERROR} (psnr {psnr:.2} dB)"
    );
    require_psnr_ge(psnr, 40.0, "4:2:0 stripes")?;

    Ok(())
}

/// Source-referenced bound for the Q100 4:2:0 stripe pattern (colour conversion rounding only).
const STRIPE_MAX_ERROR: u32 = 8;

#[test]
fn test_adobe_app14_transform_refusal() -> Result<(), Box<dyn Error>> {
    let cx = test_cx()?;
    let limits = JpegDecodeLimits::default();

    let (w, h) = (16u32, 16u32);
    let colour = encode_jpeg(
        w,
        h,
        &generate_colorbars_rgb(w, h),
        &JpegConfig {
            quality: 90,
    ...
    let colour_base = decode_baseline_jpeg(&colour, limits, &cx)?;

    let with_app14 = |bytes: &[u8], at: usize, transform: u8| {
        let mut out = bytes[..at].to_vec();
        out.extend_from_slice(&adobe_app14(transform));
        out.extend_from_slice(&bytes[at..]);
        out
    };

    // 1. Transform 0 (RGB) on 3 components, APP14 before SOF0: refused, never decoded as YCbCr.
    let res = decode_baseline_jpeg(&with_app14(&colour, 2, 0), limits, &cx);
    require!(
        matches!(res, Err(JpegDecodeError::Unsupported { ref process }) if process.contains("Adobe APP14 transform 0")),
        "Adobe transform 0 on 3 components must be Unsupported, got {res:?}"
    );

    // 2. The same refusal when APP14 follows SOF0 (component count already known).
    let sos = marker_pos(&colour, 0xDA)?;
    let res = decode_baseline_jpeg(&with_app14(&colour, sos, 0), limits, &cx);
    require!(
        matches!(res, Err(JpegDecodeError::Unsupported { ref process }) if process.contains("Adobe APP14 transform 0")),
        "Adobe transform 0 after SOF0 must be Unsupported, got {res:?}"
    );

    // 3. Transform 1 (YCbCr) on 3 components decodes with identical pixels.
    let ok = decode_baseline_jpeg(&with_app14(&colour, 2, 1), limits, &cx)?;
    require!(
        ok.pixels() == colour_base.pixels(),
        "Adobe transform 1 must decode identically to the plain stream"
    );

    // 4. Transform 2 (YCCK) is refused.
    let res = decode_baseline_jpeg(&with_app14(&colour, 2, 2), limits, &cx);
    require!(
        matches!(res, Err(JpegDecodeError::Unsupported { ref process }) if process.contains("Adobe APP14 transform 2")),
        "Adobe transform 2 must be Unsupported, got {res:?}"
    );

    // 5. Transform 0 on a 1-component image is ordinary greyscale and decodes identically.
    let gray = encode_jpeg(
        96,
        96,
        brown_luma_96x96(),
        &JpegConfig {
    ...
    let gray_base = decode_baseline_jpeg(&gray, limits, &cx)?;
    let ok = decode_baseline_jpeg(&with_app14(&gray, 2, 0), limits, &cx)?;
    require!(
        ok.pixels() == gray_base.pixels(),
        "Adobe transform 0 on a grey image must decode identically"
    );
...
#[test]
fn test_marker_prefix_required_and_fill_bytes_allowed() -> Result<(), Box<dyn Error>> {
    let cx = test_cx()?;
    let limits = JpegDecodeLimits::default();
    ...
    let base = decode_baseline_jpeg(&gray, limits, &cx)?;
    let insert = |at: usize, extra: &[u8]| {
        let mut out = gray[..at].to_vec();
        out.extend_from_slice(extra);
        out.extend_from_slice(&gray[at..]);
        out
    };

    // 1. A bare COM code (no 0xFF prefix) after SOI is invalid syntax.
    let res = decode_baseline_jpeg(&insert(2, &[0xFE, 0x00, 0x02]), limits, &cx);
    require!(
        matches!(res, Err(JpegDecodeError::InvalidSyntax(ref msg)) if msg.contains("expected marker prefix 0xFF at offset 2, found 0xFE")),
        "bare COM without 0xFF prefix must be InvalidSyntax, got {res:?}"
    );

    // 2. A stray non-0xFF byte where a marker is expected is invalid syntax.
    let res = decode_baseline_jpeg(&insert(2, &[0x00]), limits, &cx);
    require!(
        matches!(res, Err(JpegDecodeError::InvalidSyntax(ref msg)) if msg.contains("expected marker prefix 0xFF")),
        "stray byte before a marker must be InvalidSyntax, got {res:?}"
    );

    // 3. Legal 0xFF fill bytes before the first marker decode identically.
    let ok = decode_baseline_jpeg(&insert(2, &[0xFF, 0xFF, 0xFF]), limits, &cx)?;
    require!(
        ok.pixels() == base.pixels(),
        "fill bytes after SOI must be legal"
    );

    // 4. Fill bytes before a prefixed COM segment decode identically.
    let ok = decode_baseline_jpeg(&insert(2, &[0xFF, 0xFF, 0xFE, 0x00, 0x02]), limits, &cx)?;
    require!(
        ok.pixels() == base.pixels(),
        "fill bytes before COM must be legal"
    );

    // 5. Fill bytes before SOS decode identically.
    let sos = marker_pos(&gray, 0xDA)?;
    let ok = decode_baseline_jpeg(&insert(sos, &[0xFF, 0xFF]), limits, &cx)?;
    require!(
        ok.pixels() == base.pixels(),
        "fill bytes before SOS must be legal"
    );
...
#[test]
fn test_tiny_header_claiming_huge_image_refused_before_allocation() -> Result<(), Box<dyn Error>> {
    let limits = JpegDecodeLimits::default();

    // A small real stream whose SOF0 is patched to claim 8192 x 4096. The default limits admit
    // that size (33_554_432 pixels, 96 MiB of RGB), so only the scan-byte bound can refuse it.
    let patch_dims = |mut bytes: Vec<u8>| -> Result<Vec<u8>, Box<dyn Error>> {
        let sof = marker_pos(&bytes, 0xC0)?;
        bytes[sof + 5..sof + 7].copy_from_slice(&4096u16.to_be_bytes());
        bytes[sof + 7..sof + 9].copy_from_slice(&8192u16.to_be_bytes());
        Ok(bytes)
    };
    let (w, h) = (16u32, 16u32);
    let colour = patch_dims(encode_jpeg(
        w,
        h,
        &generate_colorbars_rgb(w, h),
        &JpegConfig {
    ...
    let gray = patch_dims(encode_jpeg(
        w,
        h,
        &fss_reference::media_fixture::jpeg::generate_gradient_gray(w, h),
        &JpegConfig {
            quality: 90,
            subsampling: Subsampling::Grayscale,
            restart_interval: 0,
    ...
    for (label, bytes) in [("colour 4:4:4", &colour), ("grey", &gray)] {
        require!(
            bytes.len() < 1024,
            "{label}: stream must stay tiny, got {} bytes",
            bytes.len()
        );

        // 1. Refused as Truncated at MCU row 0: the scan bytes cannot hold row 0's blocks.
        let cx = test_cx()?;
        let res = decode_baseline_jpeg(bytes, limits, &cx);
        require!(
            matches!(res, Err(JpegDecodeError::Truncated { mcu_row: 0 })),
            "{label}: huge header with tiny scan must be Truncated at row 0, got {res:?}"
        );

        // 2. Refused BEFORE allocation. The luma/RGB planes (128 MiB for colour here) are the
        // only size-proportional allocations and they precede the first per-MCU-row
        // cancellation checkpoint. With a pre-cancelled context any stream that reaches the
        // allocation ends Cancelled; Truncated proves the refusal came first.
        let cancelled = test_cx()?;
        cancelled.request_cancellation();
        let res = decode_baseline_jpeg(bytes, limits, &cancelled);
        require!(
            matches!(res, Err(JpegDecodeError::Truncated { mcu_row: 0 })),
            "{label}: scan-byte bound must refuse before the planes are allocated, got {res:?}"
        );
    }
...
/// Decoder self-output digests (regression pins, not independent evidence). Re-pinned in
/// fss-2h5zq.39 round 2 solely as a consequence of the IDCT pass-1 precision change (two extra
/// fractional bits); the new output is at or closer to an independent libjpeg decode for every
/// fixture. Independent checks live in the manifest and source-referenced tests above.
const GOLDEN_TENSOR_DIGESTS: &[(&str, &str)] = &[
    ...
    (
        ...
        "sha256:6931bb098658f4417dea0f2cc893023034e2bce63dfe7f3975efda8363ccc778",
    ),
    (
        "gray_33x17_checkerboard.jpg",
        "sha256:48935ccd5743de19f7e011edc261317c7b57ec8f777db72e161cba6f2dfd8478",
    ),
    (
        "brown_luma_q100.jpg",
        "sha256:a32e3dc74c6ecf9d33129f4fd40fbf1fc3725ec0fcd01e0ccf1109fbdb26f725",
    ),
    (
        "brown_luma_qfix.jpg",
        "sha256:72031dccb8d142e8c3a95820557f59b12aae5c1f1d01ba3764c56bafbe21bc5c",
    ),
    (
    ...
    (
        ...
        "sha256:b471b5c145c8df3034b0fa878105e85d147fefe73454b6010d7f74928c2851d1",
    ),
    (
        "rgb_33x17_checkerboard_420.jpg",
        "sha256:0e3bc9fafb704bde71fe09d1ab5e8e510acc2d78b84b87f8728cc211a0efbaad",
    ),
    (
        "rgb_64x48_colorbars_420.jpg",
        "sha256:ac14d34813e93bbadda47151a0e1d02623bc817f3693ab0109bcbbe9fd0e105a",
    ),
    (
        "rgb_64x48_colorbars_444.jpg",
        "sha256:e3f8c5a884996e873b4ebacb27265d68997938fea26c3386be0e8a98a81e2c2f",
    ),
    (
...
fn test_single_component_non_1x1_sampling_decodes_non_interleaved() -> Result<(), Box<dyn Error>> {
    ...
    let baseline = decode_baseline_jpeg(&gray, limits, &cx)?;
    ...
    for samp in [0x22u8, 0x12, 0x21, 0x44] {
        ...
        require!(
            decoded.pixels() == baseline.pixels(),
            "pixels must be byte-equal to 1x1 decode for sampling 0x{samp:02X}"
        );
    }
...
#[test]
fn test_sos_component_order_mismatch_rejected() -> Result<(), Box<dyn Error>> {
    let cx = test_cx()?;
    let limits = JpegDecodeLimits::default();
    let (w, h) = (96, 96);
    let src = generate_colorbars_rgb(w, h);
    let config = JpegConfig {
        quality: 90,
        subsampling: Subsampling::Yuv444,
        restart_interval: 0,
    ...
    let encoded = encode_jpeg(w, h, &src, &config)?;

    let sos_pos = (0..encoded.len() - 1)
        .find(|&i| encoded[i] == 0xFF && encoded[i + 1] == 0xDA)
        .ok_or("SOS not found")?;

    require!(encoded[sos_pos + 5] == 1, "comp 0 id must be 1");
    require!(encoded[sos_pos + 7] == 2, "comp 1 id must be 2");
    require!(encoded[sos_pos + 9] == 3, "comp 2 id must be 3");

    // 1. Swap Y and Cb (components 0 and 1)
    let mut swap_y_cb = encoded.clone();
    swap_y_cb[sos_pos + 5] = 2;
    swap_y_cb[sos_pos + 7] = 1;
    let res1 = decode_baseline_jpeg(&swap_y_cb, limits, &cx);
    require!(
        matches!(res1, Err(JpegDecodeError::InvalidSyntax(ref msg)) if msg.contains("SOS component order mismatch")),
        "swapping Y and Cb in SOS must be rejected with InvalidSyntax, got {res1:?}"
    );

    // 2. Swap Cb and Cr (components 1 and 2)
    let mut swap_cb_cr = encoded.clone();
    swap_cb_cr[sos_pos + 7] = 3;
    swap_cb_cr[sos_pos + 9] = 2;
    let res2 = decode_baseline_jpeg(&swap_cb_cr, limits, &cx);
    require!(
        matches!(res2, Err(JpegDecodeError::InvalidSyntax(ref msg)) if msg.contains("SOS component order mismatch")),
        "swapping Cb and Cr in SOS must be rejected with InvalidSyntax, got {res2:?}"
    );
...
#[test]
fn test_sampling_factors_range_validation() -> Result<(), Box<dyn Error>> {
    let cx = test_cx()?;
    let limits = JpegDecodeLimits::default();
    let gray_src = brown_luma_96x96();
    let config = JpegConfig {
    ...
    // Component sampling factor is at sof_pos + 11
    // 1. 0x55 (H=5, V=5 > 4): must refuse with InvalidSyntax
    let mut mutated_55 = gray.clone();
    mutated_55[sof_pos + 11] = 0x55;
    let res_55 = decode_baseline_jpeg(&mutated_55, limits, &cx);
    require!(
        matches!(res_55, Err(JpegDecodeError::InvalidSyntax(ref msg)) if msg.contains("sampling factor")),
        "sampling factor 0x55 must be rejected with InvalidSyntax, got {res_55:?}"
    );

    // 2. 0xFF (H=15, V=15 > 4): must refuse with InvalidSyntax
    let mut mutated_ff = gray.clone();
    mutated_ff[sof_pos + 11] = 0xFF;
    let res_ff = decode_baseline_jpeg(&mutated_ff, limits, &cx);
    require!(
        matches!(res_ff, Err(JpegDecodeError::InvalidSyntax(ref msg)) if msg.contains("sampling factor")),
        "sampling factor 0xFF must be rejected with InvalidSyntax, got {res_ff:?}"
    );

    // 3. 0x44 (H=4, V=4 <= 4): must decode Ok
    let mut mutated_44 = gray.clone();
    mutated_44[sof_pos + 11] = 0x44;
    let res_44 = decode_baseline_jpeg(&mutated_44, limits, &cx);
    require!(
        res_44.is_ok(),
        "sampling factor 0x44 must decode Ok, got {res_44:?}"
    );
...
#[test]
fn test_com_and_app_segments_between_last_mcu_and_eoi() -> Result<(), Box<dyn Error>> {
    let cx = test_cx()?;
    let limits = JpegDecodeLimits::default();
    let gray_src = brown_luma_96x96();
    let config = JpegConfig {
    ...
    let n = gray.len();
    require!(
        gray[n - 2] == 0xFF && gray[n - 1] == 0xD9,
        "encoded JPEG must end with EOI"
    );

    // 1. Insert COM segment (0xFF, 0xFE, len=8, 6 comment bytes) before EOI
    let mut with_com = gray[..n - 2].to_vec();
    with_com.extend_from_slice(&[0xFF, 0xFE, 0x00, 0x08, b'c', b'o', b'm', b'm', b'e', b'n']);
    with_com.extend_from_slice(&[0xFF, 0xD9]);
    let decoded_com = decode_baseline_jpeg(&with_com, limits, &cx)?;
    require!(
        decoded_com.pixels() == baseline.pixels(),
        "COM segment between last MCU and EOI must decode with identical pixels"
    );

    // 2. Insert APP1 segment (0xFF, 0xE1, len=6, 4 payload bytes) before EOI
    let mut with_app = gray[..n - 2].to_vec();
    with_app.extend_from_slice(&[0xFF, 0xE1, 0x00, 0x06, 0x01, 0x02, 0x03, 0x04]);
    with_app.extend_from_slice(&[0xFF, 0xD9]);
    let decoded_app = decode_baseline_jpeg(&with_app, limits, &cx)?;
    require!(
        decoded_app.pixels() == baseline.pixels(),
        "APP1 segment between last MCU and EOI must decode with identical pixels"
    );

    // 3. Insert both APP2 and COM segments before EOI
    let mut with_both = gray[..n - 2].to_vec();
    with_both.extend_from_slice(&[0xFF, 0xE2, 0x00, 0x05, 0x10, 0x20, 0x30]);
    with_both.extend_from_slice(&[0xFF, 0xFE, 0x00, 0x06, b't', b'e', b's', b't']);
    with_both.extend_from_slice(&[0xFF, 0xD9]);
    let decoded_both = decode_baseline_jpeg(&with_both, limits, &cx)?;
    require!(
        decoded_both.pixels() == baseline.pixels(),
        "Multiple APP/COM segments between last MCU and EOI must decode with identical pixels"
    );

    // 4. Trailing data after EOI must still be refused
    let mut trailing = with_com.clone();
    trailing.extend_from_slice(&[0x00, 0x01]);
    let res_trailing = decode_baseline_jpeg(&trailing, limits, &cx);
    require!(
        matches!(res_trailing, Err(JpegDecodeError::InvalidSyntax(ref msg)) if msg.contains("trailing data after EOI marker")),
        "trailing data after EOI must be rejected with InvalidSyntax, got {res_trailing:?}"
    );

    // 5. Truncated COM segment before EOI must be rejected as Truncated
    let mut truncated_com = gray[..n - 2].to_vec();
    truncated_com.extend_from_slice(&[0xFF, 0xFE, 0x00, 0x20, 0x01, 0x02]);
    let res_trunc = decode_baseline_jpeg(&truncated_com, limits, &cx);
    require!(
        matches!(res_trunc, Err(JpegDecodeError::Truncated { .. })),
        "truncated COM segment before EOI must be rejected as Truncated, got {res_trunc:?}"
    );

    // 6. COM and APP segments with length 0 or 1 before EOI must be refused as Truncated
    for (marker_name, marker_byte) in [("COM", 0xFEu8), ("APP0", 0xE0u8), ("APP15", 0xEFu8)] {
        for seg_len in [0u16, 1u16] {
            let mut bad_seg = gray[..n - 2].to_vec();
            bad_seg.extend_from_slice(&[0xFF, marker_byte]);
            bad_seg.extend_from_slice(&seg_len.to_be_bytes());
            bad_seg.extend_from_slice(&[0xFF, 0xD9]);
            let res = decode_baseline_jpeg(&bad_seg, limits, &cx);
            require!(
                matches!(res, Err(JpegDecodeError::Truncated { .. })),
                "{marker_name} segment with length {seg_len} before EOI must be rejected as Truncated, got {res:?}"
            );
        }
    }
...
#[test]
fn test_com_or_app_segment_length_zero_or_one_before_eoi_refused() -> Result<(), Box<dyn Error>> {
    let cx = test_cx()?;
    let limits = JpegDecodeLimits::default();
    let gray_src = brown_luma_96x96();
    let config = JpegConfig {
    ...
    for (name, marker) in [("COM", 0xFEu8), ("APP0", 0xE0u8), ("APP15", 0xEFu8)] {
        for len in [0u16, 1u16] {
            let mut buf = gray[..n - 2].to_vec();
            buf.extend_from_slice(&[0xFF, marker]);
            buf.extend_from_slice(&len.to_be_bytes());
            buf.extend_from_slice(&[0xFF, 0xD9]);
            let res = decode_baseline_jpeg(&buf, limits, &cx);
            require!(
                matches!(res, Err(JpegDecodeError::Truncated { .. })),
                "{name} with length {len} before EOI must be rejected as Truncated, got {res:?}"
            );
        }
    }
