...
use fss_reference::rtsp::recording_catalog::hevc::{
    HEVC_CATALOG_KIND, HevcCatalogBuilder, HevcCatalogWindow, HevcRecordingCatalog,
    prepare_hevc_catalog, verify_hevc_catalog,
};
use fss_reference::rtsp::recording_catalog::{
    CATALOG_KIND, CatalogBuilder, CatalogError as E, CatalogQueryLimits, CatalogScope,
    MAX_CATALOG_BYTES, MAX_CATALOG_WINDOWS, verify_catalog,
};
...
fn independent_canonical_bytes_and_flat_source_closure_are_exact() -> TestResult {
    ...
    let expected = index(
        ...
        &basis,
        &[
            (&a, first.publication_plan()),
            (&b, second.publication_plan()),
        ],
        None,
    )?;
    ...
    assert_eq!(
        ...
        &manifest(
            HEVC_CATALOG_KIND,
            &expected,
            &[first.publication_plan(), second.publication_plan()],
            None
        )?
    );
    ...
    for recording in [&first, &second] {
        assert!(
            catalog
                .manifest()
                .children()
                .contains(&recording.manifest().root())
        );
    ...
    assert_eq!(
        verified.byte_len(),
        catalog.index_bytes().len() + catalog.manifest().canonical_bytes().len()
    );
...
fn query_reports_whole_windows_requested_overlaps_and_unindexed_intervals() -> TestResult {
    ...
    assert_eq!(
        selected.unindexed(),
        &[0..1_000, 73_000..100_000, 172_000..200_000]
    );
    assert_eq!(
        selected.output_bytes(),
        c.entries().iter().map(|e| e.byte_len() as u64).sum::<u64>()
    );
...
fn half_open_edges_and_empty_matches_never_become_coverage_claims() -> TestResult {
    ...
    assert_eq!(
        c.select(73_000..100_001, CatalogQueryLimits::default())?
            .windows()[0]
            .ordinal(),
        1
    );
    for query in [
        ...
        assert_eq!(
            c.select(query, CatalogQueryLimits::default()).err(),
            Some(E::Interval)
        );
    }
    Ok(())
...
fn byte_and_window_limits_refuse_the_whole_selection_without_truncation() -> TestResult {
    ...
    for limits in [
        ...
        CatalogQueryLimits {
            max_windows: MAX_CATALOG_WINDOWS + 1,
            max_output_bytes: bytes,
        },
        CatalogQueryLimits {
            max_windows: 2,
...
fn duplicate_roots_slots_overlaps_and_scope_failures_leave_builder_retryable() -> TestResult {
    let first = window(0)?;
    let next = window(100_000)?;
    let overlap = window(71_999)?;
    let a = slot(0)?;
    let b = slot(1)?;
    ...
    let mut wrong = scope()?;
    wrong.recording.generation = 2;
    let mut builder = HevcCatalogBuilder::new(wrong)?;
    assert_eq!(builder.push(&a, &first), Err(E::Scope));
    assert!(builder.is_empty());
    Ok(())
}
...
fn expected_scope_and_decode_clock_are_not_learned_from_the_index() -> TestResult {
    ...
    for field in 0..7 {
        ...
        assert_eq!(
            verify_hevc_catalog(c.manifest(), c.index_bytes(), &wrong).err(),
            Some(E::Scope)
        );
    }
    let mut wrong = scope()?;
...
fn avc_bytes_stay_identical_and_public_entrypoints_do_not_switch_codecs() -> TestResult {
    ...
    let basis = CatalogScope {
        ...
        decode_clock: ContentDigest::sha256(b"avc-test-clock"),
        time_scale: 90_000,
    };
    let a = slot(0)?;
    ...
    assert_eq!(
        catalog.manifest(),
        &manifest(CATALOG_KIND, &expected, &[&avc], None)?
    );
...
fn rehashed_codec_relabeling_cannot_change_the_window_family() -> TestResult {
    ...
    let a = slot(0)?;
    let basis = scope()?;
    let hevc_as_avc = index(
        "fss.recording_catalog.v1",
        &basis,
        &[(&a, h.publication_plan())],
        None,
    )?;
    let root = manifest(CATALOG_KIND, &hevc_as_avc, &[h.publication_plan()], None)?;
    assert_eq!(
        verify_catalog(&root, &hevc_as_avc, &basis).err(),
        Some(E::Digest)
    );
    let avc = recording_support::fixture(1, false)?.prepare()?;
    let basis = CatalogScope {
    ...
    assert_eq!(
        verify_hevc_catalog(&root, &avc_as_hevc, &basis).err(),
        Some(E::Digest)
    );
    Ok(())
}
...
fn canonical_checksum_closure_and_declared_window_roots_fail_closed() -> TestResult {
    ...
    let last = changed.last_mut().ok_or("empty index")?;
    *last ^= 1;
    assert_eq!(
        verify_hevc_catalog(c.manifest(), &changed, &basis).err(),
        Some(E::Digest)
    );
    let mut extra = c.index_bytes().to_vec();
    extra.push(0);
    assert!(verify_hevc_catalog(c.manifest(), &extra, &basis).is_err());
    let no_leaves = ObjectManifest::new(
        HEVC_CATALOG_KIND,
        [h.manifest().root()],
        Some(ContentDigest::sha256(c.index_bytes())),
    )?;
    assert_eq!(
        verify_hevc_catalog(&no_leaves, c.index_bytes(), &basis).err(),
        Some(E::Digest)
    );
    let roots = [ContentDigest::sha256(
        b"unrelated source with plausible time",
    )];
    let changed = index(
        "fss.hevc_recording_catalog.v1",
        &basis,
    ...
    assert_eq!(
        verify_hevc_catalog(&root, &changed, &basis).err(),
        Some(E::Digest)
    );
    assert_eq!(
        verify_hevc_catalog(c.manifest(), &vec![0; MAX_CATALOG_BYTES + 1], &basis).err(),
        Some(E::Limit)
    );
    Ok(())
}
...
fn catalog_capacity_empty_pages_and_deterministic_replay_have_fixed_bounds() -> TestResult {
    assert_eq!(
        HevcCatalogBuilder::new(scope()?)?.prepare().err(),
        Some(E::Limit)
    );
    let mut builder = HevcCatalogBuilder::new(scope()?)?;
    ...
    assert_eq!(
        builder.push(
            &slot(MAX_CATALOG_WINDOWS)?,
            &window(MAX_CATALOG_WINDOWS as u64 * 72_000)?
        ),
        Err(E::Limit)
    );
    ...
    assert_eq!(
        a.select(1_000..172_000, CatalogQueryLimits::default())?,
