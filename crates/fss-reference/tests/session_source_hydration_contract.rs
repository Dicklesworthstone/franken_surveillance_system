...
use fss_core::{
    ...
    LedgerAnchor, MissionId, ObjectId, PrincipalId, SemanticHandle, SemanticHandleSpec, SessionId,
...
use fss_reference::{
    PublishedSourceReader, ReferenceHydrationCatalog, ReferenceSessionError, ReferenceSessionStore,
    SessionAlias, SessionBindingRequest, SessionRefresh, SourceHydrationError,
};
...
fn fixture() -> Result<Fixture, Box<dyn Error>> {
    ...
    let descriptor = SemanticHandle::publish(SemanticHandleSpec {
        ...
        required_capabilities: levels
            .iter()
            .map(|level| {
                let capability = if *level == HydrationLevel::H3 {
                    "capability:source"
                } else {
                    "capability:preview"
                };
...
fn request(f: &Fixture, level: HydrationLevel) -> Result<HydrationRequest, Box<dyn Error>> {
    Ok(HydrationRequest::publish(HydrationRequestSpec {
        contract_basis: f.descriptor.contract_basis.clone(),
        session_id: f.params.session_id.clone(),
        handle_id: f.descriptor.handle_id.clone(),
        expected_descriptor_digest: f.descriptor.descriptor_digest,
        expected_subject_digest: f.descriptor.subject_digest,
        anchor: f.descriptor.anchor.clone(),
...
fn original_bytes_have_exact_custody_binding_and_no_second_payload_cache() -> TestResult {
    ...
    for now in [20, 21] {
        let response = f.sessions.hydrate_from_source(
        ...
        assert_eq!(
            response.artifact.as_ref().ok_or("missing source")?.payload,
            SOURCE
        );
    }
...
fn cached_and_source_entrypoints_agree_for_lower_level_receipts_and_charges() -> TestResult {
    ...
    let source = f.sessions.hydrate_from_source(
        &f.params.principal_id,
        &f.alias,
        &req,
        &mut f.catalog,
    ...
    assert_eq!(
        sessions.remaining_token_budget(
            &f.params.principal_id,
            &f.params.session_id,
            TimestampNs(20)
        )?,
...
fn missing_wrong_principal_closed_and_expired_sessions_never_read_source() -> TestResult {
    for scenario in 0..4 {
        ...
        match scenario {
            ...
            2 => f
                .sessions
                .close(&principal, &alias.session_id, TimestampNs(20))?,
            _ => {}
        }
        let reader = Counted {
            store: &f.store,
        ...
        assert!(matches!(
            ...
            Err(DeliveryError::Session(ReferenceSessionError::Unavailable))
        ));
        assert_eq!(reader.calls.get(), 0);
    }
    Ok(())
...
fn full_request_allowance_and_cumulative_charges_are_checked_before_io() -> TestResult {
    ...
    f.sessions.open(
        f.params.clone(),
        f.descriptor.contract_basis.clone(),
        TimestampNs(20),
    )?;
    assert!(matches!(
        f.sessions.hydrate_from_source(
    ...
    assert!(matches!(
        g.sessions.hydrate_from_source(
            &g.params.principal_id,
            &g.alias,
            &excess,
            &mut g.catalog,
            &reader,
...
fn narrowed_request_can_explicitly_fall_back_without_reading_source() -> TestResult {
    ...
    assert_eq!(
        response.artifact.as_ref().ok_or("missing preview")?.level,
        HydrationLevel::H2
    );
    assert_eq!(reader.calls.get(), 0);
    assert_eq!(
...
fn source_quote_byte_and_token_refusals_do_not_touch_custody() -> TestResult {
    for bytes in [true, false] {
        ...
        let mut req = request(&f, HydrationLevel::H3)?;
        if bytes {
            req.budget.bytes = 1023;
        } else {
            req.budget.tokens = 511;
        }
        reseal(&mut req);
        let reader = Counted {
            store: &f.store,
...
fn source_failure_preserves_cursor_and_tokens_but_success_consumes_both() -> TestResult {
    ...
    assert!(matches!(
        ...
        Err(DeliveryError::Source(SourceHydrationError::SourceMismatch))
    ));
    assert!(
        !f.catalog
    ...
    assert!(matches!(
        ...
        Err(DeliveryError::Source(SourceHydrationError::Hydration(
...
fn tombstoned_source_cannot_be_retrieved_from_prior_disclosure() -> TestResult {
    ...
    assert!(matches!(
        ...
        Err(DeliveryError::Source(SourceHydrationError::Object(_)))
    ));
    assert_eq!(
        f.sessions.remaining_token_budget(
            &f.params.principal_id,
...
fn revoked_source_grant_stays_revoked_after_fresh_symbol_binding() -> TestResult {
    ...
    let mut caps = current.capabilities.clone();
    caps.remove("capability:source");
    let narrowed = f.sessions.refresh(
        &f.params.principal_id,
        &f.params.session_id,
        SessionRefresh {
    ...
    let alias = f.sessions.bind(
        ...
        &SessionBindingRequest {
            session_id: f.params.session_id.clone(),
            generation: narrowed.symbol_table_generation,
            handle_id: f.descriptor.handle_id.clone(),
            descriptor_digest: f.descriptor.descriptor_digest,
        },
        &f.catalog,
        TimestampNs(20),
    )?;
    ...
    req.available_capabilities = caps;
    reseal(&mut req);
    assert!(
        f.sessions
            .hydrate_from_source(
