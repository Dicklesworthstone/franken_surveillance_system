...
fn fresh(name: &str) -> Result<PathBuf, Error> {
    let path = Path::new(env!("CARGO_TARGET_TMPDIR"))
        .join("hevc_recording_publication_contract")
        .join(name);
...
fn slot() -> Result<SlotName, Error> {
    Ok(SlotName::parse("hevc-recording-001")?)
}
fn publish(
...
fn unchanged_publisher_stages_source_first_and_never_serves_unpublished_hevc() -> TestResult {
    let plan = fixture()?;
    let content = plan.publication_plan();
    let mut owner = LocalRootPublisher::open(fresh("source_first")?, limits())?;
    {
        let mut job =
            RecordingPublication::new(content, &mut owner, slot()?, plan.byte_len(), 100)?;
        for (now, expected) in [
        ...
        {
            assert!(
                matches!(job.step(now as u64, &NeverCancel)?, RecordingProgress::ChildStaged { role, remaining, .. }
    ...
    assert_eq!(
        owner.spool().read(content.children()[0].1)?,
        plan.objects().source
    );
    assert!(matches!(
        load_hevc_recording(
    ...
    assert_eq!(
        publish(content, &mut owner, slot()?)?,
...
fn process_reopen_replays_and_preserves_all_bytes_maps_and_source_only_witnesses() -> TestResult {
    ...
    assert_eq!(read.manifest(), plan.manifest());
    assert_eq!(read.summary(), plan.summary());
    ...
    assert_eq!(read.samples(), plan.samples());
    assert_eq!(read.mappings(), plan.mappings());
    for (a, b) in read.packets()?.iter().zip(&originals[..8]) {
        assert_eq!(a.sequence, b.sequence);
        assert_eq!(a.received_ns, b.received_ns);
...
fn a_lost_final_receipt_reconciles_without_replacing_or_republishing_a_different_root() -> TestResult
{
    ...
    let loaded = load_hevc_recording(
        &owner,
        &slot()?,
        plan.manifest().root(),
        &scope()?,
    ...
    assert_eq!(
        publish(loaded.publication_plan(), &mut owner, slot()?)?,
        PublishOutcome::AlreadyPublished
    );
    Ok(())
}
...
fn cancellation_and_root_temp_cancellation_preserve_source_without_claiming_a_root() -> TestResult {
    ...
    for at_root in [false, true] {
        let mut owner = LocalRootPublisher::open(
            fresh(if at_root {
                "cancel_root"
            } else {
                "cancel_child"
        ...
        )?;
        {
            ...
            if at_root {
                ...
                assert!(
                    job.step(4, &CancelAt(PublishCutPoint::AfterRootTempWrite))
                        .is_err()
                );
            } else {
                assert!(matches!(
                    job.step(1, &CancelAt(PublishCutPoint::AfterChildrenVerified)),
                    Err(E::Cancelled)
                ));
            }
        ...
        assert_eq!(
            owner
                .spool()
                .read(plan.publication_plan().children()[0].1)?,
            plan.objects().source
        );
        assert!(
            load_hevc_recording(
...
fn cancellation_at_each_read_or_after_full_replay_prevents_disclosure_not_durability() -> TestResult
{
    ...
    for at in 0..6 {
        let cancel = CancelReadAt {
            calls: AtomicUsize::new(0),
            at,
        };
        assert!(matches!(
            load_hevc_recording(&owner, &slot()?, plan.manifest().root(), &scope()?, &cancel),
            Err(E::Cancelled)
        ));
        assert_eq!(cancel.calls.load(Ordering::SeqCst), at + 1);
        assert_eq!(
            owner.root(&slot()?).ok_or("root was retracted")?.state,
...
fn wrong_root_scope_and_conflicting_recording_never_overwrite_the_existing_slot() -> TestResult {
    ...
    assert_eq!(
        owner.root(&slot()?).ok_or("lost root")?.root,
        plan.manifest().root()
    );
    Ok(())
}
...
fn limits_deadlines_clock_reversal_and_completed_job_semantics_are_unchanged() -> TestResult {
    ...
    assert!(matches!(
        job.step(10, &NeverCancel)?,
        RecordingProgress::ChildStaged {
            role: RecordingRole::Initialization,
            ..
        }
    ));
    for now in 11..14 {
    ...
    assert_eq!(
        job.step(200, &CancelAt(PublishCutPoint::AfterChildrenVerified))?,
...
fn range(e: &mut CanonicalEncoder, r: &std::ops::Range<usize>) {
...
fn forged_boundary_index(plan: &PreparedHevcRecording) -> Result<Vec<u8>, Error> {
    ...
    let b = plan.objects();
    e.text("fss.hevc_recording_window.index.v1");
    ...
    e.u32(SSRC);
    e.u32(u32::from(PT));
    e.u32(plan.summary().time_scale);
    e.digest(ContentDigest::sha256(b.source));
    e.digest(ContentDigest::sha256(b.initialization));
    e.digest(ContentDigest::sha256(b.media));
    for r in plan.parameter_ranges() {
        range(&mut e, r);
    }
    e.u64(plan.source_only_nals() as u64);
    e.u64(plan.samples().len() as u64);
    for (i, sample) in plan.samples().iter().enumerate() {
        range(&mut e, &sample.range);
        e.u64(sample.decode_time);
        e.u64(sample.presentation_time);
        e.u32(sample.duration);
        e.u32(sample.rtp_timestamp);
        e.bool(sample.idr);
        let tag = match sample.boundary {
            HevcBoundary::NextFirstSlice => 1,
            HevcBoundary::NextAccessUnitPrefix => 2,
            HevcBoundary::AccessUnitDelimiter => 3,
            HevcBoundary::EndOfSequence => 4,
            HevcBoundary::EndOfBitstream => 5,
    ...
    for m in plan.mappings() {
        ...
        for span in &m.sources {
            e.u64(span.sequence);
            range(&mut e, &span.wire_range);
            range(&mut e, &span.nal_range);
            e.bool(span.fragment_header_range.is_some());
            if let Some(r) = &span.fragment_header_range {
                range(&mut e, r);
            }
        }
    }
...
fn even_a_durable_rehashed_forged_index_must_pass_native_source_replay_on_readback() -> TestResult {
    let plan = fixture()?;
    let bytes = plan.objects();
    let fake_index = forged_boundary_index(&plan)?;
    let forged = ObjectManifest::new(
        HEVC_RECORDING_KIND,
        [
            ContentDigest::sha256(bytes.source),
            ContentDigest::sha256(bytes.initialization),
            ContentDigest::sha256(bytes.media),
        ],
    ...
    let owner = LocalRootPublisher::open(&path, limits())?;
    assert_eq!(
        owner.root(&slot()?).ok_or("forged root not durable")?.state,
        LocalPublicationState::Durable
    );
    assert!(matches!(
        load_hevc_recording(&owner, &slot()?, forged.root(), &scope()?, &NeverCancel),
        Err(E::Content(RecordingError::Source))
    ));
    Ok(())
}
