...
fn test_edge_case_appn_com_ffd9_shielding() -> Result<(), Box<dyn Error>> {
    ...
    let duration_ms = start.elapsed().as_millis().max(1);
    let passed = match &scan_res {
        Ok(scan) => {
            scan.frames.len() == 1
                && scan.frames.first().map(|f| f.end_offset) == Some(expected_len)
                && scan.frames.first().map(|f| f.has_eoi) == Some(true)
                && scan.frames.first().map(|f| !f.is_truncated) == Some(true)
                && scan.frames.first().map(|f| f.marker_count) == Some(4)
    ...
    let observed = match &scan_res {
        Ok(scan) => {
            if let Some(f0) = scan.frames.first() {
                ...
                format!(
                    r#"{{"frame_count":0,"error":"missing_frame_0","findings_count":{}}}"#,
                    scan.findings.len()
                )
            }
        }
...
fn test_edge_case_byte_stuffing() -> Result<(), Box<dyn Error>> {
    ...
    let passed = match &scan_res {
        Ok(scan) => {
            scan.frames.len() == 1
                && scan.frames.first().map(|f| f.end_offset) == Some(38)
                && scan.frames.first().map(|f| f.has_eoi) == Some(true)
                && scan.frames.first().map(|f| !f.is_truncated) == Some(true)
                && scan.frames.first().map(|f| f.marker_count) == Some(2)
                && scan.findings.is_empty()
        }
        Err(_) => false,
    };
...
fn test_edge_case_restart_markers() -> Result<(), Box<dyn Error>> {
    ...
    let passed = match &scan_res {
        Ok(scan) => {
            scan.frames.len() == 1
                && scan.frames.first().map(|f| f.end_offset) == Some(62)
                && scan.frames.first().map(|f| f.restart_interval) == Some(4)
                && scan.frames.first().map(|f| f.has_eoi) == Some(true)
                && scan.frames.first().map(|f| !f.is_truncated) == Some(true)
                && scan.findings.is_empty()
        }
...
fn test_edge_case_zero_length_segment() -> Result<(), Box<dyn Error>> {
    ...
    let duration_ms = start.elapsed().as_millis().max(1);
    let zero_length_offset = match &scan_res {
        Ok(scan) => match scan.findings.first() {
    ...
    let passed = match &scan_res {
        Ok(scan) => {
            scan.frames.len() == 1
                && scan.frames.first().map(|f| f.end_offset) == Some(8)
                && scan.frames.first().map(|f| f.marker_count) == Some(1)
...
fn test_edge_case_fill_bytes_runs() -> Result<(), Box<dyn Error>> {
    ...
    let (width, height) = match &scan_res {
        Ok(scan) => scan
            .frames
            .first()
    ...
    let passed = match &scan_res {
        Ok(scan) => {
            scan.frames.len() == 1
                && scan.frames.first().map(|f| f.end_offset) == Some(36)
                && (width, height) == (16, 8)
                && scan.frames.first().map(|f| f.marker_count) == Some(2)
                && scan.findings.is_empty()
        }
...
fn test_edge_case_zero_components() -> Result<(), Box<dyn Error>> {
    ...
    let duration_ms = start.elapsed().as_millis().max(1);
    let zero_components_offset = match &scan_res {
        Ok(scan) => match scan.findings.first() {
    ...
    let zero_components_finding = match &scan_res {
        Ok(scan) => matches!(
            scan.findings.first(),
    ...
    let passed = match &scan_res {
        Ok(scan) => {
            scan.frames.len() == 1 && zero_components_finding && zero_components_offset == 2
        }
        Err(_) => false,
    };
    let (verdict, exit_code) = if passed { ("pass", 0) } else { ("fail", 1) };
...
fn test_synthetic_stream_variants() -> Result<(), Box<dyn Error>> {
    ...
    let duration_ms = start.elapsed().as_millis().max(1);
    let clean_ok = res_clean
        .as_ref()
        .map(|s| {
            s.frame_count() == 3
                && s.valid_frame_count() == 3
                && !s.has_truncation()
                && !s.has_omissions()
                && s.findings.is_empty()
        })
        .unwrap_or(false);
    let trunc_ok = res_trunc
        .as_ref()
        .map(|s| s.frame_count() == 3 && s.valid_frame_count() == 2 && s.has_truncation())
        .unwrap_or(false);
    let junk_ok = res_junk
        .as_ref()
        .map(|s| s.frame_count() == 3 && s.omissions.len() == 2 && s.findings.len() == 2)
        .unwrap_or(false);
    let zero_ok = res_zero == Err(JpegSplitError::NoSoi);
    let dim_ok = res_dim
        .as_ref()
        .map(|s| s.frame_count() == 2)
        .unwrap_or(false);

    let passed = variants_tested == 5 && clean_ok && trunc_ok && junk_ok && zero_ok && dim_ok;
    let (verdict, exit_code) = if passed { ("pass", 0) } else { ("fail", 1) };
    let expected = r#"{"variants_tested":5,"clean_frames":3,"truncated_valid":2,"omissions":2,"zero_len_error":"NoSoi"}"#;
    let observed = format!(
        r#"{{"variants_tested":{},"clean_frames":{},"truncated_valid":{},"omissions":{},"zero_len_error":"{}"}}"#,
        variants_tested,
        res_clean
            .as_ref()
            .map(|s| s.valid_frame_count())
            .unwrap_or(0),
        res_trunc
            .as_ref()
            .map(|s| s.valid_frame_count())
            .unwrap_or(0),
        res_junk.as_ref().map(|s| s.omissions.len()).unwrap_or(0),
        escape_json_str(&format!("{:?}", res_zero))
    );
    emit_caplog(
...
fn test_delegated_jpeg_fixtures() -> Result<(), Box<dyn Error>> {
    ...
    let expected = if verdict == "pass" {
        r#"{"fixtures_verified":2,"failures":0}"#.to_string()
    } else if verdict == "skip" {
        r#"{"fixtures_verified":2,"status":"skipped"}"#.to_string()
    } else {
...
fn test_gauntlet_in_frame_anomalies() -> Result<(), Box<dyn Error>> {
    ...
    let passed = scan_res.as_ref().is_ok_and(|scan| {
        scan.frames.len() == 1
            && scan.frames.first().map(|f| f.end_offset) == Some(35)
            && scan.frames.first().map(|f| f.marker_count) == Some(5)
            && scan.findings.len() == 5
    });
    let (verdict, exit_code) = if passed { ("pass", 0) } else { ("fail", 1) };
...
fn test_gauntlet_flood_limits() -> Result<(), Box<dyn Error>> {
    ...
    let cap_boundary_exact_3_passes = scan_3.as_ref().is_ok_and(|s| {
        s.frames.len() == 1
            && s.frames.first().map(|f| f.marker_count) == Some(3)
            && s.frames.first().map(|f| f.has_eoi) == Some(true)
    });
...
fn test_mutation_gauntlet_10k() -> Result<(), Box<dyn Error>> {
    ...
    let duration_ms = start.elapsed().as_millis().max(1);
    let passed = observed_panics == 0 && invariant_violations == 0 && successful_scans > 0;
    let (verdict, exit_code) = if passed { ("pass", 0) } else { ("fail", 1) };
    let expected = r#"{"panics":0,"invariant_violations":0,"successful_scans_positive":true}"#;
    let observed = format!(
        r#"{{"panics":{},"invariant_violations":{},"successful_scans":{},"iterations":{}}}"#,
        observed_panics, invariant_violations, successful_scans, iterations
    );
    emit_caplog(
...
fn test_gauntlet_nested_soi() -> Result<(), Box<dyn Error>> {
    ...
    let passed = scan_res.as_ref().is_ok_and(|scan| {
        scan.frames.len() == 2
            && scan.frames.first().map(|f| f.is_truncated) == Some(true)
            && scan.frames.get(1).map(|f| f.has_eoi) == Some(true)
            && scan.findings.len() == 1
    });
    let (verdict, exit_code) = if passed { ("pass", 0) } else { ("fail", 1) };
    let expected =
    ...
    let observed = match &scan_res {
        Ok(scan) => {
            let f0_trunc = scan
                .frames
                .first()
                .map(|f| f.is_truncated.to_string())
                .unwrap_or_else(|| "none".to_string());
            let f1_eoi = scan
                .frames
                .get(1)
                .map(|f| f.has_eoi.to_string())
                .unwrap_or_else(|| "none".to_string());
            format!(
