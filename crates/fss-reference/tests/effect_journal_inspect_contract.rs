...
use fss_ledger::{HostJournalReadIo, IncompleteTailPolicy, JournalFileMetadata, JournalReadIo};
...
fn test_effect_journal_differential_clean() -> TestResult {
...

    {
        ...
        let digest = ContentDigest::sha256(b"observed-result");
    ...
    assert_eq!(inspection.obligation_counts.verified, 1);
    assert_eq!(inspection.obligation_counts.pending, 0);
    assert_eq!(inspection.obligation_counts.failed, 0);
    assert_eq!(inspection.obligation_counts.cancelled, 0);
    assert_eq!(inspection.obligation_counts.indeterminate, 0);
    assert_eq!(inspection.obligation_counts.terminal(), 1);
    ...
    assert_eq!(
        inspection.journal.as_ref().map(|j| j.operations().count()),
        Some(opened.operations().count())
    );
...
fn test_effect_journal_differential_indeterminate_lost_ack() -> TestResult {
    ...
    assert_eq!(inspection.obligation_counts.total, 1);
    assert_eq!(inspection.obligation_counts.pending, 0);
    assert_eq!(inspection.obligation_counts.indeterminate, 1);
...
fn test_effect_journal_differential_foreign_trailing_bytes() -> TestResult {
    ...
    let reject_open = DurableEffectJournal::open(
        copy_dir_path.join("effects.fssj"),
        IncompleteTailPolicy::Reject,
    );
    assert!(reject_open.is_err());

    let trunc_open = DurableEffectJournal::open(
        copy_dir_path.join("effects.fssj"),
