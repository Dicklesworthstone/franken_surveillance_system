...
use fss_packet::hevc::{HevcAssemblyError, HevcAssemblyLimits, HevcAssemblyStep as Step,
    HevcBoundary, HevcPictureGroup, HevcRetirementReason};
...
fn config() -> ClientConfig {
    ClientConfig { presentation_uri: "rtsp://camera.local/live/".into(),
        ...
        channels: (0, 1), response_timeout_ns: 100, default_session_timeout_seconds: 60 }
}
fn response(cseq: u32, fields: &[(&str, &str)], body: &[u8]) -> Vec<u8> {
    let mut head = format!("RTSP/1.0 200 OK\r\nCSeq: {cseq}\r\nContent-Length: {}\r\n", body.len());
    for (key, val) in fields { head.push_str(&format!("{key}: {val}\r\n")); }
    head.push_str("\r\n");
...
fn ap(nals: &[&[u8]]) -> Vec<u8> {
    ...
    for nal in nals { bytes.extend_from_slice(&(nal.len() as u16).to_be_bytes()); bytes.extend_from_slice(nal); }
    bytes
}
fn drain(client: &mut RtspHevcPictureClient, now: u64)
    -> Result<Vec<P>, Box<dyn std::error::Error>>
{
    ...
    for _ in 0..2048 {
        ...
        let stop = match &event {
            ...
            P::Client { event, work } => work.is_some() || matches!(event.as_ref(),
                Raw::AuthenticationRequired { .. } | Raw::Backpressure { .. } | Raw::KeepAliveDue | Raw::Fault { .. }),
...
fn feed(client: &mut RtspHevcPictureClient, bytes: &[u8], now: u64)
    -> Result<Vec<P>, Box<dyn std::error::Error>>
{
...
fn pictures(events: &[P]) -> Vec<&HevcPictureGroup> {
    events.iter().filter_map(|e| match e {
        P::Assembly { step: Step::Accepted(out), .. } => out.picture.as_ref(),
        P::Ended { tail: Some(out), .. } => out.picture.as_ref(),
        _ => None,
    }).collect()
}
fn prime(client: &mut RtspHevcPictureClient) -> TestResult {
    ...
    for seq in 0..4 { feed(client, &frame(seq, 0, &[72, 1, 0x80], false), 8 + u64::from(seq))?; }
    assert_eq!(client.retained_nal_bytes(), 0);
    assert_eq!(client.queued_nals(), 0);
    Ok(())
}
fn open(reorder: ReorderLimits, codec: H265Limits, assembly: HevcAssemblyLimits)
    -> Result<RtspHevcPictureClient, Box<dyn std::error::Error>>
{
    ...
    feed(&mut client, &response(1, &[("Content-Type", "application/sdp")], SDP), 1)?;
    client.request(C::Setup, 2)?;
    feed(&mut client, &response(2, &[("Session", "private-session;timeout=60"),
        ("Transport", "RTP/AVP/TCP;unicast;interleaved=0-1;ssrc=00000007")], &[]), 3)?;
    client.request(C::Play, 4)?;
    feed(&mut client, &response(3, &[("Session", "private-session")], &[]), 5)?;
    assert_eq!(client.state(), ClientState::Playing);
    prime(&mut client)?;
    Ok(client)
}
fn normal() -> Result<RtspHevcPictureClient, Box<dyn std::error::Error>> {
    open(ReorderLimits::default(), H265Limits::default(), HevcAssemblyLimits::default())
}
...
fn marked_multislice_picture_waits_for_real_next_first_slice_and_preserves_originals() -> TestResult {
    ...
    assert!(first.iter().any(|e| matches!(e, P::Source { source, .. } if source.bytes() == rtp(4, 100, FIRST, true))));
...
fn fragmented_and_reordered_first_slice_retains_every_original_copy_span() -> TestResult {
    ...
    for (index, payload) in [start, middle, end].into_iter().enumerate() {
        ...
        assert_eq!(&source[span.wire_range.clone()], &nal.bytes()[span.nal_range.clone()]);
...
fn transport_gap_invalidates_picture_and_late_continuation_cannot_repair_it() -> TestResult {
    let mut client = open(ReorderLimits { max_delay_ns: 10, ..ReorderLimits::default() },
        H265Limits::default(), HevcAssemblyLimits::default())?;
    ...
    assert!(output.iter().any(|e| matches!(e, P::Assembly { step: Step::Refused(r), .. }
...
fn codec_failure_returns_the_original_packet_and_retires_the_pending_picture() -> TestResult {
    ...
    assert!(output.iter().any(|e| matches!(e, P::CodecRefused { source, picture: Some(p), .. }
...
fn incomplete_fragment_eof_never_flushes_the_preceding_partial_picture() -> TestResult {
    ...
    assert!(matches!(client.poll(23)?, P::Ended { client: None, tail: None, interrupted_picture: None }));
    Ok(())
}

#[test]
fn fragment_expiry_propagates_without_another_network_packet_or_a_false_tail() -> TestResult {
    let mut client = open(ReorderLimits::default(),
        H265Limits { max_pending_age_ns: 10, ..H265Limits::default() },
        HevcAssemblyLimits { max_age_ns: 100, ..HevcAssemblyLimits::default() })?;
    feed(&mut client, &frame(4, 100, FIRST, false), 20)?;
    ...
    assert_eq!(client.next_wake_ns(), Some(31));
    let output = drain(&mut client, 31)?;
    assert!(output.iter().any(|e| matches!(e, P::FragmentRetired { fragment, picture: Some(p) }
...
fn picture_deadline_is_exposed_and_not_renewed_by_a_continuation_or_marker() -> TestResult {
    let mut client = open(ReorderLimits::default(), H265Limits::default(),
        HevcAssemblyLimits { max_age_ns: 10, ..HevcAssemblyLimits::default() })?;
    ...
    assert_eq!(pictures(&output)[0].boundary(), HevcBoundary::EndOfInputUnverified);
    assert!(pictures(&output)[0].discontinuity_before());
    Ok(())
}
...
fn complete_queued_nals_expire_instead_of_starting_fresh_pictures_from_aged_work() -> TestResult {
    ...
    let refusal = client.ingest(&frame(5, 200, FIRST, false), 21).err().ok_or("NAL backpressure bypassed")?;
...
fn request_deadline_is_enforced_before_draining_queued_derivatives() -> TestResult {
    ...
    let P::Client { event, work: Some(work) } = client.poll(120)? else {
        return Err("request expiry was hidden by queued NALs".into());
    };
    assert!(matches!(event.as_ref(), Raw::Fault { reason: HevcClientError::Session(ClientError::ResponseTimeout), .. }));
...
fn cancellation_accounts_for_picture_queue_and_exact_unprocessed_tcp() -> TestResult {
    ...
    assert_eq!(retired.work.picture.ok_or("picture ownership lost")?.nals, 1);
...
fn end_of_bitstream_fences_lookahead_and_preserves_the_last_picture() -> TestResult {
    ...
    assert_eq!(pictures(&output)[0].boundary(), HevcBoundary::EndOfBitstream);
    let retired = output.iter().find_map(|e| match e {
        P::Assembly { retirement: Some(retired), .. } => Some(retired), _ => None,
...
fn clean_eof_and_teardown_preserve_unverified_tail_and_distinct_remote_outcomes() -> TestResult {
    for teardown in [false, true] {
        ...
        let output = if teardown {
            ...
            feed(&mut client, &response(4, &[("Session", "private-session")], &[]), 22)?
        } else {
            client.finish(); drain(&mut client, 22)?
        };
        assert_eq!(pictures(&output).len(), 1);
        assert_eq!(pictures(&output)[0].boundary(), HevcBoundary::EndOfInputUnverified);
        let raw = output.iter().find_map(|e| match e {
            P::Ended { client: Some(raw), .. } => Some(raw.as_ref()), _ => None,
        ...
        assert!(matches!(client.poll(23)?, P::Ended { client: None, tail: None, .. }));
    }
    Ok(())
}
...
fn authenticated_keepalive_waits_preserve_challenges_and_picture_timer_wakes() -> TestResult {
    ...
    let mut client = RtspHevcPictureClient::with_digest(config(), KEY, ReorderLimits::default(),
        H265Limits::default(), HevcAssemblyLimits { max_age_ns: 10, ..HevcAssemblyLimits::default() },
        "owner", DigestPolicy::default())?;
    ...
    drain(&mut client, 2)?;
    feed(&mut client, &response(2, &[("Content-Type", "application/sdp")], SDP), 3)?;
    client.request_digest(C::Setup, &credentials, [3; 16], 4)?;
    feed(&mut client, &response(3, &[("Session", "private-session"),
        ("Transport", "RTP/AVP/TCP;unicast;interleaved=0-1;ssrc=00000007")], &[]), 5)?;
    client.request_digest(C::Play, &credentials, [4; 16], 6)?;
    feed(&mut client, &response(4, &[("Session", "private-session")], &[]), 7)?;
    prime(&mut client)?;
    feed(&mut client, &frame(4, 100, FIRST, false), 20)?;
    ...
    assert!(matches!(&wait, P::Client { event, .. } if matches!(event.as_ref(), Raw::AuthenticationRequired { .. })));
    ...
    let retired = client.cancel();
    assert_eq!(retired.client.challenge.ok_or("challenge lost at cancellation")?.expose_wire(), held);
...
fn malformed_suffix_does_not_drop_source_or_flush_the_pending_picture_as_success() -> TestResult {
    ...
    assert!(pictures(&output).is_empty());
    assert!(output.iter().any(|e| matches!(e, P::Source { source, .. } if source.sequence() == 5)));
...
fn fatal_request_retirement_and_source_restart_close_all_picture_work() -> TestResult {
    let mut client = normal()?;
    feed(&mut client, &frame(4, 100, FIRST, false), 20)?;
    let failure = client.request(C::KeepAlive, 60_000_000_004).err().ok_or("expired session accepted request")?;
    assert_eq!(failure.reason, E::Client(HevcClientError::Session(ClientError::SessionExpired)));
    assert_eq!(failure.retirement.ok_or("request failure lost work")?.work.picture.ok_or("missing picture")?.nals, 1);
    assert_eq!(client.retained_nal_bytes(), 0);
    let mut client = normal()?;
    feed(&mut client, &frame(4, 100, FIRST, false), 20)?;
...
fn clock_reversal_and_invalid_constructor_never_reset_a_live_picture() -> TestResult {
    assert!(RtspHevcPictureClient::new(config(), KEY, ReorderLimits::default(), H265Limits::default(),
        HevcAssemblyLimits { max_nals: 0, ..HevcAssemblyLimits::default() }).is_err());
    let mut client = normal()?;
    feed(&mut client, &frame(4, 100, FIRST, false), 20)?;
    assert_eq!(client.poll(19).err(), Some(E::Client(HevcClientError::Session(ClientError::ClockReversed))));
...
fn tcp_partitioning_preserves_picture_bytes_source_order_and_boundary_classification() -> TestResult {
    let wires = [frame(4, 100, FIRST, true), frame(5, 100, CONT, true), frame(6, 200, FIRST, false)];
    let stream = wires.concat();
    for width in 1..=stream.len() {
        let mut client = normal()?;
        let mut events = Vec::new();
        for chunk in stream.chunks(width) { events.extend(feed(&mut client, chunk, 20)?); }
        let groups = pictures(&events);
        assert_eq!(groups.len(), 1);
        assert_eq!(groups[0].boundary(), HevcBoundary::NextFirstSlice);
        assert_eq!(groups[0].nals().iter().map(|n| n.bytes().to_vec()).collect::<Vec<_>>(), vec![FIRST.to_vec(), CONT.to_vec()]);
        let sources: Vec<_> = events.iter().filter_map(|e| match e {
            P::Source { source, .. } => Some(source.sequence()), _ => None,
        }).collect();
...
fn assembly_refusal_preserves_the_nal_and_allows_only_explicit_discontinuous_recovery() -> TestResult {
    ...
    assert!(output.iter().any(|e| matches!(e, P::Assembly { step: Step::Refused(refusal), retirement: None }
