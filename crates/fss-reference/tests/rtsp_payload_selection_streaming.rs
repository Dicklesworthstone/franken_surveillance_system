...
const SPS_NAL: &[u8] = &[
    0x67, 0x42, 0xc0, 0x0b, 0xda, 0x0a, 0x11, 0xb0, 0x11, 0x00, 0x00, 0x03, 0x00, 0x01, 0x00, 0x00,
    0x03, 0x00, 0x32, 0x0f, 0x14, 0x2a, 0xa0,
];
const SETUP: &str =
    ...
    for _ in 0..256 {
        ...
        match &event {
            DigestAvcPoll::Client { event, .. } => match event.as_ref() {
                ...
                AvcClientPoll::Fault { .. } | AvcClientPoll::Ended { .. } => {
                    return Err(
                        "unexpected terminal event while draining selected Digest client".into(),
                    );
                }
                _ => {}
            },
...
fn plain_playing(
    mut client: RtspAvcClient,
    body: &str,
    chunk_size: usize,
) -> Result<RtspAvcClient, Error> {
    ...
    assert!(
        setup
            .bytes()
            .starts_with(b"SETUP rtsp://camera.local/live/trackID=0 RTSP/1.0\r\n")
    );
...
fn exact_payload_reaches_real_receiver_for_both_offered_modes() -> TestResult {
    for payload_type in [96, 97] {
        for chunk_size in [1, 7, 4096] {
            let client = RtspAvcClient::new_with_payload_type(
                config(),
                KEY,
...
fn ambiguous_and_absent_selections_close_before_setup() -> TestResult {
    ...
    for mut client in clients {
        ...
        assert!(events.iter().any(|event| matches!(
            event,
            AvcClientPoll::Fault {
                reason: AvcClientError::Session(ClientError::Description),
                ..
            }
        )));
...
fn authenticated_selection_survives_fragmented_challenge_and_preserves_rtp() -> TestResult {
    for chunk_size in [1, 7, 4096] {
        ...
        digest_send(
            &mut client,
            &response(play.cseq(), "Session: fixture\r\n", ""),
            chunk_size,
            7,
        )?;
        assert_eq!(client.state(), ClientState::Playing);
...
fn selected_authenticated_client_retains_original_challenge_on_cancel() -> TestResult {
    ...
    assert_eq!(
        retirement
            .wire
            .challenge
            .ok_or("missing retained challenge")?
            .expose_wire(),
        wire
    );
...
fn payload_selection_cannot_reset_the_digest_retry_deadline() -> TestResult {
    ...
    let DigestAvcPoll::Client {
        event,
        wire_retirement: Some(_),
    } = event
    else {
...
fn constructors_preserve_scope_stream_and_payload_validation() {
    for payload_type in [128, 255] {
        assert!(
            RtspAvcClient::new_with_payload_type(
                config(),
                KEY,
                AvcReceiveLimits::default(),
                payload_type
        ...
        assert!(
            DigestAvcClient::new_with_payload_type(
                config(),
                KEY,
                AvcReceiveLimits::default(),
                payload_type,
