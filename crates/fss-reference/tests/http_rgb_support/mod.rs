...
impl Authority {
    pub fn deny_on(&self, operation: HttpCameraOperation, occurrence: usize) {
        self.seen.set(0);
        self.deny.set(Some((operation, occurrence)));
    }
}
...
impl Server {
    fn poll(&mut self) -> Test {
        ...
        if self.request.len() > 4096 {
            return Err("GET fixture bound".into());
        }
...
pub fn camera(response: Vec<u8>, read_bytes: usize) -> Test<(HttpCamera, Authority, Server)> {
    ...
    if response.len() > 65536 {
        return Err("loopback fixture wire bound".into());
    }
...
pub fn session<'m, 't>(
    ...
    let capture =
        HttpRgbCapture::attach(camera, processor).map_err(|_| "fresh attachment refused")?;
...
pub fn analyze<'cx>(
    ...
    a: &Authority,
    decoder: &mut DecodeBudget<'cx>,
    post: &mut RgbDetectionBudget,
    work: &mut WorkBudget<'cx>,
    linking: &mut WorkBudget<'cx>,
...
pub fn complete(c: &mut HttpRgbCapture<'_, '_>, a: &Authority, n: u8) -> Test<HttpRgbReceipt> {
    ...
    match step {
        ...
        _ => Err("RGB computation incomplete".into()),
    }
}
