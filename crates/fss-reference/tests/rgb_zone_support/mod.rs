...
pub fn model(rows: usize) -> Test<RgbInferenceModel> {
    ...
    let mut weights = vec![0.0; fields * 3 * 64];
    let mut biases = Vec::new();
    for row in 0..rows {
        for field in [row * 6, row * 6 + 2] {
            weights[field * 3 * 64 + 3 * 8 + 1] = 4.0;
        }
        biases.extend_from_slice(&[1.0, 3.0, 2.0, 4.0, 1.0, 0.9]);
    }
    Ok(RgbInferenceModel::new(
        &graph,
        BTreeMap::from([("weight".into(), weights), ("bias".into(), biases)]),
        RgbModelSpec {
...
pub fn head(model: &RgbInferenceModel) -> Test<RgbDetectionContract> {
    Ok(RgbDetectionContract::new(RgbDetectionSpec {
        ...
        labels: vec!["numeric-a".into(), "numeric-b".into()],
...
pub fn limits() -> RgbRunLimits {
    RgbRunLimits {
        ...
        preprocess: ExecBudget::new(WORK, 32 * 1024 * 1024),
        execution: ExecBudget::new(WORK, 64 * 1024 * 1024),
        maximum_output_bytes: 1024 * 1024,
    }
}
pub fn post() -> RgbDetectionBudget {
    RgbDetectionBudget::new(WORK, 32 * 1024 * 1024)
}
pub fn jpeg(value: u8) -> Vec<u8> {
    jpeg_support::jpeg(32, 16, [1, 1], false, false, false, &[[value, 128, 128]])
}
pub fn source(bytes: &[u8], mask: &[u8], n: u8) -> RgbSourceBinding {
    RgbSourceBinding {
        ...
        camera: 1,
        clock: 2,
        capture: [u64::from(n) * 1_000_000_000; 2],
        image_domain: [3; 32],
        calibration: [4; 32],
...
pub fn detection(
    ...
    let mut detector = RgbDetector::new(model, head)?;
...
pub fn admission(
    ...
    available: TrackingAvailability,
) -> Test<RgbFrameAdmission> {
    Ok(RgbFrameAdmission::new(
        source,
        available,
        ContentDigest::sha256(b"test owner screening declaration"),
    )?)
}
...
pub fn tracker(head: &RgbDetectionContract, policy: ImageTrackingPolicy) -> Test<RgbZoneTracker> {
    Ok(RgbZoneTracker::new(
        [9; 32],
        RgbTrackingContract::new(head, 0, ContentDigest::sha256(b"selected numeric-a"))?,
        policy,
        ImageZoneBasis {
            camera: 1,
            clock: 2,
...
pub fn observe(
    tracker: &mut RgbZoneTracker,
    run: &RgbDetectionRun,
    available: TrackingAvailability,
) -> Test<RgbZoneProgress> {
    Ok(tracker.observe(
        run.inference(),
        run.report(),
        admission(run.report().source(), available)?,
        &mut WorkBudget::new(WORK),
    )?)
}
