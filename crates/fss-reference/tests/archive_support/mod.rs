...
use fss_reference::rtsp::recording_catalog::local::{CatalogProgress, CatalogPublication};
...
pub fn namespace() -> Result<ArchiveNamespace, Error> {
    Ok(ArchiveNamespace::new(CatalogScope {
        ...
        decode_clock: ContentDigest::try_sha256(b"archive-fixture-decode")?,
        time_scale: 90_000,
    })?)
}
...
pub fn owner(name: &str) -> Result<(std::path::PathBuf, LocalRootPublisher), Error> {
    ...
    if path.exists() {
        return Err("test directory already exists; refusing to replace retained data".into());
    }
    Ok((
        path.clone(),
        LocalRootPublisher::open(path, owner_limits())?,
    ))
}
pub fn window(seq: u64, dts: u64) -> Result<PreparedRecording, Error> {
    ...
    sample.source(&mut c, 1)?;
    fixture::accepted(c.push_picture(sample.timed(dts), 1))?;
    c.seal(2)?;
    c.take_ready()
        .ok_or_else(|| "fixture produced no recording".into())
}
...
pub fn publish_page(
    ...
    let selected: Vec<_> = windows
        .iter()
        .map(|(slot, recording)| CatalogWindow { slot, recording })
        .collect();
    let c = prepare_catalog(ns.scope().clone(), &selected)?;
    let mut job = CatalogPublication::new(&c, p, ns.page_slot(first)?, c.byte_len(), 100)?;
    for now in 0..windows.len() + 1 {
        let _ = job.step(now as u64, &NeverCancel)?;
    }
    assert!(matches!(
        job.step(windows.len() as u64 + 1, &NeverCancel)?,
