...
use fss_reference::rtsp::recording_archive::{ArchiveLimits, ArchiveNamespace, ArchiveSnapshot, ArchiveWriteProgress};
...
fn fresh(name: &str) -> Test<PathBuf> {
    let path = Path::new(env!("CARGO_TARGET_TMPDIR")).join("archive_pin_pipeline").join(name);
...
fn namespace() -> Test<ArchiveNamespace> {
    Ok(ArchiveNamespace::new(CatalogScope { recording: scope()?,
        decode_clock: ContentDigest::sha256(b"explicit decode clock"), time_scale: 90_000 })?)
}
fn pin_scope() -> Test<ArchivePinScope> {
    Ok(ArchivePinScope { journal_id: ContentDigest::sha256(b"independently accepted pin epoch"),
...
fn window(at: u64) -> Test<PreparedRecording> {
    ...
    let pictures: Vec<_> = f.pictures.iter().enumerate().map(|(i, picture)| TimedAvcPicture {
        picture, decode_time: at + i as u64 * 3600, duration: 3600, composition_offset: 0 }).collect();
    let packets: Vec<_> = f.packets.iter().map(|(sequence, received_ns, bytes)| RecordingPacket {
        sequence: *sequence, received_ns: *received_ns, bytes }).collect();
    Ok(prepare_recording(scope()?, 90_000, &pictures, &packets)?)
}
fn writer<'a,'p>(publisher: &'a mut LocalRootPublisher, pins: &'p mut ArchivePinJournal) -> Test<JournaledArchiveWriter<'a,'p>> {
    Ok(JournaledArchiveWriter::open(publisher, pins, namespace()?, limits(), ArchiveWorkLimits::default(), 256, 0, 1000, &NeverCancel)?)
}
fn offer(w: &mut JournaledArchiveWriter<'_,'_>) -> Test<ContentDigest> {
    ...
    w.offer(window, bytes, 0)?; Ok(root)
}
fn finish(w: &mut JournaledArchiveWriter<'_,'_>) -> Test<(usize,usize)> {
    w.finish(1)?;
    for _ in 0..64 {
        if let JournaledArchiveProgress::Archive(ArchiveWriteProgress::Finished { windows, pages, .. }) = w.step(1, &NeverCancel)? {
...
fn publish_original(p: &mut LocalRootPublisher, window: &PreparedRecording) -> Test {
    let mut job = RecordingPublication::new(window, p, namespace()?.window_slot(0)?, window.byte_len(), 1000)?;
    for _ in 0..5 { if matches!(job.step(0, &NeverCancel)?, RecordingProgress::Published(_)) { return Ok(()); } }
...
fn both_window_and_catalog_automatically_journal_before_normal_publication() -> Test {
    ...
    let (anchor, expected_root, source) = {
        ...
        let mut w = writer(&mut p, &mut pins)?; let root = offer(&mut w)?;
        let source = w.pending().ok_or("pending")?.objects().source.to_vec();
        assert!(matches!(w.step(0, &NeverCancel)?, JournaledArchiveProgress::PinPersisted { receipt, .. }
            if receipt.phase() == ArchivePinPhase::Candidate && receipt.anchor().sequence == 2));
        assert_eq!(w.snapshot().windows().len(), 0);
        assert!(matches!(w.step(0, &NeverCancel)?, JournaledArchiveProgress::WorkConfirmed { pin_receipt, .. }
            if pin_receipt.phase() == ArchivePinPhase::Confirmed && pin_receipt.anchor().sequence == 3));
        assert_eq!(w.snapshot().windows().len(), 0);
        assert!(matches!(w.step(0, &NeverCancel)?, JournaledArchiveProgress::Archive(ArchiveWriteProgress::WindowDurable { ordinal: 0, .. })));
        assert_eq!(finish(&mut w)?, (1,1));
        assert_eq!(w.journal_anchor().sequence, 5);
        let retired = w.retire(); assert!(retired.unrecorded_confirmation.is_none());
    ...
    let mut pins = ArchivePinJournal::open_existing(path.join("pins"), pin_scope()?, Some(anchor),
    ...
    assert_eq!(snapshot.windows()[0].root(), expected_root); assert_eq!(snapshot.indexed_windows(), 1);
...
fn confirmed_auxiliary_work_cannot_be_forgotten_on_new_writer_startup() -> Test {
    ...
    let anchor = {
        ...
        let _ = w.step(0, &NeverCancel)?; let _ = w.step(0, &NeverCancel)?;
        assert!(w.pin_state().candidate().is_none()); assert_eq!(w.snapshot().windows().len(), 0);
        w.journal_anchor()
    }; // Entire process state lost; source work and independent reference already committed.
    let mut p = LocalRootPublisher::open(path.join("media"), storage())?;
    let mut pins = ArchivePinJournal::open_existing(path.join("pins"), pin_scope()?, Some(anchor),
        ArchivePinLimits::default(), IncompleteTailPolicy::Reject, &NeverCancel)?;
    assert!(matches!(pins.require_settled(&p, ArchiveWorkLimits::default(), &NeverCancel), Err(ArchivePinError::Unsettled)));
    let work = pins.load_work(ArchivePinPhase::Confirmed, &p, ArchiveWorkLimits::default(), &NeverCancel)?;
    publish_original(&mut p, work.pending.as_ref().ok_or("recovery lost original")?)?;
    pins.require_settled(&p, ArchiveWorkLimits::default(), &NeverCancel)?;
...
fn candidate_without_committed_source_remains_an_explicit_recovery_obligation() -> Test {
    let path = fresh("uncommitted_candidate")?;
    {
        let mut p = LocalRootPublisher::open(path.join("media"), storage())?;
        let mut pins = ArchivePinJournal::create(path.join("pins"), pin_scope()?, ArchivePinLimits::default(), &NeverCancel)?;
        let mut w = writer(&mut p, &mut pins)?; offer(&mut w)?; let _ = w.step(0, &NeverCancel)?;
    }
    let mut p = LocalRootPublisher::open(path.join("media"), storage())?;
    let mut pins = ArchivePinJournal::open_existing(path.join("pins"), pin_scope()?, None,
        ArchivePinLimits::default(), IncompleteTailPolicy::Reject, &NeverCancel)?;
    assert!(pins.state().candidate().is_some());
    assert!(pins.load_work(ArchivePinPhase::Candidate, &p, ArchiveWorkLimits::default(), &NeverCancel).is_err());
    assert!(writer(&mut p, &mut pins).is_err());
    assert!(p.visible_roots().next().is_none()); Ok(())
}
#[test]
fn confirmation_capacity_failure_preserves_work_and_blocks_normal_window_publication() -> Test {
    let path = fresh("confirm_limit")?;
    {
        ...
        assert!(matches!(w.step(0, &NeverCancel), Err(ArchivePinError::Limit)));
        assert!(matches!(w.step(0, &NeverCancel), Err(ArchivePinError::Fenced)));
        assert_eq!(w.pending().ok_or("original lost")?.manifest().root(), root);
        let r = w.retire(); assert!(r.unrecorded_confirmation.is_some());
...
fn corrupt_pin_tip_fences_before_source_checkpoint_and_returns_original_input() -> Test {
    ...
    let mut pins = ArchivePinJournal::create(path.join("pins"), pin_scope()?, ArchivePinLimits::default(), &NeverCancel)?;
    let mut w = writer(&mut p, &mut pins)?; let root = offer(&mut w)?;
    let file = path.join("pins/pins.journal"); let mut data = std::fs::read(&file)?;
    let last = data.last_mut().ok_or("empty journal")?; *last ^= 1; std::fs::write(file, data)?;
    assert!(w.step(0, &NeverCancel).is_err());
    let retired = w.retire();
    assert_eq!(retired.writer.archive.pending.ok_or("lost source")?.manifest().root(), root);
    assert!(p.visible_roots().next().is_none()); Ok(())
}
#[test]
fn wrong_journal_namespace_refuses_without_touching_either_store() -> Test {
    ...
    assert!(writer(&mut p, &mut pins).is_err());
    assert_eq!(pins.anchor(), anchor); assert!(p.visible_roots().next().is_none()); Ok(())
}
#[test]
fn clock_reversal_is_safe_but_expiry_never_bypasses_a_persisted_candidate() -> Test {
    ...
    let mut p = LocalRootPublisher::open(path.join("media"), storage())?;
    let mut pins = ArchivePinJournal::create(path.join("pins"), pin_scope()?, ArchivePinLimits::default(), &NeverCancel)?;
    let mut w = writer(&mut p, &mut pins)?; offer(&mut w)?; let _ = w.step(10, &NeverCancel)?;
    let anchor = w.journal_anchor();
    assert!(matches!(w.step(9, &NeverCancel), Err(ArchivePinError::Archive(fss_reference::rtsp::recording_archive::ArchiveError::ClockReversed))));
    assert_eq!(w.journal_anchor(), anchor);
    assert!(w.step(1000, &NeverCancel).is_err());
    let r = w.retire(); assert!(r.pins.candidate().is_some()); assert!(r.writer.archive.pending.is_some());
    assert!(p.visible_roots().next().is_none()); Ok(())
}
#[test]
fn cancellation_does_not_unlock_work_or_erase_accepted_source() -> Test {
    ...
    let mut pins = ArchivePinJournal::create(path.join("pins"), pin_scope()?, ArchivePinLimits::default(), &NeverCancel)?;
    let mut w = writer(&mut p, &mut pins)?; let root = offer(&mut w)?;
    assert!(matches!(w.step(0, &Cancel), Err(ArchivePinError::Cancelled)));
    let retired = w.retire();
    assert_eq!(retired.writer.archive.pending.ok_or("lost source")?.manifest().root(), root);
    assert!(p.visible_roots().next().is_none()); Ok(())
}
