    ...
    fn new() -> Test<Self> {
        for attempt in 0..64 {
            let path = std::env::temp_dir()
                .join(format!("fss-http-replay-{}-{attempt}", std::process::id()));
...
fn to_frame(r: &mut HttpWireReplay<'_>, p: &LocalRootPublisher) -> Test {
    for _ in 0..100_000 {
        match next(r, p)? {
            ...
            HttpReplayStep::Complete | HttpReplayStep::PrefixExhausted => {
                return Err("frame missing".into());
            }
            _ => {}
        }
    }
...
fn key(f: &HttpJpegFrame) -> Test<FrameKey> {
    ...
    Ok(FrameKey {
        ...
        exposure: http_rgb_exposure(f, &mut work())?,
        spans: f.source_spans().to_vec(),
    })
}
struct Authority {
...
impl HttpCameraAuthority for Authority {
    fn checkpoint(
        ...
        if now >= deadline
            || self.started.elapsed().as_nanos() + u128::from(NOW) >= u128::from(deadline)
        {
...
fn acquire(
    ...
    wire: Vec<u8>,
    read_bytes: usize,
) -> Test<(HttpWirePin, Vec<FrameKey>)> {
    if wire.len() > 65536 {
        return Err("loopback response bound".into());
    }
    ...
    for _ in 0..100_000 {
        ...
        match c.step(NOW, &auth, &mut b) {
            ...
            | Err(
                HttpCameraError::Http(_)
                | HttpCameraError::Multipart(_)
                | HttpCameraError::TrailingResponse,
            ) => return Ok((a.pin(), frames)),
...
fn cold_replay_rechunks_without_changing_native_frames_or_exposure_identity() -> Test {
    for chunked in [false, true] {
        ...
        assert_eq!(pin.bytes, wire.len() as u64);
            ...
            for _ in 0..100_000 {
                match r.step(access(&p, &NeverCancel, &mut w, &mut b))? {
                    HttpReplayStep::FrameReady => {
                        ...
                        for _ in 0..3 {
                            assert_eq!(next(&mut r, &p)?, HttpReplayStep::FrameReady);
                        }
                    ...
                    HttpReplayStep::PrefixExhausted => {
                        return Err("self-delimited recording became partial".into());
                    }
                    _ => {}
                }
            }
...
fn archive_exhaustion_does_not_fabricate_source_eof_for_close_or_truncated_inputs() -> Test {
    let mut truncated = response(false, false);
    ...
    for wire in [response(false, true), truncated] {
        ...
        assert!(retired.prefix_exhausted);
        assert!(retired.complete.is_none());
        assert!(retired.multipart.is_some());
    }
    Ok(())
}
#[test]
...
fn authority_and_work_refusal_do_not_advance_or_poison_a_fresh_cursor() -> Test {
    ...
    let mut r = HttpWireReplay::new(&a, pin, HttpReplayLimits::default())?;
    assert_eq!(
        r.step(access(&p, &Stop, &mut work(), &mut framing())),
...
fn parser_budget_failure_keeps_consumed_state_and_cannot_be_retried_as_fresh_input() -> Test {
    ...
    assert_eq!(next(&mut r, &p), Err(error));
    assert_eq!(r.position(), position);
    assert!(
        !r.retire()
            .wire
            .ok_or("raw buffer dropped on parse refusal")?
            .bytes
            .is_empty()
    );
    Ok(())
...
fn corrupt_originals_and_wrong_keys_do_not_release_a_complete_frame() -> Test {
    ...
    assert!(matches!(
        ...
        Err(HttpReplayError::FrameMismatch)
    ));
    assert_eq!(
        r.take_frame(
            receipt.ordinal,
            receipt.encoded_sha256,
            access(&p, &Stop, &mut work(), &mut framing())
        )
        .err(),
        Some(HttpReplayError::Cancelled)
    );
    let (_, raw) = a
        .reads()
        .find(|(_, wire)| wire.range[0] <= span.wire_range[0] && span.wire_range[0] < wire.range[1])
    ...
    assert!(matches!(
        ...
        Err(HttpReplayError::Archive(_))
    ));
    assert_eq!(r.position(), before);
    assert_eq!(
        r.pending_frame().ok_or("held frame lost")?.part().bytes(),
        JPEG
    );
    Ok(())
}
...
fn frame_ceiling_preserves_partial_evidence_instead_of_reporting_completion() -> Test {
    ...
    assert!(r.completion().is_none());
    assert_eq!(r.position().transferred_frames, 1);
...
fn trailing_response_bytes_are_rejected_even_across_replay_read_boundaries() -> Test {
    ...
    for read_bytes in [1, 17, 65536] {
        ...
        for _ in 0..1000 {
            match next(&mut r, &p) {
                Err(HttpReplayError::TrailingResponse) => {
                    refused = true;
                    break;
                }
                Ok(
                ...
                other => {
                    return Err(
                        format!("unexpected trailing-source classification: {other:?}").into(),
                    );
                }
            }
        }
...
fn late_read_cancellation_keeps_loaded_bytes_and_resumes_without_duplicate_read() -> Test {
    ...
    assert!(matches!(
        baseline.step(access(&p, &count, &mut work(), &mut framing()))?,
        HttpReplayStep::WireLoaded { .. }
    ));
    let mut r = HttpWireReplay::new(&a, pin, HttpReplayLimits::default())?;
    next(&mut r, &p)?;
    let cancel = Probe {
        seen: Cell::new(0),
        stop: Some(count.seen.get()),
    };
    assert_eq!(
        r.step(access(&p, &cancel, &mut work(), &mut framing())),
        Err(HttpReplayError::Cancelled)
    );
    let loaded = r.position().loaded_bytes;
    assert!(loaded > 0);
    assert!(r.pending_wire().is_some());
    assert!(r.failure().is_none());
