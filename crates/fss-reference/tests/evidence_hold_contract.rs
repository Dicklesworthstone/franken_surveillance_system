...
use fss_core::{
    BatchId, BudgetVector, CaptureInterval, ContentDigest, EvidenceDelta, ObjectId,
    OperationId, Plane, SensorId, StreamId, TimestampNs,
};
use fss_reference::deletion::holds::{
    CAP_HOLD_COMMIT, CAP_HOLD_PREPARE, HOLD_OBJECT_PREFIX, HoldError, HoldOutcome,
    HoldRecord, HoldRequest, HoldState, STAGE_HOLD_COMMITTED, STAGE_HOLD_STAGED,
    commit_hold, list_holds, preview_hold,
};
use fss_reference::deletion::{
    CommitOutcome, DeletionError, STAGE_DELETION_RECORD_APPENDED,
    ...
    fn new(name: &str) -> TestResult<Self> {
        for attempt in 0..100 {
            let root = std::env::temp_dir().join(format!(
                "fss-evidence-hold-{name}-{}-{attempt}", std::process::id()
            ));
...
impl Fixture {
    ...
    fn apply(&mut self, request: &HoldRequest) -> TestResult<HoldRecord> {
        ...
        Ok(commit_hold(&mut self.deployment, request, preview.record.approval(),
            &self.authority, &self.cx)?.record)
    }
...
fn request(id: &str, import_identity: ContentDigest, state: HoldState) -> HoldRequest {
    HoldRequest {
        hold_id: id.into(), import_identity, state,
        reason: if state == HoldState::Held { "Preserve incident evidence" } else { "Owner review complete" }.into(),
    }
}
...
fn hold_survives_reopen_blocks_deletion_and_release_keeps_audit_history() -> TestResult {
    ...
    let head = f.deployment.current_anchor().clone();
    let again = commit_hold(&mut f.deployment, &req, placed.approval(), &f.authority, &f.cx)?;
    assert_eq!(again.outcome, HoldOutcome::AlreadyCurrent);
    assert_eq!(*f.deployment.current_anchor(), head);

    let Fixture { deployment, cx, authority, directory } = f;
    let root = deployment.root().to_path_buf();
    drop(deployment);
    let mut deployment = ReferenceDeployment::reopen(&root, SITE, &cx)?;
    assert_eq!(list_holds(&deployment, &authority, &cx)?, vec![placed.clone()]);
    let blocked = plan_deletion(&deployment, import, &cx)?;
    assert!(blocked.blockers.iter().any(|b| b.kind == "evidence_hold" && b.subject == "incident"));
    let result = commit_deletion(&mut deployment, blocked.digest()?, blocked.approval_digest(PRINCIPAL)?, PRINCIPAL, &cx);
    assert!(matches!(result, Err(DeletionError::Blocked(ref b)) if b.iter().any(|b| b.kind == "evidence_hold")));
    ...
    let released = commit_hold(&mut deployment, &release, preview.record.approval(), &authority, &cx)?;
    ...
    for digest in [placed.digest(), released.record.digest()] {
        assert!(plan.retained.iter().any(|o| o.digest == digest && o.reason == "authority_history"));
        assert!(!plan.deletable.iter().any(|o| o.digest == digest));
    }
    assert!(plan.tombstones.iter().all(|t| !t.object_id.starts_with(HOLD_OBJECT_PREFIX)));
    let receipt = commit_deletion(&mut deployment, plan.digest()?, plan.approval_digest(PRINCIPAL)?, PRINCIPAL, &cx)?;
    assert_eq!(receipt.outcome, CommitOutcome::Completed);
    assert_eq!(list_holds(&deployment, &authority, &cx)?, vec![released.record.clone()]);
    assert_eq!(deployment.publisher().spool().read(placed.digest())?, placed.to_bytes());
    assert!(matches!(preview_hold(&deployment, &req, &authority, &cx), Err(HoldError::ReleasedIdentifier)));
    assert!(preview_hold(&deployment, &request("new-hold", import, HoldState::Held), &authority, &cx).is_err());
    drop(deployment);
    drop(directory);
    Ok(())
}
...
fn stale_hold_and_deletion_approvals_never_write() -> TestResult {
    ...
    let head = f.deployment.current_anchor().clone();
    let objects = f.deployment.publisher().spool().digests().count();
    assert!(matches!(commit_hold(&mut f.deployment, &req, preview.record.approval(), &f.authority, &f.cx), Err(HoldError::StaleApproval)));
    assert!(matches!(commit_deletion(&mut f.deployment, old_deletion.digest()?, old_deletion.approval_digest(PRINCIPAL)?, PRINCIPAL, &f.cx), Err(DeletionError::StalePlan(_))));
    assert_eq!(*f.deployment.current_anchor(), head);
    assert_eq!(f.deployment.publisher().spool().digests().count(), objects);
    Ok(())
}
...
fn hold_protects_later_shared_derivatives_but_not_unrelated_imports() -> TestResult {
    ...
    assert!(plan_deletion(&f.deployment, north, &f.cx)?.blockers.is_empty());
    let mut bytes = north.bytes().to_vec();
    bytes.extend_from_slice(&south.bytes());
    let digest = f.deployment.stage_payload(&bytes)?;
    f.deployment.append_batch(BatchId::parse("batch:shared-hold-evidence")?, vec![EvidenceDelta {
        delta_id: "delta:shared-hold-evidence".into(), family: "test_shared_evidence".into(),
        object_id: ObjectId::parse("object:shared-hold-evidence")?,
        prior_generation: None, new_generation: 1,
        validity: CaptureInterval::new(TimestampNs(0), TimestampNs(0))?,
        plane: Plane::Cognition, payload_digest: digest, witness_digest: None, operation_id: None,
    }], vec![digest], &f.cx)?;
    let plan = plan_deletion(&f.deployment, north, &f.cx)?;
    assert!(plan.deletable.iter().any(|o| o.digest == digest));
    assert!(plan.blockers.iter().any(|b| b.kind == "evidence_hold" && b.subject == "incident"));
    assert!(plan_deletion(&f.deployment, third, &f.cx)?.blockers.is_empty());
...
fn overlapping_holds_release_independently() -> TestResult {
    ...
    let blockers: Vec<_> = plan.blockers.iter().filter(|b| b.kind == "evidence_hold").map(|b| b.subject.as_str()).collect();
...
fn capabilities_scope_and_raw_writer_cannot_bypass_hold_authority() -> TestResult {
    ...
    assert!(matches!(commit_hold(&mut f.deployment, &req, preview.record.approval(), &preview_only, &f.cx), Err(HoldError::Unauthorized)));
    let mut foreign = f.authority.clone();
    foreign.anchor_universe = ContentDigest::sha256(b"site:foreign");
    assert!(matches!(preview_hold(&f.deployment, &req, &foreign, &f.cx), Err(HoldError::Unauthorized)));
    let digest = ContentDigest::sha256(b"not staged");
    let head = f.deployment.current_anchor().clone();
    for (family, object) in [(FAMILY_EVIDENCE_HOLD, "object:any".to_owned()), ("other_family", format!("{HOLD_OBJECT_PREFIX}shadow"))] {
        let result = f.deployment.append_batch(BatchId::parse("batch:forged-hold")?, vec![EvidenceDelta {
            delta_id: "delta:forged-hold".into(), family: family.into(),
            object_id: ObjectId::parse(object)?, prior_generation: None, new_generation: 1,
            validity: CaptureInterval::new(TimestampNs(0), TimestampNs(0))?,
            plane: Plane::Authority, payload_digest: digest, witness_digest: None, operation_id: None,
        }], vec![digest], &f.cx);
        assert!(matches!(result, Err(ReferenceError::ReservedDeltaFamily { .. })));
    }
    assert_eq!(*f.deployment.current_anchor(), head);
    Ok(())
}
...
fn staged_cancellation_does_not_create_a_hold_and_exact_retry_commits_once() -> TestResult {
    ...
    assert!(matches!(commit_hold(&mut f.deployment, &req, preview.record.approval(), &f.authority, &f.cx), Err(HoldError::Cancelled)));
    assert_eq!(*f.deployment.current_anchor(), head);
    ...
    let committed = commit_hold(&mut f.deployment, &req, preview.record.approval(), &f.authority, &fresh)?;
    assert_eq!(committed.outcome, HoldOutcome::Committed);
    assert!(fresh.is_drain_completed());
    let head = f.deployment.current_anchor().clone();
    let fresh = f.fresh_cx()?;
    let again = commit_hold(&mut f.deployment, &req, preview.record.approval(), &f.authority, &fresh)?;
    assert_eq!(again.outcome, HoldOutcome::AlreadyCurrent);
    assert_eq!(*f.deployment.current_anchor(), head);
    Ok(())
}
...
fn incomplete_deletion_refuses_new_preservation_then_resumes_exactly_once() -> TestResult {
    ...
    assert!(matches!(commit_deletion(&mut f.deployment, digest, approval, PRINCIPAL, &f.cx), Err(DeletionError::Cancelled { stage: STAGE_DELETION_RECORD_APPENDED })));
    let fresh = f.fresh_cx()?;
    let head = f.deployment.current_anchor().clone();
    assert!(matches!(preview_hold(&f.deployment, &request("new", south, HoldState::Held), &f.authority, &fresh), Err(HoldError::DeletionInProgress)));
    assert_eq!(*f.deployment.current_anchor(), head);
    assert_eq!(commit_deletion(&mut f.deployment, digest, approval, PRINCIPAL, &fresh)?.outcome, CommitOutcome::Resumed);
    assert_eq!(commit_deletion(&mut f.deployment, digest, approval, PRINCIPAL, &fresh)?.outcome, CommitOutcome::AlreadyComplete);
    Ok(())
}
