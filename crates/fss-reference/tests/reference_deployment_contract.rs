...
use fss_core::{
    BatchId, CaptureInterval, ContentDigest, EvidenceDelta, ObjectId, OperationId, Plane,
    TimestampNs,
};
use fss_ledger::{DurableLedgerError, doctor_path};
...
fn open_and_reopen_empty_and_existing_root() -> Result<(), Box<dyn Error>> {
    ...
    let manifest = ObjectManifest::new("slot-sample", staged.manifest.children.clone(), None)?;
...
fn not_a_deployment_on_non_empty_root_without_layout() -> Result<(), Box<dyn Error>> {
    ...
    let entries: Vec<_> = fs::read_dir(&dir)?
        ...
        .collect();
    assert_eq!(entries, vec!["foreign.txt"]);
    ...
    let entries_after: Vec<_> = fs::read_dir(&dir)?
        .map(|e| e.unwrap().file_name().into_string().unwrap())
        .collect();
    assert_eq!(entries_after, vec!["foreign.txt"]);
...
fn append_batch_idempotency_and_conflict() -> Result<(), Box<dyn Error>> {
    ...
    assert!(matches!(
        ...
        Err(ReferenceError::DurableLedger(ref boxed))
            if matches!(**boxed, DurableLedgerError::BatchIdConflict { .. })
    ));
...
fn simulated_alert_dispatch_routes_through_deployment_provider() -> Result<(), Box<dyn Error>> {
    ...
    req_enc.text("fss.reference_alert_request.v1");
    req_enc.digest(event_root);
    req_enc.digest(event_revision_digest);
    req_enc.text(&channel);
    let request_digest = ContentDigest::sha256(&req_enc.finish());

    let plan = ReferenceAlertPlan {
        intent: fss_core::EffectIntent {
            operation_id: OperationId::parse("op:alert:001")?,
            idempotency_key: fss_core::IdempotencyKey::parse("idemp:alert:001")?,
    ...
    // Prepare intent in effect journal.
    let t_prepare = TimestampNs(1_700_000_000);
    let t_commit = TimestampNs(1_700_000_001);
    let t_outcome = TimestampNs(1_700_000_002);

    dep.effects_mut().prepare(
        plan.intent.clone(),
...
#[test]
fn exported_constants_and_tables_integrity() {
    ...
    for point in [
        ...
        match err {
            ...
            other => panic!("expected CancellationRequested, got {other:?}"),
        }
    }
}
