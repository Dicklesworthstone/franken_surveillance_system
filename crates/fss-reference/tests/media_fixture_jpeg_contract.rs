...
fn test_brown_luma_float_idct_simulation() -> Result<(), Box<dyn Error>> {
    ...
    // q100 simulation
    let (recon_q100, psnr_q100, max_err_q100) = simulate_float_idct_gray(96, 96, ref_bytes, 100)?;
    assert!(
        psnr_q100 >= 58.98 && psnr_q100 <= 59.08,
        "q100 PSNR must be within [58.98, 59.08] dB, got {psnr_q100:.4}"
    );
    assert_eq!(
        max_err_q100, 1,
        "q100 max error must be exactly 1, got {max_err_q100}"
    );
    let diff_q100 = recon_q100
        .iter()
        .zip(ref_bytes.iter())
        .filter(|(a, b)| a != b)
        .count();
    assert!(
        (700..=800).contains(&diff_q100),
        "q100 differing pixels must be within [700, 800], got {diff_q100}"
    );

    // qfix (quality 90) simulation
    let (_recon_qfix, psnr_qfix, max_err_qfix) = simulate_float_idct_gray(96, 96, ref_bytes, 90)?;
    assert!(
        psnr_qfix >= 36.52 && psnr_qfix <= 36.62,
        "qfix PSNR must be within [36.52, 36.62] dB, got {psnr_qfix:.4}"
    );
    assert_eq!(
        max_err_qfix, 16,
        "qfix max error must be exactly 16, got {max_err_qfix}"
    );
