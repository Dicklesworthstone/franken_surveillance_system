...
fn test_restart_markers() -> Result<(), Box<dyn Error>> {
    ...
    // 12 % 5 == 2 != 0, ensuring the restart interval does NOT divide the MCU count.
    assert_ne!(12 % 5, 0, "Restart interval must not divide MCU count");
    // Check DRI
    ...
    assert_eq!(
        restart_markers,
        vec![0xD0, 0xD1],
        "Expected restart markers RST0, RST1 in sequence for interval 5 and 12 MCUs"
    );
    Ok(())
}
    ...
    for m in &mjpegs {
        match m.variant.as_str() {
            ...
            "truncated_last" => {
                ...
                let eoi_count = count_subsequences(&m.file_bytes, &[0xFF, 0xD9]);
                assert_eq!(soi_count, 3);
                assert_eq!(sos_count, 3, "Truncated frame must still reach and include SOS header");
