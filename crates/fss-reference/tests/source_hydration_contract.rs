...
use fss_core::{
    ...
    HydrationRequestSpec, HydrationResponse, LaboratoryAccess, LedgerAnchor, ObjectId,
    SemanticHandle, SemanticHandleSpec, SessionId, TimestampNs, TombstoneReason, TombstoneRecord,
};
...
fn bind(fixture: &mut Fixture) -> TestResult {
    fixture.catalog.bind_source_object(
        &fixture.handle.handle_id,
        fixture.handle.descriptor_digest,
        fixture.root,
        &fixture.store,
    )?;
    Ok(())
...
fn exact_source_round_trip_has_custody_roots_and_no_payload_cache() -> TestResult {
    ...
    bind(&mut f)?;
    let binding = f.catalog.bind_source_object(
        &f.handle.handle_id,
    ...
    let response = f
        .catalog
        .hydrate_from_source(&req, &f.store, TimestampNs(20))?;
...
fn denied_privacy_capability_budget_and_future_time_do_not_read_source() -> TestResult {
    ...
    for case in 0..4 {
        ...
        assert!(
            f.catalog
                .hydrate_from_source(&req, &probe, TimestampNs(20))
                .is_err()
        );
    }
...
fn expiry_and_deleted_descriptor_return_typed_unavailability_without_io() -> TestResult {
    ...
    let expired = f
        .catalog
        .hydrate_from_source(&req, &probe, TimestampNs(100))?;
    ...
    assert!(matches!(
        f.catalog.hydrate_from_source(&req, &probe, TimestampNs(40)),
        Err(SourceHydrationError::Hydration(HydrationError::Contract(
            ContractError::StaleAnchor
        )))
    ));
    let unavailable = f.catalog.hydrate_from_source(
        &request(&deleted, HydrationLevel::H3)?,
        &probe,
        TimestampNs(40),
    )?;
    assert!(unavailable.artifact.is_none());
    assert_eq!(
        unavailable.receipt.availability,
...
fn source_continuation_uses_existing_single_use_ledger_and_survives_read_failure() -> TestResult {
    ...
    let first = f
        .catalog
        .hydrate(&request(&f.handle, HydrationLevel::H2)?, TimestampNs(20))?;
    let cursor = first
        .receipt
        .continuation
    ...
    assert!(matches!(
        f.catalog.hydrate_from_source(&req, &wrong, TimestampNs(22)),
    ...
    let delivered = f
        .catalog
        .hydrate_from_source(&req, &f.store, TimestampNs(22))?;
    delivered.validate_for(&req, &f.handle)?;
    assert!(
        f.catalog
            .issued_cursor(&cursor.cursor_digest)
            .ok_or("lost consumed cursor")?
            .consumed
    );
    let probe = ReaderProbe::new(SOURCE);
    assert!(matches!(
        f.catalog.hydrate_from_source(&req, &probe, TimestampNs(23)),
        Err(SourceHydrationError::Hydration(
...
fn unissued_continuation_and_cross_session_request_do_not_read_source() -> TestResult {
    ...
    let probe = ReaderProbe::new(SOURCE);
    assert!(matches!(
        unissued.hydrate_from_source(&req, &probe, TimestampNs(22)),
        Err(SourceHydrationError::Hydration(
            HydrationError::ContinuationUnissued
        ))
    ));
    ...
    assert!(
        f.catalog
            .hydrate_from_source(&req, &probe, TimestampNs(22))
            .is_err()
    );
    assert_eq!(probe.calls.get(), 0);
    Ok(())
...
fn tombstoned_source_cannot_be_disclosed_from_a_prior_success() -> TestResult {
    ...
    assert!(matches!(
        f.catalog
            .hydrate_from_source(&req, &f.store, TimestampNs(21)),
...
fn staged_or_unrelated_objects_are_not_source_publications() -> TestResult {
    ...
    let other_root = f
        .store
        .publish_manifest(ObjectManifest::new("other", [other], None)?)?
        .root;
    assert!(matches!(
        f.catalog.bind_source_object(
            &f.handle.handle_id,
            f.handle.descriptor_digest,
            other_root,
            &f.store
...
fn nested_published_roots_work_but_opaque_manifest_shaped_bytes_do_not_expand() -> TestResult {
    ...
    let nested_root = f
        .store
        .publish_manifest(ObjectManifest::new("parent", [f.root], None)?)?
        .root;
    f.catalog.bind_source_object(
        &f.handle.handle_id,
        f.handle.descriptor_digest,
    ...
    let outer = g
        .store
        .publish_manifest(ObjectManifest::new("opaque-parent", [opaque], None)?)?
        .root;
    assert!(matches!(
        g.catalog.bind_source_object(
            &g.handle.handle_id,
            g.handle.descriptor_digest,
            outer,
            &g.store
        ),
        Err(SourceHydrationError::NotReachable)
...
fn transformed_descriptors_and_conflicting_cache_registration_fail_closed() -> TestResult {
    ...
    let artifact = HydrationArtifact::publish(
        ...
        "text/plain",
        SOURCE.to_vec(),
        [f.handle.subject_digest, f.handle.descriptor_digest],
        Completeness::Complete,
        None,
    )?;
    assert!(
    ...
    let mut g = fixture()?;
    g.catalog
        .register_artifact(&g.handle.handle_id, g.handle.descriptor_digest, artifact)?;
    assert!(matches!(
        g.catalog.bind_source_object(
            &g.handle.handle_id,
            g.handle.descriptor_digest,
...
fn source_binding_cannot_be_retargeted_and_reader_cannot_exceed_quote() -> TestResult {
    ...
    assert!(matches!(
        f.catalog.hydrate_from_source(
            &request(&f.handle, HydrationLevel::H3)?,
            &oversized,
            TimestampNs(20)
        ),
        Err(SourceHydrationError::Hydration(
...
#[test]
fn consumer_rejects_substituted_bytes_even_with_self_consistent_receipt_and_genuine_roots()
-> TestResult {
    let mut f = fixture()?;
    bind(&mut f)?;
    let binding = f
...
fn consumer_does_not_confuse_a_preview_or_expired_receipt_with_source_evidence() -> TestResult {
    ...
    assert!(matches!(
        binding.validate_response(&req, &f.handle, &preview),
        Err(SourceHydrationError::Hydration(
    ...
    let expired = f
        .catalog
        .hydrate_from_source(&req, &f.store, TimestampNs(100))?;
    assert!(matches!(
        binding.validate_response(&req, &f.handle, &expired),
        Err(SourceHydrationError::Hydration(
            HydrationError::LevelUnavailable
        ))
    ));
...
fn consumer_rejects_source_rebinding_to_another_descriptor_revision() -> TestResult {
    ...
    let artifact = HydrationArtifact::publish(
        ...
        SOURCE_OBJECT_CONTENT_TYPE,
        SOURCE.to_vec(),
        [revised.subject_digest, revised.descriptor_digest, f.root],
        Completeness::Complete,
        None,
    )?;
    ...
    assert!(matches!(
        binding.validate_response(&req, &revised, &response),
        Err(SourceHydrationError::SourceMismatch)
    ));
    Ok(())
}
...
fn consumer_rejects_privacy_transform_claims_on_original_source() -> TestResult {
    ...
    let artifact = HydrationArtifact::publish(
        ...
        SOURCE.to_vec(),
        [
            transformed.subject_digest,
            transformed.descriptor_digest,
            f.root,
        ],
        Completeness::Complete,
        transformed.applied_transform.clone(),
    )?;
    let response = forged_response(&req, &transformed, artifact)?;
    response.validate_for(&req, &transformed)?;
    assert!(matches!(
        binding.validate_response(&req, &transformed, &response),
        Err(SourceHydrationError::TransformedSource)
    ));
    Ok(())
}
