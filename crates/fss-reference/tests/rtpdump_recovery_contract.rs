...
fn scope() -> Result<RtpImportScope, Error> {
    Ok(RtpImportScope {
        sensor: SensorId::parse("sensor:recovery")?,
        stream: StreamId::parse("stream:recovery")?,
...
) -> Result<RtpFileImportReceipt, Error> {
    Ok(publish_rtp_import(
        prepare_rtp_import(bytes, scope()?, config(), RtpImportLimits::default(), cx)?,
        cx,
        dep,
    )?)
}
...
fn root_only(
    ...
    dep.publish_and_commit(
        &slot,
        plan.manifest(),
        CaptureInterval::new(TimestampNs(0), scope()?.receive_time)?,
        cx,
    )?;
    Ok(root)
}
...
fn complete_import_survives_loss_of_receipt_and_original_path() -> TestResult {
    ...
    let anchor = receipt.anchor().clone();
    drop(receipt);
    drop(dep);
...
fn recovery_policy_cannot_be_widened_by_stored_recipe() -> TestResult {
    ...
    for policy in [
        ...
        assert!(matches!(
            inspect_rtp_import(root, 1, policy, &cx, &dep),
            Err(RtpImportError::Limit)
        ));
    }
    assert!(matches!(
        inspect_rtp_import(root, 0, RtpRecoveryPolicy::default(), &cx, &dep),
...
fn corruption_and_cancellation_cannot_produce_a_recovery_receipt() -> TestResult {
    ...
    let path = dep
        .publisher()
        .spool()
        .object_path(receipt.report().nals()[0].digest);
    ...
    assert!(matches!(
        inspect_rtp_import(root, 1, RtpRecoveryPolicy::default(), &cx, &dep),
