...
impl Budgets {
    pub fn new() -> Self {
        Self {
            copy: RgbEvidenceBudget::new(WORK),
            work: WorkBudget::new(100_000_000_000),
            import: ImportBudget::new(WORK),
            decoder: DecodeBudget::new(WORK),
            head: RgbDetectionBudget::new(WORK, 32 * 1024 * 1024),
            temporal: WorkBudget::new(WORK),
            linking: WorkBudget::new(WORK),
        }
    }
}
pub fn imported<'a>(
...
pub fn retention() -> ContentDigest {
    ContentDigest::sha256(b"explicit original/model custody test authority")
}
pub struct ArchiveAuthority {
    pub reads: Cell<bool>,
    pub writes: Cell<bool>,
}
impl ArchiveAuthority {
    ...
    pub fn access<'a>(&'a self, camera: &'a Authority) -> HttpRgbEvidenceAccess<'a> {
        HttpRgbEvidenceAccess {
            source: camera.access(&NeverCancel),
            archive: self,
        }
    }
}
...
pub fn next(
    r: &mut HttpRgbEvidenceRecording<'_, '_>,
    ...
    for _ in 0..50000 {
        ...
        match r.poll(a.access(&NeverCancel))? {
            HttpRgbRecordingStep::Advanced | HttpRgbRecordingStep::Pending => {
                std::thread::yield_now()
            }
            HttpRgbRecordingStep::WirePrepared(plan) => {
                r.commit_wire(plan, publisher, a.access(&NeverCancel))?
                    .acknowledgement()?;
            }
...
pub fn analyze(
    ...
    let context = crate::http_rgb_support::context(
        r.recording().capture(),
        &[1; 512],
        n,
    ...
    match r.analyze(
        ...
        fixture::limits().run,
        publisher,
        a.access(&NeverCancel),
        HttpRgbBudgets {
            decoder: &mut b.decoder,
            projection: &mut b.head,
            temporal: &mut b.temporal,
            linking: &mut b.linking,
        },
        &ScalarExecCx::new(),
    )? {
...
pub fn evidence(
    r: &HttpRgbEvidenceRecording<'_, '_>,
    ...
    let evidence = RgbEvidence::capture(
        imported,
        head,
        c.frame().ok_or("held source missing")?.part().bytes(),
        c.completed()
            .ok_or("held native output missing")?
            .detection_run(),
