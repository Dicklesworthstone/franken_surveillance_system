...
fn scope() -> Result<CatalogScope, Box<dyn std::error::Error>> {
    Ok(CatalogScope { recording: RecordingScope { sensor: SensorId::parse("catalog-wire-fixture")?,
        stream: StreamId::parse("main")?, generation: 1, anchor: ContentDigest::try_sha256(b"owner")?,
        receive_clock: ContentDigest::try_sha256(b"receive")? }, decode_clock: ContentDigest::try_sha256(b"decode")?, time_scale: 90000 })
}
#[test]
fn independent_metadata_bytes_pin_format_and_interval_meaning() -> TestResult {
    assert_eq!(ContentDigest::try_sha256(INDEX)?.to_text(), "sha256:ea8bb342222032cc881bffcc83bbc6a8c9cc4690263dd86a16cc961e0dff9048");
    assert_eq!(ContentDigest::try_sha256(ROOT)?.to_text(), "sha256:a848001ea49d905ba5d8e904ef2bbe74cfe42a2a6a401c25e4124cf90d4f2529");
...
fn rechecksummed_unknown_version_is_still_refused() -> TestResult {
    ...
    let children: Vec<_> = original.children().iter().copied().filter(|d| *d != meta).collect();
    let forged = ObjectManifest::new(CATALOG_KIND, children, Some(ContentDigest::try_sha256(&bytes)?))?;
    assert!(matches!(verify_catalog(&forged, &bytes, &scope()?), Err(CatalogError::Malformed)));
    Ok(())
}
