...
fn negotiate(parameters: (&str, &str), limits: AvcReceiveLimits) -> Result<RtspAvcClient, Error> {
    ...
    for (command, fields, body) in [
        ...
        println!(
            "{{\"kind\":\"request_prepared\",\"command\":\"{:?}\",\"cseq\":{},\"bytes\":{},\"sent_to_camera\":false}}",
            request.command(),
            request.cseq(),
            request.bytes().len()
        );
...
fn replay(
    ...
    for (index, nal) in nals.iter().enumerate() {
        let timestamp = *timestamps
            .get(frame)
            .ok_or("more fixture pictures than expected")?;
        feed(
            &mut client,
            name,
            index as u16 + 1,
    ...
    if counts.pictures != timestamps.len()
        || counts.sources != nals.len() + 1
        || counts.gaps != 0
        || client.retained_nal_bytes() != 0
        || client.buffered_wire_bytes() != 0
    {
        return Err("clean replay count or quiescence mismatch".into());
    }
    println!(
        "{{\"kind\":\"verified_replay_counts\",\"fixture\":\"{name}\",\"sources\":{},\"picture_groups\":{},\"retained_bytes\":0,\"rust_decoder\":false,\"live_camera_qualified\":false}}",
        counts.sources, counts.pictures
    );
    Ok(())
}
...
fn lost_fragment() -> Result<(), Error> {
    ...
    if counts.gaps != 1 || counts.pictures != 0 {
        return Err("loss was not classified".into());
    }
    feed(
        &mut client,
        "lost_fragment",
    ...
    println!(
        "{{\"kind\":\"cancelled_loss_replay\",\"gaps\":1,\"fragment_retired\":true,\"remote_session_may_exist\":{},\"retained_bytes\":0}}",
        retired.session.remote_session_may_exist
    );
    Ok(())
}

fn feed(
    client: &mut RtspAvcClient,
    name: &str,
    sequence: u16,
    timestamp: u32,
    nal: &[u8],
    ...
    for _ in 0..4_096 {
        match client.poll(now)? {
            AvcClientPoll::Control(progress) => {
                if progress == ClientProgress::KeepAliveDue {
                    return Err("unexpected fixture keepalive deadline".into());
                }
            ...
            AvcClientPoll::Rtp {
                ...
                counts.sources += 1;
                println!(
                    "{{\"kind\":\"source\",\"fixture\":\"{name}\",\"digest\":\"{digest}\",\"bytes\":{},\"disposition\":\"{:?}\"}}",
                    source.payload().len(),
                    admission.transport.transport.disposition
                );
            }
            AvcClientPoll::Media(AvcReceivePoll::Source {
                fragment,
                picture,
                gap_before,
                ..
            }) => {
                if !allow_loss && (fragment.is_some() || picture.is_some() || gap_before) {
                    return Err("unexpected clean source discontinuity".into());
                }
            }
            AvcClientPoll::Media(AvcReceivePoll::Assembly(AvcAssemblyStep::Accepted(out))) => {
                if !allow_loss && out.retired.is_some() {
                    return Err("clean assembly retired".into());
                }
                if let Some(picture) = out.picture {
                    report(name, &picture)?;
                    counts.pictures += 1;
                }
            }
            AvcClientPoll::Media(AvcReceivePoll::Picture(picture)) => {
                report(name, &picture)?;
                counts.pictures += 1;
            }
            AvcClientPoll::Media(AvcReceivePoll::Gap {
                gap,
                fragment,
                picture,
            }) if allow_loss => {
                counts.gaps += 1;
                println!(
                    "{{\"kind\":\"delivery_gap\",\"gap\":\"{gap:?}\",\"fragment_retired\":{},\"picture_retired\":{}}}",
                    fragment.is_some(),
                    picture.is_some()
                );
            }
            ...
            AvcClientPoll::Pending { wake_at_ns } if wake_at_ns.is_none_or(|at| at > now) => {
                return Ok(());
            }
            ...
            AvcClientPoll::Ended {
                ...
                if let Some(tail) = tail {
                    if tail.retired.is_some() {
                        return Err("clean EOF retired metadata only".into());
                    }
                ...
                println!(
                    "{{\"kind\":\"local_eof\",\"fixture\":\"{name}\",\"remote_session_may_exist\":{}}}",
                    retired.session.remote_session_may_exist
                );
                return Ok(());
            }
...
fn report(name: &str, picture: &AvcPictureGroup) -> Result<(), Error> {
    ...
    println!(
        "{{\"kind\":\"picture_group\",\"fixture\":\"{name}\",\"timestamp\":{},\"frame_num\":{},\"width\":{width},\"height\":{height},\"boundary\":\"{:?}\",\"complete_picture_certified\":false}}",
        picture.timestamp(),
    ...
    for nal in picture.nals() {
        ...
        println!(
            "{{\"kind\":\"picture_nal\",\"digest\":\"{digest}\",\"source_spans\":{}}}",
            nal.sources().len()
        );
    }
    Ok(())
