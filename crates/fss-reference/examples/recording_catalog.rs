...
use fss_publication::{
    LocalPublicationLimits, LocalRootPublisher, PublishCancellation, PublishCutPoint, SlotName,
};
use fss_reference::rtsp::recording::local::load_recording;
use fss_reference::rtsp::recording::{MAX_RECORDING_BYTES, RecordingScope};
...
use fss_reference::rtsp::recording_catalog::{
    CatalogBuilder, CatalogQueryLimits, CatalogScope, MAX_CATALOG_WINDOWS,
};
use std::{
    path::Path,
...
impl PublishCancellation for Deadline {
    fn cancel_requested(&self, _: PublishCutPoint) -> bool {
        self.start.elapsed() >= Duration::from_nanos(DEADLINE_NS)
    }
}
fn main() {
    if let Err(error) = run() {
        eprintln!("catalog command refused: {error}");
        std::process::exit(1);
    }
}
fn run() -> Result<()> {
    ...
    for arg in std::env::args().skip(1) {
        if args.len() >= MAX_CATALOG_WINDOWS + 10 || arg.len() > 4096 {
            return Err("argument budget exceeded".into());
        }
        args.push(arg);
    }
    if args.len() < 11 {
    ...
    let scope = CatalogScope {
        recording: RecordingScope {
            sensor: SensorId::parse(&args[3])?,
            stream: StreamId::parse(&args[4])?,
            generation: args[5].parse()?,
            anchor: ContentDigest::parse(&args[6])?,
            receive_clock: ContentDigest::parse(&args[7])?,
        },
        decode_clock: ContentDigest::parse(&args[8])?,
        time_scale: args[9].parse()?,
    };
    if scope.recording.generation == 0 || scope.time_scale == 0 {
        return Err("nonzero epoch and tick rate required".into());
    }
    match args[0].as_str() {
    ...
    if args[0] == "index" {
        ...
        for reference in &args[10..] {
            let (name, digest) = reference
                .split_once('=')
                .ok_or("use WINDOW_SLOT=algorithm:digest")?;
        ...
        for _ in 0..MAX_CATALOG_WINDOWS + 3 {
            match job.step(clock.now()?, &clock)? {
                ...
                CatalogProgress::Published(receipt) => {
                    println!(
                        "{{\"kind\":\"catalog_published\",\"root\":\"{}\",\"windows\":{},\"state\":\"{:?}\"}}",
                        receipt.root,
                        catalog.entries().len(),
        ...
        let start = args[11].parse()?;
        let end = args[12].parse()?;
        let limits = CatalogQueryLimits {
            max_windows: MAX_CATALOG_WINDOWS,
            max_output_bytes: args[13].parse()?,
        };
        let catalog = load_catalog(&publisher, &slot, expected, &scope, &clock)?;
        let mut read =
            RecordingRangeRead::new(&publisher, &catalog, &slot, start..end, limits, DEADLINE_NS)?;
        for _ in 0..MAX_CATALOG_WINDOWS + 1 {
            match read.step(clock.now()?, &clock)? {
                RangeProgress::Window {
                    ...
                    recording.manifest().root(),
                    requested_interval.start,
                    requested_interval.end,
                    recording.byte_len()
                ),
                RangeProgress::Complete(receipt) => {
                    for gap in &receipt.unindexed {
                        println!(
                            "{{\"kind\":\"unindexed\",\"start\":{},\"end\":{},\"evidence_of_absence\":false}}",
                            gap.start, gap.end
                        );
                    }
                    println!(
                        "{{\"kind\":\"retrieval_complete\",\"catalog\":\"{}\",\"windows\":{},\"verified_payload_bytes\":{},\"unindexed_intervals\":{},\"camera_coverage_certified\":false}}",
                        receipt.catalog_root,
                        receipt.windows,
                        receipt.output_bytes,
                        receipt.unindexed.len()
                    );
                    return Ok(());
                }
