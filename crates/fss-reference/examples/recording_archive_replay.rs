...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let root = std::path::PathBuf::from(args.next().ok_or("supply an explicit new reference archive directory")?);
    ...
    let mut publisher = LocalRootPublisher::open(&root, limits)?;
    {
        ...
        for now in 0..5 {
            match job.step(now, &NeverCancel)? {
                RecordingProgress::ChildStaged { role, digest, bytes, remaining } => println!(
                    "{{\"kind\":\"child_staged\",\"role\":\"{role:?}\",\"digest\":\"{}\",\"bytes\":{bytes},\"remaining\":{remaining}}}", digest.to_text()),
                RecordingProgress::Published(receipt) => println!(
                    "{{\"kind\":\"root_publication\",\"root\":\"{}\",\"local\":\"{:?}\",\"outcome\":\"{:?}\"}}",
                    receipt.root.to_text(), receipt.claims.local, receipt.outcome),
                RecordingProgress::Complete => return Err("unexpected duplicate terminal step".into()),
            }
        }
    }
    drop(publisher);
    let reopened = LocalRootPublisher::open(&root, limits)?;
    let recovered = load_recording(&reopened, &slot, plan.manifest().root(), &fixture::scope()?, &NeverCancel)?;
    if recovered.objects().source != plan.objects().source || recovered.objects().media != plan.objects().media {
        return Err("reopened source/media differ from the prepared bytes".into());
    }
    println!("{{\"kind\":\"verified_readback\",\"root\":\"{}\",\"samples\":{},\"packets\":{},\"decoded_here\":false,\"qualification\":\"reference_rehearsal_only\"}}",
        recovered.manifest().root().to_text(), recovered.summary().samples, recovered.summary().packets);
    Ok(())
}
