...
use fss_reference::ingest::{
    ADP_FILE_ROW_ID,
    rtpdump::recovery::{RtpRecoveryPolicy, RtpRecoveryState, inspect_rtp_import},
};
...
fn main() -> std::process::ExitCode {
    match run() {
        ...
        Err(error) => {
            eprintln!("RTP recovery refused: {error}");
...
fn run() -> Result<()> {
    ...
    if args.len() != 4 {
        return Err("use inspect|resume DEPLOYMENT ROOT_DIGEST PRINCIPAL".into());
    }
    ...
    let resume = match text(0)? {
        "inspect" => false,
        "resume" => true,
        _ => return Err("use inspect or resume".into()),
    };
    let directory = std::path::PathBuf::from(&args[1]);
    if !directory.join("LAYOUT").is_file() {
        return Err("existing deployment required".into());
    }
    let root: ContentDigest = text(2)?.parse()?;
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:rtpdump-recovery".into(),
        operation_id: OperationId::parse("operation:rtpdump-recovery")?,
        principal: text(3)?.to_owned(),
    ...
    let state = match recovered.state() {
        RtpRecoveryState::Complete { .. } => "complete",
        RtpRecoveryState::LedgerPending { .. } => "ledger_pending",
    };
    println!(
        "{{\"kind\":\"rtpdump_root_verified\",\"root\":\"{}\",\"state\":\"{state}\",\"source_bytes\":{},\"nals\":{},\"absence_certifiable\":false}}",
        root,
        recovered.source().len(),
        recovered.report().nals().len()
    );
    if resume {
        let receipt = recovered.resume(&cx, &mut deployment)?;
        println!(
            "{{\"kind\":\"rtpdump_resume_complete\",\"root\":\"{}\",\"commit_sequence\":{}}}",
            receipt.root(),
