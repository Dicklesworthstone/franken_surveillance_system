...
use fss_publication::{
    LocalPublicationLimits, LocalPublicationState, LocalRootPublisher, NeverCancel, SlotName,
};
...
use fss_reference::rtsp::recording_catalog::local::{
    CatalogProgress, CatalogPublication, RangeProgress, RecordingRangeRead, load_catalog,
};
use fss_reference::rtsp::recording_catalog::{CatalogBuilder, CatalogQueryLimits, CatalogScope};
...
impl Archive {
    fn publish(&mut self, window: PreparedRecording, now: u64) -> Result<()> {
        if self.expected.len() == MAX_WINDOWS {
            return Err("fixture window bound".into());
        }
        ...
        let expected = ExpectedWindow {
            slot: slot.clone(),
            root: window.manifest().root(),
            source: ContentDigest::try_sha256(window.objects().source)?,
            media: ContentDigest::try_sha256(window.objects().media)?,
            samples: window.summary().samples,
        };
        let mut publication = RecordingPublication::new(
            &window,
            &mut self.publisher,
            slot,
            window.byte_len(),
            now.checked_add(100).ok_or("fixture deadline overflow")?,
        )?;
        for step in 0..5 {
            match publication.step(now + step, &NeverCancel)? {
                RecordingProgress::ChildStaged {
                    ...
                    "{{\"kind\":\"child_staged\",\"role\":\"{role:?}\",\"digest\":\"{}\",\"bytes\":{bytes}}}",
                    digest.to_text()
                ),
                RecordingProgress::Published(receipt)
                    if step == 4 && receipt.claims.local == LocalPublicationState::Durable =>
                {
                    println!(
        ...
        for _ in 0..64 {
            match capture.poll(now)? {
                CapturePoll::Receiver(AvcReceivePoll::Source { source, .. }) => {
                    self.packets += 1;
                    println!(
                        "{{\"kind\":\"source\",\"sequence\":{},\"digest\":\"{}\"}}",
                        source.sequence(),
                        ContentDigest::try_sha256(source.bytes())?.to_text()
                    );
                    return Ok(false);
                }
                ...
                CapturePoll::TimingRequired(request) => {
                    if self.samples >= 4 {
                        return Err("unexpected extra fixture picture".into());
                    }
                    match capture.supply_timing(fixture::timing(self.samples * 3600), now)? {
                        TimedCapture::Collected { unselected, .. } if unselected.is_empty() => {}
                        other => {
                            return Err(format!("unexpected fixture selection: {other:?}").into());
                        }
                    }
                    self.samples += 1;
                    println!(
                        "{{\"kind\":\"picture_timed\",\"basis\":\"synthetic_fixture_25fps\",\"frame_num\":{},\"boundary\":\"{:?}\",\"complete_picture_certified\":false}}",
                ...
                other => {
                    return Err(format!(
                        "fixture stopped instead of claiming clean capture: {other:?}"
                    )
                    .into());
                }
            }
    ...
    fn drain(
        ...
        for _ in 0..1024 {
            ...
            if self.capture_step(capture, now)? {
                return Ok(true);
            }
            if pending {
                return Ok(false);
...
fn main() -> Result<()> {
    ...
    let root = std::path::PathBuf::from(
        args.next()
            .ok_or("supply an explicit new laboratory archive directory")?,
    );
    ...
    let catalog_scope = CatalogScope {
        recording: fixture::scope()?,
        decode_clock: ContentDigest::try_sha256(b"fixture-dts-epoch")?,
        time_scale: 90_000,
    };
    let mut archive = Archive {
        publisher: LocalRootPublisher::open(&root, limits)?,
        expected: Vec::new(),
        catalog: CatalogBuilder::new(catalog_scope.clone())?,
        samples: 0,
        packets: 0,
    };
    ...
    receiver.ingest(
        fixture::key(),
        &fixture::packet(0, 90_000, false, nals[0]),
        0,
    )?;
    let mut frame = 0;
    for (index, nal) in nals.iter().enumerate() {
        ...
        if archive.drain(&mut receiver, &mut capture, sequence)? {
            return Err("premature fixture EOF".into());
        }
        if vcl {
            frame += 1;
        }
    ...
    receiver.finish();
    if !archive.drain(&mut receiver, &mut capture, 1000)?
        || archive.samples != 4
        || archive.expected.len() != 2
        || archive.packets != nals.len()
    {
    ...
    let catalog_slot = SlotName::parse("catalog-page-1")?;
    {
        ...
        for i in 0..catalog.entries().len() {
            if !matches!(
                job.step(1000 + i as u64, &NeverCancel)?,
    ...
    for expected in expected {
        ...
        println!(
            "{{\"kind\":\"verified_readback\",\"root\":\"{}\",\"samples\":{},\"packets\":{}}}",
            expected.root.to_text(),
            loaded.summary().samples,
            loaded.summary().packets
        );
    }
    ...
    match read.step(1202, &NeverCancel)? {
        RangeProgress::Complete(receipt)
            if receipt.windows == 2 && receipt.unindexed.is_empty() =>
        {
            println!(
                "{{\"kind\":\"catalog_range_verified\",\"root\":\"{}\",\"windows\":2,\"unindexed_intervals\":0,\"camera_coverage_certified\":false}}",
    ...
    println!(
        "{{\"kind\":\"terminal\",\"windows\":2,\"samples\":4,\"pending_bytes\":0,\"live_camera\":false,\"qualification\":\"reference_rehearsal_only\"}}"
    );
    Ok(())
}
