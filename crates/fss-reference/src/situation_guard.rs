...
enum ReceiptSource {
    ...
    /// receipts, together with that journal's own prepared record of the operation.
    DurableJournal(Box<PreparedEffect>),
}

impl ReceiptSource {
    const fn is_durable_journal(&self) -> bool {
        matches!(self, Self::DurableJournal(_))
    }
}
...
fn compile_against_durable_journal(
    ...
    if let Some(plan) = request.alert_plan {
        ...
        compile_with_operation_receipt(
            ...
            operation_receipt,
            authority,
            ReceiptSource::DurableJournal(Box::new(prepared)),
        )
...
///
/// The effect journal keeps the rule a v1 or v2 record was written under, so a cancellation read
/// from that journal for a v1 or v2 receipt carries the digest it was written with; the guard
/// exempts exactly those receipts, keyed on the same record version the journal keys the rule on,
/// and only when it reads them from the journal itself. Every other cancelled receipt, including
...
fn cancellation_proof_is_admissible(
    ...
    if receipt.record_version() < EffectRecordVersion::V3 && source.is_durable_journal() {
        return true;
    }
