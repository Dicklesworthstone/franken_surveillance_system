...
impl fmt::Display for LiveRecordingError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "live recording refusal: {self:?}")
    }
}
...
impl fmt::Debug for LiveRecordingRetirement {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("LiveRecordingRetirement")
            .field("connection", &self.connection.is_some())
            .field("unoffered_media", &self.unoffered_media.is_some())
...
impl fmt::Debug for LiveRecordingStep {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            Self::Network(_) => "LiveRecordingStep::Network",
            Self::MediaQueued => "LiveRecordingStep::MediaQueued",
            Self::Capture { .. } => "LiveRecordingStep::Capture",
            Self::Stopped { .. } => "LiveRecordingStep::Stopped",
...
impl fmt::Debug for LiveAvcRecording {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("LiveAvcRecording")
...
impl LiveAvcRecording {
    ...
    pub fn connect(
        ...
        let collector = RecordingCollector::new(
            recording.scope,
            config.binding.key(),
            recording.payload_type,
            recording.time_scale,
            recording.limits,
        )
        .map_err(|e: CollectorError| LiveRecordingConnectFailure {
            reason: LiveRecordingError::Capture(CaptureError::Collection(e)),
            connection_attempted: false,
        })?;
        let binding = config.binding.clone();
        let deadline_ns = config.deadline_ns;
        let remaining_steps = config.max_steps;
        let connection = LiveAvcConnection::connect(config, now, authority).map_err(|e| {
            LiveRecordingConnectFailure {
                reason: LiveRecordingError::Network(e.reason),
                connection_attempted: e.connection_attempted,
            }
        })?;
        Ok(Self {
            connection,
            capture: RecordingCapture::new(collector),
            binding,
            deadline_ns,
            remaining_steps,
            last_ns: now,
    ...
    pub fn next_wake_ns(&self) -> Option<u64> {
        ...
        earlier(
            ...
            earlier(self.capture.next_wake_ns(), self.connection.next_wake_ns()),
        )
    }
    ...
    pub fn request(
        ...
        now: u64,
        authority: &dyn TcpAuthority,
    ) -> Result<QueuedAvcRequest, LiveRecordingFailure> {
    ...
    pub fn supply_timing(
        ...
        self.admit(now, authority)?;
        self.capture
            .supply_timing(timing, now)
            .map_err(|e| LiveRecordingFailure {
                reason: LiveRecordingError::Capture(e),
                retirement: None,
            })
    }
    ...
    pub fn seal(
        ...
        now: u64,
        authority: &dyn TcpAuthority,
    ) -> Result<bool, LiveRecordingFailure> {
        self.admit(now, authority)?;
        self.capture.seal(now).map_err(|e| LiveRecordingFailure {
            reason: LiveRecordingError::Capture(e),
            retirement: None,
        })
    }
    ...
    pub fn poll(
        ...
        if self.closed {
            return Ok(LiveRecordingStep::Ended);
        }
        self.admit(now, authority)?;
        match self.capture.poll(now) {
            ...
            Ok(event) => {
                ...
                let connection = if terminal {
                    self.closed = true;
                    self.connection.cancel().map(Box::new)
                } else {
                    None
                };
                return Ok(LiveRecordingStep::Capture {
                    event: Box::new(event),
                    connection,
                });
            }
        ...
        if self.network_ended {
            ...
            return Err(self.fatal(
                LiveRecordingError::Capture(CaptureError::Closed),
                None,
                None,
            ));
        }
        ...
        match step {
            LiveAvcStep::Protocol {
                ...
                match *event {
                    AvcClientPoll::Media(media)
                        if transport.is_none() && wire_retirement.is_none() =>
                    {
                        ...
                        match self.capture.offer(media, now) {
                            Ok(()) => {
                                ...
                                Ok(LiveRecordingStep::Network(LiveAvcStep::Protocol {
                                    event: DigestAvcPoll::Client {
                                        event: Box::new(AvcClientPoll::Ended {
                                            media: None,
                                            retirement,
                                        }),
                                        wire_retirement,
                    ...
                    event => {
                        ...
                        if terminal {
                            Ok(self.stop_event(step))
                        } else {
                            Ok(LiveRecordingStep::Network(step))
                        }
                    }
                }
            }
            step @ LiveAvcStep::Protocol {
                event: DigestAvcPoll::Fault { .. },
                ..
            } => Ok(self.stop_event(step)),
    ...
    fn offer(&mut self, media: AvcReceivePoll, now: u64) -> Result<(), LiveRecordingFailure> {
        self.capture
            .offer(media, now)
            .map_err(|e| self.fatal(LiveRecordingError::Capture(e.reason), None, Some(e.event)))
    }
    fn admit(
        &mut self,
        now: u64,
        authority: &dyn TcpAuthority,
    ) -> Result<(), LiveRecordingFailure> {
        let safe = |e| LiveRecordingFailure {
            reason: LiveRecordingError::Network(e),
            retirement: None,
        };
        if self.closed {
        ...
        if self.remaining_steps == 0 {
            return Err(self.fatal(
                LiveRecordingError::Network(LiveAvcError::WorkBudget),
                None,
                None,
            ));
        }
        ...
        self.last_ns = now;
        let result = if now >= self.deadline_ns {
            Err(TcpError::Deadline)
        } else {
            authority
                .checkpoint(&self.binding, TcpOperation::Poll, now, self.deadline_ns)
                .map_err(TcpError::Denied)
        };
        result.map_err(|e| {
            self.fatal(
                LiveRecordingError::Network(LiveAvcError::Transport(e)),
                None,
                None,
            )
        })
    ...
    fn stop_event(&mut self, trigger: LiveAvcStep) -> LiveRecordingStep {
        ...
        LiveRecordingStep::Stopped {
            ...
            retained: Box::new(self.capture.cancel()),
        }
    }
    fn fatal(
        &mut self,
        reason: LiveRecordingError,
        connection: Option<LiveAvcRetirement>,
        media: Option<Box<AvcReceivePoll>>,
    ) -> LiveRecordingFailure {
        LiveRecordingFailure {
            reason,
            retirement: Some(Box::new(self.retire(connection, media))),
        }
    }
    fn retire(
        &mut self,
        ...
        self.closed = true;
        LiveRecordingRetirement {
            connection: connection.or_else(|| self.connection.cancel()),
            capture: self.capture.cancel(),
            unoffered_media: media,
        }
    }
}
