...
pub(super) const KEY: StreamKey = StreamKey {
    ingress: 71,
    generation: 1,
    ssrc: 7,
};
...
impl TcpAuthority for Authority {
    fn checkpoint(
        ...
        if self.reject_queue.get() && op == TcpOperation::QueueRequest {
            return Err(TcpDenial::Budget);
        }
...
impl Fixture {
    ...
    fn command(
        ...
        now: u64,
    ) -> TestResult<(QueuedAvcRequest, Vec<u8>)> {
        let nonce = now.to_le_bytes()[0].wrapping_add(30);
        let queued =
            self.connection
                .request(command, &credentials()?, [nonce; 16], now, &self.authority)?;
...
fn mismatched_route_is_rejected_before_connect_or_authority_access() -> TestResult {
    ...
    let error = LiveAvcConnection::connect(config, 0, &authority)
        .err()
...
fn local_send_requires_readiness_and_preserves_exact_short_write_suffixes() -> TestResult {
    ...
    let queued = f.connection.request(
    ...
    assert!(matches!(
        f.connection
            .poll(SocketReadiness::default(), 0, &f.authority)?,
    ...
    assert_eq!(
        f.connection.totals().ok_or("missing totals")?.write_calls,
        0
    );
    ...
    assert_eq!(
        f.connection.totals().ok_or("missing totals")?.sent_bytes,
        bytes.len() as u64
    );
...
fn real_socket_digest_challenge_and_acknowledged_setup_play_use_existing_protocol() -> TestResult {
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, DigestAvcPoll::AuthenticationRequired { cseq: 1, .. }))
    );
    let queued = f
        .connection
        .respond(&credentials()?, [18; 16], 2, &f.authority)?;
    ...
    f.receive(
        &response(2, "Content-Type: application/sdp\r\n", description()),
        3,
    )?;
    ...
    for now in 2..10 {
        assert!(matches!(
            ...
            LiveAvcStep::Protocol {
                event: DigestAvcPoll::AuthenticationRequired { .. },
                transport: None
            }
        ));
    }
    assert_eq!(
        f.connection.totals().ok_or("missing totals")?.read_calls,
        before
    );
    let step = f
        .connection
        .poll(SocketReadiness::default(), RESPONSE_TIMEOUT, &f.authority)?;
    assert!(matches!(
        step,
        LiveAvcStep::Protocol {
            transport: Some(_),
...
fn partial_request_timeout_retires_exact_prefix_without_resending() -> TestResult {
    ...
    let LiveAvcStep::Protocol {
        ...
        return Err("missing terminal protocol event".into());
    };
    ...
    assert_eq!(
        transport
            .request
            .as_ref()
            .ok_or("lost prepared request")?
            .cseq(),
        1
    );
    match event {
        DigestAvcPoll::Client { event, .. } => assert!(matches!(
            *event,
            AvcClientPoll::Fault {
...
fn queue_refusal_preserves_the_prepared_command_and_closes_both_owners() -> TestResult {
    ...
    let error = f
        ...
        .err()
        .ok_or("unexpected queued request")?;
    assert_eq!(
        error.reason,
        LiveAvcError::Transport(TcpError::Denied(TcpDenial::Budget))
    );
    let retired = error.retirement.ok_or("missing retirement")?;
    assert_eq!(
        retired
            .unqueued_request
            .ok_or("lost unqueued command")?
            .cseq(),
        1
    );
...
fn revocation_cancels_even_without_socket_readiness_and_debug_never_prints_secrets() -> TestResult {
    ...
    let error = f
        .connection
        .poll(SocketReadiness::default(), 3, &f.authority)
        .err()
        .ok_or("revocation ignored")?;
    assert_eq!(
        error.reason,
        LiveAvcError::Transport(TcpError::Denied(TcpDenial::Revoked))
    );
...
fn work_budget_bounds_semantic_polling_and_clock_regression_is_non_mutating() -> TestResult {
    ...
    let _ = f
        .connection
        .poll(SocketReadiness::default(), 10, &f.authority)?;
    let before = f.connection.remaining_steps();
    assert_eq!(
        f.connection
            .poll(SocketReadiness::default(), 9, &f.authority)
            .err()
            .ok_or("regressed clock accepted")?
            .reason,
        LiveAvcError::ClockReversed
    );
    ...
    for _ in 0..1024 {
        match f.connection.poll(
            ...
            LiveAvcStep::Protocol {
                ...
                assert!(matches!(
                    f.connection
                        .poll(SocketReadiness::default(), 1, &f.authority)?,
                    LiveAvcStep::Ended
                ));
                return Ok(());
            }
