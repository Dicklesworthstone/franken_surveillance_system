...
fn recording(limits: CollectorLimits) -> TestResult<LiveRecordingConfig> {
    Ok(LiveRecordingConfig {
        scope: RecordingScope {
            sensor: SensorId::parse("sensor:live-fixture")?,
            stream: StreamId::parse("stream:live-fixture")?,
...
fn timing(decode_time: u64) -> RecordingTiming {
    RecordingTiming {
        decode_time,
        duration: 3_600,
...
impl Fixture {
    ...
    fn command(&mut self, command: ClientCommand, now: u64) -> TestResult<u32> {
        let queued = self.driver.request(
            command,
            &credentials()?,
            [now.to_le_bytes()[0].wrapping_add(40); 16],
            now,
            &self.authority,
        )?;
        for _ in 0..32_768 {
            match self.driver.poll(
                ...
                LiveRecordingStep::Network(LiveAvcStep::Write(TcpWriteStep::Sent {
                    ...
                    self.peer.read_exact(&mut original)?;
                    return Ok(cseq);
                }
            ...
            match step {
                ...
                other => {
                    let stop = match &other {
                        ...
                        LiveRecordingStep::Capture { event, .. } => matches!(
                            **event,
        ...
        let request = outputs
            ...
            .find_map(|step| match step {
                LiveRecordingStep::Capture { event, .. } => match **event {
                    CapturePoll::TimingRequired(request) => Some(request),
                    _ => None,
                },
                _ => None,
            })
    ...
    fn completed_window(&mut self) -> TestResult<PreparedRecording> {
        ...
        assert!(request.idr);
        assert_eq!(request.rtp_timestamp, 9_000);
        assert!(matches!(
            self.driver
                .supply_timing(timing(700), 12, &self.authority)?,
            TimedCapture::Collected { .. }
        ));
        assert!(self.driver.seal(13, &self.authority)?);
        match self
            .driver
            .poll(SocketReadiness::default(), 13, &self.authority)?
        {
...
fn invalid_recording_generation_refuses_before_network_authority() -> TestResult {
    ...
    let mut recording = recording(CollectorLimits::default())?;
    recording.scope.generation += 1;
    let error = LiveAvcRecording::connect(config, recording, 0, &authority)
        .err()
        .ok_or("accepted mismatched generation")?;
    assert_eq!(
        error.reason,
        LiveRecordingError::Capture(CaptureError::Collection(CollectorError::Configuration))
    );
...
fn actual_socket_picture_requests_timing_and_backpressures_further_reads() -> TestResult {
    ...
    f.peer
        .write_all(b"must not be read while timing is pending")?;
    for now in 12..16 {
        assert!(
            matches!(f.driver.poll(SocketReadiness { readable: true, writable: true }, now, &f.authority)?,
    ...
    assert!(
        retired
            ...
            .sources
            .iter()
            .any(|s| s.bytes() == original)
    );
    assert!(
        retired
            .connection
            .ok_or("lost network retirement")?
            .protocol
            .client
            .session
            .remote_session_may_exist
    );
...
fn timing_refusal_preserves_picture_and_does_not_substitute_rtp_or_arrival_time() -> TestResult {
    ...
    let (_, picture) = f.idr()?;
    let mut bad = timing(700);
    bad.duration = 0;
    let error = f
        .driver
        .supply_timing(bad, 12, &f.authority)
        .err()
        .ok_or("zero duration accepted")?;
    assert!(error.retirement.is_none());
    assert!(
        matches!(f.driver.poll(SocketReadiness::default(), 12, &f.authority)?,
    ...
    let LiveRecordingStep::Capture { event, .. } =
        f.driver
            .poll(SocketReadiness::default(), 14, &f.authority)?
    else {
        return Err("window missing".into());
    };
...
fn timing_deadline_stops_the_socket_and_retains_unsealed_source() -> TestResult {
    ...
    let step = f
        .driver
        .poll(SocketReadiness::default(), deadline, &f.authority)?;
    let LiveRecordingStep::Capture {
        event,
        connection: Some(connection),
    } = step
    else {
        return Err("timing expiry did not close both layers".into());
    };
    let CapturePoll::Stopped {
    ...
    assert!(
        retained
            ...
            .sources
            .iter()
            .any(|source| source.bytes() == original)
    );
...
fn revocation_during_timing_wait_closes_without_reading_more_media() -> TestResult {
    ...
    let error = f
        ...
        .err()
        .ok_or("revoked session delivered a picture")?;
    let retired = error.retirement.ok_or("revocation lost source")?;
    assert_eq!(
        retired
            .connection
            .as_ref()
            .ok_or("missing network retirement")?
            .transport
            .totals
            .read_calls,
        reads
...
fn collection_pressure_never_reads_ahead_and_sealing_releases_the_bounded_prefix() -> TestResult {
    ...
    let reads = f.driver.totals().ok_or("missing totals")?.read_calls;
    assert!(
        matches!(f.driver.poll(SocketReadiness { readable: true, writable: true }, 13, &f.authority)?,
        LiveRecordingStep::Capture { event, .. } if matches!(*event, CapturePoll::Backpressure(_)))
    );
    assert_eq!(f.driver.totals().ok_or("missing totals")?.read_calls, reads);
    assert!(f.driver.seal(14, &f.authority)?);
    assert!(
        matches!(f.driver.poll(SocketReadiness::default(), 14, &f.authority)?,
    ...
    fn new() -> TestResult<Self> {
        ...
        for _ in 0..128 {
            ...
            let path = std::env::temp_dir()
                .join(format!("fss-live-recording-{}-{id}", std::process::id()));
...
fn socket_recording_publishes_source_first_and_reopens_with_identical_root_and_bytes() -> TestResult
{
    ...
    for chunk in [1, 7, 4096] {
        ...
        if let Some(root) = expected_root {
            ...
            expected_root = Some(window.manifest().root());
        }
        assert_eq!(window.summary().packets, 1);
        ...
        let slot = SlotName::parse("live-avc-window")?;
        {
            let mut publisher = LocalRootPublisher::open(&path, limits)?;
            {
                ...
                assert!(matches!(
                    job.step(14, &NeverCancel)?,
                    RecordingProgress::ChildStaged {
                        role: RecordingRole::Source,
                        remaining: 3,
                        ..
                    }
                ));
                for at in 15..18 {
                    let _ = job.step(at, &NeverCancel)?;
                }
            }
            ...
            for at in 18..23 {
                if let RecordingProgress::Published(receipt) = job.step(at, &NeverCancel)? {
                    assert_eq!(receipt.root, window.manifest().root());
                    published = true;
                }
            }
            assert!(published);
...
fn actual_eof_drains_completed_window_without_fabricating_another_picture() -> TestResult {
    let mut f = Fixture::playing(4096, CollectorLimits::default())?;
    f.idr()?;
    let _ = f.driver.supply_timing(timing(700), 12, &f.authority)?;
    ...
    for _ in 0..2048 {
        match f.driver.poll(
            ...
            LiveRecordingStep::Stopped { .. } => {
                return Err("clean EOF incorrectly became a source failure".into());
            }
            _ => std::thread::yield_now(),
        }
    }
    ...
    assert!(matches!(
        f.driver
            .poll(SocketReadiness::default(), 13, &f.authority)?,
        LiveRecordingStep::Ended
    ));
    Ok(())
}
...
fn wrong_stream_fault_stops_capture_without_sealing_pending_pictures() -> TestResult {
    ...
    let stopped = events
        .into_iter()
        .find_map(|step| match step {
            LiveRecordingStep::Stopped { trigger, retained } => Some((trigger, retained)),
            _ => None,
        })
        .ok_or("wrong stream did not stop the recording owner")?;
    assert!(matches!(
        *stopped.0,
        LiveAvcStep::Protocol {
            transport: Some(_),
            ..
        }
    ));
        ...
        match f.driver.poll(
            ...
            LiveRecordingStep::Capture { event, .. } => match *event {
                CapturePoll::Window(_) | CapturePoll::TimingRequired(_) => {
                    return Err(
                        "incomplete FU-A acquired invented picture or recording boundary".into(),
                    );
                }
