...
use super::recording_archive::checkpoint::{ArchiveWorkLimits, MAX_ARCHIVE_WORK_BYTES, PreparedArchiveWork, load_archive_work};
...
impl Default for ArchivePinLimits {
    fn default() -> Self { Self { max_records: 8193, max_bytes: 8 * 1024 * 1024 } }
}
impl ArchivePinLimits {
    fn validate(self) -> PinResult<()> {
        if self.max_records == 0 || self.max_records > 16_384
            || self.max_bytes < RECORD_OVERHEAD || self.max_bytes > 16 * 1024 * 1024 {
...
impl StoredArchivePin {
    ...
    fn checkpoint(pin: &ArchiveCheckpoint) -> Self {
        Self { slot: pin.slot().clone(), root: pin.root(), retirement: pin.retirement_digest(),
...
impl ArchivePinState {
    ...
    pub fn candidate(&self) -> Option<&StoredArchivePin> { self.candidate.as_ref() }
    /// Last confirmed work root, retained alongside a newer unconfirmed candidate.
    pub fn last_confirmed(&self) -> Option<&StoredArchivePin> { self.confirmed.as_ref() }
}
...
impl ArchivePinReceipt {
    ...
    pub fn anchor(&self) -> ArchivePinAnchor { self.anchor }
    /// Exact checkpoint whose reference was retained.
    pub fn pin(&self) -> &StoredArchivePin { &self.pin }
    /// Candidate persistence versus confirmed work custody.
    pub fn phase(&self) -> ArchivePinPhase { self.phase }
}
...
impl fmt::Display for ArchivePinError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            ...
            Self::Io(_) => "archive pin I/O refused", Self::Archive(_) => "archive pin custody refused",
            Self::Layout => "invalid archive pin layout", Self::Busy => "archive pin owner busy",
            Self::Limit => "archive pin bound exceeded", Self::Scope => "archive pin scope mismatch",
            Self::History => "invalid archive pin history", Self::Unsettled => "archive work requires recovery",
            Self::RootMismatch => "archive pin prefix mismatch",
            Self::Cancelled => "archive pin operation cancelled", Self::Fenced => "archive pin owner fenced",
        })
    }
}
...
impl From<JournalError> for ArchivePinError { fn from(e: JournalError) -> Self { Self::Journal(e) } }
impl From<std::io::Error> for ArchivePinError { fn from(e: std::io::Error) -> Self { Self::Io(e.kind()) } }
impl From<ArchiveError> for ArchivePinError { fn from(e: ArchiveError) -> Self { Self::Archive(e) } }
...
impl fmt::Debug for ArchivePinJournal {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ArchivePinJournal").field("records", &self.records)
            .field("fenced", &self.fenced).finish_non_exhaustive()
    }
}
impl ArchivePinJournal {
    ...
    pub fn create(directory: impl AsRef<Path>, scope: ArchivePinScope, limits: ArchivePinLimits,
        cancel: &dyn PublishCancellation) -> PinResult<Self> {
        limits.validate()?; codec::validate_scope(scope)?; check(cancel)?;
        ...
        let lock = OpenOptions::new().read(true).write(true).create_new(true).open(dir.join("LOCK"))?;
        acquire_lock(&lock)?;
        lock.sync_all()?;
        let path = dir.join(JOURNAL_FILE);
        let initial = OpenOptions::new().write(true).create_new(true).open(&path)?;
        initial.sync_all()?; drop(initial);
        check(cancel)?;
        ...
        check(cancel)?;
        Ok(Self { journal, _lock: lock, scope, limits, replay: ReplayState::default(), records: 1, fenced: false })
    }
    ...
    pub fn open_existing(directory: impl AsRef<Path>, scope: ArchivePinScope,
        minimum: Option<ArchivePinAnchor>, limits: ArchivePinLimits, tail: IncompleteTailPolicy,
        cancel: &dyn PublishCancellation) -> PinResult<Self> {
        limits.validate()?; codec::validate_scope(scope)?; check(cancel)?;
        ...
        if journal.last_root() != report.last_root() || journal.committed_len() != report.committed_len() {
            return Err(ArchivePinError::RootMismatch);
        }
        OpenOptions::new().write(true).open(&path)?.sync_all()?;
        File::open(&dir)?.sync_all()?;
        check(cancel)?;
        Ok(Self { journal, _lock: lock, scope, limits, replay, records: report.records().len(), fenced: false })
    }
    /// Accepted independent scope; it grants no source access or camera permission.
    pub fn scope(&self) -> ArchivePinScope { self.scope }
    /// Last acknowledged metadata prefix. When fenced, an attempted append may also exist.
    pub fn anchor(&self) -> ArchivePinAnchor {
        ArchivePinAnchor { sequence: self.records as u64, root: self.journal.last_root() }
    }
    /// Candidate and predecessor from the last acknowledged committed prefix.
    pub fn state(&self) -> &ArchivePinState { &self.replay.pins }
    ...
    pub fn verify(&mut self, cancel: &dyn PublishCancellation) -> PinResult<()> {
        if self.fenced { return Err(ArchivePinError::Fenced); }
        let result = (|| {
            let report = read_report(self.journal.path(), self.limits, cancel)?;
            if report.incomplete_tail().is_some() || report.last_root() != self.journal.last_root()
    ...
    pub fn verify_tip(&mut self, cancel: &dyn PublishCancellation) -> PinResult<()> {
        ...
        let result = (|| {
            check(cancel)?;
            if !fs::symlink_metadata(self.journal.path())?.is_file() { return Err(ArchivePinError::Layout); }
    ...
    pub fn persist_candidate(&mut self, pin: &ArchiveCheckpoint,
        cancel: &dyn PublishCancellation) -> PinResult<ArchivePinReceipt> {
        self.persist(StoredArchivePin::checkpoint(pin), ArchivePinPhase::Candidate, cancel)
    }
    ...
    pub fn confirm_recovered(&mut self, publisher: &LocalRootPublisher, bounds: ArchiveWorkLimits,
        cancel: &dyn PublishCancellation) -> PinResult<ArchivePinReceipt> {
        let _ = self.load_work(ArchivePinPhase::Candidate, publisher, bounds, cancel)?;
        let pin = self.replay.pins.candidate.clone().ok_or(ArchivePinError::History)?;
    ...
    pub fn load_work(&mut self, which: ArchivePinPhase, publisher: &LocalRootPublisher,
        bounds: ArchiveWorkLimits, cancel: &dyn PublishCancellation) -> PinResult<ArchiveRetirement> {
        self.verify(cancel)?;
        ...
        if work.snapshot.namespace().digest() != self.scope.archive_namespace
            ...
            || PreparedArchiveWork::prepare(&work, publisher, bounds, cancel)?.new_payload_bytes() != pin.new_payload_bytes {
    ...
    pub fn require_settled(&mut self, publisher: &LocalRootPublisher, bounds: ArchiveWorkLimits,
        cancel: &dyn PublishCancellation) -> PinResult<()> {
        self.verify(cancel)?;
        if self.replay.pins.candidate.is_some() { return Err(ArchivePinError::Unsettled); }
        if self.replay.pins.confirmed.is_none() { return Ok(()); }
        ...
        if let Some(window) = &work.pending {
            ...
            if publisher.root(&slot).is_none_or(|r| r.root != window.manifest().root()
                || r.state != LocalPublicationState::Durable) { return Err(ArchivePinError::Unsettled); }
        }
        if let Some(page) = &work.prepared_page {
            let slot = work.snapshot.namespace().page_slot(work.snapshot.indexed_windows())?;
            if publisher.root(&slot).is_none_or(|r| r.root != page.manifest().root()
                || r.state != LocalPublicationState::Durable) { return Err(ArchivePinError::Unsettled); }
        }
        check(cancel)
    }
    ...
    fn confirm_receipt(&mut self, pin: &ArchiveCheckpoint, receipt: &LocalPublicationReceipt,
        cancel: &dyn PublishCancellation) -> PinResult<ArchivePinReceipt> {
        if receipt.root != pin.root() || receipt.claims.local != LocalPublicationState::Durable {
            return Err(ArchivePinError::History);
        }
        self.persist(StoredArchivePin::checkpoint(pin), ArchivePinPhase::Confirmed, cancel)
    }
    fn persist(&mut self, pin: StoredArchivePin, phase: ArchivePinPhase,
        cancel: &dyn PublishCancellation) -> PinResult<ArchivePinReceipt> {
        self.verify(cancel)?; codec::validate_pin(&pin)?;
        let current = if phase == ArchivePinPhase::Candidate { self.replay.pins.candidate.as_ref() }
            else { self.replay.pins.confirmed.as_ref().filter(|_| self.replay.pins.candidate.is_none()) };
        if current == Some(&pin) { return Ok(ArchivePinReceipt { anchor: self.anchor(), pin, phase }); }
        let tag = if phase == ArchivePinPhase::Candidate { 1 } else { 2 };
        let payload = codec::encode(self.scope, tag, Some(&pin))?;
        let mut next = self.replay.clone();
        codec::apply(&mut next, tag, pin.clone())?;
        capacity(self.records, self.journal.committed_len(), payload.len(), self.limits)?;
        check(cancel)?;
        ...
        self.records += 1; self.replay = next;
        Ok(ArchivePinReceipt { anchor: self.anchor(), pin, phase })
    }
}
fn capacity(records: usize, bytes: u64, payload: usize, limits: ArchivePinLimits) -> PinResult<()> {
    if records >= limits.max_records || payload > MAX_PAYLOAD
        || bytes.checked_add((payload + RECORD_OVERHEAD) as u64).is_none_or(|n| n > limits.max_bytes as u64) {
        return Err(ArchivePinError::Limit);
    }
    Ok(())
}
fn check(cancel: &dyn PublishCancellation) -> PinResult<()> {
    if cancel.cancel_requested(PublishCutPoint::AfterChildrenVerified) { Err(ArchivePinError::Cancelled) } else { Ok(()) }
}
...
fn existing(path: &Path) -> PinResult<(PathBuf, File)> {
    if !fs::symlink_metadata(path)?.is_dir() { return Err(ArchivePinError::Layout); }
    let dir = fs::canonicalize(path)?;
    for name in ["LOCK", JOURNAL_FILE] {
        if !fs::symlink_metadata(dir.join(name))?.is_file() { return Err(ArchivePinError::Layout); }
    }
    let lock = OpenOptions::new().read(true).write(true).open(dir.join("LOCK"))?;
    acquire_lock(&lock)?;
    Ok((dir, lock))
}
fn read_report(path: &Path, limits: ArchivePinLimits, cancel: &dyn PublishCancellation) -> PinResult<RecoveryReport> {
    check(cancel)?;
    if !fs::symlink_metadata(path)?.is_file() { return Err(ArchivePinError::Layout); }
    let mut file = File::open(path)?;
    let bytes = usize::try_from(file.metadata()?.len()).map_err(|_| ArchivePinError::Limit)?;
    if bytes > limits.max_bytes { return Err(ArchivePinError::Limit); }
    let mut data = Vec::new();
    data.try_reserve_exact(bytes.saturating_add(1)).map_err(|_| ArchivePinError::Limit)?;
    file.seek(SeekFrom::Start(0))?;
    file.take(bytes as u64 + 1).read_to_end(&mut data)?;
    if data.len() != bytes { return Err(ArchivePinError::RootMismatch); }
