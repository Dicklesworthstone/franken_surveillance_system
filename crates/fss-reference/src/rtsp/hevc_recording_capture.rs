...
use super::hevc_recording_collector::{HevcCollectionAdmission, HevcCollectionRetirement,
    HevcCollectionSeal, HevcRecordingCollector};
...
impl HevcPictureTimingRequest {
    fn new(p: &HevcPictureGroup) -> Self {
        Self { key: p.key(), rtp_timestamp: p.timestamp(), idr: matches!(p.prefix().nal_type, 19 | 20),
            boundary: p.boundary(), bytes: p.byte_len() }
    }
}
...
impl HevcRecordingCapture {
    ...
    pub fn new(collector: HevcRecordingCollector) -> Self {
        Self { collector, event: None, event_deadline_ns: None, timing_required: false,
            blocked: false, input_ended: false, finished: false, closed: false, last_ns: 0 }
    }
    /// Read-only source/window accounting; mutation must use the ordered event contract.
    pub fn collector(&self) -> &HevcRecordingCollector { &self.collector }
    ...
    pub fn next_wake_ns(&self) -> Option<u64> {
        ...
        if self.collector.has_ready() || (self.event.is_some() && !self.timing_required && !self.blocked)
            || (self.input_ended && self.event.is_none()) { return Some(self.last_ns); }
    ...
    pub fn offer(&mut self, event: Event, now: u64) -> std::result::Result<(), HevcCaptureRefusal> {
        let result = self.check_time(now).and_then(|()| {
            ...
            if self.event.is_some() || self.collector.has_ready() { return Err(CaptureError::Backpressure); }
            self.collector.check_admission(now).map_err(CaptureError::Collection)
        });
        if let Err(reason) = result { return Err(HevcCaptureRefusal { reason, event: Box::new(event) }); }
        let Some(deadline) = now.checked_add(MAX_PENDING_EVENT_AGE_NS) else {
            return Err(HevcCaptureRefusal { reason: CaptureError::Collection(CollectorError::Deadline), event: Box::new(event) });
        };
        self.event = Some(Box::new(event)); self.event_deadline_ns = Some(deadline);
        self.blocked = false; self.last_ns = now;
        Ok(())
    }
    ...
    pub fn supply_timing(&mut self, timing: HevcRecordingTiming, now: u64)
        -> std::result::Result<TimedHevcCapture, CaptureError>
    {
        self.check_live(now)?;
        if !self.timing_required { return Err(CaptureError::NoPicture); }
        let picture = self.event.as_deref().and_then(picture).ok_or(CaptureError::NoPicture)?;
        let admission = self.collector.push_picture(picture, timing, now).map_err(CaptureError::Collection)?;
        let event = self.event.take().ok_or(CaptureError::NoPicture)?;
        self.input_ended |= terminal(&event);
        self.timing_required = false; self.event_deadline_ns = None; self.last_ns = now;
    ...
    pub fn seal(&mut self, now: u64) -> std::result::Result<HevcCollectionSeal, CaptureError> {
        self.check_live(now)?;
        if self.event.as_deref().is_some_and(invalidates) { return Err(CaptureError::Backpressure); }
        let output = self.collector.seal(now).map_err(CaptureError::Collection)?;
        self.last_ns = now; self.blocked = false;
        Ok(output)
    }
    ...
    pub fn poll(&mut self, now: u64) -> std::result::Result<HevcCapturePoll, CaptureError> {
        self.check_time(now)?; self.last_ns = now;
        if self.closed { return Ok(HevcCapturePoll::Ended { retained: None }); }
        if let Some(collection) = self.collector.expire(now).map_err(CaptureError::Collection)? {
            return Ok(self.stopped(CollectionStop::Deadline, Some(CollectorError::Deadline), collection));
        }
        ...
        if self.timing_required {
            let p = self.event.as_deref().and_then(picture).ok_or(CaptureError::NoPicture)?;
            return Ok(HevcCapturePoll::TimingRequired(HevcPictureTimingRequest::new(p)));
        }
        if self.input_ended && self.event.is_none() {
            let sealed = self.collector.finish(now).map_err(CaptureError::Collection)?;
            self.finished = true;
            return Ok(HevcCapturePoll::Sealed(sealed));
        }
        let Some(event) = self.event.take() else {
            return Ok(HevcCapturePoll::Pending { wake_at_ns: self.next_wake_ns() });
        };
        if let Event::Source { source, .. } = event.as_ref()
            && let Err(error) = self.collector.push_ordered(source, now)
        {
            self.event = Some(event);
            if matches!(error, CollectorError::Capacity | CollectorError::Allocation | CollectorError::Backpressure) {
                self.blocked = true;
                return Ok(HevcCapturePoll::Backpressure { reason: error, wake_at_ns: self.next_wake_ns() });
            }
            let collection = self.collector.invalidate();
        ...
        if let Some(p) = picture(&event) {
            ...
            self.event = Some(event); self.timing_required = true;
            return Ok(HevcCapturePoll::TimingRequired(request));
        }
        self.input_ended |= terminal(&event);
        self.event_deadline_ns = None; self.blocked = false;
    ...
    pub fn cancel(&mut self) -> HevcCaptureRetirement {
        self.closed = true; self.event_deadline_ns = None; self.timing_required = false;
        HevcCaptureRetirement { collection: self.collector.cancel(), event: self.event.take() }
    }
    fn stopped(&mut self, reason: CollectionStop, error: Option<CollectorError>, collection: HevcCollectionRetirement)
        -> HevcCapturePoll
    {
        self.closed = true; self.event_deadline_ns = None; self.timing_required = false;
        HevcCapturePoll::Stopped { reason, error,
            retained: Box::new(HevcCaptureRetirement { collection, event: self.event.take() }) }
    }
    fn check_live(&self, now: u64) -> std::result::Result<(), CaptureError> {
        self.check_time(now)?;
        if self.closed || self.finished { return Err(CaptureError::Closed); }
        if self.event_deadline_ns.is_some_and(|at| now >= at) {
    ...
    fn check_time(&self, now: u64) -> std::result::Result<(), CaptureError> {
        if now < self.last_ns { Err(CaptureError::Collection(CollectorError::ClockReversed)) } else { Ok(()) }
    }
}

fn picture(event: &Event) -> Option<&HevcPictureGroup> {
    match event {
        Event::Assembly { step: HevcAssemblyStep::Accepted(output), .. } => output.picture.as_ref(),
        _ => None,
    }
}
fn terminal(event: &Event) -> bool {
    matches!(event, Event::Ended { .. } | Event::Assembly { retirement: Some(_), .. })
}
fn invalidates(event: &Event) -> bool {
    match event {
        Event::Source { gap_before, fragment, picture, .. } => *gap_before || fragment.is_some() || picture.is_some(),
        Event::CodecRefused { .. } | Event::Gap { .. } | Event::FragmentRetired { .. }
            | Event::PictureRetired(_) | Event::QueueRetired(_) | Event::Fault { .. } => true,
        ...
        Event::Ended { client, interrupted_picture, tail } => interrupted_picture.is_some()
...
fn client_invalidates(event: &HevcClientPoll) -> bool {
    match event {
        ...
        HevcClientPoll::Rtp { admission, retirement, .. } => retirement.is_some()
            || admission.discarded.is_some() || admission.transport.discarded.is_some()
            || admission.transport.disposition == ReorderDisposition::RestartRequired,
        HevcClientPoll::Ended { media, .. } => matches!(media, Some(H265ReceivePoll::Ended { discarded: Some(_) })),
        ...
        HevcClientPoll::Control { .. } | HevcClientPoll::Rtcp { .. }
            | HevcClientPoll::AuthenticationRequired { .. } | HevcClientPoll::Backpressure { .. }
            | HevcClientPoll::KeepAliveDue | HevcClientPoll::Pending { .. } => false,
    }
}
