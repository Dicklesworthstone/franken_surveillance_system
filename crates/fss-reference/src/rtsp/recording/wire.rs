...
pub(super) fn encode_packets(packets: &[RecordingPacket<'_>]) -> Result<Vec<u8>> {
    ...
    for p in packets {
        ...
        size = size.checked_add(24).and_then(|n| n.checked_add(p.bytes.len())).ok_or(RecordingError::Limit)?;
        if size > MAX_RECORDING_BYTES { return Err(RecordingError::Limit); }
        last = Some(p.sequence);
    }
    let mut e = CanonicalEncoder::new();
    e.text(SOURCE_SCHEMA); e.u64(packets.len() as u64);
    for p in packets { e.u64(p.sequence); e.u64(p.received_ns); e.bytes(p.bytes); }
    e.finish_checked().map_err(|_| RecordingError::Malformed)
}

pub(super) fn decode_packets(bytes: &[u8]) -> Result<Vec<RecordingPacket<'_>>> {
    if bytes.len() > MAX_RECORDING_BYTES { return Err(RecordingError::Limit); }
    let mut d = Decoder::new(bytes);
    if d.text()? != SOURCE_SCHEMA { return Err(RecordingError::Malformed); }
    ...
    for _ in 0..count {
        ...
        packets.push(RecordingPacket { sequence, received_ns, bytes: data });
...
pub(super) fn encode_index(index: &Index) -> Result<Vec<u8>> {
    ...
    let spans = index.mappings.iter().try_fold(0_usize, |s, m| s.checked_add(m.sources.len()))
        .ok_or(RecordingError::Limit)?;
    if spans > MAX_RECORDING_MAPPINGS { return Err(RecordingError::Limit); }
    let mut e = CanonicalEncoder::new();
    e.text(INDEX_SCHEMA); e.text(index.scope.sensor.as_str()); e.text(index.scope.stream.as_str());
    ...
    range(&mut e, &index.sps); range(&mut e, &index.pps);
    ...
    for s in &index.samples {
        ...
        e.u64(boundary_code(s.boundary)); range(&mut e, &s.mappings);
    }
    e.u64(index.mappings.len() as u64);
    for m in &index.mappings {
        e.u64(m.sample as u64); e.u64(m.nal as u64);
        match &m.target {
            NalTarget::Initialization(r) => { e.bool(false); range(&mut e, r); }
            NalTarget::Media(r) => { e.bool(true); range(&mut e, r); }
        }
        e.u64(m.sources.len() as u64);
        for s in &m.sources {
            e.u64(s.sequence); range(&mut e, &s.wire_range); range(&mut e, &s.nal_range);
            e.bool(s.fragment_header_range.is_some());
            if let Some(r) = &s.fragment_header_range { range(&mut e, r); }
        }
    }
    e.finish_checked().map_err(|_| RecordingError::Malformed)
}
...
pub(super) fn decode_index(bytes: &[u8]) -> Result<Index> {
    ...
    if bytes.len() > 2 * 1024 * 1024 { return Err(RecordingError::Limit); }
    let mut d = Decoder::new(bytes);
    if d.text()? != INDEX_SCHEMA { return Err(RecordingError::Malformed); }
    ...
    for _ in 0..count {
        samples.push(SampleMapping { range: d.range()?, decode_time: d.u64()?,
    ...
    for _ in 0..count {
        ...
        let r = d.range()?;
        let target = if in_media { NalTarget::Media(r) } else { NalTarget::Initialization(r) };
        let count = d.count(spans_left)?;
        if count == 0 { return Err(RecordingError::Source); }
        spans_left -= count;
        let mut sources = bounded_vec(count, MAX_RECORDING_MAPPINGS)?;
        for _ in 0..count {
            sources.push(NalSourceSpan { sequence: d.u64()?, wire_range: d.range()?, nal_range: d.range()?,
                fragment_header_range: if d.boolean()? { Some(d.range()?) } else { None } });
        }
        mappings.push(NalMapping { sample, nal, target, sources });
    }
    d.end()?;
    Ok(Index { scope: RecordingScope { sensor, stream, generation, anchor, receive_clock },
        ssrc, payload_type, time_scale, source, initialization, media, sps, pps, samples, mappings })
}

fn range(e: &mut CanonicalEncoder, r: &Range<usize>) {
    e.u64(r.start as u64); e.u64(r.end as u64);
}
fn boundary_code(b: AvcBoundary) -> u64 {
    match b {
        AvcBoundary::NextPrimaryPicture => 1, AvcBoundary::NextAccessUnitPrefix => 2,
        AvcBoundary::RtpMarker => 3, AvcBoundary::EndOfSequence => 4,
        AvcBoundary::EndOfStream => 5, AvcBoundary::EndOfInputUnverified => 6,
    }
}
fn decode_boundary(code: u64) -> Result<AvcBoundary> {
    match code {
        1 => Ok(AvcBoundary::NextPrimaryPicture), 2 => Ok(AvcBoundary::NextAccessUnitPrefix),
        3 => Ok(AvcBoundary::RtpMarker), 4 => Ok(AvcBoundary::EndOfSequence),
        5 => Ok(AvcBoundary::EndOfStream), 6 => Ok(AvcBoundary::EndOfInputUnverified),
        _ => Err(RecordingError::Malformed),
    }
}
struct Decoder<'a>(CanonicalDecoder<'a>);
impl<'a> Decoder<'a> {
    fn new(bytes: &'a [u8]) -> Self { Self(CanonicalDecoder::new(bytes)) }
    ...
    fn end(&mut self) -> Result<()> { self.0.ensure_finished().map_err(|_| RecordingError::Malformed) }
}
