...
use super::{
    MAX_RECORDING_BYTES, MAX_RECORDING_MAPPINGS, MAX_RECORDING_PACKETS,
    MAX_RECORDING_SAMPLES, PreparedRecording, RecordingError, RecordingObjects,
    RecordingPacket, RecordingScope, RecordingSummary, bounded_vec,
};
...
impl std::fmt::Debug for PreparedHevcRecording {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PreparedHevcRecording").field("root", &self.manifest().root())
            .field("packets", &self.summary().packets).field("samples", &self.summary().samples)
            .field("source_only_nals", &self.source_only_nals()).finish_non_exhaustive()
    }
}
...
pub fn prepare_hevc_recording(
    ...
    if scope.generation == 0 { return Err(RecordingError::Scope); }
    ...
    let packet = RtpPacket::parse(first.bytes, PacketLimits::default()).map_err(|_| RecordingError::Source)?;
    let reconstructed = replay::run(scope.generation, packet.ssrc(), packet.payload_type(),
        configuration, time_scale, timings, packets)?;
    ...
    for mapping in fragment.mappings() {
        ...
        mappings.push(HevcNalMapping { sample: mapping.sample, nal: mapping.nal,
            range: mapping.range.clone(), sources });
    }
    let index = Index {
        scope, ssrc: packet.ssrc(), payload_type: packet.payload_type(), time_scale,
        source: ContentDigest::sha256(&source),
        initialization: ContentDigest::sha256(&reconstructed.initialization),
        media: ContentDigest::sha256(fragment.bytes()), parameters: reconstructed.parameters,
        source_only_nals: reconstructed.source_only_nals, samples, mappings,
    };
    let encoded = wire::encode(&index)?;
    let manifest = manifest_for(&index, &encoded)?;
    check_total(&manifest, RecordingObjects { source: &source, initialization: &reconstructed.initialization,
        media: fragment.bytes(), index: &encoded })?;
    let summary = summary(&manifest, &index, packets.len())?;
    let plan = PreparedRecording { manifest, source, initialization: reconstructed.initialization,
        media: copy(fragment.bytes())?, index: encoded, summary };
...
pub fn verify_hevc_recording(
    ...
    for sample in &index.samples {
        ...
        timings.push(HevcRecordingTiming { decode_time: sample.decode_time, duration: sample.duration,
    ...
    let replayed = replay::run(index.scope.generation, index.ssrc, index.payload_type,
        &configuration, index.time_scale, &timings, &packets)?;
    if replayed.initialization != objects.initialization || replayed.parameters != index.parameters
        || replayed.fragment.bytes() != objects.media || replayed.fragment.samples() != index.samples
        || replayed.fragment.mappings() != index.mappings || replayed.source_only_nals != index.source_only_nals
    { return Err(RecordingError::Source); }
    Ok(HevcRecordingVerification { recording: summary(manifest, &index, packets.len())?,
        source_only_nals: index.source_only_nals })
}

fn manifest_for(index: &Index, encoded: &[u8]) -> Result<ObjectManifest> {
    ObjectManifest::new(HEVC_RECORDING_KIND, [index.source, index.initialization, index.media],
        Some(ContentDigest::sha256(encoded))).map_err(|_| RecordingError::Digest)
}
fn summary(manifest: &ObjectManifest, index: &Index, packets: usize) -> Result<RecordingSummary> {
    ...
    Ok(RecordingSummary { root: manifest.root(), scope: index.scope.clone(), packets,
        samples: index.samples.len(), nals: index.mappings.len(), time_scale: index.time_scale,
