...
pub(super) fn encode(index: &Index) -> Result<Vec<u8>> {
    if index.samples.is_empty()
        || index.samples.len() > MAX_RECORDING_SAMPLES
        || index.mappings.is_empty()
        || index.mappings.len() > MAX_RECORDING_MAPPINGS
    ...
    let spans = index
        .mappings
        .iter()
        .try_fold(0_usize, |count, m| count.checked_add(m.sources.len()))
    ...
    for m in &index.mappings {
        ...
        for source in &m.sources {
            e.u64(source.sequence);
            range(&mut e, &source.wire_range);
            range(&mut e, &source.nal_range);
            e.bool(source.fragment_header_range.is_some());
            if let Some(r) = &source.fragment_header_range {
                range(&mut e, r);
            }
        }
    }
...
pub(super) fn decode(bytes: &[u8]) -> Result<Index> {
    ...
    for _ in 0..count {
        samples.push(HevcSampleMapping {
            range: d.range()?,
            decode_time: d.u64()?,
...
fn decode_boundary(code: u64) -> Result<HevcBoundary> {
    match code {
        1 => Ok(HevcBoundary::NextFirstSlice),
        2 => Ok(HevcBoundary::NextAccessUnitPrefix),
        3 => Ok(HevcBoundary::AccessUnitDelimiter),
        4 => Ok(HevcBoundary::EndOfSequence),
        5 => Ok(HevcBoundary::EndOfBitstream),
        6 => Ok(HevcBoundary::EndOfInputUnverified),
