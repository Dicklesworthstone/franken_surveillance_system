...
use fss_packet::avc::{
    AvcBoundary, AvcSliceIdentity, AvcSyntaxLimits, parse_pps, parse_slice_identity, parse_sps,
};
...
pub(super) fn content(
    ...
    let sps_bytes = objects
        .initialization
        .get(index.sps.clone())
        .ok_or(RecordingError::Malformed)?;
    let pps_bytes = objects
        .initialization
        .get(index.pps.clone())
        .ok_or(RecordingError::Malformed)?;
    ...
    if mux.initialization().bytes() != objects.initialization
        || mux.initialization().sps_range() != index.sps
        || mux.initialization().pps_range() != index.pps
    {
    ...
    for p in packets {
        let packet = RtpPacket::parse(p.bytes, PacketLimits::default())
            .map_err(|_| RecordingError::Source)?;
        let output = depacketizer
            .push(key(index), p.sequence, packet, 0)
            .map_err(|_| RecordingError::Source)?;
        if output.discarded.is_some() {
        ...
        for nal in output.nals {
            ...
            if mapping.sources != nal.sources() || bytes != nal.bytes() {
                return Err(RecordingError::Source);
            }
            if mapping.sample != active_sample {
                ...
                saw_first = false;
                active_sample = mapping.sample;
            }
            if matches!(nal.nal_type(), 1 | 5) {
                let id = parse_slice_identity(bytes, &sps, &pps, syntax)
                    .map_err(|_| RecordingError::Source)?;
                let sample = &index.samples[mapping.sample]; // check_layout proved the index.
                if id.redundant_pic_cnt() != 0
                    || id.field_pic()
                    || id.idr_pic_id().is_some() != sample.idr
                    || nal.timestamp() != sample.rtp_timestamp
                    || sample_identity.is_some_and(|previous| id.starts_new_picture(previous))
                {
                    return Err(RecordingError::Source);
                }
                sample_identity = Some(id);
                saw_first |= id.first_mb_in_slice() == 0;
    ...
    if depacketizer.finish().is_some()
        || ordinal != index.mappings.len()
        || sample_identity.is_none()
        || !saw_first
        || active_sample + 1 != index.samples.len()
    {
        return Err(RecordingError::Source);
    }
    Ok(())
...
fn check_layout(index: &Index, objects: RecordingObjects<'_>) -> Result<()> {
    ...
    if count == 0 || !index.samples[0].idr {
        return Err(RecordingError::Malformed);
    }
    let offset = 96 + 16 * count;
    if objects.media.len() < offset {
        return Err(RecordingError::Malformed);
    }
    ...
    box_header(&mut header, 16, b"mfhd");
    u32be(&mut header, 0);
    u32be(&mut header, 1);
    box_header(&mut header, 64 + 16 * count, b"traf");
    box_header(&mut header, 16, b"tfhd");
    u32be(&mut header, 0x0002_0000);
    u32be(&mut header, 1);
    box_header(&mut header, 20, b"tfdt");
    u32be(&mut header, 0x0100_0000);
    header.extend_from_slice(&index.samples[0].decode_time.to_be_bytes());
    box_header(&mut header, 20 + 16 * count, b"trun");
    u32be(&mut header, 0x0100_0f01);
    u32be(&mut header, count as u32);
    u32be(&mut header, offset as u32);
    ...
    for (sample_index, s) in index.samples.iter().enumerate() {
        if s.duration == 0
            ...
            || s.range.end <= cursor
            || s.range.end > objects.media.len()
            || s.mappings.start != map_cursor
            || s.mappings.end <= map_cursor
            || s.mappings.end > index.mappings.len()
            || s.boundary == AvcBoundary::EndOfInputUnverified
        {
            return Err(RecordingError::Malformed);
        }
        end = end
            .checked_add(u64::from(s.duration))
            .ok_or(RecordingError::Malformed)?;
        ...
        u32be(&mut header, s.duration);
        u32be(&mut header, s.range.len() as u32);
        ...
        for (nal_index, m) in index.mappings[s.mappings.clone()].iter().enumerate() {
            if m.sample != sample_index || m.nal != nal_index {
                return Err(RecordingError::Malformed);
            }
            let bytes = target(m, objects)?;
            match &m.target {
                NalTarget::Initialization(r) => {
                    if !((bytes[0] & 31 == 7 && r == &index.sps)
                        || (bytes[0] & 31 == 8 && r == &index.pps))
                    {
                        return Err(RecordingError::Malformed);
                    }
                }
                NalTarget::Media(r) => {
                    if matches!(bytes[0] & 31, 7 | 8)
                        || r.start != cursor + 4
                        || r.end > s.range.end
                    {
                        return Err(RecordingError::Malformed);
                    ...
                    let prefix = objects
                        ...
                        .get(cursor..r.start)
                        .ok_or(RecordingError::Malformed)?;
                    if prefix != (bytes.len() as u32).to_be_bytes() {
                        return Err(RecordingError::Malformed);
                    }
                    cursor = r.end;
                }
    ...
    if cursor != objects.media.len() || map_cursor != index.mappings.len() {
        return Err(RecordingError::Malformed);
    }
    box_header(&mut header, objects.media.len() - offset + 8, b"mdat");
    if header != objects.media[..offset] {
        return Err(RecordingError::Malformed);
    }
    Ok(())
}
