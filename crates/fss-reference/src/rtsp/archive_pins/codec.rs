...
pub(super) fn validate_scope(scope: ArchivePinScope) -> PinResult<()> {
    if !digest_ok(scope.journal_id) || !digest_ok(scope.archive_namespace) {
        return Err(ArchivePinError::Scope);
    }
    Ok(())
}
pub(super) fn validate_pin(pin: &StoredArchivePin) -> PinResult<()> {
    if !digest_ok(pin.root)
        || !digest_ok(pin.retirement)
        || pin.new_payload_bytes == 0
        || pin.new_payload_bytes > MAX_ARCHIVE_WORK_BYTES
    {
    ...
    if pin.slot.as_str() != format!("fssaw1-{hex}-r") {
        return Err(ArchivePinError::History);
    }
    Ok(())
}
pub(super) fn encode(
    ...
    tag: u8,
    pin: Option<&StoredArchivePin>,
) -> PinResult<Vec<u8>> {
    validate_scope(scope)?;
    if tag > 2 || (tag == 0) != pin.is_none() {
        return Err(ArchivePinError::History);
    }
    let mut e = CanonicalEncoder::new();
    e.text(DOMAIN);
    e.digest(scope.journal_id);
    e.digest(scope.archive_namespace);
    e.tag(tag);
    if let Some(pin) = pin {
        validate_pin(pin)?;
        e.text(pin.slot.as_str());
        e.digest(pin.root);
        e.digest(pin.retirement);
        e.u64(pin.new_payload_bytes as u64);
    }
...
fn decode(bytes: &[u8], expected: ArchivePinScope) -> PinResult<(u8, Option<StoredArchivePin>)> {
    ...
    let pin = fields
        .map(|(slot, root, retirement, count)| -> PinResult<_> {
            Ok(StoredArchivePin {
                slot: SlotName::parse(&slot).map_err(|_| ArchivePinError::History)?,
                root,
                retirement,
                new_payload_bytes: usize::try_from(count).map_err(|_| ArchivePinError::Limit)?,
            })
        })
        .transpose()?;
    if encode(scope, tag, pin.as_ref())? != bytes {
        return Err(ArchivePinError::History);
    }
    Ok((tag, pin))
}
pub(super) fn apply(state: &mut ReplayState, tag: u8, pin: StoredArchivePin) -> PinResult<()> {
    ...
    match tag {
        1 => {
            if state.pins.candidate.is_some()
                || state
                    .seen
                    .iter()
                    .any(|(root, retirement)| *root == pin.root || *retirement == pin.retirement)
            {
                return Err(ArchivePinError::History);
            }
            state
        ...
        2 if state.pins.candidate.as_ref() == Some(&pin) => {
            state.pins.confirmed = Some(pin);
            state.pins.candidate = None;
        }
