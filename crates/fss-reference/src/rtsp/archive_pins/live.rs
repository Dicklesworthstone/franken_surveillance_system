...
use crate::rtsp::live_archive::checkpointed::{
    CheckpointedLiveArchiveFailure, CheckpointedLiveArchiveRetirement, CheckpointedLiveArchiveStep,
    CheckpointedLiveAvcArchive,
};
use crate::rtsp::live_archive::{LiveArchiveConfig, LiveArchiveError, LiveArchiveStep};
use crate::rtsp::live_avc::recording::{
    LiveRecordingConfig, LiveRecordingRetirement, LiveRecordingStep,
};
...
impl fmt::Display for JournaledLiveArchiveError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Live(e) => fmt::Display::fmt(e, f),
            Self::Pins(e) => fmt::Display::fmt(e, f),
        }
    }
}
...
impl fmt::Debug for JournaledLiveAvcArchive<'_, '_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("JournaledLiveAvcArchive")
...
impl<'a, 'p> JournaledLiveAvcArchive<'a, 'p> {
    ...
    pub fn connect(
        ...
        if pins.scope().archive_namespace != archive.namespace.digest() {
            return Err(JournaledLiveConnectFailure {
                reason: JournaledLiveArchiveError::Pins(ArchivePinError::Scope),
                connection_attempted: false,
            });
        }
        pins.require_settled(publisher, work, cancel)
            .map_err(|e| JournaledLiveConnectFailure {
                reason: JournaledLiveArchiveError::Pins(e),
                connection_attempted: false,
            })?;
        let inner = CheckpointedLiveAvcArchive::connect(
            live, recording, archive, work, publisher, now, authority, cancel,
        )
        .map_err(|e| JournaledLiveConnectFailure {
            reason: JournaledLiveArchiveError::Live(e.reason),
    ...
    pub fn next_wake_ns(&self) -> Option<u64> {
        if self.finished {
            ...
            self.inner.as_ref().and_then(|i| i.next_wake_ns())
        }
    }
    /// Last acknowledged independent journal prefix.
    pub fn journal_anchor(&self) -> ArchivePinAnchor {
        self.pins.anchor()
    }
    /// Read-only independent candidate/predecessor state.
    pub fn pin_state(&self) -> &ArchivePinState {
        self.pins.state()
    }
    ...
    pub fn request(
        ...
        authority: &dyn TcpAuthority,
        cancel: &dyn PublishCancellation,
    ) -> Result<QueuedAvcRequest, JournaledLiveFailure> {
        self.admit(now, cancel)?;
        let result = self.inner.as_mut().ok_or_else(closed)?.request(
            command,
            credentials,
            cnonce,
            now,
    ...
    pub fn supply_timing(
        ...
        authority: &dyn TcpAuthority,
        cancel: &dyn PublishCancellation,
    ) -> Result<TimedCapture, JournaledLiveFailure> {
        self.admit(now, cancel)?;
        let result = self
            .inner
            .as_mut()
    ...
    pub fn seal(
        ...
        authority: &dyn TcpAuthority,
        cancel: &dyn PublishCancellation,
    ) -> Result<bool, JournaledLiveFailure> {
        self.admit(now, cancel)?;
    ...
    pub fn poll(
        ...
        authority: &dyn TcpAuthority,
        cancel: &dyn PublishCancellation,
    ) -> Result<JournaledLiveArchiveStep, JournaledLiveFailure> {
        if self.finished || self.inner.is_none() {
            return Ok(JournaledLiveArchiveStep::Live(LiveArchiveStep::Ended));
        }
        self.admit(now, cancel)?;
        let step = self
            .inner
        ...
        match step {
            CheckpointedLiveArchiveStep::Checkpoint(CheckpointedArchiveProgress::PinRequired(
                ...
                let receipt = self
                    .pins
                    .persist_candidate(&checkpoint, cancel)
                    .map_err(|e| self.pin_failure(e, None))?;
                self.inner
                    .as_mut()
                    .ok_or_else(closed)?
                    .acknowledge_checkpoint(&checkpoint, now, authority, cancel)
                    .map_err(|e| self.live_failure(e))?;
                Ok(JournaledLiveArchiveStep::Checkpoint(Box::new(
                    JournaledArchiveProgress::PinPersisted {
                        checkpoint,
                        receipt,
                    },
                )))
            ...
            CheckpointedLiveArchiveStep::Checkpoint(CheckpointedArchiveProgress::WorkDurable {
                ...
                Ok(pin_receipt) => Ok(JournaledLiveArchiveStep::Checkpoint(Box::new(
                    JournaledArchiveProgress::WorkConfirmed {
                        checkpoint,
                        work_receipt: receipt,
                        pin_receipt,
                    },
                ))),
                Err(e) => Err(self.pin_failure(
                    e,
                    Some(UnrecordedWorkConfirmation {
                        checkpoint,
                        receipt,
                    }),
                )),
            },
            CheckpointedLiveArchiveStep::Checkpoint(CheckpointedArchiveProgress::Archive(
                progress,
            )) => Ok(JournaledLiveArchiveStep::Live(LiveArchiveStep::Archive(
                progress,
            ))),
            CheckpointedLiveArchiveStep::Live(step) => {
                if matches!(
                    &step,
                    LiveArchiveStep::Finished { .. } | LiveArchiveStep::Ended
                ) {
                    self.finished = true;
                }
            ...
            CheckpointedLiveArchiveStep::Stopped { trigger, retained } => {
                self.finished = true;
                self.inner = None;
                Ok(JournaledLiveArchiveStep::Stopped {
                    trigger,
                    retained: Box::new(self.retirement(*retained, None)),
                })
            }
        }
    }
    ...
    pub fn cancel(&mut self) -> Option<JournaledLiveRetirement> {
        ...
        self.inner
            .take()
            .and_then(|mut inner| inner.cancel())
            .map(|r| self.retirement(r, None))
    }
    fn admit(
        &mut self,
        now: u64,
        ...
        if now < self.last_ns {
            return Err(JournaledLiveFailure {
                reason: JournaledLiveArchiveError::Live(LiveArchiveError::ClockReversed),
                retirement: None,
            });
        }
        self.last_ns = now;
    ...
    fn retirement(
        &self,
        live: CheckpointedLiveArchiveRetirement,
        unrecorded: Option<UnrecordedWorkConfirmation>,
    ) -> JournaledLiveRetirement {
        JournaledLiveRetirement {
            live,
            unrecorded_confirmation: unrecorded,
            journal_anchor: self.pins.anchor(),
            pins: self.pins.state().clone(),
        }
    }
    fn pin_failure(
        &mut self,
        error: ArchivePinError,
        unrecorded: Option<UnrecordedWorkConfirmation>,
    ) -> JournaledLiveFailure {
        self.finished = true;
        let retirement = self
            .inner
            .take()
        ...
        JournaledLiveFailure {
            reason: JournaledLiveArchiveError::Pins(error),
            retirement,
        }
    }
    fn live_failure(&mut self, error: CheckpointedLiveArchiveFailure) -> JournaledLiveFailure {
        let retirement = error.retirement.map(|r| {
            self.finished = true;
            self.inner = None;
            Box::new(self.retirement(*r, None))
        });
        JournaledLiveFailure {
            reason: JournaledLiveArchiveError::Live(error.reason),
            retirement,
        }
    }
}
fn closed() -> JournaledLiveFailure {
    JournaledLiveFailure {
        reason: JournaledLiveArchiveError::Live(LiveArchiveError::Closed),
        retirement: None,
    }
}
