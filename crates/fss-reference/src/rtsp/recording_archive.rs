...
mod codec {
    ...
    use crate::rtsp::recording_catalog::{
        CatalogBuilder,
        hevc::{HevcCatalogBuilder, HevcRecordingCatalog},
    };
    ...
    pub trait ArchiveCodec: sealed::Sealed + std::fmt::Debug {
        ...
        fn load_recording(
            ...
            slot: &SlotName,
            root: ContentDigest,
            scope: &super::super::recording::RecordingScope,
    ...
    impl ArchiveCodec for AvcArchiveCodec {
        ...
        fn push(
            ...
            Ok(builder.push(slot, recording)?)
        }
        fn prepare(builder: Self::Builder) -> ArchiveResult<Self::Catalog> {
            Ok(builder.prepare()?)
        }
    ...
    impl ArchiveCodec for HevcArchiveCodec {
        ...
        fn builder(scope: CatalogScope) -> ArchiveResult<Self::Builder> {
            Ok(HevcCatalogBuilder::new(scope)?)
        }
        fn push(
            builder: &mut Self::Builder,
            slot: &SlotName,
...
use super::recording_catalog::{
    CatalogEntry, CatalogError, CatalogScope, MAX_CATALOG_WINDOWS, RecordingCatalog,
};
use fss_core::{CanonicalEncoder, ContentDigest};
use fss_publication::{
    LocalPublicationState, LocalRootPublisher, PublishCancellation, PublishCutPoint, SlotName,
};
...
impl From<CatalogError> for ArchiveError {
    fn from(e: CatalogError) -> Self {
        Self::Catalog(e)
    }
}
impl From<CatalogIoError> for ArchiveError {
    fn from(e: CatalogIoError) -> Self {
        Self::CatalogIo(e)
    }
}
impl From<RecordingIoError> for ArchiveError {
...
impl<C: ArchiveCodec> CodecArchiveNamespace<C> {
    ...
    pub fn new(scope: CatalogScope) -> ArchiveResult<Self> {
        ...
        Ok(Self {
            scope,
            digest,
            prefix: format!("{}-{hex}", C::SLOT_PREFIX),
    ...
    pub fn window_slot(&self, ordinal: usize) -> ArchiveResult<SlotName> {
        self.slot('w', ordinal)
    }
    /// Bounded deterministic slot for a page's first recording ordinal.
    pub fn page_slot(&self, first: usize) -> ArchiveResult<SlotName> {
        self.slot('c', first)
    }
    fn slot(&self, role: char, ordinal: usize) -> ArchiveResult<SlotName> {
        if ordinal >= MAX_ARCHIVE_WINDOWS {
            return Err(ArchiveError::Limit);
        }
        SlotName::parse(&format!("{}-{role}-{ordinal:016x}", self.prefix))
            .map_err(|_| ArchiveError::Sequence)
    }
    fn parse_slot(&self, slot: &SlotName) -> ArchiveResult<Option<(char, usize)>> {
        let Some(rest) = slot.as_str().strip_prefix(&self.prefix) else {
            return Ok(None);
        };
        let (role, value) = rest
            .strip_prefix('-')
            .and_then(|s| s.split_once('-'))
            .ok_or(ArchiveError::Sequence)?;
        if !matches!(role, "w" | "c")
            || value.len() != 16
            || !value
                .bytes()
...
impl<C: ArchiveCodec> CodecArchiveSnapshot<C> {
    ...
    pub fn load(
        ...
        for (n, slot) in p.broken_slots().enumerate() {
            ...
            if slot.as_str().starts_with(&namespace.prefix) {
        ...
        for (n, broken) in report.broken_roots.iter().enumerate() {
            scan_admit(n, limits, cancel)?;
            if broken.path.file_name().is_some_and(|s| {
                s.as_encoded_bytes()
                    .starts_with(namespace.prefix.as_bytes())
            }) {
                return Err(ArchiveError::RecoveryRequired);
            }
        }
        ...
        for (n, root) in p.visible_roots().enumerate() {
            scan_admit(n, limits, cancel)?;
            let Some((role, ordinal)) = namespace.parse_slot(&root.slot)? else {
                continue;
            };
            if root.state != LocalPublicationState::Durable {
                return Err(RecordingIoError::NotDurable.into());
            }
            let (target, maximum) = if role == 'w' {
                (&mut roots, limits.max_windows)
            } else {
                (&mut page_roots, limits.max_pages)
            };
        ...
        page_roots.sort_by_key(|r| r.0);
        if roots.iter().enumerate().any(|(n, r)| n != r.0) {
            return Err(ArchiveError::Sequence);
        }
        ...
        if roots.len() - indexed > MAX_CATALOG_WINDOWS {
            return Err(ArchiveError::Sequence);
        }
        ...
        for (ordinal, slot, root) in roots {
            ...
            if windows
                .last()
                .is_some_and(|last| last.decode_interval().end > entry.decode_interval().start)
                || windows.iter().any(|old| old.root() == entry.root())
            {
                return Err(ArchiveError::Sequence);
            }
            if ordinal < indexed {
                while ordinal >= pages[page_at].first + C::entries(&pages[page_at].catalog).len() {
                    page_at += 1;
                }
                if entry != C::entries(&pages[page_at].catalog)[ordinal - pages[page_at].first] {
                    return Err(ArchiveError::Metadata);
                }
            }
            windows.push(entry);
    ...
    pub fn digest(&self) -> ArchiveResult<ContentDigest> {
        let mut e = CanonicalEncoder::new();
        e.text(C::SNAPSHOT_DOMAIN);
        e.digest(self.namespace.digest);
        e.u64(self.windows.len() as u64);
        e.u64(self.indexed as u64);
        for entry in &self.windows {
            e.digest(entry.root());
        }
        e.u64(self.pages.len() as u64);
        for page in &self.pages {
            e.u64(page.first as u64);
            e.digest(C::manifest(&page.catalog).root());
        }
...
fn descriptor_for<C: ArchiveCodec>(
    scope: &CatalogScope,
    slot: &SlotName,
...
fn verify_descriptor_for<C: ArchiveCodec>(
    scope: &CatalogScope,
    entry: &CatalogEntry,
    recording: &C::Recording,
) -> ArchiveResult<()> {
    if &descriptor_for::<C>(scope, entry.slot(), recording)? != entry {
        return Err(ArchiveError::Metadata);
    }
    Ok(())
}
