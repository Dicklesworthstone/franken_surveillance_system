...
use super::recording_archive::{
    ArchiveAdmission, ArchiveError, ArchiveNamespace, ArchiveRetirement, ArchiveSnapshot,
...
pub fn archive_retirement_digest(
    retired: &ArchiveRetirement,
) -> Result<ContentDigest, ArchiveError> {
    ...
    if let Some(pending) = &retired.pending {
        e.digest(pending.manifest().root());
    }
    e.bool(retired.prepared_page.is_some());
    if let Some(page) = &retired.prepared_page {
...
impl fmt::Debug for ArchiveResumeRefusal {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("ArchiveResumeRefusal")
    }
}
impl fmt::Display for ArchiveResumeRefusal {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("archive resume preflight refused")
    }
}
...
impl fmt::Debug for ArchiveResumeRetirement {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ArchiveResumeRetirement")
            ...
            .field("expected_page", &self.expected_page.is_some())
...
impl ArchiveResumeRetirement {
    ...
    pub fn into_retry(self) -> Result<ArchiveRetirement, Box<Self>> {
        if self.unoffered_window.is_some() && self.archive.pending.is_some() {
...
impl fmt::Debug for RecordingArchiveResume<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("RecordingArchiveResume")
            .field("blocked", &self.blocked)
            .field("unoffered", &self.unoffered.is_some())
            .finish_non_exhaustive()
    }
}
impl<'a> RecordingArchiveResume<'a> {
    ...
    pub fn open(
        ...
        let (writer, reconciliation) = match preflight {
            Ok(values) => values,
            Err(reason) => {
                return Err(ArchiveResumeRefusal {
                    reason,
        ...
        let unoffered = if matches!(
            reconciliation.window,
            RetiredPublicationState::NotPublished(_)
        ) {
            retired.pending
        } else {
            None
        };
        let expected_page = if matches!(
            reconciliation.page,
            RetiredPublicationState::NotPublished(_)
        ) {
    ...
    pub fn reconciliation(&self) -> ArchiveReconciliation {
        self.reconciliation
    }
    ...
    pub fn pending(&self) -> Option<&PreparedRecording> {
        self.unoffered.as_ref().or(self.writer.pending())
    }
    ...
    pub fn step(
        ...
        if self.phase == Phase::Finished {
            return Ok(ArchiveResumeProgress::Ended);
        }
        if self.blocked {
        ...
        if now >= self.config.deadline_ns {
            self.blocked = true;
    ...
    fn advance(
        ...
        if self.phase == Phase::Offer {
            ...
            match self.writer.offer(window, self.config.max_window_bytes, now) {
                ...
                Err(refusal) => {
                    self.unoffered = Some(*refusal.recording);
        ...
        {
            if *root != expected.manifest().root() {
        ...
        if matches!(&progress, ArchiveWriteProgress::Ready { .. }) {
            if self.phase != Phase::InitialDrain || self.expected_page.is_some() {
                return Err(ArchiveError::Metadata);
            }
            if self.unoffered.is_some() {
    ...
    pub fn retire(self) -> ArchiveResumeRetirement {
        ArchiveResumeRetirement {
            archive: self.writer.retire(),
            unoffered_window: self.unoffered,
...
fn prepare<'a>(
    ...
    if now >= config.deadline_ns {
        return Err(ArchiveError::Deadline);
    }
    if config.max_steps == 0
        || config.max_window_bytes > MAX_RECORDING_BYTES
        || retired
            .pending
            .as_ref()
            .is_some_and(|w| w.byte_len() > config.max_window_bytes)
    {
        return Err(ArchiveError::Limit);
    }
    let digest = archive_retirement_digest(retired)?;
    if digest != config.expected_retirement_digest {
        return Err(ArchiveError::Metadata);
    }
    ...
    let old_count = old.windows().len();
    let page_count = old.pages().len();
    if current.windows().len() < old_count
        || current.windows().len() > old_count + usize::from(retired.pending.is_some())
        || current.pages().len() < page_count
        || current.pages().len() > page_count + usize::from(retired.prepared_page.is_some())
    ...
    for (prior, recovered) in old.pages().iter().zip(current.pages()) {
        if prior.slot() != recovered.slot()
            || prior.first_ordinal() != recovered.first_ordinal()
            || prior.catalog().manifest() != recovered.catalog().manifest()
            || prior.catalog().index_bytes() != recovered.catalog().index_bytes()
        {
            return Err(ArchiveError::Metadata);
        }
    }
    ...
    if retired.pending.is_some()
        && retired.prepared_page.is_some()
        && current.windows().len() > old_count
        && current.pages().len() == page_count
    {
        return Err(ArchiveError::Metadata);
    }
    ...
    let page = match &retired.prepared_page {
        None => RetiredPublicationState::NotPending,
        Some(page) => {
            if page.scope() != old.namespace().scope()
            ...
            if let Some(recovered) = current.pages().get(page_count) {
                if recovered.first_ordinal() != old.indexed_windows()
                    ...
                    || recovered.catalog().index_bytes() != page.index_bytes()
                {
                    return Err(ArchiveError::Metadata);
                }
                RetiredPublicationState::AlreadyDurable(root)
    ...
    let reconciliation = ArchiveReconciliation {
        retirement_digest: digest,
        recovered_snapshot_digest: current.digest()?,
        window,
        page,
    };
