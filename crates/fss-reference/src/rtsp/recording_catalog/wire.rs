...
pub(super) fn encode(
    ...
    out.text(family.domain());
    out.u64(VERSION);
    out.text(scope.recording.sensor.as_str());
    out.text(scope.recording.stream.as_str());
    out.u64(scope.recording.generation);
    out.digest(scope.recording.anchor);
    out.digest(scope.recording.receive_clock);
    out.digest(scope.decode_clock);
    out.u32(scope.time_scale);
    out.u64(entries.len() as u64);
    for e in entries {
        out.text(e.slot.as_str());
        out.digest(e.root);
        out.u64(e.interval.start);
        out.u64(e.interval.end);
        out.u64(e.packets as u64);
        out.u64(e.samples as u64);
        out.u64(e.nals as u64);
        out.u64(e.bytes as u64);
        for d in e.objects {
            out.digest(d);
        }
    }
    let mut bytes = out.finish_checked().map_err(|_| CatalogError::Limit)?;
    if bytes.len() > MAX_CATALOG_BYTES - CHECKSUM_BYTES {
...
pub(super) fn decode(
    bytes: &[u8],
    family: CatalogFamily,
) -> Result<(CatalogScope, Vec<CatalogEntry>)> {
    ...
    if trailer.digest().map_err(bad)? != digest(&bytes[..body_end])? {
        return Err(CatalogError::Digest);
    }
    trailer.ensure_finished().map_err(bad)?;
    let mut d = CanonicalDecoder::new(&bytes[..body_end]);
    if d.text().map_err(bad)? != family.domain() || d.u64().map_err(bad)? != VERSION {
    ...
    if count == 0 || count > MAX_CATALOG_WINDOWS {
        return Err(CatalogError::Limit);
    }
    ...
    for _ in 0..count {
        ...
        let start = d.u64().map_err(bad)?;
        let end = d.u64().map_err(bad)?;
        let packets = count_field(&mut d)?;
        let samples = count_field(&mut d)?;
        let nals = count_field(&mut d)?;
        let bytes = count_field(&mut d)?;
        let objects = [
            d.digest().map_err(bad)?,
            d.digest().map_err(bad)?,
            d.digest().map_err(bad)?,
            d.digest().map_err(bad)?,
        ];
...
fn bad(_: ContractError) -> CatalogError {
