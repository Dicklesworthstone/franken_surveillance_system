...
impl TcpBinding {
    ...
    pub fn security(&self) -> TcpSecurityPolicy {
        self.security
    }
}
impl fmt::Debug for TcpBinding {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("TcpBinding")
            .field("key", &self.key)
...
impl Default for TcpLimits {
    fn default() -> Self {
        Self {
            max_received_bytes: 1024 * 1024 * 1024,
            max_sent_bytes: 1024 * 1024,
            max_io_calls: 1_000_000,
            max_request_bytes: 32 * 1024,
            chunk_bytes: MAX_WIRE_CHUNK,
...
impl TcpLimits {
    ...
    pub fn validate(self) -> Result<(), TcpError> {
        if self.max_received_bytes == 0
            || self.max_sent_bytes == 0
            || self.max_io_calls == 0
...
impl fmt::Debug for TcpReadChunk {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("TcpReadChunk")
            .field("bytes", &self.bytes.len())
            .field("admitted_ns", &self.admitted_ns)
            .finish()
    }
}
...
#[derive(Debug)]
#[must_use]
pub struct TcpRequestRefusal {
    /// Typed queue refusal.
    pub reason: TcpError,
    /// Original request, including any borrowed-owner Digest result.
...
pub enum TcpWriteStep {
    ...
    Advanced {
        ...
        cseq: u32,
        /// Total bytes sent from this request so far.
        sent: usize,
        /// Complete request size.
        total: usize,
    },
    ...
    Sent {
        /// Correlation sequence.
        cseq: u32,
        /// Complete request byte count.
        bytes: usize,
    },
}
/// One nonblocking read attempt; bytes stay retained until explicitly acknowledged.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum TcpReadStep {
    /// Wait for readability or a timer; no bytes were read.
    Pending,
    /// Exact bytes are available through pending_read().
    Buffered,
    /// Zero-byte read; the socket is released, and accepted protocol input must drain.
    Eof,
}
...
impl fmt::Debug for RtspTcpLink {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("RtspTcpLink")
            .field("binding", &self.binding)
            .field("totals", &self.totals)
            .field("closed", &self.socket.is_none())
...
impl RtspTcpLink {
    ...
    pub fn connect(
    ...
    ) -> Result<Self, TcpConnectFailure> {
        let fail = |reason, attempted| TcpConnectFailure {
            reason,
            connection_attempted: attempted,
        };
        limits.validate().map_err(|e| fail(e, false))?;
        if now_ns >= deadline_ns {
            return Err(fail(TcpError::Deadline, false));
        }
        authority
            .checkpoint(&binding, TcpOperation::Connect, now_ns, deadline_ns)
            .map_err(|e| fail(TcpError::Denied(e), false))?;
        let timeout = Duration::from_nanos(limits.connect_timeout_ns.min(deadline_ns - now_ns));
        let socket = TcpStream::connect_timeout(&binding.peer, timeout).map_err(|e| {
            fail(
                TcpError::Io {
        ...
        authority
            .checkpoint(&binding, TcpOperation::Configure, now_ns, deadline_ns)
            .map_err(|e| fail(TcpError::Denied(e), true))?;
        if socket.peer_addr().map_err(|e| {
            fail(
                TcpError::Io {
                    operation: TcpOperation::Configure,
                    kind: e.kind(),
                },
                true,
            )
        ...
        authority
            .checkpoint(&binding, TcpOperation::Poll, now_ns, deadline_ns)
            .map_err(|e| fail(TcpError::Denied(e), true))?;
        Ok(Self {
    ...
    pub fn pending_read(&self) -> Option<&TcpReadChunk> {
        self.read.as_ref()
    }
    ...
    pub fn acknowledge_read(&mut self) -> Option<TcpReadChunk> {
        self.read.take()
    }
    ...
    pub fn next_deadline_ns(&self) -> Option<u64> {
        ...
        Some(
            self.read
                .as_ref()
                .map_or(self.deadline_ns, |r| r.deadline_ns.min(self.deadline_ns)),
        )
    }
    ...
    pub fn queue_request(
        ...
        let result = self
            .admit(now, TcpOperation::QueueRequest, authority)
            .and_then(|()| {
                if self.request.is_some() {
                    return Err(TcpError::Backpressure);
                }
                if request.bytes().len() > self.limits.max_request_bytes
                    || request.bytes().len() as u64
                        > self.limits.max_sent_bytes - self.totals.sent_bytes
                {
        ...
        if let Err(reason) = result {
            return Err(TcpRequestRefusal { reason, request });
        }
    ...
    pub fn write_step(
        ...
        now: u64,
        authority: &dyn TcpAuthority,
    ) -> Result<TcpWriteStep, TcpError> {
        self.admit(now, TcpOperation::Write, authority)?;
        if self.request.is_none() {
            return Ok(TcpWriteStep::Idle);
        }
        self.reserve_call()?;
        ...
        let end = pending
            ...
            .bytes()
            .len()
            .min(pending.sent + self.limits.chunk_bytes);
        self.totals.write_calls += 1;
        let result = self
            .socket
            .as_mut()
            .ok_or(TcpError::Closed)?
            .write(&pending.request.bytes()[pending.sent..end]);
        match result {
            ...
            Ok(n) => {
                ...
                if let Err(e) =
                    authority.checkpoint(&self.binding, TcpOperation::Write, now, self.deadline_ns)
                {
                ...
                if complete {
                    ...
                    Ok(TcpWriteStep::Advanced { cseq, sent, total })
                }
            }
            ...
            Err(e) if e.kind() == io::ErrorKind::Interrupted => {
                ...
                if self.interrupted_write >= 8 {
                    self.stop(TcpError::Interrupted)
                } else {
                    Ok(TcpWriteStep::Pending)
                }
            }
            Err(e) => self.stop(TcpError::Io {
                operation: TcpOperation::Write,
                kind: e.kind(),
            }),
        }
    }
    ...
    pub fn read_step(
        ...
        if remaining == 0 {
            return self.stop(TcpError::WorkBudget);
        }
        let length = remaining.min(self.limits.chunk_bytes as u64) as usize;
        let Some(deadline_ns) = now.checked_add(WIRE_LIFETIME_NS) else {
            return self.stop(TcpError::Exhausted);
        };
            ...
            .read(&mut bytes)
        {
            Ok(0) => {
                ...
                self.socket = None;
                authority
                    .checkpoint(&self.binding, TcpOperation::Read, now, self.deadline_ns)
    ...
    fn admit(
        ...
        if now < self.last_ns {
            return Err(TcpError::ClockReversed);
        }
        ...
        if now >= self.deadline_ns {
            return self.stop(TcpError::Deadline);
        }
        if self.read.as_ref().is_some_and(|r| now >= r.deadline_ns) {
            return self.stop(TcpError::ReadDeadline);
        }
    ...
    fn reserve_call(&mut self) -> Result<(), TcpError> {
        if self
            .totals
            .read_calls
            .checked_add(self.totals.write_calls)
            .is_none_or(|n| n >= self.limits.max_io_calls)
        {
            return self.stop(TcpError::WorkBudget);
        }
        Ok(())
