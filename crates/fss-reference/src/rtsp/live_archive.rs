...
impl fmt::Debug for LiveArchiveRetirement {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("LiveArchiveRetirement")
            ...
            .field("unoffered_window", &self.unoffered_window.is_some()).finish()
    }
}
...
impl fmt::Debug for LiveAvcArchive<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("LiveAvcArchive").field("ready", &self.ready)
            .field("completion", &self.completion).field("finished", &self.finished)
...
impl<'a> LiveAvcArchive<'a> {
    ...
    pub fn connect(live: LiveAvcConfig, recording: LiveRecordingConfig, archive: LiveArchiveConfig,
        publisher: &'a mut LocalRootPublisher, now: u64, authority: &dyn TcpAuthority,
        cancel: &dyn PublishCancellation) -> Result<Self, LiveArchiveConnectFailure>
    {
        let refused = |reason| LiveArchiveConnectFailure { reason, connection_attempted: false };
        if archive.namespace.scope().recording != recording.scope
            ...
            || recording.limits.validate().is_err() || recording.payload_type > 127
            || archive.limits.validate().is_err() || archive.max_steps == 0
            || !(1..=MAX_RECORDING_BYTES).contains(&archive.max_window_bytes)
            || !(1..=60_000_000_000).contains(&archive.max_storage_pause_ns)
            || now >= archive.publication_deadline_ns || now >= live.deadline_ns
        { return Err(refused(LiveArchiveError::Configuration)); }
        let binding = live.binding.clone();
        let live_deadline_ns = live.deadline_ns;
        let writer = RecordingArchiveWriter::open(publisher, archive.namespace, archive.limits,
            now, archive.publication_deadline_ns, cancel)
        ...
        Ok(Self { recording: Some(recording), writer: Some(writer), binding, live_deadline_ns,
            ...
            max_window_bytes: archive.max_window_bytes, remaining_steps: archive.max_steps,
            max_storage_pause_ns: archive.max_storage_pause_ns, storage_since_ns: Some(now),
            last_ns: now, ready: false, completion: None, finished: false })
    }

    /// Acknowledged durable/indexed inventory only. Absent after terminal ownership transfer.
    pub fn snapshot(&self) -> Option<&ArchiveSnapshot> { self.writer.as_ref().map(|w| w.snapshot()) }
    /// One accepted recording not yet acknowledged durable; source bytes are never logged.
    pub fn pending(&self) -> Option<&PreparedRecording> { self.writer.as_ref().and_then(|w| w.pending()) }
    /// The existing protocol's local state, not camera health or archive completeness.
    pub fn state(&self) -> ClientState { self.recording.as_ref().map_or(ClientState::Closed, |r| r.state()) }
    /// Transport observations, absent after their terminal transfer.
    pub fn totals(&self) -> Option<TcpTotals> { self.recording.as_ref().and_then(|r| r.totals()) }
    ...
    pub fn next_wake_ns(&self) -> Option<u64> {
        if self.finished || self.writer.is_none() { return None; }
        if !self.ready { return Some(self.last_ns); }
        Some(self.recording.as_ref().and_then(|r| r.next_wake_ns())
            .map_or(self.publication_deadline_ns, |at| at.min(self.publication_deadline_ns)))
    }
    ...
    {
        self.admit(now, authority)?; self.require_input()?;
        let result = self.recording.as_mut().ok_or_else(|| safe(LiveArchiveError::Closed))?
    ...
    pub fn respond(&mut self, credentials: &DigestCredentials<'_>, cnonce: [u8; 16], now: u64,
        authority: &dyn TcpAuthority) -> Result<QueuedAvcRequest, LiveArchiveFailure>
    {
        self.admit(now, authority)?; self.require_input()?;
        let result = self.recording.as_mut().ok_or_else(|| safe(LiveArchiveError::Closed))?
    ...
    pub fn seal(&mut self, now: u64, authority: &dyn TcpAuthority) -> Result<bool, LiveArchiveFailure> {
        self.admit(now, authority)?; self.require_input()?;
        let result = self.recording.as_mut().ok_or_else(|| safe(LiveArchiveError::Closed))?.seal(now, authority);
        result.map_err(|e| self.capture_failure(e))
    }
    /// Index the acknowledged tail without ending input. Repeated calls cannot restart a pause.
    pub fn flush(&mut self, now: u64, authority: &dyn TcpAuthority) -> Result<(), LiveArchiveFailure> {
        self.admit(now, authority)?; self.require_input()?;
        self.writer.as_mut().ok_or_else(|| safe(LiveArchiveError::Closed))?.flush();
        self.pause(now); Ok(())
    }
    ...
    pub fn finish_capture(&mut self, now: u64, authority: &dyn TcpAuthority)
        -> Result<Option<LiveRecordingRetirement>, LiveArchiveFailure>
    {
        self.admit(now, authority)?;
        if self.completion.is_some() { return Err(safe(LiveArchiveError::Closed)); }
    ...
    pub fn poll(&mut self, readiness: SocketReadiness, now: u64, authority: &dyn TcpAuthority,
        cancel: &dyn PublishCancellation) -> Result<LiveArchiveStep, LiveArchiveFailure>
    {
        if self.finished || self.writer.is_none() { return Ok(LiveArchiveStep::Ended); }
        self.admit(now, authority)?;
        if cancel.cancel_requested(PublishCutPoint::AfterChildrenVerified) {
            return Err(self.fatal(LiveArchiveError::Archive(ArchiveError::Cancelled), None, None));
        }
        if !self.ready {
            let progress = match self.writer.as_mut().ok_or_else(|| safe(LiveArchiveError::Closed))?.step(now, cancel) {
            ...
            if matches!(&progress, ArchiveWriteProgress::Ready { .. }) {
                self.ready = true; self.storage_since_ns = None;
            }
            if matches!(&progress, ArchiveWriteProgress::Finished { .. }) {
                ...
                self.finished = true;
                return Ok(LiveArchiveStep::Finished { archive: progress, cause });
            }
            return Ok(LiveArchiveStep::Archive(progress));
        }
        let step = match self.recording.as_mut().ok_or_else(|| safe(LiveArchiveError::Closed))?
        ...
        match step {
            LiveRecordingStep::Capture { event: CapturePoll::Window(window), connection: None } => {
                ...
                match result {
                    Ok(admission) => { self.pause(now); Ok(LiveArchiveStep::WindowAccepted(admission)) }
                    Err(refusal) => Err(self.fatal(LiveArchiveError::Archive(*refusal.reason),
    ...
    pub fn cancel(&mut self) -> Option<LiveArchiveRetirement> {
        if self.writer.is_none() && self.recording.is_none() { return None; }
        Some(self.retire(None, None))
    }
    fn require_input(&self) -> Result<(), LiveArchiveFailure> {
        if self.completion.is_some() { return Err(safe(LiveArchiveError::Closed)); }
        if !self.ready { return Err(safe(LiveArchiveError::Backpressure)); }
        Ok(())
    }
    ...
    fn admit(&mut self, now: u64, authority: &dyn TcpAuthority) -> Result<(), LiveArchiveFailure> {
        if self.finished || self.writer.is_none() { return Err(safe(LiveArchiveError::Closed)); }
        if now < self.last_ns { return Err(safe(LiveArchiveError::ClockReversed)); }
        if self.remaining_steps == 0 { return Err(self.fatal(LiveArchiveError::WorkBudget, None, None)); }
        self.remaining_steps -= 1; self.last_ns = now;
        if now >= self.publication_deadline_ns { return Err(self.fatal(LiveArchiveError::Deadline, None, None)); }
        if self.storage_since_ns.is_some_and(|since| now - since >= self.max_storage_pause_ns) {
        ...
        if self.completion.is_none() {
            let allowed = if now >= self.live_deadline_ns { Err(TcpDenial::Deadline) }
                else { authority.checkpoint(&self.binding, TcpOperation::Poll, now, self.live_deadline_ns) };
            if let Err(error) = allowed { return Err(self.fatal(LiveArchiveError::Authority(error), None, None)); }
        }
        Ok(())
    }
    ...
    fn fatal(&mut self, reason: LiveArchiveError, recording: Option<LiveRecordingRetirement>,
        window: Option<PreparedRecording>) -> LiveArchiveFailure {
        LiveArchiveFailure { reason, retirement: Some(Box::new(self.retire(recording, window))) }
    }
    fn retire(&mut self, recording: Option<LiveRecordingRetirement>, window: Option<PreparedRecording>)
        -> LiveArchiveRetirement {
        let owned = self.recording.take().and_then(|mut r| r.cancel());
        self.finished = true;
        LiveArchiveRetirement { recording: recording.or(owned),
            archive: self.writer.take().map(|w| w.retire()), unoffered_window: window }
    }
}
fn safe(reason: LiveArchiveError) -> LiveArchiveFailure { LiveArchiveFailure { reason, retirement: None } }
