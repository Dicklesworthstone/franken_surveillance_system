...
impl<C: ArchiveCodec> CodecArchiveSnapshot<C> {
    ...
    pub fn select(
        &self,
        query: Range<u64>,
        limits: ArchiveQueryLimits,
    ) -> ArchiveResult<ArchiveSelection> {
        if query.start >= query.end {
            return Err(CatalogError::Interval.into());
        }
        if limits.max_windows == 0
            || limits.max_windows > MAX_ARCHIVE_WINDOWS
        ...
        for (ordinal, entry) in self.windows[..self.indexed].iter().enumerate() {
            ...
            let start = query.start.max(interval.start);
            let end = query.end.min(interval.end);
            if start >= end {
                continue;
            }
            if ordinals.len() == limits.max_windows {
                return Err(ArchiveError::Limit);
            }
            bytes = bytes
                .checked_add(entry.byte_len() as u64)
...
impl<'a, C: ArchiveCodec> CodecArchiveRead<'a, C> {
    ...
    pub fn selection(&self) -> &ArchiveSelection {
        &self.selection
    }
    ...
    ) -> ArchiveResult<ArchiveReadProgress<C>> {
        if self.done {
            return Ok(ArchiveReadProgress::Exhausted);
        }
        if self.blocked {
    ...
    fn advance(
        ...
        if let Some(&ordinal) = self.selection.ordinals.get(self.next) {
            ...
            let page = self
                ...
                .pages
                .iter()
                .find(|p| ordinal >= p.first && ordinal < p.first + C::entries(&p.catalog).len())
                .ok_or(ArchiveError::Metadata)?;
            // Recheck the selected catalog before returning its original media.
            let loaded = C::load_catalog(
                self.publisher,
                &page.slot,
                C::manifest(&page.catalog).root(),
                &self.snapshot.namespace.scope,
                cancel,
            )?;
            if C::index(&loaded) != C::index(&page.catalog) {
            ...
            let bytes = self
                .bytes
                .checked_add(C::plan(&recording).byte_len() as u64)
                .ok_or(ArchiveError::Limit)?;
            if bytes > self.selection.bytes {
                return Err(ArchiveError::Limit);
            }
            let interval = entry.decode_interval();
            let requested_interval = self.selection.query.start.max(interval.start)
                ..self.selection.query.end.min(interval.end);
            self.bytes = bytes;
            self.next += 1;
            return Ok(ArchiveReadProgress::Window {
                ordinal,
                requested_interval,
                recording,
            });
        }
        if self.bytes != self.selection.bytes {
            return Err(ArchiveError::Metadata);
        }
        ...
        self.done = true;
        Ok(ArchiveReadProgress::Complete(Box::new(
            ArchiveReadReceipt {
                snapshot_digest: self.selection.snapshot,
                scope: self.snapshot.namespace.scope.clone(),
