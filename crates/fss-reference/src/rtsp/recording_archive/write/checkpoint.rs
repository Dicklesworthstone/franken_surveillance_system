...
use super::*;
use crate::rtsp::recording::{RECORDING_KIND, RecordingScope};
...
pub const MAX_ARCHIVE_WORK_BYTES: usize =
    MAX_RECORDING_BYTES + MAX_CATALOG_BYTES + MAX_MANIFEST + MAX_METADATA;
...
impl Default for ArchiveWorkLimits {
    fn default() -> Self {
        Self {
            ...
            max_pending_bytes: MAX_RECORDING_BYTES,
            max_graph_objects: MAX_MANIFEST_CHILDREN,
            max_new_bytes: MAX_ARCHIVE_WORK_BYTES,
        }
    }
}
impl ArchiveWorkLimits {
    pub(crate) fn validate(self, stored: ArchiveLimits) -> ArchiveResult<()> {
        ...
        if self.max_pending_bytes > MAX_RECORDING_BYTES
            || self.max_graph_objects == 0
            || self.max_graph_objects > MAX_MANIFEST_CHILDREN
            || self.max_new_bytes > MAX_ARCHIVE_WORK_BYTES
            || stored.max_windows > self.archive.max_windows
            || stored.max_pages > self.archive.max_pages
...
impl<'a> WorkView<'a> {
    ...
    fn writer(writer: &'a RecordingArchiveWriter<'_>) -> Self {
        Self {
            ...
            pending: writer.pending.as_ref().map(|p| &p.recording),
    ...
    fn retirement_digest(self) -> ArchiveResult<ContentDigest> {
        ...
        for value in [
            limits.max_windows,
            limits.max_pages,
            limits.max_scan_roots,
            limits.windows_per_page,
        ] {
            e.u64(u64::try_from(value).map_err(|_| ArchiveError::Limit)?);
        }
        e.bool(self.pending.is_some());
        if let Some(pending) = self.pending {
            e.digest(pending.manifest().root());
        }
        e.bool(self.prepared_page.is_some());
        if let Some(page) = self.prepared_page {
            e.digest(page.manifest().root());
        }
...
impl std::fmt::Debug for PreparedArchiveWork<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PreparedArchiveWork")
            .field("root", &self.root())
            .field("new_payload_bytes", &self.bytes)
...
impl<'a> PreparedArchiveWork<'a> {
    ...
    fn prepare_view(
        ...
        if bytes > limits.max_new_bytes
            || manifest.children().len() > publisher.limits().max_children
        {
            return Err(ArchiveError::Limit);
        }
        check_slot(publisher, &slot, manifest.root())?;
        if let Some(window) = work.pending {
            check_slot(publisher, &window_slot, window.manifest().root())?;
        }
        if let Some(page) = work.prepared_page {
            check_slot(publisher, &page_slot, page.manifest().root())?;
        }
        probe(cancel)?;
        Ok(Self {
            work,
    ...
    pub fn publish(
        ...
        deadline: u64,
        cancel: &dyn PublishCancellation,
    ) -> ArchiveResult<LocalPublicationReceipt> {
        if now >= deadline {
        ...
        if publisher.root(&self.slot).is_some() {
            return durable(
                publisher
                    .publish_cancellable(&self.slot, &self.manifest, cancel)
        ...
        if let Some(page) = self.work.prepared_page {
            ...
            durable(
                publisher
                    .publish_cancellable(&self.page_slot, page.manifest(), cancel)
                    .map_err(RecordingIoError::from)?,
            )?;
        }
        probe(cancel)?;
        let metadata = publisher
            .stage_object(&self.metadata)
            .map_err(RecordingIoError::from)?;
        if Some(metadata) != self.manifest.metadata_digest() {
            return Err(ArchiveError::Metadata);
        }
        durable(
            publisher
...
pub fn load_archive_work(
    ...
    if slot != &work_slot(stamp.retirement, 'r')?
        || stamp
            .pending
            .is_some_and(|(_, bytes)| bytes > limits.max_pending_bytes)
    {
        return Err(ArchiveError::Limit);
    }
    ...
    let indexed: usize = snapshot
        .pages
        .iter()
        .map(|p| p.catalog.entries().len())
        .sum();
    if indexed != stamp.indexed || indexed > snapshot.windows.len() {
        return Err(ArchiveError::Metadata);
    }
    snapshot.indexed = indexed;
    if snapshot.digest()? != stamp.snapshot {
        return Err(ArchiveError::Metadata);
    }
    let pending = match stamp.pending {
        None => None,
        Some((root, bytes)) => {
            let window = load_recording(
                publisher,
                &work_slot(stamp.retirement, 'w')?,
                root,
                &stamp.scope.recording,
                cancel,
            )?;
            if window.byte_len() != bytes {
                return Err(ArchiveError::Metadata);
            }
            Some(window)
        }
    ...
    let prepared_page = match stamp.page {
        None => None,
        Some(root) => Some(load_catalog(
            publisher,
            &work_slot(stamp.retirement, 'c')?,
            root,
            &stamp.scope,
            cancel,
        )?),
    };
    let work = ArchiveRetirement {
        snapshot,
        pending,
        prepared_page,
    };
...
fn validate_pending(work: WorkView<'_>, limits: ArchiveWorkLimits) -> ArchiveResult<()> {
    ...
    if let Some(window) = &work.pending {
        if window.byte_len() > limits.max_pending_bytes
            || old.windows.len() >= old.limits.max_windows
        {
            return Err(ArchiveError::Limit);
        }
        ...
        if old
            .windows
            .last()
            .is_some_and(|p| p.decode_interval().end > entry.decode_interval().start)
            || old.windows.iter().any(|p| p.root() == entry.root())
        {
            return Err(ArchiveError::Sequence);
        }
    }
    if let Some(page) = &work.prepared_page
        && (page.scope() != old.namespace.scope()
            || page.entries() != old.unindexed_windows()
            || page.entries().is_empty()
            || old.pages.len() >= old.limits.max_pages)
    {
        return Err(ArchiveError::Metadata);
    }
    Ok(())
...
fn graph(
    ...
    let bound = work
        ...
        .and_then(|n| {
            n.checked_add(
                work.prepared_page
                    .as_ref()
                    .map_or(0, |page| page.manifest().children().len() + 1),
            )
        })
        .and_then(|n| n.checked_add(1))
        .ok_or(ArchiveError::Limit)?;
    if bound > limits.max_graph_objects {
        return Err(ArchiveError::Limit);
    }
    ...
    for entry in work.snapshot.windows() {
        ...
        if manifest.kind() != RECORDING_KIND || manifest.children().len() != 4 {
            return Err(ArchiveError::Metadata);
        }
        children.push(entry.root());
        children.extend_from_slice(manifest.children());
    }
    for page in work.snapshot.pages() {
        require_slot(p, page.slot(), page.catalog().manifest().root())?;
        let manifest = read_manifest(
            p,
            page.catalog().manifest().root(),
            MAX_CATALOG_BYTES,
            cancel,
        )?;
        if &manifest != page.catalog().manifest() {
            return Err(ArchiveError::Metadata);
        }
        children.push(manifest.root());
        children.push(manifest.metadata_digest().ok_or(ArchiveError::Metadata)?);
    }
    if let Some(window) = &work.pending {
        children.push(window.manifest().root());
        children.extend_from_slice(window.manifest().children());
    }
    if let Some(page) = &work.prepared_page {
        children.push(page.manifest().root());
        children.extend_from_slice(page.manifest().children());
    }
...
fn check_slot(p: &LocalRootPublisher, slot: &SlotName, root: ContentDigest) -> ArchiveResult<()> {
    if p.root(slot).is_some_and(|r| r.root != root) {
...
fn require_slot(p: &LocalRootPublisher, slot: &SlotName, root: ContentDigest) -> ArchiveResult<()> {
    ...
    if p.root(slot)
        .is_none_or(|r| r.state != LocalPublicationState::Durable)
    {
...
fn read(
    ...
    if p.limits().spool.max_object_bytes > MAX_RECORDING_BYTES {
        return Err(ArchiveError::Limit);
    }
    if p.tombstones().any(|d| *d == digest) {
        return Err(RecordingIoError::Tombstoned.into());
    }
...
fn read_manifest(
    ...
    let manifest =
        ObjectManifest::from_canonical_bytes(&bytes).map_err(|_| ArchiveError::Metadata)?;
...
struct Stamp {
    ...
    pending: Option<(ContentDigest, usize)>,
    page: Option<ContentDigest>,
}
impl Stamp {
    fn from_work(work: WorkView<'_>) -> ArchiveResult<Self> {
        Ok(Self {
            scope: work.snapshot.namespace().scope().clone(),
            limits: work.snapshot.limits(),
            windows: work.snapshot.windows().len(),
            pages: work.snapshot.pages().len(),
            indexed: work.snapshot.indexed_windows(),
            snapshot: work.snapshot.digest()?,
            retirement: work.retirement_digest()?,
            pending: work
                .pending
                .as_ref()
                .map(|w| (w.manifest().root(), w.byte_len())),
    ...
    fn encode(&self) -> ArchiveResult<Vec<u8>> {
        ...
        e.u64(s.generation);
        e.digest(s.anchor);
        e.digest(s.receive_clock);
        e.digest(self.scope.decode_clock);
        e.u32(self.scope.time_scale);
        for value in [
            self.limits.max_windows,
            self.limits.max_pages,
            self.limits.max_scan_roots,
            self.limits.windows_per_page,
            self.windows,
            self.pages,
            self.indexed,
        ] {
        ...
        e.digest(self.snapshot);
        e.digest(self.retirement);
        e.bool(self.pending.is_some());
        if let Some((root, bytes)) = self.pending {
            e.digest(root);
            e.u64(bytes as u64);
        }
        e.bool(self.page.is_some());
        if let Some(root) = self.page {
            e.digest(root);
        }
    ...
    fn decode(bytes: &[u8]) -> ArchiveResult<Self> {
        let decode = || -> Result<Self, fss_core::ContractError> {
            ...
            let limits = ArchiveLimits {
                max_windows: count()?,
                max_pages: count()?,
                max_scan_roots: count()?,
                windows_per_page: count()?,
            };
            ...
            let pending = if d.bool()? {
                Some((
                    d.digest()?,
                    usize::try_from(d.u64()?)
                        .map_err(|_| fss_core::ContractError::ArithmeticOverflow)?,
                ))
            } else {
                None
            };
        ...
        if value.windows > value.limits.max_windows
            || value.pages > value.limits.max_pages
            || value.indexed > value.windows
            || value
                .pending
                .is_some_and(|(_, n)| n == 0 || n > MAX_RECORDING_BYTES)
            || value.encode()? != bytes
        {
            return Err(ArchiveError::Metadata);
        }
