...
impl CheckpointBarrier {
    ...
    pub(crate) fn pending(&self) -> Option<&ArchiveCheckpoint> {
        self.pending.as_ref()
    }
    pub(crate) fn last_durable(&self) -> Option<&ArchiveCheckpoint> {
        self.last_durable.as_ref()
    }
    pub(crate) fn awaiting_pin(&self) -> bool {
        self.pending.is_some() && !self.acknowledged
    }
    pub(crate) fn max_pending_bytes(&self) -> usize {
        self.limits.max_pending_bytes
    }
    pub(crate) fn acknowledge(&mut self, pin: &ArchiveCheckpoint) -> ArchiveResult<()> {
        if self.pending.as_ref() != Some(pin) {
    ...
    pub(crate) fn step(
        &mut self,
        writer: &mut RecordingArchiveWriter<'_>,
        now: u64,
        cancel: &dyn PublishCancellation,
    ) -> ArchiveResult<CheckpointedArchiveProgress> {
        if writer.done {
            return Ok(CheckpointedArchiveProgress::Archive(
    ...
    fn advance(
        ...
        if writer.pending.is_none() && writer.page.is_none() {
            ...
            return writer
                .step(now, cancel)
                .map(CheckpointedArchiveProgress::Archive);
        }
        ...
        let prepared =
            PreparedArchiveWork::prepare_view(view, writer.publisher, self.limits, cancel)?;
        let pin = ArchiveCheckpoint::prepared(&prepared);
        if self.pending.is_none() {
            self.pending = Some(pin.clone());
        ...
        self.last_durable = Some(saved);
        self.protected = Some(identity);
        self.pending = None;
        self.acknowledged = false;
        Ok(CheckpointedArchiveProgress::WorkDurable {
            checkpoint: pin,
            receipt,
        })
    }
...
impl std::fmt::Debug for CheckpointedArchiveWriter<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CheckpointedArchiveWriter")
...
impl<'a> CheckpointedArchiveWriter<'a> {
    ...
    pub fn open(
        ...
        let writer =
            RecordingArchiveWriter::open(publisher, namespace, archive, now, deadline, cancel)?;
        Ok(Self {
            writer,
            barrier,
    ...
    pub fn offer(
        ...
        let admission = self.admit(now).and_then(|()| {
            if recording.byte_len() > self.barrier.max_pending_bytes() {
                return Err(ArchiveError::Limit);
            }
            Ok(())
        });
    ...
    fn admit(&mut self, now: u64) -> ArchiveResult<()> {
        if self.writer.blocked {
            return Err(ArchiveError::Blocked);
        }
        if self.writer.done {
            return Err(ArchiveError::Closed);
        }
        if now < self.writer.last_ns {
            return Err(ArchiveError::ClockReversed);
        }
        if now >= self.writer.deadline_ns {
            self.writer.blocked = true;
