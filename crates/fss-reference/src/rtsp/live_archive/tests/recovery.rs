...
use super::*;
use crate::rtsp::archive_recovery::{
...
fn resume_config(retired: &ArchiveRetirement) -> Test<ArchiveResumeConfig> {
    Ok(ArchiveResumeConfig {
        expected_retirement_digest: archive_retirement_digest(retired)?,
...
fn complete(resume: &mut RecordingArchiveResume<'_>, now: u64) -> Test<Vec<ArchiveResumeProgress>> {
    ...
    for _ in 0..128 {
        ...
        let finished = matches!(
            &step,
            ArchiveResumeProgress::Archive(ArchiveWriteProgress::Finished { .. })
        );
        all.push(step);
        if finished {
            return Ok(all);
...
fn unpublished_original_is_resumed_once_at_the_reserved_ordinal_without_camera_io() -> Test {
    ...
    let mut f = Fixture::playing(&mut p, 7, 4)?;
    ...
    let retired = f
        ...
        .archive
        .ok_or("archive")?;
    let config = resume_config(&retired)?;
    drop(f);
    drop(p);
    let mut p = LocalRootPublisher::open(&path, limits())?;
    ...
    assert_eq!(
        ...
        RetiredPublicationState::NotPublished(admission.root)
    );
    ...
    assert_eq!(
        outputs
            .iter()
            .filter(|s| matches!(s, ArchiveResumeProgress::WindowAccepted(_)))
            .count(),
        1
    );
    assert_eq!(resume.snapshot().windows().len(), 1);
    assert_eq!(resume.snapshot().indexed_windows(), 1);
    assert_eq!(resume.snapshot().windows()[0].slot(), &admission.slot);
    assert!(matches!(
        resume.step(22, &NeverCancel)?,
        ArchiveResumeProgress::Ended
    ));
    Ok(())
}
...
fn lost_window_ack_is_reconciled_as_durable_without_duplicate_or_republication() -> Test {
    ...
    let error = f
        .driver
        .poll(SocketReadiness::default(), 14, &f.authority, &NeverCancel)
        .err()
        .ok_or("injected publication fault did not stop capture")?;
    let retired = error
        .retirement
        .ok_or("retirement")?
        .archive
    ...
    assert_eq!(
        retired.pending.as_ref().ok_or("pending")?.manifest().root(),
        admission.root
    );
    let config = resume_config(&retired)?;
    drop(f);
    ...
    assert_eq!(
        resume.reconciliation().window,
        RetiredPublicationState::AlreadyDurable(admission.root)
    );
    assert!(resume.pending().is_none());
    let outputs = complete(&mut resume, 21)?;
    assert!(!outputs.iter().any(|s| matches!(
        s,
        ArchiveResumeProgress::WindowAccepted(_)
...
fn lost_catalog_ack_recovers_the_exact_page_without_preparing_a_replacement() -> Test {
    ...
    for _ in 0..128 {
        match resume.step(21, &NeverCancel) {
            Ok(ArchiveResumeProgress::Archive(ArchiveWriteProgress::CatalogPrepared { root })) => {
                prepared = Some(root)
            }
            Ok(_) => {}
            Err(_) => {
    ...
    assert!(failed);
    let root = prepared.ok_or("prepared catalog")?;
    let retired = resume
        .retire()
        .into_retry()
        .map_err(|_| "competing retained pages")?;
    assert_eq!(
        retired
            .prepared_page
            .as_ref()
            .ok_or("original page lost")?
            .manifest()
            .root(),
        root
    );
...
fn unresolved_root_temp_is_not_deleted_or_mistaken_for_absence() -> Test {
    ...
    let mut f = Fixture::playing(&mut p, 4096, 1)?;
    let admission = f.window()?;
    let error = f
        .driver
        .poll(SocketReadiness::default(), 14, &f.authority, &NeverCancel)
        .err()
        .ok_or("injected temporary did not stop capture")?;
    let retired = error
        .retirement
        .ok_or("retirement")?
        .archive
    ...
    drop(p);
    let mut p = LocalRootPublisher::open(&path, limits())?;
    let temps = p.recovery_report().orphaned_temps.clone();
    assert!(!temps.is_empty());
    let refusal = RecordingArchiveResume::open(&mut p, retired, config, 20, &NeverCancel)
        .err()
        .ok_or("unresolved temporary adopted")?;
    assert_eq!(
        refusal
            .retired
            .pending
...
fn pre_root_crash_keeps_original_bytes_for_explicit_republication() -> Test {
    for cut in [
        ...
        let error = f
            ...
            .poll(SocketReadiness::default(), 14, &f.authority, &NeverCancel)
            .err()
            .ok_or("injected pre-root failure missing")?;
        let retired = error
            .retirement
            .ok_or("retirement")?
            .archive
...
fn wrong_retirement_pin_refuses_without_consuming_the_original_or_writing_storage() -> Test {
    ...
    let retired = f
        ...
        .archive
        .ok_or("archive")?;
    let mut config = resume_config(&retired)?;
    config.expected_retirement_digest = ContentDigest::sha256(b"wrong pin");
    drop(f);
    let refusal = RecordingArchiveResume::open(&mut p, retired, config, 20, &NeverCancel)
        .err()
        .ok_or("wrong pin admitted")?;
    assert_eq!(
        refusal
            .retired
            .pending
...
fn recovered_pending_slot_cannot_name_different_valid_recording_bytes() -> Test {
    ...
    let mut second = Fixture::playing(&mut q, 4096, 1)?;
    second.picture()?;
    let mut changed = timing();
    changed.decode_time += 1;
    let _ = second
        .driver
        .supply_timing(changed, 12, &second.authority)?;
    second.driver.seal(13, &second.authority)?;
    let _ = second.driver.poll(
    ...
    let other = second
        .driver
        .cancel()
        .ok_or("second retirement")?
        .archive
        .ok_or("second archive")?
        .pending
        .ok_or("second pending")?;
    assert_ne!(other.manifest().root(), admission.root);
    drop(second);
    ...
    let refusal = RecordingArchiveResume::open(&mut p, retired, config, 30, &NeverCancel)
        .err()
        .ok_or("conflicting pending root adopted")?;
    ...
    assert_eq!(
        p.root(&admission.slot).ok_or("overwrote conflict")?.root,
        other.manifest().root()
    );
    Ok(())
}
...
fn cancelled_resume_retains_unoffered_window_and_does_not_implicitly_retry() -> Test {
    ...
    let retired = resume
        .retire()
        .into_retry()
        .map_err(|_| "lost retry ownership")?;
    assert_eq!(
        retired
            .pending
            .as_ref()
...
fn retry_preserves_both_a_prepared_older_page_and_the_next_unoffered_window() -> Test {
    ...
    let mut later = timing();
    later.decode_time += u64::from(later.duration);
    let _ = f.driver.supply_timing(later, 16, &f.authority)?;
    assert!(f.driver.seal(17, &f.authority)?);
    let LiveArchiveStep::WindowAccepted(second) =
        f.driver
            .poll(SocketReadiness::default(), 17, &f.authority, &NeverCancel)?
    else {
        return Err("second window was not admitted".into());
    };
    ...
    for _ in 0..128 {
        if let ArchiveResumeProgress::Archive(ArchiveWriteProgress::CatalogPrepared { root }) =
            resume.step(21, &NeverCancel)?
        {
            expected = Some(root);
            break;
        }
    ...
    let retired = resume
        .retire()
        .into_retry()
        .map_err(|_| "lost split ownership")?;
    assert_eq!(
        retired
            .prepared_page
            .as_ref()
    ...
    assert_eq!(
        resume.reconciliation().page,
        RetiredPublicationState::NotPublished(expected)
    );
    assert_eq!(
        resume.reconciliation().window,
        RetiredPublicationState::NotPublished(second.root)
    );
    complete(&mut resume, 31)?;
    assert_eq!(resume.snapshot().windows().len(), 2);
    assert_eq!(resume.snapshot().indexed_windows(), 2);
    assert_eq!(resume.snapshot().pages().len(), 2);
    assert_eq!(
        resume.snapshot().pages()[0].catalog().manifest().root(),
        expected
    );
    ...
    for _ in 0..128 {
        match f.driver.poll(
            ...
            LiveArchiveStep::Finished { .. } => {
                return Err("protocol failure was fabricated into EOF completion".into());
            }
            _ => {}
        }
    }
