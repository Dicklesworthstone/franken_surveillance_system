...
fn config(
    ...
    let recording = LiveRecordingConfig {
        scope: RecordingScope {
            sensor: SensorId::parse("sensor:archive-fixture")?,
            stream: StreamId::parse("stream:archive-fixture")?,
            generation: KEY.generation,
            anchor: ContentDigest::sha256(b"archive owner anchor"),
...
impl<'a> Fixture<'a> {
    fn new(publisher: &'a mut LocalRootPublisher, chunk: usize, page_size: usize) -> Test<Self> {
        ...
        let (live, recording, archive, authority) =
            config(listener.local_addr()?, chunk, page_size)?;
        let namespace = archive.namespace.clone();
        let driver = LiveAvcArchive::connect(
            live,
            recording,
            archive,
            publisher,
    ...
    fn command(&mut self, command: ClientCommand, now: u64) -> Test<u32> {
        ...
        for _ in 0..32_768 {
            if let LiveArchiveStep::Recording(step) = self.driver.poll(
                ...
                return Err("unexpected archive output during command dispatch".into());
            }
        }
    ...
    fn playing(
        ...
        let mut f = Self::new(publisher, chunk, page_size)?;
        f.storage(0)?;
...
fn archive_scope_mismatch_refuses_before_connection_and_authority_probe() -> Test {
    ...
    let error = LiveAvcArchive::connect(
        live,
        recording,
        archive,
        &mut p,
    ...
    .err()
    .ok_or("mismatched archive connected")?;
...
fn admission_durability_and_indexing_are_separate_and_storage_blocks_all_socket_reads() -> Test {
    ...
    f.peer
        .write_all(b"must remain in socket until storage is ready")?;
    let progress = f.storage(14)?;
    assert!(matches!(
        progress.first(),
        Some(ArchiveWriteProgress::WindowDurable { ordinal: 0, .. })
    ));
    assert!(
        progress
            .iter()
            .any(|p| matches!(p, ArchiveWriteProgress::CatalogPublished { windows: 1, .. }))
    );
    ...
    assert!(
        retired
            .archive
            .ok_or("archive retirement")?
            .pending
            .is_none()
    );
    drop(f);
    assert_eq!(
        p.root(&admission.slot).ok_or("durable root")?.root,
        admission.root
    );
    Ok(())
}
...
fn eof_flushes_partial_page_then_reopens_with_identical_source_and_media() -> Test {
    ...
    for chunk in [1, 7, 4096] {
        ...
        for _ in 0..128 {
            match f.driver.poll(
                ...
                LiveArchiveStep::Finished {
                    ...
                    cause: LiveArchiveCompletion::InputEnded,
                } => {
                    complete = true;
                    break;
                }
        ...
        assert!(matches!(
            f.driver
                .poll(SocketReadiness::default(), 15, &f.authority, &NeverCancel)?,
            LiveArchiveStep::Ended
        ));
        let namespace = f.namespace.clone();
        drop(f);
        drop(p);
        let p = LocalRootPublisher::open(&path, limits())?;
        let snapshot = ArchiveSnapshot::load(
            &p,
            namespace.clone(),
        ...
        if let Some(prior) = expected {
            assert_eq!(admission.root, prior);
        } else {
            expected = Some(admission.root);
        }
    }
    Ok(())
}
...
fn deliberate_stop_returns_untimed_source_without_claiming_a_recorded_window() -> Test {
    ...
    let retired = f
        .driver
        .finish_capture(12, &f.authority)?
        .ok_or("capture retirement")?;
    assert!(retired.capture.picture.is_some());
    assert!(!retired.capture.collection.pending.sources.is_empty());
    assert!(matches!(
        f.driver
            .poll(SocketReadiness::default(), 13, &f.authority, &NeverCancel)?,
        LiveArchiveStep::Finished {
            archive: ArchiveWriteProgress::Finished {
                windows: 0,
                pages: 0,
                ..
            },
...
fn undersized_window_allowance_returns_the_unoffered_original_and_stops_the_socket() -> Test {
    let mut p = LocalRootPublisher::open(fresh()?, limits())?;
    let mut f = Fixture::playing(&mut p, 4096, 1)?;
    f.driver.max_window_bytes = 1;
    f.picture()?;
    let _ = f.driver.supply_timing(timing(), 12, &f.authority)?;
    assert!(f.driver.seal(13, &f.authority)?);
    let error = f
        .driver
        .poll(SocketReadiness::default(), 13, &f.authority, &NeverCancel)
        .err()
        .ok_or("overbudget admission")?;
    assert!(matches!(
        error.reason,
        LiveArchiveError::Archive(ArchiveError::Limit)
    ));
    let retired = error.retirement.ok_or("retirement")?;
    assert!(retired.unoffered_window.is_some());
    assert!(retired.recording.is_some());
...
fn revocation_during_storage_pause_preserves_pending_window_without_publication() -> Test {
    ...
    let mut f = Fixture::playing(&mut p, 4096, 1)?;
    let admission = f.window()?;
    let totals = f.driver.totals().ok_or("totals")?;
...
fn refused_flush_and_clock_regression_do_not_renew_storage_pause() -> Test {
    ...
    assert!(matches!(
        f.driver
            .poll(SocketReadiness::default(), 12, &f.authority, &NeverCancel),
        Err(LiveArchiveFailure {
            reason: LiveArchiveError::ClockReversed,
            retirement: None
        })
    ));
    assert_eq!(f.driver.remaining_steps, steps);
    assert!(matches!(
        f.driver.flush(14, &f.authority),
        Err(LiveArchiveFailure {
            reason: LiveArchiveError::Backpressure,
            retirement: None
        })
    ...
    let error = f
        .driver
        .poll(SocketReadiness::default(), 18, &f.authority, &NeverCancel)
        .err()
        .ok_or("renewed pause")?;
    assert!(matches!(
        error.reason,
...
fn finite_driver_allowance_cannot_be_bypassed_by_no_readiness_polls() -> Test {
    ...
    let mut f = Fixture::playing(&mut p, 4096, 1)?;
    f.window()?;
    f.driver.remaining_steps = 0;
    let error = f
        .driver
        .poll(SocketReadiness::default(), 14, &f.authority, &NeverCancel)
        .err()
        .ok_or("work budget bypass")?;
