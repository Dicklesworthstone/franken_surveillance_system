...
impl fmt::Debug for CheckpointedLiveAvcArchive<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("CheckpointedLiveAvcArchive").field("inner", &self.inner)
            .field("awaiting_pin", &self.barrier.awaiting_pin()).finish_non_exhaustive()
    }
}
impl<'a> CheckpointedLiveAvcArchive<'a> {
    ...
    pub fn connect(live: LiveAvcConfig, recording: LiveRecordingConfig, archive: LiveArchiveConfig,
        work: ArchiveWorkLimits, publisher: &'a mut LocalRootPublisher, now: u64,
        authority: &dyn TcpAuthority, cancel: &dyn PublishCancellation)
        -> Result<Self, LiveArchiveConnectFailure> {
        let refused = |e| LiveArchiveConnectFailure { reason: LiveArchiveError::Archive(e), connection_attempted: false };
        ...
        let inner = LiveAvcArchive::connect(live, recording, archive, publisher, now, authority, cancel)?;
        Ok(Self { inner, barrier })
    }
    /// Normal acknowledged archive inventory, excluding auxiliary checkpoint roots.
    pub fn snapshot(&self) -> Option<&ArchiveSnapshot> { self.inner.snapshot() }
    ...
    pub fn totals(&self) -> Option<TcpTotals> { self.inner.totals() }
    ...
    pub fn next_wake_ns(&self) -> Option<u64> {
        if self.inner.finished || self.inner.writer.is_none() { return None; }
        if self.barrier.awaiting_pin() {
            ...
            if self.inner.completion.is_none() { at = at.min(self.inner.live_deadline_ns); }
            return Some(at);
        }
    ...
    pub fn request(&mut self, command: ClientCommand, credentials: &DigestCredentials<'_>,
        ...
        let result = self.inner.request(command, credentials, cnonce, now, authority);
    ...
    pub fn respond(&mut self, credentials: &DigestCredentials<'_>, cnonce: [u8; 16], now: u64,
        authority: &dyn TcpAuthority) -> Result<QueuedAvcRequest, CheckpointedLiveArchiveFailure> {
    ...
    pub fn seal(&mut self, now: u64, authority: &dyn TcpAuthority)
        -> Result<bool, CheckpointedLiveArchiveFailure> {
        let result = self.inner.seal(now, authority); result.map_err(|e| self.failure(e))
    }
    /// Flush discovery metadata through its own work barrier without ending capture.
    pub fn flush(&mut self, now: u64, authority: &dyn TcpAuthority)
        -> Result<(), CheckpointedLiveArchiveFailure> {
        let result = self.inner.flush(now, authority); result.map_err(|e| self.failure(e))
    }
    ...
    pub fn finish_capture(&mut self, now: u64, authority: &dyn TcpAuthority)
        ...
        let result = self.inner.finish_capture(now, authority); result.map_err(|e| self.failure(e))
    }
    ...
    pub fn acknowledge_checkpoint(&mut self, pin: &ArchiveCheckpoint, now: u64,
        authority: &dyn TcpAuthority, cancel: &dyn PublishCancellation)
        -> Result<(), CheckpointedLiveArchiveFailure> {
        self.inner.admit(now, authority).map_err(|e| self.failure(e))?;
        ...
        self.barrier.acknowledge(pin).map_err(|reason| CheckpointedLiveArchiveFailure {
            reason: LiveArchiveError::Archive(reason), retirement: None,
        })
    }
    ...
    pub fn poll(&mut self, readiness: SocketReadiness, now: u64, authority: &dyn TcpAuthority,
        cancel: &dyn PublishCancellation) -> Result<CheckpointedLiveArchiveStep, CheckpointedLiveArchiveFailure> {
        ...
        if self.inner.ready {
            ...
            let step = self.inner.poll(readiness, now, authority, cancel).map_err(|e| self.failure(e))?;
            return Ok(match step {
                LiveArchiveStep::Stopped { trigger, retained } => CheckpointedLiveArchiveStep::Stopped {
                    trigger, retained: Box::new(self.retirement(*retained)),
                },
                other => CheckpointedLiveArchiveStep::Live(other),
            });
        }
        self.inner.admit(now, authority).map_err(|e| self.failure(e))?;
        let Some(writer) = self.inner.writer.as_mut() else { return Err(self.fatal(LiveArchiveError::Invariant)); };
        let progress = self.barrier.step(writer, now, cancel)
        ...
        if matches!(&progress, ArchiveWriteProgress::Ready { .. }) {
            self.inner.ready = true; self.inner.storage_since_ns = None;
        }
        if matches!(&progress, ArchiveWriteProgress::Finished { .. }) {
            let Some(cause) = self.inner.completion else { return Err(self.fatal(LiveArchiveError::Invariant)); };
            self.inner.finished = true;
            return Ok(CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Finished { archive: progress, cause }));
        }
        Ok(CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Archive(progress)))
    }
    ...
    pub fn cancel(&mut self) -> Option<CheckpointedLiveArchiveRetirement> {
        let live = self.inner.cancel()?; Some(self.retirement(live))
    }
    fn retirement(&self, live: LiveArchiveRetirement) -> CheckpointedLiveArchiveRetirement {
        CheckpointedLiveArchiveRetirement { live, pending_checkpoint: self.barrier.pending().cloned(),
    ...
    fn failure(&self, failure: LiveArchiveFailure) -> CheckpointedLiveArchiveFailure {
        CheckpointedLiveArchiveFailure { reason: failure.reason,
    ...
    fn fatal(&mut self, reason: LiveArchiveError) -> CheckpointedLiveArchiveFailure {
        let failure = self.inner.fatal(reason, None, None); self.failure(failure)
    }
}
