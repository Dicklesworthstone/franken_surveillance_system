...
fn fresh() -> Test<PathBuf> {
    for _ in 0..256 {
        let path = std::env::temp_dir().join(format!("fss-checkpointed-live-{}-{}",
...
fn config(peer: std::net::SocketAddr, chunk: usize)
    ...
    let recording = LiveRecordingConfig { scope: RecordingScope {
        sensor: SensorId::parse("sensor:checkpoint-fixture")?, stream: StreamId::parse("stream:checkpoint-fixture")?,
        generation: KEY.generation, anchor: ContentDigest::sha256(b"checkpoint live owner anchor"),
    ...
    let archive = LiveArchiveConfig { namespace: ArchiveNamespace::new(CatalogScope {
        recording: recording.scope.clone(), decode_clock: ContentDigest::sha256(b"checkpoint decode clock"),
        time_scale: recording.time_scale,
    })?, limits: ArchiveLimits { max_windows: 8, max_pages: 8, max_scan_roots: 128, windows_per_page: 2 },
        max_window_bytes: MAX_RECORDING_BYTES, max_steps: 100_000,
        publication_deadline_ns: 2 * LEASE, max_storage_pause_ns: PAUSE };
...
fn ready() -> SocketReadiness { SocketReadiness { readable: true, writable: true } }
struct Fixture<'a> {
    driver: CheckpointedLiveAvcArchive<'a>, peer: TcpStream,
    authority: Authority, namespace: ArchiveNamespace,
}
impl<'a> Fixture<'a> {
    fn new(p: &'a mut LocalRootPublisher, chunk: usize, work: ArchiveWorkLimits) -> Test<Self> {
        ...
        let driver = CheckpointedLiveAvcArchive::connect(live, recording, archive, work, p, 0, &authority, &NeverCancel)?;
    ...
    fn poll(&mut self, now: u64) -> Test<CheckpointedLiveArchiveStep> {
        Ok(self.driver.poll(ready(), now, &self.authority, &NeverCancel)?)
    }
    fn command(&mut self, command: ClientCommand, now: u64) -> Test<u32> {
        ...
        for _ in 0..32_768 {
            let CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Recording(step)) =
                self.driver.poll(SocketReadiness { readable: false, writable: true }, now, &self.authority, &NeverCancel)?
                else { return Err("unexpected storage output during command dispatch".into()); };
            match *step {
    ...
    fn receive(&mut self, bytes: &[u8], now: u64) -> Test<Vec<LiveRecordingStep>> {
        ...
        for _ in 0..32_768 {
            let CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Recording(step)) =
                self.driver.poll(SocketReadiness { readable: true, writable: false }, now, &self.authority, &NeverCancel)?
    ...
    fn playing(p: &'a mut LocalRootPublisher, chunk: usize) -> Test<Self> {
        ...
        assert!(matches!(f.poll(0)?, CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Archive(ArchiveWriteProgress::Ready { .. }))));
    ...
    fn window(&mut self) -> Test<ArchiveAdmission> {
        ...
        match self.poll(13)? {
            CheckpointedLiveArchiveStep::Live(LiveArchiveStep::WindowAccepted(admission)) => Ok(admission),
            _ => Err("window not admitted".into()),
        }
    }
    fn pin(&mut self, now: u64) -> Test<ArchiveCheckpoint> {
        match self.poll(now)? {
            CheckpointedLiveArchiveStep::Checkpoint(CheckpointedArchiveProgress::PinRequired(pin)) => Ok(pin),
            _ => Err("pin not announced before publication".into()),
        }
    }
    fn protect(&mut self, pin: &ArchiveCheckpoint, now: u64) -> Test {
        self.driver.acknowledge_checkpoint(pin, now, &self.authority, &NeverCancel)?;
        match self.poll(now)? {
            CheckpointedLiveArchiveStep::Checkpoint(CheckpointedArchiveProgress::WorkDurable { checkpoint, receipt }) => {
                assert_eq!(checkpoint, *pin); assert_eq!(receipt.root, pin.root()); Ok(())
            }
    ...
    fn finish(&mut self, now: u64) -> Test<(Vec<ArchiveCheckpoint>, LiveArchiveCompletion)> {
        ...
        for _ in 0..256 {
            match self.poll(now)? {
                CheckpointedLiveArchiveStep::Checkpoint(CheckpointedArchiveProgress::PinRequired(pin)) => {
                    // The test harness retains the independent pin; no external journal is implied.
                    self.protect(&pin, now)?; pins.push(pin);
                }
                CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Finished { archive, cause }) => {
                    assert!(matches!(archive, ArchiveWriteProgress::Finished { windows: 1, pages: 1, .. }));
                    return Ok((pins, cause));
                }
                CheckpointedLiveArchiveStep::Stopped { .. } => return Err("fault became finish".into()),
                CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Ended) => return Err("finish receipt missing".into()),
                _ => {},
            }
        }
...
#[test]
fn native_capture_is_checkpointed_before_ordinary_archive_and_cold_replays_across_chunk_sizes() -> Test {
    let mut identities = Vec::new();
    for chunk in [1, 7, 4096] {
        let path = fresh()?;
        let (namespace, root, objects, window_pin, page_pin) = {
            let mut p = LocalRootPublisher::open(&path, limits())?;
            let mut f = Fixture::playing(&mut p, chunk)?; let admission = f.window()?;
            let objects = f.driver.pending().ok_or("source missing")?.children().map(|(_, digest, _)| digest);
            let pin = f.pin(14)?;
            let calls = f.driver.totals().ok_or("totals missing")?;
            f.peer.write_all(b"PRIVATE_UNREAD_SOURCE_SENTINEL")?;
            for _ in 0..4 {
                assert!(matches!(f.poll(14)?, CheckpointedLiveArchiveStep::Checkpoint(
                    CheckpointedArchiveProgress::PinRequired(p)) if p == pin));
            }
            assert_eq!(f.driver.totals(), Some(calls));
            assert!(f.driver.snapshot().ok_or("snapshot missing")?.windows().is_empty());
            f.protect(&pin, 14)?;
            assert_eq!(f.driver.totals(), Some(calls));
            assert!(f.driver.snapshot().ok_or("snapshot missing")?.windows().is_empty());
            ...
            assert_eq!(cause, LiveArchiveCompletion::OwnerStopped); assert_eq!(pins.len(), 1);
            assert_eq!(f.driver.snapshot().ok_or("snapshot missing")?.indexed_windows(), 1);
        ...
        let recording = load_recording(&p, &namespace.window_slot(0)?, root,
            &namespace.scope().recording, &NeverCancel)?;
    ...
    assert!(identities.windows(2).all(|pair| pair[0] == pair[1])); Ok(())
}
...
fn work_limits_are_rejected_before_storage_recovery_or_network_authority() -> Test {
    ...
    for work in [ArchiveWorkLimits { max_pending_bytes: 0, ..ArchiveWorkLimits::default() },
        ArchiveWorkLimits { max_new_bytes: 0, ..ArchiveWorkLimits::default() },
        ArchiveWorkLimits { max_graph_objects: 0, ..ArchiveWorkLimits::default() }] {
        let listener = TcpListener::bind((Ipv4Addr::LOCALHOST, 0))?;
        let (live, recording, archive, authority) = config(listener.local_addr()?, 7)?;
        let error = CheckpointedLiveAvcArchive::connect(live, recording, archive, work,
            &mut p, 0, &authority, &NeverCancel).err().ok_or("invalid protection connected")?;
...
fn revocation_while_waiting_for_a_pin_closes_capture_and_returns_source_and_candidate_once() -> Test {
    let mut p = LocalRootPublisher::open(fresh()?, limits())?;
    let mut f = Fixture::playing(&mut p, 7)?; let admission = f.window()?; let pin = f.pin(14)?;
    f.authority.revoked.set(true);
    let error = f.driver.poll(ready(), 15, &f.authority, &NeverCancel).err().ok_or("revocation ignored")?;
    ...
    assert_eq!(retired.pending_checkpoint, Some(pin)); assert!(retired.last_durable_checkpoint.is_none());
    assert!(retired.live.recording.is_some());
    let archive = retired.live.archive.ok_or("archive retirement missing")?;
    assert_eq!(archive.pending.ok_or("source lost")?.manifest().root(), admission.root);
    assert!(f.driver.cancel().is_none());
    assert!(matches!(f.poll(15)?, CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Ended)));
...
fn pin_wait_has_a_fixed_deadline_and_blocks_protocol_commands_without_resetting_the_pause() -> Test {
    let mut p = LocalRootPublisher::open(fresh()?, limits())?;
    let mut f = Fixture::playing(&mut p, 7)?; f.window()?; let pin = f.pin(14)?;
    let expires = 13 + PAUSE;
    assert_eq!(f.driver.next_wake_ns(), Some(expires));
    for now in 15..20 {
        let error = f.driver.request(ClientCommand::KeepAlive, &credentials()?, [90; 16], now, &f.authority)
            .err().ok_or("storage wait allowed a command")?;
        assert!(matches!(error.reason, LiveArchiveError::Backpressure)); assert!(error.retirement.is_none());
        assert!(matches!(f.poll(now)?, CheckpointedLiveArchiveStep::Checkpoint(
            CheckpointedArchiveProgress::PinRequired(p)) if p == pin));
        assert_eq!(f.driver.next_wake_ns(), Some(expires));
    }
    let error = f.driver.acknowledge_checkpoint(&pin, expires, &f.authority, &NeverCancel)
        .err().ok_or("late pin acknowledgement renewed pause")?;
    assert!(matches!(error.reason, LiveArchiveError::StoragePauseExpired));
    assert_eq!(error.retirement.ok_or("retirement")?.pending_checkpoint, Some(pin));
    assert_eq!(f.driver.next_wake_ns(), None); Ok(())
}
...
fn cancellation_at_the_work_barrier_never_publishes_an_ordinary_window() -> Test {
    let mut p = LocalRootPublisher::open(fresh()?, limits())?;
    let mut f = Fixture::playing(&mut p, 4096)?; let admission = f.window()?; let pin = f.pin(14)?;
    f.driver.acknowledge_checkpoint(&pin, 14, &f.authority, &NeverCancel)?;
    let error = f.driver.poll(ready(), 14, &f.authority, &Cancel).err().ok_or("cancellation ignored")?;
    assert!(matches!(error.reason, LiveArchiveError::Archive(ArchiveError::Cancelled)));
    let retired = error.retirement.ok_or("retirement")?;
    assert_eq!(retired.pending_checkpoint, Some(pin)); assert!(retired.last_durable_checkpoint.is_none());
    assert_eq!(retired.live.archive.ok_or("archive")?.pending.ok_or("pending")?.manifest().root(), admission.root);
    assert!(f.driver.cancel().is_none()); drop(f); assert_eq!(p.visible_roots().count(), 0); Ok(())
}
...
fn an_uncertain_auxiliary_root_closes_the_socket_and_retains_original_window_and_pin() -> Test {
    ...
    let namespace = {
        ...
        let mut f = Fixture::playing(&mut p, 7)?; let admission = f.window()?; let pin = f.pin(14)?;
        f.driver.acknowledge_checkpoint(&pin, 14, &f.authority, &NeverCancel)?;
        let error = f.driver.poll(ready(), 14, &f.authority, &NeverCancel).err().ok_or("injected crash did not stop")?;
        let retired = error.retirement.ok_or("retirement missing")?;
        assert_eq!(retired.pending_checkpoint, Some(pin)); assert!(retired.last_durable_checkpoint.is_none());
        assert!(retired.live.recording.is_some());
...
fn recovered_tail_requires_its_page_checkpoint_before_any_new_camera_command() -> Test {
    ...
    {
        let mut p = LocalRootPublisher::open(&path, limits())?;
        let mut f = Fixture::playing(&mut p, 7)?; f.window()?; let pin = f.pin(14)?; f.protect(&pin, 14)?;
        assert!(matches!(f.poll(14)?, CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Archive(
            ArchiveWriteProgress::WindowDurable { .. }))));
        let retired = f.driver.cancel().ok_or("retirement")?;
        assert_eq!(retired.live.archive.ok_or("archive")?.snapshot.unindexed_windows().len(), 1);
    }
    let mut p = LocalRootPublisher::open(&path, limits())?;
    let mut f = Fixture::new(&mut p, 7, ArchiveWorkLimits::default())?;
    assert!(matches!(f.poll(0)?, CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Archive(ArchiveWriteProgress::PageStarted { .. }))));
    assert!(matches!(f.poll(0)?, CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Archive(ArchiveWriteProgress::PageWindowVerified { .. }))));
    assert!(matches!(f.poll(0)?, CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Archive(ArchiveWriteProgress::CatalogPrepared { .. }))));
    let pin = f.pin(0)?;
    assert!(matches!(f.driver.request(ClientCommand::Describe, &credentials()?, [40; 16], 0, &f.authority),
        Err(CheckpointedLiveArchiveFailure { reason: LiveArchiveError::Backpressure, retirement: None })));
    assert_eq!(f.driver.totals().ok_or("totals")?.sent_bytes, 0);
    assert_eq!(f.driver.totals().ok_or("totals")?.read_calls, 0);
    f.protect(&pin, 0)?;
    assert!(matches!(f.poll(0)?, CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Archive(ArchiveWriteProgress::CatalogIndexStaged { .. }))));
    assert!(matches!(f.poll(0)?, CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Archive(ArchiveWriteProgress::CatalogPublished { .. }))));
    assert!(matches!(f.poll(0)?, CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Archive(ArchiveWriteProgress::Ready { .. }))));
    assert_eq!(f.driver.snapshot().ok_or("snapshot")?.indexed_windows(), 1);
    assert_eq!(f.command(ClientCommand::Describe, 0)?, 1); Ok(())
}
...
fn real_eof_flushes_a_protected_final_page_without_becoming_owner_stop_or_repeated_completion() -> Test {
    ...
    let (pins, cause) = f.finish(15)?;
    assert_eq!(cause, LiveArchiveCompletion::InputEnded); assert_eq!(pins.len(), 1);
    assert_eq!(f.driver.snapshot().ok_or("snapshot")?.indexed_windows(), 1);
    assert!(matches!(f.poll(16)?, CheckpointedLiveArchiveStep::Live(LiveArchiveStep::Ended)));
    assert_eq!(f.driver.next_wake_ns(), None); Ok(())
}
