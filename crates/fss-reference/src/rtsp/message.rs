...
pub struct RtspResponse {
    ...
    pub body: Vec<u8>,
    /// Authentication challenge scheme if presented on the response (e.g. "Basic" or "Digest").
...
impl RtspParser {
    ...
    /// Reset parser state and clear all buffered data, pending errors, and discard counters.
    ...
    fn extract_declared_content_length(header_text: &str) -> usize {
    ...
    pub fn feed(&mut self, incoming: &[u8]) -> Result<Vec<RtspEvent>, RtspError> {
        ...
        loop {
            ...
            let (start_line_str, headers, auth_scheme) = match self.parse_headers(header_text) {
                Ok(res) => res,
                Err(err) => {
                    let declared_body = Self::extract_declared_content_length(header_text);
                    self.discard_message_with_declared_body(
                        header_boundary.start_of_body,
                        declared_body,
                    );
                    if !events.is_empty() {
                        self.pending_error = Some(err);
            ...
            let content_len = match headers.content_length() {
                Some(Ok(len)) => {
                    if len > self.limits.max_body_bytes {
                        self.discard_message_with_declared_body(header_boundary.start_of_body, len);
    ...
    fn parse_headers<'a>(
        ...
        for raw_line in lines {
            ...
            if let Some((name, val)) = line.split_once(':') {
                ...
                // Forbid userinfo in URI headers
                if (name.eq_ignore_ascii_case("content-base")
                    || name.eq_ignore_ascii_case("content-location"))
                    && has_userinfo(val)
                {
                    return Err(RtspError::UserinfoNotPermitted(
                        "header contains forbidden userinfo credentials".to_string(),
                    ));
                }
                ...
                return Err(RtspError::MalformedStartLine(
                    "header line missing colon separator".to_string(),
                ));
            }
        }
    ...
    fn build_event(
        ...
        if start_line.starts_with("RTSP/") {
            ...
            if has_userinfo(uri) {
                return Err(RtspError::UserinfoNotPermitted(
                    "URI contains forbidden userinfo credentials".to_string(),
                ));
            }
