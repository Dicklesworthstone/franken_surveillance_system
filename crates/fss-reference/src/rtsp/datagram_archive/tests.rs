...
fn scope() -> Test<DatagramScope> {
    Ok(DatagramScope {
        ...
        channels: (0, 1),
        receive_clock: ContentDigest::sha256(b"receive epoch"),
...
fn add(
    a: &mut DatagramArchive,
    p: &mut LocalRootPublisher,
    channel: u8,
    time: u64,
    bytes: &[u8],
) -> Test<DatagramPublication> {
...
fn cold_recovery_replays_order_channel_time_and_original_bytes_not_codec_eof() -> Test {
    ...
    let original: &[(u8, u64, &[u8])] = &[
        (0, 10, b"original RTP padding\0"),
        (1, 10, b"RTCP"),
        (0, 11, b"incomplete FU-A"),
    ];
    let pin = {
        let mut p = dir.open()?;
        let mut a = DatagramArchive::new(scope()?, limits())?;
        for &(channel, time, bytes) in original {
            add(&mut a, &mut p, channel, time, bytes)?;
        }
        a.pin()
    };
    let p = dir.open()?;
    ...
    for &(channel, time, bytes) in original {
        let DatagramReplayStep::Datagram(d) = replay.step(1, &NeverCancel, &mut work())? else {
            return Err("premature exhaustion".into());
        };
        assert_eq!(d.payload(), bytes);
        assert_eq!(
            (d.record().channel, d.record().received_ns),
            (channel, time)
        );
    }
    assert!(
        matches!(replay.step(2, &NeverCancel, &mut work())?, DatagramReplayStep::PrefixExhausted(p) if p == pin)
    );
    assert!(matches!(
        replay.step(3, &NeverCancel, &mut work())?,
        DatagramReplayStep::Ended
    ));
    Ok(())
}
#[test]
fn equal_retransmissions_are_distinct_but_same_prepared_publication_is_idempotent() -> Test {
    ...
    assert_eq!(
        a.publish(&first, &mut p, &NeverCancel, &mut work())?
            .local
            .outcome,
    ...
    assert_eq!(two.record.pin.datagrams, 2);
    assert_eq!(two.record.pin.payload_bytes, 22);
    assert!(
        a.publish(&first, &mut p, &NeverCancel, &mut work())
            .is_err()
    );
...
fn zero_length_and_malformed_datagrams_remain_bounded_evidence() -> Test {
    ...
    let mut p = dir.open()?;
    let mut a = DatagramArchive::new(scope()?, limits())?;
    add(&mut a, &mut p, 0, 0, b"")?;
    add(&mut a, &mut p, 1, 0, b"\xffnot RTCP")?;
    assert_eq!(a.read(1, &p, &NeverCancel, &mut work())?.payload(), b"");
    let recovered = DatagramArchive::recover(
        &p,
        scope()?,
        limits(),
        Some(a.pin()),
...
fn all_root_failure_cuts_preserve_original_plan_and_never_advance_on_error() -> Test {
    for cut in [
        ...
        let mut a = DatagramArchive::new(scope()?, limits())?;
        let before = a.pin();
        let plan = a.prepare_bytes(0, 10, b"unsealed picture source", &mut work())?;
        p.inject_crash_at(cut);
        assert!(matches!(
            a.publish(&plan, &mut p, &NeverCancel, &mut work()),
            Err(DatagramArchiveError::Publication(_))
        ));
        assert_eq!(a.pin(), before);
        assert_eq!(plan.bytes, b"unsealed picture source");
        assert!(p.is_poisoned());
        drop(p);
        let p = dir.open()?;
        let result = DatagramArchive::recover(
            &p,
            scope()?,
            limits(),
            Some(plan.pin()),
...
fn stale_owner_can_reconcile_only_its_exact_last_publication() -> Test {
    ...
    let mut p = dir.open()?;
    let mut a = DatagramArchive::new(scope()?, limits())?;
    let mut stale = DatagramArchive::new(scope()?, limits())?;
    ...
    let different =
        DatagramArchive::new(scope()?, limits())?.prepare_bytes(0, 1, b"other", &mut work())?;
    assert!(
        stale
            .publish(&different, &mut p, &NeverCancel, &mut work())
            .is_err()
    );
...
fn minimum_prefix_allows_verified_descendants_but_rejects_rollback_or_forks() -> Test {
    ...
    for pin in [
        DatagramPin {
            head: ContentDigest::sha256(b"fork"),
            ..first
        },
        DatagramPin {
            datagrams: 3,
        ...
        assert!(
            DatagramArchive::recover(&p, scope()?, limits(), Some(pin), &NeverCancel, &mut work())
                .is_err()
        );
    }
    let last_path = p
...
fn missing_intermediate_root_never_becomes_a_shorter_valid_prefix() -> Test {
    ...
    let mut p = dir.open()?;
    let mut a = DatagramArchive::new(scope()?, limits())?;
    add(&mut a, &mut p, 0, 1, b"one")?;
    add(&mut a, &mut p, 0, 2, b"two")?;
    let path = p
        .root_dir()
        .join("roots")
        .join(format!("{}.root", a.slot(1)?));
    drop(p);
    std::fs::remove_file(path)?;
    let p = dir.open()?;
    assert!(
        DatagramArchive::recover(&p, scope()?, limits(), None, &NeverCancel, &mut work()).is_err()
    );
    Ok(())
}
#[test]
fn scope_changes_cannot_escape_an_occupied_connection_namespace() -> Test {
    ...
    for changed in [
        DatagramScope {
            receive_clock: ContentDigest::sha256(b"other clock"),
            ..scope()?
        },
        DatagramScope {
            retention_evidence: ContentDigest::sha256(b"other policy"),
            ..scope()?
        },
        DatagramScope {
            channels: (2, 3),
...
fn cancellation_work_and_input_refusals_do_not_publish_or_mutate_source() -> Test {
    ...
    assert!(matches!(
        a.publish(&plan, &mut p, &Cancel, &mut work()),
        Err(DatagramArchiveError::Cancelled)
    ));
    for budget in [0, 1, 1024, 4095] {
        assert!(
            a.publish(&plan, &mut p, &NeverCancel, &mut WorkBudget::new(budget))
                .is_err()
        );
        assert_eq!(a.pin(), before);
...
fn all_external_prefix_and_preallocation_limits_are_enforced() -> Test {
    ...
    for bound in [
        ...
        assert!(
            DatagramArchive::recover(&p, scope()?, bound, None, &NeverCancel, &mut work()).is_err()
        );
    }
    assert!(DatagramReplay::new(&a, &p, 5, 0, 10).is_err());
    assert!(a.read(0, &p, &NeverCancel, &mut work()).is_err());
    assert!(a.read(3, &p, &NeverCancel, &mut work()).is_err());
    Ok(())
}
#[test]
...
fn foreign_families_extra_children_and_noncanonical_source_metadata_are_refused() -> Test {
    for extra in [false, true] {
        let dir = Directory::new()?;
        let mut p = dir.open()?;
        let a = DatagramArchive::new(scope()?, limits())?;
        let plan = a.prepare_bytes(0, 1, b"source", &mut work())?;
        let payload = p.stage_object(plan.bytes)?;
        let metadata = p.stage_object(&plan.metadata)?;
        let children = if extra {
            vec![payload, p.stage_object(b"extra")?]
        } else {
            vec![payload]
        };
        ...
        assert!(
            DatagramArchive::recover(&p, scope()?, limits(), None, &NeverCancel, &mut work())
                .is_err()
        );
        for end in 0..plan.metadata.len() {
            assert!(DatagramRecord::decode(&plan.metadata[..end], plan.pin().head).is_err());
        }
        let mut suffix = plan.metadata.clone();
        suffix.push(0);
