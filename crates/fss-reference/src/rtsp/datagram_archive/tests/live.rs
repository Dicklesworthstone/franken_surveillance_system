...
fn config(
    ...
    chunk: usize,
) -> Test<(LiveAvcConfig, LiveRecordingConfig, DatagramScope, Authority)> {
    let mut selected = scope()?;
    selected.binding = TcpBinding::new(
    ...
    let recording = LiveRecordingConfig {
        scope: RecordingScope {
            sensor: SensorId::parse("sensor:retained-fixture")?,
            stream: StreamId::parse("stream:retained-fixture")?,
            generation: selected.binding.key().generation,
            anchor: ContentDigest::sha256(b"owner anchor"),
...
impl Fixture {
    fn new(p: &mut LocalRootPublisher, chunk: usize, bounds: DatagramArchiveLimits) -> Test<Self> {
        ...
        let driver = RetainedAvcRecording::connect(
            live,
            recording,
            scope,
            bounds,
    ...
    fn receive(
        ...
        bytes: &[u8],
        now: u64,
    ) -> Test<Vec<RetainedRecordingStep>> {
        self.peer.write_all(bytes)?;
        let mut raw = 0;
        let mut outputs = Vec::new();
        for _ in 0..32_768 {
            let before = self.driver.totals();
            let step = self.poll(p, now)?;
            if let Some(publication) = &step.datagram {
                assert_eq!(
                    before,
                    self.driver.totals(),
                    "source publication must not read ahead"
                );
                assert_eq!(
                    p.root(&publication.local.slot).ok_or("root missing")?.state,
                    LocalPublicationState::Durable
                );
                assert_eq!(
                    ContentDigest::sha256(
                        media_source(&step.event).ok_or("source missing")?.payload()
                    ),
                    publication.record.payload_digest
                );
            }
            match &step.event {
                LiveRecordingStep::Network(LiveAvcStep::Wire(chunk)) => {
                    raw += chunk.expose().len();
                }
                LiveRecordingStep::Network(LiveAvcStep::Pending(wait)) => {
                    if raw == bytes.len() && wait.wake_at_ns.is_none_or(|at| at > now) {
                        return Ok(outputs);
                    }
    ...
    fn playing(
        ...
        f.receive(
            p,
            &response(seq, "Session: private-fixture-token\r\n", ""),
            5,
        )?;
        assert_eq!(f.driver.pin().datagrams, 0);
        assert_eq!(p.visible_roots().count(), 0);
        Ok(f)
    }
    fn probation(&mut self, p: &mut LocalRootPublisher) -> Test<Vec<u8>> {
        let data = nals();
        let sps = data.iter().find(|n| n[0] & 31 == 7).ok_or("SPS")?;
        let bytes = packet(1, false, sps);
        self.receive(p, &wire(0, &bytes)?, 10)?;
        Ok(bytes)
    }
    fn idr(&mut self, p: &mut LocalRootPublisher) -> Test<Vec<u8>> {
        self.probation(p)?;
        let data = nals();
        let idr = data.iter().find(|n| n[0] & 31 == 5).ok_or("IDR")?;
        let bytes = packet(2, true, idr);
        let outputs = self.receive(p, &wire(0, &bytes)?, 11)?;
        assert!(outputs.iter().any(|s| matches!(
            s.event.capture_event(),
            Some(CapturePoll::TimingRequired(_))
        )));
        assert_eq!(self.driver.pin().datagrams, 2);
        Ok(bytes)
    }
}
...
fn native_datagrams_precede_timing_and_same_owner_recording_publication_for_all_chunk_sizes() -> Test
{
    for chunk in [1, 7, 4096] {
        ...
        let bad = f
            ...
            .err()
            .ok_or("bad timing accepted")?;
        assert!(bad.retirement.is_none());
        assert_eq!(f.driver.pin(), source_pin);
        let _ = f.driver.supply_timing(
            RecordingTiming {
                decode_time: 700,
                duration: 3600,
                composition_offset: 0,
        ...
        let RetainedRecordingStep {
            event: LiveRecordingStep::Capture { event, .. },
            datagram: None,
        } = f.poll(&mut p, 13)?
        else {
            return Err("sealed window missing".into());
        };
        ...
        let slot = SlotName::parse("retained-window")?;
        let mut job =
            RecordingPublication::new(&window, &mut p, slot.clone(), window.byte_len(), LEASE)?;
        let mut done = false;
        for _ in 0..5 {
            if matches!(job.step(14, &NeverCancel)?, RecordingProgress::Published(_)) {
                done = true;
                break;
            }
        }
        ...
        drop(job);
        drop(window);
        let recording_scope = f.recording.clone();
        let _ = f.driver.cancel();
        drop(f);
        drop(p);
        let p = dir.open()?;
        let archive = DatagramArchive::recover(
            &p,
            scope,
            limits(),
            Some(source_pin),
        ...
        assert_eq!(
            archive.read(2, &p, &NeverCancel, &mut work())?.payload(),
            idr
        );
        assert_eq!(
            load_recording(&p, &slot, root, &recording_scope, &NeverCancel)?
                .summary()
                .decode_interval,
            700..4300
        );
...
fn unsealed_fragment_survives_process_state_loss_without_becoming_a_frame_or_eof() -> Test {
    ...
    assert!(!output.iter().any(|s| matches!(
        s.event.capture_event(),
        Some(CapturePoll::Window(_) | CapturePoll::TimingRequired(_))
    )));
    let scope = f.driver.scope().clone();
    let pin = f.driver.pin();
    assert_eq!(pin.datagrams, 2);
    ...
    assert!(matches!(
        replay.step(21, &NeverCancel, &mut work())?,
        DatagramReplayStep::Datagram(_)
    ));
    let DatagramReplayStep::Datagram(source) = replay.step(21, &NeverCancel, &mut work())? else {
        return Err("fragment missing".into());
    };
    assert_eq!(source.payload(), bytes);
    assert!(matches!(
        replay.step(21, &NeverCancel, &mut work())?,
        DatagramReplayStep::PrefixExhausted(_)
    ));
    Ok(())
}
#[test]
fn repeated_rtp_and_invalid_rtcp_are_retained_without_promoting_their_admission() -> Test {
    ...
    let a = DatagramArchive::recover(
        &p,
        f.driver.scope().clone(),
        limits(),
        Some(f.driver.pin()),
        &NeverCancel,
        &mut work(),
...
fn source_capacity_failure_returns_the_exact_withheld_datagram_and_stops_capture() -> Test {
    let dir = Directory::new()?;
    let mut p = dir.open()?;
    let mut f = Fixture::playing(
        &mut p,
        4096,
        DatagramArchiveLimits {
            max_datagrams: 1,
            ..limits()
        },
    )?;
    ...
    let idr = data.iter().find(|n| n[0] & 31 == 5).ok_or("IDR")?;
    let bytes = packet(2, true, idr);
    f.peer.write_all(&wire(0, &bytes)?)?;
    for _ in 0..32768 {
        match f.poll(&mut p, 11) {
            Ok(step) => assert!(!matches!(
                step.event.capture_event(),
                Some(CapturePoll::TimingRequired(_) | CapturePoll::Window(_))
            )),
            Err(failure) => {
                assert!(matches!(
                    failure.reason,
                    RetainedRecordingError::Custody(DatagramArchiveError::Limit)
                ));
                let r = failure.retirement.ok_or("retirement lost")?;
                assert_eq!(
                    media_source(r.trigger.as_deref().ok_or("source trigger lost")?)
                        .ok_or("source lost")?
                        .payload(),
                    bytes
                );
                assert_eq!(r.prefix.datagrams, 1);
                assert!(r.capture.is_some());
                assert!(matches!(
                    f.poll(&mut p, 12)?.event,
                    LiveRecordingStep::Ended
                ));
                return Ok(());
            }
        }
...
fn lost_datagram_root_ack_preserves_candidate_and_recovers_without_recapturing() -> Test {
    ...
    for _ in 0..32768 {
        if let Err(failure) = f.poll(&mut p, 10) {
            ...
            assert_eq!(r.prefix.datagrams, 0);
            assert!(r.publication.is_none());
            assert_eq!(
                media_source(r.trigger.as_deref().ok_or("trigger")?)
                    .ok_or("source")?
                    .payload(),
                bytes
            );
    ...
    let pin = pin.ok_or("candidate missing")?;
    ...
    let a = DatagramArchive::recover(&p, scope, limits(), Some(pin), &NeverCancel, &mut work())?;
    assert_eq!(a.pin(), pin);
    assert_eq!(a.read(1, &p, &NeverCancel, &mut work())?.payload(), bytes);
    Ok(())
}
#[test]
...
fn scope_mismatch_and_occupied_source_refuse_before_a_new_network_attempt() -> Test {
    ...
    let mut f = Fixture::playing(&mut p, 4096, limits())?;
    f.probation(&mut p)?;
    let peer = f.driver.scope().binding.peer();
    let _ = f.driver.cancel();
    drop(f);
    let (live, recording, scope, authority) = config(peer, 4096)?;
    let failure = RetainedAvcRecording::connect(
        live,
        recording,
        scope,
        limits(),
    ...
    .err()
    .ok_or("occupied connection reused")?;
    assert!(matches!(
        failure.reason,
        RetainedRecordingError::ExistingSource
    ));
    assert!(!failure.connection_attempted);
    assert_eq!(authority.checks.get(), 0);
    let (live, recording, mut scope, authority) = config(peer, 4096)?;
    scope.receive_clock = ContentDigest::sha256(b"wrong clock");
    let failure = RetainedAvcRecording::connect(
        live,
        recording,
        scope,
...
fn switching_publishers_stops_input_and_does_not_start_a_second_source_history() -> Test {
    ...
    assert!(matches!(
        failure.reason,
        RetainedRecordingError::Configuration
    ));
    assert_eq!(failure.retirement.ok_or("retirement")?.prefix, pin);
    assert_eq!(q.visible_roots().count(), 0);
    Ok(())
}
#[test]
fn replay_clock_regression_is_safe_but_deadline_stops_the_attempt() -> Test {
    ...
    assert!(matches!(
        replay.step(9, &NeverCancel, &mut work()),
        Err(DatagramArchiveError::ClockReversed)
    ));
    assert!(matches!(
        replay.step(10, &NeverCancel, &mut work())?,
        DatagramReplayStep::Datagram(_)
    ));
    assert!(matches!(
        replay.step(20, &NeverCancel, &mut work()),
        Err(DatagramArchiveError::Deadline)
    ));
    assert!(matches!(
        replay.step(20, &NeverCancel, &mut work()),
        Err(DatagramArchiveError::Stopped)
    ));
    Ok(())
}
...
fn post_commit_cancellation_retains_the_actual_receipt_with_the_withheld_source() -> Test {
    ...
    impl PublishCancellation for CancelOnRoot {
        fn cancel_requested(&self, _: PublishCutPoint) -> bool {
            std::fs::read_dir(&self.0).map_or(true, |entries| {
                entries.take(129).any(|entry| {
                    entry.map_or(true, |entry| {
                        entry.path().extension().is_some_and(|e| e == "root")
                    })
                })
            })
        }
    ...
    f.peer.write_all(&wire(0, &bytes)?)?;
    for _ in 0..32768 {
        if let Err(failure) = f.driver.poll(
            SocketReadiness {
                readable: true,
                writable: true,
            },
            ...
            assert!(matches!(
                failure.reason,
                RetainedRecordingError::Custody(DatagramArchiveError::Cancelled)
            ));
            let retired = failure.retirement.ok_or("retirement")?;
            let publication = retired
                .publication
                .as_ref()
                .ok_or("durable receipt discarded")?;
            assert_eq!(publication.record.pin, retired.prefix);
            assert_eq!(retired.candidate, Some(retired.prefix));
            assert_eq!(
                publication.local.claims.local,
                LocalPublicationState::Durable
            );
            assert_eq!(
                media_source(retired.trigger.as_deref().ok_or("trigger")?)
                    .ok_or("source")?
                    .payload(),
                bytes
            );
...
fn terminal_bad_rtp_retains_its_original_source_without_changing_the_failure_to_eof() -> Test {
    ...
    for _ in 0..32768 {
        ...
        if let LiveRecordingStep::Stopped { .. } = &step.event {
            assert_eq!(
                media_source(&step.event)
                    .ok_or("terminal source discarded")?
                    .payload(),
                bytes
            );
            let publication = step
                .datagram
                .as_ref()
                .ok_or("failed RTP source not retained")?;
            assert_eq!(
                publication.record.payload_digest,
            ...
            assert_eq!(
                p.root(&publication.local.slot).ok_or("source root")?.state,
                LocalPublicationState::Durable
            );
            assert!(matches!(
                f.poll(&mut p, 11)?.event,
        ...
        assert!(!matches!(
            step.event.capture_event(),
            Some(CapturePoll::Window(_) | CapturePoll::Ended { .. })
        ));
    }
