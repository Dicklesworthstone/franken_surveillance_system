...
fn scope() -> std::result::Result<DatagramScope, Box<dyn std::error::Error>> {
    Ok(DatagramScope {
        ...
        channels: (0, 1),
        receive_clock: ContentDigest::sha256(b"prefix clock"),
...
) -> std::result::Result<DatagramPrefix, Box<dyn std::error::Error>> {
    Ok(DatagramPrefix::recover(
        p,
        scope()?,
        limits(),
        selected,
...
fn cold_selection_preserves_old_input_and_reports_the_distinct_verified_head() -> Test {
    ...
    assert_eq!(prefix.observed_head(), head);
    assert_eq!(prefix.archive().records().len(), 1);
    assert_eq!(
        prefix.read(1, &p, &NeverCancel, &mut work())?.payload(),
        b"first original"
    );
...
fn every_prefix_pin_field_must_match_an_actual_position() -> Test {
    ...
    let mut a = DatagramArchive::new(scope()?, limits())?;
    let selected = append(&mut a, &mut p, b"one")?;
    append(&mut a, &mut p, b"two")?;
    for wrong in [
        DatagramPin {
            head: ContentDigest::sha256(b"forged"),
            ..selected
        },
        DatagramPin {
            scope: ContentDigest::sha256(b"other scope"),
            ..selected
        },
        DatagramPin {
            datagrams: 2,
...
fn missing_selected_root_and_missing_intermediate_descendant_both_refuse() -> Test {
    for ordinal in [1, 2] {
        ...
        let selected = append(&mut a, &mut p, b"one")?;
        append(&mut a, &mut p, b"two")?;
        append(&mut a, &mut p, b"three")?;
        let root_file = p
            .root_dir()
            .join("roots")
            .join(format!("{}.root", a.slot(ordinal)?));
        drop(p);
...
fn all_uncertain_descendant_write_cuts_keep_ordinary_recovery_semantics() -> Test {
    for cut in [
        ...
        let mut a = DatagramArchive::new(scope()?, limits())?;
        let selected = append(&mut a, &mut p, b"one")?;
        let plan = a.prepare_bytes(0, 20, b"next", &mut work())?;
        let candidate = plan.pin();
...
fn independent_limits_cover_full_current_chain_not_just_selected_records() -> Test {
    ...
    for ceiling in [
        ...
        assert!(
            DatagramPrefix::recover(&p, scope()?, ceiling, selected, &NeverCancel, &mut work())
                .is_err()
        );
    }
