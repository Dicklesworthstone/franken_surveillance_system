...
use crate::rtsp::authentication::{AuthenticationError, DigestChallenge, DigestCredentials,
    DigestPolicy, MAX_AUTHORIZATION_BYTES, MAX_CHALLENGE_BYTES};
...
impl From<ClientError> for DigestClientError { fn from(e: ClientError) -> Self { Self::Session(e) } }
impl From<AuthenticationError> for DigestClientError { fn from(e: AuthenticationError) -> Self { Self::Authentication(e) } }
impl fmt::Display for DigestClientError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { write!(f, "RTSP Digest client refusal: {self:?}") }
}
...
impl DigestState {
    pub(super) fn settled(&mut self) { self.template.clear(); self.attempts = 0; }
    fn request(&mut self, session: &mut RtspClientSession, command: ClientCommand,
        credentials: &DigestCredentials<'_>, cnonce: [u8; 16], now: u64) -> Result<ClientRequest>
    {
        session.tick(now)?;
        if session.pending.is_some() { return Err(ClientError::State.into()); }
        ...
        let counter = self.nonce_count.checked_add(1).ok_or(AuthenticationError::Replay)?;
        let authorization = self.challenge.as_ref().map(|c| c.authorize(method, uri, credentials, cnonce, counter)).transpose()?;
        let mut signed = String::new();
        signed.try_reserve_exact(MAX_URI + 512 + MAX_AUTHORIZATION_BYTES + 32).map_err(|_| AuthenticationError::Capacity)?;
        ...
        if let Some(value) = &authorization {
            signed.push_str("Authorization: "); signed.push_str(value.expose()); signed.push_str("\r\n");
        }
        signed.push_str("\r\n");
        self.template = unsigned.bytes;
        self.attempts = u8::from(authorization.is_some());
        if authorization.is_some() { self.nonce_count = counter; }
        Ok(ClientRequest { command, cseq: unsigned.cseq, bytes: signed })
    }
    fn retry(&mut self, session: &mut RtspClientSession, wire: &[u8], credentials: &DigestCredentials<'_>,
        cnonce: [u8; 16], now: u64) -> Result<ClientRequest>
    {
        ...
        if cseq != u64::from(pending.cseq) { return Err(ClientError::CseqMismatch.into()); }
        session.tick(now)?;
        if self.template.is_empty() { return Err(AuthenticationError::State.into()); }
        if self.attempts >= 2 { return Err(AuthenticationError::RetryLimit.into()); }
        if let Some(value) = singleton(&response.headers, "Session")? {
            let (id, _) = super::session(value, session.timeout_ns)?;
            if session.session_id.as_deref() != Some(id) { return Err(ClientError::Session.into()); }
        }
        ...
        if self.attempts != 0 {
            ...
            if !challenge.stale() || challenge.same_nonce(previous) || !challenge.no_weaker_than(previous) {
                return Err(AuthenticationError::Replay.into());
            }
        }
        if self.used_nonces.iter().any(|old| challenge.same_nonce(old)) { return Err(AuthenticationError::Replay.into()); }
        if self.used_nonces.len() + usize::from(self.challenge.is_some()) >= 16 { return Err(AuthenticationError::Capacity.into()); }
        self.used_nonces.try_reserve_exact(1).map_err(|_| AuthenticationError::Capacity)?;
        ...
        let (start, rest) = self.template.split_once("\r\nCSeq: ").ok_or(AuthenticationError::State)?;
        let (_, remaining) = rest.split_once("\r\n").ok_or(AuthenticationError::State)?;
        let mut template = String::new();
        template.try_reserve_exact(MAX_URI + 512).map_err(|_| AuthenticationError::Capacity)?;
        write!(&mut template, "{start}\r\nCSeq: {new_cseq}\r\n{remaining}").map_err(|_| AuthenticationError::Capacity)?;
        let mut signed = String::new();
        signed.try_reserve_exact(MAX_URI + 512 + MAX_AUTHORIZATION_BYTES + 32).map_err(|_| AuthenticationError::Capacity)?;
        signed.push_str(&template[..template.len() - 2]);
        signed.push_str("Authorization: "); signed.push_str(authorization.expose()); signed.push_str("\r\n\r\n");
        ...
        session.pending = Some(Pending { cseq: new_cseq, ..pending });
        if let Some(previous) = self.challenge.replace(challenge) { self.used_nonces.push(previous); }
        self.nonce_count = 1; self.attempts += 1; self.template = template;
        Ok(ClientRequest { command: pending.command, cseq: new_cseq, bytes: signed })
    }
}
...
impl RtspClientSession {
    ...
    pub fn enable_digest(&mut self, realm: &str, policy: DigestPolicy) -> Result<()> {
        if self.state != ClientState::Idle || self.pending.is_some() || self.next_cseq != 1 || self.digest.is_some() {
            return Err(ClientError::State.into());
        }
        if realm.len() > 512 || !realm.bytes().all(|b| (32..127).contains(&b)) { return Err(AuthenticationError::Realm.into()); }
        let mut pinned = String::new();
        pinned.try_reserve_exact(realm.len()).map_err(|_| AuthenticationError::Capacity)?;
        pinned.push_str(realm);
        self.digest = Some(DigestState { realm: pinned, policy, challenge: None, nonce_count: 0,
            attempts: 0, template: String::new(), used_nonces: Vec::new() });
        Ok(())
    }
    ...
    pub fn retry_digest_response(&mut self, response: &[u8], credentials: &DigestCredentials<'_>,
        cnonce: [u8; 16], now: u64) -> Result<ClientRequest>
    {
        let mut state = self.digest.take().ok_or(AuthenticationError::State)?;
...
fn challenge_response(wire: &[u8]) -> Result<(RtspResponse, &str)> {
    ...
    if wire.len() > 135_168 { return Err(AuthenticationError::Capacity.into()); }
    let end = wire.windows(4).position(|s| s == b"\r\n\r\n").ok_or(AuthenticationError::Response)?;
    ...
    for line in header.split("\r\n").skip(1) {
        ...
        if name.eq_ignore_ascii_case("WWW-Authenticate") {
            if challenge.is_some() || value.trim().len() > MAX_CHALLENGE_BYTES { return Err(AuthenticationError::Response.into()); }
            challenge = Some(value.trim());
        }
        if name.eq_ignore_ascii_case("Content-Length") {
            if length.is_some() { return Err(AuthenticationError::Response.into()); }
            let parsed = decimal(value.trim())?;
            if parsed > 65_536 { return Err(AuthenticationError::Capacity.into()); }
            length = Some(parsed as usize);
        }
        if name.eq_ignore_ascii_case("Proxy-Authenticate") { return Err(AuthenticationError::Unsupported.into()); }
    }
    // Reject concatenated frames before parsing can allocate an event batch.
    if end + 4 + length.unwrap_or(0) != wire.len() { return Err(AuthenticationError::Response.into()); }
    let mut parser = RtspParser::with_limits(RtspLimits { max_line_bytes: 2_048, max_headers: 32,
