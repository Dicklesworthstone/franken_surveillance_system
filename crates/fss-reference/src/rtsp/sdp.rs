...
pub fn parse_sdp(input: &str) -> Result<SdpSession, SdpError> {
    ...
    for raw_line in input.lines() {
        if raw_line.len() > MAX_SDP_LINE_BYTES {
            return Err(SdpError::Limit(format!(
                "SDP line length {} exceeds limit {MAX_SDP_LINE_BYTES}",
                raw_line.len()
            )));
        }
...
impl MediaSectionBuilder {
    fn new(m_line_val: &str) -> Result<Self, SdpError> {
        ...
        if payload_type > 127 {
            return Err(SdpError::Malformed(format!(
                "RTP payload type {payload_type} exceeds 127"
            )));
        }
    ...
    fn parse_rtpmap(&mut self, val: &str) -> Result<(), SdpError> {
        ...
        if pt > 127 {
            return Err(SdpError::Malformed(format!(
                "RTP payload type {pt} exceeds 127"
            )));
        }
            ...
            if let Some((k, v)) = param.split_once('=') {
                ...
                if k.eq_ignore_ascii_case("packetization-mode") {
                    let pm = v.parse::<u8>().map_err(|_| {
                        SdpError::Malformed(format!("invalid packetization-mode '{v}'"))
                    })?;
                    if pm > 2 {
                        return Err(SdpError::Malformed(format!(
                            "packetization-mode {pm} outside valid range 0..=2"
                        )));
                    }
