...
impl std::fmt::Debug for HevcCollectedSource {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HevcCollectedSource")
...
impl std::fmt::Debug for HevcRecordingCollector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HevcRecordingCollector")
            .field("key", &self.key)
...
impl HevcRecordingCollector {
    ...
    pub fn next_wake_ns(&self) -> Option<u64> {
        ...
        self.sources
            .first()
            .and_then(|p| p.admitted_ns.checked_add(self.limits.max_age_ns))
    }
    /// Transfer prepared bytes exactly once, not a durable-publication receipt.
    pub fn take_ready(&mut self) -> Option<PreparedHevcRecording> {
        self.ready.take()
    }
    ...
    pub fn push_source(
        ...
        if key != self.key {
            return Err(E::StreamMismatch);
        }
        let packet =
            RtpPacket::parse(source.bytes, PacketLimits::default()).map_err(|_| E::Source)?;
        if packet.ssrc() != key.ssrc || packet.payload_type() != self.payload_type {
            return Err(E::StreamMismatch);
        }
        ...
        if self
            .last_source
            .is_some_and(|last| last.checked_add(1) != Some(source.sequence))
        {
            return Err(E::SourceOrder);
        }
        ...
        self.sources.push(HevcCollectedSource {
            sequence: source.sequence,
            received_ns: source.received_ns,
            admitted_ns: now,
            bytes,
    ...
    pub fn push_picture(
        &mut self,
        picture: &HevcPictureGroup,
        timing: HevcRecordingTiming,
        now: u64,
    ) -> Result<HevcCollectionAdmission> {
        self.check_admission(now)?;
        let end = timing
            .decode_time
            .checked_add(u64::from(timing.duration))
            .ok_or(E::Timeline)?;
        if timing.duration == 0
            || timing
                .decode_time
                .checked_add_signed(i64::from(timing.composition_offset))
                .is_none()
            || self.last_end.is_some_and(|last| timing.decode_time != last)
        {
            return Err(E::Timeline);
        }
        ...
        if !waiting
            && (samples == self.limits.max_samples
                ...
                || receipt.source_spans > self.limits.max_source_spans.saturating_sub(spans))
        {
            return Err(E::Capacity);
        }
        let release_before = if waiting {
            Some(receipt.last.0)
        } else if cut || self.pictures.is_empty() {
            Some(receipt.first.0)
        } else {
            None
        };
        let release_count =
            release_before.map_or(0, |seq| self.sources.partition_point(|s| s.sequence < seq));
        ...
        for packet in self.sources.drain(..release_count) {
            self.source_bytes -= packet.bytes.len();
            released.push(packet);
        }
        self.last_picture = Some(receipt.last);
        self.last_end = Some(end);
        self.last_ns = now;
        if waiting {
            return Ok(HevcCollectionAdmission::AwaitingIdr { released });
        }
        self.picture_bytes += receipt.bytes;
        self.nals += receipt.nals;
        self.spans += receipt.source_spans;
        self.pictures.push(receipt);
        Ok(HevcCollectionAdmission::Accepted {
            window_ready: cut,
            released,
        })
    }
    ...
    pub fn seal(&mut self, now: u64) -> Result<HevcCollectionSeal> {
        ...
        for p in self.sources.drain(..count) {
            self.source_bytes -= p.bytes.len();
            released.push(p);
        }
        self.clear_pictures();
        self.ready = Some(plan);
        self.last_ns = now;
        Ok(HevcCollectionSeal {
            window_ready: true,
            released,
        })
    }
    ...
    pub fn finish(&mut self, now: u64) -> Result<HevcCollectionSeal> {
        ...
        self.closed = true;
        self.stop_reason = Some(CollectionStop::EndOfInput);
        Ok(sealed)
    }
    ...
    pub fn invalidate(&mut self) -> HevcCollectionRetirement {
        self.retire(CollectionStop::InputDiscontinuity)
    }
    ...
    pub(crate) fn check_admission(&self, now: u64) -> Result<()> {
        ...
        if self.next_wake_ns().is_some_and(|at| now >= at) {
            return Err(E::Deadline);
        }
        if self.ready.is_some() {
            return Err(E::Backpressure);
        }
        Ok(())
    }
    ...
    pub(crate) fn retire(&mut self, reason: CollectionStop) -> HevcCollectionRetirement {
        ...
        self.nals = 0;
        self.spans = 0;
        HevcCollectionRetirement {
            key: self.key,
            reason,
            ready: self.ready.take(),
            sources: std::mem::take(&mut self.sources),
            pictures: std::mem::take(&mut self.pictures),
        }
    }
    fn describe(
        &self,
        p: &HevcPictureGroup,
        timing: HevcRecordingTiming,
    ) -> Result<HevcCollectedPicture> {
        if p.key() != self.key {
            return Err(E::StreamMismatch);
        }
        if p.discontinuity_before()
            || p.boundary() == HevcBoundary::EndOfInputUnverified
            || !p.prefix().first_slice
            || p.slice_count() == 0
        {
            return Err(E::UnverifiedPicture);
        }
        if !matches!(p.prefix().nal_type, 0..=5 | 19 | 20) {
            return Err(E::UnverifiedPicture);
        }
        if p.prefix().pps_id != self.configuration.pps_id() {
            return Err(E::ConfigurationChanged);
        }
        if p.nals().len() > self.limits.max_nals
            || p.source_span_count() > self.limits.max_source_spans
            || p.byte_len() > self.limits.max_picture_bytes
        {
            return Err(E::Capacity);
        }
        let mut first = None;
        ...
        for nal in p.nals() {
            ...
            if expected.is_some_and(|b| b != nal.bytes()) {
                return Err(E::ConfigurationChanged);
            }
            for span in nal.sources() {
                let start = span
                    ...
                    .as_ref()
                    .map_or(span.wire_range.start, |r| r.start);
        ...
        Ok(HevcCollectedPicture {
            ...
            nals: p.nals().len(),
            bytes: p.byte_len(),
            source_spans: p.source_span_count(),
            content,
            first: first.ok_or(E::MissingSource)?,
    ...
    fn starts_packet(&self, p: &HevcPictureGroup) -> Result<bool> {
        let span = p
            ...
            .ok_or(E::MissingSource)?;
        let index = self
            .sources
            .binary_search_by_key(&span.sequence, |s| s.sequence)
            .map_err(|_| E::MissingSource)?;
        let packet = RtpPacket::parse(&self.sources[index].bytes, PacketLimits::default())
            .map_err(|_| E::Source)?;
    ...
    fn prepare_prefix(&self) -> Result<PreparedHevcRecording> {
        ...
        .map_err(E::Recording)?;
        if plan.samples().len() != self.pictures.len() {
            return Err(E::Source);
        }
        for (sample, observed) in plan.samples().iter().zip(&self.pictures) {
            if sample.rtp_timestamp != observed.rtp_timestamp
                || sample.boundary != observed.boundary
                || sample.idr != observed.idr
                || sample.mappings.len() != observed.nals
            {
                return Err(E::Source);
            }
            let mappings = plan
                .mappings()
                .get(sample.mappings.clone())
                .ok_or(E::Source)?;
            ...
            for mapping in mappings {
                let bytes = plan
                    .objects()
                    .media
                    .get(mapping.range.clone())
                    .ok_or(E::Source)?;
                inputs.push((bytes, mapping.sources.as_slice()));
            }
            if fingerprint(inputs.into_iter())? != observed.content {
                return Err(E::Source);
            }
        }
        Ok(plan)
...
fn fingerprint<'a>(
    nals: impl Iterator<Item = (&'a [u8], &'a [H265SourceSpan])>,
    ...
    for (bytes, sources) in nals {
        ...
        e.u64(sources.len() as u64);
        for s in sources {
            e.u64(s.sequence);
            e.u64(s.wire_range.start as u64);
            e.u64(s.wire_range.end as u64);
            e.u64(s.nal_range.start as u64);
            e.u64(s.nal_range.end as u64);
...
fn reserved<T>(count: usize, max: usize) -> Result<Vec<T>> {
    ...
    let mut out = Vec::new();
    out.try_reserve_exact(count).map_err(|_| E::Allocation)?;
    Ok(out)
}
