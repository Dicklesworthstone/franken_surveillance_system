...
use crate::rtsp::recording_capture::{CaptureError, CapturePoll, CaptureRetirement, RecordingCapture, TimedCapture};
...
impl<'a> DatagramRecordingReplay<'a> {
    ...
    pub fn new(archive: &'a DatagramArchive, avc: AvcReplaySpec<'_>, recording: RecordingReplaySpec,
        bounds: AvcReplayBounds, now_ns: u64) -> std::result::Result<Self, RecordingReplayError> {
        ...
        let collector = RecordingCollector::new(recording.scope.clone(), archive.scope().binding.key(),
            avc.payload_type, recording.time_scale, recording.limits)
            .map_err(|e| RecordingReplayError::Capture(CaptureError::Collection(e)))?;
        let replay = DatagramAvcReplay::new(archive, avc, bounds, now_ns).map_err(RecordingReplayError::Replay)?;
        let interpretation = interpretation(replay.interpretation(), &recording)?;
        let capture_cost = (recording.limits.max_source_bytes as u64 + recording.limits.max_picture_bytes as u64)
            * 8 + (recording.limits.max_nals as u64 + recording.limits.max_source_spans as u64) * 128 + 4096;
        Ok(Self { replay, capture: Some(RecordingCapture::new(collector)), interpretation,
            prefix: None, finalizing: false, closed: false, now_ns,
            remaining_steps: bounds.max_steps, deadline_ns: bounds.deadline_ns, capture_cost })
    }
    ...
    pub fn observations_read(&self) -> u64 { self.replay.observations_read() }
    /// The independently selected source prefix; no hidden latest-head following.
    pub fn source(&self) -> DatagramPin { self.replay.source() }
    ...
    pub fn supply_timing(&mut self, timing: RecordingTiming, now_ns: u64,
        ...
        if self.finalizing { return Err(safe(RecordingReplayError::Closed)); }
        let result = self.capture.as_mut().ok_or_else(|| safe(RecordingReplayError::Closed))?
        ...
        if let Err(error) = current(cancel, budget) {
            ...
            if let Some(retired) = &mut failure.retirement { retired.withheld_timing = Some(result); }
            return Err(failure);
        }
        Ok(result)
    }
    ...
    pub fn seal(&mut self, now_ns: u64, cancel: &dyn PublishCancellation, budget: &mut WorkBudget<'_>)
        -> std::result::Result<bool, RecordingReplayFailure> {
        self.guard(now_ns, cancel, budget)?;
        if self.finalizing { return Err(safe(RecordingReplayError::Closed)); }
        let result = self.capture.as_mut().ok_or_else(|| safe(RecordingReplayError::Closed))?.seal(now_ns);
        let sealed = result.map_err(|e| self.capture_error(e))?;
        current(cancel, budget).map_err(|e| self.fatal(RecordingReplayError::Replay(e), None, None, None))?;
        Ok(sealed)
    }
    ...
    pub fn finish_prefix(&mut self, now_ns: u64, cancel: &dyn PublishCancellation, budget: &mut WorkBudget<'_>)
        -> std::result::Result<(), RecordingReplayFailure> {
        self.guard(now_ns, cancel, budget)?;
        if self.finalizing { return Err(safe(RecordingReplayError::Closed)); }
        if self.prefix.is_none() { return Err(safe(RecordingReplayError::PrefixNotReady)); }
        let result = self.capture.as_mut().ok_or_else(|| safe(RecordingReplayError::Closed))?.seal(now_ns);
    ...
    pub fn step(&mut self, publisher: &LocalRootPublisher, now_ns: u64,
        ...
        if self.closed { return Ok(RecordingReplayStep::Ended); }
        self.guard(now_ns, cancel, budget)?;
    ...
    fn advance(&mut self, publisher: &LocalRootPublisher, now_ns: u64,
        cancel: &dyn PublishCancellation, budget: &mut WorkBudget<'_>)
        -> std::result::Result<RecordingReplayStep, RecordingReplayFailure> {
        let result = self.capture.as_mut().ok_or_else(|| safe(RecordingReplayError::Closed))?.poll(now_ns);
        match result {
            Err(error) => return Err(self.fatal(RecordingReplayError::Capture(error), None, None, None)),
            Ok(event @ CapturePoll::Stopped { .. }) => {
                ...
                return Ok(RecordingReplayStep::Stopped { trigger: RecordingReplayStop::Collection(Box::new(event)),
        ...
        if self.prefix.is_some() {
            if self.finalizing {
                return Ok(RecordingReplayStep::FinishedPrefix { retained: Box::new(self.retire(None, None, None)) });
            }
            return Ok(self.prefix_ready());
        }
        let step = match self.replay.step(publisher, now_ns, cancel, budget) {
            Ok(step) => step,
            Err(failure) => return Err(match failure.retirement {
                Some(retired) => self.fatal(RecordingReplayError::Replay(failure.reason), Some(*retired), None, None),
                None => safe(RecordingReplayError::Replay(failure.reason)),
            }),
        };
        match step {
            AvcReplayStep::Media { event, .. } => {
                let result = self.capture.as_mut().ok_or_else(|| safe(RecordingReplayError::Closed))?.offer(event, now_ns);
                if let Err(refusal) = result {
                    return Err(self.fatal(RecordingReplayError::Capture(refusal.reason), None, None, Some(refusal.event)));
                }
                Ok(RecordingReplayStep::MediaQueued)
            }
            step @ AvcReplayStep::PrefixExhausted { .. } => {
                self.prefix = Some(Box::new(step)); Ok(self.prefix_ready())
            }
            step @ (AvcReplayStep::InputRefused { .. } | AvcReplayStep::CodecEnded { .. }
                | AvcReplayStep::Rtp { retired: Some(_), .. } | AvcReplayStep::Ended) => {
                let retained = self.retire(None, None, None);
    ...
    pub fn cancel(&mut self) -> Option<RecordingReplayRetirement> {
        if self.closed { None } else { Some(self.retire(None, None, None)) }
    }
    fn prefix_ready(&self) -> RecordingReplayStep {
        RecordingReplayStep::PrefixReady { source: self.source(), interpretation: self.interpretation }
    }
    fn capture_error(&mut self, reason: CaptureError) -> RecordingReplayFailure {
        if matches!(reason, CaptureError::Closed | CaptureError::Collection(CollectorError::Deadline)) {
            self.fatal(RecordingReplayError::Capture(reason), None, None, None)
        } else { safe(RecordingReplayError::Capture(reason)) }
    }
    fn retire(&mut self, replay: Option<AvcReplayRetirement>, withheld: Option<RecordingReplayStep>,
        unoffered_media: Option<Box<AvcReceivePoll>>) -> RecordingReplayRetirement {
        self.closed = true;
        RecordingReplayRetirement { interpretation: self.interpretation, source: self.source(),
            replay: replay.or_else(|| self.replay.cancel()), capture: self.capture.take().map(|mut c| c.cancel()),
            prefix: self.prefix.take(), unoffered_media, withheld: withheld.map(Box::new), withheld_timing: None }
    }
    fn fatal(&mut self, reason: RecordingReplayError, replay: Option<AvcReplayRetirement>,
        withheld: Option<RecordingReplayStep>, unoffered_media: Option<Box<AvcReceivePoll>>) -> RecordingReplayFailure {
        RecordingReplayFailure { reason, retirement: Some(Box::new(self.retire(replay, withheld, unoffered_media))) }
    }
    fn guard(&mut self, now_ns: u64, cancel: &dyn PublishCancellation, budget: &mut WorkBudget<'_>)
        -> std::result::Result<(), RecordingReplayFailure> {
        if self.closed { return Err(safe(RecordingReplayError::Closed)); }
        if now_ns < self.now_ns { return Err(safe(RecordingReplayError::Replay(AvcReplayError::ClockReversed))); }
        let result = if now_ns >= self.deadline_ns { Err(DatagramArchiveError::Deadline.into()) }
            else if self.remaining_steps == 0 { Err(DatagramArchiveError::Limit.into()) }
            else { current(cancel, budget).and_then(|()| budget.charge(self.capture_cost)
        ...
        self.now_ns = now_ns; self.remaining_steps -= 1; Ok(())
    }
}
fn safe(reason: RecordingReplayError) -> RecordingReplayFailure { RecordingReplayFailure { reason, retirement: None } }

fn interpretation(receiver: ContentDigest, spec: &RecordingReplaySpec) -> std::result::Result<ContentDigest, RecordingReplayError> {
    let mut e = CanonicalEncoder::new(); e.text("fss.datagram_recording_interpretation.v1");
    e.digest(receiver); e.digest(spec.timing_evidence);
    e.text(spec.scope.sensor.as_str()); e.text(spec.scope.stream.as_str()); e.u64(spec.scope.generation);
    e.digest(spec.scope.anchor); e.digest(spec.scope.receive_clock); e.u32(spec.time_scale);
    let CollectorLimits { max_packets, max_source_bytes, max_samples, max_picture_bytes,
        max_nals, max_source_spans, max_age_ns } = spec.limits;
    for n in [max_packets, max_source_bytes, max_samples, max_picture_bytes, max_nals, max_source_spans] {
        e.u64(u64::try_from(n).map_err(|_| RecordingReplayError::Configuration)?);
    }
    e.u64(max_age_ns);
    let bytes = e.finish_checked().map_err(|_| RecordingReplayError::Configuration)?;
