...
impl Default for RecipeStorageLimits {
    fn default() -> Self {
        Self {
            max_source_roots: MAX_MANIFEST_CHILDREN.saturating_sub(1),
            max_spool_object_bytes: 32 * 1024 * 1024,
        }
    }
}
impl RecipeStorageLimits {
    fn check(
        ...
        if self.max_source_roots >= MAX_MANIFEST_CHILDREN
            || self.max_spool_object_bytes == 0
...
impl std::fmt::Debug for PreparedRecordingRecipe<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PreparedRecordingRecipe")
            .field("pin", &self.pin)
...
pub fn load_recording_recipe(
    p: &LocalRootPublisher,
    pin: &RecordingRecipePin,
    archive: &DatagramArchive,
    recipe_limits: RecordingRecipeLimits,
    storage_limits: RecipeStorageLimits,
    cancel: &dyn PublishCancellation,
    budget: &mut WorkBudget<'_>,
    ...
    if p.is_broken_slot(&pin.slot)
        || p.root(&pin.slot)
            .is_none_or(|r| r.root != pin.root || r.state != LocalPublicationState::Durable)
    {
        return Err(DatagramArchiveError::NotDurable.into());
    }
    let root_bytes = object(
        p,
        pin.root,
        storage_limits.max_spool_object_bytes,
        cancel,
        budget,
    )?;
    let stored = ObjectManifest::from_canonical_bytes(&root_bytes)
        .map_err(|_| RecordingRecipeError::Mismatch)?;
    if stored.root() != pin.root
        || stored.kind() != RECORDING_RECIPE_KIND
...
fn manifest(
    ...
    budget
        .charge(archive.records().len() as u64 * 128 + 1024)
        .map_err(DatagramArchiveError::Work)?;
    let mut children = Vec::new();
    children
        .try_reserve_exact(archive.records().len())
...
fn slot(recipe: ContentDigest) -> Result<SlotName, RecordingRecipeError> {
    ...
    let hex = text
        .strip_prefix("sha256:")
        .ok_or(RecordingRecipeError::Mismatch)?;
...
fn probe(
    ...
    budget.charge(0).map_err(DatagramArchiveError::Work)?;
    Ok(())
}
fn verify_sources(
    archive: &DatagramArchive,
...
fn object(
    ...
    cancel: &dyn PublishCancellation,
    budget: &mut WorkBudget<'_>,
) -> Result<Vec<u8>, RecordingRecipeError> {
    probe(cancel, budget)?;
    // Charge the actual spool's allocating read ceiling BEFORE I/O, not the untrusted payload size.
    budget
        .charge(p.limits().spool.max_object_bytes as u64 * 3 + p.limits().max_tombstones as u64 + 1)
        .map_err(DatagramArchiveError::Work)?;
    if p.tombstones().any(|d| *d == digest) {
        return Err(DatagramArchiveError::Tombstoned.into());
    }
    let bytes = p
        .spool()
        .read(digest)
        .map_err(DatagramArchiveError::Spool)?;
