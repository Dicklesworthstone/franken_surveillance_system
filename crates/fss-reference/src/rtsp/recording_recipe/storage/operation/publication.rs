    ...
    pub fn publish(
        ...
        let result = (|| -> Result<_, ReconstructionError> {
            ...
            for (index, window) in self.windows.iter().enumerate() {
                ...
                budget
                    .charge(window.byte_len() as u64 * 8 + 4096)
                    .map_err(|e| RecordingRecipeError::Source(DatagramArchiveError::Work(e)))?;
                let slot = self.window_slot(index)?;
                let mut job =
                    RecordingPublication::new(window, p, slot, window.byte_len(), deadline_ns)?;
            ...
            budget
                .charge(
                    self.metadata.len() as u64 * 8
                        + self.manifest.children().len() as u64 * 128
                        + 4096,
                )
                .map_err(|e| RecordingRecipeError::Source(DatagramArchiveError::Work(e)))?;
            let metadata = p
            ...
            if self.manifest.metadata_digest() != Some(metadata) {
        ...
        match result {
            Ok((pin, completion)) => Ok(ReconstructionPublication {
                pin,
                windows: receipts,
                completion,
            }),
            Err(reason) => Err(ReconstructionPublishFailure {
                reason,
                windows: receipts,
            }),
        }
    ...
    fn check_outputs(
        ...
        let mut scan = || -> Result<(), ReconstructionError> {
            ...
            budget
                ...
                .map_err(|e| RecordingRecipeError::Source(DatagramArchiveError::Work(e)))?;
            examined += 1;
            if examined > self.limits.max_scan_roots {
        ...
        for root in p.visible_roots() {
            scan()?;
            let Some(suffix) = root.slot.as_str().strip_prefix(&prefix) else {
                continue;
            };
            let expected = if suffix == "complete" {
                self.pin.root
            } else {
                let index = suffix
                    .strip_prefix('w')
                    ...
                    .ok_or(ReconstructionError::Conflict)?;
                if root.slot != self.window_slot(index)? {
                    return Err(ReconstructionError::Conflict);
                }
                self.windows
                    .get(index)
        ...
        for slot in p.broken_slots() {
            scan()?;
            if slot.as_str().starts_with(&prefix) {
                return Err(ReconstructionError::Conflict);
            }
        }
            ...
            .chain(report.broken_roots.iter().map(|b| &b.path))
        {
            ...
            if path
                .file_name()
                .is_some_and(|n| n.as_encoded_bytes().starts_with(prefix.as_bytes()))
            {
                return Err(ReconstructionError::Conflict);
            }
        }
