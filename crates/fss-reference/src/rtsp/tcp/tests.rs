...
impl TcpAuthority for Permit {
    fn checkpoint(
        ...
        if binding.key == KEY && binding.authority == "camera.local" {
            Ok(())
        } else {
            Err(TcpDenial::Unauthorized)
        }
    }
}
struct Deny;
...
impl TcpAuthority for DenyAt {
    fn checkpoint(&self, _: &TcpBinding, _: TcpOperation, _: u64, _: u64) -> Result<(), TcpDenial> {
        if self.0.fetch_add(1, std::sync::atomic::Ordering::SeqCst) == self.1 {
            Err(TcpDenial::Cancelled)
        } else {
            Ok(())
        }
    }
...
fn short_writes_wouldblock_and_interruption_send_only_the_original_suffix() -> TestResult {
    ...
    for now in 1..100 {
        if matches!(
            link.write_step(now, &Permit)?,
            TcpWriteStep::Sent { cseq: 1, .. }
        ) {
            break;
        }
    }
...
fn partial_write_error_retains_exact_command_and_accepted_prefix() -> TestResult {
    let script = Script {
        writes: [Ok(7), Err(io::ErrorKind::ConnectionReset.into())].into(),
        ..Script::default()
    };
    ...
    link.queue_request(request, 0, &Permit)?;
    assert!(matches!(
        link.write_step(1, &Permit)?,
        TcpWriteStep::Advanced { sent: 7, .. }
    ));
    assert_eq!(
        link.write_step(2, &Permit),
        Err(TcpError::Io {
    ...
    assert_eq!(
        retired.request.ok_or("lost partial request")?.bytes(),
        original
    );
...
fn post_write_cancellation_preserves_even_a_fully_accepted_unacknowledged_request() -> TestResult {
    ...
    assert_eq!(
        link.write_step(1, &cancel),
        Err(TcpError::Denied(TcpDenial::Cancelled))
    );
    let retired = link.retire();
    ...
    assert_eq!(
        retired
            .request
            .ok_or("dispatch uncertainty disappeared")?
            .bytes()
            .len(),
        len
    );
...
fn post_read_revocation_keeps_exact_bytes_without_returning_a_successful_read() -> TestResult {
    let script = Script {
        reads: [Ok(b"private-response".to_vec())].into(),
        ..Script::default()
    };
    let mut link = make_link(script, TcpLimits::default())?;
    let cancel = DenyAt(std::sync::atomic::AtomicUsize::new(0), 1);
    assert_eq!(
        link.read_step(1, &cancel),
        Err(TcpError::Denied(TcpDenial::Cancelled))
    );
    let retired = link.retire();
    assert_eq!(retired.totals.received_bytes, 16);
    assert_eq!(
        retired.unread.ok_or("revoked source lost")?.expose(),
        b"private-response"
    );
    Ok(())
}
...
fn budgets_are_not_eof_and_wouldblock_attempts_consume_work() -> TestResult {
    let mut read = make_link(
        Script {
            reads: [Ok(b"abcd".to_vec())].into(),
            ..Script::default()
        },
        TcpLimits {
            max_received_bytes: 3,
            ..TcpLimits::default()
        },
    )?;
    read.read_step(1, &Permit)?;
    let _ = read.acknowledge_read();
...
fn whole_request_budget_and_queue_pressure_refuse_without_consuming_the_request() -> TestResult {
    ...
    let original = request_a.bytes().to_vec();
    let refused = small
        .queue_request(request_a, 1, &Permit)
        .err()
        .ok_or("send budget ignored")?;
    ...
    let refused = busy
        .queue_request(request()?, 1, &Permit)
        .err()
        .ok_or("queue widened")?;
...
fn eight_interruptions_fail_without_an_unbounded_retry_loop() -> TestResult {
    ...
    let mut link = make_link(script, TcpLimits::default())?;
    for now in 0..7 {
        assert_eq!(link.read_step(now, &Permit)?, TcpReadStep::Pending);
    }
...
fn zero_write_and_true_eof_have_different_receipts() -> TestResult {
    ...
    let mut reader = make_link(
        Script {
            reads: [Ok(Vec::new())].into(),
            ..Script::default()
        },
        TcpLimits::default(),
    )?;
...
fn reversed_time_is_retryable_but_expiry_and_revocation_close_without_io() -> TestResult {
    ...
    link.check(6, &Permit)?;
    assert_eq!(
        link.read_step(7, &Deny),
        Err(TcpError::Denied(TcpDenial::Revoked))
    );
    assert_eq!(link.totals().read_calls, 0);
    let mut expired = make_link(Script::default(), TcpLimits::default())?;
    assert_eq!(
        expired.check(60_000_000_000, &Permit),
...
fn buffered_source_retains_original_age_and_debug_does_not_print_routes_or_payloads() -> TestResult
{
    let mut link = make_link(
        Script {
            reads: [Ok(b"private-media".to_vec())].into(),
            ..Script::default()
        },
        TcpLimits::default(),
    )?;
    ...
    for secret in ["private-media", "camera.local", "127.0.0.1", "DESCRIBE"] {
        assert!(!debug.contains(secret));
    }
    assert_eq!(
        link.check(1 + WIRE_LIFETIME_NS, &Permit),
        Err(TcpError::ReadDeadline)
    );
    assert_eq!(
        link.retire()
            .unread
            .ok_or("expired source missing")?
            .expose(),
        b"private-media"
    );
    Ok(())
...
fn invalid_configuration_and_denied_connect_make_no_connection_attempt() -> TestResult {
    for limits in [
        ...
        assert!(
            !RtspTcpLink::connect(binding()?, limits, 0, 100, &Permit)
                .err()
...
fn actual_loopback_socket_uses_exact_route_and_transfers_native_bytes() -> TestResult {
    ...
    link.queue_request(request, 0, &Permit)?;
    for now in 1..100 {
        if matches!(link.write_step(now, &Permit)?, TcpWriteStep::Sent { .. }) {
            break;
        }
    }
    assert!(!link.has_pending_request());
    ...
    for now in 100..200 {
        match link.read_step(now, &Permit)? {
            TcpReadStep::Buffered => combined.extend_from_slice(
                link.acknowledge_read()
