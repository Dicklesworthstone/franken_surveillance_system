...
use fss_core::{
    CanonicalEncode, CanonicalEncoder, ContentDigest, EffectIntent, EffectState, OperationReceipt,
    TimestampNs,
};
...
impl WebhookEndpoint {
    ...
    pub fn new(
        ...
        e.text(&peer.to_string());
        e.text(target);
        e.digest(plaintext_approval);
    ...
    pub fn plaintext_approval(&self) -> ContentDigest {
        self.approval
    }
...
impl WebhookLimits {
    fn validate(self) -> Result<(), WebhookError> {
        if !(1..=4096).contains(&self.io_calls)
            || !(1..=4096).contains(&self.io_bytes)
...
impl From<ReferenceError> for WebhookError {
    fn from(e: ReferenceError) -> Self {
        Self::Authority(e)
    }
}
impl From<DurableEffectError> for WebhookError {
    fn from(e: DurableEffectError) -> Self {
        Self::Journal(e)
    }
}
impl std::fmt::Display for WebhookError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::Configuration => "invalid webhook configuration",
            Self::RouteMismatch => "webhook route mismatch",
            Self::Denied(_) => "webhook authority refused",
            Self::ClockReversed => "webhook admission clock reversed",
            Self::NotFinished => "webhook observation unfinished",
            Self::EffectMismatch => "webhook effect history mismatch",
            Self::Allocation => "webhook allocation refused",
            Self::Authority(_) => "webhook event authority refused",
...
impl WebhookEvidence {
    ...
    pub fn route_digest(&self) -> ContentDigest {
        self.route
    }
    /// Exact durable journal prefix AFTER commit, binding the prepared authority.
    pub fn commitment_root(&self) -> ContentDigest {
    ...
    pub fn encoded(&self) -> Result<Vec<u8>, WebhookError> {
        ...
        let mut e = CanonicalEncoder::new();
        e.text("fss.http-webhook-observation.v1");
        self.intent.encode_canonical(&mut e);
        e.digest(self.route);
        e.digest(self.commitment_root);
        e.bytes(&self.request);
        e.u64(self.sent as u64);
        e.bytes(&self.response);
        e.u64(self.calls);
        e.u64(self.committed_ns);
        e.u64(self.deadline_ns);
        e.u64(self.last_admission_ns);
    ...
    pub fn digest(&self) -> Result<ContentDigest, WebhookError> {
        Ok(ContentDigest::sha256(&self.encoded()?))
    }
}
impl std::fmt::Debug for WebhookEvidence {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("WebhookEvidence")
            .field("sent_bytes", &self.sent)
            .field("received_bytes", &self.response.len())
...
pub struct WebhookAttempt<'a> {
    ...
    ledger: &'a DurableReferenceLedger,
    journal: &'a mut DurableEffectJournal,
...
impl<'a> WebhookAttempt<'a> {
    ...
    pub fn begin<E: Into<ReferenceError>>(
        ...
        if plan.channel != endpoint.channel {
            return Err(WebhookError::RouteMismatch);
        }
        let operation = journal
            .operation(&plan.intent.operation_id)
            .ok_or(WebhookError::EffectMismatch)?;
        let scope = WebhookScope {
            endpoint,
            operation,
            effect_journal: journal.path(),
        };
        owner
            .checkpoint(scope, WebhookBoundary::Commit, now_ns, deadline_ns)
            .map_err(WebhookError::Denied)?;
        let request = wire::request(plan, endpoint)?;
        let mut response = Vec::new();
        response
            .try_reserve_exact(limits.response_bytes)
            .map_err(|_| WebhookError::Allocation)?;
        let mut evidence = WebhookEvidence {
            intent: plan.intent.clone(),
            route: endpoint.digest,
            commitment_root: journal.last_root(),
            request,
            response,
            sent: 0,
            calls: 0,
            committed_ns: now_ns,
            deadline_ns,
            last_admission_ns: now_ns,
            outcome: None,
        };
        revalidate_alert_event_authority(
            plan,
            ledger,
            read_payload,
            TimestampNs(i128::from(now_ns)),
            journal,
        )?;
        let operation = journal
            .operation(&plan.intent.operation_id)
    ...
    pub fn evidence(&self) -> &WebhookEvidence {
        &self.evidence
    }
    /// Durable receipt may be Committed while a local observation awaits recording.
    pub fn operation(&self) -> Option<&OperationReceipt> {
        self.journal.operation(&self.plan.intent.operation_id)
    }
    ...
    pub fn poll<E: Into<ReferenceError>>(
        &mut self,
        now_ns: u64,
        owner: &impl WebhookAuthority,
        read_payload: impl FnMut(ContentDigest) -> Result<Vec<u8>, E>,
    ) -> WebhookProgress {
        if let Some(outcome) = self.evidence.outcome {
            return WebhookProgress::ReadyToRecord(outcome);
        }
        if now_ns < self.last_ns {
            return self.finish(WebhookOutcome::Interrupted(
                WebhookInterruption::ClockReversed,
            ));
        }
        self.last_ns = now_ns;
        self.evidence.last_admission_ns = now_ns;
        ...
        let boundary = match self.phase {
            Phase::Connect => WebhookBoundary::Connect,
            Phase::Configure => WebhookBoundary::Configure,
            Phase::Send => WebhookBoundary::Send,
            Phase::Receive => WebhookBoundary::Receive,
            Phase::Finished => {
                return self.finish(WebhookOutcome::Interrupted(WebhookInterruption::Retired));
            }
        };
        if let Err(reason) = self.probe(owner, boundary, now_ns) {
            return self.finish(WebhookOutcome::Interrupted(WebhookInterruption::Denied(
                reason,
            )));
        }
        if matches!(self.phase, Phase::Connect | Phase::Send) {
            ...
            let valid = self
                .journal
                .operation(&self.plan.intent.operation_id)
                .is_some_and(|op| {
                    op.state == EffectState::Committed && op.intent == self.plan.intent
                });
            if !valid
                || check_alert_dispatch_authority(
                    self.plan,
                    self.ledger,
                    read_payload,
                    &self.evidence.intent,
                    obligation,
                )
                .is_err()
            {
        ...
        if self.phase == Phase::Receive
            && self.evidence.response.len() == self.limits.response_bytes
        {
            return self.finish(WebhookOutcome::Interrupted(WebhookInterruption::Limit));
        }
        ...
        let result: Result<(), WebhookInterruption> = match self.phase {
            ...
            Phase::Send => {
                let end =
                    (self.evidence.sent + self.limits.io_bytes).min(self.evidence.request.len());
                match self
                    .stream
                    .as_mut()
                    .map(|s| s.write(&self.evidence.request[self.evidence.sent..end]))
                {
                    Some(Ok(0)) => Err(WebhookInterruption::Disconnected),
                    Some(Ok(count)) => {
                        self.evidence.sent += count;
                        if self.evidence.sent == self.evidence.request.len() {
            ...
            Phase::Receive => {
                ...
                let count = self
                    .limits
                    .io_bytes
                    .min(self.limits.response_bytes - self.evidence.response.len());
                match self.stream.as_mut().map(|s| s.read(&mut bytes[..count])) {
                    Some(Ok(0)) => Err(WebhookInterruption::Disconnected),
                    Some(Ok(count)) => {
                        self.evidence.response.extend_from_slice(&bytes[..count]);
                        Ok(())
                    }
                    Some(Err(e))
                        if matches!(
        ...
        if let Err(reason) = result {
            return self.finish(WebhookOutcome::Interrupted(reason));
        }
        if self.phase == Phase::Receive {
            match wire::response(&self.evidence.response) {
                Ok(Some(status)) => {
                    return self.finish(if (200..300).contains(&status) {
                        WebhookOutcome::ReceiverAccepted(status)
                    } else {
                        WebhookOutcome::ReceiverStatus(status)
                    });
                }
                Ok(None) => {}
                Err(()) => {
    ...
    fn probe(
        ...
        let operation = self
            .journal
            .operation(&self.plan.intent.operation_id)
            .ok_or(WebhookDenial::Unauthorized)?;
        owner.checkpoint(
            WebhookScope {
                endpoint: self.endpoint,
                operation,
                effect_journal: self.journal.path(),
            },
            boundary,
            now_ns,
            self.deadline_ns,
    ...
    fn finish(&mut self, outcome: WebhookOutcome) -> WebhookProgress {
        ...
        self.evidence.outcome = Some(outcome);
    ...
    pub fn record(
        ...
        if now_ns < self.last_ns {
            return Err(WebhookError::ClockReversed);
        }
        if self.evidence.outcome.is_none() {
            return Err(WebhookError::NotFinished);
        }
        self.last_ns = now_ns;
...
pub fn record_webhook_evidence<'a>(
    evidence: &WebhookEvidence,
    ...
    if now_ns < evidence.last_admission_ns {
        return Err(WebhookError::ClockReversed);
    }
    if evidence.route != endpoint.digest {
        return Err(WebhookError::RouteMismatch);
    }
    let operation = journal
    ...
    if !journal
        .committed_roots()?
        .contains(&evidence.commitment_root)
    {
        return Err(WebhookError::EffectMismatch);
    }
    ...
    let operation = journal
        .operation(&evidence.intent.operation_id)
        .ok_or(WebhookError::EffectMismatch)?;
    if operation.intent != evidence.intent {
        return Err(WebhookError::EffectMismatch);
    }
    ...
    let reason =
        (next == EffectState::Indeterminate).then(|| "webhook_delivery_unresolved".to_owned());
    journal
        .transition(
