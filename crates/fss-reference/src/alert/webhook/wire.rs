...
pub(super) fn request(
    ...
    let key = hex(ContentDigest::sha256(
        plan.intent.idempotency_key.as_str().as_bytes(),
    ));
    let operation = hex(ContentDigest::sha256(
        plan.intent.operation_id.as_str().as_bytes(),
    ));
    let body = format!(
        concat!(
            "{{\"schema\":\"fss.webhook-alert.v1\",",
            ...
            "\"event_revision_sha256\":\"{}\"}}"
        ),
        operation,
        hex(plan.intent.request_digest),
        hex(plan.intent.precondition_digest),
        hex(plan.event_root),
        hex(plan.event_revision_digest)
    );
    let request = format!(
        concat!(
            "POST {} HTTP/1.1\r\nHost: {}\r\n",
...
pub(super) fn response(bytes: &[u8]) -> Result<Option<u16>, ()> {
    ...
    loop {
        ...
        if head
            .iter()
            .any(|b| *b > 126 || (*b < 32 && !matches!(b, b'\r' | b'\n' | b'\t')))
        {
            return Err(());
        }
        ...
        if status.len() < 13
            || ![b"HTTP/1.1 ".as_slice(), b"HTTP/1.0 ".as_slice()]
                .iter()
        ...
        for raw in lines {
            ...
            if name.eq_ignore_ascii_case(b"content-length") {
                ...
                if transfer || !value.eq_ignore_ascii_case(b"chunked") {
                    return Err(());
                }
                transfer = true;
            }
...
pub(super) fn encode_outcome(e: &mut CanonicalEncoder, outcome: WebhookOutcome) {
    match outcome {
        WebhookOutcome::ReceiverAccepted(code) => {
            e.u8(0);
            e.u32(u32::from(code));
        }
        ...
        WebhookOutcome::Interrupted(reason) => {
            ...
            let (tag, detail) = match reason {
                ...
                WebhookInterruption::Denied(reason) => (
                    1,
                    match reason {
                        WebhookDenial::Unauthorized => 0,
                        WebhookDenial::Revoked => 1,
                        WebhookDenial::Cancelled => 2,
                        WebhookDenial::Deadline => 3,
                        WebhookDenial::Budget => 4,
                    },
                ),
                WebhookInterruption::ClockReversed => (8, 0),
                WebhookInterruption::EventAuthorityRefused => (2, 0),
                WebhookInterruption::Limit => (3, 0),
                WebhookInterruption::Io => (4, 0),
                WebhookInterruption::Disconnected => (5, 0),
                WebhookInterruption::InvalidResponse => (6, 0),
                WebhookInterruption::Retired => (7, 0),
            };
            e.u8(tag);
            e.u8(detail);
        }
