...
#[test]
fn honest_cancellation_verifies_in_the_journal_and_the_guard() -> Result<(), Box<dyn Error>> {
    ...
    // The guard recomputes the same proof from the plan and the ledger.
    let verdict = case.caller_verdict(&cancelled)?;
    assert!(
        matches!(verdict, Ok(())),
        "honest cancellation: {verdict:?}"
    );
    // A refusal before the ledger moved names the prepared anchor itself; that is ledger
    // evidence too, in both.
...
-> Result<(), Box<dyn Error>> {
    ...
    // The control: the same receipt with the honest proof passes the guard.
    let verdict = case.caller_verdict(&honest)?;
    assert!(matches!(verdict, Ok(())), "control: {verdict:?}");
    case.harness.cleanup();
    Ok(())
}
...
#[test]
fn durable_bound_cancellation_reopens_and_passes_the_guard() -> Result<(), Box<dyn Error>> {
