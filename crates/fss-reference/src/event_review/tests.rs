...
impl Directory {
    fn new(name: &str) -> Test<Self> {
        for n in 0..100 {
            let path = std::env::temp_dir().join(format!("fss-review-{name}-{}-{n}", std::process::id()));
...
fn authority(capabilities: &[&str]) -> Test<ContextAuthority> {
    Ok(ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:event-review".into(), operation_id: OperationId::parse("operation:event-review")?,
        principal: ACTOR.into(), capabilities: capabilities.iter().map(|c| (*c).into()).collect(),
...
fn initial_event(tamper: bool) -> Test<EventHypothesis> {
    Ok(EventHypothesis {
        schema: EventHypothesis::SCHEMA.into(), event_id: EventId::parse("event:review-fixture")?,
        ...
        zone_ids: vec!["door".into()], track_ids: vec!["track:synthetic".into()],
        probability: ProbabilityInterval::new(0.0, 1.0)?,
        evidence: vec![EventEvidence {
            digest: ContentDigest::sha256(b"synthetic source assertion"), class: EvidenceClass::Assertion,
            failure_domain: "synthetic-source".into(), supports: false,
            relation: if tamper { EvidenceEdgeRelation::SensorTamper } else { EvidenceEdgeRelation::DerivedFrom },
            capsule_digest: None, identity_digest: Some(ContentDigest::sha256(b"synthetic-source")),
        }],
        model_receipts: vec![], decision_path: DecisionPath {
            policy_generation: ContentDigest::sha256(b"synthetic policy"), fingerprint: ContentDigest::sha256(b"synthetic decision"),
            abstained: true, abstention_reason: Some("Synthetic unclassified fixture".into()),
        },
    })
}
...
fn read_and_preview_write_nothing_and_preserve_every_source_field() -> Test {
    ...
    assert_eq!(read_review_event(&f.deployment, &f.event.event_id, &f.authority, &f.cx)?.0, f.event);
    let preview = preview_review(&f.deployment, &f.request(ReviewDisposition::Reject), &f.authority, &f.cx)?;
...
fn rejection_publishes_once_survives_reopen_and_never_mutates_effects() -> Test {
    ...
    let receipt = commit_review(&mut f.deployment, &request, preview.approval(), &f.authority, &f.cx)?;
    assert!(receipt.published);
    assert_eq!(receipt.review.event().state, EventState::Rejected);
    assert_eq!(crate::committed_reference_policy_action(receipt.review.event()), ReferencePolicyAction::Hold);
    let after = f.snapshot()?;
    assert_eq!(after.2, effects);
    let Fixture { deployment, cx, authority, event, _directory } = f;
    let root = deployment.root().to_path_buf(); drop(deployment);
    let mut deployment = ReferenceDeployment::reopen(&root, SITE, &cx)?;
    let retry = commit_review(&mut deployment, &request, preview.approval(), &authority, &cx)?;
    ...
    assert_eq!(deployment.publisher().spool().read(receipt.review.record().digest())?, receipt.review.record().to_bytes());
    // A previously prepared producer cannot overwrite the operator successor with revision one.
    assert!(deployment.publish_event(&ReferencePolicyDecision { event, action: ReferencePolicyAction::Hold }, &cx).is_err());
    assert_eq!(*deployment.current_anchor(), after.0);
    Ok(())
}
...
fn stale_approval_or_competing_review_never_writes() -> Test {
    ...
    let preview = preview_review(&f.deployment, &request, &f.authority, &f.cx)?;
    let before = f.snapshot()?;
    let mut changed = request.clone(); changed.reason.push('!');
    assert!(matches!(commit_review(&mut f.deployment, &changed, preview.approval(), &f.authority, &f.cx), Err(ReviewError::StaleApproval)));
    assert_eq!(f.snapshot()?, before);
    ...
    commit_review(&mut f.deployment, &competing, competing_preview.approval(), &f.authority, &f.cx)?;
    let after = f.snapshot()?;
    assert!(matches!(commit_review(&mut f.deployment, &request, preview.approval(), &f.authority, &f.cx), Err(ReviewError::StaleRevision)));
...
fn capabilities_site_and_cancellation_are_checked_before_event_disclosure() -> Test {
    ...
    let before = f.snapshot()?;
    assert!(matches!(preview_review(&f.deployment, &request, &denied, &f.cx), Err(ReviewError::Unauthorized)));
    ...
    assert!(matches!(commit_review(&mut f.deployment, &request, preview.approval(), &prepare_only, &f.cx), Err(ReviewError::Unauthorized)));
    let mut foreign = f.authority.clone(); foreign.anchor_universe = ContentDigest::sha256(b"foreign");
    assert!(matches!(preview_review(&f.deployment, &request, &foreign, &f.cx), Err(ReviewError::Unauthorized)));
    f.cx.request_cancellation();
    assert!(matches!(preview_review(&f.deployment, &request, &f.authority, &f.cx), Err(ReviewError::Cancelled)));
    assert_eq!(f.snapshot()?, before);
    Ok(())
}
...
fn provenance_cut_resumes_with_original_approval_without_duplicating_root() -> Test {
    ...
    assert!(matches!(commit_review(&mut f.deployment, &request, preview.approval(), &f.authority, &f.cx), Err(ReviewError::Cancelled)));
    assert_eq!(f.deployment.current_event_authority(&request.event_id)?.0, f.event);
    let head = f.deployment.current_anchor().clone();
    let cx = f.fresh_cx()?;
    let receipt = commit_review(&mut f.deployment, &request, preview.approval(), &f.authority, &cx)?;
    assert!(receipt.published);
    assert_eq!(f.deployment.current_anchor().commit_sequence, head.commit_sequence + 1);
...
fn prepublication_cancellation_has_no_writes_postcommit_cancellation_reports_success() -> Test {
    ...
    assert!(matches!(commit_review(&mut f.deployment, &request, preview.approval(), &f.authority, &f.cx), Err(ReviewError::Cancelled)));
    assert_eq!(f.snapshot()?, before);
    let cx = f.fresh_cx()?; cx.set_cancel_at_checkpoint(STAGE_REVIEW_COMMITTED);
    let receipt = commit_review(&mut f.deployment, &request, preview.approval(), &f.authority, &cx)?;
    assert!(receipt.published);
    assert!(cx.is_drain_completed());
    assert_eq!(f.deployment.current_event_authority(&request.event_id)?.0.state, EventState::Rejected);
    Ok(())
}
...
fn investigation_then_resolution_keeps_tamper_and_full_lineage() -> Test {
    ...
    let request = ReviewRequest {
        expected_revision: receipt.review.event().revision_digest(), disposition: ReviewDisposition::Resolve,
        ..request
    };
    let preview = preview_review(&f.deployment, &request, &f.authority, &f.cx)?;
    let receipt = commit_review(&mut f.deployment, &request, preview.approval(), &f.authority, &f.cx)?;
    ...
    assert!(receipt.authority.lineage_tamper_status.has_open_tamper());
    assert!(receipt.review.event().evidence.iter().any(EventEvidence::reports_sensor_tamper));
    assert_eq!(receipt.authority.prior_revision_encodings.len(), 2);
    let terminal = ReviewRequest { expected_revision: receipt.review.event().revision_digest(), disposition: ReviewDisposition::Investigate, ..request };
...
fn record_roundtrip_truncation_tamper_and_approval_binding() -> Test {
    ...
    let preview = preview_review(&f.deployment, &f.request(ReviewDisposition::Reject), &f.authority, &f.cx)?;
    let record = preview.record(); let bytes = record.to_bytes();
    ...
    for n in 0..bytes.len() {
        ...
        let mut changed = bytes.clone(); changed[n] ^= 1;
    ...
    let mut request = record.request.clone(); request.reason.push('!');
    let other = preview_review(&f.deployment, &request, &f.authority, &f.cx)?;
    assert_ne!(preview.approval(), other.approval());
    let mut actor = f.authority.clone(); actor.principal = "principal:other".into();
...
fn direct_transition_table_and_effect_guard_never_invent_authority() {
    for value in ["corroborate", "adjudicate", "alert_delivered", "verified", ""] {
