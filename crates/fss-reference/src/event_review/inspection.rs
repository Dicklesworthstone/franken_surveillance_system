...
pub fn read_current_review(
    ...
    if event_id.as_str().len() > MAX_EVENT_ID_LEN { return Err(ReviewError::Limit); }
    ...
    if event != current.event { return Err(ReviewError::CustodyMismatch); }
    if event.decision_path.policy_generation != ContentDigest::sha256(POLICY) { return Ok(None); }
    let index = revisions.len().checked_sub(2).ok_or(ReviewError::CustodyMismatch)?;
    ...
    let digest = manifest.children().iter().copied().find(|d| *d != prior.root)
    ...
    if record.site != deployment.site_lineage() || record.request.event_id != *event_id
        || record.request.expected_revision != prior.event.revision_digest()
        || record.previous_event_root != prior.root || record.previous_anchor != prior.anchor
    { return Err(ReviewError::CustodyMismatch); }
    let chain: Vec<_> = revisions[..=index].iter().map(|r| r.event.clone()).collect();
...
mod tests {
    ...
    fn readback_keeps_original_actor_even_for_a_different_reader() -> Test {
        ...
        assert!(read_current_review(&f.deployment, &f.event.event_id, &f.authority, &f.cx)?.is_none());
        let request = f.request(ReviewDisposition::Resolve);
        let p = preview_review(&f.deployment, &request, &f.authority, &f.cx)?;
        let receipt = commit_review(&mut f.deployment, &request, p.approval(), &f.authority, &f.cx)?;
        let mut reader = f.authority.clone(); reader.principal = "principal:another-reader".into();
        let before = f.snapshot()?;
        let read = read_current_review(&f.deployment, &request.event_id, &reader, &f.cx)?.ok_or("missing review")?;
