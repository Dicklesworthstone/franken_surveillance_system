...
fn assert_same_state(actual: &VirtualSource, expected: &VirtualSource) {
    ...
    assert_eq!(actual.execution_failure_faults, expected.execution_failure_faults);
}
...
fn first_interval_overflow_leaves_sequence_zero_and_payload_unchanged() -> Test {
    ...
    for _ in 0..3 {
        assert!(matches!(source.emit_packet(), Err(ReferenceError::ArithmeticOverflow)));
...
fn interval_overflow_rolls_back_the_successful_speculative_clock_step() -> Test {
    ...
    assert!(matches!(source.emit_packet()?, OperationOutcome::Success(_)));
    let before = source.clone();
...
fn failed_retry_keeps_an_indeterminate_read_pinned_to_the_same_sequence() -> Test {
    ...
    assert!(matches!(source.emit_interval()?, OperationOutcome::Indeterminate(_)));
    assert!(source.clear_indeterminate_read(1).is_some());
    let before = source.clone();
    assert!(matches!(source.emit_interval(), Err(ReferenceError::ArithmeticOverflow)));
    assert_same_state(&source, &before);
...
fn accepted_fault_outcomes_keep_existing_sequence_and_retry_semantics() -> Test {
    ...
    assert!(matches!(source.emit_packet()?, OperationOutcome::Indeterminate(_)));
    assert_eq!(source.current_sequence(), 1);
    ...
    assert!(matches!(source.emit_packet()?, OperationOutcome::UnauthorizedOrNotObservable(_)));
    ...
    let before = source.clone();
    assert!(matches!(source.emit_packet(), Err(ReferenceError::UnknownSourceSequence(4))));
    assert_same_state(&source, &before);
    Ok(())
}
