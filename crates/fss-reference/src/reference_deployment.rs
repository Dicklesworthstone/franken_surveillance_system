...
use fss_core::{
    ...
    EventId, EvidenceDelta, EvidenceDeltaBatch, HandoffCapsule, HandoffId, LedgerAnchor, ObjectId,
...
use fss_publication::{
    ...
    RootLedgerReceipt, RootLedgerReconciliation, SlotName,
};
...
#[doc(hidden)]
pub static LAYOUT_INJECT_FAIL_BEFORE_RENAME: std::sync::atomic::AtomicBool =
    std::sync::atomic::AtomicBool::new(false);

pub fn write_layout_atomic(root: &Path, layout: &DeploymentLayout) -> Result<(), ReferenceError> {
    ...
    if LAYOUT_INJECT_FAIL_BEFORE_RENAME.load(std::sync::atomic::Ordering::SeqCst) {
        return Err(ReferenceError::Io(std::io::Error::other(
            "injected fault before layout rename",
        )));
    }
...
impl ReferenceDeployment {
    ...
    /// Stages child payload bytes and manifest for `slot` without publishing it to disk or ledger.
    ///
    /// The manifest is staged in the local publisher and every reference is verified.
    /// Returns [`StagedManifestHandle`] and makes nothing visible.
