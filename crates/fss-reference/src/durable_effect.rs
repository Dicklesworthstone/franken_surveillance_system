...
/// Breakdown of obligations by lifecycle state.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct ObligationCounts {
    /// Total number of obligations recorded.
    pub total: usize,
    /// Obligations awaiting settlement.
    pub pending: usize,
    /// Obligations discharged successfully.
    pub discharged: usize,
    /// Obligations that failed permanently.
    pub failed: usize,
}
...
impl DurableEffectJournal {
    ...
    pub fn inspect_with_io(
        ...
        let doctor_report = fss_ledger::doctor(&buf).map_err(|err| match err {
            ...
            _ => DurableEffectError::Journal(JournalError::CorruptHeader {
                offset: 0,
                kind: fss_ledger::CorruptionKind::CorruptMagic,
            }),
        })?;

        let recovery = fss_ledger::recover_bytes(&buf[..doctor_report.committed_len as usize])?;
        ...
        for obl in journal.obligations() {
            ...
            match obl.state {
                ...
                ObligationState::Discharged => obligation_counts.discharged += 1,
