...
///
/// This entry point sees neither the durable publication lineage nor the durable effect journal, so
/// it cannot rule out a second successor of the basis or a substitute journal: it reports every
/// change, but never as a terminal transition (fss-mnlz1). Use
/// [`classify_reference_meaningful_delta_in_lineage`] for terminal classification.
...
///
/// A result cell of the same claim as a basis cell is that claim's next version, so the basis
/// cell is the claim's previous version and the pairwise check applies:
...
fn classify(
    basis: &ReferenceSituationPublication,
    result: &ReferenceSituationPublication,
    binding: Option<LineageBinding<'_>>,
) -> Result<MeaningfulDelta, ReferenceError> {
    basis.verify()?;
    result.verify()?;
    ...
    let obligation_terminalized = obligation_terminalized
        && verify_discharge(
            ...
            &basis_obligations,
            &result_obligations,
            binding.and_then(|binding| binding.journal),
        )?;
    ...
    delta.validate_removals_against(&basis_frame.knowledge_cells, &result_frame.knowledge_cells)?;
    delta.validate()?;
    Ok(delta)
}
...
fn removed_claim_ids(basis: &[KnowledgeCell], result: &[KnowledgeCell]) -> Vec<String> {
    let current: BTreeSet<&str> = result.iter().map(|cell| cell.claim_id()).collect();
    let removed: BTreeSet<String> = basis
        .iter()
        .filter(|cell| !current.contains(cell.claim_id()))
...
fn contradiction_changed(
    ...
    basis.iter().any(|prior| {
        let had_contradiction = prior.knowledge_state() == KnowledgeState::Conflicted
            || !prior.contradictions().is_empty();
        match result
            .iter()
...
pub(crate) struct ComparisonWitnessInputs<'a> {
    pub(crate) basis: &'a ReferenceSituationPublication,
    pub(crate) result: &'a ReferenceSituationPublication,
    pub(crate) classes: &'a BTreeSet<MeaningfulDeltaClass>,
    pub(crate) changed_cells: &'a [KnowledgeCell],
    pub(crate) removed_claim_ids: &'a [String],
    pub(crate) invalidated_assumptions: &'a [String],
    pub(crate) coverage_changes: &'a [String],
    pub(crate) obligation_changes: &'a [String],
    pub(crate) effect_uncertainty_changes: &'a [String],
    pub(crate) lineage_commitment: Option<ContentDigest>,
    pub(crate) journal_commitment: Option<ContentDigest>,
}

pub(crate) fn comparison_witness(inputs: ComparisonWitnessInputs<'_>) -> ContentDigest {
    let mut encoder = CanonicalEncoder::new();
    encoder.text("fss.reference_meaningful_delta_selection.v2");
    ...
    // The durable stores a bound classification was checked against are part of its witness.
    for commitment in [inputs.lineage_commitment, inputs.journal_commitment] {
        match commitment {
            Some(commitment) => {
                encoder.bool(true);
                encoder.digest(commitment);
            }
