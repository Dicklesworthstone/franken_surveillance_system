...
fn packetize_nals_for_stream(
    ...
    // 1. Sacrificial packet (Probation): repeated SPS+PPS STAP-A
    ...
    // 2. Baseline packet: SPS+PPS STAP-A (Baseline)
    packets.push(ProtoPacket {
        offset_ms: 0,
        sequence: seq,
        timestamp: base_timestamp,
    ...
    // 3. Process remaining NALs (excluding SPS/PPS which were in STAP-A)
...
fn serialize_rtpdump(
    ...
    for (idx, p) in proto_packets.into_iter().enumerate() {
        ...
        if truncate_last_record && is_last {
            ...
            // Intentionally omit offset_ms and packet payload
            break;
        }
...
pub fn generate_rtpdump_reorder(
    ...
    if proto.len() > 3 {
        ...
        // Index 3 is now seq 0 (Reordered under 128-window rules)
...
pub fn generate_rtpdump_ssrc_reset(
    ...
    // Generation 2: new SSRC (0x55667788), leading sacrificial STAP-A, baseline STAP-A, then AUD + slice
    let ssrc2 = 0x5566_7788u32;
    let mut seq2 = 1_000u16;
    let ts2 = 180_000u32;
    let offset2 = 132u32;

    // Sacrificial STAP-A for generation 2
    let stap_payload = &proto[0].payload;
    reset_proto.push(ProtoPacket {
        offset_ms: offset2,
        sequence: seq2,
        timestamp: ts2,
    ...
    // Baseline STAP-A for generation 2
    reset_proto.push(ProtoPacket {
        offset_ms: offset2,
        sequence: seq2,
        timestamp: ts2,
    ...
    // Additional packets under generation 2
    for p in proto.iter().skip(4).take(2) {
        reset_proto.push(ProtoPacket {
            offset_ms: offset2.wrapping_add(33),
            sequence: seq2,
            timestamp: ts2.wrapping_add(3_000),
            ssrc: ssrc2,
            marker: p.marker,
            payload_type: p.payload_type,
            payload: p.payload.clone(),
