...
fn test_receipt_ok_outcome() -> Result<(), Box<dyn Error>> {
    ...
    let clock = VirtualClock::new(1, TimestampNs(50_000));
...
fn test_receipt_unsupported_dtype_outcome() -> Result<(), Box<dyn Error>> {
    let g = test_gen();
    // Embedding requires I64 indices and F32 table; ModelIrGraph validates it,
    // but ScalarExecutor only supports F32 inputs, triggering ERR-EXEC-UNSUPPORTED-DTYPE-001.
    let idx_port = TensorPort::new("idx", DType::I64, Shape::new(vec![1, 2])?, g)?;
    let tbl_port = TensorPort::new("tbl", DType::F32, Shape::new(vec![10, 4])?, g)?;
    let out_port = TensorPort::new("emb", DType::F32, Shape::new(vec![1, 2, 4])?, g)?;
    let node = GraphNode::new(
        "emb_1",
        OpCode::Embedding,
        "embedding",
    ...
    let idx_vals = [0_i64, 1];
    let idx_tensor = Tensor::from_values(Shape::new(vec![1, 2])?, &idx_vals, g)?;
    let tbl_vals = [0.1_f32; 40];
    let tbl_tensor = Tensor::from_values(Shape::new(vec![10, 4])?, &tbl_vals, g)?;
    let inputs = vec![("idx", idx_tensor), ("tbl", tbl_tensor)];
...
fn test_bit_exact_reproducibility() -> Result<(), Box<dyn Error>> {
    ...
    let clock1 = VirtualClock::new(1, TimestampNs(12345));
    let clock2 = VirtualClock::new(1, TimestampNs(12345));
...
fn test_virtual_clock_determinism() -> Result<(), Box<dyn Error>> {
    ...
    let cx = ScalarExecCx::new();
    let clock = VirtualClock::new(1, TimestampNs(987_654_321));
