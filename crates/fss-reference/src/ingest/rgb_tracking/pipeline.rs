...
use super::{
    RgbFrameAdmission, RgbTrackingDecision, RgbTrackingError, RgbZoneProgress, RgbZoneTracker,
};
use crate::ScalarExecCx;
use crate::ingest::rgb_detections::pipeline::{
    PendingRgbDetection, RgbDetectionInput, RgbDetectionRun, RgbDetectionStep, RgbDetector,
    RgbDetectorError,
};
...
use fss_twin::image_tracking::{
    ExpiredImageTrack, ImageAssociationCandidate, ImageDetectionDecision, ImageTrack,
    ImageTrackingFrame, ImageTrackingReport,
};
...
impl From<RgbDetectorError> for RgbJpegZoneError {
    fn from(e: RgbDetectorError) -> Self {
        Self::Detector(e)
    }
}
...
pub struct RgbTemporalSnapshot {
    ...
    tracking: [u8; 32],
    tracking_prior: [u8; 32],
    assignment_cost: u64,
    zones: [u8; 32],
    zones_prior: [u8; 32],
    zone_config: [u8; 32],
    frame: ImageTrackingFrame,
    rgb_decisions: Vec<RgbTrackingDecision>,
...
impl RgbTemporalSnapshot {
    ...
    pub fn preparation_digest(&self) -> ContentDigest {
        self.preparation
    }
    ...
    pub fn tracking_prior(&self) -> [u8; 32] {
        self.tracking_prior
    }
    ...
    pub fn zone_digest(&self) -> [u8; 32] {
        self.zones
    }
    /// Immediately preceding zone receipt.
    pub fn zone_prior(&self) -> [u8; 32] {
        self.zones_prior
    }
    /// Exact frozen zone configuration identity.
    pub fn zone_config_digest(&self) -> [u8; 32] {
        self.zone_config
    }
    ...
    pub fn rgb_decisions(&self) -> &[RgbTrackingDecision] {
        &self.rgb_decisions
    }
...
fn copy<T: Copy>(values: &[T]) -> Result<Vec<T>, RgbTrackingError> {
    ...
    out.try_reserve_exact(values.len())
        .map_err(|_| RgbTrackingError::Limit)?;
...
) -> Result<RgbTemporalSnapshot, RgbTrackingError> {
    let p = owner.prepared().ok_or(RgbTrackingError::NoObservation)?;
    let t = owner
        .tracking_report()
        .ok_or(RgbTrackingError::NoObservation)?;
    let z = owner.zone_report().ok_or(RgbTrackingError::PendingZones)?;
    let bytes = [
        std::mem::size_of_val(p.decisions()),
        std::mem::size_of_val(owner.tracker().tracks()),
        std::mem::size_of_val(t.candidates()),
        std::mem::size_of_val(t.decisions()),
        std::mem::size_of_val(t.expired()),
        std::mem::size_of_val(z.cells()),
        std::mem::size_of_val(z.events()),
    ]
    .into_iter()
    .try_fold(std::mem::size_of::<RgbTemporalSnapshot>(), |n, b| {
        n.checked_add(b).ok_or(RgbTrackingError::Limit)
    })?;
    if bytes > MAX_RGB_ZONE_SNAPSHOT_BYTES {
        return Err(RgbTrackingError::Limit);
    }
    budget.charge(bytes as u64)?;
    let result = RgbTemporalSnapshot {
        preparation: p.digest(),
        selected_class: owner.contract().class_index(),
        tracking: t.digest(),
        tracking_prior: t.prior_digest(),
        assignment_cost: t.assignment_cost(),
        zones: z.digest(),
        zones_prior: z.prior_digest(),
        zone_config: z.config_digest(),
        frame: z.frame(),
        rgb_decisions: copy(p.decisions())?,
        tracks: copy(owner.tracker().tracks())?,
        candidates: copy(t.candidates())?,
        decisions: copy(t.decisions())?,
        expired: copy(t.expired())?,
        cells: copy(z.cells())?,
        events: copy(z.events())?,
    };
    budget.charge(0)?;
    Ok(result)
}
...
pub struct RgbJpegZonePipeline<'model, 'temporal> {
    detector: RgbDetector<'model>,
    temporal: &'temporal mut RgbZoneTracker,
    phase: RgbZonePhase,
    admission: Option<RgbFrameAdmission>,
    current: Option<RgbDetectionRun>,
    complete: Option<RgbZoneCompletion>,
}
impl<'model, 'temporal> RgbJpegZonePipeline<'model, 'temporal> {
    ...
    pub fn new(
        model: &'model RgbInferenceModel,
        head: &'model RgbDetectionContract,
        temporal: &'temporal mut RgbZoneTracker,
    ) -> Result<Self, RgbJpegZoneError> {
        if temporal.contract().head_digest() != head.digest() {
            return Err(RgbJpegZoneError::ContractMismatch);
        }
        if temporal.is_pending() {
            return Err(RgbJpegZoneError::PendingFrame);
        }
        Ok(Self {
            detector: RgbDetector::new(model, head)?,
            temporal,
            phase: RgbZonePhase::Ready,
            admission: None,
            current: None,
            complete: None,
        })
    ...
    pub fn pending_inference(&self) -> Option<&PendingRgbDetection> {
    ...
    pub fn detection_run(&self) -> Option<&RgbDetectionRun> {
        self.current
            .as_ref()
            .or_else(|| self.complete.as_ref().map(|c| &c.run))
    }
    ...
    pub fn tracking_report(&self) -> Option<&ImageTrackingReport> {
        if matches!(
            self.phase,
            RgbZonePhase::Zones | RgbZonePhase::Snapshot | RgbZonePhase::Complete
        ) {
    ...
    pub fn run_jpeg(
        ...
        if input.source != admission.source() {
            return Err(RgbJpegZoneError::AdmissionMismatch);
        }
        let step = self.detector.run_jpeg(input, limits, decoder, post, cx)?;
        self.admission = Some(admission);
        self.accept_detection_step(step, work, cx)
    }
    fn accept_detection_step(
        &mut self,
        step: RgbDetectionStep,
        work: &mut WorkBudget<'_>,
        cx: &ScalarExecCx,
    ) -> Result<RgbJpegZoneProgress, RgbJpegZoneError> {
        match step {
            RgbDetectionStep::Pending(error) => {
                self.phase = RgbZonePhase::Projection;
                Ok(RgbJpegZoneProgress::ProjectionPending(error))
            }
            RgbDetectionStep::Complete(run) => {
                self.current = Some(run);
                self.phase = RgbZonePhase::Tracking;
                self.advance(work, cx)
            }
        }
    }
    ...
    pub fn resume(
        ...
        match self.phase {
            ...
            RgbZonePhase::Projection => {
                let step = self.detector.resume(post, cx)?;
                self.accept_detection_step(step, work, cx)
            }
    ...
    fn advance(
        ...
        cx.checkpoint("rgb-zones:advance")
            .map_err(|_| RgbJpegZoneError::Cancelled)?;
        let progress = match self.phase {
            RgbZonePhase::Tracking => {
                let run = self
                    .current
                    .as_ref()
                    .ok_or(RgbJpegZoneError::StageInvariant)?;
                let admission = self.admission.ok_or(RgbJpegZoneError::StageInvariant)?;
                match self
                    .temporal
                    .observe(run.inference(), run.report(), admission, work)
                {
                    Ok(progress) => Some(progress),
                    Err(error) => return Ok(RgbJpegZoneProgress::TrackingPending(error)),
                }
            }
            RgbZonePhase::Zones => Some(
                self.temporal
                    .resume(work)
                    .map_err(|_| RgbJpegZoneError::StageInvariant)?,
            ),
        ...
        if let Some(progress) = progress {
            match progress {
                RgbZoneProgress::Pending { error, .. } => {
                    self.phase = RgbZonePhase::Zones;
                    return Ok(RgbJpegZoneProgress::ZonesPending(error));
                }
        ...
        cx.checkpoint("rgb-zones:complete")
            .map_err(|_| RgbJpegZoneError::Cancelled)?;
        ...
        let run = self
            ...
            .take()
            .ok_or(RgbJpegZoneError::StageInvariant)?;
        self.complete = Some(RgbZoneCompletion {
            run,
            admission,
            temporal,
        });
    ...
    fn complete_progress(&self) -> Result<RgbJpegZoneProgress, RgbJpegZoneError> {
        let c = self
            ...
            .as_ref()
            .ok_or(RgbJpegZoneError::StageInvariant)?;
        Ok(RgbJpegZoneProgress::Complete {
            inference: c.run.inference().identity(),
            detections: c.run.report().digest(),
            tracking: c.temporal.tracking,
            zones: c.temporal.zones,
        })
    }
    ...
    pub fn take_complete(&mut self) -> Option<RgbZoneCompletion> {
        if self.phase != RgbZonePhase::Complete {
            return None;
        }
        let result = self.complete.take()?;
        self.admission = None;
        self.phase = RgbZonePhase::Ready;
        Some(result)
    }
    ...
    pub fn retire(self) -> RetiredRgbZoneWork {
        RetiredRgbZoneWork {
            ...
            inference: self.detector.retire(),
            detections: self.current,
