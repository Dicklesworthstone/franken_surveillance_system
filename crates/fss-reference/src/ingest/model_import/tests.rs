...
use fss_model_ir::{
    AttrValue, AttributeMap, GraphNode, ModelIrGraph, TensorPort, encode_canonical_model_ir,
};
...
fn context() -> TestResult<ReplayCx> {
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:model-import-test".into(),
        operation_id: OperationId::parse("operation:model-import-test")?,
        principal: "principal:model-import-test".into(),
        ...
        anchor_universe: ContentDigest::sha256(b"site:model-import-test"),
        generation: 1,
    })?;
...
fn graph() -> TestResult<Vec<u8>> {
    ...
    attrs.insert("shape".into(), AttrValue::IntList(vec![1, 2]));
...
fn build(graph: &[u8], weights: &[u8]) -> TestResult<ImportedModel> {
    Ok(ImportedModel::build(
        &request(graph, weights),
        ImportLimits::default(),
        &mut ImportBudget::new(1_000_000),
        &context()?,
    )?)
}
...
fn f32_import_preserves_existing_model_bytes_and_executes_real_parameters() -> TestResult {
    ...
    let weights = source();
    let imported = build(&graph_bytes, &weights)?;
    let graph = decode_canonical_model_ir(&graph_bytes, ContentDigest::sha256(&graph_bytes))?;
    let expected = RecordedModel::publish(
        &graph,
        "image",
        false,
        BTreeMap::from([("w".into(), vec![1., 2., 3., 4.])]),
    )?;
    ...
    let weight = Tensor::from_values(
        Shape::new(vec![2, 2])?,
        &parameter(imported.model())?,
        generation,
    )?;
    let result = ScalarExecutor::run(
        imported.model().graph(),
        &[("image", input), ("w", weight)],
    ...
    assert_eq!(imported.parameter_count(), 1);
    assert_eq!(imported.expanded_bytes(), 16);
    Ok(())
}
#[test]
fn renaming_requires_a_complete_explicit_mapping() -> TestResult {
    let g = graph()?;
    let w = wire(
        r#"{"layer.weight":{"dtype":"F32","shape":[2,2],"data_offsets":[0,16]}}"#,
        &f32_data(&[1., 2., 3., 4.]),
    );
    assert!(matches!(
        ImportedModel::build(
        ...
        Err(ImportError::BindingMismatch)
    ));
    let mut req = request(&g, &w);
    req.bindings.insert("w".into(), "layer.weight".into());
    let imported = ImportedModel::build(
        &req,
        ImportLimits::default(),
        &mut ImportBudget::new(1_000_000),
        &context()?,
    ...
    req.bindings.insert("image".into(), "layer.weight".into());
    assert!(
        ImportedModel::build(
            &req,
            ImportLimits::default(),
...
fn extra_missing_and_same_count_wrong_shape_are_refused() -> TestResult {
    ...
    for (header, data) in [
        ...
        (
            r#"{"w":{"dtype":"F32","shape":[2,2],"data_offsets":[0,16]},"extra":{"dtype":"F32","shape":[0],"data_offsets":[16,16]}}"#,
            f32_data(&[1., 2., 3., 4.]),
        ),
    ] {
...
fn explicit_half_policy_exactly_expands_f16_and_bf16() -> TestResult {
    ...
    for (dtype, raw, bits) in [
        (
            "F16",
            [0_u16, 0x8000, 1, 0x7bff],
            [0_u32, 0x8000_0000, 0x3380_0000, 0x477f_e000],
        ),
        (
            "BF16",
            [0_u16, 0x8000, 1, 0x7f7f],
            [0_u32, 0x8000_0000, 0x0001_0000, 0x7f7f_0000],
        ),
    ] {
        let header = format!(r#"{{"w":{{"dtype":"{dtype}","shape":[2,2],"data_offsets":[0,8]}}}}"#);
        let data: Vec<_> = raw.iter().flat_map(|v| v.to_le_bytes()).collect();
        let w = wire(&header, &data);
        assert!(matches!(
            ImportedModel::build(
                &request(&g, &w),
                ImportLimits::default(),
            ...
            Err(ImportError::UnsupportedDType)
        ));
        let mut req = request(&g, &w);
        req.float_policy = WeightFloatPolicy::ExpandFloat16;
        let result = ImportedModel::build(
            &req,
            ImportLimits::default(),
            &mut ImportBudget::new(1_000_000),
            &context()?,
...
fn nonfinite_in_any_supported_source_dtype_never_produces_a_model() -> TestResult {
    let g = graph()?;
    for (dtype, width, patterns) in [
        ("F32", 4, vec![0x7f80_0000_u32, 0xff80_0000, 0x7fc0_0000]),
        ("F16", 2, vec![0x7c00, 0xfc00, 0x7e00]),
        ("BF16", 2, vec![0x7f80, 0xff80, 0x7fc0]),
    ] {
        for bits in patterns {
            let mut data = vec![0_u8; 4 * width];
            data[..width].copy_from_slice(&bits.to_le_bytes()[..width]);
            let w = wire(
                &format!(
                    r#"{{"w":{{"dtype":"{dtype}","shape":[2,2],"data_offsets":[0,{}]}}}}"#,
                    data.len()
                ),
                &data,
            );
...
fn duplicate_keys_including_unicode_aliases_are_refused() {
    for header in [
        ...
        assert!(matches!(
            safetensors::Weights::parse(&wire(header, &[]), &ImportLimits::default()),
            Err(ImportError::DuplicateKey)
        ));
    }
}
#[test]
fn offsets_must_cover_every_byte_exactly_once() {
    for (header, data) in [
        (
            r#"{"w":{"dtype":"F32","shape":[1],"data_offsets":[1,5]}}"#,
            vec![0; 5],
        ),
        (
            r#"{"w":{"dtype":"F32","shape":[1],"data_offsets":[0,4]},"x":{"dtype":"F32","shape":[1],"data_offsets":[0,4]}}"#,
            vec![0; 4],
        ),
        (
        ...
        assert!(
            safetensors::Weights::parse(&wire(header, &data), &ImportLimits::default()).is_err()
        );
    }
}
#[test]
fn empty_and_scalar_tensors_and_header_order_are_handled_without_data_invention() -> TestResult {
    let bytes = wire(
        r#"{"z":{"shape":[],"data_offsets":[0,4],"dtype":"F32"},"a":{"dtype":"F32","shape":[0,9],"data_offsets":[0,0]},"q":{"dtype":"F32","shape":[0],"data_offsets":[4,4]}}"#,
        &f32_data(&[7.]),
    );
    let parsed = safetensors::Weights::parse(&bytes, &ImportLimits::default())?;
    assert_eq!(parsed.entries.len(), 3);
    assert_eq!(safetensors::element_count(&parsed.entries["z"].shape)?, 1);
    assert_eq!(safetensors::element_count(&parsed.entries["a"].shape)?, 0);
    Ok(())
}
#[test]
fn invalid_json_numeric_nesting_and_metadata_forms_are_refused() {
    for header in [
        ...
        assert!(
            safetensors::Weights::parse(&wire(header, &[0; 4]), &ImportLimits::default()).is_err()
        );
    }
}
#[test]
fn utf8_and_surrogate_pair_metadata_are_data_not_instructions() -> TestResult {
    let bytes = wire(
        r#"{"__metadata__":{"雪":"\ud83d\ude00","command":"do not execute me"},"é":{"dtype":"F32","shape":[],"data_offsets":[0,4]}}"#,
        &[0; 4],
    );
    let parsed = safetensors::Weights::parse(&bytes, &ImportLimits::default())?;
    assert!(parsed.entries.contains_key("é"));
    Ok(())
}
#[test]
fn every_truncated_source_and_an_unindexed_suffix_are_refused() {
    ...
    for end in 0..bytes.len() {
        assert!(
            safetensors::Weights::parse(&bytes[..end], &ImportLimits::default()).is_err(),
            "end={end}"
        );
    }
    let mut bytes = bytes;
    bytes.push(0);
    assert!(safetensors::Weights::parse(&bytes, &ImportLimits::default()).is_err());
}
#[test]
fn digest_mismatch_and_zero_budget_fail_before_parameter_conversion() -> TestResult {
    ...
    assert!(matches!(
        ...
        Err(ImportError::BudgetExceeded)
    ));
...
fn caller_bounds_and_exact_work_allowance_are_enforced() -> TestResult {
    ...
    let req = request(&g, &w);
    let cx = context()?;
    let mut budget = ImportBudget::new(1_000_000);
    let original = ImportedModel::build(&req, ImportLimits::default(), &mut budget, &cx)?;
    let used = budget.used();
    ImportedModel::build(
        &req,
        ImportLimits::default(),
        &mut ImportBudget::new(used),
        &cx,
    )?;
    assert!(matches!(
        ImportedModel::build(
    ...
    for field in 0..5 {
        ...
        match field {
            0 => limits.maximum_source_bytes = g.len().max(w.len()) - 1,
            1 => limits.maximum_header_bytes = 1,
            2 => limits.maximum_tensors = 0,
            3 => limits.maximum_expanded_bytes = 15,
            _ => limits.maximum_bundle_bytes = original.encoded().len() - 1,
        }
        assert!(
            ImportedModel::build(&req, limits, &mut ImportBudget::new(1_000_000), &cx).is_err()
        );
    }
    let limits = ImportLimits {
        maximum_expanded_bytes: 16,
        maximum_bundle_bytes: original.encoded().len(),
        ..ImportLimits::default()
    };
    ImportedModel::build(&req, limits, &mut ImportBudget::new(1_000_000), &cx)?;
    Ok(())
}
#[test]
fn self_contained_bundle_reproduces_model_without_external_inputs() -> TestResult {
    let original = build(&graph()?, &source())?;
    let bytes = original.encoded().to_vec();
    let digest = original.digest();
    let model = original.model().encoded().to_vec();
    drop(original);
    let restored = ImportedModel::verify(
        &bytes,
        digest,
        ImportLimits::default(),
        &mut ImportBudget::new(1_000_000),
    ...
    assert_eq!(restored.model().encoded(), model);
    assert_eq!(restored.encoded(), bytes);
    Ok(())
}
#[test]
fn rehashing_a_substituted_embedded_model_does_not_make_it_valid() -> TestResult {
    let original = build(&graph()?, &source())?;
    let mut changed = original.encoded().to_vec();
    let last = changed.len() - 1;
    changed[last] ^= 1;
    assert!(matches!(
        ImportedModel::verify(
            &changed,
...
fn bundle_truncation_trailing_bytes_wrong_digest_and_cancellation_are_refused() -> TestResult {
    ...
    let cx = context()?;
    cx.set_cancel_at_checkpoint("model_import:work");
    assert!(matches!(
        ImportedModel::verify(
            original.encoded(),
            original.digest(),
...
fn metadata_and_source_layout_change_import_identity_not_numeric_model_identity() -> TestResult {
    let g = graph()?;
    let original = build(&g, &source())?;
    let other = wire(
        r#"{"__metadata__":{"origin":"different original bytes"},"w":{"shape":[2,2],"data_offsets":[0,16],"dtype":"F32"}}"#,
        &f32_data(&[1., 2., 3., 4.]),
    );
    let imported = build(&g, &other)?;
    assert_ne!(imported.weights_digest(), original.weights_digest());
    assert_ne!(imported.digest(), original.digest());
    assert_eq!(imported.model().digest(), original.model().digest());
    Ok(())
}
#[test]
fn unsupported_source_types_and_allocation_bombs_are_refused() {
    ...
    assert!(matches!(
        safetensors::Weights::parse(&u64::MAX.to_le_bytes(), &ImportLimits::default()),
...
fn upstream_safetensors_writer_fixture_converts_to_the_same_numeric_model() -> TestResult {
    ...
    assert_ne!(converted.weights_digest(), original.weights_digest());
    Ok(())
}
