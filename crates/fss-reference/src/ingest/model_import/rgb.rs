...
impl<'a> ImportedRgbModel<'a> {
    ...
    pub fn build(
        request: &RgbModelImportRequest<'a>,
        limits: ImportLimits,
        ...
        if image.dtype() != DType::F32
            || image.shape().dims() != [1, 3, p.target_height, p.target_width]
        {
        ...
        let params: Vec<_> = graph
            .inputs()
            .iter()
            .filter(|port| port.name() != input.as_str())
            .collect();
        ...
        if bindings.len() != params.len() || bindings.contains_key(input) {
            return Err(ImportError::BindingMismatch.into());
        }
        ...
        let total = request
            .graph
            .len()
            .checked_add(request.weights.len())
            .and_then(|n| n.checked_add(expanded_bytes))
            .ok_or(ImportError::Limit)?;
        ...
        let recipe_work = 1024
            + bindings
                .iter()
                .map(|(port, name)| port.len() + name.len() + 16)
                .sum::<usize>();
        budget.charge(
            recipe_work + include_bytes!("rgb.rs").len() + include_bytes!("safetensors.rs").len(),
            cx,
        )?;
        ...
        e.digest(request.graph_digest);
        e.digest(request.weights_digest);
        e.digest(model.digest());
        ...
        checkpoint(cx)?;
        scalar
            .checkpoint("rgb-import:complete")
            .map_err(RgbInferenceError::from)?;
        Ok(Self {
            model,
            graph: request.graph,
            ...
            identity,
            bindings,
            float_policy: request.float_policy,
            expanded_bytes,
        })
    }
    ...
    pub fn float_policy(&self) -> WeightFloatPolicy {
        self.float_policy
    }
