...
impl Default for ImportLimits {
    fn default() -> Self {
        Self {
            maximum_source_bytes: MAX_IMPORT_SOURCE_BYTES,
            maximum_header_bytes: MAX_IMPORT_HEADER_BYTES,
            maximum_tensors: MAX_TENSORS,
            maximum_expanded_bytes: MAX_RECORDED_MODEL_BYTES,
...
impl ImportLimits {
    fn validate(self) -> Result<(), ImportError> {
        if self.maximum_source_bytes == 0
            || self.maximum_source_bytes > MAX_IMPORT_SOURCE_BYTES
            || self.maximum_header_bytes == 0
            || self.maximum_header_bytes > MAX_IMPORT_HEADER_BYTES
            || self.maximum_tensors > MAX_TENSORS
            || self.maximum_expanded_bytes > MAX_RECORDED_MODEL_BYTES
            || self.maximum_bundle_bytes == 0
            || self.maximum_bundle_bytes > MAX_IMPORT_BUNDLE_BYTES
        {
...
impl fmt::Display for ImportError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            Self::Limit => "model import bound exceeded",
            Self::DigestMismatch => "model import digest mismatch",
            Self::MalformedWeights => "malformed or noncontiguous Safetensors input",
            Self::DuplicateKey => "duplicate Safetensors key",
            Self::UnsupportedDType => "source dtype not admitted by import policy",
            Self::NonFinite => "nonfinite model parameter",
            Self::BindingMismatch => "model parameter names or shapes do not match",
            Self::InvalidGraph => "invalid canonical model graph",
            Self::Cancelled => "model import cancelled",
            Self::BudgetExceeded => "model import work budget exhausted",
            Self::InvalidBundle => "invalid or incompatible model import bundle",
            Self::Model(_) => "recorded model contract refused import",
...
impl From<ContractError> for ImportError {
    fn from(e: ContractError) -> Self {
        Self::Contract(e)
    }
}
impl From<ModelRunError> for ImportError {
    fn from(e: ModelRunError) -> Self {
        Self::Model(Box::new(e))
    }
}
fn checkpoint(cx: &ReplayCx) -> Result<(), ImportError> {
    cx.checkpoint("model_import:work")
...
fn encode_bundle(
    ...
    e.bytes(if include_payloads {
        request.weights
    } else {
        &[]
    });
    e.text(request.frame_input);
    e.bool(request.scale_to_unit);
    e.u8(match request.float_policy {
...
impl ImportedModel {
    ...
    pub fn build(
        ...
        checkpoint(cx)?;
        limits.validate()?;
        if request.graph.len() > limits.maximum_source_bytes
            || request.weights.len() > limits.maximum_source_bytes
            || request.frame_input.is_empty()
            || request.frame_input.len() > safetensors::MAX_NAME
            || request.bindings.len() > limits.maximum_tensors
            || request.bindings.iter().any(|(a, b)| {
                a.is_empty()
                    || b.is_empty()
                    || a.len() > safetensors::MAX_NAME
                    || b.len() > safetensors::MAX_NAME
            })
        {
            return Err(ImportError::Limit);
        }
        ...
        let image = graph
            .find_input(request.frame_input)
            .ok_or(ImportError::BindingMismatch)?;
        if image.dtype() != DType::F32 {
            return Err(ImportError::BindingMismatch);
        }
        let params: Vec<_> = graph
            .inputs()
            .iter()
            .filter(|p| p.name() != request.frame_input)
            .collect();
        if params.len() > limits.maximum_tensors {
            return Err(ImportError::Limit);
        }
        let bindings = if request.bindings.is_empty() {
            params
                .iter()
                .map(|p| (p.name().to_owned(), p.name().to_owned()))
                .collect::<BTreeMap<_, _>>()
        } else {
            request.bindings.clone()
        };
        if bindings.len() != params.len() || bindings.contains_key(request.frame_input) {
            return Err(ImportError::BindingMismatch);
        }
        ...
        for port in &params {
            ...
            if entry.dtype != safetensors::FloatType::F32
                && request.float_policy == WeightFloatPolicy::F32Only
            {
            ...
            expanded_bytes = expanded_bytes
                .checked_add(
                    safetensors::element_count(&entry.shape)?
                        .checked_mul(4)
        ...
        if referenced.len() != weights.entries.len() {
            return Err(ImportError::BindingMismatch);
        }
        ...
        for (port, name) in &bindings {
            ...
            values
                .try_reserve_exact(safetensors::element_count(&entry.shape)?)
        ...
        let total = overhead
            .checked_add(request.graph.len())
            .and_then(|n| n.checked_add(request.weights.len()))
            .and_then(|n| n.checked_add(model.encoded().len()))
            .ok_or(ImportError::Limit)?;
        if total > limits.maximum_bundle_bytes {
            return Err(ImportError::Limit);
        }
    ...
    pub fn verify(
        ...
        checkpoint(cx)?;
        limits.validate()?;
        if bytes.len() > limits.maximum_bundle_bytes {
            return Err(ImportError::Limit);
        }
        ...
        if d.bytes()? != b"FSSIMPT1"
            || d.u32()? != 1
            || d.text()? != DOMAIN
            || d.digest()? != importer_identity()
        {
            return Err(ImportError::InvalidBundle);
        }
        let graph = d.bytes()?;
        let weights = d.bytes()?;
        let frame_input = d.text()?;
        let scale_to_unit = d.bool()?;
        let float_policy = match d.u8()? {
        ...
        for _ in 0..n {
            let port = d.text()?;
            let name = d.text()?;
            if port.len() > safetensors::MAX_NAME || name.len() > safetensors::MAX_NAME {
                return Err(ImportError::Limit);
            }
            if previous.is_some_and(|p| p >= port) {
                return Err(ImportError::InvalidBundle);
            }
            bindings.insert(port.to_owned(), name.to_owned());
            previous = Some(port);
        }
        let embedded = d.bytes()?;
        d.ensure_finished()?;
        if embedded.len() > MAX_RECORDED_MODEL_BYTES
            || graph.len() > limits.maximum_source_bytes
            || weights.len() > limits.maximum_source_bytes
        {
            return Err(ImportError::Limit);
        }
    ...
    #[must_use]
    pub fn model(&self) -> &RecordedModel {
        &self.model
    }
    ...
    #[must_use]
    pub fn weights_digest(&self) -> ContentDigest {
        self.weights_digest
    }
