...
use super::super::rgb_inference::{RgbInference, RgbInferenceError, RgbInferenceModel, RgbRunLimits, RgbSourceBinding};
use super::{RgbDetectionBudget, RgbDetectionContract, RgbDetectionError, RgbDetectionReport, project_rgb_detections};
...
impl From<RgbInferenceError> for RgbDetectorError {
    fn from(error: RgbInferenceError) -> Self { Self::Inference(error) }
}
...
#[derive(Debug)]
pub struct PendingRgbDetection { inference: RgbInference, allowed: Vec<u8>, mask_copy_work: u64 }
impl PendingRgbDetection {
    /// Complete immutable neural result, even when postprocessing was refused.
    pub fn inference(&self) -> &RgbInference { &self.inference }
...
impl<'a> RgbDetector<'a> {
    ...
    pub fn new(model: &'a RgbInferenceModel, contract: &'a RgbDetectionContract) -> Result<Self, RgbDetectorError> {
        if model.digest() != contract.spec().model { return Err(RgbDetectorError::ModelContractMismatch); }
        Ok(Self { model, contract, pending: None })
    }
    /// Accepted outputs that must not be hidden by optional-stage failure.
    pub fn pending(&self) -> Option<&PendingRgbDetection> { self.pending.as_ref() }
    ...
    pub fn run_jpeg(&mut self, input: RgbDetectionInput<'_>, limits: RgbRunLimits,
        decoder: &mut DecodeBudget<'_>, postprocess: &mut RgbDetectionBudget, cx: &ScalarExecCx)
        -> Result<RgbDetectionStep, RgbDetectorError> {
        if self.pending.is_some() { return Err(RgbDetectorError::PendingFrame); }
        cx.checkpoint("rgb-detector:retain-mask").map_err(RgbInferenceError::from)?;
        let count = input.allowed.len();
        if count == 0 || count > 4_194_304 { return Err(RgbDetectorError::InputLimit); }
        let mut adjusted = limits;
        adjusted.preprocess = ExecBudget::new(
            limits.preprocess.max_macs.checked_sub(count as u64).ok_or(RgbDetectorError::InputLimit)?,
            limits.preprocess.max_bytes.checked_sub(count).ok_or(RgbDetectorError::InputLimit)?);
        let mut allowed = Vec::new();
        allowed.try_reserve_exact(count).map_err(|_| RgbDetectorError::InputLimit)?;
        for chunk in input.allowed.chunks(4096) {
            cx.checkpoint("rgb-detector:mask-copy").map_err(RgbInferenceError::from)?;
            allowed.extend_from_slice(chunk);
        }
        let inference = self.model.run_jpeg(input.bytes, input.interpretation, input.source,
            &allowed, adjusted, decoder, cx)?;
        self.pending = Some(PendingRgbDetection { inference, allowed, mask_copy_work: count as u64 });
    ...
    pub fn resume(&mut self, budget: &mut RgbDetectionBudget, cx: &ScalarExecCx)
        -> Result<RgbDetectionStep, RgbDetectorError> {
        let pending = self.pending.as_ref().ok_or(RgbDetectorError::NoPendingFrame)?;
        match project_rgb_detections(&pending.inference, self.contract, &pending.allowed, budget, cx) {
            Ok(report) => {
                let pending = self.pending.take().ok_or(RgbDetectorError::NoPendingFrame)?;
                Ok(RgbDetectionStep::Complete(RgbDetectionRun { pending, report }))
            }
    ...
    #[must_use]
    pub fn retire(self) -> Option<PendingRgbDetection> { self.pending }
}
