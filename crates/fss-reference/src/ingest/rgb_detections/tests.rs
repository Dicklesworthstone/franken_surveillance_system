...
fn spec() -> RgbDetectionSpec {
    RgbDetectionSpec {
        model: ContentDigest::sha256(b"numeric-fixture"),
        output_port: "head".into(),
        labels: vec!["fixture-a".into(), "fixture-b".into()],
...
fn both_dense_axis_orders_produce_identical_source_proposals() -> Test {
    let rows = [
        [1.0, 3.0, 3.0, 5.0, 0.8, 0.1],
        [4.0, 3.0, 6.0, 5.0, 0.1, 0.9],
    ];
    ...
    assert_eq!(a.detections[1].bounds(), [512, 512, 1536, 1536]);
    Ok(())
}
#[test]
fn all_four_box_encodings_agree_after_letterbox_reversal() -> Test {
    ...
    for (encoding, b) in [
        (HeadBoxes::PixelCorners, [1.0, 3.0, 3.0, 5.0]),
        ...
        assert_eq!(r.detections[0].bounds(), expected);
        assert!(!r.detections[0].clipped());
    }
    Ok(())
}
#[test]
fn padding_and_clipping_are_explicit_not_phantom_source_boxes() -> Test {
    let r = decode(
        &[
            [0.0, 0.0, 8.0, 1.0, 0.8, 0.1],
            [-1.0, 1.0, 9.0, 7.0, 0.9, 0.1],
        ],
        spec(),
        &[1; 128],
    )?;
    ...
    assert!(r.detections[0].clipped());
    assert_eq!(r.detections[0].bounds(), [0, 0, 4096, 2048]);
    Ok(())
}
#[test]
fn full_pixel_footprint_not_just_center_controls_privacy() -> Test {
    ...
    let r = decode(
        &[
            [1.0, 3.0, 3.0, 5.0, 0.9, 0.1],
            [4.0, 3.0, 6.0, 5.0, 0.9, 0.1],
        ],
        spec(),
        &mask,
    )?;
    ...
    assert_eq!(r.detections.len(), 1);
    assert_eq!(r.detections[0].row(), 1);
    let mut wrong = mask;
    wrong[0] = 2;
    assert!(matches!(
        decode(&[], spec(), &wrong),
...
fn class_aware_nms_retains_all_decisions_and_ties_choose_original_row() -> Test {
    ...
    assert_eq!(
        r.detections.iter().map(|d| d.row()).collect::<Vec<_>>(),
        [0, 2]
    );
    Ok(())
}
...
fn threshold_is_inclusive_and_iou_comparison_is_strict() -> Test {
    ...
    assert!(suppresses([0, 0, 512, 512], [0, 0, 256, 512], 499_999));
    Ok(())
}
#[test]
fn multilabel_is_explicit_and_best_class_ties_are_deterministic() -> Test {
    ...
    assert_eq!(best.detections[0].class_index(), 0);
    let multi = decode(
        &[row],
        RgbDetectionSpec {
    ...
    assert_eq!(multi.rows[0].disposition, HeadRowDisposition::Candidates(2));
    Ok(())
}
#[test]
fn objectness_and_logit_rules_are_not_inferred_or_applied_twice() -> Test {
    let s = RgbDetectionSpec {
        objectness: Some(HeadScore::Logit),
        class_score: HeadScore::Logit,
        minimum_score_ppm: 250_000,
        ..spec()
    };
    let c = RgbDetectionContract::new(s)?;
    let r = decode_head(
        &[1, 1, 7],
        &[1.0, 3.0, 3.0, 5.0, 0.0, 0.0, -100.0],
        geometry(),
        &[1; 128],
        ContentDigest::sha256(&[1; 128]).bytes(),
        &c,
        &mut budget(),
        &ScalarExecCx::new(),
    )?;
    assert_eq!(r.detections[0].score(), 0.25);
    assert_eq!(r.detections[0].class_index(), 0);
    let s = RgbDetectionSpec {
    ...
    let r = decode_head(
        &[1, 1, 7],
        &[1.0, 3.0, 3.0, 5.0, 0.25, 0.99, 0.01],
        geometry(),
        &[1; 128],
        ContentDigest::sha256(&[1; 128]).bytes(),
        &c,
    ...
    assert_eq!(r.rows[0].disposition, HeadRowDisposition::BelowThreshold);
    Ok(())
}
#[test]
fn malformed_low_score_rows_and_losing_classes_cannot_hide_behind_filtering() -> Test {
    for row in [
        [1.0, 3.0, 1.0, 5.0, 0.0, 0.0],
        [1.0, 3.0, 3.0, 5.0, 0.0, -0.1],
        [1.0, 3.0, 3.0, 5.0, 0.9, f32::NAN],
        [1.0, 3.0, 3.0, 5.0, 0.9, 1.1],
        [f32::INFINITY, 3.0, 3.0, 5.0, 0.0, 0.0],
    ] {
        assert!(matches!(
            decode(&[row], spec(), &[1; 128]),
            Err(RgbDetectionError::InvalidOutput)
        ));
    }
    Ok(())
}
...
fn no_pre_or_post_nms_topk_truncation_occurs() -> Test {
    ...
    let c = RgbDetectionSpec {
        maximum_candidates: 1,
        maximum_detections: 1,
        ..spec()
    };
    assert!(matches!(
        decode(&[row, row], c, &[1; 128]),
        Err(RgbDetectionError::Limit)
    ));
    let c = RgbDetectionSpec {
        maximum_detections: 1,
        nms_iou_ppm: 1_000_000,
...
fn shape_mask_scratch_and_cancellation_refusals_are_explicit() -> Test {
    let c = RgbDetectionContract::new(spec())?;
    let cx = ScalarExecCx::new();
    let mask = [1; 128];
    let hash = ContentDigest::sha256(&mask).bytes();
    for shape in [vec![1, 6], vec![2, 1, 6], vec![1, 0, 7]] {
        assert!(matches!(
            decode_head(&shape, &[], geometry(), &mask, hash, &c, &mut budget(), &cx),
            Err(RgbDetectionError::InvalidOutput)
        ));
    }
    assert!(matches!(
    ...
    assert!(matches!(
        decode_head(
            ...
            hash,
            &c,
            &mut RgbDetectionBudget::new(1_000_000, 1),
            &cx
        ),
        Err(RgbDetectionError::Limit)
    ));
    ...
    assert!(matches!(
        ...
        Err(RgbDetectionError::Cancelled)
    ));
    Ok(())
}
#[test]
fn every_core_budget_cut_refuses_atomically_and_retry_reproduces() -> Test {
    ...
    assert_eq!(expected.candidates, replay.candidates);
    Ok(())
}
#[test]
...
fn contract_identity_binds_interpretation_and_rejects_ambiguous_vocabulary() -> Test {
    ...
    for s in [
        ...
        RgbDetectionSpec {
            labels: vec!["other-a".into(), "other-b".into()],
            ..spec()
        },
    ] {
    ...
    for s in [
        ...
        RgbDetectionSpec {
            minimum_score_ppm: 1_000_001,
            ..spec()
        },
    ] {
        assert!(matches!(
            RgbDetectionContract::new(s),
            Err(RgbDetectionError::InvalidContract)
        ));
    }
    Ok(())
}
