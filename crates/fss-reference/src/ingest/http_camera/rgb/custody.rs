...
use super::{HttpRgbCapture, HttpCameraAuthority, HttpCameraError, HttpCameraOperation,
    HttpWireReceipt};
use crate::ingest::http_archive::{HttpArchiveError, HttpWireArchive, HttpWirePin,
    HttpWirePublication};
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct HttpRgbWirePlan { wire: HttpWireReceipt, pin: HttpWirePin }
...
impl HttpRgbWireCommit {
    ...
    pub fn publication(&self) -> &HttpWirePublication { &self.publication }
    /// Whether the original camera's parse barrier was released under live authority.
    pub fn acknowledgement(&self) -> Result<(), HttpCameraError> { self.acknowledgement }
...
impl HttpRgbCapture<'_, '_> {
    ...
    pub fn prepare_wire_custody(&mut self, archive: &HttpWireArchive, now: u64,
        auth: &dyn HttpCameraAuthority, work: &mut WorkBudget<'_>)
        -> Result<HttpRgbWirePlan, HttpRgbCustodyError> {
        self.camera.admit(HttpCameraOperation::Poll, now, auth)?;
        let read = self.camera.pending_wire().filter(|read| !read.acknowledged())
            .ok_or(HttpRgbCustodyError::NotPending)?;
        let plan = archive.prepare(read, work)?;
        let result = HttpRgbWirePlan { wire: read.receipt(), pin: plan.pin() };
        self.camera.admit(HttpCameraOperation::Poll, now, auth)?;
        Ok(result)
    }
    ...
    pub fn retain_wire(&mut self, expected: HttpRgbWirePlan, now: u64,
        auth: &dyn HttpCameraAuthority, custody: HttpRgbCustody<'_, '_>)
        -> Result<HttpRgbWireCommit, HttpRgbCustodyError> {
        self.camera.admit(HttpCameraOperation::Poll, now, auth)?;
        let read = self.camera.pending_wire().filter(|read| !read.acknowledged())
            .ok_or(HttpRgbCustodyError::NotPending)?;
        if read.receipt() != expected.wire { return Err(HttpRgbCustodyError::PlanMismatch); }
        let plan = custody.archive.prepare(read, custody.work)?;
        if plan.pin() != expected.pin { return Err(HttpRgbCustodyError::PlanMismatch); }
        let publication = custody.archive.publish(&plan, custody.publisher,
            custody.cancellation, custody.work)?;
        ...
        Ok(HttpRgbWireCommit { publication, acknowledgement })
    }
}
