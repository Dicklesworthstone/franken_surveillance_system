...
impl SampledReport {
    pub(super) fn assemble(
        ...
        if decisions.len() != plan.count
            || comparisons_used > plan.maximum_comparisons
        ...
        for (ordinal, decision) in decisions.iter().enumerate() {
            if decision.ordinal() != ordinal as u64
                || decision.import_identity() != plan.import
        ...
        if selected.len() != analysis.observations().len() {
            return Err(SamplingError::Mismatch);
        }
        ...
        if bytes.len() > MAX_SAMPLED_REPORT_BYTES {
            return Err(SamplingError::Limit);
        }
        Ok(Self {
            plan,
    ...
    pub fn verify(
        ...
        if limits.maximum_frames == 0
            || limits.maximum_frames > MAX_ANALYSIS_FRAMES
            || limits.maximum_report_bytes == 0
            || limits.maximum_report_bytes > MAX_ANALYSIS_REPORT_BYTES
            || plan.count > limits.maximum_frames
            || plan.maximum_comparisons > maximum_comparisons
        {
            return Err(SamplingError::Limit);
        }
        let claimed_work = d.u64()?;
        let n = bounded_count(&mut d, MAX_ANALYSIS_FRAMES)?;
        if n != plan.count || claimed_work > plan.maximum_comparisons {
            return Err(SamplingError::Mismatch);
        }
        ...
        if analysis_bytes.len() > limits.maximum_report_bytes {
            return Err(SamplingError::Limit);
        }
        d.ensure_finished()?;
        ...
        for (ordinal, encoded) in encoded_decisions.iter().enumerate() {
            ...
            if decision.encoded().as_slice() != *encoded {
                return Err(SamplingError::Mismatch);
            }
            decisions.push(decision);
        }
        if sampler.comparisons_used() != claimed_work {
            return Err(SamplingError::Mismatch);
        }
        let analysis = AnalysisReport::verify(
            deployment,
            analysis_bytes,
            ContentDigest::sha256(analysis_bytes),
            limits,
            budget,
            cx,
        )?;
    ...
    #[must_use]
    pub fn decisions(&self) -> &[SamplingDecision] {
        &self.decisions
    }
...
fn check(cx: &ReplayCx) -> Result<(), SamplingError> {
    cx.checkpoint("sampled_recording:verify")
