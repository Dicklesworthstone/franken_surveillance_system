...
impl SamplingPlan {
    ...
    fn build(
        ...
        if import.algorithm() != DigestAlgorithm::Sha256
            || count == 0
            || count > MAX_ANALYSIS_FRAMES
            || required.len() > count
            || maximum_comparisons > MAX_SAMPLING_COMPARISONS
        {
        ...
        for item in required {
            if !(first..end).contains(&item.segment_index)
                ...
                || required_map
                    .insert(item.segment_index, item.basis)
                    .is_some()
            {
                return Err(SamplingError::InvalidPlan);
            }
        ...
        e.bytes(b"FSSASPL1");
        e.u32(1);
        e.text("fss.recorded_activity_plan.v1");
        e.digest(import);
        ...
        e.u64(first as u64);
        e.u64(count as u64);
        policy.encode_canonical(&mut e);
        e.u64(maximum_comparisons);
        e.u64(required_map.len() as u64);
        for (segment, basis) in &required_map {
            e.u64(*segment as u64);
            e.digest(*basis);
        }
        let bytes = e.finish_checked()?;
        if bytes.len() > MAX_SAMPLING_PLAN_BYTES {
    ...
    pub fn decode(bytes: &[u8], expected: ContentDigest) -> Result<Self, SamplingError> {
        ...
        if d.bytes()? != b"FSSASPL1"
            || d.u32()? != 1
            || d.text()? != "fss.recorded_activity_plan.v1"
        {
            return Err(SamplingError::InvalidPlan);
        }
        let import = d.digest()?;
        ...
        let policy = ActivityPolicy {
            change: PixelChangeConfig {
                minimum_delta: d.u8()?,
                minimum_changed_pixels: d.u64()?,
                minimum_changed_fraction_ppm: d.u32()?,
            },
            sentinel_stride: d.u32()?,
            post_activity_frames: d.u32()?,
        };
        let maximum_comparisons = d.u64()?;
        let n = bounded_count(&mut d, MAX_ANALYSIS_FRAMES)?;
        let mut required = Vec::with_capacity(n);
        for _ in 0..n {
            required.push(RequiredFrame {
                segment_index: bounded_count(&mut d, usize::MAX)?,
                basis: d.digest()?,
            });
        }
        d.ensure_finished()?;
    ...
    #[must_use]
    pub fn maximum_comparisons(&self) -> u64 {
        self.maximum_comparisons
    }
    /// Exact sorted required-frame inclusion constraints.
    #[must_use]
    pub fn required_frames(&self) -> &BTreeMap<usize, ContentDigest> {
        &self.required
    }
    fn binds(&self, request: &RecordingRequest) -> bool {
        self.import == request.import_identity
            && self.interpretation == request.interpretation
            && self.first == request.first_segment
            && self.count == request.segment_count
    }
    fn source(&self, index: usize, limits: &AnalysisLimits) -> RecordedDecodeRequest {
        RecordedDecodeRequest {
            import_identity: self.import,
...
impl fmt::Display for SamplingError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            Self::InvalidPlan => "invalid recorded sampling plan",
            Self::Limit => "recorded sampling bound exceeded",
            Self::Mismatch => "recorded sampling replay mismatch",
            Self::Cancelled => "recorded sampling cancelled",
            Self::Contract(_) => "invalid recorded sampling encoding",
            Self::Activity(_) => "activity sampling refused",
            Self::Decode(_) => "sampling source revalidation refused",
            Self::Analysis(_) => "sampling analysis revalidation refused",
        })
    }
}
...
impl From<ContractError> for SamplingError {
    fn from(e: ContractError) -> Self {
        Self::Contract(e)
    }
}
impl From<ActivityError> for SamplingError {
    fn from(e: ActivityError) -> Self {
        Self::Activity(e)
    }
}
impl From<RecordedDecodeError> for SamplingError {
    fn from(e: RecordedDecodeError) -> Self {
        Self::Decode(Box::new(e))
    }
}
impl From<AnalysisError> for SamplingError {
...
pub fn run_sampled_recording(
    ...
    if !sampling.binds(request) {
        return Err(Box::new(SampledRecordingFailure {
            cause: SampledFailureCause::Sampling(SamplingError::InvalidPlan),
            progress: initial,
            decisions: Vec::new(),
            comparisons_used: 0,
        }));
    ...
    let mut sampler = match ActivitySampler::new(sampling.policy, sampling.maximum_comparisons) {
        ...
        Err(e) => {
            return Err(Box::new(SampledRecordingFailure {
                cause: SampledFailureCause::Sampling(e.into()),
                progress: initial,
                decisions: Vec::new(),
                comparisons_used: 0,
            }));
    ...
    match result {
        Err(error) => {
            ...
            Err(Box::new(SampledRecordingFailure {
                cause: SampledFailureCause::Pipeline(cause),
                progress,
                decisions,
                comparisons_used,
            }))
        ...
        Ok(outcome) => {
            ...
            let sealed = cx
                .checkpoint("sampled_recording:seal")
                .map_err(|_| SamplingError::Cancelled)
                .and_then(|()| {
                    SampledReport::assemble(
                        sampling.clone(),
                        decisions.clone(),
                        outcome.report,
                        comparisons_used,
            ...
            match sealed {
                Ok(report) => {
                    ...
                    Ok(SampledRecordingOutcome { report, progress })
                }
                Err(cause) => Err(Box::new(SampledRecordingFailure {
                    cause: SampledFailureCause::Sampling(cause),
                    progress,
                    decisions,
                    comparisons_used,
                })),
