...
use fss_core::{CanonicalDecoder, CanonicalEncoder, CaptureInterval, ContentDigest,
    DigestAlgorithm, TimestampNs};
use fss_geometry::{GeometryError, WorkBudget};
use fss_object::ObjectManifest;
use fss_publication::{PublishCancellation, PublishCutPoint, RootLedgerReceipt,
...
use super::rgb_evidence::{ReplayedRgbEvidence, RgbEvidence, RgbEvidenceBudget,
    RgbEvidenceError, RgbEvidenceLimits};
...
pub trait RgbArchiveAuthority {
    ...
    fn permits(&self, operation: RgbArchiveOperation, retention: ContentDigest,
        evidence: ContentDigest) -> bool;
}
...
impl RgbArchivePin {
    ...
    pub fn slot(self) -> Result<SlotName> {
        if ![self.root, self.evidence, self.retention].iter().copied().all(valid_digest)
            || self.capture[0] > self.capture[1] { return Err(RgbArchiveError::Mismatch); }
        let mut e = CanonicalEncoder::new(); e.text(DOMAIN); e.digest(self.retention); e.digest(self.evidence);
...
impl From<RgbEvidenceError> for RgbArchiveError {
    fn from(e: RgbEvidenceError) -> Self { Self::Evidence(e) }
}
impl From<GeometryError> for RgbArchiveError {
    fn from(e: GeometryError) -> Self { Self::Work(e) }
}
fn storage<E: std::error::Error + 'static>(e: E) -> RgbArchiveError { RgbArchiveError::Storage(Box::new(e)) }
/// Results from original-byte custody operations.
pub type Result<T> = std::result::Result<T, RgbArchiveError>;
fn valid_digest(d: ContentDigest) -> bool { d.algorithm() == DigestAlgorithm::Sha256 && d.bytes() != [0; 32] }
fn hex(d: ContentDigest) -> String { d.bytes().iter().map(|b| format!("{b:02x}")).collect() }

#[derive(Clone, Copy)]
struct Part { digest: ContentDigest, bytes: usize }
struct Index { pin: RgbArchivePin, parts: [Part; 5] }
impl Index {
    fn encode(&self) -> Result<Vec<u8>> {
        let mut e = CanonicalEncoder::new(); e.text(DOMAIN);
        e.digest(self.pin.evidence); e.digest(self.pin.retention);
        for n in self.pin.capture { e.u64(n); }
        for part in self.parts { e.digest(part.digest); e.u64(part.bytes as u64); }
    ...
    fn decode(bytes: &[u8], pin: RgbArchivePin) -> Result<Self> {
        ...
        if d.text().map_err(storage)? != DOMAIN { return Err(RgbArchiveError::Mismatch); }
        ...
        let mut parts = [Part { digest: evidence, bytes: 0 }; 5];
        for part in &mut parts {
            part.digest = d.digest().map_err(storage)?;
            part.bytes = usize::try_from(d.u64().map_err(storage)?).map_err(|_| RgbArchiveError::Limit)?;
            if !valid_digest(part.digest) { return Err(RgbArchiveError::Mismatch); }
        }
        d.ensure_finished().map_err(storage)?;
        let index = Self { pin, parts };
        if index.parts[0].digest != evidence || index.encode()? != bytes || index.manifest()?.root() != pin.root {
            return Err(RgbArchiveError::Mismatch);
        }
        Ok(index)
    }
    fn size(&self, limits: RgbEvidenceLimits) -> Result<usize> {
        if !(1..=64 * 1024 * 1024).contains(&limits.maximum_bytes)
            || !(1..=16 * 1024 * 1024).contains(&limits.maximum_source_bytes) { return Err(RgbArchiveError::Limit); }
        let ceilings = [256 * 1024, limits.maximum_source_bytes, limits.maximum_source_bytes,
            limits.maximum_source_bytes, 4_194_304];
        let mut size = 48usize;
        for (part, ceiling) in self.parts.iter().zip(ceilings) {
            if part.bytes == 0 || part.bytes > ceiling { return Err(RgbArchiveError::Limit); }
            size = size.checked_add(part.bytes).ok_or(RgbArchiveError::Limit)?;
        }
        if size > limits.maximum_bytes { return Err(RgbArchiveError::Limit); }
        Ok(size)
    }
}
...
pub struct PreparedRgbArchive {
    index: Index, envelope: Vec<u8>, ranges: [Range<usize>; 5], limits: RgbArchiveLimits,
}
impl std::fmt::Debug for PreparedRgbArchive {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PreparedRgbArchive").field("pin", &self.index.pin).finish_non_exhaustive()
    }
}
impl PreparedRgbArchive {
    /// Bind originals to a real native replay, never to an unverified output digest.
    pub fn new(evidence: &RgbEvidence, replay: &ReplayedRgbEvidence, retention: ContentDigest,
        limits: RgbArchiveLimits, copy_work: &mut RgbEvidenceBudget, work: &mut WorkBudget<'_>,
        cx: &ReplayCx) -> Result<Self> {
        if !valid_digest(retention) || evidence.identity() != replay.evidence_identity() { return Err(RgbArchiveError::Mismatch); }
        ...
        let mut parts = [Part { digest: evidence.identity(), bytes: 0 }; 5];
        for (range, part) in ranges.iter_mut().zip(&mut parts) {
            ...
            let n = usize::try_from(u64::from_le_bytes(raw.try_into().map_err(|_| RgbArchiveError::Mismatch)?))
                .map_err(|_| RgbArchiveError::Limit)?;
            at = end; let end = at.checked_add(n).ok_or(RgbArchiveError::Limit)?;
            let bytes = envelope.get(at..end).ok_or(RgbArchiveError::Mismatch)?;
            *range = at..end; *part = Part { digest: ContentDigest::sha256(bytes), bytes: n }; at = end;
        }
        if at != envelope.len() || parts[0].digest != evidence.identity() { return Err(RgbArchiveError::Mismatch); }
        let mut index = Index { pin: RgbArchivePin { root: evidence.identity(), evidence: evidence.identity(),
            retention, capture: replay.admission().source().capture }, parts };
        index.size(limits.evidence)?;
        index.pin.root = index.manifest()?.root(); index.pin.slot()?;
        cx.checkpoint("rgb-archive:prepared").map_err(|_| RgbArchiveError::Cancelled)?;
        Ok(Self { index, envelope, ranges, limits })
    }
    /// Candidate known before disk mutation, useful for exact reconciliation after ACK loss.
    pub fn pin(&self) -> RgbArchivePin { self.index.pin }
    ...
    pub fn publish(&self, deployment: &mut ReferenceDeployment, authority: &dyn RgbArchiveAuthority,
        work: &mut WorkBudget<'_>, cx: &ReplayCx) -> Result<RootLedgerReceipt> {
        let pin = self.pin(); let operation = RgbArchiveOperation::RetainOriginals;
        authorize(pin, authority, operation, cx)?;
        admit(deployment, pin, self.limits, work, cx)?;
        let metadata = self.index.encode()?; let manifest = self.index.manifest()?;
        ...
        for (part, range) in self.index.parts.iter().zip(&self.ranges) {
            ...
            if deployment.stage_payload(&self.envelope[range.clone()]).map_err(storage)? != part.digest {
                return Err(RgbArchiveError::Mismatch);
            }
        }
        authorize(pin, authority, operation, cx)?;
        ...
        let validity = CaptureInterval::new(TimestampNs(i128::from(pin.capture[0])),
            TimestampNs(i128::from(pin.capture[1]))).map_err(storage)?;
        let receipt = deployment.ledgered_publisher().publish_and_commit_cancellable(
            &pin.slot()?, &manifest, validity, &cancel).map_err(storage)?;
        Ok(receipt)
    }
}
...
pub fn restore_rgb_evidence(deployment: &mut ReferenceDeployment, pin: RgbArchivePin,
    limits: RgbArchiveLimits, authority: &dyn RgbArchiveAuthority, copy_work: &mut RgbEvidenceBudget,
    work: &mut WorkBudget<'_>, cx: &ReplayCx) -> Result<RgbEvidence> {
    authorize(pin, authority, RgbArchiveOperation::ReadOriginals, cx)?;
    admit(deployment, pin, limits, work, cx)?;
    match deployment.ledgered_publisher().state(&pin.slot()?).map_err(storage)? {
    ...
    let metadata = manifest.metadata_digest().ok_or(RgbArchiveError::Mismatch)?;
    let bytes = read(deployment, pin, metadata, INDEX_LIMIT, authority, work, cx)?;
    let index = Index::decode(&bytes, pin)?;
    if index.manifest()? != manifest { return Err(RgbArchiveError::Mismatch); }
    let size = index.size(limits.evidence)?;
    work.charge(size as u64)?;
    let mut envelope = Vec::new(); envelope.try_reserve_exact(size).map_err(|_| RgbArchiveError::Limit)?;
    envelope.extend_from_slice(b"FSSRGBE1");
    for part in index.parts {
        let bytes = read(deployment, pin, part.digest, part.bytes, authority, work, cx)?;
        if bytes.len() != part.bytes { return Err(RgbArchiveError::Mismatch); }
        envelope.extend_from_slice(&(bytes.len() as u64).to_le_bytes()); envelope.extend_from_slice(&bytes);
    }
...
fn authorize(pin: RgbArchivePin, auth: &dyn RgbArchiveAuthority, operation: RgbArchiveOperation, cx: &ReplayCx) -> Result<()> {
    cx.checkpoint("rgb-archive:authority").map_err(|_| RgbArchiveError::Cancelled)?;
    if !auth.permits(operation, pin.retention, pin.evidence) { return Err(RgbArchiveError::Denied); }
    Ok(())
}
fn admit(d: &ReferenceDeployment, pin: RgbArchivePin, l: RgbArchiveLimits,
    ...
    if d.publisher().is_poisoned() { return Err(RgbArchiveError::NotCommitted); }
    let mut count = 0usize;
    for batch in d.ledger().batches() {
        work.charge(1 + batch.deltas.len() as u64)?;
        count = count.checked_add(batch.deltas.len()).ok_or(RgbArchiveError::Limit)?;
        if count > l.maximum_ledger_deltas { return Err(RgbArchiveError::Limit); }
    }
    work.charge(d.publisher().limits().spool.max_object_bytes as u64 * 4)?;
    Ok(())
}
fn not_deleted(d: &ReferenceDeployment, digest: ContentDigest, work: &mut WorkBudget<'_>) -> Result<()> {
    work.charge(d.publisher().limits().max_tombstones as u64 + 1)?;
    if d.publisher().tombstones().any(|v| *v == digest) { return Err(RgbArchiveError::Tombstoned); }
    Ok(())
}
fn read(d: &ReferenceDeployment, pin: RgbArchivePin, digest: ContentDigest, maximum: usize,
    auth: &dyn RgbArchiveAuthority, work: &mut WorkBudget<'_>, cx: &ReplayCx) -> Result<Vec<u8>> {
    ...
    if bytes.len() > maximum { return Err(RgbArchiveError::Limit); }
    if ContentDigest::sha256(&bytes) != digest { return Err(RgbArchiveError::Mismatch); }
    authorize(pin, auth, RgbArchiveOperation::ReadOriginals, cx)?;
    Ok(bytes)
}
struct Gate<'a> { pin: RgbArchivePin, authority: &'a dyn RgbArchiveAuthority, cx: &'a ReplayCx }
impl PublishCancellation for Gate<'_> {
    fn cancel_requested(&self, point: PublishCutPoint) -> bool {
        ReplayCancellationBridge(self.cx).cancel_requested(point)
            || !self.authority.permits(RgbArchiveOperation::RetainOriginals, self.pin.retention, self.pin.evidence)
    }
}
