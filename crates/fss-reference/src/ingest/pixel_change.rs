...
impl PixelChangeConfig {
    ...
    pub fn validate(self) -> Result<(), PixelChangeError> {
        if self.minimum_delta == 0
            || self.minimum_changed_pixels == 0
            || self.minimum_changed_pixels > 4_194_304
            || self.minimum_changed_fraction_ppm > 1_000_000
        {
    ...
    pub fn digest(self) -> ContentDigest {
        self.canonical_digest("fss.pixel_change_config.v1")
    }
}
impl CanonicalEncode for PixelChangeConfig {
    fn encode_canonical(&self, e: &mut CanonicalEncoder) {
        e.u8(self.minimum_delta);
        e.u64(self.minimum_changed_pixels);
        e.u32(self.minimum_changed_fraction_ppm);
    }
}
...
impl PixelChangeReset {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::RecordingChanged => "recording_changed",
            Self::SourceChanged => "source_changed",
            Self::SourceGap => "source_gap",
            Self::DimensionsChanged => "dimensions_changed",
            Self::InterpretationChanged => "interpretation_changed",
        }
    }
}
...
fn resets(previous: Option<&Basis>, current: &Basis) -> BTreeSet<PixelChangeReset> {
    ...
    if previous.import != current.import {
        result.insert(PixelChangeReset::RecordingChanged);
    }
    if previous.sensor != current.sensor || previous.stream != current.stream {
        result.insert(PixelChangeReset::SourceChanged);
    }
    if current.gap
        || previous.segment.checked_add(1) != Some(current.segment)
        || previous.sequence.checked_add(1) != Some(current.sequence)
    {
        result.insert(PixelChangeReset::SourceGap);
    }
    if previous.dimensions != current.dimensions {
        result.insert(PixelChangeReset::DimensionsChanged);
    }
...
impl PixelChangeDetector {
    ...
    pub fn comparisons_remaining(&self) -> u64 {
        self.maximum_comparisons - self.used
    }
    ...
    pub fn push(
        ...
        frame: &RecordedFrame,
        cx: &ReplayCx,
    ) -> Result<PixelChangeObservation, PixelChangeError> {
        let mut check = || {
        ...
        if previous_basis.as_ref().is_some_and(|previous| {
            previous.import == current_basis.import && current_basis.segment <= previous.segment
        }) {
        ...
        let observation = PixelChangeObservation {
            ...
            capture: frame.receipt().capsule().capture,
...
fn compare_pixels(
    ...
    maximum: u64,
    used: &mut u64,
    check: &mut impl FnMut() -> Result<(), PixelChangeError>,
    ...
    if width == 0
        ...
        || height > 4096
        || count > 4_194_304
        || previous.len() as u64 != count
        || current.len() as u64 != count
    {
        return Err(PixelChangeError::InvalidImage);
    }
    if used.checked_add(count).is_none_or(|next| next > maximum) {
        return Err(PixelChangeError::BudgetExceeded);
    }
        ...
        for column in 0..width {
            ...
            match &mut bounds {
                Some(bounds) => {
                    ...
                    bounds.top = bounds.top.min(row);
                    bounds.right = bounds.right.max(column + 1);
                    bounds.bottom = bounds.bottom.max(row + 1);
                }
                None => {
                    bounds = Some(PixelChangeBounds {
                        left: column,
                        top: row,
                        right: column + 1,
                        bottom: row + 1,
...
mod tests {
    ...
    fn config() -> PixelChangeConfig {
        PixelChangeConfig {
            minimum_delta: 20,
            minimum_changed_pixels: 2,
            minimum_changed_fraction_ppm: 333_333,
        }
    }
    #[test]
    fn exact_threshold_counts_bounds_and_fraction() -> TestResult {
        let previous = [10; 6];
        let current = [10, 30, 10, 10, 10, 50];
        let mut used = 0;
        let measured = compare_pixels(
            &previous,
            &current,
            [3, 2],
            config(),
        ...
        assert_eq!(measured.changed_pixels, 2);
        assert_eq!(measured.absolute_difference_sum, 60);
        assert_eq!(measured.maximum_difference, 40);
        assert!(measured.candidate);
        assert_eq!(used, 6);
        assert_eq!(
            measured.changed_bounds,
            Some(PixelChangeBounds {
                left: 1,
                top: 0,
                right: 3,
                bottom: 2
        ...
        let stricter = PixelChangeConfig {
            minimum_changed_fraction_ppm: 333_334,
            ..config()
        };
        assert!(
            !compare_pixels(
    ...
    fn unchanged_images_have_measured_zero_not_a_reset() -> TestResult {
        ...
        assert_eq!(measured.changed_pixels, 0);
        assert_eq!(measured.changed_bounds, None);
        assert!(!measured.candidate);
        Ok(())
    }
    #[test]
    fn cumulative_budget_refuses_before_work_and_counts_cancelled_rows() {
        ...
        assert_eq!(
            ...
            Err(PixelChangeError::BudgetExceeded)
        );
        assert_eq!(used, 1);
        let mut rows = 0;
        let mut check = || {
    ...
    fn malformed_dimensions_and_degenerate_thresholds_fail() {
        for dimensions in [[0, 1], [1, 0], [4097, 1], [2, 2]] {
            assert_eq!(
                compare_pixels(&[0], &[0], dimensions, config(), 10, &mut 0, &mut || Ok(())),
                Err(PixelChangeError::InvalidImage)
            );
        }
        assert!(
            PixelChangeDetector::new(
                PixelChangeConfig {
                    minimum_delta: 0,
                    ..config()
                },
        ...
        assert!(
            PixelChangeDetector::new(
                PixelChangeConfig {
                    minimum_changed_fraction_ppm: 1_000_001,
                    ..config()
                },
                100
            )
    ...
    fn gaps_and_all_simultaneous_invalidations_are_retained() {
        ...
        current.gap = true;
        current.dimensions = [2, 3];
        current.decoder = [2; 32];
        current.sensor = "sensor:b".to_owned();
        current.import = ContentDigest::sha256(b"other import");
        assert_eq!(
            resets(Some(&previous), &current),
            BTreeSet::from([
                PixelChangeReset::RecordingChanged,
                PixelChangeReset::SourceChanged,
                PixelChangeReset::SourceGap,
                PixelChangeReset::DimensionsChanged,
                PixelChangeReset::InterpretationChanged,
            ])
        );
        assert_eq!(
            resets(None, &current),
            BTreeSet::from([PixelChangeReset::NoPredecessor, PixelChangeReset::SourceGap])
        );
    }
    #[test]
