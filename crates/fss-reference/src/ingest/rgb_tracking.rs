...
use fss_twin::image_tracking::{
    ImageDetection, ImageTracker, ImageTrackingError, ImageTrackingFrame, ImageTrackingPolicy,
    ImageTrackingReport, MAX_IMAGE_TRACKS, TrackingAvailability,
};
use fss_twin::image_zones::{
    ImageZoneBasis, ImageZoneError, ImageZoneMonitor, ImageZonePolicy, ImageZoneReport,
    ImageZoneSpec,
};
...
fn identity(e: CanonicalEncoder) -> Result<ContentDigest, RgbTrackingError> {
    Ok(ContentDigest::sha256(
        &e.finish_checked().map_err(|_| RgbTrackingError::Encoding)?,
    ))
}
...
impl RgbTrackingContract {
    ...
    pub fn new(
        head: &RgbDetectionContract,
        class_index: usize,
        selection_evidence: ContentDigest,
    ) -> Result<Self, RgbTrackingError> {
        let label = head
            .spec()
            .labels
            .get(class_index)
            .ok_or(RgbTrackingError::InvalidInput)?;
        if !valid_digest(selection_evidence) {
        ...
        e.digest(head.digest());
        e.digest(selection_evidence);
        e.u64(class_index as u64);
        Ok(Self {
            digest: identity(e)?,
            head: head.digest(),
            model: head.spec().model,
            class_index,
            label: label.clone(),
        })
    }
...
impl RgbFrameAdmission {
    ...
    pub fn availability(&self) -> TrackingAvailability {
        self.availability
    }
...
pub struct PreparedRgbTracking {
    ...
    proposals: Vec<ImageDetection>,
    decisions: Vec<RgbTrackingDecision>,
}
impl PreparedRgbTracking {
    ...
    pub fn detection_digest(&self) -> ContentDigest {
        self.detections
    }
    ...
    pub fn proposals(&self) -> &[ImageDetection] {
        &self.proposals
    }
    /// Every post-NMS class proposal, including explicit nonselected classes.
    pub fn decisions(&self) -> &[RgbTrackingDecision] {
        &self.decisions
    }
}
...
pub fn prepare_rgb_tracking(
    inference: &RgbInference,
    report: &RgbDetectionReport,
    contract: &RgbTrackingContract,
    admission: RgbFrameAdmission,
    budget: &mut WorkBudget<'_>,
) -> Result<PreparedRgbTracking, RgbTrackingError> {
    budget.charge(256)?;
    if report.inference_identity() != inference.identity()
        || report.source() != inference.source()
        || report.geometry() != inference.geometry()
        || admission.source != report.source()
        || report.contract_digest() != contract.head
        || inference.model_digest() != contract.model
    {
    ...
    let count = all
        .iter()
        .filter(|d| d.class_index() == contract.class_index)
        .count();
    if count > MAX_IMAGE_TRACKS {
        return Err(RgbTrackingError::Limit);
    }
    let mut proposals = reserve(count)?;
    let mut decisions = reserve(all.len())?;
    let g = report.geometry();
    let dimensions = [g.source_width as u32, g.source_height as u32];
    let mut e = CanonicalEncoder::new();
    e.text("fss.rgb-tracking-input.reference.v1");
    e.digest(contract.digest());
    e.digest(report.digest());
    e.digest(inference.identity());
    e.digest(admission.evidence);
    e.u8(admission.availability as u8);
    ...
    for d in all {
        ...
        let proposal = if d.class_index() == contract.class_index {
            ...
            let partial = d.clipped()
                || min.contains(&0)
                || max[0] == dimensions[0]
                || max[1] == dimensions[1];
            let id = (d.row() as u64)
                .checked_add(1)
                .ok_or(RgbTrackingError::Limit)?;
            let mut p = CanonicalEncoder::new();
            p.text("fss.rgb-tracking-proposal.reference.v1");
            p.digest(report.digest());
            p.digest(contract.digest());
            p.u64(id);
            for value in b {
                p.u32(value);
            }
            ...
            p.u32(d.score().to_bits());
            p.bool(partial);
            let proposal = ImageDetection {
                id,
                evidence: identity(p)?.bytes(),
                min,
                max,
                partial,
            };
        ...
        e.u64(d.row() as u64);
        e.u64(d.class_index() as u64);
        e.u32(d.score().to_bits());
        for value in d.bounds() {
            e.u32(value);
        }
        e.bool(d.clipped());
        e.bool(proposal.is_some());
        if let Some(p) = proposal {
            e.bytes(&p.evidence);
        }
        decisions.push(RgbTrackingDecision {
            detection: *d,
            proposal,
        });
    }
    let digest = identity(e)?;
    let s = report.source();
    let frame = ImageTrackingFrame {
        source: ForegroundSource {
            image: ImageIdentity {
                exposure: s.exposure,
                pixels: inference.decode_receipt().rgb_sha256,
                image_domain: s.image_domain,
                dimensions,
            },
            camera: s.camera,
            clock: s.clock,
        ...
        detector: contract.digest().bytes(),
        permission_mask: s.permission_mask,
        evidence: digest.bytes(),
        availability: admission.availability,
    };
    budget.charge(0)?;
    Ok(PreparedRgbTracking {
        digest,
        inference: inference.identity(),
        detections: report.digest(),
        admission,
        frame,
        proposals,
        decisions,
...
pub struct RgbZoneTracker {
    ...
    prepared: Option<PreparedRgbTracking>,
...
impl RgbZoneTracker {
    ...
    pub fn new(
        ...
        let tracker =
            ImageTracker::new(episode, tracking, budget).map_err(RgbTrackingError::Tracking)?;
        let monitor = ImageZoneMonitor::new(&tracker, basis, policy, zones, budget)
            .map_err(RgbTrackingError::Zones)?;
        Ok(Self {
            contract,
            tracker,
            monitor,
    ...
    pub fn contract(&self) -> &RgbTrackingContract {
        &self.contract
    }
    ...
    pub fn prepared(&self) -> Option<&PreparedRgbTracking> {
    ...
    pub fn observe(
        ...
        admission: RgbFrameAdmission,
        budget: &mut WorkBudget<'_>,
    ) -> Result<RgbZoneProgress, RgbTrackingError> {
        if self.pending {
            return Err(RgbTrackingError::PendingZones);
        }
        let prepared = prepare_rgb_tracking(inference, report, &self.contract, admission, budget)?;
        let s = prepared.frame.source;
        let basis = self.monitor.basis();
        if s.camera != basis.camera
            || s.clock != basis.clock
            || s.calibration != basis.calibration
            || s.image.image_domain != basis.image_domain
            || s.image.dimensions != basis.dimensions
        {
            return Err(RgbTrackingError::BindingMismatch);
        }
        let tracking = self
            .tracker
            .update(prepared.frame, &prepared.proposals, budget)
    ...
    pub fn resume(
        ...
        if !self.pending {
            let zones = self
                ...
                .latest()
                .ok_or(RgbTrackingError::NoObservation)?;
            return Ok(RgbZoneProgress::Complete {
                tracking: tracking.digest(),
                zones: zones.digest(),
            });
        }
        match self.monitor.observe(&self.tracker, tracking, budget) {
            Ok(zones) => {
                self.pending = false;
                Ok(RgbZoneProgress::Complete {
                    tracking: tracking.digest(),
                    zones: zones.digest(),
                })
            }
            Err(error) => Ok(RgbZoneProgress::Pending {
                tracking: tracking.digest(),
                error,
            }),
        }
