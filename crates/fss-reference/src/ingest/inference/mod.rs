...
use crate::{
    ExecBudget, ExecError, ReferenceDeployment, ReferenceError, ReplayCx, ScalarExecCx,
    ScalarExecutor,
};
use fss_core::{
    BatchId, CanonicalDecode, CanonicalDecoder, CanonicalEncode, CanonicalEncoder, ContentDigest,
    ContractError, EvidenceDelta, LedgerAnchor, ObjectId, Plane,
};
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            ...
            Self::Reference(_) | Self::Object(_) | Self::Publication(_) | Self::Spool(_) => {
                "recorded model storage refused"
            }
        })
    }
}
...
pub fn executor_profile_digest() -> ContentDigest {
    ...
    e.text("fss.recorded_executor_source_profile.v1");
...
fn run_identity(
    ...
    let mut e = CanonicalEncoder::new();
    e.text("fss.recorded_model_run_key.v1");
    e.digest(model);
    e.digest(frame);
    e.digest(runtime);
    ContentDigest::sha256(&e.finish())
...
impl Receipt {
    ...
    fn encoded(&self) -> Result<Vec<u8>> {
        ...
        e.bytes(b"FSSMRUN1");
        e.u32(1);
        e.text(RECORDED_INFERENCE_DOMAIN);
        e.digest(self.model);
        e.digest(self.frame);
        e.digest(self.frame_receipt);
        self.source_anchor.encode_canonical(&mut e);
        e.digest(self.runtime);
        e.digest(self.input);
        e.digest(self.output);
        e.u64(self.macs);
    ...
    fn decode(bytes: &[u8], expected: ContentDigest) -> Result<Self> {
        if bytes.len() > 4096 || ContentDigest::sha256(bytes) != expected {
        ...
        let value = Self {
            model: d.digest()?,
            frame: d.digest()?,
            frame_receipt: d.digest()?,
            source_anchor: LedgerAnchor::decode_canonical(&mut d)?,
            runtime: d.digest()?,
            input: d.digest()?,
            output: d.digest()?,
            macs: d.u64()?,
        ...
        d.ensure_finished()?;
        if value.runtime != executor_profile_digest()
            || value.allocated > MAX_RUN_TENSOR_BYTES as u64
            || value.nodes > fss_model_ir::MAX_MODEL_IR_ITEMS as u64
            || value.encoded()? != bytes
        {
            return Err(ModelRunError::Mismatch);
        }
    ...
    fn delta(&self, frame: &RecordedFrame, root: ContentDigest) -> Result<EvidenceDelta> {
        ...
        Ok(EvidenceDelta {
            delta_id: format!("delta:model-run:{id}"),
            family: "model_invocation_receipt".to_owned(),
            object_id: ObjectId::parse(format!("object:model-run:{id}"))?,
            prior_generation: None,
            new_generation: 1,
            validity: frame.receipt().capsule().capture,
            plane: Plane::Cognition,
            payload_digest: ContentDigest::sha256(&self.encoded()?),
...
fn tensor_bytes(
    ...
    generation: fss_core::Generation,
) -> Result<Vec<u8>> {
    if tensors.len() > 256 {
    ...
    for (name, tensor) in tensors {
        if tensor.dtype() != DType::F32 || tensor.generation() != generation {
            return Err(ModelRunError::Mismatch);
        }
        size = size
            .checked_add(name.len() + 48 + tensor.shape().rank() * 8)
            .and_then(|n| {
                tensor
                    .shape()
    ...
    for (name, tensor) in tensors {
        e.text(name);
        e.u8(DType::F32.type_tag());
        e.u64(tensor.shape().rank() as u64);
        ...
        for value in values {
            if !value.is_finite() {
                return Err(ModelRunError::NonFinite);
            }
            e.u32(value.to_bits());
        }
    }
...
fn decode_outputs(bytes: &[u8], model: &RecordedModel) -> Result<BTreeMap<String, Vec<f32>>> {
    if bytes.len() > MAX_RUN_OUTPUT_BYTES {
        return Err(ModelRunError::Limit);
    }
    let mut d = CanonicalDecoder::new(bytes);
    if d.bytes()? != b"FSSRTEN1"
        || d.u32()? != 1
        || d.u64()? != model.graph().generation().get()
    ...
    for _ in model.graph().outputs() {
        ...
        let port = model
            .graph()
            .find_output(&name)
            .ok_or(ModelRunError::Mismatch)?;
        if d.u64()? != port.rank() as u64 {
            return Err(ModelRunError::Mismatch);
        }
        for &dimension in port.shape().dims() {
            if d.u64()? != dimension as u64 {
                return Err(ModelRunError::Mismatch);
            }
        }
...
fn execute(
    ...
    model: &RecordedModel,
    budget: ExecBudget,
    exec_cx: &ScalarExecCx,
) -> Result<(Receipt, Vec<u8>, Vec<u8>)> {
    exec_cx.checkpoint("recorded-model:bind")?;
    let inputs = model.bind(frame, budget)?;
    let image = inputs
        .iter()
        .find(|(name, _)| name == model.frame_input())
        .ok_or(ModelRunError::Mismatch)?;
    let input = tensor_bytes(
        &BTreeMap::from([(image.0.clone(), image.1.clone())]),
    ...
    let receipt = Receipt {
        model: model.digest(),
        frame: frame.publication_root(),
        frame_receipt: frame.receipt().digest()?,
        source_anchor: frame.authority_anchor().clone(),
        runtime: executor_profile_digest(),
        input: ContentDigest::sha256(&input),
        output: ContentDigest::sha256(&output),
        macs: outcome.executed_macs(),
        allocated: outcome.allocated_bytes() as u64,
        nodes: outcome.nodes_executed() as u64,
    };
...
impl RecordedInference {
    ...
    pub fn run_and_publish(
        ...
        if existing_root.is_none() {
            deployment
                .publisher_mut()
                .stage_manifest(&target, &manifest)?;
        }
        ...
        let authority_anchor =
            deployment.append_batch(batch_id(receipt.identity())?, vec![delta], children, cx)?;
    ...
    pub fn open(
        ...
        let batch = deployment
            ...
            .ok_or(ModelRunError::Unavailable)?;
        ...
        let frame = RecordedFrame::open(deployment, source, cx)?;
        if frame.publication_root() != receipt.frame
            || frame.receipt().digest()? != receipt.frame_receipt
            || *frame.authority_anchor() != receipt.source_anchor
        {
            return Err(ModelRunError::Mismatch);
        }
        let manifest = receipt.manifest()?;
        if *delta != receipt.delta(&frame, manifest.root())? {
            return Err(ModelRunError::Mismatch);
        }
        let mut children = manifest.children().to_vec();
        children.push(manifest.root());
        ...
        if batch.children != children
            || deployment
                .publisher()
                .root(&slot(identity)?)
                .is_none_or(|r| r.root != manifest.root())
            || deployment.publisher().spool().read(manifest.root())? != manifest.canonical_bytes()
        {
            return Err(ModelRunError::Mismatch);
        }
        let model = RecordedModel::decode(
            &deployment.publisher().spool().read(receipt.model)?,
            receipt.model,
        )?;
        let input = deployment.publisher().spool().read(receipt.input)?;
        if input.len() > MAX_RUN_OUTPUT_BYTES || ContentDigest::sha256(&input) != receipt.input {
            return Err(ModelRunError::Mismatch);
        }
        ...
        if ContentDigest::sha256(&output_bytes) != receipt.output {
            return Err(ModelRunError::Mismatch);
        }
    ...
    pub fn verify_by_replay(
        ...
        if restored.receipt != self.receipt || restored.output_bytes != self.output_bytes {
            return Err(ModelRunError::Mismatch);
        }
        let frame = RecordedFrame::open(deployment, source, cx)?;
        let (receipt, _, output) = execute(&frame, &restored.model, budget, exec_cx)?;
        if receipt != self.receipt || output != self.output_bytes {
            return Err(ModelRunError::Mismatch);
        }
    ...
    #[must_use]
    pub fn outputs(&self) -> &BTreeMap<String, Vec<f32>> {
        &self.outputs
    }
    ...
    #[must_use]
    pub fn allocated_tensor_bytes(&self) -> u64 {
        self.receipt.allocated
    }
}
