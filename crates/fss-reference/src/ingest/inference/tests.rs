...
impl Directory {
    fn new(name: &str) -> TestResult<Self> {
        for attempt in 0..100_u32 {
            let path = std::env::temp_dir().join(format!("fss-inference-{name}-{}-{attempt}", std::process::id()));
...
fn context(root: &Path) -> TestResult<ReplayCx> {
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:model-test".to_owned(), operation_id: OperationId::parse("operation:model-test")?,
        principal: "principal:model-test".to_owned(), capabilities: vec!["ADP-REPLAY-001".to_owned()],
        ...
        anchor_universe: ContentDigest::sha256(b"site:model-test"), generation: 1,
    })?;
    authority.validate()?;
    Ok(ReplayCx::from_context_authority(&authority, root.to_path_buf())?)
}
fn fixture(name: &str) -> TestResult<(Directory, ReplayCx, ReferenceDeployment, RecordedDecodeRequest, RecordedFrame)> {
    ...
    let imported = FileIngestAdapter::ingest(FileIngestRequest::new(path,
        SensorId::parse("sensor:model-test")?, StreamId::parse("stream:model-test")?)
...
fn graph(width: usize, height: usize, op: OpCode) -> TestResult<ModelIrGraph> {
    ...
    Ok(ModelIrGraph::new_validated("model:numeric-fixture", ModelIrVersion::V1, Generation(3),
        vec![TensorPort::new("image", DType::F32, shape.clone(), Generation(3))?,
            TensorPort::new("parameter", DType::F32, Shape::new(vec![1])?, Generation(3))?],
        vec![TensorPort::new("result", DType::F32, shape, Generation(3))?],
        vec![GraphNode::new("node:0", op, "numeric fixture, not trained detector",
            vec!["image".to_owned(), "parameter".to_owned()], vec!["result".to_owned()], AttributeMap::new())?])?)
}
fn model(frame: &RecordedFrame, value: f32, scale: bool) -> TestResult<RecordedModel> {
    let [w, h] = frame.receipt().dimensions();
    Ok(RecordedModel::publish(&graph(w as usize, h as usize, OpCode::Add)?, "image", scale,
        BTreeMap::from([("parameter".to_owned(), vec![value])]))?)
}
fn budget() -> ExecBudget { ExecBudget::new(100_000_000, 64 * 1024 * 1024) }

#[test]
fn model_roundtrip_pins_weights_and_preprocessing() -> TestResult {
    let g = graph(8, 8, OpCode::Add)?;
    let a = RecordedModel::publish(&g, "image", true, BTreeMap::from([("parameter".to_owned(), vec![0.25])]))?;
    let b = RecordedModel::publish(&g, "image", true, BTreeMap::from([("parameter".to_owned(), vec![0.5])]))?;
    let c = RecordedModel::publish(&g, "image", false, BTreeMap::from([("parameter".to_owned(), vec![0.25])]))?;
    assert_ne!(a.digest(), b.digest()); assert_ne!(a.digest(), c.digest());
    let decoded = RecordedModel::decode(a.encoded(), a.digest())?;
    assert_eq!(decoded.encoded(), a.encoded()); assert_eq!(decoded.graph().generation(), Generation(3));
...
fn missing_extra_nonfinite_and_wrong_count_parameters_are_refused() -> TestResult {
    let g = graph(8, 8, OpCode::Add)?;
    for parameters in [BTreeMap::new(), BTreeMap::from([("other".to_owned(), vec![1.0])]),
...
fn model_parser_rejects_every_prefix_and_appended_bytes() -> TestResult {
    let g = graph(8, 8, OpCode::Add)?;
    let m = RecordedModel::publish(&g, "image", true, BTreeMap::from([("parameter".to_owned(), vec![1.0])]))?;
    for n in 0..m.encoded().len() {
        let bytes = &m.encoded()[..n];
        assert!(RecordedModel::decode(bytes, ContentDigest::sha256(bytes)).is_err());
    }
    let mut bytes = m.encoded().to_vec(); bytes.push(0);
    assert!(RecordedModel::decode(&bytes, ContentDigest::sha256(&bytes)).is_err());
    Ok(())
}
...
fn real_numeric_execution_is_retained_and_reopens_without_source_or_model_file() -> TestResult {
    ...
    let run = RecordedInference::run_and_publish(&mut deployment, &request, &m, budget(), &ScalarExecCx::new(), &cx)?;
    let actual = run.outputs().get("result").ok_or(ModelRunError::Mismatch)?;
    let expected: Vec<_> = frame.pixels().iter().map(|&p| f32::from(p) * (1.0_f32 / 255.0) + 0.25).collect();
    ...
    let identity = run.identity(); let final_anchor = deployment.current_anchor().clone();
    fs::remove_file(dir.0.join("source.mjpeg"))?;
    drop(deployment); drop(m);
    ...
    assert!(reopened.ledger().batches().iter().flat_map(|b| &b.deltas)
        .filter(|d| d.family == "model_invocation_receipt").all(|d| d.plane == Plane::Cognition));
    Ok(())
}
...
fn exact_successful_retries_do_not_duplicate_authority() -> TestResult {
    ...
    let first = RecordedInference::run_and_publish(&mut deployment, &request, &m, budget(), &ScalarExecCx::new(), &cx)?;
    let anchor = deployment.current_anchor().clone();
    let second = RecordedInference::run_and_publish(&mut deployment, &request, &m, budget(), &ScalarExecCx::new(), &cx)?;
    assert_eq!(first.identity(), second.identity()); assert_eq!(first.receipt_bytes()?, second.receipt_bytes()?);
...
fn exhausted_budgets_and_dimension_mismatch_publish_no_model_results() -> TestResult {
    ...
    for limits in [ExecBudget::new(0, 64 * 1024 * 1024), ExecBudget::new(1_000_000, 1)] {
        assert!(RecordedInference::run_and_publish(&mut deployment, &request, &m, limits, &ScalarExecCx::new(), &cx).is_err());
    }
    let wrong = RecordedModel::publish(&graph(1, 1, OpCode::Add)?, "image", true,
        BTreeMap::from([("parameter".to_owned(), vec![1.0])]))?;
    assert!(RecordedInference::run_and_publish(&mut deployment, &request, &wrong, budget(), &ScalarExecCx::new(), &cx).is_err());
...
fn nonfinite_results_and_executor_cancellation_are_not_published() -> TestResult {
    ...
    let [w, h] = frame.receipt().dimensions();
    let m = RecordedModel::publish(&graph(w as usize, h as usize, OpCode::Div)?, "image", false,
        BTreeMap::from([("parameter".to_owned(), vec![0.0])]))?;
    let before = deployment.current_anchor().clone();
    assert!(RecordedInference::run_and_publish(&mut deployment, &request, &m, budget(), &ScalarExecCx::new(), &cx).is_err());
    let exec = ScalarExecCx::new(); exec.request_cancellation();
    assert!(RecordedInference::run_and_publish(&mut deployment, &request, &model(&frame, 1.0, true)?, budget(), &exec, &cx).is_err());
    assert_eq!(*deployment.current_anchor(), before);
    Ok(())
}
...
fn interrupted_final_publication_is_not_complete_and_exact_retry_resumes() -> TestResult {
    ...
    assert!(matches!(RecordedInference::open(&deployment, receipt.identity(), &request, &fresh), Err(ModelRunError::Unavailable)));
    let finished = RecordedInference::run_and_publish(&mut deployment, &request, &m, budget(), &ScalarExecCx::new(), &fresh)?;
    assert_eq!(finished.identity(), receipt.identity());
    finished.verify_by_replay(&deployment, &request, budget(), &ScalarExecCx::new(), &fresh)?;
    Ok(())
}
...
fn run_identity_cannot_rebind_identical_pixels_to_another_capsule() -> TestResult {
    ...
    let m = model(&frame, 1.0, true)?;
    let first = RecordedInference::run_and_publish(&mut deployment, &request, &m, budget(), &ScalarExecCx::new(), &cx)?;
    let mut other = request.clone(); other.segment_index = 1;
    RecordedFrame::decode_and_publish(&mut deployment, &other, &mut DecodeBudget::new(100_000_000), &cx)?;
    assert!(RecordedInference::open(&deployment, first.identity(), &other, &cx).is_err());
    let second = RecordedInference::run_and_publish(&mut deployment, &other, &m, budget(), &ScalarExecCx::new(), &cx)?;
    assert_ne!(first.identity(), second.identity()); assert_eq!(first.output_bytes(), second.output_bytes());
    Ok(())
}
...
fn receipt_truncation_and_wrong_tensor_shape_fail_closed() -> TestResult {
    ...
    let mut suffix = output; suffix.push(0); assert!(decode_outputs(&suffix, &m).is_err());
    Ok(())
}
