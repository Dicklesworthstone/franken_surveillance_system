...
use super::{MAX_RUN_TENSOR_BYTES, ModelRunError, Result};
...
impl RecordedModel {
    ...
    pub fn publish(
        ...
        let mut size = graph_bytes
            .len()
            .checked_add(128 + frame_input.len())
        ...
        if size > MAX_RECORDED_MODEL_BYTES || parameters.len() > 256 {
            return Err(ModelRunError::Limit);
        }
        let mut e = CanonicalEncoder::new();
        e.bytes(b"FSSRMDL1");
        e.u32(1);
        e.text(RECORDED_MODEL_DOMAIN);
        e.digest(ContentDigest::sha256(&graph_bytes));
        e.bytes(&graph_bytes);
        e.text(frame_input);
        e.bool(scale_to_unit);
    ...
    pub fn decode(bytes: &[u8], expected: ContentDigest) -> Result<Self> {
        if bytes.len() > MAX_RECORDED_MODEL_BYTES {
            return Err(ModelRunError::Limit);
        }
        ...
        if graph.inputs().len() > 257
            || graph.outputs().len() > 256
            || graph
                .inputs()
                .iter()
                .chain(graph.outputs())
                .any(|p| p.dtype() != DType::F32)
        {
            return Err(ModelRunError::UnsupportedInput);
        }
        let port = graph
            .find_input(&frame_input)
            .ok_or(ModelRunError::UnsupportedInput)?;
    ...
    pub(super) fn bind(
        ...
        let input_bytes = self.graph.inputs().iter().try_fold(0_usize, |sum, port| {
            sum.checked_add(port.shape().size_bytes(DType::F32)?)
                .ok_or(ModelRunError::Limit)
        })?;
        if input_bytes > budget.max_bytes {
            return Err(ModelRunError::Limit);
        }
        let port = self
            .graph
            .find_input(&self.frame_input)
            .ok_or(ModelRunError::UnsupportedInput)?;
        let dims = port.shape().dims();
        let [width, height] = frame.receipt().dimensions();
        if dims[2] != height as usize || dims[3] != width as usize {
            return Err(ModelRunError::UnsupportedInput);
        }
        let program = PreprocessProgram::new(
            height as usize,
            width as usize,
            ChannelTransform::LumaOnly,
            self.scale_to_unit,
        );
        let image = program.execute_bytes(
            frame.pixels(),
            height as usize,
            width as usize,
            1,
