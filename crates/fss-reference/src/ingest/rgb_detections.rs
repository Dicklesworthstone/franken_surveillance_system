...
#[derive(Debug)]
pub struct RgbDetectionContract { spec: RgbDetectionSpec, digest: ContentDigest }
impl RgbDetectionContract {
    ...
    pub fn new(spec: RgbDetectionSpec) -> Result<Self, RgbDetectionError> {
        ...
        if spec.model.algorithm() != DigestAlgorithm::Sha256 || spec.model.bytes() == [0; 32]
            || !name_ok(&spec.output_port) || spec.labels.is_empty() || spec.labels.len() > MAX_CLASSES
        ...
        e.u8(match spec.boxes { HeadBoxes::PixelCorners => 0, HeadBoxes::PixelCenterSize => 1,
            HeadBoxes::NormalizedCorners => 2, HeadBoxes::NormalizedCenterSize => 3 });
        e.u8(score_tag(spec.class_score));
        e.u8(spec.objectness.map_or(0, |s| score_tag(s) + 1));
        e.u8(match spec.classes { HeadClasses::Best => 0, HeadClasses::MultiLabel => 1 });
        e.u32(spec.minimum_score_ppm); e.u32(spec.nms_iou_ppm); e.u32(BOX_SUBPIXELS);
        for n in [spec.maximum_rows, spec.maximum_candidates, spec.maximum_detections] { e.u64(n as u64); }
        Ok(Self { digest: ContentDigest::sha256(&e.finish_checked()?), spec })
    }
    /// Frozen model, labels, score arithmetic, geometry and bounds.
    pub fn spec(&self) -> &RgbDetectionSpec { &self.spec }
...
fn score_tag(score: HeadScore) -> u8 {
    match score { HeadScore::Probability => 0, HeadScore::Logit => 1 }
}
...
impl From<ContractError> for RgbDetectionError { fn from(e: ContractError) -> Self { Self::Contract(e) } }
...
#[derive(Debug)]
pub struct RgbDetectionBudget { remaining: u64, used: u64, maximum_scratch_bytes: usize }
impl RgbDetectionBudget {
    ...
    pub fn new(units: u64, maximum_scratch_bytes: usize) -> Self {
        Self { remaining: units, used: 0, maximum_scratch_bytes }
    }
    ...
    fn charge(&mut self, units: u64, cx: &ScalarExecCx) -> Result<(), RgbDetectionError> {
        cx.checkpoint("rgb-detections:work").map_err(|_| RgbDetectionError::Cancelled)?;
        if units > self.remaining { return Err(RgbDetectionError::BudgetExceeded); }
...
pub struct RgbDetection {
    row: usize, class_index: usize, score: f32, bounds: [u32; 4], clipped: bool,
}
impl RgbDetection {
    ...
    pub fn bounds(&self) -> [u32; 4] { self.bounds }
...
pub struct RgbDetectionReport {
    digest: ContentDigest, inference: ContentDigest, contract: ContentDigest,
    source: RgbSourceBinding, geometry: ResizeGeometry,
    rows: Vec<HeadRowDecision>, candidates: Vec<RgbCandidateDecision>, detections: Vec<RgbDetection>,
}
impl RgbDetectionReport {
    ...
    pub fn contract_digest(&self) -> ContentDigest { self.contract }
    ...
    pub fn rows(&self) -> &[HeadRowDecision] { &self.rows }
    /// All pre-NMS candidates and stable suppressor links.
    pub fn candidates(&self) -> &[RgbCandidateDecision] { &self.candidates }
    /// All retained class-aware NMS proposals, never top-k truncated.
    pub fn detections(&self) -> &[RgbDetection] { &self.detections }
}
...
pub fn project_rgb_detections(inference: &RgbInference, contract: &RgbDetectionContract,
    allowed: &[u8], budget: &mut RgbDetectionBudget, cx: &ScalarExecCx)
    -> Result<RgbDetectionReport, RgbDetectionError> {
    budget.charge(1, cx)?;
    if inference.model_digest() != contract.spec.model { return Err(RgbDetectionError::InvalidOutput); }
    let tensor = inference.outputs().get(&contract.spec.output_port).ok_or(RgbDetectionError::InvalidOutput)?;
    let g = inference.geometry();
    let parts = decode_head(tensor.shape(), tensor.values(), g, allowed,
        inference.source().permission_mask, contract, budget, cx)?;
    let mut hash = Sha256Hasher::new(); let mut e = CanonicalEncoder::new();
    e.text("fss.rgb-detection-result.reference.v1"); e.digest(contract.digest());
    e.digest(inference.identity()); e.digest(inference.output_digest());
    for n in [g.source_width, g.source_height, g.target_width, g.target_height,
        g.image_width, g.image_height, g.left, g.top] { e.u64(n as u64); }
    e.u64(parts.rows.len() as u64); e.u64(parts.candidates.len() as u64);
    budget.charge(512, cx)?; hash.update(&e.finish_checked()?);
    for row in &parts.rows {
        budget.charge(32, cx)?;
        let (tag, count) = match row.disposition { HeadRowDisposition::BelowThreshold => (0, 0),
            HeadRowDisposition::OutsideImage => (1, 0), HeadRowDisposition::PrivateFootprint => (2, 0),
            HeadRowDisposition::Candidates(n) => (3, n) };
        hash.update(&(row.row as u64).to_le_bytes()); hash.update(&[tag]);
        hash.update(&(count as u64).to_le_bytes());
    }
    for candidate in &parts.candidates {
        budget.charge(64, cx)?;
        let d = candidate.detection;
        hash.update(&(d.row as u64).to_le_bytes()); hash.update(&(d.class_index as u64).to_le_bytes());
        hash.update(&d.score.to_bits().to_le_bytes());
        for v in d.bounds { hash.update(&v.to_le_bytes()); }
        hash.update(&[u8::from(d.clipped)]);
        hash.update(&candidate.suppressed_by.map_or(u64::MAX, |i| i as u64).to_le_bytes());
    }
    let digest = ContentDigest::new(DigestAlgorithm::Sha256,
        hash.finalize().map_err(|_| RgbDetectionError::Limit)?);
    budget.charge(0, cx)?;
    Ok(RgbDetectionReport { digest, inference: inference.identity(), contract: contract.digest(),
        source: inference.source(), geometry: g, rows: parts.rows,
        candidates: parts.candidates, detections: parts.detections })
}

struct Parts { rows: Vec<HeadRowDecision>, candidates: Vec<RgbCandidateDecision>, detections: Vec<RgbDetection> }
#[allow(clippy::too_many_arguments)]
fn decode_head(shape: &[usize], values: &[f32], g: ResizeGeometry, allowed: &[u8],
    mask: [u8; 32], contract: &RgbDetectionContract, budget: &mut RgbDetectionBudget, cx: &ScalarExecCx)
    -> Result<Parts, RgbDetectionError> {
    budget.charge(1, cx)?;
    let s = &contract.spec;
    let fields = 4 + usize::from(s.objectness.is_some()) + s.labels.len();
    if shape.len() != 3 || shape[0] != 1 { return Err(RgbDetectionError::InvalidOutput); }
    let (rows, width) = match s.layout { HeadLayout::Rows => (shape[1], shape[2]), HeadLayout::Channels => (shape[2], shape[1]) };
    if width != fields { return Err(RgbDetectionError::InvalidOutput); }
    if rows > s.maximum_rows { return Err(RgbDetectionError::Limit); }
    if rows * fields != values.len() || !valid_geometry(g) { return Err(RgbDetectionError::InvalidOutput); }
    let pixels = g.source_width * g.source_height;
    if allowed.len() != pixels { return Err(RgbDetectionError::MaskMismatch); }
    let prefix_len = (g.source_width + 1) * (g.source_height + 1);
    let scratch = prefix_len * 4 + rows * std::mem::size_of::<HeadRowDecision>()
        + s.maximum_candidates * (std::mem::size_of::<RgbDetection>() + std::mem::size_of::<RgbCandidateDecision>())
        + s.maximum_detections * std::mem::size_of::<RgbDetection>();
    if scratch > budget.maximum_scratch_bytes { return Err(RgbDetectionError::Limit); }
    budget.charge(pixels as u64, cx)?;
    if ContentDigest::sha256(allowed).bytes() != mask { return Err(RgbDetectionError::MaskMismatch); }
    let prefix = permission_prefix(allowed, g.source_width, g.source_height, budget, cx)?;
    let mut decisions = reserve(rows)?; let mut proposed = reserve(s.maximum_candidates)?;
    for row in 0..rows {
        budget.charge(64 + fields as u64 * 32, cx)?;
        let at = |field: usize| values[match s.layout { HeadLayout::Rows => row * fields + field,
            HeadLayout::Channels => field * rows + row }];
        let raw = [at(0), at(1), at(2), at(3)];
        let bounds = model_box(raw, s.boxes, g)?;
        let objectness = match s.objectness { Some(rule) => probability(at(4), rule)?, None => 1.0 };
        let offset = 4 + usize::from(s.objectness.is_some());
        let mut scores = [0.0_f32; MAX_CLASSES]; let mut best = 0; let mut best_score = -1.0;
        for (class, slot) in scores.iter_mut().enumerate().take(s.labels.len()) {
            *slot = objectness * probability(at(offset + class), s.class_score)?;
            if *slot > best_score { best = class; best_score = *slot; }
        }
        let passes = |class: usize| (s.classes == HeadClasses::MultiLabel || class == best)
            && f64::from(scores[class]) * 1_000_000.0 >= f64::from(s.minimum_score_ppm);
        let selected = (0..s.labels.len()).filter(|&c| passes(c)).count();
        let disposition = if selected == 0 { HeadRowDisposition::BelowThreshold }
            else if let Some(mapped) = g.source_box(bounds) {
                let q = f64::from(BOX_SUBPIXELS);
                let b = [(mapped[0] * q).floor() as u32, (mapped[1] * q).floor() as u32,
                    (mapped[2] * q).ceil() as u32, (mapped[3] * q).ceil() as u32];
                if !fully_allowed(&prefix, g.source_width, b) { HeadRowDisposition::PrivateFootprint }
                else {
                    let clipped = bounds[0] < g.left as f64 || bounds[1] < g.top as f64
                        || bounds[2] > (g.left + g.image_width) as f64 || bounds[3] > (g.top + g.image_height) as f64;
                    if proposed.len() + selected > s.maximum_candidates { return Err(RgbDetectionError::Limit); }
                    for (class, score) in scores.iter().copied().enumerate().take(s.labels.len()) {
                        if passes(class) { proposed.push(RgbDetection { row, class_index: class,
                            score, bounds: b, clipped }); }
                    }
                    HeadRowDisposition::Candidates(selected)
                }
            } else { HeadRowDisposition::OutsideImage };
    ...
    proposed.sort_unstable_by(|a, b| b.score.total_cmp(&a.score).then(a.row.cmp(&b.row)).then(a.class_index.cmp(&b.class_index)));
    ...
    for detection in proposed {
        ...
        for (i, prior) in candidates.iter().enumerate() {
            budget.charge(1, cx)?;
            if prior.suppressed_by.is_none() && prior.detection.class_index == detection.class_index
                && suppresses(prior.detection.bounds, detection.bounds, s.nms_iou_ppm) {
                suppressed_by = Some(i); break;
            }
        }
        if suppressed_by.is_none() {
            if detections.len() == s.maximum_detections { return Err(RgbDetectionError::Limit); }
            detections.push(detection);
        }
        candidates.push(RgbCandidateDecision { detection, suppressed_by });
    }
    budget.charge(0, cx)?;
    Ok(Parts { rows: decisions, candidates, detections })
}
fn valid_geometry(g: ResizeGeometry) -> bool {
    [g.source_width, g.source_height, g.target_width, g.target_height].iter().all(|n| (1..=4096).contains(n))
        && g.source_width * g.source_height <= 4_194_304
        && g.image_width > 0 && g.image_height > 0
        && g.left.checked_add(g.image_width).is_some_and(|n| n <= g.target_width)
        && g.top.checked_add(g.image_height).is_some_and(|n| n <= g.target_height)
}
fn probability(v: f32, rule: HeadScore) -> Result<f32, RgbDetectionError> {
    if !v.is_finite() { return Err(RgbDetectionError::InvalidOutput); }
    let score = match rule { HeadScore::Probability => v, HeadScore::Logit => deterministic_sigmoid_f32(v) };
    if !(0.0..=1.0).contains(&score) { return Err(RgbDetectionError::InvalidOutput); }
    Ok(if score == 0.0 { 0.0 } else { score })
}
fn model_box(raw: [f32; 4], encoding: HeadBoxes, g: ResizeGeometry) -> Result<[f64; 4], RgbDetectionError> {
    if raw.iter().any(|v| !v.is_finite()) { return Err(RgbDetectionError::InvalidOutput); }
    let mut b = raw.map(f64::from);
    if matches!(encoding, HeadBoxes::NormalizedCorners | HeadBoxes::NormalizedCenterSize) {
        b[0] *= g.target_width as f64; b[2] *= g.target_width as f64;
        b[1] *= g.target_height as f64; b[3] *= g.target_height as f64;
    }
    if matches!(encoding, HeadBoxes::PixelCenterSize | HeadBoxes::NormalizedCenterSize) {
        if b[2] <= 0.0 || b[3] <= 0.0 { return Err(RgbDetectionError::InvalidOutput); }
        b = [b[0] - b[2] * 0.5, b[1] - b[3] * 0.5, b[0] + b[2] * 0.5, b[1] + b[3] * 0.5];
    }
    if b[0] >= b[2] || b[1] >= b[3] { return Err(RgbDetectionError::InvalidOutput); }
    Ok(b)
}
fn reserve<T>(n: usize) -> Result<Vec<T>, RgbDetectionError> {
    let mut v = Vec::new(); v.try_reserve_exact(n).map_err(|_| RgbDetectionError::Limit)?; Ok(v)
}
fn permission_prefix(allowed: &[u8], w: usize, h: usize, budget: &mut RgbDetectionBudget, cx: &ScalarExecCx)
    -> Result<Vec<u32>, RgbDetectionError> {
    let stride = w + 1; let count = stride * (h + 1);
    budget.charge(count as u64, cx)?;
    let mut prefix = reserve(count)?; prefix.resize(count, 0_u32);
    for y in 0..h {
        budget.charge(w as u64 * 4, cx)?;
        let mut sum = 0;
        for x in 0..w {
            let v = allowed[y * w + x];
            if v > 1 { return Err(RgbDetectionError::MaskMismatch); }
            sum += u32::from(v); prefix[(y + 1) * stride + x + 1] = prefix[y * stride + x + 1] + sum;
        }
    }
    Ok(prefix)
}
fn fully_allowed(prefix: &[u32], w: usize, b: [u32; 4]) -> bool {
    let stride = w + 1;
    let [x0, y0, x1, y1] = [b[0] / BOX_SUBPIXELS, b[1] / BOX_SUBPIXELS,
        b[2].div_ceil(BOX_SUBPIXELS), b[3].div_ceil(BOX_SUBPIXELS)].map(|v| v as usize);
    let count = prefix[y1 * stride + x1] + prefix[y0 * stride + x0]
        - prefix[y0 * stride + x1] - prefix[y1 * stride + x0];
