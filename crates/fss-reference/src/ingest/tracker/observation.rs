...
pub const TRACK_ASSIGNMENT_POLICY: &str = "fss.track_assignment.v1:original-input-index:actual-global-assignment:new-track-included:no-prediction-observation";
...
mod tests {
    ...
    fn complete(result: &AssignedTrackerOutput, count: usize) {
        ...
        assert!(
            result
                .assignments
                .windows(2)
                .all(|a| a[0].track_id < a[1].track_id)
        );
        for track in &result.output.tracks {
            assert_eq!(
                result.observed_detection(track.id).is_some(),
                track.misses == 0
            );
        }
    ...
    fn global_assignment_is_not_replaced_by_nearest_filtered_box() -> Result {
        ...
        for track in &result.output.tracks {
            let overlaps: Vec<_> = detections
                .iter()
                .map(|d| {
                    iou(
                        track.cx - track.box_w / 2.0,
                        track.cy - track.box_h / 2.0,
                        track.box_w,
                        track.box_h,
                        d.box_x,
                        d.box_y,
    ...
    fn identical_boxes_keep_distinct_original_rows() -> Result {
        ...
        for _ in 0..4 {
            let result = tracker
                .try_step_assigned(&[detection(0.0), detection(0.0)], TrackerLimits::default())?;
            complete(&result, 2);
    ...
    fn canonical_sorting_does_not_renumber_the_callers_input() -> Result {
        let mut tracker = MultiObjectTracker::new(config())?;
        let initial = tracker.try_step_assigned(
            &[detection(100.0), detection(0.0)],
            TrackerLimits::default(),
        )?;
        assert_eq!(initial.observed_detection(1), Some(1));
        assert_eq!(initial.observed_detection(2), Some(0));
        let next = tracker.try_step_assigned(
            &[detection(0.0), detection(100.0)],
            TrackerLimits::default(),
        )?;
    ...
    fn deletion_reordering_and_lost_tracks_do_not_acquire_an_observation() -> Result {
        ...
        cfg.min_hits = 1;
        let mut tracker = MultiObjectTracker::new(cfg)?;
        tracker.try_step_assigned(
            &[detection(0.0), detection(100.0), detection(200.0)],
            TrackerLimits::default(),
        )?;
        for _ in 0..3 {
            let result =
                tracker.try_step_assigned(&[detection(100.0)], TrackerLimits::default())?;
            complete(&result, 1);
    ...
    fn failed_admission_returns_no_witness_and_does_not_spend_ids() -> Result {
        ...
        assert!(matches!(
            tracker.try_step_assigned(&[detection(0.0), detection(100.0)], limits),
            Err(TrackerStepError::Limit)
        ));
        assert!(matches!(
            tracker.try_step_assigned(&[detection(f64::NAN)], TrackerLimits::default()),
            Err(TrackerStepError::Detection)
        ));
    ...
    fn adding_witnesses_preserves_legacy_motion_state_over_reordered_and_missing_frames() -> Result
    {
        for seed in 0..32_u64 {
            ...
            for frame in 0..64 {
                ...
                let mut detections: Vec<_> = (0..count)
                    .map(|i| detection(i as f64 * 30.0 + f64::from(frame % 11)))
                    .collect();
                if state & 0x100 != 0 {
                    detections.reverse();
                }
