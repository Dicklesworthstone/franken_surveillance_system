...
pub struct HttpCameraRoute {
    basis: StreamBasis, peer: SocketAddr, authority: String, target: String,
...
impl HttpCameraRoute {
    ...
    pub fn new(basis: StreamBasis, peer: SocketAddr, authority: &str, target: &str,
        security: HttpCameraSecurity) -> Result<Self, HttpCameraError> {
        if basis.source == [0; 32] || basis.generation == 0 || peer.port() == 0
            ...
            || !target.starts_with('/') || target.starts_with("//") || target.len() > 2048
            || !target.bytes().all(|b| b.is_ascii_alphanumeric() || b"/._~-".contains(&b)) {
            return Err(HttpCameraError::Configuration);
        }
        Ok(Self { basis, peer, authority: authority.to_owned(), target: target.to_owned(), security })
    }
    ...
    pub fn security(&self) -> HttpCameraSecurity { self.security }
}
impl fmt::Debug for HttpCameraRoute {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("HttpCameraRoute").field("basis", &self.basis)
...
pub trait HttpCameraAuthority {
    ...
    fn checkpoint(&self, route: &HttpCameraRoute, operation: HttpCameraOperation,
        now_ns: u64, deadline_ns: u64) -> Result<(), HttpCameraDenial>;
}
...
impl Default for HttpCameraLimits {
    fn default() -> Self {
        Self { http: HttpLimits::default(), multipart: MultipartLimits::default(),
            read_bytes: 16384, io_calls: 1_000_000, frames: 100_000,
            source_runs: 65536, connect_timeout_ns: 5_000_000_000 }
    }
}
impl HttpCameraLimits {
    fn validate(self, basis: StreamBasis) -> Result<HttpResponseStream, HttpCameraError> {
        if !(1..=65536).contains(&self.read_bytes) || self.io_calls == 0
...
pub enum HttpCameraError {
    ...
    Io { /// Boundary producing the error.
        operation: HttpCameraOperation, /// OS error class only.
        kind: io::ErrorKind },
}
...
pub struct HttpWireRead { receipt: HttpWireReceipt, bytes: Vec<u8>, acknowledged: bool, parsed: usize }
impl HttpWireRead {
    /// Exact unchanged bytes, including possibly sensitive response headers.
    pub fn bytes(&self) -> &[u8] { &self.bytes }
    /// Exact original range/hash/owner-admission time.
    pub fn receipt(&self) -> HttpWireReceipt { self.receipt }
    /// Whether the caller accepted custody responsibility, not proof of durable storage.
    pub fn acknowledged(&self) -> bool { self.acknowledged }
    /// Prefix already supplied to HTTP framing, including any failed call's prefix.
    pub fn parsed_bytes(&self) -> usize { self.parsed }
}
impl fmt::Debug for HttpWireRead {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("HttpWireRead").field("receipt", &self.receipt)
            .field("acknowledged", &self.acknowledged).field("parsed", &self.parsed).finish()
    }
}
...
pub struct HttpCamera {
    route: HttpCameraRoute, limits: HttpCameraLimits, deadline: u64, clock: u64,
    socket: Option<Box<dyn CameraSocket>>, request: Vec<u8>, sent: usize,
    http: HttpResponseStream, multipart: Option<HttpMultipartStream>,
    head: Option<ResponseHead>, wire: Option<HttpWireRead>, entity: Option<(EntityData, usize)>,
    frame: Option<HttpJpegFrame>, end: Option<HttpEnd>, complete: Option<HttpMjpegEnd>,
    totals: HttpCameraTotals, failure: Option<HttpCameraError>, interruptions: u8,
}
impl fmt::Debug for HttpCamera {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("HttpCamera").field("route", &self.route).field("totals", &self.totals)
...
impl HttpCamera {
    ...
    pub fn connect(route: HttpCameraRoute, limits: HttpCameraLimits, now_ns: u64,
        deadline_ns: u64, authority: &dyn HttpCameraAuthority) -> Result<Self, HttpCameraConnectFailure> {
        let mut attempted = false;
        let result = (|| {
            let http = limits.validate(route.basis)?;
            if now_ns >= deadline_ns { return Err(HttpCameraError::Configuration); }
            let request = format!("GET {} HTTP/1.1\r\nHost: {}\r\nAccept: multipart/x-mixed-replace\r\nAccept-Encoding: identity\r\nConnection: close\r\n\r\n", route.target, route.authority).into_bytes();
            authority.checkpoint(&route, HttpCameraOperation::Connect, now_ns, deadline_ns)
                .map_err(HttpCameraError::Denied)?;
            attempted = true;
            let socket = TcpStream::connect_timeout(&route.peer,
                Duration::from_nanos(limits.connect_timeout_ns.min(deadline_ns - now_ns)))
                .map_err(|e| io_error(HttpCameraOperation::Connect, e))?;
            authority.checkpoint(&route, HttpCameraOperation::Configure, now_ns, deadline_ns)
                .map_err(HttpCameraError::Denied)?;
            if socket.peer_addr().map_err(|e| io_error(HttpCameraOperation::Configure, e))? != route.peer {
                return Err(HttpCameraError::PeerMismatch);
            }
            socket.set_nonblocking(true).map_err(|e| io_error(HttpCameraOperation::Configure, e))?;
            authority.checkpoint(&route, HttpCameraOperation::Configure, now_ns, deadline_ns)
                .map_err(HttpCameraError::Denied)?;
            Ok(Self { route, limits, deadline: deadline_ns, clock: now_ns,
                socket: Some(Box::new(socket)), request, sent: 0, http, multipart: None,
                head: None, wire: None, entity: None, frame: None, end: None, complete: None,
                totals: HttpCameraTotals::default(), failure: None, interruptions: 0 })
        })();
        result.map_err(|reason| HttpCameraConnectFailure { reason, attempted })
    }
    /// Fixed route and source generation.
    pub fn route(&self) -> &HttpCameraRoute { &self.route }
    /// Local syscall/byte/part accounting, including unsuccessful progress.
    pub fn totals(&self) -> HttpCameraTotals { self.totals }
    /// First terminal error. Later polling never retries this connection.
    pub fn failure(&self) -> Option<HttpCameraError> { self.failure }
    /// Original read to retain before acknowledging. Available even after fatal refusal.
    pub fn pending_wire(&self) -> Option<&HttpWireRead> { self.wire.as_ref() }
    ...
    pub fn acknowledge_wire(&mut self, receipt: HttpWireReceipt, now_ns: u64,
        authority: &dyn HttpCameraAuthority) -> Result<(), HttpCameraError> {
        ...
        if let Some(wire) = &mut self.wire { wire.acknowledged = true; }
        Ok(())
    }
    ...
    pub fn take_frame(&mut self, ordinal: u64, encoded_sha256: [u8; 32], now_ns: u64,
        authority: &dyn HttpCameraAuthority) -> Result<HttpJpegFrame, HttpCameraError> {
        if self.frame.as_ref().is_none_or(|f| {
            let r = f.part().receipt(); r.ordinal != ordinal || r.encoded_sha256 != encoded_sha256
        }) { return Err(HttpCameraError::ReceiptMismatch); }
    ...
    pub fn step(&mut self, now_ns: u64, authority: &dyn HttpCameraAuthority,
        budget: &mut DecodeBudget<'_>) -> Result<HttpCameraStep, HttpCameraError> {
        self.admit(HttpCameraOperation::Poll, now_ns, authority)?;
        let result = self.advance(now_ns, authority, budget);
        if let Err(error) = result { self.fence(error); }
        result
    }
    fn advance(&mut self, now: u64, auth: &dyn HttpCameraAuthority, budget: &mut DecodeBudget<'_>)
        -> Result<HttpCameraStep, HttpCameraError> {
        if self.frame.is_some() { return Ok(HttpCameraStep::FrameReady); }
        if self.complete.is_some() { return Ok(HttpCameraStep::Complete); }
        if self.totals.frames == self.limits.frames { return Err(HttpCameraError::FrameLimit); }
        if self.sent < self.request.len() { return self.write_request(now, auth); }
        if let Some(wire) = &self.wire && !wire.acknowledged {
        ...
        if self.entity.is_some() {
            ...
            let parser = self.multipart.as_mut().ok_or(HttpCameraError::Configuration)?;
            let step = parser.push(data, *cursor, budget);
            match step {
                Err(e) => { *cursor += e.consumed; return Err(HttpCameraError::Multipart(e.error)); }
                Ok(s) => {
                    *cursor += s.consumed;
                    if let Some(frame) = s.frame { self.frame = Some(frame); self.totals.frames += 1; }
                    if *cursor == data.bytes().len() { self.entity = None; }
                }
            }
            return Ok(if self.frame.is_some() { HttpCameraStep::FrameReady } else { HttpCameraStep::Advanced });
        }
        if self.wire.as_ref().is_some_and(|w| w.parsed < w.bytes.len()) {
            self.admit(HttpCameraOperation::Parse, now, auth)?;
            if self.http.body_complete() { return Err(HttpCameraError::TrailingResponse); }
            let wire = self.wire.as_mut().ok_or(HttpCameraError::Configuration)?;
            let step = self.http.push(wire.receipt.range[0] + wire.parsed as u64,
                &wire.bytes[wire.parsed..], budget);
            let event = match step {
                Err(e) => { wire.parsed += e.consumed; return Err(HttpCameraError::Http(e.error)); }
                Ok(s) => { wire.parsed += s.consumed; s.event }
            };
            match event {
                Some(HttpEvent::Head(head)) => {
                    self.head = Some(head);
                    self.multipart = Some(HttpMultipartStream::new(
                        self.head.as_ref().ok_or(HttpCameraError::Configuration)?, self.limits.multipart,
                        self.limits.source_runs, budget).map_err(HttpCameraError::Multipart)?);
                }
        ...
        if self.http.body_complete() || self.totals.peer_eof {
            ...
            let mut complete = self.multipart.as_mut().ok_or(HttpCameraError::Configuration)?
                .finish(end, budget).map_err(HttpCameraError::Multipart)?;
            self.frame = complete.final_frame.take();
            if self.frame.is_some() { self.totals.frames += 1; }
            self.complete = Some(complete); self.socket = None;
            return Ok(if self.frame.is_some() { HttpCameraStep::FrameReady } else { HttpCameraStep::Complete });
        }
        self.read_source(now, auth)
    }
    fn admit(&mut self, op: HttpCameraOperation, now: u64, auth: &dyn HttpCameraAuthority)
        -> Result<(), HttpCameraError> {
        if let Some(error) = self.failure { return Err(error); }
        if now < self.clock { return Err(HttpCameraError::ClockReversed); }
        let result = if now >= self.deadline { Err(HttpCameraError::Deadline) }
            else { auth.checkpoint(&self.route, op, now, self.deadline).map_err(HttpCameraError::Denied) };
        if let Err(e) = result { self.fence(e); return Err(e); }
        self.clock = now; Ok(())
    }
    ...
    fn write_request(&mut self, now: u64, auth: &dyn HttpCameraAuthority) -> Result<HttpCameraStep, HttpCameraError> {
        self.io_room()?; self.admit(HttpCameraOperation::Write, now, auth)?;
        self.totals.write_calls += 1;
        let end = self.request.len().min(self.sent + self.limits.read_bytes);
        let result = self.socket.as_mut().ok_or(HttpCameraError::Configuration)?.write(&self.request[self.sent..end]);
        if let Ok(n) = result { self.sent += n; self.totals.sent_bytes += n as u64; }
        self.admit(HttpCameraOperation::Write, now, auth)?;
        match result {
            Ok(0) => Err(HttpCameraError::WriteZero),
            Ok(_) => { self.interruptions = 0; Ok(HttpCameraStep::Advanced) }
            Err(e) => self.nonblocking(HttpCameraOperation::Write, e),
        }
    }
    fn read_source(&mut self, now: u64, auth: &dyn HttpCameraAuthority) -> Result<HttpCameraStep, HttpCameraError> {
        self.io_room()?;
        let remaining = self.limits.http.wire_bytes.saturating_sub(self.totals.received_bytes);
        if remaining == 0 { return Err(HttpCameraError::NetworkLimit); }
        let n = remaining.min(self.limits.read_bytes as u64) as usize;
        let mut bytes = Vec::new(); bytes.try_reserve_exact(n).map_err(|_| HttpCameraError::Allocation)?;
        bytes.resize(n, 0);
        self.admit(HttpCameraOperation::Read, now, auth)?; self.totals.read_calls += 1;
        let result = self.socket.as_mut().ok_or(HttpCameraError::Configuration)?.read(&mut bytes);
        if let Ok(n) = result {
            if n == 0 { self.totals.peer_eof = true; self.socket = None; }
            else {
                bytes.truncate(n);
                let start = self.totals.received_bytes; self.totals.received_bytes += n as u64;
                let receipt = HttpWireReceipt { basis: self.route.basis, range: [start, self.totals.received_bytes],
                    sha256: ContentDigest::sha256(&bytes).bytes(), admitted_ns: now };
                // Preserve actual source BEFORE post-read cancellation/revocation is consulted.
                self.wire = Some(HttpWireRead { receipt, bytes, acknowledged: false, parsed: 0 });
            }
        }
        self.admit(HttpCameraOperation::Read, now, auth)?;
        match result {
            Ok(0) => { self.interruptions = 0; Ok(HttpCameraStep::Advanced) }
            Ok(_) => { self.interruptions = 0; Ok(HttpCameraStep::WireReady(
                self.wire.as_ref().ok_or(HttpCameraError::Configuration)?.receipt)) }
            Err(e) => self.nonblocking(HttpCameraOperation::Read, e),
        }
    }
    fn nonblocking(&mut self, op: HttpCameraOperation, e: io::Error) -> Result<HttpCameraStep, HttpCameraError> {
        match e.kind() {
            io::ErrorKind::WouldBlock => { self.interruptions = 0; Ok(HttpCameraStep::Pending) }
            io::ErrorKind::Interrupted => {
                self.interruptions += 1;
                if self.interruptions >= 8 { Err(HttpCameraError::Interrupted) } else { Ok(HttpCameraStep::Pending) }
            }
            _ => Err(io_error(op, e)),
        }
    }
    ...
    pub fn retire(mut self) -> HttpCameraRetirement {
        self.socket = None;
        HttpCameraRetirement { route: self.route, totals: self.totals, reason: self.failure,
            request: self.request, request_sent: self.sent, head: self.head, wire: self.wire,
            entity: self.entity, frame: self.frame, http: self.http.abort(),
            multipart: self.multipart.map(HttpMultipartStream::abort), end: self.end, complete: self.complete }
    }
}
...
impl fmt::Debug for HttpCameraRetirement {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("HttpCameraRetirement").field("route", &self.route)
            .field("totals", &self.totals).field("reason", &self.reason)
            .field("request_sent", &self.request_sent).finish_non_exhaustive()
    }
}
fn io_error(operation: HttpCameraOperation, error: io::Error) -> HttpCameraError {
    HttpCameraError::Io { operation, kind: error.kind() }
}
