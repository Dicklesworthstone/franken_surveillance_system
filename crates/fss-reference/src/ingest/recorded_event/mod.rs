...
use fss_core::{
    ...
    CaptureInterval, ContentDigest, ContractError, DecisionPath, EventEvidence, EventHypothesis,
    EventId, EventKind, EventState, EvidenceClass, EvidenceEdgeRelation, LedgerAnchor,
    ObjectId, Plane, ProbabilityInterval,
};
...
const POLICY: &[u8] = b"fss.recorded_event_policy.v1:uncalibrated:unclassified:indeterminate:hold:full-prefix-only";
...
impl fmt::Display for RecordedEventError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            ...
            Self::Conflict => "recorded event changed or analysis history is not an exact extension",
            ...
            Self::Reference(_) | Self::Object(_) | Self::Publication(_) | Self::Spool(_) => "recorded event storage refused",
        })
    }
}
impl Error for RecordedEventError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            Self::Analysis(e) => Some(e), Self::Contract(e) => Some(e), Self::Event(e) => Some(e),
            Self::Reference(e) => Some(e), Self::Object(e) => Some(e),
            Self::Publication(e) => Some(e), Self::Spool(e) => Some(e), _ => None,
        }
    }
}
...
fn checkpoint(cx: &ReplayCx, stage: &'static str) -> Result<()> {
    cx.checkpoint(stage).map_err(|_| RecordedEventError::Cancelled)
}
...
fn proof_slot(track: ContentDigest, report: ContentDigest) -> Result<SlotName> {
    let mut e = CanonicalEncoder::new(); e.text(PROOF_DOMAIN); e.digest(track); e.digest(report);
    SlotName::parse(&format!("re-{}", hex(ContentDigest::sha256(&e.finish_checked()?))))
...
fn read_verified(deployment: &ReferenceDeployment, digest: ContentDigest) -> Result<Vec<u8>> {
    ...
    if ContentDigest::sha256(&bytes) != digest { return Err(RecordedEventError::Mismatch); }
    Ok(bytes)
}
...
fn insert(objects: &mut BTreeMap<ContentDigest, Vec<u8>>, bytes: Vec<u8>) -> ContentDigest {
    let digest = ContentDigest::sha256(&bytes); objects.insert(digest, bytes); digest
}

fn provenance(deployment: &ReferenceDeployment, report: &AnalysisReport, track: ContentDigest, cx: &ReplayCx) -> Result<Provenance> {
    if report.observations().len() > MAX_RECORDED_EVENT_FRAMES { return Err(RecordedEventError::Limit); }
    ...
    for entry in report.observations() {
        ...
        if batch.deltas.len() != 1 { return Err(RecordedEventError::Mismatch); }
        ...
        children.insert(delta.payload_digest); receipts.insert(delta.payload_digest);
        ...
        let sensor_digest = insert(&mut objects, update.capsule.sensor_id.as_str().as_bytes().to_vec());
        ...
        if selected.is_some() || retired {
            ...
            interval = Some(match interval {
                Some(old) => CaptureInterval::new(old.earliest.min(window.earliest), old.latest.max(window.latest))?,
                None => window,
            });
            evidence.push(EventEvidence {
                digest: update_digest, class: EvidenceClass::Derived,
                failure_domain: format!("recorded-sensor:{}", hex(sensor_digest)),
                supports: false, relation: EvidenceEdgeRelation::DerivedFrom,
                capsule_digest: Some(capsule_digest), identity_digest: Some(sensor_digest),
            });
        }
    }
    if !observed { return Err(RecordedEventError::TrackUnavailable); }
    let interval = interval.ok_or(RecordedEventError::TrackUnavailable)?;
    let mut e = CanonicalEncoder::new();
    e.bytes(b"FSSREVT1"); e.u32(1); e.text(PROOF_DOMAIN);
    e.digest(policy); e.digest(track); e.digest(report.digest());
    let metadata = insert(&mut objects, e.finish_checked()?);
    children.extend(objects.keys().copied()); children.remove(&metadata);
...
fn event_manifest(event: &EventHypothesis) -> Result<ObjectManifest> {
    Ok(ObjectManifest::new("event-revision", event.model_receipts.iter().copied(),
...
fn load_event(deployment: &ReferenceDeployment, id: &EventId, cx: &ReplayCx) -> Result<Option<(EventHypothesis, ContentDigest, LedgerAnchor)>> {
    let object = ObjectId::parse(format!("object:event:{}", id.as_str()))?;
    let Some(current) = deployment.ledger().current().objects.get(&object) else { return Ok(None); };
    let mut chain = Vec::new();
    let mut latest = None;
    for batch in deployment.ledger().batches() {
        for delta in &batch.deltas {
            if delta.family != "event_revision" || delta.object_id != object { continue; }
            checkpoint(cx, "recorded_event:lineage")?;
            if chain.len() >= fss_core::event::MAX_LINEAGE_DEPTH { return Err(RecordedEventError::Limit); }
            let root_bytes = read_verified(deployment, delta.payload_digest)?;
            let manifest = ObjectManifest::from_canonical_bytes(&root_bytes)?;
            let payload = manifest.metadata_digest().ok_or(RecordedEventError::Mismatch)?;
            let bytes = read_verified(deployment, payload)?;
            let event = EventHypothesis::from_canonical_bytes(&bytes)?;
            event.validate()?;
            if event.event_id != *id || event.revision != delta.new_generation
                || delta.prior_generation != event.revision.checked_sub(1).filter(|n| *n != 0)
                || delta.plane != Plane::Authority || delta.validity != event.interval
            ...
            { return Err(RecordedEventError::Mismatch); }
            if delta.new_generation == current.generation && delta.payload_digest == current.payload_digest {
                latest = Some((event.clone(), delta.payload_digest, batch.new_anchor.clone()));
            }
            chain.push(event);
        }
    }
    ...
    if chain.last() != Some(&latest.0) { return Err(RecordedEventError::Mismatch); }
    Ok(Some(latest))
}
...
impl RecordedEvent {
    ...
    pub fn open(deployment: &ReferenceDeployment, id: &EventId, limits: &AnalysisLimits,
        budget: &mut AnalysisBudget, cx: &ReplayCx) -> Result<Self> {
        checkpoint(cx, "recorded_event:open")?;
        let (event, root, anchor) = load_event(deployment, id, cx)?.ok_or(RecordedEventError::Unavailable)?;
        ...
        let metadata = proof.metadata_digest().ok_or(RecordedEventError::Mismatch)?;
        let bytes = read_verified(deployment, metadata)?;
        if bytes.len() > 1024 { return Err(RecordedEventError::Limit); }
        let mut d = CanonicalDecoder::new(&bytes);
        if d.bytes()? != b"FSSREVT1" || d.u32()? != 1 || d.text()? != PROOF_DOMAIN
            || d.digest()? != ContentDigest::sha256(POLICY)
        { return Err(RecordedEventError::Mismatch); }
        let track = d.digest()?; let report_digest = d.digest()?; d.ensure_finished()?;
        let report_bytes = read_verified(deployment, report_digest)?;
        let report = AnalysisReport::verify(deployment, &report_bytes, report_digest,
            &bounded_limits(limits), budget, cx)?;
        let mut rebuilt = provenance(deployment, &report, track, cx)?;
        rebuilt.event.revision = event.revision; rebuilt.event.supersedes = event.supersedes;
        if rebuilt.event != event || rebuilt.manifest.canonical_bytes() != root_bytes
            || deployment.publisher().root(&rebuilt.slot).is_none_or(|r| r.root != proof.root())
        { return Err(RecordedEventError::Mismatch); }
        for (digest, bytes) in &rebuilt.objects {
            checkpoint(cx, "recorded_event:verify")?;
            if read_verified(deployment, *digest)? != *bytes { return Err(RecordedEventError::Mismatch); }
        }
        Ok(Self { event, root, anchor, report, track })
    }
    ...
    pub fn report(&self) -> &AnalysisReport { &self.report }
    /// Local hypothesis identity; never a biometric or physical identity claim.
    pub fn track(&self) -> ContentDigest { self.track }
}
...
impl RecordedEventProposal {
    ...
    pub fn prepare(deployment: &ReferenceDeployment, report_bytes: &[u8], report_digest: ContentDigest,
        track: ContentDigest, limits: &AnalysisLimits, budget: &mut AnalysisBudget, cx: &ReplayCx) -> Result<Self> {
        ...
        if deployment.ledger().current().objects.contains_key(&object) {
            ...
            if prior.report.encoded() == report.encoded() {
                ...
                if !report.plan().frames().starts_with(prior.report.plan().frames())
                    || report.observations().len() <= prior.report.observations().len()
                { return Err(RecordedEventError::Conflict); }
                for (old, new) in prior.report.observations().iter().zip(report.observations()) {
                    if old.detection().encoded().map_err(AnalysisError::from)? != new.detection().encoded().map_err(AnalysisError::from)?
                        || old.tracking().encoded().map_err(AnalysisError::from)? != new.tracking().encoded().map_err(AnalysisError::from)?
                    { return Err(RecordedEventError::Conflict); }
                }
                proof.event.revision = prior.event.revision.checked_add(1).ok_or(RecordedEventError::Limit)?;
                proof.event.supersedes = Some(prior.event.revision_digest());
                if proof.event.revision as usize > fss_core::event::MAX_LINEAGE_DEPTH { return Err(RecordedEventError::Limit); }
            }
        }
        proof.event.validate()?;
        let mut e = CanonicalEncoder::new(); e.text("fss.recorded_event_proposal.v1");
        e.digest(proof.event.revision_digest()); e.digest(proof.manifest.root());
        let digest = ContentDigest::sha256(&e.finish_checked()?);
        Ok(Self { report, track, proof, digest })
    }
    ...
    pub fn publish(&self, deployment: &mut ReferenceDeployment, expected: ContentDigest,
        limits: &AnalysisLimits, budget: &mut AnalysisBudget, cx: &ReplayCx) -> Result<RecordedEvent> {
        checkpoint(cx, "recorded_event:revalidate")?;
        if expected != self.digest { return Err(RecordedEventError::StaleProposal); }
        let fresh = Self::prepare(deployment, self.report.encoded(), self.report.digest(), self.track, limits, budget, cx)?;
        if fresh.digest != expected { return Err(RecordedEventError::StaleProposal); }
        let existing_root = deployment.publisher().root(&fresh.proof.slot).map(|root| root.root);
        ...
        if existing_root.is_none() {
            deployment.publisher_mut().stage_manifest(&fresh.proof.slot, &fresh.proof.manifest)?;
        }
        deployment.publish_and_commit(&fresh.proof.slot, &fresh.proof.manifest, fresh.proof.event.interval, cx)?;
        checkpoint(cx, STAGE_RECORDED_EVENT_COMMIT)?;
        let receipt = deployment.publish_event(&ReferencePolicyDecision {
            event: fresh.proof.event.clone(), action: ReferencePolicyAction::Hold,
        }, cx)?;
        // Infallible only after the guarded event authority commit.
        cx.checkpoint_post_commit("recorded_event:complete");
        Ok(RecordedEvent { event: fresh.proof.event, root: receipt.event_root,
            anchor: receipt.authority_anchor, report: fresh.report, track: fresh.track })
    }
}
