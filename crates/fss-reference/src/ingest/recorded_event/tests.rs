...
const MODEL: &[u8] =
    include_bytes!("../../../../fss-cli/tests/fixtures/detector_rows_8x8.fssmodel");
struct Directory(PathBuf);
impl Directory {
    fn new(label: &str) -> TestResult<Self> {
        for attempt in 0..100 {
            let path = std::env::temp_dir().join(format!(
                "fss-recorded-event-{label}-{}-{attempt}",
                std::process::id()
            ));
...
fn context(root: &Path) -> TestResult<ReplayCx> {
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:event-test".into(),
        operation_id: OperationId::parse("operation:event-test")?,
        principal: "principal:event-test".into(),
        ...
        anchor_universe: ContentDigest::sha256(b"site:event-test"),
        generation: 1,
    })?;
    authority.validate()?;
...
fn budget() -> AnalysisBudget {
    AnalysisBudget::new(100_000_000, 100_000_000)
}
struct Fixture {
...
fn fixture(label: &str) -> TestResult<Fixture> {
    ...
    let imported = FileIngestAdapter::ingest(
        FileIngestRequest::new(
            source,
            SensorId::parse("sensor:event-test")?,
            StreamId::parse("stream:event-test")?,
        )
    ...
    for segment_index in 0..3 {
        ...
        let inference = RecordedInference::run_and_publish(
            &mut deployment,
            &request,
            &model,
        ...
        frames.push(AnalysisFrame {
            segment_index,
            run_identity: inference.identity(),
        });
    }
    ...
    let full_plan = AnalysisPlan::new(
        imported.import_identity,
        ComponentInterpretation::Grayscale,
        spec,
        tracking,
    ...
    let track = report.observations()[0]
        ...
        .tracks
        .iter()
        .find(|t| t.class_index == 0)
        .ok_or_else(|| std::io::Error::other("fixture track missing"))?
        .id;
    Ok(Fixture {
        deployment,
        cx,
...
fn prepare(f: &Fixture, full: bool) -> TestResult<RecordedEventProposal> {
    ...
    Ok(RecordedEventProposal::prepare(
        &f.deployment,
        report.encoded(),
        report.digest(),
        f.track,
...
fn preparation_is_read_only_and_never_upgrades_model_authority() -> TestResult {
    ...
    assert!(
        event
            .evidence
            .iter()
            .all(|e| !e.supports && e.class == EvidenceClass::Derived)
    );
    assert_eq!(
        event.interval,
...
fn publication_restarts_without_source_or_report_files_and_retries_idempotently() -> TestResult {
    ...
    let recovered = RecordedEvent::open(
        &reopened,
...
fn exact_history_extension_supersedes_without_dropping_evidence() -> TestResult {
    ...
    assert_eq!(
        next.event().supersedes,
        Some(first.event().revision_digest())
    );
    assert!(
        first
            .event()
            .evidence
            .iter()
            .all(|e| next.event().evidence.contains(e))
    );
...
fn wrong_approval_and_tampered_reports_publish_nothing() -> TestResult {
    ...
    assert!(matches!(
        proposal.publish(
            &mut f.deployment,
            ContentDigest::sha256(b"not approved"),
            &AnalysisLimits::default(),
            &mut budget(),
            &f.cx
        ),
        Err(RecordedEventError::StaleProposal)
    ));
    let mut bad = f.report.encoded().to_vec();
    bad.push(0);
    for digest in [f.report.digest(), ContentDigest::sha256(&bad)] {
        assert!(
            RecordedEventProposal::prepare(
                &f.deployment,
                &bad,
                digest,
                f.track,
...
fn unknown_tracks_and_exhausted_budgets_create_no_empty_absence_events() -> TestResult {
    ...
    assert!(matches!(
        RecordedEventProposal::prepare(
            ...
            f.report.encoded(),
            f.report.digest(),
            ContentDigest::sha256(b"unknown track"),
            &AnalysisLimits::default(),
            &mut budget(),
            &f.cx
        ),
...
fn cancelled_staging_changes_neither_event_authority_nor_object_count() -> TestResult {
    ...
    assert!(matches!(
        ...
        Err(RecordedEventError::Cancelled)
    ));
    assert_eq!(f.deployment.current_anchor(), &anchor);
...
fn an_independent_owner_decision_is_never_overwritten() -> TestResult {
    ...
    owned.revision = 2;
    owned.supersedes = Some(first.event().revision_digest());
...
fn event_provenance_retains_full_report_and_actual_model_receipts() -> TestResult {
    ...
    let graph =
        ObjectManifest::from_canonical_bytes(&f.deployment.publisher().spool().read(root)?)?;
    ...
    assert!(
        f.deployment
            ...
            .iter()
            .flat_map(|b| &b.deltas)
            .filter(|d| d.family == "event_revision")
            .all(|d| d.plane == Plane::Authority)
    );
    assert!(
        published
            .event()
            .evidence
            .iter()
            .all(|e| e.relation == EvidenceEdgeRelation::DerivedFrom)
    );
    Ok(())
}
