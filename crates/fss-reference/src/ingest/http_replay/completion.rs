...
use crate::ingest::http_archive::{
    HttpArchiveError, HttpWireArchive, HttpWirePin, MAX_HTTP_WIRE_READS,
};
...
impl Record {
    fn validate(self) -> Result<(), HttpCompletionError> {
        ...
        if ![
            self.wire.scope,
            self.wire.head,
            sha(e.head.wire.source),
            sha(e.head.entity.source),
            sha(e.head.header_sha256),
        ]
        .into_iter()
        .all(valid)
            || self.wire.reads == 0
            || self.wire.reads > MAX_HTTP_WIRE_READS as u64
            || self.wire.bytes == 0
            || self.wire.bytes > 256 * 1024 * 1024
            || e.wire_bytes != self.wire.bytes
            || e.entity_bytes > e.wire_bytes
            || e.head.wire.generation == 0
            || e.head.entity.generation == 0
        ...
        let framing_valid = match (e.head.framing, e.termination) {
            (BodyFraming::Length(n), HttpTermination::ExplicitFraming) => {
                n == e.entity_bytes && e.chunks == 0
            }
            (BodyFraming::Chunked, HttpTermination::ExplicitFraming) => true,
            (BodyFraming::UntilEof, HttpTermination::CloseDelimitedEof) => {
                self.peer_eof && e.chunks == 0
            }
            _ => false,
        };
        if !framing_valid {
    ...
    fn encode(self) -> Result<Vec<u8>, HttpCompletionError> {
        ...
        e.text(DOMAIN);
        e.digest(self.wire.scope);
        e.digest(self.wire.head);
        e.u64(self.wire.reads);
        e.u64(self.wire.bytes);
        for basis in [self.end.head.wire, self.end.head.entity] {
            e.digest(sha(basis.source));
            e.u64(basis.generation);
        }
        ...
        e.u64(tag);
        e.u64(length);
        e.u64(self.end.wire_bytes);
        e.u64(self.end.entity_bytes);
        e.u64(self.end.chunks);
        e.u64(match self.end.termination {
            HttpTermination::ExplicitFraming => 0,
            HttpTermination::CloseDelimitedEof => 1,
        });
        e.u64(self.frames);
        e.u64(u64::from(self.peer_eof));
        let bytes = e
            .finish_checked()
            .map_err(|_| HttpCompletionError::Metadata)?;
        if bytes.len() > METADATA_BYTES {
            return Err(HttpCompletionError::Limit);
        }
        Ok(bytes)
    }
    fn decode(bytes: &[u8]) -> Result<Self, HttpCompletionError> {
        let parse = || -> Result<Self, fss_core::ContractError> {
            ...
            let framing = match (d.u64()?, d.u64()?) {
                (0, n) => BodyFraming::Length(n),
                (1, 0) => BodyFraming::Chunked,
                (2, 0) => BodyFraming::UntilEof,
                _ => return Err(fss_core::ContractError::InvalidIdentifier),
            };
            let head = HttpHeadIdentity {
                wire: source,
                entity,
                header_sha256: header.bytes(),
                framing,
            };
            let end = HttpEnd {
                head,
                ...
                termination: match d.u64()? {
                    0 => HttpTermination::ExplicitFraming,
                    1 => HttpTermination::CloseDelimitedEof,
                    _ => return Err(fss_core::ContractError::InvalidIdentifier),
                },
            };
            let frames = d.u64()?;
            let peer_eof = match d.u64()? {
                0 => false,
                1 => true,
                _ => return Err(fss_core::ContractError::InvalidIdentifier),
            };
    ...
    ) -> Result<ObjectManifest, HttpCompletionError> {
        if archive.pin() != self.wire || archive.scope().stream != self.end.head.wire {
            return Err(HttpCompletionError::Mismatch);
        }
        if self.wire.reads as usize >= MAX_MANIFEST_CHILDREN {
            return Err(HttpCompletionError::Limit);
        }
        work.charge(4096 + self.wire.reads * 512)?;
        ObjectManifest::new(
            KIND,
            archive.reads().map(|(pin, _)| pin.head),
            Some(ContentDigest::sha256(&self.encode()?)),
        )
        .map_err(|_| HttpCompletionError::Metadata)
    }
}
...
#[derive(Debug)]
pub struct PreparedHttpCompletion {
    record: Record,
    manifest: ObjectManifest,
    pin: HttpCompletionPin,
}
impl PreparedHttpCompletion {
    ...
    pub fn from_camera(
        ...
        work.charge(1024)?;
        if camera.failure().is_some()
            || camera.pending_frame().is_some()
            || camera.pending_wire().is_some()
        {
        ...
        if complete.final_frame.is_some()
            || totals.received_bytes != archive.pin().bytes
        ...
        let record = Record {
            ...
            peer_eof: totals.peer_eof,
        };
        let manifest = record.manifest(archive, work)?;
        let pin = HttpCompletionPin {
            root: manifest.root(),
            wire: record.wire,
        };
        pin.slot()?;
    ...
    pub fn pin(&self) -> HttpCompletionPin {
        self.pin
    }
    ...
    pub fn publish(
        ...
        work: &mut WorkBudget<'_>,
    ) -> Result<LocalPublicationReceipt, HttpCompletionError> {
        probe(cancel)?;
        ...
        if self.record.manifest(archive, work)? != self.manifest {
            return Err(HttpCompletionError::Mismatch);
        }
        ...
        if publisher
            .tombstones()
            .any(|d| *d == self.pin.root || *d == metadata_digest)
        {
        ...
        let receipt = publisher
            .publish_cancellable(&self.pin.slot()?, &self.manifest, cancel)
            .map_err(|_| HttpCompletionError::Storage)?;
...
#[derive(Debug)]
pub struct VerifiedHttpCompletion {
    pin: HttpCompletionPin,
    record: Record,
}
impl VerifiedHttpCompletion {
    ...
    pub fn load(
        ...
        if publisher.is_poisoned()
            || publisher.is_broken_slot(&slot)
            || publisher.root(&slot).is_none_or(|r| {
                r.root != expected.root || r.state != LocalPublicationState::Durable
            })
        {
            return Err(HttpCompletionError::NotDurable);
        }
        ...
        let manifest = ObjectManifest::from_canonical_bytes(&raw)
            .map_err(|_| HttpCompletionError::Metadata)?;
        if manifest.root() != expected.root || manifest.kind() != KIND {
            return Err(HttpCompletionError::Mismatch);
        }
        let metadata = read(
            manifest
                .metadata_digest()
                .ok_or(HttpCompletionError::Metadata)?,
            METADATA_BYTES,
            work,
        )?;
...
impl HttpWireReplay<'_> {
    ...
    pub fn finish_completed(
        &mut self,
        witness: &VerifiedHttpCompletion,
        access: HttpReplayAccess<'_, '_>,
    ) -> Result<HttpReplayStep, HttpCompletionError> {
        probe(access.cancellation)?;
        if self.pin != witness.pin.wire {
            return Err(HttpCompletionError::Mismatch);
        }
        if let Some(error) = self.failure {
            return Err(HttpCompletionError::Replay(error));
        }
        if self.frame.is_some() {
            return Err(HttpCompletionError::NotReady);
        }
        let verified = VerifiedHttpCompletion::load(
            access.publisher,
            self.archive,
            witness.pin,
            access.cancellation,
        ...
        if record.frames > self.limits.frames {
            return Err(HttpCompletionError::Limit);
        }
        if let Some(complete) = &self.complete {
            if complete.http != record.end
                || complete.multipart.frames != record.frames
        ...
        if !self.exhausted
            || self.loaded != self.pin.bytes
            || self.http.next_offset() != self.pin.bytes
            || self.wire.is_some()
            || self.entity.is_some()
        {
            return Err(HttpCompletionError::NotReady);
        }
        if record.end.termination != HttpTermination::CloseDelimitedEof
            || !record.peer_eof
            || self
                .head
                .as_ref()
                .is_none_or(|h| h.identity() != record.end.head)
        {
            return Err(HttpCompletionError::Mismatch);
        }
        let result = (|| -> Result<HttpReplayStep, HttpReplayError> {
            ...
            if self.frames != record.frames
                || complete.multipart.frames != record.frames
                || complete.multipart.bytes != end.entity_bytes
                || complete.multipart.basis != end.head.entity
            {
                return Err(HttpReplayError::State);
            }
            self.complete = Some(complete);
            self.exhausted = false;
            Ok(if self.frame.is_some() {
...
fn valid(digest: ContentDigest) -> bool {
...
fn probe(cancel: &dyn PublishCancellation) -> Result<(), HttpCompletionError> {
    if cancel.cancel_requested(PublishCutPoint::AfterChildrenVerified) {
        Err(HttpCompletionError::Cancelled)
    } else {
        Ok(())
    }
}
