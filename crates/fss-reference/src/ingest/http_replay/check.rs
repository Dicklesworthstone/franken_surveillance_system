...
use super::{
    HttpReplayAccess, HttpReplayError, HttpReplayLimits, HttpReplayPosition, HttpReplayStep,
    HttpWireReplay,
};
...
impl HttpCheckSource {
    ...
    pub fn scope(self) -> Result<HttpWireScope, HttpCheckError> {
        if self.generation == 0
            || ![self.source, self.receive_clock, self.retention_evidence]
                .into_iter()
                .all(valid)
        {
        ...
        Ok(HttpWireScope {
            stream: StreamBasis {
                source: self.source.bytes(),
                generation: self.generation,
            },
            receive_clock: self.receive_clock.bytes(),
            retention_evidence: self.retention_evidence.bytes(),
        })
    }
}
...
impl HttpCheckLimits {
    ...
    pub fn validate(self) -> Result<(), HttpCheckError> {
        if !(1..=4096).contains(&self.maximum_reads)
            || !(1..=256 * 1024 * 1024).contains(&self.maximum_source_bytes)
            || !(1..=65536).contains(&self.maximum_scan_roots)
            || !(1024..=16 * 1024 * 1024).contains(&self.maximum_spool_object_bytes)
            || !(1..=4096).contains(&self.maximum_frames)
            || !(1..=1_000_000).contains(&self.maximum_steps)
            || !(1..=65536).contains(&self.read_bytes)
            || !(4..=16 * 1024 * 1024).contains(&self.maximum_frame_bytes)
            || !(1..=4096).contains(&self.maximum_dimension)
            || !(1..=4_194_304).contains(&self.maximum_pixels)
            || [self.source_work, self.framing_work, self.decode_work]
                .into_iter()
...
pub enum HttpCheckError {
    ...
    Decode {
        /// Original one-based MIME ordinal.
        ordinal: u64,
        /// Native codec error, without pixels or metadata.
        error: DecodeError,
    },
...
impl std::fmt::Display for HttpCheckError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "HTTP recording check refused: {self:?}")
    }
}
...
pub fn check_http_recording(
    ...
    let witness = request
        ...
        .map(|root| {
            VerifiedHttpCompletion::load(
                publisher,
                &archive,
                HttpCompletionPin { root, wire: pin },
                cancel,
                &mut work,
            )
            .map_err(HttpCheckError::Completion)
        })
        .transpose()?;
    let replay_limits = HttpReplayLimits {
        read_bytes: limits.read_bytes,
        frames: limits.maximum_frames as u64,
        multipart: fss_codec_mjpeg::multipart::MultipartLimits {
            frame_bytes: limits.maximum_frame_bytes,
            ..Default::default()
        },
        ..Default::default()
    };
    let mut replay =
        HttpWireReplay::new(&archive, pin, replay_limits).map_err(HttpCheckError::Replay)?;
    work.charge((limits.maximum_frames * std::mem::size_of::<HttpCheckedFrame>()) as u64)
        .map_err(HttpCheckError::Work)?;
    let mut frames = Vec::new();
    frames
        .try_reserve_exact(limits.maximum_frames)
        .map_err(|_| HttpCheckError::Limit)?;
    let decoder =
        (request.decode != HttpCheckDecode::None).then(|| sha(fss_codec_mjpeg::decoder_identity()));
    let mut initial = CanonicalEncoder::new();
    initial.text("fss.http_check_frames.v1");
    initial.digest(pin.scope);
    initial.digest(pin.head);
    initial.u64(pin.reads);
    initial.u64(pin.bytes);
    initial.u64(match request.decode {
        HttpCheckDecode::None => 0,
        HttpCheckDecode::Grayscale => 1,
        HttpCheckDecode::YCbCr => 2,
    });
            ...
            match step {
                HttpReplayStep::FrameReady => {
                    ...
                    let frame = replay
                        ...
                        .map_err(HttpCheckError::Replay)?;
                    let exposure =
                        sha(http_rgb_exposure(&frame, &mut work).map_err(HttpCheckError::Work)?);
                    probe(cancel)?;
                    let decoded = match request.decode {
                        ...
                        mode => Some(
                            frame
                                .decode(
                                    match mode {
                                        HttpCheckDecode::Grayscale => {
                                            ComponentInterpretation::Grayscale
                                        }
                                        _ => ComponentInterpretation::YCbCr,
                                    },
                                    DecodeLimits {
                                        maximum_bytes: limits.maximum_frame_bytes,
                    ...
                    let row = HttpCheckedFrame {
                        ordinal: receipt.ordinal,
                        encoded: sha(receipt.encoded_sha256),
                        exposure,
                        bytes: frame.part().bytes().len(),
                        source_runs: frame.source_spans().len(),
                        dimensions: decoded.as_ref().map(|image| image.dimensions()),
                        luma: decoded
                            .as_ref()
                            .map(|image| sha(image.receipt().luma_sha256)),
                    };
                    work.charge(1024).map_err(HttpCheckError::Work)?;
                    let mut e = CanonicalEncoder::new();
                    e.digest(chain);
                    e.u64(row.ordinal);
                    e.digest(row.encoded);
                    e.digest(row.exposure);
                    e.u64(row.bytes as u64);
                    e.u64(row.source_runs as u64);
                    if let (Some([w, h]), Some(luma)) = (row.dimensions, row.luma) {
                        e.u64(1);
                        e.u64(u64::from(w));
                        e.u64(u64::from(h));
                        e.digest(luma);
                    ...
                    chain = ContentDigest::sha256(
                        &e.finish_checked().map_err(|_| HttpCheckError::Limit)?,
                    );
                ...
                HttpReplayStep::Complete => {
                    ...
                    probe(cancel)?;
                    return Ok(HttpCheckStatus::Complete);
                }
    ...
    let (status, error) = match result {
        ...
        Err(error) => (HttpCheckStatus::Refused, Some(error)),
    };
    Ok(HttpCheckReport {
        pin,
        completion_root: request.completion,
        status,
        decode: request.decode,
        decoder,
        termination: if status == HttpCheckStatus::Complete {
            replay.completion().map(|c| c.http.termination)
        } else {
            None
        },
        frames,
