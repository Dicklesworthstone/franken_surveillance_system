...
fn config() -> ZoneEventConfig {
    ZoneEventConfig { policy_generation: ContentDigest::sha256(b"policy"),
        dedup_cooldown_ns: 1_000_000_000, min_probability: 0.4, max_dedup_entries: 64 }
}
...
fn zone(id: &str) -> ZoneSpec {
    ZoneSpec { zone_id: id.into(), bounds: (0.0, 0.0, 100.0, 100.0), kind: EventKind::UnknownPresence }
}
...
fn observe(g: &mut ZoneEventGenerator, t: &TrackedTarget, z: &str, ns: i128,
    digest: ContentDigest, upper: f64) -> Result<Option<EventHypothesis>, ZoneEventError> {
    g.observe(RuntimeGrant::ObserveEvent, t, z, "cam-a", TimestampNs(ns), digest, upper)
}
fn frame() -> ContentDigest { ContentDigest::sha256(b"frame") }
...
fn confirmed_track_in_registered_zone_generates_event() -> TestResult {
    let event = required(observe(&mut generator()?, &target(), "driveway", 10, frame(), 0.9)?)?;
...
fn capability_gate_denies_wrong_grant() -> TestResult {
    let mut g = generator()?;
    assert!(matches!(g.observe(RuntimeGrant::ObserveStatus, &target(), "driveway", "cam-a",
        TimestampNs(10), frame(), 0.9), Err(ZoneEventError::CapabilityDenied { .. })));
    assert_eq!(g.generated_count(), 0);
    Ok(())
}
#[test]
fn dedup_suppresses_repeat_within_cooldown() -> TestResult {
    let mut g = generator()?; let t = target();
...
fn distinct_zones_track_pairs_do_not_cross_dedup() -> TestResult {
    let mut g = generator()?; let mut porch = zone("porch"); porch.kind = EventKind::PerimeterBreach;
...
fn decision_fingerprint_is_deterministic_and_input_sensitive() -> TestResult {
    ...
    assert_eq!(first.decision_path.fingerprint, same.decision_path.fingerprint);
    let changed = required(observe(&mut a, &target(), "driveway", 2_000_000_010,
        ContentDigest::sha256(b"different"), 0.9)?)?;
    assert_ne!(first.decision_path.fingerprint, changed.decision_path.fingerprint);
    Ok(())
}
#[test]
fn probability_interval_is_bounded_and_ordered() -> TestResult {
    ...
    let b = required(observe(&mut g, &target(), "driveway", 2_000_000_010, frame(), 5.0)?)?;
...
fn zone_registration_validates_input() -> TestResult {
    let mut g = generator()?;
    for id in ["", "bad:colon"] { assert!(g.register_zone(zone(id)).is_err()); }
    let mut flat = zone("flat"); flat.bounds.2 = 0.0;
...
fn config_validates_bounds() {
    let mut c = config(); c.dedup_cooldown_ns = 0;
    assert!(ZoneEventGenerator::new(c).is_err());
    c = config(); c.max_dedup_entries = 0;
    assert!(ZoneEventGenerator::new(c).is_err());
    c = config(); c.max_dedup_entries = MAX_EVENT_TRACKS + 1;
    assert!(ZoneEventGenerator::new(c).is_err());
    c = config(); c.min_probability = 1.5;
    assert!(ZoneEventGenerator::new(c).is_err());
}
