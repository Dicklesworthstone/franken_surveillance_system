...
fn witness_advances_hypothesized_to_witnessed() -> TestResult {
    ...
    assert_eq!(l.current_revision(), 2);
    assert_eq!(l.len(), 2);
    assert!(l.current().supersedes.is_some());
    assert_eq!(l.current().evidence.len(), 2);
...
fn corroborate_advances_witnessed_to_corroborated() -> TestResult {
    ...
    g.corroborate(
        ...
        ContentDigest::sha256(b"camera-b"),
        0.95,
    )?;
    assert_eq!(l.current_state(), EventState::Corroborated);
    assert_eq!(l.current_revision(), 3);
    assert_eq!(l.current().evidence.len(), 3);
...
fn corroboration_refuses_same_failure_domain() -> TestResult {
    let (g, mut l) = witnessed()?;
    assert!(
        matches!(g.corroborate(RuntimeGrant::ObserveEvent, &mut l, &pair(), "cam-a",
