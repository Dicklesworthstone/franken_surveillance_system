...
fn generator() -> Result<ZoneEventGenerator, ZoneEventError> {
    let mut g = ZoneEventGenerator::new(ZoneEventConfig {
        ...
        dedup_cooldown_ns: 10,
        min_probability: 0.0,
        max_dedup_entries: MAX_EVENT_TRACKS,
    })?;
    g.register_zone(ZoneSpec {
        zone_id: "yard".into(),
...
fn input(target: &TrackedTarget) -> ZoneObservation<'_> {
    ZoneObservation {
        source_generation: "camera-a:stream-1",
        target,
        zone_id: "yard",
        failure_domain: "shared-power",
        capture: CaptureInterval::point(TimestampNs(100)),
        frame_digest: ContentDigest::sha256(b"pixels"),
        upper_probability: 1.0,
    }
}
fn run(
    g: &mut ZoneEventGenerator,
    input: ZoneObservation<'_>,
...
fn invalid_score_does_not_poison_retry_or_consume_sequence() -> TestResult {
    for upper in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
        let mut g = generator()?;
        let t = target();
        let mut request = input(&t);
        request.upper_probability = upper;
        assert!(matches!(
            run(&mut g, request),
            Err(ZoneEventError::InvalidConfig(_))
        ));
        assert_eq!(g.generated_count(), 0);
        assert!(g.last_emitted.is_empty());
...
fn overlong_failure_domain_does_not_suppress_corrected_retry() -> TestResult {
    let mut g = generator()?;
    let t = target();
    let domain = "x".repeat(MAX_FAILURE_DOMAIN_LEN + 1);
    let mut request = input(&t);
...
fn capture_uncertainty_controls_dedup_and_is_not_collapsed() -> TestResult {
    ...
    assert!(
        run(&mut g, request)?.is_none(),
        "earliest-to-latest gap is only five"
    );
...
fn regressed_and_overflowing_time_are_typed_and_atomic() -> TestResult {
    ...
    assert!(matches!(
        run(&mut g, request),
        Err(ZoneEventError::ClockReversed)
    ));
    assert_eq!(g.generated_count(), 1);
    let mut g = generator()?;
    g.last_emitted.insert(
        (
            request.source_generation.into(),
            request.zone_id.into(),
            t.id,
        ),
    ...
    assert!(matches!(
        run(&mut g, request),
        Err(ZoneEventError::TimeOrder)
    ));
    assert_eq!(g.generated_count(), 0);
    assert_eq!(g.last_emitted.len(), 1);
    Ok(())
}
#[test]
fn sequence_exhaustion_leaves_cooldowns_unchanged() -> TestResult {
    ...
    assert!(matches!(
        run(&mut g, input(&t)),
        Err(ZoneEventError::SequenceExhausted)
    ));
    assert_eq!(g.generated_count(), u64::MAX);
    assert!(g.last_emitted.is_empty());
    Ok(())
}
#[test]
fn nonfinite_zones_and_overflowing_extents_are_refused() -> TestResult {
    ...
    for bounds in [
        ...
        assert!(matches!(
            g.register_zone(zone),
            Err(ZoneEventError::InvalidZoneSpec(_))
        ));
    }
...
fn zone_capacity_is_exact_without_silent_replacement() -> TestResult {
    ...
    assert!(matches!(
        g.register_zone(ZoneSpec {
            zone_id: "overflow".into(),
            bounds: (0.0, 0.0, 1.0, 1.0),
            kind: EventKind::Unclassified
        }),
        Err(ZoneEventError::CapacityExceeded("zones"))
    ));
    assert_eq!(g.zones.len(), MAX_EVENT_ZONES);
...
fn track_capacity_refuses_new_key_but_allows_existing_key() -> TestResult {
    ...
    for id in 0..MAX_EVENT_TRACKS as u64 {
        g.last_emitted.insert(
            (request.source_generation.into(), request.zone_id.into(), id),
            request.capture,
        );
    }
    ...
    assert_eq!(g.generated_count(), 0);
    assert_eq!(g.last_emitted.len(), MAX_EVENT_TRACKS);
    t.id = 0;
    let mut next = input(&t);
...
fn denied_capability_precedes_other_refusals() -> TestResult {
    ...
    request.zone_id = "missing";
    request.upper_probability = f64::NAN;
    assert!(matches!(
        g.observe_interval(RuntimeGrant::ObserveStatus, request),
        Err(ZoneEventError::CapabilityDenied {
            required: "CAP-OBSERVE-EVENT-001"
        })
    ));
    assert_eq!(g.generated_count(), 0);
    assert!(g.last_emitted.is_empty());
...
fn expired_capacity_is_reclaimed_only_after_successful_admission() -> TestResult {
    let mut g = generator()?;
    g.config.max_dedup_entries = 1;
    let mut t = target();
    required(run(&mut g, input(&t))?)?;
    t.id = 2;
    let mut next = input(&t);
