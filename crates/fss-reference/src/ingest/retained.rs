...
fn invalid(detail: &str) -> FileIngestError {
    FileIngestError::CorruptSegment { detail: format!("retained import: {detail}") }
}

fn checkpoint(cx: &ReplayCx, stage: &'static str) -> Result<(), FileIngestError> {
    cx.checkpoint(stage).map_err(|_| FileIngestError::CancellationRequested { stage })
}
...
impl FileImportManifest {
    ...
    pub fn from_retained_bytes(
        bytes: &[u8], expected: ContentDigest, limits: RetainedReadLimits,
        ...
        for _ in 0..n { ordered_chunks.push(digest(&mut d)?); }
        ...
        for _ in 0..n {
            segment_spans.push(SegmentSpan {
                ...
                offset: d.u64()?, len: d.u64()?, segment_sha256: digest(&mut d)?,
                capsule_id: CapsuleId::parse(d.text()?)?, gap_before: d.bool()?,
            });
        }
        ...
        for _ in 0..n {
            omission_spans.push(FileOmissionSpan {
                offset: d.u64()?, len: d.u64()?, reason: d.text()?.to_owned(),
            });
        }
        let n = count(&mut d, 8)?;
        let mut capsule_ids = Vec::with_capacity(n);
        for _ in 0..n { capsule_ids.push(CapsuleId::parse(d.text()?)?); }
        ...
        for _ in 0..n { part_roots.push(digest(&mut d)?); }
        let capture_time_label = d.text()?.to_owned();
        d.ensure_finished()?;
        let manifest = Self {
            input_sha256, input_bytes, format, detector_evidence, chunk_bytes, ordered_chunks,
            segment_spans, omission_spans, capsule_ids, limits_digest, adapter_id,
            adapter_generation, part_roots, capture_time_label,
        };
    ...
    pub fn validate_retained(&self, limits: RetainedReadLimits) -> Result<(), FileIngestError> {
        if limits.max_source_bytes == 0 || limits.max_chunk_bytes == 0
            || limits.max_segment_bytes == 0 || limits.max_chunk_bytes > MAX_RETAINED_PAYLOAD_BYTES
            || limits.max_segment_bytes > MAX_RETAINED_PAYLOAD_BYTES
        {
            return Err(FileIngestError::InvalidLimits {
                detail: "retained read ceilings must be positive; chunk/segment ceiling is 64 MiB".to_owned(),
            });
        }
        if self.input_bytes == 0 || self.input_bytes > limits.max_source_bytes
        ...
        if self.input_sha256.algorithm() != DigestAlgorithm::Sha256
            ...
            || self.ordered_chunks.iter().any(|d| d.algorithm() != DigestAlgorithm::Sha256)
        { return Err(ContractError::UnsupportedDigestAlgorithm.into()); }
        let mut previous_end = 0;
        let mut ids = BTreeSet::new();
        for (index, segment) in self.segment_spans.iter().enumerate() {
            let end = segment.offset.checked_add(segment.len)
                .ok_or_else(|| invalid("segment range overflow"))?;
            if segment.segment_index != index || segment.len == 0 || end > self.input_bytes
            ...
            { return Err(invalid("inconsistent segment order, range, gap or capsule binding")); }
            previous_end = end;
        }
        for omission in &self.omission_spans {
            if omission.len == 0 || omission.reason.is_empty()
                || omission.offset.checked_add(omission.len).is_none_or(|end| end > self.input_bytes)
...
impl RetainedFileImport {
    ...
    pub fn open(
        deployment: &ReferenceDeployment, import_identity: ContentDigest,
        limits: RetainedReadLimits, cx: &ReplayCx,
    ) -> Result<Self, FileIngestError> {
        checkpoint(cx, STAGE_RETAINED_OPEN)?;
        if import_identity.algorithm() != DigestAlgorithm::Sha256 {
            return Err(ContractError::UnsupportedDigestAlgorithm.into());
        }
        let hex: String = import_identity.bytes().iter().map(|b| format!("{b:02x}")).collect();
        let batch_id = BatchId::parse(format!("batch:file-import:{hex}:manifest"))?;
        let slot = SlotName::parse(&format!("fi-{hex}")).map_err(|_| ContractError::InvalidIdentifier)?;
        let batch = deployment.ledger().batches().iter().find(|b| b.batch_id == batch_id)
            .ok_or_else(|| invalid("completed import authority is absent"))?;
        let object_id = format!("object:file-import-manifest:{hex}");
        let delta = batch.deltas.iter().find(|d| {
            d.family == "file_import_manifest" && d.object_id.as_str() == object_id
                && d.plane == Plane::Authority && d.prior_generation.is_none() && d.new_generation == 1
        }).ok_or_else(|| invalid("manifest authority binding is absent"))?;
        let import_root = delta.witness_digest.ok_or_else(|| invalid("root witness is absent"))?;
        let manifest_digest = delta.payload_digest;
        let import_object = format!("object:file-import:{hex}");
        if !batch.children.contains(&manifest_digest) || !batch.children.contains(&import_root)
            || !batch.deltas.iter().any(|d| {
                d.family == "file_import" && d.object_id.as_str() == import_object
                    && d.plane == Plane::Authority && d.prior_generation == Some(1)
                    && d.new_generation == 2 && d.payload_digest == manifest_digest
        ...
        if visible.root != import_root { return Err(ContractError::DigestMismatch.into()); }
        ...
        Ok(Self {
            import_identity, import_root, manifest_digest,
            authority_anchor: batch.new_anchor.clone(), manifest,
        })
    }
    ...
    #[must_use]
    pub fn import_root(&self) -> ContentDigest { self.import_root }
    /// Retained canonical manifest digest.
    #[must_use]
    pub fn manifest_digest(&self) -> ContentDigest { self.manifest_digest }
    ...
    #[must_use]
    pub fn manifest(&self) -> &FileImportManifest { &self.manifest }

    fn revalidate(
        &self, deployment: &ReferenceDeployment, limits: RetainedReadLimits, cx: &ReplayCx,
    ) -> Result<(), FileIngestError> {
        let current = Self::open(deployment, self.import_identity, limits, cx)?;
        if current != *self { return Err(ContractError::DigestMismatch.into()); }
        Ok(())
    }
    ...
    pub fn read_segment(
        &self, deployment: &ReferenceDeployment, index: usize,
        limits: RetainedReadLimits, cx: &ReplayCx,
...
fn verified_chunk(
    manifest: &FileImportManifest, index: usize,
    read: &mut impl FnMut(ContentDigest) -> Result<Vec<u8>, FileIngestError>,
) -> Result<Vec<u8>, FileIngestError> {
    let expected = *manifest.ordered_chunks.get(index).ok_or_else(|| invalid("chunk index"))?;
    let offset = (index as u64).checked_mul(manifest.chunk_bytes)
        .ok_or_else(|| invalid("chunk offset overflow"))?;
    let remaining = manifest.input_bytes.checked_sub(offset).ok_or_else(|| invalid("chunk range"))?;
...
fn assemble_segment(
    manifest: &FileImportManifest, index: usize, limits: RetainedReadLimits,
    mut read: impl FnMut(ContentDigest) -> Result<Vec<u8>, FileIngestError>,
) -> Result<Vec<u8>, FileIngestError> {
    manifest.validate_retained(limits)?;
    let span = manifest.segment_spans.get(index).ok_or(FileIngestError::SegmentIndexOutOfBounds {
        index, count: manifest.segment_spans.len(),
    })?;
    if span.len > limits.max_segment_bytes {
        return Err(FileIngestError::SpoolCapacityExceeded {
            limit: "retained_segment_bytes", required: span.len, available: limits.max_segment_bytes,
        });
    }
    let size = usize::try_from(span.len).map_err(|_| invalid("segment allocation overflow"))?;
    let mut bytes = Vec::new();
    bytes.try_reserve_exact(size).map_err(|_| invalid("segment allocation refused"))?;
    let end = span.offset.checked_add(span.len).ok_or_else(|| invalid("range overflow"))?;
    ...
    for position in first..=last {
        ...
        let chunk_start = position.checked_mul(manifest.chunk_bytes).ok_or_else(|| invalid("offset overflow"))?;
        let start = usize::try_from(span.offset.saturating_sub(chunk_start)).map_err(|_| invalid("slice offset"))?;
        let stop = usize::try_from((end - chunk_start).min(chunk.len() as u64)).map_err(|_| invalid("slice end"))?;
        let slice = chunk.get(start..stop).ok_or_else(|| invalid("source slice"))?;
...
fn verify_source_chunks(
    ...
    if actual != manifest.input_sha256 { return Err(ContractError::DigestMismatch.into()); }
    Ok(actual)
}
