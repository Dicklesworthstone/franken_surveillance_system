...
use super::detections::{
    BOX_SUBPIXELS, BoxEncoding, CoordinateSpace, DetectionBudget, DetectionContract,
...
fn checkpoint(cx: &ReplayCx) -> Result<(), AnalysisError> {
    cx.checkpoint("recorded_analysis:frame")
...
fn decode_detector(bytes: &[u8]) -> Result<DetectionSpec, AnalysisError> {
    ...
    if d.text()? != "fss.recorded_detection_contract.v1" {
    ...
    for _ in 0..n {
        labels.push(name(&mut d)?);
    }
    let encoding = match d.u8()? {
        0 => BoxEncoding::Xyxy,
        1 => BoxEncoding::CenterSize,
        _ => return Err(AnalysisError::InvalidPlan),
    };
    let coordinates = match d.u8()? {
        0 => CoordinateSpace::Pixels,
        1 => CoordinateSpace::Normalized,
        _ => return Err(AnalysisError::InvalidPlan),
    };
    if d.u32()? != BOX_SUBPIXELS {
        return Err(AnalysisError::InvalidPlan);
    }
    ...
    if DetectionContract::new(spec.clone())?.encoded() != bytes {
        return Err(AnalysisError::InvalidPlan);
    }
    Ok(spec)
}
...
impl AnalysisPlan {
    ...
    pub fn new(
        ...
        if import_identity.algorithm() != DigestAlgorithm::Sha256
            ...
            || frames
                .iter()
                .any(|f| f.run_identity.algorithm() != DigestAlgorithm::Sha256)
            || frames
                .windows(2)
                .any(|w| w[0].segment_index >= w[1].segment_index)
        {
            return Err(AnalysisError::InvalidPlan);
        }
        tracking.digest()?;
        ...
        e.u32(1);
        e.text(PLAN_DOMAIN);
        e.digest(import_identity);
        e.u8(match interpretation {
            ComponentInterpretation::Grayscale => 0,
            ComponentInterpretation::YCbCr => 1,
        });
        e.bytes(detector.encoded());
        e.u32(tracking.minimum_iou_ppm);
        e.u32(tracking.confirmation_hits);
        e.u32(tracking.maximum_missed_frames);
        e.u64(tracking.maximum_tracks as u64);
        e.u64(frames.len() as u64);
        for frame in &frames {
            e.u64(frame.segment_index as u64);
            e.digest(frame.run_identity);
        }
        let bytes = e.finish_checked()?;
        if bytes.len() > MAX_ANALYSIS_PLAN_BYTES {
            return Err(AnalysisError::Limit);
        }
        Ok(Self {
            import_identity,
    ...
    pub fn decode(bytes: &[u8], expected: ContentDigest) -> Result<Self, AnalysisError> {
        ...
        if ContentDigest::sha256(bytes) != expected {
            return Err(AnalysisError::Mismatch);
        }
        let mut d = CanonicalDecoder::new(bytes);
        ...
        for _ in 0..n {
            frames.push(AnalysisFrame {
                segment_index: count(&mut d, usize::MAX)?,
                run_identity: d.digest()?,
            });
        }
        d.ensure_finished()?;
    ...
    #[must_use]
    pub fn detector(&self) -> &DetectionContract {
        &self.detector
    }
}
...
impl Default for AnalysisLimits {
    fn default() -> Self {
        Self {
            maximum_frames: MAX_ANALYSIS_FRAMES,
...
impl AnalysisLimits {
    fn validate(&self) -> Result<(), AnalysisError> {
        if self.maximum_frames == 0
            || self.maximum_frames > MAX_ANALYSIS_FRAMES
            || self.maximum_report_bytes == 0
            || self.maximum_report_bytes > MAX_ANALYSIS_REPORT_BYTES
        {
            return Err(AnalysisError::Limit);
        }
        Ok(())
...
impl AnalysisBudget {
    ...
    pub fn new(detection_units: u64, association_units: u64) -> Self {
        Self {
            detection: DetectionBudget::new(detection_units),
            association: AssociationBudget::new(association_units),
        }
    }
}
...
impl AnalysisObservation {
    ...
    #[must_use]
    pub fn detection(&self) -> &DetectionFrame {
        &self.detection
    }
    /// History-linked active, stale, retired, ambiguous, and reset hypotheses.
    #[must_use]
    pub fn tracking(&self) -> &TrackingUpdate {
        &self.tracking
    }
}
...
impl AnalysisReport {
    ...
    pub fn read(
        ...
        checkpoint(cx)?;
        limits.validate()?;
        if plan.frames.len() > limits.maximum_frames {
            return Err(AnalysisError::Limit);
        }
        ...
        e.bytes(b"FSSARPT1");
        e.u32(1);
        e.text(REPORT_DOMAIN);
        e.bytes(plan.encoded());
        e.u64(plan.frames.len() as u64);
        let mut bytes = e.finish_checked()?;
        if bytes.len() > limits.maximum_report_bytes {
            return Err(AnalysisError::Limit);
        }
        ...
        for selected in &plan.frames {
            ...
            let detection = DetectionFrame::read(
                deployment,
                selected.run_identity,
                &source,
                &plan.detector,
                &mut budget.detection,
                cx,
            )?;
            let tracking = tracker.observe(&detection, &mut budget.association, cx)?;
            let mut e = CanonicalEncoder::new();
            e.bytes(&detection.encoded()?);
            e.bytes(&tracking.encoded()?);
            let entry = e.finish_checked()?;
            if bytes
                .len()
                .checked_add(entry.len())
                .is_none_or(|n| n > limits.maximum_report_bytes)
            {
                return Err(AnalysisError::Limit);
            }
            bytes
                .try_reserve(entry.len())
                .map_err(|_| AnalysisError::Limit)?;
            bytes.extend_from_slice(&entry);
            observations.push(AnalysisObservation {
                detection,
                tracking,
            });
        }
...
mod tests {
    ...
    fn recipe() -> Result<AnalysisPlan, AnalysisError> {
        AnalysisPlan::new(
            ...
            vec![
                AnalysisFrame {
                    segment_index: 0,
                    run_identity: ContentDigest::sha256(b"run0"),
                },
                AnalysisFrame {
                    segment_index: 2,
                    run_identity: ContentDigest::sha256(b"run2"),
                },
            ],
        )
    }
    ...
    fn every_truncated_plan_is_refused_even_when_rehashed() -> TestResult {
        ...
        let mut trailing = plan.encoded().to_vec();
        trailing.push(0);
    ...
    fn unbounded_or_empty_report_limits_are_refused() {
        let mut limits = AnalysisLimits {
            maximum_frames: MAX_ANALYSIS_FRAMES + 1,
            ..AnalysisLimits::default()
        };
        assert!(limits.validate().is_err());
        limits.maximum_frames = 1;
        limits.maximum_report_bytes = 0;
        assert!(limits.validate().is_err());
