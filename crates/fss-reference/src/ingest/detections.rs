...
use fss_core::{CanonicalEncode, CanonicalEncoder, ContentDigest, ContractError, DigestAlgorithm,
    LedgerAnchor, SensorCapsule};
...
impl Error for DetectionError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self { Self::Contract(e) => Some(e), Self::Source(e) => Some(e.as_ref()), _ => None }
    }
}
...
fn checkpoint(cx: &ReplayCx) -> Result<(), DetectionError> {
    cx.checkpoint("recorded_detections:work").map_err(|_| DetectionError::Cancelled)
}
...
impl DetectionContract {
    ...
    pub fn new(spec: DetectionSpec) -> Result<Self, DetectionError> {
        let valid_name = |s: &str| !s.is_empty() && s.len() <= 128
            && s.bytes().all(|b| b.is_ascii_alphanumeric() || matches!(b, b'_' | b'-' | b'.' | b':'));
        if spec.model_digest.algorithm() != DigestAlgorithm::Sha256 || !valid_name(&spec.output_port)
            || spec.labels.is_empty() || spec.labels.len() > 256
            || spec.labels.iter().any(|label| !valid_name(label))
            || spec.labels.iter().collect::<BTreeSet<_>>().len() != spec.labels.len()
            || spec.minimum_score_ppm > 1_000_000 || spec.nms_iou_ppm > 1_000_000
            || spec.maximum_rows == 0 || spec.maximum_rows > MAX_DETECTION_ROWS
            || spec.maximum_detections == 0 || spec.maximum_detections > MAX_DETECTIONS
        { return Err(DetectionError::InvalidContract); }
        let mut e = CanonicalEncoder::new();
        e.text("fss.recorded_detection_contract.v1"); e.digest(spec.model_digest); e.text(&spec.output_port);
        e.u64(spec.labels.len() as u64);
        for label in &spec.labels { e.text(label); }
        e.u8(match spec.encoding { BoxEncoding::Xyxy => 0, BoxEncoding::CenterSize => 1 });
        e.u8(match spec.coordinates { CoordinateSpace::Pixels => 0, CoordinateSpace::Normalized => 1 });
        e.u32(BOX_SUBPIXELS); e.u32(spec.minimum_score_ppm); e.u32(spec.nms_iou_ppm);
        e.u64(spec.maximum_rows as u64); e.u64(spec.maximum_detections as u64);
        let bytes = e.finish_checked()?;
        Ok(Self { digest: ContentDigest::sha256(&bytes), spec, bytes })
    }
    ...
    #[must_use]
    pub fn labels(&self) -> &[String] { &self.spec.labels }
}
...
impl DetectionBudget {
    ...
    fn charge(&mut self) -> Result<(), DetectionError> {
        if self.remaining == 0 { return Err(DetectionError::BudgetExceeded); }
...
impl QuantizedBox {
    ...
    #[must_use]
    pub fn coordinates(&self) -> [u32; 4] { self.coordinates }
    fn area(self) -> u64 {
...
impl Detection {
    ...
    #[must_use]
    pub fn bounds(&self) -> QuantizedBox { self.bounds }
    fn encode(&self, e: &mut CanonicalEncoder) {
        e.u64(self.row as u64); e.u64(self.class_index as u64); e.u32(self.score.to_bits());
        for value in self.bounds.coordinates { e.u32(value); }
    }
}

fn quantize(row: &[f32], spec: &DetectionSpec, dimensions: [u32; 2]) -> Result<Detection, DetectionError> {
    if row.len() != 6 || row.iter().any(|v| !v.is_finite()) || !(0.0..=1.0).contains(&row[4])
        || row[5] < 0.0 || row[5] >= spec.labels.len() as f32 || row[5].fract() != 0.0
    { return Err(DetectionError::InvalidOutput); }
    let mut b = [f64::from(row[0]), f64::from(row[1]), f64::from(row[2]), f64::from(row[3])];
    if spec.encoding == BoxEncoding::CenterSize {
        if b[2] <= 0.0 || b[3] <= 0.0 { return Err(DetectionError::InvalidOutput); }
        b = [b[0] - b[2] / 2.0, b[1] - b[3] / 2.0, b[0] + b[2] / 2.0, b[1] + b[3] / 2.0];
    }
    if spec.coordinates == CoordinateSpace::Normalized {
        if b.iter().any(|v| !(0.0..=1.0).contains(v)) { return Err(DetectionError::InvalidOutput); }
        b[0] *= f64::from(dimensions[0]); b[2] *= f64::from(dimensions[0]);
        b[1] *= f64::from(dimensions[1]); b[3] *= f64::from(dimensions[1]);
    }
    if b[0] < 0.0 || b[1] < 0.0 || b[0] >= b[2] || b[1] >= b[3]
        || b[2] > f64::from(dimensions[0]) || b[3] > f64::from(dimensions[1])
    { return Err(DetectionError::InvalidOutput); }
    let scale = f64::from(BOX_SUBPIXELS);
    let coordinates = [(b[0] * scale).floor() as u32, (b[1] * scale).floor() as u32,
        (b[2] * scale).ceil() as u32, (b[3] * scale).ceil() as u32];
    Ok(Detection { row: 0, class_index: row[5] as usize,
        score: if row[4] == 0.0 { 0.0 } else { row[4] }, bounds: QuantizedBox { coordinates } })
}

fn decode_rows(
    contract: &DetectionContract, dimensions: [u32; 2], values: &[f32],
    budget: &mut DetectionBudget, mut check: impl FnMut() -> Result<(), DetectionError>,
    ...
    for (index, row) in values.as_chunks::<6>().0.iter().enumerate() {
        ...
        if f64::from(detection.score) * 1_000_000.0 < f64::from(contract.spec.minimum_score_ppm) {
            below_threshold += 1;
        } else { candidates.push(detection); }
    }
    ...
    for candidate in candidates {
        ...
        for previous in &selected {
            if previous.class_index != candidate.class_index { continue; }
            check()?; budget.charge()?;
            let (intersection, union) = previous.bounds.overlap(candidate.bounds);
            if u128::from(intersection) * 1_000_000 > u128::from(union) * u128::from(contract.spec.nms_iou_ppm) {
                duplicate = true; break;
            }
        }
        if duplicate { suppressed += 1; }
        else {
            if selected.len() == contract.spec.maximum_detections { return Err(DetectionError::Limit); }
...
impl DetectionFrame {
    ...
    pub fn read(
        deployment: &ReferenceDeployment, run_id: ContentDigest, source: &RecordedDecodeRequest,
        contract: &DetectionContract, budget: &mut DetectionBudget, cx: &ReplayCx,
    ...
    pub fn derive(
        run: &RecordedInference, frame: &RecordedFrame, contract: &DetectionContract,
        budget: &mut DetectionBudget, cx: &ReplayCx,
    ) -> Result<Self, DetectionError> {
        checkpoint(cx)?;
        if run.frame_root() != frame.publication_root() || run.model().digest() != contract.spec.model_digest {
            return Err(DetectionError::InvalidContract);
        }
        let port = run.model().graph().find_output(&contract.spec.output_port).ok_or(DetectionError::InvalidContract)?;
        let rows = match port.shape().dims() {
            [rows, 6] | [1, rows, 6] => *rows,
            _ => return Err(DetectionError::InvalidOutput),
        };
        let values = run.outputs().get(&contract.spec.output_port).ok_or(DetectionError::InvalidOutput)?;
        if rows > contract.spec.maximum_rows || rows.checked_mul(6) != Some(values.len()) {
            return Err(DetectionError::Limit);
        }
        let before = budget.used();
        let (detections, filtered, suppressed) = decode_rows(contract, frame.receipt().dimensions(), values, budget, || checkpoint(cx))?;
        Ok(Self { contract: contract.clone(), run: run.identity(),
            run_receipt: ContentDigest::sha256(&run.receipt_bytes().map_err(source_error)?),
            output_digest: ContentDigest::sha256(run.output_bytes()), frame_root: frame.publication_root(),
            frame_receipt: frame.receipt().digest().map_err(source_error)?, anchor: run.authority_anchor().clone(),
            import_identity: frame.receipt().import_identity(), segment: frame.receipt().segment_index(),
            capsule: frame.receipt().capsule().clone(), dimensions: frame.receipt().dimensions(),
            rows, detections, filtered, suppressed, work_units: budget.used() - before })
    }
    ...
    pub fn encoded(&self) -> Result<Vec<u8>, DetectionError> {
        let mut e = CanonicalEncoder::new();
        e.bytes(b"FSSDETS1"); e.u32(1); e.text("fss.recorded_detections.v1");
        e.bytes(self.contract.encoded()); e.digest(self.run); e.digest(self.run_receipt);
        e.digest(self.output_digest); e.digest(self.frame_root); e.digest(self.frame_receipt);
        self.anchor.encode_canonical(&mut e); e.digest(self.import_identity); e.u64(self.segment);
        self.capsule.encode_canonical(&mut e); e.u32(self.dimensions[0]); e.u32(self.dimensions[1]);
        e.u64(self.rows as u64); e.u64(self.filtered as u64); e.u64(self.suppressed as u64);
        e.u64(self.work_units); e.u64(self.detections.len() as u64);
        for detection in &self.detections { detection.encode(&mut e); }
        Ok(e.finish_checked()?)
    }
    /// Identity of this complete source-linked projection.
    pub fn digest(&self) -> Result<ContentDigest, DetectionError> { Ok(ContentDigest::sha256(&self.encoded()?)) }
    /// Surviving proposals in descending score order, with row-index tie breaks.
    #[must_use]
    pub fn detections(&self) -> &[Detection] { &self.detections }
    /// Frozen class/coordinate/threshold contract.
    #[must_use]
    pub fn contract(&self) -> &DetectionContract { &self.contract }
    ...
    #[must_use]
    pub fn counts(&self) -> [usize; 3] { [self.rows, self.filtered, self.suppressed] }
...
mod tests {
    ...
    fn spec() -> DetectionSpec {
        DetectionSpec { model_digest: ContentDigest::sha256(b"model"), output_port: "detections".into(),
            labels: vec!["vehicle".into(), "animal".into()], encoding: BoxEncoding::Xyxy,
            coordinates: CoordinateSpace::Pixels, minimum_score_ppm: 500_000, nms_iou_ppm: 500_000,
            maximum_rows: MAX_DETECTION_ROWS, maximum_detections: MAX_DETECTIONS }
    }
    fn decode(values: &[f32]) -> Result<(Vec<Detection>, usize, usize), DetectionError> {
        decode_rows(&DetectionContract::new(spec())?, [100, 100], values, &mut DetectionBudget::new(1_000_000), || Ok(()))
    }
    #[test]
    fn suppression_is_class_aware_and_ties_keep_original_row() -> TestResult {
        let (items, below, suppressed) = decode(&[
            0.,0.,10.,10.,0.9,0., 0.,0.,10.,10.,0.9,0.,
            0.,0.,10.,10.,0.9,1., 20.,20.,30.,30.,0.1,0.,
        ])?;
        assert_eq!((below, suppressed), (1,1));
        assert_eq!(items.iter().map(Detection::row).collect::<Vec<_>>(), vec![0,2]);
        Ok(())
    }
    #[test]
    fn nms_threshold_is_strict_and_score_threshold_inclusive() -> TestResult {
        let mut s = spec(); s.nms_iou_ppm = 1_000_000;
        let (items, _, suppressed) = decode_rows(&DetectionContract::new(s)?, [100,100],
            &[0.,0.,10.,10.,0.5,0., 0.,0.,10.,10.,0.5,0.], &mut DetectionBudget::new(10), || Ok(()))?;
        assert_eq!(items.len(), 2); assert_eq!(suppressed, 0);
        Ok(())
    }
    #[test]
    fn touching_boxes_are_not_suppressed_at_zero_iou_threshold() -> TestResult {
        let mut s = spec(); s.nms_iou_ppm = 0;
        let (items, _, _) = decode_rows(&DetectionContract::new(s)?, [100,100],
            &[0.,0.,10.,10.,0.7,0., 10.,0.,20.,10.,0.6,0.], &mut DetectionBudget::new(10), || Ok(()))?;
        assert_eq!(items.len(), 2); Ok(())
    }
    #[test]
    fn normalized_center_size_has_explicit_outward_quantization() -> TestResult {
        let mut s = spec(); s.encoding = BoxEncoding::CenterSize; s.coordinates = CoordinateSpace::Normalized;
        let (items, _, _) = decode_rows(&DetectionContract::new(s)?, [100,80],
            &[0.5,0.5,0.5,0.5,1.,1.], &mut DetectionBudget::new(10), || Ok(()))?;
        assert_eq!(items[0].bounds().coordinates(), [6400,5120,19200,15360]); Ok(())
    }
    #[test]
    fn invalid_rows_fail_even_below_score_threshold() {
        for (index, bad) in [(0,f32::NAN),(1,f32::INFINITY),(0,-1.),(2,101.),(2,0.),(4,1.01),(5,0.5),(5,2.)] {
            let mut row = [0.,0.,10.,10.,0.1,0.]; row[index] = bad;
    ...
    fn malformed_lengths_and_dimensions_are_refused() -> TestResult {
        ...
        for dimensions in [[0,100],[4097,1]] {
            assert!(decode_rows(&c, dimensions, &[], &mut DetectionBudget::new(10), || Ok(())).is_err());
        }
        Ok(())
    }
    #[test]
    fn capacity_never_silently_truncates_valid_detections() -> TestResult {
        let mut s = spec(); s.maximum_detections = 1;
        let c = DetectionContract::new(s)?;
        assert!(matches!(decode_rows(&c,[100,100], &[0.,0.,10.,10.,0.9,0.,20.,20.,30.,30.,0.8,0.],
            &mut DetectionBudget::new(10), || Ok(())), Err(DetectionError::Limit)));
        Ok(())
    }
    #[test]
    fn shared_budget_counts_validation_and_suppression_without_refunds() -> TestResult {
        let c = DetectionContract::new(spec())?; let mut b = DetectionBudget::new(3);
        let rows = [0.,0.,10.,10.,0.9,0.,0.,0.,10.,10.,0.8,0.];
        decode_rows(&c,[100,100],&rows,&mut b,|| Ok(()))?;
        assert_eq!(b.used(),3); assert_eq!(b.remaining(),0);
        assert!(matches!(decode_rows(&c,[100,100],&rows,&mut b,|| Ok(())),Err(DetectionError::BudgetExceeded)));
        Ok(())
    }
    #[test]
    fn cancellation_never_returns_partial_detections() -> TestResult {
        let c = DetectionContract::new(spec())?; let mut checks = 0; let mut b = DetectionBudget::new(10);
        let result = decode_rows(&c,[100,100],&[0.,0.,10.,10.,1.,0.,20.,20.,30.,30.,1.,0.],&mut b,|| {
            checks += 1; if checks >= 3 { Err(DetectionError::Cancelled) } else { Ok(()) }
        });
        assert!(matches!(result,Err(DetectionError::Cancelled))); assert_eq!(b.used(),1); Ok(())
    }
    #[test]
    fn empty_output_and_all_filtered_output_remain_distinct() -> TestResult {
        let (empty, below, _) = decode(&[])?; assert!(empty.is_empty()); assert_eq!(below,0);
        let (empty, below, _) = decode(&[0.,0.,10.,10.,0.1,0.])?; assert!(empty.is_empty()); assert_eq!(below,1); Ok(())
    }
    #[test]
    fn contract_identity_binds_labels_model_and_thresholds() -> TestResult {
        let first = DetectionContract::new(spec())?.digest();
        let mut changed = spec(); changed.labels.swap(0,1);
        assert_ne!(DetectionContract::new(changed)?.digest(),first);
        let mut changed = spec(); changed.model_digest = ContentDigest::sha256(b"other model");
        assert_ne!(DetectionContract::new(changed)?.digest(),first);
        let mut changed = spec(); changed.minimum_score_ppm += 1;
        assert_ne!(DetectionContract::new(changed)?.digest(),first); Ok(())
    }
    #[test]
    fn invalid_contracts_are_not_normalized_silently() {
        let mut s = spec(); s.labels[1] = s.labels[0].clone(); assert!(DetectionContract::new(s).is_err());
        let mut s = spec(); s.maximum_rows = 0; assert!(DetectionContract::new(s).is_err());
        let mut s = spec(); s.minimum_score_ppm = 1_000_001; assert!(DetectionContract::new(s).is_err());
        let mut s = spec(); s.labels[0] = "untrusted\nlabel".into(); assert!(DetectionContract::new(s).is_err());
    }
}
