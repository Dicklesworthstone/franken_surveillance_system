    ...
    fn new(name: &str) -> TestResult<Self> {
        for attempt in 0..100 {
            let root = std::env::temp_dir().join(format!(
                "fss-interval-{name}-{}-{attempt}",
                std::process::id()
            ));
            match fs::create_dir(&root) {
                Ok(()) => {
                    let authority = ContextAuthority::new_root(RootAuthoritySpec {
                        trace_id: "trace:intervals".into(),
                        operation_id: OperationId::parse("operation:intervals")?,
                        principal: "principal:test".into(),
                        capabilities: vec!["ADP-REPLAY-001".into()],
                        deadline: None,
                        priority: 10,
                        budgets: BudgetVector::builder().bytes(1_000_000).build()?,
                        privacy_scope: "privacy:test".into(),
                        retention_scope: "retention:test".into(),
                        anchor_universe: ContentDigest::sha256(b"site:intervals"),
                        generation: 1,
                    })?;
...
fn discarded_midpoint_favorite_no_longer_hides_a_valid_proposal_pair() -> TestResult {
    ...
    let mut entries = vec![
        entry(0, 0, 0, 0, 60.0)?,
        entry(1, 0, -20, 20, 60.0)?,
        entry(1, 1, 0, 0, 61.0)?,
    ];
    let before: Vec<_> = entries
        .iter()
        .map(|e| (e.record.clone(), e.record_digest, e.capture))
        .collect();
...
fn full_width_capture_coordinates_reach_the_real_adapter_without_narrowing() -> TestResult {
    ...
    for shift in [i128::MIN + 10, i128::from(i64::MAX) + 1000, i128::MAX - 10] {
        let mut entries = vec![
            entry(0, 0, shift - 2, shift + 2, 60.0)?,
            entry(1, 0, shift, shift + 4, 61.0)?,
        ];
        let pairs = associate_entries(&tiny_plan(), &mut entries, &context.cx)?;
        assert_eq!(pairs.len(), 1);
...
fn candidate_indices_and_zone_order_survive_canonical_track_sorting() -> TestResult {
    ...
    let mut yard = plan.zones[0].clone();
    yard.zone_id = "yard".into();
    plan.zones.push(yard);
    let mut entries = vec![
        entry(1, 0, 0, 0, 60.0)?,
        entry(0, 0, 0, 0, 60.0)?,
        entry(0, 0, 0, 0, 60.0)?,
    ...
    entries[0].zone_id = "yard".into();
    entries[2].zone_id = "yard".into();
...
fn only_uncertain_pairs_keep_both_observations_and_produce_no_proposal() -> TestResult {
    ...
    assert!(
        entries
            .iter()
            .all(|e| e.disposition == EntryDisposition::TimeGateUncertain)
    );
    Ok(())
}
...
fn cancellation_before_assignment_returns_no_partial_pair_set() -> TestResult {
    ...
    context
        .cx
        .set_cancel_at_checkpoint("recorded_corroboration:associate");
    ...
    assert!(
        entries
            .iter()
            .all(|e| e.disposition == EntryDisposition::NoCounterpartEntry)
    );
    Ok(())
}
