...
/// Default maximum stream size: 512 MiB per specification bead.
pub const DEFAULT_MAX_INPUT_BYTES: usize = 512 * 1024 * 1024;
...
impl AnnexBNal {
    ...
    /// Returns `true` if this NAL contains a `first_mb_in_slice` header field (types 1, 2, 5).
    /// Data partitions B and C (types 3, 4) start with `slice_id` instead.
    #[must_use]
    pub const fn has_first_mb_in_slice(&self) -> bool {
        self.nal_unit_type == 1 || self.nal_unit_type == 2 || self.nal_unit_type == 5
    }
...
impl AnnexBLimits {
    ...
    /// Sets the maximum NAL unit bytes, clamped to [`CEILING_MAX_NAL_BYTES`].
    #[must_use]
    pub const fn with_max_nal_bytes(mut self, max: usize) -> Self {
        if max > CEILING_MAX_NAL_BYTES {
            self.max_nal_bytes = CEILING_MAX_NAL_BYTES;
        } else {
...
pub fn split_annexb(
    ...
    if cx.checkpoint("pre_scan").is_err() {
        return Err(AnnexBError::Cancelled);
    }

    // Step 1: Locate the first start code.
    let first_sc3 = match find_start_code(bytes, 0, cx)? {
        Some(pos) => pos,
    ...
    loop {
        if cur_nal_start >= next_checkpoint {
            if cx.checkpoint("nal_scan").is_err() {
                return Err(AnnexBError::Cancelled);
            }
            next_checkpoint = next_checkpoint.saturating_add(CHECKPOINT_INTERVAL_BYTES);
        }

        let next_sc3_opt = find_start_code(bytes, cur_nal_start, cx)?;
        match next_sc3_opt {
            Some(next_sc3) => {
...
fn find_start_code(bytes: &[u8], from: usize, cx: &ReplayCx) -> Result<Option<usize>, AnnexBError> {
    if from >= bytes.len() {
        return Ok(None);
    }
    let mut i = from;
    let mut next_checkpoint = from.saturating_add(CHECKPOINT_INTERVAL_BYTES);
    while i.saturating_add(2) < bytes.len() {
        if i >= next_checkpoint {
            if cx.checkpoint("find_start_code").is_err() {
                return Err(AnnexBError::Cancelled);
            }
            next_checkpoint = next_checkpoint.saturating_add(CHECKPOINT_INTERVAL_BYTES);
        }
...
fn validate_and_create_nal(
    ...
    let effective_max_nal_bytes = max_nal_bytes.min(CEILING_MAX_NAL_BYTES);
    if nal_len > effective_max_nal_bytes {
        return Err(AnnexBError::NalTooLarge {
            offset: nal_offset,
            len: nal_len,
            max: effective_max_nal_bytes,
        });
    }
...
/// Validates emulation prevention sequences in a NAL payload per H.264 7.3.1 and 7.4.1.
    ...
    while i.saturating_add(2) < payload.len() {
        if payload[i] == 0x00 && payload[i + 1] == 0x00 {
            ...
            if third == 0x03 {
                ...
                // H.264 7.4.1: cabac_zero_word allows trailing 00 00 03 at end of NAL.
                ...
                // Valid emulation prevention sequence (or valid trailing 00 00 03).
                // Skip past 0x03:
                i = i.saturating_add(3);
                continue;
            } else if third <= 0x02 {
                // H.264 7.4.1: 0x000000, 0x000001, 0x000002 are strictly forbidden within a NAL.
                return Err(AnnexBError::MalformedEmulationPrevention {
                    offset: base_offset.saturating_add(i),
                });
            }
...
fn group_access_units(
    ...
    for (nal_idx, nal) in nals.iter().enumerate() {
        ...
        let first_mb = if nal.has_first_mb_in_slice() {
        ...
        let starts_new_au = match &current_au {
            None => true,
            Some(builder) => {
                if builder.has_ended || nal.is_aud() {
                    true
                } else if builder.has_vcl {
                    if nal.has_first_mb_in_slice() {
                        first_mb == Some(0)
                    } else if nal.nal_unit_type == 3 || nal.nal_unit_type == 4 {
                        // Data partitions B and C belong to current AU
                        false
                    } else if nal.nal_unit_type == 10 || nal.nal_unit_type == 11 {
                        // End of sequence (10) and end of stream (11) belong to current AU (spec 7.4.1.2.3)
                        false
                    } else {
                        // SPS, PPS, SEI, 14..=18 after a VCL NAL start a new AU
                        nal.is_sps()
                            || nal.is_pps()
                            || nal.is_sei()
        ...
        if starts_new_au {
            if let Some(builder) = current_au.take() {
                ...
                if aus.len() >= max_aus {
                    return Err(AnnexBError::TooManyAccessUnits {
                        count: aus.len().saturating_add(1),
                        max: max_aus,
                    });
                }
            ...
            current_au = Some(AuAccumulator {
                ...
                has_ended: nal.nal_unit_type == 10 || nal.nal_unit_type == 11,
            });
        } else if let Some(ref mut builder) = current_au {
            builder.nal_indices.push(nal_idx);
            if nal.nal_unit_type == 10 || nal.nal_unit_type == 11 {
                builder.has_ended = true;
            }
            if nal.is_vcl() {
...
mod tests {
    ...
    fn test_cx(label: &str) -> Result<ReplayCx, Box<dyn Error>> {
        ...
        let scratch_root = std::env::var_os("CARGO_TARGET_TMPDIR")
            ...
            .join(format!("test-replay-cx-annexb-unit-{label}"));
        let io = ReplayIoAuthority::from_context_authority(&root_auth, scratch_root)?;
        Ok(ReplayCx::new(io))
    }

    #[test]
    fn test_m07_in_scan_checkpoint_find_start_code_and_au_grouping() -> Result<(), Box<dyn Error>> {
        // 1. In-scan checkpoint inside find_start_code kills mutant M7
        let mut stream = vec![0, 0, 1, 0x0C];
        // Fill 2 MiB (> 1 MiB checkpoint interval) with no start codes
        stream.resize(2 * 1024 * 1024, 0xFF);
        stream.extend_from_slice(&[0, 0, 1, 0x09, 0x10]);

        let cx = test_cx("m07_start_code")?;
        cx.set_cancel_at_checkpoint("find_start_code");
        let res = split_annexb(&stream, AnnexBLimits::default(), &cx);
        assert_eq!(res, Err(AnnexBError::Cancelled));

        // 2. Checkpoint inside au_grouping
        let cx_au = test_cx("m07_au")?;
        cx_au.set_cancel_at_checkpoint("au_grouping");
    ...
    #[test]
    fn test_m07_in_scan_checkpoint_nal_scan() -> Result<(), Box<dyn Error>> {
    ...
    #[test]
    fn test_header_spanning_forbidden_sequences() -> Result<(), Box<dyn Error>> {
        let cx = test_cx("hdr_seq1")?;
