...
use fss_codec_mjpeg::{ComponentInterpretation, DecodeBudget, DecodeError, DecodeLimits, DecodedLuma};
use fss_geometry::{GeometryError, WorkBudget};
use fss_object::MAX_MANIFEST_CHILDREN;
use fss_publication::{LocalPublicationReceipt, LocalRootPublisher, PublishCancellation, PublishCutPoint};

use super::http_archive::{HttpArchiveError, HttpArchiveLimits, HttpWireArchive, HttpWirePin, HttpWireScope, HttpWirePublication};
use super::http_camera::{HttpCamera, HttpCameraAuthority, HttpCameraError, HttpCameraLimits,
    HttpCameraRetirement, HttpCameraRoute, HttpCameraSecurity, HttpCameraStep, HttpWireReceipt};
...
impl Default for HttpRecordingLimits {
    fn default() -> Self {
        ...
        media.maximum_reads = media.maximum_reads.min(MAX_MANIFEST_CHILDREN.saturating_sub(1));
        Self { media, decode: HttpCheckDecode::None, io_calls: 1_000_000, connect_timeout_ns: 5_000_000_000 }
    }
}
...
impl HttpRecordingRequest {
    ...
    pub fn new(source: HttpCheckSource, peer: std::net::SocketAddr, authority: &str,
        target: &str, limits: HttpRecordingLimits, deadline_ns: u64) -> Result<Self, HttpRecordingError> {
        let scope = source.scope().map_err(|_| HttpRecordingError::Configuration)?;
        let route = HttpCameraRoute::new(scope.stream, peer, authority, target, HttpCameraSecurity::OwnerApprovedPlaintext)?;
        Ok(Self { route, scope, limits, deadline_ns })
    }
}
...
pub enum HttpRecordingError {
    ...
    Decode { /// Original part ordinal, not a capture timestamp.
        ordinal: u64, /// Payload-free native failure.
        error: DecodeError },
    /// Whole-recording source/linking budget or cancellation refused.
    Work(GeometryError),
}
impl From<HttpCameraError> for HttpRecordingError { fn from(e: HttpCameraError) -> Self { Self::Source(e) } }
impl From<HttpArchiveError> for HttpRecordingError { fn from(e: HttpArchiveError) -> Self { Self::Archive(e) } }
impl From<HttpCompletionError> for HttpRecordingError { fn from(e: HttpCompletionError) -> Self { Self::Completion(e) } }
impl From<GeometryError> for HttpRecordingError { fn from(e: GeometryError) -> Self { Self::Work(e) } }
impl std::fmt::Display for HttpRecordingError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { write!(f, "HTTP recording refused: {self:?}") }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct HttpRecordingWirePlan { wire: HttpWireReceipt, pin: HttpWirePin }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct HttpRecordingFrameKey { head: HttpHeadIdentity, ordinal: u64, encoded: [u8; 32] }
impl HttpRecordingFrameKey {
    ...
    fn of(frame: &HttpJpegFrame) -> Self {
        let r = frame.part().receipt(); Self { head: frame.head(), ordinal: r.ordinal, encoded: r.encoded_sha256 }
    }
}
...
pub struct HttpRecording {
    camera: HttpCamera, archive: HttpWireArchive, limits: HttpRecordingLimits,
    steps: u64, work: WorkBudget<'static>, framing: DecodeBudget<'static>, decoder: DecodeBudget<'static>,
    wire_plan: Option<HttpRecordingWirePlan>, terminal: Option<PreparedHttpCompletion>, complete: Option<HttpCompletionPin>,
    frame_check: Option<HttpRecordingFrameCheck>, frame_error: Option<HttpRecordingError>, transferred: u64,
}
impl HttpRecording {
    ...
    pub fn connect(request: HttpRecordingRequest, publisher: &LocalRootPublisher, access: HttpRecordingAccess<'_>)
        -> Result<Self, HttpRecordingStartFailure> {
        let before = |reason| HttpRecordingStartFailure { reason, attempted: false };
        let limits = request.limits; let m = limits.media;
        m.validate().map_err(|_| before(HttpRecordingError::Configuration))?;
        if request.route.basis() != request.scope.stream || access.now_ns >= request.deadline_ns
            || !(1..=1_000_000).contains(&limits.io_calls) || !(1..=60_000_000_000).contains(&limits.connect_timeout_ns)
            || m.maximum_reads >= MAX_MANIFEST_CHILDREN || publisher.limits().max_children <= m.maximum_reads
            || publisher.limits().spool.max_object_bytes < m.read_bytes.max(1024 + (m.maximum_reads + 1) * 64) {
            return Err(before(HttpRecordingError::Configuration));
        }
        probe(access).map_err(before)?;
        let mut work = WorkBudget::new(m.source_work);
        let bounds = HttpArchiveLimits { maximum_reads: m.maximum_reads, maximum_bytes: m.maximum_source_bytes,
            maximum_scan_roots: m.maximum_scan_roots, maximum_spool_object_bytes: m.maximum_spool_object_bytes };
        let empty = HttpWireArchive::new(request.scope, bounds).map_err(|e| before(e.into()))?;
        let archive = HttpWireArchive::load(publisher, request.scope, empty.pin(), bounds, access.storage, &mut work)
            .map_err(|e| before(e.into()))?;
        let camera_limits = HttpCameraLimits {
            http: HttpLimits { wire_bytes: m.maximum_source_bytes, entity_bytes: m.maximum_source_bytes, ..HttpLimits::default() },
            multipart: MultipartLimits { frame_bytes: m.maximum_frame_bytes, ..MultipartLimits::default() },
            read_bytes: m.read_bytes, io_calls: limits.io_calls,
            ...
            frames: m.maximum_frames as u64 + 1, source_runs: 65536, connect_timeout_ns: limits.connect_timeout_ns,
        };
        let camera = HttpCamera::connect(request.route, camera_limits, access.now_ns, request.deadline_ns, access.camera)
            .map_err(|e| HttpRecordingStartFailure { reason: e.reason.into(), attempted: e.attempted })?;
        Ok(Self { camera, archive, limits, steps: 0, work, framing: DecodeBudget::new(m.framing_work),
            decoder: DecodeBudget::new(m.decode_work), wire_plan: None, terminal: None, complete: None,
            frame_check: None, frame_error: None, transferred: 0 })
    }
    ...
    pub fn pending_wire_plan(&self) -> Option<HttpRecordingWirePlan> { self.wire_plan }
    ...
    pub fn transferred_frames(&self) -> u64 { self.transferred }
    /// Work receipts never claim measured CPU time, energy or hard real-time deadlines.
    pub fn work(&self) -> HttpRecordingWork {
        HttpRecordingWork { steps: self.steps, source: self.work.used(), framing: self.framing.used(), decode: self.decoder.used() }
    }
    ...
    pub fn poll(&mut self, access: HttpRecordingAccess<'_>) -> Result<HttpRecordingStep, HttpRecordingError> {
        probe(access)?;
        if self.steps == self.limits.media.maximum_steps { return Err(HttpRecordingError::Limit); }
        self.work.charge(1)?; self.steps += 1;
        let step = self.camera.step(access.now_ns, access.camera, &mut self.framing)?;
        match step {
            ...
            HttpCameraStep::WireReady(wire) => {
                if let Some(plan) = self.wire_plan {
                    if plan.wire != wire { return Err(HttpRecordingError::PlanMismatch); }
                    return Ok(HttpRecordingStep::WirePrepared(plan));
                }
                let read = self.camera.pending_wire().ok_or(HttpRecordingError::NotReady)?;
                let prepared = self.archive.prepare(read, &mut self.work)?;
                let plan = HttpRecordingWirePlan { wire, pin: prepared.pin() };
                self.wire_plan = Some(plan);
                Ok(HttpRecordingStep::WirePrepared(plan))
            }
            HttpCameraStep::FrameReady => {
                if self.camera.totals().frames > self.limits.media.maximum_frames as u64 { return Err(HttpRecordingError::Limit); }
                if let Some(error) = self.frame_error { return Err(error); }
                Ok(HttpRecordingStep::FrameReady(HttpRecordingFrameKey::of(self.camera.pending_frame().ok_or(HttpRecordingError::NotReady)?)))
            }
            HttpCameraStep::Complete => {
                if let Some(pin) = self.complete { return Ok(HttpRecordingStep::Complete(pin)); }
                if self.terminal.is_none() {
                    self.terminal = Some(PreparedHttpCompletion::from_camera(&self.camera, &self.archive, &mut self.work)?);
                }
                Ok(HttpRecordingStep::CompletionPrepared(self.prepared_completion().ok_or(HttpRecordingError::NotReady)?))
            }
        }
    }
    ...
    pub fn commit_wire(&mut self, expected: HttpRecordingWirePlan, publisher: &mut LocalRootPublisher,
        access: HttpRecordingAccess<'_>) -> Result<HttpRecordingWireCommit, HttpRecordingError> {
        if self.wire_plan != Some(expected) { return Err(HttpRecordingError::PlanMismatch); }
        probe(access)?;
        // Held unacknowledged input makes this an authority check, not another read.
        if self.camera.step(access.now_ns, access.camera, &mut self.framing)? != HttpCameraStep::WireReady(expected.wire) {
            return Err(HttpRecordingError::NotReady);
        }
        let read = self.camera.pending_wire().ok_or(HttpRecordingError::NotReady)?;
        let prepared = self.archive.prepare(read, &mut self.work)?;
        if prepared.pin() != expected.pin { return Err(HttpRecordingError::PlanMismatch); }
        let publication = self.archive.publish(&prepared, publisher, access.storage, &mut self.work)?;
        let acknowledgement = self.camera.acknowledge_wire(expected.wire, access.now_ns, access.camera);
        if acknowledgement.is_ok() { self.wire_plan = None; }
        Ok(HttpRecordingWireCommit { publication, acknowledgement })
    }
    ...
    pub fn take_frame(&mut self, expected: HttpRecordingFrameKey, publisher: &LocalRootPublisher,
        access: HttpRecordingAccess<'_>) -> Result<HttpRecordedFrame, HttpRecordingError> {
        probe(access)?;
        let frame = self.camera.pending_frame().ok_or(HttpRecordingError::NotReady)?;
        if HttpRecordingFrameKey::of(frame) != expected { return Err(HttpRecordingError::PlanMismatch); }
        if self.camera.totals().frames > self.limits.media.maximum_frames as u64 { return Err(HttpRecordingError::Limit); }
        if let Some(error) = self.frame_error { return Err(error); }
        if self.camera.step(access.now_ns, access.camera, &mut self.framing)? != HttpCameraStep::FrameReady {
            return Err(HttpRecordingError::NotReady);
        }
        let frame = self.camera.pending_frame().ok_or(HttpRecordingError::NotReady)?;
        ...
        if self.frame_check.is_none() {
            ...
            let interpretation = match self.limits.decode {
                HttpCheckDecode::None => None, HttpCheckDecode::Grayscale => Some(ComponentInterpretation::Grayscale),
            ...
            let decoded = if let Some(interpretation) = interpretation {
                ...
                match frame.decode(interpretation, DecodeLimits { maximum_bytes: m.maximum_frame_bytes,
                    maximum_dimension: m.maximum_dimension, maximum_pixels: m.maximum_pixels, ..DecodeLimits::default() }, &mut self.decoder) {
                    Ok(image) => Some(image),
                    Err(error) => {
                        let error = HttpRecordingError::Decode { ordinal: expected.ordinal, error };
                        self.frame_error = Some(error); return Err(error);
                    }
                }
            } else { None };
            self.frame_check = Some(HttpRecordingFrameCheck { exposure, decoded });
        }
        probe(access)?; self.work.charge(0)?;
        let check = self.frame_check.take().ok_or(HttpRecordingError::NotReady)?;
        let frame = match self.camera.take_frame(expected.ordinal, expected.encoded, access.now_ns, access.camera) {
            Ok(frame) => frame,
            Err(error) => { self.frame_check = Some(check); return Err(error.into()); }
        };
    ...
    pub fn commit_completion(&mut self, expected: HttpCompletionPin, publisher: &mut LocalRootPublisher,
        access: HttpRecordingAccess<'_>) -> Result<LocalPublicationReceipt, HttpRecordingError> {
        if self.prepared_completion() != Some(expected) { return Err(HttpRecordingError::PlanMismatch); }
        probe(access)?;
        if self.camera.step(access.now_ns, access.camera, &mut self.framing)? != HttpCameraStep::Complete {
            return Err(HttpRecordingError::NotReady);
        }
        let receipt = self.terminal.as_ref().ok_or(HttpRecordingError::NotReady)?
    ...
    pub fn retire(self) -> HttpRecordingRetirement {
        let work = self.work();
        HttpRecordingRetirement { source: self.camera.retire(), archive: self.archive, work,
            wire_plan: self.wire_plan, terminal: self.terminal, complete: self.complete,
            frame_check: self.frame_check, frame_error: self.frame_error, transferred_frames: self.transferred }
    }
}
...
fn probe(access: HttpRecordingAccess<'_>) -> Result<(), HttpRecordingError> {
    if access.storage.cancel_requested(PublishCutPoint::AfterChildrenVerified) { Err(HttpRecordingError::Cancelled) } else { Ok(()) }
}
