    ...
    fn new(name: &str) -> TestResult<Self> {
        for attempt in 0..100_u32 {
            let path = std::env::temp_dir().join(format!(
                "fss-recorded-decode-{name}-{}-{attempt}",
                std::process::id()
            ));
...
fn context(root: &std::path::Path) -> TestResult<ReplayCx> {
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        ...
        anchor_universe: ContentDigest::sha256(b"site:recorded-decode"),
        generation: 1,
    })?;
    authority.validate()?;
...
fn fixture(
    ...
    let request = FileIngestRequest::new(
        path,
        SensorId::parse("sensor:recorded-decode")?,
        StreamId::parse("stream:recorded-decode")?,
    )
...
fn decode_restart_and_replay_need_no_original_file() -> TestResult {
    ...
    let decoded = RecordedFrame::decode_and_publish(
        &mut deployment,
        &request,
    ...
    assert_eq!(
        decoded.receipt().codec().encoded_sha256,
        ContentDigest::sha256(JPEG).bytes()
    );
    assert_eq!(
        decoded.receipt().capsule().clock_basis,
        fss_core::ClockBasis::Estimated
    );
    assert_eq!(decoded.receipt().capsule().capture.earliest, TimestampNs(0));
    assert_eq!(
        decoded.receipt().capsule().capture.latest,
        TimestampNs(1_000_000_000)
    );
    assert!(decoded.pgm_bytes().starts_with(b"P5\n"));
    assert!(decoded.receipt().work_units() > 0);
    assert_eq!(
        decoded.receipt().codec().luma_sha256,
        ContentDigest::sha256(decoded.pixels()).bytes()
    );
    ...
    drop(deployment);
    let mut reopened =
        ReferenceDeployment::open(&directory.0.join("deployment"), "site:recorded-decode", &cx)?;
    let restored = RecordedFrame::open(&reopened, &request, &cx)?;
    assert_eq!(restored, decoded);
    restored.verify_by_replay(
        &reopened,
        &request,
        &mut DecodeBudget::new(100_000_000),
        &cx,
    ...
    let retried = RecordedFrame::decode_and_publish(
        &mut reopened,
        &request,
        &mut DecodeBudget::new(100_000_000),
        &cx,
...
fn identical_pixels_do_not_merge_distinct_source_capsules() -> TestResult {
    ...
    assert_ne!(
        first.receipt().capsule().capsule_id,
        second.receipt().capsule().capsule_id
    );
    assert_ne!(first.publication_root(), second.publication_root());
    assert_eq!(
        budget.used(),
        first.receipt().work_units() + second.receipt().work_units()
    );
    assert!(
        deployment
            .ledger()
            ...
            .iter()
            .flat_map(|b| &b.deltas)
            .filter(|d| d.family == "decode_receipt")
...
fn budget_limits_and_wrong_interpretation_publish_nothing() -> TestResult {
    ...
    bounded.decode_limits.maximum_pixels = 1;
    assert!(
        RecordedFrame::decode_and_publish(
            &mut deployment,
            &bounded,
    ...
    wrong.interpretation = ComponentInterpretation::YCbCr;
    assert!(
        RecordedFrame::decode_and_publish(
            &mut deployment,
            &wrong,
    ...
    invalid.decode_limits.maximum_bytes = 17 * 1024 * 1024;
    assert!(matches!(
        RecordedFrame::decode_and_publish(
            &mut deployment,
            &invalid,
    ...
    assert!(matches!(
        RecordedFrame::open(&deployment, &request, &cx),
...
fn owner_cancelled_codec_exposes_no_pixels_or_authority() -> TestResult {
    ...
    assert!(matches!(
        RecordedFrame::decode_and_publish(&mut deployment, &request, &mut budget, &cx),
...
fn receipt_binary_roundtrip_and_all_truncations_fail_closed() -> TestResult {
    ...
    assert_eq!(
        RecordedDecodeReceipt::decode(&bytes, receipt.digest()?)?,
        *receipt
    );
    for end in 0..bytes.len() {
        let shortened = &bytes[..end];
        assert!(
            RecordedDecodeReceipt::decode(shortened, ContentDigest::sha256(shortened)).is_err()
        );
    }
    ...
    assert!(
        RecordedDecodeReceipt::decode(&bytes, ContentDigest::sha256(b"another receipt")).is_err()
    );
    let mut forged = receipt.clone();
    forged.width = 4097;
    assert!(forged.encoded().is_err());
    let mut forged = receipt.clone();
    forged.codec.encoded_sha256 = [1; 32];
    assert!(forged.encoded().is_err());
    Ok(())
}
...
fn reads_do_not_substitute_another_recipe_or_broaden_pixel_bounds() -> TestResult {
    ...
    let before = deployment.current_anchor().clone();
    let mut wrong = request.clone();
    wrong.segment_index = 0;
    assert!(matches!(
        RecordedFrame::open(&deployment, &wrong, &cx),
        Err(RecordedDecodeError::Unavailable)
    ));
    wrong = request.clone();
    wrong.interpretation = ComponentInterpretation::YCbCr;
    ...
    wrong = request.clone();
    wrong.decode_limits.maximum_pixels = 1;
    assert!(matches!(
        RecordedFrame::open(&deployment, &wrong, &cx),
        Err(RecordedDecodeError::Limit)
    ));
