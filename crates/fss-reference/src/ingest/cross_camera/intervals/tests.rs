...
fn config() -> CrossCameraConfig {
    CrossCameraConfig {
        max_time_delta_ns: 10,
        max_position_distance: 10.0,
        min_confidence: 0.0,
    }
}
...
fn point(o: &IntervalCameraObservation) -> CameraObservation {
    CameraObservation {
        camera_id: o.camera_id.clone(),
        track_id: o.track_id,
        timestamp_ns: midpoint(o.capture) as i64,
        ground_x: o.ground_x,
        ground_y: o.ground_y,
    }
}
...
fn valid_assignment_ambiguity_is_not_resolved_by_discarding_uncertain_edges() -> Test {
    ...
    assert!(
        new.alternatives()
            .iter()
            .all(|a| a.columns.iter().all(|c| *c != Some(0)))
    );
    Ok(())
}
...
fn complete_point_graph_matches_legacy_including_exclusion_costs() -> Test {
    for shift in -3..=3 {
        ...
        let right = [
            obs("west", 20, shift, shift, 0.5),
            obs("west", 10, 4 + shift, 4 + shift, 3.5),
        ];
        for margin in [0, 1, 500_000] {
            let new = associate_intervals(
                &config(),
                margin,
                &left,
                &right,
            ...
            let old = associate_detailed(
                &config(),
                margin,
                &left.iter().map(point).collect::<Vec<_>>(),
                &right.iter().map(point).collect::<Vec<_>>(),
                &mut WorkBudget::new(100_000_000),
            )?;
...
fn separation_matches_enumeration_over_all_small_intervals() {
    for a0 in -4..=4 {
        for a1 in a0..=4 {
            for b0 in -4..=4 {
                for b1 in b0..=4 {
                    let values: Vec<_> = (a0..=a1)
                        .flat_map(|a: i128| (b0..=b1).map(move |b| a.abs_diff(b)))
                        .collect();
                    for gate in 1..=5 {
                        ...
                        assert_eq!(
                            bounds.gate == IntervalTimeGate::Within,
                            values.iter().all(|v| *v <= gate)
                        );
                        assert_eq!(
                            bounds.gate == IntervalTimeGate::Outside,
                            values.iter().all(|v| *v > gate)
                        );
                    }
                }
...
fn full_width_time_extremes_do_not_overflow_or_narrow() -> Test {
    ...
    let result = report(
        &[obs("east", 1, i128::MIN, i128::MAX, 0.0)],
        &[obs("west", 1, i128::MIN, i128::MAX, 0.0)],
    )?;
...
fn common_translation_preserves_assignment_even_outside_i64_time() -> Test {
    ...
    for shift in [i128::MIN + 10, i128::from(i64::MAX) + 1000, i128::MAX - 10] {
        let shifted = report(
            &[obs("east", 1, shift - 2, shift + 2, 0.0)],
            &[obs("west", 1, shift, shift + 4, 1.0)],
        )?;
...
fn time_gate_is_inclusive_but_zero_rank_does_not_force_a_match() -> Test {
    ...
    assert!(matches!(
        ...
        AssociationScore::Admissible { units: 0, .. }
    ));
    Ok(())
}
...
fn invalid_interval_and_camera_inputs_fail_closed() {
    ...
    for invalid in [
        ...
        obs("", 1, 0, 0, 0.0),
        obs("east", 0, 0, 0, 0.0),
        obs("east", 1, 0, 0, f64::NAN),
    ] {
        assert!(matches!(
            report(&[invalid], &right),
            Err(CrossCameraError::InvalidObservation(_))
        ));
    }
    let duplicate = obs("east", 1, 0, 0, 0.0);
    assert!(matches!(
        report(&[duplicate.clone(), duplicate], &right),
        Err(CrossCameraError::DuplicateObservation)
    ));
    assert!(matches!(
        report(
            &[obs("east", 1, 0, 0, 0.0), obs("other", 2, 0, 0, 0.0)],
            &right
        ),
        Err(CrossCameraError::InvalidObservation(_))
    ));
...
fn hard_input_and_work_limits_never_return_partial_matches() {
    ...
    assert!(matches!(
        report(&vec![left[0].clone(); 65], &right),
