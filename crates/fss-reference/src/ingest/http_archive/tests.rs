...
fn original_read_bytes_survive_cold_restart_and_cross_read_ranges() -> Test {
    ...
    assert_eq!(
        a.read_range(&p, [0, pin.bytes], &NeverCancel, &mut work())?,
        expected
    );
    for start in 0..expected.len() {
        let end = (start + 9).min(expected.len());
        assert_eq!(
            a.read_range(&p, [start as u64, end as u64], &NeverCancel, &mut work())?,
            expected[start..end]
        );
    }
    Ok(())
...
fn exact_retry_is_idempotent_but_changed_read_or_clock_is_not() -> Test {
    ...
    assert_eq!(
        a.publish(&again, &mut p, &NeverCancel, &mut work())?
            .local
            .outcome,
    ...
    assert!(
        a.prepare_bytes(raw(0, b"abc", 11), b"abc", &mut work())
            .is_err()
    );
...
fn lost_acknowledgement_resolves_original_prepared_pin_and_root() -> Test {
    ...
    let expected = plan.pin();
    let expected_slot = plan.slot().clone();
    ...
    assert_eq!(result.local.slot, expected_slot);
    assert_eq!(old.pin(), expected);
    drop(first);
    drop(old);
    drop(plan);
    drop(p);
    let p = d.open()?;
    assert_eq!(
        HttpWireArchive::load(&p, scope(), expected, limits(), &NeverCancel, &mut work())?.pin(),
        expected
    );
    Ok(())
}
...
fn all_root_crash_cuts_never_advance_acknowledged_index_on_error() -> Test {
    for cut in [
        ...
        let p = d.open()?;
        let result =
            HttpWireArchive::load(&p, scope(), expected, limits(), &NeverCancel, &mut work());
        if cut == PublishCutPoint::AfterRootRename {
            assert_eq!(result?.pin(), expected);
        } else {
...
fn cancellation_and_budget_refusal_leave_source_and_index_unchanged() -> Test {
    ...
    let mut a = HttpWireArchive::new(scope(), limits())?;
    let before = a.pin();
    let source = b"unconsumed";
    let plan = a.prepare_bytes(raw(0, source, 10), source, &mut work())?;
    assert!(matches!(
        a.publish(&plan, &mut p, &Stop, &mut work()),
        Err(HttpArchiveError::Cancelled)
    ));
    for cut in [0, 1, 100, 4095] {
        assert!(
            a.publish(&plan, &mut p, &NeverCancel, &mut WorkBudget::new(cut))
                .is_err()
        );
        assert_eq!(a.pin(), before);
...
fn rollback_missing_root_and_unaccounted_later_roots_are_refused() -> Test {
    ...
    assert!(
        HttpWireArchive::load(&p, scope(), missing, limits(), &NeverCancel, &mut work()).is_err()
    );
    let path = p
        .root_dir()
        .join("roots")
...
fn source_generation_retention_and_clock_cannot_rebind_existing_slots() -> Test {
    ...
    for changed in [
        ...
        assert!(
            HttpWireArchive::load(&p, changed, pin, limits(), &NeverCancel, &mut work()).is_err()
        );
...
fn limits_are_external_complete_prefix_bounds_not_serialized_authority() -> Test {
    ...
    let mut p = d.open()?;
    let mut a = HttpWireArchive::new(scope(), limits())?;
    add(&mut a, &mut p, b"abc", 10)?;
    let pin = add(&mut a, &mut p, b"def", 10)?.pin;
    for bounds in [
        HttpArchiveLimits {
            maximum_reads: 1,
            ..limits()
        ...
        assert!(
            HttpWireArchive::load(&p, scope(), pin, bounds, &NeverCancel, &mut work()).is_err()
        );
    }
...
fn incorrect_root_family_and_extra_children_cannot_supply_source_custody() -> Test {
    for extra in [false, true] {
        let d = Directory::new();
        let mut p = d.open()?;
        let a = HttpWireArchive::new(scope(), limits())?;
        ...
        let children = if extra {
            vec![raw, p.stage_object(b"unaccounted child")?]
        } else {
            vec![raw]
        };
        ...
        assert!(
            HttpWireArchive::load(&p, scope(), pin, limits(), &NeverCancel, &mut work()).is_err()
        );
    }
    Ok(())
}
