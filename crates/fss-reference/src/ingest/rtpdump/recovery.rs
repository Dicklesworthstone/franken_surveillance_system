...
use fss_core::{BatchId, CanonicalDecoder, CanonicalEncode, CaptureInterval,
    ContentDigest, DigestAlgorithm, EvidenceDelta, LedgerAnchor, ObjectId, Plane,
...
use super::{RtpDumpLimits, replay::RtpReplayConfig, import::{
    RtpFileImportReceipt, RtpImportError, RtpImportLimits, RtpImportReport,
    RtpImportScope, prepare_rtp_import, publish_rtp_import,
}};
...
impl Default for RtpRecoveryPolicy {
    fn default() -> Self {
        Self { max_source_bytes: 64 * 1024 * 1024, max_records: 65_536,
            max_object_bytes: 64 * 1024 * 1024, max_read_bytes: 512 * 1024 * 1024,
...
impl RecoveredRtpImport {
    ...
    pub fn state(&self) -> &RtpRecoveryState { &self.state }
    ...
    pub fn resume(self, cx: &ReplayCx, deployment: &mut ReferenceDeployment) -> Result<RtpFileImportReceipt> {
        checkpoint(cx)?;
        if deployment.current_anchor() != &self.basis { return Err(RtpImportError::Binding); }
        let slot = root_slot(self.root)?;
        if !deployment.publisher().root(&slot).is_some_and(|r|
            r.root == self.root && r.state == LocalPublicationState::Durable) {
            return Err(RtpImportError::Binding);
        }
        let plan = prepare_rtp_import(&self.source, self.recipe.scope, self.recipe.config,
            self.recipe.limits, cx)?;
        if plan.manifest().root() != self.root { return Err(RtpImportError::Digest); }
        publish_rtp_import(plan, cx, deployment)
    }
}
impl std::fmt::Debug for RecoveredRtpImport {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RecoveredRtpImport").field("root", &self.root)
            .field("source_bytes", &self.source.len()).field("records", &self.report.records().len())
            .field("nals", &self.report.nals().len()).finish_non_exhaustive()
    }
}
...
pub fn inspect_rtp_import(root: ContentDigest, ingress: u128, policy: RtpRecoveryPolicy,
    cx: &ReplayCx, deployment: &ReferenceDeployment) -> Result<RecoveredRtpImport>
{
    checkpoint(cx)?;
    policy.validate()?;
    ...
    if !p.root(&slot).is_some_and(|r| r.root == root && r.state == LocalPublicationState::Durable) {
        return Err(RtpImportError::Binding);
    }
    let reservation = p.spool().limits().max_object_bytes;
    if reservation > policy.max_object_bytes { return Err(RtpImportError::Limit); }
    ...
    let mut read = |digest: ContentDigest| -> Result<Vec<u8>> {
        ...
        if remaining < reservation as u64 { return Err(RtpImportError::Limit); }
        let bytes = p.spool().read(digest)?;
        remaining = remaining.checked_sub(bytes.len() as u64).ok_or(RtpImportError::Limit)?;
        Ok(bytes)
    };
    let manifest_bytes = read(root)?;
    let manifest = ObjectManifest::from_canonical_bytes(&manifest_bytes).map_err(|_| RtpImportError::Digest)?;
    ...
    source.try_reserve_exact(recipe.input_bytes).map_err(|_| RtpImportError::Limit)?;
    for digest in &recipe.chunks {
        if manifest.children().binary_search(digest).is_err() { return Err(RtpImportError::Digest); }
        let bytes = read(*digest)?;
        let expected = recipe.limits.chunk_bytes.min(recipe.input_bytes - source.len());
        if bytes.len() != expected { return Err(RtpImportError::Digest); }
    ...
    let report = {
        let plan = prepare_rtp_import(&source, recipe.scope.clone(), recipe.config, recipe.limits, cx)?;
        if plan.manifest() != &manifest || plan.report_bytes() != report_bytes.as_slice() {
            return Err(RtpImportError::Digest);
        }
        for nal in plan.report().nals() {
            let bytes = read(nal.digest)?;
            if ContentDigest::try_sha256(&bytes)? != nal.digest { return Err(RtpImportError::Digest); }
    ...
    checkpoint(cx)?;
    Ok(RecoveredRtpImport { root, source, report, recipe, state,
        basis: deployment.current_anchor().clone() })
}

fn checkpoint(cx: &ReplayCx) -> Result<()> {
    cx.checkpoint("rtpdump:recover").map_err(|_| RtpImportError::Cancelled)
}
...
fn validate_import_limits(l: RtpImportLimits) -> Result<()> {
    if !(1..=16*1024*1024).contains(&l.chunk_bytes) || !(1..=2048).contains(&l.max_nals)
        || !(1..=65536).contains(&l.max_source_spans) || !(1..=64*1024*1024).contains(&l.max_derived_bytes)
        || !(1..=16*1024*1024).contains(&l.max_report_bytes) || !(1..=256*1024*1024).contains(&l.max_payload_bytes) {
        return Err(RtpImportError::Limit);
    }
    Ok(())
}
fn limit_values(l: RtpImportLimits) -> [usize; 6] {
    [l.chunk_bytes, l.max_nals, l.max_source_spans, l.max_derived_bytes, l.max_report_bytes, l.max_payload_bytes]
}
fn decode_recipe(bytes: &[u8], ingress: u128, policy: RtpRecoveryPolicy) -> Result<Recipe> {
    if bytes.len() > policy.import.max_report_bytes { return Err(RtpImportError::Limit); }
    let mut d = CanonicalDecoder::new(bytes);
    if d.text()? != "fss.rtpdump.import.report.v1" || d.u64()? != 1 { return Err(RtpImportError::Binding); }
    let input = d.digest()?;
    let input_bytes = size(&mut d)?;
    if input.algorithm() != DigestAlgorithm::Sha256 || input_bytes == 0 || input_bytes > policy.max_source_bytes {
        return Err(RtpImportError::Limit);
    }
    let scope = RtpImportScope { sensor: SensorId::parse(d.text()?)?, stream: StreamId::parse(d.text()?)?,
        receive_time: TimestampNs(d.i128()?) };
    if scope.receive_time.0 < 0 || d.text()? != "capture_unknown" { return Err(RtpImportError::Binding); }
    let key = StreamKey { ingress, generation: d.u64()?, ssrc: d.u32()? };
    let payload_type = d.u8()?;
    let mode = match d.u8()? { 0 => H264Mode::SingleNal, 1 => H264Mode::NonInterleaved, _ => return Err(RtpImportError::Binding) };
    let rtcp = match d.u8()? { 0 => RtcpMode::Compound, 1 => RtcpMode::ReducedSize, _ => return Err(RtpImportError::Binding) };
    let dump = RtpDumpLimits { max_input_bytes: size(&mut d)?, max_records: size(&mut d)?, max_packet_bytes: size(&mut d)? };
    let packet = PacketLimits { max_packet_bytes: size(&mut d)?, max_extension_bytes: size(&mut d)?, max_rtcp_packets: size(&mut d)? };
    let codec = H264Limits { max_nal_bytes: size(&mut d)?, max_packet_nals: size(&mut d)?,
        max_fragment_packets: size(&mut d)?, max_pending_age_ns: d.u64()? };
    let config = RtpReplayConfig { key, payload_type, mode, dump, packet, codec, rtcp };
    config.validate()?;
    if input_bytes > dump.max_input_bytes || dump.max_records > policy.max_records { return Err(RtpImportError::Limit); }
    let limits = RtpImportLimits { chunk_bytes: size(&mut d)?, max_nals: size(&mut d)?, max_source_spans: size(&mut d)?,
        max_derived_bytes: size(&mut d)?, max_report_bytes: size(&mut d)?, max_payload_bytes: size(&mut d)? };
    validate_import_limits(limits)?;
    if limit_values(limits).into_iter().zip(limit_values(policy.import)).any(|(stored, ceiling)| stored > ceiling)
        || input_bytes > limits.max_payload_bytes { return Err(RtpImportError::Limit); }
    let count = size(&mut d)?;
    if count != input_bytes.div_ceil(limits.chunk_bytes) || count > fss_object::MAX_MANIFEST_CHILDREN
        || count.checked_mul(33).and_then(|n| n.checked_add(8)).is_none_or(|n| n > d.remaining()) {
        return Err(RtpImportError::Limit);
    }
    let mut chunks = Vec::new(); chunks.try_reserve_exact(count).map_err(|_| RtpImportError::Limit)?;
    for _ in 0..count {
        let digest = d.digest()?;
        if digest.algorithm() != DigestAlgorithm::Sha256 { return Err(RtpImportError::Digest); }
        chunks.push(digest);
    }
    ...
    if size(&mut d)? > config.dump.max_records { return Err(RtpImportError::Limit); }
    Ok(Recipe { input, input_bytes, chunks, scope, config, limits })
}

fn ledger_state(root: ContentDigest, metadata: ContentDigest, report: &RtpImportReport,
    scope: &RtpImportScope, dep: &ReferenceDeployment) -> Result<RtpRecoveryState>
{
    ...
    for (part, group) in report.nals().chunks(64).enumerate() {
        ...
        for nal in group {
            deltas.push(EvidenceDelta { delta_id: format!("delta:{}", nal.capsule.capsule_id.as_str()), family: "sensor_capsule".into(),
                object_id: ObjectId::parse(format!("object:{}", nal.capsule.capsule_id.as_str()))?, prior_generation: None, new_generation: 1,
                validity, plane: Plane::Authority, payload_digest: nal.capsule_object, witness_digest: Some(nal.capsule.source_digest), operation_id: None });
        ...
        if batch.deltas != deltas || batch.children != children { return Err(RtpImportError::Binding); }
        committed += group.len();
    }
    let id = BatchId::parse(format!("batch:rtp:{hex}"))?;
    if let Some(batch) = dep.ledger().batches().iter().find(|b| b.batch_id == id) {
        let expected = EvidenceDelta { delta_id: format!("delta:rtp:{hex}"), family: "rtpdump_import".into(),
            object_id: ObjectId::parse(format!("object:rtp:{hex}"))?, prior_generation: None, new_generation: 1,
            validity, plane: Plane::Authority, payload_digest: metadata, witness_digest: Some(root), operation_id: None };
        let mut children = vec![root, metadata]; children.sort_unstable(); children.dedup();
        if pending || batch.deltas.as_slice() != [expected] || batch.children != children { return Err(RtpImportError::Binding); }
        Ok(RtpRecoveryState::Complete { anchor: batch.new_anchor.clone() })
    } else {
        Ok(RtpRecoveryState::LedgerPending { committed_capsules: committed, total_capsules: report.nals().len() })
    }
}
