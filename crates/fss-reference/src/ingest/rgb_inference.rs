...
impl RgbInferenceModel {
    ...
    pub fn new(
        ...
        if spec.image_input.is_empty()
            || spec.image_input.len() > 128
            || p.channel_transform != ChannelTransform::Rgb
            || p.target_height == 0
            || p.target_width == 0
            || p.target_height > 4096
            || p.target_width > 4096
            || p.target_height
                .checked_mul(p.target_width)
                .is_none_or(|n| n > 4_194_304)
            || graph.nodes().len() > 1024
            || graph.inputs().len() > 257
            || graph.outputs().len() > 256
            || graph.outputs().is_empty()
            || parameters.len() + 1 != graph.inputs().len()
            || graph
                .inputs()
                .iter()
                .chain(graph.outputs())
                .any(|port| port.dtype() != DType::F32)
        ...
        for port in graph
            .inputs()
            .iter()
            .filter(|port| port.name() != spec.image_input.as_str())
        {
            cx.checkpoint("rgb-model:parameters")?;
            let values = parameters
                .get(port.name())
                .ok_or(RgbInferenceError::InvalidInput)?;
            if values.len() != port.shape().num_elements()? {
                return Err(RgbInferenceError::InvalidInput);
            }
            parameter_bytes = parameter_bytes
                ...
                .ok_or(RgbInferenceError::Limit)?;
            if parameter_bytes > MAX_RGB_MODEL_BYTES {
            ...
            for chunk in values.chunks(1024) {
                ...
                if chunk.iter().any(|value| !value.is_finite()) {
                    return Err(RgbInferenceError::NonFinite);
                }
            }
        }
        ...
        if graph_bytes
            ...
            .is_none_or(|n| n > MAX_RGB_MODEL_BYTES)
        {
            return Err(RgbInferenceError::Limit);
        }
        let mut e = CanonicalEncoder::new();
        e.text("fss.rgb-inference-model.reference.v1");
        e.digest(ContentDigest::sha256(&graph_bytes));
        e.text(&spec.image_input);
        e.digest(p.resize_digest(spec.filter, spec.aspect));
        e.bytes(&spec.masked_rgb);
        ...
        for port in graph.outputs() {
            output_bytes = output_bytes
                .checked_add(port.shape().size_bytes(DType::F32)?)
                .and_then(|n| n.checked_add(port.name().len() + port.rank() * 8 + 64))
                .ok_or(RgbInferenceError::Limit)?;
        }
        if output_bytes > MAX_RGB_OUTPUT_BYTES {
            return Err(RgbInferenceError::Limit);
        }
    ...
    pub fn run_luma_replicated(
        ...
        if count == 0 || count > 4_194_304 || luma.len() != count {
            return Err(RgbInferenceError::InvalidInput);
        }
        let mut rgb = Vec::new();
        rgb.try_reserve_exact(count * 3)
            .map_err(|_| RgbInferenceError::Limit)?;
        for (i, value) in luma.iter().enumerate() {
            if i % 4096 == 0 {
                cx.checkpoint("rgb-inference:replicate-copy")?;
            }
            rgb.extend_from_slice(&[*value, *value, *value]);
        }
        let mut e = CanonicalEncoder::new();
        e.text(LUMA_REPLICATION_DOMAIN);
        e.digest(codec_receipt);
        e.digest(ContentDigest::sha256(luma));
    ...
    pub fn run_rgb_pixels(
        ...
        self.admit(source, limits)?;
        if receipt.encoded_sha256 != source.encoded_sha256 {
            return Err(RgbInferenceError::SourceMismatch);
        }
        let [width, height] = receipt.dimensions;
        let count = width as usize * height as usize;
        if count == 0 || count > 4_194_304 || pixels.len() != count * 3 {
            return Err(RgbInferenceError::InvalidInput);
        }
        if ContentDigest::sha256(pixels).bytes() != receipt.rgb_sha256 {
            return Err(RgbInferenceError::SourceMismatch);
        }
        ...
        for chunk in allowed.chunks(4096) {
            ...
            if chunk.iter().any(|value| *value > 1) {
                return Err(RgbInferenceError::InvalidInput);
            }
        }
        if ContentDigest::sha256(allowed).bytes() != source.permission_mask {
            return Err(RgbInferenceError::SourceMismatch);
        }
        ...
        for (i, permission) in allowed.iter().enumerate() {
            if i % 1024 == 0 {
                cx.checkpoint("rgb-inference:mask-copy")?;
            }
            if *permission == 0 {
                masked.extend_from_slice(&self.spec.masked_rgb);
            } else {
                masked.extend_from_slice(&pixels[i * 3..i * 3 + 3]);
            }
        }
        let options = ResizeOptions {
            filter: self.spec.filter,
            aspect: self.spec.aspect,
            budget: ExecBudget::new(
                limits.preprocess.max_macs - mask_work,
                limits.preprocess.max_bytes - bytes,
            ),
        };
        let resized = self.spec.preprocess.execute_resized_bytes(
            ImageBytes {
                bytes: &masked,
        ...
        for (name, tensor) in executed.outputs() {
            ...
            encoded.text(name);
            encoded.u64(tensor.shape().rank() as u64);
            for &n in tensor.shape().dims() {
                encoded.u64(n as u64);
            }
            for chunk in values.chunks(1024) {
        ...
        if output_bytes.len() > limits.maximum_output_bytes {
            return Err(RgbInferenceError::Limit);
        }
        let output_digest = ContentDigest::sha256(&output_bytes);
        let mut e = CanonicalEncoder::new();
        e.text("fss.rgb-inference.reference.v1");
        e.digest(self.digest);
        encode_source(&mut e, source);
        e.bytes(&receipt.decoder);
        e.bytes(&receipt.rgb_sha256);
        e.digest(masked_digest);
        e.digest(input_digest);
        e.digest(output_digest);
    ...
    fn admit(&self, source: RgbSourceBinding, limits: RgbRunLimits) -> Result<()> {
        ...
        if limits.maximum_output_bytes == 0
            || limits.maximum_output_bytes > MAX_RGB_OUTPUT_BYTES
            || self.output_bytes > limits.maximum_output_bytes
            || self.input_bytes > limits.execution.max_bytes
            || limits.preprocess.max_bytes > 256 * 1024 * 1024
            || limits.execution.max_bytes > 256 * 1024 * 1024
        {
            return Err(RgbInferenceError::Limit);
        }
        Ok(())
...
impl RgbInference {
    ...
    pub fn decode_receipt(&self) -> RgbDecodeReceipt {
        self.decode
    }
    ...
    pub fn masked_digest(&self) -> ContentDigest {
        self.masked_digest
    }
    ...
    pub fn allocated_tensor_bytes(&self) -> usize {
        self.allocated_tensor_bytes
    }
}
