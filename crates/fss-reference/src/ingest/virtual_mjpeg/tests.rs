...
fn spec() -> Result<MjpegCameraSpec, Box<dyn Error>> {
    Ok(MjpegCameraSpec { capture_id: CapsuleId::parse("capture:virtual-mjpeg:1")?,
        sensor_id: SensorId::parse("sensor:virtual-mjpeg")?, seed: 7, frame_count: 5,
        width: 64, height: 48, start_ns: 1_000_000, period_ns: 33_000_000,
        uncertainty_ns: 500_000, packet_bytes: 17, warmup_frames: 2 })
}
fn generate(spec: &MjpegCameraSpec) -> Result<GeneratedMjpegSource, MjpegSourceError> {
    ...
    generate_mjpeg_source(spec, &mut clock, &mut MjpegSourceBudget::new(MAX_SCENE_PIXELS, &cancelled))
}
...
fn every_reassembled_frame_decodes_to_the_declared_scene() -> TestResult {
    ...
    for (index, span) in generated.frames().iter().enumerate() {
        ...
        let decoded = decode_luma(&bytes, span.encoded_digest.bytes(), ComponentInterpretation::Grayscale,
            DecodeLimits::default(), &mut DecodeBudget::new(100_000_000))?;
        assert_eq!(decoded.dimensions(), [u32::from(spec.width), u32::from(spec.height)]);
        let cancelled = AtomicBool::new(false);
        let expected = scene::render(&spec, span.rectangle,
            &mut MjpegSourceBudget::new(MAX_SCENE_PIXELS, &cancelled))?;
        assert_eq!(decoded.pixels(), expected.as_slice(), "pixel-exact block scene at frame {index}");
        assert!(decoded.receipt().metadata_segments >= 2, "JFIF plus retained synthetic recipe");
    }
...
fn fragmentation_preserves_every_byte_and_the_frame_capture_interval() -> TestResult {
    for packet_bytes in [1, 17, 64, MAX_VIRTUAL_PACKET_BYTES] {
        let mut spec = spec()?; spec.packet_bytes = packet_bytes;
        let generated = generate(&spec)?;
        let mut next = 0;
        for span in generated.frames() {
            ...
            let earliest = spec.start_ns + i128::from(span.frame_index) * i128::from(spec.period_ns);
            assert_eq!(span.capture, CaptureInterval::new(TimestampNs(earliest),
                TimestampNs(earliest + i128::from(spec.uncertainty_ns)))?);
            next = span.packet_range.end;
        }
        assert_eq!(next, generated.packets().len());
        for (index, packet) in generated.packets().iter().enumerate() { assert_eq!(packet.sequence, index as u64 + 1); }
    }
    Ok(())
}
#[test]
fn equal_inputs_replay_exactly_and_sensor_seed_changes_are_bound() -> TestResult {
    let spec = spec()?;
    let first = generate(&spec)?; let second = generate(&spec)?;
    assert_eq!(first.packets(), second.packets());
    assert_eq!(first.frames(), second.frames());
    let mut changed = spec.clone(); changed.seed += 1;
    assert_ne!(first.frames()[0].encoded_digest, generate(&changed)?.frames()[0].encoded_digest);
    changed = spec.clone(); changed.sensor_id = SensorId::parse("sensor:other-virtual-camera")?;
    assert_ne!(first.frames()[0].encoded_digest, generate(&changed)?.frames()[0].encoded_digest);
    assert_ne!(first.frames()[0].recipe_digest, generate(&changed)?.frames()[0].recipe_digest);
    Ok(())
}
#[test]
fn quiet_warmup_then_motion_is_visible_in_decoded_pixels() -> TestResult {
    let spec = spec()?; let generated = generate(&spec)?;
    let mut centers = Vec::new();
    for (index, span) in generated.frames().iter().enumerate() {
        let bytes = generated.frame_bytes(index).ok_or("missing frame")?;
        let decoded = decode_luma(&bytes, span.encoded_digest.bytes(), ComponentInterpretation::Grayscale,
            DecodeLimits::default(), &mut DecodeBudget::new(100_000_000))?;
        let foreground: Vec<_> = decoded.pixels().iter().enumerate().filter(|(_, p)| **p > 128).collect();
        if index < spec.warmup_frames as usize { assert!(foreground.is_empty()); }
        else {
            assert_eq!(foreground.len(), 16 * 16);
            centers.push(foreground.iter().map(|(i, _)| i % usize::from(spec.width)).sum::<usize>());
        }
    }
...
fn all_supported_axis_extremes_round_trip() -> TestResult {
    for dimension in [24, 32, 64, 128, MAX_SCENE_DIMENSION] {
        let mut spec = spec()?; spec.width = dimension; spec.height = dimension;
        spec.frame_count = 1; spec.warmup_frames = 0;
        let generated = generate(&spec)?;
        let bytes = generated.frame_bytes(0).ok_or("missing frame")?;
        assert!(bytes.len() <= MAX_SCENE_FRAME_BYTES);
        let decoded = decode_luma(&bytes, ContentDigest::sha256(&bytes).bytes(), ComponentInterpretation::Grayscale,
            DecodeLimits::default(), &mut DecodeBudget::new(100_000_000))?;
...
fn cancellation_and_insufficient_budget_leave_clock_unchanged() -> TestResult {
    let spec = spec()?; let mut clock = VirtualClock::new(spec.seed, TimestampNs(spec.start_ns));
    let original = clock.clone();
    let cancelled = AtomicBool::new(true);
    let mut budget = MjpegSourceBudget::new(MAX_SCENE_PIXELS, &cancelled);
    assert!(matches!(generate_mjpeg_source(&spec, &mut clock, &mut budget), Err(MjpegSourceError::Cancelled)));
    assert_eq!(budget.reserved(), 0); assert_eq!(clock, original);
    cancelled.store(false, Ordering::Release);
    let mut budget = MjpegSourceBudget::new(spec.rendered_pixels() - 1, &cancelled);
    assert!(matches!(generate_mjpeg_source(&spec, &mut clock, &mut budget), Err(MjpegSourceError::BudgetExhausted)));
    assert_eq!(budget.reserved(), 0); assert_eq!(clock, original);
...
fn late_clock_overflow_does_not_commit_jitter_or_partial_frames() -> TestResult {
    let mut spec = spec()?; spec.start_ns = i128::MAX - 100; spec.period_ns = 50;
    spec.uncertainty_ns = 10; spec.frame_count = 2;
    let mut clock = VirtualClock::new(spec.seed, TimestampNs(spec.start_ns));
    clock.inject_jitter(u64::MAX);
    let original = clock.clone(); let cancelled = AtomicBool::new(false);
    let mut budget = MjpegSourceBudget::new(MAX_SCENE_PIXELS, &cancelled);
    assert!(matches!(generate_mjpeg_source(&spec, &mut clock, &mut budget), Err(MjpegSourceError::Clock(_))));
    assert_eq!(clock, original);
    assert!(budget.reserved() > 0, "the failure occurs after rendering an earlier frame");
    Ok(())
}
#[test]
fn packet_capacity_refuses_the_complete_session_without_clock_mutation() -> TestResult {
    let mut spec = spec()?; spec.width = 24; spec.height = 24; spec.packet_bytes = 1; spec.frame_count = 512;
    let mut clock = VirtualClock::new(spec.seed, TimestampNs(spec.start_ns));
    let original = clock.clone(); let cancelled = AtomicBool::new(false);
    assert!(matches!(generate_mjpeg_source(&spec, &mut clock,
        &mut MjpegSourceBudget::new(MAX_SCENE_PIXELS, &cancelled)), Err(MjpegSourceError::Limit("source packets"))));
    assert_eq!(clock, original);
    Ok(())
}
#[test]
fn malformed_and_oversized_specifications_are_refused() -> TestResult {
    ...
    for dimension in [0, 8, 23, 25, 264, u16::MAX] {
        let mut bad = good.clone(); bad.width = dimension; assert!(bad.validate().is_err());
    }
    let mut bad = good.clone(); bad.frame_count = 0; assert!(bad.validate().is_err());
    bad = good.clone(); bad.warmup_frames = bad.frame_count + 1; assert!(bad.validate().is_err());
    bad = good.clone(); bad.period_ns = 0; assert!(bad.validate().is_err());
    bad = good.clone(); bad.packet_bytes = MAX_VIRTUAL_PACKET_BYTES + 1; assert!(bad.validate().is_err());
    bad = good.clone(); bad.start_ns = i128::MAX; assert!(bad.validate().is_err());
    bad = good; bad.frame_count = 4096; bad.width = 256; bad.height = 256;
    assert!(matches!(bad.validate(), Err(MjpegSourceError::Limit("aggregate rendered pixels"))));
    Ok(())
}
...
impl JournalDirectory {
    fn new() -> std::io::Result<Self> {
        ...
        let path = std::env::temp_dir().join(format!("fss-mjpeg-source-{}-{id}", std::process::id()));
...
fn real_jpeg_capture_preserves_custody_through_delivery_faults() -> TestResult {
    ...
    assert_eq!(objects.verify_closure(capture.receipt.capture_root)?, capture.receipt.closure_object_count);
    for packet in &capture.source_packets { assert_eq!(objects.read_verified(packet.digest)?, packet.bytes); }
    assert_eq!(ledger.batches()[0].children, vec![capture.receipt.capture_root]);
    Ok(())
}
#[test]
...
fn incremental_native_framing_handles_split_markers_and_multiple_frames_per_chunk() -> TestResult {
    ...
    let spec = spec()?; let generated = generate(&spec)?;
    let bytes: Vec<_> = generated.packets().iter().flat_map(|p| p.bytes.iter().copied()).collect();
        ...
        for chunk in bytes.chunks(chunk_bytes) {
            ...
            while !remaining.is_empty() {
                ...
                if let Some(frame) = step.frame {
                    assert_eq!(frame.encoded_sha256(), generated.frames()[count].encoded_digest.bytes());
                    frame.decode(ComponentInterpretation::Grayscale, DecodeLimits::default(), &mut budget)?;
                    count += 1;
                }
            }
        }
...
fn repeated_source_bytes_share_objects_without_erasing_packet_multiplicity() -> TestResult {
    let mut spec = spec()?; spec.frame_count = 1; spec.warmup_frames = 0; spec.packet_bytes = 1;
    let generated = generate(&spec)?;
