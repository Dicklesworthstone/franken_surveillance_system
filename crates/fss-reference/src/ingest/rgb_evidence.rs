...
impl Default for RgbEvidenceLimits {
    fn default() -> Self {
        Self {
            maximum_bytes: MAX_RGB_EVIDENCE_BYTES,
            maximum_source_bytes: MAX_SOURCE,
        }
    }
}
impl RgbEvidenceLimits {
    fn validate(self) -> Result<(), RgbEvidenceError> {
        if !(1..=MAX_RGB_EVIDENCE_BYTES).contains(&self.maximum_bytes)
            || !(1..=MAX_SOURCE).contains(&self.maximum_source_bytes)
        {
...
fn checkpoint(cx: &ReplayCx) -> Result<(), RgbEvidenceError> {
    cx.checkpoint("rgb-evidence:work")
        .map_err(|_| RgbEvidenceError::Cancelled)
}

/// Cumulative byte-work allowance, separate from import/decode/inference/head work.
#[derive(Debug)]
pub struct RgbEvidenceBudget {
    remaining: u64,
    used: u64,
}
impl RgbEvidenceBudget {
    ...
    fn charge(&mut self, units: usize, cx: &ReplayCx) -> Result<(), RgbEvidenceError> {
        ...
        if units > self.remaining {
            return Err(RgbEvidenceError::BudgetExceeded);
        }
...
impl RgbEvidence {
    ...
    pub fn capture(
        imported: &ImportedRgbModel<'_>,
        head: &RgbDetectionContract,
        jpeg: &[u8],
        run: &RgbDetectionRun,
        admission: RgbFrameAdmission,
        limits: RgbEvidenceLimits,
        budget: &mut RgbEvidenceBudget,
        cx: &ReplayCx,
    ) -> Result<Self, RgbEvidenceError> {
        checkpoint(cx)?;
        let inference = run.inference();
        if imported.model().digest() != inference.model_digest()
            || head.spec().model != inference.model_digest()
            || run.report().contract_digest() != head.digest()
            || run.report().inference_identity() != inference.identity()
            || run.report().source() != inference.source()
            || admission.source() != inference.source()
        {
            return Err(RgbEvidenceError::Mismatch);
        }
        budget.charge(MAX_RECIPE, cx)?; // Bound metadata cloning/encoding before allocation.
        let recipe = Recipe {
            graph: imported.graph_digest(),
            weights: imported.weights_digest(),
            spec: imported.model().spec().clone(),
            float: imported.float_policy(),
            bindings: imported.bindings().clone(),
            head: head.spec().clone(),
            source: inference.source(),
            interpretation: inference.decode_receipt().interpretation,
            availability: admission.availability(),
            admission: admission.evidence().bytes(),
            expected: [
                imported.identity(),
                head.digest(),
                inference.identity(),
                inference.masked_digest(),
        ...
        Self::from_parts(
            &bytes,
            imported.graph_source(),
            imported.weights_source(),
            jpeg,
            run.allowed(),
    ...
    pub fn decode(
        ...
        if bytes.get(..8) != Some(b"FSSRGBE1") {
        ...
        for part in &mut parts {
            ...
            let length = usize::try_from(u64::from_le_bytes(
                raw.try_into().map_err(|_| RgbEvidenceError::Format)?,
            ))
            .map_err(|_| RgbEvidenceError::Limit)?;
            at += 8;
        ...
        Self::from_parts(
            parts[0], parts[1], parts[2], parts[3], parts[4], expected, limits, budget, cx,
        )
    }
    ...
    fn from_parts(
        ...
        budget.charge(
            size.checked_mul(2)
                .and_then(|n| n.checked_add(MAX_RECIPE))
                .ok_or(RgbEvidenceError::Limit)?,
            cx,
        )?;
        if ContentDigest::sha256(recipe) != expected {
            return Err(RgbEvidenceError::Mismatch);
        }
        let r = wire::decode(recipe)?;
        if r.graph != ContentDigest::sha256(graph)
            || r.weights != ContentDigest::sha256(weights)
        ...
        let result = Self {
            identity: expected,
            recipe: copy(recipe, cx)?,
            graph: copy(graph, cx)?,
            weights: copy(weights, cx)?,
            jpeg: copy(jpeg, cx)?,
            mask: copy(mask, cx)?,
        };
        checkpoint(cx)?;
    ...
    pub fn replay(
        ...
        head_work: &mut RgbDetectionBudget,
        cx: &ReplayCx,
        scalar: &ScalarExecCx,
        ...
        let request = RgbModelImportRequest {
            ...
            weights_digest: r.weights,
            spec: r.spec,
            float_policy: r.float,
            bindings: r.bindings,
        };
        let imported = ImportedRgbModel::build(&request, limits.import, import, cx, scalar)
            .map_err(computation)?;
        let head = RgbDetectionContract::new(r.head).map_err(computation)?;
        if imported.identity() != r.expected[0]
            || head.digest() != r.expected[1]
            || imported.model().digest() != head.spec().model
        {
            return Err(RgbEvidenceError::Mismatch);
        }
        let run = {
        ...
        checkpoint(cx)?;
        scalar
            .checkpoint("rgb-evidence:verified")
            .map_err(computation)?;
        Ok(ReplayedRgbEvidence {
...
impl ReplayedRgbEvidence {
    ...
    pub fn evidence_identity(&self) -> ContentDigest {
        self.evidence
    }
    /// Reconstructed exact head vocabulary/semantics, useful to initialize a tracker.
    pub fn head(&self) -> &RgbDetectionContract {
        &self.head
    }
    ...
    pub fn run(&self) -> &RgbDetectionRun {
        &self.run
    }
...
fn admit_parts(
    ...
    mask: &[u8],
    limits: RgbEvidenceLimits,
) -> Result<usize, RgbEvidenceError> {
    limits.validate()?;
    if recipe.is_empty()
        || recipe.len() > MAX_RECIPE
        || mask.is_empty()
        || mask.len() > MAX_MASK
        || [graph, weights, jpeg]
            .iter()
            .any(|p| p.is_empty() || p.len() > limits.maximum_source_bytes)
    {
        return Err(RgbEvidenceError::Limit);
    }
    let size = [recipe, graph, weights, jpeg, mask]
        .iter()
        .try_fold(48_usize, |n, p| {
            n.checked_add(p.len()).ok_or(RgbEvidenceError::Limit)
        })?;
...
struct Recipe {
    graph: ContentDigest,
    weights: ContentDigest,
    spec: RgbModelSpec,
    float: WeightFloatPolicy,
    bindings: BTreeMap<String, String>,
    head: RgbDetectionSpec,
