...
use fss_core::{
    ...
    EvidenceDelta, LedgerAnchor, ObjectId, Plane, SensorCapsule, SensorId, SensorSourceBytesSpec,
...
use crate::ingest::annexb::{split_annexb, AnnexBError, AnnexBLimits};
use crate::ingest::mjpeg::{split_jpeg_stream, JpegSplitError, MjpegLimits};
...
impl FileIngestLimits {
    ...
    pub fn standard() -> Self {
        Self {
            ...
            mjpeg_limits: MjpegLimits::standard(),
        }
    }
...
pub enum FileIngestError {
    ...
    UnsupportedFormat { format: DetectedFileFormat },
    ...
    InvalidCaptureHint { detail: String },
    ...
    CancellationRequested { stage: &'static str },
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::FileTooLarge { path, len, max } => {
                write!(f, "file {} size {} exceeds limit {}", path.display(), len, max)
            }
            Self::SpoolCapacityExceeded { limit, required, available } => {
                write!(f, "spool capacity exceeded for {}: required {} > available {}", limit, required, available)
            }
            Self::FormatConflict { hint, detected } => {
                write!(f, "format conflict: hint {:?} != detected {:?}", hint, detected)
            }
            ...
            Self::CaptureHintAfterReceive { hint_start, receive_time } => {
                write!(f, "capture hint start {:?} > receive time {:?}", hint_start, receive_time)
            }
            Self::InvalidCaptureHint { detail } => {
                write!(f, "invalid capture hint: {}", detail)
            }
            Self::ImportPlanConflict { batch_id, detail } => {
                write!(f, "import plan conflict for {}: {}", batch_id.as_str(), detail)
            }
            Self::SegmentDigestMismatch { segment_index, expected, actual } => {
                write!(f, "segment {} digest mismatch: expected {} != actual {}", segment_index, expected, actual)
            }
...
pub fn sniff_format(bytes: &[u8]) -> Result<(DetectedFileFormat, &'static str), FileIngestError> {
    if bytes.is_empty() {
        return Err(FileIngestError::EmptyFile { path: PathBuf::new() });
    }
    ...
    Err(FileIngestError::UnknownFormat { path: PathBuf::new() })
}
...
pub fn default_adapter_identity() -> Result<AdapterIdentity, ContractError> {
    ...
    let capabilities = AdapterCapabilities::from_bits(
        AdapterCapabilities::CAN_STREAM
            | AdapterCapabilities::SUPPORTS_DEVICE_TIMESTAMP
            | AdapterCapabilities::SUPPORTS_HEALTH_METRICS,
    )?;
...
impl FileIngestAdapter {
    ...
    pub fn ingest(
        ...
        let import_identity_hex = import_identity.to_hex();
        let import_slot = SlotName::parse(format!("fi-{import_identity_hex}"))
            .map_err(|_| ContractError::InvalidIdentifier)?;
        let manifest_batch_id = BatchId::parse(format!("batch:file-import:{import_identity_hex}:manifest"))?;
        ...
        let capsule_ids: Vec<CapsuleId> = scanned.capsules.iter().map(|c| c.capsule_id.clone()).collect();
        ...
        if cx.is_cancelled() {
            cx.drain_and_finalize();
            return Err(FileIngestError::CancellationRequested { stage: STAGE_CAPACITY });
        }
        ...
        let capsule_encodings: Vec<Vec<u8>> = scanned.capsules.iter().map(|c| c.canonical_bytes()).collect();
        ...
        deployment.publisher_mut().stage_object(&custody_manifest_bytes)?;
        // Stage capsules
        ...
        for (digest, _) in &candidate_objects {
            deployment.publisher().verify_object(*digest)?;
        }
        ...
        if cx.is_cancelled() {
            cx.drain_and_finalize();
            return Err(FileIngestError::CancellationRequested { stage: STAGE_COMMIT_CAPSULES });
        }

        let overall_validity = if let (Some(first), Some(last)) = (scanned.capsules.first(), scanned.capsules.last()) {
            CaptureInterval::new(first.capture.earliest, last.capture.latest)?
        } else {
            CaptureInterval::new(TimestampNs(0), receive_time)?
        };

        let import_object_id = ObjectId::parse(format!("object:file-import:{import_identity_hex}"))?;
        ...
        let capsule_batch_id = BatchId::parse(format!("batch:file-import:{import_identity_hex}:c0"))?;
        ...
        if cx.is_cancelled() {
            cx.drain_and_finalize();
            return Err(FileIngestError::CancellationRequested { stage: STAGE_PUBLISH_ROOT });
        }
        ...
        if cx.is_cancelled() {
            cx.drain_and_finalize();
            return Err(FileIngestError::CancellationRequested { stage: STAGE_COMMIT_MANIFEST });
        }

        let final_manifest_object_id = ObjectId::parse(format!("object:file-import-manifest:{import_identity_hex}"))?;
        ...
        let final_anchor = deployment.append_batch(
        ...
        match format {
            DetectedFileFormat::AnnexB => {
                ...
                for (idx, au) in scan.access_units.iter().enumerate() {
                    ...
                    let capsule_id = CapsuleId::parse(format!(
                        "capsule:{}:{:06}",
                        import_identity_hex, idx
                    ))?;

                    let has_gap_before = (au.span.start > last_segment_end)
            ...
            DetectedFileFormat::JpegStream => {
                ...
                for frame in &scan.frames {
                    ...
                    let capsule_id = CapsuleId::parse(format!(
                        "capsule:{}:{:06}",
                        import_identity_hex, capsules.len()
                    ))?;

                    let has_gap_before = (frame.start_offset > last_segment_end)
                        || prev_was_truncated;
    ...
    pub fn fetch_segment_bytes(
        ...
        if assembled.len() != segment.len as usize {
            return Err(ContractError::InvariantViolation.into());
        }
