...
impl RetainedPackageDetection {
    ...
    #[must_use]
    pub fn boundaries(&self) -> &[PackageTrackingBoundary] {
        &self.boundaries
    }
}
...
impl PackageAnalysisReport {
    ...
    pub fn read(
        ...
        let mut tracks =
            tracking::build_tracks(record, class_index, config, retained.boundaries(), || {
    ...
    pub fn verify(
        ...
        if magic != REPORT_MAGIC || d.u32()? != 2 || d.text()? != PACKAGE_ANALYSIS_REPORT_DOMAIN {
            return Err(PackageEventError::InvalidRequest(
                "not a supported package analysis report",
            ));
        }
        if d.digest()? != ContentDigest::sha256(POLICY) {
            return Err(PackageEventError::InvalidRequest(
                "unsupported package analysis policy; regenerate report",
            ));
        }
        let report_digest = d.digest()?;
