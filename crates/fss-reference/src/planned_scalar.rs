...
use fss_model_ir::{
    MemoryPlan, MemoryPlanError, MemoryPlanLimits, ModelIrGraph, ModelIrVersion, OpCode, TensorPort,
};
...
impl From<MemoryPlanError> for PlannedExecError {
    fn from(e: MemoryPlanError) -> Self {
        Self::Planning(e)
    }
}
...
impl CompiledScalarPlan {
    ...
    pub fn compile(
        ...
        let memory = match MemoryPlan::compile_cancellable(graph, limits, || cx.is_cancelled()) {
            Err(MemoryPlanError::Cancelled) => {
                ...
                return Err(ExecError::CancellationRequested {
                    stage: "memory-plan:compile",
                }
                .into());
            }
            result => result?,
            ...
            if input.dtype() != DType::F32 {
                ...
                if !admitted || !used {
                    return Err(ExecError::UnsupportedDType {
                        expected: DType::F32,
                        actual: input.dtype(),
                        tensor_name: input.name().to_owned(),
                    }
                    .into());
                }
            }
        ...
        for value in memory.values().values() {
            if value.producer().is_some() && value.port().dtype() != DType::F32 {
                return Err(ExecError::UnsupportedDType {
                    expected: DType::F32,
                    actual: value.port().dtype(),
                    tensor_name: value.port().name().to_owned(),
                }
                .into());
            }
        }
        for output in graph.outputs() {
            if output.dtype() != DType::F32 {
                return Err(ExecError::UnsupportedDType {
                    expected: DType::F32,
                    actual: output.dtype(),
                    tensor_name: output.name().to_owned(),
                }
                .into());
            }
        }
        ...
        for step in memory.steps() {
            ...
            let unique: BTreeMap<_, _> =
                input_ports.iter().map(|p| (p.name(), p.clone())).collect();
        ...
        e.text(PLANNED_SCALAR_DOMAIN);
        e.digest(memory.digest());
        e.digest(profile);
        e.u64(macs);
        e.u64(peak_bytes as u64);
        e.u64(scratch_bytes as u64);
        for (program, temporary) in programs.iter().zip(scratch) {
            e.u64(program.macs);
            e.u64(program.cumulative_bytes as u64);
            e.u64(temporary as u64);
        }
    ...
    pub fn minimum_requirements(&self) -> ScalarMemoryRequirements {
        ScalarMemoryRequirements {
            macs: self.macs,
            peak_bytes: self.peak_bytes,
            scratch_bytes: self.scratch_bytes,
            input_backing_bytes: self.memory.input_bytes(),
        }
    }
    ...
    pub fn run<S: AsRef<str>>(
        ...
        budget: PeakExecBudget,
        cx: &ScalarExecCx,
    ) -> Result<PlannedExecOutcome, PlannedExecError> {
        cx.checkpoint("planned-scalar:admit")?;
        if inputs.len() != self.inputs.len() {
            return Err(PlannedExecError::InputSet);
        }
        let mut bindings = BTreeMap::new();
        for (name, tensor) in inputs {
            if bindings.insert(name.as_ref(), tensor).is_some() {
                return Err(PlannedExecError::InputSet);
            }
        }
        ...
        for port in &self.inputs {
            ...
            if tensor.dtype() != port.dtype()
                || tensor.shape() != port.shape()
                || tensor.generation() != port.generation()
            {
                return Err(PlannedExecError::InputContract);
            }
            ...
            extra_backing = add(
                extra_backing,
                tensor.view().storage().len().saturating_sub(bytes(port)?),
            )?;
        }
        let required = ScalarMemoryRequirements {
            macs: self.macs,
            peak_bytes: add(self.peak_bytes, extra_backing)?,
            scratch_bytes: self.scratch_bytes,
            input_backing_bytes: add(self.memory.input_bytes(), extra_backing)?,
        };
        if required.macs > budget.max_macs
            || required.peak_bytes > budget.max_peak_bytes
        ...
        for (program, step) in self.programs.iter().zip(self.memory.steps()) {
            ...
            let result = ScalarExecutor::run(
                &program.graph,
                &args,
                ExecBudget::new(program.macs, program.cumulative_bytes),
                cx,
            )?;
            if result.executed_macs() != program.macs
            ...
            executed_macs = executed_macs
                .checked_add(result.executed_macs())
                .ok_or(PlannedExecError::Overflow)?;
            let mut output = result.into_outputs();
            if output.len() != program.graph.output_count() {
                return Err(PlannedExecError::BackendMismatch);
            }
            for port in program.graph.outputs() {
                ...
                if env.insert(port.name().to_owned(), tensor).is_some() {
                    return Err(PlannedExecError::BackendMismatch);
                }
            }
            ...
            if live != add(step.live_bytes(), extra_backing)? {
                return Err(PlannedExecError::BackendMismatch);
            }
            ...
            if resident_bytes(&env)? != step.retained_bytes() {
                return Err(PlannedExecError::BackendMismatch);
            }
        }
        if executed_macs != self.macs {
            return Err(PlannedExecError::BackendMismatch);
        }
        ...
        for port in &self.outputs {
            outputs.insert(
                port.name().to_owned(),
                env.remove(port.name())
                    .ok_or(PlannedExecError::BackendMismatch)?,
            );
        }
        cx.checkpoint("planned-scalar:publish")?;
        let receipt = ScalarMemoryReceipt {
            graph: self.memory.graph_digest(),
            plan: self.digest,
            profile: self.profile,
            required,
            peak_live_tensor_bytes: peak_live,
            cumulative_tensor_bytes: self.memory.cumulative_bytes(),
            released_values: released,
        };
        Ok(PlannedExecOutcome {
            execution: ExecOutcome::new(
                outputs,
                executed_macs,
                self.memory.cumulative_bytes(),
                self.programs.len(),
            ),
            receipt,
        })
...
impl ScalarMemoryReceipt {
    ...
    pub fn graph(&self) -> ContentDigest {
        self.graph
    }
    /// Memory/work/backend plan identity.
    pub fn plan(&self) -> ContentDigest {
        self.plan
    }
    ...
    pub fn digest(&self) -> ContentDigest {
        let mut e = CanonicalEncoder::new();
        e.text("fss.reference.scalar_memory_receipt.v1");
        for digest in [self.graph, self.plan, self.profile] {
            e.digest(digest);
        }
        e.u64(self.required.macs);
        for count in [
            self.required.peak_bytes,
            self.required.scratch_bytes,
            self.required.input_backing_bytes,
            self.peak_live_tensor_bytes,
            self.cumulative_tensor_bytes,
            self.released_values,
        ] {
...
impl PlannedExecOutcome {
    ...
    pub fn execution(&self) -> &ExecOutcome {
        &self.execution
    }
    /// Peak/scratch/lifetime witness, deliberately separate from cumulative allocations.
    pub fn receipt(&self) -> &ScalarMemoryReceipt {
        &self.receipt
    }
    /// Transfer complete outputs and the resource witness to the caller.
    pub fn into_parts(self) -> (ExecOutcome, ScalarMemoryReceipt) {
        (self.execution, self.receipt)
    }
}
...
fn execution_profile() -> ContentDigest {
    let mut e = CanonicalEncoder::new();
    e.text("fss.reference.planned_scalar_profile.v1");
    for source in [
        include_bytes!("planned_scalar.rs").as_slice(),
        include_bytes!("planned_scalar/budget.rs").as_slice(),
