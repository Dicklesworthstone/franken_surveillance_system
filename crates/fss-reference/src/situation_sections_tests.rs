...
fn redaction_basis_on_known_cell_is_refused() -> Result<(), Box<dyn Error>> {
    let cell = withheld_cell("claim:misattached:known", "SECRET-misattached-redaction", Vec::new())?;
...
fn redaction_basis_on_stale_cell_is_refused() -> Result<(), Box<dyn Error>> {
    let cell = withheld_cell("claim:misattached:stale", "SECRET-misattached-redaction", Vec::new())?;
    let mut params = cell.to_params();
