...
use fss_core::{
    CanonicalEncode, CanonicalEncoder, ContentDigest, DigestAlgorithm, Generation,
    Sha256Hasher, SourceId, StreamGeneration,
};
...
impl ActivityConfig {
    ...
    pub fn new(
        ...
        let count = sample_width.checked_mul(sample_height)
            .ok_or_else(|| invalid("sample-grid size overflows"))?;
        if count == 0 || count > MAX_ACTIVITY_SAMPLES || pixel_delta == 0
            || !(1..=10_000).contains(&changed_basis_points)
        {
            return Err(invalid("invalid sample grid or activity threshold"));
        }
        Ok(Self { sample_width, sample_height, pixel_delta, changed_basis_points })
    }
...
impl ActivityReceipt {
    ...
    #[must_use]
    pub const fn decision(&self) -> ActivityDecision { self.decision }
    /// Returns statistics only when two comparable frames were evaluated.
    #[must_use]
    pub const fn measurement(&self) -> Option<ActivityMeasurement> { self.measurement }
    /// Returns the observed source-pixel identity, absent on every skip.
    #[must_use]
    pub const fn input_digest(&self) -> Option<ContentDigest> { self.input }
    /// Returns the exact previous input used in a measured comparison.
    #[must_use]
    pub const fn previous_input_digest(&self) -> Option<ContentDigest> { self.previous_input }
    /// Returns the source and interpretation basis identity.
    #[must_use]
    pub const fn basis_digest(&self) -> ContentDigest { self.basis }
    ...
    #[must_use]
    pub const fn admitted_work(&self) -> u64 { self.admitted_work }
    /// Returns the logical input/current/prior sample-buffer bound.
    #[must_use]
    pub const fn pixel_buffer_bytes(&self) -> usize { self.pixel_buffer_bytes }
    /// Activity scoring never certifies semantic absence, even for an unchanged frame.
    #[must_use]
    pub const fn supports_absence_claim(&self) -> bool { false }
    ...
    pub fn digest(&self) -> ContentDigest {
        ...
        match self.decision {
            ...
            ActivityDecision::BaselineOnly(reason) => { e.u8(3); e.u8(reason as u8); }
            ActivityDecision::NotEvaluated(reason) => { e.u8(4); e.u8(reason as u8); }
        }
        for input in [self.input, self.previous_input] {
            e.bool(input.is_some());
            if let Some(digest) = input { e.digest(digest); }
        }
        ...
        if let Some(m) = self.measurement {
            ...
            if let Some(rectangle) = m.changed_sample_box {
                for coordinate in rectangle { e.u64(coordinate as u64); }
            }
        }
...
impl ActivityGate {
    ...
    #[must_use]
    pub const fn new(config: ActivityConfig) -> Self { Self { config, state: None } }

    /// Reports the number of retained luma samples, bounded by `MAX_ACTIVITY_SAMPLES`.
    #[must_use]
    pub fn retained_samples(&self) -> usize {
        self.state.as_ref().and_then(|s| s.baseline.as_ref()).map_or(0, |b| b.pixels.len())
    }
    ...
    pub fn observe(
        ...
        match self.evaluate(frame, budget, cx) {
            Ok((receipt, state)) => { self.state = Some(state); Ok(receipt) }
            Err(error) => {
                if let Some(state) = &mut self.state { state.baseline = None; }
                Err(error)
            }
        }
    }
    ...
    fn skipped(
        &self, basis: &ActivityBasis, sequence: u64, reason: ActivitySkip,
    ) -> Result<(ActivityReceipt, GateState), ExecError> {
        self.check_sequence(basis, sequence)?;
        Ok((ActivityReceipt {
            basis: basis.digest(), config: self.config.digest(), sequence,
            decision: ActivityDecision::NotEvaluated(reason), input: None, previous_input: None,
            measurement: None, admitted_work: 0, pixel_buffer_bytes: 0,
        }, GateState { basis: basis.clone(), sequence, baseline: None }))
    }

    fn evaluate(
        &self, frame: ActivityFrame<'_>, budget: ExecBudget, cx: &ScalarExecCx,
    ) -> Result<(ActivityReceipt, GateState), ExecError> {
        ...
        if image.generation != frame.basis.decoder_generation {
            return Err(ExecError::GenerationMismatch {
                expected: frame.basis.decoder_generation, actual: image.generation,
        ...
        if image.height == 0 || image.width == 0 || !matches!(image.channels, 1 | 3) {
            return Err(invalid("expected positive HWC dimensions with one or three channels"));
        }
        let input_bytes = image.height.checked_mul(image.width)
        ...
        for dim in dims { header.u64(dim as u64); }
        ...
        for y in 0..sh {
            for x in 0..sw {
                ...
                let luma = if image.channels == 1 { image.bytes[offset] } else {
        ...
        let reset = match &self.state {
            ...
            Some(state) if state.sequence.checked_add(1) != Some(frame.sequence) => Some(ActivityReset::SequenceGap),
        ...
        let decision = if let Some(reason) = reset {
            ...
            let baseline = self.state.as_ref().and_then(|s| s.baseline.as_ref())
            ...
            let m = compare(&baseline.pixels, &samples, sw, sh, self.config.pixel_delta, cx)?;
            ...
            measurement = Some(m);
            if changed { ActivityDecision::Changed } else { ActivityDecision::BelowThreshold }
        };
        cx.checkpoint("activity:publish")?;
        Ok((ActivityReceipt {
            basis: basis_digest, config: self.config.digest(), sequence: frame.sequence,
            decision, input: Some(digest), previous_input, measurement,
            admitted_work: work, pixel_buffer_bytes: bytes,
        }, GateState {
            basis: frame.basis.clone(), sequence: frame.sequence,
            baseline: Some(Baseline { dims, pixels: samples, digest }),
        }))
    }
}

fn compare(
    prior: &[u8], current: &[u8], sw: usize, sh: usize, threshold: u8, cx: &ScalarExecCx,
    ...
    for (index, (&before, &after)) in prior.iter().zip(current).enumerate() {
        if index % 1024 == 0 { cx.checkpoint("activity:compare")?; }
    ...
    Ok(ActivityMeasurement {
        sample_width: sw, sample_height: sh, changed_samples: changed,
...
fn invalid(reason: &str) -> ExecError {
    ExecError::ShapeMismatch {
        node_id: "activity".to_owned(), op_id: ACTIVITY_DOMAIN, reason: reason.to_owned(),
    }
}
