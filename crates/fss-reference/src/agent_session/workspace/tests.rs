...
impl Fixture {
    fn new() -> Result<Self, Box<dyn Error>> {
        ...
        Ok(Self {
            sessions,
            workspaces: ReferenceWorkspaceStore::default(),
            principal,
            capsule,
        })
    }
    ...
    fn write(&mut self, write: WorkspaceWrite) -> Result<WorkspaceRevision, WorkspaceError> {
        self.workspaces
            .publish(&mut self.sessions, &self.principal, write, TimestampNs(20))
    }
    ...
    fn move_anchor(&mut self) -> Result<LedgerAnchor, Box<dyn Error>> {
        let session =
            self.sessions
                .session(&self.principal, &self.capsule.session_id, TimestampNs(20))?;
...
fn stale_writer_and_identity_reuse_cannot_overwrite() -> TestResult {
    ...
    conflicting
        .capsule
        .unknowns
        .push("unknown:competing-write".to_owned());
    assert!(matches!(
        f.write(conflicting),
        Err(WorkspaceError::StaleHead)
    ));
...
fn every_protected_omission_is_refused_atomically() -> TestResult {
    ...
    for field in 0..8 {
        ...
        assert!(matches!(
            f.write(request),
            Err(WorkspaceError::PreservationRequired)
        ));
        assert_eq!(f.workspaces.retained_bytes(), bytes);
    }
    Ok(())
...
fn rebase_is_explicit_and_retains_invalidated_actions() -> TestResult {
    ...
    assert!(matches!(
        f.write(request.clone()),
        Err(WorkspaceError::RebaseRequired)
    ));
...
fn zero_capacity_and_revision_exhaustion_are_not_unlimited() -> TestResult {
    let mut f = Fixture::new()?;
    f.workspaces = ReferenceWorkspaceStore::with_limits(WorkspaceLimits {
        max_workspaces: 0,
