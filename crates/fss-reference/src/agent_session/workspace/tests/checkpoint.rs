...
fn restore(bytes: &[u8], digest: ContentDigest) -> Result<ReferenceWorkspaceStore, WorkspaceCheckpointError> {
    ReferenceWorkspaceStore::restore_checkpoint(
        bytes, digest, WorkspaceLimits::default(), MAX_WORKSPACE_CHECKPOINT_BYTES,
    )
}
...
fn checkpoint_roundtrip_preserves_heads_history_charges_and_retries() -> TestResult {
    ...
    assert_eq!(f.workspaces.checkpoint(MAX_WORKSPACE_CHECKPOINT_BYTES)?, checkpoint);
...
fn recovery_retains_rebase_debt_obligations_and_invalidated_actions() -> TestResult {
    ...
    f.workspaces = restore(checkpoint.as_bytes(), checkpoint.digest())?;
    let resumed = f.workspaces.resume(
        &mut f.sessions, &f.principal, &f.capsule.session_id, rebased.digest(), TimestampNs(20),
    )?;
    assert_eq!(resumed.revision, rebased);
    assert_eq!(resumed.revision.invalidated_actions(), first.capsule.next_actions.as_slice());
    assert!(!resumed.rebase_required);
    assert_eq!(resumed.revision.capsule().open_obligations, first.capsule.open_obligations);
    Ok(())
}
...
fn a_valid_old_snapshot_does_not_satisfy_the_latest_pin() -> TestResult {
    ...
    assert!(matches!(restore(old.as_bytes(), current.digest()),
        Err(WorkspaceCheckpointError::IntegrityMismatch)));
    Ok(())
}
...
fn no_session_is_created_and_restored_grants_do_not_authorize_reads() -> TestResult {
    ...
    assert!(matches!(f.workspaces.resume(
        &mut empty_sessions, &f.principal, &f.capsule.session_id, first.digest(), TimestampNs(20),
    ), Err(WorkspaceError::Session(_))));
    f.sessions.close(&f.principal, &f.capsule.session_id, TimestampNs(20))?;
    assert!(matches!(f.workspaces.resume(
        &mut f.sessions, &f.principal, &f.capsule.session_id, first.digest(), TimestampNs(20),
    ), Err(WorkspaceError::Session(_))));
    Ok(())
...
fn runtime_ceilings_and_output_capacity_are_not_silently_widened() -> TestResult {
    ...
    f.write(f.initial())?;
    let checkpoint = f.workspaces.checkpoint(MAX_WORKSPACE_CHECKPOINT_BYTES)?;
    assert!(matches!(f.workspaces.checkpoint(checkpoint.as_bytes().len() - 1),
        Err(WorkspaceCheckpointError::CapacityExceeded)));
    assert!(matches!(ReferenceWorkspaceStore::restore_checkpoint(
        checkpoint.as_bytes(), checkpoint.digest(),
        WorkspaceLimits { max_revisions_per_workspace: 1, ..WorkspaceLimits::default() },
        MAX_WORKSPACE_CHECKPOINT_BYTES,
    ), Err(WorkspaceCheckpointError::CapacityExceeded)));
    assert!(matches!(ReferenceWorkspaceStore::restore_checkpoint(
        checkpoint.as_bytes(), checkpoint.digest(), WorkspaceLimits::default(), 0,
    ), Err(WorkspaceCheckpointError::CapacityExceeded)));
    Ok(())
}
...
fn rehashed_trailing_bytes_and_unknown_format_are_rejected() -> TestResult {
    ...
    assert!(matches!(restore(&unknown, ContentDigest::sha256(&unknown)),
...
fn oversized_count_is_rejected_before_record_allocation() -> TestResult {
    ...
    for limit in [limits.max_workspaces, limits.max_revisions_per_workspace,
    ...
    assert!(matches!(restore(&bytes, ContentDigest::sha256(&bytes)),
        Err(WorkspaceCheckpointError::CapacityExceeded)));
    Ok(())
}
...
fn rehashed_invalid_revision_history_is_still_refused() -> TestResult {
    for mutation in 0..8 {
        ...
        let first = f.write(f.initial())?;
        f.write(f.append(&first))?;
        let history = f.workspaces.histories.get_mut(&f.capsule.session_id)
        ...
        match mutation {
            ...
            4 => { second.capability_scope.insert("capability:forged".to_owned()); }
            5 => second.privacy_scope.clear(),
            6 => second.invalidated_actions.push("action:invented".to_owned()),
...
fn independent_replay_has_identical_checkpoint_bytes() -> TestResult {
    ...
    assert_eq!(left.workspaces.checkpoint(MAX_WORKSPACE_CHECKPOINT_BYTES)?,
        right.workspaces.checkpoint(MAX_WORKSPACE_CHECKPOINT_BYTES)?);
    Ok(())
}
