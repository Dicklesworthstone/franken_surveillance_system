...
impl From<fss_core::ContractError> for DurableSessionError {
    fn from(error: fss_core::ContractError) -> Self {
        Self::CoordinationEncoding(error)
    }
}

impl From<SessionCheckpointError> for DurableSessionError {
    fn from(error: SessionCheckpointError) -> Self {
...
impl PendingSession {
    fn kind(&self) -> u16 {
        self.record
            .as_ref()
            .map_or(SESSION_CHECKPOINT_RECORD_KIND, |(kind, _)| *kind)
    }

    fn payload(&self) -> &[u8] {
        self.record
            .as_ref()
            .map_or_else(|| self.checkpoint.as_bytes(), |(_, bytes)| bytes.as_slice())
    }
}
...
impl DurableSessionStore {
    ...
    pub fn open(
        ...
        basis: ContractBasis,
        now: TimestampNs,
    ) -> Result<AgentSession, DurableSessionError> {
    ...
    pub fn bind(
        ...
        now: TimestampNs,
    ) -> Result<SessionAlias, DurableSessionError> {
    ...
    pub fn resolve(
        ...
        catalog: &ReferenceHydrationCatalog,
        now: TimestampNs,
    ) -> Result<ResolvedSessionHandle, DurableSessionError> {
    ...
    pub fn hydrate(
        ...
        now: TimestampNs,
    ) -> Result<HydrationResponse, DurableSessionError> {
        if self.fenced {
            return Err(DurableSessionError::ReconciliationRequired);
        }
        let mut staged = catalog.clone();
        let response =
            self.transact(|store| store.hydrate(principal, alias, request, &mut staged, now))?;
        *catalog = staged;
        Ok(response)
    }
    ...
    ) -> Result<SessionAppendRecovery, DurableSessionError> {
        ...
        match self.journal.reconcile_pending(tail_policy)? {
            AppendReconciliation::Committed(record) => {
                let pending = self
                    .pending
                    .as_ref()
                    .ok_or(DurableSessionError::ReconciliationRequired)?;
    ...
    fn transact<T>(
        &mut self,
        operation: impl FnOnce(&mut ReferenceSessionStore) -> Result<T, ReferenceSessionError>,
    ) -> Result<T, DurableSessionError> {
        self.preflight()?;
...
fn check_capacity(
    ...
    let length = usize::try_from(committed)
        .ok()
    ...
    if records >= limits.max_records
        || length
            .is_none_or(|value| value > limits.max_journal_bytes.min(MAX_SESSION_JOURNAL_BYTES))
    {
...
fn read_report(
    path: &Path,
    limits: DurableSessionLimits,
) -> Result<RecoveryReport, DurableSessionError> {
    ...
    if report.records().len() > limits.max_records {
        return Err(DurableSessionError::CapacityExceeded);
    }
    Ok(report)
}
...
fn replay_all(
    ...
    for record in report.records() {
        match record.kind() {
            SESSION_CHECKPOINT_RECORD_KIND => {
                ...
                if memory
                    .as_ref()
                    .is_some_and(|previous| previous.limits != candidate.limits)
                {
                    return Err(DurableSessionError::InvalidHistory);
                }
            ...
            coordination::COORDINATION_INIT_RECORD_KIND => {
                ...
                coordination = Some(coordination::restore_initialization(
                    record.payload(),
                    sessions.checkpoint(limits.max_checkpoint_bytes)?.digest(),
                    ceiling,
                )?);
            }
            ...
            workspace::WORKSPACE_INIT_RECORD_KIND => {
                let sessions = memory.as_ref().ok_or(DurableSessionError::InvalidHistory)?;
                workspace::replay_initialization(
                    record.payload(),
                    sessions,
                    &mut workspaces,
                    limits,
    ...
    Ok(ReplayedSessionState {
        memory: memory.ok_or(DurableSessionError::InvalidHistory)?,
        coordination,
        workspaces,
    })
}
