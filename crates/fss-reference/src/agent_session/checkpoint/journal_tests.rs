...
fn restart_retains_closed_identity_and_original_opening_parameters() -> TestResult {
    ...
    assert!(matches!(
        restored.open(params()?, basis(), TimestampNs(30)),
...
fn failed_expiry_read_is_committed_before_the_refusal() -> TestResult {
    ...
    let mut store =
        DurableSessionStore::open_existing(path, root, DurableSessionLimits::default())?;
    assert!(matches!(
        store.open(params()?, basis(), TimestampNs(10)),
        Err(DurableSessionError::Session(
            ReferenceSessionError::Unavailable
        ))
    ));
...
fn exact_retry_and_same_time_read_do_not_append() -> TestResult {
    ...
    assert_eq!(
        store.session(&session.principal_id, &session.session_id, TimestampNs(10))?,
        session
    );
...
fn every_append_phase_fences_until_exact_reconciliation() -> TestResult {
    for (phase, committed) in [
        ...
        assert!(matches!(
            store.session(&session.principal_id, &session.session_id, TimestampNs(21)),
...
fn missing_or_empty_journal_is_never_an_implicit_new_store() -> TestResult {
    ...
    assert!(
        DurableSessionStore::open_existing(
            &path,
...
fn create_never_overwrites_and_reopen_rejects_a_stale_tip() -> TestResult {
    ...
    assert!(matches!(
        DurableSessionStore::open_existing(&path, old, DurableSessionLimits::default()),
...
fn byte_capacity_preflight_and_v1_record_framing_are_exact() -> TestResult {
    ...
    let path = unused_path()?;
    let too_small = DurableSessionLimits {
        max_journal_bytes: exact - 1,
        ..DurableSessionLimits::default()
    };
    assert!(matches!(
        DurableSessionStore::create(&path, too_small),
        Err(DurableSessionError::CapacityExceeded)
    ));
    assert!(!path.exists());
    let store = DurableSessionStore::create(
        &path,
        DurableSessionLimits {
            max_journal_bytes: exact,
            ..DurableSessionLimits::default()
        },
    )?;
...
fn foreign_application_records_and_torn_tails_are_not_repaired_on_open() -> TestResult {
    ...
    assert!(
        DurableSessionStore::open_existing(&path, root, DurableSessionLimits::default()).is_err()
    );
...
fn symlink_journals_are_refused() -> TestResult {
    ...
    assert!(matches!(
        DurableSessionStore::inspect(&link, DurableSessionLimits::default()),
...
fn delivered_hydration_charge_survives_restart() -> TestResult {
    ...
    assert_eq!(
        store
            .resolve(&session.principal_id, &alias, &catalog, TimestampNs(10))?
            .subject_digest,
        request.expected_subject_digest
    );
    Ok(())
}
...
fn ambiguous_hydration_never_publishes_catalog_changes_or_refunds_a_committed_charge() -> TestResult
{
    ...
    assert_eq!(
        store.reconcile_pending(IncompleteTailPolicy::Reject)?,
        SessionAppendRecovery::Committed
    );
    assert_eq!(
        store.remaining_token_budget(
