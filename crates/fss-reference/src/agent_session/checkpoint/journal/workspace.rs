...
use fss_core::{CanonicalDecoder, CanonicalEncoder, ContentDigest, PrincipalId, SessionId, TimestampNs};

use super::{DurableSessionError, DurableSessionStore, DurableSessionLimits, PendingSession,
    ReferenceSessionStore, SessionCheckpoint, read_report, replay_all};
use crate::agent_session::workspace::{ReferenceWorkspaceStore, WorkspaceError, WorkspaceLimits,
...
impl From<DurableSessionError> for DurableWorkspaceError {
    fn from(error: DurableSessionError) -> Self { Self::Durability(error) }
}
...
impl WorkspaceState {
    fn new(limits: WorkspaceLimits) -> Result<Self, DurableSessionError> {
        ...
        Ok(Self { store, requested_limits: limits, digest })
    }
}
...
impl DurableSessionStore {
    ...
    pub fn initialize_workspaces(&mut self, limits: WorkspaceLimits) -> Result<ContentDigest, DurableWorkspaceError> {
        if let Some(existing) = self.read_workspace_state()? {
            if existing.requested_limits != limits { return Err(DurableWorkspaceError::LimitsMismatch); }
        ...
        self.commit_candidate(PendingSession {
            memory: self.memory.clone(), checkpoint, coordination: None,
        ...
        -> Result<JournaledWorkspace<WorkspaceRevision>, DurableWorkspaceError>
    {
        ...
        let revision = match result {
            ...
            Err(error) => return self.finish_workspace_read(memory, checkpoint, before, Err(error)),
        };
        ...
        let payload = match encode_write(self.checkpoint_digest, before, checkpoint.digest(), state.digest,
            mode, now, revision.as_bytes())
        {
            Ok(payload) => payload,
            Err(error) => { self.fenced = true; return Err(error.into()); }
        };
        self.commit_candidate(PendingSession {
            memory, checkpoint, coordination: None, record: Some((WORKSPACE_WRITE_RECORD_KIND, payload)),
        })?;
        ...
        -> Result<JournaledWorkspace<WorkspaceResume>, DurableWorkspaceError>
    {
        ...
        let mut memory = self.memory.clone();
        let result = state.store.resume(&mut memory, principal, session, revision, now);
        let checkpoint = self.workspace_session_checkpoint(&memory)?;
    ...
    fn finish_workspace_read<T>(&mut self, memory: ReferenceSessionStore, checkpoint: SessionCheckpoint,
        digest: ContentDigest, result: Result<T, WorkspaceError>) -> Result<JournaledWorkspace<T>, DurableWorkspaceError>
    {
        if checkpoint.digest() != self.checkpoint_digest {
            self.commit_candidate(PendingSession { memory, checkpoint, coordination: None, record: None })?;
        }
        result.map(|result| self.workspace_result(result, digest)).map_err(DurableWorkspaceError::Refused)
    }

    fn workspace_session_checkpoint(&mut self, memory: &ReferenceSessionStore) -> Result<SessionCheckpoint, DurableWorkspaceError> {
        match memory.checkpoint(self.limits.max_checkpoint_bytes) {
            Ok(checkpoint) => Ok(checkpoint),
            Err(error) => { self.fenced = true; Err(DurableSessionError::from(error).into()) }
        }
    }
    ...
    fn read_workspace_state(&mut self) -> Result<Option<WorkspaceState>, DurableWorkspaceError> {
        ...
        let result = (|| -> Result<_, DurableSessionError> {
            ...
            let restored = replay_all(&report, self.limits, self.coordination.as_ref().map(|state| state.limits))?;
            if restored.memory.checkpoint(self.limits.max_checkpoint_bytes)?.digest() != self.checkpoint_digest {
                return Err(DurableSessionError::RootMismatch);
            }
        ...
        match result {
            Ok(state) => Ok(state),
            Err(error) => { self.fenced = true; Err(error.into()) }
        }
    }
}

fn checkpoint_digest(store: &ReferenceWorkspaceStore) -> Result<ContentDigest, DurableSessionError> {
    store.checkpoint(MAX_WORKSPACE_CHECKPOINT_BYTES).map(|checkpoint| checkpoint.digest())
        .map_err(|_| DurableSessionError::InvalidHistory)
}

fn encode_init(before: ContentDigest, limits: WorkspaceLimits, workspace: ContentDigest)
    -> Result<Vec<u8>, DurableSessionError>
{
    let mut e = CanonicalEncoder::new(); e.text(INIT_DOMAIN); e.digest(before);
    for value in [limits.max_workspaces, limits.max_revisions_per_workspace, limits.max_revision_bytes, limits.max_history_bytes] {
...
pub(super) fn replay_initialization(payload: &[u8], sessions: &ReferenceSessionStore,
    history: &mut Option<WorkspaceState>, limits: DurableSessionLimits) -> Result<(), DurableSessionError>
{
    if history.is_some() || payload.len() > 1024 { return Err(DurableSessionError::InvalidHistory); }
    let mut d = CanonicalDecoder::new(payload);
    if d.text()? != INIT_DOMAIN { return Err(DurableSessionError::InvalidHistory); }
    let before = d.digest()?;
    let requested = WorkspaceLimits { max_workspaces: count(&mut d)?, max_revisions_per_workspace: count(&mut d)?,
        max_revision_bytes: count(&mut d)?, max_history_bytes: count(&mut d)? };
    let digest = d.digest()?; d.ensure_finished()?;
    let state = WorkspaceState::new(requested)?;
    if before != sessions.checkpoint(limits.max_checkpoint_bytes)?.digest() || digest != state.digest
...
fn encode_write(before_sessions: ContentDigest, before_workspaces: ContentDigest,
    after_sessions: ContentDigest, after_workspaces: ContentDigest, mode: WorkspaceWriteMode,
    now: TimestampNs, revision: &[u8]) -> Result<Vec<u8>, DurableSessionError>
{
    if revision.len() > MAX_WORKSPACE_REVISION_BYTES { return Err(DurableSessionError::CapacityExceeded); }
    let mut e = CanonicalEncoder::new(); e.text(WRITE_DOMAIN);
    for digest in [before_sessions, before_workspaces, after_sessions, after_workspaces] { e.digest(digest); }
    e.u8(match mode { WorkspaceWriteMode::Advance => 0, WorkspaceWriteMode::Rebase => 1 });
    e.i128(now.0); e.bytes(revision);
    ...
    history: &mut Option<WorkspaceState>, limits: DurableSessionLimits) -> Result<(), DurableSessionError>
{
    ...
    let mut d = CanonicalDecoder::new(payload);
    if d.text()? != WRITE_DOMAIN { return Err(DurableSessionError::InvalidHistory); }
    let before_sessions = d.digest()?; let before_workspaces = d.digest()?;
    let after_sessions = d.digest()?; let after_workspaces = d.digest()?;
    let mode = match d.u8()? {
        0 => WorkspaceWriteMode::Advance, 1 => WorkspaceWriteMode::Rebase,
        _ => return Err(DurableSessionError::InvalidHistory),
    };
    let now = TimestampNs(d.i128()?); let raw = d.bytes()?; d.ensure_finished()?;
    if before_sessions != sessions.checkpoint(limits.max_checkpoint_bytes)?.digest() || before_workspaces != state.digest {
        return Err(DurableSessionError::InvalidHistory);
    }
    let recorded = decode_revision(raw, state.requested_limits).map_err(|_| DurableSessionError::InvalidHistory)?;
    let principal = PrincipalId::parse(&recorded.capsule().principal)?;
    let revision = state.store.publish(sessions, &principal, WorkspaceWrite {
        expected_head: recorded.parent_digest(), capsule: recorded.capsule().clone(), mode,
    }, now).map_err(|_| DurableSessionError::InvalidHistory)?;
    state.digest = checkpoint_digest(&state.store)?;
    if revision.as_bytes() != raw || state.digest != after_workspaces
        || sessions.checkpoint(limits.max_checkpoint_bytes)?.digest() != after_sessions
        || encode_write(before_sessions, before_workspaces, after_sessions, after_workspaces, mode, now, raw)? != payload
    { return Err(DurableSessionError::InvalidHistory); }
    Ok(())
}
