...
impl DurableSessionStore {
    ...
    pub fn rebase_workspace(
        ...
        let observed_session = match observed.session(principal, session_id, now) {
            ...
            Err(error) => {
                ...
                return self.finish_workspace_read(
                    observed,
                    checkpoint,
                    before_workspace,
                    Err(error.into()),
        ...
        if request.mode != WorkspaceWriteMode::Rebase {
            ...
            return self.finish_workspace_read(
                ...
                checkpoint,
                before_workspace,
                Err(WorkspaceError::InvalidAnchor),
            );
        }
        ...
        if let Some(digest) = retry {
            let result = state
                .store
                .resume(&mut observed, principal, session_id, digest, now)
        ...
        let outcome = candidate
            .refresh(principal, session_id, refresh.clone(), now)
        ...
        let prepared = (|| -> Result<_, DurableSessionError> {
            ...
            let record = RebaseRecord {
                before_session: self.checkpoint_digest,
                before_workspace,
                after_session: checkpoint.digest(),
                after_workspace,
                refresh,
                observed_at: now,
    ...
    fn find_rebase_retry(
        ...
        for record in report.records() {
            if record.kind() != WORKSPACE_WRITE_RECORD_KIND || !is_record(record.payload()) {
                continue;
            }
            let saved = RebaseRecord::decode(record.payload(), self.limits)?;
            if !same_refresh(&saved.refresh, refresh) {
                continue;
            }
            let revision = decode_revision(saved.revision, state.requested_limits)
                .map_err(|_| DurableSessionError::InvalidHistory)?;
            if revision.capsule() == &request.capsule
                && revision.parent_digest() == request.expected_head
            {
...
impl<'a> RebaseRecord<'a> {
    fn encode(&self, limits: DurableSessionLimits) -> Result<Vec<u8>, DurableSessionError> {
        if self.revision.len() > MAX_WORKSPACE_REVISION_BYTES {
        ...
        let count = self
            ...
            .capabilities
            .len()
            .checked_add(self.refresh.privacy_scope.len())
            .ok_or(DurableSessionError::CapacityExceeded)?;
        let bytes = self
            .refresh
            .capabilities
            .iter()
            .chain(&self.refresh.privacy_scope)
        ...
        if count
            > limits
                ...
                .min(MAX_REFRESH_GRANTS)
            || bytes
                > limits
                    .sessions
                    .max_grant_bytes_per_session
                    .min(MAX_WORKSPACE_REVISION_BYTES)
        {
            return Err(DurableSessionError::CapacityExceeded);
        }
        ...
        for digest in [
            self.before_session,
            self.before_workspace,
            self.after_session,
            self.after_workspace,
            self.refresh.expected_session_digest,
        ] {
            e.digest(digest);
        }
        ...
        e.bytes(self.revision);
        let bytes = e.finish_checked()?;
        if bytes.len() > MAX_REBASE_RECORD_BYTES {
            return Err(DurableSessionError::CapacityExceeded);
        }
        Ok(bytes)
    }
    ...
    fn decode(
        ...
        if payload.len() > MAX_REBASE_RECORD_BYTES {
            return Err(DurableSessionError::CapacityExceeded);
        }
        ...
        let before_session = d.digest()?;
        let before_workspace = d.digest()?;
        let after_session = d.digest()?;
        let after_workspace = d.digest()?;
        ...
        if record.encode(limits)? != payload {
            return Err(DurableSessionError::InvalidHistory);
        }
        Ok(record)
    }
    ...
    *remaining_count = remaining_count
    ...
    for _ in 0..count {
        ...
        *remaining_bytes = remaining_bytes
        ...
        if result
            .last()
            .is_some_and(|old: &String| old.as_str() >= text)
        {
            return Err(DurableSessionError::InvalidHistory);
        }
...
pub(in crate::agent_session::checkpoint::journal) fn replay_rebase(
    payload: &[u8],
    ...
    if record.before_session != sessions.checkpoint(limits.max_checkpoint_bytes)?.digest()
        || record.before_workspace != state.digest
        || record.before_workspace == record.after_workspace
    {
        return Err(DurableSessionError::InvalidHistory);
    }
    let saved = decode_revision(record.revision, state.requested_limits)
        .map_err(|_| DurableSessionError::InvalidHistory)?;
    let principal = PrincipalId::parse(&saved.capsule().principal)?;
    let observed = sessions
        .session(&principal, &saved.capsule().session_id, record.observed_at)
        .map_err(|_| DurableSessionError::InvalidHistory)?;
    ...
    let result = state
        ...
        .publish(
            ...
            &principal,
            WorkspaceWrite {
                capsule: saved.capsule().clone(),
                expected_head: saved.parent_digest(),
                mode: WorkspaceWriteMode::Rebase,
            },
            record.observed_at,
        )
        .map_err(|_| DurableSessionError::InvalidHistory)?;
    state.digest = checkpoint_digest(&state.store)?;
    if result.as_bytes() != record.revision
        || state.digest != record.after_workspace
