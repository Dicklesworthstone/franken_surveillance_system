...
fn change(
    ...
    request
        .capsule
        .epistemic_debt
        .extend(request.capsule.assumptions.clone());
...
fn rebase_commits_anchor_invalidation_and_workspace_in_one_record() -> TestResult {
    ...
    let result = store.rebase_workspace(
    ...
    assert_eq!(
        current.symbol_table_generation,
        session.symbol_table_generation + 1
    );
    ...
    assert_eq!(
        resumed.session_checkpoint_digest,
    ...
    assert_eq!(
        result.result.invalidated_actions(),
        first.result.capsule().next_actions.as_slice()
    );
    ...
    assert!(
        result
            ...
            .capsule()
            .epistemic_debt
            .contains(&"assumption:coverage".to_owned())
    );
    Ok(())
}
...
fn refused_rebase_discards_refresh_but_durably_preserves_observed_time() -> TestResult {
    ...
    assert!(matches!(
        store.rebase_workspace(&session.principal_id, refresh, request, TimestampNs(30)),
    ...
    assert_eq!(
        store.session(&session.principal_id, &session.session_id, TimestampNs(30))?,
        session
    );
    let resumed = store.resume_workspace(
        &session.principal_id,
...
fn exact_retry_after_restart_and_later_work_never_reruns_old_refresh() -> TestResult {
    ...
    let (refresh, request) = change(&session, &first.result);
    let rebased = store.rebase_workspace(
        &session.principal_id,
        refresh.clone(),
        request.clone(),
        TimestampNs(30),
    )?;
    let newer = store.publish_workspace(
        &session.principal_id,
        successor(&rebased.result),
        TimestampNs(30),
    )?;
    let mut store = reopen(store)?;
    let before = store.verify_storage()?;
    let retry = store.rebase_workspace(
        &session.principal_id,
        refresh.clone(),
        request.clone(),
        TimestampNs(30),
    ...
    let current = store.session(&session.principal_id, &session.session_id, TimestampNs(30))?;
    let mut future = current.current_anchor.clone();
    future.commit_sequence += 1;
    ...
    assert_eq!(
        store
            .session(&session.principal_id, &session.session_id, TimestampNs(40))?
            .current_anchor,
        future
    );
...
fn changed_refresh_is_not_a_retry_even_with_the_same_capsule() -> TestResult {
    ...
    let first = store.publish_workspace(&session.principal_id, initial, TimestampNs(20))?;
    let (refresh, request) = change(&session, &first.result);
    let done = store.rebase_workspace(
        &session.principal_id,
        refresh.clone(),
        request.clone(),
        TimestampNs(30),
    ...
    assert!(matches!(
        store.rebase_workspace(&session.principal_id, changed, request, TimestampNs(30)),
        Err(DurableWorkspaceError::Refused(
            WorkspaceError::InvalidAnchor
        ))
    ));
...
fn current_authority_is_rechecked_before_returning_a_committed_retry() -> TestResult {
    ...
    let mut store = reopen(store)?;
    assert!(matches!(
        store.rebase_workspace(&session.principal_id, refresh, request, TimestampNs(40)),
        Err(DurableWorkspaceError::Refused(WorkspaceError::Unavailable))
    ));
    Ok(())
}
...
fn expiry_during_rebase_records_tombstone_instead_of_an_anchor_move() -> TestResult {
    ...
    let (refresh, request) = change(&session, &first.result);
    assert!(matches!(
        store.rebase_workspace(&session.principal_id, refresh, request, TimestampNs(1000)),
    ...
    let mut store = reopen(store)?;
    assert!(matches!(
        store.session(&session.principal_id, &session.session_id, TimestampNs(20)),
        Err(DurableSessionError::Session(
            ReferenceSessionError::Unavailable
        ))
    ));
...
fn stale_session_cas_and_forged_anchor_cannot_mutate_the_workspace() -> TestResult {
    for variant in 0..3 {
        ...
        assert!(
            store
                .rebase_workspace(&session.principal_id, refresh, request, TimestampNs(30))
                .is_err()
        );
        let mut store = reopen(store)?;
        assert_eq!(
        ...
        assert_eq!(
            old.workspace_checkpoint_digest,
            first.workspace_checkpoint_digest
        );
    }
    Ok(())
...
fn append_cut_matrix_cannot_recover_only_one_half_of_a_rebase() -> TestResult {
    for (phase, committed) in [
        ...
        assert_eq!(
            actual.current_anchor,
            if committed {
                refresh.current_anchor.clone()
            } else {
...
fn canonical_rebase_record_truncations_and_rehashed_refresh_forgery_fail() -> TestResult {
    ...
    let revision =
        state
            .store
            .publish(&mut memory, &session.principal_id, request, TimestampNs(30))?;
    let record = RebaseRecord {
        before_session: store.checkpoint_digest,
        before_workspace,
        after_session: memory
            .checkpoint(store.limits.max_checkpoint_bytes)?
            .digest(),
        after_workspace: checkpoint_digest(&state.store)?,
        refresh,
        observed_at: TimestampNs(30),
        revision: revision.as_bytes(),
    };
    let payload = record.encode(store.limits)?;
    for end in 0..payload.len() {
        assert!(RebaseRecord::decode(&payload[..end], store.limits).is_err());
    }
    let mut trailing = payload.clone();
    trailing.push(0);
    assert!(RebaseRecord::decode(&trailing, store.limits).is_err());
    let mut forged = record;
    forged.refresh.expected_session_digest =
        ContentDigest::sha256(b"wrong-cas-with-valid-checksum");
