...
use fss_core::{AgentSession, AgentSessionParams, BudgetVector, ContractBasis, ContractBasisRegistryBytes,
    LedgerAnchor, MissionId, SessionCapsule, SessionCapsuleParams};
use fss_ledger::{AppendPhase, IncompleteTailPolicy, Journal};

use super::*;
use super::super::{SessionAppendRecovery, ReferenceSessionError, SessionRefresh};
...
fn unused_path() -> Result<PathBuf, Box<dyn Error>> {
    for _ in 0..128 {
        ...
        let path = std::env::temp_dir().join(format!("fss-workspace-journal-{}-{serial}", std::process::id()));
...
fn basis() -> ContractBasis {
    ContractBasis::from_registry_bytes(ContractBasisRegistryBytes::new(
        b"schemas", b"operations", b"views", b"capabilities", b"errors", b"costs", "fss-reference:workspace-journal-test",
    ).with_accepted_nightly("nightly-2026-08-31"))
}

fn fixture_with_limits(limits: DurableSessionLimits) -> Result<(DurableSessionStore, AgentSession, WorkspaceWrite), Box<dyn Error>> {
    ...
    let session = store.open(AgentSessionParams {
        session_id: session_id.clone(), mission_id: MissionId::parse("mission:workspace-journal")?,
        principal_id: principal.clone(), capabilities: BTreeSet::from(["capability:read".to_owned()]),
        privacy_scope: BTreeSet::from(["private:property".to_owned()]), current_anchor: anchor.clone(),
    ...
    let capsule = SessionCapsule::new(SessionCapsuleParams {
        session_id, revision: 0, principal: principal.as_str().to_owned(),
        capability_projection: vec!["capability:read".to_owned()], objective_digest: "objective:0001".to_owned(),
        base_anchor: anchor.clone(), current_anchor: anchor, situation_capsule_digest: "situation:0001".to_owned(),
        active_hypotheses: vec!["hypothesis:protected".to_owned()], assumptions: vec!["assumption:coverage".to_owned()],
        unknowns: vec!["unknown:blind-spot".to_owned()], not_observable_domains: vec!["domain:rear-gate".to_owned()],
        epistemic_debt: vec!["debt:calibration".to_owned()], open_obligations: vec!["obligation:indeterminate-alert".to_owned()],
        ...
        next_actions: vec!["action:review-before-dispatch".to_owned()], decision_digest: "decision:0001".to_owned(),
    })?;
...
fn initialization_is_one_way_and_exact_retry_never_resets_history() -> TestResult {
    ...
    assert_eq!(store.initialize_workspaces(WorkspaceLimits::default())?, first.committed_root);
    assert!(matches!(store.initialize_workspaces(WorkspaceLimits { max_workspaces: 1, ..WorkspaceLimits::default() }),
        Err(DurableWorkspaceError::LimitsMismatch)));
    let mut store = reopen(store)?;
    let resumed = store.resume_workspace(&session.principal_id, &session.session_id, first.result.digest(), TimestampNs(20))?;
...
fn publication_and_clock_are_one_record_and_recover_identically() -> TestResult {
    ...
    let mut store = reopen(store)?;
    assert!(matches!(store.resume_workspace(&session.principal_id, &session.session_id, result.result.digest(), TimestampNs(19)),
        Err(DurableWorkspaceError::Refused(WorkspaceError::Session(ReferenceSessionError::ClockRegression)))));
    let restored = store.resume_workspace(&session.principal_id, &session.session_id, result.result.digest(), TimestampNs(20))?;
    assert_eq!(restored.workspace_checkpoint_digest, result.workspace_checkpoint_digest);
    assert_eq!(restored.session_checkpoint_digest, result.session_checkpoint_digest);
...
fn lost_ack_retry_after_restart_and_later_revision_never_rewinds_head() -> TestResult {
    ...
    let second = store.publish_workspace(&session.principal_id, successor(&first.result), TimestampNs(20))?;
    let mut store = reopen(store)?;
    ...
    let old = store.resume_workspace(&session.principal_id, &session.session_id, first.result.digest(), TimestampNs(20))?;
...
fn omission_refusal_preserves_clock_without_publishing_a_revision() -> TestResult {
    let (mut store, session, request) = fixture()?;
    let first = store.publish_workspace(&session.principal_id, request, TimestampNs(20))?;
    let mut bad = successor(&first.result); bad.capsule.open_obligations.clear();
    assert!(matches!(store.publish_workspace(&session.principal_id, bad, TimestampNs(30)),
...
fn expiry_refusal_is_durable_and_never_reopens_the_session() -> TestResult {
    let (mut store, session, request) = fixture()?;
    let first = store.publish_workspace(&session.principal_id, request, TimestampNs(20))?;
    assert!(matches!(store.resume_workspace(&session.principal_id, &session.session_id, first.result.digest(), TimestampNs(1000)),
        Err(DurableWorkspaceError::Refused(WorkspaceError::Session(ReferenceSessionError::Unavailable)))));
    let mut store = reopen(store)?;
    assert!(matches!(store.resume_workspace(&session.principal_id, &session.session_id, first.result.digest(), TimestampNs(20)),
        Err(DurableWorkspaceError::Refused(WorkspaceError::Session(ReferenceSessionError::Unavailable)))));
    Ok(())
}
...
fn revoked_grants_block_restored_history_and_exact_retries() -> TestResult {
    ...
    store.refresh(&session.principal_id, &session.session_id, SessionRefresh {
        expected_session_digest: session.session_digest(), current_anchor: session.current_anchor.clone(),
        capabilities: BTreeSet::new(), privacy_scope: session.privacy_scope.clone(),
    }, TimestampNs(30))?;
    let mut store = reopen(store)?;
    assert!(matches!(store.resume_workspace(&session.principal_id, &session.session_id, first.result.digest(), TimestampNs(30)),
        Err(DurableWorkspaceError::Refused(WorkspaceError::Unavailable))));
    assert!(store.publish_workspace(&session.principal_id, request, TimestampNs(30)).is_err());
    Ok(())
}
...
fn wrong_principal_and_unknown_revision_never_disclose_history() -> TestResult {
    ...
    let wrong = store.resume_workspace(&stranger, &session.session_id, first.result.digest(), TimestampNs(30));
    let absent = store.resume_workspace(&stranger, &session.session_id, ContentDigest::sha256(b"absent"), TimestampNs(30));
    assert_eq!(wrong.err().ok_or("expected refusal")?.to_string(), absent.err().ok_or("expected refusal")?.to_string());
...
fn every_append_phase_withholds_revision_and_reconciles_both_states() -> TestResult {
        ...
        (AppendPhase::CommitWrite, true), (AppendPhase::CommitSync, true)]
    {
        ...
        let expected = reference.publish(&mut reference_sessions, &session.principal_id, request.clone(), TimestampNs(20))?;
        let before = store.committed_root();
        store.journal.fail_after_phase(phase);
        assert!(matches!(store.publish_workspace(&session.principal_id, request.clone(), TimestampNs(20)),
            Err(DurableWorkspaceError::Durability(DurableSessionError::Journal(_)))));
        assert!(store.needs_reconciliation());
        assert!(store.resume_workspace(&session.principal_id, &session.session_id, expected.digest(), TimestampNs(20)).is_err());
        if !committed {
        ...
        let resumed = store.resume_workspace(&session.principal_id, &session.session_id, expected.digest(), TimestampNs(20));
        if committed { assert_eq!(resumed?.result.revision, expected); } else { assert!(resumed.is_err()); }
        let retry = store.publish_workspace(&session.principal_id, request, TimestampNs(20))?;
...
fn capacity_failure_cannot_acknowledge_an_uncommitted_workspace() -> TestResult {
    let (mut store, session, request) = fixture_with_limits(DurableSessionLimits { max_records: 3, ..DurableSessionLimits::default() })?;
    let bytes = std::fs::read(store.path())?;
    assert!(matches!(store.publish_workspace(&session.principal_id, request, TimestampNs(20)),
...
fn rehashed_forged_after_state_and_duplicate_initialization_are_rejected() -> TestResult {
    for bad_init in [false, true] {
        ...
        let payload = if bad_init {
            encode_init(store.checkpoint_digest, WorkspaceLimits::default(), state.digest)?
        } else {
            let mut reference = state.store.clone(); let mut memory = store.memory.clone();
            let revision = reference.publish(&mut memory, &session.principal_id, request, TimestampNs(20))?;
            encode_write(store.checkpoint_digest, state.digest, ContentDigest::sha256(b"forged-session"),
                checkpoint_digest(&reference)?, WorkspaceWriteMode::Advance, TimestampNs(20), revision.as_bytes())?
        };
        let path = store.path().to_path_buf(); drop(store);
        let mut raw = Journal::open(&path, IncompleteTailPolicy::Reject)?;
        raw.append(if bad_init { WORKSPACE_INIT_RECORD_KIND } else { WORKSPACE_WRITE_RECORD_KIND }, &payload)?;
...
fn write_records_are_replayed_not_trusted_as_snapshots() -> TestResult {
    ...
    let mut reference = state.store.clone(); let mut memory = store.memory.clone();
    let revision = reference.publish(&mut memory, &session.principal_id, request, TimestampNs(20))?;
    let payload = encode_write(store.checkpoint_digest, state.digest, memory.checkpoint(store.limits.max_checkpoint_bytes)?.digest(),
        checkpoint_digest(&reference)?, WorkspaceWriteMode::Advance, TimestampNs(20), revision.as_bytes())?;
    for end in 0..payload.len() {
        let mut memory = store.memory.clone(); let mut state = Some(state.clone());
