...
use fss_core::{
    CanonicalDecoder, CanonicalEncoder, CaseId, ContentDigest, PrincipalId, SessionId, TimestampNs,
};
...
const HARD_LIMITS: WorkClaimLimits = WorkClaimLimits {
    max_claims: 16_384,
    max_revisions: 262_144,
    max_dependencies: MAX_DEPENDENCIES,
    max_lease_ns: u64::MAX,
};
...
fn read_text(
    ...
    limit: usize,
) -> Result<String, DurableSessionError> {
...
fn check_limits(
    value: WorkClaimLimits,
    ceiling: WorkClaimLimits,
...
pub(super) fn decode_initialization(
    ...
    session: ContentDigest,
    ceiling: WorkClaimLimits,
) -> Result<WorkClaimLimits, DurableSessionError> {
    if bytes.len() > MAX_COORDINATION_RECORD_BYTES {
        return Err(DurableSessionError::CapacityExceeded);
    }
    let mut decoder = CanonicalDecoder::new(bytes);
    if decoder.text()? != INIT_FORMAT {
        return Err(DurableSessionError::InvalidHistory);
    }
    let limits = WorkClaimLimits {
        max_claims: usize::try_from(decoder.u64()?)
            .map_err(|_| DurableSessionError::CapacityExceeded)?,
        max_revisions: usize::try_from(decoder.u64()?)
            .map_err(|_| DurableSessionError::CapacityExceeded)?,
        max_dependencies: usize::try_from(decoder.u64()?)
            .map_err(|_| DurableSessionError::CapacityExceeded)?,
        max_lease_ns: decoder.u64()?,
    };
    check_limits(limits, ceiling)?;
    if decoder.digest()? != session {
        return Err(DurableSessionError::InvalidHistory);
    }
    decoder.ensure_finished()?;
    if encode_initialization(limits, session)? != bytes {
        return Err(DurableSessionError::InvalidHistory);
    }
    Ok(limits)
}
...
pub(super) fn encode_request(request: &Request) -> Result<Vec<u8>, DurableSessionError> {
    ...
    match command {
        CoordinationCommand::Acquire(input) => {
            ...
            encoder.u32(
                u32::try_from(input.dependencies.len())
                    .map_err(|_| DurableSessionError::CapacityExceeded)?,
            );
            for dependency in &input.dependencies {
        ...
        CoordinationCommand::Recover {
            ...
            match recovery {
                ...
                WorkClaimRecovery::Transfer { recipient } => {
                    encoder.tag(1);
                    encoder.text(recipient.as_str());
                }
                WorkClaimRecovery::Reclaim { expires_at } => {
                    encoder.tag(2);
                    encoder.i128(expires_at.0);
                }
            }
        }
    }
    let bytes = encoder.finish_checked()?;
    if bytes.len() > MAX_COORDINATION_RECORD_BYTES - 256 {
        return Err(DurableSessionError::CapacityExceeded);
    }
    Ok(bytes)
}
...
fn decode_request(bytes: &[u8]) -> Result<Request, DurableSessionError> {
    ...
    let mut decoder = CanonicalDecoder::new(bytes);
    if decoder.text()? != REQUEST_FORMAT {
        return Err(DurableSessionError::InvalidHistory);
    }
    ...
    let command = match tag {
        0 => {
            ...
            for _ in 0..count {
                ...
                if dependencies.last().is_some_and(|prior| prior >= &value) {
                    return Err(DurableSessionError::InvalidHistory);
                }
        ...
        2 => CoordinationCommand::InspectRevision {
            claim_id,
        ...
        4 => {
            ...
            let recovery = match decoder.tag()? {
                ...
                1 => WorkClaimRecovery::Transfer {
                    recipient: SessionId::parse(read_text(&mut decoder, 128)?)?,
                },
                2 => WorkClaimRecovery::Reclaim {
...
pub(super) fn encode_record(
    ...
    if request.len() > MAX_COORDINATION_RECORD_BYTES - 256 {
        return Err(DurableSessionError::CapacityExceeded);
    }
    let mut encoder = CanonicalEncoder::new();
...
pub(super) fn decode_record(bytes: &[u8]) -> Result<Record, DurableSessionError> {
    ...
    decoder.ensure_finished()?;
    if encode_record(request_bytes, before, after, outcome)? != bytes {
        return Err(DurableSessionError::InvalidHistory);
    }
    Ok(Record {
        request,
...
pub(super) fn outcome_digest(
    result: &Result<WorkClaimRevision, WorkClaimError>,
    ...
    match result {
        Ok(revision) => {
            encoder.tag(0);
            encoder.digest(revision.digest());
        }
        Err(error) => {
