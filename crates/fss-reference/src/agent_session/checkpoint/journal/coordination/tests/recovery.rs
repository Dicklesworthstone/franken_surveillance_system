...
type FailedTransfer = (
    DurableSessionStore, AgentSession, AgentSession, WorkClaimRevision, SessionJournalInspection,
);
...
fn failed_transfer(phase: AppendPhase) -> Result<FailedTransfer, Box<dyn Error>> {
    ...
    assert!(matches!(run(&mut store, &owner, recover(&first, WorkClaimRecovery::Transfer {
        recipient: recipient.session_id.clone(),
    }), 21), Err(DurableSessionError::Journal(_))));
...
fn every_transfer_append_phase_fences_both_stores_until_exact_reconciliation() -> TestResult {
    for (phase, committed) in phases() {
        ...
        assert!(matches!(run(&mut store, &recipient, inspect("claim:a"), 22),
            Err(DurableSessionError::ReconciliationRequired)));
        assert!(matches!(store.session(&owner.principal_id, &owner.session_id, TimestampNs(22)),
            Err(DurableSessionError::ReconciliationRequired)));
        if !committed {
        ...
        assert_eq!(head.claim().lease_incarnation, if committed { 2 } else { 1 });
        ...
        if committed {
            ...
            assert!(matches!(run(&mut store, &owner, update(&head, WorkClaimUpdate::Activate), 23),
                Err(DurableSessionError::WorkClaim(WorkClaimError::StaleRevision))));
        } else {
            assert_eq!(head, first);
            let transferred = run(&mut store, &owner, recover(&first, WorkClaimRecovery::Transfer {
                recipient: recipient.session_id.clone(),
            }), 23)?;
            assert_eq!(transferred.claim().lease_incarnation, 2);
        }
        let mut store = restart(store)?;
        assert_eq!(run(&mut store, &recipient, inspect("claim:a"), 24)?.claim().lease_incarnation, 2);
...
fn cold_recovery_trims_only_incomplete_bytes_and_never_rolls_back_a_complete_transfer() -> TestResult {
    for (phase, committed) in phases() {
        ...
        let approved = DurableSessionStore::inspect_with_coordination(
            store.path(), DurableSessionLimits::default(), WorkClaimLimits::default(),
        )?;
        ...
        if committed {
            ...
            assert!(matches!(DurableSessionStore::recover_existing_with_coordination(
                &path, before.root, DurableSessionLimits::default(), WorkClaimLimits::default(),
            ...
            ), Err(DurableSessionError::Journal(JournalError::IncompleteTail { .. }))));
            assert_eq!(std::fs::read(&path)?, bytes);
        }
        let (mut store, receipt) = DurableSessionStore::recover_existing_with_coordination(
            &path, approved.root, DurableSessionLimits::default(), WorkClaimLimits::default(),
            IncompleteTailPolicy::Truncate,
        )?;
        assert_eq!(receipt.observed_file_digest(), ContentDigest::sha256(&bytes));
        assert_eq!(receipt.restored().root, approved.root);
        assert_eq!(receipt.restored().committed_bytes, approved.committed_bytes);
        assert_eq!(receipt.restored().checkpoint_digest, approved.checkpoint_digest);
        ...
        assert_eq!(receipt.discarded_bytes(), u64::try_from(bytes.len() - prefix_len)?);
        assert_eq!(receipt.discarded_tail_digest(), if committed {
            None
        } else {
        ...
        if committed {
            ...
            assert!(matches!(run(&mut store, &owner, recover(&first, WorkClaimRecovery::Transfer {
                recipient: recipient.session_id.clone(),
            }), 23), Err(DurableSessionError::WorkClaim(WorkClaimError::StaleRevision))));
        } else {
            assert_eq!(head, first);
            let transferred = run(&mut store, &owner, recover(&first, WorkClaimRecovery::Transfer {
                recipient: recipient.session_id.clone(),
...
fn uncertain_owner_close_cannot_authorize_reclaim_before_its_commit_is_resolved() -> TestResult {
    for (phase, committed) in phases() {
        ...
        let first = run(&mut store, &owner, CoordinationCommand::Acquire(request("claim:a")?), 20)?;
        store.journal.fail_after_phase(phase);
        assert!(matches!(store.close(&owner.principal_id, &owner.session_id, TimestampNs(21)),
            Err(DurableSessionError::Journal(_))));
        let reclaim = recover(&first, WorkClaimRecovery::Reclaim { expires_at: TimestampNs(950) });
        assert!(matches!(run(&mut store, &recipient, reclaim.clone(), 22),
            Err(DurableSessionError::ReconciliationRequired)));
        assert_eq!(store.reconcile_pending(IncompleteTailPolicy::Truncate)?,
            if committed { SessionAppendRecovery::Committed } else { SessionAppendRecovery::NotCommitted });
        let mut store = restart(store)?;
        ...
        if committed {
            ...
            assert!(matches!(store.session(&owner.principal_id, &owner.session_id, TimestampNs(23)),
                Err(DurableSessionError::Session(ReferenceSessionError::Unavailable))));
        } else {
            assert!(matches!(result, Err(DurableSessionError::WorkClaim(WorkClaimError::Conflict))));
...
fn cold_recovery_checks_semantic_history_before_trimming_a_torn_tail() -> TestResult {
    ...
    let command = codec::Request {
        principal: owner.principal_id, session: owner.session_id,
        command: inspect("claim:a"), now: TimestampNs(20),
    };
    let forged = codec::encode_record(&codec::encode_request(&command)?, checkpoint, checkpoint,
    ...
    assert!(raw.append(COORDINATION_COMMAND_RECORD_KIND, &forged).is_err());
    drop(raw);
    let before = std::fs::read(&path)?;
    assert!(matches!(DurableSessionStore::recover_existing_with_coordination(
        &path, root, DurableSessionLimits::default(), WorkClaimLimits::default(),
        IncompleteTailPolicy::Truncate,
...
fn capacity_failure_never_acknowledges_transfer_or_discards_the_previous_fence() -> TestResult {
    ...
    assert!(matches!(run(&mut store, &owner, recover(&first, WorkClaimRecovery::Transfer {
        recipient: recipient.session_id.clone(),
    }), 21), Err(DurableSessionError::CapacityExceeded)));
    assert!(matches!(store.session(&owner.principal_id, &owner.session_id, TimestampNs(22)),
        Err(DurableSessionError::ReconciliationRequired)));
    assert!(matches!(run(&mut store, &recipient, inspect("claim:a"), 22),
        Err(DurableSessionError::ReconciliationRequired)));
...
fn legacy_cold_recovery_preserves_session_closure_without_enabling_claims() -> TestResult {
    ...
    let (mut store, receipt) = DurableSessionStore::recover_existing(
        &path, root, DurableSessionLimits::default(), IncompleteTailPolicy::Truncate,
    )?;
    assert_eq!(receipt.discarded_bytes(), 4);
    assert_eq!(std::fs::read(&path)?, prefix);
    assert!(matches!(store.open(input("session:legacy")?, basis(), TimestampNs(30)),
        Err(DurableSessionError::Session(ReferenceSessionError::Unavailable))));
    assert!(matches!(run(&mut store, &owner, inspect("claim:a"), 30),
        Err(DurableSessionError::InvalidHistory)));
    Ok(())
}
...
fn cold_recovery_neither_creates_missing_files_nor_initializes_empty_ones() -> TestResult {
    ...
    assert!(DurableSessionStore::recover_existing_with_coordination(
        &path, root, DurableSessionLimits::default(), WorkClaimLimits::default(),
        IncompleteTailPolicy::Truncate,
    ).is_err());
    assert!(!path.exists());
    OpenOptions::new().write(true).create_new(true).open(&path)?;
    assert!(DurableSessionStore::recover_existing(
        &path, root, DurableSessionLimits::default(), IncompleteTailPolicy::Truncate,
    ).is_err());
...
fn cold_recovery_refuses_symlinks_without_touching_the_target() -> TestResult {
    ...
    assert!(matches!(DurableSessionStore::recover_existing_with_coordination(
        &link, root, DurableSessionLimits::default(), WorkClaimLimits::default(),
        IncompleteTailPolicy::Truncate,
