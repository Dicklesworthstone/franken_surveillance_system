...
impl InvestigationValidity {
    ...
    #[must_use]
    pub const fn inherited_roots(&self) -> &BTreeSet<ContentDigest> { &self.inherited_roots }
    ...
    #[must_use]
    pub fn needs_readmission(&self, hypothesis: &str, evidence: ContentDigest, contradicts: bool) -> bool {
        self.inherited_roots.contains(&evidence) && !self.readmissions.contains_key(
            &InvestigationCitation { hypothesis: hypothesis.to_owned(), evidence, contradicts })
    }
}
...
impl CanonicalEncode for InvestigationValidity {
    fn encode_canonical(&self, e: &mut CanonicalEncoder) {
        ...
        for root in &self.inherited_roots { e.digest(*root); }
...
impl ReferenceInvestigationStore {
    ...
    pub fn evolve(
        ...
        session_id: &SessionId, request: &InvestigationEvolutionRequest, now: TimestampNs,
    ) -> Result<InvestigationRevision, InvestigationError> {
        validate_request(request)?;
        let mut next = self.execute(sessions, principal, session_id,
            &InvestigationCommand::Inspect { case_id: request.case_id.clone(), revision: None }, now)?;
        if next.digest() != request.expected { return Err(InvestigationError::StaleRevision); }
        if !open_state(next.record.state) || next.control.is_terminal() {
            return Err(InvestigationError::InvalidTransition);
        }
        if now.0 >= next.record.decision_deadline_ns { return Err(InvestigationError::DeadlineElapsed); }
        ...
        let entry = sessions.sessions.get(session_id).ok_or(InvestigationError::Unavailable)?;
        match &request.change {
            InvestigationEvolution::Rebase { anchor, witness } => {
                let old = &next.record.basis_anchor;
                if &entry.session.current_anchor != anchor || entry.basis != next.record.contract_basis
                    || old.site_lineage != anchor.site_lineage || old.ledger_epoch != anchor.ledger_epoch
                ...
                let inherited_roots = next.record.hypotheses.iter()
                    .flat_map(|h| h.evidence.iter().chain(&h.contradictions)).copied().collect();
                next.validity = Some(InvestigationValidity {
                    source_revision: request.expected, source_anchor: old.clone(), witness: *witness,
                    inherited_roots, readmissions: BTreeMap::new(),
                });
                next.record.basis_anchor = anchor.clone();
                for h in &mut next.record.hypotheses { invalidate_positive(&mut h.epistemic_state); }
                for s in next.record.knowns.iter_mut().chain(&mut next.record.unknowns) {
            ...
            InvestigationEvolution::ReadmitCitation { citation, witness } => {
                ...
                let h = next.record.hypotheses.iter().find(|h| h.hypothesis_id == citation.hypothesis)
                    .ok_or(InvestigationError::InvalidRecord)?;
                let roots = if citation.contradicts { &h.contradictions } else { &h.evidence };
                let validity = next.validity.as_mut().ok_or(InvestigationError::InvalidTransition)?;
                if !roots.contains(&citation.evidence) || !validity.inherited_roots.contains(&citation.evidence) {
                    return Err(InvestigationError::EvidenceRequired);
                }
                if validity.readmissions.contains_key(citation) { return Err(InvestigationError::Conflict); }
                validity.readmissions.insert(citation.clone(), *witness);
                next.assessment = Some(*witness);
            }
            InvestigationEvolution::Expand { hypothesis, discriminator, probe } => {
                Self::basis(&next.record, &entry.session, &entry.basis)?;
                if next.record.hypotheses.iter().any(|h| h.hypothesis_id == hypothesis.hypothesis_id)
                    || next.record.discriminators.iter().any(|d| d.discriminator_id == discriminator.discriminator_id)
                { return Err(InvestigationError::Conflict); }
                if next.record.hypotheses.len() >= 64 || next.record.discriminators.len() >= 128
                ...
                next.record.discriminators.push(discriminator.as_ref().clone());
                if !next.record.probes.contains(probe) { next.record.probes.push(probe.clone()); }
        ...
        let current = self.entries.get_mut(&request.case_id).ok_or(InvestigationError::Unavailable)?;
...
fn invalidate_positive(state: &mut KnowledgeState) {
    if matches!(state, KnowledgeState::Known | KnowledgeState::Estimated) { *state = KnowledgeState::Stale; }
}

fn make_control(record: &InvestigationState, prior: Option<&InvestigationCaseState>)
    -> Result<InvestigationCaseState, InvestigationError>
{
    let ids = record.hypotheses.iter().map(|h| h.hypothesis_id.clone()).collect();
    let mut control = InvestigationCaseState::create(record.investigation_id.clone(), record.mission_id.clone(), &ids)
    ...
    if let Some(prior) = prior {
        for (id, disposition) in prior.hypotheses() {
            if *disposition != HypothesisDisposition::Live {
                control.advance_hypothesis(id, *disposition).map_err(|_| InvestigationError::InvalidTransition)?;
            }
        }
    }
    Ok(control)
...
pub(super) fn validate_request(request: &InvestigationEvolutionRequest) -> Result<(), InvestigationError> {
    let id = |s: &str| CaseId::parse(s).map(|_| ()).map_err(|_| InvestigationError::InvalidRecord);
    let text = |s: &str, bound: usize| if s.is_empty() || s.len() > bound {
        Err(InvestigationError::InvalidRecord)
    } else { Ok(()) };
    id(&request.case_id)?;
    match &request.change {
        InvestigationEvolution::Rebase { anchor, .. } => {
            let bytes = anchor.try_canonical_bytes().map_err(|_| InvestigationError::InvalidRecord)?;
            if bytes.len() > 4_096 { return Err(InvestigationError::CapacityExceeded); }
        }
        InvestigationEvolution::ReadmitCitation { citation, .. } => id(&citation.hypothesis)?,
        InvestigationEvolution::Expand { hypothesis: h, discriminator: d, probe } => {
            id(&h.hypothesis_id)?; id(&d.discriminator_id)?; id(probe)?;
            text(&h.description, 8_192)?; text(&d.description, 1_024)?;
            if h.epistemic_state != KnowledgeState::Unknown || !h.evidence.is_empty() || !h.contradictions.is_empty()
                || h.predictions.is_empty() || d.separates.len() < 2 || d.expected_outcomes.len() != d.separates.len()
                || !d.separates.contains(&h.hypothesis_id)
            { return Err(InvestigationError::InvalidRecord); }
            if h.predictions.len() > 128 || d.separates.len() > 64 { return Err(InvestigationError::CapacityExceeded); }
            for p in &h.predictions { text(p, 1_024)?; }
            for s in &d.separates { id(s)?; }
            for outcome in &d.expected_outcomes { text(outcome, 1_024)?; }
