...
pub(super) fn record_fields(r: &InvestigationState) -> Result<(), InvestigationError> {
    ...
    for s in r.knowns.iter().chain(&r.unknowns) {
        id(&s.statement_id)?;
        if !statements.insert(&s.statement_id) {
            return Err(InvestigationError::InvalidRecord);
        }
        text(&s.text, 8_192)?;
    ...
    for d in &r.discriminators {
        ...
        if !discriminators.insert(&d.discriminator_id) {
            return Err(InvestigationError::InvalidRecord);
        }
        ...
        unique(&d.separates)?;
        if d.separates.len() != d.expected_outcomes.len() {
            return Err(InvestigationError::InvalidRecord);
        }
        text_bytes += d.description.len();
        for h in &d.separates {
            id(h)?;
            text_bytes += h.len();
        }
        for outcome in &d.expected_outcomes {
            text(outcome, 1_024)?;
            text_bytes += outcome.len();
        }
    }
    unique(&r.probes)?;
    unique(&r.stop_rules)?;
    for probe in &r.probes {
        id(probe)?;
        text_bytes += probe.len();
    }
    for rule in &r.stop_rules {
        text(rule, 1_024)?;
        text_bytes += rule.len();
    }
