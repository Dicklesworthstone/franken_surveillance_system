...
impl Fixture {
    ...
    fn open(&mut self) -> Result<InvestigationRevision, Box<dyn Error>> {
        ...
        if let InvestigationCommand::Open { record, .. } = &mut command {
            ...
            record.knowns.push(KnownStatement {
                statement_id: "known:continuity".to_owned(),
                text: "Continuity was observed".to_owned(),
    ...
    fn change(
        ...
        self.run(&InvestigationCommand::Change {
    ...
    fn advance(&mut self) -> Result<LedgerAnchor, Box<dyn Error>> {
        let session =
            self.sessions
                .session(&self.p.principal_id, &self.p.session_id, TimestampNs(10))?;
    ...
    fn rebase(
        &mut self,
        head: &InvestigationRevision,
    ) -> Result<InvestigationRevision, Box<dyn Error>> {
        let anchor = self.advance()?;
        Ok(self.evolve(
            head,
            InvestigationEvolution::Rebase {
                anchor,
                witness: root("basis changed"),
            },
        )?)
    }
}
...
pub(super) fn expansion() -> InvestigationEvolution {
    InvestigationEvolution::Expand {
        hypothesis: Box::new(CaseHypothesis {
            ...
            description: "A third explanation".to_owned(),
            epistemic_state: KnowledgeState::Unknown,
            predictions: vec!["new observation".to_owned()],
            evidence: vec![],
            contradictions: vec![],
        }),
        discriminator: Box::new(CaseDiscriminator {
            discriminator_id: "discriminator:c".to_owned(),
...
fn readmit(h: &str, evidence: ContentDigest, contradicts: bool) -> InvestigationEvolution {
    InvestigationEvolution::ReadmitCitation {
        ...
        witness: root("owner readmission receipt"),
    }
}
fn assess(
    h: &str,
...
fn rebase_preserves_evidence_residuals_deadline_and_exact_predecessor() -> TestResult {
    ...
    assert_eq!(
        next.record.decision_deadline_ns,
        prior.record.decision_deadline_ns
    );
    assert_eq!(next.record.unknowns, prior.record.unknowns);
    assert_eq!(
        next.record.hypotheses[0].evidence,
        prior.record.hypotheses[0].evidence
    );
    assert_eq!(
        next.record.hypotheses[1].contradictions,
        prior.record.hypotheses[1].contradictions
    );
    assert_eq!(next.record.knowns[0].epistemic_state, KnowledgeState::Stale);
    assert_eq!(
        next.record.hypotheses[0].epistemic_state,
    ...
    assert!(
        next.control
            .hypotheses()
            .values()
            .all(|d| *d == HypothesisDisposition::Live)
    );
    ...
    let historical = f.run(&InvestigationCommand::Inspect {
        case_id: prior.record.investigation_id.clone(),
        revision: Some(supported.digest()),
    })?;
...
fn repeated_cite_does_not_launder_inherited_support_after_rebase() -> TestResult {
    ...
    assert_eq!(
        supported.record.hypotheses[0].epistemic_state,
        KnowledgeState::Stale
    );
    assert_eq!(
        supported
...
fn inherited_root_cannot_move_to_another_hypothesis_or_side_to_escape_invalidation() -> TestResult {
    ...
    let attached = f.change(
        &admitted,
        InvestigationChange::Cite {
            hypothesis: "hypothesis:b".to_owned(),
            evidence: root("old"),
            contradicts: false,
        },
    )?;
    ...
    let current = f.run(&InvestigationCommand::Inspect {
        case_id: "case:window".to_owned(),
        revision: None,
    })?;
    let opposite = f.change(
        &current,
        InvestigationChange::Cite {
            hypothesis: "hypothesis:a".to_owned(),
            evidence: root("old"),
    ...
    assert_eq!(
        f.change(
            &opposite,
            assess("hypothesis:a", root("old"), HypothesisDisposition::Refuted)
        ),
        Err(InvestigationError::EvidenceRequired)
    );
    Ok(())
...
fn new_alternative_preserves_prior_refutation_and_blocks_premature_conclusion() -> TestResult {
    ...
    assert_eq!(
        expanded.record.hypotheses[..2],
        refuted.record.hypotheses[..]
    );
    assert_eq!(
        expanded.control.hypotheses().get("hypothesis:b"),
        Some(&HypothesisDisposition::Refuted)
    );
    assert_eq!(
        expanded.control.hypotheses().get("hypothesis:c"),
...
fn expansion_refuses_fake_knowledge_duplicate_ids_and_non_discriminating_probes() -> TestResult {
    ...
    for mutation in 0..6 {
        ...
        if let InvestigationEvolution::Expand {
            hypothesis: h,
            discriminator: d,
            ..
        } = &mut candidate
    ...
    assert_eq!(
        f.evolve(&expanded, expansion()),
        Err(InvestigationError::Conflict)
    );
    Ok(())
}
...
fn rebase_requires_the_actual_forward_session_anchor_and_unchanged_contract() -> TestResult {
    ...
    for anchor in [old.record.basis_anchor.clone(), wrong] {
        assert_eq!(
            ...
            Err(InvestigationError::StaleBasis)
        );
    }
    let entry = f
        .sessions
        .sessions
        .get_mut(&f.p.session_id)
...
fn rebased_conclusion_must_acknowledge_stale_previously_known_premises() -> TestResult {
    ...
    let conclude = |residual_unknowns| InvestigationChange::Conclude {
        refuted: false,
        stop_rule: "bounded assessment".to_owned(),
        assessment: root("conclusion"),
        residual_unknowns,
    ...
    assert_eq!(
        f.change(&r, conclude(residuals.clone())),
    ...
    assert_eq!(
        ended.record.knowns[0].epistemic_state,
        KnowledgeState::Stale
    );
    Ok(())
}
...
fn all_nine_knowledge_states_are_preserved_or_conservatively_invalidated() -> TestResult {
    for state in [
        ...
        assert_eq!(
            old.digest(),
            old.canonical_digest(INVESTIGATION_REVISION_DOMAIN)
        );
        let new = f.rebase(&old)?;
        let expected = if matches!(state, KnowledgeState::Known | KnowledgeState::Estimated) {
            KnowledgeState::Stale
        } else {
            state
        };
