...
fn unused_path() -> Result<PathBuf, Box<dyn Error>> {
    for _ in 0..128 {
        ...
        let p = std::env::temp_dir().join(format!("fss-source-citation-{}-{n}.journal", std::process::id()));
...
fn fixture(tokens: u64) -> Result<Fixture, Box<dyn Error>> { fixture_privacy(tokens, "private:property") }
fn fixture_privacy(tokens: u64, privacy: &str) -> Result<Fixture, Box<dyn Error>> {
    ...
    let root = source.publish_manifest(ObjectManifest::new("source", [subject], None)?)?.root;
    let levels = BTreeSet::from([HydrationLevel::H0, HydrationLevel::H1, HydrationLevel::H2, HydrationLevel::H3]);
    let cost = BudgetVector::builder().tokens(tokens).bytes(1_024).build()?;
    let handle = SemanticHandle::publish(SemanticHandleSpec {
        contract_basis: basis(), anchor: p.current_anchor.clone(), subject_id: "subject:source".to_owned(),
        subject_digest: subject, semantic_type: "source_object".to_owned(), source_id: "sensor:source".to_owned(),
        capture_interval: None, spatial_scope: None, privacy_class: privacy.to_owned(), applied_transform: None,
        availability: HandleAvailability::Available, retention_until: TimestampNs(900),
        required_capabilities: levels.iter().map(|l| (*l, BTreeSet::from(["capability:source".to_owned()]))).collect(),
        estimated_costs: levels.iter().map(|l| (*l, cost)).collect(), levels,
    ...
    let alias = store.bind(&p.principal_id, &SessionBindingRequest {
        session_id: p.session_id.clone(), generation: 0, handle_id: handle.handle_id.clone(),
        descriptor_digest: handle.descriptor_digest,
    }, &catalog, TimestampNs(10))?;
    let revision = store.investigate(&p.principal_id, &p.session_id, opening()?, TimestampNs(10))?;
    let target = SourceCitationTarget { case_id: revision.record().investigation_id.clone(),
        expected: revision.digest(), hypothesis: "hypothesis:a".to_owned(), contradicts: false };
    let request = HydrationRequest::publish(HydrationRequestSpec {
        contract_basis: basis(), session_id: p.session_id.clone(), handle_id: handle.handle_id.clone(),
        expected_descriptor_digest: handle.descriptor_digest, expected_subject_digest: subject,
        anchor: p.current_anchor.clone(), requested_level: HydrationLevel::H3, allow_lower_level: false,
        available_capabilities: p.capabilities.clone(), authorized_privacy_classes: p.privacy_scope.clone(),
        budget: cost, purpose: HydrationPurpose::Routine, continuation: None, issued_at: TimestampNs(10),
    })?;
    Ok(Fixture { store, catalog, source, p, alias, handle, target, request })
}

impl Fixture {
    fn cite(&mut self) -> Result<SourceCitationReceipt, SourceCitationError> {
        self.store.cite_investigation_source(&self.p.principal_id, &self.alias, &self.target,
            &self.request, &mut self.catalog, &self.source, TimestampNs(10))
    }
    fn head(&self) -> Result<&InvestigationRevision, Box<dyn Error>> {
        Ok(&self.store.coordination.as_ref().ok_or("coordination")?.cases.as_ref().ok_or("cases")?
            .entries.get(&self.target.case_id).ok_or("case")?.head)
    }
    fn remaining(&mut self) -> Result<u64, Box<dyn Error>> {
        Ok(self.store.remaining_token_budget(&self.p.principal_id, &self.p.session_id, TimestampNs(10))?)
    }
}

struct Probe<'a> { store: &'a InMemoryObjectStore, calls: Cell<usize>, wrong: bool }
impl PublishedSourceReader for Probe<'_> {
    fn read_published_source(&self, root: ContentDigest, subject: ContentDigest, max: u64)
        -> Result<Vec<u8>, SourceHydrationError>
    {
        self.calls.set(self.calls.get() + 1);
        if self.wrong { Ok(b"not the original source".to_vec()) }
        else { self.store.read_published_source(root, subject, max) }
    }
}
fn reseal(r: &mut HydrationRequest) {
    r.request_digest = r.computed_digest(); r.request_id = format!("hydration-request:{}", r.request_digest);
}

#[test]
fn exact_source_citation_binds_charge_and_case_without_copying_footage_or_promoting_truth() -> TestResult {
    for contradicts in [false, true] {
        let mut f = fixture(3)?; f.target.contradicts = contradicts;
        let old = f.head()?.clone();
        let result = f.cite()?;
        assert_eq!(result.acquisition().subject_digest(), f.handle.subject_digest);
        ...
        assert_eq!(if contradicts { h.contradictions.as_slice() } else { h.evidence.as_slice() }, &[f.handle.subject_digest]);
        ...
        assert_eq!(acquired.charge_root, report.records()[report.records().len() - 2].root());
...
fn stale_target_bad_hypothesis_wrong_principal_and_downgrade_fail_before_io() -> TestResult {
    for case in 0..6 {
        let mut f = fixture(3)?;
        let principal = if case == 2 { PrincipalId::parse("principal:other")? } else { f.p.principal_id.clone() };
        match case {
            0 => f.target.expected = ContentDigest::sha256(b"stale"),
            1 => f.target.hypothesis = "hypothesis:missing".to_owned(),
            3 => { f.request.requested_level = HydrationLevel::H2; reseal(&mut f.request); }
            4 => { f.request.allow_lower_level = true; reseal(&mut f.request); }
            5 => f.target.case_id = "case:missing".to_owned(),
            _ => {}
        }
        let root = f.store.committed_root();
        let probe = Probe { store: &f.source, calls: Cell::new(0), wrong: false };
        assert!(matches!(f.store.cite_investigation_source(&principal, &f.alias, &f.target,
            &f.request, &mut f.catalog, &probe, TimestampNs(10)), Err(SourceCitationError::Preflight(_))));
...
fn source_privacy_must_match_case_even_when_the_session_holds_both_domains() -> TestResult {
    ...
    let probe = Probe { store: &f.source, calls: Cell::new(0), wrong: false };
    assert!(matches!(f.store.cite_investigation_source(&f.p.principal_id, &f.alias, &f.target,
        &f.request, &mut f.catalog, &probe, TimestampNs(10)), Err(SourceCitationError::Preflight(_))));
    assert_eq!(probe.calls.get(), 0);
...
fn source_substitution_and_revoked_alias_cannot_attach_or_charge_evidence() -> TestResult {
    let mut f = fixture(3)?;
    let probe = Probe { store: &f.source, calls: Cell::new(0), wrong: true };
    assert!(matches!(f.store.cite_investigation_source(&f.p.principal_id, &f.alias, &f.target,
        &f.request, &mut f.catalog, &probe, TimestampNs(10)), Err(SourceCitationError::Acquisition(_))));
    assert_eq!(probe.calls.get(), 1);
    assert_eq!(f.head()?.digest(), f.target.expected);
    assert_eq!(f.remaining()?, 100);
    let s = f.store.session(&f.p.principal_id, &f.p.session_id, TimestampNs(10))?;
    f.store.refresh(&f.p.principal_id, &f.p.session_id, SessionRefresh {
        expected_session_digest: s.session_digest(), current_anchor: s.current_anchor,
        capabilities: BTreeSet::from([super::super::super::CAPABILITY_INVESTIGATE.to_owned()]),
        privacy_scope: s.privacy_scope,
    }, TimestampNs(10))?;
    let probe = Probe { store: &f.source, calls: Cell::new(0), wrong: false };
    assert!(f.store.cite_investigation_source(&f.p.principal_id, &f.alias, &f.target,
        &f.request, &mut f.catalog, &probe, TimestampNs(10)).is_err());
    assert_eq!(probe.calls.get(), 0);
    Ok(())
}
...
fn restart_preserves_citation_and_stale_retry_cannot_charge_twice() -> TestResult {
    let mut f = fixture(3)?;
    let receipt = f.cite()?;
    let root = f.store.committed_root(); let path = f.store.path().to_path_buf();
    drop(f.store);
    f.store = DurableSessionStore::open_existing_with_coordination(path, root,
...
fn acquisition_uncertainty_fences_without_publishing_a_case_citation() -> TestResult {
    for (phase, committed) in phases() {
        ...
        assert!(matches!(f.cite(), Err(SourceCitationError::Acquisition(DurableSourceHydrationError::Durability(_)))));
...
fn link_uncertainty_and_cold_recovery_never_erase_a_complete_case_change() -> TestResult {
    for (phase, committed) in phases() {
        for cold in [false, true] {
            ...
            assert_eq!(f.head()?.digest(), f.target.expected);
            if cold {
                let path = f.store.path().to_path_buf(); drop(f.store);
                let tip = fss_ledger::inspect(&path)?.last_root();
                if committed {
                    let before = std::fs::read(&path)?;
                    assert!(DurableSessionStore::recover_existing_with_coordination(&path, previous,
                        DurableSessionLimits::default(), WorkClaimLimits::default(), IncompleteTailPolicy::Truncate).is_err());
                    assert_eq!(std::fs::read(&path)?, before);
                }
                f.store = DurableSessionStore::recover_existing_with_coordination(path, tip,
                    DurableSessionLimits::default(), WorkClaimLimits::default(), IncompleteTailPolicy::Truncate)?.0;
            } else {
                let outcome = f.store.reconcile_pending(IncompleteTailPolicy::Truncate)?;
                assert_eq!(outcome == SessionAppendRecovery::Committed, committed);
            }
...
fn link_capacity_failure_retains_the_committed_charge_and_reports_partial_completion() -> TestResult {
    ...
    let acquired = match f.cite() {
        Err(SourceCitationError::Link { acquisition, cause: DurableInvestigationError::Durability(
            DurableSessionError::CapacityExceeded) }) => acquisition,
        _ => return Err("expected charged but unlinked capacity failure".into()),
    };
    assert_eq!(f.store.committed_root(), acquired.charged_root());
    assert_eq!(f.head()?.digest(), f.target.expected);
    let path = f.store.path().to_path_buf(); let root = f.store.committed_root(); drop(f.store);
    f.store = DurableSessionStore::open_existing_with_coordination(path, root,
        DurableSessionLimits::default(), WorkClaimLimits::default())?;
    assert_eq!(f.remaining()?, 97);
    assert_eq!(f.head()?.digest(), f.target.expected);
...
fn source_tombstone_after_citation_is_not_bypassed_by_retained_case_history() -> TestResult {
    let mut f = fixture(3)?;
    let result = f.cite()?; f.target.expected = result.revision().digest();
    let prior = Generation::parse_positive(1)?;
    let proof = f.source.put_verified(b"authorized deletion receipt")?;
    let tombstone = TombstoneRecord::new(ObjectId::parse("source:deleted")?, prior.next()?, prior,
        TombstoneReason::Deleted, Some(proof), f.handle.subject_digest)?;
...
fn forged_charge_root_alias_or_revision_is_rejected_before_tail_truncation() -> TestResult {
    for mutation in 0..3 {
        let mut f = fixture(3)?; let mut receipt = f.cite()?;
        ...
        let root = forged.last_root(); drop(forged);
...
fn every_truncated_record_prefix_trailing_byte_and_noncanonical_flag_is_refused() -> TestResult {
    let mut f = fixture(3)?; let receipt = f.cite()?;
    ...
    assert_eq!(target, receipt.target); assert_eq!(acquired, receipt.acquisition);
    assert_eq!(revision, receipt.revision.digest()); assert_eq!(digest, receipt.digest());
    assert_eq!(after, f.store.checkpoint_digest);
    let mut flag = bytes;
    let mut e = CanonicalEncoder::new(); e.text(RECORD); e.text(&target.case_id);
    e.digest(target.expected); e.text(&target.hypothesis);
