...
use super::*;
use super::super::{CoordinationState, COORDINATION_COMMAND_RECORD_KIND};
use super::super::super::{DurableSessionError, DurableSessionLimits, DurableSessionStore, PendingSession};
...
impl DurableSessionStore {
    ...
    pub fn enable_investigations(&mut self, limits: InvestigationLimits)
        -> Result<(), DurableInvestigationError>
    {
        self.preflight()?;
        limits.validate().map_err(DurableInvestigationError::Refused)?;
        let current = self.coordination.as_ref().ok_or(DurableSessionError::InvalidHistory)?;
        if let Some(cases) = &current.cases {
            return if cases.limits == limits { Ok(()) }
                else { Err(DurableSessionError::InvalidHistory.into()) };
        }
        let mut state = current.fork();
        state.cases = Some(ReferenceInvestigationStore::new(limits)
        ...
        self.commit_candidate(PendingSession { memory: self.memory.clone(), checkpoint,
            coordination: Some(state), record: Some((COORDINATION_COMMAND_RECORD_KIND, payload)) })?;
        Ok(())
    }
    ...
    pub fn investigate(
        ...
        command: InvestigationCommand, now: TimestampNs,
    ) -> Result<InvestigationRevision, DurableInvestigationError> {
        self.preflight()?;
        let request = codec::Request { principal: principal.clone(), session: session.clone(), command, now };
...
pub(in crate::agent_session::checkpoint::journal::coordination) fn replay_record(
    ...
    match d.text()? {
        INIT => {
            if state.cases.is_some() { return Err(DurableSessionError::InvalidHistory); }
            let case_limits = InvestigationLimits {
                max_cases: usize::try_from(d.u64()?).map_err(|_| DurableSessionError::CapacityExceeded)?,
                max_revisions: usize::try_from(d.u64()?).map_err(|_| DurableSessionError::CapacityExceeded)?,
                max_retained_bytes: usize::try_from(d.u64()?).map_err(|_| DurableSessionError::CapacityExceeded)?,
            };
            ...
            state.cases = Some(ReferenceInvestigationStore::new(case_limits)
        ...
        RECORD => {
            ...
            let cases = state.cases.as_mut().ok_or(DurableSessionError::InvalidHistory)?;
            let result = cases.execute(sessions, &request.principal, &request.session,
                &request.command, request.now);
            if outcome(&result) != expected || after != sessions.checkpoint(limits.max_checkpoint_bytes)?.digest() {
                return Err(DurableSessionError::InvalidHistory);
            }
        }
...
fn outcome(result: &Result<InvestigationRevision, InvestigationError>) -> ContentDigest {
    ...
    match result {
        Ok(revision) => { e.tag(0); e.digest(revision.digest()); }
        Err(error) => e.tag(match error {
            InvestigationError::Unavailable => 1, InvestigationError::Denied => 2,
            InvestigationError::InvalidRecord => 3, InvestigationError::Conflict => 4,
            InvestigationError::StaleRevision => 5, InvestigationError::StaleBasis => 6,
            InvestigationError::DeadlineElapsed => 7, InvestigationError::InvalidTransition => 8,
            InvestigationError::UnresolvedAlternatives => 9, InvestigationError::EvidenceRequired => 10,
            InvestigationError::ResidualsRequired => 11, InvestigationError::CapacityExceeded => 12,
            InvestigationError::CounterExhausted => 13, InvestigationError::ClockRegression => 14,
        }),
    }
