...
use fss_core::{AgentSession, AgentSessionParams, CanonicalDecoder, CanonicalEncoder,
    ContentDigest, ContractBasis, HydrationError, HydrationRequest, HydrationResponse,
...
use super::{DurableSessionError, DurableSessionLimits, DurableSessionStore, PendingSession,
    ReferenceSessionStore, SESSION_CHECKPOINT_RECORD_KIND, SessionAppendRecovery, read_report};
use crate::agent_session::{ReferenceSessionError, SessionAlias, SessionBindingRequest, SessionRefresh};
use crate::agent_session::context_hydration::{BoundContextHydration, ContextHydrationError, ContextSlotRead};
...
impl From<DurableSessionError> for DisclosureError {
    fn from(e: DurableSessionError) -> Self { Self::Durability(e) }
}
impl From<HydrationError> for DisclosureError {
    fn from(e: HydrationError) -> Self { Self::Cursors(e) }
}
...
impl DisclosureAdmission {
    ...
    #[must_use]
    pub const fn request_digest(&self) -> ContentDigest { self.request }
    /// Exact ordinary hydration receipt, without retaining its payload.
    #[must_use]
    pub const fn response_digest(&self) -> ContentDigest { self.response }
    /// Quoted tokens committed for this invocation; retries can have separate admissions.
    #[must_use]
    pub const fn charged_tokens(&self) -> u64 { self.tokens }
    /// Trusted service time at the admission boundary.
    #[must_use]
    pub const fn observed_at(&self) -> TimestampNs { self.observed_at }
    /// Single-use cursor consumed by this invocation, when evidence was delivered.
    #[must_use]
    pub const fn consumed_cursor(&self) -> Option<ContentDigest> { self.consumed }
}
...
impl DurableDisclosureStore {
    ...
    pub fn create(path: impl AsRef<Path>, limits: DurableSessionLimits,
        catalog: ReferenceHydrationCatalog) -> Result<Self, DisclosureError>
    {
        if catalog.issued_cursor_count() != 0 {
            return Err(HydrationError::WrongContinuation.into());
        }
        Ok(Self { sessions: DurableSessionStore::create(path, limits)?, catalog })
    }
    ...
    pub fn open_existing(path: impl AsRef<Path>, expected_root: ContentDigest,
        limits: DurableSessionLimits, catalog: ReferenceHydrationCatalog) -> Result<Self, DisclosureError>
    {
        let mut owner = Self { sessions: DurableSessionStore::open_existing(path, expected_root, limits)?, catalog };
        owner.synchronize_catalog(None)?;
        Ok(owner)
    }

    /// Explicitly recovers a torn final append using the existing root-pinned recovery protocol.
    pub fn recover_existing(path: impl AsRef<Path>, expected_root: ContentDigest,
        limits: DurableSessionLimits, policy: IncompleteTailPolicy, catalog: ReferenceHydrationCatalog)
        -> Result<(Self, super::coordination::SessionRecoveryReceipt), DisclosureError>
    {
        let (sessions, receipt) = DurableSessionStore::recover_existing(path, expected_root, limits, policy)?;
    ...
    #[must_use]
    pub fn committed_root(&self) -> ContentDigest { self.sessions.committed_root() }
    /// True when all disclosure is withheld pending journal recovery.
    #[must_use]
    pub fn needs_reconciliation(&self) -> bool { self.sessions.needs_reconciliation() }
    /// Read-only access to exact descriptors and trusted source bindings, never mutation authority.
    #[must_use]
    pub fn catalog(&self) -> &ReferenceHydrationCatalog { &self.catalog }

    /// Opens a projected session under the existing durable session contract.
    pub fn open_session(&mut self, params: AgentSessionParams, basis: ContractBasis, now: TimestampNs)
    ...
    pub fn session(&mut self, principal: &PrincipalId, id: &SessionId, now: TimestampNs)
        -> Result<AgentSession, DisclosureError>
    { Ok(self.sessions.session(principal, id, now)?) }
    /// Binds an exact authorized descriptor, without changing any hydration cursor.
    pub fn bind(&mut self, principal: &PrincipalId, request: &SessionBindingRequest, now: TimestampNs)
    ...
    pub fn refresh(&mut self, principal: &PrincipalId, id: &SessionId, change: SessionRefresh, now: TimestampNs)
        -> Result<AgentSession, DisclosureError>
    ...
    pub fn close(&mut self, principal: &PrincipalId, id: &SessionId, now: TimestampNs)
        -> Result<(), DisclosureError>
    { Ok(self.sessions.close(principal, id, now)?) }
    /// Returns the remaining cumulative grant, never replenishing it during recovery.
    pub fn remaining_token_budget(&mut self, principal: &PrincipalId, id: &SessionId, now: TimestampNs)
    ...
    pub fn register_descriptor(&mut self, descriptor: fss_core::SemanticHandle) -> Result<(), DisclosureError> {
    ...
    pub fn register_artifact(&mut self, handle: &str, descriptor: ContentDigest,
        artifact: fss_core::HydrationArtifact) -> Result<(), DisclosureError>
    {
        self.sessions.preflight()?;
        Ok(self.catalog.register_artifact(handle, descriptor, artifact)?)
    }
    ...
    pub fn bind_source_object(&mut self, handle: &str, descriptor: ContentDigest,
        publication_root: ContentDigest, reader: &dyn PublishedSourceReader)
        -> Result<crate::SourceObjectBinding, DisclosureError>
    {
        self.sessions.preflight()?;
        self.catalog.bind_source_object(handle, descriptor, publication_root, reader)
    ...
    pub fn hydrate(&mut self, principal: &PrincipalId, alias: &SessionAlias,
        request: &HydrationRequest, now: TimestampNs)
        -> Result<JournaledDisclosure<HydrationResponse>, DisclosureError>
    {
        self.deliver(now, |sessions, catalog| {
            let response = sessions.hydrate(principal, alias, request, catalog, now)
    ...
    pub fn hydrate_from_source(&mut self, principal: &PrincipalId, alias: &SessionAlias,
        request: &HydrationRequest, reader: &dyn PublishedSourceReader, now: TimestampNs)
        -> Result<JournaledDisclosure<HydrationResponse>, DisclosureError>
    {
        self.deliver(now, |sessions, catalog| {
            let response = sessions.hydrate_from_source(principal, alias, request, catalog, reader, now)
    ...
    pub fn hydrate_from_local_source(&mut self, principal: &PrincipalId, alias: &SessionAlias,
        request: &HydrationRequest, publisher: &LocalRootPublisher, io: &dyn SpoolIo, now: TimestampNs)
        -> Result<JournaledDisclosure<HydrationResponse>, DisclosureError>
    {
        self.deliver(now, |sessions, catalog| {
            let response = sessions.hydrate_from_local_source(principal, alias, request, catalog, publisher, io, now)
                .map_err(DisclosureError::Source)?;
            let admission = admission(principal, request, &response, None);
            Ok((response, admission))
        })
    ...
    pub fn hydrate_context_slot(&mut self, principal: &PrincipalId,
        publication: &BoundReferenceSituationPublication, read: &ContextSlotRead, now: TimestampNs)
        -> Result<JournaledDisclosure<BoundContextHydration>, DisclosureError>
    {
        self.deliver(now, |sessions, catalog| {
            let result = sessions.hydrate_context_slot(principal, publication, read, catalog, now)
                .map_err(DisclosureError::Context)?;
            let admission = admission(principal, &result.request, &result.response, Some(result.delivery_digest));
            Ok((result, admission))
        })
    }

    /// Expands an exact context slot using live session grants and live source custody.
    pub fn hydrate_context_slot_from_source(&mut self, principal: &PrincipalId,
        publication: &BoundReferenceSituationPublication, read: &ContextSlotRead,
        reader: &dyn PublishedSourceReader, now: TimestampNs)
        -> Result<JournaledDisclosure<BoundContextHydration>, DisclosureError>
    {
        self.deliver(now, |sessions, catalog| {
            let result = sessions.hydrate_context_slot_from_source(principal, publication, read, catalog, reader, now)
                .map_err(DisclosureError::Context)?;
            let admission = admission(principal, &result.request, &result.response, Some(result.delivery_digest));
            Ok((result, admission))
        })
    ...
    pub fn hydrate_context_slot_from_local_source(&mut self, principal: &PrincipalId,
        publication: &BoundReferenceSituationPublication, read: &ContextSlotRead,
        publisher: &LocalRootPublisher, io: &dyn SpoolIo, now: TimestampNs)
        -> Result<JournaledDisclosure<BoundContextHydration>, DisclosureError>
    {
        self.deliver(now, |sessions, catalog| {
            let result = sessions.hydrate_context_slot_from_local_source(principal, publication, read, catalog, publisher, io, now)
                .map_err(DisclosureError::Context)?;
            let admission = admission(principal, &result.request, &result.response, Some(result.delivery_digest));
            Ok((result, admission))
        })
    ...
    pub fn reconcile_pending(&mut self, policy: IncompleteTailPolicy)
    ...
    pub fn admissions(&mut self, principal: &PrincipalId, session: &SessionId,
        request: ContentDigest, max_results: usize, now: TimestampNs)
        -> Result<Vec<(ContentDigest, DisclosureAdmission)>, DisclosureError>
    {
        ...
        if report.last_root() != self.sessions.committed_root() || report.incomplete_tail().is_some() {
        ...
        for record in report.records() {
            if record.kind() == SESSION_CHECKPOINT_RECORD_KIND && is_record(record.payload()) {
                ...
                if saved.admission.principal == *principal && saved.admission.session == *session
                    && saved.admission.request == request
                {
                    if results.len() >= max_results { return Err(DurableSessionError::CapacityExceeded.into()); }
    ...
    fn synchronize_catalog(&mut self, now: Option<TimestampNs>) -> Result<(), DisclosureError> {
        ...
        if let Some(record) = latest {
            ...
            if now.is_some_and(|now| now < saved.admission.observed_at) {
                return Err(DisclosureError::Session(ReferenceSessionError::ClockRegression));
            }
            self.catalog.restore_cursor_checkpoint(saved.cursors, MAX_RECORD_BYTES, saved.admission.observed_at)?;
    ...
    fn deliver<T>(&mut self, now: TimestampNs,
        operation: impl FnOnce(&mut ReferenceSessionStore, &mut ReferenceHydrationCatalog)
        ...
        -> Result<JournaledDisclosure<T>, DisclosureError>
    {
        ...
        let checkpoint = match memory.checkpoint(self.sessions.limits.max_checkpoint_bytes) {
            ...
            Err(error) => { self.sessions.fenced = true; return Err(DurableSessionError::from(error).into()); }
        };
        match result {
            Err(error) => {
                ...
                if checkpoint.digest() != self.sessions.checkpoint_digest {
                    self.sessions.commit_candidate(PendingSession {
                        memory, checkpoint, coordination: None, record: None,
                    })?;
                }
                Err(error)
            }
            Ok((response, admission)) => {
                let prepared = (|| -> Result<Vec<u8>, DisclosureError> {
                    ...
                    let payload = encode(self.sessions.checkpoint_digest, checkpoint.as_bytes(), &cursors, &admission)?;
                    ...
                    for record in report.records() {
                        if record.kind() == SESSION_CHECKPOINT_RECORD_KIND && is_record(record.payload()) {
                            let saved = decode(record.payload(), self.sessions.limits)?;
                            cursor_history = Some(CursorCheckpoint::decode(saved.cursors, MAX_RECORD_BYTES, MAX_RECORDS)?);
                        }
                    }
                    restore_record(&payload, ContentDigest::sha256(&payload), Some(&self.sessions.memory),
                        &mut cursor_history, self.sessions.limits)?;
                    Ok(payload)
                })();
                let payload = match prepared {
                    Ok(payload) => payload,
                    Err(error) => { self.sessions.fenced = true; return Err(error); }
                };
                // Even a zero-token, same-clock disclosure gets a record: it can issue a cursor.
                self.sessions.commit_candidate(PendingSession {
                    memory, checkpoint, coordination: None,
                    record: Some((SESSION_CHECKPOINT_RECORD_KIND, payload)),
                })?;
                self.catalog = catalog;
                Ok(JournaledDisclosure { response, admission, committed_root: self.sessions.committed_root() })
            }
        }
    }
}

fn admission(principal: &PrincipalId, request: &HydrationRequest,
    response: &HydrationResponse, context: Option<ContentDigest>) -> DisclosureAdmission
{
    DisclosureAdmission {
        principal: principal.clone(), session: request.session_id.clone(), request: request.request_digest,
        response: response.receipt.receipt_digest, context, tokens: response.receipt.cost.tokens,
        observed_at: response.receipt.issued_at,
        consumed: request.continuation.as_ref().filter(|_| response.artifact.is_some()).map(|c| c.cursor_digest),
        issued: response.receipt.continuation.as_ref().map(|c| c.cursor_digest),
    }
}
...
fn digest_option(e: &mut CanonicalEncoder, digest: Option<ContentDigest>) {
    e.bool(digest.is_some()); if let Some(digest) = digest { e.digest(digest); }
}
fn read_digest_option(d: &mut CanonicalDecoder<'_>) -> Result<Option<ContentDigest>, DurableSessionError> {
    Ok(if d.bool()? { Some(d.digest()?) } else { None })
}
fn encode(before: ContentDigest, checkpoint: &[u8], cursors: &[u8], a: &DisclosureAdmission)
    -> Result<Vec<u8>, DurableSessionError>
{
    if checkpoint.len().checked_add(cursors.len()).is_none_or(|n| n > MAX_RECORD_BYTES - 16_384) {
        return Err(DurableSessionError::CapacityExceeded);
    }
    let mut e = CanonicalEncoder::new(); e.text(DOMAIN); e.digest(before);
    e.bytes(checkpoint); e.bytes(cursors); e.text(a.principal.as_str()); e.text(a.session.as_str());
    e.digest(a.request); e.digest(a.response); digest_option(&mut e, a.context);
    e.u64(a.tokens); e.i128(a.observed_at.0);
    digest_option(&mut e, a.consumed); digest_option(&mut e, a.issued);
    Ok(e.finish_checked()?)
}
fn decode(payload: &[u8], limits: DurableSessionLimits) -> Result<SavedDisclosure<'_>, DurableSessionError> {
    ...
    if session_checkpoint.len() > limits.max_checkpoint_bytes { return Err(DurableSessionError::CapacityExceeded); }
    let cursors = d.bytes()?;
    let principal = PrincipalId::parse(d.text()?)?;
    let session = SessionId::parse(d.text()?)?;
    let request = d.digest()?; let response = d.digest()?; let context = read_digest_option(&mut d)?;
    let tokens = d.u64()?; let observed_at = TimestampNs(d.i128()?);
    let consumed = read_digest_option(&mut d)?; let issued = read_digest_option(&mut d)?;
    d.ensure_finished()?;
    Ok(SavedDisclosure { before, session_checkpoint, cursors,
        admission: DisclosureAdmission { principal, session, request, response, context, tokens, observed_at, consumed, issued } })
}
...
pub(super) fn restore_record(payload: &[u8], payload_digest: ContentDigest,
    previous: Option<&ReferenceSessionStore>, cursor_history: &mut Option<CursorCheckpoint>,
    limits: DurableSessionLimits) -> Result<ReferenceSessionStore, DurableSessionError>
{
    if !is_record(payload) {
        return Ok(ReferenceSessionStore::restore_checkpoint(payload, payload_digest,
            limits.sessions, limits.max_checkpoint_bytes)?);
    }
    if ContentDigest::sha256(payload) != payload_digest { return Err(DurableSessionError::RootMismatch); }
    ...
    if cursors.captured_at != saved.admission.observed_at { return Err(DurableSessionError::InvalidHistory); }
    if let Some(old) = cursor_history.as_ref() {
        cursors.validate_successor(old).map_err(|_| DurableSessionError::InvalidHistory)?;
    }
    let mut expected = prior.clone();
    let a = &saved.admission;
    let entry = expected.live_entry(&a.principal, &a.session, a.observed_at)
        .map_err(|_| DurableSessionError::InvalidHistory)?;
    entry.spent_tokens = entry.spent_tokens.checked_add(a.tokens)
        .filter(|spent| *spent <= entry.session.token_budget).ok_or(DurableSessionError::InvalidHistory)?;
    let restored = ReferenceSessionStore::restore_checkpoint(saved.session_checkpoint,
        ContentDigest::sha256(saved.session_checkpoint), limits.sessions, limits.max_checkpoint_bytes)?;
    if restored.checkpoint(limits.max_checkpoint_bytes)?.digest() != expected.checkpoint(limits.max_checkpoint_bytes)?.digest() {
        return Err(DurableSessionError::InvalidHistory);
    }
    cursors.validate_disclosure(cursor_history.as_ref(), &a.session, a.consumed, a.issued)
        .map_err(|_| DurableSessionError::InvalidHistory)?;
