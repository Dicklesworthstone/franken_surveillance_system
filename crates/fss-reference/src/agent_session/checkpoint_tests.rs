...
use fss_core::{
    BudgetVector, ContractBasisRegistryBytes, HandleAvailability, HydrationLevel, LaboratoryAccess,
...
fn live_aliases_survive_but_still_require_current_catalog_and_owner() -> TestResult {
    ...
    assert_eq!(
        store.bind(&session.principal_id, &request, &catalog, TimestampNs(21))?,
        alias
    );
    assert_eq!(
        store
            .resolve(&session.principal_id, &alias, &catalog, TimestampNs(22))?
            .subject_digest,
    ...
    let next = store.bind(
        ...
        &SessionBindingRequest {
            ...
            descriptor_digest: second.descriptor_digest,
            ..request
        },
        &catalog,
        TimestampNs(25),
...
fn restart_retains_charges_acknowledgement_and_clock_watermark() -> TestResult {
    ...
    entry.session.last_acknowledged_situation_fingerprint =
        Some(ContentDigest::sha256(b"situation"));
    ...
    assert!(matches!(
        store.session(&session.principal_id, &session.session_id, TimestampNs(49)),
...
fn close_cannot_be_undone_by_a_lost_acknowledgement_retry() -> TestResult {
    ...
    assert!(matches!(
        store.open(params()?, basis(), TimestampNs(22)),
        Err(ReferenceSessionError::Unavailable)
    ));
    Ok(())
}
...
fn opening_identity_and_rotated_generation_survive_restart() -> TestResult {
    let (mut store, session) = opened()?;
    let rotated = store.rotate_symbols(
        &session.principal_id,
        &session.session_id,
        0,
        TimestampNs(20),
...
fn quotas_and_exact_byte_boundary_are_enforced() -> TestResult {
    ...
    assert!(matches!(
        store.checkpoint(size - 1),
        Err(SessionCheckpointError::CapacityExceeded)
    ));
    assert!(matches!(
        store.checkpoint(0),
        Err(SessionCheckpointError::CapacityExceeded)
...
fn unsorted_or_duplicate_grants_are_not_silently_normalized() -> TestResult {
    for values in [["b", "a"], ["a", "a"]] {
        ...
        assert!(matches!(
            decode_grants(&mut CanonicalDecoder::new(&bytes), &mut 2, &mut 10),
...
fn invalid_accounting_is_not_sealed() -> TestResult {
    ...
    store
        ...
        .spent_tokens = 101;
    assert!(matches!(
        store.checkpoint(MAX_SESSION_CHECKPOINT_BYTES),
        Err(SessionCheckpointError::InvalidState)
    ));
    Ok(())
}
