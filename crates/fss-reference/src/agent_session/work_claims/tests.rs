...
fn request(id: &str, work: &str) -> Result<WorkClaimRequest, ContractError> {
    Ok(WorkClaimRequest {
        ...
        case_id: CaseId::parse("case:test")?,
...
impl Fixture {
    fn new() -> Result<Self, Box<dyn Error>> {
        ...
        Ok(Self {
            sessions,
            claims: ReferenceWorkClaimStore::default(),
            owner: alice.principal_id,
            alice: alice.session_id,
            bob: bob.session_id,
        })
    }
...
fn first_committer_wins_in_both_session_orders() -> TestResult {
    for reverse in [false, true] {
        ...
        let claim = f.claims.acquire(
            &mut f.sessions,
            &f.owner,
            winner,
            request("claim:first", "work")?,
            TimestampNs(2),
        )?;
...
fn opening_retry_preserves_progress_fence_and_expiry_even_at_capacity() -> TestResult {
    ...
    assert!(matches!(
        f.update(
            &retried,
            WorkClaimUpdate::Progress(ContentDigest::sha256(b"p")),
            7
        ),
...
fn stale_progress_and_nonowner_writes_do_not_overwrite_the_head() -> TestResult {
    ...
    let active = f.update(&initial, WorkClaimUpdate::Activate, 3)?;
    assert!(matches!(
        f.update(&initial, WorkClaimUpdate::Release, 4),
    ...
    assert_eq!(
        progress.claim().lease_incarnation,
        initial.claim().lease_incarnation
    );
    assert_ne!(progress.digest(), active.digest());
    let old = f.claims.inspect_revision(
        &mut f.sessions,
        &f.owner,
        &f.alice,
        "claim:one",
    ...
    assert!(matches!(
        f.update(&old, WorkClaimUpdate::Activate, 5),
        Err(WorkClaimError::StaleRevision)
    ));
    Ok(())
}
...
fn exact_expiry_and_backward_time_never_resurrect_a_lease() -> TestResult {
    ...
    assert!(matches!(
        f.update(&initial, WorkClaimUpdate::Renew(TimestampNs(200)), 100),
...
fn lease_duration_and_session_expiry_are_hard_bounds() -> TestResult {
    ...
    let initial = f.acquire(request("claim:one", "work")?, 2)?;
    assert!(matches!(
        f.update(&initial, WorkClaimUpdate::Renew(TimestampNs(100)), 3),
        Err(WorkClaimError::InvalidLease)
    ));
    assert!(matches!(
        f.update(&initial, WorkClaimUpdate::Renew(TimestampNs(104)), 3),
        Err(WorkClaimError::InvalidLease)
    ));
    assert_eq!(f.claims.revisions, 1);
    Ok(())
...
fn dependencies_block_work_until_exact_basis_completion() -> TestResult {
    ...
    assert!(matches!(
        f.update(&second, WorkClaimUpdate::Activate, 4),
    ...
    assert!(matches!(
        f.update(&first, WorkClaimUpdate::Release, 8),
        Err(WorkClaimError::InvalidLease)
    ));
    Ok(())
}
...
fn principal_mission_privacy_and_revoked_grants_are_rechecked() -> TestResult {
    ...
    for (id, principal, mission, privacy, granted) in [
        ...
        assert_eq!(
            present.err().map(|e| e.to_string()),
            absent.err().map(|e| e.to_string())
        );
    }
    ...
    assert!(matches!(
        f.update(&head, WorkClaimUpdate::Activate, 5),
...
fn world_drift_refuses_mutation_but_keeps_recovery_information() -> TestResult {
    ...
    assert!(matches!(
        f.update(&head, WorkClaimUpdate::Activate, 4),
        Err(WorkClaimError::StaleBasis)
    ));
    assert_eq!(
        f.claims.inspect(
            &mut f.sessions,
...
fn fence_overflow_refuses_without_appending_and_claim_never_grants_effect_authority() -> TestResult
{
    ...
    f.claims
        ...
        .head
        .claim
        .lease_incarnation = u64::MAX;
    let head = f.claims.inspect(
        &mut f.sessions,
        &f.owner,
        &f.alice,
        "claim:one",
    ...
    assert!(matches!(
        f.update(&head, WorkClaimUpdate::Renew(TimestampNs(200)), 4),
        Err(WorkClaimError::CounterExhausted)
    ));
    assert_eq!(f.claims.revisions, 1);
...
fn deterministic_replay_has_identical_revision_chain() -> TestResult {
    fn run() -> Result<Vec<ContentDigest>, Box<dyn Error>> {
        ...
        let blocked = f.update(
            &active,
            WorkClaimUpdate::Block(ContentDigest::sha256(b"blocked")),
            4,
        )?;
        let resumed = f.update(&blocked, WorkClaimUpdate::Activate, 5)?;
        let completed = f.update(
            &resumed,
            WorkClaimUpdate::Complete(ContentDigest::sha256(b"result")),
            6,
        )?;
        Ok([initial, active, blocked, resumed, completed]
            .iter()
