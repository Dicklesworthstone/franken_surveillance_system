...
/// exact `ContractError::EvidenceRequired`, and attempting to construct or clear evidence on a
/// Known effect cell post-build is refused with exact `EvidenceRequired` (fss-kdhh7 Item B2).
#[test]
fn evidence_less_known_variant_publication_and_successor_and_classification_refused()
-> Result<(), Box<dyn Error>> {
