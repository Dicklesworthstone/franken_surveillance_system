...
pub(super) fn node_resources(node: &GraphNode, inputs: &[TensorPort], outputs: &[TensorPort])
    -> Result<(u64, usize), PlannedExecError> {
    if outputs.len() != 1 || inputs.is_empty() { return Err(PlannedExecError::BackendMismatch); }
    let output = &outputs[0];
    let count = output.shape().num_elements().map_err(ExecError::Tensor)? as u64;
    let mut copied_inputs = 0;
    for input in inputs { copied_inputs = add(copied_inputs, bytes(input)?)?; }
    ...
    let work = match node.op() {
        ...
        OpCode::MatMul => {
            let width = inputs[0].shape().dims().last().copied().ok_or(PlannedExecError::BackendMismatch)?;
            mul(count, width as u64)?
        }
        OpCode::MaxPool2d => {
            let kernel = match node.attributes().get("kernel_size") {
                Some(value) => value.as_usize_list(node.id(), "kernel_size").map_err(ExecError::Ir)?,
                None => vec![1, 1],
            };
        ...
        | OpCode::Relu | OpCode::Sigmoid => count,
        OpCode::Softmax => {
            ...
            let length = *output.shape().dims().get(axis).ok_or(PlannedExecError::BackendMismatch)?;
            let axis_bytes = length.checked_mul(4).ok_or(PlannedExecError::Overflow)?;
            if axis_bytes > fss_tensor::MAX_STORAGE_BYTES {
                return Err(PlannedExecError::Planning(fss_model_ir::MemoryPlanError::Limit("softmax scratch")));
            }
        ...
        OpCode::LayerNorm | OpCode::RMSNorm => {
            if count == 0 { 0 } else {
                let dims = match node.attributes().get("normalized_shape") {
                    ...
                    Some(value) => value.as_usize_list(node.id(), "normalized_shape").map_err(ExecError::Ir)?,
                    None => match inputs.get(1) {
                        Some(weight) => weight.shape().dims().to_vec(),
                        None => inputs[0].shape().dims().last().copied().into_iter().collect(),
                    },
                };
                let width = product(&dims)? as u64;
                if width == 0 { return Err(PlannedExecError::BackendMismatch); }
                mul(count, 8)?.checked_add(mul(count / width, 4)?).ok_or(PlannedExecError::Overflow)?
            }
        }
        OpCode::Gelu => {
            let tanh = match node.attributes().get("approximate") {
                Some(value) => value.as_str(node.id(), "approximate").map_err(ExecError::Ir)? == "tanh",
                None => false,
            };
        ...
        OpCode::Embedding => {
            ...
            scratch = 0;
            let indices = inputs[0].shape().num_elements().map_err(ExecError::Tensor)? as u64;
            mul(indices, 2 * inputs[0].rank() as u64 + 2)?
                .checked_add(mul(count, 4)?).ok_or(PlannedExecError::Overflow)?
        }
    };
    Ok((work, scratch))
}
...
fn product(dims: &[usize]) -> Result<usize, PlannedExecError> {
    dims.iter().try_fold(1_usize, |value, &dim| value.checked_mul(dim).ok_or(PlannedExecError::Overflow))
}
