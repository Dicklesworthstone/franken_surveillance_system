...
use fss_core::{CanonicalEncoder, ContentDigest, ContractError, Generation};
use fss_model_ir::{
    GraphNode, GraphValidator, ModelIrError, ModelIrGraph, OPERATOR_TABLE_FREEZE_DIGEST,
    ProducerId, compute_model_ir_digest, verify_operator_table_frozen,
};
...
pub enum ReceiptVerificationError {
    ...
    SchemaConstMismatch {
        /// Expected schema const string.
        expected: String,
        /// Actual schema const string observed.
        actual: String,
    },
    ...
    DigestMismatch {
        ...
        /// Actual computed canonical digest.
        actual: ContentDigest,
    },
    /// Inconsistent outcome fields.
    InconsistentOutcome {
        /// Explanation of inconsistent fields.
        reason: String,
    },
...
impl ModelInvocationReceipt {
    ...
    pub fn compute_canonical_digest(&self) -> ContentDigest {
        ...
        encoder.u64(self.generation.get());
...
/// Helper to sort ModelIrGraph nodes in topological order using GraphValidator.
fn topological_sort<'a>(graph: &'a ModelIrGraph) -> Result<Vec<&'a GraphNode>, ModelIrError> {
    ...
    GraphValidator::topological_sort(graph, &producer_map)
}
...
pub fn compute_decision_path_digest(
    ...
    if let Ok(nodes) = topological_sort(graph) {
        encoder.u64(nodes.len() as u64);
        for node in nodes {
            encoder.text(node.id());
    ...
    let (outcome, cancel_reason, error_id, output_root, operator_trace_digest, usage) =
        match &run_res {
            Ok(outcome) => {
                ...
                let wall_ns = virtual_clock.map(|c| c.now().0.max(0) as u64).unwrap_or(0);
