...
use fss_core::event::{
    EventDecodeError, EventSupersedeParams, MAX_EVENT_ID_LEN, MAX_LINEAGE_DEPTH,
};
...
impl ReviewDisposition {
    ...
    pub const fn state(self) -> EventState {
        match self {
            Self::Investigate => EventState::Indeterminate,
            Self::Resolve => EventState::Resolved,
            Self::Reject => EventState::Rejected,
        }
    }
    ...
    pub fn parse(text: &str) -> Result<Self, ReviewError> {
        match text {
            ...
            _ => Err(ReviewError::InvalidRequest(
                "expected investigate, resolve or reject",
            )),
        }
    }
    /// Whether the core table permits this direct transition, without an urgent exception.
    #[must_use]
    pub fn allowed_from(self, state: EventState) -> bool {
        state.can_transition_to(self.state(), false)
    }
}
    ...
    pub fn validate(&self) -> Result<(), ReviewError> {
            ...
            || self.reason.chars().any(char::is_control)
        {
            return Err(ReviewError::InvalidRequest(
                "invalid event, SHA-256 revision, or bounded nonempty reason",
            ));
        }
        Ok(())
    }
...
impl ReviewRecord {
    ...
    #[must_use]
    pub fn previous_anchor(&self) -> &LedgerAnchor {
        &self.previous_anchor
    }
    /// Original retained event root.
    #[must_use]
    pub fn previous_event_root(&self) -> ContentDigest {
    ...
    pub fn from_bytes(bytes: &[u8], expected: ContentDigest) -> Result<Self, ReviewError> {
        ...
        let request = ReviewRequest {
            ...
            disposition: ReviewDisposition::parse(d.text()?)?,
            reason: d.text()?.to_owned(),
        };
        let record = Self {
            request,
            principal: d.text()?.to_owned(),
            site: d.text()?.to_owned(),
            previous_event_root: d.digest()?,
            previous_anchor: LedgerAnchor::decode_canonical(&mut d)?,
        };
        d.ensure_finished()?;
        record.request.validate()?;
        PrincipalId::parse(&record.principal)?;
        validate_site_lineage(&record.site)?;
        if record.principal.len() > 256
            || record.site.len() > 256
    ...
    fn manifest(&self) -> Result<ObjectManifest, ReviewError> {
        Ok(ObjectManifest::new(
            self.slot()?.as_str(),
            [self.digest(), self.previous_event_root],
            None,
        )?)
    }
}
...
impl ReviewPreview {
    ...
    pub fn approval(&self) -> ContentDigest {
        let mut e = CanonicalEncoder::new();
        e.text(REVIEW_APPROVAL_DOMAIN);
        e.digest(ContentDigest::sha256(POLICY));
        e.digest(self.record.digest());
        e.digest(self.event.revision_digest());
        e.digest(self.provenance_root);
...
impl From<ContractError> for ReviewError {
    fn from(e: ContractError) -> Self {
        Self::Contract(e)
    }
}
impl From<EventDecodeError> for ReviewError {
    fn from(e: EventDecodeError) -> Self {
        Self::Transition(e)
    }
}
impl From<ReferenceError> for ReviewError {
    fn from(e: ReferenceError) -> Self {
        Self::Reference(Box::new(e))
    }
}
impl From<ObjectError> for ReviewError {
    fn from(e: ObjectError) -> Self {
        Self::Object(e)
    }
}
impl From<SpoolError> for ReviewError {
...
fn history(
    ...
    id: &EventId,
    cx: &ReplayCx,
) -> Result<Vec<Revision>, ReviewError> {
    if d.ledger().batches().len() > MAX_REVIEW_LEDGER_BATCHES {
    ...
    for batch in d.ledger().batches() {
        ...
        for delta in &batch.deltas {
            ...
            if event.event_id != *id
                || event.revision != history.len() as u64 + 1
...
pub fn read_review_event(
    ...
    if event.as_str().len() > MAX_EVENT_ID_LEN
        || deployment.ledger().batches().len() > MAX_REVIEW_LEDGER_BATCHES
    {
        return Err(ReviewError::Limit);
    }
...
fn derive(record: ReviewRecord, chain: &[EventHypothesis]) -> Result<ReviewPreview, ReviewError> {
    ...
    if !record.request.disposition.allowed_from(prior.state) {
        return Err(ReviewError::Transition(EventDecodeError::Contradiction {
            field: "state",
            detail: format!(
                "operator review cannot transition {} to {}",
                prior.state.as_str(),
                record.request.disposition.state().as_str()
            ),
        }));
    }
    if record.to_bytes().len() > MAX_REVIEW_RECORD_BYTES {
        return Err(ReviewError::Limit);
    }
    ...
    evidence.push(EventEvidence {
        ...
        failure_domain: format!(
            "operator-review:{}",
            hex(ContentDigest::sha256(record.principal.as_bytes()))
        ),
        supports: false,
        relation: if record.request.disposition == ReviewDisposition::Reject {
            EvidenceEdgeRelation::Contradicts
        } else {
            EvidenceEdgeRelation::Explains
        },
        capsule_digest: None,
        identity_digest: Some(ContentDigest::sha256(record.principal.as_bytes())),
    });
    let event = prior.supersede(
        EventSupersedeParams {
            state: record.request.disposition.state(),
            kind: prior.kind,
            interval: prior.interval,
            uncertainty_reason: prior.uncertainty_reason.clone(),
            zone_ids: prior.zone_ids.clone(),
            track_ids: prior.track_ids.clone(),
...
fn verify_provenance(d: &ReferenceDeployment, preview: &ReviewPreview) -> Result<(), ReviewError> {
    ...
    let visible = d
        ...
        .root(&slot)
        .ok_or(ReviewError::CustodyMismatch)?;
    if visible.root != preview.provenance_root {
        return Err(ReviewError::CustodyMismatch);
    }
    let root = d.publisher().spool().read(preview.provenance_root)?;
    if root != record.manifest()?.canonical_bytes() {
        return Err(ReviewError::CustodyMismatch);
    }
    let bytes = d.publisher().spool().read(record.digest())?;
    if ReviewRecord::from_bytes(&bytes, record.digest())? != *record {
        return Err(ReviewError::CustodyMismatch);
    }
...
pub fn preview_review(
    ...
    let index = if retry {
        if current.event.supersedes != Some(request.expected_revision) {
            return Err(ReviewError::StaleRevision);
        }
        history
            .len()
            .checked_sub(2)
            .ok_or(ReviewError::StaleRevision)?
    } else {
        history.len() - 1
    };
    let prior = &history[index];
    if prior.event.revision_digest() != request.expected_revision {
        return Err(ReviewError::StaleRevision);
    }
    ...
    let record = ReviewRecord {
        ...
        previous_event_root: prior.root,
        previous_anchor: prior.anchor.clone(),
    };
    ...
    if authoritative != current.event {
        return Err(ReviewError::CustodyMismatch);
    }
    if retry {
        if preview.event.canonical_bytes() != current.event.canonical_bytes() {
            return Err(ReviewError::StaleRevision);
        }
        ...
        for (index, operation) in deployment.effects().operations().enumerate() {
            ...
            if index >= MAX_REVIEW_EFFECT_OPERATIONS {
                return Err(ReviewError::Limit);
            }
            // Conservative across the deployment, including unrelated outstanding effects.
            if effect_blocks_review(operation.state) {
                return Err(ReviewError::OpenEffects);
            }
        }
    }
...
pub fn commit_review(
    ...
    if approval != preview.approval() {
        return Err(ReviewError::StaleApproval);
    }
    ...
    let previous = deployment
        .publisher()
        .spool()
        .read(record.previous_event_root)?;
    let slot = record.slot()?;
    if let Some(visible) = deployment.publisher().root(&slot) {
        ...
        if staged.root != preview.provenance_root {
            return Err(ReviewError::CustodyMismatch);
        }
    }
