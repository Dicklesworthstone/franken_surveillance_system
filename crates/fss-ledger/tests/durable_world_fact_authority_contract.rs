...
fn test_probe_p1_stale_anchor_refused_after_append_with_fresh_handles() -> Result<(), Box<dyn Error>>
{
    ...
    let mut durable = DurableReferenceLedger::open(&path, SITE, IncompleteTailPolicy::Reject)?;
    advance_durable(&mut durable, "p1_a")?;
    let old_head = durable.current().anchor.clone();
    assert_eq!(old_head.commit_sequence, 1);

    advance_durable(&mut durable, "p1_b")?;
    let new_head = durable.current().anchor.clone();
    assert_eq!(new_head.commit_sequence, 2);

    let fresh_auth_ledger = durable.authoritative_ledger()?;
    let fresh_authority = AuthorityAnchor::from_committed_head(&fresh_auth_ledger)?;
    assert_eq!(fresh_authority.current_anchor(), &new_head);

    let old_claim = claim_at(&old_head, witness_at(&old_head));
    let res = evaluate_negative_read(&old_claim, &fresh_authority);
    assert_eq!(
        res.err(),
        Some(ContractError::StaleAnchor),
        "claim at old sequence 1 evaluated against fresh authority at sequence 2 must be StaleAnchor"
    );
