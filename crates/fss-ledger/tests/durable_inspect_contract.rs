...
use fss_ledger::{
    AppendPhase, DurableAppendReconciliation, DurableLedgerError, DurableLedgerLimits,
...
const ALL_APPEND_PHASES: [AppendPhase; 8] = [
...
fn test_durable_differential_clean() -> TestResult {
    ...
    assert_eq!(inspection.committed_len, opened.committed_length());
...
fn test_durable_differential_append_phases() -> TestResult {
    for &phase in &ALL_APPEND_PHASES {
        ...
        match phase {
            AppendPhase::BodyWrite | AppendPhase::BodySync | AppendPhase::CommitWrite => {
                ...
                let mut trunc_ledger = DurableReferenceLedger::open(
                ...
                let recon = trunc_ledger.reconcile_pending(IncompleteTailPolicy::Truncate)?;
                assert!(matches!(
                    recon,
                    DurableAppendReconciliation::NotCommitted { .. }
                ));
            }
            AppendPhase::CommitSync => {
