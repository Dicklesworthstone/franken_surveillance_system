...
use fss_reference::rtsp::recording_archive::{
    ArchiveLimits, ArchiveNamespace, ArchiveSnapshot, ArchiveWriteProgress, RecordingArchiveWriter,
};
...
fn namespace() -> Result<ArchiveNamespace, Box<dyn std::error::Error>> {
    Ok(ArchiveNamespace::new(CatalogScope {
        ...
        decode_clock: ContentDigest::sha256(b"CLI work decode clock"),
        time_scale: 90_000,
    })?)
}
struct Fixture {
...
fn recording(
    at: u64,
) -> Result<fss_reference::rtsp::recording::PreparedRecording, Box<dyn std::error::Error>> {
    ...
    assert!(matches!(
        collector.push_picture(sample.timed(at), 1),
...
fn mutation_requires_explicit_commit_and_read_command_refuses_mutation_flags() -> Test {
    ...
    let mut inspect = args(&f, "inspect-work");
...
fn wrong_namespace_and_tighter_bounds_fail_before_any_normal_publication() -> Test {
    ...
    let f = fixture("scope")?;
    let mut command = args(&f, "restore-work");
    *command.last_mut().ok_or("namespace missing")? =
        ContentDigest::sha256(b"other archive").to_text().into();
    command.extend(["--commit".into(), "yes".into()]);
    let denied = run(command)?;
    ...
    bound.extend([
        "--commit".into(),
        "yes".into(),
        "--max-pending-bytes".into(),
        "0".into(),
    ]);
    let denied = run(bound)?;
    assert!(!denied.status.success());
    assert!(denied.stdout.is_empty());
    let p = LocalRootPublisher::open(&f.path, storage())?;
    assert!(
...
fn help_and_duplicate_or_malformed_values_have_no_storage_side_effects() -> Test {
    ...
    for extra in [
        vec!["--work-slot", "other"],
        vec!["--timeout-ms", "-1"],
        vec!["--max-graph-objects", "99999999"],
        vec!["--timeout-ms"],
        vec!["--network", "yes"],
    ] {
        let mut command = args(&f, "inspect-work");
...
fn symlink_root_is_not_followed_by_recovery_commands() -> Test {
    ...
    let mut command = args(&f, "inspect-work");
    command[2] = alias.as_os_str().to_owned();
