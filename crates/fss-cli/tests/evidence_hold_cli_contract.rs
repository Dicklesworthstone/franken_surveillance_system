...
impl Directory {
    fn new() -> TestResult<Self> {
        for n in 0..100 {
            let path =
                std::env::temp_dir().join(format!("fss-hold-cli-{}-{n}", std::process::id()));
...
fn approval(json: &str) -> TestResult<String> {
    ...
    if json.matches(marker).count() != 1 {
        return Err("expected one approval digest".into());
    }
    let value = json
        .split_once(marker)
...
fn binary_hold_release_and_deletion_preview_share_authority() -> TestResult {
    ...
    let request = FileIngestRequest::new(
        path,
        SensorId::parse("sensor:hold-cli")?,
        StreamId::parse("stream:hold-cli")?,
    )
