...
fn success(output: Output) -> Test<String> {
    if !output.status.success() {
        return Err(format!("CLI refused: {}", String::from_utf8_lossy(&output.stderr)).into());
    }
    Ok(String::from_utf8(output.stdout)?)
}
fn digest_field(json: &str, field: &str) -> Test<String> {
    let marker = format!("\"{field}\":\"");
    if json.matches(&marker).count() != 1 {
        return Err(format!("expected one {field}").into());
    }
    let text = json
        .split_once(&marker)
        .ok_or("missing field")?
...
fn seed(directory: &Directory) -> Test<(PathBuf, String, String)> {
    ...
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:review-cli-fixture".into(),
        operation_id: OperationId::parse("operation:review-cli-fixture")?,
    ...
    let request = FileIngestRequest::new(
        path,
        SensorId::parse("sensor:review-cli")?,
        StreamId::parse("stream:review-cli")?,
    )
    ...
    assert_eq!(
        report.publish(&mut deployment, &BTreeSet::from([approve]), &cx)?,
        1
    );
    drop(deployment);
...
#[test]
fn native_watch_candidate_is_reviewable_across_processes_and_old_approvals_cannot_replace_it()
-> Test {
    ...
    let call =
        |action: &str, expected: &str, reason: &str, approval: Option<&str>| -> Test<Output> {
    ...
    assert!(
        !call("resolve", &original, REASON, Some(&approval))?
            .status
            .success()
    );
    let current = digest_field(&shown, "revision_digest")?;
    assert!(
        !call("investigate", &current, REASON, None)?
            .status
            .success()
    );
    assert_eq!(journals(&root)?, after);
