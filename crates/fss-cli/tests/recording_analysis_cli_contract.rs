...
const MODEL_DIGEST: &str = "sha256:2b1c64990f1c1820b3bdb77ea32cc25b2fff484db11fe7d68b0e1f04bd2c45c8";

struct Directory(PathBuf);
impl Directory {
    fn new(label: &str) -> TestResult<Self> {
        for n in 0..100 {
            let path = std::env::temp_dir().join(format!("fss-analyze-cli-{label}-{}-{n}", std::process::id()));
...
struct Fixture { dir: Directory, root: PathBuf, source: PathBuf, model: PathBuf, import: String }
fn fixture(label: &str) -> TestResult<Fixture> {
    ...
    let result = Command::new(env!("CARGO_BIN_EXE_fss-file")).arg("import")
        .arg("--root").arg(&root).args(["--site", "site:analyze-cli", "--sensor", "sensor:analyze-cli",
            "--stream", "stream:analyze-cli", "--receive-time-ns", "1000000000"])
        .arg("--input").arg(&source).output()?;
    success(&result);
    let import = field(&result, "import_identity")?;
    Ok(Fixture { dir, root, source, model, import })
}
fn command(f: &Fixture, report: &Path, frames: usize, from_file: bool) -> Command {
    let mut c = Command::new(env!("CARGO_BIN_EXE_fss-infer"));
    c.arg("analyze").arg("--root").arg(&f.root)
        .args(["--site", "site:analyze-cli", "--import-id", &f.import, "--first-segment", "0",
            "--frames", &frames.to_string(), "--interpretation", "gray", "--model-digest", MODEL_DIGEST,
            "--output-port", "detections", "--labels", "vehicle,animal", "--box-format", "xyxy",
            "--coordinates", "normalized"])
        .arg("--report-out").arg(report);
    if from_file { c.arg("--model").arg(&f.model); }
    c
}
...
fn snapshot(root: &Path) -> TestResult<BTreeMap<PathBuf, Vec<u8>>> {
    fn walk(path: &Path, root: &Path, files: &mut BTreeMap<PathBuf, Vec<u8>>) -> TestResult {
        for entry in fs::read_dir(path)? {
            let entry = entry?;
            if entry.file_type()?.is_dir() { walk(&entry.path(), root, files)?; }
            else { files.insert(entry.path().strip_prefix(root)?.to_path_buf(), fs::read(entry.path())?); }
        }
        Ok(())
    }
    let mut files = BTreeMap::new(); walk(root, root, &mut files)?; Ok(files)
}
...
fn one_command_report_replays_without_external_inputs_and_prepares_an_event() -> TestResult {
    ...
    let first = command(&f, &report, 3, true).arg("--runs-out").arg(&runs).output()?;
    success(&first);
    ...
    assert_eq!(ContentDigest::sha256(&bytes).to_text(), field(&first, "report_digest")?);
    let run_list = fs::read_to_string(&runs)?;
    assert_eq!(run_list.lines().count(), 3);
    for segment in 0..3 {
        assert!(run_list.contains(&format!("{segment} {}\n", field(&first, &format!("run.{segment}"))?)));
    }
    fs::remove_file(&f.source)?; fs::remove_file(&f.model)?;
    let before = snapshot(&f.root)?;
    let repeated_path = f.dir.0.join("repeated.bin");
    let repeated = command(&f, &repeated_path, 3, false)
        .args(["--decode-work-units", "0", "--max-macs", "0"]).output()?;
    success(&repeated);
    assert_eq!(fs::read(&repeated_path)?, bytes);
    assert_eq!(field(&repeated, "authority_sequence")?, field(&first, "authority_sequence")?);
    ...
    let prepared = Command::new(env!("CARGO_BIN_EXE_fss-event")).arg("prepare")
        .arg("--root").arg(&f.root).args(["--site", "site:analyze-cli"])
        .arg("--report").arg(&repeated_path)
        .args(["--report-digest", &field(&repeated, "report_digest")?, "--track", &field(&repeated, "track")?])
        .arg("--event-out").arg(&event_path).output()?;
    success(&prepared);
    ...
    assert_eq!(snapshot(&f.root)?, before, "event preparation cannot publish authority");
    Ok(())
}
...
fn exhausted_model_budget_reports_pending_frame_then_reuses_its_decode() -> TestResult {
    ...
    let refused = command(&f, &refused_path, 3, true).args(["--max-macs", "0"]).output()?;
...
fn postprocessing_failure_retains_all_numeric_work_without_exporting_partial_report() -> TestResult {
    ...
    let refused_path = f.dir.0.join("refused.bin");
    let refused = command(&f, &refused_path, 3, true).args(["--detection-work-units", "0"]).output()?;
    assert_eq!(refused.status.code(), Some(1));
    assert_eq!(field(&refused, "complete")?, "false");
    ...
    for segment in 0..3 {
        assert_eq!(field(&completed, &format!("run.{segment}"))?, field(&refused, &format!("run.{segment}"))?);
    }
    assert_eq!(snapshot(&f.root)?, before);
    Ok(())
}
...
fn export_refusals_precede_numeric_execution_and_do_not_modify_deployment() -> TestResult {
    ...
    let before = snapshot(&f.root)?;
    let existing = f.dir.0.join("existing.bin"); fs::write(&existing, b"operator-owned report")?;
    ...
    assert!(!command(&f, &alias, 3, true).arg("--runs-out").arg(&alias).output()?.status.success());
    assert!(!alias.exists());
    #[cfg(unix)] {
...
fn invalid_ranges_models_and_absent_deployments_fail_without_creating_results() -> TestResult {
    ...
    let before = snapshot(&f.root)?;
    let report = f.dir.0.join("report.bin");
    assert_eq!(command(&f, &report, 0, true).output()?.status.code(), Some(2));
    assert_eq!(command(&f, &report, 257, true).output()?.status.code(), Some(2));
    assert_eq!(command(&f, &report, 4, true).output()?.status.code(), Some(1));
    assert!(!command(&f, &report, 3, false).output()?.status.success(), "unretained model is not inferred");
    ...
    assert_eq!(command(&absent, &absent.dir.0.join("report.bin"), 3, true).output()?.status.code(), Some(1));
...
fn cached_prefix_does_not_consume_the_new_suffix_model_budget() -> TestResult {
    ...
    let completed = command(&f, &f.dir.0.join("all.bin"), 3, false)
        .args(["--max-macs", &(2 * cost).to_string()]).output()?;
    success(&completed);
    assert_eq!(field(&completed, "reused_inferences")?, "1");
    assert_eq!(field(&completed, "new_inferences")?, "2");
    assert_eq!(field(&completed, "model_work_charged")?, (2 * cost).to_string());
...
fn empty_thresholded_analysis_does_not_invent_tracks_or_absence() -> TestResult {
    let f = fixture("empty")?;
    let report = f.dir.0.join("report.bin");
    let completed = command(&f, &report, 3, true).args(["--minimum-score-ppm", "1000000"]).output()?;
    success(&completed);
    ...
    assert_eq!(ContentDigest::sha256(&fs::read(report)?).to_text(), field(&completed, "report_digest")?);
    Ok(())
}
