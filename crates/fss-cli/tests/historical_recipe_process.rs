...
fn args(f: &Fixture, publish: bool) -> Test<Vec<OsString>> {
    ...
    let mut a = vec![
        ...
        "--ingress".into(),
        key.ingress.to_string().into(),
        "--generation".into(),
        key.generation.to_string().into(),
        "--ssrc".into(),
        key.ssrc.to_string().into(),
        "--peer".into(),
        scope.binding.peer().to_string().into(),
        "--authority".into(),
        scope.binding.authority().into(),
        "--rtp-channel".into(),
        scope.channels.0.to_string().into(),
        "--rtcp-channel".into(),
        scope.channels.1.to_string().into(),
        "--receive-clock".into(),
...
fn lost_stdout_followed_by_capture_growth_and_new_process_retry_reuses_all_outputs() -> Test {
    ...
    let root = field(&first, "result_root")?.to_owned();
    drop(first);
    let (head, roots) = {
        let mut p = open(&f.path)?;
        let head = append_tail(&mut p)?;
...
fn namespace_limits_admit_full_chain_ceiling_but_never_ignore_real_limit_exhaustion() -> Test {
    ...
    too_small.extend([
        "--max-datagrams".into(),
        f.pin.source.datagrams.to_string().into(),
    ]);
