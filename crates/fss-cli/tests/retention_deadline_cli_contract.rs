...
impl Directory {
    fn new() -> TestResult<Self> {
        for n in 0..100 {
            let path = std::env::temp_dir().join(format!("fss-retention-cli-{}-{n}", std::process::id()));
...
fn digest_field(json: &str, field: &str) -> TestResult<String> {
    ...
    if json.matches(marker.as_str()).count() != 1 { return Err("expected one digest field".into()); }
    let value = json.split_once(marker.as_str()).ok_or("missing digest")?.1
        .split('"').next().ok_or("unterminated digest")?;
...
fn due_is_read_only_and_only_approved_expiry_unblocks_native_deletion() -> TestResult {
    ...
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:retention-cli".into(), operation_id: OperationId::parse("operation:retention-cli")?,
    ...
    let request = FileIngestRequest::new(path, SensorId::parse("sensor:retention-cli")?, StreamId::parse("stream:retention-cli")?)
    ...
    let mutation = |action: &str, now: Option<&str>, approval: Option<&str>| -> TestResult<Output> {
        let mut command = Command::new(HOLD);
        command.arg(action).arg("--root").arg(&root).args(["--site", SITE,
            "--hold-id", "minimum", "--import-id", &import, "--reason", "Owner retention decision"]);
        if action != "release" { command.args(["--until-ns", "1000"]); }
        if let Some(now) = now { command.args(["--attested-now-ns", now]); }
    ...
    assert!(success(mutation("retain", None, Some(&approval))?)?.contains("\"status\":\"committed\""));
    assert!(success(mutation("retain", None, Some(&approval))?)?.contains("\"status\":\"already_current\""));
    ...
    for (now, readiness, count) in [
        ...
        let due = success(Command::new(HOLD).arg("due").arg("--root").arg(&root)
            .args(["--site", SITE, "--attested-now-ns", now]).output()?)?;
        assert!(due.contains(&format!("\"time_readiness\":\"{readiness}\"")), "{due}");
        assert!(due.contains(&format!("\"eligible_for_expiry\":{count}")), "{due}");
    ...
    assert!(!mutation("expire", Some("1000:1002"), Some(&approval))?.status.success());
    assert!(!mutation("expire", Some("1000:1003"), Some(&expiry))?.status.success());
    assert_eq!(fs::read(root.join("ledger/journal.fssj"))?, before);
    assert!(success(mutation("expire", Some("1000:1002"), Some(&expiry))?)?.contains("\"state\":\"expired\""));
    assert!(success(mutation("expire", Some("1000:1002"), Some(&expiry))?)?.contains("\"status\":\"already_current\""));
    ...
    let deleted = success(Command::new(EVENT).args(["delete", "commit"]).arg("--root").arg(&root)
        .args(["--site", SITE, "--plan", &digest_field(&plan, "plan_digest")?,
            "--approve", &digest_field(&plan, "approval_digest")?]).output()?)?;
...
fn absent_clock_malformed_bounds_and_cross_command_options_never_create_a_root() -> TestResult {
    ...
    for suffix in [vec![], vec!["--attested-now-ns", "2:1"], vec!["--attested-now-ns", "1:2:3"],
        vec!["--attested-now-ns", "1:2", "--approve", "sha256:bad"]]
    {
        let result = Command::new(HOLD).arg("due").arg("--root").arg(&absent)
            .args(["--site", SITE]).args(suffix).output()?;
        assert!(!result.status.success());
        assert!(!absent.exists());
    }
    let result = Command::new(HOLD).arg("due").arg("--root").arg(&absent)
        .args(["--site", SITE, "--attested-now-ns", "1:2"]).output()?;
    assert!(!result.status.success());
    assert!(!absent.exists());
    Ok(())
}
