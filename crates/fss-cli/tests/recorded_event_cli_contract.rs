...
use fss_core::{
    BudgetVector, ContentDigest, EventHypothesis, EventState, OperationId, SensorId, StreamId,
    TimestampNs,
};
...
fn fixture(label: &str) -> TestResult<Fixture> {
    ...
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:event-cli-test".into(),
        operation_id: OperationId::parse("operation:event-cli-test")?,
        principal: "principal:event-cli-test".into(),
        ...
        anchor_universe: ContentDigest::sha256(b"site:event-cli"),
        generation: 1,
    })?;
    authority.validate()?;
    ...
    let imported = FileIngestAdapter::ingest(
        FileIngestRequest::new(
            &source,
            SensorId::parse("sensor:event-cli")?,
            StreamId::parse("stream:event-cli")?,
        )
    ...
    fs::write(
        ...
        frames
            .iter()
            .map(|f| format!("{} {}\n", f.segment_index, f.run_identity))
            .collect::<String>(),
    )?;
    ...
    let report = AnalysisReport::read(
        &deployment,
        &plan,
        &AnalysisLimits::default(),
        &mut AnalysisBudget::new(100_000_000, 100_000_000),
        &cx,
    )?;
    let track = report.observations()[0]
        .tracking()
        .tracks
        .first()
        ...
        .id
        .to_text();
    let bytes = report.encoded().to_vec();
    let digest = report.digest().to_text();
    let path = directory.0.join("analysis.bin");
    fs::write(&path, &bytes)?;
...
fn command(f: &Fixture, action: &str) -> Command {
    ...
    c.arg(action)
        .arg("--root")
        .arg(&f.root)
        .args(["--site", "site:event-cli"]);
    if matches!(action, "prepare" | "publish") {
        c.arg("--report")
            .arg(&f.report)
            .args(["--report-digest", &f.digest, "--track", &f.track]);
    }
    c
}
...
fn prepare_publish_and_restart_recover_canonical_event_and_report() -> TestResult {
    ...
    let prepared = command(&f, "prepare").output()?;
    success(&prepared);
    let id = field(&prepared, "event_id")?;
    let missing = command(&f, "read").args(["--event-id", &id]).output()?;
    assert_eq!(missing.status.code(), Some(1));
    let published = command(&f, "publish")
        .args(["--proposal-digest", &field(&prepared, "proposal_digest")?])
        .output()?;
    success(&published);
    fs::remove_file(&f.report)?;
    let event_path = f.directory.0.join("event.json");
    let report_path = f.directory.0.join("recovered.bin");
    let read = command(&f, "read")
        .args(["--event-id", &id])
        .arg("--event-out")
        .arg(&event_path)
        .arg("--report-out")
    ...
    success(&read);
    assert_eq!(
        field(&read, "authority_sequence")?,
        field(&published, "authority_sequence")?
    );
    assert_eq!(
        field(&read, "event_root")?,
        field(&published, "event_root")?
    );
...
fn wrong_approval_refuses_and_exact_publication_retry_reuses_authority() -> TestResult {
    ...
    let result = command(&f, "publish")
        .args(["--proposal-digest", &bad])
        .output()?;
    assert_eq!(result.status.code(), Some(1));
    let result = command(&f, "read")
        .args(["--event-id", &field(&prepared, "event_id")?])
        .output()?;
    assert_eq!(result.status.code(), Some(1));
    let approved = field(&prepared, "proposal_digest")?;
    let first = command(&f, "publish")
        .args(["--proposal-digest", &approved])
        .output()?;
    let second = command(&f, "publish")
        .args(["--proposal-digest", &approved])
        .output()?;
    success(&first);
    success(&second);
...
fn exports_never_overwrite_or_enter_the_deployment() -> TestResult {
    ...
    let existing = f.directory.0.join("existing.json");
    fs::write(&existing, b"operator-owned")?;
    let refused = command(&f, "prepare")
        .arg("--event-out")
        .arg(&existing)
        .output()?;
...
fn malformed_approval_and_missing_deployment_do_not_create_storage() -> TestResult {
    ...
    let parsed = Command::new(env!("CARGO_BIN_EXE_fss-event"))
        .arg("publish")
        .arg("--root")
        .arg(&root)
        .args([
    ...
    assert!(!root.exists());
    let missing = Command::new(env!("CARGO_BIN_EXE_fss-event"))
        .arg("read")
        .arg("--root")
        .arg(&root)
        .args(["--site", "site:event-cli", "--event-id", "event:missing"])
        .output()?;
    assert_eq!(missing.status.code(), Some(1));
    assert!(!root.exists());
    Ok(())
...
fn report_command_reproduces_the_library_recipe_and_lists_event_candidates() -> TestResult {
    ...
    let result = command(&f, "report")
        .args([
            ...
            "gray",
            "--model-digest",
            &ContentDigest::sha256(MODEL).to_text(),
            "--output-port",
            "detections",
            "--labels",
            "vehicle,animal",
