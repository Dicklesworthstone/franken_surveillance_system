...
fn args(f: &Fixture, command: &str) -> Test<Vec<OsString>> {
    ...
    let mut result: Vec<OsString> = vec![command.into(), "--root".into(), f.path.as_os_str().to_owned(),
        "--recipe-id".into(), f.pin.recipe.to_text().into(), "--recipe-root".into(), f.pin.root.to_text().into(),
        "--ingress".into(), source::KEY.ingress.to_string().into(), "--generation".into(), source::KEY.generation.to_string().into(),
        "--ssrc".into(), source::KEY.ssrc.to_string().into(), "--peer".into(), "127.0.0.1:554".into(),
        "--authority".into(), "camera.local".into(), "--rtp-channel".into(), "0".into(), "--rtcp-channel".into(), "1".into(),
        "--receive-clock".into(), scope.receive_clock.to_text().into(),
        "--retention-evidence".into(), scope.retention_evidence.to_text().into(),
        "--max-object-bytes".into(), "1048576".into()];
    if command == "reconstruct-recipe" { result.extend(["--commit".into(), "yes".into()]); }
    Ok(result)
}
fn run(args: &[OsString]) -> Test<Output> { Ok(Command::new(env!("CARGO_BIN_EXE_fss-archive")).args(args).output()?) }
fn success(output: Output) -> Test<String> {
    assert!(output.status.success(), "{}", String::from_utf8_lossy(&output.stderr));
    assert!(output.stderr.is_empty());
    let text = String::from_utf8(output.stdout)?;
    assert!(text.len() < 8192); assert!(text.ends_with("}\n"));
    ...
    assert!(!text.contains("camera.local")); assert!(!text.contains("camera-password"));
    Ok(text)
}
...
fn check_then_reconstruct_then_lost_stdout_retry_has_identical_api_result_root() -> Test {
    ...
    { let p = open(&f.path)?; assert_eq!(p.visible_roots().count(), initial_count); }
    ...
    assert!(repeated.contains("\"new_windows\":0")); assert!(repeated.contains("\"reused_windows\":2"));
    let p = open(&f.path)?; assert_eq!(p.root(&candidate.slot).ok_or("result root missing")?.root, candidate.root);
    assert_eq!(p.visible_roots().count(), initial_count + 3); Ok(())
}
#[test]
fn incomplete_fragment_is_reported_without_a_fabricated_output_recording() -> Test {
    ...
    assert!(!result.contains("\"fragment_bytes\":0")); Ok(())
}
#[test]
fn wrong_late_recipe_never_publishes_earlier_valid_windows() -> Test {
    ...
    assert_eq!(out.status.code(), Some(i32::from(ExitIdentity::RUNTIME_FAILURE.code)));
    assert!(out.stdout.is_empty()); assert_eq!(open(&f.path)?.visible_roots().count(), count); Ok(())
}
#[test]
fn explicit_commit_is_required_and_check_rejects_mutation_flags() -> Test {
    ...
    let mut command = args(&f, "check-recipe")?; command.extend(["--commit".into(), "yes".into()]);
    let out = run(&command)?; assert!(out.stdout.is_empty());
    assert_eq!(out.status.code(), Some(i32::from(ExitIdentity::MALFORMED_VALUE.code))); Ok(())
}
#[test]
fn changed_scope_and_tighter_execution_bounds_refuse_before_publication() -> Test {
    ...
    replace(&mut wrong, "--receive-clock", ContentDigest::sha256(b"different clock").to_text().into())?;
    assert!(!run(&wrong)?.status.success());
    for bound in ["--max-work", "--max-output-bytes", "--max-steps"] {
        let mut command = args(&f, "reconstruct-recipe")?; command.extend([bound.into(), "1".into()]);
        let out = run(&command)?; assert!(!out.status.success()); assert!(out.stdout.is_empty());
    }
    assert_eq!(open(&f.path)?.visible_roots().count(), count); Ok(())
}
#[test]
fn missing_archive_is_not_created_and_unknown_secret_values_are_not_echoed() -> Test {
    ...
    let out = run(&command)?; assert!(!out.status.success()); assert!(out.stdout.is_empty()); assert!(!missing.exists());
    let out = run(&["reconstruct-recipe".into(), "--password".into(), "PRIVATE_OPERATOR_SENTINEL".into()])?;
    assert!(!out.status.success()); assert!(!String::from_utf8(out.stderr)?.contains("PRIVATE_OPERATOR_SENTINEL")); Ok(())
}
#[test]
fn held_native_owner_lock_blocks_the_other_process_without_repair() -> Test {
    ...
    let out = run(&args(&f, "reconstruct-recipe")?)?;
    assert!(!out.status.success()); assert!(out.stdout.is_empty()); assert_eq!(p.visible_roots().count(), before); Ok(())
}
#[test]
fn duplicate_overflow_and_incomplete_options_fail_before_storage() -> Test {
    ...
    let help = run(&["help".into()])?; assert!(help.status.success());
    assert!(String::from_utf8(help.stdout)?.contains("check-recipe|reconstruct-recipe")); Ok(())
}
#[cfg(unix)]
#[test]
fn symlink_archive_root_is_not_followed() -> Test {
    ...
    assert!(!out.status.success()); assert!(out.stdout.is_empty()); Ok(())
}
