...
use fss_reference::rtsp::archive_pins::{
    ArchivePinAnchor, ArchivePinJournal, ArchivePinLimits, ArchivePinScope,
};
...
fn namespace() -> Test<ArchiveNamespace> {
    Ok(ArchiveNamespace::new(CatalogScope {
        ...
        decode_clock: ContentDigest::sha256(b"CLI pin decode clock"),
        time_scale: 90_000,
    })?)
}
fn recording() -> Test<PreparedRecording> {
    ...
    assert!(matches!(
        collector.push_picture(sample.timed(0), 1),
...
fn fresh(name: &str) -> Test<PathBuf> {
    let path = Path::new(env!("CARGO_TARGET_TMPDIR"))
        .join(format!("archive_pin_process-{}", std::process::id()))
        .join(name);
...
struct Fixture {
    ...
    work: ContentDigest,
    window: ContentDigest,
    source: ContentDigest,
}
fn fixture(name: &str, durable: bool) -> Test<Fixture> {
    let path = fresh(name)?;
    let scope = ArchivePinScope {
        journal_id: ContentDigest::sha256(b"CLI independently accepted journal epoch"),
    ...
    let mut writer = CheckpointedArchiveWriter::open(
        &mut p,
        namespace()?,
        limits(),
    ...
    let source = window.children()[0].1;
    let bytes = window.byte_len();
    writer.offer(window, bytes, 0)?;
    let CheckpointedArchiveProgress::PinRequired(pin) = writer.step(0, &NeverCancel)? else {
    ...
    if durable {
        ...
        assert!(matches!(
            ...
            CheckpointedArchiveProgress::WorkDurable { .. }
        ));
    }
...
fn args(f: &Fixture, restore: bool) -> Vec<OsString> {
    let mut args = vec![
        ...
        .into(),
        "--pin-root".into(),
        f.path.join("pins").into_os_string(),
        "--journal-id".into(),
        f.scope.journal_id.to_text().into(),
        "--expected-namespace".into(),
        f.scope.archive_namespace.to_text().into(),
    ];
    if restore {
        args.extend([
            "--root".into(),
            f.path.join("media").into_os_string(),
            "--commit".into(),
            "yes".into(),
        ]);
    }
...
fn run(args: Vec<OsString>) -> Test<Output> {
...
fn inspection_needs_no_media_owner_and_does_not_claim_current_custody() -> Test {
    ...
    assert!(text.contains(&f.work.to_text()));
    assert!(text.contains("\"last_confirmed\":null"));
...
fn new_process_restores_candidate_and_survives_lost_stdout_without_duplicate_roots() -> Test {
    ...
    assert!(first.contains("\"confirmation_recorded\":true"));
    assert!(first.contains("\"outcome\":\"published\""));
    drop(first);
    let before = std::fs::read(f.path.join("pins/pins.journal"))?;
    let repeated = success(run(args(&f, true))?)?;
    assert!(repeated.contains("\"confirmation_recorded\":false"));
    assert!(repeated.contains("\"outcome\":\"already_durable\""));
    assert!(repeated.contains("\"candidate\":null"));
...
fn missing_candidate_source_is_never_discarded_or_replaced_by_a_success_report() -> Test {
    let _serial = serial();
    let f = fixture("missing_work", false)?;
    let before = std::fs::read(f.path.join("pins/pins.journal"))?;
...
fn commit_consent_is_required_and_inspection_rejects_mutating_options() -> Test {
    let _serial = serial();
    let f = fixture("consent", true)?;
    let before = std::fs::read(f.path.join("pins/pins.journal"))?;
    let mut no_commit = args(&f, true);
    no_commit.truncate(no_commit.len() - 2);
    refused(run(no_commit)?, true)?;
    let mut inspect = args(&f, false);
    inspect.extend(["--commit".into(), "yes".into()]);
    refused(run(inspect)?, true)?;
    unchanged(&f, &before)
}
#[test]
fn minimum_prefix_and_external_limits_refuse_before_confirmation_or_publication() -> Test {
    ...
    wrong.extend([
        "--minimum-sequence".into(),
        f.anchor.sequence.to_string().into(),
        "--minimum-root".into(),
        ContentDigest::sha256(b"other journal prefix")
            .to_text()
            .into(),
    ]);
    refused(run(wrong)?, false)?;
    unchanged(&f, &before)?;
    for (key, value) in [
        ("--max-pin-records", "1"),
        ("--max-pin-bytes", "128"),
        ...
        let mut bounded = args(&f, true);
        bounded.extend([key.into(), value.into()]);
        refused(run(bounded)?, false)?;
        unchanged(&f, &before)?;
    }
    ...
    valid.extend([
        ...
        f.anchor.sequence.to_string().into(),
        "--minimum-root".into(),
        f.anchor.root.to_text().into(),
    ]);
    assert!(success(run(valid)?)?.contains("\"minimum_prefix_supplied\":true"));
    Ok(())
}
#[test]
fn torn_and_corrupt_journals_are_refused_without_implicit_repair() -> Test {
    ...
    for corrupt in [false, true] {
        ...
        let file = f.path.join("pins/pins.journal");
...
fn separate_process_cannot_bypass_an_existing_native_journal_lock() -> Test {
    ...
    let guard = ArchivePinJournal::open_complete(
        f.path.join("pins"),
        f.scope,
        None,
        ArchivePinLimits::default(),
...
fn corrupt_original_source_cannot_create_a_confirmation_or_normal_archive_window() -> Test {
    ...
    let before = std::fs::read(f.path.join("pins/pins.journal"))?;
    let text = f.source.to_text();
    let source = f
        .path
        .join("media/spool/objects")
        .join(text.strip_prefix("sha256:").ok_or("digest")?);
    let mut bytes = std::fs::read(&source)?;
    *bytes.last_mut().ok_or("empty source")? ^= 1;
...
fn malformed_arguments_never_echo_values_or_create_storage() -> Test {
    let _serial = serial();
    let f = fixture("arguments", true)?;
    let before = std::fs::read(f.path.join("pins/pins.journal"))?;
    for extra in [
        vec!["--pin-root", "OTHER"],
        vec!["--minimum-sequence", "2"],
        vec!["--minimum-root", "sha256:bad"],
        vec!["--timeout-ms", "-1"],
        vec!["--timeout-ms"],
        vec!["--max-pin-bytes", "99999999999999"],
    ] {
        let mut command = args(&f, false);
        command.extend(extra.into_iter().map(OsString::from));
        refused(run(command)?, true)?;
        unchanged(&f, &before)?;
    }
    let output = run(vec![
        "restore-pins".into(),
        "--password".into(),
        "PRIVATE_SENTINEL_DO_NOT_ECHO".into(),
    ])?;
    assert!(!String::from_utf8_lossy(&output.stderr).contains("PRIVATE_SENTINEL_DO_NOT_ECHO"));
    refused(output, true)?;
    let missing = f.path.join("missing");
    let mut command = args(&f, false);
    command[2] = missing.clone().into_os_string();
    refused(run(command)?, false)?;
...
fn help_exposes_explicit_reference_and_mutation_boundaries() -> Test {
    ...
    for command in [
        vec!["help".into()],
        vec!["inspect-pins".into(), "--help".into()],
        vec!["restore-pins".into(), "--help".into()],
    ] {
        let out = run(command)?;
...
fn symlinked_or_nested_pin_owners_are_refused_without_publication() -> Test {
    let _serial = serial();
    let f = fixture("layout", true)?;
    let before = std::fs::read(f.path.join("pins/pins.journal"))?;
    let alias = f.path.join("pin-alias");
    std::os::unix::fs::symlink(f.path.join("pins"), &alias)?;
    let mut command = args(&f, false);
    command[2] = alias.into_os_string();
    refused(run(command)?, false)?;
    unchanged(&f, &before)?;
    let nested = f.path.join("media/nested-pins");
    std::fs::create_dir(&nested)?;
    for name in ["LOCK", "pins.journal"] {
        std::fs::copy(f.path.join("pins").join(name), nested.join(name))?;
    }
    let mut command = args(&f, true);
    command[2] = nested.clone().into_os_string();
    refused(run(command)?, false)?;
    assert_eq!(std::fs::read(nested.join("pins.journal"))?, before);
    unchanged(&f, &before)
}
