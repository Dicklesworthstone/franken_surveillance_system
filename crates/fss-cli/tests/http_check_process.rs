...
fn invoke(root: &std::path::Path, f: &Fixture, extra: &[&str]) -> Test<Output> {
    let r = f.request;
    Ok(Command::new(env!("CARGO_BIN_EXE_fss-archive")).arg("check-http").arg("--root").arg(root)
        .args(["--source", &r.source.source.to_string(), "--generation", &r.source.generation.to_string(),
            "--receive-clock", &r.source.receive_clock.to_string(), "--retention-evidence", &r.source.retention_evidence.to_string(),
            "--head", &r.head.to_string(), "--reads", &r.reads.to_string(), "--bytes", &r.bytes.to_string(),
...
fn real_process_checks_cold_originals_and_emits_no_media_or_paths() -> Test {
    ...
    assert!(text.starts_with("{\"schema\":\"fss.local_http_check.v1\",\"command\":\"check-http\",\"status\":\"complete\""));
    assert!(text.ends_with("]}\n")); assert_eq!(text.matches("\"ordinal\":").count(), 2);
    assert!(text.contains("\"checked_frames\":2,")); assert_eq!(text.matches("\"dimensions\":[17,13]").count(), 2);
    assert!(text.contains("\"source_bytes_emitted\":false")); assert!(text.contains("\"new_roots_published\":false"));
    assert!(!text.contains("camera.invalid")); assert!(!text.contains(d.0.to_str().ok_or("fixture path")?));
...
fn incomplete_prefix_is_a_nonzero_exit_with_a_preserved_report() -> Test {
    ...
    let text = String::from_utf8(output.stdout)?;
    assert!(text.contains("\"status\":\"prefix_exhausted\"")); assert!(text.contains("\"checked_frames\":1,"));
    assert!(text.contains("\"termination\":null")); assert!(text.contains("\"completion_root\":null"));
    Ok(())
}
#[test]
fn bounded_refusal_preserves_error_class_and_never_claims_success() -> Test {
    ...
    for extra in [["--max-steps", "1"], ["--max-decode-work", "0"]] {
        ...
        let text = String::from_utf8(output.stdout)?;
        assert!(text.contains("\"status\":\"refused\"")); assert!(text.contains("\"checked_frames\":0,"));
...
fn malformed_options_never_echo_secrets_or_create_an_archive() -> Test {
    ...
    assert!(!output.status.success()); assert!(!missing.exists()); assert!(output.stdout.is_empty());
