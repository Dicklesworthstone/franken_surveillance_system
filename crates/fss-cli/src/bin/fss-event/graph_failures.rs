...
pub(super) fn parse_domain(value: &str) -> Result<FailureDomain, String> {
    ...
    let (label, members) = value.split_once('=')
    ...
    let members: Vec<String> = members.split(',').take(MAX_DOMAIN_MEMBERS + 1)
        .map(str::to_owned).collect();
    FailureDomain::new(kind, id, &members).map_err(|error| error.to_string())
}

pub(super) fn render(value: &CoverageGraphReport, domains: &[FailureDomain]) -> Result<String, String> {
    let result = analyse_failure_domains(&value.projection, domains, GraphBudget {
    ...
    for scenario in &result.scenarios {
        ...
        let projection_id = format!("DeviceFailureGraph@parent:{parent_digest}@{}", scenario.domain.node_id());
        let witness = scenario.analysis.witness(&projection_id, value.anchor.clone())
        ...
        if bytes > MAX_REPORT_BYTES {
            return Err("ERR-GRAPH-BUDGET-EXHAUSTED-001: shared failure report exceeds 8 MiB".to_owned());
        }
        rows.push(row);
    }
    Ok(object(&[
        ...
        ("claim", string("conditional loss of retained qualifying witnesses, not current availability, calibrated clock independence, or evidence of absence")),
    ]))
}
...
mod tests {
    ...
    fn sample(context: &str) -> Result<CoverageGraphReport, Box<dyn std::error::Error>> {
        let projection = SensorCoverageProjection::build("site:test", &[
            CoverageObservation { sensor_id: "sensor:a".to_owned(), zone_scope: "zone:door".to_owned(), witnesses: 1 },
            CoverageObservation { sensor_id: "sensor:b".to_owned(), zone_scope: "zone:door".to_owned(), witnesses: 1 },
            CoverageObservation { sensor_id: "sensor:c".to_owned(), zone_scope: "zone:blind".to_owned(), witnesses: 0 },
        ])?;
        ...
        Ok(CoverageGraphReport { site: "site:test".to_owned(), anchor, projection_id: context.to_owned(), records: 3, projection, answer, witness })
    }
    ...
    fn declarations_are_bounded_strict_and_preserve_colon_sensor_ids() -> Result<(), String> {
        ...
        assert_eq!(value.members().iter().map(String::as_str).collect::<Vec<_>>(), ["sensor:a", "sensor:b"]);
        for malformed in ["", "network:x", "network=x", "other:x=a", "power:=a", "host:x=", "clock:x=a,a", "network:x=a,"] {
            assert!(parse_domain(malformed).is_err(), "accepted {malformed}");
        }
        assert!(parse_domain(&format!("power:x={}", vec!["a"; MAX_DOMAIN_MEMBERS + 1].join(","))).is_err());
        Ok(())
    }

    #[test]
    fn shared_loss_is_explicit_and_never_promotes_independence() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn identical_topology_at_different_capture_windows_has_different_scenario_witnesses() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn missing_members_do_not_turn_into_a_claim_of_redundancy() -> Result<(), Box<dyn std::error::Error>> {
        let value = sample("SensorCoverageGraph@commit:0")?;
        let declared = parse_domain("host:recorder=sensor:missing")?;
        assert!(matches!(render(&value, &[declared]), Err(error) if error.contains("ERR-GRAPH-INPUT-INVALID-001")));
        Ok(())
    }
}
