...
//! authority.
//!
//! `--during START_NS:END_NS` restricts every sensor--zone edge to one retained witness whose
//! certain capture bounds contain the entire inclusive window. Zero-witness zones remain in the
//! report; partial witnesses are never combined. Endpoints are signed capture nanoseconds, not
//! wall-clock freshness or evidence of calibrated clock alignment. Omission keeps historical output.
...
const HELP: &str = "fss-event graph single-points --root DIR --site SITE [--during START_NS:END_NS]\n\
  ...
  absence, or a resilience certificate; only sensor failures are modelled.\n";
...
pub(super) fn main(args: &[OsString]) -> ExitCode {
    ...
    match result {
        Ok(value) => {
            ...
            match writeln!(out, "{}", report(&value, request.window)) {
                Ok(()) => ExitCode::from(ExitIdentity::SUCCESS.code),
                Err(_) => ExitCode::from(ExitIdentity::RUNTIME_FAILURE.code),
            }
        }
...
mod window_tests {
    ...
    fn duplicate_and_missing_windows_are_refused() {
        ...
        assert!(matches!(
            parse(&duplicate), Err(message) if message == "duplicate option --during"
        ));
    ...
    fn sample() -> Result<CoverageGraphReport, Box<dyn std::error::Error>> {
        ...
        Ok(CoverageGraphReport {
            site: "site:test".to_owned(), anchor, projection_id, records: 1,
            projection, answer, witness,
        })
    }
