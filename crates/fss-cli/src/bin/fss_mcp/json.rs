...
impl Parser<'_> {
    ...
    fn literal(&mut self, literal: &str) -> Result<(), ()> {
        if !self.text.get(self.at..).ok_or(())?.starts_with(literal) {
            return Err(());
        }
        self.at += literal.len();
        Ok(())
    ...
    fn string(&mut self) -> Result<String, ()> {
        ...
        loop {
            ...
            if result.len() + ch.len_utf8() > MAX_STRING_BYTES {
                return Err(());
            }
            result.push(ch);
        }
...
mod tests {
    ...
    fn unicode_and_exact_number_spelling() {
        assert_eq!(
            parse(r#""\uD83D\uDE80 café""#),
            Ok(Value::Text("🚀 café".to_owned()))
        );
        assert_eq!(
            parse("-1234567890123456789"),
            Ok(Value::Number("-1234567890123456789".to_owned()))
        );
