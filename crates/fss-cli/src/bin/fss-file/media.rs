...
enum FrameMode { Decode, Read, Verify }
...
#[derive(Debug)]
pub(super) enum Action { Frame(FrameAction), Motion(MotionAction) }
impl Action {
    pub(super) fn identity(&self) -> ContentDigest {
        match self { Self::Frame(a) => a.request.import_identity, Self::Motion(a) => a.request.import_identity }
    }
    pub(super) fn name(&self) -> &'static str {
        match self {
            Self::Frame(a) => match a.mode { FrameMode::Decode => "decode", FrameMode::Read => "read-decoded", FrameMode::Verify => "verify-decoded" },
...
pub(super) fn is_command(command: &str) -> bool {
    matches!(command, "decode" | "read-decoded" | "verify-decoded" | "motion")
}

pub(super) fn accepts_option(command: &str, option: &str) -> bool {
    if !is_command(command) { return false; }
...
pub(super) fn parse(
    command: &str, identity: ContentDigest, read_limits: RetainedReadLimits, values: &Values,
) -> ParseResult<Action> {
    ...
    let decode_limits = DecodeLimits {
        maximum_bytes: usize::try_from(read_limits.max_segment_bytes.min(defaults.maximum_bytes as u64))
    ...
    if decode_limits.maximum_dimension == 0 || decode_limits.maximum_dimension > 4096
        || decode_limits.maximum_pixels == 0 || decode_limits.maximum_pixels > 4_194_304
        || decode_limits.maximum_markers == 0 || decode_limits.maximum_markers > 4096
    { return Err(malformed("positive codec limits required: dimension <=4096, pixels <=4194304, markers <=4096")); }
    let request = RecordedDecodeRequest {
        import_identity: identity,
        segment_index: number(values, if command == "motion" { "--start-segment" } else { "--segment" }, None)?,
        interpretation, read_limits, decode_limits,
    };
    let work_units = number(values, "--work-units", Some(100_000_000_u64))?;
    if command == "motion" {
        let frame_count: usize = number(values, "--frame-count", None)?;
        if frame_count == 0 || frame_count > MAX_SCAN_FRAMES || request.segment_index.checked_add(frame_count).is_none() {
            return Err(malformed("frame count must be 1..128 and its source range must not overflow"));
        }
        ...
        thresholds.validate().map_err(|_| malformed("pixel-change thresholds must be positive and fraction <=1000000 ppm"))?;
        Ok(Action::Motion(MotionAction {
            request, frame_count, work_units, thresholds,
...
pub(super) fn run(
    action: &Action, retained: &RetainedFileImport, deployment: &mut ReferenceDeployment,
...
fn run_frame(
    ...
    let frame = match action.mode {
        ...
        FrameMode::Read | FrameMode::Verify => RecordedFrame::open(deployment, &action.request, cx)?,
    };
    ...
    if let Some(path) = &action.receipt_output { write_new(path, &frame.receipt().encoded()?, root, cx)?; }
    ...
    writeln!(out, "decode_authority_sequence={}", frame.authority_anchor().commit_sequence)?;
    writeln!(out, "source_capsule={}", receipt.capsule().capsule_id)?;
    writeln!(out, "capture_earliest_ns={}", receipt.capsule().capture.earliest.0)?;
    writeln!(out, "capture_latest_ns={}", receipt.capsule().capture.latest.0)?;
    writeln!(out, "clock_basis={}", receipt.capsule().clock_basis)?;
    writeln!(out, "width={width}\nheight={height}\npixel_format=jpeg_full_range_y")?;
    writeln!(out, "luma_sha256={}", fss_core::ContentDigest::new(fss_core::DigestAlgorithm::Sha256, receipt.codec().luma_sha256))?;
    writeln!(out, "decoder_identity={}", fss_core::ContentDigest::new(fss_core::DigestAlgorithm::Sha256, receipt.codec().decoder))?;
...
fn observation_json(segment: usize, observation: &PixelChangeObservation) -> String {
    let predecessor = observation.predecessor_root.map(|root| format!("\"{root}\"")).unwrap_or_else(|| "null".to_owned());
    let resets = observation.reset_reasons.iter().map(|reason| format!("\"{}\"", reason.as_str())).collect::<Vec<_>>().join(",");
    let comparison = match &observation.statistics {
        None => "null".to_owned(),
        Some(statistics) => {
            let bounds = statistics.changed_bounds.map(|b| format!("[{}, {}, {}, {}]", b.left, b.top, b.right, b.bottom)).unwrap_or_else(|| "null".to_owned());
            format!("{{\"compared_pixels\":{},\"changed_pixels\":{},\"absolute_difference_sum\":{},\"maximum_difference\":{},\"changed_bounds_half_open\":{},\"candidate\":{}}}",
                statistics.compared_pixels, statistics.changed_pixels, statistics.absolute_difference_sum,
                statistics.maximum_difference, bounds, statistics.candidate)
        }
    };
    // Digests, stable IDs, and enum spellings are validated portable strings, not arbitrary text.
    format!("{{\"segment\":{segment},\"frame_root\":\"{}\",\"predecessor_root\":{predecessor},\"capsule_id\":\"{}\",\"capture_earliest_ns\":\"{}\",\"capture_latest_ns\":\"{}\",\"reset_reasons\":[{resets}],\"comparison\":{comparison}}}",
        observation.frame_root, observation.capsule_id, observation.capture.earliest.0, observation.capture.latest.0)
}

fn run_motion(
    action: &MotionAction, retained: &RetainedFileImport, deployment: &mut ReferenceDeployment,
    root: &Path, cx: &ReplayCx, out: &mut impl Write,
) -> RunResult<()> {
    let start = action.request.segment_index;
    let end = start.checked_add(action.frame_count).ok_or_else(|| std::io::Error::other("scan range overflow"))?;
    if end > retained.manifest().segment_spans.len() {
        return Err(std::io::Error::other("scan range exceeds retained recording; nothing decoded").into());
    }
    let mut budget = DecodeBudget::new(action.work_units);
    ...
    let result = (|| -> RunResult<()> {
        for segment in start..end {
            next = segment;
            let mut request = action.request.clone(); request.segment_index = segment;
    ...
    let error = if complete { "null" } else { "\"analysis_incomplete\"" };
    let report = format!(concat!(
        ...
        retained.import_identity(), retained.manifest_digest(), action.thresholds.digest(),
        action.thresholds.minimum_delta, action.thresholds.minimum_changed_pixels, action.thresholds.minimum_changed_fraction_ppm,
        start, action.frame_count, observations.join(",\n"), complete, next,
        next.saturating_sub(1).max(start), error, budget.used(), detector.comparisons_used());
