...
fn unicode(value: &OsStr) -> Result<&str> {
    value
        .to_str()
        .ok_or_else(|| Failure::new("evaluation.cli.invalid_unicode"))
}
...
fn parse_options(args: impl IntoIterator<Item = OsString>) -> Result<Option<Options>> {
    ...
    while let Some(flag) = args.next() {
        ...
        let value = args
            .next()
            .ok_or_else(|| Failure::new("evaluation.cli.missing_value"))?;
    ...
    let optional_number = |key| -> Result<u64> {
        values
            .get(key)
            .map_or(Ok(0), |value| integer(unicode(value)?))
    };
...
fn parse_labels(input: &str) -> Result<LabelSet> {
    ...
    for (clip, gap, line) in gaps {
        clips
            .get_mut(&clip)
            .ok_or_else(|| Failure::new("evaluation.unknown_clip").at(line))?
            .not_observable
            .push(gap);
    }
    Ok(LabelSet {
...
fn ppm(value: PpmMetric) -> String {
    match value {
        PpmMetric::Defined(value) => {
            object(&[("state", text("defined")), ("ppm", value.to_string())])
        }
...
fn timing(value: Option<TimeToDetectSummary>) -> String {
    match value {
        ...
        None => object(&[
            ...
            ("reason", text("no_detected_observable_events")),
        ]),
    }
}
...
fn point(value: &PrPoint) -> String {
    object(&[
        ...
        ("precision", ratio(value.precision)),
        ("recall", ratio(value.recall)),
        (
            "within_false_alert_budget",
            value.within_false_alert_budget.to_string(),
        ),
    ])
}
...
fn operating(value: OperatingPoint) -> String {
    match value {
        ...
        OperatingPoint::NoThresholdWithinBudget {
            ...
            (
                "false_positives_at_highest_threshold",
                false_positives_at_highest_threshold.to_string(),
            ),
        ]),
    ...
    let truth = report.truth_outcomes.iter().map(|outcome| {
        let disposition = match &outcome.disposition {
            TruthDisposition::Detected {
                ...
                ("score_ppm", score_ppm.to_string()),
                ("time_to_detect_ns", text(&time_to_detect_ns.to_string())),
            ]),
        ...
        object(&[
            ("event_id", text(&outcome.event_id)),
    ...
    let candidates = report.candidate_outcomes.iter().map(|outcome| {
        let disposition = match &outcome.disposition {
            ...
            CandidateDisposition::MatchedNotObservableTruth { event_id } => object(&[
                ("state", text("matched_not_observable_truth")),
                ("event_id", text(event_id)),
            ]),
            CandidateDisposition::InsideNotObservable => {
                object(&[("state", text("inside_not_observable"))])
            }
        };
        object(&[
            ("candidate_id", text(&outcome.candidate_id)),
            ("disposition", disposition),
        ])
    });
    object(&[
        ("schema", text("fss.evaluation.cli.v1")),
        ("derived_only", "true".to_owned()),
        ("threshold_activated", "false".to_owned()),
        ...
        (
            ...
            object(&[
                (
                    "pipeline_generation",
                    text(&report.identity.pipeline_generation),
                ),
                ("model_generation", text(&report.identity.model_generation)),
                (
                    "policy_generation",
                    text(&report.identity.policy_generation),
                ),
            ]),
        ),
        (
            ...
            object(&[
                (
                    "early_tolerance_ns",
                    text(&report.policy.early_tolerance_ns.to_string()),
                ),
                (
                    "late_tolerance_ns",
                    text(&report.policy.late_tolerance_ns.to_string()),
                ),
                (
                    "max_false_alerts",
                    text(
                ...
                (
                    "per_observed_ns",
                    text(&report.policy.false_alert_budget.per_observed_ns.to_string()),
                ),
            ]),
        ),
        (
            "label_set_digest",
            text(&report.label_set_digest.to_string()),
        ),
        (
            "candidate_set_digest",
            text(&report.candidate_set_digest.to_string()),
        ),
        ("report_digest", text(&report.digest.to_string())),
        (
            "counts_full_matching",
            object(&[
                ("truth_events", counts.truth_events.to_string()),
                (
                    "observable_truth_events",
                    counts.observable_truth_events.to_string(),
                ),
                (
                    "not_observable_truth_events",
                    counts.not_observable_truth_events.to_string(),
                ),
                ("candidates", counts.candidates.to_string()),
                (
                    "candidates_inside_not_observable",
                    counts.candidates_inside_not_observable.to_string(),
                ),
                (
                    "candidates_matched_not_observable_truth",
                    counts.candidates_matched_not_observable_truth.to_string(),
                ),
        ...
        (
            ...
            object(&[
                ...
                (
                    "not_observable_ns",
                    text(&report.durations.not_observable_ns.to_string()),
                ),
                (
                    "observed_ns",
                    text(&report.durations.observed_ns.to_string()),
                ),
            ]),
        ),
        ("auprc", ppm(report.auprc)),
        ("precision", ratio(report.precision)),
        ("recall", ratio(report.recall)),
        ("operating_point", operating(report.operating_point)),
        ("time_to_detect", timing(report.time_to_detect)),
        ("pr_curve", array(report.pr_curve.iter().map(point))),
        ("truth_outcomes", array(truth)),
        ("candidate_outcomes", array(candidates)),
    ])
}
    ...
    match run(std::env::args_os().skip(1)) {
        ...
        Err(error) => {
            let diagnostic = object(&[
                ("schema", text("fss.evaluation.cli_error.v1")),
...
mod tests {
    ...
    fn score(labels: &str, candidates: &str) -> Result<EvaluationReport> {
        Ok(evaluate(
            ...
            &EvaluationPolicy {
                early_tolerance_ns: 0,
                late_tolerance_ns: 0,
                false_alert_budget: FalseAlertBudget {
                    max_false_alerts: 0,
                    per_observed_ns: 1000,
                },
            },
            &parse_labels(labels)?,
    ...
    fn coverage_budget_and_per_event_outcomes_survive_the_adapter() -> Result<()> {
        ...
        assert!(
            matches!(report.operating_point, OperatingPoint::WithinBudget { point, .. }
    ...
    fn reordered_rows_produce_identical_report_bytes() -> Result<()> {
        ...
        assert_eq!(
            report_json(&score(LABELS, CANDIDATES)?),
