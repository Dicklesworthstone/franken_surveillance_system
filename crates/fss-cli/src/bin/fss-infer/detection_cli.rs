...
use fss_reference::ingest::detections::{
    BoxEncoding, CoordinateSpace, DetectionBudget, DetectionContract, DetectionFrame, DetectionSpec,
};
use fss_reference::ingest::recorded_decode::{
    ComponentInterpretation, DecodeLimits, RecordedDecodeRequest,
};
use fss_reference::ingest::tracking::{
    AssociationBudget, LocalBoxTracker, TrackEnd, TrackingConfig, TrackingUpdate,
};
...
fn parse(args: &[OsString]) -> Result<Options, String> {
    ...
    while i < args.len() {
        ...
        if !(common.contains(&key)
            || (tracking && track_flags.contains(&key))
    ...
    let principal = values
        .get("--principal")
        .map(|v| v.to_str().ok_or("principal must be UTF-8"))
        .transpose()?
    ...
    let track_config = if tracking {
        ...
        c.digest().map_err(|e| e.to_string())?;
        Some(c)
    } else {
        None
    };
    Ok(Options {
        ...
        selection: if tracking {
            Selection::List(PathBuf::from(value(&values, "--runs")?))
        } else {
            Selection::Single(
                number(&values, "--segment", None)?,
                digest(&values, "--run-id")?,
            )
        },
        spec,
        contract,
...
fn run_list(text: &str) -> RunResult<Vec<(usize, ContentDigest)>> {
    if text.len() > MAX_RUN_LIST_BYTES {
        return Err(io::Error::other("run list exceeds byte bound").into());
    }
    let mut entries = Vec::new();
    let mut previous = None;
    for line in text.lines() {
        let mut fields = line.split_ascii_whitespace();
        let segment: usize = fields
            .next()
            .ok_or_else(|| io::Error::other("empty run-list line"))?
            .parse()?;
        let id = ContentDigest::parse(
            fields
                .next()
                .ok_or_else(|| io::Error::other("run identity required"))?,
        )?;
        if fields.next().is_some()
            || id.algorithm() != fss_core::DigestAlgorithm::Sha256
            || previous.is_some_and(|p| p >= segment)
            || entries.len() == MAX_FRAMES
        {
            return Err(io::Error::other(
                "run list requires at most 128 strictly increasing SEGMENT SHA256 pairs",
            )
            .into());
        }
        entries.push((segment, id));
    ...
    match selection {
        ...
        Selection::List(path) => {
            ...
            if !metadata.file_type().is_file() || metadata.len() > MAX_RUN_LIST_BYTES as u64 {
                return Err(io::Error::other(
                    "run list must be a bounded regular file, not a symlink",
                )
                .into());
            }
            ...
            file.take(MAX_RUN_LIST_BYTES as u64 + 1)
    ...
    for c in value.chars() {
        match c {
            ...
            c if c <= '\u{1f}' => {
                ...
                result.push_str("\\u00");
                result.push(HEX[(c as usize) >> 4] as char);
                result.push(HEX[(c as usize) & 15] as char);
            }
            c => result.push(c),
        }
    }
...
fn render_frame(frame: &DetectionFrame, update: Option<&TrackingUpdate>) -> RunResult<String> {
    let c = frame.capsule();
    let [rows, filtered, suppressed] = frame.counts();
    let mut s = String::new();
    write!(
        s,
        "{{\"detection_digest\":\"{}\",\"run_id\":\"{}\",\"frame_root\":\"{}\",\"segment\":{},\"capsule_id\":{},\"sensor_id\":{},\"stream_id\":{},\"sequence\":{},",
        frame.digest()?,
        frame.run_identity(),
        frame.frame_root(),
        frame.segment_index(),
        ...
        json_string(c.stream_id.as_str()),
        c.sequence
    )?;
    write!(
        s,
        "\"capture\":{{\"earliest_ns\":\"{}\",\"latest_ns\":\"{}\",\"clock_basis\":\"{}\",\"gap_before\":{}}},\"dimensions\":{:?},\"rows\":{},\"filtered\":{},\"suppressed\":{},\"work_units\":{},\"detections\":[",
        c.capture.earliest.0,
        c.capture.latest.0,
        c.clock_basis.as_str(),
        c.gap_before,
        frame.dimensions(),
        rows,
        filtered,
    ...
    for (i, d) in frame.detections().iter().enumerate() {
        ...
        write!(
            s,
            "{{\"row\":{},\"class_index\":{},\"score\":{},\"bounds_subpixel\":{:?}}}",
            d.row(),
            d.class_index(),
            d.score(),
            d.bounds().coordinates()
        )?;
    }
    s.push_str("],\"tracking\":");
    if let Some(update) = update {
        ...
        write!(
            s,
            "{{\"digest\":\"{}\",\"predecessor\":{},\"config_digest\":\"{}\",\"association_is_hypothesis\":true,\"eligible_edges\":{},\"work_units\":{},\"resets\":[",
            update.digest()?,
            predecessor,
            update.config_digest,
            update.eligible_edges,
            update.work_units
        )?;
        for (i, r) in update.resets.iter().enumerate() {
            if i != 0 {
                s.push(',');
            }
            s.push_str(&json_string(r.as_str()));
        }
        s.push_str("],\"tracks\":[");
        for (i, t) in update.tracks.iter().enumerate() {
            ...
            write!(
                s,
                "{{\"id\":\"{}\",\"class_index\":{},\"bounds_subpixel\":{:?},\"score\":{},\"observed_row\":{},\"first_sequence\":{},\"last_seen_sequence\":{},\"observations\":{},\"consecutive_hits\":{},\"missed_frames\":{},\"confirmed\":{},\"ambiguous\":{}}}",
                t.id,
                t.class_index,
                t.bounds,
                f32::from_bits(t.score_bits),
                row,
                t.first_sequence,
                t.last_seen_sequence,
                t.observations,
                t.consecutive_hits,
                t.missed_frames,
                t.confirmed,
        ...
        for (i, t) in update.retired.iter().enumerate() {
            ...
            write!(
                ...
                t.id,
                match t.reason {
                    TrackEnd::Reset => "reset",
                    TrackEnd::MissedLimit => "missed_limit",
                }
            )?;
        }
        s.push_str("]}");
...
fn run(options: Options, out: &mut impl Write) -> RunResult<bool> {
    ...
    let auth = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:detector-cli".into(),
        operation_id: OperationId::parse("operation:detector-cli")?,
    ...
    let result = (|| -> RunResult<bool> {
        ...
        write!(
            report,
            "{{\"schema\":\"fss.detector_track_report.v1\",\"operation\":\"{}\",\"source_import\":\"{}\",\"contract_digest\":\"{}\",\"contract\":{{\"model_digest\":\"{}\",\"output_port\":{},\"box_format\":\"{}\",\"coordinates\":\"{}\",\"box_subpixels\":256,\"minimum_score_ppm\":{},\"nms_iou_ppm\":{},\"maximum_rows\":{},\"maximum_detections\":{},\"labels\":[",
            if tracker.is_some() { "track" } else { "detect" },
            options.source.import_identity,
            options.contract.digest(),
            spec.model_digest,
            json_string(&spec.output_port),
            match spec.encoding {
                BoxEncoding::Xyxy => "xyxy",
                BoxEncoding::CenterSize => "cxcywh",
            },
            match spec.coordinates {
                CoordinateSpace::Pixels => "pixels",
        ...
        for (i, label) in spec.labels.iter().enumerate() {
            ...
            report.push_str(&json_string(label));
        }
        report.push_str("]},\"tracking_config\":");
        if let Some(c) = options.tracking {
            write!(
                report,
                "{{\"minimum_iou_ppm\":{},\"confirmation_hits\":{},\"maximum_missed_frames\":{},\"maximum_tracks\":{}}}",
        ...
        for (segment, id) in &entries {
            ...
            let frame = match DetectionFrame::read(
                &deployment,
                *id,
                &source,
                &options.contract,
                &mut detection_budget,
                &cx,
            ) {
                Ok(frame) => frame,
                Err(error) => {
            ...
            let update = match tracker
                .as_mut()
                .map(|t| t.observe(&frame, &mut association_budget, &cx))
                .transpose()
            {
                Ok(update) => update,
                Err(error) => {
            ...
            if report
                .len()
                .checked_add(record.len() + 1024)
                .is_none_or(|n| n > MAX_REPORT_BYTES)
            {
                failure = Some("report byte capacity exceeded".into());
                break;
            }
            if completed != 0 {
                report.push(',');
        ...
        let next = entries
            .get(completed)
            .map_or("null".to_owned(), |(s, _)| s.to_string());
        let next_entry = if complete {
            "null".into()
        } else {
            completed.to_string()
        };
        writeln!(
            report,
            "],\"complete\":{},\"completed_entries\":{},\"requested_entries\":{},\"next_entry\":{},\"next_segment\":{},\"error\":{},\"detector_units_used\":{},\"association_units_used\":{},\"tracking_resume_requires_full_history\":true}}",
            complete,
            completed,
            entries.len(),
            next_entry,
            next,
            failure.as_deref().map_or("null".into(), json_string),
            detection_budget.used(),
            association_budget.used()
        )?;
        cx.checkpoint("detector_cli:report")?;
        if let Some(path) = &options.report {
            export(path, report.as_bytes(), &options.root, &cx)?;
        }
...
pub(super) fn dispatch(args: &[OsString], out: &mut impl Write) -> Option<ExitCode> {
    ...
    if args.len() == 2 && matches!(args[1].to_str(), Some("--help" | "-h" | "help")) {
        return Some(ExitCode::from(if out.write_all(HELP.as_bytes()).is_ok() {
            0
        } else {
            1
        }));
    ...
    Some(match parse(args) {
        Ok(options) => match run(options, out) {
            ...
            Err(e) => {
                eprintln!("{}: {e}", fss_cli::ERR_CLI_RUNTIME_FAILURE);
                ExitCode::from(1)
            }
        },
        Err(reason) => {
            eprintln!(
                "{}: {reason}; use fss-infer detect --help",
...
mod tests {
    ...
    fn strict_contract_and_command_specific_options() {
        ...
        for extra in [
            ...
            let mut a = args("detect");
            a.extend(extra.into_iter().map(OsString::from));
            assert!(parse(&a).is_err());
        }
        let mut a = args("track");
        a.extend(["--confirmation-hits", "0"].into_iter().map(OsString::from));
        assert!(parse(&a).is_err());
    }
    #[test]
    ...
    fn report_paths_preserve_native_os_bytes() {
        ...
        let mut a = args("detect");
        a.push("--report-out".into());
        a.push(OsString::from_vec(b"report-\xff.json".to_vec()));
