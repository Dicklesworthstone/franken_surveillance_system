...
use super::*;
use fss_reference::ingest::analysis::{AnalysisBudget, AnalysisLimits, MAX_ANALYSIS_FRAMES, MAX_ANALYSIS_REPORT_BYTES};
use fss_reference::ingest::detections::{BoxEncoding, CoordinateSpace, DetectionContract, DetectionSpec};
use fss_reference::ingest::recording_pipeline::{RecordingBudget, RecordingProgress, RecordingRequest, run_recording};
...
fn parse(args: &[OsString]) -> Result<Option<Options>, String> {
    ...
    if matches!(args[0].to_str(), Some("help" | "--help" | "-h")) {
        return if args.len() == 1 { Ok(None) } else { Err("help takes no other arguments".into()) };
    }
    let allowed = ["--root", "--site", "--principal", "--import-id", "--first-segment", "--frames",
        "--interpretation", "--model", "--model-digest", "--output-port", "--labels", "--box-format",
        "--coordinates", "--report-out", "--runs-out", "--minimum-score-ppm", "--nms-iou-ppm",
        "--minimum-iou-ppm", "--confirmation-hits", "--maximum-missed-frames", "--maximum-tracks",
        "--maximum-rows", "--maximum-detections", "--decode-work-units", "--max-macs",
        "--detection-work-units", "--association-work-units", "--max-tensor-bytes", "--max-report-bytes"];
    let mut values = Values::new();
    for pair in args.chunks(2) {
        let key = pair[0].to_str().ok_or("option names require UTF-8")?;
        if !allowed.contains(&key) { return Err("unknown or inapplicable analyze option".into()); }
        if values.contains_key(key) { return Err(format!("duplicate {key}")); }
        let v = pair.get(1).ok_or_else(|| format!("missing value for {key}"))?;
    ...
    let maximum_tensor_bytes: usize = number(&values, "--max-tensor-bytes", Some(64 * 1024 * 1024))?;
    let maximum_report_bytes: usize = number(&values, "--max-report-bytes", Some(MAX_ANALYSIS_REPORT_BYTES))?;
    if segment_count == 0 || segment_count > MAX_ANALYSIS_FRAMES
        || first_segment.checked_add(segment_count).is_none()
        || maximum_tensor_bytes == 0 || maximum_tensor_bytes > MAX_RUN_TENSOR_BYTES
        || maximum_report_bytes == 0 || maximum_report_bytes > MAX_ANALYSIS_REPORT_BYTES
    { return Err("invalid range or tensor/report bound".into()); }
    let request = RecordingRequest {
        import_identity: digest(&values, "--import-id")?, interpretation, first_segment, segment_count,
        detector, tracking, maximum_tensor_bytes,
        limits: AnalysisLimits { maximum_report_bytes, ..AnalysisLimits::default() },
    };
    Ok(Some(Options {
        root: PathBuf::from(value(&values, "--root")?), site, principal,
        model: values.get("--model").map(PathBuf::from),
        report: PathBuf::from(value(&values, "--report-out")?),
        runs: values.get("--runs-out").map(PathBuf::from), request,
        budget: RecordingBudget::new(
            ...
            AnalysisBudget::new(number(&values, "--detection-work-units", Some(10_000_000))?,
...
fn destination(path: &Path, root: &Path) -> RunResult<PathBuf> {
    ...
    Ok(parent.join(path.file_name().ok_or_else(|| io::Error::other("export file name required"))?))
}

fn progress(out: &mut impl Write, state: &RecordingProgress, work: &RecordingBudget<'_>, complete: bool) -> io::Result<()> {
    writeln!(out, "operation=analyze\ncomplete={complete}\nstage={}\ncompleted_inferences={}",
        state.stage.as_str(), state.completed.len())?;
    match state.next_segment {
        Some(segment) => writeln!(out, "next_segment={segment}")?,
        None => writeln!(out, "next_segment=none")?,
    }
    writeln!(out, "new_decodes={}\nreused_decodes={}\nnew_inferences={}\nreused_inferences={}",
        state.new_decodes, state.reused_decodes, state.new_inferences, state.reused_inferences)?;
    writeln!(out, "decode_work_units={}\nmodel_work_charged={}\nmodel_work_remaining={}",
        work.decode.used(), work.model_charged(), work.model_remaining())?;
    writeln!(out, "detection_work_units={}\nassociation_work_units={}\nauthority_sequence={}",
        work.analysis.detection.used(), work.analysis.association.used(), state.anchor.commit_sequence)?;
    for frame in &state.completed { writeln!(out, "run.{}={}", frame.segment_index, frame.run_identity)?; }
    writeln!(out, "model_outputs=uncalibrated\nassociation_is_hypothesis=true\nabsence_certifiable=false\neffects_authorized=false")
}
...
fn run(mut options: Options, out: &mut impl Write) -> RunResult<()> {
    ...
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:recording-cli".into(), operation_id: OperationId::parse("operation:recording-cli")?,
        principal: options.principal.clone(), capabilities: vec!["ADP-REPLAY-001".into()], deadline: None,
        priority: 10, budgets: BudgetVector::builder().bytes(options.request.maximum_tensor_bytes as u64).build()?,
    ...
    let result = (|| -> RunResult<()> {
        ...
        let model = match &options.model {
            ...
            None => RecordedModel::decode(&deployment.publisher().spool().read(expected)?, expected)?,
        };
        let outcome = match run_recording(&mut deployment, &options.request, &model, &mut options.budget, &exec, &cx) {
            Ok(outcome) => outcome,
            Err(error) => {
        ...
        export(&options.report, outcome.report.encoded(), &options.root, &cx)?;
        if let Some(path) = &options.runs {
            let mut bytes = Vec::new();
            for frame in &outcome.progress.completed { writeln!(&mut bytes, "{} {}", frame.segment_index, frame.run_identity)?; }
            export(path, &bytes, &options.root, &cx)?;
        }
        progress(out, &outcome.progress, &options.budget, true)?;
        writeln!(out, "report_digest={}\nanalysis_plan_digest={}\nmodel_digest={}",
            outcome.report.digest(), outcome.report.plan().digest(), model.digest())?;
        let tracks: BTreeSet<_> = outcome.report.observations().iter()
            .flat_map(|observation| &observation.tracking().tracks)
            .filter(|track| track.observed_row.is_some()).map(|track| track.id).collect();
        writeln!(out, "candidate_tracks={}", tracks.len())?;
        for track in tracks { writeln!(out, "track={track}")?; }
        Ok(())
    })();
    exec.drain_and_finalize(); cx.drain_and_finalize(); result
}

pub(super) fn dispatch(args: &[OsString], out: &mut impl Write) -> Option<ExitCode> {
    if args.first().and_then(|arg| arg.to_str()) != Some("analyze") { return None; }
    Some(match parse(&args[1..]) {
        Ok(None) => match out.write_all(HELP.as_bytes()) {
            Ok(()) => ExitCode::SUCCESS, Err(_) => ExitCode::from(ExitIdentity::RUNTIME_FAILURE.code),
        },
        Ok(Some(options)) => match run(options, out) {
            Ok(()) => ExitCode::SUCCESS,
            Err(error) => {
                eprintln!("{ERR_CLI_RUNTIME_FAILURE}: {error}");
                eprintln!("Retained frame/model results survive failure. Retry the original range; partial exports may remain.");
                ExitCode::from(ExitIdentity::RUNTIME_FAILURE.code)
            }
        },
        Err(error) => {
            eprintln!("{}: {error}; use fss-infer analyze --help", fss_cli::ERR_CLI_MALFORMED_VALUE);
...
mod tests {
    ...
    fn args() -> Vec<OsString> {
        ["--root", "unused", "--site", "site:analyze", "--import-id",
            "sha256:1111111111111111111111111111111111111111111111111111111111111111",
            "--first-segment", "0", "--frames", "3", "--interpretation", "gray", "--model-digest",
            "sha256:2222222222222222222222222222222222222222222222222222222222222222",
            "--output-port", "detections", "--labels", "vehicle,animal", "--box-format", "xyxy",
    ...
    fn cached_numeric_zero_budgets_are_admitted_and_help_is_storage_free() {
        let mut a = args(); a.extend(["--max-macs", "0", "--decode-work-units", "0"].map(OsString::from));
