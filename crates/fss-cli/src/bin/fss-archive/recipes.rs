...
use fss_publication::{LocalPublicationLimits, LocalRootPublisher, PublishCancellation, PublishCutPoint, PublishOutcome};
...
use fss_reference::rtsp::recording_recipe::{RecordingRecipeLimits, MAX_RECORDING_RECIPE_BYTES, MAX_RECORDING_RECIPE_TIMINGS};
...
pub(super) fn handles(args: &[OsString]) -> bool {
    args.first().and_then(|s| s.to_str()).is_some_and(|s| matches!(s, "check-recipe" | "reconstruct-recipe"))
}
...
struct Options {
    root: PathBuf, selected: RecipeSelection, load: RecipeLoadLimits, output: ReconstructionLimits,
    storage: LocalPublicationLimits, timeout: Duration, steps: u64, work: u64, publish: bool,
}
fn parse(args: &[OsString]) -> Result<Options> {
    ...
    if !handles(args) { return Err(malformed("expected a recipe operation")); }
    let publish = args[0].as_os_str() == OsStr::new("reconstruct-recipe");
    let allowed = ["--root", "--recipe-id", "--recipe-root", "--ingress", "--generation", "--ssrc",
        "--peer", "--authority", "--rtp-channel", "--rtcp-channel", "--receive-clock", "--retention-evidence",
        "--timeout-ms", "--max-steps", "--max-work", "--max-datagrams", "--max-source-bytes",
        "--max-recipe-bytes", "--max-timings", "--max-windows", "--max-output-bytes", "--max-roots",
        "--max-scan-roots", "--max-objects", "--max-object-bytes", "--max-total-bytes"];
    ...
    let number = |key: &str, default: u64, minimum: u64, maximum: u64| -> Result<u64> {
        ...
        if !(minimum..=maximum).contains(&n) { return Err(malformed("numeric bound exceeded")); }
        Ok(n)
    };
    ...
    if publish && text("--commit")? != "yes" { return Err(malformed("publication requires explicit commit acknowledgement")); }
    for name in ["--generation", "--ssrc", "--rtp-channel", "--rtcp-channel"] { let _ = required(name)?; }
    let ingress = text("--ingress")?;
    if !ingress.bytes().all(|b| b.is_ascii_digit()) { return Err(malformed("invalid source ingress")); }
    let key = StreamKey { ingress: ingress.parse::<u128>().map_err(|_| malformed("invalid source ingress"))?,
        generation: number("--generation", 0, 1, u64::MAX)?, ssrc: number("--ssrc", 0, 0, u32::MAX as u64)? as u32 };
    let peer = text("--peer")?.parse().map_err(|_| malformed("expected a literal source IP and port"))?;
    let binding = TcpBinding::new(key, peer, text("--authority")?, TcpSecurityPolicy::OwnerApprovedPlaintext)
        .map_err(|_| malformed("invalid original source binding"))?;
    let scope = DatagramScope { binding,
        channels: (number("--rtp-channel", 0, 0, 255)? as u8, number("--rtcp-channel", 1, 0, 255)? as u8),
        receive_clock: digest("--receive-clock")?, retention_evidence: digest("--retention-evidence")? };
    scope.digest().map_err(|_| malformed("invalid original source scope"))?;
    let object_bytes = number("--max-object-bytes", 32 * 1024 * 1024, 1024, 32 * 1024 * 1024)? as usize;
    ...
    let load = RecipeLoadLimits {
        source: DatagramArchiveLimits { max_datagrams: datagrams,
            ...
            max_scan_roots: scan, max_spool_object_bytes: object_bytes },
    ...
    Ok(Options { root: PathBuf::from(required("--root")?),
        ...
        load, output: ReconstructionLimits { max_windows: number("--max-windows", 64, 1, 1024)? as usize,
            max_output_bytes: number("--max-output-bytes", 64 * 1024 * 1024, 1, 1024 * 1024 * 1024)?, max_scan_roots: scan },
        storage, timeout: Duration::from_millis(number("--timeout-ms", 30_000, 1, 3_600_000)?),
        steps: number("--max-steps", 65_536, 1, 1_000_000)?,
        work: number("--max-work", 1_000_000_000_000, 1, 1_000_000_000_000_000)?, publish })
}
struct Clock { start: Instant, deadline: u64 }
impl ReconstructionClock for Clock {
    fn now_ns(&self) -> Option<u64> { u64::try_from(self.start.elapsed().as_nanos()).ok() }
}
impl PublishCancellation for Clock {
    fn cancel_requested(&self, _: PublishCutPoint) -> bool { self.now_ns().is_none_or(|n| n >= self.deadline) }
}
...
fn run(options: Options) -> Result<String> {
    let clock = Clock { start: Instant::now(), deadline: u64::try_from(options.timeout.as_nanos())
    ...
    let mut p = LocalRootPublisher::open(root, options.storage).map_err(|_| runtime("archive open or exclusive ownership refused"))?;
    ...
    let bounds = AvcReplayBounds { max_source_bytes: options.load.source.max_payload_bytes,
        max_steps: options.steps, deadline_ns: clock.deadline };
    let plan = PreparedReconstruction::prepare(&loaded, &p, bounds, options.output, &clock, &clock, &mut work)
        .map_err(|_| runtime("complete native reconstruction refused; no output was published by preparation"))?;
    ...
    let mut report = String::new(); report.try_reserve_exact(8192).map_err(|_| runtime("report capacity exhausted"))?;
    let (status, published, reused) = if options.publish {
        ...
        let new = outcome.windows.iter().filter(|r| r.outcome == PublishOutcome::Published).count();
        (if outcome.completion.outcome == PublishOutcome::AlreadyPublished { "already_durable" } else { "published" },
    ...
    if report.len() > 8192 { return Err(runtime("report bound exceeded")); }
    Ok(report)
}
pub(super) fn dispatch(args: &[OsString]) -> ExitCode {
    let result = if args.len() == 2 && args[1].as_os_str() == OsStr::new("--help") { Ok(HELP.to_owned()) }
        else { parse(args).and_then(run) };
    match result {
        ...
        Err(error) => {
            ...
            eprintln!("No complete report was emitted. Retain the original recipe pin and storage; reconcile uncertain roots before exact retry. No operator cleanup, source deletion, camera connection or archive indexing was requested.");
            ExitCode::from(if error.usage { ExitIdentity::MALFORMED_VALUE.code } else { ExitIdentity::RUNTIME_FAILURE.code })
        }
    }
}
