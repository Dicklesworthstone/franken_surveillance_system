...
use fss_publication::{LocalPublicationLimits, LocalPublicationReceipt, LocalRootPublisher,
    PublishCancellation, PublishCutPoint, PublishOutcome};
use fss_reference::rtsp::archive_pins::{ArchivePinAnchor, ArchivePinError, ArchivePinJournal,
    ArchivePinLimits, ArchivePinScope, StoredArchivePin};
use fss_reference::rtsp::recording::MAX_RECORDING_BYTES;
use fss_reference::rtsp::recording_archive::ArchiveLimits;
use fss_reference::rtsp::recording_archive::checkpoint::{ArchiveWorkLimits, MAX_ARCHIVE_WORK_BYTES};
...
pub(super) fn handles(args: &[OsString]) -> bool {
    args.first().and_then(|a| a.to_str()).is_some_and(|a| matches!(a, "inspect-pins" | "restore-pins"))
}
...
impl From<ArchivePinError> for Error { fn from(e: ArchivePinError) -> Self { Self::Pins(e) } }
type Result<T> = std::result::Result<T, Error>;
fn malformed(message: &'static str) -> Error { Error::Argument(ERR_CLI_MALFORMED_VALUE, message) }
struct Options {
    pin_root: PathBuf, root: Option<PathBuf>, scope: ArchivePinScope,
    minimum: Option<ArchivePinAnchor>, pins: ArchivePinLimits, work: ArchiveWorkLimits,
    storage: LocalPublicationLimits, timeout: Duration,
}
fn parse(args: &[OsString]) -> Result<Options> {
    ...
    let common = ["--pin-root", "--journal-id", "--expected-namespace", "--minimum-sequence",
        "--minimum-root", "--timeout-ms", "--max-pin-records", "--max-pin-bytes"];
    let mutation = ["--root", "--commit", "--max-windows", "--max-pages", "--max-scan-roots",
        "--max-page-windows", "--max-pending-bytes", "--max-graph-objects", "--max-new-bytes",
        "--max-objects", "--max-total-bytes"];
    let mut values: BTreeMap<&str, &OsStr> = BTreeMap::new();
    for pair in args[1..].chunks(2) {
        let key = pair[0].to_str().ok_or(Error::Argument(ERR_CLI_INVALID_UNICODE, "option name must be UTF-8"))?;
        if !common.contains(&key) && !(restore && mutation.contains(&key)) {
            return Err(Error::Argument(ERR_CLI_UNKNOWN_OPTION, "unknown or inapplicable option"));
        }
        if values.contains_key(key) { return Err(Error::Argument(ERR_CLI_DUPLICATE_OPTION, "duplicate option")); }
        let value = pair.get(1).filter(|v| !v.is_empty() && !v.to_str().is_some_and(|s| s.starts_with("--")))
            .ok_or(Error::Argument(ERR_CLI_MISSING_VALUE, "missing option value"))?;
        values.insert(key, value.as_os_str());
    }
    let required = |key: &str| values.get(key).copied().ok_or(Error::Argument(ERR_CLI_MISSING_VALUE, "required option missing"));
    let text = |key: &str| required(key)?.to_str().ok_or(Error::Argument(ERR_CLI_INVALID_UNICODE, "identity must be UTF-8"));
    let number = |key: &str, default: u64, minimum: u64, maximum: u64| -> Result<u64> {
        let value = if values.contains_key(key) {
            let raw = text(key)?;
            if !raw.bytes().all(|b| b.is_ascii_digit()) { return Err(malformed("unsigned decimal integer required")); }
            raw.parse::<u64>().map_err(|_| malformed("integer overflow"))?
        } else { default };
        if !(minimum..=maximum).contains(&value) { return Err(malformed("numeric bound refused")); }
        Ok(value)
    };
    ...
    if restore && text("--commit")? != "yes" { return Err(malformed("restoration requires --commit yes")); }
    if values.contains_key("--minimum-sequence") != values.contains_key("--minimum-root") {
        return Err(malformed("minimum sequence and root must be supplied together"));
    }
    let minimum = if values.contains_key("--minimum-root") {
        Some(ArchivePinAnchor { sequence: number("--minimum-sequence", 0, 1, 16_384)?, root: digest("--minimum-root")? })
    } else { None };
    ...
    Ok(Options {
        ...
        root: if restore { Some(PathBuf::from(required("--root")?)) } else { None },
        scope: ArchivePinScope { journal_id: digest("--journal-id")?, archive_namespace: digest("--expected-namespace")? }, minimum,
        pins: ArchivePinLimits { max_records: number("--max-pin-records", 8193, 1, 16_384)? as usize,
            max_bytes: number("--max-pin-bytes", 8 * 1024 * 1024, 128, 16 * 1024 * 1024)? as usize },
        work: ArchiveWorkLimits { archive: ArchiveLimits {
...
impl Clock {
    fn now(&self) -> Result<u64> {
        ...
        if n >= self.duration.as_nanos() { return Err(Error::Deadline); }
        u64::try_from(n).map_err(|_| Error::Deadline)
    }
    fn deadline(&self) -> u64 { self.duration.as_nanos() as u64 }
}
...
fn directory(path: &Path) -> Result<PathBuf> {
    if !fs::symlink_metadata(path).map_err(|_| Error::Layout)?.is_dir() { return Err(Error::Layout); }
...
fn archive(path: &Path) -> Result<PathBuf> {
    ...
    for name in ["roots", "tombstones", "spool", "spool/objects", "spool/staging", "spool/verified"] {
        if !fs::symlink_metadata(root.join(name)).map_err(|_| Error::Layout)?.is_dir() { return Err(Error::Layout); }
    }
    for name in ["LOCK", "spool/LOCK"] {
        if !fs::symlink_metadata(root.join(name)).map_err(|_| Error::Layout)?.is_file() { return Err(Error::Layout); }
    }
    Ok(root)
}
...
fn receipt_json(receipt: Option<&LocalPublicationReceipt>) -> String {
    receipt.map_or_else(|| "null".to_owned(), |r| format!("{{\"root\":\"{}\",\"outcome\":\"{}\"}}",
        r.root, if r.outcome == PublishOutcome::AlreadyPublished { "already_durable" } else { "published" }))
}
fn run(options: Options) -> Result<String> {
    ...
    if media.as_ref().is_some_and(|p| p.starts_with(&pin_root) || pin_root.starts_with(p)) {
        return Err(Error::Layout);
    }
    clock.now()?;
    let mut pins = ArchivePinJournal::open_complete(&pin_root, options.scope, options.minimum, options.pins, &clock)?;
    let mut report = String::new();
    report.try_reserve_exact(MAX_REPORT_BYTES).map_err(|_| Error::Report)?;
    let restored = if let Some(media) = media {
        clock.now()?;
        let mut publisher = LocalRootPublisher::open(media, options.storage).map_err(Error::Storage)?;
        clock.now()?; // Existing owner open is bounded, not a preemptible syscall.
        Some(pins.restore_work(&mut publisher, options.work, clock.now()?, clock.deadline(), &clock)?)
    } else { None };
    pins.verify(&clock)?;
    clock.now()?;
    ...
    if report.len() > MAX_REPORT_BYTES { return Err(Error::Report); }
    clock.now()?;
    Ok(report)
}
pub(super) fn dispatch(args: &[OsString]) -> ExitCode {
    ...
    match result {
        ...
        Err(error) => {
            ...
            eprintln!("{code}: {error}");
            eprintln!("No complete report was emitted. Keep the journal and archive: confirmation or original roots may have committed. Inspect/reconcile and retry the same work; no automatic cleanup, tail repair or camera operation occurred.");
            ExitCode::from(exit)
        }
    }
}
