...
mod detection_cli;
#[path = "fss-infer/package_cli.rs"]
mod package_cli;
#[path = "fss-infer/recording_cli.rs"]
mod recording_cli;

const HELP: &str = "fss-infer <run|read|replay|detect|track|analyze|package-detect> [options]\n\
  ...
  package-detect: use fss-infer package-detect --help (verified RGB detector package).\n\
...
fn text<'a>(values: &'a Values, key: &str) -> Result<&'a str, String> {
    value(values, key)?
        .to_str()
        .ok_or_else(|| format!("{key} must be UTF-8"))
}
fn number<T: FromStr>(values: &Values, key: &str, default: Option<T>) -> Result<T, String> {
    if !values.contains_key(key) {
        return default.ok_or_else(|| format!("required option {key}"));
    }
    text(values, key)?
        .parse()
...
fn digest(values: &Values, key: &str) -> Result<ContentDigest, String> {
    let d = ContentDigest::parse(text(values, key)?)
        .map_err(|_| format!("invalid digest for {key}"))?;
    if d.algorithm() != fss_core::DigestAlgorithm::Sha256 {
...
fn parse(args: &[OsString]) -> Result<Option<Options>, String> {
    ...
    if matches!(action, "help" | "--help" | "-h") {
        return if args.len() == 1 {
            ...
            Err("help takes no other arguments".to_owned())
        };
    }
    if !matches!(action, "run" | "read" | "replay") {
        return Err("expected run, read or replay".to_owned());
    }
    let common = [
        "--root",
        "--site",
    ...
    while i < args.len() {
        ...
        let allowed = common.contains(&key)
            || (action == "run"
                && matches!(key, "--model" | "--model-digest" | "--decode-work-units"))
    ...
    let source = RecordedDecodeRequest {
        ...
        segment_index: number(&values, "--segment", None)?,
        interpretation,
        read_limits: RetainedReadLimits::default(),
        decode_limits: DecodeLimits::default(),
    };
    let budget = ExecBudget::new(
        number(&values, "--max-macs", Some(100_000_000))?,
        number(&values, "--max-tensor-bytes", Some(64 * 1024 * 1024))?,
    );
    if budget.max_bytes == 0 || budget.max_bytes > MAX_RUN_TENSOR_BYTES {
        return Err("tensor ceiling must be 1..268435456 bytes".to_owned());
    }
    let action = match action {
        "run" => Action::Run {
            path: PathBuf::from(value(&values, "--model")?),
            digest: digest(&values, "--model-digest")?,
    ...
    Ok(Some(Options {
        ...
        receipt: values.get("--receipt-out").map(PathBuf::from),
...
fn load_model(path: &Path, expected: ContentDigest) -> RunResult<RecordedModel> {
    ...
    if !file.metadata()?.file_type().is_file() {
        return Err(io::Error::other("model is not regular").into());
    }
    let mut bytes = Vec::new();
    file.take(MAX_RECORDED_MODEL_BYTES as u64 + 1)
...
fn run(options: Options, out: &mut impl Write) -> RunResult<()> {
    ...
    let authority = ContextAuthority::new_root(RootAuthoritySpec {
        trace_id: "trace:inference-cli".to_owned(),
        operation_id: OperationId::parse("operation:inference-cli")?,
        principal: options.principal,
        capabilities: vec!["ADP-REPLAY-001".to_owned()],
        deadline: None,
        priority: 10,
        budgets: BudgetVector::builder()
            .bytes(options.budget.max_bytes as u64)
            .build()?,
    ...
    let result = (|| -> RunResult<()> {
        ...
        let model = match &options.action {
            Action::Run { path, digest, .. } => Some(load_model(path, *digest)?),
            _ => None,
        };
        ...
        let (result, operation) = match &options.action {
            Action::Run { decode_work, .. } => {
                let model = model
                    .as_ref()
                    .ok_or_else(|| io::Error::other("model not loaded"))?;
                RecordedFrame::decode_and_publish(
                    &mut deployment,
                    &options.source,
                    &mut DecodeBudget::new(*decode_work),
                    &cx,
                )?;
                (
            ...
            Action::Read(id) => (
                RecordedInference::open(&deployment, *id, &options.source, &cx)?,
                "read",
            ),
            Action::Replay(id) => {
                let result = RecordedInference::open(&deployment, *id, &options.source, &cx)?;
                result.verify_by_replay(
                    &deployment,
                    &options.source,
                    options.budget,
                    &exec_cx,
        ...
        if let Some(path) = &options.output {
            export(path, result.output_bytes(), &options.root, &cx)?;
        }
        if let Some(path) = &options.receipt {
            export(path, &result.receipt_bytes()?, &options.root, &cx)?;
        }
        if let Some(path) = &options.model_out {
            export(path, result.model().encoded(), &options.root, &cx)?;
        }
        writeln!(
            out,
            "operation={operation}\nrun_identity={}\nmodel_digest={}\nframe_root={}",
        ...
        writeln!(
            ...
            result.authority_anchor().commit_sequence,
            result.executed_macs(),
            result.allocated_tensor_bytes()
        )?;
        writeln!(
            out,
            "output_digest={}\noutput_ports={}\nmodel_outputs=uncalibrated\nabsence_certifiable=false\neffects_authorized=false",
            ContentDigest::sha256(result.output_bytes()),
            result.outputs().len()
        )?;
        Ok(())
    })();
    exec_cx.drain_and_finalize();
    cx.drain_and_finalize();
    result
}
fn main() -> ExitCode {
    let args: Vec<_> = std::env::args_os().skip(1).collect();
    if let Some(exit) = recording_cli::dispatch(&args, &mut io::stdout().lock()) {
        return exit;
    }
    if let Some(exit) = detection_cli::dispatch(&args, &mut io::stdout().lock()) {
        return exit;
    }
    if let Some(exit) = package_cli::dispatch(&args, &mut io::stdout().lock()) {
        return exit;
    }
    match parse(&args) {
        ...
        Ok(Some(options)) => match run(options, &mut io::stdout().lock()) {
            ...
            Err(e) => {
                ...
                eprintln!(
                    "Completed frame/model publications remain retained after a later failure. Partial exports may remain."
                );
...
mod tests {
    ...
    fn model_paths_preserve_native_os_bytes() {
        ...
        let mut a = args();
        a[12] = OsString::from_vec(b"model-\xff.bin".to_vec());
