...
fn main() -> ExitCode {
    ...
    if http::handles(&args) {
        return http::dispatch(&args);
    }
    if recipes::handles(&args) {
        return recipes::dispatch(&args);
    }
    if pins::handles(&args) {
        return pins::dispatch(&args);
    }
    let result = if work::handles(&args) {
        work::execute(&args)
    } else {
        match parse_archive_args(&args) {
            Ok(None) => Ok(format!(
                "{HELP}\n{}\n{}\n{}\n{}",
                work::HELP,
                pins::HELP,
    ...
    match result {
        ...
        Err(error) => {
            ...
            if work::handles(&args) {
                eprintln!(
                    "No complete report was emitted. Only exact checkpointed roots may have committed. Retain the original work pin, inspect storage and reconcile before retrying; no automatic cleanup occurred. Use fss-archive help."
                );
            } else {
                eprintln!(
                    "No complete report was emitted. Existing roots are not repaired, deleted or replaced. An incomplete export may remain; reconcile COMPLETE.json before reuse. Use fss-archive help."
                );
            }
