...
pub(super) const MAX_EXPORT_BYTES: u64 =
    2 * 4096 * MAX_RECORDING_BYTES as u64 + MAX_REPORT_BYTES as u64 + 4096;
...
impl Destination {
    pub(super) fn begin(
        ...
        let reservation = selected_bytes
            .checked_mul(2)
            .and_then(|n| n.checked_add(MAX_REPORT_BYTES as u64 + 4096))
            .ok_or(ArchiveCommandError::ExportBudget)?;
        if reservation > options.export_budget {
            return Err(ArchiveCommandError::ExportBudget);
        }
        let archive =
            fs::canonicalize(&options.root).map_err(|e| io_error("canonicalize source", e))?;
        let output = options
            .output
            .as_deref()
        ...
        builder.create(&root).map_err(|e| {
            ArchiveCommandError::ExportIncomplete(Box::new(io_error("create export directory", e)))
        })?;
        let mut destination = Self {
            root,
            limit: options.export_budget,
            written: 0,
        };
        let request = format!(
            "{{\"schema\":\"fss.local_archive_export_intent.v1\",\"snapshot\":{},\"codec\":\"{}\",\"query\":{},\"whole_windows_authorized\":true,\"max_export_bytes\":{},\"completion_file\":\"COMPLETE.json\"}}\n",
            quoted(&snapshot.to_text()),
            options.codec.name(),
            interval(query),
            options.export_budget
        );
        let result = destination
            .file("REQUEST.json", request.as_bytes(), clock)
    ...
    pub(super) fn window(
        ...
        for ((_, _, bytes), suffix) in recording.children().into_iter().zip([
            "source.bin",
            "init.mp4",
            "media.m4s",
            "index.bin",
        ...
        playback
            ...
            .map_err(|_| ArchiveCommandError::ExportBudget)?;
    ...
    fn file(&mut self, name: &str, bytes: &[u8], clock: &impl OperationClock) -> Result<String> {
        ...
        let digest =
            ContentDigest::try_sha256(bytes).map_err(|_| ArchiveCommandError::ExportBudget)?;
        ...
        let mut file = options
            ...
            .map_err(|e| io_error("create export file", e))?;
        write_bounded(&mut file, bytes, clock)?;
        file.sync_all()
            .map_err(|e| io_error("sync export file", e))?;
        file.seek(SeekFrom::Start(0))
            .map_err(|e| io_error("seek export readback", e))?;
    ...
    pub(super) fn complete(&mut self, report: &str, clock: &impl OperationClock) -> Result<()> {
        ...
        fs::hard_link(
            self.root.join("COMPLETE.json.pending"),
...
fn new_destination(archive: &Path, output: &Path) -> Result<PathBuf> {
    ...
    if target.starts_with(archive) || archive.starts_with(&target) {
        return Err(ArchiveCommandError::OutputScope);
    }
...
fn sync_directory(path: &Path) -> Result<()> {
    File::open(path)
        ...
        .map_err(|e| io_error("sync export directory", e))
}
fn write_bounded(
    ...
    while !bytes.is_empty() {
        ...
        match writer.write(part) {
            ...
            Err(e)
                if e.kind() == io::ErrorKind::Interrupted && interrupted + 1 < MAX_INTERRUPTS =>
            {
                interrupted += 1
            }
