...
fn controls(start: u64, count: usize, x_shift: f64) -> Vec<Correspondence> {
    (0..count).map(|i| {
        ...
        Correspondence { landmark: id, physical_group: id, world,
            pixel: [800.0 * x / z + 960.0, 820.0 * y / z + 540.0] }
    }).collect()
}
fn truth() -> Result<RigidPose, GeometryError> {
    RigidPose::new([[0.8, 0.0, 0.6], [0.0, 1.0, 0.0], [-0.6, 0.0, 0.8]], [0.3, -0.4, 8.0])
}
fn distance(a: [f64; 3], b: [f64; 3]) -> f64 {
    (a[0] - b[0]).hypot(a[1] - b[1]).hypot(a[2] - b[2])
}
fn fast() -> PoseSolverOptions { PoseSolverOptions { ransac_trials: 0, ..PoseSolverOptions::default() } }

#[test]
fn recovers_camera_from_independent_image_controls() -> TestResult {
    let basis = GeometryBasis::new(1, 1)?;
    let search = estimate_camera_pose(basis, intrinsics()?, &controls(1, 24, 0.0), fast(), &mut WorkBudget::new(10_000_000))?;
    ...
    assert!(search.validate_candidate(0, basis, &controls(101, 12, 0.0), 0.01, &mut WorkBudget::new(100_000))?.passed);
    Ok(())
}

#[test]
fn six_point_nonplanar_seed_is_supported_explicitly() -> TestResult {
    let options = PoseSolverOptions { minimum_inliers: 6, minimum_inlier_fraction: 1.0, ..fast() };
    let search = estimate_camera_pose(GeometryBasis::new(1, 1)?, intrinsics()?, &controls(1, 6, 0.0), options, &mut WorkBudget::new(10_000_000))?;
...
fn rejects_planar_maps_instead_of_claiming_a_general_solver() -> TestResult {
    ...
    for point in &mut points { point.world[2] = 0.0; }
    assert!(matches!(estimate_camera_pose(GeometryBasis::new(1, 1)?, intrinsics()?, &points,
        fast(), &mut WorkBudget::new(10_000_000)), Err(GeometryError::UnsupportedGeometry)));
    Ok(())
}
...
fn input_order_cannot_change_the_seeded_result() -> TestResult {
    ...
    assert_eq!(a.candidates()[0].inlier_landmarks(), b.candidates()[0].inlier_landmarks());
...
fn robust_sampling_rejects_a_quarter_of_incorrect_matches() -> TestResult {
    ...
    for (i, point) in points.iter_mut().enumerate() {
        if i % 4 == 0 { point.pixel = [110.0 + 23.0 * i as f64, 90.0 + 17.0 * i as f64]; }
        else { point.pixel[0] += 0.15 * (i as f64 * 3.0).sin(); point.pixel[1] += 0.15 * (i as f64 * 5.0).cos(); }
    }
    let search = estimate_camera_pose(GeometryBasis::new(1, 1)?, intrinsics()?, &points,
        PoseSolverOptions::default(), &mut WorkBudget::new(100_000_000))?;
    let candidate = &search.candidates()[0];
...
fn physical_aliases_and_repeated_world_points_are_not_extra_support() -> TestResult {
    ...
    assert!(matches!(estimate_camera_pose(GeometryBasis::new(1, 1)?, intrinsics()?, &points,
        fast(), &mut WorkBudget::new(100_000)), Err(GeometryError::InvalidCorrespondence)));
