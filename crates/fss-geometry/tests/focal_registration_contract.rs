...
fn points(start:u64,count:usize)->Vec<Correspondence>{
    (0..count).map(|i|{
        let id=start+i as u64; let n=id as f64;
...
fn scan_preserves_all_samples_and_contains_true_focal_solution()->Test{
    let scan=scan_camera_focal_length(GeometryBasis::new(1,1)?,[1920,1080],&points(1,32),options(),&mut WorkBudget::new(500_000_000))?;
    assert_eq!(scan.samples().len(),33); assert!(scan.successful_samples()>0);
...
fn excluded_landmarks_can_leave_a_unique_focal_pose_without_refitting()->Test{
    ...
    let validation=scan.validate_all_candidates(basis,&points(101,12),0.01,&mut WorkBudget::new(100_000_000))?;
...
fn invalid_family_and_control_failures_refuse_partial_scans()->Test{
    ...
    assert!(matches!(scan_camera_focal_length(GeometryBasis::new(1,1)?,[1920,1080],&points(1,32),bad,&mut WorkBudget::new(100_000)),Err(GeometryError::InvalidSolverOptions)));
    let flag=AtomicBool::new(true);
    assert!(matches!(scan_camera_focal_length(GeometryBasis::new(1,1)?,[1920,1080],&points(1,32),options(),&mut WorkBudget::cancellable(500_000_000,&flag)),Err(GeometryError::Cancelled)));
    assert!(matches!(scan_camera_focal_length(GeometryBasis::new(1,1)?,[1920,1080],&points(1,32),options(),&mut WorkBudget::new(1)),Err(GeometryError::BudgetExhausted)));
    Ok(())
}
