...
/// Default ceiling on the largest off-plane distance relative to the plane's RMS extent (1e-8),
/// used by `estimate_camera_pose_adaptive` when it falls back to the planar solver. The admissible
/// range for this ratio in `estimate_planar_camera_pose` is `1e-12..=1e-4`.
pub const DEFAULT_PLANAR_RESIDUAL_RATIO: f64 = 1e-8;

/// Summary of the supporting plane fitted to a set of world points, in the plane frame used for seeding.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct PlanarSupport {
    /// Centroid of the supporting points in world coordinates.
    pub origin: V3,
    /// Unit normal of the fitted plane, oriented so its largest-magnitude component is positive.
    pub normal: V3,
    /// RMS distance of the supporting points from `origin`, in world units; the plane-frame scale, always above 1e-9.
    pub rms_extent: f64,
    /// Largest absolute distance of any supporting point from the plane, in world units; at most `rms_extent * admitted_residual_ratio`.
    pub maximum_off_plane: f64,
    /// Second-largest over largest covariance eigenvalue of the supporting points; near-collinear sets score small and are rejected.
    pub in_plane_axis_ratio: f64,
    /// The residual-ratio bound this plane passed: off-plane residual over extent never exceeded it.
...
impl PlaneFrame {
    fn point(&self, point: V3) -> V3 {
        scale(
            mv(self.axes, sub(point, self.support.origin)),
            1.0 / self.support.rms_extent,
        )
    }
}

/// Solves the camera pose by first running the general RANSAC solver and, when that reports
/// `GeometryError::UnsupportedGeometry` (planar scene), falling back to the homography-based planar
/// solver at `DEFAULT_PLANAR_RESIDUAL_RATIO`. Any other outcome is returned unchanged, with
/// `PoseSearch::work_units` recomputed to cover whichever branch ran.
pub fn estimate_camera_pose_adaptive(
    basis: GeometryBasis,
    intrinsics: PinholeIntrinsics,
    correspondences: &[Correspondence],
    options: PoseSolverOptions,
    ...
    match super::search::estimate_camera_pose(basis, intrinsics, correspondences, options, budget) {
        Err(GeometryError::UnsupportedGeometry) => {
            let mut result = estimate_planar_camera_pose(
                basis,
                intrinsics,
                correspondences,
                options,
                DEFAULT_PLANAR_RESIDUAL_RATIO,
                budget,
            )?;
...
/// Solves the camera pose assuming the scene landmarks are coplanar: fits a plane frame, derives
/// two homography-based seeds per RANSAC trial, refines and scores them, and returns surviving
/// candidates ranked by inlier count then RMS error, with `PoseSearch::planar_support` set.
/// `maximum_residual_ratio` must be finite and within `1e-12..=1e-4`; control errors abort while
/// other per-trial failures are skipped.
pub fn estimate_planar_camera_pose(
    basis: GeometryBasis,
    intrinsics: PinholeIntrinsics,
    correspondences: &[Correspondence],
    options: PoseSolverOptions,
    maximum_residual_ratio: f64,
    budget: &mut WorkBudget<'_>,
) -> Result<PoseSearch, GeometryError> {
    budget.charge(0)?;
    let started = budget.used();
    ...
    let frame = plane_frame(
        &points,
        options.minimum_axis_ratio,
        maximum_residual_ratio,
        budget,
    )?;
    let required = options
        .minimum_inliers
        .max((options.minimum_inlier_fraction * points.len() as f64).ceil() as usize);
    ...
    for trial in 0..=options.ransac_trials {
        ...
        let sample = if trial == 0 {
            ...
            for i in 0..6 {
                small[i] = points[indices[i]];
            }
            &small[..]
        };
        let sample_frame = match plane_frame(
            sample,
            options.minimum_axis_ratio,
            maximum_residual_ratio,
            budget,
        ...
        for seed in seeds {
            let initial = match refine(
                sample,
                intrinsics,
                seed,
                sample_frame.support.rms_extent,
                options.refinement_iterations.min(10),
            ...
            let (inliers, rms_px, maximum_error_px) = score(
                &points,
                intrinsics,
                pose,
                options.inlier_threshold_px,
            ...
            if inliers.len() < required
                || !image_spread(&points, &inliers, intrinsics, options.minimum_image_span)
            {
                continue;
            }
            let selected = gather(&points, &inliers, budget)?;
            let support_scale = match plane_frame(
                &selected,
                options.minimum_axis_ratio,
                maximum_residual_ratio,
                budget,
            ...
            if let Some(existing) = candidates
                .iter_mut()
                .find(|c| equivalent_pose(c.pose, pose, c.support_scale.min(support_scale)))
            {
                ...
                if candidates.len() == 8 {
                    return Err(GeometryError::TooManyPoseCandidates);
                }
    ...
    if candidates.is_empty() {
        return Err(GeometryError::NoPoseConsensus);
    }
    candidates.sort_by(|a, b| {
        b.inlier_landmarks
            .len()
            .cmp(&a.inlier_landmarks.len())
            .then(a.rms_px.total_cmp(&b.rms_px))
    });
    budget.charge(0)?;
    Ok(PoseSearch {
        basis,
        ...
        fit: points,
        candidates,
        trials_attempted: options.ransac_trials + 1,
...
fn signed_axis(mut axis: V3) -> Result<V3, GeometryError> {
    ...
    let mut largest = 0;
    for i in 1..3 {
        if axis[i].abs() > axis[largest].abs() {
            largest = i;
        }
    }
    if axis[largest] < 0.0 {
...
fn plane_frame(
    ...
    maximum_residual_ratio: f64,
    budget: &mut WorkBudget<'_>,
) -> Result<PlaneFrame, GeometryError> {
    let first = points.first().ok_or(GeometryError::EmptyInput)?.world;
    let count = points.len() as f64;
    let mut offset = [0.0; 3];
    for point in points {
        budget.charge(4)?;
        offset = add(offset, scale(sub(point.world, first), 1.0 / count));
    }
    let origin = add(first, offset);
    let mut squared = 0.0;
    for point in points {
        budget.charge(4)?;
        let delta = sub(point.world, origin);
        squared += dot(delta, delta) / count;
    }
    ...
    if largest <= 0.0 || eigen.values[order[1]] <= largest * minimum_ratio {
    ...
    for point in points {
        budget.charge(4)?;
        residual = residual.max(dot(sub(point.world, origin), normal).abs());
    }
    if residual / extent > maximum_residual_ratio {
        return Err(GeometryError::UnsupportedGeometry);
    }
    Ok(PlaneFrame {
        support: PlanarSupport {
            ...
            normal,
            rms_extent: extent,
            maximum_off_plane: residual,
            in_plane_axis_ratio: eigen.values[order[1]] / largest,
            admitted_residual_ratio: maximum_residual_ratio,
        },
        axes: [x, y, normal],
    })
}
...
fn planar_seeds(
    ...
    frame: &PlaneFrame,
    budget: &mut WorkBudget<'_>,
) -> Result<[RigidPose; 2], GeometryError> {
    ...
    if !lambda.is_finite() || lambda <= 1e-24 {
        return Err(GeometryError::Degenerate);
    }
    let alpha2 = ((lambda - aa) / g2).max(0.0);
    let beta2 = ((lambda - bb) / g2).max(0.0);
    let (alpha, beta) = if alpha2.max(beta2) <= lambda * 1e-28 {
        (0.0, 0.0)
    ...
    for (index, sign) in [1.0, -1.0].into_iter().enumerate() {
        ...
        let shift = sub(
            scale(shift, frame.support.rms_extent),
            mv(rotation, frame.support.origin),
        );
...
fn image_point(point: &Correspondence, k: PinholeIntrinsics) -> Result<[f64; 2], GeometryError> {
    let focal = k.focal_lengths();
    let center = k.principal_point();
    let uv = [
        (point.pixel[0] - center[0]) / focal[0],
        (point.pixel[1] - center[1]) / focal[1],
    ];
    if uv.iter().any(|x| !x.is_finite() || x.abs() > 1e4) {
...
fn homography(
    ...
    for point in points {
        budget.charge(8)?;
        let uv = image_point(point, k)?;
        squared += ((uv[0] - center[0]).powi(2) + (uv[1] - center[1]).powi(2)) / count;
    }
    let extent = squared.sqrt();
    ...
    order.sort_by(|a, b| eigen.values[*a].total_cmp(&eigen.values[*b]).then(a.cmp(b)));
    if eigen.values[order[1]] <= eigen.values[order[8]] * 1e-10 {
        return Err(GeometryError::Degenerate);
    }
    let h: M3 =
        std::array::from_fn(|i| std::array::from_fn(|j| eigen.vectors[i * 3 + j][order[0]]));
    let mut h = multiply(
        [
            [extent, 0.0, center[0]],
            [0.0, extent, center[1]],
...
fn translation(
    ...
    if eigen.values.iter().any(|x| *x <= maximum * 1e-12) {
        return Err(GeometryError::Degenerate);
    }
