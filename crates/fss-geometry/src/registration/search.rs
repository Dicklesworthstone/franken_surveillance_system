...
/// General (non-planar) RANSAC pose solve: each trial fits a linear pose to six sampled
/// correspondences, refines it, and scores the full set; surviving candidates are ranked by inlier
/// count then RMS error with `PoseSearch::planar_support` left unset. Fails up front with
/// `GeometryError::UnsupportedGeometry` when the landmark cloud is planar or nearly degenerate
/// along one axis, so callers can fall back to the planar solver.
pub fn estimate_camera_pose(
    ...
    for trial in 0..=options.ransac_trials {
        budget.charge(1)?;
        let sample: Vec<Correspondence> = if trial == 0 {
            points.clone()
        } else {
            for i in 0..6 {
        ...
        let candidate = PoseCandidate {
            pose,
            inlier_landmarks: selected.iter().map(|p| p.landmark).collect(),
            rms_px,
            maximum_error_px,
            support_scale,
        };
...
pub(super) fn control_failure(error: GeometryError) -> bool {
    matches!(
        error,
        GeometryError::Cancelled | GeometryError::BudgetExhausted | GeometryError::LimitExceeded
    )
}
...
pub(super) fn score(
    ...
    threshold: f64,
    budget: &mut WorkBudget<'_>,
) -> Result<(Vec<usize>, f64, f64), GeometryError> {
    ...
    let rms = if inliers.is_empty() {
        ...
        (squared / inliers.len() as f64).sqrt()
    };
    Ok((inliers, rms, maximum))
}

pub(super) fn image_spread(
    points: &[Correspondence],
    indices: &[usize],
    k: PinholeIntrinsics,
    floor: f64,
    ...
    for &i in indices {
        for axis in 0..2 {
            minimum[axis] = minimum[axis].min(points[i].pixel[axis]);
            maximum[axis] = maximum[axis].max(points[i].pixel[axis]);
        }
    }
...
pub(super) fn equivalent_pose(a: RigidPose, b: RigidPose, size: f64) -> bool {
    if norm(sub(a.center(), b.center())) > 0.01 * size {
        return false;
    }
    let ra = a.rotation();
    let rb = b.rotation();
