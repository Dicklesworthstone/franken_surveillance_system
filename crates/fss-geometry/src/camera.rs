...
use crate::math::{IDENTITY, M3, V3, add, checked, determinant, dot, mv, normalize, scale, transpose};
...
impl PinholeIntrinsics {
    ...
    pub fn new(width: u32, height: u32, fx: f64, fy: f64, cx: f64, cy: f64) -> Result<Self, GeometryError> {
        if [fx, fy, cx, cy].iter().any(|x| !x.is_finite()) { return Err(GeometryError::NonFinite); }
        if width == 0 || height == 0 || width > 65536 || height > 65536
            || fx < 1e-6 || fy < 1e-6 || fx > 1e9 || fy > 1e9 || cx.abs() > 1e9 || cy.abs() > 1e9 {
            return Err(GeometryError::InvalidCamera);
        }
        Ok(Self { width, height, fx, fy, cx, cy })
    }
    ...
    pub fn focal_lengths(self) -> [f64; 2] { [self.fx, self.fy] }
    /// Principal point in the declared pixel-edge coordinates.
    pub fn principal_point(self) -> [f64; 2] { [self.cx, self.cy] }
    /// Whether a finite pixel coordinate belongs to this half-open image domain.
    pub fn contains(self, pixel: [f64; 2]) -> bool {
        pixel[0].is_finite() && pixel[1].is_finite() && pixel[0] >= 0.0 && pixel[1] >= 0.0
            && pixel[0] < f64::from(self.width) && pixel[1] < f64::from(self.height)
    }
    ...
    pub fn project(self, camera_point: V3) -> Result<[f64; 2], GeometryError> {
        ...
        if camera_point[2] <= 1e-9 { return Err(GeometryError::BehindCamera); }
        let pixel = [self.fx * (camera_point[0] / camera_point[2]) + self.cx,
                     self.fy * (camera_point[1] / camera_point[2]) + self.cy];
        if pixel.iter().any(|x| !x.is_finite()) { return Err(GeometryError::NonFinite); }
        Ok(pixel)
    }
    ...
    pub fn bearing(self, pixel: [f64; 2]) -> Result<V3, GeometryError> {
        if !self.contains(pixel) { return Err(GeometryError::OutOfImage); }
        normalize([(pixel[0] - self.cx) / self.fx, (pixel[1] - self.cy) / self.fy, 1.0])
    }
}
...
impl RigidPose {
    ...
    pub const IDENTITY: Self = Self { rotation: IDENTITY, translation: [0.0; 3] };
    ...
    pub fn new(rotation: M3, translation: V3) -> Result<Self, GeometryError> {
        checked(translation)?;
        for row in rotation { checked(row)?; }
        if (determinant(rotation) - 1.0).abs() > 1e-8 { return Err(GeometryError::InvalidRotation); }
        for (i, a) in rotation.iter().enumerate() {
            for (j, b) in rotation.iter().enumerate() {
                let expected = if i == j { 1.0 } else { 0.0 };
                if (dot(*a, *b) - expected).abs() > 1e-8 { return Err(GeometryError::InvalidRotation); }
            }
        }
    ...
    pub fn rotation(self) -> M3 { self.rotation }
    /// World-to-camera translation. This is not the optical center.
    pub fn translation(self) -> V3 { self.translation }
    /// Camera optical center expressed in world coordinates: `-R^T t`.
    pub fn center(self) -> V3 { scale(mv(transpose(self.rotation), self.translation), -1.0) }
    ...
    pub fn project(self, intrinsics: PinholeIntrinsics, point: V3) -> Result<[f64; 2], GeometryError> {
    ...
    pub fn ray(self, intrinsics: PinholeIntrinsics, pixel: [f64; 2]) -> Result<Ray, GeometryError> {
        Ray::new(self.center(), mv(transpose(self.rotation), intrinsics.bearing(pixel)?))
    }
}

/// Validated positive-distance ray with a unit world-space direction.
#[derive(Clone, Copy, PartialEq)]
pub struct Ray { origin: V3, direction: V3 }
...
impl Ray {
    ...
    pub fn new(origin: V3, direction: V3) -> Result<Self, GeometryError> {
        Ok(Self { origin: checked(origin)?, direction: normalize(direction)? })
    }
    /// Origin in the caller's declared world frame.
    pub fn origin(self) -> V3 { self.origin }
    /// Unit direction in that same frame.
    pub fn direction(self) -> V3 { self.direction }
    ...
    pub fn at(self, distance: f64) -> Result<V3, GeometryError> {
        if !distance.is_finite() || !(0.0..=1e12).contains(&distance) { return Err(GeometryError::OutOfRange); }
