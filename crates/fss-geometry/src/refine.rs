...
fn cost(
    ...
    for point in points {
        ...
        total += huber(
            (point.pixel[0] - pixel[0]).hypot(point.pixel[1] - pixel[1]),
            threshold,
        );
    }
    if !total.is_finite() {
...
pub(crate) fn refine(
    ...
    for _ in 0..iterations {
        ...
        let candidate = RigidPose::new(
            multiply(delta, pose.rotation()),
