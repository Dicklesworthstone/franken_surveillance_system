...
use crate::{
    CameraHandoffForecast, ForecastBasis, GeometryError, HandoffCamera, HandoffOptions, ImageRect,
    MotionForecast, NanosecondInterval, NextCameraOutcome, RouteBody, TriangleMesh, WorkBudget,
...
impl std::fmt::Debug for HandoffScenario<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HandoffScenario")
            .field("id", &self.id)
...
pub struct ScenarioCameraEnvelope {
    /// Camera ID, unique within the envelope list.
    pub camera: u64,
...
pub struct ScenarioRouteEnvelope {
    /// Route ID, identical across all scenarios (routes are matched by ID).
    pub route: u64,
    /// Unchanged heuristic route mass copied from every scenario; never renormalized.
    pub mass: u64,
    /// Protected alternative flag, identical across all scenarios.
    pub protected: bool,
...
pub struct ScenarioHandoffForecast {
    /// Forecast basis shared by every per-scenario result; mismatch is a refusal.
    pub basis: ForecastBasis,
    /// Scenario IDs in the caller-supplied order.
    pub scenario_ids: Vec<u64>,
    /// One complete forecast per scenario, in `scenario_ids` order.
    pub forecasts: Vec<CameraHandoffForecast>,
    /// Conservative per-route envelopes, in route-ID order.
...
pub fn predict_camera_handoff_scenarios(
    expected_basis: ForecastBasis,
    motion: &MotionForecast,
    mesh: &TriangleMesh,
    scenarios: &[HandoffScenario<'_>],
    bodies: &[RouteBody<'_>],
    options: HandoffOptions,
    budget: &mut WorkBudget<'_>,
) -> Result<ScenarioHandoffForecast, GeometryError> {
    budget.charge(0)?;
    if scenarios.is_empty() {
        return Err(GeometryError::EmptyInput);
    }
    if scenarios.len() > MAX_HANDOFF_SCENARIOS {
        return Err(GeometryError::LimitExceeded);
    }
    let mut ids = Vec::new();
    let mut forecasts = Vec::new();
    ids.try_reserve_exact(scenarios.len())
    ...
    for (index, scenario) in scenarios.iter().enumerate() {
        ...
        if scenario.id == 0 || ids.contains(&scenario.id) {
        ...
        if camera_ids.windows(2).any(|w| w[0] == w[1]) {
            return Err(GeometryError::InvalidIndex);
        }
        ...
        let forecast = predict_camera_handoffs(
            expected_basis,
            motion,
            mesh,
            scenario.cameras,
    ...
    for forecast in &forecasts[1..] {
        if forecast.basis != first.basis
            || forecast.reference_ns != first.reference_ns
            || forecast.horizon_ns != first.horizon_ns
            || forecast.total_mass != first.total_mass
            || forecast.routes.len() != first.routes.len()
        {
    ...
    routes
        .try_reserve_exact(first.routes.len())
        .map_err(|_| GeometryError::LimitExceeded)?;
    for route_index in 0..first.routes.len() {
        ...
        camera_acc
            .try_reserve_exact(canonical_cameras.as_ref().map_or(0, Vec::len))
            .map_err(|_| GeometryError::LimitExceeded)?;
        let mut robust: Option<Vec<u64>> = None;
        let mut robust_possible = true;
                ...
                NextCameraOutcome::Predicted {
                    ...
                    for &camera in cameras {
                        let observation = route
                            ...
                            .iter()
                            .find(|o| {
                                o.camera == camera && o.nominal_capture_ns == *nominal_capture_ns
                            })
        ...
        cameras
            .try_reserve_exact(camera_acc.len())
            .map_err(|_| GeometryError::LimitExceeded)?;
        for item in camera_acc {
            cameras.push(ScenarioCameraEnvelope {
                camera: item.camera,
        ...
        routes.push(ScenarioRouteEnvelope {
            route: source.route,
            mass: source.mass,
            protected: source.protected,
            cameras,
            robust_next_cameras: if robust_possible { robust } else { None },
            has_indeterminate,
            has_no_modeled_observation: has_none,
        });
    }
    budget.charge(0)?;
    Ok(ScenarioHandoffForecast {
...
fn add_camera(
    ...
    if let Some(item) = acc.iter_mut().find(|item| item.camera == camera) {
        ...
        item.earliest = item.earliest.min(time);
        item.latest = item.latest.max(time);
        for axis in 0..2 {
            item.min[axis] = item.min[axis].min(lo[axis]);
            item.max[axis] = item.max[axis].max(hi[axis]);
        }
    } else {
