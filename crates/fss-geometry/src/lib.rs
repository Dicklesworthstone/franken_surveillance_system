...
pub enum GeometryError {
    NonFinite, OutOfRange, InvalidCamera, InvalidRotation, BehindCamera, OutOfImage,
    Degenerate, EmptyInput, LimitExceeded, BudgetExhausted, Cancelled, BasisMismatch,
    InvalidIndex, InvalidSolverOptions, InvalidCorrespondence, InsufficientCorrespondences,
    UnsupportedGeometry, SolverDidNotConverge, NoPoseConsensus, TooManyPoseCandidates, HoldoutLeak,
}
impl std::fmt::Display for GeometryError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::NonFinite => "nonfinite geometry", Self::OutOfRange => "geometry outside admitted numeric range",
            Self::InvalidCamera => "invalid pinhole image model", Self::InvalidRotation => "invalid proper rotation",
            Self::BehindCamera => "point not in front of camera", Self::OutOfImage => "observation outside declared image",
            Self::Degenerate => "degenerate geometric constraint", Self::EmptyInput => "empty geometry",
            Self::LimitExceeded => "geometry resource limit exceeded", Self::BudgetExhausted => "geometry work budget exhausted",
            Self::Cancelled => "geometry operation cancelled", Self::BasisMismatch => "geometry basis mismatch",
            Self::InvalidIndex => "invalid geometry reference", Self::HoldoutLeak => "validation landmark overlaps fitting evidence",
            Self::TooManyPoseCandidates => "pose ambiguity exceeds candidate limit", Self::NoPoseConsensus => "no admissible pose consensus",
            Self::SolverDidNotConverge => "bounded pose solver did not converge", Self::UnsupportedGeometry => "planar or weak geometry requires another solver",
            Self::InsufficientCorrespondences => "insufficient pose correspondences", Self::InvalidCorrespondence => "invalid or duplicate correspondence",
...
#[derive(Debug)]
pub struct WorkBudget<'a> { limit:u64, used:u64, cancellation:Option<&'a AtomicBool> }
impl WorkBudget<'_> {
...
impl<'a> WorkBudget<'a>{
    pub fn cancellable(limit:u64,cancellation:&'a AtomicBool)->Self{Self{limit,used:0,cancellation:Some(cancellation)}}
}
