...
use crate::{
    ForecastBasis, GeometryBasis, GeometryError, MotionForecast, MotionHypothesis,
...
impl NanosecondInterval {
    ...
    fn shifted(self, time: u64) -> Result<Self, GeometryError> {
        Self::new(
            time.checked_add(self.earliest)
                .ok_or(GeometryError::OutOfRange)?,
            time.checked_add(self.latest)
                .ok_or(GeometryError::OutOfRange)?,
        )
    }
}
...
impl CaptureSchedule {
    ...
    pub fn new(period_ns: u64, phase_ns: u64) -> Result<Self, GeometryError> {
        if period_ns == 0 || phase_ns >= period_ns {
            return Err(GeometryError::OutOfRange);
        }
        Ok(Self {
            period_ns,
    ...
    pub fn first_at_or_after(self, time: u64) -> Option<u64> {
        ...
        let wait = if remainder <= self.phase_ns {
            self.phase_ns - remainder
        } else {
            self.period_ns - (remainder - self.phase_ns)
        };
...
impl std::fmt::Debug for BodySamples {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BodySamples")
            .field("generation", &self.generation)
            .field("sample_count", &self.offsets.len())
...
impl BodySamples {
    ...
    pub fn new(
        ...
        if !(2..=MAX_BODY_SAMPLES).contains(&offsets.len()) {
        ...
        for (index, offset) in offsets.iter().enumerate() {
            ...
            if offsets[..index]
                .iter()
                .any(|prior| norm(sub(*offset, *prior)) <= 1e-12)
            {
...
pub fn predict_camera_handoffs(
    expected_basis: ForecastBasis,
    ...
    if cameras.len() > MAX_HANDOFF_CAMERAS
        || bodies.len() != motion.hypotheses().len()
        || options.max_samples_per_camera == 0
        || options.max_samples_per_camera > 1_000_000
    {
        return Err(GeometryError::LimitExceeded);
    }
    if !options.endpoint_margin.is_finite()
        || options.endpoint_margin < 0.0
        || options.endpoint_margin > 1e6
    {
        return Err(GeometryError::OutOfRange);
    }
    let end = motion
        .reference_ns()
        .checked_add(motion.horizon_ns())
        .ok_or(GeometryError::OutOfRange)?;
    ...
    for (index, camera) in cameras.iter().enumerate() {
        ...
        if camera.geometry != expected_basis.geometry()
            || camera.clock != expected_basis.clock()
    ...
    for (index, binding) in bodies.iter().enumerate() {
        ...
        if bodies[..index]
            .iter()
            .any(|prior| prior.route == binding.route)
            || !motion
                .hypotheses()
                .iter()
                .any(|route| route.id() == binding.route)
        {
            return Err(GeometryError::InvalidIndex);
        }
    }
    ...
    for camera in &ordered {
        camera_scope.push(HandoffCameraScope {
            camera: camera.id,
            observation_generation: camera.observation_generation,
            image_mode: camera.image_mode,
            availability: camera.availability,
            already_observing: camera.already_observing,
        });
    }
    ...
    for path in motion.hypotheses() {
        ...
        let body = bodies
            .iter()
            .find(|binding| binding.route == path.id())
        ...
        for camera in &ordered {
            budget.charge(1)?;
            if camera.already_observing || camera.availability == CameraAvailability::Unavailable {
                continue;
            }
            ...
            if let Some(observation) =
                first_sample(motion, path, mesh, camera, body, options, budget)?
            {
        ...
        let next = if !unresolved.is_empty() || (unmodeled_tail && observations.is_empty()) {
            ...
            for observation in &observations {
                budget.charge(1)?;
                if observation.nominal_capture_ns != time {
                    break;
                }
                simultaneous.push(observation.camera);
            }
            NextCameraOutcome::Predicted {
                nominal_capture_ns: time,
                cameras: simultaneous,
            }
        } else {
            NextCameraOutcome::NoModeledObservation
        };
        routes.push(RouteHandoff {
            route: path.id(),
            mass: path.mass(),
    ...
    budget.charge(0)?;
    Ok(CameraHandoffForecast {
        basis: expected_basis,
        reference_ns: motion.reference_ns(),
        horizon_ns: motion.horizon_ns(),
        total_mass: motion.total_mass(),
        camera_scope,
        routes,
    })
}
...
fn first_sample(
    motion: &MotionForecast,
    path: &MotionHypothesis,
    mesh: &TriangleMesh,
    camera: &HandoffCamera<'_>,
    body: &BodySamples,
    options: HandoffOptions,
    budget: &mut WorkBudget<'_>,
) -> Result<Option<PredictedObservation>, GeometryError> {
    let start = motion.reference_ns().max(camera.valid.earliest());
    let end = motion
        .reference_ns()
        .checked_add(path.modeled_until_ns())
        .ok_or(GeometryError::OutOfRange)?
        .min(camera.valid.latest());
    ...
    while let Some(time) = sample {
        ...
        if samples == options.max_samples_per_camera {
            return Err(GeometryError::LimitExceeded);
        }
        samples += 1;
        let position = path
            .position_at(time - motion.reference_ns(), budget)?
            .ok_or(GeometryError::OutOfRange)?;
        if let Some((region, visible)) =
            eligible_region(mesh, camera, body, position, options, budget)?
        {
            return Ok(Some(PredictedObservation {
                camera: camera.id,
                observation_generation: camera.observation_generation,
                image_mode: camera.image_mode,
                nominal_capture_ns: time,
                availability_ns: camera.latency.shifted(time)?,
                region,
                visible_samples: visible,
                body_samples: body.sample_count(),
            }));
        }
...
fn eligible_region(
    ...
    for mask in camera.privacy_masks {
        budget.charge(1)?;
        if projected_region.overlaps(*mask) {
            return Ok(None);
        }
    }
    min = [1.0; 2];
    ...
    for index in 0..projected {
        ...
        if mesh.segment_occluded(
            camera.geometry,
            camera.pose.center(),
            point,
            options.endpoint_margin,
            budget,
        )? {
            continue;
        }
