...
impl ForecastBasis {
    ...
    pub fn track_revision(self) -> u64 {
        self.track_revision
    }
    /// Version of the caller's route and motion assumptions.
    pub fn motion_generation(self) -> u64 {
...
impl RoutePriors {
    ...
    pub fn new(class_masses: [u32; 4], person_path_multiplier: u32) -> Result<Self, GeometryError> {
        if class_masses == [0; 4]
            || class_masses.iter().any(|x| *x > 1_000_000)
    ...
    pub fn class_masses(self) -> [u32; 4] {
        self.class_masses
    }
    ...
    fn mass(self, surface: RouteSurface, base: u32) -> Result<u64, GeometryError> {
        ...
        for (index, mass) in self.class_masses.iter().enumerate() {
            ...
            class_mass = class_mass
                .checked_add(u64::from(*mass) * multiplier)
...
impl MotionHypothesis {
    ...
    pub fn position_at(
        ...
        budget.charge(10)?;
        if offset_ns > self.modeled_until_ns() {
            return Ok(None);
        }
        let index = self
            .knots
            .partition_point(|knot| knot.offset_ns < offset_ns);
        // <= 258 knots: at most nine binary-search comparisons.
        let right = self.knots.get(index).ok_or(GeometryError::OutOfRange)?;
        if right.offset_ns == offset_ns {
            return Ok(Some(right.position));
        }
        let left = self
            .knots
            .get(index.checked_sub(1).ok_or(GeometryError::OutOfRange)?)
            .ok_or(GeometryError::OutOfRange)?;
        let fraction =
            (offset_ns - left.offset_ns) as f64 / (right.offset_ns - left.offset_ns) as f64;
        Ok(Some(checked(add(
            left.position,
...
impl MotionForecast {
    ...
    pub fn basis(&self) -> ForecastBasis {
        self.basis
    }
...
pub fn forecast_routes(
    basis: ForecastBasis,
    reference_ns: u64,
    horizon_ns: u64,
    priors: RoutePriors,
    routes: &[RouteCandidate<'_>],
    budget: &mut WorkBudget<'_>,
) -> Result<MotionForecast, GeometryError> {
    budget.charge(0)?;
    if horizon_ns == 0
        || horizon_ns > MAX_FORECAST_NS
    ...
    if routes.len() > MAX_MOTION_ROUTES {
    ...
    for (ordinal, route) in routes.iter().enumerate() {
        ...
        if !route.speed.is_finite()
            || !(1e-9..=1e12).contains(&route.speed)
        ...
        knots
            .try_reserve_exact(route.points.len() + 2)
            .map_err(|_| GeometryError::LimitExceeded)?;
        knots.push(MotionKnot {
            offset_ns: 0,
        ...
        for segment in route.points.windows(2) {
            budget.charge(1)?;
            if offset == horizon_ns {
                break;
            }
            let duration = (norm(sub(segment[1], segment[0])) / route.speed * 1e9)
                .ceil()
                .max(1.0);
            ...
            if duration > remaining as f64 {
                let position = checked(add(
                    segment[0],
                    scale(sub(segment[1], segment[0]), remaining as f64 / duration),
                ))?;
                knots.push(MotionKnot {
                    offset_ns: horizon_ns,
                    position,
                });
                offset = horizon_ns;
                break;
        ...
        hypotheses.push(MotionHypothesis {
            id: route.id,
            mass,
