...
pub struct GeometryBasis { property: u64, revision: u64 }
...
impl GeometryBasis {
    ...
    pub fn new(property: u64, revision: u64) -> Result<Self, GeometryError> {
        if property == 0 || revision == 0 { return Err(GeometryError::BasisMismatch); }
...
impl Default for MeshLimits {
    fn default() -> Self { Self { max_vertices: 262_144, max_triangles: 524_288 } }
}
...
impl std::fmt::Debug for SurfaceHit {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SurfaceHit").field("triangle", &self.triangle).finish_non_exhaustive()
    }
}
...
impl std::fmt::Debug for TriangleMesh {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TriangleMesh").field("vertex_count", &self.vertices.len())
            .field("triangle_count", &self.triangles.len()).finish_non_exhaustive()
    }
}
...
impl TriangleMesh {
    ...
    pub fn from_indexed(basis: GeometryBasis, vertices: &[V3], triangles: &[IndexedTriangle],
        limits: MeshLimits, budget: &mut WorkBudget<'_>) -> Result<Self, GeometryError> {
        budget.charge(0)?;
        if vertices.is_empty() || triangles.is_empty() { return Err(GeometryError::EmptyInput); }
        if limits.max_vertices > 262_144 || limits.max_triangles > 524_288
            || vertices.len() > limits.max_vertices || triangles.len() > limits.max_triangles {
            return Err(GeometryError::LimitExceeded);
        }
        for vertex in vertices { budget.charge(1)?; checked(*vertex)?; }
        for triangle in triangles {
            budget.charge(1)?;
            if triangle.feature == 0 { return Err(GeometryError::InvalidIndex); }
        ...
        owned_vertices.try_reserve_exact(vertices.len()).map_err(|_| GeometryError::LimitExceeded)?;
        owned_vertices.extend_from_slice(vertices);
        let mut owned_triangles = Vec::new();
        owned_triangles.try_reserve_exact(triangles.len()).map_err(|_| GeometryError::LimitExceeded)?;
        owned_triangles.extend_from_slice(triangles);
        budget.charge(0)?;
        Ok(Self { basis, vertices: owned_vertices, triangles: owned_triangles })
    }
    ...
    pub fn support_hits(&self, basis: GeometryBasis, ray: Ray, near: f64, far: f64,
        max_hits: usize, budget: &mut WorkBudget<'_>) -> Result<Vec<SurfaceHit>, GeometryError> {
        self.validate_query(basis, near, far, budget)?;
        if max_hits == 0 || max_hits > 4096 { return Err(GeometryError::LimitExceeded); }
        let mut hits = Vec::new();
        hits.try_reserve(max_hits).map_err(|_| GeometryError::LimitExceeded)?;
        for (index, triangle) in self.triangles.iter().enumerate() {
            ...
            if let Some(hit) = intersect(&self.vertices, *triangle, index as u32, ray, near, far)? {
                if hits.len() == max_hits { return Err(GeometryError::LimitExceeded); }
                hits.push(hit);
            }
        }
        budget.charge((hits.len() as u64) * 12)?;
        hits.sort_by(|a, b| a.distance.total_cmp(&b.distance)
            .then(a.feature.cmp(&b.feature)).then(a.triangle.cmp(&b.triangle)));
        budget.charge(0)?;
        Ok(hits)
    }
    ...
    pub fn segment_occluded(&self, basis: GeometryBasis, from: V3, to: V3,
        endpoint_margin: f64, budget: &mut WorkBudget<'_>) -> Result<bool, GeometryError> {
        checked(from)?;
        checked(to)?;
        ...
        for (index, triangle) in self.triangles.iter().enumerate() {
            budget.charge(1)?;
            if triangle.opaque && intersect(&self.vertices, *triangle, index as u32, ray,
                endpoint_margin, length - endpoint_margin)?.is_some() {
                budget.charge(0)?;
                return Ok(true);
            }
        }
    ...
    fn validate_query(&self, basis: GeometryBasis, near: f64, far: f64,
        budget: &mut WorkBudget<'_>) -> Result<(), GeometryError> {
        budget.charge(0)?;
...
fn triangle_vertices(vertices: &[V3], triangle: IndexedTriangle) -> Result<[V3; 3], GeometryError> {
    ...
    for (point, index) in points.iter_mut().zip(triangle.vertices) {
        *point = *vertices.get(index as usize).ok_or(GeometryError::InvalidIndex)?;
    }
    Ok(points)
}

fn intersect(vertices: &[V3], triangle: IndexedTriangle, index: u32, ray: Ray,
    near: f64, far: f64) -> Result<Option<SurfaceHit>, GeometryError> {
    let [a, b, c] = triangle_vertices(vertices, triangle)?;
    let e1 = sub(b, a);
    let e2 = sub(c, a);
    let p = cross(ray.direction(), e2);
    let det = dot(e1, p);
    if det.abs() <= 1e-12 * norm(e1) * norm(e2) { return Ok(None); }
    let tvec = sub(ray.origin(), a);
    let u = dot(tvec, p) / det;
    let q = cross(tvec, e1);
    let v = dot(ray.direction(), q) / det;
    if u < -1e-10 || v < -1e-10 || u + v > 1.0 + 1e-10 { return Ok(None); }
    let distance = dot(e2, q) / det;
    if distance < near || distance > far { return Ok(None); }
    let normal = cross(e1, e2);
    Ok(Some(SurfaceHit { distance, point: ray.at(distance)?, barycentric: [1.0 - u - v, u, v],
        normal: scale(normal, 1.0 / norm(normal)), feature: triangle.feature, triangle: index }))
}
