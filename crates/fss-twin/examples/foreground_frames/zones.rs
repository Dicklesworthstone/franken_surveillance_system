...
use fss_twin::image_zones::{
    ImageZoneBasis, ImageZoneMonitor, ImageZonePolicy, ImageZoneReport, ImageZoneSpec,
    MAX_IMAGE_ZONES, MAX_ZONE_VERTICES,
};
use std::collections::BTreeMap;
use std::io::Write;

pub(super) const SETTINGS: [&str; 3] = [
    "zone_selection_evidence",
    "zone_maximum_sample_gap_ns",
    "zones",
];
...
fn parse_zones(text: &str) -> Result<Vec<ImageZoneSpec>> {
    if text.len() > 32768 {
        return Err("zone declaration exceeds byte limit".into());
    }
    let mut zones = Vec::new();
    zones.try_reserve_exact(MAX_IMAGE_ZONES)?;
    for declaration in text.split(';') {
        if zones.len() == MAX_IMAGE_ZONES {
        ...
        let dwell_ns = if dwell == "off" {
            None
        } else {
            Some(dwell.parse()?)
        };
        let mut vertices = Vec::new();
        vertices.try_reserve_exact(MAX_ZONE_VERTICES)?;
        for point in parts {
            if vertices.len() == MAX_ZONE_VERTICES {
                return Err("too many zone vertices".into());
            }
            let (x, y) = point.split_once(',').ok_or("expected x,y zone vertex")?;
            vertices.push([x.parse()?, y.parse()?]);
        }
        if vertices.len() < 3 {
            return Err("a zone requires at least three vertices".into());
        }
...
) -> Result<ImageZoneMonitor> {
    let get = |key| settings.get(key).copied().ok_or("missing zone setting");
    let policy = ImageZonePolicy {
        selection_evidence: hash(get("zone_selection_evidence")?)?,
        maximum_sample_gap_ns: get("zone_maximum_sample_gap_ns")?.parse()?,
    };
    Ok(ImageZoneMonitor::new(
        tracker,
        basis,
        policy,
        &parse_zones(get("zones")?)?,
        budget,
    )?)
}
pub(super) fn write_configuration(out: &mut impl Write, monitor: &ImageZoneMonitor) -> Result<()> {
    ...
    write!(
        out,
        "{{\"kind\":\"zone_configuration\",\"config\":\"{}\",\"camera\":{},\"clock\":{},\"calibration\":\"{}\",\"image_domain\":\"{}\",\"dimensions\":{:?},\"selection_evidence\":\"{}\",\"maximum_sample_gap_ns\":{},\"zones\":[",
        hex(monitor.config_digest()),
        b.camera,
        b.clock,
        hex(b.calibration),
        hex(b.image_domain),
        b.dimensions,
        hex(p.selection_evidence),
        p.maximum_sample_gap_ns
    )?;
    for (i, z) in monitor.zones().iter().enumerate() {
        ...
        write!(
            out,
            "{{\"id\":{},\"vertices\":{:?},\"margin\":{},\"dwell_ns\":",
            z.id, z.vertices, z.margin
        )?;
        if let Some(n) = z.dwell_ns {
            write!(out, "{n}")?;
        } else {
            write!(out, "null")?;
        }
...
fn observation(out: &mut impl Write, o: ImageTrackObservation) -> Result<()> {
    ...
    write!(
        out,
        "{{\"exposure\":\"{}\",\"pixels\":\"{}\",\"camera\":{},\"clock\":{},\"capture\":{:?},\"image_domain\":\"{}\",\"calibration\":\"{}\",\"dimensions\":{:?},\"detector\":\"{}\",\"mask\":\"{}\",\"foreground\":\"{}\",\"availability\":\"{:?}\",\"detection\":{},\"evidence\":\"{}\",\"min\":{:?},\"max\":{:?},\"partial\":{}}}",
...
pub(super) fn write_report(out: &mut impl Write, report: &ImageZoneReport) -> Result<()> {
    ...
    write!(
        out,
        "{{\"kind\":\"zones\",\"report\":\"{}\",\"prior\":\"{}\",\"config\":\"{}\",\"tracking\":\"{}\",\"foreground\":\"{}\",\"exposure\":\"{}\",\"capture\":{:?},\"availability\":\"{:?}\",\"cells\":[",
        hex(report.digest()),
        hex(report.prior_digest()),
        hex(report.config_digest()),
        hex(report.tracking_digest()),
        hex(f.evidence),
        hex(f.source.image.exposure),
        f.source.capture,
        f.availability
    )?;
    for (i, c) in report.cells().iter().enumerate() {
        ...
        write!(
            out,
            "{{\"track\":{},\"zone\":{},\"relation\":\"{:?}\",\"last_observation\":",
            c.track, c.zone, c.relation
        )?;
        observation(out, c.last_observation)?;
        write!(out, ",\"dwell_start\":")?;
        if let Some(first) = c.dwell_start {
            observation(out, first)?;
        } else {
            write!(out, "null")?;
        }
        write!(out, ",\"sampled_span_ns\":")?;
        if let Some(span) = c.sampled_span_ns {
            write!(out, "{span:?}")?;
        } else {
            write!(out, "null")?;
        }
    ...
    for (i, e) in report.events().iter().enumerate() {
        ...
        write!(
            out,
            "{{\"digest\":\"{}\",\"track\":{},\"zone\":{},\"event\":\"{:?}\",\"relation\":\"{:?}\",\"from\":",
            hex(e.digest),
            e.track,
            e.zone,
            e.kind,
...
mod tests {
    ...
    use fss_twin::image_tracking::{
        ImageDetection, ImageTrackingFrame, ImageTrackingPolicy, TrackingAvailability,
    };
    ...
    fn settings() -> BTreeMap<&'static str, &'static str> {
        SETTINGS
            ...
            .zip([
                ...
                "100",
                "1 0 2 20,1 60,1 60,14 20,14;2 1 off 1,1 10,1 1,10",
            ])
            .collect()
    }
    #[test]
    fn explicit_multi_zone_configuration_preserves_geometry_and_policy() -> Result<()> {
        let t = tracker()?;
        let m = configure(&settings(), &t, basis(), &mut work())?;
        assert_eq!(m.zones().len(), 2);
        assert_eq!(m.zones()[0].dwell_ns, Some(2));
        assert_eq!(m.zones()[1].dwell_ns, None);
        assert_eq!(m.zones()[1].margin, 1);
        let mut bytes = Vec::new();
        write_configuration(&mut bytes, &m)?;
        let text = String::from_utf8(bytes)?;
        assert!(text.contains("\"dwell_ns\":null"));
        assert!(text.contains("\"maximum_sample_gap_ns\":100"));
        assert!(text.contains(&hex(m.config_digest())));
    ...
    fn incomplete_malformed_unbounded_or_nonconvex_zone_configuration_refuses() -> Result<()> {
        ...
        assert!(
            values
                .insert("zones", "1 0 off 1,1 20,1 10,5 20,14 1,14")
                .is_some()
        );
    ...
    {
        let mut t = tracker()?;
        let mut m = configure(&settings(), &t, basis(), &mut work())?;
        let frame = |n: u8| ImageTrackingFrame {
        ...
        assert_eq!(
            text.matches("\"event\":\"ObservationInterrupted\"").count(),
            2
        );
        assert_eq!(text.matches("\"sampled_span_ns\":null").count(), 2);
        assert!(text.contains(&format!(
            "\"exposure\":\"{}\",\"capture\":[2, 2]",
            hex([2; 32])
        )));
        assert!(text.contains(&format!(
            "\"last_observation\":{{\"exposure\":\"{}\"",
            hex([1; 32])
        )));
        assert!(!text.contains("LeftBetweenObservations"));
        assert!(text.ends_with("]}\n"));
        Ok(())
    }
}
