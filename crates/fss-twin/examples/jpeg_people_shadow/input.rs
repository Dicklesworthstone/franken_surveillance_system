...
use fss_twin::hog_scan::{
    MAX_SCAN_CANDIDATES, MAX_SCAN_LEVELS, MAX_SCAN_WINDOWS, ScanLevel, ScanPolicy,
};
...
impl Config {
    ...
    pub fn binding(&self, row: &Row) -> JpegFrameBinding {
        JpegFrameBinding {
            ...
            camera_image_domain: self.domain,
            calibration: self.calibration,
            interpretation: self.color,
        }
    }
...
pub fn read(path: &Path, limit: usize) -> Result<Vec<u8>> {
    ...
    bytes.try_reserve_exact(metadata.len() as usize + 1)?;
...
pub fn hash(value: &str) -> Result<[u8; 32]> {
    ...
    if digest == [0; 32] {
        return Err("zero source identity".into());
    }
    Ok(digest)
}
...
fn confined(root: &Path, name: &str) -> Result<PathBuf> {
    ...
    if !path.starts_with(root) || !path.is_file() {
        return Err("input escapes manifest directory".into());
    }
    Ok(path)
}
...
pub fn parse(text: &str, root: &Path) -> Result<Config> {
    ...
    if lines.next() != Some("FSS_JPEG_PEOPLE_SHADOW_1") {
    ...
    for line in lines {
        ...
        if let Some((_, count)) = GROUPS.iter().find(|(name, _)| *name == p[0]) {
            ...
            if !rows.is_empty()
                || zones.len() == MAX_IMAGE_ZONES
                || !(7..=4 + MAX_ZONE_VERTICES).contains(&p.len())
            {
            ...
            if !ids.insert(exposure) {
                return Err("exposure identity reused".into());
            }
            ...
            let (sequence, received) = if query {
                (number::<u64>(p[4])?, number::<u64>(p[5])?)
            } else if p[4] == "-" && p[5] == "-" {
                (0, 0)
            } else {
                return Err("references require unassigned sequence and receive time".into());
            };
            if query
                && (sequence == 0
    ...
    if settings.len() != GROUPS.len()
        || !saw_query
        || !(3..=31).contains(&references)
        || zones.is_empty()
    {
        return Err("missing settings, zones, references or queries".into());
    }
    if settings["model"][1] != "opencv-people-shadow-1" {
        return Err("explicit pinned shadow model required".into());
    }
    let s = &settings["sensor"];
    let im = &settings["image"];
    let bp = &settings["background"];
    let fp = &settings["foreground"];
    let hp = &settings["health"];
    let tp = &settings["tracking"];
    let b = &settings["budgets"];
    let sp = &settings["scan"];
    let zp = &settings["zone_policy"];
    let size = [number(im[1])?, number(im[2])?];
    let count = pixels(size)?;
    if references * count > 4 * MAX_FOREGROUND_PIXELS {
        return Err("reference-set pixel limit".into());
    }
    let color = match im[8] {
        "grayscale" => ComponentInterpretation::Grayscale,
        "ycbcr" => ComponentInterpretation::YCbCr,
        _ => return Err("unknown JPEG interpretation".into()),
    };
    ...
    let scan = ScanPolicy {
        stride: [number(sp[1])?, number(sp[2])?],
        minimum_margin: number(sp[3])?,
        suppression_iou_ppm: number(sp[4])?,
        maximum_windows: number(sp[5])?,
        maximum_candidates: number(sp[6])?,
    };
    // Preflight the current fixed scanner contract so a bad frozen profile cannot consume source.
    if scan
        .stride
        .iter()
        .any(|n| *n == 0 || *n > 4096 || *n % 8 != 0)
        || !scan.minimum_margin.is_finite()
        || scan.minimum_margin.abs() > 1e10
    ...
    for grid in sp[7].split(',') {
        if levels.len() == MAX_SCAN_LEVELS {
            return Err("scale-set limit".into());
        }
        let d = dimensions(grid, 'x')?;
        pixels(d)?;
        if !seen.insert(d) {
            return Err("duplicate scale".into());
        }
        if d[0] >= 64 && d[1] >= 128 {
            windows += ((d[0] - 64) / scan.stride[0] + 1) as usize
                * ((d[1] - 128) / scan.stride[1] + 1) as usize;
        }
        levels.push(ScanLevel { dimensions: d });
    }
    if windows == 0 || windows > scan.maximum_windows {
        return Err("no supported windows or complete-window limit".into());
    }
    let maximum_bytes: usize = number(b[7])?;
    let output_bytes: usize = number(b[8])?;
    if maximum_bytes == 0
    ...
    Ok(Config {
        camera: number(s[1])?,
        clock: number(s[2])?,
        generation: number(s[3])?,
        started: number(s[4])?,
        domain: hash(s[5])?,
        calibration: hash(s[6])?,
        episode: hash(s[7])?,
        dimensions: size,
        intrinsics,
        radius: number(im[7])?,
        ...
        background: BackgroundPolicy {
            ...
            validity: [number(bp[2])?, number(bp[3])?],
            maximum_spread: number(bp[4])?,
        },
        foreground: ForegroundPolicy {
            minimum_change: number(fp[1])?,
            minimum_area: number(fp[2])?,
            maximum_regions: number(fp[3])?,
            widespread_per_mille: number(fp[4])?,
        },
        health: ScreeningPolicy {
            minimum_visible_pixels: number(hp[1])?,
            dark_luma: number(hp[2])?,
            bright_luma: number(hp[3])?,
            extreme_per_mille: number(hp[4])?,
            flat_range: number(hp[5])?,
            repeat_frames: number(hp[6])?,
            repeat_duration_ns: number(hp[7])?,
            stall_after_ns: number(hp[8])?,
            maximum_capture_uncertainty_ns: number(hp[9])?,
            recovery_frames: number(hp[10])?,
            minimum_analysis_interval_ns: number(hp[11])?,
            sentinel_interval_ns: number(hp[12])?,
            activity_hold_ns: number(hp[13])?,
        },
        tracking: ImageTrackingPolicy {
            maximum_tracks: number(tp[1])?,
            maximum_detections: number(tp[2])?,
            maximum_exposures: number(tp[3])?,
            minimum_observations: number(tp[4])?,
            maximum_misses: number(tp[5])?,
            maximum_gap_ns: number(tp[6])?,
            maximum_speed: number(tp[7])?,
            gate_padding: number(tp[8])?,
            miss_cost: number(tp[9])?,
            ambiguity_margin: number(tp[10])?,
        },
        scan,
        levels,
        zone_policy: ImageZonePolicy {
            selection_evidence: hash(zp[1])?,
            maximum_sample_gap_ns: number(zp[2])?,
        },
        zones,
        units: [
            number(b[1])?,
