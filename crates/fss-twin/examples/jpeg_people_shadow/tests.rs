...
impl Fixture {
    fn new() -> Result<Self> {
        let root = std::env::temp_dir().join(format!("fss-jpeg-people-shadow-{}-{}",
            std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
        fs::create_dir(&root)?;
        let fixture = Self { root: root.canonicalize()? };
...
fn native_jpeg_real_model_replay_emits_every_window_and_current_tracking_zones() -> Result<()> {
    let fixture = Fixture::new()?; let text = fixture.manifest(WORK, 2, false, 1_000_000);
    let mut bytes = Vec::new(); execute(&text, &fixture.root, &mut bytes)?;
    ...
    assert!(json.lines().last().ok_or("missing completion")?.contains("\"kind\":\"complete\",\"frames\":2"));
...
fn inference_refusal_retains_source_output_without_false_completion() -> Result<()> {
    let fixture = Fixture::new()?; let mut work = WorkBudget::new(WORK);
    load_opencv_people_candidate(&mut work)?;
    let text = fixture.manifest(work.used(), 1, false, 1_000_000);
    let mut bytes = Vec::new(); assert!(execute(&text, &fixture.root, &mut bytes).is_err());
    let json = String::from_utf8(bytes)?;
...
fn private_jpeg_windows_are_unknown_not_negative_detections() -> Result<()> {
    let fixture = Fixture::new()?; let mut bytes = Vec::new();
    execute(&fixture.manifest(WORK, 1, true, 1_000_000), &fixture.root, &mut bytes)?;
    let json = String::from_utf8(bytes)?;
    let windows: Vec<_> = json.lines().filter(|l| l.contains("\"kind\":\"window\"")).collect();
    assert_eq!(windows.len(), 9);
    assert!(windows.iter().all(|l| l.contains("\"state\":\"unobservable\",\"margin\":null")));
...
fn frozen_source_remains_degraded_despite_trained_model_scores() -> Result<()> {
    let fixture = Fixture::new()?; let mut bytes = Vec::new();
    execute(&fixture.manifest(WORK, 3, false, 1_000_000), &fixture.root, &mut bytes)?;
    let json = String::from_utf8(bytes)?;
    let screen = json.lines().filter(|l| l.contains("\"kind\":\"source_screen\"")).last().ok_or("screen missing")?;
    let tracking = json.lines().filter(|l| l.contains("\"kind\":\"tracking\"")).last().ok_or("tracking missing")?;
...
fn output_quota_failure_never_emits_a_session_completion() -> Result<()> {
    let fixture = Fixture::new()?; let mut bytes = Vec::new();
    assert!(execute(&fixture.manifest(WORK, 1, false, 1000), &fixture.root, &mut bytes).is_err());
    assert!(bytes.len() <= 1000);
    assert!(!String::from_utf8(bytes)?.contains("\"kind\":\"complete\""));
    let mut tiny = Vec::new();
    { let mut bounded = output::Limited::new(&mut tiny, 3);
      assert!(bounded.write_all(b"1234").is_err()); bounded.write_all(b"123")?;
...
fn malformed_profiles_sequences_and_paths_are_refused_before_output() -> Result<()> {
    let fixture = Fixture::new()?; let good = fixture.manifest(WORK, 2, false, 1_000_000);
    for bad in [good.replace("model opencv-people-shadow-1", "model latest"),
        good.replace("80x144\n", "80x144,80x144\n"),
        ...
        let mut bytes = Vec::new(); assert!(execute(&bad, &fixture.root, &mut bytes).is_err());
...
fn exact_source_hash_mismatch_is_not_silently_rebound() -> Result<()> {
    let fixture = Fixture::new()?; let good = fixture.manifest(WORK, 1, false, 1_000_000);
