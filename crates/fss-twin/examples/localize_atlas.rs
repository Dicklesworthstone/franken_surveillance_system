...
use std::{error::Error, fs::File, io::Read, path::{Component, Path, PathBuf}};
...
fn hex(value: [u8; 32]) -> String { value.iter().map(|x| format!("{x:02x}")).collect() }

fn run() -> Result<(), Box<dyn Error>> {
    let mut args = std::env::args_os().skip(1);
    let path = PathBuf::from(args.next().ok_or("expected one config file argument")?).canonicalize()?;
    ...
    let pixels = file("query", if rectification.is_some() {64 * 1024 * 1024} else {4_194_304})?;
    ...
    let (result, rectified) = if let Some((spec, row_stride)) = rectification {
        ...
        let source = RawGrayFrame::new(RawFrameIdentity { exposure, storage: source_pixels,
            allowed_mask: source_mask, image_domain: source_domain, calibration: spec.calibration,
            dimensions: source_k.dimensions(), row_stride, range: spec.range },
            &pixels, &allowed, &mut budget)?;
        let outcome = localize_raw_frame(archived.atlas(), &twin, &plan, &source,
        ...
        let identity = ImageIdentity { exposure, pixels: source_pixels,
            image_domain: source_domain, dimensions: source_k.dimensions() };
        let image = GrayImage::new(identity, &pixels, &allowed, &mut budget)?;
        (localize_gray_frame(archived.atlas(), &twin, &image,
            LocalizationCamera { intrinsics: source_k, image_domain: identity.image_domain },
            ImageLocalizationOptions::default(), &mut budget)?, None)
    };
    // Nothing from a partially failed pipeline is emitted as a successful receipt.
    if let Some(receipt) = rectified {
        println!("{{\"schema\":\"fss.rectified-localization-rehearsal/1\",\"exposure_sha256\":\"{}\",\"source_sha256\":\"{}\",\"source_domain_sha256\":\"{}\",\"calibration_sha256\":\"{}\",\"source_mask_sha256\":\"{}\",\"output_sha256\":\"{}\",\"output_domain_sha256\":\"{}\",\"output_mask_sha256\":\"{}\",\"map_sha256\":\"{}\",\"allowed_pixels\":{},\"privacy_rejected\":{},\"outside_source\":{},\"outside_lens_domain\":{}}}",
            hex(receipt.source.exposure), hex(receipt.source.storage), hex(receipt.source.image_domain),
            hex(receipt.source.calibration), hex(receipt.source.allowed_mask), hex(receipt.output.pixels),
            hex(receipt.output.image_domain), hex(receipt.allowed_mask), hex(receipt.map_digest),
            receipt.allowed_pixels, receipt.privacy_rejected, receipt.coverage.outside_source,
