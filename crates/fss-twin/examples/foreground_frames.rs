...
fn hash(s: &str) -> Result<[u8; 32]> {
    ...
    for (i, v) in h.iter_mut().enumerate() {
        *v = u8::from_str_radix(&s[2 * i..2 * i + 2], 16)?;
    }
    if h == [0; 32] {
...
fn confined(root: &Path, name: &str) -> Result<PathBuf> {
    ...
    if !actual.starts_with(root) || !actual.is_file() {
        return Err("file outside manifest directory".into());
    }
    Ok(actual)
}
...
fn run() -> Result<()> {
    ...
    let file =
        PathBuf::from(args.next().ok_or("usage: foreground_frames MANIFEST")?).canonicalize()?;
    ...
    for line in lines {
        if let Some((key, value)) = line.split_once('=') {
            if !(names.contains(&key)
                || tracking_enabled && tracking::SETTINGS.contains(&key)
                || zones_enabled && zones::SETTINGS.contains(&key))
                || settings.insert(key, value).is_some()
            {
                return Err("unknown/duplicate setting".into());
            }
        } else {
            let p: Vec<_> = line.split_whitespace().collect();
            if p.len() != 8 || !["reference", "query"].contains(&p[0]) || rows.len() >= 128 {
                return Err("invalid/too many frame rows".into());
            }
            let query = p[0] == "query";
            if !query && saw_query {
            ...
            rows.push(Row {
                ...
                exposure: hash(p[1])?,
                capture,
                pixels: confined(root, p[4])?,
                pixels_hash: hash(p[5])?,
                mask: confined(root, p[6])?,
                mask_hash: hash(p[7])?,
            });
        }
    }
    if settings.len()
        ...
        || !saw_query
    {
        return Err("missing settings or query frames".into());
    }
    let get = |k| settings.get(k).copied().ok_or("missing setting");
    let width: u32 = get("width")?.parse()?;
    let height: u32 = get("height")?.parse()?;
    if width == 0 || height == 0 || width > 4096 || height > 4096 {
        return Err("invalid dimensions".into());
    }
    let count = width as usize * height as usize;
    if count > MAX_FOREGROUND_PIXELS {
        return Err("image exceeds limit".into());
    }
    let camera = get("camera")?.parse()?;
    let clock = get("clock")?.parse()?;
    let calibration = hash(get("calibration")?)?;
    let domain = hash(get("image_domain")?)?;
    let bp = BackgroundPolicy {
        selection_evidence: hash(get("selection_evidence")?)?,
        validity: [get("valid_from")?.parse()?, get("valid_until")?.parse()?],
        maximum_spread: get("maximum_spread")?.parse()?,
    };
    let fp = ForegroundPolicy {
        minimum_change: get("minimum_change")?.parse()?,
        minimum_area: get("minimum_area")?.parse()?,
        maximum_regions: get("maximum_regions")?.parse()?,
        widespread_per_mille: get("widespread_per_mille")?.parse()?,
    };
    let mut budget = WorkBudget::new(get("work_units")?.parse()?);
    let mut tracker = if tracking_enabled {
        Some(tracking::configure(&settings, &mut budget)?)
    } else {
        None
    };
    let mut zone_monitor = if zones_enabled {
        let basis = fss_twin::image_zones::ImageZoneBasis {
            camera,
            clock,
            calibration,
            image_domain: domain,
        ...
        Some(zones::configure(
            &settings,
            tracker.as_ref().ok_or("zone mode requires tracking")?,
            basis,
            &mut budget,
        )?)
    } else {
    ...
    if !(3..=MAX_BACKGROUND_FRAMES).contains(&refs) {
        return Err("reference count outside 3..31".into());
    }
    let load = |r: &Row| -> Result<Loaded> {
        let pixels = bytes(&r.pixels, count)?;
        let allowed = bytes(&r.mask, count)?;
        if pixels.len() != count
            || allowed.len() != count
            || ContentDigest::sha256(&pixels).bytes() != r.pixels_hash
            || ContentDigest::sha256(&allowed).bytes() != r.mask_hash
        {
            return Err("frame or mask binding mismatch".into());
        }
        Ok(Loaded {
            source: ForegroundSource {
                image: ImageIdentity {
                    exposure: r.exposure,
                    pixels: r.pixels_hash,
                    image_domain: domain,
                    dimensions: [width, height],
                },
                camera,
    ...
    if let Some(monitor) = &zone_monitor {
        zones::write_configuration(&mut out, monitor)?;
    }
    for row in &rows[refs..] {
        let f = load(row)?;
        let query = ForegroundFrame::new(f.source, &f.pixels, &f.allowed, &mut budget)?;
        let r = model.detect(&query, fp, &mut budget)?;
        let tracked = tracker
            .as_mut()
            .map(|t| t.update_foreground(&r, &mut budget))
            .transpose()?;
        write!(
            out,
            "{{\"kind\":\"frame\",\"exposure\":\"{}\",\"report\":\"{}\",\"assessment\":\"{:?}\",\"comparable\":{},\"changed\":{},\"small_components\":{},\"small_pixels\":{},\"regions\":[",
            hex(row.exposure),
            hex(r.digest()),
            r.assessment(),
            r.comparable_pixels(),
            r.changed_pixels(),
            r.small_component_count(),
            r.small_component_pixels()
        )?;
        for (i, region) in r.regions().iter().enumerate() {
            ...
            write!(
                out,
                "{{\"id\":{},\"area\":{},\"min\":{:?},\"max\":{:?},\"unknown_boundary\":{},\"image_edge\":{}}}",
                region.id,
                region.area,
                region.min,
                region.max,
                region.touches_unknown,
                region.touches_edge
            )?;
        }
        writeln!(out, "]}}")?;
            ...
            if let Some(monitor) = &mut zone_monitor {
                match monitor.observe(tracker, report, &mut budget) {
                    ...
                    Err(error) => {
                        writeln!(
                            out,
                            "{{\"kind\":\"zone_incomplete\",\"tracking\":\"{}\",\"error\":\"{:?}\"}}",
                            hex(report.digest()),
                            error
                        )?;
                        out.flush()?;
    ...
    writeln!(
        out,
        "{{\"kind\":\"complete\",\"frames\":{},\"work_units\":{}}}",
        completed,
        budget.used()
    )?;
    out.flush()?;
...
fn main() {
    if let Err(e) = run() {
        eprintln!("foreground replay failed: {e}");
        std::process::exit(1);
    }
}
