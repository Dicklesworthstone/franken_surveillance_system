...
use fss_twin::hog::{HOG_PARAMETERS, HogModel, MAX_HOG_PIXELS};
...
fn read(path: &Path, maximum: usize) -> Result<Vec<u8>> {
    ...
    if !f.metadata()?.is_file() || f.metadata()?.len() > maximum as u64 {
        return Err("input size/type refused".into());
    }
    let mut bytes = Vec::new();
    bytes.try_reserve_exact(maximum + 1)?;
...
fn confined(root: &Path, name: &str) -> Result<PathBuf> {
    ...
    if !path.starts_with(root) {
        return Err("input escapes the manifest directory".into());
    }
    Ok(path)
}
...
fn settings(text: &str) -> Result<BTreeMap<&str, &str>> {
    ...
    if lines.next() != Some("FSS_HOG_SCAN_1") {
        return Err("unsupported scan manifest".into());
    }
    ...
    if settings.len() != KEYS.len() {
        return Err("incomplete scan manifest".into());
    }
    Ok(settings)
}
fn output(out: &mut impl Write, scan: &HogScan, used: u64) -> Result<()> {
    writeln!(
        out,
        "{{\"kind\":\"hog_scan\",\"report\":\"{}\",\"generation\":\"{}\",\"model\":\"{}\",\"exposure\":\"{}\",\"pixels\":\"{}\",\"mask\":\"{}\",\"capture\":{:?},\"dimensions\":{:?},\"meaning\":\"uncalibrated_model_windows_not_person_identity\"}}",
        hex(scan.digest()),
        hex(scan.generation()),
        hex(scan.model_digest()),
        hex(scan.source().image.exposure),
        hex(scan.source().image.pixels),
        hex(scan.mask_digest()),
        scan.source().capture,
        scan.source().image.dimensions
    )?;
    for (index, level) in scan.levels().iter().enumerate() {
        writeln!(
            out,
            "{{\"kind\":\"level\",\"index\":{},\"dimensions\":{:?},\"pixels\":\"{}\",\"image_domain\":\"{}\",\"mask\":\"{}\",\"windows\":{}}}",
            index,
            level.dimensions,
    ...
    for w in scan.windows() {
        write!(
            out,
            "{{\"kind\":\"window\",\"id\":{},\"level\":{},\"origin\":{:?},\"min\":{:?},\"max\":{:?},\"margin\":",
        ...
        match w.disposition {
            ...
            WindowDisposition::Suppressed { by } => {
                writeln!(out, ",\"state\":\"suppressed\",\"by\":{by}}}")?
            }
        }
    }
    writeln!(
        out,
        "{{\"kind\":\"complete\",\"windows\":{},\"selected\":{},\"work_units\":{}}}",
        scan.windows().len(),
        scan.selected().count(),
        used
    )?;
...
fn run() -> Result<()> {
    ...
    let model = HogModel::from_f32_le(
        &bytes,
        digest(get("weights_sha256")?)?,
        digest(get("provenance")?)?,
        &mut budget,
    )?;
    ...
    for spec in get("levels")?.split(',') {
        ...
        levels.push(ScanLevel {
            dimensions: pair(spec)?,
        });
    }
    let source = ForegroundSource {
        image: ImageIdentity {
            exposure: digest(get("exposure")?)?,
            pixels: digest(get("pixels_sha256")?)?,
            image_domain: digest(get("image_domain")?)?,
            dimensions,
        },
        camera: get("camera")?.parse()?,
        clock: get("clock")?.parse()?,
        calibration: digest(get("calibration")?)?,
        capture: [
            get("capture_earliest_ns")?.parse()?,
            get("capture_latest_ns")?.parse()?,
        ],
    };
    let scan = scan_hog(
        source,
        ...
        ScanPolicy {
            ...
            minimum_margin: get("minimum_margin")?.parse()?,
            suppression_iou_ppm: get("suppression_iou_ppm")?.parse()?,
            maximum_windows: get("maximum_windows")?.parse()?,
            maximum_candidates: get("maximum_candidates")?.parse()?,
        },
        &mut budget,
    )?;
...
fn main() {
    if let Err(error) = run() {
        eprintln!("HOG scan refused: {error}");
        std::process::exit(1);
    }
}
...
mod tests {
    ...
    fn numeric_pairs_and_identities_are_not_silently_coerced() -> Result<()> {
        ...
        assert!(pair("64x128x256").is_err());
        assert!(pair("-1x128").is_err());
        assert!(digest(&"0".repeat(64)).is_err());
        assert!(digest(&"A".repeat(64)).is_err());
        assert_eq!(digest(&"1".repeat(64))?, [17; 32]);
        Ok(())
    }
}
