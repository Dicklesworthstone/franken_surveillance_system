...
use super::{ContentDigest, ForegroundSource, HogError, WorkBudget, count, put, reserve};
...
pub(super) fn resample(
    ...
    for y in 0..dimensions[1] {
        for x in 0..dimensions[0] {
            ...
            for (iy, wy) in ys {
                for (ix, wx) in xs {
                    if wx != 0 && wy != 0 {
                        total += u64::from(original[iy * width as usize + ix]) * wx * wy;
                    }
                }
            }
    ...
    for value in source.image.dimensions.into_iter().chain(dimensions) {
        put(&mut identity, u64::from(value));
    }
