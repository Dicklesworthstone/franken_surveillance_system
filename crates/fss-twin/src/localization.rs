...
use fss_geometry::{Correspondence, GeometryBasis, GeometryError, PinholeIntrinsics,
    PoseSearch, PoseSolverOptions, WorkBudget, estimate_camera_pose};
...
impl BinaryDescriptor {
    ...
    pub fn distance(self, other: Self) -> u16 {
        self.0.iter().zip(other.0).map(|(a, b)| (a ^ b).count_ones() as u16).sum()
    }
}
...
impl std::fmt::Debug for FeatureFrame {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FeatureFrame").field("feature_count", &self.features.len()).finish_non_exhaustive()
    }
}
impl FeatureFrame {
    /// Validate the complete input before exposing it, then order by image-local ID.
    pub fn new(identity: ImageIdentity, descriptor_domain: [u8; 32],
        mut features: Vec<ImageFeature>, budget: &mut WorkBudget<'_>) -> Result<Self, LocalizationError> {
        budget.charge(0)?;
        identity.validate()?;
        if descriptor_domain == [0; 32] { return Err(LocalizationError::InvalidInput); }
        if features.len() > MAX_IMAGE_FEATURES { return Err(LocalizationError::Limit); }
        for (i, point) in features.iter().enumerate() {
            budget.charge(1)?;
            if point.id == 0 || (0..2).any(|a| !point.pixel[a].is_finite()
                || point.pixel[a] < 0.0 || point.pixel[a] >= f64::from(identity.dimensions[a])) {
                return Err(LocalizationError::InvalidInput);
            }
            for other in &features[..i] {
                budget.charge(1)?;
                if point.id == other.id || point.pixel == other.pixel { return Err(LocalizationError::InvalidInput); }
            }
        }
        budget.charge(features.len() as u64 * 10)?;
        features.sort_by_key(|point| point.id);
        Ok(Self { identity, descriptor_domain, features })
    }
    ...
    pub fn descriptor_domain(&self) -> [u8; 32] { self.descriptor_domain }
    /// Complete selected feature set; detection completeness is not claimed.
    pub fn features(&self) -> &[ImageFeature] { &self.features }
}
...
impl std::fmt::Debug for LocalizationAtlas {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LocalizationAtlas").field("landmarks", &self.landmarks.len())
            .field("references", &self.references.len()).finish_non_exhaustive()
    }
}
impl LocalizationAtlas {
    ...
    pub fn new(twin: &PropertyTwin, mut landmarks: Vec<AtlasLandmark>,
        mut references: Vec<AtlasReference>, mut bindings: Vec<AtlasBinding>,
        budget: &mut WorkBudget<'_>) -> Result<Self, LocalizationError> {
        budget.charge(0)?;
        if landmarks.is_empty() || references.is_empty() || bindings.is_empty() {
            return Err(LocalizationError::InvalidInput);
        }
        if landmarks.len() > MAX_ATLAS_LANDMARKS || references.len() > MAX_ATLAS_REFERENCES
        ...
        for (i, point) in landmarks.iter().enumerate() {
            if point.id == 0 || point.physical_group == 0 || point.evidence == [0; 32]
                || twin.features().get(point.feature as usize).is_none()
                || point.world.iter().any(|x| !x.is_finite() || x.abs() > 1e12)
                || point.error.is_some_and(|e| e.iter().any(|x| !x.is_finite() || *x < 0.0 || *x > 1e12)) {
                return Err(LocalizationError::InvalidInput);
            }
            for other in &landmarks[..i] {
                budget.charge(1)?;
                if point.id == other.id || point.physical_group == other.physical_group || point.world == other.world {
                    return Err(LocalizationError::InvalidInput);
                }
            }
        }
        ...
        for (i, view) in references.iter().enumerate() {
            budget.charge(1)?;
            if view.id == 0 || view.frame.descriptor_domain != descriptor_domain { return Err(LocalizationError::BasisMismatch); }
            for other in &references[..i] {
                budget.charge(1)?;
                if view.id == other.id || view.frame.identity.exposure == other.frame.identity.exposure {
                    return Err(LocalizationError::InvalidInput);
                }
            }
        }
        ...
        samples.try_reserve_exact(bindings.len()).map_err(|_| LocalizationError::Limit)?;
        let mut previous = None;
        for binding in &bindings {
            budget.charge(32)?;
            let p = landmarks.binary_search_by_key(&binding.landmark, |p| p.id).map_err(|_| LocalizationError::InvalidInput)?;
            let r = references.binary_search_by_key(&binding.reference, |r| r.id).map_err(|_| LocalizationError::InvalidInput)?;
            let f = references[r].frame.features.binary_search_by_key(&binding.image_feature, |f| f.id)
                .map_err(|_| LocalizationError::InvalidInput)?;
            let key = (binding.landmark, binding.reference);
            let slot = r * MAX_IMAGE_FEATURES + f;
            if previous == Some(key) || used_features[slot] { return Err(LocalizationError::InvalidInput); }
            previous = Some(key);
            used_points[p] = true;
            used_views[r] = true;
        ...
        if used_points.contains(&false) || used_views.contains(&false) { return Err(LocalizationError::InvalidInput); }
        let mut atlas = Self { basis: twin.basis(), twin_digest: twin.digest(), digest: [0; 32],
            descriptor_domain, landmarks, references, bindings, samples };
    ...
    pub fn landmarks(&self) -> &[AtlasLandmark] { &self.landmarks }
    /// Original explicit map associations in canonical order.
    pub fn bindings(&self) -> &[AtlasBinding] { &self.bindings }
    /// Original reference exposure records, not synthetic independent evidence.
    pub fn references(&self) -> &[AtlasReference] { &self.references }
    ...
    pub fn match_frame(&self, twin: &PropertyTwin, query: &FeatureFrame, options: MatchOptions,
        budget: &mut WorkBudget<'_>) -> Result<MatchReport, LocalizationError> {
        budget.charge(0)?;
        if twin.basis() != self.basis || twin.digest() != self.twin_digest
            || query.descriptor_domain != self.descriptor_domain { return Err(LocalizationError::BasisMismatch); }
        ...
        for p in 0..n {
            ...
            for q in 0..qn {
                ...
                if d < best { best = d; reverse[p] = Some(q); }
                else if d == best { reverse[p] = None; }
            }
        }
        let mut decisions = Vec::new();
        let mut correspondences = Vec::new();
        decisions.try_reserve_exact(qn).map_err(|_| LocalizationError::Limit)?;
        correspondences.try_reserve_exact(qn).map_err(|_| LocalizationError::Limit)?;
        for (q, feature) in query.features.iter().enumerate() {
            ...
            for p in 0..n {
                budget.charge(1)?;
                let d = distances[q * n + p];
                if d < first.0 { second = first.0; first = (d, p); }
                else { second = second.min(d); }
            }
            let rejection = if first.0 > options.maximum_distance { Some(MatchRejection::TooDistant) }
                else if second == 257 || first.0 == second
                    || u32::from(first.0) * 100 >= u32::from(second) * u32::from(options.ratio_percent) {
                    Some(MatchRejection::Ambiguous)
                } else if reverse[first.1] != Some(q) { Some(MatchRejection::NonMutual) }
                else { None };
            let landmark = if rejection.is_none() {
                let point = self.landmarks[first.1];
                correspondences.push(Correspondence { landmark: point.id, physical_group: point.physical_group,
                    world: point.world, pixel: feature.pixel });
                Some(point.id)
            } else { None };
            decisions.push(MatchDecision { image_feature: feature.id, landmark, rejection,
                distance: first.0, runner_up_distance: (second != 257).then_some(second) });
        }
        budget.charge(qn as u64 * 10)?;
        correspondences.sort_by_key(|point| point.landmark);
        Ok(MatchReport { atlas: self.digest, query: query.identity, decisions, correspondences })
    }
    ...
    pub fn localize(&self, twin: &PropertyTwin, query: &FeatureFrame,
        camera: LocalizationCamera, matching: MatchOptions, solving: PoseSolverOptions,
        budget: &mut WorkBudget<'_>) -> Result<CameraLocalization, LocalizationError> {
        budget.charge(0)?;
        if query.identity.dimensions != camera.intrinsics.dimensions()
            || query.identity.image_domain != camera.image_domain { return Err(LocalizationError::BasisMismatch); }
        if self.references.iter().any(|r| r.frame.identity.exposure == query.identity.exposure) {
    ...
    fn fingerprint(&self, budget: &mut WorkBudget<'_>) -> Result<[u8; 32], LocalizationError> {
        ...
        let capacity = 128 + self.landmarks.len() * 128 + self.references.len() * 160
            + count * 64 + self.bindings.len() * 24;
        budget.charge(capacity as u64)?;
        let mut b = Vec::new();
        b.try_reserve_exact(capacity).map_err(|_| LocalizationError::Limit)?;
        ...
        for p in &self.landmarks {
            budget.charge(0)?;
            integer(&mut b, p.id); integer(&mut b, p.physical_group); integer(&mut b, u64::from(p.feature));
            for x in p.world { float(&mut b, x); }
            b.extend_from_slice(&p.evidence);
            b.push(u8::from(p.error.is_some()));
            if let Some(error) = p.error { for x in error { float(&mut b, x); } }
        }
        integer(&mut b, self.references.len() as u64);
        for r in &self.references {
            budget.charge(0)?;
            integer(&mut b, r.id);
            for hash in [r.frame.identity.exposure, r.frame.identity.pixels, r.frame.identity.image_domain] { b.extend_from_slice(&hash); }
            for n in r.frame.identity.dimensions { integer(&mut b, u64::from(n)); }
            integer(&mut b, r.frame.features.len() as u64);
            for f in &r.frame.features {
                integer(&mut b, f.id); for x in f.pixel { float(&mut b, x); }
                for word in f.descriptor.0 { integer(&mut b, word); }
            }
        }
        integer(&mut b, self.bindings.len() as u64);
        for v in &self.bindings { integer(&mut b, v.landmark); integer(&mut b, v.reference); integer(&mut b, v.image_feature); }
        Ok(ContentDigest::sha256(&b).bytes())
    }
}
fn allocated<T: Clone>(count: usize, value: T) -> Result<Vec<T>, LocalizationError> {
    let mut result = Vec::new();
    result.try_reserve_exact(count).map_err(|_| LocalizationError::Limit)?;
    result.resize(count, value);
    Ok(result)
}
fn integer(bytes: &mut Vec<u8>, value: u64) { bytes.extend_from_slice(&value.to_le_bytes()); }
fn float(bytes: &mut Vec<u8>, value: f64) { integer(bytes, if value == 0.0 { 0 } else { value.to_bits() }); }
...
impl Default for MatchOptions {
    fn default() -> Self { Self { maximum_distance: 64, ratio_percent: 80 } }
}
