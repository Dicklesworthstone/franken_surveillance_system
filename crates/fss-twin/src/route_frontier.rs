...
use crate::{
    MovementClass, NavigationError, NavigationProfile, PropertyTwin, RouteOutcome, RouteQuery,
    RouteSearch, SupportLocation, SupportNetwork, TwinError,
};
...
impl From<NavigationError> for RouteFrontierError {
    fn from(value: NavigationError) -> Self {
        Self::Navigation(value)
    }
}
impl From<TwinError> for RouteFrontierError {
    fn from(value: TwinError) -> Self {
        Self::Twin(value)
    }
}
impl From<fss_geometry::GeometryError> for RouteFrontierError {
    fn from(value: fss_geometry::GeometryError) -> Self {
        Self::Twin(value.into())
    }
}
...
pub fn build_route_frontier(
    twin: &PropertyTwin,
    network: &SupportNetwork,
    snapshot: TrackSnapshot<'_>,
    options: RouteFrontierOptions,
    budget: &mut WorkBudget<'_>,
) -> Result<RouteFrontier, RouteFrontierError> {
    budget.charge(0)?;
    if network.basis() != twin.basis()
        || network.twin_digest() != twin.digest()
        || snapshot.projection().twin_digest() != twin.digest()
    {
        return Err(RouteFrontierError::BasisMismatch);
    }
    if options.maximum_features == 0
        || options.maximum_features > 4096
        || options.maximum_routes == 0
        || options.maximum_routes > 16384
        || options.maximum_points == 0
        || options.maximum_points > 256
        || twin.features().len() > options.maximum_features
    {
        return Err(RouteFrontierError::InvalidOptions);
    }
    let neutral = options.retain_without_preference
        && options.profile.class() == MovementClass::Person
        && options.profile.person_path_multiplier() != 1;
    let usable = snapshot
        .projection()
        .hypotheses()
        .iter()
        .filter(|h| h.nominal.is_some())
        .count();
    let branches = if neutral { 2 } else { 1 };
    let route_count = usable
        .checked_mul(twin.features().len())
        .and_then(|n| n.checked_mul(branches))
        .ok_or(RouteFrontierError::Limit)?;
    if route_count > options.maximum_routes {
        return Err(RouteFrontierError::Limit);
    }
    let mut routes = Vec::new();
    routes
        .try_reserve_exact(route_count)
        .map_err(|_| RouteFrontierError::Limit)?;
    let mut unresolved = Vec::new();
    unresolved
        .try_reserve_exact(snapshot.projection().hypotheses().len())
        .map_err(|_| RouteFrontierError::Limit)?;
    ...
    for (support_index, hypothesis) in snapshot.projection().hypotheses().iter().enumerate() {
        ...
        let Some(hit) = hypothesis.nominal else {
            unresolved.push(FrontierUnresolvedSource {
                support: support_index,
                triangle: hypothesis.triangle,
                reason: FrontierSourceLimitation::NoNominalSupport,
            });
            continue;
        };
        if hypothesis.nominal_occluded == Some(true) {
            unresolved.push(FrontierUnresolvedSource {
                support: support_index,
                triangle: hypothesis.triangle,
                reason: FrontierSourceLimitation::NominalOcclusionConflict,
            });
        }
        let start = SupportLocation::new(hit.triangle, hit.barycentric)
            .map_err(RouteFrontierError::Navigation)?;
        for feature in 0..twin.features().len() {
            for kind in [
                FrontierRouteKind::Preferred,
                FrontierRouteKind::WithoutPathPreference,
            ] {
                if kind == FrontierRouteKind::WithoutPathPreference && !neutral {
                    continue;
                }
                let profile = if kind == FrontierRouteKind::WithoutPathPreference {
                    options.profile.without_preference()
                } else {
                    options.profile
                };
                let search = network.route_to_feature(
                    RouteQuery {
                        basis: twin.basis(),
                        twin_digest: twin.digest(),
                        start,
                        destination_feature: feature as u32,
                        profile,
                        max_points: options.maximum_points,
                    },
                    budget,
                )?;
                let mut compatibility = Vec::new();
                if let Some(world_motion) = motion {
                    let matching = world_motion
                        .modes()
                        .iter()
                        .enumerate()
                        .filter(|(_, mode)| mode.current_triangle == hypothesis.triangle)
                        .count();
                    compatibility
                        .try_reserve_exact(matching)
                        .map_err(|_| RouteFrontierError::Limit)?;
                    for (mode_index, mode) in world_motion.modes().iter().enumerate() {
                        if mode.current_triangle != hypothesis.triangle {
                            continue;
                        }
                        let speed = mode.nominal_velocity.map(|v| v[0].hypot(v[1]).hypot(v[2]));
                        let cosine = match (&search.outcome, mode.nominal_velocity) {
                            (RouteOutcome::Found(path), Some(v)) => {
                                direction_cosine(path.points(), v)
                            }
                            _ => None,
                        };
                        compatibility.push(MotionCompatibility {
                            mode: mode_index,
                            speed,
                            initial_direction_cosine: cosine,
                        });
                    }
                }
                routes.push(FrontierRoute {
                    route: routes.len() as u64 + 1,
                    support: support_index,
                    source_triangle: hypothesis.triangle,
                    destination_feature: feature as u32,
                    kind,
                    search,
    ...
    Ok(RouteFrontier {
        ...
        destination_features: twin.features().len(),
    })
}
...
fn direction_cosine(points: &[[f64; 3]], velocity: [f64; 3]) -> Option<f64> {
    ...
    for pair in points.windows(2) {
        ...
        if length > 1e-12 {
            return Some(
                (0..3)
                    .map(|i| d[i] / length * velocity[i] / speed)
                    .sum::<f64>()
                    .clamp(-1.0, 1.0),
            );
        }
