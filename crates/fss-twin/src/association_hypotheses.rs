...
impl AssociationAssignment {
    ...
    pub fn links(&self) -> &[AssignmentLink] { &self.links }
    /// Unassigned source tracks, not observations that targets disappeared.
    pub fn unmatched_tracks(&self) -> &[u64] { &self.unmatched_tracks }
    /// Unassigned proposals, not automatically allocated new identities.
    pub fn unmatched_detections(&self) -> &[u64] { &self.unmatched_detections }
}
...
impl AssociationFactor {
    ...
    pub fn tracks(&self) -> &[u64] { &self.tracks }
    /// Complete component detection scope.
    pub fn detections(&self) -> &[u64] { &self.detections }
    /// Every possible edge, including unresolved geometry; pair diagnostics remain in the graph.
    pub fn allowed_links(&self) -> &[AssignmentLink] { &self.allowed_links }
    /// Complete explicit or implicit matching family; never a ranked top-k result.
    pub fn alternatives(&self) -> &FactorAlternatives { &self.alternatives }
}
...
impl<'a> AssociationHypotheses<'a> {
    ...
    pub fn graph(&self) -> &'a AssociationGraph { self.graph }
    /// Retained one-to-one assumption and exact representation policy.
    pub fn policy(&self) -> AssignmentPolicy { self.policy }
    /// Components whose matching families combine by Cartesian product.
    pub fn factors(&self) -> &[AssociationFactor] { &self.factors }
    ...
    pub fn joint_count(&self) -> Option<u128> { self.joint_count }
    ...
    pub fn check_assignment(&self, links: &[AssignmentLink], budget: &mut WorkBudget<'_>)
        ...
        for link in links {
            budget.charge(16)?;
            let t = topology.tracks.binary_search(&link.track).map_err(|_| AssociationError::InvalidInput)?;
            let d = topology.detections.binary_search(&link.detection).map_err(|_| AssociationError::InvalidInput)?;
            let bit = 1_u32 << d;
            if choices[t].is_some() || used & bit != 0 || topology.forward[t] & bit == 0 {
                return Err(AssociationError::InvalidInput);
            }
            choices[t] = Some(d); used |= bit;
        }
        let domain = Domain { tracks: (0..topology.tracks.len()).collect(),
            detections: (0..topology.detections.len()).collect() };
        let assignment = materialize(&topology, &domain, &choices, used, budget)?;
        budget.charge(0)?;
        Ok(AssociationSelection { graph: self.graph, policy: self.policy, assignment })
    }
}
...
impl AssociationSelection<'_> {
    ...
    pub fn assignment(&self) -> &AssociationAssignment { &self.assignment }
    /// Original basis; call `check_current` before later consuming this proposal.
    pub fn graph(&self) -> &AssociationGraph { self.graph }
    ...
    pub fn observations(&self, budget: &mut WorkBudget<'_>) -> Result<Vec<ContactObservation>, AssociationError> {
        budget.charge(0)?;
        ...
        for link in &self.assignment.links {
            ...
            let d = self.graph.detections().iter().find(|d| d.detection().id == link.detection)
                .ok_or(AssociationError::InvalidInput)?.detection();
            output.push(ContactObservation { evidence: d.evidence, track: link.track,
                camera: frame.camera.camera, exposure: frame.exposure,
                image_domain: frame.camera.image_domain, clock: frame.camera.clock,
                capture: frame.capture, pixel_min: d.pixel_min, pixel_max: d.pixel_max,
...
pub fn factorize_associations<'a>(graph: &'a AssociationGraph, policy: AssignmentPolicy,
    budget: &mut WorkBudget<'_>) -> Result<AssociationHypotheses<'a>, AssociationError> {
    budget.charge(0)?;
    if policy.one_to_one_basis == [0; 32] || !(1..=1024).contains(&policy.maximum_per_factor)
        || !(1..=4096).contains(&policy.maximum_materialized) { return Err(AssociationError::InvalidInput); }
    ...
    for domain in domains {
        ...
        for &t in &domain.tracks {
            ...
            for &d in &domain.detections {
                ...
                if topology.forward[t] & (1_u32 << d) != 0 {
                    allowed_links.push(AssignmentLink { track: topology.tracks[t], detection: topology.detections[d] });
                }
            }
        }
        for &d in &domain.detections { detections.push(topology.detections[d]); }
        ...
        let alternatives = if let Some(all) = enumerate(&topology, &domain, limit, budget)? {
            ...
            } else { ImplicitReason::FactorLimit })
        };
        factors.push(AssociationFactor { tracks, detections, allowed_links, alternatives });
    }
    budget.charge(0)?;
    Ok(AssociationHypotheses { graph, policy, factors, joint_count })
}

struct Domain { tracks: Vec<usize>, detections: Vec<usize> }
struct Topology {
    tracks: Vec<u64>, detections: Vec<u64>,
    forward: [u32; MAX_ASSOCIATION_ITEMS], reverse: [u32; MAX_ASSOCIATION_ITEMS],
}
impl Topology {
    fn from_graph(graph: &AssociationGraph, budget: &mut WorkBudget<'_>) -> Result<Self, AssociationError> {
        ...
        if graph.pairs().len() != tracks.len() * n { return Err(AssociationError::InvalidInput); }
        let mut value = Self { tracks, detections, forward: [0; MAX_ASSOCIATION_ITEMS], reverse: [0; MAX_ASSOCIATION_ITEMS] };
        for (i, pair) in graph.pairs().iter().enumerate() {
            budget.charge(1)?;
            if pair.possible() { value.forward[i/n] |= 1_u32 << (i%n); value.reverse[i%n] |= 1_u32 << (i/n); }
        }
        Ok(value)
    }
    fn components(&self, budget: &mut WorkBudget<'_>) -> Result<Vec<Domain>, AssociationError> {
        let n = self.tracks.len(); let m = self.detections.len();
        let mut seen = [false; 2 * MAX_ASSOCIATION_ITEMS];
        let mut output = reserved(n+m)?;
        for seed in 0..n+m {
            if seen[seed] { continue; }
            let mut stack = reserved(n+m)?;
            let mut domain = Domain { tracks: reserved(n)?, detections: reserved(m)? };
            seen[seed] = true; stack.push(seed);
            while let Some(node) = stack.pop() {
                budget.charge((n+m+1) as u64)?;
                if node < n {
                    domain.tracks.push(node);
                    for d in 0..m { if self.forward[node] & (1_u32 << d) != 0 && !seen[n+d] {
                        seen[n+d] = true; stack.push(n+d);
                    }}
                } else {
            ...
            domain.tracks.sort_unstable(); domain.detections.sort_unstable();
            output.push(domain);
        }
        Ok(output)
    }
...
struct Enumerator<'a> {
    topology: &'a Topology, domain: &'a Domain,
    choices: [Option<usize>; MAX_ASSOCIATION_ITEMS],
    output: Vec<AssociationAssignment>, limit: usize,
}
impl Enumerator<'_> {
    fn visit(&mut self, depth: usize, used: u32, budget: &mut WorkBudget<'_>) -> Result<bool, AssociationError> {
        budget.charge(1)?;
        if depth == self.domain.tracks.len() {
            if self.output.len() == self.limit { return Ok(false); }
            self.output.push(materialize(self.topology,self.domain,&self.choices,used,budget)?);
            return Ok(true);
        }
        self.choices[depth] = None;
        if !self.visit(depth+1,used,budget)? { return Ok(false); }
        let t = self.domain.tracks[depth];
        for index in 0..self.domain.detections.len() {
            budget.charge(1)?;
            let d = self.domain.detections[index]; let bit = 1_u32 << d;
            if used & bit == 0 && self.topology.forward[t] & bit != 0 {
                self.choices[depth] = Some(d);
                if !self.visit(depth+1,used|bit,budget)? { return Ok(false); }
            }
        }
        Ok(true)
    }
}
fn enumerate(topology: &Topology, domain: &Domain, limit: usize, budget: &mut WorkBudget<'_>)
    -> Result<Option<Vec<AssociationAssignment>>, AssociationError> {
    if limit == 0 { return Ok(None); }
    let mut search = Enumerator { topology, domain, choices: [None; MAX_ASSOCIATION_ITEMS],
        output: reserved(limit)?, limit };
    if search.visit(0,0,budget)? { Ok(Some(search.output)) } else { Ok(None) }
}
fn materialize(topology: &Topology, domain: &Domain, choices: &[Option<usize>; MAX_ASSOCIATION_ITEMS],
    used: u32, budget: &mut WorkBudget<'_>) -> Result<AssociationAssignment, AssociationError> {
    ...
    for (i,&t) in domain.tracks.iter().enumerate() {
        if let Some(d) = choices[i] {
            links.push(AssignmentLink { track: topology.tracks[t], detection: topology.detections[d] });
        } else { unmatched_tracks.push(topology.tracks[t]); }
    }
    for &d in &domain.detections { if used & (1_u32 << d) == 0 { unmatched_detections.push(topology.detections[d]); } }
    Ok(AssociationAssignment { links, unmatched_tracks, unmatched_detections })
}
fn reserved<T>(count: usize) -> Result<Vec<T>, AssociationError> {
    let mut output = Vec::new();
    output.try_reserve_exact(count).map_err(|_| AssociationError::Limit)?;
    Ok(output)
}
...
mod tests {
    ...
    fn topology(n: usize, m: usize, edges: &[(usize,usize)]) -> Topology {
        let mut value = Topology { tracks: (1..=n as u64).collect(), detections: (101..101+m as u64).collect(),
            forward: [0; MAX_ASSOCIATION_ITEMS], reverse: [0; MAX_ASSOCIATION_ITEMS] };
        for &(t,d) in edges { value.forward[t] |= 1_u32 << d; value.reverse[d] |= 1_u32 << t; }
        value
    }
    fn full(n: usize,m: usize) -> Domain { Domain {tracks:(0..n).collect(),detections:(0..m).collect()} }

    #[test]
    fn every_three_by_three_graph_matches_an_independent_edge_subset_oracle() -> Result<(),AssociationError> {
        let mut budget = WorkBudget::new(100_000_000);
        for mask in 0_u16..512 {
            let edges: Vec<_> = (0..9).filter(|i| mask & (1 << i) != 0).map(|i|(i/3,i%3)).collect();
            let topo = topology(3,3,&edges);
            let got = enumerate(&topo,&full(3,3),34,&mut budget)?.ok_or(AssociationError::Limit)?;
            let actual: BTreeSet<_> = got.iter().map(|a|a.links.clone()).collect();
            assert_eq!(actual.len(),got.len());
            let mut expected = BTreeSet::new();
            for chosen in 0_u16..512 {
                if chosen & !mask != 0 { continue; }
                let selected: Vec<_> = (0..9).filter(|i| chosen & (1 << i) != 0).map(|i|(i/3,i%3)).collect();
                ...
                if rows.len() == selected.len() && columns.len() == selected.len() {
                    expected.insert(selected.iter().map(|&(t,d)|AssignmentLink{track:t as u64+1,detection:d as u64+101}).collect::<Vec<_>>());
                }
            }
            assert_eq!(actual,expected);
            let mut product = 1;
            for component in topo.components(&mut budget)? {
                product *= enumerate(&topo,&component,34,&mut budget)?.ok_or(AssociationError::Limit)?.len();
            }
    ...
    fn exact_list_boundary_does_not_expose_an_incomplete_prefix() -> Result<(),AssociationError> {
        ...
        let topo = topology(3,3,&edges); let mut budget = WorkBudget::new(100000);
        assert_eq!(enumerate(&topo,&full(3,3),34,&mut budget)?.ok_or(AssociationError::Limit)?.len(),34);
        assert!(enumerate(&topo,&full(3,3),33,&mut budget)?.is_none());
        Ok(())
    }
    #[test]
    fn independent_pairs_factor_billions_of_assignments_and_use_bit_31() -> Result<(),AssociationError> {
        let edges: Vec<_> = (0..32).map(|i|(i,i)).collect();
        let topo = topology(32,32,&edges); let mut budget = WorkBudget::new(100000);
        let domains = topo.components(&mut budget)?; assert_eq!(domains.len(),32);
        let mut count = 1_u128;
        for domain in domains {
            let all = enumerate(&topo,&domain,2,&mut budget)?.ok_or(AssociationError::Limit)?;
            assert_eq!(all.len(),2); count *= all.len() as u128;
            assert!(all[0].links.is_empty()); assert_eq!(all[1].links.len(),1);
        }
    ...
    fn isolated_nodes_have_explicit_unmatched_assignments() -> Result<(),AssociationError> {
        let topo = topology(2,3,&[]); let mut budget = WorkBudget::new(100000);
        let domains = topo.components(&mut budget)?; assert_eq!(domains.len(),5);
        for domain in domains {
            let all = enumerate(&topo,&domain,1,&mut budget)?.ok_or(AssociationError::Limit)?;
            assert_eq!(all.len(),1); assert!(all[0].links.is_empty());
            assert_eq!(all[0].unmatched_tracks.len()+all[0].unmatched_detections.len(),1);
        }
        Ok(())
    }
    #[test]
