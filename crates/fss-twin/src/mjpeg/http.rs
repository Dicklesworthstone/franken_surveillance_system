...
pub struct HttpForeground<'a> {source:&'a HttpJpegFrame,result:MultipartForeground}
impl HttpForeground<'_> {
    /// Original HTTP identity and complete JPEG-to-wire spans; not a completion claim.
    pub fn source(&self)->&HttpJpegFrame {self.source}
    /// Existing source-bound decoded, rectified, masked foreground/crop result.
    pub fn foreground(&self)->&JpegForeground {self.result.foreground()}
}
...
pub fn detect_http<'a>(model:&JpegBackground,plan:&RectificationPlan,query:HttpQuery<'a>,
    decoder:&mut DecodeBudget<'_>,geometry:&mut WorkBudget<'_>)->Result<HttpForeground<'a>,JpegPipelineError> {
    geometry.charge(0)?;
    if query.frame.head().wire!=query.expected_response {return Err(JpegPipelineError::BasisMismatch);}
    let result=detect_multipart(model,plan,MultipartQuery{expected_entity:query.frame.head().entity,
        frame:query.frame.part(),mask:query.mask,binding:query.binding,capture:query.capture,
        policy:query.policy,limits:query.limits},decoder,geometry)?;
    Ok(HttpForeground{source:query.frame,result})
}
