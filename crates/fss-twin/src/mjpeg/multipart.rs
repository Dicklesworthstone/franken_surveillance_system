...
pub fn detect_multipart(
    model: &JpegBackground,
    plan: &RectificationPlan,
    query: MultipartQuery<'_>,
    decoder: &mut DecodeBudget<'_>,
    geometry: &mut WorkBudget<'_>,
) -> Result<MultipartForeground, JpegPipelineError> {
    geometry.charge(0)?;
    let source = query.frame.receipt();
    if source.basis != query.expected_entity
        || source.encoded_sha256 != query.binding.encoded_sha256
    {
