...
impl CameraNextSummary {
    ...
    pub fn normalized_center(&self) -> [f64; 2] {
        let a = self.region_envelope.min();
        let b = self.region_envelope.max();
...
impl From<GeometryError> for HandoffSummaryError {
    fn from(v: GeometryError) -> Self {
        Self::Geometry(v)
    }
}
...
pub fn summarize_next_cameras(
    forecast: &FrontierHandoffForecast,
) -> Result<NextCameraSummary, HandoffSummaryError> {
    if forecast.motion.total_mass() != forecast.handoff.total_mass {
        return Err(HandoffSummaryError::InconsistentForecast);
    }
    let mut acc: Vec<Accumulator> = Vec::new();
    acc.try_reserve_exact(forecast.handoff.camera_scope.len())
        .map_err(|_| HandoffSummaryError::Overflow)?;
        ...
        match &route.next {
            NextCameraOutcome::Indeterminate => {
                indeterminate = indeterminate
                    .checked_add(route.mass)
                    .ok_or(HandoffSummaryError::Overflow)?
            }
            ...
            NextCameraOutcome::Predicted {
                ...
                for &camera in cameras {
                    let observation = route
                        .observations
                        .iter()
                        .find(|o| o.camera == camera && o.nominal_capture_ns == *nominal_capture_ns)
                        .ok_or(HandoffSummaryError::InconsistentForecast)?;
                    let r = observation.region;
                    let lo = r.min();
                    let hi = r.max();
                    if let Some(item) = acc.iter_mut().find(|x| x.camera == camera) {
                        ...
                        item.routes = item
                            ...
                            .checked_add(1)
                            .ok_or(HandoffSummaryError::Overflow)?;
                        item.min_time = item.min_time.min(*nominal_capture_ns);
                        item.max_time = item.max_time.max(*nominal_capture_ns);
                        for axis in 0..2 {
                        ...
                        acc.push(Accumulator {
                            camera,
                            mass: route.mass,
                            protected: if route.protected { route.mass } else { 0 },
                            routes: 1,
                            min_time: *nominal_capture_ns,
                            max_time: *nominal_capture_ns,
                            min: lo,
                            max: hi,
                        });
                    }
    ...
    cameras
        .try_reserve_exact(acc.len())
        .map_err(|_| HandoffSummaryError::Overflow)?;
    for item in acc {
        cameras.push(CameraNextSummary {
            camera: item.camera,
            heuristic_support_mass: item.mass,
            protected_support_mass: item.protected,
            supporting_routes: item.routes,
            nominal_capture_range: NanosecondInterval::new(item.min_time, item.max_time)?,
            region_envelope: ImageRect::new(item.min, item.max)?,
        });
    }
    cameras.sort_by(|a, b| {
        b.heuristic_support_mass
            .cmp(&a.heuristic_support_mass)
            .then(
                a.nominal_capture_range
                    .earliest()
                    .cmp(&b.nominal_capture_range.earliest()),
            )
            .then(a.camera.cmp(&b.camera))
    });
    Ok(NextCameraSummary {
        total_route_mass: forecast.handoff.total_mass,
        indeterminate_mass: indeterminate,
        no_modeled_observation_mass: none,
        cameras,
    })
}
