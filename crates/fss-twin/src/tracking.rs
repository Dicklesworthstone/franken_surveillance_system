...
impl ProjectionError {
    ...
    pub(crate) fn valid_for(self, intrinsics: PinholeIntrinsics) -> bool {
        self.centre
            .iter()
            .chain(self.focal.iter())
            .chain(self.principal.iter())
...
impl ContactProjection {
    ...
    pub fn observation(&self) -> ContactObservation {
        self.observation
    }
    /// Exact camera snapshot used by the operation.
    pub fn camera(&self) -> TrackingCamera {
        self.camera
    }
...
pub fn project_contact(
...
) -> Result<ContactProjection, TwinError> {
    budget.charge(0)?;
    validate(camera, observation, twin.basis(), options)?;
    let mut output = ContactProjection {
        observation,
        camera,
        twin_digest: twin.digest(),
        quality: ProjectionQuality::ContactUnknown,
        hypotheses: Vec::new(),
    };
    if !observation.visible_contact {
        return Ok(output);
    }
    let pixel = [
        observation.pixel_min[0] * 0.5 + observation.pixel_max[0] * 0.5,
        observation.pixel_min[1] * 0.5 + observation.pixel_max[1] * 0.5,
    ];
    let ray = camera.pose.ray(camera.intrinsics, pixel)?;
    let nominal = twin.mesh().support_hits(
        twin.basis(),
        ray,
        options.near,
        options.far,
    ...
    output
        .hypotheses
        .try_reserve_exact(options.max_hypotheses)
    ...
    if let (Some(error), Some(map_error)) = (camera.error, twin.geometry_error()) {
        ...
        for (i, triangle) in twin.triangles().iter().enumerate() {
            ...
            if let Some((bounds, grazing)) =
                surface_bounds(points, map_error, origin, direction, options)?
            {
                if output.hypotheses.len() == options.max_hypotheses {
                    return Err(TwinError::Limit);
                }
                let hit = nominal
                    .iter()
                    .find(|hit| hit.triangle as usize == i)
                    .copied();
                output.hypotheses.push(ContactHypothesis {
                    triangle: i as u32,
...
fn validate(
    ...
    if c.geometry != basis
        ...
        || o.exposure == 0
        || o.camera != c.camera
        || o.image_domain != c.image_domain
        || o.clock != c.clock
        || c.validity[0] > c.validity[1]
        || o.capture[0] > o.capture[1]
        || o.capture[0] < c.validity[0]
        || o.capture[1] > c.validity[1]
    {
        return Err(TwinError::Basis);
    }
    if !c.intrinsics.contains(o.pixel_min)
        || !c.intrinsics.contains(o.pixel_max)
        || (0..2).any(|i| o.pixel_min[i] > o.pixel_max[i])
        || !opt.near.is_finite()
        || !opt.far.is_finite()
        || opt.near <= 0.0
        || opt.far <= opt.near
    ...
    if opt.max_hypotheses == 0 || opt.max_hypotheses > 128 {
        return Err(TwinError::Limit);
    }
...
fn occluded(
    ...
    budget: &mut WorkBudget<'_>,
) -> Result<Option<bool>, TwinError> {
    if let Some(hit) = hit {
        let margin = (hit.distance * 1e-8).min(hit.distance * 0.25);
        return Ok(Some(twin.mesh().segment_occluded(
            twin.basis(),
            camera.pose.center(),
            hit.point,
            margin,
...
fn ray_bounds(
    ...
    o: ContactObservation,
    e: ProjectionError,
) -> Result<(I3, I3), TwinError> {
    let [fx, fy] = c.intrinsics.focal_lengths();
    let [cx, cy] = c.intrinsics.principal_point();
    let x = Interval::new(o.pixel_min[0], o.pixel_max[0])?
        .sub(Interval::around(cx, e.principal[0])?)?
        .div(Interval::around(fx, e.focal[0])?)?;
    let y = Interval::new(o.pixel_min[1], o.pixel_max[1])?
        .sub(Interval::around(cy, e.principal[1])?)?
    ...
    let centre = c.pose.center();
    let rotation = c.pose.rotation();
    let mut origin = [Interval::point(0.0)?; 3];
    let mut direction = origin;
    for i in 0..3 {
        origin[i] = Interval::around(centre[i], e.centre[i])?;
        direction[i] = dot(
            [
                Interval::around(rotation[0][i], e.rotation_entry)?,
                Interval::around(rotation[1][i], e.rotation_entry)?,
                Interval::around(rotation[2][i], e.rotation_entry)?,
            ],
            bearing,
        )?;
    }
...
fn surface_bounds(
    ...
    for i in 0..3 {
        for j in 0..3 {
            vertices[i][j] = Interval::around(points[i][j], error)?;
        }
    }
    let normal = cross(
    ...
    if !grazing {
        ...
        let Some(value) = distance.intersect(plane) else {
            return Ok(None);
        };
        distance = value;
    }
    ...
    for axis in 0..3 {
        triangle_bounds[axis] = Interval::new(
            vertices
                .iter()
                .map(|v| v[axis].lower())
        ...
        if !direction[axis].contains(0.0) {
            let slab = triangle_bounds[axis]
                .sub(origin[axis])?
                .div(direction[axis])?;
            let Some(value) = distance.intersect(slab) else {
                return Ok(None);
            };
            distance = value;
        }
    ...
    for axis in 0..3 {
        ...
        let Some(value) = ray.intersect(triangle_bounds[axis]) else {
            return Ok(None);
        };
        position[axis] = value;
    }
    ...
    for edge in 0..3 {
        ...
        if dot(cross(sub(b, a)?, sub(position, a)?)?, normal)?.upper() < 0.0 {
            return Ok(None);
        }
    }
