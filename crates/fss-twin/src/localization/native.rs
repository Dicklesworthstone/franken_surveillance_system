...
use super::{BinaryDescriptor, CameraLocalization, FeatureFrame, ImageFeature, ImageIdentity,
    LocalizationAtlas, LocalizationCamera, LocalizationError, MatchOptions, MAX_IMAGE_FEATURES};

const RADIUS: usize = 16;
const CIRCLE: [(isize, isize); 16] = [(0,-3),(1,-3),(2,-2),(3,-1),(3,0),(3,1),(2,2),(1,3),
...
impl std::fmt::Debug for GrayImage<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GrayImage").field("dimensions", &self.identity.dimensions).finish_non_exhaustive()
    }
}
impl<'a> GrayImage<'a> {
    ...
    pub fn new(identity: ImageIdentity, pixels: &'a [u8], allowed: &'a [u8],
        ...
        if width > 4096 || height > 4096 || count > 4_194_304 { return Err(LocalizationError::Limit); }
        if count != pixels.len() || count != allowed.len() { return Err(LocalizationError::InvalidInput); }
        budget.charge((count as u64) * 3)?;
        if allowed.iter().any(|x| *x > 1) { return Err(LocalizationError::InvalidInput); }
        if ContentDigest::sha256(pixels).bytes() != identity.pixels { return Err(LocalizationError::BasisMismatch); }
        let mask_digest = ContentDigest::sha256(allowed).bytes();
        budget.charge(0)?;
        Ok(Self { identity, pixels, allowed, mask_digest, width, height })
    }
...
impl Default for ExtractionOptions {
    fn default() -> Self { Self { threshold:20, maximum_features:512, separation:6 } }
}
...
#[derive(Clone, Copy)]
struct Corner { score: u8, index: usize }
...
pub fn extract_gray(image: &GrayImage<'_>, options: ExtractionOptions,
    budget: &mut WorkBudget<'_>) -> Result<ExtractedFrame, LocalizationError> {
    budget.charge(0)?;
    if options.threshold == 0 || options.threshold == 255 || options.maximum_features == 0
        || options.maximum_features > MAX_IMAGE_FEATURES || !(1..=128).contains(&options.separation) {
    ...
    if w > 2*RADIUS && h > 2*RADIUS {
        for y in RADIUS..h-RADIUS {
            for x in RADIUS..w-RADIUS {
                ...
                if excluded[b*stride+r]+excluded[t*stride+l] != excluded[t*stride+r]+excluded[b*stride+l] { continue; }
                budget.charge(320)?;
                tested_centers += 1;
                let score=fast_score(image,x,y);
                if score > options.threshold { scores[y*w+x]=score; }
            }
        }
    }
    ...
    for y in 1..h.saturating_sub(1) {
        for x in 1..w.saturating_sub(1) {
            ...
            for yy in y-1..=y+1 {for xx in x-1..=x+1 {
                let other=yy*w+xx;
                if scores[other]>score || (scores[other]==score && other<index) {maximum=false;}
            }}
            if !maximum {continue;}
            local_maxima+=1;
            ...
            for slot in 0..8 {
                ...
                if better(candidate,tiles[tile][slot]) {
                    for j in (slot+1..8).rev() {tiles[tile][j]=tiles[tile][j-1];}
    ...
    candidates.try_reserve_exact(512).map_err(|_|LocalizationError::Limit)?;
    for tile in tiles {for corner in tile {if corner.score>0 {candidates.push(corner);}}}
    budget.charge(5120)?;
    candidates.sort_by(|a,b|b.score.cmp(&a.score).then(a.index.cmp(&b.index)));
    let mut features:Vec<ImageFeature>=Vec::new();
    features.try_reserve_exact(options.maximum_features).map_err(|_|LocalizationError::Limit)?;
    for corner in candidates {
        if features.len()==options.maximum_features {break;}
        let (x,y)=(corner.index%w,corner.index/w);
        let mut separated=true;
        for feature in &features {
            budget.charge(1)?;
            let dx=(x as f64+0.5)-feature.pixel[0];let dy=(y as f64+0.5)-feature.pixel[1];
            if dx*dx+dy*dy < f64::from(options.separation).powi(2) {separated=false;break;}
        }
        if !separated {continue;}
        let descriptor=describe(image,x,y,budget)?;
        features.push(ImageFeature{id:corner.index as u64+1,pixel:[x as f64+0.5,y as f64+0.5],descriptor});
    }
    let selected=features.len();
    let frame=FeatureFrame::new(image.identity,descriptor_domain(),features,budget)?;
    budget.charge(0)?;
    Ok(ExtractedFrame{frame,selection:FeatureSelection{mask_digest:image.mask_digest,threshold:options.threshold,
        maximum_features:options.maximum_features,separation:options.separation,
        tested_centers,local_maxima,selected,omitted:local_maxima-selected}})
}
fn better(a:Corner,b:Corner)->bool {a.score>b.score || (a.score==b.score && a.index<b.index)}
...
fn fast_score(image:&GrayImage<'_>,x:usize,y:usize)->u8 {
    ...
    let d:[i16;16]=std::array::from_fn(|i| {
        let (dx,dy)=CIRCLE[i];
        i16::from(image.pixels[(y as isize+dy) as usize*image.width+(x as isize+dx) as usize])-center
    });
    let mut strongest=0_i16;
    for start in 0..16 {
        let mut bright=255_i16;let mut dark=255_i16;
        for k in 0..9 {let delta=d[(start+k)%16];bright=bright.min(delta);dark=dark.min(-delta);}
...
fn mean3(image:&GrayImage<'_>,x:usize,y:usize)->i64 {
    let mut sum=0;
    for yy in y-1..=y+1 {for xx in x-1..=x+1 {sum+=i64::from(image.pixels[yy*image.width+xx]);}}
    sum
}
fn describe(image:&GrayImage<'_>,x:usize,y:usize,budget:&mut WorkBudget<'_>)->Result<BinaryDescriptor,LocalizationError> {
    budget.charge(8192)?;
    let mut mx=0_i64;let mut my=0_i64;
    for dy in -8_i32..=8 {for dx in -8_i32..=8 {
        if dx*dx+dy*dy<=64 {
            let value=mean3(image,(x as i32+dx) as usize,(y as i32+dy) as usize);
            mx+=i64::from(dx)*value;my+=i64::from(dy)*value;
        }
    }}
    let angle=(my as f64).atan2(mx as f64);
    let (sin,cos)=angle.sin_cos();
    let mut state=731_u32;let mut words=[0_u64;4];
    for bit in 0..256 {
        ...
        if sample(pair[0],pair[1])<sample(pair[2],pair[3]) {words[bit/64]|=1_u64<<(bit%64);}
    }
    budget.charge(0)?;
...
pub fn describe_reference_pixels(image:&GrayImage<'_>,points:&[ReferencePixel],
    budget:&mut WorkBudget<'_>)->Result<FeatureFrame,LocalizationError> {
    budget.charge(0)?;
    if points.len()>MAX_IMAGE_FEATURES {return Err(LocalizationError::Limit);}
    let mut features=Vec::new();
    features.try_reserve_exact(points.len()).map_err(|_|LocalizationError::Limit)?;
    for point in points {
        let (x,y)=(point.column as usize,point.row as usize);
        if x<RADIUS || y<RADIUS || x>=image.width.saturating_sub(RADIUS) || y>=image.height.saturating_sub(RADIUS) {return Err(LocalizationError::InvalidInput);}
        for yy in y-RADIUS..=y+RADIUS {
            budget.charge((2*RADIUS+1) as u64)?;
            if image.allowed[yy*image.width+x-RADIUS..=yy*image.width+x+RADIUS].contains(&0) {return Err(LocalizationError::InvalidInput);}
        }
        features.push(ImageFeature{id:point.id,pixel:[x as f64+0.5,y as f64+0.5],descriptor:describe(image,x,y,budget)?});
    }
...
pub fn localize_gray_frame(atlas:&LocalizationAtlas,twin:&PropertyTwin,image:&GrayImage<'_>,
    camera:LocalizationCamera,options:ImageLocalizationOptions,budget:&mut WorkBudget<'_>)
    ->Result<ImageLocalization,LocalizationError> {
    budget.charge(0)?;
    if image.identity.dimensions!=camera.intrinsics.dimensions() || image.identity.image_domain!=camera.image_domain {
        return Err(LocalizationError::BasisMismatch);
    }
    if atlas.basis!=twin.basis() || atlas.twin_digest!=twin.digest()
        || atlas.descriptor_domain!=descriptor_domain() {return Err(LocalizationError::BasisMismatch);}
    if atlas.references.iter().any(|r|r.frame.identity.exposure==image.identity.exposure) {
        return Err(LocalizationError::ReferenceExposure);
    }
    let extracted=extract_gray(image,options.extraction,budget)?;
    let localization=atlas.localize(twin,&extracted.frame,camera,options.matching,options.solving,budget)?;
    Ok(ImageLocalization{selection:extracted.selection,localization})
}
