...
use crate::observed_handoff::{DestinationHypothesis, ObservedForecastError, ObservedForecastOptions,
    ObservedHandoffForecast, forecast_to_feature};
...
impl From<CalibrationGateError> for MonitoredForecastError {
    fn from(value: CalibrationGateError) -> Self { Self::Calibration(value) }
}
impl From<ObservedForecastError> for MonitoredForecastError {
    fn from(value: ObservedForecastError) -> Self { Self::Forecast(value) }
}
...
pub fn forecast_to_feature_monitored(twin:&PropertyTwin, network:&SupportNetwork,
    snapshot:TrackSnapshot<'_>, destination:DestinationHypothesis<'_>,
    cameras:&[MonitoredHandoffCamera<'_,'_>], options:ObservedForecastOptions,
    budget:&mut WorkBudget<'_>)->Result<ObservedHandoffForecast,MonitoredForecastError> {
    budget.charge(0).map_err(|e| MonitoredForecastError::Forecast(e.into()))?;
    if cameras.is_empty() || cameras.len()>64 { return Err(MonitoredForecastError::Limit); }
    let source_capture=snapshot.projection().observation().capture;
    let mut views=Vec::new();
    views.try_reserve_exact(cameras.len()).map_err(|_|MonitoredForecastError::Limit)?;
    for candidate in cameras {
        budget.charge(1).map_err(|e| MonitoredForecastError::Forecast(e.into()))?;
        candidate.monitor.check_handoff_camera(candidate.view,source_capture)?;
        views.push(candidate.view);
    }
    Ok(forecast_to_feature(twin,network,snapshot,destination,&views,options,budget)?)
}
