...
pub struct ScreenedTrackingReport {
    ...
    stream: Option<StreamFrameReceipt>,
...
impl ScreenedImageTracker {
    ...
    ) -> Result<Self, ScreenedTrackingError> {
    ...
    pub fn update_framed_jpeg(
        &mut self,
        input: &FramedScreenedJpeg,
        budget: &mut WorkBudget<'_>,
    ) -> Result<ScreenedTrackingReport, ScreenedTrackingError> {
        budget.charge(0).map_err(ImageTrackingError::from)?;
        let source = input.source();
        let screen = input.result().screening();
        ...
        self.update_inner(
            ...
            screen,
            input.digest(),
            Lane::Framed(source.basis.source),
            Some(source),
            budget,
        )
    }
    ...
    fn update_inner(
        ...
        if screen.source != foreground.source()
            || screen.mask != foreground.mask_digest()
        ...
        let (mut frame, detections) =
            foreground_input(foreground, self.tracker.policy().maximum_detections, budget)?;
        let mut basis = [0; 128];
        ...
        let digest = ContentDigest::sha256(&bytes).bytes();
        let result = ScreenedTrackingReport {
            digest,
            prior: self.digest,
            input,
            screen: *screen,
