...
use crate::image_tracking::{ImageDetection, ImageTrackingError, ImageTrackingFrame, TrackingAvailability};
...
impl From<ZonePipelineError> for HogZoneError {
    fn from(error: ZonePipelineError) -> Self { Self::Pipeline(error) }
}
fn charge(budget: &mut WorkBudget<'_>, n: u64) -> Result<(), HogZoneError> {
    budget.charge(n).map_err(|e| HogZoneError::Pipeline(ZonePipelineError::Tracking(e.into())))
}
...
impl HogZonePipeline {
    ...
    pub fn new(pipeline: ImageZonePipeline, stream_generation: u64,
        budget: &mut WorkBudget<'_>) -> Result<Self, HogZoneError> {
        charge(budget, 1)?;
        if stream_generation == 0 || pipeline.tracker().exposure_count() != 0 {
            return Err(HogZoneError::BasisMismatch);
        }
        Ok(Self { pipeline, generation: stream_generation, last_screen: None, scan: None, history_gap: false })
    }
    /// Immutable existing trajectory and zone reports, including accepted-but-pending state.
    pub fn pipeline(&self) -> &ImageZonePipeline { &self.pipeline }
    /// Exact complete scan backing the last accepted observation, even while zones are pending.
    pub fn scan_digest(&self) -> Option<[u8; 32]> { self.scan }
    /// Independent health report, never modified to acknowledge semantic completion.
    pub fn last_screening(&self) -> Option<&ScreeningReport> { self.last_screen.as_ref() }
    ...
    pub fn requires_analysis(&self) -> bool { !self.pipeline.tracker().tracks().is_empty() }
    /// Finish only the accepted frame's outstanding zone computation.
    pub fn resume(&mut self, budget: &mut WorkBudget<'_>) -> Result<ZonePipelineProgress, HogZoneError> {
    ...
    pub fn observe(&mut self, scan: &HogScan, screen: &ScreeningReport,
        budget: &mut WorkBudget<'_>) -> Result<ZonePipelineProgress, HogZoneError> {
        charge(budget, 1024)?;
        if self.pipeline.is_pending() { return Err(ZonePipelineError::PendingAnalysis.into()); }
        if scan.source() != screen.source || scan.mask_digest() != screen.mask
            || screen.stamp.stream_generation != self.generation
            || self.last_screen.is_some_and(|old| old.policy != screen.policy) {
            return Err(HogZoneError::BasisMismatch);
        }
        let mut skipped = 0;
        let history_gap = if let Some(old) = self.last_screen {
            ...
            if screen.previous.is_none() { return Err(HogZoneError::BasisMismatch); }
        ...
        } else { TrackingAvailability::Available };
        let mut binding = [0_u8; 160];
        binding[..32].copy_from_slice(&scan.generation()); binding[32..64].copy_from_slice(&screen.policy);
        ...
        binding.fill(0);
        binding[..32].copy_from_slice(&scan.digest()); binding[32..64].copy_from_slice(&screen.digest());
        binding[64..72].copy_from_slice(&skipped.to_le_bytes()); binding[72] = u8::from(history_gap);
        ...
        detections.try_reserve_exact(count).map_err(|_| HogZoneError::Pipeline(
            ZonePipelineError::Tracking(ImageTrackingError::Limit)))?;
        for window in scan.windows() {
            if window.disposition != WindowDisposition::Selected { continue; }
            charge(budget, 128)?;
            let mut record = [0_u8; 80]; record[..32].copy_from_slice(&evidence);
            ...
            let partial = (0..2).any(|axis| window.source_min[axis] == 0
                || window.source_max[axis] == scan.source().image.dimensions[axis]);
            detections.push(ImageDetection { id: window.id, evidence: ContentDigest::sha256(&record).bytes(),
                min: window.source_min, max: window.source_max, partial });
        }
        let frame = ImageTrackingFrame { source: scan.source(), detector,
            permission_mask: scan.mask_digest(), evidence, availability };
        ...
        let progress = self.pipeline.observe_detections(frame, &detections, budget)?;
        self.last_screen = Some(*screen); self.scan = Some(scan.digest()); self.history_gap = history_gap;
        Ok(progress)
    }
}
