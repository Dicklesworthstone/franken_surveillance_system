...
pub fn import_twin(
...
) -> Result<PropertyTwin, TwinError> {
    budget.charge(0)?;
    if limits.bytes > MAX_BYTES
        || limits.features > 65_536
        || limits.objects > 65_536
        || limits.vertices > 262_144
        || limits.triangles > 524_288
    ...
    if source == [0; 32] || source != expected.source_scene_sha256 {
    ...
    let scale = match tag {
        ...
        1 | 2
            if factor > 0.0
                && factor <= 1e9
                && (scale_error == -1.0 || (scale_error >= 0.0 && scale_error < factor)) =>
        {
            let error = (scale_error >= 0.0).then_some(scale_error);
            if tag == 1 {
                ScaleEvidence::Estimated {
                    metres_per_unit: factor,
                    error,
                }
            } else {
                ScaleEvidence::MeasuredAnchor {
                    metres_per_unit: factor,
                    error,
                }
            }
    ...
    let error = r.number()?;
    if error != -1.0 && !(0.0..=1e12).contains(&error) {
    ...
    for _ in 0..nf {
        budget.charge(1)?;
        let id = r.text(256)?;
        if features.last().is_some_and(|last| last.id >= id) {
            return Err(TwinError::Reference);
        }
        let surface = match r.u8()? {
            0 => SurfaceKind::Unknown,
            1 => SurfaceKind::PedestrianPath,
            2 => SurfaceKind::Grass,
            3 => SurfaceKind::Stairs,
            4 => SurfaceKind::Deck,
            5 => SurfaceKind::Structure,
    ...
    for _ in 0..no {
        budget.charge(1)?;
        let id = r.text(256)?;
        if objects.last().is_some_and(|last| last.id >= id) {
            return Err(TwinError::Reference);
        }
        let feature = r.u32()?;
        if feature as usize >= nf {
            return Err(TwinError::Reference);
        }
        objects.push(TwinObject {
            id,
            feature,
            support: r.boolean()?,
            opaque: r.boolean()?,
    ...
    for _ in 0..nt {
        ...
        triangles.push(IndexedTriangle {
            vertices: indices,
            feature: u64::from(object.feature) + 1,
    ...
    let mesh = TriangleMesh::from_indexed(
        ...
        &triangles,
        MeshLimits {
            max_vertices: limits.vertices,
            max_triangles: limits.triangles,
        },
        budget,
    )?;
    budget.charge(0)?;
...
impl<'a> Reader<'a> {
    ...
    fn u8(&mut self) -> Result<u8, TwinError> {
        Ok(self.take(1)?[0])
    }
    fn u32(&mut self) -> Result<u32, TwinError> {
        Ok(u32::from_le_bytes(
            self.take(4)?.try_into().map_err(|_| TwinError::Format)?,
        ))
    }
    fn u64(&mut self) -> Result<u64, TwinError> {
        Ok(u64::from_le_bytes(
            self.take(8)?.try_into().map_err(|_| TwinError::Format)?,
        ))
    }
    fn number(&mut self) -> Result<f64, TwinError> {
        let n = f64::from_bits(self.u64()?);
        if !n.is_finite() || n.to_bits() == (-0.0f64).to_bits() {
            return Err(TwinError::Numeric);
        }
        Ok(n)
    }
    ...
    fn text(&mut self, max: usize) -> Result<String, TwinError> {
        let len =
            u16::from_le_bytes(self.take(2)?.try_into().map_err(|_| TwinError::Format)?) as usize;
        if len == 0 || len > max {
            return Err(TwinError::Limit);
        }
