...
pub struct RadialFocalScanOptions {
    ...
    /// Brown radial k1 interval. k2/k3/tangential terms are deliberately not inferred.
    pub minimum_k1:f64,
    pub maximum_k1:f64,
    pub k1_samples:usize,
    /// Largest undistorted normalized radius admitted for every candidate.
...
impl RadialFocalScanOptions {
    fn validate(self,dimensions:[u32;2])->Result<(),LocalizationError>{
        ...
        if dimensions.iter().any(|n|*n==0) || !(3..=65).contains(&self.focal_samples)
            ...
            || !self.maximum_undistorted_radius.is_finite() || !(1e-3..=64.0).contains(&self.maximum_undistorted_radius){
...
impl RadialFocalScan {
    /// Re-undistort held-out RAW observations under each sample before fixed-pose validation.
    pub fn validate_all_candidates<'a>(&'a self,holdout_raw:&[Correspondence],maximum_error_px:f64,
        ...
        for (sample_index,sample) in self.samples.iter().enumerate(){
            ...
            let adjusted=match undistort_correspondences(holdout_raw,sample.intrinsics,sample.k1,
                self.options.maximum_undistorted_radius,budget){Ok(v)=>v,Err(RadialLensFailure::NonInvertible|RadialLensFailure::ObservationOutsideAdmittedDomain)=>continue};
...
pub fn localize_radial_focal_scan(atlas:&LocalizationAtlas,twin:&PropertyTwin,query:&FeatureFrame,
    matching:MatchOptions,options:RadialFocalScanOptions,budget:&mut WorkBudget<'_>)
...
pub fn scan_radial_focal(basis:fss_geometry::GeometryBasis,dimensions:[u32;2],raw:&[Correspondence],
    ...
    for fi in 0..options.focal_samples{
        ...
        for ki in 0..options.k1_samples{
            budget.charge(1)?;
            let kt=if options.k1_samples==1{0.0}else{ki as f64/(options.k1_samples-1) as f64};
            ...
            let outcome=match undistort_correspondences(raw,intrinsics,k1,options.maximum_undistorted_radius,budget){
                Err(reason)=>RadialSampleOutcome::LensFailure(reason),
...
fn undistort_correspondences(raw:&[Correspondence],intrinsics:PinholeIntrinsics,k1:f64,max_radius:f64,
    budget:&mut WorkBudget<'_>)->Result<Vec<Correspondence>,RadialLensFailure>{
    if !k1.is_finite() || !max_radius.is_finite() || max_radius<=0.0{return Err(RadialLensFailure::NonInvertible);}
    let r2=max_radius*max_radius;
    if 1.0+k1*r2<=1e-8 || 1.0+3.0*k1*r2<=1e-8{return Err(RadialLensFailure::NonInvertible);}
    let mut output=Vec::new();output.try_reserve_exact(raw.len()).map_err(|_|RadialLensFailure::ObservationOutsideAdmittedDomain)?;
    let [fx,fy]=intrinsics.focal_lengths();let [cx,cy]=intrinsics.principal_point();
    let maximum_distorted=max_radius*(1.0+k1*r2);
    for point in raw{
        budget.charge(64).map_err(|_|RadialLensFailure::ObservationOutsideAdmittedDomain)?;
        let xd=(point.pixel[0]-cx)/fx;let yd=(point.pixel[1]-cy)/fy;let rd=xd.hypot(yd);
        if !rd.is_finite() || rd>maximum_distorted+1e-12{return Err(RadialLensFailure::ObservationOutsideAdmittedDomain);}
        ...
        if !intrinsics.contains(pixel){return Err(RadialLensFailure::ObservationOutsideAdmittedDomain);}
