...
use fss_geometry::{
    Correspondence, GeometryError, PinholeIntrinsics, PoseSearch, PoseSolverOptions,
    PoseValidation, WorkBudget, estimate_camera_pose,
};
...
impl RadialFocalScanOptions {
    fn validate(self, dimensions: [u32; 2]) -> Result<(), LocalizationError> {
        let product = self
            .focal_samples
            .checked_mul(self.k1_samples)
            .ok_or(LocalizationError::Limit)?;
        if dimensions.contains(&0)
            || !(3..=65).contains(&self.focal_samples)
            || !(1..=33).contains(&self.k1_samples)
            || product > 1024
            ...
            || !self.minimum_k1.is_finite()
            || !self.maximum_k1.is_finite()
            || self.minimum_k1 > self.maximum_k1
            || self.minimum_k1.abs() > 10.0
            || self.maximum_k1.abs() > 10.0
            || !self.maximum_undistorted_radius.is_finite()
            || !(1e-3..=64.0).contains(&self.maximum_undistorted_radius)
        {
...
impl<'a> RadialValidationSet<'a> {
    ...
    pub fn scan(&self) -> &'a RadialFocalScan {
        self.scan
    }
    /// Verdicts for every validated candidate, in scan order.
    pub fn reports(&self) -> &[RadialCandidateValidation] {
        &self.reports
    }
...
impl RadialFocalScan {
    ...
    pub fn validate_all_candidates<'a>(
        &'a self,
        holdout_raw: &[Correspondence],
        maximum_error_px: f64,
        budget: &mut WorkBudget<'_>,
    ) -> Result<RadialValidationSet<'a>, LocalizationError> {
        budget.charge(0)?;
        ...
        for (sample_index, sample) in self.samples.iter().enumerate() {
            ...
            let adjusted = match undistort_correspondences(
                holdout_raw,
                sample.intrinsics,
                sample.k1,
                self.options.maximum_undistorted_radius,
                budget,
            ) {
                Ok(v) => v,
                Err(UndistortError::Lens(_)) => continue,
            ...
            for candidate in 0..search.candidates().len() {
                let validation = search.validate_candidate(
                    candidate,
                    self.basis,
                    &adjusted,
                    maximum_error_px,
                ...
                if validation.passed {
                    passing.push((sample_index, candidate));
                }
                reports.push(RadialCandidateValidation {
                    sample: sample_index,
                    candidate,
                    validation,
                });
...
pub fn localize_radial_focal_scan(
    atlas: &LocalizationAtlas,
    twin: &PropertyTwin,
    query: &FeatureFrame,
    matching: MatchOptions,
    options: RadialFocalScanOptions,
    budget: &mut WorkBudget<'_>,
) -> Result<RadialLocalization, LocalizationError> {
    budget.charge(0)?;
    options.validate(query.identity().dimensions)?;
    if atlas
        .references()
        .iter()
    ...
    let required = 6.max(options.pose.minimum_inliers);
    let outcome = if matches.correspondences.len() < required {
        RadialLocalizationOutcome::InsufficientMatches {
            found: matches.correspondences.len(),
            required,
        }
    } else {
        RadialLocalizationOutcome::Scan(scan_radial_focal(
            twin.basis(),
    ...
    budget.charge(0)?;
    Ok(RadialLocalization { matches, outcome })
}

/// Extracts the grayscale frame, then runs [`localize_radial_focal_scan`] on it.
pub fn localize_gray_radial_focal_scan(
    atlas: &LocalizationAtlas,
    twin: &PropertyTwin,
    image: &GrayImage<'_>,
    ...
    options: RadialFocalScanOptions,
    budget: &mut WorkBudget<'_>,
) -> Result<GrayRadialLocalization, LocalizationError> {
    budget.charge(0)?;
    let extracted = extract_gray(image, extraction, budget)?;
    let localization =
        localize_radial_focal_scan(atlas, twin, &extracted.frame, matching, options, budget)?;
    budget.charge(0)?;
    Ok(GrayRadialLocalization {
...
pub fn scan_radial_focal(
    basis: fss_geometry::GeometryBasis,
    dimensions: [u32; 2],
    raw: &[Correspondence],
    options: RadialFocalScanOptions,
    budget: &mut WorkBudget<'_>,
) -> Result<RadialFocalScan, LocalizationError> {
    budget.charge(0)?;
    options.validate(dimensions)?;
    let started = budget.used();
    let count = options.focal_samples * options.k1_samples;
    let mut samples = Vec::new();
    samples
        .try_reserve_exact(count)
        .map_err(|_| LocalizationError::Limit)?;
    let l0 = options.minimum_fx_px.ln();
    let l1 = options.maximum_fx_px.ln();
    for fi in 0..options.focal_samples {
        let ft = fi as f64 / (options.focal_samples - 1) as f64;
        let fx = (l0 + (l1 - l0) * ft).exp();
        ...
        for ki in 0..options.k1_samples {
            budget.charge(1)?;
            let kt = if options.k1_samples == 1 {
                0.0
            } else {
                ki as f64 / (options.k1_samples - 1) as f64
            };
            let k1 = options.minimum_k1 + (options.maximum_k1 - options.minimum_k1) * kt;
            let outcome = match undistort_correspondences(
                raw,
                intrinsics,
                k1,
                options.maximum_undistorted_radius,
                budget,
            ) {
                ...
                Err(UndistortError::Limit) => return Err(LocalizationError::Limit),
                Ok(points) => {
                    match estimate_camera_pose(basis, intrinsics, &points, options.pose, budget) {
            ...
            samples.push(RadialFocalSample {
                focal_index: fi,
                k1_index: ki,
                k1,
                intrinsics,
...
fn undistort_correspondences(
    raw: &[Correspondence],
    intrinsics: PinholeIntrinsics,
    k1: f64,
    max_radius: f64,
    budget: &mut WorkBudget<'_>,
) -> Result<Vec<Correspondence>, UndistortError> {
    if !k1.is_finite() || !max_radius.is_finite() || max_radius <= 0.0 {
        return Err(UndistortError::Lens(RadialLensFailure::NonInvertible));
    }
    let r2 = max_radius * max_radius;
    if 1.0 + k1 * r2 <= 1e-8 || 1.0 + 3.0 * k1 * r2 <= 1e-8 {
        return Err(UndistortError::Lens(RadialLensFailure::NonInvertible));
    }
    let mut output = Vec::new();
    output
        .try_reserve_exact(raw.len())
        .map_err(|_| UndistortError::Limit)?;
    let [fx, fy] = intrinsics.focal_lengths();
    let [cx, cy] = intrinsics.principal_point();
    let maximum_distorted = max_radius * (1.0 + k1 * r2);
    for point in raw {
        budget.charge(64).map_err(UndistortError::Geometry)?;
        let xd = (point.pixel[0] - cx) / fx;
        let yd = (point.pixel[1] - cy) / fy;
        let rd = xd.hypot(yd);
        if !rd.is_finite() || rd > maximum_distorted + 1e-12 {
            return Err(UndistortError::Lens(
                RadialLensFailure::ObservationOutsideAdmittedDomain,
            ));
        }
        let ru = if rd <= 1e-15 {
            0.0
            ...
            for _ in 0..48 {
                let mid = (lo + hi) * 0.5;
                let value = mid * (1.0 + k1 * mid * mid);
                if value < rd { lo = mid } else { hi = mid }
            }
            (lo + hi) * 0.5
        };
        let scale = if rd <= 1e-15 { 1.0 } else { ru / rd };
        let pixel = [fx * xd * scale + cx, fy * yd * scale + cy];
        if !intrinsics.contains(pixel) {
            return Err(UndistortError::Lens(
                RadialLensFailure::ObservationOutsideAdmittedDomain,
            ));
        }
