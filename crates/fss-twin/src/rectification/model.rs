...
impl LensDistortion {
    ...
    pub(super) fn encode(self, bytes: &mut Vec<u8>) {
        match self {
            ...
            Self::BrownConrady { radial, tangential } => {
                bytes.push(1);
                for value in radial.into_iter().chain(tangential) {
                    super::float(bytes, value);
                }
            }
    ...
    pub(super) fn validate(
        ...
        match self {
            ...
            Self::BrownConrady { radial, tangential } => {
                ...
                let tangential_bound = point(8.0)?
                    .mul(point(radius)?)?
                    .mul(point(tangential[0].abs())?.add(point(tangential[1].abs())?)?)?
                    .upper();
                ...
                let g = [
                    point(1.0)?,
                    point(radial[0])?.mul(point(3.0)?)?,
                    point(radial[1])?.mul(point(5.0)?)?,
                    point(radial[2])?.mul(point(7.0)?)?,
                ];
            ...
            Self::Fisheye { coefficients } => {
                ...
                let derivative = [
                    point(1.0)?,
                    point(coefficients[0])?.mul(point(3.0)?)?,
                    point(coefficients[1])?.mul(point(5.0)?)?,
                    point(coefficients[2])?.mul(point(7.0)?)?,
                    point(coefficients[3])?.mul(point(9.0)?)?,
                ];
    ...
    fn distort(self, x: f64, y: f64) -> Result<[f64; 2], RectificationError> {
        ...
        if result.iter().any(|v| !v.is_finite()) {
            return Err(RectificationError::Numeric);
        }
        Ok(result)
    }
}

fn point(x: f64) -> Result<Interval, RectificationError> {
    Ok(Interval::point(x)?)
}
...
fn positive_polynomial(
    ...
    for cell in 0..128 {
        budget.charge(32)?;
        let x = Interval::outward(
            maximum * (f64::from(cell) / 128.0),
...
pub(super) fn source_pixel(
    ...
    if x.hypot(y) > spec.maximum_radius {
        return Ok(None);
    }
    if spec.distortion.is_pinhole() && spec.source == spec.target {
