...
pub fn hog_recipe_digest() -> [u8; 32] {
    ContentDigest::sha256(RECIPE).bytes()
}
...
impl HogModel {
    ...
    pub fn from_f32_le(
        ...
        if ContentDigest::sha256(bytes).bytes() != expected {
    ...
    pub fn weights_digest(&self) -> [u8; 32] {
        self.weights_digest
    }
...
impl std::fmt::Debug for HogFrame<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HogFrame")
            .field("dimensions", &self.source.image.dimensions)
            .finish_non_exhaustive()
    }
}
impl<'a> HogFrame<'a> {
        ...
        if nx != 0 && ny != 0 {
            ...
            for y in 0..height {
                for x in 0..width {
                    ...
                    let indices = [
                        y * width + x,
                        y * width + reflect_prev(x, width),
                        y * width + reflect_next(x, width),
                        reflect_prev(y, height) * width + x,
                        reflect_next(y, height) * width + x,
                    ];
                    if indices.iter().any(|i| frame.allowed[*i] == 0) {
                        gradients.push(Gradient::default());
                        continue;
                    }
                    let dx = f64::from(frame.pixels[indices[2]]).sqrt()
                        - f64::from(frame.pixels[indices[1]]).sqrt();
                    let dy = f64::from(frame.pixels[indices[4]]).sqrt()
                        - f64::from(frame.pixels[indices[3]]).sqrt();
                    ...
                    gradients.push(Gradient {
                        votes: [magnitude * (1.0 - fraction), magnitude * fraction],
                        bin,
                        allowed: true,
                    });
                }
...
fn histogram(gradients: &[Gradient], width: usize, x: usize, y: usize) -> Block {
        ...
        for iy in 0..16 {
            ...
            for (cellx, wx) in [(x0, 1.0 - fx), (x0 + 1, fx)] {
                for (celly, wy) in [(y0, 1.0 - fy), (y0 + 1, fy)] {
                    if !(0..2).contains(&cellx) || !(0..2).contains(&celly) {
                        continue;
                    }
    ...
    for value in &mut hist {
        *value = (*value * scale).min(0.2);
    }
    let scale = 1.0 / (hist.iter().map(|v| v * v).sum::<f64>().sqrt() + 0.001);
    Block {
        values: hist.map(|v| (v * scale) as f32),
        allowed: true,
    }
}
fn reflect_prev(n: usize, size: usize) -> usize {
    if n == 0 { usize::from(size > 1) } else { n - 1 }
}
fn reflect_next(n: usize, size: usize) -> usize {
    if n + 1 == size {
