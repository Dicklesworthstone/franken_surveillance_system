...
use crate::image_tracking::{ImageTrackObservation, ImageTracker, ImageTrackingFrame,
...
impl ImageMotionPolicy {
    fn validate(self) -> Result<(), ImageMotionError> {
        if [self.measurement_variance, self.initial_velocity_variance].iter()
...
impl ImageMotionReport {
    ...
    pub fn policy(&self) -> ImageMotionPolicy { self.policy }
    /// Every live track in tracker order; no top-k pruning or missing-result elision.
    pub fn outcomes(&self) -> &[ImageMotionOutcome] { &self.outcomes }
}
...
pub fn estimate_image_motion(tracker: &ImageTracker, report: &ImageTrackingReport,
    policy: ImageMotionPolicy, budget: &mut WorkBudget<'_>) -> Result<ImageMotionReport, ImageMotionError> {
    budget.charge(1)?;
    policy.validate()?;
    if tracker.digest() != report.digest() { return Err(ImageMotionError::StaleReport); }
    budget.charge(tracker.tracks().len() as u64)?;
    let mut outcomes = Vec::new();
    outcomes.try_reserve_exact(tracker.tracks().len()).map_err(|_| ImageMotionError::Limit)?;
    ...
    for track in tracker.tracks() {
        ...
        let reason = if frame.availability != TrackingAvailability::Available {
            ...
            } else if [previous.frame.source.capture, latest.frame.source.capture, frame.source.capture]
                .iter().any(|t| t[0] != t[1]) {
                Some(MotionUnavailable::CaptureUncertainty)
            } else if latest.frame.source.capture[0] - previous.frame.source.capture[0] > policy.maximum_pair_gap_ns {
                Some(MotionUnavailable::PairHorizon)
            } else if frame.source.capture[0] - latest.frame.source.capture[0] > policy.maximum_prediction_ns {
                Some(MotionUnavailable::PredictionHorizon)
            } else { None }
        } else { Some(MotionUnavailable::MissingPair) };
        if let Some(reason) = reason {
            outcomes.push(ImageMotionOutcome::Unavailable { track: track.id(), reason });
            continue;
        }
        let previous = track.previous().ok_or(ImageMotionError::Numeric)?;
        let fit_dt = (latest.frame.source.capture[0] - previous.frame.source.capture[0]) as f64 * 1e-9;
        let forecast_ns = frame.source.capture[0] - latest.frame.source.capture[0];
        let start = centroid(previous); let end = centroid(latest);
        let mut axes = start.map(|position| Axis::new(position, policy));
        for (axis, measurement) in axes.iter_mut().zip(end) {
            *axis = axis.predict(fit_dt, policy.acceleration_variance)?
                .update(measurement, policy.measurement_variance)?;
            if forecast_ns > 0 {
                *axis = axis.predict(forecast_ns as f64 * 1e-9, policy.acceleration_variance)?;
            }
        }
        outcomes.push(ImageMotionOutcome::Estimated(ImageMotionEstimate { track: track.id(),
            observations: [previous, latest], at_ns: frame.source.capture[0], predicted: forecast_ns > 0, axes }));
    }
    budget.charge(0)?;
    Ok(ImageMotionReport { tracking: report.digest(), frame, policy, outcomes })
}
...
#[derive(Clone, Copy, Debug, PartialEq)]
struct Axis { position: f64, velocity: f64, pp: f64, pv: f64, vv: f64 }
impl Axis {
    fn new(position: f64, policy: ImageMotionPolicy) -> Self {
        Self { position, velocity: 0.0, pp: policy.measurement_variance,
            pv: 0.0, vv: policy.initial_velocity_variance }
    }
    fn validate(self) -> Result<Self, ImageMotionError> {
        if [self.position, self.velocity, self.pp, self.pv, self.vv].iter().any(|v| !v.is_finite())
            || self.pp <= 0.0 || self.vv < 0.0
    ...
    fn predict(self, dt: f64, q: f64) -> Result<Self, ImageMotionError> {
        let dt2 = dt * dt;
        Self { position: self.position + dt * self.velocity, velocity: self.velocity,
            pp: self.pp + 2.0 * dt * self.pv + dt2 * self.vv + 0.25 * dt2 * dt2 * q,
            pv: self.pv + dt * self.vv + 0.5 * dt2 * dt * q,
            vv: self.vv + dt2 * q }.validate()
    }
    fn update(self, measurement: f64, variance: f64) -> Result<Self, ImageMotionError> {
        ...
        let kp = self.pp / denominator; let kv = self.pv / denominator; let a = 1.0 - kp;
        Self { position: self.position + kp * innovation, velocity: self.velocity + kv * innovation,
...
mod tests {
    ...
    fn frame(time: u64) -> ImageTrackingFrame {
        ImageTrackingFrame { source: ForegroundSource { image: ImageIdentity {
            exposure: ContentDigest::sha256(&time.to_le_bytes()).bytes(), pixels: [4; 32],
            image_domain: [5; 32], dimensions: [64, 16] }, camera: 1, calibration: [6; 32],
            clock: 1, capture: [time, time] }, detector: [7; 32], permission_mask: [8; 32],
            evidence: [9; 32], availability: TrackingAvailability::Available }
    }
    fn detection(left: u32, evidence: u8) -> ImageDetection {
        ImageDetection { id: 1, evidence: [evidence; 32], min: [left, 4], max: [left + 4, 8], partial: false }
    }
    fn tracker() -> ImageTracker {
        ImageTracker::new([10; 32], ImageTrackingPolicy { maximum_tracks: 8, maximum_detections: 8,
            maximum_exposures: 100, minimum_observations: 2, maximum_misses: 2,
            maximum_gap_ns: 10_000_000_000, maximum_speed: 100, gate_padding: 2,
            miss_cost: 1000, ambiguity_margin: 0 }, &mut WorkBudget::new(100_000)).unwrap()
    }
    ...
    fn fitted() -> (ImageTracker, ImageTrackingReport) {
        ...
        tracker.update(frame(1_000_000_000), &[detection(4, 20)], &mut WorkBudget::new(100_000)).unwrap();
        let report = tracker.update(frame(2_000_000_000), &[detection(9, 21)], &mut WorkBudget::new(100_000)).unwrap();
        (tracker, report)
    }
    #[test]
    fn actual_tracker_pair_learns_velocity_and_coasting_retains_sources() {
        ...
        let ImageMotionOutcome::Estimated(observed) = model.outcomes()[0] else { unreachable!() };
        assert!(observed.velocity()[0] > 4.0);
        assert!(!observed.predicted());
        let coast = tracker.update(frame(3_000_000_000), &[], &mut WorkBudget::new(100_000)).unwrap();
        let model = estimate(&tracker, &coast);
        let ImageMotionOutcome::Estimated(predicted) = model.outcomes()[0] else { unreachable!() };
    ...
    fn stale_report_is_refused_without_changing_tracker() {
        let (mut tracker, report) = fitted();
        tracker.update(frame(3_000_000_000), &[], &mut WorkBudget::new(100_000)).unwrap();
        let digest = tracker.digest();
        assert!(matches!(estimate_image_motion(&tracker, &report, policy(), &mut WorkBudget::new(100_000)),
    ...
    fn missing_partial_uncertain_and_unavailable_remain_explicit() {
        let mut tracker = tracker();
        let mut first = frame(1_000_000_000); first.source.capture[1] += 1;
        let report = tracker.update(first, &[detection(4, 20)], &mut WorkBudget::new(100_000)).unwrap();
        assert!(matches!(estimate(&tracker, &report).outcomes()[0],
            ImageMotionOutcome::Unavailable { reason: MotionUnavailable::MissingPair, .. }));
        let report = tracker.update(frame(2_000_000_000), &[detection(9, 21)], &mut WorkBudget::new(100_000)).unwrap();
        assert!(matches!(estimate(&tracker, &report).outcomes()[0],
            ImageMotionOutcome::Unavailable { reason: MotionUnavailable::CaptureUncertainty, .. }));
        let mut partial = detection(10, 22); partial.partial = true;
        let report = tracker.update(frame(3_000_000_000), &[partial], &mut WorkBudget::new(100_000)).unwrap();
        assert!(matches!(estimate(&tracker, &report).outcomes()[0],
            ImageMotionOutcome::Unavailable { reason: MotionUnavailable::PartialObservation, .. }));
        let mut disturbed = frame(4_000_000_000); disturbed.availability = TrackingAvailability::Disturbed;
        let report = tracker.update(disturbed, &[], &mut WorkBudget::new(100_000)).unwrap();
        assert!(matches!(estimate(&tracker, &report).outcomes()[0],
            ImageMotionOutcome::Unavailable { reason: MotionUnavailable::FrameUnavailable, .. }));
    }
    #[test]
    fn fitting_and_prediction_horizons_are_not_silently_extrapolated() {
        let (mut tracker, report) = fitted();
        let mut p = policy(); p.maximum_pair_gap_ns = 1;
        let model = estimate_image_motion(&tracker, &report, p, &mut WorkBudget::new(100_000)).unwrap();
        assert!(matches!(model.outcomes()[0], ImageMotionOutcome::Unavailable { reason: MotionUnavailable::PairHorizon, .. }));
        let report = tracker.update(frame(3_000_000_000), &[], &mut WorkBudget::new(100_000)).unwrap();
        p = policy(); p.maximum_prediction_ns = 0;
        let model = estimate_image_motion(&tracker, &report, p, &mut WorkBudget::new(100_000)).unwrap();
        assert!(matches!(model.outcomes()[0], ImageMotionOutcome::Unavailable { reason: MotionUnavailable::PredictionHorizon, .. }));
    }
    #[test]
    fn every_budget_cut_leaves_source_state_unchanged_and_retryable() {
        let (tracker, report) = fitted(); let digest = tracker.digest();
        ...
        for limit in 0..full.used() {
            assert!(matches!(estimate_image_motion(&tracker, &report, policy(), &mut WorkBudget::new(limit)),
    ...
    fn covariance_stays_valid_under_irregular_measurements() {
        ...
        for i in 0..10_000 {
            ...
            axis = axis.predict(dt, 0.1).unwrap().update(f64::from(i % 200), 1.0).unwrap();
    ...
    fn invalid_priors_and_cancellation_fail_explicitly() {
        ...
        for value in [0.0, -1.0, f64::NAN, f64::INFINITY] {
            let mut p = policy(); p.measurement_variance = value;
            assert!(matches!(estimate_image_motion(&tracker, &report, p, &mut WorkBudget::new(1000)),
                Err(ImageMotionError::InvalidPolicy)));
        }
        let cancellation = std::sync::atomic::AtomicBool::new(true);
        assert!(matches!(estimate_image_motion(&tracker, &report, policy(),
            &mut WorkBudget::cancellable(1000, &cancellation)), Err(ImageMotionError::Geometry(GeometryError::Cancelled))));
    }
    #[test]
    fn integer_time_differences_preserve_large_clock_small_steps() {
        let mut tracker = tracker(); let time = u64::MAX - 10;
        tracker.update(frame(time), &[detection(4, 20)], &mut WorkBudget::new(100_000)).unwrap();
        let report = tracker.update(frame(time + 1), &[detection(5, 21)], &mut WorkBudget::new(100_000)).unwrap();
        let model = estimate(&tracker, &report);
        let ImageMotionOutcome::Estimated(result) = model.outcomes()[0] else { unreachable!() };
