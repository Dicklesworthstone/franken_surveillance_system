...
use crate::association_hypotheses::{AssociationSelection, AssignmentLink};
...
impl From<TrackError> for BatchError {
    fn from(e: TrackError) -> Self { Self::Track(e) }
}
impl From<AssociationError> for BatchError {
    fn from(e: AssociationError) -> Self { Self::Association(e) }
}
impl From<GeometryError> for BatchError {
    fn from(e: GeometryError) -> Self { Self::Track(e.into()) }
}
...
impl BatchReceipt {
    ...
    pub fn before(&self) -> &[TrackReceipt] { &self.before }
    /// Full membership after the atomic update, including untouched tracks.
    pub fn after(&self) -> &[TrackReceipt] { &self.after }
    /// Only the explicitly selected source updates, in track-ID order.
    pub fn updates(&self) -> &[TrackUpdate] { &self.updates }
    ...
    fn try_copy(&self) -> Result<Self, BatchError> {
        Ok(Self { session: self.session, operation: self.operation, request: self.request,
            twin: self.twin, camera: self.camera, exposure: self.exposure,
            frame_evidence: self.frame_evidence, capture: self.capture,
            one_to_one_basis: self.one_to_one_basis, links: copied(&self.links)?,
            before: copied(&self.before)?, after: copied(&self.after)?, updates: copied(&self.updates)?,
...
impl std::fmt::Debug for ContactTrackSet {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ContactTrackSet").field("tracks", &self.tracks.len())
            .field("sequence", &self.sequence).field("invalidated", &self.invalidated).finish_non_exhaustive()
    }
}
impl ContactTrackSet {
    ...
    pub fn new(twin: &PropertyTwin, session: [u8; 32], mut tracks: Vec<ContactTrack>,
        receipt_capacity: usize, budget: &mut WorkBudget<'_>) -> Result<Self, BatchError> {
        budget.charge(0)?;
        if session == [0; 32] || tracks.is_empty() || tracks.len() > MAX_ASSOCIATION_ITEMS
            || !(1..=256).contains(&receipt_capacity) { return Err(BatchError::InvalidInput); }
        budget.charge(tracks.len() as u64 * 64)?;
        tracks.sort_by_key(|t| t.scope.track);
        let first = &tracks[0];
        for (i, track) in tracks.iter().enumerate() {
            track.snapshot(track.revision())?;
            if track.geometry != twin.basis() || track.twin_digest != twin.digest()
                || track.scope.clock != first.scope.clock || track.cameras != first.cameras
        ...
        for camera in &first.cameras {
            ...
            for track in &tracks {
                ...
                if source.camera == camera.camera {
                    latest = Some(latest.map_or(source.capture[0], |t: u64| t.max(source.capture[0])));
                }
            }
            watermarks.push(latest);
        }
        let mut receipts = VecDeque::new();
        receipts.try_reserve_exact(receipt_capacity).map_err(|_| BatchError::Limit)?;
        budget.charge(0)?;
        Ok(Self { session, geometry: twin.basis(), twin: twin.digest(), tracks, sequence: 0,
            capacity: receipt_capacity, receipts, watermarks, invalidated: false })
    }
    ...
    pub fn track(&self, id: u64) -> Option<&ContactTrack> {
        self.tracks.binary_search_by_key(&id, |t| t.scope.track).ok().map(|i| &self.tracks[i])
    }
    /// Active snapshots for the existing gate_contact_batch input; no state is copied.
    pub fn snapshots(&self, budget: &mut WorkBudget<'_>) -> Result<Vec<TrackSnapshot<'_>>, BatchError> {
        budget.charge(0)?;
        if self.invalidated { return Err(BatchError::Invalidated); }
    ...
    pub fn last_receipt(&self) -> Option<&BatchReceipt> { self.receipts.back() }
    /// Latch invalidation on the entire set without requiring a compute allowance.
    pub fn invalidate(&mut self) {
        self.invalidated = true;
        for track in &mut self.tracks { track.invalidate(); }
    }
    ...
    pub fn apply_selection(&mut self, twin: &PropertyTwin, selection: &AssociationSelection<'_>,
        operation: BatchOperation, budget: &mut WorkBudget<'_>) -> Result<BatchUpdate, BatchError> {
        budget.charge(0)?;
        if self.invalidated { return Err(BatchError::Invalidated); }
        if twin.basis() != self.geometry || twin.digest() != self.twin
            || selection.graph().twin_digest() != self.twin { return Err(BatchError::BasisMismatch); }
        if operation.sequence == 0 || operation.adjudication == [0; 32] { return Err(BatchError::InvalidInput); }
        let request = binding::request(self.session, selection, operation, budget)?;
        if operation.sequence <= self.sequence {
            let old = self.receipts.iter().find(|r| r.operation.sequence == operation.sequence)
                .ok_or(BatchError::ExpiredReplay)?;
            if old.request != request { return Err(BatchError::ConflictingReplay); }
            let receipt = old.try_copy()?;
            budget.charge(0)?;
            return Ok(BatchUpdate { receipt, replayed: true });
        }
        if self.sequence.checked_add(1) != Some(operation.sequence) { return Err(BatchError::Sequence); }
        let frame = selection.graph().frame();
        let camera_index = self.tracks[0].cameras.binary_search_by_key(&frame.camera.camera, |c| c.camera)
        ...
        if self.watermarks[camera_index].is_some_and(|t| frame.capture[0] <= t)
            || self.receipts.iter().any(|r| r.frame_evidence == frame.evidence
        ...
        selection.graph().check_current(twin, camera, &snapshots, budget)?;
        let mut before = reserved(snapshots.len())?;
        for snapshot in snapshots { before.push(snapshot.receipt()); }
        ...
        for observation in observations {
            budget.charge(1)?;
            let index = self.tracks.binary_search_by_key(&observation.track, |t| t.scope.track)
                .map_err(|_| BatchError::BasisMismatch)?;
            let prepared = self.tracks[index].prepare_ingest(twin, observation, Some(operation.adjudication), budget)?;
            if prepared.update.replayed { return Err(BatchError::ReusedExposure); }
        ...
        let receipt = BatchReceipt { session: self.session, operation, request, twin: self.twin,
            camera: camera.camera, exposure: frame.exposure, frame_evidence: frame.evidence,
            capture: frame.capture, one_to_one_basis: selection.policy().one_to_one_basis,
            links: copied(assignment.links())?, before, after, updates,
        ...
        budget.charge(0)?;
        for (index, prepared) in staged { self.tracks[index].apply_prepared(prepared); }
        self.watermarks[camera_index] = Some(frame.capture[0]);
        self.sequence = operation.sequence;
        if self.receipts.len() == self.capacity { self.receipts.pop_front(); }
        self.receipts.push_back(remembered);
        Ok(BatchUpdate { receipt, replayed: false })
    }
}

fn reserved<T>(count: usize) -> Result<Vec<T>, BatchError> {
    let mut result = Vec::new();
    result.try_reserve_exact(count).map_err(|_| BatchError::Limit)?;
    Ok(result)
}
