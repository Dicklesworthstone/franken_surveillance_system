...
use crate::{
    ...
    localization::{
        ...
        ImageIdentity, LocalizationAtlas, LocalizationError, MAX_ATLAS_LANDMARKS,
        MAX_ATLAS_REFERENCES, MAX_IMAGE_FEATURES,
    },
};
...
impl From<GeometryError> for ArchiveError {
    fn from(error: GeometryError) -> Self {
        Self::Localization(error.into())
    }
}
impl std::fmt::Display for ArchiveError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::Format => "invalid atlas archive",
            Self::Digest => "atlas checksum mismatch",
            Self::Basis => "atlas archive basis mismatch",
            Self::Limit => "atlas archive limit exceeded",
...
impl ArchivedAtlas {
    ...
    pub fn atlas(&self) -> &LocalizationAtlas {
        &self.atlas
    }
    /// All source/mask bindings in reference-ID order.
    pub fn references(&self) -> &[ReferenceProvenance] {
        &self.references
    }
...
pub fn encode_atlas(
    ...
    budget.charge(0)?;
    if provenance == [0; 32] || references.len() != atlas.references().len() {
        return Err(ArchiveError::Basis);
    }
    for (source, view) in references.iter().zip(atlas.references()) {
        budget.charge(1)?;
        if source.reference != view.id
            || source.source_record == [0; 32]
            || source.allowed_mask == [0; 32]
        {
            return Err(ArchiveError::Basis);
        }
    }
    let features: usize = atlas
        .references()
        .iter()
        .map(|r| r.frame.features().len())
        .sum();
    let capacity = 188
        + atlas.landmarks().len() * 101
        + references.len() * 180
        + features * 56
        + atlas.bindings().len() * 24;
    if capacity > MAX_ATLAS_BYTES {
    ...
    b.extend_from_slice(&atlas.digest());
    b.extend_from_slice(&provenance);
    put32(&mut b, atlas.landmarks().len() as u32);
    put32(&mut b, references.len() as u32);
    put32(&mut b, atlas.bindings().len() as u32);
    for p in atlas.landmarks() {
        budget.charge(1)?;
        put64(&mut b, p.id);
        put64(&mut b, p.physical_group);
        put32(&mut b, p.feature);
        for x in p.world {
            put_float(&mut b, x);
        }
    ...
    for (r, source) in atlas.references().iter().zip(references) {
        budget.charge(1)?;
        put64(&mut b, r.id);
        let identity = r.frame.identity();
        for hash in [identity.exposure, identity.pixels, identity.image_domain] {
            b.extend_from_slice(&hash);
        }
        for n in identity.dimensions {
            put32(&mut b, n);
        }
        b.extend_from_slice(&source.source_record);
        b.extend_from_slice(&source.allowed_mask);
    ...
    for v in atlas.bindings() {
        budget.charge(1)?;
        put64(&mut b, v.landmark);
        put64(&mut b, v.reference);
        put64(&mut b, v.image_feature);
    }
    let length = (b.len() - 16) as u64;
    b[8..16].copy_from_slice(&length.to_le_bytes());
    budget.charge(b.len() as u64)?;
    let checksum = ContentDigest::sha256(&b).bytes();
...
pub fn decode_atlas(
    ...
    budget: &mut WorkBudget<'_>,
) -> Result<ArchivedAtlas, ArchiveError> {
    budget.charge(0)?;
    ...
    if [expected.package, expected.provenance, expected.descriptor].contains(&[0; 32]) {
        return Err(ArchiveError::Basis);
    }
    budget.charge(bytes.len() as u64 * 2)?;
    if ContentDigest::sha256(bytes).bytes() != expected.package {
        return Err(ArchiveError::Digest);
    }
    let end = bytes.len() - 32;
    if ContentDigest::sha256(&bytes[..end]).bytes().as_slice() != &bytes[end..] {
        return Err(ArchiveError::Digest);
    }
    let mut r = Reader {
        bytes: &bytes[..end],
    ...
    if r.take(8)? != MAGIC || r.u64()? != (end - 16) as u64 {
        return Err(ArchiveError::Format);
    }
    if r.hash()? != twin.digest() || r.hash()? != expected.descriptor {
        return Err(ArchiveError::Basis);
    }
    let semantic = r.hash()?;
    if r.hash()? != expected.provenance {
        return Err(ArchiveError::Basis);
    }
    ...
    for _ in 0..np {
        ...
        let physical_group = r.u64()?;
        let feature = r.u32()?;
        let world = [r.float()?, r.float()?, r.float()?];
        let evidence = r.hash()?;
        let error = match r.byte()? {
            0 => None,
            1 => Some([r.float()?, r.float()?, r.float()?]),
    ...
    let mut views = reserved(nr)?;
    let mut references = reserved(nr)?;
    last = 0;
    for _ in 0..nr {
        budget.charge(1)?;
        let id = r.ordered_id(&mut last)?;
        ...
        let source = ReferenceProvenance {
            reference: id,
            source_record: r.hash()?,
            allowed_mask: r.hash()?,
        };
        let nf = r.count(MAX_IMAGE_FEATURES)?;
        r.require_bytes(nf * 56)?;
        let mut features = reserved(nf)?;
        let mut last_feature = 0;
        for _ in 0..nf {
            budget.charge(1)?;
            features.push(ImageFeature {
                id: r.ordered_id(&mut last_feature)?,
                pixel: [r.float()?, r.float()?],
                descriptor: BinaryDescriptor([r.u64()?, r.u64()?, r.u64()?, r.u64()?]),
            });
        }
        views.push(AtlasReference {
            id,
            frame: FeatureFrame::new(identity, expected.descriptor, features, budget)?,
        });
        references.push(source);
    }
    r.require_bytes(nb * 24)?;
    let mut bindings = reserved(nb)?;
    let mut last_binding = (0, 0, 0);
    for _ in 0..nb {
        budget.charge(1)?;
...
fn put_float(b: &mut Vec<u8>, n: f64) {
    put64(b, if n == 0.0 { 0 } else { n.to_bits() });
}
struct Reader<'a> {
    bytes: &'a [u8],
    offset: usize,
