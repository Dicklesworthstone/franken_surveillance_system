...
use crate::image_tracking::{
    ImageTracker, ImageTrackingError, ImageTrackingPolicy, ImageTrackingReport,
};
...
impl ImageZonePipeline {
    ...
    pub fn new(
        ...
        let tracker =
            ImageTracker::new(episode, tracking, budget).map_err(ZonePipelineError::Tracking)?;
        let monitor = ImageZoneMonitor::new(&tracker, basis, policy, zones, budget)
            .map_err(ZonePipelineError::Zones)?;
        Ok(Self {
            tracker,
            monitor,
            tracking: None,
    ...
    pub fn observe_foreground(
        ...
        if self.pending {
            return Err(ZonePipelineError::PendingAnalysis);
        }
        let s = foreground.source();
        ...
        let report = self
            .tracker
            .update_foreground(foreground, budget)
            .map_err(ZonePipelineError::Tracking)?;
        self.tracking = Some(report);
        self.foreground = Some(foreground.digest());
        self.pending = true;
        self.resume(budget)
    }
    ...
    pub fn resume(
        ...
        let tracking = self
            .tracking
            .as_ref()
            .ok_or(ZonePipelineError::NoObservation)?;
        if !self.pending {
            let zones = self
                .monitor
                .latest()
                .ok_or(ZonePipelineError::NoObservation)?;
            return Ok(ZonePipelineProgress::Complete {
        ...
        match self.monitor.observe(&self.tracker, tracking, budget) {
            Ok(zones) => {
                self.pending = false;
                Ok(ZonePipelineProgress::Complete {
                    tracking: tracking.digest(),
                    zones: zones.digest(),
                })
            }
            Err(error) => Ok(ZonePipelineProgress::Pending {
                tracking: tracking.digest(),
                error,
            }),
        }
    ...
    pub fn analyze_luma(
        ...
        budget: &mut WorkBudget<'_>,
    ) -> Result<LumaZoneAnalysis, ForegroundPipelineError> {
    ...
    pub fn analyze_jpeg(
        ...
        plan: &RectificationPlan,
        input: JpegZoneInput<'_>,
        policy: ForegroundPolicy,
        decoder: &mut DecodeBudget<'_>,
        budget: &mut WorkBudget<'_>,
    ) -> Result<JpegZoneAnalysis, JpegPipelineError> {
...
pub struct LumaZoneAnalysis {
    ...
    progress: Result<ZonePipelineProgress, ZonePipelineError>,
}
impl LumaZoneAnalysis {
    ...
    pub fn progress(&self) -> Result<ZonePipelineProgress, ZonePipelineError> {
        self.progress
    }
...
impl ImageZonePipeline {
    ...
    pub(crate) fn observe_detections(
        &mut self,
        frame: crate::image_tracking::ImageTrackingFrame,
        detections: &[crate::image_tracking::ImageDetection],
        budget: &mut WorkBudget<'_>,
    ) -> Result<ZonePipelineProgress, ZonePipelineError> {
        if self.pending {
            return Err(ZonePipelineError::PendingAnalysis);
        ...
        let report = self
            .tracker
            .update(frame, detections, budget)
            .map_err(ZonePipelineError::Tracking)?;
        self.tracking = Some(report);
