...
impl WorldMotion {
    ...
    pub fn geometry(&self) -> GeometryBasis {
        self.geometry
    }
...
pub fn fit_world_motion(
    previous: &ContactProjection,
    current: &ContactProjection,
    options: MotionFitOptions,
    budget: &mut WorkBudget<'_>,
) -> Result<WorldMotion, TwinError> {
    budget.charge(0)?;
    let a = previous.observation();
    let b = current.observation();
    let ca = previous.camera();
    let cb = current.camera();
    if ca.geometry != cb.geometry
        || previous.twin_digest() != current.twin_digest()
        || a.track != b.track
        || a.clock != b.clock
        || a.evidence == b.evidence
        || (a.camera == b.camera
            && (a.exposure == b.exposure
                || ca.calibration != cb.calibration
                || ca.image_domain != cb.image_domain
                || ca.pose != cb.pose
                || ca.intrinsics != cb.intrinsics
                || ca.error != cb.error))
    {
        return Err(TwinError::Basis);
    }
    if a.camera != b.camera && options.association.is_none_or(|id| id == [0; 32]) {
        return Err(TwinError::Basis);
    }
    if options.association == Some([0; 32]) {
        return Err(TwinError::Basis);
    }
    if options.max_gap_ns == 0
    ...
    let count = previous
        .hypotheses()
        .len()
        .checked_mul(current.hypotheses().len())
        .ok_or(TwinError::Limit)?;
    if count == 0 {
        return Err(TwinError::Unobservable);
    }
    if count > options.max_modes {
        return Err(TwinError::Limit);
    }
    ...
    for first in previous.hypotheses() {
        for last in current.hypotheses() {
            budget.charge(32)?;
            let velocity = if let (Some(pa), Some(pb)) = (first.bounds, last.bounds) {
                Some(Bounds3([
                    pb.0[0].sub(pa.0[0])?.div(elapsed)?,
                    pb.0[1].sub(pa.0[1])?.div(elapsed)?,
            ...
            let nominal_velocity = if let (Some(pa), Some(pb)) = (first.nominal, last.nominal) {
                let v: [f64; 3] =
                    std::array::from_fn(|i| (pb.point[i] - pa.point[i]) / nominal_elapsed);
            ...
            modes.push(WorldMotionMode {
                previous_triangle: first.triangle,
                current_triangle: last.triangle,
                nominal_position: last.nominal.map(|hit| hit.point),
                nominal_velocity,
                position: last.bounds,
                velocity,
                nominal_occlusion_conflict: first.nominal_occluded == Some(true)
                    || last.nominal_occluded == Some(true),
            });
        }
    }
    budget.charge(0)?;
    Ok(WorldMotion {
        ...
        twin_digest: previous.twin_digest(),
        observations: [a, b],
...
pub fn propagate_motion(
    motion: &WorldMotion,
    capture: [u64; 2],
    acceleration: [f64; 3],
    budget: &mut WorkBudget<'_>,
) -> Result<Vec<PropagatedPosition>, TwinError> {
    budget.charge(0)?;
    ...
    let historical_span = seconds(
        pair[1].capture[0] - pair[0].capture[1],
        pair[1].capture[1] - pair[0].capture[0],
    )?;
    let mut output = Vec::new();
    output
        .try_reserve_exact(motion.modes.len())
        .map_err(|_| TwinError::Limit)?;
    for (index, mode) in motion.modes.iter().enumerate() {
        ...
        let bounds = if let (Some(position), Some(velocity)) = (mode.position, mode.velocity) {
            ...
            for i in 0..3 {
                ...
                let growth = time
                    .square()?
                    .add(time.mul(historical_span)?)?
                    .mul(Interval::point(acceleration[i])?)?
                    .mul(half)?
                    .upper()
                    .max(0.0);
                next[i] = position.0[i]
                    .add(velocity.0[i].mul(time)?)?
                    .add(Interval::new(-growth, growth)?)?;
            }
            Some(Bounds3(next))
        } else {
            None
        };
        let nominal = if let (Some(position), Some(velocity)) =
            (mode.nominal_position, mode.nominal_velocity)
        {
        ...
        output.push(PropagatedPosition {
            mode: index,
            capture,
            nominal,
            bounds,
