...
impl From<TwinError> for RectificationError {
    fn from(error: TwinError) -> Self {
        match error {
            TwinError::Geometry(e) => Self::Geometry(e),
            _ => Self::Numeric,
        }
    }
}
...
impl<'a> RawGrayFrame<'a> {
    ...
    pub fn new(
    ...
    ) -> Result<Self, RectificationError> {
        budget.charge(0)?;
        let count = pixel_count(identity.dimensions)?;
        if [
            identity.exposure,
            identity.storage,
            identity.allowed_mask,
        ...
        let span = (identity.row_stride as usize)
            .checked_mul(identity.dimensions[1] as usize)
            .ok_or(RectificationError::Limit)?;
        if span > MAX_STORAGE {
            return Err(RectificationError::Limit);
        }
        if span != storage.len() || count != allowed.len() {
            return Err(RectificationError::InvalidInput);
        }
    ...
    pub fn identity(&self) -> RawFrameIdentity {
        self.identity
    }
}
...
impl std::fmt::Debug for RectificationPlan {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RectificationPlan")
...
impl RectificationPlan {
    ...
    pub fn compile(
        ...
        if spec.source_domain == [0; 32]
            || spec.calibration == [0; 32]
            || !spec.maximum_radius.is_finite()
            || !(1e-6..=64.0).contains(&spec.maximum_radius)
        {
        ...
        for row in 0..h {
            for column in 0..w {
                budget.charge(96)?;
                let source =
                    model::source_pixel(spec, [f64::from(column) + 0.5, f64::from(row) + 0.5])?;
                let sample = match source {
                    None => {
                        coverage.outside_lens_domain += 1;
                        [INVALID; 2]
                    }
                    Some(pixel) => {
                        let indices = [pixel[0] - 0.5, pixel[1] - 0.5];
                        if (0..2).any(|i| {
                            indices[i] < 0.0 || indices[i] > f64::from(source_dimensions[i] - 1)
                        }) {
                ...
                for coordinate in sample {
                    map.extend_from_slice(&coordinate.to_le_bytes());
                }
            }
        }
        ...
        for camera in [spec.source, spec.target] {
            for n in camera.dimensions() {
                basis.extend_from_slice(&n.to_le_bytes());
            }
            for value in camera
                .focal_lengths()
                .into_iter()
                .chain(camera.principal_point())
            {
                float(&mut basis, value);
            }
        }
        spec.distortion.encode(&mut basis);
        float(&mut basis, spec.maximum_radius);
        basis.push(match spec.range {
            LumaRange::Full => 0,
            LumaRange::Video => 1,
        });
    ...
    pub fn spec(&self) -> RectificationSpec {
        self.spec
    }
    ...
    pub fn output_domain(&self) -> [u8; 32] {
        self.output_domain
    }
    ...
    pub fn apply(
        ...
        if id.dimensions != self.spec.source.dimensions()
            || id.image_domain != self.spec.source_domain
            || id.calibration != self.spec.calibration
            || id.range != self.spec.range
        {
            return Err(RectificationError::BasisMismatch);
        }
        ...
        for (out, encoded) in self.map.as_chunks::<8>().0.iter().enumerate() {
            ...
            if taps
                .iter()
                .any(|&(x, y, weight)| weight != 0 && source.allowed[y * width + x] == 0)
            {
                privacy_rejected += 1;
                continue;
            }
            ...
            for (x, y, weight) in taps {
                if weight != 0 {
                    sum += weight
                        * id.range
                            .sample(source.storage[y * id.row_stride as usize + x]);
                }
            }
        ...
        let identity = ImageIdentity {
            ...
            image_domain: self.output_domain,
            dimensions: self.spec.target.dimensions(),
        };
        ...
        Ok(RectifiedFrame {
            ...
            receipt: RectificationReceipt {
                ...
                allowed_mask: mask_digest,
                coverage: self.coverage,
                allowed_pixels: visible,
                privacy_rejected,
            },
        })
    }
...
impl RectifiedFrame {
    ...
    pub fn receipt(&self) -> RectificationReceipt {
        self.receipt
    }
    /// Borrow the existing native extractor's checked pinhole input type.
    pub fn as_gray_image(
        &self,
        budget: &mut WorkBudget<'_>,
    ) -> Result<GrayImage<'_>, LocalizationError> {
...
fn pixel_count(dimensions: [u32; 2]) -> Result<usize, RectificationError> {
    ...
    if count > MAX_RECTIFICATION_PIXELS {
        return Err(RectificationError::Limit);
    }
    Ok(count)
}
