...
impl From<fss_geometry::GeometryError> for NavigationError {
    fn from(error: fss_geometry::GeometryError) -> Self { Self::Twin(error.into()) }
}
...
impl NavigationProfile {
    ...
    pub fn new(class: MovementClass, minimum_up_cosine: f64,
        minimum_portal_width: f64, person_path_multiplier: u32)
        -> Result<Self, NavigationError> {
        if !minimum_up_cosine.is_finite() || !(0.0..=1.0).contains(&minimum_up_cosine)
            || !minimum_portal_width.is_finite() || !(0.0..=1e12).contains(&minimum_portal_width)
        ...
        Ok(Self { class, minimum_up_cosine, minimum_portal_width, person_path_multiplier })
    }
    /// Explicit class conditioning this query.
    pub fn class(self) -> MovementClass { self.class }
    /// Requested minimum absolute normal/up cosine.
    pub fn minimum_up_cosine(self) -> f64 { self.minimum_up_cosine }
    /// Necessary shared-edge width floor, not a swept-body proof.
    pub fn minimum_portal_width(self) -> f64 { self.minimum_portal_width }
    ...
    pub fn without_preference(self) -> Self { Self { person_path_multiplier: 1, ..self } }
}
...
impl SupportLocation {
    ...
    pub fn new(triangle: u32, barycentric: [f64; 3]) -> Result<Self, NavigationError> {
        ...
        if barycentric.iter().any(|v| !v.is_finite() || *v < 0.0 || *v > 1.0)
            || (sum - 1.0).abs() > 1e-10 {
            return Err(NavigationError::Start);
        }
        Ok(Self { triangle, barycentric: barycentric.map(|v| v / sum) })
    }
    /// Triangle ordinal, valid only with the network's exact twin digest.
    pub fn triangle(self) -> u32 { self.triangle }
    /// Closed-simplex weights in the source triangle's vertex order.
    pub fn barycentric(self) -> [f64; 3] { self.barycentric }
}

#[derive(Clone, Copy, Debug)]
struct Portal { neighbor: usize, midpoint: Point, width: f64 }
#[derive(Clone, Copy, Debug)]
struct Node {
    vertices: [Point; 3],
    center: Point,
...
impl std::fmt::Debug for SupportNetwork {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SupportNetwork").field("support_count", &self.support_count)
            .field("portal_count", &self.portal_count).finish_non_exhaustive()
    }
}
impl SupportNetwork {
    ...
    pub fn compile(twin: &PropertyTwin, max_triangles: usize, budget: &mut WorkBudget<'_>)
        -> Result<Self, NavigationError> {
        budget.charge(0)?;
        if max_triangles == 0 || max_triangles > MAX_NAVIGATION_TRIANGLES
            || twin.triangles().len() > max_triangles { return Err(NavigationError::Limit); }
        let mut nodes = Vec::new();
        nodes.try_reserve_exact(twin.triangles().len()).map_err(|_| NavigationError::Limit)?;
        ...
        for (index, triangle) in twin.triangles().iter().enumerate() {
            budget.charge(128)?;
            if !triangle.support { nodes.push(None); continue; }
            let vertices = triangle.vertices.map(|i| twin.vertices()[i as usize]);
            let mut face = vertices.map(point_key);
            face.sort();
            if !faces.insert(face) { return Err(NavigationError::DuplicateFace); }
            ...
            let surface = twin.features().get(feature as usize).ok_or(TwinError::Reference)?.surface;
            nodes.push(Some(Node { vertices, center, feature, surface,
                up_cosine: (normal[2].abs() / normal_length).min(1.0), portals: [None; 3], degree: 0 }));
            support_count += 1;
            for edge in [[0, 1], [1, 2], [2, 0]] {
                ...
                if let Some((prior, paired)) = edges.get_mut(&key) {
                    if *paired { return Err(NavigationError::NonManifold); }
                    let midpoint = std::array::from_fn(|axis| p[axis] / 2.0 + q[axis] / 2.0);
                    let width = length(subtract(p, q));
                    add_portal(&mut nodes, *prior, Portal { neighbor: index, midpoint, width })?;
                    add_portal(&mut nodes, index, Portal { neighbor: *prior, midpoint, width })?;
                    *paired = true;
                    portal_count += 1;
                } else { edges.insert(key, (index, false)); }
            }
        }
        budget.charge(0)?;
        Ok(Self { basis: twin.basis(), twin_digest: twin.digest(), nodes,
            feature_count: twin.features().len(), support_count, portal_count })
    }
    ...
    pub fn support_count(&self) -> usize { self.support_count }
    /// Undirected exact shared-edge connections.
    pub fn portal_count(&self) -> usize { self.portal_count }
    ...
    pub fn point(&self, location: SupportLocation) -> Result<Point, NavigationError> {
        let node = self.node(location.triangle as usize).ok_or(NavigationError::Start)?;
        Ok(std::array::from_fn(|axis| (0..3)
            .map(|i| node.vertices[i][axis] * location.barycentric[i]).sum()))
    }
    ...
    pub fn route_to_feature(&self, query: RouteQuery, budget: &mut WorkBudget<'_>)
        -> Result<RouteSearch, NavigationError> {
        budget.charge(0)?;
        let RouteQuery { basis, twin_digest, start, destination_feature, profile, max_points } = query;
        if self.basis != basis || self.twin_digest != twin_digest {
            return Err(NavigationError::Twin(TwinError::Basis));
        }
        if max_points == 0 || max_points > 256 { return Err(NavigationError::Limit); }
        if destination_feature as usize >= self.feature_count { return Err(TwinError::Reference.into()); }
        ...
        if first.up_cosine < profile.minimum_up_cosine {
            return Ok(RouteSearch { outcome: RouteOutcome::StartExcludedByProfile, visited: 0 });
        }
        let mut targets = 0;
        for node in self.nodes.iter().flatten() {
            budget.charge(1)?;
            if node.feature == destination_feature && node.up_cosine >= profile.minimum_up_cosine { targets += 1; }
        }
        if targets == 0 { return Ok(RouteSearch { outcome: RouteOutcome::NoAdmissibleDestination, visited: 0 }); }
        if first.feature == destination_feature {
            return Ok(RouteSearch { outcome: RouteOutcome::Found(SurfaceRoute {
                basis: self.basis, twin_digest: self.twin_digest,
                points: vec![position], triangles: vec![start.triangle], destination_feature,
                weighted_cost: 0.0, distance: 0.0, profile, all_pedestrian: first.surface == SurfaceKind::PedestrianPath,
            }), visited: 1 });
        }
        ...
        heap.try_reserve(n.saturating_mul(3).saturating_add(1)).map_err(|_| NavigationError::Limit)?;
        costs[source] = length(subtract(position, first.center)) / profile.divisor(first.surface);
        heap.push(QueueEntry { cost: costs[source], node: source });
        let mut visited = 0;
        while let Some(entry) = heap.pop() {
            budget.charge(64)?;
            if settled[entry.node] || entry.cost != costs[entry.node] { continue; }
            settled[entry.node] = true;
            visited += 1;
            let node = self.node(entry.node).ok_or(TwinError::Reference)?;
            if node.feature == destination_feature {
                let route = self.reconstruct(PathEndpoint { source, target: entry.node, position,
                    weighted_cost: entry.cost }, &previous, profile, max_points, budget)?;
                return Ok(RouteSearch { outcome: RouteOutcome::Found(route), visited });
            }
            for portal in node.portals.iter().flatten() {
                budget.charge(32)?;
                let next = self.node(portal.neighbor).ok_or(TwinError::Reference)?;
                if settled[portal.neighbor] || portal.width < profile.minimum_portal_width
                    || next.up_cosine < profile.minimum_up_cosine { continue; }
                let candidate = entry.cost
                    + length(subtract(node.center, portal.midpoint)) / profile.divisor(node.surface)
                    + length(subtract(next.center, portal.midpoint)) / profile.divisor(next.surface);
                if !candidate.is_finite() { return Err(TwinError::Numeric.into()); }
                if candidate < costs[portal.neighbor] {
                    ...
                    heap.push(QueueEntry { cost: candidate, node: portal.neighbor });
                }
            }
        }
        budget.charge(0)?;
        Ok(RouteSearch { outcome: RouteOutcome::NoModeledConnection, visited })
    }
    fn node(&self, index: usize) -> Option<&Node> { self.nodes.get(index)?.as_ref() }
    fn reconstruct(&self, endpoint: PathEndpoint, previous: &[Option<(usize, Point)>],
        profile: NavigationProfile, max_points: usize, budget: &mut WorkBudget<'_>)
        -> Result<SurfaceRoute, NavigationError> {
        let PathEndpoint { source, target, position, weighted_cost } = endpoint;
        let mut corridor = Vec::new();
        corridor.try_reserve(max_points).map_err(|_| NavigationError::Limit)?;
        let mut cursor = target;
        loop {
            budget.charge(1)?;
            if corridor.len() == max_points { return Err(NavigationError::Limit); }
        ...
        points.try_reserve_exact(max_points).map_err(|_| NavigationError::Limit)?;
        push_point(&mut points, position, max_points)?;
        push_point(&mut points, self.node(source).ok_or(TwinError::Reference)?.center, max_points)?;
        ...
        for (index, triangle) in corridor.iter().enumerate() {
            ...
            if index != 0 {
                push_point(&mut points, previous[*triangle as usize].ok_or(TwinError::Reference)?.1, max_points)?;
                push_point(&mut points, node.center, max_points)?;
            }
        }
        let distance = points.windows(2).map(|p| length(subtract(p[1], p[0]))).sum::<f64>();
        if !distance.is_finite() { return Err(TwinError::Numeric.into()); }
        budget.charge(0)?;
        Ok(SurfaceRoute { basis: self.basis, twin_digest: self.twin_digest, points, triangles: corridor,
            destination_feature: self.node(target).ok_or(TwinError::Reference)?.feature,
            weighted_cost, distance, profile, all_pedestrian })
    }
}
...
impl SurfaceRoute {
    ...
    pub fn destination_feature(&self) -> u32 { self.destination_feature }
    /// Graph objective value; never divide this by speed to estimate elapsed time.
    pub fn weighted_cost(&self) -> f64 { self.weighted_cost }
    ...
    pub fn profile(&self) -> NavigationProfile { self.profile }
    /// Whether every traversed support face explicitly has pedestrian-path semantics.
    pub fn all_pedestrian(&self) -> bool { self.all_pedestrian }
}

struct PathEndpoint { source: usize, target: usize, position: Point, weighted_cost: f64 }

#[derive(Clone, Copy, Debug)]
struct QueueEntry { cost: f64, node: usize }
...
fn filled<T: Clone>(n: usize, value: T) -> Result<Vec<T>, NavigationError> {
    ...
    output.try_reserve_exact(n).map_err(|_| NavigationError::Limit)?;
    output.resize(n, value);
    Ok(output)
}
fn add_portal(nodes: &mut [Option<Node>], index: usize, portal: Portal) -> Result<(), NavigationError> {
    let node = nodes.get_mut(index).and_then(Option::as_mut).ok_or(TwinError::Reference)?;
    if node.degree == 3 { return Err(NavigationError::NonManifold); }
    node.portals[node.degree] = Some(portal);
    node.degree += 1;
    Ok(())
}
fn push_point(points: &mut Vec<Point>, point: Point, maximum: usize) -> Result<(), NavigationError> {
    if points.last() == Some(&point) { return Ok(()); }
    if points.len() == maximum { return Err(NavigationError::Limit); }
    points.push(point);
    Ok(())
}
fn point_key(point: Point) -> PointKey { point.map(|v| if v == 0.0 { 0 } else { v.to_bits() }) }
fn subtract(a: Point, b: Point) -> Point { std::array::from_fn(|i| a[i] - b[i]) }
fn length(a: Point) -> f64 { a[0].hypot(a[1]).hypot(a[2]) }
