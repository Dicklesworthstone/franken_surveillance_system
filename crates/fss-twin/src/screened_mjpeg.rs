...
use crate::foreground::{ForegroundError, ForegroundFrame, ForegroundPolicy, ForegroundReport,
    ForegroundSource};
use crate::foreground::pipeline::FrameCapture;
use crate::rectification::{RectificationPlan, RectifiedFrame};
use crate::screening::{ScreeningError, ScreeningHealth, ScreeningMonitor, ScreeningReport,
    ScreeningStamp};
use crate::mjpeg::{JpegBackground, JpegFrameBinding, JpegPipelineError, JpegReceipt, JpegRectified,
    decode_rectified};
...
impl From<JpegPipelineError> for JpegScreeningError {
    fn from(error: JpegPipelineError) -> Self { Self::Image(error) }
}
impl From<ScreeningError> for JpegScreeningError {
    fn from(error: ScreeningError) -> Self { Self::Screening(error) }
}
impl std::fmt::Display for JpegScreeningError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self { Self::Image(_) => "JPEG screening image refused",
            Self::Screening(_) => "JPEG screening state refused", Self::Cancelled => "JPEG screening cancelled" })
    }
}
...
impl std::fmt::Debug for ScreenedJpeg {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ScreenedJpeg").field("health", &self.screening.health())
            .field("analysis_due", &self.screening.analysis_due()).finish_non_exhaustive()
    }
}
impl ScreenedJpeg {
    ...
    pub fn source_receipt(&self) -> JpegReceipt { self.image.receipt() }
    /// Masked, corrected source frame. Pixels behind denied samples remain zero and denied.
    pub fn image(&self) -> &JpegRectified { &self.image }
    ...
    pub fn attempted_background(&self) -> Option<[u8; 32]> { self.attempted_background }
    /// Actual candidate-stage thresholds, retained even when validation refused them.
    pub fn foreground_policy(&self) -> ForegroundPolicy { self.foreground_policy }
    /// Deterministic candidate work allowance at admission; not elapsed time or energy.
    pub fn foreground_allowance(&self) -> u64 { self.foreground_allowance }
    /// Candidate work charged, including a refused attempt.
    pub fn foreground_units(&self) -> u64 { self.foreground_units }
    /// Full always-on health and downstream-sampling decision.
    pub fn screening(&self) -> &ScreeningReport { &self.screening }
...
pub fn screen_jpeg(monitor: &mut ScreeningMonitor, background: Option<&JpegBackground>,
    plan: &RectificationPlan, query: JpegScreeningQuery<'_>, decode: &mut DecodeBudget<'_>,
    rectification: &mut WorkBudget<'_>, foreground_work: &mut WorkBudget<'_>,
    health_work: &mut WorkBudget<'_>) -> Result<ScreenedJpeg, JpegScreeningError> {
    health_work.charge(0).map_err(ScreeningError::from)?;
    let image = decode_rectified(plan, query.bytes, query.mask, query.binding,
        query.decode_limits, decode, rectification)?;
    let frame = image.frame();
    let source = ForegroundSource { image: frame.identity(), camera: query.capture.camera,
        clock: query.capture.clock, capture: query.capture.capture, calibration: query.binding.calibration };
    ...
    let result = background.map(|model| {
        ForegroundFrame::new(source, frame.pixels(), frame.allowed(), foreground_work)
            .and_then(|input| model.background().model().detect(&input, query.foreground_policy, foreground_work))
    });
    let (foreground, refusal) = match result {
        Some(Ok(report)) => (Some(report), None),
        Some(Err(ForegroundError::Geometry(GeometryError::Cancelled))) => return Err(JpegScreeningError::Cancelled),
    ...
    let mut e = CanonicalEncoder::new(); e.text("fss.screened_jpeg.reference.v1");
    for id in [receipt.source.encoded_sha256, receipt.source.exposure, receipt.source.allowed_mask,
        receipt.source.camera_image_domain, receipt.source.calibration, receipt.decode.decoder,
        receipt.decode.luma_sha256, frame.receipt().map_digest] { e.digest(ContentDigest::sha256(&id)); }
    e.bool(attempted_background.is_some());
    if let Some(id) = attempted_background { e.digest(ContentDigest::sha256(&id)); }
    e.u8(query.foreground_policy.minimum_change);
    for n in [query.foreground_policy.minimum_area as u64, query.foreground_policy.maximum_regions as u64,
        u64::from(query.foreground_policy.widespread_per_mille), foreground_allowance, foreground_units] { e.u64(n); }
    e.u8(match (&foreground, refusal) { (Some(_), _) => 1, (_, Some(_)) => 2, _ => 0 });
    if let Some(error) = refusal {
        e.text(&error.to_string());
        if let ForegroundError::Geometry(geometry) = error { e.text(&geometry.to_string()); }
    }
    let screening = monitor.observe(source, frame.pixels(), frame.allowed(), foreground.as_ref(),
        query.stamp, health_work)?;
    e.digest(ContentDigest::sha256(&screening.digest()));
    let digest = ContentDigest::sha256(&e.finish()).bytes();
    Ok(ScreenedJpeg { image, foreground, refusal, screening, attempted_background,
        foreground_policy: query.foreground_policy, foreground_allowance, foreground_units, digest })
}

/// Source-offset-preserving version of the same pipeline for a completed MJPEG stream frame.
pub struct FramedScreenedJpeg { source: StreamFrameReceipt, result: ScreenedJpeg, digest: [u8; 32] }
impl FramedScreenedJpeg {
    ...
    pub fn result(&self) -> &ScreenedJpeg { &self.result }
...
#[allow(clippy::too_many_arguments)]
pub fn screen_framed_jpeg(monitor: &mut ScreeningMonitor, background: Option<&JpegBackground>,
    plan: &RectificationPlan, query: FramedQuery<'_>, stamp: ScreeningStamp,
    decode: &mut DecodeBudget<'_>, rectification: &mut WorkBudget<'_>,
    foreground_work: &mut WorkBudget<'_>, health_work: &mut WorkBudget<'_>)
    ...
    let source = StreamFrameReceipt { basis: query.frame.basis(), ordinal: query.frame.ordinal(),
        byte_range: query.frame.byte_range(), encoded_sha256: query.frame.encoded_sha256() };
    let result = screen_jpeg(monitor, background, plan, JpegScreeningQuery {
        bytes: query.frame.bytes(), mask: query.mask, binding: query.binding, capture: query.capture,
        foreground_policy: query.policy, decode_limits: query.limits, stamp },
        decode, rectification, foreground_work, health_work)?;
    let mut e = CanonicalEncoder::new(); e.text("fss.framed_screened_jpeg.reference.v1");
    e.digest(ContentDigest::sha256(&source.basis.source)); e.u64(source.basis.generation);
    e.u64(source.ordinal); e.u64(source.byte_range[0]); e.u64(source.byte_range[1]);
    e.digest(ContentDigest::sha256(&result.digest()));
    let digest = ContentDigest::sha256(&e.finish()).bytes();
    Ok(FramedScreenedJpeg { source, result, digest })
}
