...
use fss_geometry::{
    FocalSampleOutcome, FocalValidationSet, PinholeIntrinsics, PlanarSupport, PoseValidation,
    RigidPose,
};
...
impl ValidatedCameraRegistration {
    ...
    pub fn frozen_calibration(
        &self,
        id: [u8; 32],
    ) -> Result<FrozenCalibration, RegistrationCandidateError> {
        if id == [0; 32] {
        ...
        Ok(FrozenCalibration {
            ...
            camera: LocalizationCamera {
                ...
                image_domain: self.query.image_domain,
            },
            pose: self.pose,
        })
    }
    ...
    pub fn bind_tracking_camera(
        ...
        if twin.digest() != self.twin_digest
            || binding.image_domain_digest != self.query.image_domain
            || binding.camera == 0
...
pub fn select_unique_focal_registration(
    twin: &PropertyTwin,
    atlas: &LocalizationAtlas,
    localization: &FocalLocalization,
    validation: &FocalValidationSet<'_>,
    ...
    let sample = scan
        .samples()
        .get(si)
        .ok_or(RegistrationCandidateError::InvalidIndex)?;
    ...
    Ok(ValidatedCameraRegistration {
        ...
        query: localization.matches.query,
        sample: si,
        candidate: ci,
        intrinsics: sample.intrinsics(),
        pose: candidate.pose(),
        planar_support: search.planar_support(),
