...
pub fn admit_tracking_camera(camera: TrackingCamera, report: &CalibrationMonitorReport,
    ...
    if [basis.twin_digest,basis.atlas_digest,basis.calibration_digest,basis.image_domain_digest].contains(&[0;32])
        ...
        || basis.checked_capture[0] > basis.checked_capture[1] { return Err(CalibrationGateError::InvalidInput); }
    if report.twin_digest != basis.twin_digest || report.atlas_digest != basis.atlas_digest
        || report.calibration != basis.calibration_digest || report.matches.atlas != basis.atlas_digest
...
fn contains(outer:[u64;2],inner:[u64;2])->bool{inner[0]<=inner[1]&&outer[0]<=inner[0]&&inner[1]<=outer[1]}
