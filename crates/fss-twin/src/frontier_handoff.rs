...
use fss_geometry::{
    BodySamples, CameraHandoffForecast, ForecastBasis, HandoffOptions, MotionForecast, RouteBody,
    RouteCandidate, RouteEnd, RoutePriors, RouteSurface, WorkBudget, forecast_routes,
...
impl HeadingMassPolicy {
    fn validate(self) -> bool {
        [self.opposite, self.aligned, self.unknown, self.stopped]
            .iter()
            .all(|x| (1..=1_000_000).contains(x))
            && self.opposite <= self.aligned
    }
    fn moving(self, cosine: Option<f64>) -> u32 {
        let Some(c) = cosine else {
            return self.unknown;
        };
...
impl From<fss_geometry::GeometryError> for FrontierHandoffError {
    fn from(v: fss_geometry::GeometryError) -> Self {
        Self::Twin(v.into())
    }
}
impl From<CalibrationGateError> for FrontierHandoffError {
    fn from(v: CalibrationGateError) -> Self {
        Self::Calibration(v)
    }
}
impl std::fmt::Display for FrontierHandoffError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::Basis => "frontier-handoff basis mismatch",
            Self::Options => "invalid frontier-handoff options",
            Self::Limit => "frontier-handoff complete-output limit exceeded",
            Self::Empty => "frontier has no usable nominal motion paths",
            Self::Frontier(_) => "route frontier invalid",
            Self::Calibration(_) => "handoff camera calibration unavailable",
...
pub fn forecast_frontier_handoffs(
    twin: &PropertyTwin,
    snapshot: TrackSnapshot<'_>,
    frontier: &RouteFrontier,
    cameras: &[MonitoredHandoffCamera<'_, '_>],
    body: &BodySamples,
    options: FrontierHandoffOptions,
    budget: &mut WorkBudget<'_>,
) -> Result<FrontierHandoffForecast, FrontierHandoffError> {
    budget.charge(0)?;
    frontier
        .check_source_current(snapshot)
        .map_err(FrontierHandoffError::Frontier)?;
    let motion = snapshot.motion().ok_or(FrontierHandoffError::Empty)?;
    if frontier.twin_digest != twin.digest()
        || motion.twin_digest() != twin.digest()
        || motion.geometry() != twin.basis()
    {
        return Err(FrontierHandoffError::Basis);
    }
    if options.horizon_ns == 0
        || options.horizon_ns > 3_600_000_000_000
        || options.motion_generation == 0
        || options.initial_pause_ns > options.horizon_ns
        || !options.masses.validate()
        || cameras.is_empty()
        || cameras.len() > 64
    {
        return Err(FrontierHandoffError::Options);
    }
    ...
    if reference.checked_add(options.horizon_ns).is_none() {
        return Err(FrontierHandoffError::Options);
    }
    let mut views = Vec::new();
    views
        .try_reserve_exact(cameras.len())
        .map_err(|_| FrontierHandoffError::Limit)?;
    for candidate in cameras {
        budget.charge(8)?;
        candidate
            .monitor
            .check_handoff_camera(candidate.view, observation.capture)?;
    ...
    bindings
        ...
        .map_err(|_| FrontierHandoffError::Limit)?;
    unmodeled
        .try_reserve_exact(frontier.routes.len() + motion.modes().len())
        .map_err(|_| FrontierHandoffError::Limit)?;
    for route in &frontier.routes {
        let RouteOutcome::Found(path) = &route.search.outcome else {
            unmodeled.push(FrontierHandoffUnmodeled {
                frontier_route: Some(route.route),
                source_mode: None,
                reason: FrontierHandoffUnmodeledReason::NavigationDidNotProduceRoute,
            });
            continue;
        };
        if route.motion.is_empty() {
            unmodeled.push(FrontierHandoffUnmodeled {
                frontier_route: Some(route.route),
                source_mode: None,
                reason: FrontierHandoffUnmodeledReason::NoObservedMotionMode,
            });
            continue;
        }
        for compatibility in &route.motion {
            let Some(speed) = compatibility
                .speed
                .filter(|s| s.is_finite() && (1e-9..=1e12).contains(s))
            else {
                unmodeled.push(FrontierHandoffUnmodeled {
                    frontier_route: Some(route.route),
                    source_mode: Some(compatibility.mode),
                    reason: FrontierHandoffUnmodeledReason::NoUsableSpeed,
                });
                continue;
            };
            if route_storage.len() == 64 {
                return Err(FrontierHandoffError::Limit);
            }
            let id = route_storage.len() as u64 + 1;
            let surface = if path.all_pedestrian() {
                RouteSurface::PedestrianPath
            } else {
                RouteSurface::OffPath
            };
            let base = options
                .masses
                .moving(compatibility.initial_direction_cosine);
            route_storage.push((
                id,
                path.points().to_vec(),
                speed,
                surface,
                base,
                route.kind == FrontierRouteKind::WithoutPathPreference,
                route.route,
                options.route_end,
            ));
            bindings.push(FrontierMotionBinding {
                motion_route: id,
                frontier_route: Some(route.route),
                source_mode: compatibility.mode,
                kind: FrontierMotionKind::Route,
                base_mass: base,
                heading_cosine: compatibility.initial_direction_cosine,
            });
        }
    }
    ...
    for (mode_index, mode) in motion.modes().iter().enumerate() {
        let Some(position) = mode.nominal_position else {
            unmodeled.push(FrontierHandoffUnmodeled {
                frontier_route: None,
                source_mode: Some(mode_index),
                reason: FrontierHandoffUnmodeledReason::NoNominalStopPosition,
            });
            continue;
        };
        if route_storage.len() == 64 {
        ...
        bindings.push(FrontierMotionBinding {
            ...
            kind: FrontierMotionKind::Stop,
            base_mass: options.masses.stopped,
            heading_cosine: None,
        });
    }
    if route_storage.is_empty() {
        return Err(FrontierHandoffError::Empty);
    }
    let mut candidates = Vec::new();
    let mut bodies = Vec::new();
    candidates
        .try_reserve_exact(route_storage.len())
        .map_err(|_| FrontierHandoffError::Limit)?;
    bodies
        .try_reserve_exact(route_storage.len())
        .map_err(|_| FrontierHandoffError::Limit)?;
    for (id, points, speed, surface, base, protected, _, end) in &route_storage {
        candidates.push(RouteCandidate {
            ...
            points,
            speed: *speed,
            initial_pause_ns: if points.len() == 1 {
                0
            } else {
                options.initial_pause_ns
            },
            end: *end,
            surface: *surface,
            base_mass: *base,
    ...
    let mut class = [0; 4];
    class[match frontier.profile.class() {
