...
use crate::foreground::{
    ForegroundError, ForegroundFrame, ForegroundReport, ForegroundSource, MAX_FOREGROUND_PIXELS,
};
...
impl ScreeningPolicy {
    ...
    pub fn validate(self) -> Result<(), ScreeningError> {
        if self.minimum_visible_pixels == 0
            || self.minimum_visible_pixels > MAX_FOREGROUND_PIXELS
            || self.dark_luma >= self.bright_luma
            || self.flat_range == 255
            || !(1..=1000).contains(&self.extreme_per_mille)
            || self.repeat_frames < 2
            || self.repeat_duration_ns == 0
            || self.stall_after_ns == 0
            || self.recovery_frames == 0
            || self.sentinel_interval_ns == 0
    ...
    pub fn digest(self) -> [u8; 32] {
        ...
        for value in [
            self.minimum_visible_pixels as u64,
            u64::from(self.dark_luma),
            u64::from(self.bright_luma),
            u64::from(self.extreme_per_mille),
            u64::from(self.flat_range),
            u64::from(self.repeat_frames),
            self.repeat_duration_ns,
            self.stall_after_ns,
            self.maximum_capture_uncertainty_ns,
            u64::from(self.recovery_frames),
            self.minimum_analysis_interval_ns,
            self.sentinel_interval_ns,
            self.activity_hold_ns,
        ] {
            e.u64(value);
        }
...
impl HealthFlags {
    ...
    pub fn contains(self, flag: HealthFlag) -> bool {
    ...
    fn insert(&mut self, flag: HealthFlag) {
        self.0 |= 1 << flag as u8;
    }
}
...
impl AnalysisReasons {
    ...
    pub fn contains(self, reason: AnalysisReason) -> bool {
    ...
    fn insert(&mut self, reason: AnalysisReason) {
...
impl From<ForegroundError> for ScreeningError {
    fn from(e: ForegroundError) -> Self {
        Self::Frame(e)
    }
}
impl From<GeometryError> for ScreeningError {
...
impl std::fmt::Display for ScreeningError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::InvalidPolicy => "invalid screening policy",
            Self::BasisMismatch => "screening basis mismatch",
            Self::OutOfOrder => "screening clock or sequence did not advance",
            Self::StaleCompletion => "screening analysis completion is stale",
            Self::Frame(_) => "invalid screening frame",
            Self::Work(_) => "screening work interrupted",
...
impl ScreeningReport {
    ...
    pub fn stamp(&self) -> ScreeningStamp {
        self.stamp
    }
    ...
    pub fn health(&self) -> ScreeningHealth {
        self.health
    }
    /// Reasons for downstream semantic-analysis admission.
    pub fn reasons(&self) -> AnalysisReasons {
        self.reasons
    }
    ...
    pub fn last_completed_analysis(&self) -> Option<(u64, [u8; 32], [u8; 32])> {
        self.last_completed
    }
    fn computed_digest(&self) -> [u8; 32] {
        ...
        for id in [
            self.source.image.exposure,
            self.source.image.pixels,
            self.source.image.image_domain,
        ...
        for n in [
            self.source.camera,
            self.source.clock,
            self.source.capture[0],
            self.source.capture[1],
            u64::from(self.source.image.dimensions[0]),
            u64::from(self.source.image.dimensions[1]),
            self.stamp.stream_generation,
            self.stamp.sequence,
            self.stamp.received_at_ns,
            self.visible as u64,
            self.dark as u64,
            self.saturated as u64,
            u64::from(self.identical_run),
            self.repeat_since_ns,
            u64::from(self.clean_run),
            u64::from(self.flags.0),
            self.health as u64,
            u64::from(self.reasons.0),
        ] {
            e.u64(n);
        }
        e.bool(self.last_activity_ns.is_some());
        if let Some(t) = self.last_activity_ns {
            e.u64(t);
        }
        e.bool(self.last_completed.is_some());
        if let Some((t, screen, result)) = self.last_completed {
            e.u64(t);
            e.digest(ContentDigest::sha256(&screen));
            e.digest(ContentDigest::sha256(&result));
        }
        e.bool(self.stamp.owner_requests_analysis);
        e.bool(self.luma_range.is_some());
        if let Some([lo, hi]) = self.luma_range {
            e.u8(lo);
            e.u8(hi);
        }
        ContentDigest::sha256(&e.finish()).bytes()
...
impl std::fmt::Debug for ScreeningMonitor {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ScreeningMonitor")
            .field("generation", &self.generation)
            .field("has_prior_frame", &self.last.is_some())
...
impl ScreeningMonitor {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            clock: started_at_ns,
            last: None,
            prior_visible: Vec::new(),
            repeat_since: started_at_ns,
            clean_run: 0,
            last_activity: None,
            last_completed: None,
            pending: None,
    ...
    pub fn last_report(&self) -> Option<&ScreeningReport> {
        self.last.as_ref()
    }
    ...
    pub fn watchdog_deadline_ns(&self) -> Option<u64> {
        self.last
            .map_or(self.started_at, |r| r.stamp.received_at_ns)
            .checked_add(self.policy.stall_after_ns)
    }
    ...
    pub fn poll(&mut self, now_ns: u64) -> Result<StallObservation, ScreeningError> {
        ...
        let since_ns = self
            .last
            .map_or(self.started_at, |r| r.stamp.received_at_ns);
        let result = StallObservation {
            stream_generation: self.generation,
            policy_digest: self.policy.digest(),
            last_report: self.last.map(|r| r.digest),
            since_ns,
            checked_at_ns: now_ns,
            stalled: now_ns - since_ns >= self.policy.stall_after_ns,
        };
        self.clock = now_ns;
        Ok(result)
    }
    ...
    pub fn observe(
        ...
        if stamp.received_at_ns < self.clock
            || self
                .last
                .is_some_and(|r| stamp.sequence <= r.stamp.sequence)
        {
            return Err(ScreeningError::OutOfOrder);
        }
        ...
        if let Some(prior) = self.last
            && (prior.source.camera != source.camera
                || prior.source.clock != source.clock
        ...
        visible_plane
            ...
            .map_err(|_| ScreeningError::Allocation)?;
        ...
        for (i, permission) in allowed.iter().enumerate() {
            ...
            if *permission == 0 {
                visible_plane.push(0);
                continue;
            }
            let v = pixels[i];
            visible_plane.push(v);
            visible += 1;
        ...
        if visible < self.policy.minimum_visible_pixels {
            flags.insert(HealthFlag::InsufficientVisible);
        }
        if visible > 0 {
            ...
            if hi - lo <= self.policy.flat_range {
                flags.insert(HealthFlag::LowTexture);
            }
        }
        ...
        if let Some(prior) = self.last {
            ...
            let receive_gap =
                stamp.received_at_ns - prior.stamp.received_at_ns >= self.policy.stall_after_ns;
            if sequence_gap {
                flags.insert(HealthFlag::SequenceGap);
            }
            if receive_gap {
                flags.insert(HealthFlag::ReceiveGap);
            }
            if source.image.exposure == prior.source.image.exposure {
                flags.insert(HealthFlag::ReusedExposure);
            }
            if source.capture[0] <= prior.source.capture[1] {
                flags.insert(HealthFlag::CaptureUncertain);
            }
            if mask != prior.mask {
                flags.insert(HealthFlag::MaskChanged);
            }
            if visible > 0
                && !sequence_gap
                && !receive_gap
        ...
        if visible >= self.policy.minimum_visible_pixels
            && identical_run >= self.policy.repeat_frames
        ...
        match foreground {
            ...
            Some(r) if r.comparable_pixels() < visible => {
                flags.insert(HealthFlag::BackgroundIncomplete)
            }
            Some(_) => {}
        }
        let clean_run = if flags.0 == 0 {
            self.clean_run.saturating_add(1)
        } else {
            0
        };
        let health = if flags.contains(HealthFlag::InsufficientVisible) {
            ScreeningHealth::NotObservable
        } else if flags.0 != 0 {
            ScreeningHealth::Degraded
        } else if clean_run < self.policy.recovery_frames {
            ScreeningHealth::Recovering
        } else {
            ScreeningHealth::NoFaultObserved
        };
        let changed = foreground.is_some_and(|r| r.changed_pixels() > 0);
        let activity = if changed {
            Some(stamp.received_at_ns)
        } else {
            self.last_activity
        };
        ...
        let elapsed = self
            .last_completed
            .map(|(t, _, _)| stamp.received_at_ns - t);
        if elapsed.is_none() {
            reasons.insert(AnalysisReason::Initial);
        }
        if elapsed.is_some_and(|n| n >= self.policy.sentinel_interval_ns) {
            reasons.insert(AnalysisReason::Sentinel);
        }
        if self
            .last
            .is_none_or(|r| r.flags != flags || r.health != health)
        {
            reasons.insert(AnalysisReason::HealthChanged);
        }
        if stamp.owner_requests_analysis {
            reasons.insert(AnalysisReason::OwnerRequested);
        }
        if elapsed.is_none_or(|n| n >= self.policy.minimum_analysis_interval_ns) {
            if changed {
                reasons.insert(AnalysisReason::Foreground);
            } else if activity
                .is_some_and(|t| stamp.received_at_ns - t <= self.policy.activity_hold_ns)
            {
        ...
        let mut report = ScreeningReport {
            ...
            foreground: foreground.map(ForegroundReport::digest),
            mask,
            previous: self.last.map(|r| r.digest),
            visible,
            dark,
            saturated,
            luma_range: (visible > 0).then_some([lo, hi]),
            identical_run,
            repeat_since_ns: repeat_since,
            clean_run,
            last_activity_ns: activity,
        ...
        self.clock = stamp.received_at_ns;
        self.repeat_since = repeat_since;
        self.clean_run = clean_run;
        self.last_activity = activity;
        self.prior_visible = visible_plane;
        self.pending = report
            .analysis_due()
            .then_some((stamp.received_at_ns, report.digest));
    ...
    pub fn acknowledge_analysis(
        &mut self,
        report_digest: [u8; 32],
        result_root: [u8; 32],
    ) -> Result<(), ScreeningError> {
        if result_root == [0; 32] {
            return Err(ScreeningError::StaleCompletion);
        }
        if self
            .last_completed
            .is_some_and(|(_, id, result)| id == report_digest && result == result_root)
        {
            return Ok(());
        }
        let pending = self
            .pending
            .filter(|(_, id)| *id == report_digest)
            .ok_or(ScreeningError::StaleCompletion)?;
        self.last_completed = Some((pending.0, pending.1, result_root));
        self.pending = None;
        Ok(())
    }
}
