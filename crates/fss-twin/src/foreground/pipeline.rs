...
use super::{
    BackgroundModel, BackgroundPolicy, ForegroundError, ForegroundFrame, ForegroundPolicy,
    ForegroundReport, ForegroundSource, MAX_BACKGROUND_FRAMES, MAX_FOREGROUND_PIXELS, integer,
    reserve,
};
use crate::association::UnassignedContact;
use crate::rectification::{
    RawGrayFrame, RectificationError, RectificationPlan, RectificationReceipt, RectificationSpec,
    RectifiedFrame,
};
...
impl std::fmt::Debug for RectifiedBackground {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RectifiedBackground")
            .field("references", &self.receipts.len())
...
impl RectifiedBackground {
    ...
    pub fn build(
        ...
        references: &[RectifiedReference<'_>],
        policy: BackgroundPolicy,
        budget: &mut WorkBudget<'_>,
    ) -> Result<Self, ForegroundPipelineError> {
        budget.charge(0)?;
        if !(3..=MAX_BACKGROUND_FRAMES).contains(&references.len()) {
            return Err(ForegroundError::Limit.into());
        }
        ...
        Ok(Self {
            ...
            spec: plan.spec(),
            map: plan.map_digest(),
            output_domain: plan.output_domain(),
            receipts,
        })
    }
    ...
    pub fn reference_receipts(&self) -> &[RectificationReceipt] {
        &self.receipts
    }
    ...
    pub fn detect_luma(
        ...
        budget.charge(0)?;
        if self.spec != plan.spec()
            || self.map != plan.map_digest()
            || self.output_domain != plan.output_domain()
        {
        ...
        let report = self
            .model
            .detect(&borrow(&frame, capture, budget)?, policy, budget)?;
        budget.charge(0)?;
...
) -> Result<(), ForegroundPipelineError> {
    let receipt = frame.receipt();
    if frame.identity().image_domain != plan.output_domain()
        || receipt.map_digest != plan.map_digest()
...
fn borrow<'a>(
    ...
    ForegroundFrame::new(
        ForegroundSource {
            ...
            calibration: frame.receipt().source.calibration,
        },
        frame.pixels(),
        frame.allowed(),
        budget,
...
impl RectifiedForeground {
    ...
    pub fn report(&self) -> &ForegroundReport {
        &self.report
    }
    ...
    pub fn crop(
        ...
        budget: &mut WorkBudget<'_>,
    ) -> Result<ForegroundCrop, ForegroundPipelineError> {
        budget.charge(0)?;
        ...
        let end = [
            component.max[0].saturating_add(padding).min(dims[0]),
        ...
        for row in 0..dimensions[1] {
            for column in 0..dimensions[0] {
                ...
                if self.frame.allowed()[from] != 0 {
                    allowed[to] = 1;
                    pixels[to] = self.frame.pixels()[from];
        ...
        bytes.extend_from_slice(&self.report.digest());
        integer(&mut bytes, u64::from(region));
        for n in origin.into_iter().chain(dimensions) {
            integer(&mut bytes, u64::from(n));
        }
        budget.charge(count as u64 * 3)?;
        for data in [&pixels, &allowed, &membership] {
            bytes.extend_from_slice(&ContentDigest::sha256(data).bytes());
        }
...
impl ForegroundCrop {
    ...
    pub fn prepare_contact(
        ...
        budget: &mut WorkBudget<'_>,
    ) -> Result<PreparedForegroundContact, ForegroundPipelineError> {
        budget.charge(0)?;
        if [
        ...
        .contains(&evidence)
            || (0..2).any(|i| {
                !min[i].is_finite()
                    ...
                    || min[i] < 0.0
                    || min[i] > max[i]
                    || max[i] >= f64::from(self.dimensions[i])
            })
        {
            return Err(ForegroundPipelineError::InvalidInput);
        }
        let lower = min.map(|x| x.floor() as u32);
        let upper = max.map(|x| x.floor() as u32);
        let mut intersects_component = false;
        for y in lower[1]..=upper[1] {
            for x in lower[0]..=upper[0] {
                budget.charge(1)?;
                let index = (y * self.dimensions[0] + x) as usize;
                if self.allowed[index] == 0 {
                    return Err(ForegroundPipelineError::InvalidInput);
                }
        ...
        let pixel_min = [
            min[0] + f64::from(self.origin[0]),
            min[1] + f64::from(self.origin[1]),
        ];
        let pixel_max = [
            max[0] + f64::from(self.origin[0]),
            max[1] + f64::from(self.origin[1]),
        ];
        let detection = UnassignedContact {
            id: u64::from(self.region),
            evidence,
            pixel_min,
            pixel_max,
            visible_contact,
