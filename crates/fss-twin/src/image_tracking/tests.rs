...
fn frame(sequence: u64) -> ImageTrackingFrame {
    ImageTrackingFrame {
        source: ForegroundSource {
            image: ImageIdentity {
                exposure: key(sequence),
                pixels: key(10_000 + sequence),
                image_domain: key(60_000),
                dimensions: [1000, 100],
            },
            camera: 1,
            clock: 1,
            calibration: key(50_000),
            capture: [sequence * SECOND, sequence * SECOND],
        },
        detector: key(30_000),
        permission_mask: key(40_000),
        evidence: key(20_000 + sequence),
...
fn detection(sequence: u64, id: u64, x: u32) -> ImageDetection {
    ImageDetection {
        id,
        evidence: key(100_000 + sequence * 1000 + id),
        min: [x, 10],
        max: [x + 4, 14],
        partial: false,
    }
...
fn brute(
    ...
    for column in 0..columns {
        ...
        if used[column] || cost >= FORBIDDEN || excluded == Some((row, column)) {
            continue;
        }
        used[column] = true;
...
fn assignment_and_edge_exclusion_match_exhaustive_oracle() -> Result<(), ImageTrackingError> {
    for encoding in 0..256 {
        let mut bits = encoding;
        let mut costs = [9_i64; 8];
        for slot in [0, 1, 4, 5] {
            costs[slot] = match bits % 4 {
                0 => FORBIDDEN,
                n => n - 1,
            };
            bits /= 4;
...
fn rectangular_assignment_matches_larger_generated_oracles() -> Result<(), ImageTrackingError> {
    ...
    for rows in 1..=4 {
        for _ in 0..40 {
            let columns = rows * 2;
            let mut costs = vec![100; rows * columns];
            for row in 0..rows {
                for column in 0..rows {
                    random = random.wrapping_mul(6364136223846793005).wrapping_add(1);
                    costs[row * columns + column] = if random.is_multiple_of(7) {
                        FORBIDDEN
                    } else {
                        (random % 99) as i64
                    };
            ...
            assert_eq!(
                assign(&costs, rows, columns, None, &mut budget())?.cost,
                exact
            );
        }
    }
...
fn constant_velocity_preserves_two_paths_through_a_crossing() -> Result<(), ImageTrackingError> {
    ...
    tracker.update(
        frame(1),
        &[detection(1, 1, 10), detection(1, 2, 50)],
        &mut budget(),
    )?;
    tracker.update(
        frame(2),
        &[detection(2, 1, 25), detection(2, 2, 35)],
        &mut budget(),
    )?;
    let report = tracker.update(
        frame(3),
        &[detection(3, 1, 20), detection(3, 2, 40)],
        &mut budget(),
    )?;
...
fn ambiguous_global_assignments_are_retained_without_merging() -> Result<(), ImageTrackingError> {
    ...
    tracker.update(
        frame(1),
        &[detection(1, 1, 10), detection(1, 2, 30)],
        &mut budget(),
    )?;
    let report = tracker.update(
        frame(2),
        &[detection(2, 1, 20), detection(2, 2, 20)],
        &mut budget(),
    )?;
    assert_eq!(
        report
            .candidates()
            .iter()
            .filter(|c| c.selected && c.ambiguous)
            .count(),
        2
    );
    assert!(
        report
            .decisions()
            .iter()
            .all(|d| d.disposition == ImageDetectionDisposition::Unresolved)
    );
    assert!(
        tracker
            .tracks()
            .iter()
            .all(|t| t.observations() == 1 && t.state() == ImageTrackState::Coasting)
    );
...
#[test]
fn empty_and_unobservable_frames_coast_without_inventing_observations()
-> Result<(), ImageTrackingError> {
    let mut tracker = tracker(policy())?;
    tracker.update(frame(1), &[detection(1, 1, 10)], &mut budget())?;
    let observed = tracker.tracks()[0].latest();
    tracker.update(frame(2), &[], &mut budget())?;
    let mut unavailable = frame(3);
    unavailable.availability = TrackingAvailability::Unobservable;
...
fn input_order_does_not_change_tracks_or_receipt() -> Result<(), ImageTrackingError> {
    let mut a = tracker(policy())?;
    let mut b = tracker(policy())?;
    for n in 1..=3 {
        let detections = [
            detection(n, 9, 100 + n as u32),
            detection(n, 2, 10 + n as u32),
        ];
...
fn privacy_and_generation_drift_fail_atomically() -> Result<(), ImageTrackingError> {
    ...
    let tracks = tracker.tracks().to_vec();
    let mut changed = [frame(2); 7];
    changed[0].source.camera += 1;
    changed[1].source.clock += 1;
    changed[2].source.calibration = key(987);
    changed[3].source.image.image_domain = key(988);
    changed[4].source.image.dimensions[0] += 1;
    changed[5].detector = key(989);
    changed[6].permission_mask = key(990);
    for input in changed {
        assert!(matches!(
            tracker.update(input, &[], &mut budget()),
            Err(ImageTrackingError::BasisMismatch)
        ));
        assert_eq!(tracker.digest(), prior);
        assert_eq!(tracker.tracks(), tracks.as_slice());
...
fn replays_and_overlapping_capture_intervals_are_rejected() -> Result<(), ImageTrackingError> {
    ...
    let mut replay = frame(2);
    replay.source.image.exposure = frame(1).source.image.exposure;
    assert!(matches!(
        tracker.update(replay, &[], &mut budget()),
        Err(ImageTrackingError::ReusedExposure)
    ));
    let mut overlap = frame(2);
    overlap.source.capture = [SECOND, 2 * SECOND];
    assert!(matches!(
        tracker.update(overlap, &[], &mut budget()),
        Err(ImageTrackingError::CaptureOrder)
    ));
    Ok(())
}
#[test]
fn limits_and_duplicate_records_do_not_drop_or_partially_update_tracks()
-> Result<(), ImageTrackingError> {
    let mut p = policy();
    p.maximum_tracks = 1;
    p.maximum_exposures = 2;
    let mut tracker = tracker(p)?;
    let prior = tracker.digest();
    assert!(matches!(
        tracker.update(
            frame(1),
            &[detection(1, 1, 10), detection(1, 2, 900)],
            &mut budget()
        ),
    ...
    assert!(matches!(
        tracker.update(frame(1), &[duplicate, duplicate], &mut budget()),
        Err(ImageTrackingError::InvalidInput)
    ));
    tracker.update(frame(1), &[duplicate], &mut budget())?;
    tracker.update(frame(2), &[], &mut budget())?;
    assert!(matches!(
        tracker.update(frame(3), &[], &mut budget()),
        Err(ImageTrackingError::Limit)
    ));
    Ok(())
}
...
{
    let mut measured = tracker(policy())?;
    let mut work = budget();
    ...
    let mut tracker = tracker(policy())?;
    assert!(matches!(
        tracker.update(frame(1), &[], &mut WorkBudget::cancellable(100_000, &flag)),
...
fn partial_boxes_do_not_supply_speed_exclusions() -> Result<(), ImageTrackingError> {
    ...
    let mut tracker = tracker(p)?;
    tracker.update(frame(1), &[detection(1, 1, 10)], &mut budget())?;
    let mut partial = detection(2, 1, 900);
...
fn uncertain_capture_times_do_not_invent_velocity() -> Result<(), ImageTrackingError> {
    ...
    let mut uncertain = frame(3);
    uncertain.source.capture[1] += SECOND / 2;
    let report = tracker.update(
        uncertain,
        &[detection(3, 1, 20), detection(3, 2, 40)],
        &mut budget(),
