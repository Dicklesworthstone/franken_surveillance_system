...
fn exact_image_matches_flow_into_unknown_focal_scan_without_supplied_correspondences() -> Test {
    ...
    let mut budget = WorkBudget::new(800_000_000);
    ...
    let FocalLocalizationOutcome::Scan(scan) = result.outcome else {
        return Err("focal scan not produced".into());
    };
    assert_eq!(scan.samples().len(), 33);
    let FocalSampleOutcome::Candidates(search) = scan.samples()[16].outcome() else {
...
fn reference_reuse_wrong_domain_and_insufficient_matches_do_not_invent_scan() -> Test {
    ...
    let sparse = FeatureFrame::new(
        query.identity(),
        query.descriptor_domain(),
        sparse_features,
        &mut budget,
    )?;
    let result = localize_focal_scan(
    ...
    assert!(matches!(
        result.outcome,
        FocalLocalizationOutcome::InsufficientMatches { found: 5, .. }
    ));
    Ok(())
}
