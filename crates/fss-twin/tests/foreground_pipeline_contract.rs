...
fn identity(e: u8, p: &[u8], mask: &[u8], s: RectificationSpec, stride: u32) -> RawFrameIdentity {
    RawFrameIdentity {
        exposure: [e; 32],
        storage: ContentDigest::sha256(p).bytes(),
...
fn baseline(
    ...
    masks: &[Vec<u8>; 3],
) -> Result<RectifiedBackground, Box<dyn Error>> {
    let p = vec![100; 30];
    let mut budget = WorkBudget::new(10_000_000);
    let mut frames = Vec::new();
    for (i, mask) in masks.iter().enumerate() {
        let raw = RawGrayFrame::new(
            identity(i as u8 + 1, &p, mask, plan.spec(), 6),
            &p,
            mask,
            &mut budget,
        )?;
    ...
    let refs: Vec<_> = frames
        ...
        .map(|(i, frame)| RectifiedReference {
            frame,
            capture: capture(i as u64 + 1),
        })
        .collect();
    Ok(RectifiedBackground::build(
        plan,
        &refs,
        BackgroundPolicy {
...
fn query(
    plan: &RectificationPlan,
    m: &RectifiedBackground,
    p: &[u8],
    mask: &[u8],
) -> Result<RectifiedForeground, Box<dyn Error>> {
    let mut budget = WorkBudget::new(10_000_000);
...
fn raw_pixels_rectify_detect_crop_and_prepare_explicit_contact() -> Test {
    ...
    assert_eq!(
        result.frame().receipt().source.storage,
        ContentDigest::sha256(&p).bytes()
    );
    assert_eq!(m.reference_receipts().len(), 3);
    let mut budget = WorkBudget::new(100000);
    let crop = result.crop(8, 0, 30, &mut budget)?;
    assert_eq!(crop.origin(), [1, 1]);
    assert_eq!(crop.dimensions(), [2, 2]);
    assert_eq!(crop.pixels(), [150, 150, 150, 100]);
    assert_eq!(crop.membership(), [1, 1, 1, 0]);
    let prepared = crop.prepare_contact([88; 32], [0.5, 1.5], [0.5, 1.5], true, &mut budget)?;
    assert_eq!(prepared.detection.pixel_min, [1.5, 2.5]);
    assert_eq!(prepared.detection.pixel_max, [1.5, 2.5]);
    assert_eq!(prepared.detection.id, 8);
    assert_eq!(prepared.source.capture, [40; 2]);
    assert_eq!(prepared.crop, crop.digest());
    assert_eq!(prepared.report, result.report().digest());
    assert!(prepared.detection.visible_contact);
    let unknown = crop.prepare_contact([89; 32], [0.5, 1.5], [0.5, 1.5], false, &mut budget)?;
    assert!(!unknown.detection.visible_contact);
    Ok(())
}
#[test]
fn padded_crop_never_exposes_masked_context_or_accepts_masked_contact() -> Test {
    ...
    p[14] = 211;
    let mut mask = vec![1; 30];
    mask[14] = 0;
    let result = query(&plan, &m, &p, &mask)?;
    let mut budget = WorkBudget::new(100000);
    let crop = result.crop(8, 1, 30, &mut budget)?;
    assert_eq!(crop.origin(), [0, 0]);
    assert_eq!(crop.dimensions(), [4, 4]);
    assert_eq!(crop.pixels()[10], 0);
    assert_eq!(crop.allowed()[10], 0);
    assert_eq!(crop.membership()[10], 0);
    assert!(result.report().regions()[0].touches_unknown);
    assert!(
        crop.prepare_contact([88; 32], [1.5, 2.5], [2.5, 2.5], true, &mut budget)
            .is_err()
    );
    assert!(
        crop.prepare_contact([89; 32], [0.5, 0.5], [0.5, 0.5], true, &mut budget)
            .is_err()
    );
    Ok(())
}
...
fn unknown_baseline_is_not_misused_as_the_current_privacy_mask() -> Test {
    ...
    assert_eq!(crop.pixels()[3], 211);
    assert_eq!(crop.allowed()[3], 1);
    assert_eq!(crop.membership()[3], 0);
    Ok(())
}
#[test]
fn changed_rectification_or_capture_basis_cannot_reuse_background() -> Test {
    ...
    changed.calibration = [5; 32];
    let other = RectificationPlan::compile(changed, &mut WorkBudget::new(100000))?;
    let p = [100; 30];
    let mask = [1; 30];
    let mut budget = WorkBudget::new(100000);
    let raw = RawGrayFrame::new(
        identity(4, &p, &mask, plan.spec(), 6),
        &p,
        &mask,
        &mut budget,
    )?;
    assert!(
        m.detect_luma(&other, &raw, capture(4), policy(), &mut budget)
            .is_err()
    );
    let wrong = FrameCapture {
        camera: 5,
        ..capture(4)
    };
    assert!(
        m.detect_luma(&plan, &raw, wrong, policy(), &mut budget)
            .is_err()
    );
    Ok(())
}
...
fn video_range_and_row_padding_are_normalized_before_comparison() -> Test {
    ...
    let mut budget = WorkBudget::new(100000);
    let raw = RawGrayFrame::new(
        identity(4, &pixels, &mask, s, 8),
        &pixels,
        &mask,
        &mut budget,
    )?;
    let result = m.detect_luma(&plan, &raw, capture(4), policy(), &mut budget)?;
    assert_eq!(result.report().changed_pixels(), 2);
    assert_eq!(result.frame().pixels()[7], 214);
    assert_eq!(result.frame().receipt().source.row_stride, 8);
    Ok(())
}
#[test]
fn crop_bounds_cancellation_and_forged_contact_identity_fail_atomically() -> Test {
    ...
    let result = query(&plan, &m, &p, &[1; 30])?;
    let mut budget = WorkBudget::new(100000);
    assert!(result.crop(999, 0, 30, &mut budget).is_err());
    assert!(result.crop(8, 1, 2, &mut budget).is_err());
    let flag = AtomicBool::new(true);
    assert!(
        result
            .crop(8, 0, 30, &mut WorkBudget::cancellable(1000, &flag))
            .is_err()
    );
    let crop = result.crop(8, 0, 30, &mut budget)?;
    assert!(
        crop.prepare_contact([0; 32], [0.5, 0.5], [0.5, 0.5], true, &mut budget)
            .is_err()
    );
    assert!(
        crop.prepare_contact(crop.digest(), [0.5, 0.5], [0.5, 0.5], true, &mut budget)
            .is_err()
    );
    assert!(
        crop.prepare_contact([88; 32], [f64::NAN, 0.5], [0.5, 0.5], true, &mut budget)
            .is_err()
    );
    assert!(
        crop.prepare_contact([88; 32], [0.5, 0.5], [2.0, 0.5], true, &mut budget)
            .is_err()
    );
    Ok(())
}
