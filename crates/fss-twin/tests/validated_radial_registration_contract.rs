...
fn world(id:u64)->[f64;3]{let n=id as f64;[2.*(n*0.7).sin(),1.5*(n*1.3).cos(),1.2*(n*0.31).sin()]}
fn raw_pixel(w:[f64;3])->[f64;2]{let x=0.8*w[0]+0.6*w[2]+0.3;let y=w[1]-0.4;let z=-0.6*w[0]+0.8*w[2]+8.;let xu=x/z;let yu=y/z;let q=xu*xu+yu*yu;let d=1.+K1*q;[FX*xu*d+960.,820.*yu*d+540.]}
...
fn options()->RadialFocalScanOptions{RadialFocalScanOptions{minimum_fx_px:400.,maximum_fx_px:1600.,focal_samples:17,y_over_x:820./800.,principal_point:[960.,540.],minimum_k1:-0.2,maximum_k1:0.2,k1_samples:17,maximum_undistorted_radius:2.,pose:PoseSolverOptions{ransac_trials:0,..PoseSolverOptions::default()}}}
...
fn unique_lens_pose_compiles_rectification_and_tracking_generation()->Test{
    ...
    let tracking=candidate.bind_tracking_camera(&twin,&plan,TrackingCameraBinding{camera:7,calibration:9,image_domain:11,image_domain_digest:plan.output_domain(),clock:13,validity:[100,200],error:Some(fss_twin::ProjectionError{centre:[0.1;3],rotation_entry:0.001,focal:[0.1;2],principal:[0.1;2]})})?;
...
fn wrong_derived_domain_cannot_bind_tracking_snapshot()->Test{
    let twin=common::twin(&[0.0],None)?;let mut budget=WorkBudget::new(6_000_000_000);let atlas=atlas(&twin,&mut budget)?;let query=frame(2,true,&mut budget)?;
    let localization=localize_radial_focal_scan(&atlas,&twin,&query,MatchOptions{maximum_distance:0,ratio_percent:80},options(),&mut budget)?;
    let RadialLocalizationOutcome::Scan(scan)=&localization.outcome else{return Err("no scan".into());};let validation=scan.validate_all_candidates(&holdout(),0.01,&mut budget)?;
