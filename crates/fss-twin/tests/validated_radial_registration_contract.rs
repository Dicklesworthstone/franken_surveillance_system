...
use fss_twin::rectification::LumaRange;
...
fn raw_pixel(w: [f64; 3]) -> [f64; 2] {
    let x = 0.8 * w[0] + 0.6 * w[2] + 0.3;
    let y = w[1] - 0.4;
    let z = -0.6 * w[0] + 0.8 * w[2] + 8.;
    let xu = x / z;
    let yu = y / z;
    let q = xu * xu + yu * yu;
    let d = 1. + K1 * q;
    [FX * xu * d + 960., 820. * yu * d + 540.]
}
fn frame(
    exposure: u8,
    query: bool,
    ...
    let features = (1..=32)
        .map(|id| ImageFeature {
            ...
            pixel: if query {
                ...
                [60. + id as f64 * 20., 80. + (id % 10) as f64 * 40.]
            },
...
fn unique_lens_pose_compiles_rectification_and_tracking_generation() -> Test {
    ...
    let mut budget = WorkBudget::new(6_000_000_000);
    let atlas = atlas(&twin, &mut budget)?;
    let query = frame(2, true, &mut budget)?;
    let localization = localize_radial_focal_scan(
        &atlas,
        &twin,
        &query,
        MatchOptions {
    ...
    assert!((candidate.raw_intrinsics.focal_lengths()[0] - FX).abs() < 1e-9);
    assert!((candidate.k1 - K1).abs() < 1e-12);
    let plan = candidate.compile_rectification([8; 32], LumaRange::Full, &mut budget)?;
    assert_eq!(plan.spec().source_domain, [6; 32]);
    assert_ne!(plan.output_domain(), [6; 32]);
    let tracking = candidate.bind_tracking_camera(
        ...
        TrackingCameraBinding {
            ...
            calibration: 9,
            image_domain: 11,
            image_domain_digest: plan.output_domain(),
            clock: 13,
            validity: [100, 200],
    ...
    assert_eq!(tracking.intrinsics, candidate.raw_intrinsics);
    assert_eq!(tracking.pose, candidate.pose);
    let frozen = candidate.frozen_calibration(&plan, [8; 32])?;
    assert_eq!(frozen.camera.image_domain, plan.output_domain());
...
fn wrong_derived_domain_cannot_bind_tracking_snapshot() -> Test {
    ...
    let plan = candidate.compile_rectification([8; 32], LumaRange::Full, &mut budget)?;
    assert!(matches!(
        candidate.bind_tracking_camera(
            &twin,
            &plan,
            TrackingCameraBinding {
                camera: 7,
                calibration: 9,
                image_domain: 11,
                image_domain_digest: [9; 32],
                clock: 13,
                validity: [100, 200],
                error: None
            }
        ),
        Err(RadialRegistrationError::InvalidBinding)
    ));
    Ok(())
}
