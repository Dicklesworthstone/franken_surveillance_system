...
const TRUE_FX: f64 = 800.0;
const TRUE_K1: f64 = 0.1;
...
fn raw_pixel(w: [f64; 3], fx: f64, k1: f64) -> [f64; 2] {
    ...
    [
        fx * xu * factor + 960.0,
        fx * (820.0 / 800.0) * yu * factor + 540.0,
    ]
}
fn frame(
    exposure: u8,
    ...
    let features = (1..=count)
        .map(|id| ImageFeature {
            id,
            pixel: if query {
                raw_pixel(world(id), TRUE_FX, TRUE_K1)
            } else {
                [60.0 + id as f64 * 20.0, 80.0 + (id % 10) as f64 * 40.0]
            },
    ...
    FeatureFrame::new(
        ImageIdentity {
            ...
            pixels: [exposure.wrapping_add(30); 32],
            image_domain: if query { [6; 32] } else { [3; 32] },
...
fn options() -> RadialFocalScanOptions {
    RadialFocalScanOptions {
        ...
        maximum_k1: 0.2,
        k1_samples: 17,
        maximum_undistorted_radius: 2.0,
...
fn holdout() -> Vec<Correspondence> {
    (101..=112)
        .map(|id| Correspondence {
            ...
            physical_group: id,
            world: world(id),
            pixel: raw_pixel(world(id), TRUE_FX, TRUE_K1),
        })
        .collect()
}
...
fn exact_distorted_matches_recover_true_focal_and_k1_sample() -> Test {
    ...
    let query = frame(2, true, 32, &mut budget)?;
    let result = localize_radial_focal_scan(
        &atlas,
        &twin,
        &query,
        MatchOptions {
    ...
    let RadialLocalizationOutcome::Scan(scan) = result.outcome else {
        return Err("no radial scan".into());
    };
    let sample = scan
        .samples
        .iter()
        .find(|s| s.focal_index == 8 && s.k1_index == 12)
        .ok_or("true sample missing")?;
    assert!((sample.intrinsics.focal_lengths()[0] - TRUE_FX).abs() < 1e-9);
    assert!((sample.k1 - TRUE_K1).abs() < 1e-12);
    let RadialSampleOutcome::Candidates(search) = &sample.outcome else {
        return Err("true lens sample failed".into());
    };
...
fn held_out_raw_landmarks_select_true_lens_pose_without_refit() -> Test {
    let twin = common::twin(&[0.0], None)?;
    let mut budget = WorkBudget::new(5_000_000_000);
    let atlas = atlas(&twin, &mut budget)?;
    let query = frame(2, true, 32, &mut budget)?;
    let result = localize_radial_focal_scan(
        &atlas,
    ...
    let (sample_index, candidate) = validation
        .unique_passing_candidate()
        .ok_or("lens not uniquely validated")?;
    assert_eq!(candidate, 0);
    let sample = &scan.samples[sample_index];
...
fn noninvertible_candidates_are_retained_as_lens_failures() -> Test {
    ...
    assert!(scan.samples.iter().any(|s| matches!(
        s.outcome,
        RadialSampleOutcome::LensFailure(RadialLensFailure::NonInvertible)
    )));
    Ok(())
}
...
fn cancellation_and_resource_exhaustion_abort_complete_scan() -> Test {
    let twin = common::twin(&[0.0], None)?;
    let mut setup = WorkBudget::new(100_000_000);
    let atlas = atlas(&twin, &mut setup)?;
    let query = frame(2, true, 32, &mut setup)?;
    let flag = AtomicBool::new(true);
    assert!(matches!(
        localize_radial_focal_scan(
            &atlas,
            &twin,
            &query,
            MatchOptions {
            ...
            options(),
            &mut WorkBudget::cancellable(4_000_000_000, &flag)
        ),
