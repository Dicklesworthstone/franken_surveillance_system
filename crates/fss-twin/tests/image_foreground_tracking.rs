...
use fss_twin::foreground::{
    ...
    ForegroundSource, FrameAssessment,
};
use fss_twin::image_tracking::{
    ImageDetectionDisposition, ImageTrackState, ImageTracker, ImageTrackingError,
    ImageTrackingPolicy, TrackingAvailability,
};
...
fn key(value: u64) -> [u8; 32] {
    ContentDigest::sha256(&value.to_le_bytes()).bytes()
}
...
fn source(sequence: u64, pixels: &[u8]) -> ForegroundSource {
    ForegroundSource {
        image: ImageIdentity {
            ...
            image_domain: key(1000),
            dimensions: [WIDTH as u32, HEIGHT as u32],
        },
        camera: 1,
        clock: 1,
        calibration: key(2000),
...
fn tracker() -> TestResult<ImageTracker> {
    Ok(ImageTracker::new(key(3000), tracking(), &mut budget())?)
}
fn background(allowed: &[u8], selection: u64) -> TestResult<BackgroundModel> {
    ...
    Ok(BackgroundModel::build(
        &frames,
        BackgroundPolicy {
            selection_evidence: key(selection),
            validity: [0, 1000 * SECOND],
            maximum_spread: 2,
        },
        &mut budget(),
    )?)
...
fn actual_luma_regions_form_paths_and_stopped_targets_are_not_learned_away() -> TestResult {
    let allowed = vec![1; WIDTH * HEIGHT];
    let model = background(&allowed, 4000)?;
...
fn merged_components_keep_both_old_paths_unresolved() -> TestResult {
    ...
    assert!(
        tracker
            .tracks()
            .iter()
            .all(|t| t.state() == ImageTrackState::Coasting && t.observations() == 1)
    );
    Ok(())
}
#[test]
fn widespread_scene_change_is_not_assimilated_or_replaced_with_births() -> TestResult {
    ...
    let model = background(&allowed, 4000)?;
    let mut tracker = tracker()?;
    tracker.update_foreground(
        &detect(&model, 4, &pixels(&[[8, 5, 12, 9]]), &allowed, detector())?,
        &mut budget(),
    )?;
    let prior = tracker.tracks()[0].latest();
    let report = tracker.update_foreground(
        &detect(&model, 5, &[200; WIDTH * HEIGHT], &allowed, detector())?,
        &mut budget(),
    )?;
...
fn unknown_comparison_pixels_do_not_certify_absence() -> TestResult {
    let denied = vec![0; WIDTH * HEIGHT];
    let model = background(&denied, 4000)?;
    let mut tracker = tracker()?;
...
fn privacy_background_and_detector_changes_require_a_new_episode() -> TestResult {
    ...
    let model = background(&allowed, 4000)?;
    let mut tracker = tracker()?;
    let image = pixels(&[[8, 5, 12, 9]]);
    tracker.update_foreground(
        &detect(&model, 4, &image, &allowed, detector())?,
        &mut budget(),
    )?;
...
fn size_omissions_and_partial_boundaries_remain_source_linked() -> TestResult {
    ...
    let model = background(&allowed, 4000)?;
    let mut tracker = tracker()?;
    let foreground = detect(
        &model,
        4,
        &pixels(&[[0, 5, 4, 9], [30, 5, 31, 6]]),
        &allowed,
        detector(),
    )?;
    ...
    assert_ne!(
        report.decisions()[0].detection.evidence,
        foreground.digest()
    );
    Ok(())
}
#[test]
fn unmatched_foreground_does_not_become_new_observations_and_reacquisition_reuses_path()
-> TestResult {
    let allowed = vec![1; WIDTH * HEIGHT];
    let model = background(&allowed, 4000)?;
    let mut tracker = tracker()?;
...
fn failed_preparation_and_replay_do_not_age_tracks() -> TestResult {
    ...
    let prior = tracker.digest();
    assert!(matches!(
        tracker.update_foreground(&report, &mut WorkBudget::new(511)),
