...
use fss_twin::screened_mjpeg::{
    ForegroundStage, JpegScreeningError, JpegScreeningQuery, screen_framed_jpeg, screen_jpeg,
};
use fss_twin::screening::{
    AnalysisReason, HealthFlag, ScreeningHealth, ScreeningMonitor, ScreeningPolicy, ScreeningStamp,
};
use std::error::Error;
...
fn query<'a>(bytes: &'a [u8], mask: &'a [u8], seq: u64, now: u64) -> JpegScreeningQuery<'a> {
    JpegScreeningQuery {
        bytes,
        mask,
        binding: binding(bytes, mask, (seq + 3) as u8),
        capture: FrameCapture {
            camera: 1,
            clock: 2,
            capture: [now, now],
...
fn compressed_source_matches_existing_detector_and_reaches_analysis_admission() -> Test {
    ...
    let ForegroundStage::Complete(report) = actual.foreground() else {
        return Err("foreground not complete".into());
    };
    assert_eq!(report.digest(), expected.analysis().report().digest());
    assert!(!report.regions().is_empty());
    assert!(actual.analysis_frame().is_some());
    ...
    assert!(
        actual
            .screening()
            .reasons()
            .contains(AnalysisReason::Initial)
    );
...
fn foreground_work_exhaustion_preserves_real_health_and_an_uncompleted_analysis() -> Test {
    ...
    assert!(out.screening().analysis_due());
    assert!(out.analysis_frame().is_some());
    assert!(out.screening().last_completed_analysis().is_none());
    assert_eq!(out.attempted_background(), Some(before));
    assert_eq!(out.foreground_allowance(), 0);
    assert_eq!(out.foreground_units(), 0);
...
fn decode_corruption_and_wrong_mask_never_advance_temporal_state() -> Test {
    ...
    let mut bad = query(QUERY, &mask, 2, 50);
    bad.binding.allowed_mask = [99; 32];
    assert!(
        screen_jpeg(
            &mut m,
            Some(&model),
...
fn unconfigured_and_expired_backgrounds_are_not_empty_successful_detections() -> Test {
    ...
    let mask = vec![1; 221];
    let model = baseline(&p, &mask)?;
    let run = |background: Option<&JpegBackground>,
               now: u64|
     -> Result<fss_twin::screened_mjpeg::ScreenedJpeg, Box<dyn Error>> {
        let mut state = monitor()?;
        Ok(screen_jpeg(
            &mut state,
            background,
            &p,
            query(QUERY, &mask, 1, now),
            &mut DecodeBudget::new(100_000_000),
            &mut WorkBudget::new(100_000_000),
            &mut WorkBudget::new(100_000_000),
            &mut WorkBudget::new(100_000_000),
    ...
    let absent = run(None, 40)?;
    let expired = run(Some(&model), 101)?;
    assert!(matches!(
        absent.foreground(),
    ...
    assert!(matches!(
        expired.foreground(),
        ForegroundStage::Refused(ForegroundError::OutsideValidity)
    ));
    assert!(
        absent
            .screening()
...
fn frame(bytes: &[u8], basis: StreamBasis) -> Result<FramedJpeg, Box<dyn Error>> {
    ...
    let frame = stream
        .push(0, bytes, &mut DecodeBudget::new(100_000_000))?
        .frame
        .ok_or("frame missing")?;
    stream.finish(&mut DecodeBudget::new(100_000_000))?;
    Ok(frame)
}
#[test]
fn framed_byte_ranges_and_capture_survive_screening_without_ordinal_time_invention() -> Test {
    ...
    let frame = frame(QUERY, basis)?;
    let q = query(QUERY, &mask, 1, 40);
    let framed = FramedQuery {
    ...
    assert_eq!(result.source().ordinal, 1);
    assert_eq!(result.result().screening().source().capture, [40, 40]);
    let mut wrong = q.stamp;
    wrong.sequence = 2;
    assert!(matches!(
        screen_framed_jpeg(
            &mut m,
    ...
    assert_eq!(m.last_report(), Some(result.result().screening()));
    Ok(())
}
#[test]
    ...
    let run = || -> Result<Vec<[u8; 32]>, Box<dyn Error>> {
        ...
        for seq in 1..=3 {
            let out = screen_jpeg(
                ...
                Some(&model),
                &p,
                query(QUERY, &mask, seq, 30 + seq * 10),
                &mut DecodeBudget::new(100_000_000),
                &mut WorkBudget::new(100_000_000),
                &mut WorkBudget::new(100_000_000),
                &mut WorkBudget::new(100_000_000),
            ...
            let permitted: Vec<_> = out
                .image()
                .frame()
                .pixels()
                .iter()
                .zip(out.image().frame().allowed())
                .filter_map(|(v, mask)| (*mask == 1).then_some(*v))
                .collect();
            assert_eq!(
                out.screening().dark_pixels(),
                permitted.iter().filter(|p| **p <= 10).count()
            );
            assert_eq!(
                out.screening().saturated_pixels(),
                permitted.iter().filter(|p| **p >= 245).count()
            );
