...
fn frame(
    ...
    let features = (1..=32)
        .map(|id| ImageFeature {
            ...
            pixel: if query {
                ...
                [40.0 + id as f64 * 20.0, 100.0 + (id % 8) as f64 * 40.0]
            },
    ...
    FeatureFrame::new(
        ImageIdentity {
            ...
            pixels: [exposure.wrapping_add(20); 32],
            image_domain: [3; 32],
...
fn holdout() -> Vec<Correspondence> {
    (101..=112)
        .map(|id| Correspondence {
            landmark: id,
            physical_group: id,
            world: world(id),
            pixel: pixel(world(id)),
        })
        .collect()
}
...
fn only_unique_held_out_mode_becomes_registration_candidate_and_owner_bound_snapshot() -> Test {
    ...
    let mut budget = WorkBudget::new(1_500_000_000);
    ...
    let FocalLocalizationOutcome::Scan(focal) = &localization.outcome else {
    ...
    assert_eq!(candidate.query.exposure, [2; 32]);
    assert_eq!(candidate.fit_landmarks.len(), 32);
    assert!(candidate.holdout.passed);
    assert!(candidate.fit_rms_px < 1e-5);
    let frozen = candidate.frozen_calibration([8; 32])?;
    assert_eq!(frozen.camera.image_domain, [3; 32]);
    assert_eq!(frozen.pose, candidate.pose);
    let tracking = candidate.bind_tracking_camera(
        &twin,
        TrackingCameraBinding {
            camera: 7,
            calibration: 9,
            image_domain: 11,
            image_domain_digest: [3; 32],
            clock: 13,
            validity: [100, 200],
            error: Some(fss_twin::ProjectionError {
                centre: [0.1; 3],
                rotation_entry: 0.001,
                focal: [0.1; 2],
                principal: [0.1; 2],
            }),
        },
    )?;
    assert_eq!(tracking.camera, 7);
    assert_eq!(tracking.calibration, 9);
    assert_eq!(tracking.pose, candidate.pose);
    assert_eq!(tracking.intrinsics, candidate.intrinsics);
    let mut wrong = TrackingCameraBinding {
        camera: 7,
        calibration: 9,
        image_domain: 11,
    ...
    assert!(matches!(
        candidate.bind_tracking_camera(&twin, wrong),
        Err(RegistrationCandidateError::InvalidBinding)
    ));
    wrong.image_domain_digest = [3; 32];
    wrong.camera = 0;
    assert!(matches!(
        candidate.bind_tracking_camera(&twin, wrong),
        Err(RegistrationCandidateError::InvalidBinding)
    ...
    wrong.camera = 7;
    wrong.error = Some(fss_twin::ProjectionError {
        rotation_entry: f64::NAN,
        ..fss_twin::ProjectionError::EXACT
    });
    assert!(matches!(
        candidate.bind_tracking_camera(&twin, wrong),
        Err(RegistrationCandidateError::InvalidBinding)
...
fn validation_from_another_scan_cannot_be_rebound() -> Test {
    ...
    let second = localize_focal_scan(
        &atlas,
        &twin,
        &query,
        [3; 32],
    ...
    let FocalLocalizationOutcome::Scan(second_scan) = &second.outcome else {
        return Err("no second scan".into());
    };
    let second_validation =
        second_scan.validate_all_candidates(twin.basis(), &holdout(), 0.01, &mut budget)?;
    assert!(matches!(
        select_unique_focal_registration(&twin, &atlas, &first, &second_validation),
        Err(RegistrationCandidateError::ValidationMismatch)
    ));
    Ok(())
}
