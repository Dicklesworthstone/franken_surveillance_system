...
use fss_twin::hog::{HOG_PARAMETERS, HogError, HogFrame, HogLevel, HogModel};
...
fn complete_native_and_resized_scales_have_stable_scores_suppression_and_source_bounds() -> Test {
    let pixels = vec![100; 80 * 136];
    let mask = vec![1; pixels.len()];
    let source = source(&pixels, [80, 136]);
    let model = model(1.0)?;
    let levels = [
        ScanLevel {
            dimensions: [80, 136],
    ...
    let a = run(&levels)?;
    let b = run(&[levels[2], levels[0], levels[1]])?;
    assert_eq!(a.digest(), b.digest());
    assert_eq!(a.windows(), b.windows());
    assert_eq!(a.windows().len(), 7);
    assert_eq!(a.selected().count(), 1);
    assert_eq!(a.levels()[2].windows, 0);
    assert_eq!(a.windows()[6].source_min, [0, 0]);
    assert_eq!(a.windows()[6].source_max, [80, 136]);
    for window in a.windows() {
        assert_eq!(window.margin, Some(1.0));
        if window.id != 1 {
            assert_eq!(window.disposition, WindowDisposition::Suppressed { by: 1 });
        }
    }
    ...
    assert_ne!(
        a.levels()[0].source.image.image_domain,
        a.levels()[1].source.image.image_domain
    );
    Ok(())
}
#[test]
fn margins_are_actual_native_kernel_outputs_not_a_constant_detector_stub() -> Test {
    let pixels: Vec<_> = (0..64 * 128)
        .map(|i| ((i * 17 + i / 64 * 11) % 256) as u8)
        .collect();
    let mask = vec![1; pixels.len()];
    let source = source(&pixels, [64, 128]);
    let mut bytes = Vec::new();
    for i in 0..HOG_PARAMETERS {
        bytes.extend_from_slice(&(((i % 11) as f32 - 5.0) * 0.01).to_le_bytes());
    }
    let mut budget = WorkBudget::new(BUDGET);
    let model = HogModel::from_f32_le(
        &bytes,
        ContentDigest::sha256(&bytes).bytes(),
        [9; 32],
        &mut budget,
    ...
    let descriptor = level
        .descriptor([0, 0], &mut budget)?
        .ok_or("unexpected private fixture")?;
    let parameters: Vec<_> = bytes
        .as_chunks::<4>()
        .0
        .iter()
        .map(|v| f32::from_le_bytes(*v))
        .collect();
    let expected = descriptor.iter().zip(&parameters).fold(
        f64::from(parameters[HOG_PARAMETERS - 1]),
    ...
    assert_ne!(expected, f64::from(parameters[HOG_PARAMETERS - 1]));
    Ok(())
}
#[test]
fn denied_windows_remain_unscored_instead_of_becoming_negatives() -> Test {
    let pixels = vec![100; 80 * 136];
    let mask = vec![0; pixels.len()];
    let model = model(1.0)?;
    let scan = scan_hog(
        source(&pixels, [80, 136]),
        &pixels,
        &mask,
        &model,
        &[
    ...
    assert_eq!(scan.windows().len(), 7);
    assert_eq!(scan.selected().count(), 0);
    assert!(
        scan.windows()
            .iter()
            .all(|w| w.margin.is_none() && w.disposition == WindowDisposition::Unobservable)
    );
    Ok(())
}
#[test]
fn native_gradient_halo_privacy_is_preserved_by_scan() -> Test {
    let pixels = vec![100; 72 * 128];
    let mut mask = vec![1; pixels.len()];
    mask[64] = 0;
    let scan = scan_hog(
        source(&pixels, [72, 128]),
        &pixels,
        &mask,
        &model(1.0)?,
        &[ScanLevel {
    ...
    assert_eq!(
        scan.windows()[0].disposition,
        WindowDisposition::Unobservable
    );
    Ok(())
}
#[test]
fn below_threshold_is_distinct_from_masked_and_small_grid() -> Test {
    let pixels = vec![100; 64 * 128];
    let mask = vec![1; pixels.len()];
    let scan = scan_hog(
        source(&pixels, [64, 128]),
        &pixels,
        &mask,
        &model(0.5)?,
        &[
    ...
    assert_eq!(scan.windows()[0].margin, Some(0.5));
    assert_eq!(
        scan.windows()[0].disposition,
...
fn duplicate_grids_invalid_policy_and_forged_source_are_rejected() -> Test {
    ...
    let src = source(&pixels, level.dimensions);
    assert!(matches!(
        scan_hog(
            src,
            &pixels,
        ...
        Err(HogError::InvalidInput)
    ));
    for p in [
        ScanPolicy {
            minimum_margin: f64::NAN,
            ..policy()
        },
        ScanPolicy {
            stride: [1, 8],
...
fn cancellation_budget_refusal_and_exact_retry_preserve_complete_identity() -> Test {
    ...
    for limit in [0, 1, 100, 10_000, budget.used() - 1] {
        assert!(matches!(
            ...
            Err(HogError::Work(GeometryError::BudgetExhausted))
        ));
    }
    ...
    assert!(matches!(
        scan_hog(
            ...
            &[level],
            policy(),
            &mut WorkBudget::cancellable(BUDGET, &cancelled)
        ),
        Err(HogError::Work(GeometryError::Cancelled))
    ));
    let repeated = scan_hog(
        src,
        &pixels,
        &mask,
        &model,
    ...
    assert_eq!(expected.digest(), repeated.digest());
    Ok(())
}
#[test]
fn generation_changes_with_weights_threshold_and_grid_but_not_frame_pixels() -> Test {
    ...
    assert_eq!(a.generation(), d.generation());
