...
use fss_twin::hog::{HOG_PARAMETERS, HogError, HogFrame, HogLevel, HogModel, hog_recipe_digest};
...
fn unchanged_trained_assets_and_model_generation_are_pinned() -> Test {
    ...
    assert_eq!(
        ContentDigest::sha256(opencv_people_weights()).bytes(),
        OPENCV_PEOPLE_WEIGHTS_SHA256
    );
    assert_eq!(
        ContentDigest::sha256(opencv_people_provenance().as_bytes()).bytes(),
        OPENCV_PEOPLE_PROVENANCE_SHA256
    );
    assert_eq!(
        ContentDigest::sha256(opencv_people_license().as_bytes()).bytes(),
        OPENCV_PEOPLE_LICENSE_SHA256
    );
    ...
    let expected = [
        0xf7, 0x5b, 0x82, 0xd4, 0x53, 0x2b, 0x05, 0x36, 0x5b, 0x77, 0xb7, 0x8a, 0xe2, 0xb4, 0xce,
        0x58, 0xfe, 0xf8, 0x53, 0xa3, 0xf4, 0xdd, 0x57, 0x5b, 0x81, 0x66, 0x16, 0x79, 0x46, 0x84,
        0xa7, 0xe6,
    ];
...
fn real_trained_margins_match_retained_procedural_numeric_examples() -> Test {
    ...
    let expected = [
        (-6.6657915115356445, -6.6657915115356445),
        (-6.6657915115356445, -6.6657915115356445),
        (-3.8759877860091767, -3.8759877860091767),
        (-5.5279403317694005, -5.5279403317694005),
        (-4.45516621963684, -4.453545276177976),
        (-4.579090416443567, -4.578850730765833),
        (-4.387786588320922, -4.387497959530209),
        (-4.392289084759522, -4.392013219118344),
        (-4.244129512657451, -4.243925858567829),
        (-3.9917389524622355, -3.991523228126974),
    ];
    let model = load_opencv_people_candidate(&mut WorkBudget::new(WORK))?;
    for (k, (native, oracle)) in expected.into_iter().enumerate() {
        let pixels = image(k as u32);
        ...
        let margin = level
            .score(&model, [0, 0], &mut budget)?
            .ok_or("unexpected denied window")?;
        assert!(
            (margin - native).abs() < 0.000002,
            "native fixture {k}: {margin}"
        );
        assert!(
            (margin - oracle).abs() < 0.003,
            "oracle fixture {k}: {margin}"
        );
    }
    Ok(())
}
...
fn actual_model_scans_keep_scores_below_policy_threshold() -> Test {
    ...
    assert!(
        (scan.windows()[0].margin.ok_or("missing actual score")? + 3.8759877860091767).abs()
            < 0.000002
    );
...
fn insufficient_work_never_publishes_a_partial_model_and_exact_retry_is_stable() -> Test {
    for limit in [0, 1, 100, 10_000] {
        assert!(matches!(
            load_opencv_people_candidate(&mut WorkBudget::new(limit)),
            Err(HogError::Work(_))
        ));
    }
...
fn normal_loader_rejects_mutated_pretrained_weights() -> Test {
    let mut bytes = opencv_people_weights().to_vec();
    bytes[8] ^= 1;
    assert!(matches!(
