...
fn texture(width: usize, height: usize) -> Vec<u8> {
    (0..height)
        .flat_map(|y| (0..width).map(move |x| ((x * 17 + y * 29 + (x * y) % 251) % 256) as u8))
        .collect()
}
...
fn descriptor_matches_retained_opencv_cpu_oracle_with_declared_tolerance() -> Test {
    ...
    let descriptor = level
        .descriptor([8, 8], &mut budget())?
        .ok_or("private descriptor")?;
    let oracle = [
        (0, 0.0),
        (1, 0.23236859),
        (8, 0.0),
    ...
    for (index, expected) in oracle {
        assert!(
            (descriptor[index] - expected).abs() < 0.0003,
            "feature {index}"
        );
    }
...
fn constant_image_produces_zero_descriptor_and_exact_classifier_intercept() -> Test {
    let pixels = vec![73; 64 * 128];
    let allowed = vec![1; pixels.len()];
    let frame = HogFrame::new(source(&pixels, [64, 128]), &pixels, &allowed, &mut budget())?;
    let level = HogLevel::compute(&frame, &mut budget())?;
    assert_eq!(
        level.descriptor([0, 0], &mut budget())?,
        Some([0.0; HOG_FEATURES])
    );
    assert_eq!(
        level.score(&model(&weights(-2.5))?, [0, 0], &mut budget())?,
        Some(-2.5)
    );
    Ok(())
}
...
fn classifier_really_uses_every_learned_feature_and_sign() -> Test {
    ...
    let mut bytes = weights(-0.25);
    let mut expected = -0.25_f64;
    ...
    assert_eq!(
        level.score(&learned, [0, 0], &mut budget())?,
        Some(expected)
    );
...
fn gradient_halo_denial_invalidates_window_even_outside_its_box() -> Test {
    let pixels = texture(80, 144);
    let mut allowed = vec![1; pixels.len()];
    ...
    assert!(
        level
            .score(&model(&weights(1.0))?, [8, 8], &mut budget())?
            .is_none()
    );
    Ok(())
}
...
fn all_private_image_is_unknown_even_with_strong_positive_intercept() -> Test {
    let pixels = texture(64, 128);
    let allowed = vec![0; pixels.len()];
    let frame = HogFrame::new(source(&pixels, [64, 128]), &pixels, &allowed, &mut budget())?;
    let level = HogLevel::compute(&frame, &mut budget())?;
    assert!(
        level
            .score(&model(&weights(1000.0))?, [0, 0], &mut budget())?
            .is_none()
    );
    Ok(())
}
...
fn source_and_mask_validation_fail_before_feature_computation() -> Test {
    let pixels = texture(64, 128);
    let mut allowed = vec![1; pixels.len()];
    let mut identity = source(&pixels, [64, 128]);
    identity.image.pixels = [8; 32];
    assert!(matches!(
        HogFrame::new(identity, &pixels, &allowed, &mut budget()),
        Err(HogError::DigestMismatch)
    ));
    allowed[7] = 2;
    assert!(matches!(
        HogFrame::new(source(&pixels, [64, 128]), &pixels, &allowed, &mut budget()),
        Err(HogError::InvalidInput)
    ));
    assert!(matches!(
        HogFrame::new(
            source(&pixels, [4096, 4096]),
            &pixels,
            &allowed,
            &mut budget()
        ),
...
fn model_rejects_wrong_digest_nonfinite_truncated_and_trailing_coefficients() -> Test {
    ...
    assert!(matches!(
        HogModel::from_f32_le(&valid, [1; 32], [9; 32], &mut budget()),
        Err(HogError::DigestMismatch)
    ));
    ...
    assert!(matches!(
        model(&valid[..valid.len() - 1]),
        Err(HogError::InvalidInput)
    ));
    let mut longer = valid.clone();
    longer.push(0);
    assert!(matches!(model(&longer), Err(HogError::InvalidInput)));
    let other = HogModel::from_f32_le(
        &valid,
        ContentDigest::sha256(&valid).bytes(),
        [10; 32],
        &mut budget(),
    )?;
...
fn unaligned_or_out_of_image_windows_are_not_implicitly_clamped() -> Test {
    ...
    for origin in [[1, 0], [0, 1], [8, 0], [0, 8], [u32::MAX, u32::MAX]] {
        assert!(matches!(
            level.descriptor(origin, &mut budget()),
            Err(HogError::InvalidInput)
        ));
    }
    let pixels = vec![0; 8];
    ...
    assert!(matches!(
        small.descriptor([0, 0], &mut budget()),
        Err(HogError::InvalidInput)
    ));
    Ok(())
}
...
fn budget_and_cancellation_refuse_complete_work_without_rewriting_inputs() -> Test {
    ...
    let level = HogLevel::compute(&frame, &mut complete)?;
    let used = complete.used();
    for limit in [0, 1, 8192, 65536, used - 1] {
        assert!(matches!(
            HogLevel::compute(&frame, &mut WorkBudget::new(limit)),
            Err(HogError::Work(GeometryError::BudgetExhausted))
        ));
    }
    let cancelled = AtomicBool::new(true);
    assert!(matches!(
        HogLevel::compute(&frame, &mut WorkBudget::cancellable(used, &cancelled)),
        Err(HogError::Work(GeometryError::Cancelled))
    ));
    assert_eq!(
        frame.source().image.pixels,
        ContentDigest::sha256(&pixels).bytes()
    );
    let model = model(&weights(1.0))?;
    let mut score_budget = budget();
    let expected = level.score(&model, [0, 0], &mut score_budget)?;
    assert!(matches!(
        level.score(
            &model,
            [0, 0],
            &mut WorkBudget::new(score_budget.used() - 1)
        ),
        Err(HogError::Work(GeometryError::BudgetExhausted))
    ));
