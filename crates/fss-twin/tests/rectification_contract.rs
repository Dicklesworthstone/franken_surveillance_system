...
fn spec(width: u32, height: u32) -> Result<RectificationSpec, Box<dyn Error>> {
    let camera = PinholeIntrinsics::new(
        width,
        height,
        5.0,
        5.0,
...
fn identity(s: RectificationSpec, bytes: &[u8], mask: &[u8], stride: u32) -> RawFrameIdentity {
    RawFrameIdentity {
        exposure: [7; 32],
        storage: ContentDigest::sha256(bytes).bytes(),
        allowed_mask: ContentDigest::sha256(mask).bytes(),
        image_domain: s.source_domain,
...
fn identity_is_exact_at_every_center_including_borders() -> Test {
    ...
    for y in 0..7 {
        for x in 0..9 {
            assert_eq!(
                plan.sampling_position(x, y)?,
                Some([f64::from(x) + 0.5, f64::from(y) + 0.5])
            );
        }
    }
...
fn brown_and_fisheye_maps_match_independent_opencv_goldens() -> Test {
    ...
    s.distortion = LensDistortion::BrownConrady {
        radial: [0.04, 0.002, 0.0001],
        tangential: [0.003, -0.002],
    };
    let brown = RectificationPlan::compile(s, &mut budget)?;
    assert_eq!(
        hex(brown.map_digest()),
        "7605020b6cb5d4962b937fe94b3590bbe1f11ab06be8063c87fafca6cd783ba0"
    );
    s.distortion = LensDistortion::Fisheye {
        coefficients: [0.01, -0.0001, 0.00001, 0.0],
    };
    let fish = RectificationPlan::compile(s, &mut budget)?;
    assert_eq!(
        hex(fish.map_digest()),
        "3fd8619dc9ce9a66ac93cdbb30e70d07019736eeca10b015571114986a99a780"
    );
...
fn zero_fisheye_coefficients_do_not_mean_pinhole() -> Test {
    ...
    let fish = RectificationPlan::compile(s, &mut budget)?;
    assert_ne!(
        fish.source_pixel([8.5, 3.5])?,
        pinhole.source_pixel([8.5, 3.5])?
    );
...
fn masked_contributors_cannot_affect_any_exposed_output() -> Test {
    ...
    let mut budget = WorkBudget::new(1_000_000);
    let plan = RectificationPlan::compile(s, &mut budget)?;
    let mut first = vec![40; 12];
    let mut second = first.clone();
    first[5] = 0;
    second[5] = 255;
    let mut mask = vec![1; 12];
    mask[5] = 0;
    ...
    let a = plan.apply(&a, &mut budget)?;
    let b = plan.apply(&b, &mut budget)?;
    assert_eq!(a.pixels(), b.pixels());
    assert_eq!(a.allowed(), b.allowed());
    assert_eq!(a.allowed()[0], 0);
    assert_eq!(a.pixels()[0], 0);
    assert!(a.receipt().privacy_rejected > 0);
    assert_eq!(
        a.receipt().allowed_pixels + a.receipt().privacy_rejected,
        plan.coverage().mapped
    );
...
fn zero_weight_neighbors_do_not_expand_a_mask_in_identity_mode() -> Test {
    ...
    assert_eq!(image.allowed(), mask);
    assert_eq!(image.receipt().privacy_rejected, 1);
...
fn crop_resize_and_outside_samples_never_replicate_border_values() -> Test {
    let mut s = spec(3, 3)?;
    s.target = PinholeIntrinsics::new(5, 5, 5.0, 5.0, 2.5, 2.5)?;
    let pixels = vec![255; 9];
    let mask = vec![1; 9];
    let mut budget = WorkBudget::new(1_000_000);
    let plan = RectificationPlan::compile(s, &mut budget)?;
    assert_eq!(plan.coverage().mapped, 9);
    assert_eq!(plan.coverage().outside_source, 16);
    let raw = RawGrayFrame::new(identity(s, &pixels, &mask, 3), &pixels, &mask, &mut budget)?;
    let output = plan.apply(&raw, &mut budget)?;
    for y in 0..5 {
        for x in 0..5 {
    ...
    s = spec(3, 3)?;
    s.maximum_radius = 0.01;
    let small = RectificationPlan::compile(s, &mut budget)?;
    assert_eq!(small.coverage().mapped, 1);
    assert_eq!(small.coverage().outside_lens_domain, 8);
    Ok(())
}
...
fn explicit_row_padding_is_not_image_content() -> Test {
    let s = spec(2, 2)?;
    let pixels = [10, 20, 250, 249, 30, 40, 248, 247];
    let mask = [1; 4];
    let mut budget = WorkBudget::new(1_000_000);
    let plan = RectificationPlan::compile(s, &mut budget)?;
...
fn video_luma_range_is_explicit_and_bound_to_image_domain() -> Test {
    let mut s = spec(6, 1)?;
    let pixels = [0, 16, 17, 125, 235, 255];
    let mask = [1; 6];
    let mut budget = WorkBudget::new(1_000_000);
    let full = RectificationPlan::compile(s, &mut budget)?;
    s.range = LumaRange::Video;
    let video = RectificationPlan::compile(s, &mut budget)?;
    let raw = RawGrayFrame::new(identity(s, &pixels, &mask, 6), &pixels, &mask, &mut budget)?;
    assert_eq!(
        video.apply(&raw, &mut budget)?.pixels(),
        [0, 0, 1, 127, 255, 255]
    );
    assert_eq!(full.map_digest(), video.map_digest());
    assert_ne!(full.output_domain(), video.output_domain());
    assert!(matches!(
        full.apply(&raw, &mut budget),
        Err(RectificationError::BasisMismatch)
    ));
    Ok(())
}
...
fn stale_calibration_and_source_domains_cannot_reuse_a_map() -> Test {
    ...
    for is_calibration in [false, true] {
        ...
        if is_calibration {
            ...
            id.image_domain = [8; 32];
        }
        let raw = RawGrayFrame::new(id, &pixels, &mask, &mut budget)?;
        assert!(matches!(
            plan.apply(&raw, &mut budget),
            Err(RectificationError::BasisMismatch)
        ));
    }
    let mut changed = s;
    changed.calibration = [8; 32];
    let another = RectificationPlan::compile(changed, &mut budget)?;
    assert_eq!(plan.map_digest(), another.map_digest());
    assert_ne!(plan.output_domain(), another.output_domain());
    Ok(())
}
...
fn malformed_masks_storage_and_dimensions_fail_before_output() -> Test {
    ...
    id.storage = [1; 32];
    assert!(matches!(
        RawGrayFrame::new(id, &pixels, &mask, &mut budget),
        Err(RectificationError::BasisMismatch)
    ));
    let mut bad = mask;
    bad[8] = 2;
    assert!(matches!(
        RawGrayFrame::new(identity(s, &pixels, &bad, 3), &pixels, &bad, &mut budget),
        Err(RectificationError::InvalidInput)
    ));
    assert!(matches!(
        RawGrayFrame::new(identity(s, &pixels, &mask, 4), &pixels, &mask, &mut budget),
        Err(RectificationError::InvalidInput)
    ));
    let mut id = identity(s, &pixels, &mask, 3);
    id.dimensions = [4097, 1];
    assert!(matches!(
        RawGrayFrame::new(id, &pixels, &mask, &mut budget),
        Err(RectificationError::Limit)
    ));
    Ok(())
}
...
fn folded_and_nonfinite_lens_models_are_not_accepted() -> Test {
    ...
    for distortion in [
        ...
        assert!(matches!(
            RectificationPlan::compile(s, &mut budget),
            Err(RectificationError::NonInvertibleModel)
        ));
    }
    s.distortion = LensDistortion::Fisheye {
        coefficients: [f64::NAN, 0.0, 0.0, 0.0],
    };
    assert!(matches!(
        RectificationPlan::compile(s, &mut budget),
        Err(RectificationError::InvalidModel)
    ));
    Ok(())
}
...
fn budget_and_cancellation_leave_source_and_compiled_plan_reusable() -> Test {
    ...
    let mut budget = WorkBudget::new(1_000_000);
    assert!(matches!(
        RectificationPlan::compile(s, &mut WorkBudget::new(0)),
    ...
    assert!(matches!(
        plan.apply(&raw, &mut WorkBudget::cancellable(1_000_000, &cancelled)),
        Err(RectificationError::Geometry(GeometryError::Cancelled))
    ));
    assert!(matches!(
        plan.apply(&raw, &mut WorkBudget::new(126)),
        Err(RectificationError::Geometry(GeometryError::BudgetExhausted))
    ));
