...
use fss_twin::image_zones::{
    ImageZoneBasis, ImageZoneError, ImageZoneEventKind, ImageZoneMonitor, ImageZonePolicy,
    ImageZoneRelation, ImageZoneSpec,
};
...
fn source(exposure: u8, pixels: &[u8]) -> ForegroundSource {
    ForegroundSource {
        ...
        calibration: [6; 32],
        clock: 2,
        capture: [u64::from(exposure) * SECOND; 2],
    }
}
fn report(
    exposure: u8,
    ...
    let baseline = pixels(&[]);
    let allowed = vec![1; baseline.len()];
    let mut budget = work();
    let mut frames = Vec::new();
    for exposure in 1..=3 {
        frames.push(ForegroundFrame::new(
            source(exposure, &baseline),
            &baseline,
            &allowed,
            &mut budget,
        )?);
    }
    let model = BackgroundModel::build(
        &frames,
        BackgroundPolicy {
...
fn pixels_reach_entry_dwell_and_source_linked_operator_events() -> Test {
    ...
    for (exposure, left) in [(4, 8), (5, 16), (6, 24), (7, 28), (8, 28)] {
        ...
        assert_eq!(result.digest(), zones);
        assert_eq!(result.tracking_digest(), tracking);
        ...
        assert_eq!(
            ...
            match exposure {
                ...
                8 => vec![ImageZoneEventKind::SampledDwell],
                _ => vec![],
            }
        );
        if exposure == 8 {
...
fn direct_native_tracking_and_pipeline_have_identical_receipts() -> Test {
    ...
    for exposure in 4..=7 {
        ...
        assert_eq!(
            complete(p.observe_foreground(&f, &mut work())?)?,
            (track.digest(), expected)
        );
    }
    Ok(())
}
...
fn every_work_cut_preserves_accepted_receipts_and_resumes_only_unfinished_stage() -> Test {
    ...
    for cut in 0..measure.used() {
        ...
        match p.observe_foreground(&second, &mut WorkBudget::new(cut)) {
            Err(ZonePipelineError::Tracking(ImageTrackingError::Geometry(
                GeometryError::BudgetExhausted,
            ))) => {
                saw_before = true;
                assert_eq!(p.tracker().digest(), initial.0);
                assert_eq!(p.tracker().exposure_count(), 1);
                assert_eq!(
                    p.zone_report().ok_or("lost prior result")?.digest(),
                    initial.1
                );
                assert_eq!(
                    complete(p.observe_foreground(&second, &mut work())?)?,
                    expected
                );
            }
            Ok(ZonePipelineProgress::Pending {
                tracking,
                error: ImageZoneError::Geometry(GeometryError::BudgetExhausted),
            }) => {
                saw_after = true;
                assert_eq!(tracking, expected.0);
                assert_eq!(p.tracker().exposure_count(), 2);
                assert_eq!(
                    p.tracking_report().ok_or("lost accepted receipt")?.digest(),
                    expected.0
                );
                ...
                assert!(p.is_pending());
                assert_eq!(
                    p.observe_foreground(&third, &mut work()),
                    Err(ZonePipelineError::PendingAnalysis)
                );
...
fn pending_cancellation_keeps_source_and_completed_retry_never_duplicates_dwell() -> Test {
    ...
    let mut direct = ImageTracker::new([40; 32], tracking_policy(), &mut work())?;
    let mut measure = work();
    direct.update_foreground(&f, &mut measure)?;
    let mut p = pipeline()?;
    assert!(matches!(
        p.observe_foreground(&f, &mut WorkBudget::new(measure.used()))?,
        ZonePipelineProgress::Pending { .. }
    ));
    let before = p.tracker().digest();
    let cancel = AtomicBool::new(true);
    assert!(matches!(
        p.resume(&mut WorkBudget::cancellable(ALLOWANCE, &cancel))?,
        ZonePipelineProgress::Pending {
            error: ImageZoneError::Geometry(GeometryError::Cancelled),
            ..
        }
    ));
    assert_eq!(p.tracker().digest(), before);
    assert!(p.zone_report().is_none());
    let result = complete(p.resume(&mut work())?)?;
    assert_eq!(complete(p.resume(&mut work())?)?, result);
    assert_eq!(
        p.zone_report().ok_or("missing zone result")?.events().len(),
        1
    );
    assert_eq!(p.tracker().exposure_count(), 1);
    assert!(matches!(
        p.observe_foreground(&f, &mut work()),
        Err(ZonePipelineError::Tracking(
...
fn zone_basis_and_privacy_generation_fail_before_consuming_source() -> Test {
    ...
    let mut wrong = basis();
    wrong.camera = 99;
    let mut p = ImageZonePipeline::new(
        [40; 32],
        tracking_policy(),
        wrong,
        policy(),
    ...
    assert_eq!(
        p.observe_foreground(&f, &mut work()),
        Err(ZonePipelineError::Zones(ImageZoneError::BasisMismatch))
    );
    assert_eq!(p.tracker().digest(), before);
    assert!(p.tracking_report().is_none());
    let mut p = pipeline()?;
    complete(p.observe_foreground(&f, &mut work())?)?;
    let before = p.tracker().digest();
    let mut mask = [1; 1024];
    ...
    assert_eq!(
        p.observe_foreground(&masked, &mut work()),
        Err(ZonePipelineError::Tracking(
            ImageTrackingError::BasisMismatch
        ))
    );
    assert_eq!(p.tracker().digest(), before);
    assert_eq!(p.foreground_digest(), Some(f.digest()));
    assert!(!p.is_pending());
    Ok(())
}
#[test]
fn broad_pixel_change_and_missing_foreground_do_not_accumulate_dwell_or_exit() -> Test {
    for (rectangles, relation) in [
        (vec![], ImageZoneRelation::Unobserved),
...
fn raw_luma_rectification_retains_source_and_reaches_zone_entry() -> Test {
    ...
    let mut b = basis();
    b.calibration = [8; 32];
    b.image_domain = plan.output_domain();
    let mut p = ImageZonePipeline::new(
        [40; 32],
        tracking_policy(),
        b,
    ...
    for (exposure, left) in [(4, 8), (5, 28)] {
        ...
        if exposure == 5 {
            assert_eq!(
                p.zone_report().ok_or("missing result")?.events()[0].kind,
...
fn jpeg_pipeline(
    plan: &RectificationPlan,
    maximum_exposures: usize,
) -> Result<ImageZonePipeline, ZonePipelineError> {
    let mut b = basis();
    b.dimensions = [17, 13];
    b.calibration = [8; 32];
    b.image_domain = plan.output_domain();
    let zones = [ImageZoneSpec {
        id: 1,
        vertices: vec![[0, 0], [17, 0], [17, 13], [0, 13]],
        margin: 0,
        dwell_ns: None,
    }];
    ImageZonePipeline::new(
...
fn native_jpeg_keeps_encoded_source_and_all_derived_zone_relations() -> Test {
    ...
    let mut p = jpeg_pipeline(&plan, 64)?;
    let binding = jpeg_binding(QUERY_JPEG, &mask, 4);
    let result = p.analyze_jpeg(
        &background,
        &plan,
        JpegZoneInput {
            bytes: QUERY_JPEG,
    ...
    let f = result.foreground().analysis().report();
    assert!(!f.regions().is_empty());
    assert_eq!(
        p.tracking_report().ok_or("no tracking")?.decisions().len(),
        f.regions().len()
    );
    let z = p.zone_report().ok_or("no zone result")?;
    assert_eq!(z.digest(), zones);
    assert_eq!(z.tracking_digest(), tracking);
    assert_eq!(z.frame().source.image.exposure, [4; 32]);
    assert_eq!(z.frame().evidence, f.digest());
...
fn decoded_evidence_survives_when_episode_capacity_refuses_tracking() -> Test {
    let (plan, background, mask) = jpeg_background()?;
    let mut p = jpeg_pipeline(&plan, 1)?;
    for exposure in [4, 5] {
        ...
        if exposure == 4 {
            ...
            assert_eq!(
                result.progress(),
                Err(ZonePipelineError::Tracking(ImageTrackingError::Limit))
            );
