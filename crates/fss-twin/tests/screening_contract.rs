...
use fss_twin::screening::{
    AnalysisReason as A, HealthFlag as H, ScreeningError, ScreeningHealth as S, ScreeningMonitor,
...
fn source(pixels: &[u8], exposure: u64, capture: [u64; 2]) -> ForegroundSource {
    ForegroundSource {
        image: ImageIdentity {
            exposure: hash(&exposure.to_le_bytes()),
            pixels: hash(pixels),
            image_domain: hash(b"domain"),
            dimensions: [4, 4],
        },
        camera: 1,
        calibration: hash(b"calibration"),
        clock: 1,
        capture,
    }
}
fn pixels() -> Vec<u8> {
    (0..16).map(|i| if i % 2 == 0 { 80 } else { 160 }).collect()
}
fn baseline(mask: &[u8]) -> Result<BackgroundModel, Box<dyn Error>> {
    ...
    Ok(BackgroundModel::build(
        &refs,
        BackgroundPolicy {
            selection_evidence: hash(b"selection"),
            validity: [0, 10_000],
            maximum_spread: 0,
        },
        &mut budget,
...
fn quiet_never_disables_sentinel_and_unacknowledged_work_stays_due() -> TestResult {
    ...
    assert_eq!(
        m.acknowledge_analysis(first.digest(), hash(b"synthetic-analysis-result")),
        Err(ScreeningError::StaleCompletion)
    );
    m.acknowledge_analysis(second.digest(), hash(b"synthetic-analysis-result"))?;
    m.acknowledge_analysis(second.digest(), hash(b"synthetic-analysis-result"))?;
    assert_eq!(
        m.acknowledge_analysis(second.digest(), hash(b"different-result")),
        Err(ScreeningError::StaleCompletion)
    );
...
fn actual_small_and_stationary_changes_survive_area_filtering_and_sampling() -> TestResult {
    ...
    let model = baseline(&mask)?;
    let mut m = ScreeningMonitor::new(policy(), 1, 0)?;
    let first = observe(&mut m, &model, &pixels(), &mask, 1, 10)?;
    m.acknowledge_analysis(first.digest(), hash(b"synthetic-analysis-result"))?;
    let mut p = pixels();
    p[0] = 200;
    ...
    for (seq, now) in [(2, 20), (3, 30), (4, 40)] {
        ...
        assert!(r.reasons().contains(A::Foreground));
        m.acknowledge_analysis(r.digest(), hash(b"synthetic-analysis-result"))?;
    }
    Ok(())
}
#[test]
fn repeated_pixels_with_advancing_source_records_raise_freeze_suspicion() -> TestResult {
    ...
    assert_eq!(r.identical_run(), 3);
    assert!(r.flags().contains(H::SuspectedFreeze));
    assert_eq!(r.health(), S::Degraded);
    assert!(r.reasons().contains(A::HealthChanged));
    let mut p = p;
    p[0] += 1;
    assert_eq!(
        observe(&mut m, &model, &p, &mask, 4, 31)?.health(),
        S::Recovering
    );
    p[0] += 1;
    assert_eq!(
        observe(&mut m, &model, &p, &mask, 5, 32)?.health(),
        S::NoFaultObserved
    );
    Ok(())
}
...
fn silence_is_detected_without_any_new_frame_and_reentry_is_degraded() -> TestResult {
    ...
    assert_eq!(m.watchdog_deadline_ns(), Some(105));
    assert!(!m.poll(104)?.stalled);
    let empty = m.poll(105)?;
    assert!(empty.stalled);
    assert!(empty.last_report.is_none());
    let mask = vec![1; 16];
    let model = baseline(&mask)?;
    let first = observe(&mut m, &model, &pixels(), &mask, 1, 106)?;
    assert!(first.flags().contains(H::ReceiveGap));
    assert_eq!(m.watchdog_deadline_ns(), Some(206));
    assert!(m.poll(206)?.stalled);
    assert_eq!(m.poll(205), Err(ScreeningError::OutOfOrder));
    let next = observe(&mut m, &model, &pixels(), &mask, 2, 207)?;
    assert!(next.flags().contains(H::ReceiveGap));
    assert_eq!(next.identical_run(), 1);
    Ok(())
}
#[test]
fn privacy_excluded_pixels_never_change_statistics_or_the_repeat_run() -> TestResult {
    let mut mask = vec![1; 16];
    mask[0] = 0;
    let model = baseline(&mask)?;
    let mut m = ScreeningMonitor::new(policy(), 1, 0)?;
    let mut p = pixels();
    let a = observe(&mut m, &model, &p, &mask, 1, 10)?;
    p[0] = 0;
    let b = observe(&mut m, &model, &p, &mask, 2, 20)?;
    p[0] = 255;
    let c = observe(&mut m, &model, &p, &mask, 3, 30)?;
    assert_eq!(a.visible_pixels(), 15);
    assert_eq!(a.luma_range(), b.luma_range());
    assert_eq!(b.luma_range(), c.luma_range());
    assert_eq!(c.dark_pixels(), 0);
    assert_eq!(c.saturated_pixels(), 0);
    assert_eq!(c.identical_run(), 3);
...
fn all_masked_is_not_observable_not_dark_or_a_healthy_quiet_scene() -> TestResult {
    ...
    assert_eq!(r.health(), S::NotObservable);
    assert_eq!(r.luma_range(), None);
    assert!(!r.flags().contains(H::Dark));
    assert!(!r.flags().contains(H::SuspectedFreeze));
    assert!(r.analysis_due());
    Ok(())
}
#[test]
fn dark_saturated_and_flat_inputs_produce_distinct_diagnostics() -> TestResult {
    ...
    for (value, expected) in [(0, H::Dark), (255, H::Saturated), (100, H::LowTexture)] {
        ...
        assert!(r.flags().contains(expected));
        assert!(r.flags().contains(H::LowTexture));
        assert_eq!(r.health(), S::Degraded);
    }
    Ok(())
}
...
fn sequence_gaps_and_mask_changes_reset_repeat_history() -> TestResult {
    ...
    observe(&mut m, &model, &p, &mask, 2, 20)?;
    let gap = observe(&mut m, &model, &p, &mask, 4, 30)?;
    assert!(gap.flags().contains(H::SequenceGap));
    assert_eq!(gap.identical_run(), 1);
    mask[0] = 0;
    let changed = observe(&mut m, &model, &p, &mask, 5, 40)?;
    assert!(changed.flags().contains(H::MaskChanged));
    assert_eq!(changed.identical_run(), 1);
    Ok(())
}
#[test]
...
fn failure_cancellation_and_wrong_basis_do_not_consume_state_or_sampling_credit() -> TestResult {
    ...
    assert!(matches!(
        ...
        Err(ScreeningError::Work(GeometryError::Cancelled))
    ));
    assert!(
        m.observe(
            r.source(),
    ...
    assert_eq!(
        ...
        Err(ScreeningError::BasisMismatch)
    );
    let mut wrong = stamp(2, 20);
    wrong.stream_generation = 2;
    assert_eq!(
        m.observe(
            r.source(),
            &p,
    ...
    assert_eq!(m.last_report(), Some(&first));
    m.acknowledge_analysis(first.digest(), hash(b"synthetic-analysis-result"))?;
    assert_eq!(
        m.acknowledge_analysis([42; 32], hash(b"synthetic-analysis-result")),
        Err(ScreeningError::StaleCompletion)
    );
    Ok(())
}
...
fn missing_foreground_and_unknown_background_do_not_suppress_initial_analysis() -> TestResult {
    ...
    assert!(r.flags().contains(H::ForegroundUnavailable));
    assert!(r.analysis_due());
    let mut old_mask = mask.clone();
    old_mask[0] = 0;
    let model = baseline(&old_mask)?;
    let r = observe(&mut m, &model, &p, &mask, 2, 20)?;
    assert!(r.flags().contains(H::BackgroundIncomplete));
    assert!(r.analysis_due());
    Ok(())
}
#[test]
fn owner_floor_and_activity_hold_bypass_quiet_sampling_without_granting_effects() -> TestResult {
    ...
    m.acknowledge_analysis(first.digest(), hash(b"synthetic-analysis-result"))?;
    p = pixels();
    let hold = observe(&mut m, &model, &p, &mask, 2, 16)?;
    assert!(hold.reasons().contains(A::ActivityHold));
    m.acknowledge_analysis(hold.digest(), hash(b"synthetic-analysis-result"))?;
    let r = detect(&model, &p, &mask, 103, [17, 17])?;
    let mut s = stamp(3, 17);
    s.owner_requests_analysis = true;
    let result = m.observe(
        r.source(),
        &p,
        &mask,
    ...
    assert!(result.reasons().contains(A::OwnerRequested));
    Ok(())
}
#[test]
...
fn invalid_policy_and_extreme_clock_values_fail_without_wraparound() -> TestResult {
    let mut p = policy();
    p.minimum_analysis_interval_ns = p.sentinel_interval_ns + 1;
    assert!(matches!(
        ScreeningMonitor::new(p, 1, 0),
        Err(ScreeningError::InvalidPolicy)
    ));
    assert!(matches!(
        ScreeningMonitor::new(policy(), 0, 0),
        Err(ScreeningError::BasisMismatch)
    ));
    let mut m = ScreeningMonitor::new(policy(), 1, u64::MAX - 1)?;
    assert_eq!(m.watchdog_deadline_ns(), None);
    assert!(!m.poll(u64::MAX)?.stalled);
    Ok(())
}
