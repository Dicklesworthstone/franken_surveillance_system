...
use fss_twin::screening::{
    AnalysisReason, HealthFlag, ScreeningHealth, ScreeningPolicy, ScreeningStamp,
};
use std::error::Error;
...
impl Fixture {
    fn new(dimensions: [u32; 2]) -> Result<Self, Box<dyn Error>> {
        ...
        let target = PinholeIntrinsics::new(
            dimensions[0],
            dimensions[1],
            f64::from(dimensions[0]) * 3.125,
            f64::from(dimensions[1]) * 3.125,
            f64::from(dimensions[0]) / 2.0,
            f64::from(dimensions[1]) / 2.0,
        )?;
    ...
    fn processor(&self) -> Result<JpegHogPipeline, Box<dyn Error>> {
        ...
        let zones = ImageZonePipeline::new(
            ...
            &[ImageZoneSpec {
                id: 1,
                vertices: vec![[1, 1], [w - 1, 1], [w - 1, h - 1], [1, h - 1]],
                margin: 0,
                dwell_ns: Some(20),
            }],
            &mut work(),
        ...
        let levels = [ScanLevel {
            dimensions: self.basis.dimensions,
        }];
    ...
    fn run(
        &self,
        p: &mut JpegHogPipeline,
        n: u64,
        inf: &mut WorkBudget<'_>,
        down: &mut WorkBudget<'_>,
...
fn complete(p: JpegHogProgress) -> Result<JpegHogCompletion, Box<dyn Error>> {
    match p {
        JpegHogProgress::Complete(c) => Ok(c),
        other => Err(format!("not complete: {other:?}").into()),
    }
}
#[test]
fn native_jpeg_reaches_learned_scan_and_existing_zone_receipts() -> Test {
    let f = Fixture::new([64, 128])?;
    let mut p = f.processor()?;
    let c = complete(f.run(&mut p, 1, &mut work(), &mut work())?)?;
    let image = p.image().ok_or("image lost")?;
    let scan = p.scan().ok_or("scan lost")?;
    assert_eq!(image.source_receipt().source.encoded_sha256, hash(JPEG));
    assert_eq!(scan.source(), image.screening().source());
    assert_eq!(scan.selected().count(), 1);
    assert_eq!(scan.digest(), c.scan);
    assert_eq!(image.digest(), c.image);
    assert_eq!(
        p.tracking_report().ok_or("tracking lost")?.digest(),
        c.tracking
    );
...
fn inference_and_tracking_retries_do_not_redecode_or_reingest() -> Test {
    ...
    assert!(matches!(
        ...
        JpegHogProgress::Pending {
            ...
            error: JpegHogRefusal::Inference(HogError::Work(GeometryError::BudgetExhausted)),
            ..
        }
    ));
    let image = p.image().ok_or("image lost")?.digest();
    assert!(p.scan().is_none());
    assert_eq!(
        f.run(&mut p, 2, &mut work(), &mut work()),
        Err(JpegHogError::PendingAnalysis)
    );
    assert!(matches!(
    ...
    assert_eq!(p.image().ok_or("image changed")?.digest(), image);
    assert!(p.scan().is_some());
    assert_eq!(p.zones().pipeline().tracker().exposure_count(), 0);
    let mut zero = WorkBudget::new(0);
    let c = complete(p.resume(&mut zero, &mut work())?)?;
...
fn downstream_budget_boundaries_keep_the_exact_accepted_stage() -> Test {
    ...
    for cut in [0, 1, 1023, 1024, units / 2, units - 1, units] {
        let mut p = f.processor()?;
        let result = f.run(&mut p, 1, &mut work(), &mut WorkBudget::new(cut))?;
        match result {
            JpegHogProgress::Pending {
                stage: JpegHogStage::Tracking,
                ..
...
fn cancellation_preserves_image_before_inference_and_scan_before_tracking() -> Test {
    ...
    for cancel_inference in [true, false] {
        let mut p = f.processor()?;
        let mut cancelled = WorkBudget::cancellable(ALLOWANCE, &flag);
        let mut ordinary = work();
        let (inf, down) = if cancel_inference {
            (&mut cancelled, &mut ordinary)
        } else {
            (&mut ordinary, &mut cancelled)
        };
        assert!(matches!(
            f.run(&mut p, 1, inf, down)?,
            JpegHogProgress::Pending { .. }
        ));
        assert!(p.image().is_some());
        assert_eq!(p.scan().is_none(), cancel_inference);
        assert_eq!(p.zones().pipeline().tracker().exposure_count(), 0);
...
fn foreground_failure_cannot_starve_health_or_admit_learned_measurements() -> Test {
    ...
    assert!(matches!(
        image.foreground(),
        ForegroundStage::Refused(ForegroundError::Geometry(GeometryError::BudgetExhausted))
    ));
    assert!(
        image
            .screening()
    ...
    assert!(p.zones().pipeline().tracker().tracks().is_empty());
    assert!(p.zone_report().ok_or("zones missing")?.events().is_empty());
    Ok(())
}
#[test]
fn stationary_native_jpeg_candidates_reach_dwell_without_crediting_semantic_custody() -> Test {
    let f = Fixture::new([192, 320])?;
    let mut p = f.processor()?;
    for n in 1..=3 {
        ...
        assert_eq!(
            image
                .screening()
                .reasons()
                .contains(AnalysisReason::OwnerRequested),
            n > 1
        );
        assert!(image.screening().last_completed_analysis().is_none());
        ...
        assert_eq!(
            p.zone_report()
                ...
                .events()
                .iter()
                .filter(|e| e.kind == ImageZoneEventKind::SampledDwell)
                .count(),
            usize::from(n == 3)
        );
    }
...
fn watchdog_remains_available_while_inference_is_pending() -> Test {
    ...
    assert_eq!(
        ...
        Err(JpegHogError::NoObservation)
    );
    f.run(&mut p, 1, &mut WorkBudget::new(0), &mut work())?;
    let original = p.image().ok_or("image missing")?.digest();
    let watchdog = p.poll(2000)?;
    assert!(watchdog.stalled);
    assert_eq!(p.stage(), JpegHogStage::Inference);
    assert_eq!(p.image().ok_or("image lost")?.digest(), original);
    assert!(p.scan().is_none());
    complete(p.resume(&mut work(), &mut work())?)?;
    assert_eq!(p.zones().pipeline().tracker().exposure_count(), 1);
    Ok(())
