...
fn framed() -> Result<(Vec<u8>, HttpJpegFrame), Box<dyn std::error::Error>> {
    ...
    for chunk in entity.chunks(17) {
        wire.extend_from_slice(format!("{:x}\r\n", chunk.len()).as_bytes());
        wire.extend_from_slice(chunk);
        wire.extend_from_slice(b"\r\n");
    }
    wire.extend_from_slice(b"0\r\n\r\n");
    let scope = StreamBasis {
        source: ContentDigest::sha256(&wire).bytes(),
        generation: 1,
    };
    let mut h = HttpResponseStream::new(scope, HttpLimits::default())?;
    let mut p = None;
    let mut frame = None;
    let mut b = DecodeBudget::new(100_000_000);
        ...
        match s.event {
            ...
            Some(HttpEvent::Data(data)) => {
                ...
                while at < data.bytes().len() {
                    let s = p.as_mut().ok_or("missing MIME")?.push(&data, at, &mut b)?;
                    at += s.consumed;
                    if s.frame.is_some() {
                        frame = s.frame;
                    }
...
fn chunked_response_reaches_masked_foreground_with_original_wire_spans() -> Test {
    ...
    let result = detect_http(
        &model,
        &p,
        HttpQuery {
            expected_response: frame.head().wire,
            frame: &frame,
            mask: &mask,
    ...
    let reconstructed: Vec<u8> = result
        ...
        .iter()
        .flat_map(|r| {
            wire[r.wire_range[0] as usize..r.wire_range[1] as usize]
                .iter()
                .copied()
        })
        .collect();
    assert_eq!(reconstructed, QUERY);
    assert!(result.source().source_spans().len() > 1);
...
fn stale_http_generation_is_refused_before_decoding() -> Test {
    ...
    assert!(matches!(result, Err(JpegPipelineError::BasisMismatch)));
    Ok(())
}
