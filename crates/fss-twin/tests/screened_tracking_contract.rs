...
use fss_twin::image_tracking::{
    ImageDetectionDisposition, ImageTrackState, ImageTrackingError, ImageTrackingPolicy,
...
use fss_twin::screening::{
    AnalysisReason, HealthFlag, ScreeningHealth, ScreeningMonitor, ScreeningPolicy,
    ScreeningReport, ScreeningStamp,
};
use std::error::Error;
...
fn pixels() -> Vec<u8> {
    (0..N).map(|i| 100 + (i % 2) as u8 * 20).collect()
}
...
impl Scene {
    fn new() -> Result<Self, Box<dyn Error>> {
        ...
        let mut b = work();
        let frames = (1..=3)
            .map(|id| ForegroundFrame::new(source(id, &pixels), &pixels, &mask, &mut b))
            .collect::<Result<Vec<_>, _>>()?;
        Ok(Self {
            model: BackgroundModel::build(
                &frames,
                BackgroundPolicy {
                    selection_evidence: [77; 32],
                    validity: [0, 10_000],
                    maximum_spread: 0,
                },
                &mut b,
    ...
    fn explicit(
        ...
        Ok(Image {
            report: self.model.detect(&frame, policy, &mut budget)?,
            pixels,
            mask,
        })
    }
...
fn screen(
    ...
    Ok(monitor.observe(
        ...
        ScreeningStamp {
            stream_generation: 7,
            sequence,
            received_at_ns: image.report.source().capture[1],
            owner_requests_analysis: owner,
        },
        &mut work(),
    )?)
}
...
fn clean_source_pairs_continue_without_acknowledging_semantic_analysis() -> Test {
    ...
    let mut m = monitor()?;
    let mut t = tracker()?;
    let a = scene.image(4, &[[2, 2, 4, 4]])?;
    let sa = screen(&mut m, &a, 1, false)?;
    let first = t.update_foreground(&a.report, &sa, &mut work())?;
    assert_eq!(first.screening().health(), ScreeningHealth::NoFaultObserved);
    assert_eq!(
        first.tracking().decisions()[0].disposition,
        ImageDetectionDisposition::Started(1)
    );
    assert_eq!(first.tracking().frame().evidence, sa.digest());
    assert!(t.requires_analysis());
    let b = scene.image(5, &[[3, 2, 5, 4]])?;
    let sb = screen(&mut m, &b, 2, t.requires_analysis())?;
    let next = t.update_foreground(&b.report, &sb, &mut work())?;
    assert_eq!(next.prior_digest(), first.digest());
    assert_eq!(next.input_digest(), sb.digest());
    assert_eq!(
        next.tracking().decisions()[0].disposition,
        ImageDetectionDisposition::Continued(1)
    );
    assert_eq!(t.tracker().tracks()[0].observations(), 2);
    assert_eq!(
        t.tracker().tracks()[0].latest().frame.source,
        b.report.source()
    );
    ...
    assert!(
        m.last_report()
            .ok_or("missing screen")?
            .last_completed_analysis()
            .is_none()
    );
    assert_eq!(next.skipped_sequences(), 0);
    assert!(!next.health_history_gap());
    Ok(())
}
#[test]
fn freeze_suspicion_keeps_real_proposals_but_cannot_update_a_track() -> Test {
    ...
    for id in 4..=6 {
        ...
        if id == 6 {
            ...
            assert_eq!(
                update.tracking().frame().availability,
                TrackingAvailability::Disturbed
            );
            assert_eq!(
                update.tracking().decisions()[0].disposition,
                ImageDetectionDisposition::Unavailable
            );
            assert_eq!(
                t.tracker().tracks()[0].latest(),
                before.ok_or("missing track")?.latest()
            );
            assert_eq!(t.tracker().tracks()[0].observations(), 2);
...
fn recovery_window_withholds_births_until_clean_observations_are_sufficient() -> Test {
    let scene = Scene::new()?;
    let mut m = ScreeningMonitor::new(
        ScreeningPolicy {
            recovery_frames: 2,
            ..health_policy()
        },
...
fn a_clean_health_screen_cannot_clear_a_widespread_foreground_disturbance() -> Test {
    ...
    let mut m = monitor()?;
    let mut t = tracker()?;
    let image = scene.explicit(
        4,
        pixels().into_iter().map(|v| v + 60).collect(),
        vec![1; N],
        detection_policy(),
    )?;
    ...
    assert_eq!(
        result.tracking().decisions()[0].disposition,
        ImageDetectionDisposition::Unavailable
    );
...
fn mismatched_source_mask_and_derivation_are_refused_atomically() -> Test {
    ...
    for other in [&b.report, &masked.report, &changed.report] {
        assert!(matches!(
            t.update_foreground(other, &sa, &mut work()),
            Err(ScreenedTrackingError::BasisMismatch)
        ));
        assert_eq!(t.digest(), root);
        assert!(t.last_screening().is_none());
...
fn generation_policy_restart_and_sequence_cannot_silently_reinterpret_an_episode() -> Test {
    ...
    assert!(matches!(
        wrong.update_foreground(&a.report, &sa, &mut work()),
        Err(ScreenedTrackingError::BasisMismatch)
    ));
    t.update_foreground(&a.report, &sa, &mut work())?;
    let root = t.digest();
    assert!(matches!(
        t.update_foreground(&a.report, &sa, &mut work()),
        Err(ScreenedTrackingError::OutOfOrder)
    ));
    let b = scene.image(5, &[[3, 2, 5, 4]])?;
    for policy in [
        health_policy(),
        ...
        assert!(matches!(
            t.update_foreground(&b.report, &sb, &mut work()),
            Err(ScreenedTrackingError::BasisMismatch)
        ));
    }
    assert_eq!(t.digest(), root);
    assert_eq!(t.last_screening(), Some(&sa));
    Ok(())
}
#[test]
fn joining_a_health_history_does_not_assimilate_across_its_unseen_prefix() -> Test {
    ...
    let sb = screen(&mut m, &b, 2, false)?;
    let mut t = tracker()?;
    let result = t.update_foreground(&b.report, &sb, &mut work())?;
    assert!(result.health_history_gap());
    assert_eq!(result.skipped_sequences(), 0);
    assert_eq!(
        result.tracking().decisions()[0].disposition,
        ImageDetectionDisposition::Unavailable
    );
...
fn capacity_failure_preserves_sequence_and_health_for_an_exact_retry() -> Test {
    ...
    let health = screen(&mut m, &image, 1, false)?;
    let mut t = ScreenedImageTracker::new(
        [88; 32],
        7,
        ImageTrackingPolicy {
...
fn every_work_cut_rolls_back_both_receipt_heads_and_a_retry_is_exact() -> Test {
    ...
    let sa = screen(&mut m, &a, 1, false)?;
    let b = scene.image(5, &[[3, 2, 5, 4]])?;
    let sb = screen(&mut m, &b, 2, true)?;
    let mut control = tracker()?;
    control.update_foreground(&a.report, &sa, &mut work())?;
    let mut measured = work();
    let expected = control.update_foreground(&b.report, &sb, &mut measured)?;
    assert_eq!(expected.work_units(), measured.used());
    for allowance in 0..measured.used() {
        let mut t = tracker()?;
        t.update_foreground(&a.report, &sa, &mut work())?;
        let before = t.digest();
        let inner = t.tracker().digest();
        let tracks = t.tracker().tracks().to_vec();
        assert!(
            matches!(
                t.update_foreground(&b.report, &sb, &mut WorkBudget::new(allowance)),
        ...
        assert_eq!(t.digest(), before);
        assert_eq!(t.tracker().digest(), inner);
        assert_eq!(t.tracker().tracks(), tracks.as_slice());
        assert_eq!(t.last_screening(), Some(&sa));
        assert_eq!(
            t.update_foreground(&b.report, &sb, &mut work())?.digest(),
            expected.digest()
        );
    }
    let mut t = tracker()?;
    t.update_foreground(&a.report, &sa, &mut work())?;
    assert_eq!(
        t.update_foreground(&b.report, &sb, &mut WorkBudget::new(measured.used()))?
            .digest(),
        expected.digest()
    );
    Ok(())
...
fn cancellation_cannot_spend_an_exposure_or_hide_screening_evidence() -> Test {
    let scene = Scene::new()?;
    let mut m = monitor()?;
    let image = scene.image(4, &[[2, 2, 4, 4]])?;
    let health = screen(&mut m, &image, 1, false)?;
    let mut t = tracker()?;
    let before = t.digest();
    ...
    let expected = tracker()?
        .update_foreground(&image.report, &health, &mut work())?
        .digest();
    assert_eq!(
        t.update_foreground(&image.report, &health, &mut work())?
            .digest(),
