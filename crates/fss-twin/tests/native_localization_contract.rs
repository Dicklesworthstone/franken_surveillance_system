...
fn descriptor_has_an_independent_golden_and_rotation_brightness_controls() -> Test {
    ...
    let brighter: Vec<_> = pixels.iter().map(|x| *x + 20).collect();
    ...
    let original = describe_reference_pixels(
        &source,
        &[ReferencePixel {
            id: 1,
            column: 31,
    ...
    let turned = describe_reference_pixels(
        &rotated,
        &[ReferencePixel {
            id: 1,
            column: 70,
    ...
    let lit = describe_reference_pixels(
        &brighter,
        &[ReferencePixel {
            id: 1,
            column: 31,
    ...
    assert_eq!(d, turned.features()[0].descriptor);
    assert_eq!(d, lit.features()[0].descriptor);
    Ok(())
}
#[test]
fn detected_and_explicit_reference_descriptors_are_identical() -> Test {
    ...
    let points: Vec<_> = extracted
        .frame
        .features()
        .iter()
        .map(|f| ReferencePixel {
            id: f.id,
            column: f.pixel[0] as u32,
            row: f.pixel[1] as u32,
        })
        .collect();
...
fn masked_pixels_cannot_enter_descriptor_neighborhoods() -> Test {
    ...
    for feature in result.frame.features() {
        let x = feature.pixel[0] as usize;
        let y = feature.pixel[1] as usize;
...
fn blank_and_fully_masked_images_do_not_invent_features() -> Test {
    let pixels = vec![100; 96 * 96];
    let allowed = vec![1; 96 * 96];
    let denied = vec![0; 96 * 96];
    let mut budget = WorkBudget::new(100_000_000);
    for mask in [&allowed, &denied] {
        let image = GrayImage::new(identity(1, &pixels), &pixels, mask, &mut budget)?;
        let result = extract_gray(&image, ExtractionOptions::default(), &mut budget)?;
        assert!(result.frame.features().is_empty());
        assert_eq!(result.selection.local_maxima, 0);
    }
    Ok(())
}
#[test]
fn pixel_hash_and_mask_shape_are_checked_before_use() -> Test {
    ...
    assert!(matches!(
        GrayImage::new(identity(1, &pixels), &changed, &mask, &mut budget),
        Err(LocalizationError::BasisMismatch)
    ));
    mask[0] = 2;
    assert!(matches!(
        GrayImage::new(identity(1, &pixels), &pixels, &mask, &mut budget),
        Err(LocalizationError::InvalidInput)
    ));
    assert!(matches!(
        GrayImage::new(identity(1, &pixels), &pixels, &mask[..20], &mut budget),
        Err(LocalizationError::InvalidInput)
    ));
    Ok(())
}
...
fn limits_omissions_and_cancellation_are_explicit() -> Test {
    ...
    let a = extract_gray(&image, options, &mut budget)?;
    let b = extract_gray(&image, options, &mut budget)?;
    assert_eq!(a.frame.features(), b.frame.features());
    assert_eq!(a.selection, b.selection);
    assert_eq!(a.selection.selected, 5);
    assert!(a.selection.omitted > 0);
    assert_eq!(
        a.selection.selected + a.selection.omitted,
        a.selection.local_maxima
    );
    assert!(matches!(
        extract_gray(&image, options, &mut WorkBudget::new(1)),
...
fn raw_pixels_to_rotated_camera_pose_without_query_correspondences() -> Test {
    ...
    for (i, f) in reference.features().iter().enumerate() {
        ...
        landmarks.push(AtlasLandmark {
            id: i as u64 + 1,
            physical_group: i as u64 + 1,
            feature: 0,
            world: [
                (f.pixel[0] - 48.0) * depth / 80.0,
                (f.pixel[1] - 48.0) * depth / 80.0,
                depth,
            ],
            evidence: [4; 32],
            error: None,
    ...
    let result = localize_gray_frame(
        &atlas,
        &twin,
        &query,
        camera,
    ...
    let LocalizationOutcome::Candidates(search) = result.localization.outcome else {
        return Err("raw image did not localize".into());
    };
    assert_eq!(search.candidates().len(), 1);
    let pose = search.candidates()[0].pose();
