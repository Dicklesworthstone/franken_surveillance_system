...
impl Huffman {
    pub fn new(count: [u8; 16], values: &[u8], ac: bool) -> Result<Self, DecodeError> {
        ...
        if total == 0 || total > 256 || total != values.len() {
            return Err(DecodeError::Malformed);
        }
    ...
    pub fn symbol(&self, input: &mut Bits<'_, '_>) -> Result<u8, DecodeError> {
        ...
        for i in 0..16 {
            ...
            if code >= self.first[i] && code - self.first[i] < u32::from(self.count[i]) {
                return self
                    .values
                    .get(self.offset[i] + (code - self.first[i]) as usize)
                    .copied()
...
pub(crate) struct Bits<'a, 'b> {
    pub reader: &'b mut Reader<'a>,
    byte: u8,
    left: u8,
}
impl<'a, 'b> Bits<'a, 'b> {
    pub fn new(reader: &'b mut Reader<'a>) -> Self {
        Self {
            reader,
            byte: 0,
            left: 0,
    ...
    pub fn read(&mut self, count: u8) -> Result<u32, DecodeError> {
        ...
        for _ in 0..count {
            if self.left == 0 {
                ...
                if self.byte == 255 && self.reader.byte()? != 0 {
                    return Err(DecodeError::Malformed);
                }
                self.left = 8;
            }
    ...
    pub fn restart(&mut self, expected: u8) -> Result<(), DecodeError> {
        self.align()?;
        if self.reader.marker()? != 0xd0 + expected {
            return Err(DecodeError::Malformed);
        }
        Ok(())
    }
...
mod tests {
    ...
    fn stuffed_ff_is_data_but_marker_ff_is_not() -> Result<(), DecodeError> {
        ...
        assert_eq!(Bits::new(&mut r).read(8), Err(DecodeError::Malformed));
        Ok(())
    }
    #[test]
