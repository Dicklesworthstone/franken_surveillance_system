...
impl Default for RgbDecodeLimits {
    fn default() -> Self {
        Self { frame: DecodeLimits::default(), maximum_output_bytes: MAX_RGB_BYTES }
    }
}
...
pub fn rgb_decoder_identity() -> [u8; 32] {
    ...
    bytes[..32].copy_from_slice(&ContentDigest::sha256(b"fss/jpeg-rgb/reference/1;nearest-chroma;q16-half-up;rgb-hwc").bytes());
    for (i, source) in [include_bytes!("color.rs").as_slice(), include_bytes!("image.rs").as_slice(),
        include_bytes!("entropy.rs").as_slice(), include_bytes!("transform.rs").as_slice(),
        include_bytes!("lib.rs").as_slice()].iter().enumerate() {
...
pub struct DecodedRgb { pixels: Vec<u8>, receipt: RgbDecodeReceipt }
impl std::fmt::Debug for DecodedRgb {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DecodedRgb").field("dimensions", &self.receipt.dimensions).finish_non_exhaustive()
    }
}
impl DecodedRgb {
    /// Coded image width and height.
    pub fn dimensions(&self) -> [u32; 2] { self.receipt.dimensions }
    ...
    pub fn receipt(&self) -> RgbDecodeReceipt { self.receipt }
}
...
pub fn decode_rgb(bytes: &[u8], expected_sha256: [u8; 32], interpretation: ComponentInterpretation,
    limits: RgbDecodeLimits, budget: &mut DecodeBudget<'_>) -> Result<DecodedRgb, DecodeError> {
    budget.charge(0)?;
    let l = limits.frame;
    if l.maximum_bytes == 0 || l.maximum_bytes > 16 * 1024 * 1024
        || l.maximum_dimension == 0 || l.maximum_dimension > 4096
        || l.maximum_pixels == 0 || l.maximum_pixels > 4_194_304
        || l.maximum_markers == 0 || l.maximum_markers > 4096 || bytes.len() > l.maximum_bytes
        || limits.maximum_output_bytes == 0 || limits.maximum_output_bytes > MAX_RGB_BYTES {
    ...
    let (dimensions, pixels, stats) = image::decode_with_output(bytes, interpretation, l,
        image::Reconstruction::Rgb { maximum_bytes: limits.maximum_output_bytes }, budget)?;
    // Source-profile hashing is explicit work too, rather than an unpriced post-decode pass.
    let profile_bytes = include_bytes!("color.rs").len() + include_bytes!("image.rs").len()
        + include_bytes!("entropy.rs").len() + include_bytes!("transform.rs").len() + include_bytes!("lib.rs").len();
    budget.charge(pixels.len() as u64 + profile_bytes as u64 + 2048)?;
    let receipt = RgbDecodeReceipt { encoded_sha256: expected_sha256,
        rgb_sha256: ContentDigest::sha256(&pixels).bytes(), decoder: rgb_decoder_identity(),
        interpretation, dimensions, mcus: stats.mcus, entropy_blocks: stats.blocks,
        restarts: stats.restarts, metadata_segments: stats.metadata_segments, metadata_bytes: stats.metadata_bytes };
    budget.charge(0)?;
...
pub(crate) fn ycbcr_to_rgb(y: u8, cb: u8, cr: u8) -> [u8; 3] {
    ...
    [channel(y + 91_881 * cr), channel(y - 22_554 * cb - 46_802 * cr),
