...
impl Default for FramingLimits {
    fn default() -> Self {
        Self { maximum_frame_bytes: 16 * 1024 * 1024, maximum_markers: 4096 }
    }
}
...
impl std::fmt::Debug for FramedJpeg {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FramedJpeg").field("ordinal", &self.ordinal)
            .field("byte_count", &self.bytes.len()).finish_non_exhaustive()
    }
}
impl FramedJpeg {
    ...
    pub fn byte_range(&self) -> [u64; 2] { self.range }
    ...
    pub fn encoded_sha256(&self) -> [u8; 32] { self.digest }
    /// Non-restart marker count, including SOI/EOI.
    pub fn markers(&self) -> usize { self.markers }
...
impl std::fmt::Debug for DiscardedFragment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DiscardedFragment").field("byte_count", &self.bytes.len())
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum Phase {
    Start, Soi, MarkerPrefix, MarkerCode, LengthHigh(u8), LengthLow(u8, u8),
    Payload { remaining: usize, scan: bool }, Entropy, EntropyCode { fill: bool },
}
...
impl std::fmt::Debug for JpegStream {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("JpegStream").field("frames", &self.completed)
            .field("buffered_bytes", &self.buffered.len()).field("failure", &self.failure)
...
impl JpegStream {
    ...
    pub fn new(basis: StreamBasis, limits: FramingLimits) -> Result<Self, FramingError> {
        ...
        Ok(Self { basis, limits, phase: Phase::Start, offset: 0, start: 0, completed: 0,
            markers: 0, scanned: false, buffered: Vec::new(), failure: None, closed: false })
    }
    ...
    pub fn failure(&self) -> Option<FramingError> { self.failure }
    /// Current retained incomplete-frame size; never a count of hidden emitted frames.
    pub fn buffered_bytes(&self) -> usize { self.buffered.len() }
    ...
    pub fn push(&mut self, expected_offset: u64, input: &[u8], budget: &mut DecodeBudget<'_>)
        ...
        if self.closed { return Err(self.error(FramingError::Closed, 0)); }
        if self.failure.is_some() { return Err(self.error(FramingError::Poisoned, 0)); }
        if expected_offset != self.offset { return Err(self.fail(FramingError::OffsetMismatch, 0)); }
        if let Err(e) = budget.charge(0) { return Err(self.fail(FramingError::Work(e), 0)); }
        for (index, &byte) in input.iter().enumerate() {
            if let Err(e) = budget.charge(4) { return Err(self.fail(FramingError::Work(e), index)); }
            ...
            if self.buffered.try_reserve(1).is_err() { return Err(self.fail(FramingError::Limit, index)); }
            ...
            if complete {
                ...
                if let Err(e) = budget.charge(0) { return Err(self.fail(FramingError::Work(e), index + 1)); }
                let frame = FramedJpeg { basis: self.basis, ordinal, range: [self.start, self.offset],
                    bytes: std::mem::take(&mut self.buffered), digest, markers: self.markers };
                self.completed = ordinal;
                self.start = self.offset;
                self.markers = 0;
                self.scanned = false;
                self.phase = Phase::Start;
                return Ok(StreamStep { consumed: index + 1, frame: Some(frame) });
            }
        }
        if let Err(e) = budget.charge(0) { return Err(self.fail(FramingError::Work(e), input.len())); }
        Ok(StreamStep { consumed: input.len(), frame: None })
    }
    ...
    pub fn finish(&mut self, budget: &mut DecodeBudget<'_>) -> Result<StreamEnd, StreamFailure> {
        ...
        self.closed = true;
        Ok(StreamEnd { basis: self.basis, frames: self.completed, bytes: self.offset })
    }
    ...
    pub fn abort(&mut self) -> DiscardedFragment {
        self.closed = true;
        let fragment = DiscardedFragment { basis: self.basis, byte_range: [self.start, self.offset],
            reason: self.failure, bytes: std::mem::take(&mut self.buffered) };
        self.start = self.offset;
        fragment
    }
    fn error(&self, error: FramingError, consumed: usize) -> StreamFailure {
        StreamFailure { error, consumed, next_offset: self.offset }
    }
    ...
    fn marker(&mut self, code: u8) -> Result<bool, FramingError> {
        ...
        self.markers += 1;
        if self.markers > self.limits.maximum_markers { return Err(FramingError::Limit); }
        if code == 0xd9 {
            return if self.scanned { Ok(true) } else { Err(FramingError::Malformed) };
        }
        if code == 1 { self.phase = Phase::MarkerPrefix; return Ok(false); }
        if code < 0xc0 { return Err(FramingError::UnsupportedMarker); }
    ...
    fn accept(&mut self, byte: u8) -> Result<bool, FramingError> {
        self.phase = match self.phase {
            Phase::Start => {
                if byte != 255 { return Err(FramingError::Malformed); }
                Phase::Soi
            }
            Phase::Soi => {
                if byte != 0xd8 { return Err(FramingError::Malformed); }
                self.markers = 1;
                Phase::MarkerPrefix
            }
            Phase::MarkerPrefix => {
            ...
            Phase::MarkerCode => {
                if byte == 255 { Phase::MarkerCode } else { return self.marker(byte); }
            }
            ...
            Phase::LengthLow(marker, high) => {
                ...
                if remaining == 0 {
                    if scan { self.scanned = true; Phase::Entropy } else { Phase::MarkerPrefix }
                } else { Phase::Payload { remaining, scan } }
            }
            Phase::Payload { remaining, scan } => {
                if remaining == 1 {
                    if scan { self.scanned = true; Phase::Entropy } else { Phase::MarkerPrefix }
                } else { Phase::Payload { remaining: remaining - 1, scan } }
            }
            Phase::Entropy => {
                if byte == 255 { Phase::EntropyCode { fill: false } } else { Phase::Entropy }
            }
