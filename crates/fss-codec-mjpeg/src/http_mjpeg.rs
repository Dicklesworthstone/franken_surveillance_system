...
use crate::multipart::{
    MultipartEnd, MultipartError, MultipartFrame, MultipartLimits, MultipartRemainder,
    MultipartStream,
};
...
impl From<DecodeError> for HttpMjpegError {
    fn from(e: DecodeError) -> Self {
        Self::Work(e)
    }
}
impl From<MultipartError> for HttpMjpegError {
...
impl std::fmt::Display for HttpMjpegError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::BasisMismatch => "HTTP/multipart source mismatch",
            Self::Limit => "HTTP/multipart map limit",
            Self::Multipart(_) => "HTTP/multipart input refused",
            Self::Work(_) => "HTTP/multipart work interrupted",
            Self::Poisoned => "HTTP/multipart has a prior failure",
            Self::Closed => "HTTP/multipart closed",
        })
    }
}
...
#[derive(Debug)]
pub struct HttpJpegFrame {
    head: HttpHeadIdentity,
    part: MultipartFrame,
    spans: Vec<JpegWireSpan>,
}
impl HttpJpegFrame {
    ...
    pub fn decode(
        ...
        self.part.decode(interpretation, limits, budget)
    }
}
...
pub struct HttpMultipartStream {
    ...
    runs: Vec<EntityWireRun>,
    maximum_runs: usize,
    last_wire_end: u64,
    failure: Option<HttpMjpegError>,
    closed: bool,
    pending_frame: Option<MultipartFrame>,
    pending_end: Option<MultipartEnd>,
}
impl std::fmt::Debug for HttpMultipartStream {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HttpMultipartStream")
            .field("entity_offset", &self.next_entity_offset())
...
impl HttpMultipartStream {
    ...
    pub fn new(
        ...
        if !(1..=65536).contains(&maximum_runs) {
            return Err(HttpMjpegError::Limit);
        }
        let multipart =
            MultipartStream::new(head.identity().entity, head.content_type(), limits, budget)?;
        Ok(Self {
            head: head.identity(),
            multipart,
            runs: Vec::new(),
            maximum_runs,
            last_wire_end: head.raw().range()[1],
            failure: None,
            closed: false,
            pending_frame: None,
            pending_end: None,
    ...
    pub fn next_entity_offset(&self) -> u64 {
        self.multipart.next_offset()
    }
    /// First terminal error, without source byte disclosure.
    pub fn failure(&self) -> Option<HttpMjpegError> {
        self.failure
    }
    ...
    pub fn push(
        ...
        let result = (|| {
            ...
            if map.head != self.head
                || start > data.bytes().len()
                || map.entity_range[0] + start as u64 != self.multipart.next_offset()
                || map.wire_range[0] + (start as u64) < self.last_wire_end
            {
                return Err(HttpMjpegError::BasisMismatch);
            }
            if start == data.bytes().len() {
                return Ok(None);
            }
            let entity_start = map.entity_range[0] + start as u64;
            let wire_start = map.wire_range[0] + start as u64;
            let join = self.runs.last().is_some_and(|r| {
                r.wire_range[1] == wire_start
                    && r.entity_range[1] == entity_start
                    && r.chunk == map.chunk
            });
            if !join {
                ...
                self.runs
                    .try_reserve(1)
                    .map_err(|_| HttpMjpegError::Limit)?;
            }
            budget.charge(32)?;
            let step = self
                .multipart
                .push(entity_start, &data.bytes()[start..], budget);
            consumed = match &step {
                Ok(s) => s.consumed,
                Err(e) => e.consumed,
            };
            if consumed != 0 {
                let wire_end = wire_start + consumed as u64;
                let entity_end = entity_start + consumed as u64;
                if join {
                    if let Some(last) = self.runs.last_mut() {
                        last.wire_range[1] = wire_end;
                        last.entity_range[1] = entity_end;
                    }
                } else {
                    self.runs.push(EntityWireRun {
                        wire_range: [wire_start, wire_end],
                        entity_range: [entity_start, entity_end],
                        chunk: map.chunk,
                    });
                }
        ...
        match result {
            Ok(frame) => Ok(HttpMjpegStep { consumed, frame }),
            Err(error) => {
                if self.failure.is_none() {
                    self.failure = Some(error);
                }
                Err(HttpMjpegFailure {
                    error,
                    consumed,
                    next_entity_offset: self.multipart.next_offset(),
                })
            }
        }
    }
    fn active(&self, budget: &mut DecodeBudget<'_>) -> Result<(), HttpMjpegError> {
        ...
        if self.failure.is_some() {
            return Err(HttpMjpegError::Poisoned);
        }
        if self.closed {
            return Err(HttpMjpegError::Closed);
        }
        Ok(())
    }
    fn publish_frame(
        &mut self,
        budget: &mut DecodeBudget<'_>,
    ) -> Result<Option<HttpJpegFrame>, HttpMjpegError> {
        let Some(part) = self.pending_frame.as_ref() else {
            return Ok(None);
        };
        let receipt = part.receipt();
        let [begin, end] = receipt.jpeg_range;
        let mut covered = begin;
        ...
        for run in &self.runs {
            let lo = run.entity_range[0].max(begin);
            let hi = run.entity_range[1].min(end);
            if lo >= hi {
                continue;
            }
            if lo != covered {
            ...
            spans.push(JpegWireSpan {
                wire_range: [wire, wire + hi - lo],
                jpeg_range: [lo - begin, hi - begin],
                chunk: run.chunk,
            });
            covered = hi;
        }
        if covered != end || spans.is_empty() {
            return Err(HttpMjpegError::BasisMismatch);
        }
        budget.charge(0)?;
        let Some(part) = self.pending_frame.take() else {
            return Err(HttpMjpegError::BasisMismatch);
        };
        ...
        self.runs
            .retain(|r| r.entity_range[1] > receipt.closing_range[0]);
        Ok(Some(HttpJpegFrame {
            head: self.head,
            part,
            spans,
        }))
    ...
    pub fn finish(
        ...
        budget: &mut DecodeBudget<'_>,
    ) -> Result<HttpMjpegEnd, HttpMjpegError> {
        let result = (|| {
            self.active(budget)?;
            if end.head != self.head
                || end.entity_bytes != self.multipart.next_offset()
                || end.wire_bytes < self.last_wire_end
            {
                return Err(HttpMjpegError::BasisMismatch);
            }
            let finish = self
                .multipart
                .finish(budget)
                .map_err(|e| HttpMjpegError::Multipart(e.error))?;
            self.pending_frame = finish.frame;
            self.pending_end = Some(finish.end);
            let final_frame = self.publish_frame(budget)?;
            // No fallible work follows removal of retained final-frame ownership.
            let multipart = self
                .pending_end
                .take()
                .ok_or(HttpMjpegError::BasisMismatch)?;
            self.closed = true;
            Ok(HttpMjpegEnd {
                http: end,
                multipart,
    ...
    pub fn abort(self) -> HttpMjpegRemainder {
        HttpMjpegRemainder {
            head: self.head,
            reason: self.failure,
            multipart: self.multipart.abort(),
