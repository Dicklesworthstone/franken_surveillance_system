...
fn trim(bytes: &[u8], mut range: Range<usize>) -> Range<usize> {
    while range.start < range.end && b" \t".contains(&bytes[range.start]) {
        range.start += 1;
    }
    while range.start < range.end && b" \t".contains(&bytes[range.end - 1]) {
        range.end -= 1;
    }
    range
}
/// One parsed header field: whole-line span and its value span, in wire order.
type FieldSpans = (Range<usize>, Range<usize>);
fn field_ranges(bytes: &[u8], start: usize) -> Result<Vec<FieldSpans>, HttpError> {
    ...
    while offset < bytes.len() {
        ...
        fields.push((
            offset..offset + colon,
            trim(bytes, offset + colon + 1..offset + n),
        ));
        offset += n + 2;
    }
    Err(HttpError::Malformed)
...
pub(super) fn head(bytes: &[u8]) -> Result<(BodyFraming, Range<usize>), HttpError> {
    ...
    if line.len() < 13
        ...
        || line[13..].iter().any(|b| (*b < 32 && *b != 9) || *b == 127)
    {
        return Err(HttpError::Malformed);
    }
    if &line[..8] != b"HTTP/1.1" && &line[..8] != b"HTTP/1.0" {
    ...
    for (name, value) in field_ranges(bytes, end + 2)? {
        ...
        if key.eq_ignore_ascii_case(b"content-length") {
            ...
            once(&mut content, value)?;
        } else if key.eq_ignore_ascii_case(b"content-encoding") {
            if !v.eq_ignore_ascii_case(b"identity") {
                return Err(HttpError::Unsupported);
            }
            once(&mut encoding, ())?;
            ...
            for part in v.split(|b| *b == b',') {
                ...
                if part.is_empty() || !part.iter().all(|b| token(*b)) {
                    return Err(HttpError::Malformed);
                }
                if forbidden_trailer(part) {
                    return Err(HttpError::Malformed);
                }
            }
        }
    }
    if transfer.is_some() && (length.is_some() || &line[..8] == b"HTTP/1.0") {
        return Err(HttpError::Malformed);
    }
    Ok((
        if transfer.is_some() {
...
pub(super) fn trailers(bytes: &[u8]) -> Result<(), HttpError> {
    ...
    for (i, (name, _)) in fields.iter().enumerate() {
        if forbidden_trailer(&bytes[name.clone()])
            || fields[..i]
                .iter()
                .any(|(other, _)| bytes[name.clone()].eq_ignore_ascii_case(&bytes[other.clone()]))
        {
            return Err(HttpError::Malformed);
        }
    }
    ...
    while i < line.len() {
        ...
        if line.get(i) == Some(&b'"') {
            ...
            while i < line.len() {
                ...
                if b == b'\\' {
                    let b = *line.get(i).ok_or(HttpError::Malformed)?;
                    i += 1;
                    if b != 9 && !(32..=126).contains(&b) && b < 128 {
                        return Err(HttpError::Malformed);
                    }
                } else if b != 9
                    && b != 32
...
fn skip_ows(line: &[u8], i: &mut usize) {
