...
impl Default for HttpLimits {
    fn default() -> Self { Self { header_bytes:32768, fragment_bytes:16384,
        chunk_bytes:16*1024*1024, entity_bytes:1<<40, wire_bytes:1<<42, chunks:1_000_000 } }
}
...
impl From<DecodeError> for HttpError { fn from(e:DecodeError)->Self {Self::Work(e)} }
impl std::fmt::Display for HttpError {
    fn fmt(&self,f:&mut std::fmt::Formatter<'_>)->std::fmt::Result {
        f.write_str(match self { Self::Configuration=>"invalid HTTP response configuration",
            Self::Malformed=>"malformed HTTP response framing",Self::Unsupported=>"unsupported HTTP response",
            Self::Status(_)=>"HTTP response status is not admitted",Self::Offset=>"HTTP response offset mismatch",
            Self::Limit=>"HTTP response limit",Self::Truncated=>"incomplete HTTP response",
            Self::Closed=>"HTTP response closed",Self::Poisoned=>"HTTP response has a prior failure",
...
pub struct WireSpan { start:u64, bytes:Vec<u8> }
impl WireSpan {
...
impl std::fmt::Debug for WireSpan {
    fn fmt(&self,f:&mut std::fmt::Formatter<'_>)->std::fmt::Result {
        f.debug_struct("WireSpan").field("range",&self.range()).finish_non_exhaustive()
    }
}
...
pub struct ResponseHead { identity:HttpHeadIdentity, raw:WireSpan, content_type:std::ops::Range<usize> }
impl ResponseHead {
    /// Frozen basis and declared transfer framing.
    pub fn identity(&self)->HttpHeadIdentity {self.identity}
    ...
    pub fn raw(&self)->&WireSpan {&self.raw}
}
impl std::fmt::Debug for ResponseHead {
    fn fmt(&self,f:&mut std::fmt::Formatter<'_>)->std::fmt::Result {
        f.debug_struct("ResponseHead").field("identity",&self.identity).finish_non_exhaustive()
    }
}
...
#[derive(Debug)]
pub struct EntityData { mapping:EntityMapping, raw:WireSpan }
impl EntityData {
    /// Exact wire/entity correspondence, including chunk identity.
    pub fn mapping(&self)->EntityMapping {self.mapping}
    /// Bytes ready for the existing multipart parser, still source-owned here.
    pub fn bytes(&self)->&[u8] {self.raw.bytes()}
...
pub enum ControlKind {
    ...
    ChunkHeader { ordinal:u64, length:u64 },
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum Phase { Head, Fixed(u64), UntilEof, Size, Chunk(u64), ChunkEnd, Trailers, Complete, Closed }
...
pub struct HttpResponseStream {
    basis:StreamBasis, limits:HttpLimits, phase:Phase, offset:u64, entity_offset:u64,
    buffer:Vec<u8>, buffer_start:u64, line_start:usize, chunks:u64,
    head:Option<HttpHeadIdentity>, failure:Option<HttpError>,
}
impl std::fmt::Debug for HttpResponseStream {
    fn fmt(&self,f:&mut std::fmt::Formatter<'_>)->std::fmt::Result {
        f.debug_struct("HttpResponseStream").field("offset",&self.offset).field("failure",&self.failure).finish_non_exhaustive()
    }
}
impl HttpResponseStream {
    ...
    pub fn new(basis:StreamBasis,limits:HttpLimits)->Result<Self,HttpError> {
        if basis.source==[0;32] || basis.generation==0 || !(4..=32768).contains(&limits.header_bytes)
            || !(1..=65536).contains(&limits.fragment_bytes) || !(1..=16*1024*1024).contains(&limits.chunk_bytes)
            || !(1..=1<<40).contains(&limits.entity_bytes) || !(1..=1<<42).contains(&limits.wire_bytes)
            || !(1..=1_000_000).contains(&limits.chunks) {return Err(HttpError::Configuration);}
        Ok(Self{basis,limits,phase:Phase::Head,offset:0,entity_offset:0,buffer:Vec::new(),
            buffer_start:0,line_start:0,chunks:0,head:None,failure:None})
    }
    ...
    pub fn entity_offset(&self)->u64 {self.entity_offset}
    /// True only after explicit framing; close-delimited streams need EOF.
    pub fn body_complete(&self)->bool {self.phase==Phase::Complete}
    /// First latched failure, including cancellation.
    pub fn failure(&self)->Option<HttpError> {self.failure}
    /// Consume through the next header/control/data record. Retain the input suffix.
    pub fn push(&mut self,offset:u64,bytes:&[u8],budget:&mut DecodeBudget<'_>)->Result<HttpStep,HttpFailure> {
        let mut consumed=0;
        let result=self.advance(offset,bytes,&mut consumed,budget);
        match result {Ok(event)=>Ok(HttpStep{consumed,event}),Err(error)=>{
            if self.failure.is_none() {self.failure=Some(error);}
            Err(HttpFailure{error,consumed,next_offset:self.offset})
        }}
    }
    fn advance(&mut self,offset:u64,bytes:&[u8],consumed:&mut usize,budget:&mut DecodeBudget<'_>)
        ->Result<Option<HttpEvent>,HttpError> {
        budget.charge(0)?;
        if self.failure.is_some() {return Err(HttpError::Poisoned);}
        if offset!=self.offset {return Err(HttpError::Offset);}
        if matches!(self.phase,Phase::Complete|Phase::Closed) {return Err(HttpError::Closed);}
        while *consumed<bytes.len() {
            if let Phase::Fixed(remaining)|Phase::Chunk(remaining)=self.phase {
                let chunk=matches!(self.phase,Phase::Chunk(_));
                let n=(bytes.len()-*consumed).min(self.limits.fragment_bytes).min(remaining.min(usize::MAX as u64) as usize);
                return self.emit_data(&bytes[*consumed..*consumed+n],consumed,Some((remaining,chunk)),budget).map(Some);
            }
            if self.phase==Phase::UntilEof {
                let n=(bytes.len()-*consumed).min(self.limits.fragment_bytes);
                return self.emit_data(&bytes[*consumed..*consumed+n],consumed,None,budget).map(Some);
            }
            budget.charge(4)?;
            if self.offset==self.limits.wire_bytes {return Err(HttpError::Limit);}
            let limit=if matches!(self.phase,Phase::Size|Phase::ChunkEnd) {4096} else {self.limits.header_bytes};
            if self.buffer.len()==limit {return Err(HttpError::Limit);}
            self.buffer.try_reserve(1).map_err(|_|HttpError::Limit)?;
            let b=bytes[*consumed];self.buffer.push(b);self.offset+=1;*consumed+=1;
            let len=self.buffer.len();
            if self.phase==Phase::ChunkEnd {
                if self.buffer.as_slice()!=b"\r" && self.buffer.as_slice()!=b"\r\n" {return Err(HttpError::Malformed);}
                if len==2 {budget.charge(0)?;self.phase=Phase::Size;
                    return Ok(Some(HttpEvent::Control(HttpControl{kind:ControlKind::ChunkEnd,raw:self.take_buffer()})));}
                continue;
            }
            if (b==b'\n' && (len<2 || self.buffer[len-2]!=b'\r'))
                || (len>=2 && self.buffer[len-2]==b'\r' && b!=b'\n') {return Err(HttpError::Malformed);}
            if len-self.line_start>4096 {return Err(HttpError::Limit);}
            if b!=b'\n' {continue;}
            let blank=len-self.line_start==2;self.line_start=len;
            if self.phase==Phase::Size {
                budget.charge(len as u64*4)?;
                let length=fields::chunk_size(&self.buffer)?;
                if length>self.limits.chunk_bytes || length>self.limits.entity_bytes-self.entity_offset
                    || (length!=0 && self.chunks==self.limits.chunks) {return Err(HttpError::Limit);}
                budget.charge(0)?;
                if length!=0 {self.chunks+=1;}
                self.phase=if length==0 {Phase::Trailers} else {Phase::Chunk(length)};
                let ordinal=if length==0 {self.chunks+1} else {self.chunks};
                return Ok(Some(HttpEvent::Control(HttpControl{kind:ControlKind::ChunkHeader{ordinal,length},raw:self.take_buffer()})));
            }
            if !blank {continue;}
            budget.charge(len as u64*4)?;
            if self.phase==Phase::Head {
                let (framing,content_type)=fields::head(&self.buffer)?;
                if matches!(framing,BodyFraming::Length(n) if n>self.limits.entity_bytes) {return Err(HttpError::Limit);}
                let header_sha256=ContentDigest::sha256(&self.buffer).bytes();
                let mut id=[0_u8;100];id[..28].copy_from_slice(b"fss/http-mjpeg-entity/ref/1\0");
                ...
                let identity=HttpHeadIdentity{wire:self.basis,entity:StreamBasis{
                    source:ContentDigest::sha256(&id).bytes(),generation:self.basis.generation},header_sha256,framing};
                budget.charge(0)?;self.head=Some(identity);
                self.phase=match framing {BodyFraming::Length(0)=>Phase::Complete,BodyFraming::Length(n)=>Phase::Fixed(n),
                    BodyFraming::Chunked=>Phase::Size,BodyFraming::UntilEof=>Phase::UntilEof};
                return Ok(Some(HttpEvent::Head(ResponseHead{identity,content_type,raw:self.take_buffer()})));
            }
            fields::trailers(&self.buffer)?;budget.charge(0)?;self.phase=Phase::Complete;
            return Ok(Some(HttpEvent::Control(HttpControl{kind:ControlKind::Trailers,raw:self.take_buffer()})));
        }
        Ok(None)
    }
    fn emit_data(&mut self,input:&[u8],consumed:&mut usize,remaining:Option<(u64,bool)>,budget:&mut DecodeBudget<'_>)
        ->Result<HttpEvent,HttpError> {
        let n=input.len() as u64;
        if n>self.limits.entity_bytes-self.entity_offset || n>self.limits.wire_bytes-self.offset {return Err(HttpError::Limit);}
        budget.charge(n*3)?;
        let head=self.head.ok_or(HttpError::Malformed)?;
        let mut bytes=Vec::new();bytes.try_reserve_exact(input.len()).map_err(|_|HttpError::Limit)?;bytes.extend_from_slice(input);
        let mapping=EntityMapping{head,wire_range:[self.offset,self.offset+n],entity_range:[self.entity_offset,self.entity_offset+n],
            chunk:remaining.filter(|(_,chunk)|*chunk).map(|_|self.chunks),sha256:ContentDigest::sha256(input).bytes()};
        budget.charge(0)?;
        let raw=WireSpan{start:self.offset,bytes};self.offset+=n;self.entity_offset+=n;*consumed+=input.len();
        self.buffer_start=self.offset;
        if let Some((left,chunk))=remaining {self.phase=match (left-n,chunk) {
            (0,true)=>Phase::ChunkEnd,(0,false)=>Phase::Complete,(n,true)=>Phase::Chunk(n),(n,false)=>Phase::Fixed(n)};}
        Ok(HttpEvent::Data(EntityData{mapping,raw}))
    }
    fn take_buffer(&mut self)->WireSpan {
        let span=WireSpan{start:self.buffer_start,bytes:std::mem::take(&mut self.buffer)};
        self.buffer_start=self.offset;self.line_start=0;span
    }
    ...
    pub fn finish(&mut self,budget:&mut DecodeBudget<'_>)->Result<HttpEnd,HttpError> {
        let result=(||{budget.charge(0)?;
            if self.failure.is_some() {return Err(HttpError::Poisoned);}
            let termination=match self.phase {Phase::Complete=>HttpTermination::ExplicitFraming,
                Phase::UntilEof=>HttpTermination::CloseDelimitedEof,Phase::Closed=>return Err(HttpError::Closed),
                _=>return Err(HttpError::Truncated)};
            let head=self.head.ok_or(HttpError::Truncated)?;
            self.phase=Phase::Closed;Ok(HttpEnd{head,wire_bytes:self.offset,entity_bytes:self.entity_offset,chunks:self.chunks,termination})})();
    ...
    pub fn abort(self)->HttpRemainder {HttpRemainder{basis:self.basis,reason:self.failure,next_offset:self.offset,
        pending:WireSpan{start:self.buffer_start,bytes:self.buffer}}}
}
