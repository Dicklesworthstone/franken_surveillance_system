...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 3 {
        return Err("usage: decode_stream INPUT SHA256 grayscale|ycbcr".into());
    }
    ...
    if bytes.len() > maximum {
        return Err("input exceeds 64 MiB harness limit".into());
    }
    ...
    while offset < bytes.len() {
        ...
        if let Some(frame) = step.frame {
            if frame.ordinal() > 1024 {
                return Err("input exceeds 1024-frame harness limit".into());
            }
            let decoded = frame.decode(color, DecodeLimits::default(), &mut budget)?;
            let receipt = decoded.receipt();
            let encoded = hex(frame.encoded_sha256());
            let luma = hex(receipt.luma_sha256);
            println!(
    ...
    println!(
        "{{\"kind\":\"complete\",\"frames\":{},\"bytes\":{},\"work\":{}}}",
        end.frames,
        end.bytes,
        budget.used()
    );
