#![forbid(unsafe_code)]
//! Multipart JPEG split contracts: boundary framing, truncation, and reassembly identity.
...
fn collect(
    data: &[u8],
    chunk: usize,
) -> Result<(Vec<MultipartFrame>, MultipartEnd), Box<dyn std::error::Error>> {
    let mut p = parser()?;
...
fn length_and_delimiter_parts_decode_and_preserve_source_ranges() -> Test {
    for lengths in [false, true] {
        let data = entity(&[JPEG, JPEG], lengths);
        let (frames, end) = collect(&data, 7)?;
        assert_eq!(frames.len(), 2);
        assert_eq!(end.frames, 2);
        assert_eq!(end.bytes, data.len() as u64);
        for (i, frame) in frames.iter().enumerate() {
            let r = frame.receipt();
            assert_eq!(r.ordinal, i as u64 + 1);
            assert_eq!(r.basis, basis());
            assert_eq!(
                r.content_type_sha256,
                ContentDigest::sha256(TYPE.as_bytes()).bytes()
            );
            assert_eq!(frame.bytes(), JPEG);
            assert_eq!(
                &data[r.jpeg_range[0] as usize..r.jpeg_range[1] as usize],
                JPEG
            );
            assert_eq!(
                &data[r.headers_range[0] as usize..r.headers_range[1] as usize],
                frame.headers()
            );
            assert_eq!(
                &data[r.opening_range[0] as usize..r.opening_range[1] as usize],
                frame.opening_bytes()
            );
            assert_eq!(
                &data[r.closing_range[0] as usize..r.closing_range[1] as usize],
                frame.closing_bytes()
            );
            assert_eq!(r.declared_length, lengths.then_some(JPEG.len()));
            assert_eq!(r.closes_entity, i == 1);
            assert_eq!(
                frame
                    .decode(
        ...
        assert_eq!(
            frames[0].receipt().closing_range,
            frames[1].receipt().opening_range
        );
    }
    Ok(())
}
...
fn every_two_chunk_split_preserves_exact_parts() -> Test {
    ...
    for split in 0..=data.len() {
        let mut p = parser()?;
        let mut b = DecodeBudget::new(10_000_000);
        let mut parts = Vec::new();
        let mut at = 0;
        for stop in [split, data.len()] {
            while at < stop {
                let step = p.push(at as u64, &data[at..stop], &mut b)?;
                assert!(step.consumed > 0);
                at += step.consumed;
                if let Some(frame) = step.frame {
                    parts.push(frame);
...
fn missing_final_crlf_is_accepted_only_at_explicit_eof() -> Test {
    let mut data = entity(&[JPEG], false);
    data.truncate(data.len() - 2);
    let mut p = parser()?;
    let mut b = DecodeBudget::new(1_000_000);
    let step = p.push(0, &data, &mut b)?;
    assert!(step.frame.is_none());
    assert_eq!(p.completed_frames(), 0);
    let end = p.finish(&mut b)?;
    assert_eq!(end.frame.ok_or("missing final part")?.bytes(), JPEG);
    assert_eq!(end.end.frames, 1);
    assert_eq!(
        p.finish(&mut b).err().ok_or("repeated EOF")?.error,
        MultipartError::Closed
    );
    Ok(())
}
#[test]
fn all_prefixes_before_a_complete_closing_delimiter_fail() {
    ...
    for end in 0..data.len() - 2 {
        assert!(collect(&data[..end], 3).is_err(), "prefix {end}");
    }
}
#[test]
fn preamble_epilogue_and_boundary_padding_are_retained() -> Test {
    ...
    let (frames, end) = collect(&data, 1)?;
    assert_eq!(frames.len(), 1);
    assert_eq!(end.preamble.bytes(), b"owner-selected preamble");
    assert_eq!(end.epilogue.bytes(), b"epilogue");
...
fn jpeg_marker_like_metadata_is_not_a_mime_boundary() -> Test {
    ...
    let mut image = vec![255, 216, 255, 225];
    image.extend_from_slice(&((payload.len() + 2) as u16).to_be_bytes());
    image.extend_from_slice(payload);
    image.extend_from_slice(&JPEG[2..]);
    let data = entity(&[&image], false);
    let (parts, _) = collect(&data, 1)?;
...
fn boundary_configuration_never_guesses_or_strips_prefixes() -> Test {
    for value in [
        ...
        "multipart/x-mixed-replace; boundary=camera:7",
    ...
    let mut p = MultipartStream::new(
        basis(),
        "MULTIPART/X-MIXED-REPLACE; BOUNDARY=--real",
        MultipartLimits::default(),
        &mut DecodeBudget::new(10000),
    )?;
    let mut data = b"----real\r\nContent-Type: image/jpeg\r\n\r\n".to_vec();
    ...
    data.extend_from_slice(b"\r\n----real--\r\n");
    assert_eq!(
        p.push(0, &data, &mut DecodeBudget::new(1_000_000))?
            .frame
            .ok_or("missing part")?
            .bytes(),
        JPEG
...
fn raw_part_is_not_an_entropy_validation_claim() -> Test {
    let jpeg = [255, 216, 255, 217];
    let (parts, _) = collect(&entity(&[&jpeg], true), 5)?;
    assert!(
        parts[0]
            .decode(
                ComponentInterpretation::Grayscale,
...
fn offset_failure_latches_and_retains_exact_unpublished_bytes() -> Test {
    let data = entity(&[JPEG], true);
    let mut p = parser()?;
    let mut b = DecodeBudget::new(1_000_000);
    p.push(0, &data[..120], &mut b)?;
    let failure = p
        .push(119, &data[120..], &mut b)
        .err()
        .ok_or("offset accepted")?;
    assert_eq!(
        (failure.error, failure.consumed, failure.next_offset),
        (MultipartError::Offset, 0, 120)
    );
    assert_eq!(
        p.push(120, &data[120..], &mut b)
            .err()
            .ok_or("resumed")?
            .error,
        MultipartError::Poisoned
    );
    let recovery = p.abort();
    let mut spans: Vec<_> = recovery
        .spans
        .iter()
        .filter(|s| !s.bytes().is_empty())
        .collect();
    spans.sort_by_key(|s| s.range()[0]);
    let mut reconstructed = Vec::new();
    for span in spans {
        assert_eq!(span.range()[0], reconstructed.len() as u64);
        reconstructed.extend_from_slice(span.bytes());
    }
    assert_eq!(reconstructed, &data[..120]);
    Ok(())
}
#[test]
fn cancellation_and_work_exhaustion_do_not_emit_a_part() -> Test {
    ...
    let e = p
        .push(0, &data, &mut DecodeBudget::cancellable(1_000_000, &flag))
        .err()
        .ok_or("cancel ignored")?;
    assert_eq!(
        (e.error, e.consumed),
        (MultipartError::Work(DecodeError::Cancelled), 0)
    );
    let mut p = parser()?;
    let e = p
        .push(0, &data, &mut DecodeBudget::new(16))
        .err()
        .ok_or("work ignored")?;
    assert_eq!(
        (e.error, e.consumed),
        (MultipartError::Work(DecodeError::BudgetExhausted), 2)
    );
    assert_eq!(p.abort().spans[3].bytes(), &data[..2]);
    Ok(())
}
#[test]
fn header_and_wrapper_ceilings_fail_without_silent_omission() -> Test {
    ...
    p.push(0, &data, &mut DecodeBudget::new(1_000_000))?;
    assert!(
        p.push(data.len() as u64, b"x", &mut DecodeBudget::new(1000))
            .is_err()
    );
    assert!(p.finish(&mut DecodeBudget::new(1000)).is_err());
    Ok(())
}
#[test]
fn maximum_boundary_padding_fits_exact_payload_ceiling() -> Test {
    let name = "b".repeat(70);
    let ct = format!("multipart/x-mixed-replace; boundary={name}");
    let mut data = format!(
        "--{name}\r\nContent-Type: image/jpeg\r\nContent-Length: {}\r\n\r\n",
        JPEG.len()
    )
    ...
    assert_eq!(
        p.push(0, &data, &mut DecodeBudget::new(10_000_000))?
            .frame
            .ok_or("maximum delimiter lost")?
            .bytes(),
        JPEG
    );
    Ok(())
...
fn stop_after_one_part_preserves_unread_suffix_and_later_errors() -> Test {
    let first = entity(&[JPEG, JPEG], true);
    let mut data = first.clone();
    let at = data
        .windows(2)
        .rposition(|w| w == [255, 217])
        .ok_or("missing EOI")?;
    data[at + 1] = 0;
    let mut p = parser()?;
    let mut b = DecodeBudget::new(1_000_000);
    let one = p.push(0, &data, &mut b)?;
    assert!(one.frame.is_some());
    assert!(one.consumed < data.len());
    assert!(
        p.push(one.consumed as u64, &data[one.consumed..], &mut b)
            .is_err()
    );
    assert_eq!(p.completed_frames(), 1);
    assert!(p.finish(&mut b).is_err());
    Ok(())
}
