#![forbid(unsafe_code)]
//! Streaming JPEG contracts: incremental feeds, budget accounting, and decoder identity.
...
fn new() -> Result<JpegStream, FramingError> {
    JpegStream::new(basis(), FramingLimits::default())
}
...
fn concatenated_real_images_keep_exact_source_ranges_and_decode() -> Test {
    ...
    for (i, (frame, encoded)) in frames.iter().zip([GRAY, COLOR]).enumerate() {
        ...
        assert_eq!(frame.ordinal(), i as u64 + 1);
        assert_eq!(frame.bytes(), encoded);
        assert_eq!(
            frame.encoded_sha256(),
            ContentDigest::sha256(encoded).bytes()
        );
    ...
    assert_eq!(
        frames[1].byte_range(),
        [GRAY.len() as u64, data.len() as u64]
    );
    Ok(())
}
#[test]
fn every_two_chunk_split_has_identical_bytes_and_offsets() -> Test {
    ...
    for split in 0..=data.len() {
        ...
        let frame = if let Some(frame) = first.frame {
            frame
        } else {
            stream
                .push(split as u64, &data[split..], &mut budget)?
                .frame
                .ok_or("missing frame")?
        };
        assert_eq!(frame.bytes(), data);
        assert_eq!(frame.byte_range(), [0, data.len() as u64]);
...
fn bytewise_input_and_restart_splits_do_not_create_extra_frames() -> Test {
    ...
    for chunk in 1..=23 {
        ...
        assert_eq!(frames[1].bytes(), COLOR);
...
fn embedded_eoi_soi_and_sos_in_metadata_are_not_boundaries() -> Test {
    let data = metadata_frame();
    let frames = collect(&data, 1)?;
    assert_eq!(frames.len(), 1);
    assert_eq!(frames[0].bytes(), data);
    let decoded = frames[0].decode(
        Color::Grayscale,
        DecodeLimits::default(),
...
fn stops_before_next_frame_or_bad_suffix_without_dropping_it() -> Test {
    ...
    assert_eq!(first.consumed, GRAY.len());
    assert!(first.frame.is_some());
    let failure = stream
        .push(first.consumed as u64, &data[first.consumed..], &mut budget)
        .err()
        .ok_or("accepted bad suffix")?;
    assert_eq!(failure.error, FramingError::Malformed);
    assert_eq!(failure.consumed, 1);
    assert_eq!(stream.completed_frames(), 1);
    assert!(stream.finish(&mut budget).is_err());
    let discarded = stream.abort();
    assert_eq!(discarded.bytes(), &[1]);
    assert_eq!(
        discarded.byte_range,
        [GRAY.len() as u64, GRAY.len() as u64 + 1]
    );
    Ok(())
}
#[test]
fn every_incomplete_prefix_is_not_clean_eof() -> Test {
    for end in 1..GRAY.len() {
        ...
        let failure = stream
            .finish(&mut budget)
            .err()
            .ok_or("partial frame accepted")?;
        assert_eq!(failure.error, FramingError::Truncated);
        let fragment = stream.abort();
        assert_eq!(fragment.bytes(), &GRAY[..end]);
...
fn input_offset_mismatch_is_terminal_and_does_not_read_bytes() -> Test {
    ...
    let e = stream
        .push(19, &GRAY[20..], &mut budget)
        .err()
        .ok_or("replay accepted")?;
    assert_eq!(
        (e.error, e.consumed, e.next_offset),
        (FramingError::OffsetMismatch, 0, 20)
    );
    assert_eq!(
        stream
            .push(20, &GRAY[20..], &mut budget)
            .err()
            .ok_or("resumed")?
            .error,
        FramingError::Poisoned
    );
    assert_eq!(stream.abort().bytes(), &GRAY[..20]);
    Ok(())
}
#[test]
fn invalid_markers_and_lengths_never_resynchronize_silently() -> Test {
    for bad in [
        ...
        assert_eq!(stream.completed_frames(), 0);
...
fn malformed_entropy_fill_cannot_hide_stuffing() -> Test {
    ...
    assert!(matches!(
        stream
            .push(0, bytes, &mut DecodeBudget::new(1000))
            .err()
            .map(|e| e.error),
...
fn frame_and_marker_ceilings_refuse_whole_result() -> Test {
    for limits in [
        FramingLimits {
            maximum_frame_bytes: GRAY.len() - 1,
        ...
        let failure = stream
            .push(0, GRAY, &mut DecodeBudget::new(1_000_000))
            .err()
            .ok_or("limit ignored")?;
        assert_eq!(failure.error, FramingError::Limit);
        assert_eq!(stream.completed_frames(), 0);
...
fn budget_failure_reports_consumed_prefix_and_abort_needs_no_budget() -> Test {
    let mut stream = new()?;
    let mut budget = DecodeBudget::new(40);
    let failure = stream
        .push(0, GRAY, &mut budget)
        .err()
        .ok_or("budget ignored")?;
    assert_eq!(
        failure.error,
        FramingError::Work(DecodeError::BudgetExhausted)
    );
    assert_eq!((failure.consumed, failure.next_offset), (10, 10));
    let fragment = stream.abort();
    assert_eq!(fragment.bytes(), &GRAY[..10]);
    assert!(stream.abort().bytes().is_empty());
    Ok(())
}
#[test]
fn hash_budget_failure_does_not_emit_complete_but_unpublished_frame() -> Test {
    let mut stream = new()?;
    let mut budget = DecodeBudget::new(GRAY.len() as u64 * 4);
    let failure = stream
        .push(0, GRAY, &mut budget)
        .err()
        .ok_or("hash work omitted")?;
    assert_eq!(failure.consumed, GRAY.len());
    assert_eq!(stream.completed_frames(), 0);
    assert_eq!(stream.abort().bytes(), GRAY);
    Ok(())
}
#[test]
fn cancellation_preserves_partial_source_without_emitting_a_frame() -> Test {
    let mut stream = new()?;
    stream.push(0, &GRAY[..20], &mut DecodeBudget::new(1000))?;
    let flag = AtomicBool::new(true);
    let mut budget = DecodeBudget::cancellable(1_000_000, &flag);
    let e = stream
        .push(20, &GRAY[20..], &mut budget)
        .err()
        .ok_or("cancellation ignored")?;
    assert_eq!(e.error, FramingError::Work(DecodeError::Cancelled));
...
fn empty_input_is_not_eof_and_closed_stream_cannot_resume() -> Test {
    ...
    let step = stream.push(0, &[], &mut budget)?;
