...
use fss_codec_mjpeg::{
    ComponentInterpretation as Color, DecodeBudget, DecodeError, DecodeLimits, decode_luma,
};
...
fn grayscale_is_exact_luma_replication_with_distinct_receipt() -> Test {
    ...
    for (channels, &y) in rgb.pixels().as_chunks::<3>().0.iter().zip(luma.pixels()) {
        assert_eq!(*channels, [y; 3]);
    }
    assert_eq!(rgb.dimensions(), luma.dimensions());
    assert_eq!(rgb.receipt().entropy_blocks, luma.receipt().entropy_blocks);
    assert_ne!(rgb.receipt().decoder, luma.receipt().decoder);
    Ok(())
}
#[test]
fn every_sampling_mode_and_scan_order_reconstructs_color_at_odd_edges() -> Test {
    for sampling in [[1, 1], [2, 1], [2, 2]] {
        for reversed in [false, true] {
            let bytes =
                rgb_support::jpeg(19, 17, sampling, false, false, reversed, &[[100, 150, 200]]);
            let image = decode(&bytes, Color::YCbCr)?;
            assert_eq!(image.dimensions(), [19, 17]);
            assert_eq!(image.pixels().len(), 19 * 17 * 3);
            for pixel in image.pixels().as_chunks::<3>().0 {
                assert_eq!(*pixel, [201, 41, 139]);
            }
            assert_eq!(
                image.receipt().entropy_blocks,
                image.receipt().mcus * (sampling[0] * sampling[1] + 2)
            );
        }
    }
    Ok(())
...
fn chroma_is_real_not_luma_replicated_into_three_channels() -> Test {
    ...
    assert_ne!(
        decode(&a, Color::YCbCr)?.pixels(),
        decode(&b, Color::YCbCr)?.pixels()
    );
    Ok(())
}
#[test]
fn mcu_chroma_does_not_bleed_or_shift_into_adjacent_tiles() -> Test {
    ...
    for y in 0..17 {
        for x in 0..19 {
            ...
            assert_eq!(
                &image.pixels()[(y * 19 + x) * 3..(y * 19 + x + 1) * 3],
                expected
            );
        }
    }
...
fn complete_source_digest_and_rgb_digest_are_checked_and_reproducible() -> Test {
    ...
    let a = decode(&bytes, Color::YCbCr)?;
    let b = decode(&bytes, Color::YCbCr)?;
    assert_eq!(a.receipt(), b.receipt());
    assert_eq!(a.receipt().decoder, rgb_decoder_identity());
    assert_eq!(
        a.receipt().rgb_sha256,
        ContentDigest::sha256(a.pixels()).bytes()
    );
    assert_eq!(
        a.receipt().encoded_sha256,
        ContentDigest::sha256(&bytes).bytes()
    );
    assert!(matches!(
        decode_rgb(
...
fn invalid_suffix_truncated_chroma_and_interpretation_refuse_whole_frame() -> Test {
    ...
    for end in (bytes.len() - 7)..bytes.len() {
        assert!(decode(&bytes[..end], Color::YCbCr).is_err());
    }
    let mut extra = bytes.clone();
    extra.push(0);
    assert!(matches!(
        decode(&extra, Color::YCbCr),
    ...
    let at = progressive
        .windows(2)
        .position(|w| w == [255, 192])
        .ok_or("missing SOF")?;
    progressive[at + 1] = 194;
    assert!(matches!(
        decode(&progressive, Color::YCbCr),
        Err(DecodeError::Unsupported)
    ));
    Ok(())
}
...
fn independent_output_and_pixel_limits_refuse_without_silent_resize() -> Test {
    ...
    let limits = RgbDecodeLimits {
        frame: DecodeLimits {
            maximum_pixels: 19 * 17 - 1,
            ..DecodeLimits::default()
        },
        ..RgbDecodeLimits::default()
    };
...
fn exact_budget_completes_while_earlier_cuts_and_cancellation_return_no_image() -> Test {
    ...
    let hash = ContentDigest::sha256(&bytes).bytes();
    let limits = RgbDecodeLimits::default();
    ...
    assert_eq!(
        decode_rgb(
            ...
            Color::YCbCr,
            limits,
            &mut DecodeBudget::new(full.used())
        )?
        .receipt(),
        expected.receipt()
    );
    ...
    assert!(matches!(
        decode_rgb(
            ...
            Color::YCbCr,
            limits,
            &mut DecodeBudget::cancellable(100_000_000, &flag)
        ),
