        ...
        for &component in &order {
            for _ in 0..if component == 0 { h * v } else { 1 } {
                ...
                let encoded = if difference < 0 {
                    (difference + (1_i32 << size) - 1) as u32
                } else {
                    magnitude
                };
                bits(&mut pending, encoded, size);
                bits(&mut pending, 0, 1); // AC EOB
            }
        }
    }
    flush(&mut result, &mut pending);
    result.extend_from_slice(&[255, 217]);
    result
}
