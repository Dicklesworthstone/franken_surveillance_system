...
use common::{BASELINE, Error, HIGH, KEY, groups, timed};
        ...
        for mapping in &fragment.mappings()[sample.mappings.clone()] {
            ...
            match &mapping.target {
                ...
                NalTarget::Media(range) => {
                    ...
                    assert_eq!(
                        &fragment.bytes()[at..at + 4],
                        &(nal.bytes().len() as u32).to_be_bytes()
                    );
...
fn baseline_matches_independent_layout_oracle_and_every_source_byte() -> TestResult {
    fixture(
        BASELINE,
        &[0, 3600, 7200, 10800],
        include_bytes!("fixtures/baseline_init.bin"),
...
fn high_b_pictures_keep_exact_signed_composition_offsets_and_cropping() -> TestResult {
    fixture(
        HIGH,
        &[0, 10800, 3600, 7200, 18000, 14400],
        include_bytes!("fixtures/high_cropped_init.bin"),
...
fn same_inputs_produce_identical_fragments_and_receipts() -> TestResult {
    ...
    let t = timed(&p, &[0, 3600, 7200, 10800]);
    let mut a = mux(&p[0], Mp4Limits::default())?;
    let mut b = mux(&p[0], Mp4Limits::default())?;
...
fn refusal_never_consumes_fragment_number_timeline_or_source_cursor() -> TestResult {
    let p = groups(BASELINE, KEY, true, false)?;
    let good = timed(&p, &[0, 3600, 7200, 10800]);
    ...
    for (index, decode_time, duration, offset) in [
        ...
        (0, 0, 3600, -1),
        (0, u64::MAX, 1, 0),
        (0, u64::MAX - 3600, 3600, i32::MAX),
    ] {
        let mut bad = good.clone();
        bad[index].decode_time = decode_time;
        bad[index].duration = duration;
        bad[index].composition_offset = offset;
...
fn all_count_and_byte_limits_fail_before_state_change() -> TestResult {
    ...
    for limits in [
        ...
        assert_eq!(m.fragment(&t), Err(Mp4Error::Limit));
        assert_eq!(m.next_sequence(), Some(1));
    }
    assert!(matches!(
        mux(
...
fn non_idr_start_unverified_tail_and_discontinuity_are_not_playability_claims() -> TestResult {
    ...
    let mut m = mux(&p[0], Mp4Limits::default())?;
    assert_eq!(
        m.fragment(&timed(&p[1..], &[0, 3600, 7200])),
        Err(Mp4Error::RandomAccessRequired)
    );
    let unmarked = groups(BASELINE, KEY, false, false)?;
    assert_eq!(
        m.fragment(&timed(&unmarked, &[0, 3600, 7200, 10800])),
        Err(Mp4Error::UnverifiedPicture)
    );
    let interrupted = groups(BASELINE, KEY, true, true)?;
    assert_eq!(
        m.fragment(&timed(&interrupted, &[0, 3600, 7200, 10800])),
        Err(Mp4Error::Discontinuity)
    );
    assert_eq!(m.next_sequence(), Some(1));
    Ok(())
}
...
fn fragments_start_at_idr_and_timeline_gaps_are_explicit_not_filled() -> TestResult {
    ...
    let mut second = timed(&p[3..], &[18000]);
    second[0].decode_time = 18_000;
    second[0].composition_offset = 0;
    let last = m.fragment(&second)?;
    assert_eq!(last.sequence(), 2);
    assert_eq!(last.timeline_gap(), Some(10_800..18_000));
...
fn advancing_timestamps_cannot_replay_the_same_source() -> TestResult {
    let p = groups(BASELINE, KEY, true, false)?;
    let mut m = mux(&p[0], Mp4Limits::default())?;
    let mut t = timed(&p, &[0, 3600, 7200, 10800]);
    m.fragment(&t)?;
    for sample in &mut t {
        sample.decode_time += 14_400;
    }
...
fn wrong_epoch_or_exact_parameters_cannot_join_a_movie_track() -> TestResult {
    ...
    assert_eq!(
        m.fragment(&timed(&other, &[0, 3600, 7200, 10800])),
        Err(Mp4Error::StreamMismatch)
    );
    let high = groups(HIGH, KEY, true, false)?;
    assert_eq!(
        m.fragment(&timed(&high, &[0, 10800, 3600, 7200, 18000, 14400])),
        Err(Mp4Error::ParameterSet)
    );
    Ok(())
}
#[test]
