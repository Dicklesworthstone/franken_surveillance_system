...
use fss_packet::avc::{AvcAssembler, AvcAssemblyLimits, AvcAssemblyStep, AvcPictureGroup,
    AvcSyntaxLimits, parse_pps, parse_sps};
...
pub fn groups(data: &[u8], key: StreamKey, marked: bool, discontinuity: bool) -> Result<Vec<AvcPictureGroup>, Error> {
    let nals = split(data);
    ...
    if discontinuity { a.discontinuity(key, 0)?; }
        ...
        for nal in d.push(key, sequence, packet, sequence)?.nals {
            match a.push(nal, sequence) {
                AvcAssemblyStep::Accepted(step) => {
                    if step.retired.is_some() { return Err("unexpected clean fixture retirement".into()); }
                    if let Some(picture) = step.picture { output.push(picture); }
                }
    ...
    if let Some(tail) = a.finish(100)?.picture { output.push(tail); }
    Ok(output)
}

pub fn timed<'a>(groups: &'a [AvcPictureGroup], pts: &[u64]) -> Vec<TimedAvcPicture<'a>> {
    groups.iter().zip(pts).enumerate().map(|(i, (picture, pts))| TimedAvcPicture {
        picture, decode_time: i as u64 * 3600, duration: 3600,
