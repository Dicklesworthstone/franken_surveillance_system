...
pub(super) fn build(
    ...
    let vmhd = w.full(b"vmhd", 1)?;
    w.zeros(8)?;
    w.end(vmhd)?;
    let dinf = w.start(b"dinf")?;
    let dref = w.full(b"dref", 0)?;
    w.u32(1)?;
    let url = w.full(b"url ", 1)?;
    w.end(url)?;
    w.end(dref)?;
    w.end(dinf)?;
    let stbl = w.start(b"stbl")?;
    let stsd = w.full(b"stsd", 0)?;
    w.u32(1)?;
    ...
    w.u16(0)?;
    w.put(&[
        (c.temporal_layers() << 3) | (u8::from(c.temporal_nested()) << 2) | 3,
        3,
    ])?;
    let mut ranges = [0..0, 0..0, 0..0];
    for (index, (kind, bytes)) in [(32, c.vps()), (33, c.sps()), (34, c.pps())]
        .into_iter()
        .enumerate()
    {
