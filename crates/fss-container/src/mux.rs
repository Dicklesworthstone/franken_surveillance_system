...
impl InitializationSegment {
    ...
    pub fn sps_range(&self) -> Range<usize> { self.sps_range.clone() }
    /// Exact original PPS NAL location, excluding its avcC length prefix.
    pub fn pps_range(&self) -> Range<usize> { self.pps_range.clone() }
}
impl std::fmt::Debug for InitializationSegment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("InitializationSegment").field("bytes", &self.bytes.len()).finish_non_exhaustive()
    }
}
...
impl AvcFragment {
    ...
    pub fn samples(&self) -> &[SampleMapping] { &self.samples }
    /// Exact per-NAL locations and source spans, including parameter-set relocation.
    pub fn mappings(&self) -> &[NalMapping] { &self.mappings }
...
impl std::fmt::Debug for AvcFragment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AvcFragment").field("key", &self.key).field("sequence", &self.sequence)
...
impl std::fmt::Debug for AvcMuxer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AvcMuxer").field("key", &self.key).field("next_sequence", &self.next_sequence)
            .field("last_end", &self.last_end).finish_non_exhaustive()
    }
}
...
impl AvcMuxer {
    ...
    pub fn new(key: StreamKey, sps: AvcSps, pps: AvcPps, time_scale: u32, limits: Mp4Limits) -> Result<Self, Mp4Error> {
        limits.validate()?;
        if key.ingress == 0 || key.generation == 0 || time_scale == 0 { return Err(Mp4Error::Configuration); }
        if !sps.frame_mbs_only() { return Err(Mp4Error::UnsupportedFormat); }
        ...
        let bound = parse_pps(pps.nal_bytes(), &sps, AvcSyntaxLimits {
            max_width: 16_384, max_height: 16_384, max_luma_samples: 268_435_456,
            ..AvcSyntaxLimits::default()
        }).map_err(|_| Mp4Error::ParameterSet)?;
        if bound != pps { return Err(Mp4Error::ParameterSet); }
        let initialization = init::build(&sps, &pps, time_scale, limits.max_initialization_bytes)?;
        Ok(Self { key, sps, pps, limits, initialization, next_sequence: Some(1), last_end: None, last_source: None })
    }

    /// Reuse these initialization bytes for every fragment of this exact track.
    pub fn initialization(&self) -> &InitializationSegment { &self.initialization }
    ...
    pub fn fragment(&mut self, samples: &[TimedAvcPicture<'_>]) -> Result<AvcFragment, Mp4Error> {
        ...
        for sample in samples {
            ...
            w.u32(if sample.picture.identity().idr_pic_id().is_some() { 0x02000000 } else { 0x01010000 })?;
        ...
        for (index, sample) in samples.iter().enumerate() {
            let begin = w.data.len(); let map_begin = mappings.len();
            for (ordinal, nal) in sample.picture.nals().iter().enumerate() {
                let target = match nal.nal_type() {
                    ...
                    _ => {
                        ...
                        let start = w.data.len(); w.put(nal.bytes())?;
                ...
                mappings.push(NalMapping { sample: index, nal: ordinal, target, sources });
            }
            receipts.push(SampleMapping {
                range: begin..w.data.len(), decode_time: sample.decode_time,
                presentation_time: presentation_time(sample)?, duration: sample.duration,
                rtp_timestamp: sample.picture.timestamp(), idr: sample.picture.identity().idr_pic_id().is_some(),
                boundary: sample.picture.boundary(), mappings: map_begin..mappings.len(),
            });
        }
        w.end(mdat)?;
        if w.data.len() != plan.bytes { return Err(Mp4Error::Layout); }
        let gap = self.last_end.filter(|end| *end < samples[0].decode_time).map(|end| end..samples[0].decode_time);
        let output = AvcFragment { key: self.key, sequence, bytes: w.data, samples: receipts, mappings, timeline_gap: gap };
    ...
    fn preflight(&self, samples: &[TimedAvcPicture<'_>]) -> Result<Plan, Mp4Error> {
        ...
        if samples[0].picture.identity().idr_pic_id().is_none() { return Err(Mp4Error::RandomAccessRequired); }
        ...
        let mut plan = Plan { bytes: media_offset, media_offset, nals: 0, end: samples[0].decode_time, last_source: self.last_source };
        let mut spans = 0_usize;
        for sample in samples {
            let picture = sample.picture;
            if picture.key() != self.key { return Err(Mp4Error::StreamMismatch); }
            if picture.sps().nal_bytes() != self.sps.nal_bytes() || picture.pps().nal_bytes() != self.pps.nal_bytes() {
                return Err(Mp4Error::ParameterSet);
            }
            if !picture.saw_first_macroblock() || picture.boundary() == AvcBoundary::EndOfInputUnverified {
                return Err(Mp4Error::UnverifiedPicture);
            }
            if picture.discontinuity_before() { return Err(Mp4Error::Discontinuity); }
            ...
            plan.nals = plan.nals.checked_add(picture.nals().len()).ok_or(Mp4Error::Limit)?;
            if plan.nals > self.limits.max_nals { return Err(Mp4Error::Limit); }
            plan.bytes = plan.bytes.checked_add(media_size(picture)?).ok_or(Mp4Error::Limit)?;
            if plan.bytes > self.limits.max_fragment_bytes { return Err(Mp4Error::Limit); }
            for nal in picture.nals() {
                spans = spans.checked_add(nal.sources().len()).ok_or(Mp4Error::Limit)?;
                if spans > self.limits.max_source_spans { return Err(Mp4Error::Limit); }
                let first = nal.sources().first().ok_or(Mp4Error::SourceOrder)?;
                let last = nal.sources().last().ok_or(Mp4Error::SourceOrder)?;
                if plan.last_source.is_some_and(|(seq, end)| first.sequence < seq || (first.sequence == seq && first.wire_range.start < end)) {
...
fn media_size(picture: &AvcPictureGroup) -> Result<usize, Mp4Error> {
    ...
    for nal in picture.nals() {
        if !matches!(nal.nal_type(), 7 | 8) {
            size = size.checked_add(4).and_then(|s| s.checked_add(nal.bytes().len())).ok_or(Mp4Error::Limit)?;
        }
    }
    if size == 0 { return Err(Mp4Error::Layout); }
    Ok(size)
}
