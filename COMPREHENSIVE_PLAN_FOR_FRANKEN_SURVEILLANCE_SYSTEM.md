...
- Appendix F. First 250 implementation issues
---
...
The canonical seed catalog is `architecture/seed_requirements.json`; this list and the count
mirrors are generated by `python3 scripts/seed_requirement_checker.py --generate`. Version 1
retains every published identity and full intent in numeric order. The earlier first-240 claim
is superseded history, not permission to remove the final ten requirements. See
`docs/PLAN_ERRATA.md` for compatibility and release-claim impact.
