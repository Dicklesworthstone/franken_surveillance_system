...
def parse_agent_operation_modes(root: Path) -> set[str]:
    ...
    if not md_path.is_file():
        md_path = ROOT / "registries/AGENT_OPERATIONS.md"
    if not md_path.is_file():
        return {"session_control", "query", "investigation", "action_planning", "effect_execution", "learning", "diagnostic"}
...
def parse_registered_statuses(root: Path) -> set[str]:
    ...
    statuses: set[str] = {"specified", "active", "preview", "deprecated", "tombstone", "tombstoned", "superseded"}
    for md_file in ["registries/AGENT_OPERATIONS.md", "registries/AGENT_VIEWS.md", "registries/OPERATION_CROSSWALK.md"]:
        p = root / md_file
        if not p.is_file():
            p = ROOT / md_file
        if p.is_file():
...
VALID_VIEW_SECTIONS = frozenset({
    "adversarialResiduals", "atRisk", "authority", "basisAfter", "basisBefore",
    "budgets", "cases", "certified", "certifiedAbsences", "certifiedCore",
    "changed", "changedEvidence", "continuity", "contradictions", "controlEnvelope",
    "coverage", "coverageChanges", "criticalChanges", "decisionImpact", "derivations",
    "discriminators", "effectUncertainty", "evidence", "evidenceGraph", "gaps",
    "hypotheses", "invalidatedAssumptions", "materialAlternatives", "mission", "next",
    "nextDiscriminators", "now", "obligations", "plans", "possible", "progress",
    "proofExpected", "question", "receipts", "reconciliation", "redactions", "replay",
    "situation", "sourceHandles", "state", "states", "stopRule", "unknown", "unknowns",
    "urgentObligations", "whatWouldReverse", "why",
})

try:
    VALID_RECOVERY_CLASSES: frozenset[str] = load_valid_recovery_classes(ROOT)
except Exception:
...
def collect_cli_discovery_endpoints(root: Path) -> dict[str, dict[str, str]]:
    ...
    # Extract usage lines from help_text()
    help_match = re.search(r"fn help_text\(\)[^{]*\{([^}]+)\}", content)
    usage_cmds: dict[str, str] = {}
    if help_match:
        for line in help_match.group(1).splitlines():
            m_usage = re.search(r'(fss\s+([a-z0-9_-]+)[^\n"\\]*)', line)
            if m_usage:
                cmd_word = m_usage.group(2)
                full_cmd = m_usage.group(1).strip()
    ...
    # Parse variants, doc comments, and optional #[command(name = "...")] attributes
    variant_pattern = re.compile(
        r"((?:///[^\n]*\n)+)\s*(?:#\[command\(name\s*=\s*\"([^\"]+)\"\)\]\s*)?([A-Za-z0-9_]+)",
        re.MULTILINE,
    )

    var_docs: dict[str, str] = {}
    var_names: dict[str, str] = {}
    for doc_block, cmd_override, var_ident in variant_pattern.findall(enum_match.group(1)):
        doc = " ".join(line.strip().lstrip("/").strip() for line in doc_block.strip().splitlines())
        cmd_name = cmd_override if cmd_override else re.sub(r"(?<!^)(?=[A-Z])", "-", var_ident).lower()
        var_docs[var_ident] = doc
        var_names[var_ident] = cmd_name

    scan_for_secrets(var_docs, "crates/fss-cli/src/fss_cmd.rs:doc_comments")

    endpoints: dict[str, dict[str, str]] = {}
    if "Capabilities" in var_docs:
        c_name = var_names["Capabilities"]
        cli_str = usage_cmds.get(c_name, f"fss {c_name} --json")
        if "--json" not in cli_str:
            cli_str += " --json"
        ep_key = c_name if c_name != "capabilities" else "capabilities"
        endpoints[ep_key] = {
            "cli": cli_str,
            "description": var_docs["Capabilities"],
        }
    if "Doctor" in var_docs:
        d_name = var_names["Doctor"]
        cli_str = usage_cmds.get(d_name, f"fss {d_name} --json")
        if "--json" not in cli_str:
            cli_str += " --json"
        ep_key = d_name if d_name != "doctor" else "doctor"
        endpoints[ep_key] = {
            "cli": cli_str,
            "description": var_docs["Doctor"],
        }
    if "Status" in var_docs:
        s_name = var_names["Status"]
        cli_str = usage_cmds.get(s_name, f"fss {s_name} --json")
        if "--json" not in cli_str:
            cli_str += " --json"
        ep_key = s_name if s_name != "status" else "status"
        endpoints[ep_key] = {
            "cli": cli_str,
            "description": var_docs["Status"],
        }
    if "NegativeEvidence" in var_docs:
        ne_name = var_names["NegativeEvidence"]
        ne_cmd_path = root / "crates/fss-cli/src/negative_evidence_cmd.rs"
        ne_cli = f"fss {ne_name} list --json"
        if ne_cmd_path.is_file():
            ne_text = ne_cmd_path.read_text(encoding="utf-8")
            ne_help = re.search(r"fss negative-evidence list[^\n\\]*", ne_text)
            if ne_help:
                ne_cli = f"fss {ne_name} list --json"
        ep_key = ne_name if ne_name != "negative-evidence" else "negative_evidence"
        endpoints[ep_key] = {
            "cli": ne_cli,
...
def collect_robot_docs_model(root: Path) -> dict[str, Any]:
    ...
    for op_id in sorted(live_op_ids):
        ...
        for str_field in ["name", "owner", "status", "responseEnvelope", "requestEnvelope", "requestPayloadSchema", "defaultView", "cliCommand", "mcpToolName"]:
