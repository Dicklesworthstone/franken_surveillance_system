...
def collect_cli_discovery_endpoints(root: Path) -> dict[str, dict[str, str]]:
    ...
    if "Capabilities" in var_docs:
        ...
        if not usage_line and "capabilities" in usage_cmds:
            usage_line = re.sub(r"^fss\s+[a-z0-9_-]+", f"fss {cmd_word}", usage_cmds["capabilities"])
        if not usage_line:
            raise RobotDocsError(
                ERR_ROBOT_DOCS_CORRUPT,
    ...
    if "Doctor" in var_docs:
        ...
        usage_line = usage_cmds.get(cmd_word)
        if not usage_line and "doctor" in usage_cmds:
            usage_line = re.sub(r"^fss\s+[a-z0-9_-]+", f"fss {cmd_word}", usage_cmds["doctor"])
        if not usage_line:
            raise RobotDocsError(
                ERR_ROBOT_DOCS_CORRUPT,
                f"Missing help usage line for {cmd_word} in help_text()",
    ...
    if "Status" in var_docs:
        ...
        usage_line = usage_cmds.get(cmd_word)
        if not usage_line and "status" in usage_cmds:
            usage_line = re.sub(r"^fss\s+[a-z0-9_-]+", f"fss {cmd_word}", usage_cmds["status"])
        if not usage_line:
            raise RobotDocsError(
                ERR_ROBOT_DOCS_CORRUPT,
                f"Missing help usage line for {cmd_word} in help_text()",
    ...
    if "NegativeEvidence" in var_docs:
        ...
        usage_line = usage_cmds.get(cmd_word)
        if not usage_line and "negative-evidence" in usage_cmds:
            usage_line = re.sub(r"^fss\s+[a-z0-9_-]+", f"fss {cmd_word}", usage_cmds["negative-evidence"])
        if not usage_line:
            raise RobotDocsError(
                ERR_ROBOT_DOCS_CORRUPT,
                f"Missing help usage line for {cmd_word} in help_text()",
