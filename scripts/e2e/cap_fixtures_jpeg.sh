#!/usr/bin/env bash
# scripts/e2e/cap_fixtures_jpeg.sh
# End-to-end integration test runner for CAP- fixtures JPEG/MJPEG companion (fss-2h5zq.6).
set -euo pipefail
...
if [[ -f "${SCRIPT_DIR}/lib.sh" ]]; then
    ...
    e2e_init "fixtures_jpeg" "fss-2h5zq.6" "${PASS_THROUGH_ARGS[@]}"
    e2e_cargo_test "fss-reference" "media_fixture_jpeg_companion_contract"
    e2e_summary
else
    # Standalone harness conforming to CAP- structured JSON-lines logging specification.
    SUITE_NAME="fixtures_jpeg"
    BEAD_ID="fss-2h5zq.6"
    ...
    # 1. Write environment header record
    GIT_SHA=$(git -C "$REPO_ROOT" rev-parse HEAD 2>/dev/null || echo "0000000000000000000000000000000000000000")
    DIRTY=false
    if [[ -n "$(git -C "$REPO_ROOT" status --porcelain 2>/dev/null || true)" ]]; then
        DIRTY=true
    fi
    HOST_TRIPLE=$(rustc -vV 2>/dev/null | awk '/^host:/ {print $2}' || echo "x86_64-unknown-linux-gnu")
...
bins = []
search_bin = bin_dir if bin_dir else str(pathlib.Path(log_file).parent.parent.parent / "target" / "debug")
if os.path.isdir(search_bin):
    for entry in sorted(os.listdir(search_bin)):
        ep = os.path.join(search_bin, entry)
...
' "$LOG_FILE" "cap_fixtures_jpeg.sh" "$BEAD_ID" "$GIT_SHA" "$DIRTY" "$HOST_TRIPLE" "${FSS_BIN_DIR:-}" "${FSS_E2E_LOG_DIR:-}"

    # 2. Execute test companion via RCH or cargo (retrying on 103 up to 3 times)
    ...
    while true; do
        TEST_EXIT=0
        if [[ -n "${RCH_STUB_LOG:-}" ]]; then
            cargo test -p fss-reference --test media_fixture_jpeg_companion_contract --locked --offline -- "${CARGO_TEST_ARGS[@]}" > "$STDOUT_TMP" 2>&1 || TEST_EXIT=$?
        elif command -v rch >/dev/null 2>&1; then
            RCH_REQUIRE_REMOTE=1 rch exec -- cargo test -p fss-reference --test media_fixture_jpeg_companion_contract --locked --offline -j 1 -- "${CARGO_TEST_ARGS[@]}" > "$STDOUT_TMP" 2>&1 || TEST_EXIT=$?
        else
            cargo test -p fss-reference --test media_fixture_jpeg_companion_contract --locked --offline -- "${CARGO_TEST_ARGS[@]}" > "$STDOUT_TMP" 2>&1 || TEST_EXIT=$?
        fi
...
stdout_file, log_file, script, bead, test_exit_str, only_step = sys.argv[1:7]
        ...
        if "CAPLOG " in line:
            ...
            try:
                ...
                rec = {
                    ...
                    "bead": bead,
                    "step": step_name,
                    "cmd": "cargo test -p fss-reference --test media_fixture_jpeg_companion_contract",
                    "exit": 1,
                    "duration_ms": 1,
                    ...
                    "stderr_excerpt": "",
                    "verdict": "fail",
                    "repro": "scripts/e2e/cap_fixtures_jpeg.sh"
                }
            ...
            if step in seen_steps:
                ...
                rec = {
                    ...
                    "bead": bead,
                    "step": step,
                    "cmd": f"cargo test -p fss-reference --test media_fixture_jpeg_companion_contract -- {step}",
                    "exit": 1,
                    "duration_ms": 1,
                    ...
                    "stderr_excerpt": "",
                    "verdict": "fail",
                    "repro": f"scripts/e2e/cap_fixtures_jpeg.sh --only {step}"
                }
                lf.write(json.dumps(rec) + "\n")
                continue
...
' "$STDOUT_TMP" "$LOG_FILE" "cap_fixtures_jpeg.sh" "$BEAD_ID" "$TEST_EXIT" "${ONLY_STEP:-}")
    ...
    REPRO="scripts/e2e/cap_fixtures_jpeg.sh"
    if [[ -n "$ONLY_STEP" ]]; then
        REPRO="scripts/e2e/cap_fixtures_jpeg.sh --only ${ONLY_STEP}"
    fi
...
verdict = "pass"
if test_exit != 0 or fail_count > 0 or step_count == 0 or (len(skipped) == step_count and step_count > 0):
    verdict = "fail"
...
' "$LOG_FILE" "$SUMMARY_JSON" "$DURATION_MS" "$REPRO" "$TEST_EXIT")
    ...
    if [[ "$VERDICT" == "pass" ]]; then
        ...
        echo "fail summary: ${PASS_COUNT} steps passed, ${FAIL_COUNT} failures: $(python3 -c 'import json, sys; print(json.dumps(json.loads(sys.argv[1])["failures"]))' "$SUMMARY_JSON")"
        exit 1
    fi
fi
