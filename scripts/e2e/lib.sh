...
# other tests' CAPLOG println output can land behind one or more such prefixes ("test a ... test b
# ... CAPLOG {...}", "ok" on later lines; seen in real rch runs). Exactly those prefixes (a Rust test
# path, then " ... ") are removed. Any OTHER line that still contains "CAPLOG {" is malformed, so
# leading text can never hide a record (a hidden fail record would otherwise leave a pass summary).
libtest_prefix_re = re.compile(r"^(?:test [A-Za-z_][A-Za-z0-9_:]* \.\.\. )+(?=CAPLOG )")
CAPLOG_MARK = "CAPLOG {"
...
def collect_caplog_payloads(stdout_path, stream="stdout"):
    ...
    with open(stdout_path, "r", encoding="utf-8", errors="replace", newline="\n") as f:
        while True:
            ...
            if overlong:
                # The whole over-long line is scanned, chunk by chunk with an overlap, for a record.
                hit = "CAPLOG " in sline[:64] or CAPLOG_MARK in sline
                tail = ansi_re.sub("", raw)[-len(CAPLOG_MARK):]
                while True:
                    rest = f.readline(CAPLOG_LINE_LIMIT)
                    if not rest:
                        break
                    chunk = ansi_re.sub("", rest)
                    if CAPLOG_MARK in tail + chunk:
                        hit = True
                    tail = chunk[-len(CAPLOG_MARK):]
                    if rest.endswith("\n"):
                        break
                if hit:
            ...
            if not sline.startswith("CAPLOG "):
                if CAPLOG_MARK in sline:
                    return payloads, "CAPLOG record not at the start of its line"
                continue
