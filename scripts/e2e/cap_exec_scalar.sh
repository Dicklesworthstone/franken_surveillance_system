...
# End-to-end integration test runner for CAP- EXEC scalar reference executor (fss-2h5zq.46).
set -euo pipefail
...
if [[ -f "${SCRIPT_DIR}/lib.sh" ]]; then
    ...
    e2e_init "exec_scalar" "fss-2h5zq.46" "${PASS_THROUGH_ARGS[@]}"
    ...
    SUITE_NAME="exec_scalar"
    BEAD_ID="fss-2h5zq.46"
...
' "$LOG_FILE" "cap_exec_scalar.sh" "$BEAD_ID" "$GIT_SHA" "$DIRTY" "$HOST_TRIPLE" "${FSS_BIN_DIR:-}" "${FSS_E2E_LOG_DIR:-}"
    ...
    while true; do
        TEST_EXIT=0
        RCH_REQUIRE_REMOTE=1 rch exec -- cargo test -p fss-reference --test scalar_executor_contract --locked --offline -- "${CARGO_TEST_ARGS[@]}" > "$STDOUT_TMP" 2>&1 || TEST_EXIT=$?
        ...
        if "CAPLOG " in line:
            ...
            try:
                ...
                rec = {
                    ...
                    "bead": bead,
                    "step": step_name,
                    "cmd": "cargo test -p fss-reference --test scalar_executor_contract",
                    "exit": 1,
                    "duration_ms": 1,
                    ...
                    "stderr_excerpt": "",
                    "verdict": "fail",
                    "repro": "scripts/e2e/cap_exec_scalar.sh"
                }
            ...
            if step in seen_steps:
                ...
                rec = {
                    ...
                    "bead": bead,
                    "step": step,
                    "cmd": f"cargo test -p fss-reference --test scalar_executor_contract -- {step}",
                    "exit": 1,
                    "duration_ms": 1,
                    ...
                    "stderr_excerpt": "",
                    "verdict": "fail",
                    "repro": f"scripts/e2e/cap_exec_scalar.sh --only {step}"
                }
                lf.write(json.dumps(rec) + "\n")
                continue
...
' "$STDOUT_TMP" "$LOG_FILE" "cap_exec_scalar.sh" "$BEAD_ID" "$TEST_EXIT")
    ...
    REPRO="scripts/e2e/cap_exec_scalar.sh"
    if [[ -n "$ONLY_STEP" ]]; then
        REPRO="scripts/e2e/cap_exec_scalar.sh --only ${ONLY_STEP}"
    fi
