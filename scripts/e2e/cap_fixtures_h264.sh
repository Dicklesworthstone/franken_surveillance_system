...
run_ffprobe() {
    ...
    ffprobe -v error -show_format -show_streams "$full_path" 2>&1 || true
    return 0
}
...
if [[ -f "${SCRIPT_DIR}/lib.sh" ]]; then
    ...
    for step in "${FIXTURE_STEPS[@]}"; do
        ...
        if command -v ffprobe >/dev/null 2>&1; then
            ...
            e2e_expect_exit "ffprobe_${step}" 0
        else
    ...
    sum_rc=0
    sum_out="$(e2e_summary 2>&1)" || sum_rc=$?
    echo "$sum_out"
    if [[ $sum_rc -ne 0 ]] || echo "$sum_out" | grep -q 'verdict=fail' || echo "$sum_out" | grep -q 'failed=1'; then
        exit 1
    fi
    exit 0
fi
...
rec = {
    ...
    "bead": bead,
    "step": step,
    "cmd": ["ffprobe", "-v", "error", "-show_format", "-show_streams", fix_path],
    "exit": int(rc),
    "duration_ms": int(dur),
    "expected": 0,
    "observed": int(rc),
...
if [[ ${#failures[@]} -gt 0 || $step_count -eq 0 ]]; then
...
if [[ "$overall_verdict" == "pass" && $step_count -gt 0 ]]; then
    exit 0
else
    exit 1
fi
