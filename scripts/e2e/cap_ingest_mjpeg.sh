...
' "$LOG_FILE" "cap_ingest_mjpeg.sh" "$BEAD_ID" "$GIT_SHA" "$DIRTY" "$HOST_TRIPLE" "${FSS_BIN_DIR:-}" "${FSS_E2E_LOG_DIR:-}"
    ...
    # Capture file lives under the run's log dir (FSS_E2E_LOG_DIR or target/e2e-logs), never
    # under the ambient TMPDIR or /tmp; removed on every exit path.
    STDOUT_TMP=$(mktemp -p "$LOG_DIR" cargo_stdout.XXXXXX)
...
# Step-level failures only. The synthetic entries below (cargo exit, no CAPLOG, all skipped)
# are counted in fail_count and listed in failures, but never reported as failed steps.
...
# The verdict reads step-level failures; each synthetic condition (cargo exit, no CAPLOG, all
# steps skipped) is its own guard, so no single counter can mask another.
fail_count = info["step_fail_count"]
total_fail_count = info["fail_count"]
...
rec = {
    ...
    "verdict": verdict,
    "steps": step_count,
    "passed": step_count - fail_count - len(skipped),
...
' "$LOG_FILE" "$SUMMARY_JSON" "$DURATION_MS" "$REPRO" "$TEST_EXIT")
    ...
    # Step-level failures only: synthetic entries (cargo_test_failed, no_caplog_emitted,
    # all_steps_skipped) are never printed as failed steps.
    FAIL_COUNT=$(python3 -c 'import json, sys; print(json.loads(sys.argv[1])["step_fail_count"])' "$SUMMARY_JSON")
    ...
    if [[ "$VERDICT" == "pass" ]]; then
        ...
        if [[ "$FAIL_COUNT" -eq 0 && "$TEST_EXIT" -ne 0 ]]; then
            echo "fail summary: cargo exit ${TEST_EXIT} with 0 failed steps (${PASS_COUNT} passed, ${SKIP_COUNT} skipped)"
            exit 1
        fi
        if [[ "$FAIL_COUNT" -eq 0 && "$STEP_COUNT" -eq 0 ]]; then
            echo "fail summary: no CAPLOG records emitted (cargo exit ${TEST_EXIT})"
            exit 1
        fi
        if [[ "$FAIL_COUNT" -eq 0 && "$SKIP_COUNT" -eq "$STEP_COUNT" ]]; then
            echo "fail summary: all ${STEP_COUNT} steps skipped"
            exit 1
        fi
