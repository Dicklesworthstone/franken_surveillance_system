...
PY
    ...
    pw_caplog)
        ...
        ;;
    libtest_interleave)
        # The real rch run's line: libtest's "test <name> ... " with another test's CAPLOG behind it.
        {
            printf 'test test_02_synthetic_standard_stream ... CAPLOG {"step": "li1", "verdict": "pass"}\n'
            printf 'ok\n'
            cap '{"step": "li2", "verdict": "pass"}'
            printf 'noise CAPLOG {"step": "li3", "verdict": "pass"}\n'
            printf 'test a b ... CAPLOG {"step": "li4", "verdict": "pass"}\n'
        } >&2
        exit 0
        ;;
    *)
...
echo "=== Test 46: libtest's 'test <name> ... ' prefix before a CAPLOG line is removed, nothing else ==="
S="${TEST_SANDBOX}/suite46/run.sh"
new_script "$S" <<'EOF'
e2e_init "suite46" "fss-2h5zq.1"
e2e_cargo_test "fss-cli" "test_interleave"
e2e_summary
EOF
run t46 env STUB_RCH_MODE=libtest_interleave "$S"
check_eq "T46 rc" 0 "$RC"
LOG46="$(logdir suite46)/run_0001.log"
check_valid "$LOG46"
# li3 (arbitrary prefix) and li4 (not a libtest test-name shape) stay non-CAPLOG lines.
check_eq "T46 steps" '["li1", "li2"]' "$(lt steps "$LOG46")"
echo "PASS: Test 46"
