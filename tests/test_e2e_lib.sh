...
check_eq "T17 CAPLOG expected" '["", "https://u:<redacted>@h.invalid/x", "mysql -p<redacted>"]' "$(lt record "$LOG17" kw expected)"
check_eq "T17 CAPLOG cmd" '["mysql", "-p<redacted>", ""]' "$(lt record "$LOG17" kw cmd)"
check_eq "T17 CAPLOG repro" '""' "$(lt record "$LOG17" kw repro)"
echo "PASS: Test 17"
...
check_contains "T22 cap message" "E2E log cap reached" "$OUT"
