...
expected != observed (type-strict, observed required). The gate follows the target under --only,
fails closed when selected alone, and its repro reruns the target. The only rch on PATH is the stub below; a cargo tripwire fails any test that
...
case "${{STUB_MODE:-pass}}" in
  ...
  pass_stderr)
    ...
    for s in "${{ROSTER[@]}}"; do emit_pass "$s" >&2; done
    exit 0
    ;;
  libtest_interleave)
    # The real rch run 2 (round 6) shape: libtest's "test <name> ... " with a CAPLOG line behind it.
    for s in "${{ROSTER[@]:0:17}}"; do emit_pass "$s" >&2; done
    printf 'test test_02_synthetic_standard_stream ... ' >&2
    emit_pass "loop_and_mid_push_limits" >&2
    echo "ok" >&2
    emit_pass "validation_ceiling_bypass" >&2
    exit 0
    ;;
  int_float|int_bool|float_bool|obs_missing)
    for s in "${{ROSTER[@]:0:18}}"; do emit_pass "$s"; done
    case "$STUB_MODE" in
      int_float) pair='"expected":{{"n":1}},"observed":{{"n":1.0}}' ;;
      int_bool) pair='"expected":{{"n":1}},"observed":{{"n":true}}' ;;
      float_bool) pair='"expected":{{"n":1.0}},"observed":{{"n":true}}' ;;
      obs_missing) pair='"expected":{{"n":1}}' ;;
    esac
    echo "CAPLOG {{\\"step\\":\\"validation_ceiling_bypass\\",\\"verdict\\":\\"pass\\",$pair}}"
    exit 0
    ;;
  malformed)
...
"""
    ...
    def test_only_target_runs_gate(self):
        # --only annexb_split_contract selects the cargo target, and the gate follows the target.
        proc = self.run_harness(args=["--only", TARGET], mode="pass")
        self.assertEqual(proc.returncode, 0, proc.stderr)
        self.assert_valid_log()
        self.assertEqual(self.summary()["repro"], f"{REPRO} --only {TARGET}")
        self.assertIn(GATE, self.step_ids())
        self.assertEqual(self.summary()["steps"], len(ROSTER) + 1)

    def test_gate_follows_target_under_only(self):
        # A scoped rerun still enforces the roster: a missing step fails it.
        proc = self.run_harness(args=["--only", TARGET], mode="roster_missing")
        self.assertEqual(proc.returncode, 1, f"{proc.stdout}\n{proc.stderr}")
        self.assert_valid_log()
        self.assertEqual(self.summary()["failures"], [GATE])
        self.assertIn("missing roster steps: validation_ceiling_bypass", self.gate_stderr())
        self.assertEqual(self.gate_record()["repro"], f"{REPRO} --only {TARGET}")
        self.assertEqual(self.summary()["repro"], f"{REPRO} --only {TARGET}")

    def test_gate_alone_fails_closed(self):
        # Selected alone the gate has no target records to audit: that is a failure, never a pass.
        proc = self.run_harness(args=["--only", GATE], mode="pass")
        self.assertEqual(proc.returncode, 1, f"{proc.stdout}\n{proc.stderr}")
        self.assert_valid_log()
        self.assertEqual(self.calls(), [])
        self.assertEqual(self.summary()["failures"], [GATE])
        self.assertIn("no annexb_split_contract record in this run", self.gate_stderr())
        self.assertEqual(self.summary()["repro"], f"{REPRO} --only {TARGET}")
        self.assertNotIn("pass summary:", proc.stdout)

    def test_gate_repro_reproduces(self):
        # The summary repro of a gate failure reruns the target, and the rerun fails the same way.
        proc = self.run_harness(mode="mismatch")
        self.assertEqual(proc.returncode, 1)
        repro = self.summary()["repro"]
        self.assertEqual(repro, f"{REPRO} --only {TARGET}")
        rerun = self.run_harness(args=repro.split()[1:], mode="mismatch")
        self.assertEqual(rerun.returncode, 1)
        self.assertEqual(self.summary()["failures"], [GATE])
        self.assertIn("expected != observed", self.gate_stderr())

    def test_gate_type_strict(self):
        # 1, 1.0 and true are different observations; Python's 1 == 1.0 == True must not pass them.
        for mode in ("int_float", "int_bool", "float_bool"):
            with self.subTest(mode=mode):
                proc = self.run_harness(mode=mode)
                self.assertEqual(proc.returncode, 1, f"{mode}: {proc.stdout}\n{proc.stderr}")
                self.assert_valid_log()
                self.assertEqual(self.summary()["failures"], [GATE])
                self.assertIn("pass record of step 'validation_ceiling_bypass' has expected != observed",
                              self.gate_stderr())

    def test_gate_pass_without_observed_fails(self):
        proc = self.run_harness(mode="obs_missing")
        self.assertEqual(proc.returncode, 1, f"{proc.stdout}\n{proc.stderr}")
        self.assert_valid_log()
        self.assertEqual(self.summary()["failures"], [GATE])
        self.assertIn("pass record of step 'validation_ceiling_bypass' without observed", self.gate_stderr())

    def test_libtest_interleaved_caplog_kept(self):
        # A real run lost loop_and_mid_push_limits to libtest's "test <name> ... " prefix; the gate
        # caught it. The ingester now keeps that record, and the full roster passes.
        proc = self.run_harness(mode="libtest_interleave")
        self.assertEqual(proc.returncode, 0, f"{proc.stdout}\n{proc.stderr}")
        self.assert_valid_log()
        self.assertIn("loop_and_mid_push_limits", self.step_ids())
        self.assertEqual(self.summary()["failures"], [])

    def test_pass_on_stderr(self):
        # Real rch forwards the remote CAPLOG lines on stderr: a stderr-only run passes.
        proc = self.run_harness(mode="pass_stderr")
        self.assertEqual(proc.returncode, 0, proc.stderr)
        self.assert_valid_log()
        self.assertEqual(self.summary()["steps"], len(ROSTER) + 1)
        self.assertEqual(self.summary()["failures"], [])
    ...
    def _assert_target_fail(self, mode):
        # Target-failure modes run scoped to the cargo target; the gate follows the target and
        # blames the missing roster too, and the summary repro reruns the target.
        ...
        self.assertEqual(summary["failures"], [TARGET, GATE], f"{mode}: {summary['failures']}")
        self.assertEqual(summary["repro"], f"{REPRO} --only {TARGET}")
        return proc
    ...
    def test_e103_retry_bounded(self):
        ...
        self.assert_valid_log()
        self.assertEqual(self.summary()["failures"], [TARGET, GATE])
